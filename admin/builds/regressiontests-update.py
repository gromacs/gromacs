...
def do_build(context):
    ...
    # this from regressiontests, in which case we should update the
    # patch with any new results.
    project = 'regressiontests'
    if project != 'regressiontests':
        return

    context.chdir(context.workspace.get_project_dir(Project.REGRESSIONTESTS))
    cmd = ['git', 'add', '--', '*/*reference*', '*/*/*reference*']
    ...
    # message from the old HEAD commit, and then push it back to
    # gerrit for testing and review.
    ...
    cmd = ['git', 'push', 'ssh://jenkins@gerrit.gromacs.org', 'HEAD:refs/for/{0}'.format(branch)]
