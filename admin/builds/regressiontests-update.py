...
# TODO enable this when other things are working
...
#    run_build(context, cmake_opts)
    ...
    if files_were_added:
        # Reference files were added, so amend the commit and push it
        cmd = ['git', 'commit', '--amend', '--no-edit']
        context.run_cmd(cmd, failure_message='failed to amend the commit when adding new reference data']

        # TODO Work out how to find from which branch this came
        cmd = ['git', 'push', 'origin', 'HEAD:refs/for/master']
        context.run_cmd(cmd, failure_message='failed to upload commit with new reference data')
