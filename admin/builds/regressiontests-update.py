...
def do_build(context):
    ...
    # TODO Work out how to identify whether Gerrit Trigger started
    # this job from a patch on the regressiontests repository, in
    # which case we should update that patch with any new results.
    project = 'regressiontests'
    if project != 'regressiontests':
        return

    # Add any newly generated reference files to the index.
    context.chdir(context.workspace.get_project_dir(Project.REGRESSIONTESTS))
    cmd = ['git', 'add', '--', '*reference*']
    context.run_cmd(cmd, failure_message='failed to add reference files')
    ...
    no_files_were_added = (0 == context.run_cmd(cmd, failure_message='failed to count number of files in the git index', use_return_code=True))
    ...
    # Get the branch that triggered this job. TODO Can this be done better?
    branch = os.getenv('GERRIT_BRANCH')
    if not branch:
        branch = 'master'

    # Push the updated commit back to gerrit for testing and review.
    cmd = ['git', 'push', 'ssh://jenkins@gerrit.gromacs.org/regressiontests.git', 'HEAD:refs/for/{0}'.format(branch)]
