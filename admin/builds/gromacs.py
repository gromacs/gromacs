...
def do_build(context):
    ...
    if context.job_type == JobType.RELEASE:
        ...
        if context.opts.mdrun_only:
            ...
            context.build_target(target='check', keep_going=True)
            context.build_target(target='install')
        context.env.run_env_script(os.path.join(context.workspace.install_dir, 'bin', 'GMXRC'))
        # FIXME: This is the regressiontests command from Test_tarballs_5_1,
        # but it does not actually test the mdrun-only installation...
        cmd = [os.path.join(regressiontests_path, 'gmxtest.pl')]
        if context.opts.double:
            cmd += ['-double']
        if context.opts.mpi:
            cmd += ['-np', '1']
