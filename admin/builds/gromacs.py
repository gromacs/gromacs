    ...
    if context.job_type == JobType.RELEASE:
        ...
        if not context.opts.mdrun_only:
            ...
            if context.opts.gmxapi:
                gmxrc_cmd = '. ' + os.path.join(context.workspace.install_dir, 'bin', 'GMXRC') 
                gmxapi_cmd = 'bash  ' + os.path.join(context.workspace.get_project_dir(Project.GROMACS), 'admin', 'builds','gmxapitest.sh') + ' ' + context.env.gmxapi_version + ' ' + context.env.c_compiler + ' ' + context.env.cxx_compiler + ' ' + gmxrc_cmd)
