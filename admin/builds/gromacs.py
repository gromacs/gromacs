    ...
    if context.job_type == JobType.RELEASE:
        ...
        if not context.opts.mdrun_only:
            ...
            if context.opts.gmxapi:
                gmxapi_cmd = 'bash  ' + os.path.join(context.workspace.get_project_dir(Project.GROMACS), 'admin', 'builds','gmxapitest.sh' + ' ' + context.env.gmxapi_version + ' ' + context.env.c_compiler + ' ' + context.env.cxx_compiler + ' ' + os.path.join(context.workspace.build_dir, 'bin'))
