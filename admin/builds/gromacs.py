...
def do_build(context):
    ...
    if context.job_type == JobType.RELEASE:
        ...
        if not context.opts.mdrun_only:
            ...
            test_dir = os.path.join(context.workspace.install_dir, 'bin', 'tests')
            context.chdir(test_dir)
            if context.opts.gmxapi:
                gmxapi_cmd = 'bash  ' + 
                    os.path.join(context.workspace.get_project_dir(Project.GROMACS), 'admin', 'builds','gmxapitest.sh')
