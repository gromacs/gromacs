...
def do_build(context):
    ...
    if context.job_type == JobType.RELEASE:
        ...
        cmd = [os.path.join(regressiontests_path, 'gmxtest.pl'), 'all']
        # TODO: It would be nicer for gmxtest.pl to apply the suffixing logic
        # also to the -mdrun command.
        suffix = ''
        if context.opts.mpi:
            cmd += ['-np', '1']
            suffix += '_mpi'
