...
try:
    with open ("/tmp/tokenfile.txt", "r") as tokenfile:
...
r = requests.put('https://sandbox.zenodo.org/api/deposit/depositions/%s/files' % zenodo_id,
        params={'access_token': token},headers=zenodo_headers,
        files=zenodo_files,data=zenodo_data)
...
# r = requests.post('https://sandbox.zenodo.org/api/deposit/depositions/%s/actions/publish' % zenodo_id,
