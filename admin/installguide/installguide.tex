...
\verb+-DGMX_FFT_LIBRARY=mkl -DMKL_LIBRARIES="/full/path/to/libone.so;/full/path/to/libtwo.so" -DMKL_INCLUDE_DIR="/full/path/to/mkl/include"+,
...
\subsection{Backward compatible program names}
Up until verion 4.6 {\gromacs} compiled in to over 60 different
programs, which were difficult to keep track of. For ease of use all
of these have been replaced by a single binary called \verb+gmx+ which
you run similar to the old programs, for instance \verb+g_angle+ now
is \verb+gmx angle+. We think this will facilitate usage of a larger
fraction of the programs, in addition to having certain advantages
inside \gromacs. However, for those who have invested heavily
in scripts for running {\gromacs} we provide an option to generate
symbolic links to the old names.This option can be effectuated by
running cmake with the flag \verb+-DGMX_SYMLINK_OLD_BINARY_NAMES+.
