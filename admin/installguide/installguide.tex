...
\subsection{Altering compiler flags}\label{ssec:alteringflags}
...
please file a feature request on Redmine.)
...
\item Append flags to those generated by the \gromacs{} build
  ...
  does what you want:
  \verb+CC=icc cmake .. -DGMXC_CFLAGS_RELEASE="-no-ip"+. This works on
  the initial \cmake{} command line, or subsequently.
...
\item Let \gromacs{} and \cmake{} generate their default flags, and
  ...
  \verb+cmake .. -DGMX_SKIP_DEFAULT_CFLAGS=on+. Then, decide how you
  want the complete flag structure to look, and use (say)
  \verb+cmake .. -DCMAKE_C_FLAGS_RELEASE="<your flags here>"+. The
  setting of \verb+GMX_SKIP_DEFAULT_CFLAGS+ is cached, and so retained
  from last time. You can switch it back off to have the \gromacs{}
  build scripts add the recommended flags again, but that will not
  undo any changes you made to \verb+CMAKE_C_FLAGS*+ in the meantime.
\end{enumerate}
