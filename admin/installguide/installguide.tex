...
and its relatives\ref{ssec:alteringflags}.
...
\section{Advanced Techniques for building GROMACS}

\subsection{Altering compiler flags}
There may be need to alter the flags that are used with the
compiler. The default flags are contributed by the default behaviour
of both \cmake{} itself, and the build scripts written for
\gromacs{}. Altering the two types of compiler options requires
different approaches. The default flags from \cmake{} are stored in
the \cmake{} cache, which makes them straightforward to check and
modify. The flags generated by the \gromacs{} build scripts are
re-generated on the fly, so that they reflect modifications in the
build settings. This means they are not present in the
cache. Inspecting them requires looking at the output of \cmake{}
somehow. You can verify what flags will be used by inspecting the
value of \verb+BUILD_CFLAGS+ in \verb+src/buildinfo.h+. (If you'd like
a more convenient mechanism for checking these flags before building,
please file a feature request on Redmine.) Note that \cmake{} supports
configuring multiple ``build types'' that you can select, and some of
the variables provided by \cmake{} and \gromacs{} for setting compiler
flags come in flavours that either pertain to all build types, or just
to the ``release'' (i.e. maximum performance) or ``debug'' build
types, as indicated by such a suffix on the variable name.

Accordingly, there are several approaches you might use for altering
the compiler flags:
\begin{enumerate}
\item Append flags to those generated by the \gromacs{} build
  scripts. For example, with \icc{}, the default behaviour of
  \gromacs{} is to add the \verb+-ip+ flag. To remove that, you can
  read the compiler documentation to see that it will act on the last
  of a series of conflicting flags, and simply append the flag that
  does what you want:
  \verb+CC=icc cmake .. -DUSER_APPEND_C_FLAGS_RELEASE="-no-ip"+. This
  works on the initial \cmake{} command line, or subsequently. There
  is also \verb+USER_APPEND_C_FLAGS_DEBUG+ and
  \verb+USER_APPEND_C_FLAGS+ for appending flags for debug build
  types and all build types, respectively.
\item Replace the default flags supplied by \cmake{} with those of
  your choosing:
  \verb+CC=gcc cmake .. -DCMAKE_C_FLAGS_RELEASE="-fpermissive"+.
  Unfortunately, this approach will ``do what you say.'' \cmake{} will
  not add its default flags (\verb+-O3 -DNDEBUG+) at all, so with this
  approach you have to take care to address all of your own needs.
\item Let \gromacs{} and \cmake{} generate their default flags, and
  replace the set as a whole in a repeat invocation of
  \cmake{}. First, find out what the recommended flags were by doing
  \verb+cmake .. -DGMX_SKIP_DEFAULT_FLAGS=on+. Then, decide how you
  want the complete flag structure to look, and use (say)
  \verb+cmake .. -DCMAKE_C_FLAGS_RELEASE="<your flags here>"+. The
  setting of \verb+GMX_SKIP_DEFAULT_FLAGS+ is cached, and so will
  remain from however it was last set. You can switch it back off to
  have the \gromacs{} build scripts add the recommended flags again,
  but that will not undo any changes you made to \verb+CMAKE_C_FLAGS*+
  in the meantime. You cannot combine the first and third solutions -
  the \verb+USER_APPEND*+ flags will do nothing if the default flags
  are being skipped.
\end{enumerate}

\subsection{Altering linker flags}
Similarly, you can append to linking flags using
\verb+USER_APPEND_EXE_LINKER_FLAGS+ and
\verb+USER_APPEND_SHARED_LINKER_FLAGS+. \gromacs{} doesn't add linker
flags to suit internal needs, but those needed for linking to external
libraries do get added.
