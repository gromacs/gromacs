...
and its relatives. INSERT LINK TO ADVANCED FLAG-USAGE SECTION
...
compiler. The default flags are contributed by the default
behaviour of both \gromacs{} and \cmake{}. There are several
approaches you might use.
\begin{enumerate}
\item Append flags to those generated by \gromacs{}. For example, with
  \icc{}, the default behaviour of \gromacs{} is to add the \verb+-ip+
  flag. To remove that, you can read the compiler documentation to see
  that it will act on the last of a series of conflicting flags, and
  simply append the flag that does what you want:
  \verb+CC=icc cmake .. -DGMXC_CFLAGS_RELEASE="-no-ip"+. This works on
  the initial \cmake{} command line, or subsequently.
\item Replace the default flags suppled by \cmake{} with those of your
  choosing:
...
\item Let \gromacs{} and \cmake{} generate their default flags, and
  modify the set as a whole in a repeat invocation of \cmake{}. First,
  find out what the flags were with
  \verb+cmake .. -DGMX_SKIP_DEFAULT_CFLAGS=on+. Then, decide how you
  want the complete flag structure to look and use
  \verb+cmake .. -DCMAKE_C_FLAGS_RELEASE="<your flags here>"+. You
  can verify what flags will be used by inspecting the value of
  \verb+BUILD_CFLAGS+ in \verb+src/buildinfo.h+. (If you'd like a more
  convenient mechanism for checking these flags before building,
