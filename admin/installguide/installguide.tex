...
\subsubsection{Building only mdrun, and testing it}

Past versions of \gromacs{} had the ability to \verb+make mdrun+ and a
matching install instruction that would build only mdrun. Such a build
is useful when the configuration was only relevant for mdrun (such as
with \MPI and/or GPUs, or on BlueGene or Cray), or the length of time
for the compile-link-install cycle was relevant when developing. This
is now supported with the \cmake{} option
\verb+-DGMX_BUILD_MDRUN_ONLY+, which will build a cut-down version of
\verb+libgromacs+ and the \verb+mdrun+ binary. \verb+make install+
only those binaries (according to whether shared or static). For
testing such an mdrun, please see the testing section below.
