...
#  4. Specifying --update-copyright, which also updates old copyright headers.
#     Without this option, only copyright years in existing new-style are
#     updates (and completely missing headers added).
...
function usage() {
    echo "usage: uncrustify.sh [-f|--force] [--rev=REV] [--(no|update)-copyright] [action]"
...
for arg in "$@" ; do
    ...
    then
        ...
        copyright=
    elif [[ "$arg" == "--update-copyright" ]] ; then
        copyright=2
...
if [[ $copyright ]] ; then
    # TODO: Consider also adding --update-header once all relevant files
    # actually have the correct header.  Or consider running the script with
    # --update-header for new files, but not for existing ones.
    cpscript_args="--add-missing --update-year"
