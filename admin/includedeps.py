...
class ErrorReporter(object):
    def input_warning(self, file, msg):
...
class Link(object):
    ...
    def __init__(self, fromnode, tonode, type=None):
    ...
    def refresh_type(self, reporter):
        ...
        intramodule = (fromfile.module.get_top_level_module() == tofile.module.get_top_level_module())
        if tofile.type != 'publicheader' and tofile.type != 'libheader':
            if intramodule:
                type = 'intramodule'
            else:
                reporter.error(fromfile.path, 'included file "{0}" not documented as part of any API'.format(tofile.path))
                type = 'undocumented'
            ...
            if tofile.type == 'publicheader':
                ...
                reporter.error(fromfile.path, 'unknown link type to "{0}"'.format(tofile.path))
                type = 'unknown'
            ...
            if tofile.type == 'publicheader' or tofile.doctype == 'public':
                ...
                reporter.error(fromfile.path, 'public API file includes non-public header "{0}"'.format(tofile.path))
                type = 'undocumented'
        else:
            reporter.error(fromfile.path, 'unknown link type to "{0}"'.format(tofile.path))
            type = 'unknown'
    ...
    def merge_link(self, other):
        ...
        """
        if Link.priorities[other.type] < Link.priorities[self.type]:
            self.type = other.type

    def format(self):
        """Format this link for 'dot'."""
        if isinstance(self.fromnode.obj, File) and isinstance(self.tonode.obj, File):
            properties = ''
        elif self.type == 'intramodule':
            properties = ''
        elif self.type == 'test':
            properties = 'color=grey75, style=dotted'
        elif self.type == 'libimpl':
            properties = 'color=".66 .5 1"'
        elif self.type == 'pubimpl':
...
class Node(object):
    ...
    def format(self):
        ...
        if self.children:
            if not self.root:
                result += '    subgraph cluster_{0}'.format(self.obj.nodename) + ' {\n'
            ...
            result += '    {0} [{1}]\n'.format(self.obj.nodename, self.obj.node_properties())
        return result
...
class Graph(object):
        ...
        for link in self.links:
            ...
            if isfrom and isto:
                ...
                if not link.tonode in linksfrom:
                    linksfrom[link.tonode] = Link(target, link.tonode, link.type)
                else:
                    linksfrom[link.tonode].merge_link(link)
            elif isto:
                if not link.fromnode in linksto:
                    linksto[link.fromnode] = Link(link.fromnode, target, link.type)
                else:
    ...
    def write(self, fp):
        fp.write('digraph includedeps {\n')
        if self.left_to_right:
            fp.write('    rankdir = LR\n')
        if self.concentrate:
            fp.write('    concentrate = true\n')
        fp.write('    node [fontname="FreeSans",fontsize=10,height=.2,shape=box]\n')
        for link in self.links:
            fp.write('    ' + link.format() + '\n')
        for node in self.nodes:
            fp.write(node.format())
        fp.write('}\n')
...
class File(object):
    ...
    def set_installed(self, reporter):
        if self.type != 'header':
            reporter.input_warning(self.path, 'installing {0} file'.format(self.type))
            return
        self.installed = True
        ...
        with open(filename, 'r') as fp:
            for line in fp:
                ...
                if match:
                    ...
                    if os.path.abspath(os.path.join(includedirs[0], includefile)) in ignorelist:
                        continue
                    for dir in includedirs:
                        fullpath = os.path.abspath(os.path.join(dir, includefile))
                        if os.path.exists(fullpath):
                            if self.installed and dir != includedirs[0]:
                                reporter.error(self.path, 'installed header includes "{0}", which is not found using relative path'.format(includefile));
                            includefile = fullpath
                            break
                    else:
                        reporter.input_warning(self.path, 'included file "{0}" not found'.format(includefile))
                        continue
                    if includefile in allfiles:
                        other = allfiles[includefile]
                        if self.installed and not other.installed:
                            reporter.error(self.path, 'installed header includes non-installed header "{0}"'.format(other.path))
                ...
                if not foundfileblock:
                    if infileblock:
                        ...
                        if match:
                            if docmodule:
                                reporter.error(self.path, 'file documented in multiple modules')
                        ...
                        if match:
                            ...
                            if not docspec:
                                ...
                                reporter.input_warning(self.path, 'unknown specifier "{0}"'.format(docspec))
                                self.doctype = 'unknown'
                            infileblock = True
                            if self.type == 'header':
                                self.type = 'implheader' # Default type if no other found
        if self.doctype == 'none':
            reporter.error(self.path, 'file not documented')
        elif self.doctype == 'implementation' and self.type in ('publicheader', 'libheader'):
            reporter.error(self.path, 'file documentation visibility incorrect')
        elif self.doctype == 'library' and self.type == 'publicheader':
            reporter.error(self.path, 'file documentation visibility incorrect')
        if self.installed and self.doctype not in ('public', 'unknown'):
            reporter.error(self.path, 'installed header has no public documentation')
        elif not self.installed and self.doctype == 'public':
            reporter.error(self.path, 'non-installed file has public documentation')
        if docmodule and \
                not self.module.nodename.startswith('module_' + docmodule) and \
                not self.module.nodename.startswith('module_gromacs_' + docmodule):
            reporter.error(self.path, 'file documented in incorrect module "{0}"'.format(docmodule))
...
class Module(object):
    ...
    def add_nested_file(self, modules, path):
        if len(modules) == 1:
            ...
            file = module.add_nested_file(modules[1:], path)
        return file

    def create_node(self):
        node = Node(self)
        for file in self.files:
            node.add_child(file.get_node())
        for child in self.children.itervalues():
            node.add_child(child.create_node())
        return node
...
class Dependencies(object):
    def __init__(self, rootdir, includedirs, installedfiles):
        ...
        if includedirs:
            ...
            self.includedirs = self.rootpath + [os.path.join(rootdir, 'gromacs', 'legacyheaders')]
    ...
    def add_file(self, filename, reporter):
        ...
        for root in self.rootpath:
            ...
            reporter.input_warning(filename, 'input file not under root path, skipped')
            return
        modules = relpath.split(os.sep)
        file = self.root.add_nested_file(modules, relpath)
        if fullpath in self.installedfiles:
            file.set_installed(reporter)
        self.files[os.path.abspath(filename)] = file

    def scan_files(self, ignorelist, reporter):
        for (filename, file) in self.files.iteritems():
            file.scan(filename, self.files, self.includedirs, ignorelist, reporter)
        for file in self.files.itervalues():
            for link in file.get_links():
...
def print_module_graph(fp, deps, options):
    graph = deps.create_modules_graph()
    graph.write(fp)

def print_file_graph(fp, deps, options):
    ...
    #    if sourcenodes:
    #        fp.write('    { rank = min; ' + '; '.join(sourcenodes) + '}\n')
    ...
    #    if extnodes:
    #        fp.write('    { rank = max; ' + '; '.join(extnodes) + '}\n')

def print_module_file_graph(fp, deps, module, options):
...
parser.add_option('-I', '--includedir', action='append',
                  help='Specify additional directories to search for include files')
...
#                  help='Force external dependencies files at the bottom of the graph')
...
if options.installed:
    with open(options.installed, 'r') as fp:
        for line in fp:
...
if options.module_graph:
    with open(os.path.join(options.outdir, options.module_graph), 'w') as fp:
        print_module_graph(fp, deps, options)
if options.file_graph:
    with open(os.path.join(options.outdir, options.file_graph), 'w') as fp:
        print_file_graph(fp, deps, options)
if options.module_file_graphs:
    ...
    for module in deps.get_toplevel_modules():
        ...
        with open(os.path.join(options.outdir, filename), 'w') as fp:
            print_module_file_graph(fp, deps, module, options)
