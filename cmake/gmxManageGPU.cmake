...
macro(gmx_gpu_setup)
    ...
    cmake_dependent_option(GMX_CUDA_NB_SINGLE_COMPILATION_UNIT
        "Whether to compile the CUDA non-bonded module using a single compilation unit." OFF
        "GMX_GPU" ON)
    ...
    if (GMX_GPU)
        ...
        # - with CUDA <v5.0
        # - when compiling for CC 2.x devices where otherwise buggy kernel code is generated
        #   (the non-bonded parameter texture fetch returns 0)
        if (_cuda_version_changed)
            if (CUDA_VERSION VERSION_LESS 5.0 OR)
                message(STATUS "Disabling multiple compilation units for the CUDA non-bonded module: not supported by CUDA ${CUDA_VERSION}.")
        ...
        gmx_check_if_changed(_gmx_cuda_target_changed GMX_CUDA_TARGET_SM GMX_CUDA_TARGET_COMPUTE)
        if(_gmx_cuda_target_changed)
            ...
            (GMX_CUDA_TARGET_SM MATCHES "2[01]" OR GMX_CUDA_TARGET_COMPUTE MATCHES "2[01]"))
                message(STATUS "Disabling multiple compilation units for the CUDA non-bonded module: not compatible with CC 2.x devices. To use multiple compilation units specify only CC >=3.0 target architectures in GMX_CUDA_TARGET_SM/GMX_CUDA_TARGET_COMPUTE")
                set_property(CACHE GMX_CUDA_NB_SINGLE_COMPILATION_UNIT PROPERTY VALUE ON)
            else()
                message(STATUS "Enabling multiple compilation units for the CUDA non-bonded module.")
                set_property(CACHE GMX_CUDA_NB_SINGLE_COMPILATION_UNIT PROPERTY VALUE OFF)
