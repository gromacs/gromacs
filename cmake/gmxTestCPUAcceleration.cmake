...
# Helper routine to find the flag (from a list) needed to compile a specific source file
...
# SOURCE_FILE         Source code file containing the instructions to test
...
function(gmx_try_compile_flags VARIABLE DESCRIPTION SOURCE_FILE FLAGSVAR)
    if(NOT DEFINED ${VARIABLE})
        # Insert a blank element last in the list, so we try without any flags too
        # This must come last, since some compilers (Intel) might try to emulate e.g. 
        # AVX instructions with SSE4.1 otherwise.
        foreach(_testflag ${ARGN} "")
            message(STATUS "Try ${DESCRIPTION} = [${_testflag}]")
            message(STATUS "Performing Test ${VARIABLE}")
            try_compile(${VARIABLE} ${CMAKE_BINARY_DIR} ${SOURCE_FILE} COMPILE_DEFINITIONS "${${FLAGSVAR}} ${_testflag}")
            if(${VARIABLE})
                message(STATUS "Performing Test ${VARIABLE} - Success")
                set(${FLAGSVAR} "${${FLAGSVAR}} ${_testflag}" PARENT_SCOPE)
                break()
            else(${VARIABLE})
...
# the most recent instruction set. I.e., if your machine supports AVX2, 
# we will set flags both for AVX2, AVX, SSE4.1, and SSE2 support.
...
elseif(${GMX_CPU_ACCELERATION} STREQUAL "SSE2")
    ...
    gmx_try_compile_flags(CFLAGS_SSE2
                          "C compiler SSE2 flag"
                          "${CMAKE_SOURCE_DIR}/cmake/TestSSE2.c"
                          ACCELERATION_C_FLAGS
                          "-msse2" "/arch:SSE2")
    gmx_try_compile_flags(CXXFLAGS_SSE2
                          "C++ compiler SSE2 flag"
                          "${CMAKE_SOURCE_DIR}/cmake/TestSSE2.cpp"
                          ACCELERATION_CXX_FLAGS
                          "-msse2" "/arch:SSE2")

    if(NOT CFLAGS_SSE2 OR NOT CXXFLAGS_SSE2)
        message(FATAL_ERROR "Cannot find SSE2 compiler flag."
                            "Use a newer compiler, or disable acceleration (slower).")
    endif()
...
elseif(${GMX_CPU_ACCELERATION} STREQUAL "SSE4.1")
    ...
    gmx_try_compile_flags(CFLAGS_SSE4_1
                          "C compiler SSE4.1 flag"
                          "${CMAKE_SOURCE_DIR}/cmake/TestSSE4_1.c"
                          ACCELERATION_C_FLAGS
                          "-msse4.1" "/arch:SSE4.1" "/arch:SSE2")
    gmx_try_compile_flags(CXXFLAGS_SSE4_1
                          "C++ compiler SSE4.1 flag"
                          "${CMAKE_SOURCE_DIR}/cmake/TestSSE4_1.cpp"
                          ACCELERATION_CXX_FLAGS
                          "-msse4.1" "/arch:SSE4.1" "/arch:SSE2")
    ...
    if(CMAKE_C_COMPILER_ID MATCHES "Intel" AND CMAKE_C_COMPILER_VERSION VERSION_EQUAL "11.1")
        message(FATAL_ERROR "You are using Intel compiler version 11.1, which produces incorrect "
                            "results with SSE4.1 acceleration. You need to use a newer "
                            "compiler (e.g. icc >= 12.0) or in worst case try a lower level of "
                            "acceleration if performance is not critical.")
    endif()
...
elseif(${GMX_CPU_ACCELERATION} STREQUAL "AVX_128_FMA")
    ...
    # On OS X, we often want to use gcc instead of clang, since gcc supports OpenMP. However, by default
    # gcc uses the external system assembler, which does not support AVX, so we need to tell the linker
    # to use the clang compilers assembler instead - and this has to happen before we detect AVX flags!
    ...
    gmx_try_compile_flags(CFLAGS_AVX
                          "C compiler AVX flag"
                          "${CMAKE_SOURCE_DIR}/cmake/TestAVX.c"
                          ACCELERATION_C_FLAGS
                          "-mavx")
    gmx_try_compile_flags(CXXFLAGS_AVX
                          "C++ compiler AVX flag"
                          "${CMAKE_SOURCE_DIR}/cmake/TestAVX.cpp"
                          ACCELERATION_CXX_FLAGS
                          "-mavx")
    ...
    if(HAVE_X86INTRIN_H)
        set(CMAKE_C_FLAGS "${CMAKE_C_FLAGS} -DHAVE_X86INTRIN_H")
        set(CMAKE_CXX_FLAGS "${CMAKE_CXX_FLAGS} -DHAVE_X86INTRIN_H")
    endif()
    if(HAVE_INTRIN_H)
        set(CMAKE_C_FLAGS "${CMAKE_C_FLAGS} -DHAVE_INTRIN_H")
        set(CMAKE_CXX_FLAGS "${CMAKE_CXX_FLAGS} -DHAVE_INTRIN_H")
    endif()

    gmx_try_compile_flags(CFLAGS_AVX_128_FMA
                          "C compiler AVX FMA4 flag"
                          "${CMAKE_SOURCE_DIR}/cmake/TestAVX_128_FMA.c"
                          ACCELERATION_C_FLAGS
                          "-mfma4")
    gmx_try_compile_flags(CXXFLAGS_AVX_128_FMA
                          "C++ compiler AVX FMA4 flag"
                          "${CMAKE_SOURCE_DIR}/cmake/TestAVX_128_FMA.cpp"
                          ACCELERATION_CXX_FLAGS
                          "-mfma4")
    ...
    if(NOT CFLAGS_AVX_128_FMA OR NOT CXXFLAGS_AVX_128_FMA OR NOT CFLAGS_AVX_128_FMA OR NOT CXXFLAGS_AVX_128_FMA)
        message(FATAL_ERROR "Cannot find compiler flags for 128 bit AVX with FMA support. "
                            "Use a newer compiler, or choose SSE4.1 acceleration (slower).")
    endif()

    ### STAGE 3: Optional: Find the XOP instruction flag (No point in yelling if this does not work)
    gmx_try_compile_flags(CFLAGS_AVX_XOP
                          "C compiler AVX XOP flag"
                          "${CMAKE_SOURCE_DIR}/cmake/TestAVX_XOP.c"
                          ACCELERATION_C_FLAGS
                          "-mxop")
    gmx_try_compile_flags(CXXFLAGS_AVX_XOP
                          "C++ compiler AVX XOP flag"
                          "${CMAKE_SOURCE_DIR}/cmake/TestAVX_XOP.cpp"
                          ACCELERATION_CXX_FLAGS
                          "-mxop")
    ...
    if (APPLE AND (${CMAKE_C_COMPILER_ID} STREQUAL "Clang" OR
        ...
        message(WARNING "Due to a known compiler bug, Clang up to version 3.2 (and Apple "
                        "Clang up to version 4.1) produces incorrect code with AVX_128_FMA "
                        "acceleration. As we cannot work around this bug on OS X, you will "
                        "have to select a different compiler or CPU acceleration.")
    endif()
...
elseif(${GMX_CPU_ACCELERATION} STREQUAL "AVX_256")
    ...
    if(NOT CFLAGS_AVX OR NOT CXXFLAGS_AVX)
        message(FATAL_ERROR "Cannot find AVX compiler flag. "
                            "Use a newer compiler, or choose SSE4.1 acceleration (slower).")
    endif()
...
elseif(${GMX_CPU_ACCELERATION} STREQUAL "AVX2_256")
    ...
    gmx_try_compile_flags(CFLAGS_AVX2
                          "C compiler AVX2 flag"
                          "${CMAKE_SOURCE_DIR}/cmake/TestAVX2.c"
                          ACCELERATION_C_FLAGS
                          "-march=core-avx2" "-march=core-avx2 -mavx2" "-mavx2")
    gmx_try_compile_flags(CXXFLAGS_AVX2
                          "C++ compiler AVX2 flag"
                          "${CMAKE_SOURCE_DIR}/cmake/TestAVX2.cpp"
                          ACCELERATION_CXX_FLAGS
                          "-march=core-avx2" "-march=core-avx2 -mavx2" "-mavx2")

    if(NOT CFLAGS_AVX2 OR NOT CXXFLAGS_AVX2)
        message(FATAL_ERROR "Cannot find AVX2 compiler flag. "
                            "Use a newer compiler, or choose AVX acceleration (slower).")
    endif()
...
elseif(${GMX_CPU_ACCELERATION} STREQUAL "IBM_QPX")
    ...
    if (TEST_QPX)
        message(WARNING "IBM QPX acceleration was selected. This will work, but "
                        "SIMD-accelerated kernels are only available for the Verlet "
                        "cut-off scheme. The plain C kernels that are used for the "
                        "group cut-off scheme kernels will be slow, so please consider "
        ...
        message(FATAL_ERROR "Cannot compile the requested IBM QPX intrinsics. If you "
                            "are compiling for BlueGene/Q with the XL compilers, "
                            "use 'cmake .. -DCMAKE_TOOLCHAIN_FILE=Platform/BlueGeneQ-static-XL-C' "
                            "to set up the tool chain.")
    endif()
