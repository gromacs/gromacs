...
macro(manage_linear_algebra_library name function_in_library)
    ...
    if(GMX_${name}_USER AND (GMX_EXTERNAL_${name} OR NOT DEFINED GMX_EXTERNAL_${name}))
        ...
        if(NOT _${name}_user_works AND NOT "${GMX_${name}_USER}" STREQUAL "${GMX_${name}_USER_PREVIOUS_VALUE}")
            # Write output on the first pass, and if GMX_${name}_USER has changed.
        ...
        if(HAVE_LIBMKL AND NOT "${GMX_${name}_USER}" STREQUAL "${GMX_${name}_USER_PREVIOUS_VALUE}")
            # Write output on the first pass, and if GMX_${name}_USER has changed.
            message(STATUS "MKL and GMX_${name}_USER were both specified. Using the latter for ${name}.")
        endif()
        set(GMX_${name}_USER_PREVIOUS_VALUE ${GMX_${name}_USER} CACHE INTERNAL "Previous value for GMX_${name}_USER, so that we can detect when it changes")
    endif()
...
#            list(APPEND LINEAR_ALGEBRA_LIBRARIES ${FFT_LINKER_FLAGS} ${FFT_LIBRARIES})
    ...
    if (NOT GMX_${name}_FOUND)
        ...
        # Write message on the first pass through CMake, and
        # thereafter only if the user changes something such that
        # GROMACS is now using internal ${name}.
        if(NOT "${GMX_EXTERNAL_${name}}" STREQUAL "${GMX_EXTERNAL_${name}_PREVIOUS_VALUE}")
            ...
            if(GMX_EXTERNAL_${name})
                ...
                #
                # An alternative would be setting a variable of some
                # other name, which would later be used to determine
                # the linker line and/or whether the internal version
                # of the libraries was compiled.
        ...
        if(NOT "${GMX_EXTERNAL_${name}}" STREQUAL "${GMX_EXTERNAL_${name}_PREVIOUS_VALUE}")
            if(GMX_EXTERNAL_${name} OR NOT DEFINED GMX_EXTERNAL_${name})
                message(STATUS "Found ${name} external to GROMACS")
            else()
                message(STATUS "Using ${name} internal to GROMACS")
            endif()
        endif()
    endif()
    ...
    set(GMX_EXTERNAL_${name}_PREVIOUS_VALUE ${GMX_EXTERNAL_${name}} CACHE INTERNAL "Previous value for GMX_EXTERNAL_${name}, so that we can detect when it changes")
endmacro()
