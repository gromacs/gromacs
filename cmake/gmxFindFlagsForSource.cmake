...
FUNCTION(GMX_FIND_FLAG_FOR_SOURCE RESULT_VARIABLE SOURCE LANGUAGE TOOLCHAIN_FLAGS)
    ...
    foreach(_testflag ${ARGN} "")
        ...
        gmx_check_compiler_flag("${_testflag}" "${LANGUAGE}" ${FLAG_WORKS_VARIABLE})
        ...
        if(${FLAG_WORKS_VARIABLE})
            ...
            gmx_check_source_compiles_with_flags("${SOURCE}" "${TOOLCHAIN_FLAGS} ${_testflag}" "${LANGUAGE}" ${COMPILE_WORKS_VARIABLE})
...
# fails to build, sets SUGGEST_BINUTILS_UPDATE in parent scope to suggest
# that the calling code tell the user about this issue if needed.
function(gmx_find_flags SOURCE TOOLCHAIN_C_FLAGS TOOLCHAIN_CXX_FLAGS C_FLAGS_VARIABLE CXX_FLAGS_VARIABLE)
    gmx_find_flag_for_source(${C_FLAGS_VARIABLE} "${SOURCE}" "C" "${TOOLCHAIN_C_FLAGS}" ${ARGN})
    set(${C_FLAGS_VARIABLE} ${${C_FLAGS_VARIABLE}} PARENT_SCOPE)
    gmx_find_flag_for_source(${CXX_FLAGS_VARIABLE} "${SOURCE}" "CXX" "${TOOLCHAIN_CXX_FLAGS}" ${ARGN})
    set(${CXX_FLAGS_VARIABLE} ${${CXX_FLAGS_VARIABLE}} PARENT_SCOPE)
