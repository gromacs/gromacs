...
# the cache variable BUILD_CPU_${TYPE} as the result. If the detect
# program fails to compile, or fails to run, the value of
# ${DEFAULT_OUTPUT} is stored instead.
#
...
#
function(gmx_run_cpu_detection TYPE DEFAULT_OUTPUT)
    string(TOUPPER ${TYPE} UPPERTYPE)
    string(TOLOWER ${TYPE} LOWERTYPE)
    if(BUILD_CPU_${UPPERTYPE})
        return()
    endif()

    set(OUTPUT_VAR "${DEFAULT_OUTPUT}")
    ...
    if(CMAKE_CROSSCOMPILING)
        ...
        set(BUILD_CPU_BINARY "${PROJECT_BINARY_DIR}/CMakeFiles/GmxDetectCpu${CMAKE_EXECUTABLE_SUFFIX}")
        ...
        if(NOT BUILD_CPU_COMPILED)
            ...
            if(NOT DEFINED BUILD_CPU_RUN_${UPPERTYPE})
                ...
                if (RESULT_VAR EQUAL 0)
                    ...
                    set(BUILD_CPU_RUN_${UPPERTYPE} TRUE CACHE INTERNAL "Records that cpu detection code with argument -${LOWERTYPE} has run, to prevent re-running")
                else()
                    message(STATUS "Did not detect build CPU ${TYPE} - detection program did not run successfully")
                endif()
            endif()
        endif()
    endif()
