...
if(${GMX_FFT_LIBRARY} STREQUAL "FFTW3")
    ...
    if(NOT ${FFTW}_FOUND)
      MESSAGE(FATAL_ERROR "Cannot find FFTW 3 (with correct precision - "
                          "libfftw3f for single-precision GROMACS or libfftw3 "
                          "for double-precision GROMACS). Either choose the "
                          "right precision, choose another FFT(W) library "
                          "(-DGMX_FFT_LIBRARY), enable the advanced option to "
                          "let GROMACS build FFTW 3 for you "
                          "(-GMX_BUILD_OWN_FFTW=ON), or use the really slow "
                          "GROMACS built-in fftpack library "
    ...
    if ((${GMX_CPU_ACCELERATION} MATCHES "SSE" OR ${GMX_CPU_ACCELERATION} MATCHES "AVX") AND NOT ${FFTW}_HAVE_SIMD)
      message(WARNING "The fftw library found is compiled without SIMD "
                      "support, which makes it slow. Consider recompiling it "
    ...
    if((${GMX_CPU_ACCELERATION} MATCHES "SSE" OR ${GMX_CPU_ACCELERATION} MATCHES "AVX") AND ${FFTW}_HAVE_AVX)
        ...
        message(WARNING "The FFTW library was compiled with --enable-avx to "
                        "enable AVX SIMD instructions. That might sound like a "
                        "good idea for your processor, but for FFTW versions "
                        "up to 3.3.3, these are slower than the SSE/SSE2 SIMD "
                        "instructions for the way GROMACS uses FFTs. "
                        "Limitations in the way FFTW allows GROMACS to measure "
                        "performance make it awkward for either GROMACS or FFTW "
                        "to make the decision for you based on runtime "
                        "performance. You should compile a different FFTW "
                        "library with --enable-sse or --enable-sse2. If you "
                        "have a more recent FFTW, you may like to compare the "
                        "performance of GROMACS with FFTW libraries compiled "
                        "with and without --enable-avx. However, the GROMACS "
                        "developers do not really expect the FFTW AVX "
                        "optimization to help, because the performance is "
...
elseif(${GMX_FFT_LIBRARY} STREQUAL "MKL")
    ...
    if (NOT MKL_MANUALLY)
        ...
        set(MKL_ERROR_MESSAGE "Make sure you have configured your compiler so "
                              "that ${FFT_LINKER_FLAGS} will work.")
    else()
        include_directories(${MKL_INCLUDE_DIR})
        set(FFT_LIBRARIES "${MKL_LIBRARIES}")
        set(MKL_ERROR_MESSAGE "The include path to mkl.h in MKL_INCLUDE_DIR, "
                              "and the link libraries in "
                              "MKL_LIBRARIES=${MKL_LIBRARIES} need to match "
                              "what the MKL documentation says you need for "
                              "your system: "
                              "${MKL_LIBRARIES_FORMAT_DESCRIPTION}")
    ...
    if(NOT TEST_MKL)
        ...
        message(FATAL_ERROR "Linking with MKL was requested, but was not "
                            "successful: ${MKL_ERROR_MESSAGE}")
    endif()
