...
macro(gmx_test_compiler_problems)
    ...
    if(NOT "${CMAKE_C_COMPILER_ID}" STREQUAL "${CMAKE_CXX_COMPILER_ID}")
        message(WARNING "The ids of the C and C++ compilers do not match "
                        "(${CMAKE_C_COMPILER_ID} and ${CMAKE_CXX_COMPILER_ID}, "
                        "respectively). Mixing different C/C++ compilers can "
                        "cause problems.")
    endif()
    if(NOT "${CMAKE_C_COMPILER_VERSION}" STREQUAL "${CMAKE_CXX_COMPILER_VERSION}")
        message(WARNING "The versions of the C and C++ compilers do not match "
                        "(${CMAKE_C_COMPILER_VERSION} and "
                        "${CMAKE_CXX_COMPILER_VERSION}, respectively). Mixing "
    ...
    if (CMAKE_C_COMPILER_ID STREQUAL "PGI")
        message(WARNING "All tested PGI compiler versions (up to 12.9.0) "
                        "generate binaries which produce incorrect results, or "
                        "even fail to  compile Gromacs. Highly recommended to "
                        "use a different compiler. If you choose to use PGI, "
                        "make sure to run the regressiontests.")
    endif()

    if(CMAKE_C_COMPILER_ID MATCHES "Intel" AND CMAKE_C_COMPILER_VERSION VERSION_LESS "12.0.0")
        message(WARNING "Intel compilers before 12.0.0 are not routinely "
                        "tested, so there may be problems. Version 11.1 with "
                        "SSE4.1 is known to produce incorrect results. It is "
                        "highly recommended to use a more up-to-date compiler. "
                        "If you choose to use this version, make sure you run "
                        "the regressiontests.")
    endif()
