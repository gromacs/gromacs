...
if(NOT GMX_GET_RELEASE_DOI)
    set(GMX_MANUAL_DOI "This is not a release version of GROMACS, please refer to one of the published articles for referencing." CACHE INTERNAL "reserved doi for GROMACS manual")
    ...
    if("${GMX_MANUAL_DOI}" STREQUAL "")
        gmx_register_doi("manual" ${GMX_MANUAL_DOI} ${GMX_MANUAL_ID})
    endif()
    # see above, not overwriting exisiting variables
    if("${GMX_SOURCE_DOI}" STREQUAL "")
        gmx_register_doi("source" ${GMX_SOURCE_DOI} ${GMX_SOURCE_ID})
    endif()
    add_custom_target(gmx-publish-source
        COMMAND
            ${PYTHON_EXECUTABLE} 
...
function(gmx_register_doi REGTYPE RETURNVAR1 RETURNVAR2)
    ...
    list(GET GMX_DOI_OUTPUT_VARIABLE 0 RETURNVAR1)
    # second list element is the submission id
    list(GET GMX_DOI_OUTPUT_VARIABLE 1 RETURNVAR2)
endfunction()
