...
# be carried out. All but the /dev-based check provide GPU names. Additionally,
# when lspci is used and a device with uknown PCI ID is encountered, lspci tries
# to check the online PCI ID database. if this is not available or the device is
# simply not recognized, no device names will be available.
#
...
# itnerfacing with the CUDA driver API.
#
...
macro(gmx_detect_gpu)
    ...
    if (NOT DEFINED GMX_DETECT_GPU_COUNT OR NOT DEFINED GMX_DETECT_GPU_INFO)
        ...
        if (NOT DEFINED GMX_DETECT_GPU_COUNT_NVIDIA_SMI OR TRUE)
        ...
        if (UNIX AND NOT (APPLE OR CYGWIN))
            ...
            # /dev/nvidia[0-9]-based detection (does not provide GPU information)
            if (NOT DEFINED GMX_DETECT_GPU_COUNT_DEV AND GMX_DETECT_GPU_COUNT EQUAL 0)
                set(GMX_DETECT_GPU_COUNT_DEV 0)
                file(GLOB _dev_nv_gpu "/dev/nvidia*")
                string(REGEX REPLACE "\n" ";" _dev_nv_gpu "${_dev_nv_gpu}")
                foreach (_dev_entry ${_dev_nv_gpu})
                    if (${_dev_entry} MATCHES "/dev/nvidia[0-9]+")
                        math(EXPR GMX_DETECT_GPU_COUNT_DEV "${GMX_DETECT_GPU_COUNT_DEV}+1")
                    endif()
                endforeach()

                set(GMX_DETECT_GPU_COUNT ${GMX_DETECT_GPU_COUNT_DEV})
        ...
        set(GMX_DETECT_GPU_COUNT_DEV ${GMX_DETECT_GPU_COUNT_DEV}
            CACHE INTERNAL "Number of NVIDIA GPUs detected in /dev")
