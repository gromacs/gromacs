...
if(${GMX_SIMD} STREQUAL "NONE")
    ...
    set(SIMD_STATUS_MESSAGE "CPU SIMD instructions disabled")
elseif(${GMX_SIMD} STREQUAL "SSE2")
    ...
    if(NOT CFLAGS_SSE2 OR NOT CXXFLAGS_SSE2)
        message(FATAL_ERROR "Cannot find SSE2 compiler flag. Use a newer compiler, or disable SIMD instructions (slower).")
    endif()
...
elseif(${GMX_SIMD} STREQUAL "SSE4.1")
    ...
    if(NOT CFLAGS_SSE4_1 OR NOT CXXFLAGS_SSE4_1)
        message(FATAL_ERROR "Cannot find SSE4.1 compiler flag. "
                            "Use a newer compiler, or choose SSE2 instructions (slower).")
    endif()

    if(CMAKE_C_COMPILER_ID MATCHES "Intel" AND CMAKE_C_COMPILER_VERSION VERSION_EQUAL "11.1")
        message(FATAL_ERROR "You are using Intel compiler version 11.1, which produces incorrect results with SSE4.1 instructions. You need to use a newer compiler (e.g. icc >= 12.0) or in worst case try a lower level of SIMD instruction set if performance is not critical.")
    endif()

    set(GMX_SIMD_X86_SSE4_1 1)
    set(SIMD_STATUS_MESSAGE "Enabling SSE4.1 SIMD intrinsics")
...
int main(){__m128 x=_mm_set1_ps(0.5);x=_mm_macc_ps(x,x,x);return 0;}"
    ...
    if(NOT CFLAGS_AVX_128_FMA OR NOT CXXFLAGS_AVX_128_FMA)
        message(FATAL_ERROR "Cannot find compiler flags for 128 bit AVX with FMA support. Use a newer compiler, or choose SSE4.1 intrinsics (slower).")
    endif()
...
int main(){__m128 x=_mm_set1_ps(0.5);x=_mm_frcz_ps(x);return 0;}"
    ...
    if (APPLE AND (${CMAKE_C_COMPILER_ID} STREQUAL "Clang" OR
        ...
        message(WARNING "Due to a known compiler bug, Clang up to version 3.2 (and Apple Clang up to version 4.1) produces incorrect code with AVX_128_FMA intrinsics. As we cannot work around this bug on OS X, you will have to select a different compiler or lower SIMD instruction set level.")
    endif()
    ...
    set(SIMD_STATUS_MESSAGE "Enabling 128-bit AVX SIMD instructions (with fused-multiply add)")
...
elseif(${GMX_SIMD} STREQUAL "AVX_256")
    ...
    if(NOT CFLAGS_AVX OR NOT CXXFLAGS_AVX)
        message(FATAL_ERROR "Cannot find AVX compiler flag. Use a newer compiler, or choose SSE4.1 instructions (slower).")
    endif()
...
elseif(${GMX_SIMD} STREQUAL "AVX2_256")
    ...
    if(NOT CFLAGS_AVX2 OR NOT CXXFLAGS_AVX2)
        message(FATAL_ERROR "Cannot find AVX2 compiler flag. Use a newer compiler, or choose AVX instructions (slower).")
    endif()

    # No need to test for Maskload bug - it was fixed before gcc added AVX2 support

    set(GMX_SIMD_X86_AVX2_256 1)
    set(SIMD_STATUS_MESSAGE "Enabling 256-bit AVX2 instructions")
...
elseif(${GMX_SIMD} STREQUAL "IBM_QPX")
    ...
    if (TEST_QPX)
        message(WARNING "IBM QPX SIMD instructions were selected. This will work, but SIMD instructions are only available for the Verlet cut-off scheme. The plain C kernels that are used for the group cut-off scheme kernels will be slow, so please consider using the Verlet cut-off scheme.")
...
elseif(${GMX_SIMD} STREQUAL "REFERENCE")
    ...
    set(SIMD_STATUS_MESSAGE "Enabling reference (emulated) SIMD instructions. Warning, this can be VERY slow!")

else()
    gmx_invalid_option_value(GMX_CPU_SIMD)
endif()
