...
function(gmx_suggest_simd _suggested_simd)
    ...
    set(SUGGEST_SIMD_QUIETLY 0)
    if(GMX_DETECTSIMD_FEATURES)
        # There's already been a suggestion made, so keep quiet.
        set(SUGGEST_SIMD_QUIETLY 1)
    endif()
    ...
    if(NOT CMAKE_CROSSCOMPILING)
        ...
        if(GMX_DETECTSIMD_COMPILED)
            if(NOT DEFINED GMX_DETECTSIMD_FEATURES)
                execute_process(COMMAND ${GMX_DETECTSIMD_BINARY} "-features"
                    RESULT_VARIABLE OUTPUT_RESULT
                    OUTPUT_VARIABLE OUTPUT_TMP
                    ERROR_QUIET)
                if(OUTPUT_RESULT EQUAL 0)
                    set(GMX_DETECTSIMD_FEATURES "${OUTPUT_TMP}" CACHE INTERNAL "Output from running cpuinfo for CPU features")
                else()
            ...
            if(GMX_DETECTSIMD_FEATURES)
                ...
                if(GMX_TARGET_X86)
                    if(GMX_DETECTSIMD_FEATURES MATCHES " avx512er ")
                        set(OUTPUT_SIMD "AVX_512_KNL")
                    elseif(GMX_DETECTSIMD_FEATURES MATCHES " avx512f ")
                        set(OUTPUT_SIMD "AVX_512")
                    elseif(GMX_DETECTSIMD_FEATURES MATCHES " avx2 ")
                        if(GMX_DETECTSIMD_FEATURES MATCHES " amd ")
                            set(OUTPUT_SIMD "AVX2_128")
                        else()
                            set(OUTPUT_SIMD "AVX2_256")
                        endif()
                    elseif(GMX_DETECTSIMD_FEATURES MATCHES " avx ")
                        if(GMX_DETECTSIMD_FEATURES MATCHES " fma4 ")
                    ...
                    elseif(GMX_DETECTSIMD_FEATURES MATCHES " sse4.1 ")
                        set(OUTPUT_SIMD "SSE4.1")
                    elseif(GMX_DETECTSIMD_FEATURES MATCHES " sse2 ")
                        set(OUTPUT_SIMD "SSE2")
                    endif()
                else()
                    if(GMX_DETECTSIMD_FEATURES MATCHES " vsx ")
                        set(OUTPUT_SIMD "IBM_VSX")
                    elseif(GMX_DETECTSIMD_FEATURES MATCHES " vmx ")
                        set(OUTPUT_SIMD "IBM_VMX")
                    elseif(GMX_DETECTSIMD_FEATURES MATCHES " qpx ")
                        set(OUTPUT_SIMD "IBM_QPX")
                    elseif(GMX_DETECTSIMD_FEATURES MATCHES " neon_asimd ")
                        set(OUTPUT_SIMD "ARM_NEON_ASIMD")
                    elseif(GMX_DETECTSIMD_FEATURES MATCHES " neon ")
