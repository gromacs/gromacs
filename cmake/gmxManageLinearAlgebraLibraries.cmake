...
macro(manage_linear_algebra_library name function_in_library)
    ...
    # GROMACS, not from the user.
    set(GMX_${name}_USER_DEFAULT OFF)
    ...
    if(GMX_EXTERNAL_${name} OR NOT DEFINED GMX_EXTERNAL_${name})
        ...
        if(GMX_${name}_USER)
            set(GMX_${name}_USER_DEFAULT ${GMX_${name}_USER})
...
#                list(APPEND LINEAR_ALGEBRA_LIBRARIES ${FFT_LINKER_FLAGS} ${FFT_LIBRARIES})
        ...
        if (NOT _library_was_found)
            # Couldn't find a linear algebra library, so default to the
            # internal one.
    ...
    set(GMX_${name}_USER ${GMX_${name}_USER_DEFAULT} CACHE BOOL "Use a ${name} library found on the system (OFF), or a ${name} library supplied by the user (any other value, which is a full path to that ${name} library)")
