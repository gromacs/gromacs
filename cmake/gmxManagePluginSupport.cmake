...
function(gmx_manage_plugin_support)
    gmx_option_trivalue(GMX_LOAD_PLUGINS "Compile with plugin support, needed to read VMD supported file formats (ON/OFF/AUTO)" AUTO)
    ...
    # changed.
    gmx_check_if_changed(EMIT_STATUS_MESSAGES GMX_LOAD_PLUGINS BUILD_SHARED_LIBS GMX_BUILD_MDRUN_ONLY GMX_PREFER_STATIC_LIBS HAVE_DLOPEN)
    ...
    if (WIN32 OR (BUILD_SHARED_LIBS AND HAVE_DLOPEN))
        ...
        set(issue_fatal_error OFF)
        set(fatal_error_message "")
        if (NOT HAVE_DLOPEN)
            set(message "dlopen() support for using dynamic plugins for VMD-supported file formats is missing. ")
            if (GMX_LOAD_PLUGINS_FORCE)
                set(issue_fatal_error ON)
                set(fatal_error_message "${fatal_error_message}${message}")
            endif()
            if (GMX_LOAD_PLUGINS_AUTO AND EMIT_STATUS_MESSAGES)
                message(STATUS "${message}So GROMACS will build without plugin support.")
            endif()
        endif()
        if(NOT BUILD_SHARED_LIBS)
            set(message "GROMACS only supports plugins in a build that uses shared libraries, which can be disabled for various reasons. BUILD_SHARED_LIBS=on and a toolchain that supports dynamic linking is required. (Hint: GMX_PREFER_STATIC_LIBS and GMX_BUILD_MDRUN_ONLY can influence BUILD_SHARED_LIBS, so if you need plugins, reconsider those choices.) ")
            if (GMX_LOAD_PLUGINS_FORCE)
                set(issue_fatal_error ON)
                set(fatal_error_message "${fatal_error_message}${message}")
            endif()
        ...
        if(issue_fatal_error)
            message(FATAL_ERROR "${fatal_error_message}Cannot build with GMX_LOAD_PLUGINS=${GMX_LOAD_PLUGINS}.")
        endif()
    endif()

    if(GMX_USE_PLUGINS AND EMIT_STATUS_MESSAGES)
