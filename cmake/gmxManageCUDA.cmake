...
# GMX_GPU has already been analysed to be on, auto or off.
# In the latter cases, GMX_GPU_AUTO or GMX_GPU_FORCE is true.
#
# Off does nothing.
# On requires that the build is not double precision, and that
#   CUDA is found (but does not require finding a GPU).
# Auto means we'll
# - not build for CUDA if the build is double precision
# - search for CUDA and set GMX_USE_CUDA=ON we find it.
# - check whether GPUs are present, and issue a warning
#   if CUDA is not found but GPUs were detected.

include(CMakeDependentOption)
include(gmxOptionUtilities)

if (NOT GMX_GPU)
    # The user turned GPU support off, there's nothing further to do
    return()
endif()
# After this point, we know GMX_GPU was ON or AUTO, thus
# exactly one of GMX_GPU_FORCE or GMX_GPU_AUTO is true.

if(GMX_GPU_FORCE AND GMX_DOUBLE)
...
if(GMX_GPU_AUTO AND GMX_DOUBLE)
    ...
    set(GMX_USE_CUDA OFF)
    return()
endif()

if (GMX_MANAGE_CUDA_DONE)
    set(MANAGE_CUDA_QUIETLY TRUE)
endif()
...
if(NOT GMX_GPU_FORCE AND NOT GMX_DETECT_GPU_AVAILABLE)
    # Stay quiet when detection has occured and found no GPU.
    # (Noise is acceptable when there is a GPU, or the user
    # required one.)
...
# Annoyingly enough, FindCUDA leaves a few variables behind as
# non-advanced.  We need to mark these advanced outside the
# conditional, otherwise, if the user turns GMX_GPU=OFF, these
# variables will be left behind in the non-advanced cache.
mark_as_advanced(CUDA_SDK_ROOT_DIR
                 CUDA_USE_STATIC_CUDA_RUNTIME
                 CUDA_dl_LIBRARY CUDA_rt_LIBRARY
                 )

# Depending on the current values of GMX_GPU_* and perhaps CUDA_FOUND:
# - GMX_GPU=OFF         : Skip this detection/setup (see above return() statement)
# - GMX_GPU_AUTO =TRUE,
#   CUDA_FOUND   =FALSE : Don't attempt a GPU build, but assemble
#                         a warning message which will be issued at the end of the
#                         configuration if GPU(s) are found below.
# - GMX_GPU_FORCE=TRUE,
#   CUDA_FOUND   =FALSE : The user requested GPU build and this requires CUDA, so we will
#                         fail if it is not available.
# - GMX_GPU_AUTO =TRUE,
#   CUDA_FOUND   =TRUE  : Go ahead and set GMX_USE_CUDA.
# - GMX_GPU_FORCE=TRUE,
#   CUDA_FOUND   =TRUE  : Go ahead and set GMX_USE_CUDA.
...
${GMX_DETECT_GPU_COUNT} NVIDIA GPU(s) found in the system")
        ...
        if (NOT GMX_DETECT_GPU_INFO STREQUAL "")
            set(CUDA_NOTFOUND_MESSAGE "${CUDA_NOTFOUND_MESSAGE}:")
...
https://developer.nvidia.com/cuda-gpus")
        ...
        # Don't issue warnings if we found neither CUDA or a GPU
        unset(CUDA_NOTFOUND_MESSAGE)
    endif()

    if (GMX_GPU_AUTO)
        set(GMX_USE_CUDA OFF)
        # A possible warning message constructed from
        # CUDA_NOTFOUND_MESSAGE is issued at the end of the main
        # CMakeLists.txt, unless we already did this in a previous
        # pass of CMake. Also keep the status quiet in that case.
        if (MANAGE_CUDA_QUIETLY)
            unset(CUDA_NOTFOUND_MESSAGE)
            message(STATUS "No compatible CUDA toolkit found, disabling native GPU acceleration")
        endif()
        return()
    endif()
    if (GMX_GPU_FORCE)
    ...
    if (NOT MANAGE_CUDA_QUIETLY)
        # Only show this message on the first pass
        message(STATUS "Enabling native CUDA GPU acceleration")
    endif()
    set(GMX_USE_CUDA ON)
endif()

# After here, we've found CUDA and are building with it.
...
macro(get_cuda_compiler_info COMPILER_INFO COMPILER_FLAGS)
    if(NOT CUDA_NVCC_EXECUTABLE)
        message(FATAL_ERROR "Need nvcc executable to get CUDA compiler information")
    endif()
...
# Set this property if the target has changed (including when it's the first time we're here)
if(_gmx_cuda_target_changed)
...
# Tell future CMake passes that we completed gmxManageCUDA without
# exiting with a fatal error, so it can e.g. keep quiet as
# appropriate.
set(GMX_MANAGE_CUDA_DONE TRUE CACHE INTERNAL "Whether gmxManageCUDA has already been run in this or a previous pass of CMake.")
