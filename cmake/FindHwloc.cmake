...
if(NOT Hwloc_FIND_QUIETLY)
    message(STATUS "Looking for Portable Hardware Locality (hwloc)")
endif()
...
if(HWLOC_INCLUDE_DIR AND HWLOC_LIBRARY)
    ...
    set(HWLOC_INCLUDE_DIRS "${HWLOC_INCLUDE_DIR}" CACHE STRING "Portable Hardware locality (hwloc) headers")
    set(HWLOC_LIBRARIES "${HWLOC_LIBRARY}" CACHE STRING "Portable Hardware locality (hwloc) library")
    unset(HWLOC_INCLUDE_DIR CACHE)
    ...
    if(NOT CMAKE_CROSSCOMPILING)
        ...
        if(HWLOC_INFO)
            execute_process(COMMAND ${HWLOC_INFO} "--version"
                            OUTPUT_VARIABLE HWLOC_VERSION_TMP)
            string(REGEX MATCH "[0-9]+.*[0-9]+" HWLOC_VERSION_TMP "${HWLOC_VERSION_TMP}")
            set(HWLOC_VERSION ${HWLOC_VERSION_TMP} CACHE STRING "Hwloc library version")
        endif()
    endif()
    ...
    if(NOT HWLOC_VERSION)
        ...
        file(READ "${HWLOC_INCLUDE_DIRS}/hwloc.h"
             HWLOC_H_CONTENTS LIMIT 16384)
        string(REGEX REPLACE ".*#define HWLOC_API_VERSION (0[xX][0-9a-fA-F]+).*" "\\1"
               HWLOC_API_VERSION "${HWLOC_H_CONTENTS}")
    ...
    if(NOT Hwloc_FIND_QUIETLY)
        message(STATUS "Looking for Portable Hardware Locality (hwloc) - found version ${HWLOC_VERSION}")
    endif()

else()

    if(NOT Hwloc_FIND_QUIETLY)
        message(STATUS "Looking for Portable Hardware Locality (hwloc) - not found")
    endif()
    if(Hwloc_FIND_REQUIRED)
        message(FATAL_ERROR "Required package Portable Hardware Locality (hwloc) not found.")
    endif()
