...
macro(gmx_set_build_information)
    ...
    if(NOT CMAKE_CROSSCOMPILING)
        ...
        try_run(GMX_DETECTCPU_RUN_VENDOR GMX_DETECTCPU_COMPILED_VENDOR
            ${CMAKE_BINARY_DIR}
        ...
        try_run(GMX_DETECTCPU_RUN_BRAND GMX_DETECTCPU_COMPILED_BRAND
            ${CMAKE_BINARY_DIR}
            ${CMAKE_SOURCE_DIR}/src/gmxlib/gmx_detectcpu.c
            COMPILE_DEFINITIONS "@GCC_INLINE_ASM_DEFINE@ -I${CMAKE_SOURCE_DIR}/include -DGMX_DETECTCPU_STANDALONE"
            RUN_OUTPUT_VARIABLE OUTPUT_CPU_BRAND ARGS "-brand")
        try_run(GMX_DETECTCPU_RUN_FAMILY GMX_DETECTCPU_COMPILED_FAMILY
            ${CMAKE_BINARY_DIR}
            ${CMAKE_SOURCE_DIR}/src/gmxlib/gmx_detectcpu.c
            COMPILE_DEFINITIONS "@GCC_INLINE_ASM_DEFINE@ -I${CMAKE_SOURCE_DIR}/include -DGMX_DETECTCPU_STANDALONE"
            RUN_OUTPUT_VARIABLE OUTPUT_CPU_FAMILY ARGS "-family")
        try_run(GMX_DETECTCPU_RUN_MODEL GMX_DETECTCPU_COMPILED_MODEL
            ${CMAKE_BINARY_DIR}
            ${CMAKE_SOURCE_DIR}/src/gmxlib/gmx_detectcpu.c
            COMPILE_DEFINITIONS "@GCC_INLINE_ASM_DEFINE@ -I${CMAKE_SOURCE_DIR}/include -DGMX_DETECTCPU_STANDALONE"
            RUN_OUTPUT_VARIABLE OUTPUT_CPU_MODEL ARGS "-model")
       try_run(GMX_DETECTCPU_RUN_STEPPING GMX_DETECTCPU_COMPILED_STEPPING
            ${CMAKE_BINARY_DIR}
            ${CMAKE_SOURCE_DIR}/src/gmxlib/gmx_detectcpu.c
            COMPILE_DEFINITIONS "@GCC_INLINE_ASM_DEFINE@ -I${CMAKE_SOURCE_DIR}/include -DGMX_DETECTCPU_STANDALONE"
            RUN_OUTPUT_VARIABLE OUTPUT_CPU_STEPPING ARGS "-stepping")
        try_run(GMX_DETECTCPU_RUN_FEATURES GMX_DETECTCPU_COMPILED_FEATURES
            ${CMAKE_BINARY_DIR}
            ${CMAKE_SOURCE_DIR}/src/gmxlib/gmx_detectcpu.c
            COMPILE_DEFINITIONS "@GCC_INLINE_ASM_DEFINE@ -I${CMAKE_SOURCE_DIR}/include -DGMX_DETECTCPU_STANDALONE"
        ...
        if(GMX_DETECTCPU_COMPILED_VENDOR AND GMX_DETECTCPU_RUN_VENDOR EQUAL 0)
            set(BUILD_CPU_VENDOR   "@OUTPUT_CPU_VENDOR@"   CACHE INTERNAL "Build CPU vendor")
        else()
            set(BUILD_CPU_VENDOR   "Unkown, detect failed"   CACHE INTERNAL "Build CPU vendor")
        endif()
        if(GMX_DETECTCPU_COMPILED_BRAND AND GMX_DETECTCPU_RUN_BRAND EQUAL 0)
            set(BUILD_CPU_BRAND   "@OUTPUT_CPU_BRAND@"   CACHE INTERNAL "Build CPU brand")
        else()
            set(BUILD_CPU_BRAND   "Unkown, detect failed"   CACHE INTERNAL "Build CPU brand")
        endif()
        if(GMX_DETECTCPU_COMPILED_FAMILY AND GMX_DETECTCPU_RUN_FAMILY EQUAL 0)
        ...
        if(GMX_DETECTCPU_COMPILED_MODEL AND GMX_DETECTCPU_RUN_MODEL EQUAL 0)
            set(BUILD_CPU_MODEL   "@OUTPUT_CPU_MODEL@"   CACHE INTERNAL "Build CPU model")
        else()
            set(BUILD_CPU_MODEL   "0"   CACHE INTERNAL "Build CPU model")
        endif()
        if(GMX_DETECTCPU_COMPILED_STEPPING AND GMX_DETECTCPU_RUN_STEPPING EQUAL 0)
        ...
        if(GMX_DETECTCPU_COMPILED_FEATURES AND GMX_DETECTCPU_RUN_FEATURES EQUAL 0)
