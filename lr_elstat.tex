...
Don't use \normindex{Ewald} unless you are absolutely sure this is
what you want - for almost all cases the \normindex{PME} method below
will perform much better.  If you still want to employ classical Ewald
summation enter this in your {\tt .mdp} file, if the side of your box
is about $3$~nm:
\begin{verbatim}
eeltype         = Ewald
rvdw            = 0.9
...
\normindex{PPPM}, but it is possible to obtain the trace. This means
that the sum of the pressure components is correct (and therefore the
isotropic pressure) but not necessarily the individual pressure
components!
...
to take at most 0.075 nm per gridpoint (e.g. 20 gridpoints for 1.5
nm).  PPPM does not provide the same accuracy as
\normindex{PME} but is faster in most cases. PPPM
can not be used with pressure coupling.
