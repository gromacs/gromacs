...
\section{Computational Electrophysiology}
\label{sec:compel}

The ``Computational Electrophysiology'' (CompEL) protocol allows the simulation of 
...
{\tt .mdp} options, which allow to set reference counts for the ions on either side
...
Alongside the calculation of channel conductance and ion selectivity, CompEL setups
...
a copy (or copies) of the channel/pore of interest in each bilayer (Fig.~\figref{compelsetup} A, B).
...
\caption{Typical double-membrane setup for doing computational electrophysiology 
measurements (A, B). The right plot (C) shows the potential difference $\Delta U$ resulting
from the requested charge imbalance $\Delta q_{ref}$ between the compartments.}
\label{fig:compelsetup}
\end{figure}

The potential difference $\Delta U$ across the membrane is easily calculated with the
{\tt gmx potential} utility. By this, the potential drop along the $z$ or
pore axis is exactly known in each time interval of the simulation (Fig.~\figref{compelsetup} C).
...
\eeq
The ionic selectivity is calculated as the number flux ratio of different species.
Best results are obtained by averaging these values over several overlapping time intervals.

The calculation of reversal potentials is achieved using a small set of simulations in which a given
...
communicated between the parallel processes, so it can decrease the simulation 
performance if chosen too small.
{\small
\begin{verbatim}
...
geometrical centers will be used ($\times$ in the figure). If, such as here, a membrane 
...
have to be defined in the index file.
{\small
\begin{verbatim}
...
\end{verbatim}}
{\tt swap_group} is the name of the index group that corresponds to those ions that 
...
flux or exchange, but will provide you with numbers how many ions permeated through
each of the channels. Note that an ion can only be counted as having passed a particular
channel if it is detected \emph{within} the defined split cylinder in a swap step.
For too large values of {\tt swap_frequency}, a particluar ion may be detected in compartment {\bf A}
in one swap step, and in compartment {\bf B} in the following swap step, so one cannot tell 
through which of the channels it passed.

{\tt coupl_steps} sets the number of swap attempt steps over which a discrepancy between 
actual and reference ion numbers in each compartment must persist before an exchange takes 
place. If {\tt coupl_steps} is set to 1, then the momentary ion distribution determines 
whether ions are exchanged. {\tt coupl_steps} \textgreater 1 will use the time-average
of ion distributions over this number of attempt steps instead. This is useful, for example, 
when ions diffuse near compartment boundaries, which would lead to numerous meaningless 
ion exchanges. A {\tt threshold} of 1 means that a swap is performed if the average ion 
count in a compartment differs by 1 or more from the requested values. Higher thresholds 
mean that larger differences are accepted. Ions are exchanged until the requested 
