...
\begin{itemize}
\item {\tt type} the only value is 1
\item {\tt label} unused, and has been removed from the code
\item {\tt phi} the value of φ0 in equations ~(\ref{dphi}) and ~(\ref{dihre}) of the manual
\item {\tt dphi} the value of Δφ in equation  ~(\ref{dihre}) of the manual
\item {\tt kfac} analogous to fac in the implementation of distance restraints, it is the factor by which the force constant is multiplied. By doing so, different restraints can be maintained with different force constants.
\item {\tt power} unused, and has been removed from the code
\end{itemize}
