...
In order to treat systems that are non-homogeneous outside of the cut-off distance we
can instead use the Particle-mesh Ewald method as discussed for electrostatics above.
In this case the modified Ewald equations becomes
\begin{eqnarray}
V &=& V_{dir} + V_{rec} + V_{0} \\[0.5ex]
...
parameter for particle $i$ and $j$. The star indicates that termis
with $i = j$ should be omitted when $((n_x,n_y,n_z)=(0,0,0))$, and
${\bf r}_{ij,{\bf n}}$ is the real distance between the particles.
We have also introduced the functions $f$ and $g$ given by
\begin{eqnarray}
...
\end{equation}
where $P_{n}$ is the Pascal triangle coefficients.
This introduces a non-negligible cost to the simulations, but the cost
can be reduced significantly by using geometrical combination rules in
reciprocal space. 

\begin{figure}
\centerline{\includegraphics[width=15cm]{plots/ljpmedifference}}
\caption {Dispersion potential between phosphorous and oxygen, Real and
reciprocal parts of the total dispersion are shown using Lorentz-Berthelot rules.
The difference introduced by a geometric approximation in the reciprocal part is small
compared to the total interaction energy.}
...
splitting between real and reciprocal space in the same way as {\tt ewald-rtol}.  
In addition to this the combination rules to be used in reciprocal
space is determined by {\tt lj-pme-comb-rule}. If the applied force
field uses Lorentz-Berthelot combination rules it is possible to set
{\tt lj-pme-comb-rule = geometric} in order to gain a significant
increase in performance. This will give a warning that can be
circumvented by passing the flag -maxwarn 1 to grompp. The details of
this approximation is touched upon in the section above.
%} % Brace matches ifthenelse test for gmxlite
