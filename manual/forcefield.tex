...
A second option is too explicitly subtract the reciprocal space contribution up to the cut-off distance:
\begin{eqnarray}
...
Here the LJ force up to the cut-off is unmodified and the error is significantly smaller than in the first option. This second option is used by the Verlet cut-off scheme.

A long-range dispersion correction can be applied to correct for
the approximation using a combination rule reciprocal space. This correction
assumes, as for the cut-off LJ potential, a uniform particle distribution.
But since the error of the combination rule approximation, especially with the second option described above, is extremely small, this long-range correction is not necessary in most cases.
