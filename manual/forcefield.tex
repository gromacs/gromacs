...
One possibility is to simply add the contribution of $g(r)$ in real space:
\begin{eqnarray}
V(r<r_c) & = & C_6 \,g(r) r^{-6} + C^\mathrm{comb}_6 [1 - g(r)] r^{-6} \\
...
This option is currently used in the group cut-off scheme.
A second option is to explicitly subtract the reciprocal space contribution up to the cut-off distance:
\begin{eqnarray}
V(r<r_c) & = & C_6 r^{-6} + \{-C_6 + C^\mathrm{comb}_6 [1 - g(r_c)]\} r_c^{-6}
\label {eqn:ljpmecorr2}\\
V(r>r_c) & = & C^\mathrm{comb}_6 [1 - g(r)] r_c^{-6}
\end{eqnarray}
where the second term in \eqnref{ljpmecorr2} is a constant which ensures that the potential is continuous at the cut-off.
Here the LJ force up to the cut-off is unmodified and the error is an order of magnitude smaller than in the first option. This second option is used by the Verlet cut-off scheme.
