...
with the standard one \ref{eq:G96angle}.
%
\begin{figure}
...
not allow the dihedral angle to move from the [$-180^{\circ}$:0] to [0:$180^{\circ}$] interval i.e.
not to have both $-\phi_0$ and $+\phi_0$ maxima, but only one of them.
Note that this is equivalent to a harmonic potential.

\subsubsection{Proper dihedrals: Combined bending-torsion potential}
\label{subsec:CBT}
When the four particles forming the dihedral angle become collinear (situation very rare in
atomistic simulations, but possible in coarse-grained simulations) the calculation of the
...
that prevents the bending angles $\theta_{i-1}$ and $\theta_{i}$, forming the dihedral,
...
for polymer melt simulations and extensively described in~\cite{MonicaGoga2013}.
...
but also on the bending angles $\theta_{i-1}$ and $\theta_i$ formed by three adjacent beads
...
in gray --- the same torsion potential without the $\sin^{3}\theta$ terms (Ryckaert-Bellemans type).
