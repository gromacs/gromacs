...
energy. For a simulations of a lipid bilayer, using a cut-off of 1.0
nm, the relative error in total dispersion energy will stay below
0.5\%. A more thorough discussion of this can be found in
\cite{Wennberg13}.
...
interactions calculated in direct space
\begin{equation}
  V^{ij}_\mathrm{dir} = C^{ij}_6 r^{-6}_{ij} - C^\mathrm{recip}_6 [1 - g(r_{ij})] r^{-6}_{ij}
\end{equation}
...
\begin{eqnarray}
V(r<r_c) & = & C_6 r^{-6} + \left(-C_6 + C^\mathrm{recip}_6 [1 - g(r_c)]\right) r_c^{-6}
\label {eqn:ljpmecorr2}\\
V(r>r_c) & = & C^\mathrm{recip}_6 [1 - g(r)] r^{-6}
\end{eqnarray}
in which the second term in \eqnref{ljpmecorr2} is a constant, which ensures
that the potential is continuous at the cut-off.
Here the LJ force up to the cut-off is unmodified and the error is an
order of magnitude smaller than in simulations where the reciprocal
contribution is left in place.
