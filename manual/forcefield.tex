...
\begin{equation}
C^{ij}_6 = \left(C^{ii}_6 \, C^{jj}_6\right)^{1/2}
\end{equation}
...
\begin{equation}
C^{ij}_6 = (\sigma_i+\sigma_j)^6=\sum_{n=0}^{6} P_{n}\sigma_{i}^{n}\sigma_{j}^{(6-n)},
\end{equation}
...
energy and an example of the error is plotted in \figref{ljpmedifference}.
The relative error in the total dispersion energy will stay below
0.5\% in a lipid bilayer simulation, when using a real space cut-off
of 1.0 nm. A more thorough discussion of this can be found in
\cite{Wennberg13}.

In {\gromacs} we correct for this reciprocal error by subtracting
the approximated reciprocal space contribution, up to the cut-off distance, from the
interactions calculated in real space
\begin{equation}
  V^{ij}_{real} = C^{ij}_6 r^{-6}_{ij} - C^\mathrm{recip}_6 [1 - g(r_{ij})] r^{-6}_{ij}
\end{equation}
were $C^\mathrm{recip}_6$ is constructed using the same combination rules as in the
reciprocal part of the potential. This will give a total interaction of
\begin{eqnarray}
V(r<r_c) & = & C_6 r^{-6} + \{-C_6 + C^\mathrm{recip}_6 [1 - g(r_c)]\} r_c^{-6}
\label {eqn:ljpmecorr2}\\
V(r>r_c) & = & C^\mathrm{recip}_6 [1 - g(r)] r^{-6}
\end{eqnarray}
where the second term in \eqnref{ljpmecorr2} is a constant which ensures
that the potential is continuous at the cut-off.
Here the LJ force up to the cut-off is unmodified and the error is an
order of magnitude smaller than in the first option.

A long-range dispersion correction can be applied to correct for the
approximation using a combination rule in reciprocal space. This
correction assumes, as for the cut-off LJ potential, a uniform
particle distribution.  But since the error of the combination rule
approximation is very small this long-range correction is not
necessary in most cases. Also note that this homogenous correction
does not correct the surface tension, which is an inhomogeneous
property.
