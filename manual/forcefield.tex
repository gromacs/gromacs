...
\sum_{n_x}\sum_{n_y}
\sum_{n_{z}*} \frac{C^{ij}_6 g(\beta {r}_{ij,{\bf n}})}{{r_{ij,{\bf n}}}^6} \\[0.5ex]
...
geometrical combination rules, yielding a total interaction of
\begin{eqnarray}
V(r<r_c) & = & \underbrace{C_6 g(\beta r) r^{-6}}_{\mathrm{Direct \  space}} + \underbrace{C^\mathrm{recip}_{6,\mathrm{geom}} [1 - g(\beta r)] r^{-6}}_{\mathrm{Reciprocal \  space}} \\
V(r>r_c) & = & \underbrace{C^\mathrm{recip}_{6,\mathrm{geom}} [1 - g(\beta r)] r^{-6}}_{\mathrm{Reciprocal \  space}}
\end{eqnarray}
which preserves a well-defined Hamiltonian and significantly increases
the performance of the simulations. This approximation does introduce
...
In {\gromacs} we correct for this error in the reciprocal-space
component by subtracting, from the direct-space interactions, the
contribution made by the approximation used in reciprocal space as
\begin{equation}
  V^{ij}_\mathrm{dir} = C^{ij}_6 r^{-6}_{ij} - C^\mathrm{recip}_6 [1 - g(\beta r_{ij})] r^{-6}_{ij}
\end{equation}
where $C^\mathrm{recip}_6$ is constructed using the same combination rules as in the
reciprocal part of the potential. This will give a total interaction of
\begin{eqnarray}
\nonumber V(r<r_c) &=& \underbrace{C_6 r^{-6} - C^\mathrm{recip}_6 [1 - g(\beta r)] r^{-6}}_{\mathrm{Direct \  space}} + \underbrace{C^\mathrm{recip}_6 [1 - g(\beta r)] r^{-6}}_{\mathrm{Reciprocal \  space}} \\ 
&=&C_6 r^{-6}
\label {eqn:ljpmecorr2} \\
V(r>r_c) &=& C^\mathrm{recip}_6 [1 - g(\beta r)] r^{-6}
\end{eqnarray}
Here the LJ force up to the cut-off is unmodified and the error is an
order of magnitude smaller than in simulations where the direct-space
interactions do not contain the correction for the approximation used
in reciprocal space. When using a potential-shift, the constant
\begin{equation}
\left(-C_6 + C^\mathrm{recip}_6 [1 - g(\beta r_c)]\right) r_c^{-6}
\end{equation}
is added to \eqnref{ljpmecorr2} in order to ensure that the potential
is continuous at the cutoff. In addition to this, a long-range
dispersion correction can be applied to correct for the approximation
using a combination rule in reciprocal space. This correction assumes,
as for the cut-off LJ potential, a uniform particle distribution.  But
since the error of the combination rule approximation is very small
this long-range correction is not necessary in most cases. Also note
that this homogenous correction does not correct the surface tension,
which is an inhomogeneous property.
