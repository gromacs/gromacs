...
energy. The approximation can be treated in two different ways.
One possibility is to simply compute the real space part involving
$g(r)$ using the real combination rule
\begin{equation}
  V^{ij}_{dir} = C_6 \, g(r) r^{-6}
\end{equation}
which gives a total interaction of
\begin{eqnarray}
V(r<r_c) & = & = C_6 r^{-6} + (C^\mathrm{comb}_6 - C_6)[1 - g(r)] r^{-6} \\
V(r>r_c) & = & C^\mathrm{comb}_6 [1 - g(r)] r^{-6}
\end{eqnarray}
An example of the error is plotted in \figref{ljpmedifference}.
...
This option was used in early implementations, but is currently not used
in {\gromacs}.

A second option is to explicitly subtract the reciprocal space
contribution up to the cut-off distance using the correct combination
rule for the reciprocal part
\begin{equation}
  V^{ij}_{dir} = C_6 r^{-6} - C^\mathrm{comb}_6 [1 - g(r)] r^{-6}
\end{equation}
which gives a total interaction of
\begin{eqnarray}
V(r<r_c) & = & C_6 r^{-6} + \{-C_6 + C^\mathrm{comb}_6 [1 - g(r_c)]\} r_c^{-6}
\label {eqn:ljpmecorr2}\\
V(r>r_c) & = & C^\mathrm{comb}_6 [1 - g(r)] r_c^{-6}
\end{eqnarray}
...
A long-range dispersion correction can be applied to correct for
the approximation using a combination rule in reciprocal space. This correction
assumes, as for the cut-off LJ potential, a uniform particle distribution.
But since the error of the combination rule approximation is very small, especially with the second option described above, this long-range correction is not necessary in most cases. Also note that this homogenous correction does not correct the surface tension, which is an inhomogeneous property.
