...
public:
    ...
    RefDataChecker(real relativeFloatingPointTolerance, std::string fileNameOverride) :
        refData_(fileNameOverride), checker_(refData_.rootChecker())
    {
        gmx::test::FloatingPointTolerance tolerance(gmx::test::FloatingPointTolerance(
                1e-6, 1.0e-9, relativeFloatingPointTolerance, relativeFloatingPointTolerance, 200, 100, true));
        checker_.setDefaultTolerance(tolerance);
    ...
    template<class T>
    void test3DVectors(gmx::ArrayRef<T> tArray, const std::string& testString)
    {
        // check as gmx::BasicVector
        std::vector<gmx::BasicVector<typename T::value_type>> gmxVectors(tArray.size());
        for (std::size_t i = 0; i < tArray.size(); ++i)
        {
            gmxVectors[i] = { tArray[i][0], tArray[i][1], tArray[i][2] };
        }

        checker_.checkSequence(gmxVectors.begin(), gmxVectors.end(), testString.c_str());
    }
