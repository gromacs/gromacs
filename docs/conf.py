...
#
# This is slightly redundant, since Gromacs is using a CMake
# configuration stage to set some values in some files. This is needed
# because it is apparently impossible to use sphinx substitution
# inside a URL, which we need for making links to files with Gromacs
# version numbers. For most configurable variables, we prefer to use
# sphinx substitution, so that editors can do better syntax
# highlighting.
rst_epilog = """
.. |GMX_CMAKE_MINIMUM_REQUIRED_VERSION| replace:: @GMX_CMAKE_MINIMUM_REQUIRED_VERSION@
.. |REQUIRED_CUDA_VERSION| replace:: @REQUIRED_CUDA_VERSION@
.. |REQUIRED_CUDA_COMPUTE_CAPABILITY| replace:: @REQUIRED_CUDA_COMPUTE_CAPABILITY@
