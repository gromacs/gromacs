...
.. TODO: Someone who knows how things work on Windows could write a few
   sentences here, since neither of the above mechanisms does.

For external libraries, standard CMake linking mechanisms are used.
...
The RPATH settings are in the root ``CMakeLists.txt``.

Finding data files
------------------

Other, |Gromacs|-specific part, of making the binaries relocatable is making
them able to find data files from the installation tree.  Such data files are
used for multiple purposes, including showing the quotes at the end of program
execution.  If the quote database is not found, the quotes are simply not
printed, but other files (mostly used by system preparation tools like
:ref:`gmx pdb2gmx` and :ref:`gmx grompp`, and by various analysis tools for
static data) will cause fatal errors if not found.
...
* When running executables from the build tree, it is desirable that they will
  ...
  CMake configuration).
* Also programs that link against ``libgromacs`` need to be able to find the
  data files if they call certain functions in the library.  In this case, the
  executable may not be in the same directory where |Gromacs| is.
...
Not all the above considerations are not fully addressed by the current
implementation, which works like this:

1. It finds the path to the current executable based on ``argv[0]``.  If the
   value contains a directory, it is interpreted as absolute or as relative to
   the current working directory.  If there is no directory, then a file by
   that name is searched from the directories listed in ``PATH``.  On Windows,
   it is taken into account that the current directory is also searched before
   ``PATH``.  If a file with a matching name is found, it is used without
   further checking.
...
3. If an absolute path to the executable was found, it is checked whether it is
   located in the build output directory (using ``stat()`` or similar to
   account for possible symbolic links in the directory components).  If it is,
   then the hard-coded source tree location is returned.
4. If an absolute path to the executable was found and it was not in the build
   tree, then all parent directories are checked.  If a directory here contains
   :file:`share/{gromacs}/top/gurgle.dat`, it is used for the data files.  The
   *gromacs* can be customized during initial CMake configuration.  The file
   name ``gurgle.dat`` and the location are considered unique enough to ensure
   that the correct directory has been found.
