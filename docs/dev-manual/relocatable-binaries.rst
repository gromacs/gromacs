...
For external libraries, standard CMake linking mechanisms are used; on Windows,
...
* Also programs that link against ``libgromacs`` need to be able to find the
  ...
  executable may not be in the same directory where |Gromacs| is.  In case of
  static linking, no part of the code is actually loaded from the |Gromacs|
  installation prefix, which makes it impossible to find the data files without
