...
``-DCLANG_TIDY=...``, and the full path to it can be set with ``-DCLANG_TIDY_EXE=...``.
  ...
  ``clang-tidy -fix {file}``, to fix all files in parallel
  ``run-clang-tidy.py -fix '(?<!/selection/parser\.cpp|selection/scanner\.cpp)$'``, and to fix all modified files
  ``run-clang-tidy.py -fix $(git diff HEAD --name-only)``. The run-clang-tidy.py script is in the
