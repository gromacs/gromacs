...
<dt>`GMX_SIMD`</dt>
<dd>
This is set when the SIMD module is enabled, i.e. when there is some sort
of SIMD support. It is a good idea to use a conditional with GMX_SIMD around
your entire SIMD code, although you will also have to check the specific
features you need - the SIMD support might e.g. only be single precision, which
will be of limited use if the present build is double precision.
</dd>
