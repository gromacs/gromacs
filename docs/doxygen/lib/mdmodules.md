...
the structured format defined in initMdpOptions().

The framework (and other code that interacts with the modules) will do the
following things to make mdp input work:
...
* When grompp reads the mdp file, it will first construct a flat
  ...
  defined conversions in the process.
