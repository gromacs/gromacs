...
FEP kernels are not supported for the Gapsys soft-core flavor.
Formerly ``gmx mdrun -nbfe gpu`` ran and did pair search that suited GPU NBFE kernels
despite never running those kernels. Now mdrun refuses to run with
``-nbfe gpu`` unless Beutler soft-core flavor is selected.
