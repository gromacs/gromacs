...
grompp now warns when a GROMOS force field is used. The GROMOS force field
has been parametrized with a physically incorrect multiple time stepping
cheme for a twin-range cut-off. When used with a single range cut-off scheme,
