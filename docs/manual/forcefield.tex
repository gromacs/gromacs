...
$F(r) = F_\alpha(r) = c\alpha \, r^{-(\alpha+1)}$.
where $c$ is a constant depending on the exact force being modeled. In
the remainder of this section we leave out $c$.
...
\end{array}
\eeq
Thus the total force function is ($r_1 \le r < r_c$):
\beq
...
\Phi_s(r)  & = & \displaystyle{\frac{1}{r^{\alpha}} - \frac{A}{3}(r-r_1)^3 + \frac{B}{4}(r-r_1)^4 - C}
\quad&r_1 \le r < r_c \\
\Phi_s(r)  & = & 0 \quad&r_c \le r
\end{array}
\eeq
where 
\beq
