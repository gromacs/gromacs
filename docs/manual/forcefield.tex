...
Neighbor searching (NS) is performed using a single-range approach,
where list of neighbors is formed from those particles whose current
distance is within a specified radius {\tt rlist}. That radius must be
least as large as max({\tt rcoulomb},{\tt rvdw}). This list is then
re-used for multiple steps (parameter {\tt nstlist}) before being
rebuilt. If the lifetime of the list is too large, particle movement
would prevent energy conservation.
