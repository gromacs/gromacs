...
In coarse-grained polymer simulations the begin{eqnarray}ds are often connected
...
\begin {figure}
\centerline{\includegraphics[width=4cm]{plots/ring-imp}~
\includegraphics[width=3cm]{plots/subst-im}~\includegraphics[width=3cm]{plots/tetra-im}}
...
\begin{itemize}
\item
it does not only depend on the dihedral angle $\phi_i$ (between the $i-2$, $i-1$, $i$ and $i+1$ begin{eqnarray}ds)
but also on the bending angles $\theta_{i-1}$ and $\theta_i$ defined from three adjacent begin{eqnarray}ds
...
However, these two forms have the inconveniences of the force derivation ($1/\sin\phi$) and of the alignment of begin{eqnarray}ds
...
Coarse-graining is a systematic way of reducing the number of degrees of freedom representing a system of interest. To achieve this, typically whole groups of atoms are represented by single begin{eqnarray}ds and the coarse-grained force fields describes their effective interactions. Depending on the choice of parameterization, the functional form of such an interaction can be complicated and often tabulated potentials are used.
