...
A stricter criterion, which helps avoid such issues, is to require that before a simulation marks a  point $\lambda_d$ along dimension $d$ as visited,
...
Difficult regions of sampling may also be detected by calculating the local friction tensor $\eta(\xi)$ (inversely proportional to the diffusion) along the reaction coordinate.
AWH calculates $\eta$~\cite{sivak2012thermodynamic} during simulation and writes it to the energy file.
A high peak in the friction metric ($\sqrt{\det\eta}$) is an indicator of a region that takes relatively long to sample properly.
