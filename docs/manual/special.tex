...
{\gromacs} can be used for solution-structural refinement by biasing the 
potential energy landscape towards configurations which reproduce experimental 
X-Ray scattering data~\cite{Bjorling2015}\index{SAXS/WAXS refinement}. 
Currently this is implemented as a force based on the Debye equation, which 
describes the intensity ($S$) scattered by randomly oriented molecules 
composed of spherically scattering atoms.
\begin{equation}
  S(q) = \sum_i\sum_j f_i(q)f_j(q) \frac{\sin qr_{ij}}{qr_{ij}} \quad,
  \label{eqn:debye}
\end{equation}
In this expression, $q$ is the magnitude of the scattering vector 
($q=4\pi\sin\theta/\lambda$, with $2\theta$ the scattering angle and 
$\lambda$ the wavelength), $f_i$ is the scattering factor for atom $i$, and 
$r_{ij}=|\mathbf{r}_{ij}|=|\mathbf{r}_i-\mathbf{r}_j|$, with $\mathbf{r}_i$ 
the coordinate vector for particle $i$. For macromolecules in solution, the 
excess scattering compared to the solvent is normally measured. The Debye 
equation can be approximately corrected for this by using modified atomic 
scattering factors~\cite{Fraser1978}.

For refinement, an artificial energy term $U_{waxs}$ is introduced, 
proportional to the sum-of-squares ($\chi^2$) of deviation from experiment.
\begin{eqnarray}
  U_{waxs} &=& \frac{k_\chi}{2}\chi^2 \quad, \\
  \chi^2 &=& \sum_q\frac{1}{{\sigma_q}^2} \left[ \Delta S_{exp}(q) - \alpha \{S_{calc}(q) - S_{ref}(q)\} \right]^2 \nonumber \\
  &=& \sum_q\frac{1}{{\sigma_q}^2} \left[ \Delta S_{exp}(q) - \alpha S_{calc}(q) + \alpha S_{ref}(q) \right]^2 \quad, \label{eqn:debye_chisq}
\end{eqnarray}
Here, $k_\chi$ is a coupling parameter, $\sigma_q$ is the experimental error at 
each $q$ point, and $S_{calc}$ is the calculated scattering intensity from the
current conformation.

It is possible to run against absolute intensities (as recorded in a normal 
SAXS measurement) or against difference scattering corresponding to two states 
(as recorded in time-resolved measurements). In the former case 
$\Delta S_{exp}(q)=0$ and $\alpha=1$ while $S_{ref}(q)$ is the experimental 
target scattering. In the latter $\Delta S_{exp}(q)$ is the experimental 
difference curve, $S_{ref}(q)$ is the scattering from the starting structure 
and $\alpha$ is the fraction of the observed sample which undergoes 
conformational change. Taking appropriate derivatives of (\ref{eqn:debye_chisq}) 
gives the force on a given atom $k$ (see~\cite{Bjorling2015} for details):
\begin{equation}
  \mathbf{F}_k^{scat} = 4\alpha\sum_q\Biggr\{A_q \sum_j
  f_k(q)f_j(q) \biggl[ \cos qr_{kj} - \frac{\sin qr_{kj}}{qr_{kj}}\biggr] \frac{\mathbf{r}_{kj}}{{r_{kj}}^2} \Biggr\} \quad,
...
Each term of Equation~\ref{eqn:debye} is explicitly specified in the toplogy 
using the {\tt [~pairs~]} directive (see Table~\ref{tab:topfile2}). Scattering 
factors are supplied as {\tt xml} files, and the tool {\tt gmx genrestr} will 
generate the topology section automatically.

Calculating these forces usually amounts to evaluating two double sums over 
all scatterers, an $N^2$ problem which can be made computationally cheaper by 
considering, for example, entire amino acid residues as coarse-grained 
scatterers~\cite{Niebling2014}. This can be accomplished by using  virtual 
sites (see section \ref{sec:vsitetop}), which can be set up using the 
{\tt gmx genrestr} utility.

To further accelerate refinement, the slowly varying SAXS/WAXS forces can be 
calculated and applied less often than other forces, using a multiple time 
step algorithm (see section \ref{sec:twinrange}). The period of calculating 
these forces is controlled by the mdp option {\tt waxs-nstcalc}.

For difference scattering measurements, the quantity $\alpha$ reflects the 
experimental conversion efficiency. An independent estimate is required to fit 
the data. The SAXS/WAXS refinement code optionally minimizes $\chi^2$ with 
respect to $\alpha$ using golden-ratio bisection within limits set by the 
{\tt debye-alpha-min} and {\tt debye-alpha-max} mdp keywords. This behavior is 
toggled using the {\tt debye-alpha-mode} option.
