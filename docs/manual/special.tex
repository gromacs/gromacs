...
\includegraphics[width=6cm]{plots/awh-invN.eps} % 1/N
}
\centerline{
\includegraphics[width=6cm]{plots/awh-sampleweights} % weights
\includegraphics[width=6cm]{plots/awh-pmfs.eps} % pmfs
}
\caption{AWH evolution in time for a Brownian particle in a double-well potential.
The reaction coordinate $\xi$ traverses the sampling interval multiple times in the initial stage before exiting and entering the final stage (top left).
...
An estimate of the PMF is also extracted from the simulation, which after exiting the initial stage this should estimate global free energy differences fairly accurately
(bottom right).
}
...
According to  Eq.~\ref{eq:awh:dfsize}, this scales down the update size by a factor of $1/\gamma$.
...
Furthermore note that for this choice $\rho(\lambda)$, there is no initial stage (section~\ref{sec:awh:initial-stage}).
