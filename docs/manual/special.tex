...
The initial sampling stage in AWH makes the method robust against the choice of input parameters.
...
The reaction coordinate $\xi(x)$ is coupled to $\lambda$ with a harmonic potential $Q(\xi,\lambda) = \frac{1}{2} k (\xi - \lambda)^2$, so that for large force constants $k$, $\xi \approx \lambda$.
$\lambda$ samples the user-defined interval $I$.
For multidimensional $\xi$, the sampling interval is the Cartesian product
...
Though often chosen to be a flat distribution, $g(\lambda)$, may be any distribution along $\lambda$.
...
\end{equation}
where the free energy $F(\lambda)$ is the convolution of the PMF with the Gaussian from the harmonic potential $Q$,
\begin{equation}\label{eq:flambda}
F(\lambda) = -\ln \int e^{- \beta Q(\xi(x),\lambda) - \beta V(x)}  dx.
\end{equation}
Note the dimensionless notation of free energies for compatibility with previously published work.
Units of energy are obtained by multiplication with $k_BT=1/\beta$.
Eq.~\ref{eq:flambda} shows that convergence of $P(\lambda)$ to the pre-defined target distribution $\rho(\lambda)$ strictly depends on convergence of $F(\lambda)$.
Thus, AWH focuses on convergence of $F(\lambda)$ through adaptive calculation.
When this estimate of the free energy converges, $P(\lambda)$ simultaneously converges to the target distribution $\rho(\lambda)$.


\subsubsection{The free energy update}
AWH iterates between data collection for a certain number of steps, and using that information to update and improve the free energy estimate.
The applied bias function $g_n$ at iteration $n$ is given by the free energy estimate $F_n(\lambda)$
and target distribution $\rho_n(\lambda)$, following Eq.~\ref{eq:plambda},
\begin{equation}\label{eq:grhofrelation}
g_n(\lambda) = \ln \rho_n(\lambda) +F_n(\lambda)
\end{equation}
Note that also the target distribution may be updated during the simulation (see examples in section~\ref{sec:targets}).
Substituting this choice of $g=g_n$ back into Eq.~\ref{eq:plambda} yields the noisy free energy correction
\begin{equation}\label{eq:dfnaive}
\Delta F_n(\lambda) 
= F(\lambda) - F_n(\lambda) 
= -\ln\frac{P_n(\lambda)}{\rho_n(\lambda)},
\end{equation}
which requires a well converged estimate of $P_n(\lambda)$ to refine the free energy estimate as $F_{n+1} = F_n + \Delta F_n$.
...
With the limited sampling in practice, update scheme \ref{eq:dfnaive} yields very noisy results.
AWH instead applies a free energy update that has the same form but which can be applied repeatedly with limited, localized, sampling,
\begin{equation}
\Delta F_n = -\ln \frac{W_n(\lambda) + \sum_t \omega_n(\lambda|x(t))}{W_n(\lambda) + \sum_t\rho_n(\lambda)) }.
\end{equation}
Here $W_n(\lambda)$ is the \emph{reference weight histogram} from prior sampling.
The update for $W(\lambda)$, disregarding the initial stage (see section~\ref{sec:initial-stage}), 
is
