...
calculates the PMF along a reaction coordinate by applying an adaptively determined biasing potential.
...
Characteristic for AWH is the inclusion of an initial sampling stage, which makes the method robust against the choice of input parameters.
...
Rather than biasing the reaction coordinate $\xi(x)$  directly,
AWH acts on a \emph{reference coordinate}  $\lambda$,
where $\lambda$ denotes the center of an  harmonic potential $Q(\xi,\lambda) = \frac{1}{2} k (\xi - \lambda)^2$.
If the force constant $k$ is chosen large enough, 
one will thus have $\xi \approx \lambda$.
$\lambda$ takes values in the user-defined sampling interval $I$.
In the general case where $\xi$ is multidimensional,
the sampling interval is the  Cartesian product
$I=\Pi_d I_d$ (a rectangular domain).
A suitable framework for sampling $\lambda$ is the extended ensemble~\cite{Lyubartsev1992},
\begin{equation}\label{eq:pxlambda}
P(x,\lambda) = \frac{1}{\mathcal{Z}}e^{g(\lambda) - \beta Q(\xi(x),\lambda) - \beta V(x)},
\end{equation}
where $g(\lambda)$ is a bias function (a free variable) and $V(x)$ is the unbiased potential energy of the system.
By tuning  $g(\lambda)$, any distribution along $\lambda$ can be obtained.
This is evident from
\begin{equation}\label{eq:plambda}
P(\lambda) = \int P(x,\lambda)  dx = 
\frac{1}{\mathcal{Z}}e^{g(\lambda)} \int e^{- \beta Q(\xi(x),\lambda) - \beta V(x)}  dx 
\equiv \frac{1}{\mathcal{Z}}e^{g(\lambda) - F(\lambda)},
\end{equation}
where  the free energy $F(\lambda)$ is a convolved version of the PMF,
\begin{equation}\label{eq:flambda}
F(\lambda) = -\ln \int e^{- \beta Q(\xi(x),\lambda) - \beta V(x)}  dx.
\end{equation}
We note that
AWH free energies are here dimensionless  for compatibility with notation used in previously published work,
Thus to obtain units of energy one needs to multiply by $k_BT=1/\beta$.
It is clear from Eq.~\ref{eq:flambda} that
in order to make $P(\lambda)$ equal to a pre-defined target distribution $\rho(\lambda)$,
$F(\lambda)$ needs to be determined accurately.
Thus, the goal of AWH is to adaptively calculate $F(\lambda)$,
so that when this estimate of the free energy converges, 
$P(\lambda)$ simultaneously converges to the target distribution $\rho(\lambda)$.


\subsubsection{The free energy update}
AWH works in an iterative fashion;
collecting data for a certain number of steps, then using that information to update and improve the free energy estimate.
Given the current best estimate of the free energy $F_n(\lambda)$ at iteration $n$,
 it is clear from Eq.~\ref{eq:plambda}
that to obtain the target distribution $\rho_n(\lambda)$ the current bias function should be  equal to 
\begin{equation}\label{eq:grhofrelation}
g_n(\lambda) = \ln \rho_n(\lambda) +F_n(\lambda)
\end{equation}
Note that also the target distribution may be subject to updates during the simulation (see examples in section~\ref{sec:targets}).
Substituting this choice of $g=g_n$ back into Eq.~\ref{eq:plambda}, we obtain the naive correction formula
...
if $P_n(\lambda)$ can be estimated.
...
AWH instead applies a free energy update that has the same form but which can be applied repeatedly with only limited amount of localized sampling,
\begin{equation}
...
$\Delta F_n(\lambda) < 0$,
which by Eq.~\ref{eq:grhofrelation} implies 
$\Delta g_n(\lambda) < 0$
(assuming $\Delta \rho_n \equiv 0$).
Thus, the probability to sample  $\lambda$ decreases after the update (see Eq.~\ref{eq:pxlambda}).
Secondly, the  normalization of the histogram $N_n=\sum_\lambda W_n(\lambda)$, determines the update size $|\Delta F(\lambda)|$.
For instance, 
for a single sample $\omega(\lambda|x)$, the shape of the  update is approximately a Gaussian function
...
\end{equation}
Thus, as samples accumulate in $W(\lambda)$ and $N_n$ grows, the updates get smaller, allowing for the free energy to converge.
...
\begin{equation}\label{eq:biaspotential}
U_n(\xi) = -k_BT\ln \int e^{ g_n(\lambda) -Q(\xi,\lambda)} d \lambda.
\end{equation}
...
In such cases, 
letting $W(\lambda)$  accumulate samples linearly as prescribed by Eq.~\ref{eq:w-update},
entails a too rapid decay of the free energy update size
(see  Eq.~\ref{eq:dfsize}).
This motivates introducing an \emph{initial stage}
...
a ``covering''. For the definition of a covering, see Eq.~\ref{eq:covering} below.
After having covered, $N_n$ is scaled up by a constant ``growth factor'' $\gamma$, chosen heuristically as $\gamma=3$.
According to  Eq.~\ref{eq:dfsize}, this scales down the update size by a factor of $1/\gamma$.
...
using a wide range of update sizes. %$\sim 1/(N_0\gamma^k)$.
This way, the initial stage also  makes AWH robust against the choice of $N_0$.

\subsubsection{The covering criterion}
In the general case of a multidimensional reaction coordinate $\lambda=(\lambda_d)$,
the  sampling interval $I$ is considered covered when all dimensions have been covered.
A dimension $d$ is covered if all points $\lambda_d$ in the one-dimensional sampling interval 
 $I_d$ have been ``visited''.
Finally, a point $\lambda_d \in I_d$  has been visited  if there is at least one point $\lambda\in I$
...
\Delta W(\lambda)
\ge w_{\mathrm{peak}}
 \equiv \prod_d \frac{\Delta \lambda_{d}}{\sqrt{2\pi}\sigma_k}.
\end{equation}
Here, $\Delta \lambda_d$ is the point spacing of the discretized  $I_d$ %of the $\lambda$ grid
and $\sigma_k=1/\sqrt{\beta k_d}$ (where $k_d$ is the force constant) 
...
One options is to postulate that the number of samples in the weight histogram $N_n$ should never exceed the actual number of collected samples,
...
then  the initial stage is over and from now $N$ simply grows at the sampling rate (see Eq.~\ref{eq:w-update}).
...
the last covering stage may be extended by a sufficient number of updates.
...
\end{equation}
This choice of exactly flattens $F(\lambda)$ in  user-defined sampling interval $I$.
...
Thus,  the target distribution is also updated (consistently with  Eq.~\ref{eq:grhofrelation}).
...
Furthermore note that for this choice $\rho(\lambda)$, there is no initial stage (section~\ref{sec:initial-stage}).
...
\subsection{Multiple independent or shared biases}
Multiple independent bias potentials may be applied within one simulation. 
This only makes sense if the biased coordinates $\xi_1$, $\xi_2$, etc., evolve essentially independently from one another.
...
traversing the sampling interval multiple times as is required by the initial stage (section~\ref{sec:initial-stage})
may take an infeasible mount of simulation time.
In these cases it could be advantageous to parallelize the work and have a group of multiple ``walkers'' $\xi_i(t)$ share a single bias potential.
This can be achieved  by collecting samples from all  $\xi_i$ of the same sharing group into a single histogram and update a common free energy estimate.
Samples can be shared between walkers within a simulation and/or between simulations.
...
This can be achieved e.g. by ``equilbrating'' the shared weight histogram before letting it grow;
...
One alternative is to just use the same criterion as for a single walker (but now with more samples), see Eq.~\ref{eq:covering}.
...
A stricter criterion is to require that before a simulation marks a  point $\lambda_d$ along dimension $d$ as visited,
and shares this with the other walkers, 
also all points  within a certain diameter $D_{\mathrm{cover}}$ should have been visited (i.e.fulfill Eq.~\ref{eq:covering}).
...
\subsection{Reweighting and combining biased data}
...
using the bias potential  $U_{n(t)}$, see Eq.~\ref{eq:biaspotential}.
Essentially, the biased data is binned along $u$ and 
for each sampled $x(t)$ the effect of $U_{n(t)}$ is removed by dividing by $e^{-U_{n(t)}(\xi(t))}$,
...
As can be seen $\mathcal{Z}$  depends on  $\Phi(\xi)$, the PMF of the (biased) reaction coordinate $\xi$
(which is calculated and written to file by the AWH simulation).
It is advisable to use only final stage data in the reweighting procedure, due the rapid change of the bias potential during the initial stage.
If one would include initial stage data, 
one should %when applying  Eq.~\ref{eq:unbias},
use the sample weights that are inferred by the repeated rescaling of  the histogram in the initial stage,
for sake of consistency. %with the rescaling samples applied in the initial stage (see section~\ref{sec:initial-stage})
Initial stage samples would then in any case be heavily scaled down relative to final stage samples.
Note that  Eq.~\ref{eq:unbias}  can also be used to combine data from multiple simulations (by adding another sum also over the trajectory set).
Furthermore, when multiple independent AWH simulations have generated a set of PMF estimates $\{\hat{\Phi}^i(\xi)\}$
a combined best estimate $\hat{\Phi}(\xi)$ can be obtained by applying self-consistent exponential averaging.
More details on this procedure and a derivation of Eq.~\ref{eq:unbias} (using slightly different notation) can be found in \cite{lindahl2017sequence}.
...
Monitoring the transition rate, across the sampling interval is also advisable; the trajectory $\xi(t)$ is printed to the pull output file.
...
\subsubsection{The initial update size}
For a simple model system of with constant diffusion $D$ along the reaction coordinate,
the average variance of the free energy scales as $\varepsilon^2 \sim 1/(ND)$, when $N$ is kept constant.
This provides a ballpark estimate that is used by AWH for initializing  $N$ in terms of more meaningful quantities
