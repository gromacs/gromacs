...
there is no penalty to creating a net dipole in the unit cell. This usually does
not create issues, unless an electric field is used, as this interacts favourably
...
In certain cases one might want to simulate with 2D periodicity, for example
when interfaces are involved. Ewald summation and PME are 3D periodic in nature,
so strictly speaking not suitable for 2D periodicity. However, |Gromacs| supports
a simple correction which removes the net dipole-dipole interaction along the
