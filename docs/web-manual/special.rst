...
(AWH) Lindahl, Lidmar, and Hess (2014) calculates the PMF along a
...
adaptive biasing potential methods, e.g. the Wang-Landau Wang and Landau
(2001), local elevation Huber, Torda, and Gunsteren (1994) and
metadynamics Laio and Parrinello (2002) methods. The initial sampling
...
interval is the Cartesian product :math:`I=\Pi_d I_d` (a rectangular
...
which is consistent with :eq:`Eq. (%s) <eqawhplambda>`. Note
that also the target distribution may be updated during the simulation
(see examples in section ).
Substituting this choice of :math:`g=g_n` back into
:eq:`Eq. (%s) <eqawhplambda>` yields the simple free energy
update
...
scheme :eq:`(%s) <eqawhdfnaive>` yields very noisy results. AWH
instead applies a free energy update that has the same form but which
can be applied repeatedly with limited and localized sampling,
...
disregarding the initial stage (see
section ), is
...
:math:`\Delta F_n(\lambda) < 0`, which by
:eq:`Eq. (%s) <eqawhgrhofrelation>` implies
:math:`\Delta g_n(\lambda) < 0` (assuming
:math:`\Delta \rho_n \equiv 0`). Thus, the probability to sample
:math:`\lambda` decreases after the update (see
:eq:`Eq. (%s) <eqawhplambda>`). Secondly, the normalization of
the histogram :math:`N_n=\sum_\lambda W_n(\lambda)`, determines the
update size :math:`| \Delta F(\lambda) |`. For instance, for a single
sample :math:`\omega(\lambda|x)`, the shape of the update is
approximately a Gaussian function of width
:math:`\sigma=1/\sqrt{\beta k}` and height
...
also section) and will converge at
the same rate as :math:`F(\lambda)`, see
:numref:`Fig. (%s) <fig-awhbiasevolution1>`. The PMF will be written
to output (see section ).
...
:eq:`Eq. (%s) <eqawhomega>`. This is also called Gibbs sampling
or independence sampling. Alternatively, and by default in the code, the
...
:math:`\lambda=(\lambda_d)`, the sampling interval :math:`I` is
considered covered when all dimensions have been covered. A dimension
:math:`d` is covered if all points :math:`\lambda_d` in the
one-dimensional sampling interval :math:`I_d` have been “visited”.
Finally, a point :math:`\lambda_d \in I_d` has been visited if there is
at least one point :math:`\lambda^*\in I` with
:math:`\lambda^*_d = \lambda_d` that since the last covering has
...
Here, :math:`\Delta \lambda_d` is the point spacing of the discretized
:math:`I_d` and :math:`\sigma_k=1/\sqrt{\beta k_d}` (where :math:`k_d`
...
stage (section ). The rescaling
of the weight histogram applied in the initial stage is a global
operation, which is incompatible :math:`\rho_{\mathrm{Boltz,loc}}` only
depending locally on the sampling history.
...
(section ) may take an
infeasible mount of simulation time. In these cases it could be
advantageous to parallelize the work and have a group of multiple
“walkers” :math:`\xi^{(i)}(t)` share a single bias potential. This can
be achieved by collecting samples from all :math:`\xi^{(i)}` of the same
...
before a simulation marks a point :math:`\lambda_d` along dimension
:math:`d` as visited, and shares this with the other walkers, also all
...
AWH stores data in the energy file (:ref:`edr`) with a
frequency set by the user. The data – the PMF, the convolved bias,
distributions of the :math:`\lambda` and :math:`\xi` coordinates, etc. –
can be extracted after the simulation using the :ref:`gmx awh`
tool. Furthermore, the trajectory of the reaction coordinate
:math:`\xi(t)` is printed to the pull output file
:math:`{\tt pullx.xvg}`. The log file (:ref:`log`) also
contains information; check for messages starting with “awh”, they will
tell you about covering and potential sampling issues.
...
reweighting the trajectory as described in
section ), one would generally want
to observe at least a few transitions after having exited the initial
stage. Furthermore, if the dynamics of the reaction coordinate suddenly
changes, this may be a sign of e.g. a reaction coordinate problem.

Difficult regions of sampling may also be detected by calculating the
local friction tensor :math:`\eta(\xi)` (inversely proportional to the
diffusion) along the reaction coordinate. AWH calculates
:math:`\eta`  Sivak and Crooks (2012) during simulation and writes it to
the energy file. A high peak in the friction metric
(:math:`\sqrt{\det\eta}`) is an indicator of a region that takes
relatively long to sample properly.
