...
\begin{equation}
V(\ve{r}_1,\ldots \ve{r}_N) = \sum_{i<j}V_{ij}(\ve{r}_ij);
\end{equation}
\begin{equation}
\ve{F}_i = -\sum_j \frac{dV_{ij}(r_{ij})}{dr_{ij}} \frac{\ve{r}_ij}{r_{ij}} = -\ve{F}_j
\end{equation}
...
\begin{equation}
\ve{F}_i(\ve{r}_ij) = \left( 12~\frac{C_{ij}^{(12)}}{\rij^{13}} -
                                 6~\frac{C_{ij}^{(6)}}{\rij^7} \right) \rnorm 
\end{equation}

The LJ potential may also be written in the following form:
\begin{equation}
V_{LJ}(\ve{r}_ij) = 4\epsilon_{ij}\left(\left(\frac{\sigma_{ij}} {\rij}\right)^{12}
...
The force derived from this potential is:
\begin{equation}
\ve{F}_i(\ve{r}_ij) = f \frac{q_i q_j}{\epsr\rij^2}\rnorm
\end{equation}
...
\begin{equation}
\ve{F}_i(\ve{r}_ij) = f \frac{q_i q_j}{\epsr}\left[\frac{1}{\rij^2} - 2 k_{rf}\rij\right]\rnorm
\label{eqn:fcrf}
\end{equation}
...
\begin{equation}
\ve{F}_i = c \, F(r_{ij}) \frac{\ve{r}_ij}{r_{ij}}
\end{equation}
...
\begin{equation}
\ve{F}_i(\ve{r}_ij) = k^b_{ij}(\rij-b_{ij}) \rnorm
\end{equation}
...
\begin{equation}
\ve{F}_i(\ve{r}_ij) = k^b_{ij}(\rij^2-b_{ij}^2)~\ve{r}_{ij}
\end{equation}
...
\begin{equation}
\ve{F}_i(\ve{r}_ij) = 2k^b_{ij}(\rij-b_{ij})~\rnorm + 3k^b_{ij}k^{cub}_{ij}(\rij-b_{ij})^2~\rnorm
\end{equation}
...
\begin{equation}
F_{\mbox{\small FENE}}(\ve{r}_ij) =
  -k^b_{ij} \left(1 - \frac{\rij^2}{b^2_{ij}}\right)^{-1} \ve{r}_ij
\end{equation}
...
\begin{equation}
\begin{array}{l}
\ve{F}_i    ~=~ -\displaystyle\frac{d V_a(\tijk)}{d \ve{r}_i}   \\
\ve{F}_k    ~=~ -\displaystyle\frac{d V_a(\tijk)}{d \ve{r}_k}   \\
\ve{F}_j    ~=~ -\ve{F}_i-\ve{F}_k
\end{array}
~ \mbox{ ~ where ~ } ~
 \tijk = \arccos \frac{(\ve{r}_ij \cdot \ve{r}_{kj})}{r_{ij}r_{kj}}
\end{equation}
...
where 
\begin{equation}
\cos(\tijk) = \frac{\ve{r}_ij\cdot\ve{r}_{kj}}{\rij r_{kj}}
\end{equation}
...
\begin{equation}
V_{pr}(\ve{r}_i) = \halfje{1}k_{pr}|\ve{r}_i-\ve{R}_i|^2
\end{equation}
...
\ve{F}_i~=~ \left\{
\begin{array}{lcllllll}
-k_{dr}(r_{ij}-r_0)\frac{\ve{r}_ij}{r_{ij}} 
                &\mbox{for}&     &     & r_{ij} & < & r_0       \\[1.5ex]
0               &\mbox{for}& r_0 & \le & r_{ij} & < & r_1       \\[1.5ex]
-k_{dr}(r_{ij}-r_1)\frac{\ve{r}_ij}{r_{ij}} 
                &\mbox{for}& r_1 & \le & r_{ij} & < & r_2       \\[1.5ex]
-k_{dr}(r_2-r_1)\frac{\ve{r}_ij}{r_{ij}}    
                &\mbox{for}& r_2 & \le & r_{ij} &   &
\end{array} \right.
\end{equation}
...
\ve{F}_i~=~ \left\{
\begin{array}{lcllllll}
-k^a_{dr}(\bar{r}_{ij}-r_0)\frac{\ve{r}_ij}{r_{ij}}   
                &\mbox{for}&     &     & \bar{r}_{ij} & < & r_0 \\[1.5ex]
0               &\mbox{for}& r_0 & \le & \bar{r}_{ij} & < & r_1 \\[1.5ex]
-k^a_{dr}(\bar{r}_{ij}-r_1)\frac{\ve{r}_ij}{r_{ij}}   
                &\mbox{for}& r_1 & \le & \bar{r}_{ij} & < & r_2 \\[1.5ex]
-k^a_{dr}(r_2-r_1)\frac{\ve{r}_ij}{r_{ij}}    
...
\ve{F}_i~=~ \left\{
\begin{array}{lclll}
k^a_{dr}\sqrt{(r_{ij}-r_0)(\bar{r}_{ij}-r_0)}\frac{\ve{r}_ij}{r_{ij}}   
...
~& 0 \hspace{4cm}  & r_{N} < r_1         \\
 & k_{dr}(r_{N}-r_1)\frac{\ve{r}_ij}{r_{ij}} & r_1 \le r_{N} < r_2 \\
 & k_{dr}(r_2-r_1)\frac{\ve{r}_ij}{r_{ij}}    & r_{N} \ge r_2 
\end{array} \right.
\end{equation}
...
\newcommand{\ve{r}_s}{\ve{r}_s}
\newcommand{\ve{F}_i}{\ve{F}_i'}
\newcommand{\ve{F}_j}{\ve{F}_j'}
\newcommand{\ve{F}_k}{\ve{F}_k'}
\newcommand{\ve{F}_l}{\ve{F}_l'}
\newcommand{\ve{F}_{s}}{\ve{F}_{s}}
\newcommand{\ve{r}_{ik}}{\ve{r}_{ik}}
\newcommand{\ve{r}_{is}}{\ve{r}_{is}}
\newcommand{\ve{r}_{jk}}{\ve{r}_{jk}}
\newcommand{\ve{r}_{jl}}{\ve{r}_{jl}}
...
\begin{equation}
V = V(\ve{r}_s,\ve{r}_1,\ldots,\ve{r}_n) = V^*(\ve{r}_1,\ldots,\ve{r}_n)
\end{equation}
...
\label{eqn:fvsite}
\end{equation}
where $\ve{F}_{s}$ is the force on the virtual site and $x_s$, $y_s$ and
...
\end{equation}
        ...
        $\alpha$ with $\ve{r}_ij$. Atom $k$ defines the plane and the
...
\begin{equation}
\Xi~=~-\half \ve{r}_ij \cdot \ve{F}_ij ~=~ 3\,C_6\,\rij^{-6}
\end{equation}
