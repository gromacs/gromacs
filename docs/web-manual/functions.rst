...
.. figure:: plots/f-bham.eps

   The Buckingham interaction.

See also Fig. 
. The force derived from this
is:
...
Fig.
the modified interaction is plotted,
...
is divided by a :math:`\sin^2\theta`
factor:
...
| where :math:`k` is a force constant in units of energy and :math:`f`
  ...
  Table . The table files are supplied to
  ...
  (Table ). The format for the table
  ...
  in Table . The setup of the tables is
  as follows:
...
beyond the largest bound (see Fig. 
).
...
where we use :math:`r_{ij}` or eqn. 
for the
...
the molecule, as expressed by equation (
). The
...
(eqn. 
) is non-trivial when virtual sites are
...
Fig. 
depicts 6 of the available virtual
...
-  In the plane of three atoms,
   with a fixed angle and distance (Fig. 3fad):
...
-  As a non-linear combination of
   three atoms, out of plane (Fig. 3out):
...
eqn. 
, assuming that the density in the
...
can be combined geometrically (eqn. 
) in the
