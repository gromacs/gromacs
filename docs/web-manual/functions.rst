Interaction function and force fields:raw-latex:`\index{force field}`
=====================================================================

To accommodate the potential functions used in some popular force fields
(see :raw-latex:`\ref{sec:ff}`), GROMACS offers a choice of functions,
...
#. *Applied Forces*: externally applied forces, see
   chapter :raw-latex:`\ref{ch:special}`.
...
repulsion:raw-latex:`\index{repulsion}` term, a
dispersion:raw-latex:`\index{dispersion}` term, and a Coulomb term. The
...
The Lennard-Jones:raw-latex:`\index{Lennard-Jones}` potential
...
.. math::
 ...
 See also Fig. :raw-latex:`\ref{fig:lj}` The parameters
...
two types of combination rule:raw-latex:`\index{combination rule}`s can
...
Buckingham potential:raw-latex:`\index{Buckingham potential}`
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
See also Fig. :raw-latex:`\ref{fig:bham}`. The force derived from this
is:
...
The Coulomb:raw-latex:`\index{Coulomb}` interaction between two charge
...
.. math::
 ...
 See also Fig. :raw-latex:`\ref{fig:coul}`, where
:math:`f = \frac{1}{4\pi \varepsilon_0} =
{138.935\,458}` (see chapter :raw-latex:`\ref{ch:defunits}`)
...
constant:raw-latex:`\index{dielectric constant}`:raw-latex:`\index{constant, dielectric|see{dielectric constant}}`
:math:`{\varepsilon_r}`\ :raw-latex:`\index{${\varepsilon_r}$}` may be
set in the in the input for :raw-latex:`\tt`grompp.

Coulomb interaction with reaction field:raw-latex:`\index{reaction field}`
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
eqn. :raw-latex:`\ref{eqn:vcrf}` for simplicity as
...
Fig. :raw-latex:`\ref{fig:coul}` the modified interaction is plotted,
...
in eqns. :raw-latex:`\ref{eqn:vcrf}` and :raw-latex:`\ref{eqn:fcrf}` is
absent.
...
eqns. :raw-latex:`\ref{eqn:kgrf}` and :raw-latex:`\ref{eqn:cgrf}` reduce
to the simple forms of eqns. :raw-latex:`\ref{eqn:krf}` and
:raw-latex:`\ref{eqn:crf}` respectively.
...
derivatives at the cut-off:raw-latex:`\index{cut-off}` radius. With such
...
When Ewald summation:raw-latex:`\index{Ewald sum}` or particle-mesh
Ewald:raw-latex:`\index{particle-mesh
Ewald|see{PME}}`:raw-latex:`\index{Ewald, particle-mesh}` is used to
...
sec. :raw-latex:`\ref{sec:lr_elstat}`.
...
The bond
stretching:raw-latex:`\index{bond stretching}`:raw-latex:`\index{stretching, bond|see{bond stretching}}`
between two covalently bonded atoms :math:`i` and :math:`j` is
represented by a harmonic potential:

:raw-latex:`\raisebox{2cm}{\includegraphics[width=5cm]{plots/bstretch}}`\ |Principle
of bond stretching (left), and the bond stretching potential (right).|

.. math:: V_b~({r_{ij}}) = {\frac{1}{2}}k^b_{ij}({r_{ij}}-b_{ij})^2

See also Fig. :raw-latex:`\ref{fig:bstretch1}`, with the force given by:
...
(sec. :raw-latex:`\ref{sec:bondpot}`) as
...
Morse potential:raw-latex:`\index{Morse potential}` bond stretching
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
.. math:: \displaystyle V_{morse} (r_{ij}) = D_{ij} [1 - \exp(-\beta_{ij}(r_{ij}-b_{ij}))]^2,
 ...
 See also Fig. :raw-latex:`\ref{fig:morse}`, and the corresponding force
is:
...
.. math::
 ...
 and substituting eqn. :raw-latex:`\ref{eqn:betaij}` and
eqn. :raw-latex:`\ref{eqn:expminx}` in the functional form:
...
.. math:: V_b~({r_{ij}}) = k^b_{ij}({r_{ij}}-b_{ij})^2 + k^b_{ij}k^{cub}_{ij}({r_{ij}}-b_{ij})^3
 ...
 A flexible water:raw-latex:`\index{water}` model (based on the SPC
...
(:raw-latex:`\tt`flexwat-ferguson.itp). It should be noted that the
potential is asymmetric: overstretching leads to infinitely low
energies. The integration
timestep:raw-latex:`\index{integration timestep}`:raw-latex:`\index{timestep, integration|see{integration timestep}}`
...
FENE bond stretching potential:raw-latex:`\index{FENE potential}`
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
The bond-angle
vibration:raw-latex:`\index{angle vibration}`:raw-latex:`\index{vibration, angle|see{angle vibration}}`
between a triplet of atoms :math:`i` - :math:`j` - :math:`k` is also
represented by a harmonic potential on the angle :math:`{\theta_{ijk}}`

:raw-latex:`\raisebox{1cm}{\includegraphics[width=5cm]{plots/angle}}`\ |Principle
...
(Fig. :raw-latex:`\ref{fig:bstretch1}`).
...
the ends (see Fig. :raw-latex:`\ref{fig:angle}`). **Note** that in the
...
(:raw-latex:`\ref{subsec:harmonicangle}`) by:
...
:raw-latex:`\ref{eq:G96angle}` is divided by a :math:`\sin^2\theta`
factor:
...
.. math::
 ...
 Figure  Fig. :raw-latex:`\ref{fig:ReB}` shows the comparison between
the ReB potential, :raw-latex:`\ref{eq:ReB}`, and the standard one
:raw-latex:`\ref{eq:G96angle}`.
...
Fig. :raw-latex:`\ref{fig:ReB}`).

Urey-Bradley potential
~~~~~~~~~~~~~~~~~~~~~~

The Urey-Bradley bond-angle
vibration:raw-latex:`\index{Urey-Bradley bond-angle vibration}`:raw-latex:`\index{vibration, Urey-Bradley bond-angle|see{Urey-Bradley bond-angle vibration}}`
...
sections :raw-latex:`\ref{subsec:harmonicbond}`
and :raw-latex:`\ref{subsec:harmonicangle}`.
...
Equation :raw-latex:`\ref{crossbb}`. The force associated with the
...
Improper dihedrals:raw-latex:`\index{improper dihedral}`:raw-latex:`\index{dihedral, improper|see{improper dihedral}}`
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Improper dihedrals are meant to keep planar
group:raw-latex:`\index{planar group}`:raw-latex:`\index{group, planar|see{planar group}}`s
(*e.g.* aromatic rings) planar, or to prevent molecules from flipping
over to their mirror image:raw-latex:`\index{mirror image}`s, see
Fig. :raw-latex:`\ref{fig:imp}`.
...
plotted in Fig. :raw-latex:`\ref{fig:imps}`.
...
Proper dihedrals:raw-latex:`\index{proper dihedral}`:raw-latex:`\index{dihedral, proper|see{proper dihedral}}`
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

For the normal dihedral:raw-latex:`\index{dihedral}` interaction there
...
interaction:raw-latex:`\index{1-4 interaction}`s must be excluded from
...
:raw-latex:`\tt`pdb2gmx:raw-latex:`\index{pdb2gmx}` in this case.
...
:raw-latex:`\tt[ dihedral ]` section when multiple parameters are
defined for the same atomtypes in the :raw-latex:`\tt[ dihedraltypes ]`
section.

:raw-latex:`\raisebox{1cm}{\includegraphics[width=5cm]{plots/dih}}`\ |Principle
...
| For alkanes, the following proper dihedral potential is often used
  (see Fig. :raw-latex:`\ref{fig:rbdih}`):
...
| The RB dihedral function can also be used to include Fourier dihedrals
  ...
  cos\ :math:`(\phi)`).
| :raw-latex:`\noindent{\bf Note:}` Mind the conversion from **kcal
...
| :raw-latex:`\noindent{\bf Note:}` Mind the conversion from *k*\ cal
...
:raw-latex:`\ref{subsec:ReB}`), a restricted torsion/dihedral potential
is introduced:
...
described in :raw-latex:`\ref{subsec:ReB}`, at least :math:`10^{\circ}`
...
:raw-latex:`\ref{subsec:ReB}`) that prevents the dihedral from reaching
...
Fig. :raw-latex:`\ref{fig:CBT}` illustrates the difference between a
...
.. figure:: plots/fig-04
   ...
   (:raw-latex:`\ref{eq:CBT}` with :math:`k = 10` kJ mol\ :math:`^{-1}`,
...
Tabulated bonded interaction functions:raw-latex:`\index{tabulated bonded interaction function}`
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
| For full flexibility, any functional shape can be used for bonds,
  ...
  :raw-latex:`\ref{subsec:cubicspline}`. For each interaction, the force
  ...
  Table :raw-latex:`\ref{tab:topfile2}`. The table files are supplied to
  the :raw-latex:`\tt`mdrun program. After the table file name an
  ...
  :raw-latex:`\tt`table\_b0.xvg. Multiple tables can be supplied simply
  by adding files with different values of :math:`n`, and are applied to
  the appropriate bonds, as specified in the topology
  (Table :raw-latex:`\ref{tab:topfile2}`). The format for the table
  ...
  in Table :raw-latex:`\ref{tab:topfile2}`. The setup of the tables is
  as follows:
| **bonds**: :math:`x` is the distance in nm. For distances beyond the
  table length, :raw-latex:`\tt`mdrun will quit with an error message.
...
Position restraints:raw-latex:`\index{position restraint}`:raw-latex:`\index{restraint, position|see{position restraint}}`
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
Fig. :raw-latex:`\ref{fig:positionrestraint}`.
...
generated by the :raw-latex:`\tt`pdb2gmx:raw-latex:`\index{pdb2gmx}`
program.

Flat-bottomed position restraint:raw-latex:`\index{Flat-bottomed position restraint}`:raw-latex:`\index{position restraint, Flat-bottomed|see{Flat-bottomed position restraint}}`s
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
potential is used (Figure :raw-latex:`\ref{fig:fbposres}`A):
...
| The following geometries for the flat-bottomed potential are
  supported::raw-latex:`\newline{\bfseries Sphere}` (:math:`g =1`): The
  ...
  code so that old :raw-latex:`\tt`.tpr files and topologies work. The
...
is used (Figure :raw-latex:`\ref{fig:fbposres}`B):
...
Angle restraints:raw-latex:`\index{angle restraint}`:raw-latex:`\index{restraint, angle|see{angle restraint}}`
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
Dihedral restraints:raw-latex:`\index{dihedral restraint}`:raw-latex:`\index{restraint, dihedral|see{dihedral restraint}}`
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

These are used to restrain the dihedral angle :math:`\phi` defined by
four particles as in an improper dihedral
(sec. :raw-latex:`\ref{sec:imp}`) but with a slightly modified
potential. Using:
...
Distance restraints:raw-latex:`\index{distance restraint}`:raw-latex:`\index{restraint, distance|see{distance restraint}}`
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
data:raw-latex:`\index{nmr
refinement}`:raw-latex:`\index{refinement,nmr}`. In GROMACS there are
three ways to impose restraints on pairs of atoms:

-  Simple harmonic restraints: use :raw-latex:`\tt[ bonds ]` type 6

   (see sec. :raw-latex:`\ref{sec:excl}`).

-  :raw-latex:`\label{subsec:harmonicrestraint}`Piecewise
   linear/harmonic restraints: :raw-latex:`\tt[ bonds ]` type 10.
...
beyond the largest bound (see Fig. :raw-latex:`\ref{fig:dist}`).
...
usually suffice and a section of :raw-latex:`\tt[ bonds ]` type 10 can
be used to apply individual restraints between pairs of

atoms, see :raw-latex:`\ref{subsec:topfile}`.
...
:raw-latex:`\tt[~distance_restraints~]` section and is described below.

Time averaging:raw-latex:`\index{time-averaged distance restraint}`:raw-latex:`\index{distance restraint, time-averaged|see{time-averaged distance restraint}}`
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
:raw-latex:`\tt`mdrun program is done in the following fashion:
...
Averaging over multiple pairs:raw-latex:`\index{ensemble-averaged distance restraint}`:raw-latex:`\index{distance restraint, ensemble-averaged|see{ensemble-averaged distance restraint}}`
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
.. math::
 ...
 where we use :math:`r_{ij}` or eqn. :raw-latex:`\ref{eqn:rav}` for the
...
:raw-latex:`\tt`grompp can do this automatically when the appropriate
...
:raw-latex:`\tt`ai and :raw-latex:`\tt`aj you find the atom numbers of
the particles to be restrained. The :raw-latex:`\tt`type column should
always be 1. As explained in
 :raw-latex:`\ref{subsec:distancerestraint}`, multiple distances can
contribute to a single NOE signal. In the topology this can be set using
the :raw-latex:`\tt`index column. In our example, the restraints 10-28
...
restraint. The :raw-latex:`\tt`type’ column will usually be 1, but can
...
The columns :raw-latex:`\tt`low, :raw-latex:`\tt`up1, and
:raw-latex:`\tt`up2 hold the values of :math:`r_0`, :math:`r_1`, and
:math:`r_2` from  eqn. :raw-latex:`\ref{eqn:disre}`. In some cases it
can be useful to have different force constants for some restraints;
this is controlled by the column :raw-latex:`\tt`fac. The force constant
in the parameter file is multiplied by the value in the column
:raw-latex:`\tt`fac for each restraint. Information for each restraint
is stored in the energy file and can be processed and plotted with
:raw-latex:`\tt`gmx nmr.

Orientation restraints:raw-latex:`\index{orientation restraint}`:raw-latex:`\index{restraint, orientation|see{orientation restraint}}`
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
couplings:raw-latex:`\index{dipolar couplings}` (between two nuclei) or
...
the molecule, as expressed by equation (:raw-latex:`\ref{D_rot}`). The
...
expression (:raw-latex:`\ref{S_msd}`) with :math:`\delta_i^c(t)`
...
topology file in the section :raw-latex:`\tt[~orientation_restraints~]`.
...
columns :raw-latex:`\tt`ai and :raw-latex:`\tt`aj you find the atom
numbers of the particles to be restrained. The :raw-latex:`\tt`type
column should always be 1. The :raw-latex:`\tt`exp. column denotes the
...
number larger than zero for each restraint. The :raw-latex:`\tt`alpha
column contains the power :math:`\alpha` that is used in
equation (:raw-latex:`\ref{orient_def}`) to calculate the orientation.
The :raw-latex:`\tt`const. column contains the constant :math:`c_i` used
in the same equation. The constant should have the unit of the
observable times nm\ :math:`^\alpha`. The column :raw-latex:`\tt`obs.
...
:raw-latex:`\tt`grompp.mdp file, for a study of the effect of different
...
file and can be processed and plotted with :raw-latex:`\tt`gmx nmr.

Polarization
------------

Polarization can be treated by GROMACS by attaching
shell:raw-latex:`\index{shell}` (Drude:raw-latex:`\index{Drude}`)
...
Based on early work by Thole:raw-latex:`\index{Thole}` Thole (1981),
...
The Coulomb:raw-latex:`\index{Coulomb}` interaction between two
...
chapter :raw-latex:`\ref{ch:defunits}`).
...
medium (see sec. :raw-latex:`\ref{sec:coulrf}`).

Lennard-Jones interaction
^^^^^^^^^^^^^^^^^^^^^^^^^

For the Lennard-Jones:raw-latex:`\index{Lennard-Jones}` interaction
...
eqn. :raw-latex:`\ref{eqn:sigeps}`). It may seem to make sense
...
momentum :raw-latex:`\mbox{\boldmath ${p}$}` as
m:raw-latex:`\mbox{\boldmath ${v}$}`, since that would result in the
...
:raw-latex:`\mbox{\boldmath ${p}$}` =
m:raw-latex:`\mbox{\boldmath ${v}$}`, such that:
...
calculated using the LINCS:raw-latex:`\index{LINCS}` or the
SHAKE:raw-latex:`\index{SHAKE}` algorithm. If we have
...
Soft-core interactions:raw-latex:`\index{soft-core interactions}`
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
Equations :raw-latex:`\ref{eq:dVljdlambda}` and
:raw-latex:`\ref{eq:dVcoulombdlambda}` may lead to poor convergence.
...
soft-core parameter (set with :raw-latex:`\tt`sc\_alpha in the
:raw-latex:`\tt`.mdp file), :math:`p` is the soft-core :math:`{\lambda}`
power (set with :raw-latex:`\tt`sc\_power), :math:`\sigma` is the radius
of the interaction, which is :math:`(C_{12}/C_6)^{1/6}` or an input
parameter (:raw-latex:`\tt`sc\_sigma) when :math:`C_6` or :math:`C_{12}`
is zero.
...
smaller :math:`r` (Fig. :raw-latex:`\ref{fig:softcore}`). The force is:
...
:raw-latex:`\tt`sc-sigma in the :raw-latex:`\tt`.mdp file. These
...
with :math:`p=2`. Lowering :raw-latex:`\tt`:raw-latex:`\mbox{sc-sigma}`
...
:raw-latex:`\tt`sc-alpha >0), and the Coulomb and Lennard-Jones
...
:raw-latex:`\tt`sc-coul to :raw-latex:`\tt`yes. Note that the
:raw-latex:`\tt`sc-coul is only taken into account when lambda states
are used, not with :raw-latex:`\tt`couple-lambda0 /
:raw-latex:`\tt`couple-lambda1, and you can still turn off soft-core
interactions by setting :raw-latex:`\tt`sc-alpha=0. Additionally, the
...
introduced or deleted, the sigma is set to :raw-latex:`\tt`sc-sigma-min,
which itself defaults to :raw-latex:`\tt`sc-sigma-default.
...
*first neighbors, second neighbors* and *third
neighbor:raw-latex:`\index{third neighbor}`:raw-latex:`\index{neighbor, third|see{third neighbor}}`s*,
respectively (see Fig. :raw-latex:`\ref{fig:chain}`). Since the
...
interaction
list:raw-latex:`\index{interaction list}`:raw-latex:`\index{list, interaction|see{interaction list}}`
of atom **i**; atoms **i+1** and **i+2** are called
*exclusions:raw-latex:`\index{exclusions}`* of atom **i**.
...
Charge Groups:raw-latex:`\index{charge group}`
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

In principle, the force calculation in MD is an :math:`O(N^2)` problem.
Therefore, we apply a cut-off:raw-latex:`\index{cut-off}` for non-bonded
...
(e.g. particle-mesh Ewald (sec. :raw-latex:`\ref{sec:pme}`), keeping
charge groups together is not required.

Treatment of Cut-offs in the group scheme:raw-latex:`\index{cut-off}`
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
input file for :raw-latex:`\tt`grompp. There are two sort of parameters
...
type selectors (termed :raw-latex:`\tt`vdwtype and
:raw-latex:`\tt`coulombtype) and two parameters, for a total of six
...
Table :raw-latex:`\ref{tab:funcparm}` require that neighbor searching be
...
Virtual interaction sites:raw-latex:`\index{virtual interaction sites}`
-----------------------------------------------------------------------

Virtual interaction sites (called dummy
atoms:raw-latex:`\index{dummy atoms|see{virtual interaction sites}}` in
GROMACS versions before 3.3) can be used in GROMACS in a number of ways.
We write the position of the virtual site :math:`\ve{r}_s` as a function
of the positions of other particles
:raw-latex:`\mbox{\boldmath ${r}$}`\ :math:`_i`: :math:`\ve{r}_s =
...
The computation of the virial:raw-latex:`\index{virial}`
(eqn. :raw-latex:`\ref{eqn:Xi}`) is non-trivial when virtual sites are
...
sites to the atoms (using  eqn. :raw-latex:`\ref{eqn:fvsite}`) *before*
...
.. figure:: plots/dummies
   :alt: The six different types of virtual site construction in
   :raw-latex:`\protect{{GROMACS}}`. The constructing atoms are shown as
...
Fig. :raw-latex:`\ref{fig:vsites}` depicts 6 of the available virtual
...
-  :raw-latex:`\label{subsec:vsite2}`As a linear combination of two
   atoms (Fig. :raw-latex:`\ref{fig:vsites}` 2):
...
-  :raw-latex:`\label{subsec:vsite3}`As a linear combination of three
   atoms (Fig. :raw-latex:`\ref{fig:vsites}` 3):
...
-  :raw-latex:`\label{subsec:vsite3fd}`In the plane of three atoms, with
   a fixed distance (Fig. :raw-latex:`\ref{fig:vsites}` 3fd):
...
-  :raw-latex:`\label{subsec:vsite3fad}`In the plane of three atoms,
   with a fixed angle and distance (Fig. :raw-latex:`\ref{fig:vsites}`
   3fad):
...
-  :raw-latex:`\label{subsec:vsite3out}`As a non-linear combination of
   three atoms, out of plane (Fig. :raw-latex:`\ref{fig:vsites}` 3out):
...
-  :raw-latex:`\label{subsec:vsite4fdn}`From four atoms, with a fixed
   distance, see separate Fig. :raw-latex:`\ref{fig:vsite-4fdn}`. This
...
-  :raw-latex:`\label{subsec:vsiteN}` A linear combination of :math:`N`
...
Ewald summation:raw-latex:`\index{Ewald sum}`
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

The total electrostatic energy of :math:`N` particles and their periodic
images:raw-latex:`\index{periodic boundary conditions}` is given by
...
eqn. :raw-latex:`\ref{eqn:totalcoulomb}` into two quickly-converging
...
:raw-latex:`\tt`.mdp file, if the side of your box is about
:math:`3` nm:
...
The ratio of the box dimensions and the :raw-latex:`\tt`fourierspacing
...
in each direction. The :raw-latex:`\tt`ewald-rtol parameter is the
...
PME see the section on MPMD PME (:raw-latex:`\ref{subsec:mpmd_pme}`).
...
specify the following lines in your :raw-latex:`\tt`.mdp file:
...
In this case the :raw-latex:`\tt`fourierspacing parameter determines the
maximum spacing for the FFT grid (i.e. minimum number of grid points),
and :raw-latex:`\tt`pme-order controls the interpolation order. Using
...
P3M-AD:raw-latex:`\index{P3M-AD}`
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

The Particle-Particle
Particle-Mesh:raw-latex:`\index{Particle-Particle Particle-Mesh|see{P3M}}`
...
:raw-latex:`\tt`gmx mdrun will automatically do a lot of PME
optimization, and GROMACS also includes a special tool,
:raw-latex:`\tt`gmx tune\_pme, which automates the process of selecting
the optimal number of PME-only ranks.

Long Range Van der Waals interactions
-------------------------------------

Dispersion correction:raw-latex:`\index{dispersion correction}`
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
eqn. :raw-latex:`\ref{eqn:enercorr}`, assuming that the density in the
...
.. math::
 ...
 For inhomogeneous systems, eqn. :raw-latex:`\ref{eqn:pcorr}` can be
applied under the same restriction as holds for the energy (see
sec. :raw-latex:`\ref{sec:ecorr}`).

Lennard-Jones PME:raw-latex:`\index{LJ-PME}`
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
can be combined geometrically (eqn. :raw-latex:`\ref{eqn:comb}`) in the
...
(eqn. :raw-latex:`\ref{eqn:lorentzberthelot}`), the reciprocal part of
...
eqn. :raw-latex:`\ref{eqn:ljpmerealspace}` when
...
.. math:: \left(-C^{\mathrm{dir}}_6 + C^\mathrm{recip}_6 [1 - g(\beta r_c)]\right) r_c^{-6}
 ...
 is added to eqn. :raw-latex:`\ref{eqn:ljpmecorr2}` in order to ensure
that the potential is continuous at the cutoff. Note that, in the same
way as eqn. :raw-latex:`\ref{eqn:ljpmedirectspace}`, this degenerates
...
:raw-latex:`\tt`fourierspacing and :raw-latex:`\tt`pme-order are common
to both. :raw-latex:`\tt`ewald-rtol-lj controls the splitting between
direct and reciprocal space in the same way as
:raw-latex:`\tt`ewald-rtol. In addition to this, the combination rule to
be used in reciprocal space is determined by
:raw-latex:`\tt`lj-pme-comb-rule. If the current force field uses
Lorentz-Berthelot combination rules, it is possible to set
:raw-latex:`\tt`lj-pme-comb-rule = geometric in order to gain a
...
that as with Coulomb PME, :raw-latex:`\tt`rvdw is now a free parameter
...
upon the automatic load-balancing used by :raw-latex:`\tt`mdrun.

Force field:raw-latex:`\index{force field}`
-------------------------------------------

A force field is built up from two distinct components:

-  The set of equations (called the *potential
   functions*\ :raw-latex:`\index{potential function}`) used to generate
...
GROMOS-96:raw-latex:`\index{GROMOS96 force field}`
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
-  a fourth power bond stretching potential
   (:raw-latex:`\ref{subsec:G96bond}`)

-  an angle potential based on the cosine of the angle
   (:raw-latex:`\ref{subsec:G96angle}`)
...
(:raw-latex:`\tt`rvdw) of at least 1.4. A larger cut-off is possible
because the Lennard-Jones potential and forces are almost zero beyond
1.4 nm.

GROMOS-96 files:raw-latex:`\index{GROMOS96 files}`:raw-latex:`\index{files, GROMOS96|see{GROMOS96 files}}`
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

GROMACS can read and write GROMOS-96 coordinate and trajectory files.
These files should have the extension :raw-latex:`\tt`.g96. Such a file
...
OPLS/AA:raw-latex:`\index{OPLS/AA force field}`
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

AMBER:raw-latex:`\index{AMBER force field}`
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
CHARMM:raw-latex:`\index{CHARMM force field}`
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
:raw-latex:`\tt`pdb2gmx:raw-latex:`\index{pdb2gmx}` the default option
is to use CMAP:raw-latex:`\index{CMAP}` (for torsional correction map).
To exclude CMAP, use :raw-latex:`\tt`-nocmap. The basic form of the CMAP
...
:raw-latex:`\tt`.rtp file by a :raw-latex:`\tt[ cmap ]` statement at the
...
matched to the correct CMAP type in the :raw-latex:`\tt`cmap.itp file
...
:raw-latex:`\tt`pdb2gmx:raw-latex:`\index{pdb2gmx}`, it is possible to
...
:raw-latex:`\index{force-field, coarse-grained}`
:raw-latex:`\label{sec:cg-forcefields}` Coarse-graining is a systematic
...
Applications
(VOTCA):raw-latex:`\index{Versatile Object-oriented Toolkit for Coarse-Graining Applications (VOTCA)}` (**???**).
...
MARTINI:raw-latex:`\index{Martini force field}`
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
PLUM:raw-latex:`\index{PLUM force field}`
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

The PLUM force field:raw-latex:`\index{PLUM force field}` (**???**) is
