...
processor architectures this can lead to performance loss
(e.g. on Intel Skylake-X/SP or AMD Zen).
...
4. ``AVX_128_FMA`` AMD Bulldozer, Pilerdriver (and later Family 15h) processors have this.
...
8. ``AVX_512`` Skylake-X desktop and Skylake-SP Xeon processors (2017);
   ...
   (e.g. Xeon Bronze and Silver) come with only one AVX512 FMA units
...
but support a special SIMD instruction set. If cross-compiling for such nodes
use the ``AVX_512_KNL`` SIMD flavor.
Knights Landing processors support so-called "clustering modes" which
allow re configuring the memory subsystem for lower latency. |Gromacs| can
