...
should also add ``--enable-avx2`` also. FFTW will create a fat library
with codelets for all different instruction sets, and pick the fastest
supported one at runtime. On IBM Power8, you definitely want
FFTW-3.3.5 and to compile it with ``--enable-vsx`` for SIMD
support. If you are using a Cray, there is a special modified
