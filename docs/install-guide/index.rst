...
The ARM ThuderX2 Cray XC50 machies differ only in that the recommended
...
Intel Xeon Phi
^^^^^^^^^^^^^^
In the Intel Xeon Phi family the earlier accelerator-cads also
known under the "Knights Corner" codename and the second generation
self-hosted "Knights Landing" platform are supported.

Only symmetric (aka native) mode is supported on Knights Corner, and it has
...
now the performance might not be faster than CPUs. When building for it works
...
The Knights Landing machines behave like a standard other Linux cluster,
if cross-compiling it such nodes use the ``AVX_512_KNL``
SIMD flavor. Additionally to the main DRAM memory, Knights Landing has a high-bandwidth
stacked memory called MCDRAM. Using it offers a performance benefits if
it is ensured that ``mdrun`` runs entirely from this memory; to do so
it is recommended to bind to the appropriate numa node.
