...
``GMX_TPI_DUMP``
        dump all configurations to a pdb_
...
``GMX_VIEW_XPM``
        ...
        automatically view xvg_, xpm_, eps_
        and pdb_ file types, respectively; they default to ``xv``, ``xmgrace``,
...
``GMX_LOGO_COLOR``
        set display color for logo in `gmx view`_.
...
``GMX_COMPELDUMP``
        ...
        pdb_ file, which allows to check whether multimeric channels have 
...
``GMX_DD_NST_DUMP``
        ...
        0 (never), 1 (every DD phase) or a multiple of :term:`nstlist`.
...
``GMX_DO_GALACTIC_DYNAMICS``
        ...
        this environment variable, which allows setting :term:`epsilon-r` to -1 in the mdp_
        file. Normally, :term:`epsilon-r` must be greater than zero to prevent a fatal error.
        See wwwpage_ for example input files for a planetary simulation.

``GMX_ALLOW_CPT_MISMATCH``
        when set, runs will not exit if the
        ensemble set in the tpr_ file does not match that of the
        cpt_ file.

``GMX_CUDA_NB_EWALD_TWINCUT``
        force the use of twin-range cutoff kernel even if :term:`rvdw` equals
        :term:`rcoulomb` after PP-PME load balancing. The switch to twin-range kernels is automated,
...
``GMX_DD_NST_SORT_CHARGE_GROUPS``
        ...
        be 0 (never), 1 (to mean at every domain decomposition), or a multiple of :term:`nstlist`.

``GMX_DETAILED_PERF_STATS``
        when set, print slightly more detailed performance information
        to the log_ file. The resulting output is the way performance summary is reported in versions
        4.5.x and thus may be useful for anyone using scripts to parse log_ files or standard output.
...
``GMX_DISABLE_GPU_DETECTION``
        when set, disables GPU detection even if mdrun_ was compiled
        with GPU support.
...
``GMX_ENX_NO_FATAL``
        disable exiting upon encountering a corrupted frame in an edr_
...
``GMX_IGNORE_FSYNC_FAILURE_ENV``
        allow mdrun_ to continue even if
        a file is missing.
...
``GMX_MAX_MPI_THREADS``
        sets the maximum number of MPI-threads that mdrun_
        can use.

``GMX_MAXCONSTRWARN``
        if set to -1, mdrun_ will
...
``GMX_SCSIGMA_MIN``
        ...
        using the :term:`sc-sigma` keyword in the mdp_ file, but this environment variable can be used
...
``MPIRUN``
        the ``mpirun`` command used by `gmx tune_pme`_.

``MDRUN``
        the mdrun_ command used by `gmx tune_pme`_.

``GMX_NSTLIST``
        sets the default value for :term:`nstlist`, preventing it from being tuned during
        mdrun_ startup when using the Verlet cutoff scheme.
...
``GMX_QM_ORCA_BASENAME``
        prefix of tpr_ files, used in Orca calculations
...
``DSSP``
        used by `gmx do_dssp`_ to point to the ``dssp``
...
``GMX_DIPOLE_SPACING``
        spacing used by `gmx dipoles`_.

``GMX_MAXRESRENUM``
        sets the maximum number of residues to be renumbered by
        `gmx grompp`_. A value of -1 indicates all residues should be renumbered.

``GMX_FFRTP_TER_RENAME``
        Some force fields (like AMBER) use specific names for N- and C-
        terminal residues (NXXX and CXXX) as rtp_ entries that are normally renamed. Setting
        this environment variable disables this renaming.

``GMX_PATH_GZIP``
        ``gunzip`` executable, used by `gmx wham`_.

``GMX_FONT``
        name of X11 font used by `gmx view`_.
...
``GMX_ENER_VERBOSE``
        make `gmx energy`_ and `gmx eneconv`_
        loud and noisy.
