...
.. mdp:: awh-potential
   ...
   .. mdp-value:: convolved
      ...
      set of harmonic umbrella potentials (see :mdp-value:`umbrella` below). This results
      in a smooth potential function and force. The resolution of the potential is set
      by the force constant of each umbrella, see :mdp:`awh1-dim1-pull-coord`.

   .. mdp-value:: umbrella

      The potential bias is applied by controlling the position of an harmonic potential
      using Monte-Carlo sampling.  The force constant of the umbrella is inherited
      from the relevant pull coordinate, see :mdp:`awh1-dim1-pull-coord`.
      When new umbrella positions are sampled often enough,
      the average potential and force equals that of :mdp-value:`convolved`. This option
...
.. mdp:: awh-seed
   ...
   :mdp:`awh-potential` = :mdp-value=`umbrella`.
...
.. mdp:: awh-nsamples-update
   ...
   (10)
   The number of coordinate samples used for each AWH updates.
   The update interval in steps is :mdp-value:`awh-nstsample` times this value.
...
.. mdp:: awh1-error-init
   ...
   (4.0) \[kT\]
   Estimated initial average error of the PMF. This value together with the
   given diffusion constant(s) :mdp:`awh1-dim1-diffusion` together determine the initial biasing rate.
...
.. mdp:: awh1-growth
   ...
   The reference weight histogram initially grows close to exponentially then exits to a final,
   production stage where the growth rate equals that of the sampling rate (set by :mdp:`awh-nstsample`).
   Initially, the histogram grows by scaling it by a constant factor > 1 every time the sampling interval
   has been covered. The exit to the production stage is defined such that the histogram growth never
   exceeds the sampling rate. Because the initial growth rate is determined dynamically, the time before
   exiting will depend on both the system dynamics and the realized system trajectory.
   The initial stage is typically necessary for robust convergence when starting a new simulation where
   high free energy barriers have not yet been flattened by the applied bias.

   .. mdp-value:: linear

   As :mdp-value:`exp-linear` but skip the initial stage. This may be useful if there is *a priori*
   knowledge (see :mdp:`awh1-error-init`) which eliminates the need for an initial stage. Also, this
   is the setting compatible with :mdp:`awh1-target` = :mdp-value:`local-boltzmann`.
...
.. mdp:: awh1-equilibrate-histogram
   ...
   .. mdp-value:: yes
      ...
      Before entering the initial stage (see :mdp:`awh1-growth` = :mdp-value:`exp-linear`), make sure the
      ...
      (:mdp:`awh1-share` = :mdp-value:`yes`) and the initial configurations poorly represent the target
      distribution.      
...
.. mdp:: awh1-target
   ...
   .. mdp-value:: cutoff
      ...
      Same as :mdp:`awh1-target` = :mdp-value:`constant` except that in regions with an estimated free
      energy greater than :mdp-value:`cutoff` relative to the estimated global minimum the target
      distribution equals the unbiased Boltzmann distribution.
   ...
   .. mdp-value:: local-boltzmann
      ...
      Same target distribution and use of :mdp:`aw1-target-beta-scaling` as :mdp-value:`boltzmann`
      ...
      only compatible with :mdp-value:`growth` = :mdp-value:`linear`, since for
      :mdp-value:`growth` = :mdp-value:`exp-linear` histograms are globally rescaled initially.

.. mdp:: awh1-target-beta-scaling

   [0] \[kT\]/\[\]
   For :mdp:`awh1-target` = :mdp-value:`boltzmann` and :mdp:`awh1-target` = :mdp-value:`local-boltzmann`
   it is the unitless beta scaling factor taking values in (0,1).

.. mdp:: awh1-target-cutoff

   [0] \[kT\]/\[\]
   For :mdp:`awh1-target` = :mdp-value:`cutoff` this this is the cutoff > 0 in units of kT.
...
.. mdp:: awh1-dim1-cover-diameter
   ...
   (0.0)) \[nm\]/\[rad\]
   Diameter that needs to be sampled by a single simulation around a coordinate point
   before the point is considered covered in the initial stage (see :mdp:`awh1-growth` = :mdp-value:`exp-linear`).
   This parameter is only relevant for multiple bias-sharing simulations
   (:mdp:`awh1-share` = :mdp-value:`yes`) for which the target region is covered by all simulations collectively.
   *E.g.* when the diameter >= the sampling interval length, covering will not occur until one simulation
   has independently sampled the whole interval. On the other hand, for a diameter = 0 covering occurs
   as soon as the simulations have *collectively* sampled the whole interval. It may be useful to use a
   value > 0 to ensure that for each covering there is an actual transition across each coordinate value.
