...
be attempted using e.g. a :ref:`gro` file with velocities, but such
a restart is not considered to preserve the continuity of the ensemble
as well as one implemented with a checkpoint file.
...
use that file. When :ref:`gmx mdrun` goes to write each subsequent
checkpoint file, it first renames the old file with the suffix
``_prev``, so that even if something goes wrong while writing the
new checkpoint file, only recent progress can be lost. You can
...
somewhere and vulnerable to a power failure or disk that fills in
the meantime. The UNIX tool rsync can be a useful way to periodically
copy your simulation output to a remote storage location, which works
safely even while the simulation is underway. Keeping a copy of the
final checkpoint file from each part of a job submitted to a cluster
can be useful if a file system is unreliable.
...
to instruct :ref:`gmx grompp` to copy the full simulation state in the
