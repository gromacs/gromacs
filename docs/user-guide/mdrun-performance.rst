...
- PME will run on a GPU only when exactly one rank has a
  PME task, ie. no decomposition is supported 
...
|Gromacs| before version 2018:
  ...
  Only short range non-bonded interactions (NB) can be calculated
  on GPUs. Each PP rank has a single NB task that can be offloaded to any GPU.

|Gromacs| version 2018:

  Two different types of GPU accelerated tasks are available, NB and PME.
  Each PP rank has a single NB task that can be offloaded to any GPU.
  If there is only one rank with a PME task (including if that rank is a
  PME-only rank), then that task can be offloaded to any GPU. Such a PME
