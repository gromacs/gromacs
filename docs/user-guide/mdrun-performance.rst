...
an overhead which becomes significant with decreasing time per step,
i.e. in faster iterating simulations.
The overhead is measured by :ref:`gmx mdrun` and reported in the performance
summary table in the log file (in the "Launch GPU ops" row). A few percent
overhead is normal, but in fast iterating parallel runs >10% overhead
can be observed.
Disabling GPU timing (enabled in single GPU runs without domain-decomposition)
can reduce this overhead and improve simulation speed.
The effect will be most significant
especially on NVIDIA GPUs with CUDA, AMD OpenCL runs are less affected.
in fast iterating runs,
in this cases it is recommended turning off timing using the ``GMX_DISABLE_CUDA_TIMING``
...
The separate GPU driver thread can require CPU resources
which can degrade with the concurrently running non-offloaded tasks,
...
E.g. on a machine with a 4-core/8-thread CPU (HT enabled) and an AMD GPU,
start :ref:`gmx mdrun` with 7 thread (i.e. ``-ntomp 7``);
This will leave free CPU resources for the GPU task scheduling
avoiding interference with CPU computation.
Note that assigning fewer resources to :ref:`gmx mdrun` CPU computation
involved a tradeoff which may outweight the benefits of reduced GPU driver overhead,
in particular without HT and with few CPU cores.
...
The OpenCL GPU kernels that are compiled at run time. Hence,
