...
* At least: use a thermostat that gives you the correct average temperature,
  ...
  ``tc_grps = System`` may lead to the "hot solvent/cold solute" problem
...
* Do not use separate thermostats for every component of your system. Some
  ...
  group from that solvent. For a protein simulation, using ``tc_grps = Protein
...
* you have a single water molecule somewhere within the system that is isolated
  from the other water molecules, or
...
Better system preparation can help with 1, if the problems are occurring near
...
#. Monitor various components of the system's energy using g_energy. If an
   intramolecular term is spiking, that may indicate improper bonded parameters,
   for example.
