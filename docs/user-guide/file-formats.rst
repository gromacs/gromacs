...
:ref:`g96`
    GROMOS-96 format
:ref:`pdb`
    Brookhaven Protein DataBank format (.brk and .ent file endings are accepted)
:ref:`esp`
    Espresso coordinate format
...
:ref:`atp`
    ASCII text, database of atom types used by :ref:`gmx pdb2gmx`
:ref:`dat`
...
The cpt file extension stands for checkpoint file. Its
format is based on XDR, a portable binary data representation.
...
With domain decomposition also some decomposition setup information
is stored.
...
to run Molecular Dynamics with Essential Dynamics constraints. edo files
contain such output.
...
Its format is based on XDR, a portable data representation.
...
A pqr file is a modified form of PDB file. Such a file can be written
by :ref:`gmx editconf` ``-mead`` for interacting with the MEAD
program.
