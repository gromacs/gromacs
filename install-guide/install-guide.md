...
2. Check you have CMake version @CMAKE_REQUIRED_MINIMUM_VERSION@ or later.
...
GROMACS, you'll have to read further. Sadly, the interactions of
...
Windows, BlueGene, Cray and many other architectures. Technically, it
can be compiled on any platform with an ANSI C99 compiler, an ISO
C++98 compiler, and supporting libraries, such as the GNU C and C++
libraries.

## Compiler ##

GROMACS requires an ISO C99 compiler and an ISO C++98 compiler. For
best performance, the GROMACS team strongly recommends you get the
most recent version of your preferred compiler for your platform
(e.g. GCC 4.9 or Intel 14.0 or newer on x86 hardware). There is a
large amount of GROMACS code that depends on effective compiler
...
get the latest CUDA version and driver supported by your hardware.
Note that while some CUDA compilers (nvcc) might not officially
support recent versions of gcc as the backend compiler, we still
recommend that you at least use a gcc version recent enough to get
the best SIMD support for your CPU, since GROMACS always runs some
code on the CPU.
...
version @CMAKE_REQUIRED_MINIMUM_VERSION@ or higher.

GROMACS requires CMake version @CMAKE_REQUIRED_MINIMUM_VERSION@ or
...
recommend [FFTW](http://www.fftw.org) (version 3 or higher
only) or [Intel MKL](http://software.intel.com/en-us/intel-mkl). 
...
documentation). Then set `-DGMX_FFT_LIBRARY=mkl` when you run
cmake. In this case, GROMACS will also use MKL for BLAS
and LAPACK (see [linear algebra](#linearalgebra)).
...
how to build GROMACS for you. If you are building GROMACS on hardware
that is identical to that where you will run `mdrun`, then you can be
sure that the defaults will be pretty good. The build configuration
will for instance attempt to detect the specific hardware instructions
available in your processor. However, if you want to control aspects
of the build, there are plenty of things you can set manually.
...
run CMake. This can be a subdirectory or not, it doesn't matter. It
also means you can never corrupt your source code by trying to build
it! So, the only required argument on the CMake command line is the
name of the directory containing the `CMakeLists.txt` file of the code
...
You will see `cmake` report the results of a large number of
tests on your system made by `cmake` and by GROMACS. These are
written to the `cmake` cache, kept in `CMakeCache.txt`. You
...
things seem consistent, the iterations stop. Once `cmake`
...
presented. If you make any changes, then `ccmake` will notice
that and require that you re-configure (using `c`), so that it
...
speed things up by invoking `cmake` with a command like:
...
also do this kind of thing with `ccmake`, but you should avoid
this, because the options set with `-D` will not be able to be
...
need to read more about this, unless you are getting configuration
warnings you do not understand. By default, GROMACS will query
the CPU on the host on which you are running `cmake` for capabilities,
and set the value of the `GMX_SIMD` setting to the best possible
instruction set support both by the hardware, compiler, and our
source code.
...
where e.g. an AVX_128_FMA binary will just crash on an earlier
Intel machine, since the code will try to execute general illegal
instructions (inserted by the compiler) before we get to our
hardware SIMD detection routines.
...
the GROMACS mailing lists, because GROMACS can probably be ported for
new SIMD architectures in a few days.

## CMake advanced options ##

The options that can be seen with `ccmake` are ones that we think a
reasonable number of users might want to consider changing. There are
a lot more options available, which you can see by toggling the
advanced mode in `ccmake` on and off with `t`. Even there, most of the
variables that you might want to change have a `CMAKE_` or `GMX_`
...
earlier hardware, because this will lead binaries (especially `mdrun`)
that run slowly on the new hardware. Building two full installations
and locally managing how to call the correct one (e.g. using the
module system) is the recommended approach. Alternatively, as at the
moment the GROMACS tools do not make strong use of SIMD acceleration,
it can be convenient to create an installation with tools portable
across different x86 machines, but with separate `mdrun` binaries for
each architecture. To achieve this, one can first build a full
installation with the least-common-denominator SIMD instruction set,
e.g. SSE2, then build separate `mdrun` binaries for each architecture
present in the heterogeneous environment.  By using custom binary and
library suffixes for the `mdrun`-only builds, these can be installed
...
## Linear algebra libraries ## {#linearalgebra}
...
* To link statically against external
libraries as well, the `GMX_PREFER_STATIC_LIBS=ON` option can be
...
`_d` for double precision and/or `_mpi` for MPI (and
nothing otherwise). This can be controlled manually with
`GMX_DEFAULT_SUFFIX (ON/OFF)`, `GMX_BINARY_SUFFIX` (takes 
a string) and `GMX_LIBS_SUFFIX` (also takes a string). 
This can also be useful for resolving libary-naming conflicts with 
existing packges (`GMX_PREFIX_LIBMD` also can be useful).
...
sure the cache is stable, you can re-run `cmake ..` or
`ccmake ..`' to see. Then you can run `make` to start the
compilation. Before actual compilation starts, `make` checks
...
it is expected that the `make` procedure will always complete
...
scrolling back a long way through the output of `make` to find
...
Past versions of GROMACS had the ability to `make mdrun` to
...
`-DGMX_BUILD_MDRUN_ONLY=ON`, which will build a cut-down version
of `libgromacs` and/or the `mdrun` binary (according to
whether shared or static). Naturally, now `make install` acts
...
build, then you will inherit the setting for `BUILD_SHARED_LIBS`
...
directory given in `CMAKE_INSTALL_PREFIX`. If this is an system
...
## Testing GROMACS for correctness {#testing} ##
...
2. Search the GROMACS website and users emailing list for
...
4. Post to the GROMACS users emailing list gmx-users for
   ...
   done and why you think it didn't work. Give details about the
...
want to use static libraries due to the peculiarities with parallel
job execution.
...
section on [linear algebra](#linearalgebra)). Since mdrun is the only
part of GROMACS that should normally run on the compute nodes, and
there is nearly no need for linear algebra support for mdrun, it is
recommended to use the GROMACS built-in linear algebra routines - it
is rare for this to run slowly.
...
toolchains. The GROMACS ThreadMPI or no-MPI builds are not useful at
all on BlueGene/Q.
...
system size (see [Running in parallel](#Running in parallel)), and for
...
4.8, and versions 12 and 13 of the Intel compiler as well as Clang
version 3.1 through 3.3. For this we use a variety of GNU/Linux
...
We test irregularly on BlueGene/Q, Cray, Fujitsu PRIMEHPC, Google
Native Client and other environments, and with other compilers and
compiler versions, too. In the future we expect ARM to be an important
test target too, but this is currently not included.
