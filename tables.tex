...
\caption[User specified potential function data.]{User specified
potential function data. f$^{(n)}$(x) denotes the n$^{th}$  derivative
...
function value at x=0 is not important.  In this context r$_c$ denotes
the single cut-off denoted by the variables {\tt rshort} and {\tt
rlong} (see above). These variables should be the same (but need not
be 1.0) and consistent with the table data.  The neighbor-searching
algorithm will search all atom-pairs within a distance {\tt rlong} and
compute the interactions using your potential functions.
