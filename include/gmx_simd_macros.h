...
/* If gmx_castcvtepi32_pr is not available (e.g. on x86 double precision
 * 128-bit SIMD), the next two functions are required for exclusion masking.
 */
#undef gmx_load_si
/* If the same bit is set in both input masks, return all bits 1, otherwise 0 */
#undef gmx_andmask_si
...
#define gmx_andmask_si(m1, m2) _mm_cmpeq_epi32(_mm_andnot_si128(m1, m2), _mm_setzero_si128())
