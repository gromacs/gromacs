...
/* Irritatingly, the declarations of vec_ld* use non-const pointers,
   ...
   real, and sometimes from variables of type float (even at double
   precison), so the correct cast cannot be done easily. The correct
   cast is necessary because the resulting type determines the
   alignment assumption of vec_ld*, which is different for float and
   double. So the loads of always-float variables have to be done with
   a macro that does the correct cast.
 */
