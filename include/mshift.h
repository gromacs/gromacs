...
extern void shift_atom(t_graph *g,rvec sv[],rvec x[],rvec x_s[],atom_id ai);
/* Shift single atom ai */

extern void shift_x(t_graph *g,rvec sv[],rvec x[],rvec x_s[]);
/* Add the shift vector to x, and store in x_s (may be same array as x) */

extern void shift_self(t_graph *g,rvec sv[],rvec x[]);
/* Id. but in place */

extern void unshift_atom(t_graph *g,rvec sv[],rvec x[],rvec x_s[],atom_id ai);
/* Unshift single atom ai */

extern void unshift_x(t_graph *g,rvec sv[],rvec x[],rvec x_s[]);
/* Subtract the shift vector from x_s, and store in x (may be same array) */

extern void unshift_self(t_graph *g,rvec sv[],rvec x[]);
/* Id, but in place */
