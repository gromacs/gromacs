...
{
    const vector4double = vec_splats(0.5);
    const vector4double = vec_splats(1.0);

    /* Can we hope for 12 bits in practice? Otherwise we need more iteractions... */
     vector4double lu = vec_rsqrte(x);

#ifdef GMX_DOUBLE
    lu = vec_madd(vec_nmsub(x,vec_mul(lu,lu),one),vec_mul(lu,half),lu);
#endif
    return vec_madd(vec_nmsub(x,vec_mul(lu,lu),one),vec_mul(lu,half),lu); 
}
