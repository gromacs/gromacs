...
\eeq
Please note that we changed notation as of ver. 2.0 to $\rvij=\rvj-\rvi$ since this
...
\caption{Basic units used in gromacs. Numbers in parentheses give
accuracy.}
\label{tab:basicunits}
\end{table}

Consistent with these units are a set of derived units, given in
Table~\ref{tab:derivedunits}.
\begin{table}
\centerline{
\begin{tabular}{|l|c|l|}
\dline
...
The {\bf electric conversion factor} $f=\frac{1}{4 \pi \varepsilon_o}$
that relates the mechanical quantites to the electrical quantities as
in
\beq
...
recommend strongly to follow the usual practice to include the factor
...
force. The units are those given in table~\ref{tab:derivedunits}:
...
nm, the unit of time changes to 0.1 ps. If the kcal/mol (= 4.184
kJ/mol) is used instead of kJ/mol for energy, the unit of time becomes
...
be computed in kJ/mol, expecting nm as the unit of length. Although
...
different values, see Table~\ref{tab:consts}. All quantities are per mol rather than per
...
use reduced units ({\em i.e.}, setting
...
also be in reduced units. There is one exception: the {\em
temperature}, which is expressed in $0.008\,314\,51$ reduced
units. This is a consequence of the use of Boltzmann's constant in the
