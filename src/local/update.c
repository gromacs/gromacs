...
static void do_pcoupl(t_inputrec *ir,tensor pres,int step,
...
{
  real   vn,vv;
  real   rfac,invfr,rhalf,hulp;
  int    n,d;
  const unsigned long im = 714025;
  const unsigned long ia = 4096;
  const unsigned long ic = 150889;
  unsigned long  jran;

  /* (r-0.5) n times:  var_n = n * var_1 = n/12
     n=6:  var_n = 1/2, so multiply with 2 */
  
   rfac = sqrt(12.0 * 2.0*BOLTZ*temp/(fr*dt));
   rhalf = rfac/2; 
   rfac = rfac/ (float) im;
   invfr=1.0/fr;

   jran=INT_MAX*rando(seed);
  ...
  for (n=start; (n<start+homenr); n++) {  
    for (d=0; (d<DIM); d++) {
      ...
      vold[n][d]     = vn;
      jran =(jran*ia+ic) % im;
      vv             = invfr*f[n][d] + rfac * (float) jran - rhalf;
      v[n][d]        = vv;
  ...
  if (ncons > 0) {
    ...
    *cmax=0;
	...
	if ((a1==b1 || a1==b2) || (a2==b1 || a2==b2)) 
      ...
      (*ddist)[i]=idef->iparams[type].shake.dB-len;
      if (nr <=4) {
	(*bla1)[n1]=a1;
	(*bla2)[n1]=a2;
	(*bllen)[n1]=len;
    ...
    *ncm=ncons-n1;
    ...
    fprintf(stdlog,"%d constraints\nof which %d with more than 4 neighboors\n",ncons,*ncm);
	    ...
	    t_edsamyn    *edyn)
{
  ...
  static rvec      *r;
  static real      *bllen,*blc,*blcc,*blm,*tmp1,*tmp2,*tmp3,*lincslam,*ddist;
  static int       ncm,cmax;
  ...
  if (bFirst) {
    ...
    if (ir->eShakeType == estLINCS)
      init_project(stdlog,top,ir,md,
		   ...
		   &bllen,&blc,&blcc,&blm,&tmp1,&tmp2,&tmp3,&lincslam,&ddist);
  ...
  if ((nblocks > 0) || (nsettle > 0)) {
	...
	ncons=bshakef(stdlog,natoms,md->invmass,nblocks,sblock,&(top->idef),
	...
	if (warn > 0) {
	  ...
	  cconerr(&p_max,&p_rms,&p_imax,xprime,nc,bla1,bla2,bllen);
#endif
	  fprintf(stderr,"step %d  WARNING\n",step);
          fprintf(stderr,"bond between atoms %d and %d rotated more than %.1f degrees\n",bla1[warn-1],bla2[warn-1],wang);
	  fprintf(stderr,"relative constraint deviation after LINCS:\n");
	  fprintf(stderr,"max %.6f (between atoms %d and %d) rms %.6f\n\n",p_max,bla1[p_imax],bla2[p_imax],p_rms);
	}
      }
  ...
  if (bDoUpdate) {  
    ...
    do_pcoupl(ir,pressure,step,box,start,homenr,x,md->cFREEZE,nrnb,
	      freezefac);
    where();
  }
