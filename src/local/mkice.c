...
void virial(FILE *fp,int nmol,rvec x[],matrix box,real rcut,bool bYaw,real q[])
{
  int  i,j,im,jm,natmol,ik,jk,m;
  rvec dx,ftot;
  real dx2,dx1,fscal;
  ...
  for(i=0; (i<nmol); i++) {
	...
	for(ik=0; (ik<natmol); ik++) 
	  for(jk=0; (jk<natmol); jk++) {
	    ...
	    dx2 = iprod(dx,dx);
	    dx1 = sqrt(dx2);
	    fscal = q[ik]*q[jk]/(dx2*dx1);
	    fprintf(fp,"%3s%3d-%3s%3d: %6.3f %6.3f %6.3f %6.3f %8.3f %8.3f %8.3f\n",
		    watname[ik],im+ik,watname[jk],jm+jk,
		    dx[XX],dx[YY],dx[ZZ],norm(dx),
		    dx[XX]*fscal,dx[YY]*fscal,dx[ZZ]*fscal);
	    for(m=0; (m<DIM); m++)
	      ftot[m] += dx[m]*fscal;
	  }
      }
...
#define NFILE asize(fnm)
  ...
  if (debug)
    virial(debug,nmax/natmol,xx,boxje,0.3,bYaw,bYaw ? qyaw : qspc);
