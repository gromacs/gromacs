 ...
 */
  ...
  template <class T> class Bayes : public OptParam
    ...
    public:
       ...
       */
      void changeParam(int j, gmx_rng_t rng);
  ...
  template <class T>
    void Bayes<T>::changeParam(int j, gmx_rng_t rng)
    {
      real rand  = gmx_rng_uniform_real(rng);
  ...
  template <class T>
      ...
      for ( j = iter = 0; (iter < maxiter_); iter++)
	{
	  ...
	  randProbability = gmx_rng_uniform_real(rng);
