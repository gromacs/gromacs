                                 ...
                                 const char *symm_string)
{
    ...
    if (immOK == imm)
    {
        ...
        {
            case eqgRESP:
                if (gr_->add_atom_info(&topology_->atoms, pd))
                {
                    gr_->add_atom_symmetry(symmetric_charges_);
                    gr_->update_atomtypes(&(topology_->atoms));
                    gr_->summary(debug, symmetric_charges_);
                    gr_->add_atom_coords(x_);
                    ...
                    if (ci != EndCalculation())
                    {
                        ...
                        for (ElectrostaticPotentialIterator epi = ci->BeginPotential(); (epi < ci->EndPotential()); ++epi)
                        {
                            ...
                            gr_->add_point(convert2gmx(epi->getX(), xu),
                         ...
                         output_env_t            oenv)
{
    ...
    if (NULL != gr_)
    {
            ...
            ((NULL != difffn) && (NULL != reffn)))
        {
            ...
            if (NULL != difffn)
            {
                grref->add_atom_info(&topology_->atoms, pd);
                grref->add_atom_symmetry(symmetric_charges_);
                grref->read_cube(reffn, FALSE);
                gr_->copy_grid(grref);
            }
            else
            {
                gr_->make_grid(spacing, box, x_);
            }
            if (NULL != rhofn)
            {
            ...
            {
                gr_->calc_pot();
                gr_->write_cube(potfn, buf);
            }
            if (NULL != hisfn)
            {
                gr_->write_histo(hisfn, buf, oenv);
            }
...
void MyMol::UpdateIdef(Poldata * pd, bool bOpt[])
{
    ...
    if (bOpt[ebtsBONDS])
    {
        ...
        for (i = 0; (i < ltop_->idef.il[ftb].nr); i += interaction_function[ftb].nratoms+1)
        {
            ...
            {
                gmx_fatal(FARGS, "here are no parameters for bond %s-%s in the force field", aai, aaj);
            }
        }
    }
    if (bOpt[ebtsANGLES])
    {
        ...
        for (i = 0; (i < ltop_->idef.il[fta].nr); i += interaction_function[fta].nratoms+1)
        {
            ...
            {
                gmx_fatal(FARGS, "here are no parameters for angle %s-%s-%s in the force field", aai, aaj, aak);
            }
        }
    }
    if (bOpt[ebtsPDIHS])
