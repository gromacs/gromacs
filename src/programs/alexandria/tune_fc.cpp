...
void Optimization::calcDeviation()
{
    ...
    {
        if ((mymol.eSupp_ == eSupportLocal) ||
            (bFinal && (mymol.eSupp_ == eSupportRemote)))
        {
            ...
            if (mymol.molProp()->getOptHF(&optHF))
            {
                     ...
                     ei < mymol.molProp()->EndExperiment(); ++ei)
                {
                    ...
                    if (jtype == JOB_OPT || jtype == JOB_SP)
                    {
                        ...
                        real dEcalc    = mymol.enerd_->term[F_EPOT] - mymol.Emol_;
                        real EnerDiff2 = gmx::square(dEcalc - deltaEn);
                        ...
                        if (jtype == JOB_OPT)
                        {
                            ...
                            mymol.OptEcalc_     = mymol.Emol_ + dEcalc;
                        ...
                        if (nullptr != debug)
                        {
                            ...
                            fprintf(debug, "spHF: %g  optHF: %g  DeltaEn: %g\n",
                                    spHF, optHF, deltaEn);
                            fprintf(debug, "%s Chi2 %g dEcalc %g Morse %g  "
    ...
    if (PAR(commrec()))
    {
        printf("1. %d. ePot2: %g nCalc: %g\n", commrec()->nodeid,
               ePot2, nCalc);
        gmx_sumd(1, &ePot2, commrec());
        gmx_sumd(1, &nCalc, commrec());
        printf("2. %d. ePot2: %g nCalc: %g\n", commrec()->nodeid,
               ePot2, nCalc);
    }
                                ...
                                const gmx_output_env_t *oenv)
{
    ...
    for (auto mi = mymols().begin(); mi < mymols().end(); mi++, imol++)
    {
        ...
        for (auto ei = mi->molProp()->BeginExperiment(); ei < mi->molProp()->EndExperiment(); ++ei)
        {
            ...
            if (jtype == JOB_OPT || jtype == JOB_SP)
            {
                ...
                real Halex  = mi->Hform_ + mi->enerd_->term[F_EPOT] - mi->Emol_;
                if (nullptr != hfp)
                {
                    fprintf(hfp, "%10g  %10g\n", Hexper, Halex);
                }
                gmx_stats_add_point(gmol, Hexper, Halex, 0, 0);
                msdTot += gmx::square(Hexper-Halex);
                if (jtype == JOB_OPT)
                {
                    double f2 = 0;
    ...
    fprintf(fp, "RMSD from target energies for %zu compounds and %d conformation is %g. msd = %g\n",
            mymols().size(), nconformation,
            std::sqrt(msdTot/nconformation), msdTot); 
    fprintf(fp, "\n");
    if (nullptr != hform_xvg)
    {
        xvgrclose(xfp);
