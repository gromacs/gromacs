...
typedef std::string MdrunInputParameters;
...
TEST_F(PmeTest, ReproducesEnergies)
{
    ...
    const std::string inputFile = "w48k"; //OctaneSandwich"; //spc2
    ...
    {
        runner_.edrFileName_ = fileManager_.getTemporaryFilePath(inputFile + "_" + runIt + ".edr");
        ...
        commandLine.initFromArray(temp2);
        ...
        for (int i = 0; i < nsteps; i++)
        {
            ...
            if (i == 0)
            {
                const auto conservedTolerance  = relativeToleranceAsFloatingPoint(conservedEnergy, 4e-4);
                const auto reciprocalTolerance = relativeToleranceAsFloatingPoint(reciprocalEnergy, 6e-3);
        ...
        /*
           energyReadersByMode[codePathIt] = openEnergyFileToReadFields(runner_.edrFileName_, {{"Coul. recip."}, {"Conserved En."}});
...
TEST_P(PmeTest, ReproducesEnergies)
{
    int         nsteps     = 200;
    std::string theMdpFile = formatString("coulombtype     = PME\n"
                                          "nstcalcenergy   = 1\n"
    ...
    enum class CodePath //fixme - borrowed
    {
        CPU,
        CUDA
    };

    std::map<CodePath, std::string> codePathNames;
    codePathNames[CodePath::CPU]  = "CPU";
    codePathNames[CodePath::CUDA] = "CUDA";

    std::map<CodePath, std::list<std::string> > modesByCodePath;
    modesByCodePath[CodePath::CPU].push_back("-pme cpu");
    modesByCodePath[CodePath::CUDA].push_back("-pme gpu");
...
//    /pmeModes.push_back(CodePath::CUDA, );
    ...
    for (const auto &codePathIt : modesByCodePath)
    {
        for (const auto &optionsIt : codePathIt.second)
        {
            runner_.edrFileName_ = fileManager_.getTemporaryFilePath(inputFile + "_" + codePathNames[codePathIt.first] + ".edr");
            // TODO jsut use teh test name generation? also add the options

            CommandLine commandLine;
            // PMECommandLine.addOption("-ntmpi", 1);
            //commandLine.addOption("-pme", codePathIt);
            //commandLine.append(optionsIt);

            ASSERT_EQ(0, runner_.callMdrun(commandLine));

            printf("%s\n", ::testing::UnitTest::GetInstance()->current_test_info()->name());
            //::testing::UnitTest::GetInstance()->current_test_info()->nam‌​e();
