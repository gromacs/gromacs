...
class DensfitIntegrationTest : public MdrunTestFixture
{
    ...
    public:
        ...
        real getCorrelationFromXvgFile(const std::string fn) const;
};
...
real DensfitIntegrationTest::getCorrelationFromXvgFile(const std::string fn) const
{
...
TEST_F(DensfitIntegrationTest, DensfitForcesAreCorrect)
{
    ...
    gromppCaller.addOption("-mi", inputMapFileName);
...
#ifdef WITHCCP4 // we cannot test anything meaningful yet if there is no CCP4 map support
    // ... and compare the output
    auto cc = getCorrelationFromXvgFile(densfitOutFileName);
    EXPECT_REAL_EQ_TOL(0.58290, cc, absoluteTolerance(1e-6));

    // densfit forces were multiplied by 1e10 before (see mdp input file), therefore the factor 1e10 below
#if GMX_DOUBLE
    ASSERT_EQ(0, compareTrajectories(forcesReferenceFileName, runner_.fullPrecisionTrajectoryFileName_, 0.00001e10));
#else
    ASSERT_EQ(0, compareTrajectories(forcesReferenceFileName, runner_.fullPrecisionTrajectoryFileName_, 0.00050e10));
#endif
#endif      // WITHCCP4
}
