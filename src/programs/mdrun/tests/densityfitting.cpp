...
TEST_F(DensfitIntegrationTest, DensfitForcesAreCorrect)
{
    ...
    runner_.ndxFileName_                     = fileManager_.getInputFilePath((std::string(name) + ".ndx").c_str());
    runner_.fullPrecisionTrajectoryFileName_ = fileManager_.getTemporaryFilePath((name + ".trr").c_str());

    auto        mapFileName                  = fileManager_.getInputFilePath("4AKE-aligned-Calpha.ccp4");
    ...
    gromppCaller.addOption("-n", runner_.ndxFileName_);
