...
bool DensfitIntegrationTest::trajectoriesAreEqualWithinTolerance(const std::string &fnA, const std::string &fnB, double absTolerance) const
{
    ...
    while (tfr1.readNextFrame() && tfr2.readNextFrame() )
    {
        ...
        fprintf(stderr, "\nRead trajectory frames '%s' and '%s'\n", fr1.frameName().c_str(), fr2.frameName().c_str());
        TrajectoryFrameMatchSettings matchSettings;
        matchSettings.mustComparePositions = true;
        TrajectoryTolerances         tolerances {
            absoluteTolerance(absTolerance), absoluteTolerance(absTolerance), absoluteTolerance(absTolerance), absoluteTolerance(absTolerance)
        };
        compareTrajectoryFrames(fr1, fr2, matchSettings, tolerances);
    }
...
TEST_F(DensfitIntegrationTest, DensfitForcesAreCorrect)
{
    ...
    std::string mdpString =
        ...
        "continuation             = yes      \n"
        "DensityFitting           = yes      \n"
        "densfit-potential        = cross-correlation \n"
    ...
    EXPECT_REAL_EQ_TOL(0.613195, correlationFromXvgFile(densfitOutFileName), absoluteTolerance(1e-6));
