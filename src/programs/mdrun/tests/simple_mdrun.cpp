...
{{
     {
         "angles1",
         relativeToleranceAsPrecisionDependentUlp(-1, 10, 1000)
     }
 }};
...
TEST_P(SimpleMdrunTest, WithinTolerances)
{
    ...
    // Do mdrun
    {
        ...
        checker.setDefaultTolerance(relativeToleranceAsPrecisionDependentUlp(-1, 10, 400));
        const std::string     xyz[DIM] = { "X", "Y", "Z" };
        do
        {
            ...
            for (auto &f : force)
            {
                for (int m = 0; m < DIM; m++)
                {
                    std::string forceName = frame.frameName() + " F[" + toString(atom) + "][" + xyz[m] + "]";
                    checker.checkReal(f[m], forceName.c_str());
                }
                atom++;
            }
        }
