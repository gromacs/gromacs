    ...
    // Do mdrun, preparing to check the normal modes later
    {
        ...
        do
        {
            ...
            for (auto &f : force)
            {
                ...
                {
                    char forceName[128];
                    snprintf(forceName, sizeof(forceName), "Force_%d_%d_%d",
                             static_cast<int>(frame.step()), atom, m);
                    checker.checkReal(f[m], forceName);
                }
                atom++;
            }
        }
...
std::vector<std::string> integratorsToTest_g = { "md" };
...
// such configurations.
#if GMX_DOUBLE
INSTANTIATE_TEST_CASE_P(SimpleMdrunWorks, SimpleMdrunTest,
