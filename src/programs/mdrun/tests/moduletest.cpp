...
#include "programs/mdrun/mdrun.h"
...
MdrunTestFixture::MdrunTestFixture()
    : numThreads(1), //TODO consider how varying this should be managed
      ...
      mdpFileName(fileManager_.getTemporaryFilePath("input.mdp")),
      topFileName(),
      groFileName(),
      trrFileName(),
...
{
    gmx::File::writeFileFromString(mdpFileName, mdpString);
}
...
MdrunTestFixture::callGrompp()
{
    ...
    caller.addOption(FileNameOption("f").description("Input .mdp file").store(&mdpFileName));
    ...
    int rc = gmx::CommandLineModuleManager::runAsMainCMain(args.getArgc(), args.getArgv(), &gmx_grompp);
...
MdrunTestFixture::callMdrun()
{
    ...
    ArgsBuilder args(caller.getProgramName());
    args.visitSubSection(caller.getOptions());
    int rc = gmx::CommandLineModuleManager::runAsMainCMain(args.getArgc(), args.getArgv(), &gmx_mdrun);
