...
SimulationRunner::callMdrun(const CommandLine &callerRef)
{
    ...
    const gmx_hw_info_t *hwinfo = gmx_detect_hardware(nullptr, nullptr, true);
    scoped_guard_sfree   hwinfoGuard(const_cast<gmx_hw_info_t*>(hwinfo));
#if GMX_THREAD_MPI
    ...
    std::string     message;
    if ((hwinfo->ngpu_compatible_tot > hwinfo->nphysicalnode) &&
        !gmx_multiple_gpu_per_node_supported())
    {
            ...
            !gmx_gpu_sharing_supported())
        {
            ...
            message += formatString("So, this test will run with GPU support enabled and on a single rank, rather than "
                                    "the %d ranks and %d nodes that would have been the default.", numberOfRanks, hwinfo->nphysicalnode);
             ...
             !gmx_gpu_sharing_supported())
    {
        ...
        message += formatString("So, this test will run with GPU support enabled and on as many ranks "
                                ...
                                "been the default.", hwinfo->ngpu_compatible_tot, numberOfRanks, hwinfo->nphysicalnode);
