...
bool
EnergyFrameReader::probeForNextFrame()
{
    ...
    // If there's a next frame, read it into frame_, and report the result.
    nextFrameExists_ = do_enx(energyFile_, frame_);

    return nextFrameExists_;
}

EnergyFrameInfo
EnergyFrameReader::makeFrameInfo()
{
    ...
    if (!nextFrameExists_)
    {
        GMX_THROW(APIError("There is no next frame, so there should have been no attempt to use the data, e.g. by reacting to a call to probeForNextFrame()."));
    }

    // The probe filled frame_ with new data, so now we use that data to fill frameInfo
...
EnergyFrameInfo::EnergyFrameInfo() : values_(), step_(), time_() {};
