...
TEST_F(CompelTest, SwapCanRun)
{
    ...
    useTopAndGroFromDatabase(name.c_str());
    mdpInputFileName = fileManager_.getInputFilePath((name + ".mdp").c_str());
    ndxFileName      = fileManager_.getInputFilePath((name + ".ndx").c_str());

    EXPECT_EQ(0, callGrompp());

    cptOutputFileName = fileManager_.getTemporaryFilePath(".cpt");
