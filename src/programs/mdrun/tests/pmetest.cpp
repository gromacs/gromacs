...
#include <algorithm> // for some (disappeared?) cleverness of converting std::string to CommandLine
#include <map>
#include <string>
#include <vector>
...
TEST_F(PmeTest, ReproducesEnergies)
{
    ...
    std::string theMdpFile = formatString("coulombtype     = PME\n"
                                          ...
                                          "nstenergy       = 1\n"
                                          "pme-order       = 4\n"
                                          // TODO do we need reproducibility? "ld-seed         = 765\n"
                                          "nsteps          = %d\n",
                                          nsteps);
    ...
    for (const auto &mode : runModes)
    {
        ...
        for (int i = 0; i <= nsteps; i++)
        {
            ...
            if (i == 0)
            {
                ...
                const auto conservedTolerance    = relativeToleranceAsFloatingPoint(startingKineticEnergy, 1e-7); //TODO tune relative tolerances
                const auto reciprocalTolerance   = relativeToleranceAsFloatingPoint(reciprocalEnergy, 1e-6);
