...
/*! \internal \file
 * \brief
 * This test implements basic PME sanity test (using single-rank mdrun).
 * It runs the input system with PME for several steps and checks the reciprocal and consreved energies.
 * TODO: have multi-rank tests as well.
 *
...
TEST_F(PmeTest, ReproducesEnergies)
{
    ...
    std::list<std::string> runArguments;
    runArguments.push_back("-pme cpu");
#if GMX_GPU == GMX_GPU_CUDA
    runArguments.push_back("-pme gpu");
#endif
    TestReferenceData    refData;
    TestReferenceChecker rootChecker(refData.rootChecker());

    for (const auto &runIt : runArguments)
    {
        runner_.edrFileName_ = fileManager_.getTemporaryFilePath(inputFile + "_" + runIt + ".edr");

        std::vector<std::string>  argList = splitString(runIt);
        ...
        commandLine.initFromArray(argPtrList);
