...
TEST_F(PmeTest, ReproducesEnergies)
{
    ...
    if (!thisRankChecks)
    {
        EXPECT_NONFATAL_FAILURE(rootChecker.checkUnusedEntries(), ""); // skip checks
    }
    for (const auto &mode : runModes)
    {
        ...
        if (useSeparatePme)
        {
            commandLine.addOption("-npme", 1); //TODO death tests for -npme 2, decomposition, etc
        }
        ASSERT_EQ(0, runner_.callMdrun(commandLine));

        if (GMX_MPI && parallelRun)
        {
