...
class ReplicaExchangeTest : public gmx::test::MdrunTestFixture,
                            public ::testing::WithParamInterface<const char *>
{
    public:
        ...
        ReplicaExchangeTest() : size(gmx_node_num()),
                                rank(gmx_node_rank())
        {
            originalTestTempDirectory = fileManager_.getOutputTempDirectory();
            mdrunCaller.append("mdrun_mpi");
        }

        ~ReplicaExchangeTest()
        {
            // restore the pre-test-case global state
            fileManager_.setOutputTempDirectory(originalTestTempDirectory);
        }
        ...
        void organizeMultidir()
        {
            ...
            {
                char replicaTempDirectory[STRLEN];
                snprintf(replicaTempDirectory, STRLEN,
                         "%s/multidir_%d",
            ...
            fileManager_.setOutputTempDirectory(futureTestTempDirectory.c_str());
        ...
        {
            char       mdpFileContents[STRLEN];
            const real baseTemperature = 298;
            const real basePressure    = 1;
            snprintf(mdpFileContents, STRLEN,
                     "nsteps = 2\n"
                     "nstlog = 1\n"
                     "nstcalcenergy = 1\n"
                     "tcoupl = v-rescale\n"
        ...
        //! MPI processor set size and rank of this process
        int                    size, rank;
        //! Object for building the mdrun command line
        gmx::test::CommandLine mdrunCaller;
        //! Cache the original test directory, so it can be re-set during setup.
        const char            *originalTestTempDirectory;
};
...
#else
TEST_P(ReplicaExchangeTest, DISABLED_ExitsNormally)
#endif
{
    if (size <= 1)
    {
