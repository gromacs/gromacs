    ...
    else
    {
        ...
        bIntelAVX =
            (gmx_cpuid_vendor(hwinfo->cpuid_info) == GMX_CPUID_VENDOR_INTEL &&
             gmx_cpuid_feature(hwinfo->cpuid_info, GMX_CPUID_SIMD_X86_AVX_256));

        if (nthreads_tot <= nthreads_omp_always_faster ||
            ((cpuid_is_intel_nehalem(hwinfo->cpuid_info) && nthreads_tot <= nthreads_omp_always_faster_Nehalem) ||
