             ...
             const char *deviceOptions, unsigned long Flags)
{
    ...
    if (gmx_mtop_ftype_count(mtop, F_ORIRES) > 0)
    {
        if (DOMAINDECOMP(cr))
        {
            gmx_fatal(FARGS, "Orientation restraints do not work with domain decomposition. Use a single MPI rank to get a single domain.");
        }
    ...
    else
    {
        ...
        {
            // TODO ask somebody who knows: is epbcSCREW compatible with TPI or NM?
            gmx_fatal(FARGS,
                      "pbc=%s is only implemented in parallel",
