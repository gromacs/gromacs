...
int Mdrunner::mainFunction(int argc, char *argv[])
{
    ...
    if (MASTER(cr) && !continuationOptions.appendFiles)
    {
        ...
        // We can not do this check later, since pa is not passed to mdrunner()
        if (opt2parg_bSet("-multi", asize(pa), pa))
        {
            fprintf(fplog, "\nThe -multi flag is deprecated, and may be removed in a future version. Please update your workflows to use -multidir instead.\n");
        }
    }
    else
    {
