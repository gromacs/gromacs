                                    ...
                                    int nst, int nex, int init_seed)
{
    ...
    if (!EI_DYNAMICS(ir->eI))
    {
        ...
        /* This means that PAR(cr) and DOMAINDECOMP(cr) are synonymous
         * here, even though multi-simulations always run in parallel
         * (and have MULTISIM(cr) true), and each such simulation can
         * have only one rank (and thus no DD), because PAR(cr) is
         * defined by cr->nnodes > 1, which is distinct from cr->ms
         * being non-NULL. The only way for cr->nnodes > 1 to be true
         * is if we are using DD. */
    }
