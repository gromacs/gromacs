                                        ...
                                        int                  ngpu)
{
    ...
    if (ngpu > 0)
    {
        if (hw_opt->nthreads_omp > 0)
        {
             ...
             */
            gmx_fatal(FARGS, "When using GPUs, setting only the number of OpenMP threads can lead to conflicting demands for choosing the number of thread-MPI ranks and the number of GPUs. Please specify the number of thread-MPI ranks as well (option -ntmpi).");
        }
