...
static int getMaxGpuUsable(gmx::Logger *mdlog, const gmx_hw_info_t *hwinfo,
                           int cutoff_scheme, gmx_bool bUseGpu)
{
        ...
        hwinfo->gpu_info.n_dev_compatible > 0)
    {
        ...
        else
        {
            ...
            {
                GMX_LOG(*mdlog).formatWarning("More than one compatible GPU is available, but GROMACS can only use one of them. Using a single thread-MPI rank.");
            }
            return 1;
        }
    }
                     ...
                     gmx_bool             bUseGpu)
{
    ...
    {
        GMX_LOG(*mdlog).formatWarning("The integration or electrostatics algorithm doesn't support parallel runs. Using a single thread-MPI rank.");
                                        ...
                                        gmx::Logger         *mdlog)
{
    ...
    if (DOMAINDECOMP(cr) && cr->nnodes > 1)
    {
             ...
             nth_omp_max > nthreads_omp_mpi_ok_max))
        {
            ...
            if (bNtOmpOptionSet)
            {
                GMX_LOG(*mdlog).formatWarning("NOTE: %s", buf);
            }
            else
            {
    ...
    {
        GMX_LOG(*mdlog).formatWarning("NOTE: GROMACS was compiled without OpenMP and (thread-)MPI support, can only use a single CPU core");
    }
#else
