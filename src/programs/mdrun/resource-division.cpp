 ...
 * by a factor of 2 seems to be a good esimate.
 */
                                        ...
                                        FILE                *fplog)
{
    ...
    if (DOMAINDECOMP(cr) && cr->nnodes > 1)
    {
            ...
            nthreads < nthreads_omp_mpi_ok_min)
        {
            ...
            else
            {
                 ...
                 */
                gmx_fatal(FARGS, "%s If you want to run this setup anyhow, specify the -ntomp option. But we suggest to change the number of MPI ranks%s.", buf, mpi_option);
            }
        }
    }
    else
    {
        ...
        if (nthreads > nthreads_omp_always_faster(hwinfo->cpuid_info, ngpu > 0))
        {
            /* To arrive here, the user/system set #ranks and/or #OMPthreads */
            char buf2[256];

            if (bNTOptSet || getenv("OMP_NUM_THREADS") != NULL)
            {
                sprintf(buf2, "You requested %d OpenMP threads", nthreads);
            }
            else
            {
                sprintf(buf2, "Your choice of %d MPI rank%s leads to the use of %d OpenMP threads",
