             ...
             gmx_walltime_accounting_t walltime_accounting)
{
    ...
    if (bRerunMD && fr->cutoff_scheme == ecutsVERLET && ir->opts.ngener != 0 && fr->nbv && fr->nbv->bUseGPU)
    {
        gmx_fatal(FARGS, "The Verlet scheme on GPUs does not support energy groups, so your rerun should probably use a .tpr file without energy groups, or mdrun -nb cpu");
    }
