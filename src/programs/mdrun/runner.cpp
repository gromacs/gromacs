...
{
    const int nthreads = STS::getNumThreads();
    const int nthreadsLINCS  = bUseDefaultSchedule ? nthreads : nthreads * stsConstrainRatio;
    const int nthreadsSETTLE = bUseDefaultSchedule ? nthreads : nthreads - nthreadsLINCS;
    STS* sts = new STS("constrain");
    sts->clearAssignments();

    if (nthreadsLINCS >= nthreads || nthreadsLINCS <= 0) {
        sts->setDefaultSchedule();
        ...
        for (int t=0; t<nthreadsLINCS; t++) {
            sts->assign("lincs_loop", t, {{t, nthreadsLINCS},{t+1, nthreadsLINCS}});
        }
        new MMBarrier(nthreadsLINCS, "lincs");
        sts->assign("settle", nthreadsLINCS);
        for (int t=nthreadsLINCS, numer=0; t<nthreads; t++, numer++) {
            sts->assign("settle_loop", t, {{numer, nthreadsSETTLE},{numer+1, nthreadsSETTLE}});
        }
    }
