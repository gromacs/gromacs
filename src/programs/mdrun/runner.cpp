             ...
             int imdport, unsigned long Flags)
{
     ...
     */
    cr->dev_use_count = ((bNBUseGPU && (cr->duty & DUTY_PP)) ? 1 : 0) +
        ((bPMEUseGPU && (cr->duty & DUTY_PME)) ? 1 : 0);
    ...
    /* This chooses node-local GPU ids, initializes all the rank local GPUs which might be used later */
    gmx_select_rank_gpu_ids(mdlog, cr, &hwinfo->gpu_info, bForceUseGPU, &hw_opt->gpu_opt);
    /* This sorts out the rank-local GPU to task assignment */
    gmx_select_tasks_gpu_ids(cr, bNBUseGPU, bPMEUseGPU);
