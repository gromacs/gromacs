             ...
             int imdport, unsigned long Flags)
{
    ...
    if (SIMMASTER(cr))
    {
        ...
        if (inputrec->cutoff_scheme == ecutsVERLET)
        {
             ...
             * TODO: Add a reasonable threshold for choosing PME on GPU over CPU in auto mode, e.g. number of threads available.
    ...
    if (SIMMASTER(cr))
    {
        ...
        if (useGpuPME)
        {
            hw_opt->nthreads_tmpi = 1; //FIXME what should this really do for PME on GPU?
        }
                     ...
                     inputrec->eI == eiNM))
    {
        if (useGpuPME && (npme < 0))
        {
            /* PME on GPU prefers a single PME-only rank as a defautl behaviour for multi-rank. */
            npme = 1;
        }
