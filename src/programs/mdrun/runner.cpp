...
int Mdrunner::mdrunner()
{
    ...
    const char      *pmeOption          = "cpu";
    const char      *pmeFftOption       = "auto";
    GMX_UNUSED_VALUE(pmeFftOption);
    ...
    if (SIMMASTER(cr))
    {
        ...
        if (inputrec->cutoff_scheme != ecutsVERLET)
        {
            ...
            if (!hwinfo->compatibleGpus.empty())
            {
    ...
    if (GMX_THREAD_MPI && SIMMASTER(cr))
    {
        ...
        try
        {
            ...
            useGpuForNonbonded = decideWhetherToUseGpusForNonbondedWithThreadMpi
                    (nbpu_opt, userGpuIds, emulateGpuNonbonded, *hwinfo,
                    inputrec->cutoff_scheme == ecutsVERLET,
                    gpuAccelerationOfNonbondedIsUseful(mdlog, inputrec, doRerun),
                    hw_opt.nthreads_tmpi);
            useGpuForPme = decideWhetherToUseGpusForPmeWithThreadMpi
                    (useGpuForNonbonded, pmeOption, pmeFftOption, userGpuIds,
                    *hwinfo, EEL_PME(inputrec->coulombtype), hw_opt.nthreads_tmpi);
        }
        GMX_CATCH_ALL_AND_EXIT_WITH_FATAL_ERROR;
    ...
    // Ensure that each rank on a node uses the same approach to task
    // assignment, now that each rank knows the inputrec that
    // SIMMASTER read and used, and cr->nnodes has been assigned the
    // number of thread-MPI ranks that have been chosen.
    //
    ...
    try
    {
        std::tie(useGpuForNonbonded, useGpuForPme) =
            decideWhetherToUseGpus(nbpu_opt, pmeOption, userGpuIds,
                                   emulateGpuNonbonded, *hwinfo, inputrec->cutoff_scheme == ecutsVERLET,
                                   gpuAccelerationOfNonbondedIsUseful(mdlog, inputrec, doRerun),
                                   cr->nnodes);
    }
    GMX_CATCH_ALL_AND_EXIT_WITH_FATAL_ERROR;
    ...
    auto                 haveGpus = !hwinfo->compatibleGpus.empty();
    ...
    if (thisRankHasDuty(cr, DUTY_PP))
    {
        if (useGpuForNonbonded)
        {
            if (!haveGpus)
            {
                gmx_fatal(FARGS, "Cannot run short-ranged nonbonded interactions on a GPU because there is none");
            }
    ...
    if (EEL_PME(inputrec->coulombtype) && (thisRankHasDuty(cr, DUTY_PME)))
    {
        if (useGpuForPme)
        {
            if (!haveGpus)
            {
                gmx_fatal(FARGS, "Cannot run PME on a GPU because there is none");
            }
    ...
    try
    {
        ...
        gpuTaskAssignment = runTaskAssignment(userGpuIds, *hwinfo,
