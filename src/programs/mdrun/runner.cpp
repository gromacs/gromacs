                               ...
                               const t_commrec  *cr)
{
    ...
    GMX_RELEASE_ASSERT(fr, "fr was NULL");
             ...
             const char *deviceOptions, int imdport, unsigned long Flags)
{
    ...
    GMX_RELEASE_ASSERT(cr->duty & (DUTY_PP | DUTY_PME), "Rank %d did not have a valid PP/PME duty");
    if (cr->duty & DUTY_PP)
    {
        ...
        if (DOMAINDECOMP(cr) && (cr->duty & DUTY_PP))
        {
