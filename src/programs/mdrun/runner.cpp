...
void Mdrunner::reinitializeOnSpawnedThread()
{
    ...
    /* We need a barrier to ensure that the master thread does not modify
       mdrunner data before we finished copying it here.
       The matching barrier is in Mdrunner::spawnThreads(). */
#if GMX_THREAD_MPI
...
t_commrec *Mdrunner::spawnThreads(int numThreadsToLaunch) const
{
    ...
    /* We need a barrier to ensure *this does not change while spawned
       threads are copying it.
       The matching barrier is in Mdrunner::reinitializeOnSpawnedThread(). */
    MPI_Barrier(MPI_COMM_WORLD);
#else
