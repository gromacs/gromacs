 ...
 */
bool pme_gpu_supported(const t_inputrec *ir, std::string *error, int npme)
{
             ...
             int imdport, unsigned long Flags)
{
    ...
    if (SIMMASTER(cr))
    {
        ...
        if (inputrec->cutoff_scheme == ecutsVERLET)
        {
            ...
            const bool areGpusAvailable = (hwinfo->gpu_info.n_dev_compatible > 0 ||
            ...
            if (useGpuPME && !pme_gpu_supported(inputrec, &pmeGpuError, npme))
            {
            ...
            /* With current code, default bahivour for PME is to use GPU if available.
