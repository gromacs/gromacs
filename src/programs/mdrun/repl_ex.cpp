...
enum {
    ereTEMP, ereLAMBDA, ereAWH, ereENDSINGLE, ereTL, ereNR
};
                                    ...
                                    int nst, int nex, int init_seed)
{
    ...
    if (ir->bDoAwh)
    {
        ...
        bAwh = repl_quantity(ms, re, ereAWH, (real)re->repl);
    }
    ...
    re->awh_replex = bAwh ? init_awh_replica_exchange(ir->awh_params, awh, ms,
...
static real calc_delta(FILE *fplog, gmx_bool bPrint, struct gmx_repl_ex *re, int a, int b, int ap, int bp)
{
    ...
    switch (re->type)
    {
        ...
        case ereTL:
            ...
            break;
        case ereAWH:
            delta = calc_awh_replica_exchange_delta(re->awh_replex, a, b, ap, bp);
            break;
        default:
            gmx_incons("Unknown replica exchange quantity");
    }
                          ...
                          real                  time)
{
    ...
    if (re->type == ereAWH)
    {
        ...
        bAwh         = TRUE;
        bAwh_updated = update_awh_replica_exchange_state(awh, re->awh_replex, ms);
    }
    ...
    if (bMultiEx)
    {
        ...
        for (i = 0; i < re->nex + nself; i++)
        {
            ...
            if (i0 == i1 ||
                (bAwh && !awh_replicas_can_exchange(re->awh_replex, re->ind[i0], re->ind[i1])) )
            {
                nself++;
                          ...
                          t_state *state_local, gmx_int64_t step, real time)
{
    ...
    {
        replica_id  = re->repl;
        test_for_replica_exchange(fplog, cr->ms, re, awh, enerd, det(state_local->box), step, time);
