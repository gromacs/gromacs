...
#include "gromacs/mdlib/MimicCommunicator.h"
...
/*! \libinternal
    \copydoc integrator_t (FILE *fplog, t_commrec *cr, const gmx::MDLogger &mdlog,
                           ...
                           gmx_walltime_accounting_t walltime_accounting)
 */
double gmx::do_mimic(FILE *fplog, t_commrec *cr, const gmx::MDLogger &mdlog,
                     int nfile, const t_filenm fnm[],
                     const gmx_output_env_t *oenv,
                     const MdrunOptions &mdrunOptions,
                     gmx_vsite_t *vsite, gmx_constr_t constr,
                     ...
                     gmx_walltime_accounting_t walltime_accounting)
{
    ...
    gmx_bool        bNS, bNStList, bSimAnn, bStopCM,
                    bFirstStep, bLastStep = FALSE,
                    bBornRadii, bUsingEnsembleRestraints;
    gmx_bool          bDoDHDL = FALSE, bDoFEP = FALSE, bDoExpanded = FALSE;
    gmx_bool          do_ene, do_log, do_verbose, bRerunWarnNoV = TRUE, bCPT;
    ...
    if (MASTER(cr) && !bRerunMD)
    {
        comm.init();
        comm.sendInitData(top_global, state_global->x);
        comm.getStepNumber(ir->nsteps);
    }
    ...
    top_global->mimicTop          = new GmxMimicTop();
    top_global->mimicTop->qmAtoms = gmx_mtop_gen_qmmm(top_global);
    ...
    if (MASTER(cr))
    {
        ...
        {
            rerun_fr.natoms = state_global->natoms;
            copy_mat(state_global->box, rerun_fr.box);
            snew(rerun_fr.x, state_global->natoms);
            snew(rerun_fr.v, state_global->natoms);
            rerun_fr.bBox = true;
            comm.getCoords(rerun_fr.x, state_global->natoms);
...
//   TODO: get initial coords from mimic
    ...
    while (!bLastStep)
    {
        ...
        if (!bRerunMD)
        {
            ...
            if (MASTER(cr))
            {
                comm.sendEnergies(enerd->term[F_EPOT]);
                comm.sendForces(ftemp, state_global->natoms);
            }
        }
