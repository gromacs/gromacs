                  ...
                  gmx_walltime_accounting_t walltime_accounting)
{
    ...
    {
        fr->bDoAwh = true;
        fr->awh    = new gmx::Awh(fplog, *ir, cr, *ir->awhParams, ir->pull_work);

        if (startingFromCheckpoint)
        {
            /* Restore the AWH history read from checkpoint */
            fr->awh->restoreStateFromHistory(state_global->awhHistory.get(), cr);
        }
        else
        {
            /* Initialize the AWH history here */
            state_global->awhHistory = std::shared_ptr<AwhHistory>(new AwhHistory());
            fr->awh->initHistoryFromState(state_global->awhHistory.get());
        }
    }
    ...
    while (!bLastStep)
    {
        ...
        else
        {
            ...
            {
                fr->awh->updateHistory(state_global->awhHistory.get());
            }
