                  ...
                  gmx_walltime_accounting_t walltime_accounting)
{
    ...
    {
        repl_ex = init_replica_exchange(fplog, cr->ms, state_global, ir, fr->awhbias,
    ...
    while (!bLastStep)
    {
        ...
        if (MASTER(cr))
        {
            ...
            print_ebin(mdoutf_get_fp_ene(outf), do_ene, do_dr, do_or, do_log ? fplog : NULL,
                       step, t,
                       eprNORMAL, mdebin, fcd, groups, &(ir->opts), fr->awhbias);
        ...
        if (bDoReplEx)
        {
            bExchanged = replica_exchange(fplog, cr, repl_ex, fr->awhbias,
                                          state_global, enerd,
                                          state, step, t);
        }
    ...
    if (MASTER(cr))
    {
        if (ir->nstcalcenergy > 0 && !bRerunMD)
        {
            print_ebin(mdoutf_get_fp_ene(outf), FALSE, FALSE, FALSE, fplog, step, t,
                       eprAVER, mdebin, fcd, groups, &(ir->opts), fr->awhbias);
        }
    }
