...
/* Copy the state from \p rerunFrame to \p globalState and, if requested, construct vsites */
                              ...
                              gmx_bool         *bRerunWarnNoV)
{
    ...
    if (constructVsites)
    {
        ...
        if (graph)
        {
             ...
             */
            mk_mshift(nullptr, graph, fr.ePBC, globalState->box, as_rvec_array(globalState->x.data()));
        ...
        construct_vsites(vsite, as_rvec_array(globalState->x.data()), timeStep, as_rvec_array(globalState->v.data()),
                         ...
                         fr.ePBC, fr.bMolPBC, nullptr, globalState->box);
        if (graph)
        {
                  ...
                  gmx_walltime_accounting_t walltime_accounting)
{
    ...
    while (!bLastStep)
    {
        ...
        if (bRerunMD && MASTER(cr))
        {
            ...
            prepareRerunState(rerun_fr, globalStateContainerPtr, constructVsites, vsite, top->idef, ir->delta_t, *fr, graph, &bRerunWarnNoV);
        }
