             ...
             gmx_walltime_accounting_t walltime_accounting)
{
        ...
        !bRerunMD)
    {
        ...
        /* Delay DD load balancing with GPU + seperate PME nodes or env.var. */
        if (use_GPU(fr->nbv) && DOMAINDECOMP(cr) &&
            (!(cr->duty & DUTY_PME) || getenv("GMX_PMELOADBAL_DLB_DELAY") != NULL))
        {
