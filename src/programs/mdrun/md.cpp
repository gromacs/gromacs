...
double gmx::do_md(FILE *fplog, t_commrec *cr, const gmx::MDLogger &mdlog,
                  ...
                  int nstglobalcomm,
                  gmx_vsite_t *vsite, gmx_constr_t constr,
                  int stepout, gmx::MDModules *mdModules, t_inputrec *ir,
                  ...
                  gmx_walltime_accounting_t walltime_accounting)
{
    ...
    /* Initial values */
    init_md(fplog, cr, mdModules, ir, oenv, &t, &t0, state_global->lambda,
    ...
    {
        fr->bDoAwh = true;
        fr->awh    = new AwhBiasCollection(fplog, ir, cr, ir->awh_params,
                                           &state_global->awhHistory,
                                           ir->pull_work,
    ...
    while (!bLastStep)
    {
        ...
        else
        {
            /* The AWH history need to be saved _before_ doing force calculations where the AWH bias is updated
               ...
               rid of this issue. */
            if (fr->bDoAwh && bCPT && MASTER(cr))
            {
                fr->awh->updateHistory(state_global->awhHistory);
            }
        ...
        if (MASTER(cr))
        {
            ...
            print_ebin(mdoutf_get_fp_ene(outf), do_ene, do_dr, do_or, do_log ? fplog : nullptr,
                       step, t,
                       eprNORMAL, mdebin, fcd, groups, &(ir->opts), fr->awh);
    ...
    if (MASTER(cr))
    {
        if (ir->nstcalcenergy > 0 && !bRerunMD)
        {
            print_ebin(mdoutf_get_fp_ene(outf), FALSE, FALSE, FALSE, fplog, step, t,
                       eprAVER, mdebin, fcd, groups, &(ir->opts), fr->awh);
        }
    }
