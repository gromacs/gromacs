        ...
        ir->nstlist > 1)
    {
        if (EI_DYNAMICS(ir->eI) && inputrec2nboundeddim(ir) == 3)
        {
            ...
            else
            {
                ...
                kT_runtime = BOLTZ*buffer_temp/(ir->nsteps*ir->delta_t);

                if (EI_MD(ir->eI) && ir->etc == etcNO && ir->nstlist > 1 &&
                    ir->nsteps > 0 &&
                    ir->verletbuf_tol > 0.011*kT_runtime)
                {
                    sprintf(warn_buf, "You are using a Verlet buffer tolerance of %g kJ/mol/ps for an NVE simulation of %g ps, which can give a final drift of %d%%. For conserving energy to %d%%, you might need to set verlet-buffer-tolerance to %.1e.",
                            ir->verletbuf_tol, ir->nsteps*ir->delta_t,
                            (int)(ir->verletbuf_tol/kT_runtime*100 + 0.5),
                            1, 0.01*kT_runtime);
                    warning_note(wi, warn_buf);
                }
