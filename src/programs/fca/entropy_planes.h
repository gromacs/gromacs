    ...
    public:
        Entropy(const int inNdata, const int inNbins)
        {
            ...
            for (int i = 0; i < this->ngauss; i++)
            {
                ...
                this->gaussprime[i] = -1.0 / sqrt(2.0 * M_PI) / sigma * (1.0 / utils::squareof(sigma) * (i - floor(this->ngauss / 2.0))) * exp(-utils::squareof(1.0 / sigma * (i - floor(this->ngauss / 2.0))) / 2);
        ...
        void entropy_binning(const real s[])
        {
            ...
            for (int i = 0; i < this->ndata; i++)
            {
                ...
                //fprintf(stderr,"maxv %f minv %f s %f\n",maxv,minv,s[i]);
        ...
        void entropy_convolution(const real kernel[], const real bins[], real result[]) const
        {
            ...
            {
                result[i]        = 0;
                const int gstart = this->ngauss - (this->nbins - i) - m;
                const int gstopp = i + 1 + m;
                for (int k = (0 > gstart ? 0 : gstart); k < (this->ngauss < gstopp ? this->ngauss : gstopp); k++)
                {
                    result[i] += bins[i + m - k] * kernel[k] * invn;
                }
                // OK! smoothing verglichen mit MATLAB gaussav.m
            }
        }

        real entropy_interpolate()
        {
            real       S            = 0.0;
...
class Plane
{
    ...
    public:
        ...
        real minimize_plane(Entropy* entr)
        {
            ...
            const real bx = M_PI / SCAN_STEPS / 2 * k_min;
            const real ax = M_PI / SCAN_STEPS / 2 * (k_min - 1);
            const real cx = M_PI / SCAN_STEPS / 2 * (k_min + 1);
            //  fprintf(stderr,"initial bracket [%10.5f %10.5f] %10.5f with S=%20.15f\n",ax,cx,bx,S[k_min]);
            const real newS = dbrent_plane(ax, bx, cx, entr, TOL, &theta);
            if (newS > S[k_min])
            {
                fprintf(stderr, "found minimum at [%10.5f] %10.5f Sscan=%10.5f Sold=%10.5f\n", theta, newS, S[k_min], S[0]);
            }
            ...
            if ((newS2 - newS) > 1e-6)
            {
                fprintf(stderr, "found minimum at [%10.5f] %20.15f S=%20.15f deltaS %10.5g\n", theta, newS2, newS, newS2 - newS);
            }
            return theta;
            ...
            /*
               ...
               if (k_min>0)
               fprintf(stderr,"found new minimum [%10.5f] %10.5f with S=%20.15f\n",theta,theta+(k_min-SCAN_STEPS/2)*0.01,S[k_min]);
             */
        }
        ...
        real dbrent_plane(const real ax, const real bx, const real cx, Entropy* entr, const real tol, real* xmin)
        {
            ...
            fprintf(stderr, "FATAL: Too many iterations in routine DBRENT");
            exit(1);
