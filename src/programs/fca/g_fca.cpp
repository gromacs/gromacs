...
int gmx_fca_core(int argc, char* argv[])
{
    const char* desc[] =
    {
    ...
    if (mpi.isMaster())
    {
        ...
        {
            // TODO this appear during the completion, this should not be the case
            fprintf(stdout, "you need to provide either a trajectory and eigenvec.trr or with -pos a file of any set of vectors\n");
            setupIsCorrect = 0;
        }
    } //if MASTER

#if GMX_MPI
    FCA::utils::AssertMpi(MPI_Bcast(&setupIsCorrect, 1, MPI_INT, FCA_MASTER_RANK, MPI_COMM_WORLD), __FILE__, __LINE__);
    if (!setupIsCorrect)
    {
#if GMX_THREAD_MPI
        if (mpi.getMyRank() != 0) // TODO update -- it should not be here
        {
            tMPI_Finalize();
        }
#endif
...
{
#if GMX_MPI
    bool initMpiMySelf = false;
    if (gmx_mpi_initialized() == false) // TODO update -- it should not be here
    {
#if GMX_THREAD_MPI
        int    targc = 2;
...
#endif
    const int returnedValue = gmx_fca_core(argc, argv);
#if GMX_MPI
    if (initMpiMySelf) // TODO update -- it should not be here
    {
#if GMX_THREAD_MPI
        tMPI_Finalize();
#else
