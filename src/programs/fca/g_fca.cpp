...
std::vector<int> select_eigen_vectors(const int nbAtoms, const int nvec, const int eignr[], const int first, int last);
...
int gmx_fca(int argc, char* argv[])
{
...
#ifdef GMX_MPI
    AssertMpi(MPI_Bcast(&dim, 1, MPI_INT, FCA_MASTER_RANK, MPI_COMM_WORLD));
    AssertMpi(MPI_Bcast(&nframes, 1, MPI_INT, FCA_MASTER_RANK, MPI_COMM_WORLD));
#endif
    ...
    ///////////////////////////////////////////////////////////////////////////////////////////////////////
#ifdef GMX_MPI
    AssertMpi(MPI_Bcast(&number_of_moves, 1, MPI_INT, FCA_MASTER_RANK, MPI_COMM_WORLD));
    AssertMpi(MPI_Bcast(&bMinimize, 1, MPI_INT, FCA_MASTER_RANK, MPI_COMM_WORLD));
#endif
    /// ---- Minimize
    if (bMinimize)
    {
    ...
    ///////////////////////////////////////////////////////////////////////////////////////////////////////
#ifdef GMX_MPI
    AssertMpi(MPI_Bcast(&bSort, 1, MPI_INT, FCA_MASTER_RANK, MPI_COMM_WORLD));
    AssertMpi(MPI_Bcast(&amplitude_weight, 1, MPI_INT, FCA_MASTER_RANK, MPI_COMM_WORLD));
#endif
...
#ifdef GMX_MPI
    // distribute flag
    AssertMpi(MPI_Bcast(&bWriteMI, 1, MPI_INT, FCA_MASTER_RANK, MPI_COMM_WORLD));
    AssertMpi(MPI_Bcast(&log_kappa1, 1, MPI_DOUBLE, FCA_MASTER_RANK, MPI_COMM_WORLD));
    AssertMpi(MPI_Bcast(&log_kappa2, 1, MPI_DOUBLE, FCA_MASTER_RANK, MPI_COMM_WORLD));
    AssertMpi(MPI_Bcast(&bBasicHisto, 1, MPI_INT, FCA_MASTER_RANK, MPI_COMM_WORLD));
#endif
    // compute MI matrix
    if (bWriteMI)
    {
