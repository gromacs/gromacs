...
class Task2D {
    ...
    static void _receive(void* data, int nr, MPI_Datatype type, int address, int tag, MPI_Comm comm, MPI_Status* stat) {
        utils::AssertMpi(MPI_Recv(data, nr, type, address, tag, comm, stat), __FILE__, __LINE__);
    }
    ...
    static void _send(void* dat, int nr, MPI_Datatype type, int address, int tag, MPI_Comm comm) {
        ...
        utils::AssertMpi(MPI_Send(dat, nr, type, address, tag, comm), __FILE__, __LINE__);
...
public:
    ...
    int receive_result() {
        ...
        /* fprintf(stdout,"receive from node %d for cell (%d,%d) %g\n",slave,i,j,data); */
    ...
    int next_index(int* i, int* j) {
        if(this->com.getNumNodes() > 1) {
            ...
            if(this->com.getMyRank() == FCA_MASTER_RANK) {
                ...
                utils::AssertMpi(MPI_Barrier(MPI_COMM_WORLD), __FILE__, __LINE__);
