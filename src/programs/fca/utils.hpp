...
#if defined(GMX_MPI) && !defined(AssertMpi)
#define AssertMpi(X, F, L) if(MPI_SUCCESS != (X)) { \
    std::string messageErrorMpi = "MPI Error at line " + std::to_string(L) + " in file " + (F) + ", value "  + std::to_string(X);\
...
#else
    mpi(const MPI_Comm& inComm) {
        AssertMpi(MPI_Comm_size(inComm, &num_nodes), __FILE__, __LINE__);
        AssertMpi(MPI_Comm_rank(inComm, &my_rank), __FILE__, __LINE__);
    }
