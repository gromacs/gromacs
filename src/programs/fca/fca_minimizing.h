...
class FcaMaster
{
    constexpr static real MI_MIN_CORR = -0.5;
    constexpr static int  LOGFRQ      = 50;

    int                   dim;
    int                   nframes;
    // TRUE if ic_matrix is the same on all nodes
    bool                  bSync;
    // references...
    const utils::mpi     &com;
    const bool            useLogInMinimization;
    const bool            verboseEnable;
    const bool            useKickout;
    ...
    real * move;
    int    steps;
    // MI shifts systematically with nframes -- to correct for this use min_MI
    double min_MI;

    static int random_int(gmx::ThreeFry2x64<8> &rng, int maxv)
    {
        return gmx::UniformIntDistribution<int>(0, maxv-1) (rng);
//        real ran = gmx::UniformIntDistribution<real>(0,1)(rng);
//        if (ran == 1)
//        {
//            ran = 0.99999;
//        }
...
//        return (int)ran;
    ...
    static void rand_array_int(gmx::ThreeFry2x64<8> &rng, const int n, int array[])
    {
        for (int i = 0; i < n; i++)
        {
            array[i] = i;
    ...
    void update_icmatrix(const int nr, const std::pair<int, int>* moves)
    {
        for (int i = 0, ct = 0; ct < nr; i++)
        {
            ...
            {
                AssertMpi(MPI_Bcast(&localMove, 2, MPI_INT, MASTER_RANK, MPI_COMM_WORLD));
                AssertMpi(MPI_Bcast(&theta, 1, MPI_DOUBLE, MASTER_RANK, MPI_COMM_WORLD));
            }
#endif
    ...
    std::vector < std::pair < int, int>> generate_moves(const int nr, utils::Log& logger) {
        ...
        MoveGenerator moveGenerator(nr, dim*dim);
        ...
        for (int i = 1; i < dim; i++)
        {
            for (int j = 0; j < i; j++)
            {
                ...
                // if MI-entry is larger than lowest value in list, and we really want to do this move...
                if (dum > moveGenerator.getCurrentMin() && this->move[j + i * dim] > 0.2 && (this->MI[i + j * dim] + min_MI) > MI_MIN_CORR)
                {
                    // check if any other entry uses same axis
                    moveGenerator.add(i, j, dum);
                }
            }
        ...
        if (logger.move_log)
        {
            fprintf(logger.move_log, "found %d moves with sum_val %10.5f\nresample %d kickouts...\n",
                    moveGenerator.getActualNumberOfMoves(), moveGenerator.getActualSum(), moveGenerator.getNumberOfKicks());
        }
        if (useKickout)
        {
            const std::vector < std::pair < int, int>> kickList = moveGenerator.getKickPairs();
            for (const std::pair<int, int> &kij : kickList)
            {
                const int  i   = kij.first;
                const int  j   = kij.second;
                const real dum = (this->MI[i + j * dim] - min_MI) * this->move[j + i * dim];
                moveGenerator.add(i, j, dum);
            }
            if (logger.move_log)
            {
                fprintf(logger.move_log, "after resampling: found %d moves with sum_val %10.5f\n",
                        moveGenerator.getActualNumberOfMoves(), moveGenerator.getActualSum());
            }
        }
        return moveGenerator.getMoves();
    }
    void dump_MIsum(FILE* out) const
    {
        fprintf(out, "%10.5f\n", this->MIsum);
    }
    ...
    public:
        ...
        std::unique_ptr< real[] >* getIcx() const
        {
            return icx.get();
        }
        int getDim() const
        ...
        std::unique_ptr< real[] >* getIcMatrix()
        {
            return ic_matrix.get();
        }

        FcaMaster(const int inDim, const int innframes, std::vector< std::unique_ptr< real[] > > inProjx, const utils::mpi &inCom)
            : com(inCom),
            ...
            {
                {
                    ...
                    AssertMpi(MPI_Bcast(nframesdim, 2, MPI_INT, MASTER_RANK, MPI_COMM_WORLD));
                    if (com.isMaster() == false)
                    {
                ...
                const MPI_Datatype realtype = (sizeof(real) == sizeof(float) ? MPI_FLOAT : MPI_DOUBLE);
                for (int i = 0; i < nframes; i++)
                {
                    AssertMpi(MPI_Bcast(projx[i].get(), dim, realtype, MASTER_RANK, MPI_COMM_WORLD));
                }
            }
#endif
        ...
        void write_MI_matrix(FILE* out)
        {
            utils::dump_sqrmatrix(out, this->MI.get(), this->dim);
        }
        ...
        void dump_ic_matrix(FILE* out) const
        {
            for (int j = 0; j < dim; j++)
            {
                for (int i = 0; i < dim; i++)
                {
                    fprintf(out, "%10.5f", this->ic_matrix[i][j]);
                }
                fprintf(out, "\n");
            }
        }
        ...
        void read_ic_matrix(const char inFilename[])
        {
            ...
            for (int j = 0; j < dim; j++)
            {
                for (int i = 0; i < dim; i++)
                {
                    int nread = fscanf(in, "%f", &this->ic_matrix[i][j]);
                    if (nread < 1)
                    {
                        fprintf(stderr, "FATAL: file does not contain enough elements to read (%dx%d)-matrix\n", dim, dim);
                        exit(1);
                    }
                }
            }
        ...
        {
    #ifdef MPICC
            AssertMpi(MPI_Bcast(&this->bSync, sizeof(bool), MPI_BYTE, MASTER_RANK, MPI_COMM_WORLD));
            if (!this->bSync && (this->com.getNumNodes() > 1))
            {
                for (int i = 0; i < dim; i++)
                {
                    AssertMpi(MPI_Bcast(this->ic_matrix[i].get(), sizeof(real) * dim, MPI_BYTE, MASTER_RANK, MPI_COMM_WORLD));
                }
            }
    #endif
            this->bSync = true;
        ...
        int minimization(const int nr_moves, utils::Log &logger)
        {
            // returns zero if no move has been possible
            ...
            if (com.isMaster())
            {
                ...
                if (useLogInMinimization && this->steps % LOGFRQ == 0)
                {
                    ...
                    {
                        utils::dump_sqrmatrixr(logger.move_matrix, this->to_compute.get(), this->dim);
                    }
                    if (logger.ic_matrix)
                    {
...
#ifdef MPICC
            AssertMpi(MPI_Bcast(&actual_moves, 1, MPI_INT, MASTER_RANK, MPI_COMM_WORLD));
#endif
            if (actual_moves > 0)
            {
                Task2D task2D(this->com, this->theta.get(), this->dim, this->dim, symmetricTask, next_moves.data(), nr_moves);
                if (useLogInMinimization && com.isMaster())
                {
                    fprintf(stderr, "NODE %d, starting plane rotations...\n", this->com.getMyRank());
                }
                // auch in fca-struct rein ?
                Entropy entr(this->nframes, 200);
                ...
                while (task2D.next_index(&p1, &p2))
                {
                    ...
                    {
                        fprintf(stderr, "NODE %d: warum soll ich plane (%d,%d) rotieren ?? \n", this->com.getMyRank(), p1, p2);
        ...
        void project_ic_signal()
        {
            if (verboseEnable)
            {
                fprintf(stderr, "project signal to ics\n");
            }
            for (int i = 0; i < dim; i++)
            {
        ...
        void compute_S1()
        {
            ...
            // t_mpi *com=this->com;
            Entropy entr1D(nframes, 200);
            this->MIsum = 0;

            if (verboseEnable)
            {
                fprintf(stderr, "single S 0-%d  ", dim - 1);
                fprintf(stderr, "berechne 1D entropies...\n");
            }
         ...
         */
        void compute_MI_matrix(real* to_compute, gmx::ThreeFry2x64<8>* rng)
        {
            ...
            {
                fprintf(stderr, "\n");
                fprintf(stderr, "berechne 2D entropies...");
            }

            Task2D    task2D(com, MI.get(), dim, dim, symmetricTask);
            task2D.set_to_compute(to_compute, computeAgain);
            Entropy2D entr2D(nframes, 100, rng != nullptr);
                        ...
                        const real WEIGHT_AMP, const real WEIGHT_ANH)
        {
            ...
            std::sort(&qsort_data[0], &qsort_data[dim], [&](const qsort_fca &a, const qsort_fca &b) {
                          const gmx_bool res = (WEIGHT_AMP * b.amp + WEIGHT_ANH * log(1e-9 + abs(b.anh)))
                              > ((a.amp) * WEIGHT_AMP + log(1e-9 + abs(a.anh)) * WEIGHT_ANH);
                          return (res);
                      });
        ...
        void compute_mode_amplitude(real* amplitude) const
        {
            ...
            for (int k = 0; k < dim; k++)
            {
                ...
                for (int j = 0; j < nframes; j++)
                {
                    amplitude[k] += utils::squareof(icx[k][j]);
                }
                amplitude[k] *= invnframes;
            }
        }
