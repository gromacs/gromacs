...
class FcaMaster
{
    ...
    void update_icmatrix(const int nr, const std::pair<int, int>* moves)
    {
        for (int i = 0, ct = 0; ct < nr; i++)
        {
            ...
            {
                AssertMpi(MPI_Bcast(&localMove, 2, MPI_INT, FCA_MASTER_RANK, MPI_COMM_WORLD), __FILE__, __LINE__);
                AssertMpi(MPI_Bcast(&theta, 1, MPI_DOUBLE, FCA_MASTER_RANK, MPI_COMM_WORLD), __FILE__, __LINE__);
            }
#endif
    ...
    public:
            ...
            {
                {
                    ...
                    AssertMpi(MPI_Bcast(nframesdim, 2, MPI_INT, FCA_MASTER_RANK, MPI_COMM_WORLD), __FILE__, __LINE__);
                    if (com.isMaster() == false)
                    {
                ...
                for (int i = 0; i < nframes; i++)
                {
                    AssertMpi(MPI_Bcast(projx[i].get(), dim, realtype, FCA_MASTER_RANK, MPI_COMM_WORLD), __FILE__, __LINE__);
                }
            }
#endif
        ...
        {
    #if GMX_MPI == 1
            AssertMpi(MPI_Bcast(&this->bSync, sizeof(bool), MPI_BYTE, FCA_MASTER_RANK, MPI_COMM_WORLD), __FILE__, __LINE__);
            if (!this->bSync && (this->com.getNumNodes() > 1))
            {
                for (int i = 0; i < dim; i++)
                {
                    AssertMpi(MPI_Bcast(this->ic_matrix[i].get(), sizeof(real) * dim, MPI_BYTE, FCA_MASTER_RANK, MPI_COMM_WORLD), __FILE__, __LINE__);
                }
            }
    #endif
            this->bSync = true;
...
#if GMX_MPI == 1
            AssertMpi(MPI_Bcast(&actual_moves, 1, MPI_INT, FCA_MASTER_RANK, MPI_COMM_WORLD), __FILE__, __LINE__);
#endif
            if (actual_moves > 0)
            {
