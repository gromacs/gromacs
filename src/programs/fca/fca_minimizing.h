...
class FcaMaster
{
    ...
    void update_icmatrix(const int nr, const std::pair<int, int>* moves)
    {
        for (int i = 0, ct = 0; ct < nr; i++)
        {
            ...
            {
                utils::AssertMpi(MPI_Bcast(&localMove, 2, MPI_INT, FCA_MASTER_RANK, MPI_COMM_WORLD), __FILE__, __LINE__);
                utils::AssertMpi(MPI_Bcast(&theta, 1, MPI_DOUBLE, FCA_MASTER_RANK, MPI_COMM_WORLD), __FILE__, __LINE__);
            }
#endif
            ...
            if (com.isMaster())
            {
                ...
                theta = fmod(theta, utils::PI_constant() / 2);
                if (theta > utils::PI_constant() / 4)
                {
                    theta = theta - utils::PI_constant() / 2;
                }
    ...
    public:
            ...
            {
                {
                    ...
                    utils::AssertMpi(MPI_Bcast(nframesdim, 2, MPI_INT, FCA_MASTER_RANK, MPI_COMM_WORLD), __FILE__, __LINE__);
                    if (com.isMaster() == false)
                    {
                ...
                for (int i = 0; i < nframes; i++)
                {
                    utils::AssertMpi(MPI_Bcast(projx[i].get(), dim, realtype, FCA_MASTER_RANK, MPI_COMM_WORLD), __FILE__, __LINE__);
                }
            }
#endif
        ...
        void read_ic_matrix(const char inFilename[])
        {
            ...
            for (int j = 0; j < dim; j++)
            {
                for (int i = 0; i < dim; i++)
                {
                    int nread = utils::fscanf_floating_point(in, &this->ic_matrix[i][j]);
                    if (nread < 1)
                    {
        ...
        {
    #if GMX_MPI == 1
            utils::AssertMpi(MPI_Bcast(&this->bSync, sizeof(bool), MPI_BYTE, FCA_MASTER_RANK, MPI_COMM_WORLD), __FILE__, __LINE__);
            if (!this->bSync && (this->com.getNumNodes() > 1))
            {
                for (int i = 0; i < dim; i++)
                {
                    utils::AssertMpi(MPI_Bcast(this->ic_matrix[i].get(), sizeof(real) * dim, MPI_BYTE, FCA_MASTER_RANK, MPI_COMM_WORLD), __FILE__, __LINE__);
                }
            }
    #endif
            this->bSync = true;
...
#if GMX_MPI == 1
            utils::AssertMpi(MPI_Bcast(&actual_moves, 1, MPI_INT, FCA_MASTER_RANK, MPI_COMM_WORLD), __FILE__, __LINE__);
#endif
            if (actual_moves > 0)
            {
                Task2D task2D(this->com, this->theta.get(), this->dim, this->dim, symmetricTask, next_moves.data(), int(next_moves.size()));
        ...
        {
            const real logsp = log(2 * utils::PI_constant());
            for (int i = 0; i < dim; i++)
            {
