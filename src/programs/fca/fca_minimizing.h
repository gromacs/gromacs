...
class FcaMaster
{
    ...
    std::vector < std::pair < int, int>> generate_moves(const int nr, utils::Log& /*logger*/) {
        ...
        utils::PSort::sort(currentStates.begin(), currentStates.end(), [](const MoveState &st1,
                                                                          const MoveState &st2){
    ...
    public:
        ...
        FcaMaster(const int inDim, const int innframes, std::vector< std::unique_ptr< real[] > > inProjx, const utils::mpi &inCom)
            : com(inCom),
              ...
              MIsum(0)
        {
            fprintf(stdout, "INFO FCA_MINIMIZING to catch unknown file: error: SEH exception %d\n", __LINE__); // TODO remove
            ...
            for (int i = 0; i < dim; i++)
            {
                fprintf(stdout, "INFO FCA_MINIMIZING to catch unknown file: error: SEH exception %d\n", __LINE__); // TODO remove
                fflush(stdout);                                                                                    // TODO remove
                fprintf(stdout, "nframes %d dim %d innframes %d\n", nframes, dim, innframes);                      // TODO remove
                fflush(stdout);                                                                                    // TODO remove
...
#pragma omp parallel for schedule(static) default(shared)
        ...
        void compute_S1()
        {
            ...
            // t_mpi *com=this->com;
#if GMX_OPENMP
            std::unique_ptr<std::unique_ptr<Entropy>[]> entr1DforThreads(new std::unique_ptr<Entropy>[omp_get_max_threads()]);
#else
...
#pragma omp parallel for schedule(static) default(shared) reduction(+:localMIsum)
            ...
            for (int i = 0; i < dim; i++)
            {
                ...
                {
                    entr1DforThreads[omp_get_thread_num()].reset(new Entropy(nframes, 200));
                }
                        ...
                        const real WEIGHT_AMP, const real WEIGHT_ANH)
        {
            ...
            utils::PSort::sort(&qsort_data[0], &qsort_data[dim], [&](const qsort_fca &a, const qsort_fca &b) {
...
#pragma omp parallel for schedule(static) default(shared)
            ...
            for (int k = 0; k < dim; k++)
            {
                ...
                for (int j = 0; j < nframes; j++)
                {
                    amplitude_k += utils::squareof(icx[k][j]);
                }
