...
class FcaMaster
{
    ...
    static int random_int(gmx::ThreeFry2x64<64> &rng, int maxv)
    {
...
//        return (int)ran;
    ...
    static void rand_array_int(gmx::ThreeFry2x64<64> &rng, const int n, int array[])
    {
        for (int i = 0; i < n; i++)
        {
            array[i] = i;
    ...
    std::vector < std::pair < int, int>> generate_moves(const int nr, utils::Log& logger) {
        ...
        std::sort(currentStates.begin(), currentStates.end(), [](const MoveState &st1,
                                                                 const MoveState &st2){
    ...
    public:
         ...
         */
        void compute_MI_matrix(real* to_compute, gmx::ThreeFry2x64<64>* rng)
        {
            ...
            while (task2D.next_index(&p1, &p2))
            {
                ...
                if (p1 != p2)
                {
                    if (rng)
                    {
                        fprintf(stderr, "1] %e\n", entr2D.entropy2D(perm_data[p1].get(), perm_data[p2].get()) - entr2D.entropy2D(icx[p1].get(), icx[p2].get())); // TODO
                        task2D.push_result(entr2D.entropy2D(perm_data[p1].get(), perm_data[p2].get()) - entr2D.entropy2D(icx[p1].get(), icx[p2].get()));
                    }
                    else
                    {
                        fprintf(stderr, "2] %e\n", S1[p1] + S1[p2] - entr2D.entropy2D(icx[p1].get(), icx[p2].get())); // TODO
