    ...
    if (mat)
    {
        ...
        for (int k = 0; k < dim; k++)
        {
            ...
            for (int j = 0; j < natoms; j++)
            {
                ...
                for (int d = 0; d < DIM; d++)
                {
                    sqratom += fca_utils::squareof(mat[k * ndim + j * DIM + d]);
                }
...
static std::vector<int> select_eigen_vectors(const int nbAtoms, const int nvec, const int eignr[], const int first, int last)
{
    ...
    fprintf(stdout, "%zu eigenvectors selected for output", outvec.size());
    if (outvec.size() <= 100 && eignr)
    {
        fprintf(stdout, ":");
        for (int j = 0; j < int(outvec.size()); j++)
        {
            fprintf(stdout, " %d", eignr[outvec[j]] + 1);
        }
    }
    fprintf(stdout, "\n");
    return outvec;
...
int gmx_fca(int argc, char* argv[])
{
    const char* desc[] =
    {
        ...
        "gmx_fca writes out collective coordinates describing the ensemble motion",
        ...
        "[PAR]gmx_fca can operate on two differnt kinds of inputs",
        ...
        "gmx_fca will interpret the rows as high-dimensional samples, i.e., a column contains samples of a single coordinate",
        ...
        "The file fca_vec.trr is only written if gmx_fca was started with -v. It contains the coordinate transformation from cartesian",
        ...
        "Note, that gmx_fca sorts the modes by neg-entropy and variance. You can set the weights for the sorting criterium",
        "with the options -negw and -ampw for neg-entropy and variance, respectively",
        "[PAR]Note, that you can use gmx_fca with the option -nominimize to resort the modes given in the input file.",
    ...
    t_filenm fnm[] =
    {
        { efTRN, "-v", "eigenvec", ffOPTRD },
        { efTRX, "-f", nullptr, ffOPTRD },
        { efTPS, nullptr, nullptr, ffOPTRD },
        { efNDX, nullptr, nullptr, ffOPTRD },
        { efXVG, "-pos", "pos", ffOPTRD },
        { efXVG, "-r", "restart_ic_matrix", ffOPTRD },
        { efXVG, "-ic", "ic_matrix", ffWRITE },
        { efTRN, "-o", "fca_vec", ffWRITE },
        { efLOG, "-l", "fca.log", ffWRITE },
        { efXVG, "-val", "fca_val", ffWRITE },
        { efXVG, "-mi", "mi_matrix", ffOPTWR }
    };

    std::unique_ptr< FcaEigenVec >           eigVecManager;
    std::vector< std::unique_ptr< real[] > > projx;
    fca_utils::TrxInfo  trxinfo;
    ...
    if (mpi.isMaster())
    {
                              ...
                              asize(fnm), fnm, asize(pa), pa, asize(desc), desc, 0, nullptr, &oenv) )
        {
            ...
            {
                fprintf(stdout, "start projection \n");
                eigVecManager.reset(new FcaEigenVec(opt2fn_null("-v", asize(fnm), fnm),
                                                    ftp2fn(efTPS, asize(fnm), fnm),
                                                    ftp2fn_null(efNDX, asize(fnm), fnm),
                                                    nullptr));

                std::vector<int> proj_info = select_eigen_vectors(eigVecManager->getNbAtoms(), eigVecManager->getNbVec(),
                ...
                FcaFrameSet   frameSet(opt2fn("-f", asize(fnm), fnm), oenv, load_frame_step);

                auto          frames = frameSet.releaseFrames();
                fca_utils::apply_rm_pbc_plain_to_all(frameSet.getNbAtoms(), &frames, frameSet.getBox());
            ...
            {
                fprintf(stdout, "start projection \n");
                projx          = fca_utils::InputFunctions::read_fca_proj(PosFile, &dim);
                nframes        = projx.size();
                setupIsCorrect = 1;
            }
        }
...
#if GMX_LIB_MPI
    fca_utils::AssertMpi(MPI_Bcast(&setupIsCorrect, 1, MPI_INT, FCA_MASTER_RANK, MPI_COMM_WORLD), __FILE__, __LINE__);
#endif
    if (!setupIsCorrect)
    {
        return 1;
...
#if GMX_LIB_MPI
    fca_utils::AssertMpi(MPI_Bcast(&dim, 1, MPI_INT, FCA_MASTER_RANK, MPI_COMM_WORLD), __FILE__, __LINE__);
    fca_utils::AssertMpi(MPI_Bcast(&nframes, 1, MPI_INT, FCA_MASTER_RANK, MPI_COMM_WORLD), __FILE__, __LINE__);
#endif
    FcaMaster fca(dim, nframes, std::move(projx), mpi);
    if (mpi.isMaster() && opt2bSet("-r", asize(fnm), fnm))
    {
        fprintf(stdout, "accessing restart file...\n");
    ...
    ///////////////////////////////////////////////////////////////////////////////////////////////////////
#if GMX_LIB_MPI
    fca_utils::AssertMpi(MPI_Bcast(&number_of_moves, 1, MPI_INT, FCA_MASTER_RANK, MPI_COMM_WORLD), __FILE__, __LINE__);
    fca_utils::AssertMpi(MPI_Bcast(&bMinimize, 1, MPI_INT, FCA_MASTER_RANK, MPI_COMM_WORLD), __FILE__, __LINE__);
#endif
    /// ---- Minimize
    if (bMinimize)
    {
        fprintf(stdout, "minimize correlation for %d frames, %d trial moves on %d slave-nodes per step\n", nframes, number_of_moves, mpi.getNumNodes() - 1);
        if (mpi.isMaster())
        {
            fprintf(stdout, "distribute data on nodes...\n");
        }
        ...
        if (mpi.isMaster())
        {
            fprintf(stdout, "start minimizing...\n");
        }
        static bool stop = false;
#if (!defined(__INTEL_COMPILER) && !defined(__ICC) && !defined(__ICL)) || __INTEL_COMPILER >= 1700
        ...
        while (fca.minimization(number_of_moves, logger) && !stop)
        {
            if (mpi.isMaster())
            {
                fprintf(stdout, "MI-sum: %10.5f\r", fca.getMIsum());
            }
        } // while (minimize)
    ...
    ///////////////////////////////////////////////////////////////////////////////////////////////////////
#if GMX_LIB_MPI
    fca_utils::AssertMpi(MPI_Bcast(&bSort, 1, MPI_INT, FCA_MASTER_RANK, MPI_COMM_WORLD), __FILE__, __LINE__);
    fca_utils::AssertMpi(MPI_Bcast(&amplitude_weight, 1, MPI_INT, FCA_MASTER_RANK, MPI_COMM_WORLD), __FILE__, __LINE__);
#endif
    ...
    if (mpi.isMaster())
    {
        ...
        if (logger.icx)
        {
            for (y = 0; y < fca.getNFrames(); y++)
            {
                ...
                {
                    fprintf(logger.icx, "%e ", fca.getIcx()[z][y]);
                }
                fprintf(logger.icx, "\n");
            }
        }
        ...
        // output fca_val.xvg
        {
            ...
            write_fca_valfile_fca(val_out, fca.getDim(), fca_eigvec.get(),
...
#if GMX_LIB_MPI
    // distribute flag
    fca_utils::AssertMpi(MPI_Bcast(&bWriteMI, 1, MPI_INT, FCA_MASTER_RANK, MPI_COMM_WORLD), __FILE__, __LINE__);
    fca_utils::AssertMpi(MPI_Bcast(&log_kappa1, 1, MPI_DOUBLE, FCA_MASTER_RANK, MPI_COMM_WORLD), __FILE__, __LINE__);
    fca_utils::AssertMpi(MPI_Bcast(&log_kappa2, 1, MPI_DOUBLE, FCA_MASTER_RANK, MPI_COMM_WORLD), __FILE__, __LINE__);
    fca_utils::AssertMpi(MPI_Bcast(&bBasicHisto, 1, MPI_INT, FCA_MASTER_RANK, MPI_COMM_WORLD), __FILE__, __LINE__);
#endif
    // compute MI matrix
    if (bWriteMI)
    {
        if (mpi.isMaster())
        {
            fprintf(stdout, "going to compute MI Matrix for output ....\n");
        }
        ...
        if (bBasicHisto)
        {
            FcaEntropyHisto::Basic_compute_MI_matrix(&fca, log_kappa1, log_kappa2);
        }
        else
        {
