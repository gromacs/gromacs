...
class CstdioOstreamTest : public ::testing::Test
{
    public:
        CstdioOstreamTest()
            : debug(&gmx::test::cnullpt), error(&gmx::test::cnullpt),
              stdout_buf_(stdout), stderr_buf_(stderr),
              stdout_str_(&stdout_buf_), stderr_str_(&stderr_buf_),
              stdout_str_pt_(&stdout_str_), stderr_str_pt_(&stderr_str_),
        ...
        {
            debug.setStream(&stdout_str_pt_);
        }
        void activateErrorOut()
        {
            error.setStream(&stderr_str_pt_);
        }
        void deactivateDebugOut()
        {
            debug.setStream(&gmx::test::cnullpt);
        }
        void deactivateErrorOut()
        {
            error.setStream(&gmx::test::cnullpt);
        }

        gmx::test::TestFileManager   fileManager_;
        // error stream, silent by default
        gmx::test::StreamAdapter     debug;
        // debugging output stream, silent by default
        gmx::test::StreamAdapter     error;
    private:
        // stream buffer that uses fprintf for output
        gmx::test::CstdioStreamBuffer<255> stdout_buf_;
        // stream buffer that uses fprintf for output
        gmx::test::CstdioStreamBuffer<255> stderr_buf_;
        // regular output stream
        std::ostream                       stdout_str_;
        // stream for error otput
        std::ostream                       stderr_str_;
        // regular output stream
        std::ostream*                      stdout_str_pt_;
        // stream for error otput
        std::ostream*                      stderr_str_pt_;
