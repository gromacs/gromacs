                  ...
                  const ConfMatchSettings  &settings)
{
    ...
    // Default is to just check the first two lines of the output file
    std::string          line;
                  ...
                  const ConfMatchSettings  &settings)
{
    ...
    if (settings.fullCheck_)
    {
        ...
        {
            line = stripSuffixIfPresent(line, "\n");
            if (contains(line, "ATOM") || contains(line, "HETATM"))
            {
                std::vector<std::string> tokens = splitString(line);
                int entries                     = 0;
                for (const auto &token : tokens)
            ...
            {
                std::string id = formatString("non-ATOM entry %d", lines);
