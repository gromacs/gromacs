...
/*! \brief
 * Assert for predicates that return ::testing::AssertionResult and produce
 * a full failure message.
 *
 * `expr` should evaluate to ::testing::AssertionResult, and on failure the
 * message from the result is used as-is, unlike in EXPECT_TRUE().
 *
 * \hideinitializer
 */
 ...
 *
 * `expr` should evaluate to ::testing::AssertionResult, and on failure the
 * message from the result is used as-is, unlike in ASSERT_TRUE().
 *
 * \hideinitializer
 */
