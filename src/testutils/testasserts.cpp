 ...
 */
template <typename FloatType>
gmx_int64_t calculateUlpDifference(const FloatingPoint<FloatType> &value1,
...
void initDifference(FloatType raw1, FloatType raw2, double *absoluteDifference,
                    gmx_int64_t *ulpDifference, bool *bSignDifference)
{
    ...
    *bSignDifference    = (raw1 * raw2 < 0); //(value1.sign_bit() != value2.sign_bit());
...
{
    return formatString("%g (%" GMX_PRId64 " %s-prec. ULPs)%s",
        ...
        const FloatingPointDifference &difference) const
{
    ...
    if (difference.asUlps() <= ulpTolerance_)
    {
        return true;
    }
    return false;
...
std::string FloatingPointTolerance::toString() const
{
    ...
    if (ulpTolerance_ >= 0)
    {
        ...
        result.append(formatString("%" GMX_PRId64 " ULPs", ulpTolerance_));
    }
    return result;
}
