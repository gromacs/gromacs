...
/*! \brief
 * Returns the number of thread-MPI ranks to use for an MPI test.
 *
 * This can only be called if GMX_THREAD_MPI is on, otherwise a linker failure
 * results.
 *
 * \ingroup module_testutils
 */
 ...
 *  - Put GMX_MPI_TEST() as the first statement in your test body.
 *  - Declare your unit test in CMake with gmx_add_mpi_unit_test().
 *    Note that with this option, all tests in the binary should fulfill
 *    the conditions above, and work with the same number of ranks.
 *
