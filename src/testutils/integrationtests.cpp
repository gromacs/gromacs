...
void
IntegrationTestFixture::redirectStdoutToDevNull()
{
    if (NULL == std::freopen("/dev/null", "w", stdout))
    {
        int cachedErrno = errno;
        GMX_THROW_WITH_ERRNO(FileIOError("Failed to redirect stdout to /dev/null"),
                             "freopen",
                             cachedErrno
                             );
    }
...
void
IntegrationTestFixture::redirectStderrToDevNull()
{
    if (NULL == std::freopen("/dev/null", "w", stderr))
    {
        int cachedErrno = errno;
        GMX_THROW_WITH_ERRNO(FileIOError("Failed to redirect stderr to /dev/null"),
                             "freopen",
                             cachedErrno
                             );
    }
