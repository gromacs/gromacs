...
int IntegrationTestFixture::maxBackup = 1;
bool IntegrationTestFixture::deleteTemporaryFiles = true;

GMX_TEST_OPTIONS(IntegrationTestOptions, options)
{
    options->addOption(IntegerOption("max-backup")
                       .store(&IntegrationTestFixture::maxBackup)
                       .description("Maximum number of backup files to write (-1 prevents backups being created)"));
    options->addOption(BooleanOption("delete-temporary-files")
                       .store(&IntegrationTestFixture::deleteTemporaryFiles)
                       .description("At the end of each test case, delete temporary and output files"));
}

IntegrationTestFixture::IntegrationTestFixture()
    : fileManager_(deleteTemporaryFiles)
{
}
