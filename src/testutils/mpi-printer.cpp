...
{
    public:
        MPIEventForward(TestEventListener* defaultPrinter, int rank)
            : defaultPrinter_(defaultPrinter), rank_(rank)
        {
        }
...
{
    int all_passed = true, local_passed = true;

    /* This printing races on stdout. Don't think there's an easy way
       to fix that in MPI 1, and not sure it's worth the price if that
       price is lots of barriers over lots of test cases. */
...
void gmx::test::initMPIOutput()
{
    ...
    listeners.Append(new MPIEventForward(defprinter, rank));
#endif
}
