                     ...
                     t_nrnb * gmx_restrict                  nrnb)
{
    ...
    for(iidx=0; iidx<nri; iidx++)
    {
        ...
        if(jidx<j_index_end)
        {
            if(jidx<j_index_end-2)
            {
                ...
                j_coord_offsetC  = DIM*jnrC;
                /* load j atom coordinates */
                gmx_mm_load_3rvec_3ptr_swizzle_ps(x+j_coord_offsetA,x+j_coord_offsetB,
                                                  x+j_coord_offsetC,
                                                  &jx0,&jy0,&jz0,&jx1,&jy1,&jz1,&jx2,&jy2,&jz2);
                mask             = mask3;
            }
            else if(jidx<j_index_end-1)
            {
                ...
                j_coord_offsetB  = DIM*jnrB;
                /* load j atom coordinates */
                gmx_mm_load_3rvec_2ptr_swizzle_ps(x+j_coord_offsetA,x+j_coord_offsetB,
                                                  &jx0,&jy0,&jz0,&jx1,&jy1,&jz1,&jx2,&jy2,&jz2);
                mask             = mask2;
            }
            else
            {
                ...
                j_coord_offsetA  = DIM*jnrA;
                /* load j atom coordinates */
                gmx_mm_load_3rvec_1ptr_swizzle_ps(x+j_coord_offsetA,
                                                  &jx0,&jy0,&jz0,&jx1,&jy1,&jz1,&jx2,&jy2,&jz2);
                mask             = mask1;
            }
            ...
            /* Calculate table index by multiplying r with table scale and truncate to integer */
            rt               = _mm_mul_ps(_mm_and_ps(r00,mask),vftabscale);
            vfitab           = _mm_cvttps_epi32(rt);
            vfeps            = _mm_sub_ps(rt,_mm_cvtepi32_ps(vfitab));
            vfitab           = _mm_slli_epi32(vfitab,2);
            ...
            /* Calculate table index by multiplying r with table scale and truncate to integer */
            rt               = _mm_mul_ps(_mm_and_ps(r01,mask),vftabscale);
            vfitab           = _mm_cvttps_epi32(rt);
            vfeps            = _mm_sub_ps(rt,_mm_cvtepi32_ps(vfitab));
            vfitab           = _mm_slli_epi32(vfitab,2);
            ...
            /* Calculate table index by multiplying r with table scale and truncate to integer */
            rt               = _mm_mul_ps(_mm_and_ps(r02,mask),vftabscale);
            vfitab           = _mm_cvttps_epi32(rt);
            vfeps            = _mm_sub_ps(rt,_mm_cvtepi32_ps(vfitab));
            vfitab           = _mm_slli_epi32(vfitab,2);
            ...
            /* Calculate table index by multiplying r with table scale and truncate to integer */
            rt               = _mm_mul_ps(_mm_and_ps(r10,mask),vftabscale);
            vfitab           = _mm_cvttps_epi32(rt);
            vfeps            = _mm_sub_ps(rt,_mm_cvtepi32_ps(vfitab));
            vfitab           = _mm_slli_epi32(vfitab,2);
            ...
            /* Calculate table index by multiplying r with table scale and truncate to integer */
            rt               = _mm_mul_ps(_mm_and_ps(r11,mask),vftabscale);
            vfitab           = _mm_cvttps_epi32(rt);
            vfeps            = _mm_sub_ps(rt,_mm_cvtepi32_ps(vfitab));
            vfitab           = _mm_slli_epi32(vfitab,2);
            ...
            /* Calculate table index by multiplying r with table scale and truncate to integer */
            rt               = _mm_mul_ps(_mm_and_ps(r12,mask),vftabscale);
            vfitab           = _mm_cvttps_epi32(rt);
            vfeps            = _mm_sub_ps(rt,_mm_cvtepi32_ps(vfitab));
            vfitab           = _mm_slli_epi32(vfitab,2);
            ...
            /* Calculate table index by multiplying r with table scale and truncate to integer */
            rt               = _mm_mul_ps(_mm_and_ps(r20,mask),vftabscale);
            vfitab           = _mm_cvttps_epi32(rt);
            vfeps            = _mm_sub_ps(rt,_mm_cvtepi32_ps(vfitab));
            vfitab           = _mm_slli_epi32(vfitab,2);
            ...
            /* Calculate table index by multiplying r with table scale and truncate to integer */
            rt               = _mm_mul_ps(_mm_and_ps(r21,mask),vftabscale);
            vfitab           = _mm_cvttps_epi32(rt);
            vfeps            = _mm_sub_ps(rt,_mm_cvtepi32_ps(vfitab));
            vfitab           = _mm_slli_epi32(vfitab,2);
            ...
            /* Calculate table index by multiplying r with table scale and truncate to integer */
            rt               = _mm_mul_ps(_mm_and_ps(r22,mask),vftabscale);
            vfitab           = _mm_cvttps_epi32(rt);
            vfeps            = _mm_sub_ps(rt,_mm_cvtepi32_ps(vfitab));
            vfitab           = _mm_slli_epi32(vfitab,2);
            ...
            if(jidx<j_index_end-2)
            {
                gmx_mm_decrement_3rvec_3ptr_swizzle_ps(f+j_coord_offsetA,f+j_coord_offsetB,
                                                       f+j_coord_offsetC,
                                                       fjx0,fjy0,fjz0,fjx1,fjy1,fjz1,fjx2,fjy2,fjz2);
            }
            else if(jidx<j_index_end-1)
            {
                gmx_mm_decrement_3rvec_2ptr_swizzle_ps(f+j_coord_offsetA,f+j_coord_offsetB,
                                                       fjx0,fjy0,fjz0,fjx1,fjy1,fjz1,fjx2,fjy2,fjz2);
            }
            else
            {
                gmx_mm_decrement_3rvec_1ptr_swizzle_ps(f+j_coord_offsetA,
                                                       fjx0,fjy0,fjz0,fjx1,fjy1,fjz1,fjx2,fjy2,fjz2);
            }
        }
    ...
    inc_nrnb(nrnb,eNR_NBKERNEL_ELEC_W3W3_VF,outeriter*28 + inneriter*387);
}
/*
                     ...
                     t_nrnb * gmx_restrict                  nrnb)
{
    ...
    inc_nrnb(nrnb,eNR_NBKERNEL_ELEC_W3W3_F,outeriter*27 + inneriter*351);
}
