...
#include "nb_kernel_sse_single.h"

/* Include single precision SSE intrinsics kernel headers in local directory */
#include "nb_kernel400_sse_single.h"
#include "nb_kernel410_sse_single.h"
#include "nb_kernel430_sse_single.h"
...
static nb_kernel_t *
kernellist_sse_single[eNR_NBKERNEL_NR] = 
{
    NULL,
    NULL,
    NULL,
    ...
    NULL,
    NULL,
    nb_kernel400_sse_single,
    nb_kernel410_sse_single,
    nb_kernel430_sse_single
};
 ...
 */
int 
nb_kernel_sse_single_test(FILE *                log)
{
	unsigned int level;
	...
	if(NULL != log)
    {
		fprintf(log,"Checking CPU SSE support... ");
    }
...
nb_kernel_setup_sse_single(FILE *log,nb_kernel_t **list)
{
    ...
    if(nb_kernel_sse_single_test(log) != 0)
    {
		return;
    }
