...
static int                  kernel_list_size      = 0;
static int *                kernel_list_hash      = NULL;
static int                  kernel_list_hash_size = 0;
                    ...
                    const char *   vf)
{
    ...
    /* hash function from Dan J. Bernstein.
     * We only include alphanumerical characters, and neglect case. 
     */
    while ((c = toupper(*arch++)) != '\0')
    {
        if(isalnum(c)) hash = ((hash << 5) + hash) ^ c; /* (hash * 33) xor c */
    }
    
    while ((c = toupper(*elec++)) != '\0')
    {
        if(isalnum(c)) hash = ((hash << 5) + hash) ^ c; /* (hash * 33) xor c */
    }
    while ((c = toupper(*vdw++)) != '\0')
    {
        if(isalnum(c)) hash = ((hash << 5) + hash) ^ c; /* (hash * 33) xor c */
    }
    while ((c = toupper(*geom++)) != '\0')
    {
        if(isalnum(c)) hash = ((hash << 5) + hash) ^ c; /* (hash * 33) xor c */
    }
    while ((c = toupper(*other++)) != '\0')
    {
        if(isalnum(c)) hash = ((hash << 5) + hash) ^ c; /* (hash * 33) xor c */
    }
    while ((c = toupper(*vf++)) != '\0')
    {
        if(isalnum(c)) hash = ((hash << 5) + hash) ^ c; /* (hash * 33) xor c */
    }
    return hash;
...
nb_kernel_list_hash_init(void)
{
    ...
    for(i=0;i<kernel_list_size;i++)
    {
        ...
        {
            index = ++index % kernel_list_hash_size;
        }
                          ...
                          const char *        vf)
{
    ...
    {
        fprintf(log,
                "NB kernel %s() used for architecture '%s' with\n"
                "    Elec='%s', VdW='%s'\n"
                "    Geom='%s', Other='%s', Calc='%s'\n\n",
                kernelinfo_ptr->kernelname,arch,electrostatics,vdw,geometry,other,vf);
    }
