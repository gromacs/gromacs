                              ...
                              float * gmx_restrict ptrD, __m128 xmm1)
{
    ...
    t3       = _mm_shuffle_ps(xmm1,xmm1,_MM_SHUFFLE(2,2,2,2));              
    t4       = _mm_shuffle_ps(xmm1,xmm1,_MM_SHUFFLE(3,3,3,3)); 
...
gmx_mm_load_1rvec_broadcast_ps(const float * gmx_restrict ptrA,
                               __m128 * gmx_restrict x, __m128 * gmx_restrict y, __m128 * gmx_restrict z)
{
    __m128 t1;
    
    t1 = _mm_loadu_ps(ptrA);
    
    *x = _mm_permute_ps(t1,_MM_SHUFFLE(0,0,0,0));
    *y = _mm_permute_ps(t1,_MM_SHUFFLE(1,1,1,1));
    *z = _mm_permute_ps(t1,_MM_SHUFFLE(2,2,2,2));
}
...
static gmx_inline __m128
gmx_mm_load_1rvec_1ptr_noswizzle_ps(const float * gmx_restrict p1)
{
    return _mm_maskload_ps(p1,_mm_set_epi32(0,-1,-1,-1) );
}


static gmx_inline void
gmx_mm_load_3rvec_1ptr_noswizzle_ps(const float * gmx_restrict ptrA,
                                    __m128 * gmx_restrict xyz1, __m128 * gmx_restrict xyz2, __m128 * gmx_restrict xyz3)
{
    __m128 t1,t2;
    __m128 tA,tB,tC;
    ...
    *xyz1 = t1;
    ...
    tB = _mm_blend_ps(t2,t1,_GMX_MM_BLEND(1,0,0,0)); /* x2 x3 z2 y2 */
    *xyz2 = _mm_permute_ps(tB,_MM_SHUFFLE(0,1,0,3)); /* - z2 y2 x2 */
    tC = _mm_load_ss(ptrA+8);
    *xyz3 = _mm_shuffle_ps(t2,tC,_MM_SHUFFLE(0,0,3,2)); /* - z3 y3 x3 */
}

static gmx_inline void
gmx_mm_load_4rvec_1ptr_noswizzle_ps(const float * gmx_restrict ptrA,
                                    __m128 * gmx_restrict xyz1, __m128 * gmx_restrict xyz2,
                                    __m128 * gmx_restrict xyz3, __m128 * gmx_restrict xyz4)
{
    __m128 t1,t2,t3;
    __m128 tA,tB,tC;
    ...
    *xyz4 = _mm_permute_ps(t3,_MM_SHUFFLE(0,3,2,1)); /* - z4 y4 x4 */
}
                                         ...
                                         __m128 xyz1, __m128 xyz2, __m128 xyz3)
{
    ...
    t1   = _mm_permute_ps(xyz2,_MM_SHUFFLE(0,0,2,1)); /* x2 - z2 y2 */
    t2   = _mm_permute_ps(xyz3,_MM_SHUFFLE(1,0,0,2)); /* y3 x3 - z3 */
                                         ...
                                         __m128 xyz1, __m128 xyz2, __m128 xyz3, __m128 xyz4)
{
    ...
    t3   = _mm_blend_ps(t1,xyz1,_GMX_MM_BLEND(0,1,1,1)); /* x2 z1 y1 x1 */
    t4   = _mm_blend_ps(t1,t2,_GMX_MM_BLEND(1,1,0,0)); /* y3 x3 z2 y2 */
    t5   = _mm_permute_ps(xyz4,_MM_SHUFFLE(2,1,0,0)); /* z4 y4 x4 - */
