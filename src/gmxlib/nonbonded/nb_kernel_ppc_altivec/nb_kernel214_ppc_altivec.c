...
void 
nb_kernel214_ppc_altivec  (int *             p_nri,
                       int               iinr[],
                       int               jindex[],
                       int               jjnr[],
                       int               shift[],
					   ...
					   float *           work)
{
	...
	vector float rinv22,rinv23,rinv24,rinv32,rinv33,rinv34,rinv42,rinv43,rinv44;
	vector float rinvsq11,rinvsix,rinvsq22,rinvsq23,rinvsq24;
	...
	vector float vctot,Vvdwtot,Vvdw6,Vvdw12,qqMM,qqMH,qqHH,qM,qH;
		...
		*count           = nn1;
		...
		for(n=0;n<nri;n++) {
			...
			for(k=nj0; k<(nj1-3); k+=4) {
				...
				fs11            = vec_madd(vec_twelve(),Vvdw12,nul);
				fs11            = vec_nmsub(vec_six(),Vvdw6,fs11);
			...
			if(k<(nj1-2)) {
				...
				zero_highest_element_in_vector(&rinvsq11);
				...
				Vvdw6            = vec_madd(c6t,rinvsix,nul);
				Vvdw12           = vec_madd(c12t,vec_madd(rinvsix,rinvsix,nul),
										   nul);
				Vvdwtot          = vec_add(Vvdwtot,Vvdw12);
				Vvdwtot          = vec_sub(Vvdwtot,Vvdw6);
				...
				zero_highest_2_elements_in_vector(&rinvsq11);
				...
				zero_highest_3_elements_in_vector(&rinvsq11);
...
void 
nb_kernel214nf_ppc_altivec(int *             p_nri,
                       int               iinr[],
                       int               jindex[],
                       int               jjnr[],
                       int               shift[],
					   ...
					   float *           work)
{
	...
	vector float rsq11,rsq22,rsq23,rsq24,rsq32,rsq33,rsq34,rsq42,rsq43,rsq44;
	vector float rinv22,rinv23,rinv24,rinv32,rinv33,rinv34;
	vector float rinv42,rinv43,rinv44;

	vector float vfacel,nul,rinvsq11,rinvsix;
		...
		*count           = nn1;
		...
		for(n=0;n<nri;n++) {
			...
			if(k<(nj1-2)) {
				...
				Vvdwtot          = vec_madd(c12t,vec_madd(rinvsix,rinvsix,nul),
										   Vvdwtot);
