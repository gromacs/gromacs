                          ...
                          int *                inneriter)
{
     ...
     for(n=0; (n<nlist->nri); n++)
     {
         ...
         for(k=nj0; (k<nj1); k++)
         {
            ...
            {
                /* necessary since wf defaults to 1 as is not changed for eAdressConst */
                weight_product    = const_wf;
                weight_product_cg = 1.0 - const_wf;
            }
            else
            {
		...
		/* if it's a explicit loop, include this molecule */
            ...
            else 
            {
                ...
                if (adress_type == eAdressConst)
                {
                     ...
                     * See apply_thermo_ic function in src/mdlib/adress.c for more details. */
                    hybscal = const_wf;
                }
                else
                {
