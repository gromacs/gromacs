...
#define  NSTATES  2
    ...
    for(n=0; (n<nri); n++)
    {
        ...
        for(k=nj0; (k<nj1); k++)
        {
            ...
            {
                rp             = pow(r,sc_r_power);  /* not currently supported, but can handle it */
                rpm2           = rp/rsq;
            }
            ...
            for (i=0;i<NSTATES;i++) 
            {
                ...
                dalf_coul[i] = DLF[i]*alpha_coul_eff*lam_power/sc_r_power*(lam_power==2 ? (1-LFC[i]) : 1); 
                alf_vdw[i]   = alpha_vdw_eff *(lam_power==2 ? (1-LFV[i])*(1-LFV[i]) : (1-LFV[i]));
                dalf_vdw[i]  = DLF[i]*alpha_vdw_eff*lam_power/sc_r_power*(lam_power==2 ? (1-LFV[i]) : 1); 
