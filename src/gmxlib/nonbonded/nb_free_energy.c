                          ...
                          int *                inneriter)
{
    ...
    /* Avoid warnings - refH/Vewald are always set in a loop when i=0, and
       used for i>0. 
     */
    refH = Vewald = 0.0;
    ...
    for(n=0; (n<nri); n++)
    {
        ...
        for(k=nj0; (k<nj1); k++)
        {
            ...
            for(i=0;i<nlambda;i++)
            {
                alfA  = alpha*(lam_power==2 ? lambda[i]*lambda[i] : lambda[i]);
                alfB  = alpha*(lam_power==2 ? (1.0-lambda[i])*(1.0-lambda[i]) : (1.0-lambda[i]));
                dalfA = alpha*lam_power/6.0*(lam_power==2 ? lambda[i] : 1); 
                dalfB = alpha*lam_power/6.0*(lam_power==2 ? (1.0-lambda[i]) : 1); 
                ...
                if(i==0)
                {
                    /* reference state, for which we also calculate F and dvdl */
                    Fscal = 0;
                    /* Assemble A and B states */
                    vctot         += L*VcoulB + L1*VcoulA;
                    Vvdwtot       += L*VvdwB  + L1*VvdwA;
                    
                    refH           = L*(VcoulB+VvdwB) + L1*(VcoulA+VvdwA);                    
                    
                    Fscal         += (L1*FscalA*rinv4A + L*FscalB*rinv4B)*r4;
                    dvdl          += (VcoulB + VvdwB) - (VcoulA + VvdwA);
                    dvdl          += L*dalfB*FscalB*sigma6b*rinv4B
                    ...
                    if(icoul==5)
                    {
                        ...
                        */
                        if (r != 0) 
                        {
                            Vewald = erf(ewc*r)*rinv;
                            FF     = rinv*rinv*(Vewald - 2.0*ewc*isp*exp(-ewc*ewc*rsq));
                        }
                        else 
                        {
                            Vewald = ewc*2.0/sqrt(M_PI);
                            FF     = 0;
                        }
                        vctot  -= (L*qqB + L1*qqA)*Vewald;
                        Fscal  -= (L*qqB + L1*qqA)*FF;
                        dvdl   -= (qqB - qqA)*Vewald;
                        
                        refH   -= (L*qqB + L1*qqA)*Vewald;
                    
                    }
                
                    /* accumulate forces for i==0 */
                    tx             = Fscal*dx;     
                    ty             = Fscal*dy;     
                    tz             = Fscal*dz;     
                    fix            = fix + tx;      
                ...
                {
                    /* Additional deltaH states for i != 0 */
                    
                    /* refH was calculated when i==0. */
                    deltaH[i] += lambda[i]*(VcoulB+VvdwB) + (1.0-lambda[i])*(VcoulA+VvdwA) - refH;
                    
                    /* Vewald was set for i==0. */
                    if(icoul==5)
                    {
                        deltaH[i] -= (lambda[i]*qqB + (1.0-lambda[i])*qqA)*Vewald;
                    }          
                }
            }                                                 
        }
