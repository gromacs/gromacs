...
#define  NSTATES  2
     ...
     *
     * However, we cannot use this approach for modified interactions (such
     * as switch/shift or exact cutoff) since we would then effectively end up
     * evaluating a significantly different interaction here compared to the
     * normal (non-free-energy) kernels, either by applying a cutoff at a different
     * position than what the user requested, or by switching/shifting different
     * things (1/r rather than short-range Ewald). For these settings, we just
     * use the traditional short-range Ewald interaction.
     */
    bConvertEwaldToCoulomb = (icoul == GMX_NBKERNEL_ELEC_EWALD && fr->coulomb_modifier == eintmodNONE);
        ...
        for (k = nj0; (k < nj1); k++)
        {
            ...
            for (i = 0; i < NSTATES; i++)
            {
                ...
                if ( (qq[i] != 0) || (c6[i] != 0) || (c12[i] != 0) )
                {
                     ...
                     * or if we are within the cutoff.
                     */
                    if ( (qq[i] != 0) && ( !bExactElecCutoff || rC < rcoulomb ) )
                    {
                        switch (icoul)
                        {
                            ...
                            case GMX_NBKERNEL_ELEC_EWALD:
                                ...
                                else
                                {
                                    ...
                                    rinvcorr  = (fr->coulomb_modifier == eintmodPOTSHIFT) ? rinvC-sh_ewald : rinvC;
            ...
            if(bConvertEwaldToCoulomb)
            {
                /* See comment in the preamble. When using Ewald interactions without
                 * modifiers we subtract the reciprocal-space Ewald component here
                 * which made it possible to apply the free energy interaction to
                 * 1/r (vanilla coulomb short-range part) above.
                 * Presently this only works without interaction modifiers
                 * (including the exact-cutoff one), so there is no need to check
                 * for the cutoff settings or distance.
                 */
                real v_lr,f_lr;
