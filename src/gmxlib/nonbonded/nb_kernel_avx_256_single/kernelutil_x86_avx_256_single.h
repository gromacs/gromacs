                                ...
                                __m256 * gmx_restrict c6, __m256 * gmx_restrict c12)
{
    ...
    t1   = _mm_loadu_ps(p1);   /* - - c12a  c6a */
    t2   = _mm_loadu_ps(p2);   /* - - c12b  c6b */
    t3   = _mm_loadu_ps(p3);   /* - - c12c  c6c */
    t4   = _mm_loadu_ps(p4);   /* - - c12d  c6d */
                                          ...
                                          __m256 x1, __m256 y1, __m256 z1) 
{
    ...
    tA          = gmx_mm256_set_m128(_mm_loadu_ps(ptrE),_mm_loadu_ps(ptrA));
    tB          = gmx_mm256_set_m128(_mm_loadu_ps(ptrF),_mm_loadu_ps(ptrB));
    tC          = gmx_mm256_set_m128(_mm_loadu_ps(ptrG),_mm_loadu_ps(ptrC));
    tD          = gmx_mm256_set_m128(_mm_loadu_ps(ptrH),_mm_loadu_ps(ptrD));
                                      ...
                                      float * gmx_restrict fshiftptr)
{
    ...
    t1   = _mm_permute_ps(t1,_MM_SHUFFLE(0,3,2,0));
    t1   = _mm_blend_ps(_mm_setzero_ps(),t1,0x7);
    
    t2   = _mm_loadu_ps(fptr);
    t3   = _mm_loadu_ps(fshiftptr);
    
    t2   = _mm_add_ps(t2,t1);
    t3   = _mm_add_ps(t3,t1);

    _mm_storeu_ps(fptr,t2);
    _mm_storeu_ps(fshiftptr,t3);
}
