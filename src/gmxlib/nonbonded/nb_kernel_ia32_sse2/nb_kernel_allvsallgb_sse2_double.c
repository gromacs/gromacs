                                   ...
                                   int                     natoms)
{
     ...
     * 5 will doublely start on atom 4. In addition, the kernel will be unrolled both in I and J (think 4*4 tiles),
             ...
             double *                pvdwaram)
{
	...
	*p_aadata = aadata;
	...
	snew(pr,2*natoms+2*SIMD_WIDTH);
	aadata->invsqrta_align = (double *) (((size_t) pr + 16) & (~((size_t) 15)));	
	snew(pr,2*natoms+2*SIMD_WIDTH);
	aadata->dvda_align = (double *) (((size_t) pr + 16) & (~((size_t) 15)));	
                                 ...
                                 void *                 work)
{
	...
	__m128d    fjx_SSE,fjy_SSE,fjz_SSE;
	__m128d    jx_SSE,jy_SSE,jz_SSE,jq_SSE;
    ...
    __m128d    Y_SSE0,F_SSE0,G_SSE0,H_SSE0;
    __m128d    Y_SSE1,F_SSE1,G_SSE1,H_SSE1;
    __m128d    r_SSE0,r_SSE1;
    __m128d    rtab_SSE0,rtab_SSE1;
    __m128d    eps_SSE0,eps_SSE1;
    __m128d    vgb_SSE0,vgb_SSE1;
    __m128d    fijGB_SSE0,fijGB_SSE1;
    __m128d    isaprod_SSE0,isaprod_SSE1;
    __m128d    isai_SSE,isai_SSE0,isai_SSE1;
    __m128d    dvdatmp_SSE0,dvdatmp_SSE1;
    __m128d    gbscale_SSE0,gbscale_SSE1;
    __m128d    dvdasum_SSE0,dvdasum_SSE1;
    __m128d    gbtabscale_SSE,gbfactor_SSE;
	__m128d    vctot_SSE,vvdwtot_SSE,vgbtot_SSE,isaj_SSE;
	__m128d    half_SSE,two_SSE,six_SSE,twelve_SSE;
	__m128d    imask_SSE0,imask_SSE1;
    __m128d    jmask_SSE0,jmask_SSE1;
    __m128i    n0_SSE0,n0_SSE1;
    __m128i    nnn_SSE0,nnn_SSE1;
    __m128d    tmpSSE;
    ...
    six_SSE        = _mm_set1_pd(6.0);
	twelve_SSE     = _mm_set1_pd(12.0);
    gbfactor_SSE   = _mm_set1_pd( -gbfactor ); /* Yes, sign should be negative (saves 1 SSE instruction) */
    gbtabscale_SSE = _mm_set1_pd(fr->gbtab.scale);
	...
	for(i=ni0; i<ni1; i+=UNROLLI)
	{
		...
		vvdwtot_SSE      = _mm_setzero_pd();
		vctot_SSE        = _mm_setzero_pd();
        vgbtot_SSE       = _mm_setzero_pd();
        ...
        dvdasum_SSE0       = _mm_setzero_pd();
        dvdasum_SSE1       = _mm_setzero_pd();
        ...
        {            
            ...
            jq_SSE             = _mm_load_pd(q_align+j);
            qq_SSE0            = _mm_mul_pd(iq_SSE0,jq_SSE);
            qq_SSE1            = _mm_mul_pd(iq_SSE1,jq_SSE);
            qq_SSE0            = _mm_and_pd(qq_SSE0,jmask_SSE0);
            qq_SSE1            = _mm_and_pd(qq_SSE1,jmask_SSE1);
            ...
            isaj_SSE           = _mm_load_pd(invsqrta_align+j);
            isaprod_SSE0       = _mm_mul_pd(isai_SSE0,isaj_SSE);
            isaprod_SSE1       = _mm_mul_pd(isai_SSE1,isaj_SSE);
            ...
            fscal_SSE0         = _mm_mul_pd(vcoul_SSE0,rinv_SSE0);
            fscal_SSE1         = _mm_mul_pd(vcoul_SSE1,rinv_SSE1);
                        
            vctot_SSE          = _mm_add_pd(vctot_SSE, _mm_add_pd(vcoul_SSE0,vcoul_SSE1));
            
            /* Polarization interaction */
            qq_SSE0            = _mm_mul_pd(qq_SSE0,isaprod_SSE0);
            qq_SSE1            = _mm_mul_pd(qq_SSE1,isaprod_SSE1);
            qq_SSE0            = _mm_mul_pd(qq_SSE0,gbfactor_SSE);
            qq_SSE1            = _mm_mul_pd(qq_SSE1,gbfactor_SSE);
            
            gbscale_SSE0       = _mm_mul_pd(isaprod_SSE0,gbtabscale_SSE);
            gbscale_SSE1       = _mm_mul_pd(isaprod_SSE1,gbtabscale_SSE);
            
            /* Use tables for (rescaled) GB potential */
            r_SSE0             = _mm_mul_pd(rsq_SSE0,rinv_SSE0);
            r_SSE1             = _mm_mul_pd(rsq_SSE1,rinv_SSE1);
            rtab_SSE0          = _mm_mul_pd(r_SSE0,gbscale_SSE0);
            rtab_SSE1          = _mm_mul_pd(r_SSE1,gbscale_SSE1);
            
            n0_SSE0            = _mm_cvttpd_epi32(rtab_SSE0);
            n0_SSE1            = _mm_cvttpd_epi32(rtab_SSE1);
            eps_SSE0           = _mm_sub_pd(rtab_SSE0 , _mm_cvtepi32_pd(n0_SSE0) );
            eps_SSE1           = _mm_sub_pd(rtab_SSE1 , _mm_cvtepi32_pd(n0_SSE1) );
            ...
            Y_SSE0             = _mm_load_pd(GBtab + gmx_mm_extract_epi32(nnn_SSE0,0)); /* YF */
            F_SSE0             = _mm_load_pd(GBtab + gmx_mm_extract_epi32(nnn_SSE0,1)); /* YF */
            tmpSSE             = Y_SSE0;
            Y_SSE0             = _mm_unpacklo_pd(Y_SSE0,F_SSE0);
            F_SSE0             = _mm_unpackhi_pd(tmpSSE,F_SSE0);
            
            G_SSE0             = _mm_load_pd(GBtab + gmx_mm_extract_epi32(nnn_SSE0,0)+2); /* GH */
            H_SSE0             = _mm_load_pd(GBtab + gmx_mm_extract_epi32(nnn_SSE0,1)+2); /* GH */
            tmpSSE             = G_SSE0;
            G_SSE0             = _mm_unpacklo_pd(G_SSE0,H_SSE0);
            H_SSE0             = _mm_unpackhi_pd(tmpSSE,H_SSE0);
            
            Y_SSE1             = _mm_load_pd(GBtab + gmx_mm_extract_epi32(nnn_SSE1,0)); /* YF */
            F_SSE1             = _mm_load_pd(GBtab + gmx_mm_extract_epi32(nnn_SSE1,1)); /* YF */
            tmpSSE             = Y_SSE1;
            Y_SSE1             = _mm_unpacklo_pd(Y_SSE1,F_SSE1);
            F_SSE1             = _mm_unpackhi_pd(tmpSSE,F_SSE1);
            
            G_SSE1             = _mm_load_pd(GBtab + gmx_mm_extract_epi32(nnn_SSE1,0)+2); /* GH */
            H_SSE1             = _mm_load_pd(GBtab + gmx_mm_extract_epi32(nnn_SSE1,1)+2); /* GH */
            tmpSSE             = G_SSE1;
            G_SSE1             = _mm_unpacklo_pd(G_SSE1,H_SSE1);
            H_SSE1             = _mm_unpackhi_pd(tmpSSE,H_SSE1);
            
            /* G*eps */
            G_SSE0             = _mm_mul_pd(G_SSE0,eps_SSE0);
            G_SSE1             = _mm_mul_pd(G_SSE1,eps_SSE1);
            
            /* H*eps2 */
            H_SSE0             = _mm_mul_pd(H_SSE0, _mm_mul_pd(eps_SSE0,eps_SSE0) );
            H_SSE1             = _mm_mul_pd(H_SSE1, _mm_mul_pd(eps_SSE1,eps_SSE1) );

            /* Fp=F+G*eps+H*eps2 */
            F_SSE0             = _mm_add_pd(F_SSE0, _mm_add_pd( G_SSE0 , H_SSE0 ) );
            F_SSE1             = _mm_add_pd(F_SSE1, _mm_add_pd( G_SSE1 , H_SSE1 ) );
            
            /* VV=Y+eps*Fp */
            Y_SSE0             = _mm_add_pd(Y_SSE0, _mm_mul_pd(F_SSE0, eps_SSE0));
            Y_SSE1             = _mm_add_pd(Y_SSE1, _mm_mul_pd(F_SSE1, eps_SSE1));

            /* FF = Fp+Geps+2*Heps2 */
            F_SSE0             = _mm_add_pd(F_SSE0, _mm_add_pd(G_SSE0 , _mm_mul_pd(H_SSE0,two_SSE)));
            F_SSE1             = _mm_add_pd(F_SSE1, _mm_add_pd(G_SSE1 , _mm_mul_pd(H_SSE1,two_SSE)));

            vgb_SSE0           = _mm_mul_pd(Y_SSE0, qq_SSE0);
            vgb_SSE1           = _mm_mul_pd(Y_SSE1, qq_SSE1);
            ...
            dvdatmp_SSE0       = _mm_add_pd(dvdatmp_SSE0,dvdatmp_SSE1);
            dvdatmp_SSE0       = _mm_mul_pd(dvdatmp_SSE0, _mm_mul_pd(isaj_SSE,isaj_SSE));
            
            /* update derivative wrt j atom born radius */
            _mm_store_pd(dvda_align+j,
                         _mm_sub_pd( _mm_load_pd(dvda_align+j) , dvdatmp_SSE0 ));
            ...
            vvdwtot_SSE        = _mm_add_pd(vvdwtot_SSE, _mm_add_pd(_mm_sub_pd(vvdw12_SSE0,vvdw6_SSE0),
                                                                    _mm_sub_pd(vvdw12_SSE1,vvdw6_SSE1)));
            

            fscal_SSE0         = _mm_sub_pd(_mm_mul_pd(rinvsq_SSE0, 
                                                       _mm_sub_pd(_mm_mul_pd(twelve_SSE,vvdw12_SSE0),
                                                                  _mm_mul_pd(six_SSE,vvdw6_SSE0))),
                                            _mm_mul_pd( _mm_sub_pd( fijGB_SSE0,fscal_SSE0),rinv_SSE0 ));
            
            fscal_SSE1         = _mm_sub_pd(_mm_mul_pd(rinvsq_SSE1, 
                                                       _mm_sub_pd(_mm_mul_pd(twelve_SSE,vvdw12_SSE1),
                                                                  _mm_mul_pd(six_SSE,vvdw6_SSE1))),
                                            _mm_mul_pd( _mm_sub_pd( fijGB_SSE1,fscal_SSE1),rinv_SSE1 ));
        ...
        {                      
            ...
            qq_SSE0            = _mm_mul_pd(iq_SSE0,jq_SSE);
            qq_SSE1            = _mm_mul_pd(iq_SSE1,jq_SSE);
            qq_SSE0            = _mm_and_pd(qq_SSE0,imask_SSE0);
            qq_SSE1            = _mm_and_pd(qq_SSE1,imask_SSE1);
		...
		_mm_storeu_pd(tmpsum,vctot_SSE);
		vc[ggid]         = vc[ggid] + tmpsum[0]+tmpsum[1];
		
		_mm_storeu_pd(tmpsum,vvdwtot_SSE);
		vvdw[ggid]       = vvdw[ggid] + tmpsum[0]+tmpsum[1];
	
        _mm_storeu_pd(tmpsum,vgbtot_SSE);
		vpol[ggid]       = vpol[ggid] + tmpsum[0]+tmpsum[1];
