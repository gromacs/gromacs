        ...
        for(range=0;range<2;range++)
        {
            ...
            for(i=i0; (i<i1); i++)
            {
                if (nlist[i].nri > 0)
                {
                    ...
                    if(nlist[i].free_energy==0 && (flags & GMX_DONB_FOREIGNLAMBDA))
                    {
                        /* We don't need the non-perturbed interactions */
                        continue;
                    }
                    /*
                    for(ii=0;ii<nlist[i].nri;ii++)
                    {
                        printf("inr[%3d]=%d\n",ii,nlist[i].iinr[ii]);
                        for(jj=nlist[i].jindex[ii];jj<nlist[i].jindex[ii+1];jj++)
                        {
                            printf("    jjnr[%3d] = %d\n",jj,nlist[i].jjnr[jj]);
                        }
                        printf("\n");
