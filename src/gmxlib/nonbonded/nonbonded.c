...
void
gmx_nonbonded_set_kernel_pointers(FILE *log, t_nblist *nl, t_forcerec *fr)
{
    const char *     elec;
    const char *     elec_mod;
    const char *     vdw;
    ...
    else
    {
         ...
         * electrostatics and VdW that use different switch or cutoff distances - since that would require
	...
	if ( (nl->ielec != GMX_NBKERNEL_ELEC_NONE) && (nl->ielecmod == eintmodPOTSWITCH) &&
             ...
             (fr->rcoulomb_switch != fr->rvdw_switch || fr->rcoulomb != fr->rvdw) )
        {
            nl->kernelptr_vf = NULL;
            nl->kernelptr_f  = NULL;
        }
