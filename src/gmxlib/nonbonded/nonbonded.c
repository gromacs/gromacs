                    ...
                    gmx_bool       bGenericKernelOnly)
{
    ...
    if(nonbonded_setup_done==FALSE)
    {
        ...
        /* Add accelerated SSE kernels, etc. here */
                  ...
                  int nls,int eNL,int flags)
{
    ...
    kernel_data.energygrp_elec          = egcoul;
    kernel_data.energygrp_vdw           = egnb;
    kernel_data.energygrp_polarization  = egpol;
	...
	for(n=n0; (n<n1); n++)
	{
        ...
        kernel_data.table_ewald_linear_fdv0 = &nblists->table_ewald_linear_fdv0;
        kernel_data.table_ewald_linear_vf   = &nblists->table_ewald_linear_vf;
        kernel_data.table_ewald_linear_v    = &nblists->table_ewald_linear_v;
        kernel_data.table_ewald_linear_f    = &nblists->table_ewald_linear_f;

		for(i=i0; (i<i1); i++)
		{
			if (flags & GMX_DONB_LR)
			{
			...
			if (nlist->nri > 0)
			{
				kernelptr = (nb_kernel_t *)nlist->kernelptr_vf;
				
                if(nlist->free_energy==0 && (flags & GMX_DONB_FOREIGNLAMBDA))
                {
...
{
    fprintf(stderr,
            "Warning: Listed nonbonded interaction between particles %d and %d\n"
    ...
    fprintf(stderr,
            ...
            "Since we cannot evaluate beyond the table, we skip these interactions until\n"
            "they are inside the table limit again. You will only see this message once,\n"
            "even if it occurs for several interactions.\n");
    fprintf(stderr,
