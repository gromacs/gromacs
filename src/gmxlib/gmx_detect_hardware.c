                                      ...
                                      gmx_bool bUseGPU)
{
    ...
    if (!hwinfo->consistency_checked)
    {
        ...
        hwinfo->consistency_checked = TRUE;
    }
                             ...
                             gmx_bool             *sharedTmpiGpu)
{
    ...
    if (gpu_info->bUserSet)
    {
        ...
        for (i = 0; i < ngpu - 1; i++)
        {
            for (j = i + 1; j < ngpu; j++)
            {
                bAllDifferent   &= gpu_info->cuda_dev_use[i] != gpu_info->cuda_dev_use[j];
                same_count      += gpu_info->cuda_dev_use[i] == gpu_info->cuda_dev_use[j];
            }
        }
    }
    if (sharedTmpiGpu != NULL)
    {
        *sharedTmpiGpu = !bAllDifferent;
    }
                                   ...
                                   const char *gpu_id)
{
    ...
    {
        snew(hwinfo_g, 1);
        hwinfo_g->consistency_checked = FALSE;
