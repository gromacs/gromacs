                                ...
                                const gmx_hw_info_t *hwinfo)
{
    ...
    if (!bCpuHasRdtscp)
    {
        gmx_fatal(FARGS, "This executable was compiled to use code that cannot execute on this hardware. Please either configure for this hardware on a machine that matches it, or consider using the CMake option GMX_DISTRIBUTABLE_BUILD=ON.");
    }
#else
    if (bCpuHasRdtscp)
    {
