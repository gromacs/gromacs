...
static void do_cpt_real_err(XDR *xd,const char *desc,real *f,FILE *list)
{
    bool_t res=0;
...
static void do_cpt_n_rvecs_err(XDR *xd,const char *desc,int n, rvec f[],FILE *list)
{
    ...
    {
        for (j=0; j<DIM; j++)
            do_cpt_real_err(xd, desc, &f[i][j], NULL);
    }
                          ...
                          FILE *list)
{
    ...
    *file_version = cpt_version;
    ...
    if (*file_version >= 15)
    {
        do_cpt_int_err(xd,"ED data sets",nedsam,list);
    }
        ...
        edsamstate_t *EDstate, FILE *list)
{
    ...
    EDstate->bFromCpt = (bRead? TRUE : FALSE);
