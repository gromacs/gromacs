...
void
FortranCInterface_GLOBAL(dsteqr,DSTEQR)(const char *    compz, 
	int *     n, 
	double *  d__, 
	double *  e, 
	double *  z__, 
	...
	int *     info)
{
	...
	*info = -6;
    ...
    if (icompz == 2) {
	FortranCInterface_GLOBAL(dlaset,DLASET)("Full", n, n, &c_b9, &c_b10, &z__[z_offset], ldz);
    }
...
L60:
	...
	if (m == l + 1) {
		...
		work[l] = c__;
		work[*n - 1 + l] = s;
		FortranCInterface_GLOBAL(dlasr,DLASR)("R", "V", "B", n, &c__2, &work[l], &work[*n - 1 + l], &
			z__[l * z_dim1 + 1], ldz);
	    } else {
	...
	g = (d__[l + 1] - p) / (e[l] * 2.);
	r__ = FortranCInterface_GLOBAL(dlapy2,DLAPY2)(&g, &c_b10);
	...
	if (icompz > 0) {
	    mm = m - l + 1;
	    FortranCInterface_GLOBAL(dlasr,DLASR)("R", "V", "B", n, &mm, &work[l], &work[*n - 1 + l], &z__[l 
		    * z_dim1 + 1], ldz);
	}
...
L110:
	...
	if (m == l - 1) {
		...
		work[m] = c__;
		work[*n - 1 + m] = s;
		FortranCInterface_GLOBAL(dlasr,DLASR)("R", "V", "F", n, &c__2, &work[m], &work[*n - 1 + m], &
	...
	if (icompz > 0) {
	    mm = l - m + 1;
	    FortranCInterface_GLOBAL(dlasr,DLASR)("R", "V", "F", n, &mm, &work[m], &work[*n - 1 + m], &z__[m 
		    * z_dim1 + 1], ldz);
	}
...
L160:
	...
	for (ii = 2; ii <= i__1; ++ii) {
		...
		d__[k] = d__[i__];
		d__[i__] = p;
		FortranCInterface_GLOBAL(dswap,DSWAP)(n, &z__[i__ * z_dim1 + 1], &c__1, &z__[k * z_dim1 + 1],
			 &c__1);
	    }
	}
    }
