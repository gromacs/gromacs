...
static int
init_tpxheader(t_tpxheader *tpxheader,
               t_state *state,
               t_inputrec *ir,
               gmx_mtop_t *mtop,
               rvec *f)
{
    ...
    tpxheader->natoms = state->natoms;
    tpxheader->ngtc   = state->ngtc;  /* need to add nnhpres here? */
    tpxheader->fep_state = state->fep_state;
    tpxheader->lambda = state->lambda[efptFEP];
    tpxheader->bIr  = (NULL != ir);
    tpxheader->bTop = (NULL != mtop);
    tpxheader->bX   = (NULL != state->x);
    tpxheader->bV   = (NULL != state->v);
    tpxheader->bF   = (NULL != f);
    tpxheader->bBox = TRUE;
    tpxheader->file_version = tpx_version;
    tpxheader->file_generation = tpx_generation;
		  ...
		  gmx_bool bXVallocated)
{
  ...
  if (!bRead)
  {
      init_tpxheader(&tpxheader, state, ir, mtop, f);
  }
