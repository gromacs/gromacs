...
int gmx_fio_open(const char *fn,char *mode)
{
	...
	if (fn) 
	{
		...
		if (in_ftpset(fio->iFTP,asize(ftpXDR),ftpXDR)) 
		{
			...
			{
				if(newmode[0]=='r') 
				  gmx_fatal(FARGS,"Cannot open file %s for reading - check permissions if it exists.",fn); 
				else
				  gmx_fatal(FARGS,"Cannot open file %s for writing - check your permissions, disk space and/or quota.",fn);
			}
