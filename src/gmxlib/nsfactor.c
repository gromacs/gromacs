...
gmx_nstructurefactors *gmx_nstructurefactors_init(const char *datfn) {
    /* read nsfactor.dat */
    FILE    *fp;
    char    line[STRLEN];
    int     nralloc=10;
...
gmx_sans_t *gmx_sans_init (t_topology *top, gmx_nstructurefactors *gnsf) {
...
gmx_gr_t *calc_pr (gmx_sans_t *gsans, rvec *x, atom_id *index, int isize, double binwidth, gmx_bool bMC, gmx_large_int_t nmc, unsigned int seed) {
    gmx_gr_t    *pr=NULL;
    rvec        xmin, xmax;
    double      rmax;
    int         i,j,d;
...
gmx_sq_t *pr2iq (gmx_gr_t *pr, double start_q, double end_q, double q_step) {
    gmx_sq_t    *sq=NULL;
    int         i,j;
    /* init data */
    snew(sq,1);
