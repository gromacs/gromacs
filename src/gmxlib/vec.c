...
void rvec_add(rvec a,rvec b,rvec c)
{
  real x,y,z;
...
void rvec_sub(rvec a,rvec b,rvec c)
{
  real x,y,z;
...
void svmul(real a,rvec v1,rvec v2)
{
  v2[XX]=a*v1[XX];
  v2[YY]=a*v1[YY];
  v2[ZZ]=a*v1[ZZ];
...
void oprod(rvec a,rvec b,rvec c)
{
...
void m_sub(matrix a,matrix b,matrix dest)
{
  dest[XX][XX]=a[XX][XX]-b[XX][XX];
  dest[XX][YY]=a[XX][YY]-b[XX][YY];
  dest[XX][ZZ]=a[XX][ZZ]-b[XX][ZZ];
...
void msmul(matrix m1,real r1,matrix dest)
{
...
void m_inv(matrix src,matrix dest)
{
...
real _divide(real a,real b,char *file,int line)
{
  if (b == 0.0) 
...
int _mod(int a,int b,char *file,int line)
{
  if (b == 0)
