...
#define CHECK(b) _low_check(b,err_buf,wi)
  ...
  if (ir->efep != efepNO) {
      fep = ir->fepvals;
      /* make sure the frequency of various things being computed are compatible */
      sprintf(err_buf,"nstdhdl (%d) must be an integer multiple of nstfep (%d)",
              fep->nstdhdl,fep->nstfep);
      CHECK((mod(fep->nstdhdl,fep->nstfep)!=0));
      sprintf(err_buf,"nstfep (%d) must be an integer multiple of nstlist (%d)",
              fep->nstfep,ir->nstlist);  
              /* MRS -- cannot handle variable nlist right now. One
                 future workaround for 5.0; have a large value of
                 nlist that is imposed at the correct frequency for
                 nstfep, and have variable nlists happen in
                 between. */
      CHECK((mod(fep->nstdhdl,fep->nstfep)!=0));
  }
  ...
  if ((ir->bSimTemp) || (ir->efep == efepEXPANDED)) {
      ...
      CHECK(!(ir->etc || EI_SD(ir->eI) || ir->eI==eiBD) && (expand->mc_temp <=0) && (expand->elmcmove != elmcmoveNO));
      sprintf(err_buf,"nstexpanded (%d) must be an integer multiple of nstfep (%d) if changes in lambda state are being performed",expand->nstexpanded,fep->nstfep);
      CHECK((ir->efep != efepNO) && (mod(expand->nstexpanded,fep->nstfep)!=0));
      if (expand->nstTij > 0)
      {
