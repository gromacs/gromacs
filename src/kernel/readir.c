...
#define CHECK(b) _low_check(b,err_buf,wi)
  ...
  set_warning_line(wi,mdparin,-1);
    ...
    {
        warning_error(wi,"nstlist must be evenly divisible by nstcalclr. Use nstcalclr = -1 to automatically have it follow nstlist");
    }
    else if(ir->nstcalclr==0 || ir->nstcalclr<-1)
    {
        warning_error(wi,"nstcalclr must be a positive number (divisor of nstcalclr), or -1 to have it follow nstlist");
    }
  ...
  if (EEL_MIGHT_BE_ZERO_AT_CUTOFF(ir->coulombtype)) {
    if (EEL_SWITCHED(ir->coulombtype)) {
      sprintf(err_buf,
	      "With coulombtype = %s rcoulomb_switch must be < rcoulomb. Or, better: Use the new potential modifier options!",
	      eel_names[ir->coulombtype]);
     ...
     ir->vdwtype==evdwSWITCH || ir->vdwtype==evdwSHIFT)
  {
      sprintf(warn_buf,
              ...
              "performance from applying our new potential modifiers to your interactions!\n");
      warning_note(wi,warn_buf);
  }
  ...
  if (EVDW_SWITCHED(ir->vdwtype)) {
    sprintf(err_buf,"With vdwtype = %s rvdw-switch must be < rvdw. Or, better - use the new potential modifiers.",
