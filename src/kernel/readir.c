...
#define CHECK(b) _low_check(b, err_buf, wi)
    ...
    if ((ir->epsilon_surface != 0) && EEL_FULL(ir->coulombtype))
    {
        if (ir->cutoff_scheme == ecutsVERLET)
        {
            sprintf(warn_buf, "Since molecules/charge groups are broken using the Verlet scheme, you can not use a dipole correction to the PME electrostatics. Setting epsilon_surface to 0");
            ir->epsilon_surface = 0;
            warning(wi, warn_buf);
        }
        else 
        {
            sprintf(warn_buf, "Dipole corrections to PME electrostatics only work if all molecules are dipoles. If this is not the case set epsilon_surface to 0");
            warning_note(wi, warn_buf);
        }
    }
