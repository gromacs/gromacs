...
#define CHECK(b) _low_check(b, err_buf, wi)
    ...
    {
        sprintf(warn_buf, "With PME and ewald_geometry = %s you should use pbc = %s",
                eewg_names[eewg3DC], epbc_names[epbcXY]);
        warning(wi, warn_buf);
    }
    if ((ir->epsilon_surface != 0) && EEL_FULL(ir->coulombtype))
    {
        if (ir->cutoff_scheme == ecutsVERLET)
        {
            sprintf(warn_buf, "Since molecules/charge groups are broken using the Verlet scheme, you can not use a dipole correction to the PME electrostatics.");
            warning(wi, warn_buf);
        }
        else
        {
            sprintf(warn_buf, "Dipole corrections to PME electrostatics only work if all charge groups that can cross PBC boundaries are dipoles. If this is not the case set epsilon_surface to 0");
            warning_note(wi, warn_buf);
        }
    }
