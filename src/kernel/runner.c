...
static int get_tmpi_omp_thread_distribution(const gmx_hw_info_t *hwinfo,
                                            ...
                                            int ngpu)
{
    ...
    else
    {
        ...
        /* On AMD and older Intel processors we should avoid running
         * OpenMP over multiple sockets/dies. With up to 6 physical cores
         * in a machine, we are pretty sure we are running on a single die.
         */
        const int nphyscores_omp_always_faster = 6;

        if (hwinfo->nthreads_hw_avail <= nphyscores_omp_always_faster ||
            (gmx_cpuid_x86_smt(hwinfo->cpuid_info) == GMX_CPUID_X86_SMT_ENABLED &&
             hwinfo->nthreads_hw_avail/2 <= nphyscores_omp_always_faster))
        {
                            ...
                            FILE *fplog)
{
    ...
    nthreads_tmpi =
        get_tmpi_omp_thread_distribution(hwinfo,hw_opt,nthreads_tot_max,ngpu);
