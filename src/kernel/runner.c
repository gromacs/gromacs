                                            ...
                                            int ngpu)
{
    ...
    else
    {
         ...
         * A reasonable limit for Intel, with unknown topology, is 16 threads.
         */
        const int nthreads_omp_always_faster = 4;
        const int nthreads_omp_intel_always_faster = 16;

        if (nthreads_tot <= nthreads_omp_always_faster ||
            (gmx_cpuid_vendor(hwinfo->cpuid_info) == GMX_CPUID_VENDOR_INTEL &&
             nthreads_tot <= nthreads_omp_intel_always_faster))
        {
