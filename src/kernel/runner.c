 ...
 * will honor it and disable mdrun internal thread pinning.  This
 * function should be called twice. Once before the OpenMP library
 * gets initialized with bAfterOpenMPInit=FALSE (which will detect
 * affinity set by external tools like taskset), and again later,
 * after the OpenMP initialization, with bAfterOpenMPInit=TRUE (which
 * will detect affinity changes made by the OpenMP library).
 *
    ...
    if (!bAllSet)
    {
        if (!bAfterOpenmpInit)
        {
            md_print_warn(cr, fplog,
                          "mdrun detected a non-default process affinity, "
                          "so it will not attempt to pin its threads");
        }
        else
        {
            md_print_warn(cr, fplog,
