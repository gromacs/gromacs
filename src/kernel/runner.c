                                  ...
                                  gmx_bool            *bUseGPU)
{
     ...
     */
    *bUseGPU = bHaveGPU || (getenv("GMX_EMULATE_GPU") != NULL);
             ...
             const char *deviceOptions, unsigned long Flags)
{
    ...
    if (PAR(cr))
    {
        /* The master rank decided on the cut-off scheme and use of GPUs,
         * broadcast this information to all ranks.
         *
         * With thread-MPI, initially one thread runs this code and
         * PAR(cr)==FALSE. If tMPI threads are spawned later, they will
         * enter here and call the broadcast. The master rank calls this
         * broadcast futher down right after the threads are spawned.
         */
    ...
    if (SIMMASTER(cr))
    {
        ...
        if (hw_opt->nthreads_tmpi > 1)
        {
            ...
            /* The master rank didn't call this broadcast at its usual location
             * above. Call it here now the other ranks (threads) are alive.
             */
            gmx_bcast_sim(sizeof(minf), &minf, cr);
        }
    }
