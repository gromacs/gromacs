                                            ...
                                            int ngpu)
{
    ...
    else
    {
         ...
         * On Intel running OpenMP on a single CPU is always faster.
         * With up to 6 physical cores in a machine, we are pretty sure
         * we are running on a single CPU.
         */
        const int nthreads_omp_always_faster = 4;
        const int n_Intel_cores_omp_always_faster = 6;

        if (nthreads_tot <= nthreads_omp_always_faster ||
            (gmx_cpuid_vendor(hwinfo->cpuid_info) == GMX_CPUID_VENDOR_INTEL &&
             (hwinfo->nthreads_hw_avail <= n_Intel_cores_omp_always_faster ||
              (gmx_cpuid_x86_smt(hwinfo->cpuid_info) == GMX_CPUID_X86_SMT_ENABLED &&
               hwinfo->nthreads_hw_avail/2 <= n_Intel_cores_omp_always_faster))))
        {
