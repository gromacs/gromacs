...
int write_localp_grid(int nfile,t_filenm fnm[],gmx_localp_grid_t *localp_grid, t_state *state,int frameindex)
{
	int i,j,k,ngrid;
	char buf[1024];
    FILE *fplocal;
...
void mdrunner(FILE *fplog,t_commrec *cr,int nfile,t_filenm fnm[],
	      ...
	      real pforce,real cpt_period,real max_hours, int localp_interval,
	      unsigned long Flags)
{
  ...
  if (cr->duty & DUTY_PP) {
    ...
    fr->localp_interval=localp_interval;
	     ...
	     unsigned long Flags)
{
  ...
  if(localp_grid==NULL)
  {
	  ...
	  localp_initialize(localp_grid,ngrid,fr->userreal1);
  ...
  if (!ir->bContinuation && !bRerunMD) {
    ...
    if (bHaveConstr) {
      ...
      do_shakefirst(fplog,constr,ir,mdatoms,state,buf,f,
		    graph,cr,nrnb,fr,&top->idef,localp_grid);
    }
    if (vsite) {
  ...
  else
  {
      ...
      if (ekind->cosacc.cos_accel == 0) {
          calc_ke_part(state->x,v_old,state->v,&(ir->opts),mdatoms,ekind,nrnb,state->lambda,localp_grid);
      } else {
  ...
  while (!bLastStep || (bRerunMD && bNotLastFrame)) {
	...
	nconverged++;
      ...
      do_force(fplog,cr,ir,step,nrnb,wcycle,top,groups,
	       ...
	       GMX_FORCE_ALLFORCES,localp_grid);
    }
    ...
    if (!bRerunMD || rerun_fr.bV || bForceUpdate) {
      ...
      update(fplog,step,&dvdl,ir,mdatoms,state,graph,f,buf,fcd,
	     ...
	     bNEMD,bFirstStep && bStateFromTPX,localp_grid);
	...
	*/
	...
	set_avcsixtwelve(fplog,fr,top_global);
      ...
      calc_dispcorr(fplog,ir,fr,step,top_global->natoms,
		    ...
		    pres,total_vir,enerd->term,localp_grid);
    ...
    if (!bRerunMD || !rerun_fr.bStep) {
	  ...
	  if(localp_grid)
	  {
		  ...
		  localp_grid->nframes++;
		  if (do_per_step(step,fr->localp_interval))
		  {
			write_localp_grid(nfile,fnm,localp_grid, state,localp_index);
			localp_initialize(localp_grid, ngrid, fr->userreal1);
			localp_index++;
		  }
