             ...
             gmx_runtime_t *runtime)
{
    ...
    gmx_iterate_init(&iterate,FALSE); /* will be false unless we otherwise mark it for an iterative step */
    ...
    while (!bLastStep || (bRerunMD && bNotLastFrame)) {
        ...
        if (EI_VV(ir->eI) && (!bInitStep))
        {
            /* for vv, the first half of the integration actually corresponds
               to the previous step, and so we avoid calculating on the previous step. */
        ...
        /*  ############### START FIRST UPDATE HALF-STEP FOR VV METHODS############### */
        {
            ...
            if (bIterativeCase && do_per_step(step-1,ir->nstpcouple))
            {
                gmx_iterate_init(&iterate,!bInitStep);
            }
            ...
            bFirstIterate = TRUE;
            while (bFirstIterate || iterate.bIterate)
            {
                if (iterate.bIterate)
                {
