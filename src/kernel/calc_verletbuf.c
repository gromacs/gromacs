                                        ...
                                        int                     *n_nonlin_vsite)
{
    ...
    for (mb = 0; mb < mtop->nmolblock; mb++)
    {
        ...
        for (a = 0; a < atoms->nr; a++)
        {
             ...
             * a mass of least half its own mass. This is not a critical
             ...
             */
            type[a].bConstr = (type[a].con_mass >= type[a].mass);
                                    ...
                                    real *sigma2_3d)
{
    ...
    /* Deterline the variance for linear displacement of the atom */
    sigma2_linear = kT_fac/type->mass;

    /* The variance relative to the constraint length */
    sigma2_rel_con = sigma2_linear/(type->con_len*type->con_len);
    ...
    if (sigma2_rel_con < 6)
    {
         ...
         * For sigma2_rel_con<<1 the displacement distribution is erfc
         * (exact formula is provided below). For larger sigma, it is clear
         * that the displacement can't be larger than 2*constraint_length.
         * It turns out that the distribution becomes nearly uniform.
         * For intermediate sigma2_rel_con, scaling down sigma with the third
         * order expansion of arcsin with argument sigma_rel_con turns out
         ...
         * runs to infinity, whereas it should be <= 2*constaint_length).
         * Thus we always overestimate the drift and the buffer size.
         */
        scale      = 1/(1 + sigma2_rel_con/6);
        *sigma2_2d = sigma2_linear*scale*scale;
    }
    else
    {
        /* sigma_2d is set to the maximum given by the scaling above.
         * For large sigma2 the real displacement distribution is close
         * to uniform over -2*con_len to 2*con_len.
         * Our erfc with sigma_2d=sqrt(1.5)*con_len (which means the sigma
         * of the erfc output distribution is con_len) overestimates
         * the variance and additionally has a long tail. This means
         * we have a (safe) overestimation of the drift.
         */
        *sigma2_2d = 1.5*type->con_len*type->con_len;
    }
