                                            ...
                                            const gmx_domdec_t   *dd)
{
    ...
    int          d;
    int          grid_restrict;
    const int    calc_grid_min = 6;

    /* Avoid infinite while loop below.
     * The minimum grid size calc_grid returns is 6,
     * so don't try to go below that.
     */
    if (pme_lb->setup[pme_lb->cur].grid[XX] <= calc_grid_min &&
        pme_lb->setup[pme_lb->cur].grid[YY] <= calc_grid_min &&
        pme_lb->setup[pme_lb->cur].grid[ZZ] <= calc_grid_min)
    {
        return FALSE;
    }
