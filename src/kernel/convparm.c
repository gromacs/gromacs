			 ...
			 real old[MAXFORCEPARAM],int comb,double reppow)
{
  int  i,j;
  real tmp;
  gmx_bool have_nonzero_param=FALSE;
  ...
  for(j=0; (j<MAXFORCEPARAM); j++) 
  {
       ...
       */
      have_nonzero_param = (have_nonzero_param==TRUE || fabs(old[j])<GMX_REAL_MIN);
  }
    
  if(have_nonzero_param==FALSE)
  {
			...
			int start,gmx_bool bAppend)
{
  ...
  rc = assign_param(ftype,&newparam,forceparams,comb,reppow);
