...
int set_vsites(bool bVerbose,char *molname,t_atoms *atoms,t_atomtype *atype,
	       t_params plist[])
{
  int i,j,ftype;
  ...
  for(ftype=0; (ftype<F_NRE); ftype++)
	...
	if (!bSet) {
	  if (bVerbose && bFirst) {
	    printf("Calculating parameters for virtual sites\n");
	    bFirst=FALSE;
	  }
	...
	fprintf(debug,"Calculated parameters for %d out of %d %s atoms\n",
...
void set_vsites_ptype(bool bVerbose, t_idef *idef, t_atoms *atoms)
{
  ...
  if (bVerbose)
    printf("Setting particle type to V for virtual sites\n");
  if (debug)
    fprintf(debug,"checking %d functypes\n",F_NRE);
  for(ftype=0; (ftype<F_NRE); ftype++) {
    if (interaction_function[ftype].flags & IF_VSITE) {
      ...
      if (debug && nrd)
	fprintf(debug,"doing %d %s virtual sites\n",
...
static void clean_vsite_bonds(bool bVerbose,
			      t_params *plist, t_pindex pindex[], 
			      int cftype, int vsite_type[])
{
  ...
  if (nremoved && bVerbose)
    printf("Removed   %4d %15ss with virtual sites, %5d left\n",
	   nremoved, interaction_function[cftype].longname, kept_i);
  if (nconverted && bVerbose)
    printf("Converted %4d %15ss with virtual sites to connections, %5d left\n",
	   nconverted, interaction_function[cftype].longname, kept_i);
  if (nOut && bVerbose)
    printf("Warning: removed %d %ss with vsite with %s construction\n"
...
static void clean_vsite_angles(bool bVerbose,
			       t_params *plist, t_pindex pindex[], 
			       int cftype, int vsite_type[])
{
  ...
  if ((ps->nr != kept_i) && bVerbose)
    printf("Removed   %4d %15ss with virtual sites, %5d left\n",
	   ps->nr-kept_i, interaction_function[cftype].longname, kept_i);
  ps->nr=kept_i;
}

static void clean_vsite_dihs(bool bVerbose,t_params *plist, t_pindex pindex[], 
			     int cftype, int vsite_type[])
{
...
void clean_vsite_bondeds(bool bVerbose,t_params *plist, 
			 int natoms, bool bRmVSiteBds)
{
  ...
  if (nvsite) {
    if( bVerbose)
      printf("Cleaning up constraints %swith virtual sites\n",
	...
	if (interaction_function[ftype].flags & (IF_BTYPE | IF_CONSTRAINT) )
	  clean_vsite_bonds (bVerbose,plist, pindex, ftype, vsite_type);
	else if (interaction_function[ftype].flags & IF_ATYPE)
	  clean_vsite_angles(bVerbose,plist, pindex, ftype, vsite_type);
	else if ( (ftype==F_PDIHS) || (ftype==F_IDIHS) )
	  clean_vsite_dihs  (bVerbose,plist, pindex, ftype, vsite_type);
      }
