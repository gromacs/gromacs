					 ...
					 int rowi,int rowj,int eemtype)
{
	...
	case eqgSMs:
	case eqgSMps:
		eSS = Slater_SS[rowi][rowj](r,wi,wj);
		break;
	case eqgRappe:
	case eqgYang:
		eSS = Slater_SS[rowi][rowj](r,wi,wj);
...
#define BOHR  (0.052917)
		if (eem_get_elem(eem,qgen->eem_ndx[i]) == 1) {
			qq = max(-1,min(1,qgen->qq[i]));
			qgen->qq[i] = qq;
			switch (eemtype) {
			case eqgYang:
			case eqgRappe:
				j0 = (1+qq/(BOHR*qgen->wj[i]))*j0;
				break;
			case eqgSMs:
			case eqgSMps:
			case eqgSMg:
								   ...
								   int eqg)
{
	...
	qgen_calc_rhs(qgen,eem,eqg);
	do {
		solve_q_eem(debug,qgen,1.0);
		qgen_update_J00(qgen,eem,eqg);
		rms = 0;
		for(i=0; (i<atoms->nr); i++) {
