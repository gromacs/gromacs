...
/* openmm specific information */
    ...
    "In all cases [TT]mdrun[tt] will by default try to use all the possible",
    "hardware resources available. With a normal MPI library only the options",
    ...
    "the -gpu_id option, where GPU id's are passed as a string, e.g. 02 for",
    "using GPUs 0 and 2. When you want different GPU id's on different",
    "nodes of a compute cluster, use the GMX_GPU_ID environment variable",
    "instead. When using PME, the load between the GPU, which calculates",
    "non-bonded interations, and the GPU, which calculates bonded and PME",
    "interactions, is automatically load balanced. This is done by scaling",
    ...
    "[TT]-nopin[tt]. With Intel CPUs with hyper-threading, you might want",
    "to pin consecutive threads to the same physical core for better",
    "performance, the [TT]-pinht[tt] option does this. If you want to run",
    "multiple different processes on the same physical node, you might",
    "need to start pinning from a non-zero core; with [TT]-pinoffset[tt]",
    "you can specify the offset in physical cores for pinning.",
    "[PAR]",
...
#define NFILE asize(fnm)
  ...
  t_pargs pa[] = {
    ...
    { "-pin",     FALSE, etBOOL, {&hw_opt.thread_pinning},
      "Pin OpenMP threads to cores" },
    { "-pinht",   FALSE, etBOOL, {&hw_opt.pin_hyperthreading},
    ...
    { "-nsteps",  FALSE, etINT, {&nsteps},
      "Run this number of steps (overrides mdp option)" },
