...
int cmain(int argc, char *argv[])
{
    const char   *desc[] = {
        ...
        "Note that [TT]mdrun[tt] will also honor the standard OMP_NUM_THREADS",
        "environment variable for setting the number of OpenMP threads, while",
        "GMX_PME_NUM_THREADS can be used to for the PME-only processes.",
        "Also note that using combined MPI+OpenMP parallelization is in most cases",
        "slower than MPI- or OpenMP-only parallelization. However, using OpenMP",
        "threads in MPI-parallel runs can have large performance benefits",
        "at high parallelization, especially close to the scaling limit as it",
        "reduces the number of domains and/or the number of MPI ranks. This can",
        "substantially improve scaling to higher number of cores by relieving",
        "parallelization limits imposed by domain-decomposition or constraints",
        "(only with Verlet scheme) and with PME-only processes by reducing",
        "the communication cost (with both cut-off schemes).",
