...
int cmain(int argc, char *argv[])
{
    const char   *desc[] = {
        ...
        "To use a subset of the available GPUs, or to manually provide a (per compute node)",
        "GPU to PP rank mapping, you can use the [TT]-gpu_id[tt] option. This takes as argument",
        "a string of digits (without delimiter) representing device id-s of the GPUs to be used,",
        "e.g. 02 to use GPUs 0 and 2 in the first and second PP ranks per compute node",
        "respectively. To use different sets of GPU-s",
        ...
        "on different nodes of a cluster. Multiple (thread) MPI ranks, part of a single",
        "or multi-simulation, can share GPUs; this can be accomplished by specifying",
        "the id of the shared device(s) multiple times in [TT]-gpu_id[tt], e.g. 0011.",
        "[PAR]",
