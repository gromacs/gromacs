...
#define NPA asize(pa)
  ...
  gmx_radial_distribution_histogram_t  **prframe=NULL, *prframecurrent=NULL, *pr=NULL;
...
#define NFILE asize(fnm)
  ...
  do {
      ...
      /* allocate memory for prframe[nframes+1] and for time[i] */
      if (prframe == NULL) {
          ...
          /* other frames */
          srenew(prframe,nframes+1);
          srenew(time,nframes+1);
      }
      /*  realy calc p(r) */
      prframe[nframes] = calc_radial_distribution_histogram(gsans,x,box,index,isize,binwidth,bMC,bNORM,mcover,seed);
      /* copy prframe[i] -> prframecurrent */
      snew(prframecurrent,1);
      snew(prframecurrent->gr,prframe[nframes]->grn);
      snew(prframecurrent->r,prframe[nframes]->grn);
      prframecurrent->grn = prframe[nframes]->grn;
      prframecurrent->binwidth = prframe[nframes]->binwidth;
     for(i=0;i<prframecurrent->grn;i++) {
          prframecurrent->gr[i] = prframe[nframes]->gr[i];
          prframecurrent->r[i] = prframe[nframes]->r[i];
      }
      /* normalize histo */
  ...
  /*
   * Now we will construct mean p(r) histogramm
   * since number of beans may be different for frames and
   * last beans essential has zero values then we can set number of beans to max number of beans
   */
  snew(pr,1);
  pr->binwidth = prframe[0]->binwidth;
  pr->grn = prframe[0]->grn;
  /* find max beans value prframe[i]->grn */
  for(i=0;i<nframes;i++) {
      if(pr->grn<prframe[i]->grn) {
          pr->grn = prframe[i]->grn;
          frmax = i;
      }
  }
  snew(pr->gr,pr->grn);
  ...
  /* populate p(r) */
  for(i=0;i<pr->grn;i++) {
      pr->r[i] = prframe[frmax]->r[i];
  }
  for(i=0;i<nframes;i++) {
      for(j=0;j<pr->grn;j++) {
          pr->gr[j] += prframe[i]->gr[j];
      }
  }
