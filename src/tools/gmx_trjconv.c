    ...
    else
    {
        ...
        if (bHaveFirstFrame)
        {
            ...
            do
            {
                ...
                if (bWriteFrame)
                {
                    /* TODO Either the copy of frout = fr should be
                     * done here, or t_trxframe separated into an
                     * object that keeps (e.g.) the internal fields
                     * private, and another object that permits the
                     * client code to alter the frame to suit
                     * itself. Here, we introduce frout_time
                     * to avoid modifying fr.time, in case some
                     * other code depends on it not changing. */
                    real frout_time = fr.time;
