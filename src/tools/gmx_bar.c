...
static void lambda_vec_print(const lambda_vec_t *lv, char *str)
{
    int i;
    size_t np;
                        ...
                        int nbin_default, const output_env_t oenv)
{
    ...
    while(bl!=bl_head)
    {
        ...
        while(sc!=bl->sc)
        {
            ...
            {
                lambda_vec_print(sc->native_lambda, buf);
                lambda_vec_print(sc->foreign_lambda, buf2);
                              ...
                              lambda_vec_t *lam)
{
    ...
    {
        ptr=find_value(ptr);
        if (!read_lambda_vector(ptr, lam, NULL, fn))
        {
            gmx_fatal(FARGS, "lambda vector '%s' %s faulty", ptr, fn);
        }
    }
    else
    {
                                  ...
                                  lambda_components_t *lc)
{
    ...
    if (subtitle)
    {
        if (subtitle2lambda(subtitle,ba, fn, lc))
        {
            ...
            /*lambda_vec_print(&(ba->native_lambda), buf);*/
        }
    }
    ...
    else
    {
        for(i=0; i<ba->nset; )
        {
            ...
            if (use)
            {
                lambda_vec_print(&(ba->lambda[i]), buf);
                i++;
            }
            else
            {
...
static void read_bar_xvg(char *fn, real *temp, sim_data_t *sd)
{
    ...
    *temp=barsim->temp;
    ...
    }
    {
        ...
        lambda_vec_print(s[0].native_lambda, buf);
        printf("%s: %.1f - %.1f; lambda = %s\n    dH/dl & foreign lambdas:\n", 
               fn, s[0].t[0], s[0].t[s[0].ndu-1], buf);
        for(i=0;i<barsim->nset;i++)
        {
            lambda_vec_print(s[i].foreign_lambda, buf);
            printf("        %s (%d pts)\n", buf, s[i].ndu);
        }
    }
    printf("\n\n");
                                 ...
                                 double *last_t, const char *filename)
{
    ...
    {
        char buf[STRLEN], buf2[STRLEN];
        lambda_vec_print(foreign_lambda, buf);
        lambda_vec_print(s->foreign_lambda, buf2);
...
static void read_barsim_edr(char *fn, real *temp, sim_data_t *sd)
{
            ...
            if (fr->block[i].id == enxDHCOLL )
            {
                ...
                *temp=rtemp;
                ...
                else
                {
                    ...
                    {
                        const char *name=
                                    efpt_names[fr->block[i].sub[1].ival[1+j]];
                        if (check)
                        {
...

    {
        char buf[STRLEN];
        printf("\n");
        lambda_vec_print(native_lambda, buf);
        printf("%s: %.1f - %.1f; lambda = %s\n    foreign lambdas:\n",
        ...
        for(i=0;i<nsamples;i++)
        {
            if (lambdas[i])
            {
                lambda_vec_print(lambdas[i], buf);
                if (nhists[i] > 0)
                {
                    printf("        %s (%d hists)\n", buf, nhists[i]);
                }
