		  ...
		  bool bForceInside)
{
  ...
  while ((mol < nmol_insrt) && (try < ntry*nmol_insrt)) {
    ...
    add_conf(atoms,x,r,FALSE,box,&atoms_insrt,x_n,r_insrt,FALSE,bForceInside);
	      ...
	      bool bForceInside)
{
  ...
  onr=atoms->nr;
  onres=atoms->nres;
  add_conf(atoms,x,r,TRUE,box,atoms_solvt,x_solvt,r_solvt,TRUE,bForceInside);
...
#define NFILE asize(fnm)
  ...
  t_pargs pa[] = {
    ...
    { "-shell",  FALSE, etREAL, {&r_shell},
      "Place a layer of water of thickness shell nm around the solute" },
    { "-inside", FALSE, etBOOL, {&bForceInside},
      "[HIDDEN]Force all atoms to be inside the box. Usually this is unnecessary as overlap between Van der Waals spheres is check with periodic boundary conditions taken into account" }
  };
  ...
  if (bInsert) 
    title_ins = insert_mols(opt2fn("-ci",NFILE,fnm),nmol_ins,nmol_try,seed,
			    &atoms,&x,&r,box,r_distance,bForceInside);
  else
  ...
  if (bSol) {
    ...
    add_solv(opt2fn("-cs",NFILE,fnm),&atoms,&x,&r,box,
	     r_distance,&atoms_added,&residues_added,bForceInside);
    if (opt2parg_bSet("-shell",asize(pa),pa) && (r_shell > 0))
      make_shell(&atoms,natoms_prot,r,x,box,r_shell);
  }
