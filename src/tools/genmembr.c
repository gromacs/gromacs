/*
 *       @(#) copyrgt.c 1.12 9/30/97
 *
 *       This source code is part of
 *
 *        G   R   O   M   A   C   S
 *
 * GROningen MAchine for Chemical Simulations
 *
 *            VERSION 2.0b
 * 
 * Copyright (c) 1990-1997,
 * BIOSON Research Institute, Dept. of Biophysical Chemistry,
 * University of Groningen, The Netherlands
 *
 * Please refer to:
 * GROMACS: A message-passing parallel molecular dynamics implementation
 * H.J.C. Berendsen, D. van der Spoel and R. van Drunen
 * Comp. Phys. Comm. 91, 43-56 (1995)
 *
 * Also check out our WWW page:
 * http://rugmd0.chem.rug.nl/~gmx
 * or e-mail to:
 * gromacs@chem.rug.nl
 *
 * And Hey:
 * Gyas ROwers Mature At Cryogenic Speed
 */
#include "copyrite.h"
#include "string2.h"
#include "smalloc.h"
#include "sysstuff.h"
#include "confio.h"
#include "paramio.h"
#include "statutil.h"
#include "vec.h"
#include "random.h"
#include "3dview.h"
#include "txtdump.h"

#define GCIN \
     ITYPE("nx",                   nx,         1) \
     ITYPE("ny",                   ny,         1) \
     RTYPE("dx",                   dx,      0.15) \
     RTYPE("dy",                   dy,      0.15) \
     ITYPE("seed",                 seed,    1993) \
     RTYPE("max_z_rotation",     max_rot,    0.0) \
     RTYPE("max_z_translation",  max_tr,     0.0) \
     NULL

static void rand_rot(int natoms,rvec x[],vec4 xrot[],rvec cbox,int *seed,
		     float max_rot)
{
  mat4 mt1,mt2,mr[DIM],mtemp1,mtemp2,mtemp3,mxtot;
  rvec xcm;
  real phi;
  int  i,m;

  unity_m4(mr[XX]);
  unity_m4(mr[YY]);
  unity_m4(mr[ZZ]);
  
  translate(-cbox[XX],-cbox[YY],-cbox[ZZ],mt1);  /* move c.o.ma to origin */

  phi=M_PI*max_rot*rando(seed)/180;
  rotate(2,phi,mr[ZZ]);
  
  translate(cbox[XX],cbox[YY],cbox[ZZ],mt2);
  
  mult_matrix(mtemp1,mt2,mr[XX]);
  mult_matrix(mtemp2,mr[YY],mr[ZZ]);
  mult_matrix(mtemp3,mtemp1,mtemp2);
  mult_matrix(mxtot,mtemp3,mt1);
  
  for(i=0; (i<natoms); i++) 
    m4_op(mxtot,x[i],xrot[i]);
}

static void move_x(int natoms,rvec x[],matrix box)
{
  int  i,m;
  rvec xcm;
  
  clear_rvec(xcm);
  for(i=0; (i<natoms); i++)
    for(m=0; (m<DIM); m++)
      xcm[m]+=x[i][m];
  for(m=0; (m<DIM); m++)
    xcm[m] = 0.5*box[m][m]-xcm[m]/natoms;
  for(i=0; (i<natoms); i++)
    for(m=0; (m<DIM); m++)
      x[i][m] += xcm[m];
}

void main(int argc, char *argv[])
{
  static char *desc[] = {
    "genmembr generates a monolayer consisting of one type of molecule.",
    "The molecule is randomly rotated and translated along the axis", 
    "perpendicular to the layer. It is assumed that the z-axis of the",
    "box is the long axis of the molecule. dx and dy can be used to",
    "adjust the spacing between molecules."
  };
  static char *opts[] = {
    "-r"
  };
  static char *odesc[] = {
    "Randomize, molecules are randomly rotated on their grid point."
  };
  static char *bugs[] = {
    "When option -r is used (randomize) the program does not check for overlap between molecules on grid points. It is recommended to make the box in the input file at least as big as the coordinates + Vander Waals radius. ",
    "The program should be more flexible, to allow for random displacement off lattice points (for each cartesian coordinate), and specify the (maximum) random rotation per coordinate to be useful for building membranes."
  };
  t_manual man = {asize(desc),desc,asize(opts),opts,odesc,asize(bugs),bugs};
  
  int     nx,ny,vol;    /* nx,ny number of gridpoints in x and y direction */
  t_atoms *atoms;       /* list with all atoms */
  char    title[STRLEN];
  real    dx,dy;        /* space added between molecules */
  rvec    *x,*v;        /* original coordinates and velocities */
  rvec    cbox;          /* center of mass of _one_ molecule */
  vec4    *xrot;        /* rotated coordinates of molecules */
  matrix  box;          /* box length matrix */
  bool    bRandom=FALSE; /* False: no random rotations */
  rvec    shift;        /* displacement in lattice */ 
  float   max_rot;      /* maximum rotation */
  float   max_tr,tr;    /* maximum translation, actual translation */
  int     seed;         /* seed for random number generator */
  int     natoms;       /* number of atoms in one molecule  */
  int     nres;         /* number of molecules? */
  int     i,j,l,m,ndx,nrdx;
  t_filenm fnm[] = {
    { efGCP, NULL,  NULL,     ffREAD },
    { efGCP, "-po", "gcout",  ffWRITE },
    { efGRO, "-ci", "confin", ffREAD },
    { efGRO, "-co", "confout",ffWRITE }
  };
#define NFILE asize(fnm)
    
  CopyRight(stdout,argv[0]);
  parse_common_args(&argc,argv,0,NFILE,fnm,FALSE,&man);

  if (argc > 1) {
    if (strcasecmp(argv[1],"-r")==0) 
      bRandom = TRUE;

  }

  read_params (fnm[0].fn,GCIN);
  write_params(opt2fn("-po",NFILE,fnm),GCIN);

  if ((nx <= 0) || (ny <= 0)) {
    fprintf(stderr,"(nx <= 0) || (ny <= 0))\n");
    exit(1);
  }

  vol=nx*ny;        /* number of molecules in result */

  get_coordnum(opt2fn("-ci",NFILE,fnm),&natoms); 
  snew(atoms,1); 
  snew(atoms->resname,natoms*vol); /* get space for _all_ atoms */
  snew(atoms->atomname,natoms*vol);
  snew(atoms->atom,natoms*vol);
  snew(x,natoms*vol);              /* get space for coordinates of all atoms */
  snew(xrot,natoms);               /* get space for rotation matrix? */
  snew(v,natoms*vol);              /* velocities. not really needed? */ 

  read_whole_conf(opt2fn("-ci",NFILE,fnm),title,atoms,x,v,box);
 
  nres=atoms->nres;                /* nr of residues in one element? */

  clear_rvec(cbox);
  for(m=0; (m<DIM); m++)
    cbox[m] = box[m][m]/2;   /* get center of box of one molecule */

  fprintf(stderr,"center of box: %f, %f, %f\n",cbox[0],cbox[1],cbox[2]);
  

  for(i=0; (i<nx); i++) {          /* loop over all gridpositions    */
    shift[XX]=i*(dx+box[XX][XX]);
    
    for(j=0; (j<ny); j++) {
      shift[YY]=j*(dy+box[YY][YY]);
	
      /* to get negative values too */
      tr = (rando(&seed)-0.5)*2*max_tr;
 
	ndx=(i*ny + j)*natoms;
	nrdx=(i*ny + j)*nres;
	
	if (ndx > 0) {

	  /* Random rotation on input coords */
	  if (bRandom)
	   rand_rot(natoms,x,xrot,cbox,&seed,max_rot);
	  
	  for(l=0; (l<natoms); l++) {
	    for(m=0; (m<DIM); m++) {
	      if (bRandom) {
		x[ndx+l][m]=xrot[l][m]+shift[m];
	      }
	      else {
		x[ndx+l][m]=x[l][m]+shift[m];
	      }
	    }
	    atoms->atom[ndx+l].resnr=nrdx+atoms->atom[l].resnr;
	    atoms->atomname[ndx+l]=atoms->atomname[l];
	  }

	  for(l=0; (l<nres); l++)
	    atoms->resname[nrdx+l]=atoms->resname[l];
	}
    }
  }

  box[XX][XX] = nx*(box[XX][XX]+dx); /* make box bigger */
  box[YY][YY] = ny*(box[YY][YY]+dy);

  move_x(natoms*vol,x,box);          /* put atoms in box? */
    
  atoms->nr*=vol;
  atoms->nres*=vol;
  write_conf(opt2fn("-co",NFILE,fnm),title,atoms,x,v,box);
  
  thanx(stdout);
}






