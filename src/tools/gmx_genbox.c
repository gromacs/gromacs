...
*/
static gmx_bool allPairsDistOk(t_atoms *atoms, rvec *x, real *r,
                            int ePBC, matrix box,
...
static char *insert_mols(const char *mol_insrt,int nmol_insrt,int ntry,int seed,
                         ...
                         gmx_bool bAllPairDist)
{
  t_pbc   pbc;
  static  char    *title_insrt;
  t_atoms atoms_insrt;
  rvec    *x_insrt,*x_n,*x_inspos;
  real    *r_insrt;
  int     ePBC_insrt;
  matrix  box_insrt;
  int     i,mol,onr,ncol;
  ...
  while ((mol < nmol_insrt) && (trial < ntry*nmol_insrt)) {
    ...
    /* This is a (maybe) slow workaround to avoid too many calls of add_conf, which
       ...
       small system.
    */
    if (bAllPairDist && !allPairsDistOk(atoms,*x,*r,ePBC,box,&atoms_insrt,x_n,r_insrt))
        continue;
...
int gmx_genbox(int argc,char *argv[])
{
  const char *desc[] = {
    ...
    "4) Insert a number of molecules ([TT]-ci[tt]) at positions given in",
    "positions.dat ([TT]-ip[tt]). positions.dat should have 3 columns (x/y/z),",
    "and comments starting with # are ignored. Option [TT]-dr[tt]",
...
#define NFILE asize(fnm)
  ...
  static gmx_bool bReadV=FALSE,bAllPairDist=FALSE;
  ...
  t_pargs pa[] = {
    ...
    { "-allpair",    FALSE, etBOOL, {&bAllPairDist},
