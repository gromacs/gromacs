...
cl_R Slater_4S_6S(cl_R r,cl_R xi,cl_R xj)
{
  ...
  rxj = r*xj;
  if (xi == xj) {
        if (r == 0LL) {  S = (82633LL*xi)/524288LL

    ; } else {  S = (1LL/r)*((-(1e9*cl_float(291948240,precision)+981196800LL) + (1e9*cl_float(291948240,precision)+981196800LL)*exp(2LL*rxi) - 
      ...
      if (r == 0LL) { S = (xi*xj*(2LL*Power(xi,18LL) + 38LL*Power(xi,17LL)*xj + 342LL*Power(xi,16LL)*Power(xj,2LL) + 
    ...
    ; } else { S = (1LL/r)*((1871100LL*exp(2LL*(rxi + rxj))*Power(Power(rxi,2LL) - Power(rxj,2LL),19LL) + 
