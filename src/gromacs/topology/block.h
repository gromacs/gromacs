 ...
 * Block b contains indices i for which start(b) <= i < end(b).
 */
class BlockRanges
{
    public:
        ...
        int numBlocks() const
        {
            GMX_ASSERT(index_.size() > 0 && index_[0] == 0, "numBlocks() should only be called on a valid BlockRanges struct");
        ...
        {
            GMX_ASSERT(index_.size() > 0 && index_[0] == 0, "appendBlocks() should only be called on a valid BlockRanges struct");
        ...
        void clear()
        {
            GMX_ASSERT(index_.size() > 0 && index_[0] == 0, "clear() should only be called on a valid BlockRanges struct");
            index_.resize(1);
        }
