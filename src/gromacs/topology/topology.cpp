...
static void compareFfparamAB(FILE *fp, const gmx_ffparams_t &ff1, real ftol,   real abstol)
{
    fprintf(fp, "comparing ffparamAB\n");
    for (int i = 0; i < ff1.numTypes(); i++)
    {
        std::string buf = gmx::formatString("ffparams->iparams[%d]", i);
        cmp_iparm_AB(fp, buf.c_str(), ff1.functype[i], ff1.iparams[i], ftol, abstol);
    }
}
...
static void compareMoletypeAB(FILE *fp, gmx::ArrayRef<const gmx_moltype_t> mt1, real ftol, real abstol)
{
    fprintf(fp, "comparing moltypeAB\n");
    for (int i = 0; i < mt1.size(); i++)
    {
        compareAtoms(fp, &mt1[i].atoms, nullptr, ftol, abstol);
    }
}
...
void compareMtopAB(FILE *fp, const gmx_mtop_t &mtop1, real ftol, real abstol)
{
    ...
    compareFfparamAB(fp, mtop1.ffparams, ftol, abstol);
    compareMoletypeAB(fp, mtop1.moltype, ftol, abstol);
}
