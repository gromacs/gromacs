...
static void
p_status(gmx::ArrayRef<const std::string> restype,
...
mk_aid(const t_atoms *atoms, gmx::ArrayRef<const std::string> restype, const std::string &typestring, gmx_bool bMatch)
                         ...
                         t_blocka *gb, char ***gn, gmx_bool bASK, gmx_bool bVerb)
{
    ...
    for (i = 0; (i < num_index_groups); i++)
    {
        ...
        /* if we want to add this group always or it differs from previous
           group, add it: */
        if (-1 == constructing_data[i].compareto || !grp_cmp(gb, aid.size(), aid.data(), constructing_data[i].compareto-i) )
        {
...
void analyse(const t_atoms *atoms, t_blocka *gb, char ***gn, gmx_bool bASK, gmx_bool bVerb)
{
    ...
    for (size_t k = 0; k < previousTypename.size(); k++)
    {
        ...
        if (!gmx_strcasecmp(previousTypename[k].c_str(), "Protein") && aid.size() > 0)
        {
            /* PROTEIN */
            ...
            if ((aid.size() > 0) && (static_cast<int>(aid.size()) < atoms->nr))
            {
                add_grp(gb, gn, aid, "non-Protein");
            }
        }
        else if (!gmx_strcasecmp(previousTypename[k].c_str(), "Water") && aid.size() > 0)
        {
