...
findResidueEntryWithName(gmx::ArrayRef<const ResidueTypeEntry> entries, const std::string &residueName)
{
    ...
    return (found != entries.end()) ? gmx::compat::make_optional(found) : gmx::compat::nullopt;
}
...
{
    if (auto found = findResidueEntryWithName(impl_->entry, residueName))
    {
...
{
    auto found = findResidueEntryWithName(impl_->entry, residueName);
    return found ? gmx::equalCaseInsensitive(residueType, (*found)->residueType) : false;
}
...
int ResidueType::indexFromResidueName(const std::string &residueName) const
{
    ...
    auto found = findResidueEntryWithName(temp, residueName);
    return found ? std::distance(temp.begin(), *found) : -1;
}
...
{
    auto found = findResidueEntryWithName(impl_->entry, residueName);
    return found ? (*found)->residueType : c_undefinedResidueType;
}
...
{
    auto found = findResidueEntryWithName(impl_->entry, residueName);
    return found ? gmx::compat::make_optional((*found)->residueType) : gmx::compat::nullopt;
}
