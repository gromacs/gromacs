...
static int previousResidueIndex(const ResidueTypes *rt, const char *resname)
{
    for (size_t i = 0; i < rt->size(); i++)
    {
        if (gmx_strcasecmp(rt->at(i).resname.c_str(), resname) == 0)
        {
            return i;
        }
    }
...
{
    if (isResidueInResidueTypes(rt, resname))
    {
        std::string oldentry = previouslyDefinedType(rt, resname);
