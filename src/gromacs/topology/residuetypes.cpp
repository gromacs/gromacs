...
static int previousResidueIndex(gmx::ArrayRef<const ResidueType> rt, const char *resname)
{
    for (int i = 0; i < rt.size(); i++)
    {
        if (gmx_strcasecmp(rt[i].resname.c_str(), resname) == 0)
        {
            return i;
        }
    }
...
bool isResidueInResidueTypes(gmx::ArrayRef<const ResidueType> rt, const char *resname)
{

    return previousResidueIndex(rt, resname) != -1;
}

std::string previouslyDefinedType(gmx::ArrayRef<const ResidueType> rt, const char *resname)
{
    int entry = previousResidueIndex(rt, resname);
    if (entry != -1)
    {
        return rt[entry].restype;
    }
    else
    {
...
void addResidue(std::vector<ResidueType> *rt, const char *resname, const char *restype)
{
    if (isResidueInResidueTypes(gmx::constArrayRefFromArray(rt->data(), rt->size()), resname))
    {
        std::string oldentry =
            previouslyDefinedType(gmx::constArrayRefFromArray(rt->data(), rt->size()), resname);
...
bool isResidueTypeProtein(gmx::ArrayRef<const ResidueType> rt, const char *resnm)
{
...
bool isResidueTypeDNA(gmx::ArrayRef<const ResidueType> rt, const char *resnm)
{
    std::string type = previouslyDefinedType(rt, resnm);
...
bool isResidueTypeRNA(gmx::ArrayRef<const ResidueType> rt, const char *resnm)
{
    std::string type = previouslyDefinedType(rt, resnm);
...
int findResidueIndex(gmx::ArrayRef<const ResidueType> rt, const char *resnm)
{
...
const char *
findResidueName(gmx::ArrayRef<const ResidueType> rt, int index)
{
