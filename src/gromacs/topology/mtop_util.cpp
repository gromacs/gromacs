                          ...
                          GmxQmmmMode       qmmmMode)
{
    ...
    std::vector<int> qmAtoms = gmx_mtop_gen_qmmm(mtop);

    if (qmmmMode == GMX_QMMM_MIMIC && !qmAtoms.empty())
    {
        t_blocka inter_excl;
        init_blocka(&inter_excl);
        int      n_q = qmAtoms.size();
        ...
        for (int k = 0; k < inter_excl.nr; ++k)
        {
            ...
            for (int i = 0; i < n_q; ++i)
            {
                ...
                int index = n_q * i;
                inter_excl.index[qmAtoms[i]] = index;
                prev_index                   = index + n_q;
                for (int j = 0; j < n_q; ++j)
                {
                    inter_excl.a[n_q * i + j] = qmAtoms[j];
                }
            }
        }
        inter_excl.index[qmAtoms[n_q - 1] + 1] = n_q * n_q;
