                          ...
                          bool              mimic)
{
    ...
    if (mimic && mtop->qm_atoms->size() > 0)
    {
        t_blocka inter_excl;
        init_blocka(&inter_excl);
        int      n_q = mtop->qm_atoms->size();

        inter_excl.nr  = top->excls.nr;
        inter_excl.nra = n_q * n_q;

        int total_nr  = top->excls.nr + n_q;
        ...
        for (int k = 0; k < inter_excl.nr; ++k)
        {
            ...
            for (int i = 0; i < n_q; ++i)
            {
                if (k != mtop->qm_atoms->at(i))
                {
                    continue;
                }
                int index = n_q * i;
                inter_excl.index[mtop->qm_atoms->at(i)] = index;
                prev_index = index + n_q;
                for (int j = 0; j < n_q; ++j)
                {
                    inter_excl.a[n_q * i + j] = mtop->qm_atoms->at(j);
                }
            }
        }
        inter_excl.index[mtop->qm_atoms->at(n_q - 1) + 1] = n_q * n_q;
...
gmx_localtop_t *
gmx_mtop_generate_local_top(const gmx_mtop_t *mtop, bool freeEnergyInteractionsAtEnd, bool mimic)
{
    gmx_localtop_t *top;

    snew(top, 1);

    gen_local_top(mtop, freeEnergyInteractionsAtEnd, true, top, mimic);
...
t_topology gmx_mtop_t_to_t_topology(gmx_mtop_t *mtop, bool freeMTop)
{
    ...
    gen_local_top(mtop, false, FALSE, &ltop, false);
...
void gmx_mtop_gen_qmmm(gmx_mtop_t *mtop)
{
    mtop->qm_atoms = new std::vector<int>();
    int            global_at = 0;
    ...
    for (int mb = 0; mb < mtop->nmolblock; mb++)
    {
        ...
        for (int mol = 0; mol < molb->nmol; ++mol)
        {
            for (int n_atom = 0; n_atom < molt->atoms.nr; ++n_atom)
            {
                ...
                {
                    mtop->qm_atoms->push_back(global_at);
                }
                ++global_at;
            }
        }
