...
static void ilistcat(int ftype, t_ilist *dest, const t_ilist *src, int copies,
                     int dnum, int snum)
{
    GMX_RELEASE_ASSERT(dest->nr >= 0 && dest->nr < 1000000, "dest");
    GMX_RELEASE_ASSERT(copies >= 0 && copies < 1000000, "copies");
    GMX_RELEASE_ASSERT(src->nr >= 0 && src->nr < 1000000, "src");
                          ...
                          gmx_localtop_t   *top)
{
    ...
    for (const gmx_molblock_t &molb : mtop->molblock)
    {
        ...
        srcnr  = molt.atoms.nr;
        destnr = natoms;
        GMX_RELEASE_ASSERT(srcnr >= 0 && srcnr < 1000000, "srcnr should be ok");
        GMX_RELEASE_ASSERT(destnr >= 0 && destnr < 1000000, "destnr should be ok");
        GMX_RELEASE_ASSERT(molb.nmol >= 0 && molb.nmol < 1000000, "molb.nmol should be ok");
