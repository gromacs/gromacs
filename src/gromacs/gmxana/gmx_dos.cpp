...
static void dump_w0(gmx_output_env_t *oenv, real beta,
                    const char *title, int lot)
{
    FILE       *fp;
    double      nu;
#define NLEG 4
    const char *leg[4]    = { "wCv", "wS", "wA", "wE" };
    char        buf[128];
...
static void dump_w(gmx_output_env_t *oenv, real beta)
{
...
static void print_legend(FILE *fp, t_dosprops dp[], gmx_output_env_t *oenv)
{
    const char *leg[V_NR];
    int         t, nleg = 0;
...
static void print_stuff(int n, real tt[], t_dosprops dp[], real dos_vacf[],
                        const char *vacf, const char *mvacf, gmx_output_env_t *oenv)
{
    FILE *fp;
    int   j, t;
...
#define NPA (sizeof(pa)/sizeof(pa[0]))
...
#define NFILE (sizeof(fnm)/sizeof(fnm[0]))
    ...
    else if (!opt2parg_bSet("-Emd", NPA, pa))
    {
    ...
    dp[V_ATOM].dof  = std::max(0, 3*Natmxmol - nconstr);
    dp[V_T].dof     = std::min(3.0, dp[V_ATOM].dof);
    dp[V_A].dof     = std::min(3.0, dp[V_ATOM].dof - dp[V_T].dof);
    if (Natmxmol == 1)
    {
        dp[V_A].dof = 0;
    }
    ...
    {
        dp[V_A].dof = std::min(2.0, dp[V_A].dof);
    }
...
#pragma omp parallel
                {
                    ...
                    i0        = thread_id*NN/NT;
                    i1        = std::min(NN, (thread_id+1)*NN/NT);
...
#pragma omp parallel
        {
            ...
            thread_id = gmx_omp_get_thread_num();
            j0        = thread_id*NN/NT;
            j1        = std::min(NN, (thread_id+1)*NN/NT);
