                  ...
                  t_UmbrellaOptions *opt, gmx_bool bExact)
{
    ...
    private(num,denom,j,k,invg,temp,distance,U)
    {
        #pragma omp for
              ...
              t_UmbrellaOptions *opt, gmx_bool bExact)
{
    ...
    double maxglob = -1e20, total = 0, maxloc;
    ...
    #pragma omp parallel                                                   \
        ...
        private(j,total,k,temp,distance,U,maxloc)
    {
        maxloc = -1e20;
        #pragma omp for
        for (i = 0; i < nWindows; ++i)
...
int gmx_wham(int argc, char *argv[])
{
    const char              *desc[] = {
        ...
        "The number of OpenMP threads used by g_wham is controlled with [TT]-nt[tt].[PAR]",
    ...
    t_pargs                  pa[] = {
        { "-nt", FALSE, etINT, {&nthreads},
          "Number of threads used by g_wham"},
...
#else
        gmx_fatal(FARGS, "Gromacs was not compiled with OpenMP support (found -nt %d).\n", nthreads);
#endif
    }
