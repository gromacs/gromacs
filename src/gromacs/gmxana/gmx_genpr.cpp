                               ...
                               t_atoms *myatoms, rvec **x, int resmax)
{
    ...
    vsnum = vsmin+1;
    printf("\nNOTE: Using center-of-geometry (COG) vsites. These are initially placed without regard to periodicity. If the initial positions are to be used directly (e g for calculating initial scattering), make sure the molecule does not cross a cell boundary.\n");
    for (i = 0; (i < myatoms->nr); )
    {
        resind = myatoms->atom[i].resind;
        if (resind >= nres)
                            ...
                            int atommax)
{
    ...
    if ((atommax == 0) || (atommax > myatoms->nr))
    {
        atommax = myatoms->nr;
    }
    for (ai = 0; (ai < atommax); ai++)
    {
...
int gmx_genpr(int argc, char *argv[])
{
    ...
    t_pargs            pa[] = {
        ...
        { "-atommax", FALSE, etINT, {&atommax},
          "Highest atom number to take into account when making a matrix with the atom option. If <= 0 all atoms will be taken into account." }
    };
