...
static void read_tpr_file(const char *fn_sim_tpr, t_inputinfo *info, t_state_global *state, gmx_mtop_t *mtop, t_inputrec *ir, real user_beta, real fracself)
{
 ...
 */
static void estimate_PME_error(t_inputinfo *info, const t_state_global *state,
...
int gmx_pme_error(int argc, char *argv[])
{
    ...
    real            fracself  = 1.0;
    t_inputinfo     info;
    t_state_global  state;        /* The state from the tpr input file */
