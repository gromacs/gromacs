    ...
    for (i = 0; (i < nmat); i++)
    {
        ...
        if (psr->X.lineatzero)
        {
            ...
            for (x = 0; (x < mat[i].nx); x++)
            {
                ...
                if (x != 0 && x < mat[i].nx-1 &&
                    fabsf(mat[i].axis_x[x]) <
                    0.1*fabsf(mat[i].axis_x[x+1]-mat[i].axis_x[x]) )
                {
        ...
        if (psr->Y.lineatzero)
        {
            ...
            for (y = 0; (y < mat[i].ny); y++)
            {
                ...
                /* draw lines whenever tick label almost zero (e.g. next trajectory) */
                if (y != 0 && y < mat[i].ny-1 &&
                    fabsf(mat[i].axis_y[y]) <
                    0.1*fabsf(mat[i].axis_y[y+1]-mat[i].axis_y[y]) )
                {
...
void zero_lines(int nmat, t_matrix *mat, t_matrix *mat2)
{
    ...
    for (i = 0; i < nmat; i++)
    {
        for (m = 0; m < (mat2 ? 2 : 1); m++)
        {
            ...
            {
                if (fabsf(mats[i].axis_x[x+1]) < 1e-5)
                {
            ...
            {
                if (fabsf(mats[i].axis_y[y+1]) < 1e-5)
                {
