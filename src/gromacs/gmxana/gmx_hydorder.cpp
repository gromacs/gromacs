...
static void writesurftoxpms(real ***surf, int tblocks, int xbins, int ybins, real bw, const std::vector<std::string> &outfiles, int maplevels )
{
...
#define NFILE asize(fnm)
    ...
    const std::vector<std::string> &intfn = opt2fns("-o", NFILE, fnm);
    if (intfn.size() != 2)
    {
    ...
    if (bFourier)
    {
        const std::vector<std::string> &spectra = opt2fns("-Spect", NFILE, fnm);
        if (spectra.size() != 2)
        {
    ...
    if (bRawOut)
    {
        const std::vector<std::string> &raw = opt2fns("-or", NFILE, fnm);
        if (raw.size() != 2)
        {
