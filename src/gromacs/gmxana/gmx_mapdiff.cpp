...
int gmx_mapdiff(int argc, char *argv[])
{
    ...
    std::string               fnMinus;
    options.addOption(StringOption("minus").store(&fnMinus));
    std::string               fnOutput;
    ...
    auto referenceMap   = MapConverter(fnInput).map();
    auto substractedMap = MapConverter(fnMinus).map();
    containeroperation::multiply(&substractedMap, -1.);
    containeroperation::add(&referenceMap, substractedMap);

    MapConverter(referenceMap).to(fnOutput);
