...
static void calcBoxDistance(matrix P, rvec d, ivec ibd)
{
    ...
    ibd[XX] = static_cast<int>(bd[XX]);
    ibd[YY] = static_cast<int>(bd[YY]);
    ibd[ZZ] = static_cast<int>(bd[ZZ]);
}
                        ...
                        matrix box, rvec hbox, t_donors *donors)
{
    ...
    switch (EEst)
    {
        ...
        case NN_1_over_r3:
            ...
            realE = std::pow(r, static_cast<real>(-3.0));
        ...
        case NN_dipole:
            ...
            realE = cosalpha * std::pow(r, static_cast<real>(-3.0));
            E     = (t_E)(SCALEFACTOR_E * realE);
            break;
                    ...
                    const int NN)
{
    ...
    real          *ct, *p_ct, tail, tail2, dtail, *cct;
    const real     tol     = 1e-3;
    int            nframes = hb->nframes;
...
#pragma omp barrier
                ...
                snew(rhbex, n2);
                std::memset(rhbex, 0, n2*sizeof(real)); /* Trust no-one, not even malloc()! */
...
#pragma omp barrier
        ...
        case AC_GEM:
            snew(ct, 2*n2);
            std::memset(ct, 0, 2*n2*sizeof(real));
#ifndef GMX_OPENMP
...
#pragma omp parallel \
            ...
            { /* ##########  THE START OF THE ENORMOUS PARALLELIZED BLOCK!  ########## */
                ...
                p_ct     = NULL;
                snew(p_ct, 2*n2);
                std::memset(p_ct, 0, 2*n2*sizeof(real));
