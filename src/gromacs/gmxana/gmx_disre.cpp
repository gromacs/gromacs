                       ...
                       t_fcdata *fcd)
{
    ...
    for (i = 0; (i < disres->nr); )
    {
        ...
        rt = std::pow(fcd->disres.Rt_6[0], -1.0/6.0);
        ...
        if (viol > 0)
        {
            ...
            for (j = 0; (j < isize); j++)
            {
                ...
                {
                    vvindex[j] = std::pow(fcd->disres.Rt_6[0], -1.0/6.0);
                }
            }
        }
        ndr++;
                       ...
                       int isize, atom_id index[], t_atoms *atoms)
{
    ...
    for (i = 0; (i < ndr); i++)
    {
        ...
        drs[i].rT3    = std::pow(dr->aver_3[i]/nsteps, static_cast<double>(-1.0/3.0));
        drs[i].rT6    = std::pow(dr->aver_6[i]/nsteps, static_cast<double>(-1.0/6.0));
                             ...
                             char *clust_name[], int isize, atom_id index[])
{
    ...
    for (k = 0; (k < clust->nr); k++)
    {
        ...
        for (i = 0; (i < ndr); i++)
        {
            ...
            drs[i].rT3    = std::pow(dr[k].aver_3[i]/dr[k].nframes, static_cast<double>(-1.0/3.0));
            drs[i].rT6    = std::pow(dr[k].aver_6[i]/dr[k].nframes, static_cast<double>(-1.0/6.0));
            drs[i].viol   = std::max(0.0, static_cast<double>(drs[i].r-drs[i].up1));
            drs[i].violT3 = std::max(0.0, static_cast<double>(drs[i].rT3-drs[i].up1));
            drs[i].violT6 = std::max(0.0, static_cast<double>(drs[i].rT6-drs[i].up1));
            sumV         += drs[i].viol;
                              ...
                              real max_dr, int nlevels, gmx_bool bThird)
{
    ...
    for (i = 0; (i < ndr); i++)
    {
        for (j = ptr[i]; (j < ptr[i+1]); j += nratoms+1)
        {
            ...
            if (bThird)
            {
                rav = std::pow(dr->aver_3[i]/nsteps, -1.0/3.0);
            }
            else
            {
