                       ...
                       rvec ***eigvec, real **eigval)
{
    ...
    *natoms = head.natoms;
    snew(*xav, *natoms);
    fread_htrn(status, &head, box, *xav, NULL, NULL,NULL,NULL);
    ...
    *nvec = 0;
    ...
    {
        fread_htrn(status, &head, box, x, NULL, NULL,NULL,NULL);
        if (*nvec >= snew_size)
        {
            snew_size += 10;
                        ...
                        rvec xav[], gmx_bool bDMA, real eigval[])
{
    ...
    if (WriteXref == eWXR_YES)
    {
        ...
        fwrite_trn(trnout, -1, -1, bDMR ? 1.0 : 0.0, zerobox, natoms, xref, NULL, NULL,NULL,NULL);
    }
    else if (WriteXref == eWXR_NOFIT)
    {
        /* misuse lambda: -1 no fit */
        fwrite_trn(trnout, -1, -1, -1.0, zerobox, natoms, x, NULL, NULL,NULL,NULL);
    }

    /* misuse lambda: 0/1 mass weighted analysis no/yes */
    fwrite_trn(trnout, 0, 0, bDMA ? 1.0 : 0.0, zerobox, natoms, xav, NULL, NULL,NULL,NULL);
    ...
    for (i = 0; i <= (end-begin); i++)
    {
        ...
        fwrite_trn(trnout, begin+i, eigval[vec], 0, zerobox, natoms, x, NULL, NULL,NULL,NULL);
    }
    close_trn(trnout);
