        ...
        int         maxPMEnodes)    /* Maximum number of PME nodes                      */
{
    ...
    if (npmevalues_opt != NULL && 0 == std::strcmp(npmevalues_opt, "all") )
    {
        eNPME = eNpmeAll;
    }
    else if (npmevalues_opt != NULL && 0 == std::strcmp(npmevalues_opt, "subset") )
    {
        eNPME = eNpmeSubset;
    }
    else /* "auto" or "range" */
...
#define NFILE asize(fnm)
    ...
    if (opt2parg_bSet("-ntmpi", asize(pa), pa))
    {
        ...
        {
            fprintf(stderr, "WARNING: -npcstringas no effect when using threads.\n");
        }
    ...
    else
    {
         ...
         * mpirun command. */
        if (procstring[0] != NULL && std::strcmp(procstring[0], "none") != 0)
        {
    ...
    if ((nnodes > 2) && (npme_fixed < -1))
    {
        if (0 == std::strcmp(npmevalues_opt[0], "auto"))
        {
            ...
            else
            {
                ...
                minPMEnodes   = std::floor(0.7*guessPMEnodes);
                maxPMEnodes   = std::ceil(1.6*guessPMEnodes);
        ...
        else
        {
            ...
            maxPMEnodes = std::floor(maxPMEfraction*nnodes);
