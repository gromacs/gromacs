...
typedef struct awh_energyreader_t {
    int              nawh;                   /* The number of AWH biases. */
    awh_reader_t    *awh_reader;             /* The readers, one for each AWH bias. */
} awh_energyreader_t;
...
static awh_energyreader_t *init_awh_energyreader(awhbias_params_t *awhbias_params, t_enxframe *fr, gmx_bool bMore)
{
    awh_energyreader_t *awh_energyreader = NULL;
    ...
    if (block != NULL)
    {
        ...
        snew(awh_energyreader, 1);
        awh_energyreader->nawh = awhbias_params->nawh;
        snew(awh_energyreader->awh_reader, awh_energyreader->nawh);

        /* Keep track of the first subblock of this AWH */
        imin_sub = 0;
        for (int k = 0; k < awh_energyreader->nawh; k++)
        {
            awh_params_t *awh_params = &awhbias_params->awh_params[k];
            awh_reader_t *awh_reader = &awh_energyreader->awh_reader[k];

            awh_reader->nsubblocks = (int)block->sub[imin_sub].dval[0];

            /* We at least print the grid point values (ndim) and the PMF (1) and the metadata (1). */
            awh_reader->nsubblocks_out = bMore ? awh_reader->nsubblocks : awhbias_params->awh_params[k].ndim + 2;
    ...
    return awh_energyreader;
}

static void do_awh_frame(t_enxframe *fr, awh_energyreader_t *awh_energyreader, int *frame_counter, int skip, const gmx_output_env_t *oenv)
{
    t_enxblock *block = NULL;
        ...
        if (!skip || (*frame_counter) % skip == 0)
        {
            ...
            for (int k = 0; k < awh_energyreader->nawh; k++)
            {
                ...
                out = xvgropen(filename, title,
                               "(nm or deg)", awh_reader->ylabel, oenv);
                xvgr_legend(out, awh_reader->nlegend, (const char**)awh_reader->legend, oenv);
                ...
                for (j = 0; j < npoints; j++)
                {
                    for (i = imin_sub + 1; i < imin_sub + awh_reader->nsubblocks_out; i++)
                    {
    ...
    do
    {
        ...
        if ((timecheck == 0) && bCont)
        {
            ...
            /* Currently we have the number of subblocks per AWH stored in the first subblock (we cannot get this directly from the tpr)
               so we have to read an AWH block before making the legends. Possibly its better to call init_awhbias and get the number
               of subblocks and therefore the number of legends that way instead (TODO). */
            if (awh_energyreader == NULL)
            {
                awh_energyreader = init_awh_energyreader(ir.awhbias_params, fr, bMore_output);
            }

            do_awh_frame(fr, awh_energyreader, &awh_frame_counter, skip, oenv);
        }
    }
    while (bCont && (timecheck == 0));
