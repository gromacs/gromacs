                                ...
                                const t_enxblock   *block)
{
    ...
    snew(awh_reader->biasReader, awh_reader->nbias);
    ...
    for (int k = 0; k < awh_reader->nbias; k++)
    {
        ...
        BiasReader        *biasReader      = &awh_reader->biasReader[k];
        ...
        outf->numGraph           = moreGraphs ? biasReader->numSubblocks - 2 : 1;
                            ...
                            const gmx_output_env_t *oenv)
{
    ...
    for (int k = 0; k < awh_reader->nbias; k++)
    {
        OutFile *outf = &awh_reader->biasReader[k].awh;
        ...
        imin_sub += awh_reader->biasReader[k].numSubblocks;
    }
}
