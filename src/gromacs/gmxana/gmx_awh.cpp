...
struct OutFile
{
    ...
    int           numLegend;            /**< Number of legends, includes dims. */
...
static void makeLegend(const AwhBiasParams *awhBiasParams,
                       int awhGraphType,
                       int numLegend, char **legend)
{
    ...
    GMX_RELEASE_ASSERT(legendIndex == numLegend, "The number of legends requested for printing and the number generated should be the same");
}
                                ...
                                gmx_bool             moreGraphs)
{
    ...
    outf->numLegend          = ndim - 1 + outf->numGraph;

    snew(outf->legend, outf->numLegend);
    makeLegend(awhBiasParams, awhGraphTypeAwh,
               outf->numLegend, outf->legend);
                                     ...
                                     const AwhBiasParams *awhBiasParams)
{
    ...
    outf->numLegend          = awhBiasParams->ndim - 1 + outf->numGraph;
    snew(outf->legend, outf->numLegend);
    makeLegend(awhBiasParams, awhGraphTypeFriction,
               outf->numLegend, outf->legend);
    outf->xLabel             = "(nm or deg)";
    outf->yLabel             = "(nm\\S-2\\Nps or rad\\S-2\\Nps)";
}
                     ...
                     const t_enxblock *block)
{
    ...
    for (int k = 0; k < awhParams->numBias; k++)
    {
        ...
        if (outputFrictionTensor_)
        {
            ...
            initOutfileSetupFriction(fric, subblockStart, numSubblocks, awhBiasParams);
        }
                                ...
                                const gmx_output_env_t *oenv)
{
    ...
    xvgr_legend(fp, outFile->numLegend, (const char**)outFile->legend, oenv);
