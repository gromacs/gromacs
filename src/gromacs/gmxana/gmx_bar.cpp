    ...
    *sc = *sc_orig; /* just copy all fields */
    ...
    for (j = 0; j < sc->nsamples; j++)
    {
        ...
        if (!sc->s[j]->hist)
        {
            ...
            GMX_ASSERT(new_start <= std::numeric_limits<int>::max(), "Value of 'new_start' too large for int converstion");
            GMX_ASSERT(new_end <= std::numeric_limits<int>::max(), "Value of 'new_end' too large for int converstion");
                                ...
                                double *Wmin, double *Wmax)
{
    ...
    *Wmax = std::numeric_limits<float>::min();
                           ...
                           double temp, double dg, double *stddev)
{
    ...
    *stddev = std::sqrt(((1./sigmafact) - ( (n1+n2)/n1 + (n1+n2)/n2 )));
}
...
static void read_barsim_edr(char *fn, real *temp, sim_data_t *sd)
{
    ...
    while (do_enx(fp, fr))
    {
        ...
        if (nsamples > 0)
        {
            ...
            if ( (fabs(last_t - start_time) > 2*delta_time)  && last_t >= 0 && samples_rawdh != NULL)
            {
