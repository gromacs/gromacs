...
int gmx_dos(int argc, char *argv[])
{
    ...
    real            T_CP, T_S, T_A, T_E, dofT, T_f, T_diff; /* total quantities*/
    int             t, ai, lid;
    atom_id        *index, *Index;
    t_atom         *atom;
    ...
    T_CP       = 0.0;       T_S = 0.0;        T_A = 0.0;
    T_E        = 0.0;        T_f = 0.0;        T_diff = 0.0;
...
/* read velocities and coordinated of all atoms: each atoms is pointed by j */
        ...
        for (id = 0; id < Nmol; id++) // Nmol: number of molecules
        {
            ...
            if (Natmxmol > 1.0)
            {
                ...
                // void principal_comp(int n, atom_id index[], t_atom atom[], rvec x[], matrix trans, rvec d)
                principal_comp(Natmxmol, Index, atom, r1, trans, I);
#if 0
                if (D)
                {
                ...
                if (I[XX] > 0.0)
                {
                    for (k = 0; k < 3; k++)
                    {
                        pomega[XX] += angmom[k]*trans[k][XX]/I[XX];
                    }
                }
                if (I[YY] > 0.0)
                {
                    for (k = 0; k < 3; k++)
                    {
                        pomega[YY] += angmom[k]*trans[k][YY]/I[YY];
                    }
                }
                if (I[ZZ] > 0.0)
                {
                    for (k = 0; k < 3; k++)
                    {
                        pomega[ZZ] += angmom[k]*trans[k][ZZ]/I[ZZ];
                    }
                }
                // calculate angular velocities
                for (k = 0; k < 3; k++)
                {
                    omega[XX] += pomega[k]*trans[XX][k];
                }
                for (k = 0; k < 3; k++)
                {
                    omega[YY] += pomega[k]*trans[YY][k];
                }
                for (k = 0; k < 3; k++)
                {
                    omega[ZZ] += pomega[k]*trans[ZZ][k];
                }
                // calculate inertia weighted angular velocities and save
                for (k = 0; k < 3; k++)
                {
                    vax += pomega[k]*trans[XX][k]*sqrt(I[k]);
                }
                for (k = 0; k < 3; k++)
                {
                    vay += pomega[k]*trans[YY][k]*sqrt(I[k]);
                }
                for (k = 0; k < 3; k++)
                {
                    vaz += pomega[k]*trans[ZZ][k]*sqrt(I[k]);
                }
                // save angular velocities of molecule id, weighted by sqrt(I)
                vec[V_A][id*DIM+XX][nframes] = vax;
                vec[V_A][id*DIM+YY][nframes] = vay;
                vec[V_A][id*DIM+ZZ][nframes] = vaz;
 ...
 */
    ...
    else
    {
        ...
        dof[V_T]      = 3.0;
        dof[V_A]      = 3.0;
        dof[V_V]      = 3.0*Natmxmol-6.0;
        dof[V_R]      = 3.0;
        dof[VEL_TOT]  = 3.0;
        dofT          = dof[V_T]+dof[V_A]+dof[V_V];
        vecT[V_T]     = dos[V_T][0]/(dof[V_T]*(1-corr/dofT)*BOLTZ*Nmol);
        vecT[V_A]     = dos[V_A][0]/(dof[V_A]*(1-corr/dofT)*BOLTZ*Nmol);
        vecT[V_V]     = dos[V_V][0]/(dof[V_V]*(1-corr/dofT)*BOLTZ*Natom);
        vecT[V_R]     = dos[V_R][0]/(dof[V_R]*(1-corr/dofT)*BOLTZ*Natom);
        vecT[VEL_TOT] = dos[VEL_TOT][0]/(dof[VEL_TOT]*BOLTZ*Natom);
...
//compute density of state for each kind of velocities, return in same vector
    ...
    fclose(fp);
    /* Normalize it */
    dostot[VEL_TOT] = evaluate_integral(nframes/4, nu, dos[VEL_TOT], NULL, nframes/4, &stddev);
    if (bNormalize)
    {
        for (j = 0; (j < nframes/4); j++)
        {
...
#if 0 //original dos
      /* Analyze the translational DoS[VEL_TOT] */
    DoS0 = dos[VEL_TOT][0];

    mmm   = tmass/Natom;
    Delta = ((2*DoS0/(9*Natom))*sqrt(M_PI*BOLTZ*vecT[VEL_TOT]*Natom/tmass)*pow((Natom/V), 1.0/3.0)*pow(6/M_PI, 2.0/3.0)); // eq 13 JPC B, 114, 8191 (2010)
    f     = calc_fluidicity(Delta, toler);
    y     = calc_y(f, Delta, toler);
    z     = calc_compress(y);
    Sig   = BOLTZ*(5.0/2.0+log(pow(2*mmm*M_PI*BOLTZ*vecT[VEL_TOT]/(sqr(PLANCK)), 1.5)*V/(f*Natom))); // eq 16, Lin 2010, Sackur-Tetrode eq.
    Shs   = Sig+calc_Shs(f, y);
    rho   = (tmass*AMU)/(V*NANO*NANO*NANO);
    sigHS = pow(6*y*V/(M_PI*Natom), 1.0/3.0);

    printf("\t\tAnalyzing indipendent terms\n");
    fprintf(stdout, "Delta = %g\n", Delta);
    fprintf(stdout, "fluidicity f = %g\n", f);
    fprintf(stdout, "hard sphere packing fraction fy = %g\n", f*y);
    fprintf(stdout, "hard sphere compressibility z = %g\n", z);
    fprintf(stdout, "ideal gas entropy Sig = %g\n", Sig);
    fprintf(stdout, "hard sphere entropy Shs = %g\n", Shs);
    fprintf(stdout, "sigma_HS = %g nm\n", sigHS);
    ...
    for (j = 0; (j < nframes/4); j++)
    {
        ...
        dos[DOS_SOLID][j] = dos[VEL_TOT][j]-dos[DOS_DIFF][j];

        dos[DOS_CP][j] = (dos[DOS_DIFF][j]*wCdiff +
                          dos[DOS_SOLID][j]*wCsolid(nu[j], beta[VEL_TOT]));
        dos[DOS_S][j]  = (dos[DOS_DIFF][j]*wSdiff +
                          dos[DOS_SOLID][j]*wSsolid(nu[j], beta[VEL_TOT]));
        dos[DOS_A][j]  = (dos[DOS_DIFF][j]*wAdiff +
                          dos[DOS_SOLID][j]*wAsolid(nu[j], beta[VEL_TOT]));
        dos[DOS_E][j]  = (dos[DOS_DIFF][j]*wEdiff +
                          dos[DOS_SOLID][j]*wEsolid(nu[j], beta[VEL_TOT]));
    }
    ...
    fprintf(stdout, "Diffusion coefficient from DoS0 %g 10^-5 cm^2/s\n", 1000*DoS0/(12*tmass*beta[VEL_TOT]));

    cP = BOLTZ * evaluate_integral(nframes/4, nu, dos[DOS_CP], NULL, nframes/4, &stddev);

    S  = BOLTZ * evaluate_integral(nframes/4, nu, dos[DOS_S], NULL, nframes/4, &stddev);

    A  = BOLTZ * Temp * evaluate_integral(nframes/4, nu, dos[DOS_A], NULL, nframes/4, &stddev);

    E  = BOLTZ * Temp * evaluate_integral(nframes/4, nu, dos[DOS_E], NULL, nframes/4, &stddev);
    ...
    mmm   = tmass/Natom;
    Delta = ((2*DoS0/(9*Natom))*sqrt(M_PI*BOLTZ*vecT[V_T]*Natom/tmass)*pow((Natom/V), 1.0/3.0)*pow(6/M_PI, 2.0/3.0)); // eq 13 JPC B, 114, 8191 (2010)
    f     = calc_fluidicity(Delta, toler);
    y     = calc_y(f, Delta, toler);
    z     = calc_compress(y);
    T_f  += f;
    Sig   = BOLTZ*(5.0/2.0+log(pow(2*mmm*M_PI*BOLTZ*vecT[V_T]/(sqr(PLANCK)), 1.5)*V/(f*Natom))); // eq 16, Lin 2010, Sackur-Tetrode eq.
    Shs   = Sig+calc_Shs(f, y);
    rho   = (tmass*AMU)/(V*NANO*NANO*NANO);
    sigHS = pow(6*y*V/(M_PI*Natom), 1.0/3.0);
    ...
    fprintf(fplog, "\t\tTranslational quantities\n");
    ...
    dostot[V_T] = evaluate_integral(nframes/4, nu, dos[V_T], NULL, nframes/4, &stddev);
    ...
    if (crT > 0.0)
    {
        ...
        Delta = ((2*DoS0/(9*Natom))*sqrt(M_PI*BOLTZ*vecT[V_A]*Natom/tmass)*pow((Natom/V), 1.0/3.0)*pow(6/M_PI, 2.0/3.0)); // eq 13 JPC B, 114, 8191 (2010)
        f     = calc_fluidicity(Delta, toler);
        y     = calc_y(f, Delta, toler);
        z     = calc_compress(y);
        T_f  += f;
        ...
        dostot[V_A] = evaluate_integral(nframes/4, nu, dos[V_A], NULL, nframes/4, &stddev);
    ...
    if (vecT[V_V] > 0.0)
    {
        ...
        dostot[V_V] = evaluate_integral(nframes/4, nu, dos[V_V], NULL, nframes/4, &stddev);
