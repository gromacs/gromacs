...
static void dump_w(output_env_t oenv, real beta)
{
    ...
    for (nu = 0; (nu < 100); nu += 0.05)
    {
        fprintf(fp, "%10g  %10g  %10g  %10g  %10g\n", beta*PLANCK*nu,
                weight(DOS_CV, LOT_QUANTUM, TM_2PT, C_TOTAL, nu, beta),
                weight(DOS_S,  LOT_QUANTUM, TM_2PT, C_TOTAL, nu, beta),
                weight(DOS_A,  LOT_QUANTUM, TM_2PT, C_TOTAL, nu, beta),
                weight(DOS_E,  LOT_QUANTUM, TM_2PT, C_TOTAL, nu, beta));
    }
    fclose(fp);
}
...
#define NFILE asize(fnm)
    ...
    for (t = 0; (t < V_NR); t++)
    {
        ...
        else /* t == V_V */
        {
            ...
            {
                int mm;
                for (mm = 0; (mm < LOT_NR); mm++)
                {
                    dp[t].dd[mm][TM_2PT].dos[C_GAS][j] = 0;
                }
            }
        }
        ...
        {
            int kk, mm;
            for (kk = 0; (kk < LOT_NR); kk++)
            {
                for (mm = 0; (mm < TM_NR); mm++)
                {
                    dp[t].dd[kk][mm].dos[C_TOTAL][j] = dp[t].alldos[j];
                    if (TM_2PT == mm)
                    {
                        dp[t].dd[kk][mm].dos[C_SOLID][j] = (dp[t].dd[kk][mm].dos[C_TOTAL][j]-
                                                            dp[t].dd[kk][mm].dos[C_GAS][j]);
                    }
                }
            }
        }
        ...
        }
        {
            ...
            for (i = 0; (i < DOS_NR); i++)
            {
                ...
                for (j = 0; (j < nfreq); j++)
                {
                    for (ll = 0; (ll < LOT_QUANTUM_CORRECTION); ll++)
                    {
                        for (mm = 0; (mm < TM_NR); mm++)
                        {
                            if (TM_2PT == mm)
                            {
                                dp[t].dd[ll][mm].wdos[i][C_SOLID][j] =
                                    (dp[t].dd[ll][mm].dos[C_SOLID][j] *
                                     weight(i, ll, mm, C_SOLID, nu[j], dp[t].beta));
                                dp[t].dd[ll][mm].wdos[i][C_GAS][j]   =
                                    (dp[t].dd[ll][mm].dos[C_GAS][j] *
                                     dp[t].wDiff[i]);
                                dp[t].dd[ll][mm].wdos[i][C_TOTAL][j] =
                                    (dp[t].dd[ll][mm].wdos[i][C_SOLID][j] +
                                     dp[t].dd[ll][mm].wdos[i][C_GAS][j]);
                            }
                            else
                            {
                                dp[t].dd[ll][mm].wdos[i][C_TOTAL][j] =
                                    (dp[t].dd[ll][mm].dos[C_TOTAL][j] *
                                     weight(i, ll, mm, C_TOTAL, nu[j], dp[t].beta));
                            }
                        }
                        for (mm = 0; (mm < TM_NR); mm++)
                        {
                            ...
                            for (cc = 0; (cc < C_NR); cc++)
                            {
                                dp[t].dd[LOT_QUANTUM_CORRECTION][mm].wdos[i][cc][j] =
                                    (dp[t].dd[LOT_QUANTUM][mm].wdos[i][cc][j] -
                                     dp[t].dd[LOT_CLASSICAL][mm].wdos[i][cc][j]);
                            }
                        }
                    }
                }
                for (ll = 0; (ll < LOT_NR); ll++)
                {
                    for (mm = 0; (mm < TM_NR); mm++)
                    {
                        for (k = 0; (k < C_NR); k++)
                        {
                            dp[t].dd[ll][mm].X[i][k] =
                                fff[i]*evaluate_integral(nfreq, nu,
                                                         dp[t].dd[ll][mm].wdos[i][k],
                                                         NULL, 0, &stddev);
                        }
                    }
                }
    ...
    for (t = 0; (t <= V_ATOM); t++)
    {
        ...
        dp[t].Emd = (Emd*dp[t].dof/dp[V_ATOM].dof)/Nmol;
        ...
        for (ll = 0; (ll < LOT_QUANTUM_CORRECTION); ll++)
        {
            for (mm = 0; (mm < TM_NR); mm++)
            {
                for (i = 0; (i < C_NR); i++)
                {
                    dp[t].dd[ll][mm].X[DOS_A][i] += E0[i];
                    dp[t].dd[ll][mm].X[DOS_E][i] += E0[i];
                }
            }
        }
        ...
        for (k = 0; (k < DOS_NR); k++)
        {
            ...
            for (ll = 0; (ll < LOT_NR); ll++)
            {
                for (mm = 0; (mm < TM_NR); mm++)
                {
                    for (kk = 0; (kk < C_NR); kk++)
                    {
                        dp[V_SUM].dd[ll][mm].X[k][kk] += dp[t].dd[ll][mm].X[k][kk];
                    }
                }
            }
        }
                ...
                for (ll = 0; (ll < LOT_NR); ll++)
                {
                    ...
                    for (j = j0; (j < C_NR); j++)
                    {
                        ...
                        fprintf(fplog, "%-2s  %-10s  %-6s %-6s",
                                dosShort[i], lotName[ll], tmName[mm], cName[j]);
                        for (t = 0; (t <= V_SUM); t++)
                        {
                            if ((dp[t].dof == 0) || (dp[t].npart == 0))
                            {
                            ...
                            fprintf(fplog, "  %10g", dp[t].dd[ll][mm].X[i][j]);
                        }
                        fprintf(fplog, " %s\n", dosUnit[i]);
                    }
                }
