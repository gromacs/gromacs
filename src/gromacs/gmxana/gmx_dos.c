...
enum SolidDiff {
    DS_1PT, DS_2PT, DS_SOLID, DS_DIFF, DS_NR
};
...
static real wCQsolid(real nu, real beta)
{
    return wCsolid(nu, beta) - 1.0;
}
...
static real wEcsolid(real gmx_unused nu, real gmx_unused beta) /* weight function energy for solid, eq 17 (2013) */
{
    return 1;
}
...
static real wEQCsolid(real nu, real beta)
{
    return wEsolid(nu, beta) - 1.0;
}
    ...
    if (NULL != mvacf)
    {
        ...
        for (j = 0; (j < n); j++)
        {
            ...
            for (t = 0; (t < V_SUM); t++)
            {
                ...
                {
                    fprintf(fp, "  %10g", dp[t].dos[DS_2PT][j]);
                }
            }
            fprintf(fp, "\n");
        }
                      ...
                      output_env_t oenv)
{
    ...
    for (j = 0; (j < n); j++)
    {
        ...
        for (t = 0; (t < V_SUM); t++)
        {
            if ((dp[t].npart > 0) && (dp[t].dof > 0))
            {
                fprintf(fp, " %10g", dp[t].dos[DS_2PT][j]/recip_fac);
            }
        }
        fprintf(fp, "\n");
    }
                                ...
                                output_env_t oenv)
{
    ...
    const char *DoSlegend[] = {
        "DoS(v) (1PT)", "DoS(v) (2PT)", "DoS(v)[Solid]", "DoS(v)[Diff]"
    };
    ...
    for (j = 0; (j < n); j++)
    {
        fprintf(fp, "%10g  %10g  %10g  %10g  %10g\n",
                recip_fac*nu[j],
                dp[t].dos[DS_1PT][j]/recip_fac,
                dp[t].dos[DS_2PT][j]/recip_fac,
                dp[t].dos[DS_SOLID][j]/recip_fac,
                dp[t].dos[DS_DIFF][j]/recip_fac);
    }
    fclose(fp);
}
...
#define NFILE asize(fnm)
    ...
    for (t = 0; (t < V_NR); t++)
    {
        if ((dp[t].dof > 0) && (dp[t].npart > 0))
        {
            dp[t].T    = dp[t].dos[DS_2PT][0]/(dp[t].dof*BOLTZ*Nmol);
    ...
    for (t = 0; t < V_NR; t++)
    {
        if ((dp[t].dof > 0) && (dp[t].npart > 0))
        {
            ...
            for (j = 0; (j < N_tot); j++)
            {
                in[j][0] = dp[t].bfac*dp[t].dos[DS_2PT][j];
                in[j][1] = 0;
            }
            for (; (j < N_tot); j++)
            {
            ...
            for (j = 0; (j < nfreq); j++)
            {
                dp[t].dos[DS_2PT][j] = sqrt(sqr(out[j][0])+sqr(out[j][1]));
            }
            sfree(in);
            sfree(out);
        }
...
#ifdef HACK
    {
        ...
        for (j = 0; (j < nline); j++)
        {
            ...
            {
                nnu                    = nnu/recip0;
                dp[V_V].dos[DS_2PT][j] = ddos*recip0;
    ...
    for (t = 0; (t < V_NR); t++)
    {
        ...
        if (V_V != t)
        {
            ...
            dp[t].ttdf  = evaluate_integral(nfreq, nu, dp[t].dos[DS_2PT],
                                            NULL, 0, &stddev);
        ...
        for (j = 0; (j < nfreq); j++)
        {
            dp[t].dos[DS_SOLID][j] = dp[t].dos[DS_2PT][j]-dp[t].dos[DS_DIFF][j];
            dp[t].dos[DS_1PT][j]   = dp[t].dos[DS_2PT][j];
        }
        dp[t].dostot = evaluate_integral(nfreq, nu, dp[t].dos[DS_2PT], NULL, 0, &stddev);
            ...
            for (i = 0; (i < DOS_NR); i++)
            {
                for (j = 0; (j < nfreq); j++)
                {
                    dp[t].wdos[i][DS_SOLID][j] = (dp[t].dos[DS_SOLID][j]*
                                                  wSolid[i](nu[j], dp[t].beta));
                    dp[t].wdos[i][DS_DIFF][j]  = (dp[t].dos[DS_DIFF][j]*
                                                  dp[t].wDiff[i]);
                    dp[t].wdos[i][DS_2PT][j]   = (dp[t].wdos[i][DS_SOLID][j] +
                                                  dp[t].wdos[i][DS_DIFF][j]);
                    dp[t].wdos[i][DS_1PT][j]   = (dp[t].dos[DS_1PT][j]*
                                                  wSolid[i](nu[j], dp[t].beta));
                }
                for (k = 0; (k < DS_NR); k++)
                {
    ...
    for (t = 0; (t <= V_ATOM); t++)
    {
        ...
        dp[t].E0  = dp[t].Emd/Nmol - dp[t].X[DOS_EC][DS_2PT];
    ...
    fprintf(fplog, "%-30s = %10.3f (2PT method)\n",
            dosLong[DOS_A], dp[V_SUM].X[DOS_A][DS_2PT]);
    fprintf(fplog, "%-30s = %10.3f (2PT method)\n",
            dosLong[DOS_S], dp[V_SUM].X[DOS_S][DS_2PT]);
    fprintf(fplog, "%-30s = %10.3f (Berens et al. method)\n",
            dosLong[DOS_CV], cVclassical+dp[V_SUM].X[DOS_CVQ][DS_2PT]);
    fprintf(fplog, "%-30s = %10.3f (Berens et al. method)\n",
            dosLong[DOS_E], Emd/Nmol+dp[V_SUM].X[DOS_EQC][DS_2PT]);
