...
int gmx_dos(int argc, char *argv[])
{
    ...
    double              rho, dt, V2sum, Vsum, V, tmass, *dos2, dosabs, molMass;
...
/* read velocities and coordinated of all atoms: each atoms is pointed by j */
...
#if 1
    fprintf(stdout, "T[V_T] %g T[V_A] %g T[V_V] %g T[V_R] %g T[VEL_TOT] %g\n",
            dp[V_T].T, dp[V_A].T, dp[V_V].T, dp[V_R].T, dp[VEL_TOT].T);
#endif
     ...
     */
    V0 = (Emd - 3*top.atoms.nr*BOLTZ*dp[VEL_TOT].T)/top.mols.nr;

    molMass   = tmass/top.mols.nr;
    ...
    for (t = V_T; (t < VEL_NR); t++)
    {
        ...
        if (V_V != t)
        {
            ...
            dp[t].Delta = ((2*dp[t].DoS0/(9*dp[t].npart))*sqrt(M_PI*BOLTZ*dp[t].T*dp[t].npart/tmass)*
            ...
            dp[t].Sig   = BOLTZ*(5.0/2.0+log(pow(2*molMass*M_PI*BOLTZ*dp[t].T/
                                                 (sqr(PLANCK)),
        ...
        if (Natmxmol > 1)
        {
            ...
            dp[t].S  = KILO * fac * evaluate_integral(nfreq, nu, dos[DOS_S], NULL,
                                                      nfreq, &stddev);
            dp[t].A  = V0 + fac * Temp * evaluate_integral(nfreq, nu, dos[DOS_A], NULL,
                                                           nfreq, &stddev);
            dp[t].E  = V0 + fac * Temp * evaluate_integral(nfreq, nu, dos[DOS_E], NULL,
                                                           nfreq, &stddev);
