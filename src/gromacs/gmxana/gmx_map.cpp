...
#define NPA asize(pa)
    ...
    if (opt2bSet("-mi", NFILE, fnm) )
    {
        ...
        referenceMap = gmx::gridDataToMapData(result);
        fprintf(stdout, "%s\n", metadata.to_string().c_str());

        ref_spacing = gmx::get_map_spacing(referenceMap, stderr);
    }
    else
    {
    ...
    if (opt2bSet("-f", NFILE, fnm) )
    {
        ...
        if (referenceMap.vox.size() == 0)
        {
            ...
            for (i = 0; i < 3; i++)
            {
                referenceMap.cell   [i]   = NM2A*box[i][i];
                /* Set box angles to a default of 90 degrees */
                referenceMap.cell   [i+3] = 90.0;
                referenceMap.map_dim[i]   = referenceMap.cell[i] / (NM2A*grid_spacing);
                /* Choose grid equal to map_dim */
                referenceMap.grid   [i] = referenceMap.map_dim[i];
                /* Set a default axes order */
                referenceMap.axes_order[i] = i+1;
            }
            gmx::allocate_density_grid(referenceMap.map_dim, &referenceMap.vox);
        }
        else
        {
...
//            put_atoms_in_box(ePBC, box, natoms, pos);
            ...
            for (i = 0; i < 3; i++)
            {
                box[i][i] = A2NM * referenceMap.cell[i];
            }
        ...
        do
        {
            if (0 == iframe % skip)
            {
                ...
                if (opt2bSet("-mi", NFILE, fnm))
                {
                    gmx::get_map_spacing(referenceMap, stdout);
                    densfit.setSpacingFromMap(referenceMap);
                }
                ...
                auto gridData = gmx::mapDataToGridData(densfit.simulatedMap());
    ...
    else if (opt2bSet("-mi", NFILE, fnm) && ((scale != 1.0) || bPositive ) )
    {
        if (bPositive)
        {
            ...
            gmx::make_positive(&(referenceMap));
        }
        if (scale != 1.0)
        {
            fprintf(stdout, "\nWill rescale the input density map by a factor of %g in each dimension.\n", scale);
            referenceMap = gmx::rescale_map(referenceMap, scale);
        }
    }
    ...
    if (opt2bSet("-mo", NFILE, fnm) && !bWroteMap)
    {
        if (referenceMap.vox.size() != 0)
        {
            ...
            auto gridData = gmx::mapDataToGridData(referenceMap );
