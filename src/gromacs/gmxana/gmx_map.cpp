...
#define NPA asize(pa)
    ...
    if (!densfit)
    {
        densfit = gmx::new_t_densfit(sigma, sigma_dist, 0, natoms, ind, grid_spacing, bVerbose);
    }
    ...
    {
        /* Read input file (map) */
        gmx::do_map_ccp4(TRUE, &densfit->map_ref, opt2fn("-mi", NFILE, fnm), FALSE, TRUE, stdout);
...
//            put_atoms_in_box(ePBC, box, natoms, pos);
        ...
        do
        {
            if (0 == iframe % skip)
            {
                ...
                gmx::assemble_atoms_for_spread(densfit, pos);

                /* The reference map grows and shrinks with the box */
                if (opt2bSet("-mi", NFILE, fnm))
                {
                    gmx::get_map_spacing(densfit->map_ref, stdout);
                    gmx::couple_map_spacing_to_box(box, densfit);
                    gmx::get_map_spacing(densfit->map_ref, stdout);
                }

                gmx::spread_atoms(densfit, box);

                /* Calculate the correlation coefficient to the input map */
                if (opt2bSet("-mi", NFILE, fnm) )
                {
                    gmx::calc_correlation_coeff(densfit, stdout);
                }
                ...
                gmx::do_map_ccp4(FALSE, &densfit->map_sim, fn_map, FALSE, bVerbose, bVerbose ? stdout : NULL);
                bWroteMap = TRUE;
            }
            /* Read the next frame */
            if (is_trajectory(fn_in))
    ...
    if (opt2bSet("-mo", NFILE, fnm) && !bWroteMap)
    {
        if (densfit->map_ref)
        {
            ...
            gmx::do_map_ccp4(FALSE, &densfit->map_ref, opt2fn("-mo", NFILE, fnm), FALSE, bVerbose, stdout);
        }
        else
        {
            /* What went wrong? */
