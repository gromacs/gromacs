...
#define NPA asize(pa)
    ...
    gmx::LocalAtomSetManager atomSets(false);
    /* Initialize global and local density fitting data structures */
    if (ir.densfit == nullptr)
    {
        ir.densfit    = std::unique_ptr<gmx::Densfit>(new gmx::Densfit(sigma, sigma_dist, 0, natoms, ind, grid_spacing, bVerbose, atomSets.add(natoms, ind)));
    }
    ...
    if (opt2bSet("-mi", NFILE, fnm) )
    {
        ...
        referenceMap = gmx::MrcFile().read_with_meta(opt2fn("-mi", NFILE, fnm), &metadata);
        fprintf(stdout, "%s\n", metadata.to_string().c_str());
        ref_spacing = gmx::get_map_spacing(referenceMap.getGrid(), stderr);
    }
    else
    {
    ...
    if (opt2bSet("-f", NFILE, fnm) )
    {
        ...
        if (referenceMap.size() == 0)
        {
            ...
            std::array<real, DIM> cell;
            for (int i = 0; i < 3; i++)
            {
                cell[i]    = box[i][i];
                map_dim[i] = cell[i] / grid_spacing;
            }
            gmx::Grid<3> gridFromBox(cell, map_dim);

            referenceMap.setGrid(gridFromBox.duplicate());
        }
        else
        {
...
//            put_atoms_in_box(ePBC, box, natoms, pos);
            ...
            for (int i = 0; i < 3; i++)
            {
                box[i][i] = referenceMap.getGrid().cell().basisVectorLength(i);
            }
        ...
        densfit.new_map_sim_from_ref(referenceMap);
        ...
        do
        {
            if (0 == iframe % skip)
            {
                ...
                gmx::MrcFile().write(fn_map, densfit.simulatedMap());
                if (bVerbose)
                {
                    gmx::MrcMetaData meta;
                    meta.fromGrid(densfit.simulatedMap().getGrid());
                    meta.set_grid_stats(densfit.simulatedMap());
    ...
    if (opt2bSet("-mo", NFILE, fnm) && !bWroteMap)
    {
        if (referenceMap.size() != 0)
        {
            ...
            gmx::MrcFile().write(opt2fn("-mo", NFILE, fnm), referenceMap);
            if (bVerbose)
            {
                gmx::MrcMetaData meta;
                meta.fromGrid(referenceMap.getGrid());
                meta.set_grid_stats(referenceMap);
                fprintf(stdout, "%s\n", meta.to_string().c_str());
            }
        }
        else
