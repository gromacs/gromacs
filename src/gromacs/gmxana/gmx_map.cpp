...
#define NPA asize(pa)
    ...
    t_densfit        *densfit         = NULL; /* All density fitting data       */
    ...
    /* Set precision of erf and exp evaluations */
    gmx_set_vml_precision(stdout);
    ...
    if (!densfit)
    {
        densfit = new_t_densfit(sigma, sigma_dist, 0, natoms, ind, grid_spacing, bVerbose);
    }
    ...
    {
        /* Read input file (map) */
        gmx_do_map_ccp4(TRUE, &densfit->map_ref, opt2fn("-mi", NFILE, fnm), FALSE, TRUE, stdout);
        ref_spacing = get_map_spacing(densfit->map_ref, stderr);
    }
    ...
    if (opt2bSet("-f", NFILE, fnm) )
    {
        ...
        if (NULL == densfit->map_ref)
        {
            ...
            allocate_density_grid(densfit->map_ref->map_dim, &densfit->map_ref->vox);
        }
        else
        {
        ...
        do
        {
            if (0 == iframe % skip)
            {
                ...
                assemble_atoms_for_spread(densfit, pos);

                /* The reference map grows and shrinks with the box */
                if (opt2bSet("-mi", NFILE, fnm))
                {
                    get_map_spacing(densfit->map_ref, stdout);
                    couple_map_spacing_to_box(box, densfit);
                    get_map_spacing(densfit->map_ref, stdout);
                }

                spread_atoms(densfit, box);

                /* Calculate the correlation coefficient to the input map */
                if (opt2bSet("-mi", NFILE, fnm) )
                {
                    calc_correlation_coeff(densfit, stdout);
                }

                /* If needed, attach frame numbers to output files */
                if (is_trajectory(fn_in))
                {
                    make_filename(opt2fn("-mo", NFILE, fnm), ndigit, iout++, fn_map);
                }

                /* Write out simulated map */
                gmx_do_map_ccp4(FALSE, &densfit->map_sim, fn_map, FALSE, bVerbose, bVerbose ? stdout : NULL);
                bWroteMap = TRUE;
            }
            /* Read the next frame */
            if (is_trajectory(fn_in))
    ...
    else if (opt2bSet("-mi", NFILE, fnm) && ((scale != 1.0) || bPositive ) )
    {
        if (bPositive)
        {
            ...
            make_positive(densfit->map_ref);
        }
        if (scale != 1.0)
        {
            fprintf(stdout, "\nWill rescale the input density map by a factor of %g in each dimension.\n", scale);
            densfit->map_ref = rescale_map(densfit->map_ref, scale);
        }
    }
    ...
    if (opt2bSet("-mo", NFILE, fnm) && !bWroteMap)
    {
        if (densfit->map_ref)
        {
            ...
            gmx_do_map_ccp4(FALSE, &densfit->map_ref, opt2fn("-mo", NFILE, fnm), FALSE, bVerbose, stdout);
        }
        else
        {
            /* What went wrong? */
