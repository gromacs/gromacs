...
{
    FILE      *fp = NULL;
    real       e[2], emin, prob, enorm;
    int        i, j, isw, jsw, nn, nuphill = 0;
    gmx_rng_t  rng;
    t_mat     *minimum;
    int        cur = 0;
#define next (1-cur)
    ...
    swap_rows(m, 0, isw);
    swap_rows(m, m->n1-1, jsw);
    emin = e[cur] = mat_energy2(m);
    ...
    for (i = 0; (i < maxiter); i++)
    {
        ...
        do
        {
            isw = 1+(nn-2)*gmx_rng_uniform_real(rng);
            jsw = 1+(nn-2)*gmx_rng_uniform_real(rng);
        }
        while ((isw == jsw) || (isw >= nn-1) || (jsw >= nn-1));

        /* Apply swap and compute energy */
        swap_rows(m, isw, jsw);
        e[next] = mat_energy2(m);
        
        /* Compute probability */
        prob = 0;
        if ((e[next] < e[cur]) || (i < nrandom))
        {
            prob = 1;
            if (e[next] < emin)
            {
        ...
        else if (kT > 0)
        {
            ...
            prob = exp(-(e[next]-e[cur])/(enorm*kT));
        }
        ...
        {
            swap_rows(m, jsw, isw);
            /* fprintf(log, "Iter: %d Not swapping %4d and %4d (energy: %g prob: %g)\n",
               i, isw, jsw, e[next], prob); */
        }
    }
    ...
    fprintf(log, "Globabl minimum energy %g\n", mat_energy2(minimum));
    fprintf(log, "Globabl minimum energy %g\n", mat_energy2(m));
