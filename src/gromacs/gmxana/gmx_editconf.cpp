...
#define NFILE asize(fnm)
    ...
    if (btype[0] != NULL && (btype[0][0] == 'c' || btype[0][0] == 'd' || btype[0][0] == 'o'))
    {
        bCalcDiam = TRUE;
    }
    else
    ...
    if (bScale)
    {
        ...
        if (bRho)
        {
            ...
            scale[XX] = scale[YY] = scale[ZZ] = pow(dens/rho, static_cast<real>(1.0/3.0));
        ...
        switch (btype[0][0])
        {
            ...
            case 'o':
                ...
                else if (btype[0][0] == 'd')
                {
                    ...
                    box[ZZ][XX] = d/2;
                    box[ZZ][YY] = d/2;
                    box[ZZ][ZZ] = d*sqrt(2.0)/2.0;
                }
                else
                {
                    box[XX][XX] = d;
                    box[YY][XX] = d/3;
                    box[YY][YY] = d*sqrt(2.0)*2.0/3.0;
                    box[ZZ][XX] = -d/3;
                    box[ZZ][YY] = d*sqrt(2.0)/3.0;
                    box[ZZ][ZZ] = d*sqrt(6.0)/3.0;
                }
                break;
        }
    }
