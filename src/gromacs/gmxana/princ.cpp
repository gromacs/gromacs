...
void principal_comp(int n, const int index[], gmx::ArrayRef<const AtomInfo> atom, rvec x[],
                    matrix trans, rvec d)
{
    int      i, j, ai, m, nrot;
    real     mm, rx, ry, rz;
...
real calc_xcm(const rvec x[], int gnx, const int *index, gmx::ArrayRef<const AtomInfo> atom, rvec xcm,
              gmx_bool bQ)
{
    int  i, ii, m;
    real m0, tm;
...
real sub_xcm(rvec x[], int gnx, const int *index, gmx::ArrayRef<const AtomInfo> atom, rvec xcm,
             gmx_bool bQ)
{
    int  i, ii;
    real tm;
...
void orient_princ(gmx::ArrayRef<const AtomInfo> atoms, int isize, const int *index,
                  int natoms, rvec x[], rvec *v, rvec d)
{
    ...
    calc_xcm(x, isize, index, atoms, xcm, FALSE);
    for (i = 0; i < natoms; i++)
    {
        rvec_dec(x[i], xcm);
    }
    principal_comp(isize, index, atoms, x, trans, prcomp);
    if (d)
    {
        copy_rvec(prcomp, d);
    }
