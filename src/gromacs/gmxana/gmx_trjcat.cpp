...
#define NFILE asize(fnm)
    ...
    const std::vector<std::string> &outFiles = opt2fns("-o", NFILE, fnm);
    if (outFiles.empty())
    {
    ...
    if (bDeMux)
    {
        ...
        do_demux(inFiles, fnms_out, n, val, t, dt_remd, isize, index, dt, oenv);
    }
    else
    {
