...
static void scan_trj_files(const std::vector<std::string> &files,
                           real *readtime,
...
static void do_demux(const std::vector<std::string> &inFiles,
                     const std::vector<std::string> &outFiles, int nval,
...
#define NFILE asize(fnm)
    ...
    const std::vector<std::string> &inFiles = opt2fns("-f", NFILE, fnm);
    if (inFiles.empty())
    {
    ...
    std::vector<std::string> outFiles = opt2fns("-o", NFILE, fnm);
    if (outFiles.empty())
    {
    ...
    if (bDeMux)
    {
        if (static_cast<int>(outFiles.size()) != nset)
        {
            std::string name = outFiles[0];
            outFiles.resize(nset);
            for (i = 0; (i < nset); i++)
            {
                outFiles[0] = gmx::formatString("%d_%s", i, name.c_str());
            }
        }
        do_demux(inFiles, outFiles, n, val, t, dt_remd, isize, index, dt, oenv);
    }
    else
    {
        ...
        std::vector<std::string> inFilesEdited = inFiles;
