                ...
                for (j = i+1; (j < nindex); j++)
                {
                    ...
                    if (ci != cj)
                    {
                        ...
                        if (bMol)
                        {
                            ...
                            bSame = FALSE;
                            for (ii = mols.blockStart(ai); !bSame && ii < mols.blockEnd(ai); ii++)
                            {
                                for (jj = mols.blockStart(aj); !bSame && jj < mols.blockEnd(aj); jj++)
                                {
                                    if (bPBC)
                                    {
    ...
    if (max_clust_ind >= 0)
    {
        ...
        for (i = 0; (i < nindex); i++)
        {
            if (clust_index[i] == max_clust_ind)
            {
                ...
                {
                    GMX_RELEASE_ASSERT(mols.numBlocks() > 0, "Cannot access index[] from empty mols");
                    for (j = mols.blockStart(i); j < mols.blockEnd(i); j++)
                    {
                        fprintf(fp, "%d\n", j+1);
                    }
                }
