...
static real tick_spacing(real range, int minticks)
{
    ...
    sp = 0.2*exp(std::log(10.0)*ceil(std::log(range)/std::log(10.0)));
        ...
        real *eigval1, int neig1, real *eigval2, int neig2)
{
    ...
    for (i = n; i < neig2; i++)
    {
        // The static analyzer appears to be confused by the fact that this loop
        // starts from n instead of 0. However, given all the complex code it's
        // better to be safe than sorry, so we check it.
        GMX_RELEASE_ASSERT(eigval2 != NULL, "NULL pointer provided for eigval2 with neig2>0");
        trace2 += eigval2[i];
    }
                    ...
                    const output_env_t oenv)
{
    ...
    if (threedplotfile)
    {
        ...
        if ( ( b4D || bSplit ) && bPDB && inprod != NULL)
        {
