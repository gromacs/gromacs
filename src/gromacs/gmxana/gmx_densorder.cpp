...
static void writesurftoxpms(t_interf ***surf1, t_interf ***surf2, int tblocks, int xbins, int ybins, int zbins, real bw, real bwz, const std::vector<std::string> outfiles, int maplevels )
{
    char   numbuf[STRLEN];
    int    n, i, j;
...
static void writeraw(t_interf ***int1, t_interf ***int2, int tblocks,
                     int xbins, int ybins, const std::vector<std::string> &fnms,
...
/* Calculate axis */
    ...
    if (bGraph)
    {
        ...
        /*Output surface-xpms*/
        const std::vector<std::string> &graphFiles = opt2fns("-og", NFILE, fnm);
...
/*Output raw-data*/
    if (bRawOut)
    {
        const std::vector<std::string> &rawFiles = opt2fns("-or", NFILE, fnm);
        if (rawFiles.size() != 2)
        {
