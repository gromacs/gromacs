...
static void histogramming(FILE *log, int nbin, gmx::ArrayRef<const ResidueType> rt,
                          ...
                          real bfac_max, gmx::ArrayRef<const PdbEntry> pdb,
        ...
        for (i = 0; (i < nlist); i++)
        {
                ...
                ((Dih  > edOmega) && (dlist[i].atm.Cn[Dih-NONCHI+3] != -1)))
            {
                ...
                if (bSSHisto)
                {
                    ...
                    for (nn = 0; (nn < 4); nn++, n++)
                    {
                        bBfac  = bBfac  && (pdb[index[n]].bfac_ <= bfac_max);
                        bOccup = bOccup && (pdb[index[n]].occup_ == 1);
                    }
    ...
    for (i = 0; (i < rt_size); i++)
    {
        for (Dih = 0; (Dih < edMax); Dih++)
        {
                 ...
                 (bChi && (Dih >= edChi1))))
            {
                ...
                residue_name = findResidueName(rt, i);
                switch (Dih)
                {
                    case edPhi:
...
static void order_params(FILE *log,
                         ...
                         std::vector<PdbEntry> *pdb, const rvec x[], int ePBC, matrix box,
...
#define NLEG asize(leg)
    ...
    if (nullptr != pdbfn)
    {
        ...
        {
            pdb->at(dlist[i].atm.N).bfac_ = -dlist[i].S2[0]; /* Phi */
            pdb->at(dlist[i].atm.H).bfac_ = -dlist[i].S2[0]; /* Phi */
            pdb->at(dlist[i].atm.C).bfac_ = -dlist[i].S2[1]; /* Psi */
            pdb->at(dlist[i].atm.O).bfac_ = -dlist[i].S2[1]; /* Psi */
            for (Xi = 0; (Xi < maxchi); Xi++)                      /* Chi's */
            {
                if (dlist[i].atm.Cn[Xi+3] != -1)
                {
                    pdb->at(dlist[i].atm.Cn[Xi+1]).bfac_ = -dlist[i].S2[NONCHI+Xi];
                }
            }
        }
        ...
        write_pdbfile(fp, nullptr, atoms, resinfo,
                      gmx::arrayRefFromArray(pdb->data(), pdb->size()),
                      x, ePBC, box, ' ', 0, nullptr, TRUE);
        x0 = y0 = z0 = 1000.0;
        ...
        for (int i = 0; (i < 10); i++)
        {
            gmx_fprintf_pdb_atomline(fp, epdbATOM, atoms.size()+1+i, "CA", ' ', "LEG", ' ', resinfo.size()+1, ' ',
...
#define NFILE asize(fnm)
    ...
    std::vector<PdbEntry> *pdb = &top->pdb;
    if (pdb->empty())
    {
        pdb->resize(atoms.size());
    }
    ...
    dlist = mk_dlist(log, atoms, resinfo, &nlist, bPhi, bPsi, bChi, bHChi, maxchi, r0, rt);
    ...
    histogramming(log, nbin, rt, nf, maxchi, dih, nlist, dlist, index,
                  ...
                  bNormHisto, bSSHisto, ftp2fn(efDAT, NFILE, fnm), bfac_max,
                  gmx::arrayRefFromArray(pdb->data(), pdb->size()),
    ...
    order_params(log, opt2fn("-o", NFILE, fnm), maxchi, nlist, dlist,
                 ...
                 atoms, resinfo, pdb, x, ePBC, box, bPhi, bPsi, bChi, oenv);
