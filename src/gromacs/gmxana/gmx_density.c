                           ...
                           gmx_bool bRelative, const output_env_t oenv)
{
    ...
    do
    {
        ...
        if (bCenter)
        {
            center_coords(&top->atoms, index_center, ncenter, box, x0, axis);
        }
...
int gmx_density(int argc, char *argv[])
{
    const char        *desc[] = {
        ...
        "profiles along the Z axis box box dimension bZ, output would be from -bZ/2 to",
        "bZ/2 if you center based on the entire system.",
        "Note that this behaviour has changed from Gromacs 5.0; earlier versions",
