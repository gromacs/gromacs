...
GridDataReal3D localCorrelationMap(const GridDataReal3D &map1, const GridDataReal3D &map2, const Mask &mask, float range)
{
    ...
    for (size_t i = 0; i < map1.size(); ++i)
    {
        ...
        {
            *(localCorrelationMap.iteratorAtMultiIndex(latticeIndex)) = (n * sum12 - sum1 * sum2) / sqrt( (n * sum11 - sum1 * sum1) * (n * sum22 - sum2 * sum2) );
        }
    }
...
class MapCompare final : public ToolRunner
{
    public:
...
void MapCompare::initOptions(Options* options)
{
    setHelpText("[THISMODULE] compares two density maps by calculating their"
...
{
    return gmx::MapCompare().run(argc, argv);
}
