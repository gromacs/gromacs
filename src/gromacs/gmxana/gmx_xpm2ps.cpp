...
static void get_params(const char *mpin, const char *mpout, t_psrec *psr)
{
    ...
    findOldEntry(&inp, "titlefont",      psr->titfont,        "Helvetica");
    psr->titfontsize = get_ereal(&inp, "titlefontsize",    20.0, wi);
    psr->legend      = get_eenum(&inp, "legend",         gmx_bools);
    findOldEntry(&inp, "legendfont",     psr->legfont,        psr->titfont);
    findOldEntry(&inp, "legendlabel",    psr->leglabel,       "");
    findOldEntry(&inp, "legend2label",   psr->leg2label,      psr->leglabel);
    ...
    findOldEntry(&inp, "x-label",        psr->X.label,        "");
    psr->X.fontsize = get_ereal(&inp, "x-fontsize",     16.0, wi);
    findOldEntry(&inp, "x-font",         psr->X.font,         psr->titfont);
    psr->X.tickfontsize = get_ereal(&inp, "x-tickfontsize", 10.0, wi);
    findOldEntry(&inp, "x-tickfont",     psr->X.tickfont,     psr->X.font);
    ...
    findOldEntry(&inp, "y-label",        psr->Y.label,        psr->X.label);
    psr->Y.fontsize = get_ereal(&inp, "y-fontsize",     psr->X.fontsize, wi);
    findOldEntry(&inp, "y-font",         psr->Y.font,         psr->X.font);
    psr->Y.tickfontsize = get_ereal(&inp, "y-tickfontsize", psr->X.tickfontsize, wi);
    findOldEntry(&inp, "y-tickfont",     psr->Y.tickfont,     psr->Y.font);
