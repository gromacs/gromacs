                       ...
                       const gmx_output_env_t *oenv)
{
    ...
    {
        t.push_back(output_env_conv_time(oenv, curr->time[i]));
        int k = 0;
        for (int j = 0; j < curr->ngrp; j++)
        {
            data[k++].push_back(curr->data[j][i]);
            if (bTen)
            {
                data[k++].push_back(curr->datam[j][i][XX][XX]);
                data[k++].push_back(curr->datam[j][i][YY][YY]);
                data[k++].push_back(curr->datam[j][i][ZZ][ZZ]);
                data[k++].push_back(curr->datam[j][i][YY][XX]);
                data[k++].push_back(curr->datam[j][i][ZZ][XX]);
                data[k++].push_back(curr->datam[j][i][ZZ][YY]);
            }
        }
    }
    xvg.setTimeAxis(t);
    xvg.setDataSets(data);
    xvg.plot();
}
