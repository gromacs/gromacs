...
struct t_corr {
    ...
    matrix                                             **datam;
    std::vector< std::vector< gmx::BasicVector<real> > > x0;         /* original positions */
    std::vector< gmx::BasicVector<real> >                com;        /* center of mass correction for each frame */
                         ...
                         real beginfit, real endfit)
{
    ...
    curr->ndata.resize(nrgrp);
    curr->data.resize(nrgrp);
    if (bTen)
    {
                      ...
                      gmx_bool bRmCOMM, rvec com, t_calc_func *calc1, gmx_bool bTen)
{
    ...
    if (curr->nlast < curr->nrestart)
    {
        ...
        {
            std::memcpy(curr->x0[curr->nlast].data(), xc, curr->ncoords*sizeof(xc[0]));
                    ...
                    real dt, real beginfit, real endfit, const gmx_output_env_t *oenv)
{
    ...
    nat_trx =
        corr_loop(msd, trx_file, top, ePBC, mol_file ? gnx[0] != 0 : false, gnx.data(), index,
                  (mol_file != nullptr) ? calc1_mol : (bMW ? calc1_mw : calc1_norm),
                  bTen, gnx_com.data(), index_com, dt, t_pdb,
