...
#include "gromacs/hybridMCMD/metropolisinterfaces.h"
...
class AcceptOrRewindTest : public ::testing::Test
{
    public:
        //! Constructor
        AcceptOrRewindTest() : acceptOrRewind_(metropolisStepDummy_)
        {
            // read in water atoms into localState_.x
            localState_.natoms = 51;
            localState_.x.resizeWithPadding(localState_.natoms);
            std::copy(g_positions, g_positions+localState_.natoms, localState_.x.data());
        }
        ...
        void runTest(const int64_t step)
        {
            ...
            accepted_ = acceptOrRewind_.run(step, nullptr, &localState_, nullptr, &commrec_);
            // compare vectors
    ...
    private:
        ...
        bool                              accepted_;
        gmx::PaddedHostVector<gmx::RVec>  xRef_;
};
