 ...
 */
bool AcceptOrRewind::updateTState(const gmx_int64_t     step,                                  // arguments for core functionality
                                  t_state              *localState,
                                  ...
                                  bool                 *shouldCheckNumberOfBondedInteractions) // bool required for signalling in md.cpp
{
    ...
    else
    {
        ...
        {
            t_state globalState;
            dd_collect_state(cr->dd, &localStateBackUp_, &globalState);
...
void AcceptOrRewind::printOutput(FILE *fplog, const int nstlog, const gmx_int64_t step)
{
