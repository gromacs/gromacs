...
//! \brief This class implements the hybrid MC/MD Metropolis step suggested by Mehlig et al. (1992)
class MetropolisStepMehlig : public IMetropolisStep, public IMetropolisStepVelocities
{
    public:
        ...
        bool accept(const gmx_int64_t step, const gmx_enerdata_t *enerd) override;

        //! \brief Destructor
        virtual ~MetropolisStepMehlig() = default;

    private:
        double calculateMetropolisCriterion(const double newPotentialEnergy, const double newKineticEnergy) const;
        double drawRandomNumber(const gmx_int64_t step);
