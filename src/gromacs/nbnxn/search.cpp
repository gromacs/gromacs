...
using nbnxn_bb_t = Nbnxn::BoundingBox; // TODO: Remove when refactoring this file

using Grid       = Nbnxn::Grid;        // TODO: Remove when refactoring this file
    ...
    work(nthread_max)
{
    ...
    grid.resize(numGrids, nb_kernel_type);
...
/* Returns a pointer to the, modifiable, exclusion mask for cj4-unit cj4, warp warp */
                      ...
                      int * gmx_restrict        numDistanceChecks)
{
    ...
    {
        real d2  = subc_bb_dist2(0, bb_ci, jclusterFirst, jGrid.clusterData().boundingBoxes);
        *numDistanceChecks += 2;
    ...
    {
        real d2  = subc_bb_dist2(0, bb_ci, jclusterLast, jGrid.clusterData().boundingBoxes);
        *numDistanceChecks += 2;
...
#include "gromacs/nbnxn/search_simd_4xn.h"
#endif
#ifdef GMX_NBNXN_SIMD_2XNN
#include "gromacs/nbnxn/search_simd_2xnn.h"
#endif
...
#define PRUNE_LIST_CPU_ALL 0
    ...
    for (int subc = 0; subc < jGrid.numClustersPerCell()[scj]; subc++)
    {
        ...
        subc_bb_dist2_simd4_xxxx(jGrid.clusterData().packedBoundingBoxes.data() + (cj >> STRIDE_PBB_2LOG)*NNBSBB_XXXX + (cj & (STRIDE_PBB-1)),
                                 ci1, pbb_ci, d2l);
        *numDistanceChecks += c_nbnxnGpuClusterSize*2;
        ...
        for (int ci = 0; ci < c_gpuNumClusterPerCell; ci++)
        {
            ...
            d2l[ci]             = subc_bb_dist2(ci, bb_ci, cj, jGrid.clusterData().boundingBoxes);
            *numDistanceChecks += 2;
#endif
            float d2 = d2l[ci];
                          ...
                          t_nblist               *nlist)
{
    ...
    gmx::ArrayRef<const unsigned int> fepBits = jGrid.clusterData().fepBits;

    ngid = nbatParams.nenergrp;

    if (ngid*jGrid.geometry().numAtomsJCluster > gmx::index(sizeof(gid_cj)*8))
    {
        ...
        if (ai >= 0)
        {
            ...
            for (int cj_ind = cj_ind_start; cj_ind < cj_ind_end; cj_ind++)
            {
                ...
                else if (2*numAtomsJCluster == jGrid.geometry().numAtomsICluster)
                {
                    ...
                    fep_cj = (fepBits[cjr >> 1] >> ((cjr & 1)*numAtomsJCluster)) & ((1 << numAtomsJCluster) - 1);
                    if (ngid > 1)
                    {
                ...
                else
                {
                    ...
                    fep_cj = fepBits[cjr*2] + (fepBits[cjr*2 + 1] << jGrid.geometry().numAtomsICluster);
                    if (ngid > 1)
                    {
...
static inline void set_icell_bb(const Grid &iGrid,
                                ...
                                NbnxnPairlistCpuWork *work)
{
    set_icell_bb_simple(iGrid.clusterData().boundingBoxes, ci, shx, shy, shz,
...
{
#if NBNXN_BBXXXX
    set_icell_bbxxxx_supersub(iGrid.clusterData().packedBoundingBoxes,
                              ci, shx, shy, shz,
                              work->iSuperClusterData.bbPacked.data());
#else
    set_icell_bb_supersub(iGrid.clusterData().boundingBoxes, ci, shx, shy, shz,
                                ...
                                float                *nsubpair_tot_est)
{
    ...
    r_eff_sup = rlist + nbnxn_rlist_inc_outside_fac*gmx::square((numAtomsCluster - 1.0)/numAtomsCluster)*0.5*diagonal;
                                     ...
                                     t_nblist *nbl_fep)
{
    ...
    if (bSimple)
    {
        bb_i  = iGrid.clusterData().boundingBoxes;
    }
    else
    {
        pbb_i = iGrid.clusterData().packedBoundingBoxes;
    }
#else
    ...
    gmx::ArrayRef<const int>   flags_i = iGrid.clusterData().flags;
