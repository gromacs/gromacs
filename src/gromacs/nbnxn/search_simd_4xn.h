                       ...
                       int * gmx_restrict        numDistanceChecks)
{
    ...
    while (!InRange && jclusterFirst <= jclusterLast)
    {
        ...
        d2 = subc_bb_dist2_simd4(0, bb_ci, jclusterFirst, jGrid.clusterData().jBoundingBoxes);
#else
        d2 = subc_bb_dist2(0, bb_ci, jclusterFirst, jGrid.clusterData().jBoundingBoxes);
#endif
        *numDistanceChecks += 2;
    ...
    {
#if NBNXN_SEARCH_BB_SIMD4
        d2 = subc_bb_dist2_simd4(0, bb_ci, jclusterLast, jGrid.clusterData().jBoundingBoxes);
#else
        d2 = subc_bb_dist2(0, bb_ci, jclusterLast, jGrid.clusterData().jBoundingBoxes);
#endif
        *numDistanceChecks += 2;
