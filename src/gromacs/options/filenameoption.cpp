...
{
    bool        bManagerAccepted = false;
    std::string processedValue;
    if (manager_ != NULL)
    {
        processedValue   = manager_->completeFileName(value, info_);
        bManagerAccepted = !processedValue.empty();
    }
    ...
    if (fileType == efNR)
    {
        // If the manager returned an invalid file name, assume that it knows
        // what it is doing (required currently for VMD plugins).
        if (!bManagerAccepted)
        {
            std::string message
