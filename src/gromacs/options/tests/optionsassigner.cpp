...
TEST(OptionsAssignerTest, HandlesRequiredParameterWithDefaultValue)
{
    ...
    using gmx::IntegerOption;
    ASSERT_NO_THROW(options.addOption(
            IntegerOption("p").store(&value).required().defaultValue(1)));
...
TEST(OptionsAssignerIntegerTest, HandlesBothDefaultValues)
{
    ...
    using gmx::IntegerOption;
    ASSERT_NO_THROW(options.addOption(
            IntegerOption("p").store(&value).defaultValue(1).defaultValueIfSet(2)));
    EXPECT_EQ(1, value);
...
TEST(OptionsAssignerStringTest, HandlesEnumValue)
{
    ...
    using gmx::StringOption;
    ASSERT_NO_THROW(options.addOption(
            StringOption("p").store(&value).enumValue(c_allowed)));
...
TEST(OptionsAssignerEnumTest, StoresSingleValue)
{
    ...
    ASSERT_NO_THROW(options.addOption(
            EnumOption<TestEnum>("p").store(&value).enumValue(c_allowed)));
