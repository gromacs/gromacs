...
TEST(TimeUnitManagerTest, BasicOperations)
{
    ...
    EXPECT_FLOAT_EQ(1.0, manager.timeScaleFactor());
    manager.setTimeUnit(gmx::eTimeUnit_ns);
    EXPECT_EQ(gmx::eTimeUnit_ns, manager.timeUnit());
    EXPECT_FLOAT_EQ(1e3, manager.timeScaleFactor());
    EXPECT_FLOAT_EQ(1e-3, manager.inverseTimeScaleFactor());
}
