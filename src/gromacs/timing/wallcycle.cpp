                     ...
                     struct gmx_wallclock_gpu_t *gpu_t)
{
    ...
    if (tot <= 0.0)
    {
        ...
        GMX_LOG(*mdlog).formatWarning("WARNING: A total of %f CPU cycles was recorded, so mdrun cannot print a time accounting", tot);
        return;
    }

    if (wc->haveInvalidCount)
    {
        GMX_LOG(*mdlog).formatWarning("NOTE: Detected invalid cycle counts, probably because threads moved between CPU cores that do not have synchronized cycle counters. Will not print the cycle accounting.");
        return;
    }
