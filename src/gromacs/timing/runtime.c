...
#include "gromacs/legacyheaders/types/large_int.h"
...
// the gmx_runtime_t object be owned by the runner object. When
// these become member functions, existence will be guaranteed.
...
gmx_gettime()
{
#if defined HAVE_CLOCKGETTIME && _POSIX_TIMERS >= 0
...
{
#if defined HAVE_CLOCKGETTIME && _POSIX_THREAD_CPUTIME >= 0
    struct timespec t;
    double          seconds;
