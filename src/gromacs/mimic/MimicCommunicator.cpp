...
#if !GMX_MIMIC
//! Definitions to stub the ones defined in DataTypes.h
constexpr int TYPE_INT = 0, TYPE_DOUBLE = 0;

/*! Stub communication library function to call in case if
 * GROMACS is compiled without MiMiC. Calling causes GROMACS to exit!
 *
 * @param[in] path some useless string
 */
void MCL_init_client(const char *path)
{
    GMX_UNUSED_VALUE(path);
 ...
 * GROMACS is compiled without MiMiC. Calling causes GROMACS to exit!
 *
 * @param[in] data some useless pointer
 * @param[in] a some useless int
 * @param[in] b some useless int
 * @param[in] c some useless int
 */
void MCL_send(void *data, int a, int b, int c)
{
 ...
 * @param[in] c some useless int
 */
void MCL_receive(void *data, int a, int b, int c)
{
    GMX_UNUSED_VALUE(data);
    GMX_UNUSED_VALUE(a);
    GMX_UNUSED_VALUE(b);
