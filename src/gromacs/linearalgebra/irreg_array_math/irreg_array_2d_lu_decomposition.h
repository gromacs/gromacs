...
/*! \libinternal \file
    ...
    \brief   matrix and matrix vector products ustilizing IrregArray2D and std::vector
   ...
   \inlibraryapi
   \ingroup module_linearalgebra
 */
#ifndef GMX_LINEARALGEBRA_IRREG_ARRAY_MATH_IRREG_ARRAY_2D_LU_DECOMPOSITION_H
#define GMX_LINEARALGEBRA_IRREG_ARRAY_MATH_IRREG_ARRAY_2D_LU_DECOMPOSITION_H
#include <numeric>
#include <type_traits>
...
//! \cond
// older Intel compilers do not support constexpr
#define IRREG_ARRAY_MATH_HAVE_CONSTEXPR
#ifdef __INTEL_COMPILER
 #if __INTEL_COMPILER_BUILD_DATE < 20150000
  #undef IRREG_ARRAY_MATH_HAVE_CONSTEXPR
 #endif
#endif
//! \endcond
...
/*! \brief LU decomposition with row pivoting \f$ \mathbf{A} = \mathbf{P}^{-1}\,\mathbf{L}\,\mathbf{U} \f$
    ...
    \param[out]  lu       the upper and lower triangular matrices \f$ \mathbf{L} \f$ and \f$ \mathbf{P} \f$  stored in the upper right and lower left omitting the diagonal entries of \f$ \mathbf{L} \f$, which are all equal to \f$ 1 \f$
 */
...
LUPDecompositionSimd(const IrregArray2D<T, Alloc> &a, std::vector<TInt, AllocInt> &p, IrregArray2D<T, Alloc> &lu)
{
    return LUPDecompositionRef(a, p, lu);
}

/*! \brief LU decomposition with row pivoting \f$ \mathbf{A} = \mathbf{P}^{-1}\,\mathbf{L}\,\mathbf{U} \f$
           SIMD implementation, returns true if sucessful and false otherwise
...
LUPDecompositionSimd(const IrregArray2D<T, Alloc> &a, std::vector<TInt, AllocInt> &p, IrregArray2D<T, Alloc> &lu)
{
    // even slower than the reference implementation, return that for now
    return LUPDecompositionRef(a, p, lu);
...
#ifdef IRREG_ARRAY_MATH_HAVE_CONSTEXPR
    constexpr index_type simdWidth  = SimdSetup<T>::simdWidth_;
#else
    const     index_type simdWidth  = SimdSetup<T>::simdWidth_;
#endif
    ...
    {
        value_type maxColVal         = (value_type)0;
        index_type maxColValRowIndex = (index_type)0;
        ...
        if (maxColVal == (value_type)0)
        {
            std::fprintf(stderr, "Warning in gmx::LUPDecomposition(): input matrix A is singular (all values in column %li are zero), decomposition failed.", static_cast<unsigned long int>(j));
            return false;
        }
        ...
        for (index_type i = j + 1; i < n; ++i)
        {
            ...
            {
                std::transform(lu(i) + static_cast<ptrdiff_t>(j_plus_1), lu(i) + static_cast<ptrdiff_t>(simdBlockBegin),
                               lu(j) + static_cast<ptrdiff_t>(j_plus_1),
                               lu(i) + static_cast<ptrdiff_t>(j_plus_1),
                ...
                {
                    aSimd = gmx::load(lu(j) + static_cast<ptrdiff_t>(k));
                    bSimd = gmx::load(lu(i) + static_cast<ptrdiff_t>(k));
                    aSimd = bSimd - c * aSimd;
                    gmx::store(lu(i) + static_cast<ptrdiff_t>(k), aSimd);
                }

                std::transform(lu(i) + static_cast<ptrdiff_t>(maxSimdBlockEnd), lu(i) + static_cast<ptrdiff_t>(n),
                               lu(j) + static_cast<ptrdiff_t>(maxSimdBlockEnd),
                               lu(i) + static_cast<ptrdiff_t>(maxSimdBlockEnd),
                               [&c](value_type &elementRowI, value_type &elementRowJ) { return elementRowI - c * elementRowJ; });
            }
            else
            {
                std::transform(lu(i) + static_cast<ptrdiff_t>(j_plus_1), lu(i) + static_cast<ptrdiff_t>(n),
                               lu(j) + static_cast<ptrdiff_t>(j_plus_1),
                               lu(i) + static_cast<ptrdiff_t>(j_plus_1),
                               [&c](value_type &elementRowI, value_type &elementRowJ) { return elementRowI - c * elementRowJ; });
            }
...
/*! \brief LU decomposition with row pivoting \f$ \mathbf{A} = \mathbf{P}^{-1}\,\mathbf{L}\,\mathbf{U} \f$
           reference implementation, returns true if sucessful and false otherwise
 ...
 */
template<typename T, class Alloc = std::allocator<T>, typename TInt, class AllocInt = std::allocator<TInt> >
inline bool LUPDecompositionRef(const IrregArray2D<T, Alloc> &a, std::vector<TInt, AllocInt> &p, IrregArray2D<T, Alloc> &lu)
{
    typedef typename IrregArray2D<T, Alloc>::value_type value_type;
    typedef typename IrregArray2D<T, Alloc>::index_type index_type;
    ...
    for (index_type j = 0; j < (n - 1); ++j)
    {
        ...
        for (index_type i = j + 1; i < n; ++i)
        {
            ...
            std::transform(lu(i) + static_cast<ptrdiff_t>(j + 1), lu(i) + static_cast<ptrdiff_t>(n),
                           lu(j) + static_cast<ptrdiff_t>(j + 1),
                           lu(i) + static_cast<ptrdiff_t>(j + 1),
                           [&c](value_type &elementRowI, value_type &elementRowJ) { return elementRowI - c * elementRowJ; });
        }
    }
 ...
 */
template<typename T, class Alloc = std::allocator<T>, typename TInt, class AllocInt = std::allocator<TInt> >
inline bool LUPDecomposition(const IrregArray2D<T, Alloc> &a, std::vector<TInt, AllocInt> &p, IrregArray2D<T, Alloc> &lu)
{
#if GMX_SIMD
    return LUPDecompositionRef(a, p, lu);
#else
//    LUPDecompositionSimd(a, p, lu);
    return LUPDecompositionRef(a, p, lu);
#endif
}
...
/*! \brief matrix inversion based on LUP decomposition, returns true if sucessful and false otherwise
           Currently, SIMD support is limited to float and/or double depending on the architecture.
           This template function is only active for unsupported data types and returns the reference
           implementation.
...
inline
typename std::enable_if<!(gmx::SimdSetup<T>::doSimd_), bool>::type
matrixInverseLUPSimd(const IrregArray2D<T, Alloc> &a, IrregArray2D<T, Alloc> &aInv)
{
    return matrixInverseLUPRef(a, aInv);
}

/*! \brief matrix inversion based on LUP decomposition, SIMD implemention, returns true if sucessful and false otherwise
           Currently, SIMD support is limited to float and/or double depending on the architecture.
           This template function is only active for unsupported data types and returns the reference
           implementation.
...
matrixInverseLUPSimd(const IrregArray2D<T, Alloc> &a, IrregArray2D<T, Alloc> &aInv)
{
    typedef           IrregArray2D<T, Alloc>                   array_type;
    ...
    index_vector_type p(n, (value_type)0);
    // packed upper and lower triangular matrices, diagonal entries of L omitted (all equal to 1)
    array_type        lu(0, n - 1, 0, n - 1, (value_type)0);

    // temporary work space
    vector_type  x(n, (value_type)0);
    vector_type  y(n, (value_type)0);
    array_type   b(0, n - 1, 0, n - 1, (value_type)0);
    ...
    for (index_type i = 0; i < n; ++i)
    {
        ...
        for (index_type j = 0; j < n; ++j)
        {
            ...
            {
                aSimd = gmx::load(lu(j)    + static_cast<ptrdiff_t>(k));
                bSimd = gmx::load(y.data() + static_cast<ptrdiff_t>(k));
                cSimd = gmx::fma(aSimd, bSimd, cSimd);
            }
            value_type tmpf = gmx::reduce(cSimd);
            // accumulate the rest in scalar math
            tmpf += std::inner_product(lu(j)    + static_cast<ptrdiff_t>(simdBlockWidth), lu(j) + static_cast<ptrdiff_t>(j),
                                       y.data() + static_cast<ptrdiff_t>(simdBlockWidth), (value_type)0);
            y[j] = b(i, p[j]) - tmpf;
        }
        ...
        for (index_type j = n - 1; j >= 0; --j)
        {
            ...
            if (simdBlockBegin > j && simdBlockBegin < maxSimdBlockEnd)
            {
                tmpf = std::inner_product(lu(j)    + static_cast<ptrdiff_t>(j_plus_1), lu(j) + static_cast<ptrdiff_t>(simdBlockBegin),
                                          x.data() + static_cast<ptrdiff_t>(j_plus_1), (value_type)0);

                cSimd = gmx::setZero();
                for (index_type k = simdBlockBegin; k < maxSimdBlockEnd; k += simdWidth)
                {
                    aSimd = gmx::load(lu(j)    + static_cast<ptrdiff_t>(k));
                    bSimd = gmx::load(x.data() + static_cast<ptrdiff_t>(k));
                    cSimd = gmx::fma(aSimd, bSimd, cSimd);
                }
                tmpf += gmx::reduce(cSimd);

                tmpf += std::inner_product(lu(j)    + static_cast<ptrdiff_t>(maxSimdBlockEnd), lu(j) + static_cast<ptrdiff_t>(n),
                                           x.data() + static_cast<ptrdiff_t>(maxSimdBlockEnd), (value_type)0);
            }
            else
            {
                tmpf = std::inner_product(lu(j)    + static_cast<ptrdiff_t>(j_plus_1), lu(j) + static_cast<ptrdiff_t>(n),
                                          x.data() + static_cast<ptrdiff_t>(j_plus_1), (value_type)0);
            }
            x[j] = (y[j] - tmpf) / lu(j, j);
        }
...
/*! \brief matrix inversion based on LUP decomposition, reference implemention, returns true if sucessful and false otherwise
 ...
 */
template<typename T, class Alloc = std::allocator<T> >
inline bool matrixInverseLUPRef(const IrregArray2D<T, Alloc> &a, IrregArray2D<T, Alloc> &aInv)
{
    typedef           IrregArray2D<T, Alloc>                   array_type;
    typedef typename  array_type::value_type                   value_type;
    typedef typename  array_type::allocator_type               allocator_type;
    ...
    for (index_type i = 0; i < n; ++i)
    {
        ...
        for (index_type j = 0; j < n; ++j)
        {
            const value_type tmpf = std::inner_product(lu(j), lu(j) + static_cast<ptrdiff_t>(j), y.data(), (value_type)0);
            y[j] = b(i, p[j]) - tmpf;
        }
        ...
        for (index_type j = n - 1; j >= 0; --j)
        {
            const value_type tmpf = std::inner_product(lu(j)    + static_cast<ptrdiff_t>(j + 1), lu(j) + static_cast<ptrdiff_t>(n),
                                                       x.data() + static_cast<ptrdiff_t>(j + 1), (value_type)0);
            x[j] = (y[j] - tmpf) / lu(j, j);
        }
...
/*! \brief matrix inversion based on LUP decomposition, returns true if sucessful and false otherwise
    ...
    The LUP decomposition of matrix \f$ \mathbf{A} \f$ allows us tow write (see LUPDecomposition())
    ...
    This equation system can be solved for ach column vector \f$ \mathbf{x},\mathbf{y}, \mathbf{e} \f$
 ...
 */
template<typename T, class Alloc = std::allocator<T> >
inline bool matrixInverseLUP(const IrregArray2D<T, Alloc> &a, IrregArray2D<T, Alloc> &aInv)
{
#if GMX_SIMD
    return matrixInverseLUPRef(a, aInv);
#else
//    matrixInverseLUPSimd(a, p, lu);
    return matrixInverseLUPRef(a, aInv);
#endif
}
...
inline
typename std::enable_if<!(gmx::SimdSetup<T>::doSimd_), bool>::type
matrixDeterminantLUPSimd(const IrregArray2D<T, Alloc> &a, T &det)
{
    return matrixDeterminantLUPRef(a, det);
}

/*! \brief compute the determinant of a matrix based on LUP decomposition,
           SIMD implemention, returns true if sucessful and false otherwise
...
matrixDeterminantLUPSimd(const IrregArray2D<T, Alloc> &a, T &det)
{
    ...
    vector_type udiag(n, (value_type)0);

    // call the reference version here too for a clean performance comparison
    const bool lupSuccess = gmx::LUPDecompositionSimd(a, p, lu);
    if (!lupSuccess)
    {
        std::fprintf(stderr, "Warning in gmx::matrixDeterminantLUP(): matrix inversion failed because the LUP decomposition of input matrix A failed.");
        return false;
    }
    ...
    const index_type simdBlockWidth  = (n > simdWidth ? n - n % simdWidth : 0);
    ...
    det *= ((s % 2) == 0 ? (value_type)1 : (value_type)-1);
...
template<typename T, class Alloc = std::allocator<T> >
inline bool
matrixDeterminantLUPRef(const IrregArray2D<T, Alloc> &a, T &det)
{
    typedef           IrregArray2D<T, Alloc>                   array_type;
    typedef typename  array_type::value_type                   value_type;
    typedef typename  array_type::allocator_type               allocator_type;
    ...
    det = std::accumulate(udiag.begin(), udiag.end(), (value_type)1, std::multiplies<value_type>());
...
/*! \brief compute the determinant of a matrix based on LUP decomposition,
    ...
    The entries \f$ p_{i}, p_{j} \f$ of the vector representation \f$ \mathbf{p} \f$ of the permutation matrix specify the
    new index of rows \f$ i, j \f$ after permutation/pivoting.
...
template<typename T, class Alloc = std::allocator<T> >
inline bool
matrixDeterminantLUP(const IrregArray2D<T, Alloc> &a, T &det)
{
#if GMX_SIMD
//    matrixDeterminantLUPSimd(a, p, lu);
    return matrixDeterminantLUPRef(a, det);
#else
    return matrixDeterminantLUPRef(a, det);
#endif
