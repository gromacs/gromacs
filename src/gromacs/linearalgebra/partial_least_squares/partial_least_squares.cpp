...
void setColumn(MatrixType &mt, const int columnIdx, const ColumnType &cl, const int nbRows)
{
    ...
    {
        std::fprintf(stderr, "Error in pls_denham setColumn invalid nbRows, mt.getLength1() %lu != nbRows %d",
                     mt.getLength1(), nbRows);
        throw;
    }
    if (int(mt.getLength2()) <= columnIdx)
    {
        std::fprintf(stderr, "Error in pls_denham setColumn invalid column, mt.getLength2() %lu != columnIdx %d",
                     mt.getLength2(), columnIdx);
        throw;
    }
    if (int(cl.size()) != nbRows)
    {
        std::fprintf(stderr, "Error in pls_denham setColumn invalid size, cl.size() %lu != nbRows %d",
                     cl.size(), nbRows);
        throw;
    }
...
{
    const gmx::IrregArray2D<real, typename gmx::SimdSetup<real>::allocator_type> &matrix;
    const int columnIdx;
    public:
        IrregArray2DColumnView(const gmx::IrregArray2D<real, typename gmx::SimdSetup<real>::allocator_type> &inMatrix,
                               const int                      inColumnIdx)
            : matrix(inMatrix), columnIdx(inColumnIdx)
        {
            if (int(matrix.getLength2()) <= columnIdx)
            {
                std::fprintf(stderr, "Error in pls_denham IrregArray2DColumnView invalid columnIdx, matrix.getLength2() %lu != columnIdx %d",
        ...
        const real &operator[](const int inRowIdx) const
        {
            ...
            {
                std::fprintf(stderr, "Error in pls_denham IrregArray2DColumnView invalid inRowIdx, matrix.getLength1() %lu != inRowIdx %d",
...
int pls_denham(const gmx::IrregArray2D<real, typename gmx::SimdSetup<real>::allocator_type> &X,
               const std::vector<real, typename gmx::SimdSetup<real>::allocator_type> &y,
               const int n, const int p, const int k,
               gmx::IrregArray2D<real, typename gmx::SimdSetup<real>::allocator_type>* W,
               std::vector<real, typename gmx::SimdSetup<real>::allocator_type>* q)
{
    using allocator_type = typename gmx::SimdSetup<real>::allocator_type;
    ...
    // With : T_k = (t_1, ... , t_k)
    gmx::IrregArray2D<real, allocator_type> T_k(int(0), n-1, int(0), k-1);
    // With : W_k = (W_1, ... , W_k)
    gmx::IrregArray2D<real, allocator_type> W_k(int(0), p-1, int(0), k-1);
    ...
    for (int i = 2; i < k; i++)
    {
        ...
        // With : T_k = (t_1, ... , t_i)
        gmx::IrregArray2D<real, allocator_type> T_i(int(0), n-1, int(0), i-1);
        ...
        {
            gmx::IrregArray2D<real, allocator_type> T_iT_T_i(int(0), i-1, int(0), i-1);
            gmx::matrixProductMatrixTMatrix(T_i, T_i, T_iT_T_i);

            gmx::IrregArray2D<real, allocator_type> T_iT_T_i_INV(int(0), i-1, int(0), i-1);
    ...
    {
        const int                               i = k;
        gmx::IrregArray2D<real, allocator_type> T_kT_T_k(int(0), i-1, int(0), i-1);
        gmx::matrixProductMatrixTMatrix(T_k, T_k, T_kT_T_k);

        gmx::IrregArray2D<real, allocator_type> T_kT_T_k_INV(int(0), i-1, int(0), i-1);
