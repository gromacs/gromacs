...
void setColumn(MatrixType &mt, const ptrdiff_t columnIdx, const ColumnType &cl, const ptrdiff_t nbRows)
{
    if (ptrdiff_t(mt.getLength1()) != nbRows)
    {
        std::fprintf(stderr, "Error in pls_denham setColumn invalid nbRows, mt.getLength1() %zu != nbRows %ld",
                     mt.getLength1(), nbRows);
        throw;
    }
    if (ptrdiff_t(mt.getLength2()) <= columnIdx)
    {
        std::fprintf(stderr, "Error in pls_denham setColumn invalid column, mt.getLength2() %zu != columnIdx %ld",
                     mt.getLength2(), columnIdx);
        throw;
    }
    if (ptrdiff_t(cl.size()) != nbRows)
    {
        std::fprintf(stderr, "Error in pls_denham setColumn invalid size, cl.size() %zu != nbRows %ld",
                     cl.size(), nbRows);
        throw;
    }

    for (ptrdiff_t idxRow = 0; idxRow < nbRows; ++idxRow)
    {
                     ...
                     Pls1dArray &q)
{
    ...
    std::vector<real, allocator_type> r(n);
    {
        ...
        for (ptrdiff_t idxVal = 0; idxVal < n; ++idxVal)
        {
    ...
    for (ptrdiff_t i = 2; i < k; i++)
    {
        ...
        for (ptrdiff_t i_copy = 0; i_copy < i; ++i_copy)
        {
