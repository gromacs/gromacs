...
int pls_denham(const gmx::IrregArray2D<real> &X, const std::vector<real> &y, const size_t n, const int a,
               gmx::IrregArray2D<real>* W, std::vector<real>* q)
{
    ...
    if (W == nullptr || W->getLength1() != n /*|| int(W->getLength2()) != a*/)
    {
    ...
    if (X.getLength1() != n || int(X.getLength2()) != a)
    {
        std::fprintf(stderr, "Error in pls_denham invalid y.");
        throw;
    }
    ...
    // With : T_i = (t_1, ... , t_i)
    gmx::IrregArray2D<real> T_i; //(int(0), n, int(0), 0 /*a*/);
    // With : W_i = (W_1, ... , W_i)
    gmx::IrregArray2D<real> W_i; //(int(0), n, int(0), 0 /*a*/);
