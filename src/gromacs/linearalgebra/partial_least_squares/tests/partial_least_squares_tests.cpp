...
TEST(PartialLeastSquares, SimpleTest){
    ...
    gmx::IrregArray2D<real, allocator_type> X(int(0), n-1, int(0), p-1);

    gmx::IrregArray2D<real, allocator_type> W(int(0), p-1, int(0), k-1);
    ...
    for (int i = 0; i < n; ++i)
    {
        // The objective of the function is to fint such that
...
TEST(PartialLeastSquares, LargerTest){
    ...
    for (int i = 0; i < n; ++i)
    {
        // The objective of the function is to fint such that
        // the difference between y and X t is low
        EXPECT_NEAR(std::abs(y[i] - Xt[i]), 0, 2e-5);
    }
}
...
TEST(PartialLeastSquares, ParametersFailureTest){
    ...
    }
    {
        ...
        const int                               k = 4;
        std::vector<real, allocator_type>       y(n, 0);
        gmx::IrregArray2D<real, allocator_type> X(int(0), n-1-2, int(0), p-1);
    ...
    }
    {
        ...
        gmx::IrregArray2D<real, allocator_type> W(int(0), p-1, int(0), k-1-1);
        std::vector<real, allocator_type>       q(k);
