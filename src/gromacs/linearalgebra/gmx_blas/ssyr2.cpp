                            ...
                            int*        lda__)
{
    ...
    if (n <= 0 || std::abs(alpha) < GMX_FLOAT_MIN || incx == 0 || incy == 0 || (ch != 'U' && ch != 'L'))
        return;
    ...
    if (ch == 'U')
    {
        ...
        if (incx == 1 && incy == 1)
        {
            ...
            for (j = 1; j <= n; j++)
            {
                if (std::abs(x[j - 1]) > GMX_FLOAT_MIN || std::abs(y[j - 1]) > GMX_FLOAT_MIN)
                {
                    temp1 = alpha * y[j - 1];
                    temp2 = alpha * x[j - 1];
                    for (i = 1; i <= j; i++)
        ...
        else
        {
            ...
            for (j = 1; j <= n; j++)
            {
                ...
                if (std::abs(x[jx - 1]) > GMX_FLOAT_MIN || std::abs(y[jy - 1]) > GMX_FLOAT_MIN)
                {
                    temp1 = alpha * y[jy - 1];
                    temp2 = alpha * x[jx - 1];
                    ix    = kx;
