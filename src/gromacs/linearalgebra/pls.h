...
public:
   ...
   */
  FMatrix(int m_, int n_) : data(std::vector<real>(m_*n_)), m(m_), n(n_)
  {
  }
  /*! \brief
   ...
   */
  FMatrix(int m_, int n_, real val) : data(std::vector<real>(m_*n_, val)), m(m_), n(n_)
  {
  }
  /*! \brief
   * resizing the matrix
 ...
 */
int
pls_denham(FMatrix *x, FVector *y, int a, FMatrix *w, FVector *q);
#endif
