        ...
        if (ndone < im)
        {
            ...
            for (i__ = 1; i__ <= i__2; ++i__)
            {
                ...
                if (ndepth > 0)
                {
                    ...
                    (&in, &z__[ibegin + j * z_dim1], &c__1, &d__[ibegin], &c__1);
                    i__3 = in - 1;
                    F77_FUNC(scopy, SCOPY)
                    (&i__3, &z__[ibegin + (j + 1) * z_dim1], &c__1, &l[ibegin], &c__1);
                    F77_FUNC(slaset, SLASET)
                    ("Full", &in, &c__2, &c_b5, &c_b5, &z__[ibegin + j * z_dim1], ldz);
                }
                k    = ibegin;
                i__3 = in - 1;
                for (j = 1; j <= i__3; ++j)
                ...
                {
                    if (j == oldlst || work[indgap + j] >= reltol * std::abs(work[j]))
                    {
                        newlst = j;
                    }
                    else
                        ...
                        else
                        {
                            *info = 0;
                            if (minrgp < mgstol)
                            {
                                ...
                                for (k = 1; k <= i__4; ++k)
                                {
                                    work[indwrk + k] = d__[ibegin + k] + work[indlld + k];
                                }
                                i__4 = newsiz;
                                for (k = 1; k <= i__4; ++k)
                                {
                    ...
                    else
                    {
                        ...
                        L90:
                            ...
                            {
                                d__1 = work[indgap + k - 1], d__2 = work[indgap + k];
                                gap = (d__1 < d__2) ? d__1 : d__2;
                            }
                            ++iter;
                            if (resid > *tol * gap && std::abs(rqcorr) > eps * 4. * std::abs(lambda))
                            {
                            ...
                            (&i__5, &nrminv, &z__[ibegin + zfrom - 1 + ktot * z_dim1], &c__1);
                            ++ktot;
                        }
                        if (newsiz > 1)
                        {
                            ...
                            for (p = newftt + 1; p <= i__4; ++p)
                            {
                                ...
                                {
                                    tmp = -F77_FUNC(sdot, SDOT)(&in,
                                                                &z__[ibegin + p * z_dim1],
                                                                &c__1,
                                                                &z__[ibegin + q * z_dim1],
                                                                &c__1);
                                    F77_FUNC(saxpy, SAXPY)
                                    (&in, &tmp, &z__[ibegin + q * z_dim1], &c__1, &z__[ibegin + p * z_dim1], &c__1);
                                }
                                tmp = 1. / F77_FUNC(snrm2, SNRM2)(&in, &z__[ibegin + p * z_dim1], &c__1);
                                F77_FUNC(sscal, SSCAL)(&in, &tmp, &z__[ibegin + p * z_dim1], &c__1);
