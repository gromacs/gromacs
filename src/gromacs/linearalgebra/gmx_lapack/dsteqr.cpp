...
void F77_FUNC(dsteqr,
              DSTEQR)(const char* compz, int* n, double* d__, double* e, double* z__, int* ldz, double* work, int* info)
{
    double c_b9  = 0.;
    double c_b10 = 1.;
    int    c__0  = 0;
...
L30:
    ...
    iscale = 0;
    if (std::abs(anorm) < GMX_DOUBLE_MIN)
    {
        goto L10;
    }
    if (anorm > ssfmax)
    ...
    L60:
        ...
        if (m == l + 1)
        {
            if (icompz > 0)
            {
                ...
                ("R", "V", "B", n, &c__2, &work[l], &work[*n - 1 + l], &z__[l * z_dim1 + 1], ldz);
            }
            else
            {
        ...
        if (icompz > 0)
        {
            mm = m - l + 1;
            F77_FUNC(dlasr, DLASR)
            ("R", "V", "B", n, &mm, &work[l], &work[*n - 1 + l], &z__[l * z_dim1 + 1], ldz);
        }
    ...
    L110:
        ...
        if (m == l - 1)
        {
            if (icompz > 0)
            {
                ...
                work[*n - 1 + m] = s;
                F77_FUNC(dlasr, DLASR)
                ("R", "V", "F", n, &c__2, &work[m], &work[*n - 1 + m], &z__[(l - 1) * z_dim1 + 1], ldz);
            }
            else
            {
        ...
        if (icompz > 0)
        {
            mm = l - m + 1;
            F77_FUNC(dlasr, DLASR)
            ("R", "V", "F", n, &mm, &work[m], &work[*n - 1 + m], &z__[m * z_dim1 + 1], ldz);
        }
...
L140:
    ...
    {
        if (std::abs(e[i__]) > GMX_DOUBLE_MIN)
        {
            ++(*info);
        }
    }
    ...
    else
    {
        ...
        for (ii = 2; ii <= i__1; ++ii)
        {
            ...
            if (k != i__)
            {
                ...
                (n, &z__[i__ * z_dim1 + 1], &c__1, &z__[k * z_dim1 + 1], &c__1);
            }
        }
    }
