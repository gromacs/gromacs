// we need to borrow isnan from C
#include <math.h>
...
L60:
	...
	for (i__ = j + 1; i__ <= i__1; ++i__) {
	    dplus = d__[i__] + s;
	    work[i__] = ld[i__] / dplus;
	    if (std::abs(work[i__])<GMX_DOUBLE_MIN) {
...
L90:
    ...
    *mingma = work[inds + r1 - 1] + work[indp + r1 - 1];
    if (std::abs(*mingma)<GMX_DOUBLE_MIN) {
...
L140:
	...
	for (i__ = *r__ - 1; i__ >= i__1; --i__) {
	    if (std::abs(z__[i__ + 1])<GMX_DOUBLE_MIN) {
...
L170:
	...
	for (i__ = *r__; i__ <= i__1; ++i__) {
	    if (std::abs(z__[i__])<GMX_DOUBLE_MIN) {
