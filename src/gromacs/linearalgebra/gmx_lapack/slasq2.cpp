...
void F77_FUNC(slasq2, SLASQ2)(int* n, float* z__, int* info)
{
    int   i__1, i__2, i__3;
    float d__1, d__2;
    ...
    *info  = 0;
    ...
    if (std::abs(trace) < GMX_FLOAT_MIN)
    {
        z__[(*n << 1) - 1] = 0.;
        return;
    }
    ...
    if (neginf >= 0)
        ieee = 0;
    newzro = negzro + zero;
    if (std::abs(newzro - zero) > GMX_FLOAT_MIN)
        ieee = 0;
    posinf = one / newzro;
    if (posinf <= one)
        ieee = 0;
