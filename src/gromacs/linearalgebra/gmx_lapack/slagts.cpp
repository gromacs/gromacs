...
void F77_FUNC(slagts,
              SLAGTS)(int* job, int* n, float* a, float* b, float* c__, float* d__, int* in, float* y, float* tol, int* info)
{
    int   i__1;
        ...
        if (*job == 1)
        {
            for (k = *n; k >= 1; --k)
            {
                ...
                if (absak < 1.)
                {
                    if (absak < sfmin)
                    {
                        if (std::abs(absak) < GMX_FLOAT_MIN || std::abs(temp) * sfmin > absak)
                        {
                            *info = k;
                            return;
                        }
