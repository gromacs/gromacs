    ...
    if (stat != cudaSuccess)
    {
        ...
        GMX_ASSERT(stat != cudaErrorMemoryAllocation,
                   formatString("%s %s: %s which was an unexpected error", errorMessage,
                                cudaGetErrorName(stat), cudaGetErrorString(stat)).c_str());
        GMX_THROW(InternalError(formatString("%s %s: %s", errorMessage,
...
{
    const char *errorMessage = "Could not test whether host memory pointer was already registered for page locking for GPU transfers.";

    if (pointer == nullptr || numBytes == 0)
    {
        // Such a pointer was never pinned by CUDA
        return false;
    }
    ...
    {
        // The pointer was already pinned, so leave it in that
        // original state, clear the error, and return true.
        cudaGetLastError();
        return true;
    }
    if (stat == cudaSuccess)
    {
        // The pointer was not already pinned, so unpin it, and return false.
        unpinBuffer(pointer);
        return false;
    }
    ...
    GMX_RELEASE_ASSERT(stat != cudaErrorInvalidValue &&
                       stat != cudaErrorNotSupported &&
                       stat != cudaErrorMemoryAllocation,
                       formatString("%s %s: %s", errorMessage,
                                    cudaGetErrorName(stat), cudaGetErrorString(stat)).c_str());

    // Unreachable, but some compilers don't understand that.
    return false;
}
