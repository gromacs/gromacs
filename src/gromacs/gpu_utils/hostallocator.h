 ...
 *
 * For an efficient non-blocking GPU transfer, the memory pages for a
 * buffer need to be pinned to a physical page. Aligning such buffers
 * to a physical page should miminize the number of pages that need to
 * be pinned. However, some buffers that may be used for such
 * transfers may also be used in either GROMACS builds or run paths
 * that cannot use a GPU, so the policy can be configured so that the
 * resource consumption is no higher than required for correct,
 * efficient operation in all cases. */
enum class PinningPolicy : int
{
    CannotBePinned,     // Memory is not known to be suitable for pinning.
    CanBePinned,        // Memory is suitable for efficient pinning, e.g. because it is allocated to be page aligned, and will be pinned when non-empty.
};
