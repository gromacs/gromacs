...
template <GpuFramework> struct GpuTraits
{
    ...
    using CommandTimingEvent = void; //!< Single command call timing event - used in OpenCL
};
...
template <GpuFramework framework> class GpuRegionTimerImpl
{
    ...
    using Traits = GpuTraits<framework>;
    public:
        GpuRegionTimerImpl()  = default;
        ~GpuRegionTimerImpl() = default;

        /*! \brief Will be called before the region start. */
        inline void startRecording(typename Traits::CommandStream) = 0;
        /*! \brief Will be called after the region end. */
        inline void stopRecording(typename Traits::CommandStream) = 0;
        /*! \brief Should reset any internal state if needed */
        inline void reset() = 0;
        /*! \brief Should return the last measured region timespan */
        inline float getLastTimeMilliseconds() = 0;
        /*! \brief Should return a new timing event for passing into individual GPU API calls
         * within the region if API requires it (e.g. on OpenCL). */
        inline typename Traits::CommandTimingEvent *fetchNextEvent() = 0;
};
...
template <GpuFramework framework> class GpuRegionTimerWrapper
{
    ...
    public:
         ...
         */
        void startRecording(typename Traits::CommandStream s)
        {
         ...
         * \param[in] s   The GPU command stream where the event being measured takes place.
         */
        void stopRecording(typename Traits::CommandStream s)
        {
            if (c_debugTimerState)
            {
            ...
            callCount_++;
            impl_.stopRecording(s);
        }
        /*! \brief
         * Accumulates the last timespan of all the events used into the the total duration.
         * To be called after stopRecording() and the command stream of the event having been synchronized.
        ...
        float getLastTimeMilliseconds()
        {
            ...
            {
                milliseconds        = impl_.getLastTimeMilliseconds();
         ...
         */
        typename Traits::CommandTimingEvent *fetchNextEvent()
        {
            if (c_debugTimerState)
            {
                std::string error = "GPU timer should be recording, but is " + std::string((debugState_ == TimerState::Idle) ? "idle" : "stopped") + ".";
