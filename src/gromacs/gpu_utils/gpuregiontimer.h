...
template <GpuFramework framework> class GpuRegionTimerImpl
{
    ...
    public:
        ...
        inline void openTimingRegion(typename Traits::CommandStream) = 0;
        /*! \brief Will be called after the region end. */
        inline void closeTimingRegion(typename Traits::CommandStream) = 0;
        /*! \brief Should reset any internal state if needed */
        inline void reset() = 0;
        /*! \brief Should return the last measured region timespan (in milliseconds) */
...
template <GpuFramework framework> class GpuRegionTimerWrapper
{
    ...
    public:
         ...
         */
        void openTimingRegion(typename Traits::CommandStream s)
        {
         ...
         * \param[in] s   The GPU command stream where the event being measured takes place.
         */
        void closeTimingRegion(typename Traits::CommandStream s)
        {
            if (c_debugTimerState)
            {
        ...
        double getLastRangeTime()
        {
            if (c_debugTimerState)
            {
                 ...
                 * TODO: improve internal task scheduling, re-enable the assert
                 */
                /*
