...
bool canDetectGpus(std::string *errorMessage)
{
    ...
    if (stat != cudaSuccess)
    {
        ...
        // valid devices, then cudaGetDeviceCount returns the
...
int findGpus(gmx_gpu_info_t *gpu_info)
{
    ...
    if (stat != cudaSuccess)
    {
        GMX_THROW(InternalError("Invalid call of findGpus() when CUDA API returned an error, perhaps "
