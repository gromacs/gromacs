              ...
              const struct gmx_gpu_opt_t *gpu_opt)
{
    ...
    {
        sprintf(sbuf, "On rank %s trying to initialize an non-existent GPU: "
    ...
    if (stat != cudaSuccess)
    {
        gmx_fatal(FARGS, "On rank %s failed to initialize GPU #%d: %s"
                  rank, mygpu, cudaGetErrorString(stat));
    }
