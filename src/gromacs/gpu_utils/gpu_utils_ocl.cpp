 ...
 * \returns                    Whether the device is sane
 */
...
static int checkGpu(const gmx_device_info_t *ocl_gpu_device, size_t deviceIndex)
{
    try
    {
        if (isDeviceSane(ocl_gpu_device))
        {
    ...
    {
        e.prependContext(formatString("While checking device #%zu for sanity, ", deviceIndex));
        // TODO This should be a notification level message, not a warning.
        gmx_warning(formatExceptionMessageToString(e).c_str());
    }
    // TODO When we would we ever detect a sane device that was incompatible?
    return egpuInsane;
}
