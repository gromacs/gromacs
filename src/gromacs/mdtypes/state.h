...
//! \brief Function to copy data from state_global to state_local (use instead of assignment operator)
void copyGlobalStateToLocalState(const t_state_global *state_global, t_state_local *state_local);
//! \brief Function to copy data from state_local to state_global (use instead of assignment operator)
void copyLocalStateToGlobalState(const t_state_local *state_local, t_state_global *state_global);

//! \brief Resizes the T- and P-coupling state variables (version for state_global)
void init_gtc_state_global(t_state_global *state, int ngtc, int nnhpres, int nhchainlength);
//! \brief Resizes the T- and P-coupling state variables (version for state_local)
void init_gtc_state_local(t_state_local *state, int ngtc, int nnhpres, int nhchainlength);

//! \brief Change the number of atoms represented by this state, allocating memory as needed (version for state_global).
void state_change_natoms_global(t_state_global *state, int natoms);
//! \brief Change the number of atoms represented by this state, allocating memory as needed (version for state_local).
 ...
 *
 * When \p state=nullptr, returns an empty arrayRef. Version for state_global.
 *
...
//! \brief Same as positionsFromGlobalStatePointer, but for state_local
static inline gmx::ArrayRef<const gmx::RVec>
