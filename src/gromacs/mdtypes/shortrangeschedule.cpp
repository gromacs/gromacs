...
void SRCPUSingleNodeSchedule::computeStep(int flags)
{
    ...
    bNS           = ((flags & GMX_FORCE_NS) != 0) && (!fr_->bAllvsAll);
    ...
    if (bNS)
    {
        ...
        wallcycle_sub_start(wcycle_, ewcsNBS_GRID_NONLOCAL);
        nbnxn_put_on_grid_nonlocal(nbv->nbs.get(), domdec_zones(cr_->dd),
                                   fr_->cginfo, x,
                                   nbv->grp[eintNonlocal].kernel_type,
                                   nbv->nbat);
        wallcycle_sub_stop(wcycle_, ewcsNBS_GRID_NONLOCAL);
    ...
    rvec *const f = as_rvec_array(force_.data());
    ...
    gmx::ArrayRef<gmx::RVec> forceRef = force_;
    if (bDoForces)
    {
