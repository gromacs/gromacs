...
/*! \brief
 * A convenience getter for the commrec duty task assignment;
 * asserts that duty is actually valid (have been initialized).
 *
 * \param[in] cr    Communication structure pointer
 * \param[in] task  A single task's corresponding DUTY_ flag. Combinations are not supported.
 *
 * \returns Whether this task is assigned to this rank.
 */
