...
{
    range_check(dim, 0, DIM);
    if (efield_[dim].sigma_ > 0)
    {
        real t0 = efield_[dim].t0_;
        return efield_[dim].a_ * (cos(efield_[dim].omega_*(t-t0))*
                                  exp(-sqr(t-t0)/(2.0*sqr(efield_[dim].sigma_))));
    }
    else
    {
        return efield_[dim].a_ * cos(efield_[dim].omega_*t);
    }
}

bool ElectricField::applyField() const
{
    return (efield_[XX].a_ != 0 ||
            efield_[YY].a_ != 0 ||
            efield_[ZZ].a_ != 0);
}
