...
extern void pr_density_fitting(FILE *fp, int indent, const gmx::Densfit &densfit)
{
    ...
    const auto &parameters = densfit.parameters();
    PI("densfit-npoints", parameters.npoints  );
    pr_reals(fp, indent, "densfit-time ", parameters.time_values, parameters.npoints);
    pr_reals(fp, indent, "densfit-sigma", parameters.sigma_values, parameters.npoints);
    pr_reals(fp, indent, "densfit-k    ", parameters.k_values, parameters.npoints);
    pr_reals(fp, indent, "densfit-temp ", parameters.temp_values, parameters.npoints);
    PR("densfit-dist", parameters.dist);
    PI("densfit-nstfit", parameters.nstfit);
    PI("densfit-nstout", parameters.nstout);
    PI("densfit-nstmapout", parameters.nstmapout);
    PS("densfit-bKeepMaps", EBOOL(parameters.bKeepAndNumberMaps));

    pr_ivec_block(fp, indent, "atom", parameters.ind, parameters.nat, TRUE);
