...
class IForceSchedule
{
    ...
    protected:
        ...
        IForceSchedule() :
            ...
            enforcedRotation_(nullptr), ppForceWorkload_(nullptr)
        {}
        ...
        void init(FILE* fplog, t_commrec* cr, const gmx_multisim_t* ms, t_inputrec* inputrec,
                  ...
                  t_state* state, ArrayRefWithPadding<RVec> force, tensor* vir_force, t_mdatoms* mdatoms,
...
class ScheduleBuilder
{
    ...
    public:
        ...
        {
            if (experimentalSchedules_)
            {
                if (inputrec->coulombtype == eelCUT && !EVDW_PME(inputrec->vdwtype) && !DOMAINDECOMP(cr)
                    && !(fr->nbv->bUseGPU) && inputrec->cutoff_scheme == ecutsVERLET)
                {
                    type_ = SRCPU_verlet;
                }
            }
        ...
        void initSchedule(FILE *fplog, t_commrec *cr, const gmx_multisim_t *ms, t_inputrec *inputrec, t_nrnb *nrnb,
                          ...
                          t_state *state, ArrayRefWithPadding<RVec> force, tensor *vir_force,
                          ...
                          gmx_enfrot *enforcedRotation, PpForceWorkload *ppForceWorkload)
        {
            ...
            root_->init(fplog, cr, ms, inputrec, nrnb, wcycle, top, groups, state, std::move(force),
