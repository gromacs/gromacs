...
class IForceSchedule
{
    ...
    protected:
        ...
        IForceSchedule() :
            ...
            vsite_(nullptr), mu_tot_(nullptr), ed_(nullptr), sp_(nullptr), awh_(nullptr), enforcedRotation_(nullptr), ppForceWorkload_(
                    nullptr)
        {}
...
class ScheduleBuilder
{
    ...
    public:
        ...
        {
            if (inputrec->coulombtype == eelCUT && !EVDW_PME(inputrec->vdwtype) && !DOMAINDECOMP(cr) && !(fr->nbv->bUseGPU) && inputrec->cutoff_scheme == ecutsVERLET)
            {
                type_ = SRCPU_verlet;
            }
            else
