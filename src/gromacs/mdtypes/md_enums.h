...
//! \brief Boolean strings no or yes
extern const char *yesno_names[BOOL_NR+1];
//! \brief Boolean strings false or true
...
//! \brief Strings corresponding to temperatyre coupling types
extern const char *etcoupl_names[etcNR+1];
//! \brief Macro for selecting t coupling string
#define ETCOUPLTYPE(e) enum_name(e, etcNR, etcoupl_names)
//! \brief Return whether this is andersen coupling
...
//! \brief String corresponding to pressure coupling algorithm
extern const char *epcoupl_names[epcNR+1];
//! \brief Macro to return the correct pcoupling string
#define EPCOUPLTYPE(e) enum_name(e, epcNR, epcoupl_names)

/*! \brief flat-bottom posres geometries
 */
enum {
...
//! \brief Relative coordinate scaling type for position restraints
enum {
    erscNO, erscALL, erscCOM, erscNR
};
//! \brief String corresponding to relativ coordinate scaling
extern const char *erefscaling_names[erscNR+1];
//! \brief Macro to select correct coordinate scaling string
#define EREFSCALINGTYPE(e) enum_name(e, erscNR, erefscaling_names)

/*! \brief Trotter decomposition extended variable parts
 */
enum {
...
/*! \brief Sequenced parts of the trotter decomposition
 */
enum {
    ettTSEQ0,  ettTSEQ1,  ettTSEQ2,  ettTSEQ3,  ettTSEQ4, ettTSEQMAX
};

//! \brief Pressure coupling type
enum {
    epctISOTROPIC, epctSEMIISOTROPIC, epctANISOTROPIC,
    epctSURFACETENSION, epctNR
};
//! \brief String corresponding to pressure coupling type
extern const char *epcoupltype_names[epctNR+1];
//! \brief Macro to select the right string for pcoupl type
...
//! \brief String corresponding to cutoff scheme
extern const char *ecutscheme_names[ecutsNR+1];
//! \brief Macro to select the right string for cutoff scheme
...
//! \brief String corresponding to interaction modifiers
extern const char *eintmod_names[eintmodNR+1];
//! \brief Macro to select the correct string for modifiers
...
//! \brief String corresponding to Coulomb treatment
extern const char *eel_names[eelNR+1];
//! \brief Macro for correct string for Coulomb treatment
#define EELTYPE(e)     enum_name(e, eelNR, eel_names)

/*! \brief Ewald geometry
 */
enum {
    eewg3D, eewg3DC, eewgNR
};
//! \brief String corresponding to Ewald geometry
extern const char *eewg_names[eewgNR+1];

//! \brief Macro telling us whether we use reaction field
#define EEL_RF(e) ((e) == eelRF || (e) == eelGRF || (e) == eelRF_NEC_UNSUPPORTED || (e) == eelRF_ZERO )

//! \brief Macro telling us whether we use PME
#define EEL_PME(e)  ((e) == eelPME || (e) == eelPMESWITCH || (e) == eelPMEUSER || (e) == eelPMEUSERSWITCH || (e) == eelP3M_AD)
//! \brief Macro telling us whether we use PME or full Ewald
#define EEL_PME_EWALD(e) (EEL_PME(e) || (e) == eelEWALD)
//! \brief Macro telling us whether we use full electrostatics of any sort
#define EEL_FULL(e) (EEL_PME_EWALD(e) || (e) == eelPOISSON)
//! \brief Macro telling us whether we use user defined electrostatics
#define EEL_USER(e) ((e) == eelUSER || (e) == eelPMEUSER || (e) == (eelPMEUSERSWITCH))

//! \brief Van der Waals interaction treatment
enum {
    evdwCUT, evdwSWITCH, evdwSHIFT, evdwUSER, evdwENCADSHIFT,
    evdwPME, evdwNR
};
//! \brief String corresponding to Van der Waals treatment
extern const char *evdw_names[evdwNR+1];
//! \brief Macro for selecting correct string for VdW treatment
#define EVDWTYPE(e)    enum_name(e, evdwNR, evdw_names)

//! \brief Type of long-range VdW treatment of combination rules
enum {
    eljpmeGEOM, eljpmeLB, eljpmeNR
};
//! \brief String for LJPME combination rule treatment
extern const char *eljpme_names[eljpmeNR+1];
//! \brief Macro for correct LJPME comb rule name
#define ELJPMECOMBNAMES(e) enum_name(e, eljpmeNR, eljpme_names)

//! \brief Macro to tell us whether we use LJPME
#define EVDW_PME(e) ((e) == evdwPME)

//! \brief Neighborsearching algorithm
enum {
    ensGRID, ensSIMPLE, ensNR
};
//! \brief String corresponding to neighborsearching
extern const char *ens_names[ensNR+1];
//! \brief Macro for correct NS algorithm
...
//! \brief String corresponding to printing of free energy
...
//! \brief String corresponding to lambda weights
extern const char *elamstats_names[elamstatsNR+1];
//! \brief Macro telling us whether we use expanded ensemble
#define ELAMSTATS_EXPANDED(e) ((e) > elamstatsNO)
//! \brief Macro telling us whether we use some kind of Wang-Landau
...
//! \brief String corresponding to lambda moves
...
//! \brief String corresponding to equilibrium algorithm
...
//! \brief String corresponding to separate DHDL file selection
extern const char *separate_dhdl_file_names[esepdhdlfileNR+1];
//! \brief Monster macro for DHDL file selection
...
//! \brief String for DHDL derivatives
extern const char *dhdl_derivatives_names[edhdlderivativesNR+1];
//! \brief YAMM (Yet another monster macro)
...
//! \brief String corresponding to solvent type
extern const char *esol_names[esolNR+1];
//! \brief Macro lest we print the wrong solvent model string
#define ESOLTYPE(e)    enum_name(e, esolNR, esol_names)

/*! \brief Dispersion correction
 */
enum {
    edispcNO, edispcEnerPres, edispcEner, edispcAllEnerPres, edispcAllEner, edispcNR
};
//! \brief String corresponding to dispersion corrections
extern const char *edispc_names[edispcNR+1];
//! \brief Macro for dispcorr string
#define EDISPCORR(e)   enum_name(e, edispcNR, edispc_names)

/*! \brief Center of mass motion removal algorithm
 */
enum {
    ecmLINEAR, ecmANGULAR, ecmNO, ecmNR
};
//! \brief String corresponding to COM removal
extern const char *ecm_names[ecmNR+1];
//! \brief Macro for COM removal string
#define ECOM(e)        enum_name(e, ecmNR, ecm_names)

/*! \brief Algorithm for simulated annealing
 */
enum {
    eannNO, eannSINGLE, eannPERIODIC, eannNR
};
//! \brief String for simulated annealing
extern const char *eann_names[eannNR+1];
//! \brief And macro for simulated annealing string
#define EANNEAL(e)      enum_name(e, eannNR, eann_names)

/*! \brief Implicit solvent algorithms */
enum {
    eisNO, eisGBSA, eisNR
};
//! \brief String corresponding to implicit solvent
extern const char *eis_names[eisNR+1];
//! \brief Macro for implicit solvent string
#define EIMPLICITSOL(e) enum_name(e, eisNR, eis_names)

/*! \brief Algorithms for calculating GB radii
 */
enum {
    egbSTILL, egbHCT, egbOBC, egbNR
};
//! \brief String for GB algorithm name
extern const char *egb_names[egbNR+1];
//! \brief Macro for GB string
#define EGBALGORITHM(e) enum_name(e, egbNR, egb_names)

//! \brief Surface area algorithm for implicit solvent
enum {
    esaAPPROX, esaNO, esaSTILL, esaNR
};
//! \brief String corresponding to surface area algorithm
...
//! \brief Names for swapping
extern const char *eSwapTypes_names[eSwapTypesNR+1];
//! \brief Macro for swapping string
...
//! \brief String for swap group splitting
...
//! \brief String corresponding to QMMM methods
extern const char *eQMmethod_names[eQMmethodNR+1];
//! \brief Macro to pick QMMM method name
#define EQMMETHOD(e)   enum_name(e, eQMmethodNR, eQMmethod_names)

//! \brief QMMM basis function for QM part
enum {
...
//! \brief Name for QMMM basis function
extern const char *eQMbasis_names[eQMbasisNR+1];
//! \brief Macro to pick right basis function string
...
//! \brief String corresponding to nblist geometry names
...
//! \brief String corresponding to electrostatics kernels
...
//! \brief String corresponding to VdW kernels
...
//! \brief String corresponding to interactions in neighborlist code
