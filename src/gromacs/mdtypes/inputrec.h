 ...
 *
 * This abstract class provides one part of the interface to implement
 * additional forces in GROMACS. The IInputRecExtension needs to provide
 * routines to parse data from mdp files and from tpr files, to handle
 * distribution of parameters over processors (using broadcast) and to
 * initiate the forcerec data structure. For the actual force calculation
 * another class IForceProvider is developed (see forcerec.h).
 */
