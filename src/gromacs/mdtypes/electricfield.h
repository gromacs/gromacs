...
class ElectricField
{
    public:
        ...
        real a(int dim) const { return efield_[dim].a_; }
        real omega(int dim) const { return efield_[dim].omega_; }
        real t0(int dim) const{ return efield_[dim].t0_; }
        real sigma(int dim) const { return efield_[dim].sigma_; }

        //! Return whether or not to apply a field in this dimension
        bool applyField() const;

    private:
        //! The three field strengths
