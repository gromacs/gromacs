...
class TextTableFormatter::Impl
{
    public:
        ...
        struct ColumnData
        {
             ...
             */
            std::string textForNextLine(size_t columnWidth)
            {
...
std::string TextTableFormatter::formatRow()
{
         ...
         ++column)
    {
        ...
        {
            size_t columnWidth = column->width();
            if (line < lines.size() && lines[line].length() > currentWidth)
            {
                size_t overflow = lines[line].length() - currentWidth;
        ...
        for (size_t line = 0; line < columnLines.size(); ++line)
        {
            ...
            {
                lines[line].append(" ");
                if (lines[line].length() < currentWidth)
                {
