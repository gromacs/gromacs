...
TEST(NonbondedOnGpuAutoChooserTest, ThrowsWhenGpusWontWork)
{
    for (const auto &gpuCompatibilitiesPair : allGpuCompatibilities)
    {
        ...
        }
        {
            ...
            EXPECT_NO_THROW(taskAssigner->decideWhetherToUseGpus(true,  false));
            EXPECT_FALSE(taskAssigner->areNonbondedOnGpu());

            EXPECT_NO_THROW(taskAssigner->decideWhetherToUseGpus(false, true));
            EXPECT_FALSE(taskAssigner->areNonbondedOnGpu());

            EXPECT_NO_THROW(taskAssigner->decideWhetherToUseGpus(false, false));
            EXPECT_FALSE(taskAssigner->areNonbondedOnGpu());
        }
    }
}
    ...
    for (const auto &gpuCompatibilitiesPair : allGpuCompatibilities)
    {
        ...
        for (const auto &usingVerletScheme : booleans_g)
        {
            for (const auto &gpuAccelerationIsUseful : booleans_g)
            {
                try
                {
                    taskAssigner->decideWhetherToUseGpus(usingVerletScheme, gpuAccelerationIsUseful);
                }
                catch (InconsistentInputError &)
                {
                    ...
                    {
                        ADD_FAILURE() << "Unexpected throw from decideWhetherToUseGpus";
                    }
                }
