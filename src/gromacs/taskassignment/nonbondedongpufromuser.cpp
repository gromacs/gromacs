...
void
NonbondedOnGpuFromUser::chooseWhetherToUseGpus(const bool usingVerletScheme,
                                            ...
                                            const GpuTaskAssignments &gpuTaskAssignments) const
{
    ...
    if (isAnyGpuSharedBetweenRanks(gpuTaskAssignments))
    {
        ...
        GMX_LOG(mdlog.warning).asParagraph().
            appendText("NOTE: You assigned the same GPU ID(s) to multiple ranks, which might not be optimal.");
    }
