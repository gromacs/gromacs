...
const char * g_specifyEverythingFormatString =
    "When you use mdrun -gpu_id, %s must be set to non-default "
    "values, so that the device IDs can be interpreted correctly."
#if GMX_GPU != GMX_GPU_NONE
    " If you simply want to restrict which GPUs are used, then setting the "
                                                ...
                                                const int                 numRanksPerSimulation)
{
    ...
    if (!gpuIds.empty())
    {
        // Specifying -gpu_id requires specifying everything.
        ...
        if (pmeTarget == TaskTarget::Gpu)
        {
            if (numRanksPerSimulation > 1)
            {
                GMX_THROW(InconsistentInputError
                              ("When you run mdrun -pme gpu -gpu_id, you must supply a PME .tpr file and use a single rank."));
            }
            return true;
        }
...
bool decideWhetherToUseGpusForNonbonded(const TaskTarget           nonbondedTarget,
                                        ...
                                        const int                  numRanksPerSimulation)
{
    GMX_ASSERT(numRanksPerSimulation >= 1, "Number of ranks must be known");
    ...
    if (nonbondedTarget == TaskTarget::Cpu)
    {
        ...
        {
            GMX_THROW(InconsistentInputError
                          ("GPU IDs were specified, but nonbonded interactions were "
    ...
    if (emulateGpuNonbonded == EmulateGpuNonbonded::Yes)
    {
        ...
        {
            GMX_THROW(InconsistentInputError
                          ("GPU IDs were specified, as was GPU emulation. Make no more than one of these choices."));
        }
                                  ...
                                  const int               numRanksPerSimulation)
{
    ...
    if (pmeTarget == TaskTarget::Cpu)
    {
        ...
        {
            GMX_THROW(InconsistentInputError
                          ("GPU IDs were specified, but PME interactions were "
                          "assigned to the CPU. Make no more than one of these choices."));
        }
