...
runTaskAssignment(const std::vector<int>     &userGpuIds,
                  ...
                  const std::vector<GpuTask> &gpuTasksOnThisRank)
{
    ...
    try
    {
        ...
        ArrayRef<const int> gpuIdsToUse;
        std::vector<int>    generatedGpuIds;
        if (userGpuIds.empty())
        {
            generatedGpuIds = makeGpuIds(hardwareInfo.compatibleGpus, numGpuTasksOnThisNode);
            gpuIdsToUse     = generatedGpuIds;
        }
        else
        {
            if (numGpuTasksOnThisNode != userGpuIds.size())
            {
                ...
                GMX_THROW(InconsistentInputError
                              (formatString("There were %zu GPU IDs provided on node %s, but %zu GPU tasks were "
                                            "identified, and these must match. Reconsider you GPU ID selection, "
                                            "or your use of the -nb, -pme, and -npme options.", userGpuIds.size(),
                                            host, numGpuTasksOnThisNode)));
            }
            // Did the user choose compatible GPUs?
            checkUserGpuIds(hardwareInfo.gpu_info, hardwareInfo.compatibleGpus, userGpuIds);

            gpuIdsToUse = userGpuIds;
        }
        taskAssignmentOnRanksOfThisNode =
            buildTaskAssignment(gpuTasksOnRanksOfThisNode, gpuIdsToUse);
    ...
    reportGpuUsage(mdlog, !userGpuIds.empty(), taskAssignmentOnRanksOfThisNode,
                   cr->nrank_intranode, cr->nnodes > 1);

    // If the user chose GPU IDs, give them some hints where appropriate.
    if (!userGpuIds.empty())
    {
        logPerformanceHints(mdlog, hardwareInfo.compatibleGpus.size(),
