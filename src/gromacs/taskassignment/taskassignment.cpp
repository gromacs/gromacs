                          ...
                          MPI_Comm                   communicator)
{
    ...
    /* Split up the flat vector of tasks into something that can be
     * indexed by rank. */
    GpuTasksOnRanks gpuTasksOnRanksOfThisNode;
    for (size_t i = 0; i + 1 < displacementsForEachRank.size(); ++i)
    {
        gpuTasksOnRanksOfThisNode.push_back(std::vector<GpuTask>(&flatGpuTasksOnThisNode[displacementsForEachRank[i]],
                                                                 &flatGpuTasksOnThisNode[displacementsForEachRank[i+1]]));
    }
