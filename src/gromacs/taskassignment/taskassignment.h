...
/*! \brief All types of compute tasks that contribute to task assignment.
 *
 * These names refer to existing practice in GROMACS, which is not
 * strictly accurate.
 *
 * \todo Extend to other kinds of tasks as need arises. */
enum class ComputeTask : int
{
...
//! Container of compute tasks e.g. on each rank of a node.
using TasksOnRanks = std::vector< std::vector<ComputeTask> >;
...
//! Container of mappings from task index to task type and GPU device ID e.g. on each rank of a node.
using GpuTaskAssignments = std::vector< std::vector <GpuTaskMapping> >;

//! Forward declaration.
class INodeTaskAssigner;
//! Forward declaration
using INodeTaskAssignerPtr = std::unique_ptr<INodeTaskAssigner>;

/*! \brief Factory function for task assigner that chooses
 * functionality based on user input and detection results.
 *
 * \param[in]  nbOptionString       The user's choice for mdrun -nb for where to assign nonbonded interaction tasks.
 * \param[in]  gpuIdString          The string of GPU device IDs from the user.
 ...
 * \returns    A task assigner that will later respect the user's instructions (or throw if it cannot),
 *             inasmuch as the detected hardware and required tasks allow.
 * \throws     std::bad_alloc          If out of memory
 ...
 * \param[in]   taskAssigner            Handles details of task assignment
 * \param[in]   computeTasksOnThisRank  Information about what compute tasks that
 *                                      influence task assignment exist on this rank.
 *
