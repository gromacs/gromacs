...
//! Constants used to help minimize preprocessed code
static constexpr bool hasOmpSupport   = GMX_OPENMP;
static constexpr bool usesThreadMpi   = GMX_THREAD_MPI;
static constexpr bool usesExternalMpi = GMX_LIB_MPI;
static constexpr bool usesAnyMpi      = (usesThreadMpi || usesExternalMpi);
 ...
 */
static int min_atoms_per_mpi_thread =  90;
...
gmx_unused static int nthreads_omp_efficient_max(int gmx_unused       nrank,
                                                 ...
                                                 gmx_bool             bUseGPU)
{
    if (hasOmpSupport && usesAnyMpi && (nrank > 1))
    {
                     ...
                     bool                    doMembed)
{
    ...
    if (mtop->natoms/nrank < min_atoms_per_mpi_rank)
    {
         ...
         */
        if (hasOmpSupport && hw_opt->nthreads_omp <= 0 && hw_opt->nthreads_tot <= 0)
        {
                                        ...
                                        const gmx::MDLogger &mdlog)
{
        ...
        if (DOMAINDECOMP(cr))
        {
                ...
                nth_omp_max > nthreads_omp_mpi_ok_max)
            {
                ...
                else
                {
                     ...
                     */
                    gmx_fatal(FARGS,
                              "%s If you want to run with this setup, specify the -ntomp option. But we suggest to change the number of MPI ranks%s.",
                              buf, mpi_option);
                }
            }
        }
    ...
    else
    {
         ...
         */
        if (!hasOmpSupport && !usesAnyMpi && hwinfo->hardwareTopology->numberOfCores() > 1)
        {
                                             ...
                                             const gmx_mtop_t     &mtop)
{
    if (usesThreadMpi)
    {
        ...
        if (hw_opt->nthreads_tot > 0 && hw_opt->nthreads_omp <= 0)
        {
            ...
            if (!hasOmpSupport && hw_opt->nthreads_omp > 1)
            {
                gmx_fatal(FARGS,
     ...
     */
    bool canChooseNumOpenmpThreads      = (hasOmpSupport && hw_opt->nthreads_omp <= 0);
    ...
    GMX_RELEASE_ASSERT(hasOmpSupport || hw_opt->nthreads_omp == 1, "Without OpenMP support, only one thread per rank can be used");
