                                                   ...
                                                   int                            ngpu)
{
    ...
    if (ngpu > 0)
    {
         ...
         */
        GMX_RELEASE_ASSERT(hardwareOptions.nthreads_omp() >= 0, "nthreads_omp is negative, but previous checks should "
                           "have prevented this");
        while (nrank * hardwareOptions.nthreads_omp() > hwinfo->nthreads_hw_avail && nrank > 1)
        {
            nrank--;
        }
    ...
    else if (hardwareOptions.nthreads_omp.isSet())
    {
        ...
        nrank = std::max(1, nthreads_tot / hardwareOptions.nthreads_omp());
    }
    else
    {
                     ...
                     bool                              doMembed)
{
...

    {
        ...
        if (checker.mustUseOneRank())
        {
            ...
            if (hardwareOptions->nthreads_tmpi() > 1)
            {
    ...
    if (hardwareOptions->nthreads_tmpi.isSetByUser())
    {
        ...
        return hardwareOptions->nthreads_tmpi();
    }
    ...
    nthreads_tot_max = hardwareOptions->nthreads_tot.isSetByUser() ? hardwareOptions->nthreads_tot() : nthreads_hw;
    ...
    if (inputrec->cutoff_scheme == ecutsGROUP)
    {
        ...
        GMX_RELEASE_ASSERT(hardwareOptions->nthreads_omp() == 1, "The group scheme only supports one OpenMP thread per rank");
    }
    ...
    if (mtop->natoms/nrank < min_atoms_per_mpi_rank)
    {
        ...
        while (hardwareOptions->nthreads_tot.isSet() &&
               hardwareOptions->nthreads_tot() % nrank_new != 0)
        {
            nrank_new--;
        }
...
static void print_hw_opt(FILE *fp, const hardwareOptionsManager *hw_opt)
{
    fprintf(fp, "hw_opt: nt %d ntmpi %d ntomp %d ntomp_pme %d gpu_id '%s' gputasks '%s'\n",
            ...
            hw_opt->gpuTaskAssignment().c_str());
}
                               ...
                               int                            nPmeRanks)
{
    ...
    if (GMX_OPENMP)
    {
        ...
        if (hardwareOptions->nthreads_omp_pme() > 0 && !hardwareOptions->nthreads_omp.isSet())
        {
            gmx_fatal(FARGS, "You need to specify -ntomp in addition to -ntomp_pme");
        }

        if (hardwareOptions->nthreads_omp_pme.isSet() &&
            hardwareOptions->nthreads_omp_pme() != hardwareOptions->nthreads_omp() &&
            nPmeRanks <= 0)
        {
    ...
    else
    {
        ...
        if (hardwareOptions->nthreads_omp() > 1 || hardwareOptions->nthreads_omp_pme() > 1)
        {
    ...
    if (hardwareOptions->nthreads_tot.isSet() && !hardwareOptions->nthreads_omp_pme.isSet())
    {
        ...
        if (hardwareOptions->nthreads_tmpi.isSet() &&
            ...
            hardwareOptions->nthreads_tot() != hardwareOptions->nthreads_tmpi() * hardwareOptions->nthreads_omp())
        {
    ...
    if (hardwareOptions->nthreads_tot.isSet())
    {
        if (hardwareOptions->nthreads_omp() > hardwareOptions->nthreads_tot())
        {
            gmx_fatal(FARGS, "You requested %d OpenMP threads with %d total threads. Choose a total number of threads "
                      ...
                      hardwareOptions->nthreads_omp(), hardwareOptions->nthreads_tot());
        }

        if (hardwareOptions->nthreads_tmpi() > hardwareOptions->nthreads_tot())
        {
            gmx_fatal(FARGS, "You requested %d thread-MPI ranks with %d total threads. Choose a total number of "
                      ...
                      hardwareOptions->nthreads_tmpi(), hardwareOptions->nthreads_tot());
        }
    }
                               ...
                               int                     cutoff_scheme)
{
    if (cutoff_scheme == ecutsGROUP)
    {
        ...
        if (hardwareOptions->nthreads_omp.isSetByUser() && hardwareOptions->nthreads_omp() > 1)
        {
                                             ...
                                             const gmx_mtop_t               &mtop)
{
    if (GMX_THREAD_MPI)
    {
        ...
        GMX_RELEASE_ASSERT(hardwareOptions->nthreads_tmpi() >= 1, "Must have at least one thread-MPI rank");
         ...
         */
        if (hardwareOptions->nthreads_tot.isSetByUser() && !hardwareOptions->nthreads_omp.isSetByUser())
        {
            hardwareOptions->nthreads_omp.set(hardwareOptions->nthreads_tot() / hardwareOptions->nthreads_tmpi());
    ...
    GMX_RELEASE_ASSERT(GMX_OPENMP || hardwareOptions->nthreads_omp() == 1, "Without OpenMP support, only one thread per rank can be used");
    ...
    {
        hardwareOptions->nthreads_omp_pme.set(hardwareOptions->nthreads_omp());
    }
