               ...
               PmeRunMode                     pmeRunMode)
{
    ...
    std::string output;
    {
        ...
        output += gmx::formatString("PP tasks on GPUs %s compute bonded interactions on the GPU\n",
                                    useGpuForBonded ? "will" : "will not");
        if (pmeRunMode == PmeRunMode::Mixed)
        {
            output += gmx::formatString("PME tasks on GPUs will compute only spread and gather on the GPU\n");
        }
        else if (pmeRunMode == PmeRunMode::GPU)
        {
            output += gmx::formatString("PME tasks on GPUs will compute all aspects on the GPU\n");
        }
    }
