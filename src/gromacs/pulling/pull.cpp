...
{
    double unit_factor = pull_value2init_conversion_factor(&pcrd->params);
                                               ...
                                               double             value_ref)
{
    ...
    else if (pull_coordinate_is_angletype(&pcrd->params))
    {
        if (value_ref < 0 || value_ref > M_PI)
        {
            gmx_fatal(FARGS, "Pull reference angle for coordinate %d (%f) needs to be in the allowed interval (0,180) deg", coord_ind + 1, value_ref*pull_value2init_conversion_factor(&pcrd->params));
        }
    }
...
static void update_pull_coord_reference_value(pull_coord_work_t *pcrd, int coord_ind, double t)
{
    ...
    {
        double value_ref = (pcrd->params.init + pcrd->params.rate*t)*pull_init2value_conversion_factor(&pcrd->params);
          ...
          gmx_bool bOutFile, unsigned long Flags)
{
    ...
    for (c = 0; c < pull->ncoord; c++)
    {
        ...
        if (pcrd->params.rate == 0)
        {
            ...
            low_set_pull_coord_reference_value(pcrd, c, pcrd->params.init*pull_init2value_conversion_factor(&pcrd->params));
        }
    }
