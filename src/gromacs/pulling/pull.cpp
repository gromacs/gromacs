...
static void addToPullxHistory(struct pull_t *pull)
{
    ...
    for (int c = 0; c < pull->ncoord; c++)
    {
        ...
        pcrdHistory->value     += pcrd->value;
        pcrdHistory->value_ref += pcrd->value_ref;

        for (int m = 0; m < DIM; m++)
        {
            pcrdHistory->dr01[m] += pcrd->dr01[m];
            pcrdHistory->dr23[m] += pcrd->dr23[m];
            pcrdHistory->dr45[m] += pcrd->dr45[m];
        }
...
static void addToPullfHistory(struct pull_t *pull)
{
    ...
    for (int c = 0; c < pull->ncoord; c++)
    {
        ...
        pcrdHistory->f_scal += pcrd->f_scal;
    }
}
...
static void pullResetHistory(PullHistory  *history)
{
    ...
    for (int c = 0; c < history->numCoordinates; c++)
    {
        ...
        pcrdHistory->f_scal    = 0;

        for (int m = 0; m < DIM; m++)
        {
            pcrdHistory->dr01[m]          = 0;
            pcrdHistory->dr23[m]          = 0;
            pcrdHistory->dr45[m]          = 0;
...
static void pull_print_coord_dr(FILE *out, const ivec dim,
                                ...
                                const int    numValuesInSum)
{
    fprintf(out, "\t%g", pcrd->value*unit_factor/numValuesInSum);
    ...
    if (bPrintComponents)
    {
        pull_print_coord_dr_components(out, dim, pcrd->dr01, numValuesInSum);

        if (pcrd->params.ngroup >= 4)
        {
            pull_print_coord_dr_components(out, dim, pcrd->dr23, numValuesInSum);

            if (pcrd->params.ngroup >= 6)
            {
                pull_print_coord_dr_components(out, dim, pcrd->dr45, numValuesInSum);
            }
        }
    }
}
...
static void pull_print_f(FILE *out, struct pull_t *pull, double t)
{
    ...
    if (pull->bFOutAverage)
    {
        ...
        {
            fprintf(out, "\t%g", pull->fHistory->pullCoordinateSums[c].f_scal / pull->fHistory->numValuesInSum);
        }
    }
    else
    {
...
void pull_print_output(struct pull_t *pull, gmx_int64_t step, double time)
{
    ...
    if (pull->params.nstxout != 0)
    {
         ...
         * last value twice.*/
        if (pull->bXOutAverage && pull->ncoord > 0 && pull->xHistory->numValuesInSum < pull->params.nstxout)
        {
    ...
    if (pull->params.nstfout != 0)
    {
         ...
         * appending to a file from a checkpoint, which would otherwise include the
         * last value twice.*/
        if (pull->bFOutAverage && pull->ncoord > 0 && pull->fHistory->numValuesInSum < pull->params.nstfout)
        {
...
static void initPullHistory(struct pull_t *pull, ObservablesHistory *observablesHistory)
{
    ...
    if (pull->xHistory == nullptr)
    {
        ...
        pull->xHistory->numCoordinates   = pull->ncoord;
        pull->xHistory->numGroups        = pull->ngroup;
        pull->xHistory->numValuesInSum   = 0;
        snew(pull->xHistory->pullCoordinateSums, pull->xHistory->numCoordinates);
        snew(pull->xHistory->pullGroupSums, pull->xHistory->numGroups);
        snew(pull->xHistory->pullDynaSums,  pull->xHistory->numCoordinates);
    }
    if (pull->fHistory == nullptr)
    {
        ...
        pull->fHistory->numCoordinates     = pull->ncoord;
        pull->fHistory->numGroups          = pull->ngroup;
        pull->fHistory->numValuesInSum     = 0;
        snew(pull->fHistory->pullCoordinateSums, pull->fHistory->numCoordinates);
        snew(pull->fHistory->pullGroupSums, pull->fHistory->numGroups);
        snew(pull->fHistory->pullDynaSums,  pull->fHistory->numCoordinates);
    }
}
