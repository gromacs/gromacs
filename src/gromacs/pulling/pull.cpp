                                ...
                                gmx_bool bPrintComponents)
{
    ...
    fprintf(out, "\t%g", pcrd->geometry.value*unit_factor);
    ...
    if (bPrintComponents)
    {
        pull_print_coord_dr_components(out, pcrd->params.dim, pcrd->geometry.dr01);
        if (pcrd->params.ngroup >= 4)
        {
            pull_print_coord_dr_components(out, pcrd->params.dim, pcrd->geometry.dr23);
        }
        if (pcrd->params.ngroup >= 6)
        {
            pull_print_coord_dr_components(out, pcrd->params.dim, pcrd->geometry.dr45);
        }
    }
}
...
static void apply_forces_vec_torque(const struct pull_t     *pull,
                                    ...
                                    rvec                    *f)
{
    const PullCoordGeometry &geometry = pcrd->geometry;
    ...
    for (int m = 0; m < DIM; m++)
    {
        inpr += geometry.dr01[m]*geometry.vec[m];
    }
    ...
    for (int m = 0; m < DIM; m++)
    {
        f_perp[m] = (geometry.dr01[m] - inpr*geometry.vec[m])/geometry.vec_len*pcrd->scalarForce;
    }
                               ...
                               rvec *f)
{
    ...
    if (pcrd.params.eGeom == epullgCYL)
    {
        apply_forces_cyl_grp(&pull->dyna[coord], pcrd.geometry.cyl_dev, md,
        ...
        for (int m = 0; m < DIM; m++)
        {
            f_tot[m] = forces.force01[m] + pcrd.scalarForce*pcrd.geometry.ffrad[m];
        }
                        ...
                        const t_pbc             *pbc)
{
    ...
    if (pull_coordinate_is_directional(&pcrd->params))
    {
        ...
        for (int m = 0; m < DIM; m++)
        {
            if (m < pbc->ndim_ePBC && pcrd->geometry.vec[m] != 0)
            {
                                  ...
                                  dvec dr)
{
    ...
    if (pcrd->params.eGeom == epullgDIRPBC)
    {
        for (int m = 0; m < DIM; m++)
        {
            dref[m] = pcrd->value_ref*pcrd->geometry.vec[m];
        }
    ...
    for (int m = 0; m < DIM; m++)
    {
        ...
        if (pcrd->params.dim[m] && !(directional && pcrd->geometry.vec[m] == 0))
        {
            dr2 += dr[m]*dr[m];
        }
    }
                              ...
                              const t_pbc   *pbc)
{
    ...
    if (pcrd->params.eGeom == epullgDIRRELATIVE)
    {
        ...
        geometry.vec_len = dnorm(vec);
        for (m = 0; m < DIM; m++)
        {
            geometry.vec[m] = vec[m]/geometry.vec_len;
        }
        if (debug)
        {
            fprintf(debug, "pull coord %d vector: %6.3f %6.3f %6.3f normalized: %6.3f %6.3f %6.3f\n",
                    ...
                    geometry.vec[XX], geometry.vec[YY], geometry.vec[ZZ]);
        }
    }
...
static double get_dihedral_angle_coord(PullCoordGeometry *geometry)
{
    double phi, sign;
    dvec   dr32; /* store instead of dr23? */

    dsvmul(-1, geometry->dr23, dr32);
    dcprod(geometry->dr01, dr32, geometry->planevec_m);  /* Normal of first plane */
    dcprod(dr32, geometry->dr45, geometry->planevec_n);  /* Normal of second plane */
    phi = gmx_angle_between_dvecs(geometry->planevec_m, geometry->planevec_n);
     ...
     */
    sign = (diprod(geometry->dr01, geometry->planevec_n) < 0.0) ? 1.0 : -1.0;
    return sign*phi;
}
                                    ...
                                    const t_pbc   *pbc)
{
    ...
    switch (pcrd->params.eGeom)
    {
        case epullgDIST:
            ...
            geometry.value = dnorm(geometry.dr01);
            break;
        case epullgDIR:
        case epullgDIRPBC:
        ...
        case epullgCYL:
            ...
            for (m = 0; m < DIM; m++)
            {
                geometry.value += geometry.vec[m]*geometry.dr01[m];
            }
            break;
        case epullgANGLE:
            geometry.value = gmx_angle_between_dvecs(geometry.dr01, geometry.dr23);
            break;
        case epullgDIHEDRAL:
            geometry.value = get_dihedral_angle_coord(&geometry);
            break;
        case epullgANGLEAXIS:
            geometry.value = gmx_angle_between_dvecs(geometry.dr01, geometry.vec);
            break;
        default:
                                       ...
                                       double         t)
{
    ...
    dev = pcrd->geometry.value - pcrd->value_ref;

    /* Check that values are allowed */
    if (pcrd->params.eGeom == epullgDIST && pcrd->geometry.value == 0)
    {
                            ...
                            const t_pbc   *pbc)
{
    ...
    return pull->coord[coord_ind].geometry.value;
}
    ...
    for (size_t c = 0; c < pull->coord.size(); c++)
    {
        ...
        if (debug)
        {
            fprintf(debug, "Pull coord %zu dr %f %f %f\n",
                    c, geometry.dr01[XX], geometry.dr01[YY], geometry.dr01[ZZ]);
        }
            ...
            pcrd->params.eGeom == epullgDIRPBC)
        {
            ...
            for (m = 0; m < DIM; m++)
            {
                a += geometry.vec[m]*geometry.dr01[m];
            }
            for (m = 0; m < DIM; m++)
            {
                r_ij[c][m] = a*geometry.vec[m];
            }
        }
        else
        {
            copy_dvec(geometry.dr01, r_ij[c]);
        }
                             ...
                             const PullCoordVectorForces &forces)
{
    if (vir != nullptr && pcrd.params.eGeom != epullgDIRPBC)
    {
        ...
        add_virial_coord_dr(vir, pcrd.geometry.dr01, forces.force01);
        if (pcrd.params.ngroup >= 4)
        {
            add_virial_coord_dr(vir, pcrd.geometry.dr23, forces.force23);
        }
        if (pcrd.params.ngroup >= 6)
        {
            add_virial_coord_dr(vir, pcrd.geometry.dr45, forces.force45);
        }
    }
}
                                                       ...
                                                       real *V, real *dVdl)
{
    ...
    switch (pcrd->params.eType)
    {
        ...
        case epullCONST_F:
            pcrd->scalarForce    = -k;
            *V                  +=    k*pcrd->geometry.value;
            *dVdl               += dkdl*pcrd->geometry.value;
            break;
        case epullEXTERNAL:
...
calculateVectorForces(const pull_coord_work_t &pcrd)
{
    ...
    {
        double invdr01 = geometry.value > 0 ? 1./geometry.value : 0.;
        for (int m = 0; m < DIM; m++)
        {
            forces.force01[m] = pcrd.scalarForce*geometry.dr01[m]*invdr01;
        }
    }
    else if (params.eGeom == epullgANGLE)
    {
        ...
        if (cos_theta2 < 1)
        {
            ...
            double invdr01 = 1./dnorm(geometry.dr01);
            double invdr23 = 1./dnorm(geometry.dr23);
            dvec   normalized_dr01, normalized_dr23;
            dsvmul(invdr01, geometry.dr01, normalized_dr01);
            dsvmul(invdr23, geometry.dr23, normalized_dr23);
    ...
    else if (params.eGeom == epullgANGLEAXIS)
    {
        ...
        if (cos_theta2 < 1)
        {
            ...
            for (int m = 0; m < DIM; m++)
            {
                forces.force01[m] = pcrd.scalarForce*invdr01*(a*geometry.vec[m] - b*normalized_dr01[m]);
            }
        }
        else
        {
    ...
    else if (params.eGeom == epullgDIHEDRAL)
    {
         ...
         */
        m2 = diprod(geometry.planevec_m, geometry.planevec_m);
        n2 = diprod(geometry.planevec_n, geometry.planevec_n);
        dsvmul(-1, geometry.dr23, dr32);
        ...
        if ((m2 > tol) && (n2 > tol))
        {
            ...
            dsvmul(-a_01, geometry.planevec_m, forces.force01); /* added sign to get force on group 1, not 0 */

            /* Forces on groups 4, 5 */
            a_45 = -pcrd.scalarForce*dist_32/n2;
            dsvmul(a_45, geometry.planevec_n, forces.force45);  /* force on group 5 */

            /* Force on groups 2, 3 (defining the axis) */
            a_23_01 = -diprod(geometry.dr01, dr32)*inv_sqrdist_32;
            a_23_45 = -diprod(geometry.dr45, dr32)*inv_sqrdist_32;
    ...
    else
    {
        for (int m = 0; m < DIM; m++)
        {
            forces.force01[m] = pcrd.scalarForce*geometry.vec[m];
        }
    }
          ...
          real lambda)
{
    ...
    {
        /* Construct, copying all pull coordinate parameters */
        ...
        switch (pcrd->params.eGeom)
        {
            ...
            case epullgCYL:
            case epullgANGLEAXIS:
                copy_rvec_to_dvec(pull_params->coord[c].vec, pcrd->geometry.vec);
                break;
            default:
