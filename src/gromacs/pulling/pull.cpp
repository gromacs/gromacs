...
static void pull_reset_history(PullHistory  *history)
{
    for (int i = 0; i < history->numCoordinates * history->numValuesPerCoordinate; i++)
    {
        history->sum[i] = 0;
    }
                               ...
                               gmx_int64_t *pull_hist_iter)
{
    if (bPrintAvg)
    {
        for (int m = 0; m < DIM; m++)
        {
            if (dim[m])
            {
                fprintf(out, "\t%g", history->sum[*pull_hist_iter] / history->numValuesInSum);
            }
            (*pull_hist_iter)++;
        }
    }
                                ...
                                gmx_int64_t *pull_hist_iter)
{
    ...
    if (bPrintAvg)
    {
        fprintf(out, "\t%g", (history->sum[*pull_hist_iter] / history->numValuesInSum) * unit_factor);
        (*pull_hist_iter)++;
    ...
    if (bPrintComponents)
    {
        pull_print_coord_dr_components(out, pcrd->params.dim, pcrd->dr01,
                                       bPrintAvg, history, pull_hist_iter);
...
static void pull_print_x(FILE *out, struct pull_t *pull, double t)
{
        ...
        if (pull->params.bPrintCOM)
        {
            if (pcrd->params.eGeom == epullgCYL)
            {
                pull_print_group_x(out, pcrd->params.dim, &pull->dyna[c],
                                   pull->bXOutAverage, pull->xHistory, &pull_hist_iter);
            }
            else
            {
    ...
    {
        pull_reset_history(pull->xHistory);
    }
    fprintf(out, "\n");
}
...
static void pull_print_f(FILE *out, struct pull_t *pull, double t)
{
    ...
    if (pull->bFOutAverage)
    {
        ...
        pull_reset_history(pull->fHistory);
    }
    else
    {
        for (c = 0; c < pull->ncoord; c++)
...
static void add_to_pullx_history(struct pull_t *pull)
{
    int pull_hist_iter  = 0;
    ...
    for (int c = 0; c < pull->ncoord; c++)
    {
        ...
        pull->xHistory->sum[pull_hist_iter]         += pcrd->value;
        pull_hist_iter++;

        if (pull->params.bPrintRefValue)
        {
            pull->xHistory->sum[pull_hist_iter]     += pcrd->value_ref;
            pull_hist_iter++;
        }
        if (pull->params.bPrintComp)
        {
            for (int m = 0; m < DIM; m++)
            {
                pull->xHistory->sum[pull_hist_iter] += pcrd->dr01[m];
                pull_hist_iter++;
            }
            for (int m = 0; m < DIM; m++)
            {
                pull->xHistory->sum[pull_hist_iter] += pcrd->dr23[m];
                pull_hist_iter++;
            }
            for (int m = 0; m < DIM; m++)
            {
                pull->xHistory->sum[pull_hist_iter] += pcrd->dr45[m];
                pull_hist_iter++;
            }
        }
        if (pull->params.bPrintCOM)
        {
            if (pcrd->params.eGeom == epullgCYL)
            {
                for (int m = 0; m < DIM; m++)
                {
                    pull->xHistory->sum[pull_hist_iter] += pull->dyna[c].x[m];
                    pull_hist_iter++;
                }
            }
            else
            {
                for (int m = 0; m < DIM; m++)
                {
                    pull->xHistory->sum[pull_hist_iter] += pull->group[pcrd->params.group[0]].x[m];
                    pull_hist_iter++;
                }
            }
            for (int g = 1; g < pcrd->params.ngroup; g++)
            {
                for (int m = 0; m < DIM; m++)
                {
                    pull->xHistory->sum[pull_hist_iter] += pull->group[pcrd->params.group[g]].x[m];
                    pull_hist_iter++;
                }
            }
        }
...
static void add_to_pullf_history(struct pull_t *pull)
{
        ...
        if (observablesHistory != nullptr)
        {
            ...
            if (pull->bXOutAverage)
            {
                ...
                pull->xHistory->sum.resize(pull->xHistory->numCoordinates *
                                           pull->xHistory->numValuesPerCoordinate);
            }
            if (pull->bFOutAverage)
            {
                ...
                pull->fHistory->sum.resize(pull->fHistory->numCoordinates *
                                           pull->fHistory->numValuesPerCoordinate);
            }
        }
