...
static void addToPullxHistory(struct pull_t *pull)
{
    pull->xHistory->numValuesInSum++;
    for (size_t c = 0; c < pull->coord.size(); c++)
    {
        pull_coord_work_t *pcrd, *pcrdHistory;
        pcrd        = &pull->coord[c];
        pcrdHistory = &pull->xHistory->pullCoordinateSums[c];

        pcrdHistory->spatialData.value += pcrd->spatialData.value;
        pcrdHistory->value_ref         += pcrd->value_ref;

        for (int m = 0; m < DIM; m++)
        {
            pcrdHistory->spatialData.dr01[m] += pcrd->spatialData.dr01[m];
            pcrdHistory->spatialData.dr23[m] += pcrd->spatialData.dr23[m];
            pcrdHistory->spatialData.dr45[m] += pcrd->spatialData.dr45[m];
        }
        if (pcrd->params.eGeom == epullgCYL)
        {
            for (int m = 0; m < DIM; m++)
            {
                pull->xHistory->pullDynaSums[c].x[m] += pull->dyna[c].x[m];
            }
        }
    }
    for (int g = 0; g < pull->ngroup; g++)
    {
        pull_group_work_t *pgrpHistory = &pull->xHistory->pullGroupSums[g];
        for (int m = 0; m < DIM; m++)
        {
...
static void addToPullfHistory(struct pull_t *pull)
{
    pull->fHistory->numValuesInSum++;
    for (size_t c = 0; c < pull->coord.size(); c++)
    {
        pull_coord_work_t *pcrd, *pcrdHistory;
        pcrd        = &pull->coord[c];
        pcrdHistory = &pull->fHistory->pullCoordinateSums[c];
...
{
    history->numValuesInSum = 0;

    for (int c = 0; c < history->numCoordinates; c++)
    {
        pull_coord_work_t *pcrdHistory = &history->pullCoordinateSums[c];

        pcrdHistory->spatialData.value = 0;
        pcrdHistory->value_ref         = 0;
        pcrdHistory->scalarForce       = 0;

        for (int m = 0; m < DIM; m++)
        {
            pcrdHistory->spatialData.dr01[m] = 0;
            pcrdHistory->spatialData.dr23[m] = 0;
            pcrdHistory->spatialData.dr45[m] = 0;
            history->pullDynaSums[c].x[m]    = 0;
        }
    }

    for (int g = 0; g < history->numGroups; g++)
    {
        pull_group_work_t *pgrpHistory = &history->pullGroupSums[g];
        for (int m = 0; m < DIM; m++)
        {
            pgrpHistory->x[m] = 0;
        }
                               ...
                               const int numValuesInSum)
{
    for (int m = 0; m < DIM; m++)
    {
        if (dim[m])
        {
            fprintf(out, "\t%g", pgrp->x[m] / numValuesInSum);
        }
    }
}
...
static void pull_print_coord_dr(FILE *out, const ivec dim,
                                ...
                                const int    numValuesInSum)
{
    fprintf(out, "\t%g", pcrd->spatialData.value*unit_factor/numValuesInSum);

    if (bPrintRefValue)
    {
        fprintf(out, "\t%g", pcrd->value_ref*unit_factor/numValuesInSum);
    }

    if (bPrintComponents)
    {
        pull_print_coord_dr_components(out, dim, pcrd->spatialData.dr01, numValuesInSum);
        if (pcrd->params.ngroup >= 4)
        {
            pull_print_coord_dr_components(out, dim, pcrd->spatialData.dr23, numValuesInSum);
        }
        if (pcrd->params.ngroup >= 6)
        {
            pull_print_coord_dr_components(out, dim, pcrd->spatialData.dr45, numValuesInSum);
        }
    }
}
...
static void pull_print_x(FILE *out, struct pull_t *pull, double t)
{
    ...
    for (size_t c = 0; c < pull->coord.size(); c++)
    {
        ...
        {
            numValuesInSum = pull->xHistory->numValuesInSum;
        }

        if (pull->bXOutAverage)
        {
            pull_print_coord_dr(out, pcrd->params.dim, &pull->xHistory->pullCoordinateSums[c],
                                pull->params.bPrintRefValue && pcrd->params.eType != epullEXTERNAL,
                                pull->params.bPrintComp, unit_factor, numValuesInSum);
        }
        else
        {
            pull_print_coord_dr(out, pcrd->params.dim, pcrd,
                                pull->params.bPrintRefValue && pcrd->params.eType != epullEXTERNAL,
                                pull->params.bPrintComp, unit_factor, numValuesInSum);
        }
        ...
        if (pull->params.bPrintCOM)
        {
            if (pcrd->params.eGeom == epullgCYL)
            {
                if (pull->bXOutAverage)
                {
                    pull_print_group_x(out, pcrd->params.dim, &pull->xHistory->pullDynaSums[c], numValuesInSum);
                }
                else
                {
                    pull_print_group_x(out, pcrd->params.dim, &pull->dyna[c], numValuesInSum);
                }
            }
            else
            {
                if (pull->bXOutAverage)
                {
                    pull_print_group_x(out, pcrd->params.dim,
                                       &pull->xHistory->pullGroupSums[pcrd->params.group[0]],
                                       numValuesInSum);
                }
                else
                {
            ...
            for (int g = 1; g < pcrd->params.ngroup; g++)
            {
                if (pull->bXOutAverage)
                {
                    pull_print_group_x(out, pcrd->params.dim, &pull->xHistory->pullGroupSums[pcrd->params.group[g]],
                                       numValuesInSum);
                }
                else
                {
    ...
    pullResetHistory(pull->xHistory);
}
...
static void pull_print_f(FILE *out, const pull_t *pull, double t)
{
    ...
    if (pull->bFOutAverage)
    {
        for (size_t c = 0; c < pull->coord.size(); c++)
        {
            fprintf(out, "\t%g", pull->fHistory->pullCoordinateSums[c].scalarForce / pull->fHistory->numValuesInSum);
        }
    }
    else
    {
    ...
    pullResetHistory(pull->fHistory);
}
...
void pull_print_output(struct pull_t *pull, gmx_int64_t step, double time)
{
    ...
    if (pull->params.nstxout != 0)
    {
         ...
         * last value twice.*/
        if (pull->bXOutAverage && pull->coord.size() > 0 && pull->xHistory->numValuesInSum < pull->params.nstxout)
        {
    ...
    if (pull->params.nstfout != 0)
    {
         ...
         * appending to a file from a checkpoint, which would otherwise include the
         * last value twice.*/
        if (pull->bFOutAverage && pull->coord.size() > 0 && pull->fHistory->numValuesInSum < pull->params.nstfout)
        {
...
static void initPullHistory(struct pull_t *pull, ObservablesHistory *observablesHistory)
{
    if (observablesHistory == nullptr)
    {
        return;
    }
    if (pull->xHistory == nullptr)
    {
        observablesHistory->pullXHistory = std::unique_ptr<PullHistory>(new PullHistory {});
        pull->xHistory                   = observablesHistory->pullXHistory.get();
        pull->xHistory->numCoordinates   = pull->coord.size();
        pull->xHistory->numGroups        = pull->ngroup;
        pull->xHistory->numValuesInSum   = 0;
        pull->xHistory->pullCoordinateSums.resize(pull->xHistory->numCoordinates);
        pull->xHistory->pullGroupSums.resize(pull->xHistory->numGroups);
        pull->xHistory->pullDynaSums.resize(pull->xHistory->numCoordinates);
    }
    if (pull->fHistory == nullptr)
    {
        observablesHistory->pullFHistory   = std::unique_ptr<PullHistory>(new PullHistory {});
        pull->fHistory                     = observablesHistory->pullFHistory.get();
        pull->fHistory->numCoordinates     = pull->coord.size();
        pull->fHistory->numGroups          = pull->ngroup;
        pull->fHistory->numValuesInSum     = 0;
        pull->fHistory->pullCoordinateSums.resize(pull->fHistory->numCoordinates);
        pull->fHistory->pullGroupSums.resize(pull->fHistory->numGroups);
        pull->fHistory->pullDynaSums.resize(pull->fHistory->numCoordinates);
    }
}
...
init_pull(FILE *fplog, const pull_params_t *pull_params, const t_inputrec *ir,
          ...
          real lambda, ObservablesHistory *observablesHistory)
{
    struct pull_t *pull;
    pull_comm_t   *comm;
    ...
    {
        pull->xHistory    = nullptr;
        pull->fHistory    = nullptr;
    }
    ...
    initPullHistory(pull, observablesHistory);
