                                 ...
                                 int gmx_unused nthreads)
{
     ...
     */
    int numAtomsLocal = pgrp->atomSet.numAtomsLocal();
#pragma omp parallel for num_threads(nthreads) schedule(static)
          ...
          real lambda)
{
    ...
    comm->rvecBuffer.resize(pull->group.size());
    comm->dvecBuffer.resize(pull->group.size()*DIM);
    if (pull->bCylinder)
    {
        comm->cylinderBuffer.resize(pull->group.size()*c_cylinderBufferStride);
    }
