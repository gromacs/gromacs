                                  ...
                                  const t_inputrec *ir, real lambda)
{
    ...
    for (int i = 0; i < pg->params.nat; i++)
    {
        ...
        if (bConstraint && ir->opts.nFreeze)
        {
            ...
            {
                if (pulldim_con[d] == 1 &&
                    ir->opts.nFreeze[getGroupType(groups, SimulationGroups::g_FREEZE, ii)][d])
                {
                    nfrozen++;
                }
            }
        ...
        else if (ir->eI == eiBD)
        {
            ...
            {
                if (groups.groupNumbers[static_cast<int>(SimulationGroups::g_TC)].empty())
                {
                    mbd = ir->delta_t/ir->opts.tau_t[0];
                }
                else
                {
                    mbd = ir->delta_t/ir->opts.tau_t[groups.groupNumbers[static_cast<int>(SimulationGroups::g_TC)][ii]];
                }
            }
            w                   *= m/mbd;
            m                    = mbd;
