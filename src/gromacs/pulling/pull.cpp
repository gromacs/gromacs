                          ...
                          ObservablesHistory *observablesHistory)
{
    ...
    {
        /* Keep count of the average pull coordinate values. Do not include the current value in the average
         * if the number of observations already equal (or are higher than) what should be in each average
         * output. This can happen when appending to a file from a checkpoint, which would otherwise
         * include the last value twice. */
        ...
        /* Keep count of the average pull force values. Do not include the current value in the average
         * if the number of observations already equal (or are higher than) what should be in each average
         * output. This can happen when appending to a file from a checkpoint, which would otherwise
         * include the last value twice. */
                    ...
                    ObservablesHistory *observablesHistory)
{
    ...
    if (pull->comm.bParticipate)
    {
        ...
        for (int c = 0; c < pull->ncoord; c++)
        {
            ...
            if (pull->bFOutAvg && observablesHistory->pullFHistory->nsum < pull->params.nstfout)
            {
                observablesHistory->pullFHistory->sum[c] += pcrd->f_scal;
                observablesHistory->pullFHistory->ave[c]  = observablesHistory->pullFHistory->sum[c] / observablesHistory->pullFHistory->nsum;
            }
            if (pull->bXOutAvg && observablesHistory->pullXHistory->nsum < pull->params.nstxout)
            {
                observablesHistory->pullXHistory->sum[c * 2]     += pcrd->value;
                observablesHistory->pullXHistory->sum[c * 2 + 1] += pcrd->value_ref;
                observablesHistory->pullXHistory->ave[c * 2]      = observablesHistory->pullXHistory->sum[c * 2] / observablesHistory->pullXHistory->nsum;
