...
static void assert_pull_coord_reference_value_is_allowed(const pull_coord_work_t *pcrd, int coord_ind)
{
    char errormsg[STRLEN];

    if (pcrd->params.eGeom == epullgDIST)
    {
        sprintf(errormsg, "Pull reference distance for coordinate %d (%f) needs to be non-negative", coord_ind + 1, pcrd->value_ref);
        GMX_RELEASE_ASSERT(pcrd->value_ref >= 0, errormsg);
    }
    else if (pcrd->params.eGeom == epullgANGLE)
    {
        sprintf(errormsg, "Pull reference distance for coordinate %d (%f) needs to be in the allowed interval (0,180) deg", coord_ind + 1, pcrd->value_ref*RAD2DEG);
        GMX_RELEASE_ASSERT((pcrd->value_ref >= 0 && pcrd->value_ref <= M_PI), errormsg);
    }
}
...
static void update_pull_coord_reference_value(pull_coord_work_t *pcrd, int coord_ind, double t)
{
    ...
    assert_pull_coord_reference_value_is_allowed(pcrd, coord_ind);
}
