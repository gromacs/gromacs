    ...
    for (g = 0; g < pull->ngroup; g++)
    {
        ...
        if (pgrp->params.bSliced)
        {
            ...
            for (i = 0; i < pgrp->nat_loc; i++)
            {
                ...
                real x_center = (pgrp->params.slice_x_min + pgrp->params.slice_x_max) / 2;
                real x_half   = (pgrp->params.slice_x_max - pgrp->params.slice_x_min) / 2;
                if (x[ii][0] >= pgrp->params.slice_x_min && x[ii][0] <= pgrp->params.slice_x_max)
                {
                    ...
                    real x_coord = x[ii][0];
                    real x_unit  = std::fabs(x_coord - x_center) / x_half;
                    real weight  = 1-2*x_unit*x_unit+x_unit*x_unit*x_unit*x_unit;
                    if (pgrp->weight_loc != NULL)
                    {
                        ...
                        pgrp->weight_loc_sliced[pgrp->nat_loc_sliced] = pgrp->weight_loc[i] * weight;
                    }
                    else
                    {
                        pgrp->weight_loc_sliced[pgrp->nat_loc_sliced] = weight;
                    }
                    if (debug)
                    {
                        fprintf(debug, "   adjusted atom weight %f\n", pgrp->weight_loc_sliced[i]);
                    }
                    pgrp->nat_loc_sliced++;
                }
            }
            ...
            {
#if GMX_MPI
                MPI_Allreduce(&pgrp->nat_loc_sliced, &nat_sliced, 1, MPI_INT, MPI_SUM, cr->mpi_comm_mygroup);
#endif
            }
    ...
    for (i = 0; i < pg->params.nat; i++)
    {
        ...
        if (ii >= start && ii < end)
        {
            ...
            if (pg->params.bSliced)
            {
                ...
                {
                    pg->weight_loc_sliced[pg->nat_loc_sliced] = 1.0;
                }
                pg->nat_loc_sliced++;
            }
        }
                                  ...
                                  const t_inputrec *ir, real lambda)
{
    ...
    if (cr && PAR(cr))
    {
        ...
        if (pg->params.bSliced)
        {
            ...
            pg->ind_loc_sliced    = nullptr;
            pg->weight_loc_sliced = nullptr;
        }
    }
    else
    {
        ...
        if (pg->params.bSliced)
        {
            ...
            /* If there are existing weights, use them. Otherwise, set all
             * weights to 1.0 so they can be used later for potential/force
             * smoothening. */
            if (pg->params.nweight > 0)
            {
            ...
            {
                for (int i = 0; i < pg->nat_loc_sliced; i++)
                {
                    pg->weight_loc_sliced[i] = 1.0;
                }
            }
        }
    }
