                                       ...
                                       double         t)
{
    ...
    else if (pcrd->params.eGeom == epullgANGLE || pcrd->params.eGeom == epullgANGLEAXIS)
    {
        ...
        {
            gmx_fatal(FARGS, "Pull reference distance for coordinate %d (%f) is not in the allowed interval (0,180) deg", coord_ind+1, pcrd->value_ref*RAD2DEG);
        }
    }
    ...
    switch (pcrd->params.eType)
    {
        ...
        case epullCONST_F:
            ...
            *dVdl        += dkdl*pcrd->value;
            break;
        case epullEXTERNAL:
            assert(pcrd->func != NULL);
            double x, Vd;
            x             = pcrd->value_ref + dev;
            pcrd->func(pcrd->data, coord_ind, x, &Vd, &pcrd->f_scal);
            *V           += Vd;
