                                ...
                                gmx_bool bPrintComponents)
{
    ...
    bConvert_radians_to_degrees = (pcrd->params.eGeom == epullgANGLE || pcrd->params.eGeom == epullgDIHEDRAL || pcrd->params.eGeom == epullgANGLEAXIS);
        ...
        for (int m  = 0; m < DIM; m++)
        {
            if (pcrd->params.dim[m])
            {
                ...
                if (g == 0 && pcrd->params.ngroup <= 2)
                {
                    ...
                    sprintf(legend, "%d %s%c", coord_index + 1, "d", 'X' + m);
                }
                else
                {
                    /* Otherwise, print also the group indices (relative to the coordinate, not the global ones). */
                    sprintf(legend, "%d %s %d-%d %s%c", coord_index + 1, "g", g + 1, g + 2, "d", 'X' + m);
                }
    ...
    else
    {
        ...
        for (c = 0; c < pull->ncoord; c++)
        {
            if (bCoord)
            {
                ...
                {
                    int g, ngroup;
                    ngroup = pull->coord[c].params.ngroup;
...
static double get_dihedral_angle_coord(pull_coord_work_t *pcrd)
{
    ...
    phi = gmx_dangle(pcrd->planevec_m, pcrd->planevec_n);
