...
static void assert_pull_coord_reference_value_is_allowed(const pull_coord_work_t *pcrd, int coord_ind)
{
    ...
    {
        sprintf(errormsg, "Pull reference distance for coordinate %d is negative (%f)", coord_ind + 1, pcrd->value_ref);
        GMX_RELEASE_ASSERT(pcrd->value_ref < 0, errormsg);
    }
    else if (pcrd->params.eGeom == epullgANGLE)
    {
        sprintf(errormsg, "Pull reference distance for coordinate %d (%f) is not in the allowed interval (0,180) deg", coord_ind + 1, pcrd->value_ref*RAD2DEG);
