...
static const char* FitStr      = "";                    /* Used in g_map output */
...
static real interpolateLinearly(real time, int npoints, real* time_values, real* y_values)
{
    ...
    /* We are somewhere in between, that means we nee to interpolate y linearly */
    j = 0;
...
/* Dumps forces to file with file name "fn", if no filename given,
 * forces are dumped to the default density fitting output file densfit.xvg
 */
...
extern void gmx_map_print_header(gmx_unused FILE *fpout, gmx_unused const char *fn)
{
#ifdef WITHCCP4
    int          i;
    char        *title;
 ...
 */
extern void gmx_do_map_ccp4(gmx_bool bRead, gmx_unused t_mapdata **ptr_map, const char *fn,
                            gmx_unused gmx_bool bOverwrite, gmx_bool bVerbose, FILE *DumpHeader)
{
    ...
    if (NULL == fn)
    {
        gmx_fatal(FARGS, "%sgmx_do_map_ccp4: no filename given, cannot %s map!\n", FitStr, bRead ? "read" : "write");
    }
...
/* As exp, but for n elements. This can be accelerated by using
...
//    get_map_spacing(densfit, TRUE); TODO
    ...
    if (MASTER(cr))
    {
        if (bOutputMap)
        {
            ...
            gmx_do_map_ccp4(FALSE, &densfit->map_sim, fn, TRUE, df->bVerbose, FALSE);
            gmx_do_map_ccp4(FALSE, &densfit->map_ref, "map_ref", TRUE, df->bVerbose, FALSE);
        }
