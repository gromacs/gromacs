    ...
    for (g = 0; g < pull->ngroup; g++)
    {
        ...
        if (pgrp->bCalcCOM)
        {
            if (pgrp->epgrppbc != epgrppbcCOS)
            {
                ...
                else if (pgrp->epgrppbc == epgrppbcLASTSTEPCOM)
                {
                    if (pgrp->x_last_step[XX] == 0.0 && pgrp->x_last_step[YY] == 0.0 && pgrp->x_last_step[ZZ] == 0.0)
                    {
                        copy_rvec_to_dvec(comm->rbuf[g], pgrp->x_last_step);
                    }
                    /* Set the pbc reference to the COM of the group of the last step */
                    copy_dvec_to_rvec(pgrp->x_last_step, x_pbc);
                    copy_dvec_to_rvec(pgrp->x_last_step, comm->rbuf[g]);
                }
        ...
        if (pgrp->bCalcCOM)
        {
            ...
            if (pgrp->epgrppbc != epgrppbcCOS)
            {
                ...
                for (m = 0; m < DIM; m++)
                {
                    ...
                    if (pgrp->epgrppbc == epgrppbcREFAT || epgrppbcLASTSTEPCOM)
                    {
                ...
                copy_dvec(pgrp->xp, pgrp->x_last_step);
            }
            else
            {
                /* Cosine weighting geometry */
