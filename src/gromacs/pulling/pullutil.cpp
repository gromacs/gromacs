                              ...
                              gmx::ArrayRef<gmx::RVec> x_pbc)
{
    ...
    if (cr && PAR(cr) && numPbcAtoms > 0)
    {
         ...
         */
        pull_reduce_real(cr, &pull->comm, pull->group.size()*DIM, x_pbc[0]);
    }
}
                             ...
                             t_pbc *pbc, double t, rvec *x)
{
    ...
    GMX_ASSERT(comm->cylinderBuffer.size() >= pull->coord.size()*c_cylinderBufferStride, "Cylinder buffer needs to be sufficiently large");
    ...
    for (size_t c = 0; c < pull->coord.size(); c++)
    {
        ...
        if (pcrd->params.eGeom == epullgCYL)
        {
             ...
             */
            pdyna.localWeights.resize(pref.atomSet.numAtomsLocal());
            pdyna.mdw.resize(pref.atomSet.numAtomsLocal());
            pdyna.dv.resize(pref.atomSet.numAtomsLocal());

            /* loop over all atoms in the main ref group */
            for (size_t indexInSet = 0; indexInSet < pref.atomSet.numAtomsLocal(); indexInSet++)
            {
                int    atomIndex = pref.atomSet.localIndex()[indexInSet];
                rvec   dx;
        ...
        gmx::ArrayRef<double> buffer = gmx::arrayRefFromArray(comm->cylinderBuffer.data() + c*c_cylinderBufferStride, c_cylinderBufferStride);
        buffer[0] = wmass;
        buffer[1] = wwmass;
        buffer[2] = sum_a;
    ...
    if (cr != nullptr && PAR(cr))
    {
        ...
        pull_reduce_double(cr, comm, pull->coord.size()*c_cylinderBufferStride,
    ...
    for (size_t c = 0; c < pull->coord.size(); c++)
    {
        ...
        if (pcrd->params.eGeom == epullgCYL)
        {
            ...
            gmx::ArrayRef<const double> buffer      = gmx::constArrayRefFromArray(comm->cylinderBuffer.data() + c*c_cylinderBufferStride, c_cylinderBufferStride);
            ...
            for (int m = 0; m < DIM; m++)
            {
                ...
                double dist          = -spatialData.vec[m]*comm->cylinderBuffer[c*c_cylinderBufferStride + 2]*pdyna->mwscale;
                    ...
                    rvec x[], rvec *xp)
{
    ...
    GMX_ASSERT(comm->rvecBuffer.size() >= pull->group.size(), "rvecBuffer should be sufficiently large");
    GMX_ASSERT(comm->dvecBuffer.size() >= pull->group.size()*DIM, "dvecBuffer should be sufficiently large");

    if (pull->bRefAt && pull->bSetPBCatoms)
    {
        pull_set_pbcatoms(cr, pull, x, comm->rvecBuffer);
    ...
    for (size_t g = 0; g < pull->group.size(); g++)
    {
        ...
        if (pgrp->needToCalcCom)
        {
            if (pgrp->epgrppbc != epgrppbcCOS)
            {
                ...
                {
                    /* Set the pbc atom */
                    copy_rvec(comm->rvecBuffer[g], x_pbc);
                }
...
#pragma omp parallel for num_threads(pull->nthreads) schedule(static)
                ...
                copy_dvec(comSumsTotal.sum_wmx,  comm->dvecBuffer[g*3]);
                copy_dvec(comSumsTotal.sum_wmxp, comm->dvecBuffer[g*3 + 1]);
                comm->dvecBuffer[g*3 + 2][0] = comSumsTotal.sum_wm;
                comm->dvecBuffer[g*3 + 2][1] = comSumsTotal.sum_wwm;
                comm->dvecBuffer[g*3 + 2][2] = 0;
            }
            else
            {
                /* Cosine weighting geometry.
...
#pragma omp parallel for num_threads(pull->nthreads) schedule(static)
                ...
                /* Copy local sums to a buffer for global summing */
                gmx::ArrayRef < gmx::BasicVector < double>> buffer = gmx::arrayRefFromArray(comm->dvecBuffer.data() + g*DIM, DIM);
    ...
    pull_reduce_double(cr, comm, pull->group.size()*3*DIM, comm->dvecBuffer.data()[0]);
    ...
    for (size_t g = 0; g < pull->group.size(); g++)
    {
        ...
        if (pgrp->needToCalcCom)
        {
            ...
            gmx::ArrayRef < gmx::BasicVector < double>> dvecBuffer = gmx::arrayRefFromArray(comm->dvecBuffer.data() + g*DIM, DIM);
            ...
            if (pgrp->epgrppbc != epgrppbcCOS)
            {
                ...
                for (m = 0; m < DIM; m++)
                {
                    ...
                    if (pgrp->epgrppbc == epgrppbcREFAT)
                    {
                        pgrp->x[m]      += comm->rvecBuffer[g][m];
                        if (xp)
                        {
                            pgrp->xp[m] += comm->rvecBuffer[g][m];
                        }
                    }
                }
            }
            else
            {
                ...
                {
                    int ii = pgrp->atomSet.localIndex()[i];
                    pgrp->localWeights[i] = csw*cos(twopi_box*x[ii][pull->cosdim]) +
