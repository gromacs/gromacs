...
static void apply_forces(t_pull * pull, t_mdatoms * md, rvec *f)
{
    ...
    for (c = 0; c < pull->ncoord; c++)
    {
        ...
        if (pcrd->eGeom == epullgCYL)
        {
            /* We make a small error here. Since the weights of the atoms
             * can change as a function of the radial distance, there can
             * be a small radial force component. We ignore this effect,
             * because it is expensive to calculate and the effect is
             * undesirable. Note that this violates energy conservation
             * for cylinder pulling.
             */
                          ...
                          double dt, double t)
{
    ...
    for (g = 0; g < pull->ngroup; g++)
    {
        ...
        /* We have ensured in init_pull that there can be only one coordinate
         * with constraints and cylinder geometry.
         */
        if (pull->bCylinder && g == pull_group_cylinder)
        {
            pgrp = &pull->dyna[cylinder_coord];
        }
        else
        {
            pgrp = &pull->group[g];
               ...
               gmx_bool bOutFile, unsigned long Flags)
{
    ...
    for (c = 0; c < pull->ncoord; c++)
    {
        ...
        if (pull->coord[c].eGeom == epullgCYL)
        {
            pull->bCylinder = TRUE;
            if (pull->coord[c].group[0] != pull_group_cylinder)
            {
                gmx_incons("Cylinder group index mismatch");
            }
        }
    }
    ...
    for (g = 0; g < pull->ngroup; g++)
    {
        ...
        if (pgrp->nat > 0)
        {
            ...
            /* Set the indices */
            init_pull_group_index(fplog, cr, g, pgrp, pulldim, mtop, ir, lambda);
            if (pull->bCylinder && g == pull_group_cylinder && pgrp->invtm == 0)
            {
    ...
    if (pull->bCylinder)
    {
        int ncylcoord_constraint;

        snew(pull->dyna, pull->ncoord);

        ncylcoord_constraint = 0;
        for (c = 0; c < pull->ncoord; c++)
        {
            const t_pull_coord *pcrd;
