...
static void pull_print_coord_dr(FILE *out, const pull_coord_work_t *pcrd,
                                ...
                                gmx_bool bAngle)
{
    const double unitfactor = bAngle ? RAD2DEG : 1;

    fprintf(out, "\t%g", pcrd->value*unitfactor);

    if (bPrintRefValue)
    {
        fprintf(out, "\t%g", pcrd->value_ref*unitfactor);
    }
                           ...
                           gmx_bool bCoord, unsigned long Flags)
{
    ...
    else
    {
        ...
        if (bCoord)
        {
            sprintf(buf, "Position (nm%s)", pull->bAngle ? ", degree" : "");
            xvgr_header(fp, "Pull COM",  "Time (ps)", buf,
                        exvggtXNY, oenv);
        }
        else
        {
            sprintf(buf, "Force (kJ/mol/nm%s)", pull->bAngle ? ", kJ/mol/degree" : "");
...
static void low_get_pull_coord_dr(const struct pull_t *pull,
                                  ...
                                  dvec dr)
{
    const pull_group_work_t *pgrp0, *pgrp1;
    int                      m;
