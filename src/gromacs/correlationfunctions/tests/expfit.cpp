...
class ExpfitTest : public ::testing::Test
{
    ...
    protected:
        static ExpfitData          data_[expTestNrTypes];
        ...
        static void SetUpTestCase()
        {
            ...
            fileName[5] = test::TestFileManager::getInputFilePath("testERREST.xvg");
            for (int i = 0; i < expTestNrTypes; i++)
            {
                const char * name = fileName[i].c_str();
                int          nrColumns;
                data_[i].nrLines_   = read_xvg(name, &tempValues, &nrColumns);
                data_[i].dt_        = tempValues[0][1] - tempValues[0][0];
                data_[i].startTime_ = tempValues[0][0];
                data_[i].endTime_   = tempValues[0][data_[i].nrLines_-1];
                for (int j = 0; j  < data_[i].nrLines_; j++)
                {
                    data_[i].x_.push_back((real)tempValues[0][j]);
                    data_[i].y_.push_back((real)tempValues[1][j]);
                }
...
//static var
ExpfitData  ExpfitTest::data_[expTestNrTypes];
...
TEST_F (ExpfitTest, EffnERF) {
    ...
    test(effnERF, param, 1e-1, 4);
}

TEST_F (ExpfitTest, EffnERREST) {
    double  param[] = {1, 0.9, 100};
    test(effnERREST, param, 5e-3, 5);
}
