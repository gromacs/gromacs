...
class ExpfitTest : public ::testing::Test
{
    ...
    protected:
        ...
        static real              * values_[expTestNrTypes];
        static int                 nrColumns_;
        // Need raw pointer for passing this to C routines
        static real              * standardDev_;
        ...
        static void SetUpTestCase()
        {
            ...
            for (int i = 0; i < expTestNrTypes; i++)
            {
                ...
                if (i == 0)
                {
                    ...
                    standardDev_ = new real[nrLines_];
                ...
                values_[i] = new real[nrLines_];
                for (int j = 0; j  < nrLines_; j++)
                {
                    values_[i][j]   = (real)tempValues[1][j];
                }
        ...
        void test(int type, double result[], double tolerance, int testType)
        {
            ...
            do_lmfit(nrLines_, values_[testType], standardDev_, timeDeriv_,
            ...
            checker_.checkSequenceArray(nfitparm, result, testName.c_str());
        }
};
...
real      * ExpfitTest::values_[expTestNrTypes];
int         ExpfitTest::nrColumns_;
real      * ExpfitTest::standardDev_;
