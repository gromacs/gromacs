...
class Expfit : public ::testing::Test
{
    protected:
        ...
        static void SetUpTestCase()
        {
            ...
            fileName[0] = gmx::test::getReferenceDataPath() + "/testINVEXP.xvg";
            fileName[1] = gmx::test::getReferenceDataPath() + "/testPRES.xvg";
            fileName[2] = gmx::test::getReferenceDataPath() + "/testEXP.xvg";
        ...
        void test(int type, real result[])
        {
            ...
            do_lmfit(nrLines, values[testType], standardDiv, timeDirev, NULL, startTime, endTime, NULL,
                     false, type, result, 0);
        ...
        void testResult(real result[], int nrFitParam, gmx::test::TestReferenceChecker checker, std::string testName)
        {
            ...
            for (int i = 0; i < nrFitParam; i++)
            {
                checker.setDefaultTolerance(gmx::test::FloatingPointTolerance(
                                                    std::abs(result[i]) * percentBound, -1, false));
