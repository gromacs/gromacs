...
class ExpfitTest : public ::testing::Test
{
    ...
    protected:
        ...
        void test(int type, real result[])
        {
            ...
            do_lmfit(nrLines, values[testType], standardDiv, timeDirev, NULL,
                     startTime, endTime, NULL, false, type, result, 0);
            testResult(result, nfitparm, checker, "Result");
        }

        void testResult(real result[], int nrFitParam, test::TestReferenceChecker checker, std::string testName)
        {
            double percentBound = 0.01;

            for (int i = 0; i < nrFitParam; i++)
            {
                checker.setDefaultTolerance(test::FloatingPointTolerance(std::abs(result[i]) * percentBound, -1, false));
