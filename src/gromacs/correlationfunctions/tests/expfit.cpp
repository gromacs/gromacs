...
class ExpfitTest : public ::testing::Test
{
    ...
    protected:
        ...
        void test(int type, double result[])
        {
            ...
            {
                resultMax = std::max(resultMax, fabs(resultMax));
            }
            //init data and cheker might be a beter plays to init them
#ifdef GMX_DOUBLE
            checker.setDefaultTolerance(test::relativeRealTolerance(resultMax, 200000000000ULL));
#else
            checker.setDefaultTolerance(test::relativeRealTolerance(resultMax, 100000000000ULL));
#endif
        ...
        void testResult(double result[], int nrFitParam)
        {
