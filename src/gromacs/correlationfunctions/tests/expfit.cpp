...
class Expfit : public ::testing::Test
{
    protected:
        ...
        void testParam(real params[], int size, gmx::test::TestReferenceChecker checker)
        {
            for (int i = 0; i < size; i++)
            {
                checker.checkReal(params[i], ("Param"+ convertInt(i)).c_str());
            }
        }

        void testResult(real result[], int size, gmx::test::TestReferenceChecker checker)
        {
            ...
            for (int i = 0; i < size; i++)
            {
                ...
                checker.setDefaultTolerance(gmx::test::FloatingPointTolerance(
                                                    fabs(result[i]) * percentBound, 4, false));

                checker.checkReal(result[i], ("Result" + convertInt(i)).c_str());
            }
        }
