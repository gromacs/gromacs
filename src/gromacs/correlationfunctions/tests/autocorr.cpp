...
class AutocorrTest : public ::testing::Test
{
    protected:
        ...
        static void SetUpTestCase()
        {
            ...
            fileName    = test::TestFileManager::getInputFilePath("testCOS3.xvg");
        ...
        void test(unsigned long mode)
        {
            ...
            test::TestReferenceData    data(test::erefdataCreateMissing);
            test::TestReferenceChecker checker(data.rootChecker());
        ...
        void testResult(real result[], test::TestReferenceChecker checker, std::string testName)
        {
            real testResult = 0;
            for (int i = 0; i < nrFrames; i++)
            {
            ...
            checker.setDefaultTolerance(test::FloatingPointTolerance(std::abs(testResult) * percentBound, -1, false));
...
//static var
std::string AutocorrTest::fileName;
int         AutocorrTest::nrFrames;

real      * AutocorrTest::result;
real      * AutocorrTest::values3D;
int         AutocorrTest::nrColums;
real        AutocorrTest::startTime;
real        AutocorrTest::endTime;
real        AutocorrTest::dt;
