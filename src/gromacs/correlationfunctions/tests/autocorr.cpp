...
class AutocorrTest : public ::testing::Test
{
    protected:
        ...
        AutocorrTest( )
            : refData_(test::erefdataCompare), checker_(refData_.rootChecker())
        {
#ifdef GMX_DOUBLE
        ...
        void test(unsigned long mode)
        {
            ...
            int    nrRestart     = 1;
            int    dim           = getDim(mode);
            real * result        = new real[nrFrames_*dim];

            for (int m = 0; m < dim; m++)
            {
                for (int i = 0; i < nrFrames_; i++)
                {
                    result[i*dim+m] = data_->getValue(m, i);
                }
            }
            low_do_autocorr(0, 0, 0,   nrFrames_, 1,
                            get_acfnout(), &result, data_->getDt(), mode,
        ...
        void testResult(real result[])
        {
            ...
            checker_.checkSequenceArray(nrFrames_, result, "AutocorrelationFunction");
