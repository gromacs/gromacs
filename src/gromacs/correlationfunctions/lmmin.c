...
/***  Allocate work space.  ***/
    ...
    {
        S->outcome = 9;
        if (NULL != fvec) free(fvec);
        if (NULL != diag) free(diag);
        if (NULL != qtf)  free(qtf);
        if (NULL != fjac) free(fjac);
        if (NULL != wa1)  free(wa1);
        if (NULL != wa2)  free(wa2);
        if (NULL != wa3)  free(wa3);
        if (NULL != wf)   free(wf);
        if (NULL != ipvt) free(ipvt);
        return;
    }
