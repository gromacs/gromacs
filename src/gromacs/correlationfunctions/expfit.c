...
static double lmc_pres_6_parm(double x, const double *a)
{
    ...
    if ((a[4] != 0) && (a[0] != 0))
    {
        double power = min(fabs(a[5]), pow_max);
        double eee   = min(pow((x/fabs(a[4])), power), eee_max);
        term3        = a[0] * exp(-eee);
    }
    ...
    if ((term1 != 0) && (a[2] != 0))
    {
        double power = min(fabs(a[3]), pow_max);
        double eee   = min(pow((x/fabs(a[2])), power), eee_max);
        term2        = exp(-eee) * cos(x*fabs(a[1]));
    }
...
static double lmc_vac_2_parm(double x, const double *a)
{
    ...
    {
        omega = sqrt(fabs(det));
        wv    = min(omega*v, wv_max);

        if (det > 0)
        {
            ec = em*0.5*(exp(wv)+exp(-wv));
            es = em*0.5*(exp(wv)-exp(-wv))/omega;
        }
        else
        {
            ec = em*cos(wv);
