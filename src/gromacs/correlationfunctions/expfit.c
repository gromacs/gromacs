                          ...
                          int eFitFn)
{
    ...
    lm_control            = lm_control_double;
    lm_control.verbosity  = (bVerbose ? 1 : 0);
    lm_control.n_maxpri   = 0;
    lm_control.m_maxpri   = 0;
    ...
    do
    {
        ...
        lmcurve(effnNparams(eFitFn), parm, nfit, x, y, lmcurves[eFitFn],
                &lm_control, lm_status);
        chisq = sqr(lm_status->fnorm);
        printf("lm_status: fnorm = %g, nfev = %d, outcome = %s, userbreak = %d\n",
               lm_status->fnorm, lm_status->nfev, lm_infmsg[lm_status->outcome],
               lm_status->userbreak);
        if (bVerbose)
        {
            int mmm;
        ...
        j++;
        bCont = (fabs(ochisq - chisq) > fabs(lm_control.ftol*chisq));
    }
              ...
              const char *fn_fitted)
{
    ...
    if (0 != fix)
    {
        fprintf(stderr, "Using fixed parameters in curve fitting is temperarily not working.\n");
    }
    if (NULL == sig)
    {
