 ...
 * the successive time constants are at least 5 ps larger than the
    ...
    switch (eFitFn)
    {
        ...
        case effnEXPEXP:
            ...
            if (nparm > 2)
            {
                ...
                params[2] = std::max(fabs(params[2])-params[0], 5.0);
            }
            break;
        case effnEXP5:
        case effnEXP7:
        case effnEXP9:
            ...
            if (nparm > 3)
            {
                ...
                params[3] = std::max(fabs(params[3])-params[1], 5.0);
                if (nparm > 5)
                {
                    GMX_ASSERT(params[5] >= 0, "parameters should be >= 0");
                    params[5] = std::max(fabs(params[5])-params[3], 5.0);
                    if (nparm > 7)
                    {
                        GMX_ASSERT(params[7] >= 0, "parameters should be >= 0");
                        params[7] = std::max(fabs(params[7])-params[5], 5.0);
                    }
                }
            }
            break;
