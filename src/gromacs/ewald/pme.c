...
int gmx_pme_destroy(FILE *log, struct gmx_pme_t **pmedata)
{
    ...
    pme_free_all_work(&(*pmedata)->work, (*pmedata)->nthread);
                 ...
                 int                nthread)
{
    ...
    pme_init_all_work(&pme->work, pme->nthread, pme->nkx);
...
#pragma omp parallel for num_threads(pme->nthread) schedule(static)
        ...
        if (bCalcEnerVir)
        {
            ...
            if (grid_index < 2)
            {
                get_pme_ener_vir_q(pme->work, pme->nthread, &energy_AB[grid_index], vir_AB[grid_index]);
            }
            else
            {
                get_pme_ener_vir_lj(pme->work, pme->nthread, &energy_AB[grid_index], vir_AB[grid_index]);
            }
        }
        bFirst = FALSE;
    } /* of grid_index-loop */
...
#pragma omp parallel num_threads(pme->nthread) private(thread)
            ...
            if (bCalcEnerVir)
            {
                 ...
                 */
                get_pme_ener_vir_lj(pme->work, pme->nthread, &energy_AB[2+fep_state], vir_AB[2+fep_state]);
            }
