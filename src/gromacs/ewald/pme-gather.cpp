...
/* Spline function. Force compiler to instantiate for compile-time values of
   order and allow overloadng for specific compile-time values.
   The Int template argument can be either int (runtime) or
   ...
   value the compiler is required to instantiate the function seperately for
   each value. The function can be overloaded for specific compile-time values
   using integral_constant<int, N> where N is eiter a specifc value or an
   enable_if constrained non-type template parameter. The most specifc overload
   (specifc value > int template parameter > general function) is called. Inside
           ...
           const pme_atomcomm_t *atc, const splinedata_t *spline, int nn)
{
    ...
    for (int ithx = 0; (ithx < order); ithx++)
    {
        ...
        for (int ithy = 0; (ithy < order); ithy++)
        {
            ...
            {
                const real gval = grid[index_xy+(idxptr[ZZ]+ithz)];
                fxy1 += thz[ithz]*gval;
                fz1  += dthz[ithz]*gval;
            }
            f[XX] += dx*ty*fxy1;
           ...
           const pme_atomcomm_t *atc, const splinedata_t *spline, int nn)
{
    ...
    for (int ithx = 0; (ithx < 4); ithx++)
    {
        ...
        for (int ithy = 0; (ithy < 4); ithy++)
        {
            ...
            const Simd4Real gval_S = load4U(grid+index_xy+idxptr[ZZ]);
...
do_fspline(std::integral_constant<int, order>, const struct gmx_pme_t *pme, const real *grid,
           const pme_atomcomm_t *atc, const splinedata_t *spline, int nn)
{
    int              offset;
    ...
    Simd4Real tz_S0 = load4U(thz-offset);
    Simd4Real tz_S1 = load4U(thz-offset+4);
    Simd4Real dz_S0 = load4U(dthz-offset);
    Simd4Real dz_S1 = load4U(dthz-offset+4);
#else
    ...
    Simd4Real tz_S0 = load4(thz_aligned);
    Simd4Real tz_S1 = load4(thz_aligned+4);
    Simd4Real dz_S0 = load4(dthz_aligned);
    Simd4Real dz_S1 = load4(dthz_aligned+4);
#endif
    ...
    for (int ithx = 0; (ithx < order); ithx++)
    {
        ...
        for (int ithy = 0; (ithy < order); ithy++)
        {
            ...
            const Simd4Real gval_S0 = load4(grid+index_xy+idxptr[ZZ]-offset);
            const Simd4Real gval_S1 = load4(grid+index_xy+idxptr[ZZ]-offset+4);
