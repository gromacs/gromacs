...
do_fspline(INT order,
           ...
           const real *grid,
           const struct pme_spline_work *, real *, real *,
 ...
 */
template <int ORDER>
static inline typename std::enable_if<ORDER == 4 || ORDER == 5>::type
do_fspline(std::integral_constant<int, ORDER> order,
           int pny, int pnz, int i0, int j0, int k0,
           const real *thx, const real *thy, const real *thz,
           const real *dthx, const real *dthy, const real *dthz,
           const real *grid,
           const struct pme_spline_work *work,
                       ...
                       real scale)
{
...
#else
    real *thz_aligned = nullptr, *dthz_aligned = nullptr; //unused
#endif
    ...
    for (int nn = 0; nn < spline->n; nn++)
    {
        ...
        if (coefficient != 0)
        {
            ...
            {
                case 4:
                    do_fspline(std::integral_constant<int, 4>(), //TODO: When C++14 is allowed use generic lambda to avoid passing arguments 3x (and potentially replacing switch statement with a dispatch function)
                               pny, pnz, i0, j0, k0,
