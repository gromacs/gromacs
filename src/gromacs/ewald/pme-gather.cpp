...
static inline void
do_fspline(Int order, const struct gmx_pme_t *pme, const real * gmx_restrict grid,
           ...
           RVec * gmx_restrict f)
{
    ...
    const int * gmx_restrict idxptr = atc->idx[spline->ind[nn]];
    const int                norder = nn*order;

    /* Pointer arithmetic alert, next six statements */
    const real * gmx_restrict thx  = spline->theta[XX] + norder;
    const real * gmx_restrict thy  = spline->theta[YY] + norder;
    const real * gmx_restrict thz  = spline->theta[ZZ] + norder;
    const real * gmx_restrict dthx = spline->dtheta[XX] + norder;
    const real * gmx_restrict dthy = spline->dtheta[YY] + norder;
    const real * gmx_restrict dthz = spline->dtheta[ZZ] + norder;
 ...
 */
static inline void
do_fspline(std::integral_constant<int, 4>, const struct gmx_pme_t *pme, const real * gmx_restrict grid,
           const pme_atomcomm_t *atc, const splinedata_t *spline, int nn,
           RVec * gmx_restrict f)
{
    const int                pny   = pme->pmegrid_ny;
 ...
 */
template <int Order>
static inline typename std::enable_if<Order == 4 || Order == 5, void>::type
            ...
            switch (order)
            {
                case 4:
                    do_fspline(std::integral_constant<int, 4>(),
                               pme, grid, atc, spline, nn, &f);
                    break;
                case 5:
                    do_fspline(std::integral_constant<int, 5>(),
                               pme, grid, atc, spline, nn, &f);
                    break;
                default:
                    do_fspline(order,
