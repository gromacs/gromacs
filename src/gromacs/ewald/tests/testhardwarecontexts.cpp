...
#endif
    owner_->setGpuIndex(task_, index_);
}
...
void PmeTestEnvironment::hardwareInit()
{
    ...
    pickCompatibleGpus(&hardwareInfo_->gpu_info, gpuOptions_.get());
    // At this point compatible GPUs have been detected an assigned, time for some evil:
    ...
    gpuOptions_->gpuTasks.reset(new GpuTaskManager(&hardwareInfo_->gpu_info, gpuOptions_.get()));
}
...
void PmeTestEnvironment::SetUp()
{
    ...
    {
        char        stmp[200] = {};
        const int   gpuIndex = gpuOptions_->dev_use[i];
        get_gpu_device_info_string(stmp, &hardwareInfo_->gpu_info, gpuIndex);
        std::string description   = "(GPU " + std::string(stmp) + ") ";
        auto        gpuContext    = std::make_shared<GpuTestHardwareContext>(GpuTestHardwareContext(description, gpuIndex, gpuOptions_->gpuTasks.get(), GpuTask::PME));
