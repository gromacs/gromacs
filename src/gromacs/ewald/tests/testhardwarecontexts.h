...
class GpuTestHardwareContext : public ITestHardwareContext
{
    ...
    //! GPU index. TODO: might be better to use the ID
    const int         index_;
    //! GPU task manager which is used to activate the context
    GpuTaskManager   *owner_;
    //! GPU task type for this context
    GpuTask           task_;
    public:
        //! Constructs the context with the given GPU id
        GpuTestHardwareContext(const std::string &description, int index, GpuTaskManager *owner, GpuTask task) : description_(description), index_(index), owner_(owner), task_(task){}
