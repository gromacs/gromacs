        ...
        void runTest()
        {
            ...
            for (const auto &mode : modesToTest)
            {
                ...
                if (!supportedInput)
                {
                    ...
                    EXPECT_THROW(pmeInitAtoms(&inputRec, mode.first, coordinates, charges, box), NotImplementedError);
                }
                else
                {
                    ...
                    for (const auto &context : contextsToTest)
                    {
                        ...
                        for (const auto &option : optionsToTest)
                        {
                            ...
                            PmeSafePointer pmeSafe = pmeInitAtoms(&inputRec, mode.first, coordinates, charges, box);
