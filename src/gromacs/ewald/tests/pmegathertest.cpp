 ...
 * FIXME
 * There are more inputs to the force gathering, namely: gridline indices, spline theta values, spline dtheta values;
 * however, their size depends on the PME order and atom count, and they only have one sample input,
 * so they are fetched inside the test body instead to avoid multiple initializations.
 */
typedef std::tuple<Matrix3x3, int, IVec, SparseRealGridValuesInput, PmeGatherInputHandling,
                   GridLineIndicesVector, SplineParamsVector, SplineParamsVector, ChargesVector> GatherInputParameters;
    ...
    public:
        ...
        void runTest()
        {
            ...
            for (const auto &mode : modesToTest)
            {
                ...
                pmeSetGridLineIndices(pmeSafe.get(), mode.first, gridLineIndices);
                for (int dimIndex = 0; dimIndex < DIM; dimIndex++)
                {
                    pmeSetSplineData(pmeSafe.get(), mode.first, splineValues[dimIndex], PmeSplineDataType::Values, dimIndex);
                    pmeSetSplineData(pmeSafe.get(), mode.first, splineDerivatives[dimIndex], PmeSplineDataType::Derivatives, dimIndex);
                }

                std::vector<RVec> inputForcesFull {
                ...
                const auto           ulpTolerance = pmeOrder;
...
static std::vector<IVec> const sampleGridLineIndicesFull
{
    IVec {
        4, 2, 6
    },
...
//! 1 atom
static auto const sampleGridLineIndices1 = GridLineIndicesVector::fromVector(sampleGridLineIndicesFull.begin(), sampleGridLineIndicesFull.begin() + 1);
//! 2 atoms
static auto const sampleGridLineIndices2 = GridLineIndicesVector::fromVector(sampleGridLineIndicesFull.begin() + 1, sampleGridLineIndicesFull.begin() + 3);
//! 13 atoms
static auto const sampleGridLineIndices13 = GridLineIndicesVector::fromVector(sampleGridLineIndicesFull.begin() + 3, sampleGridLineIndicesFull.begin() + 16);
...
//! A helper for shaping the input sample real values into input spline data
static SplineParamsVector getSplineInput(const std::vector<real> &input, const int pmeOrder, size_t atomIndex, size_t atomCount)
{
    const size_t       dimSize = atomCount * pmeOrder;
    SplineParamsVector result;
    auto               it = input.begin() + DIM * pmeOrder * atomIndex;
    for (int dimIndex = 0; dimIndex < DIM; dimIndex++, it += dimSize)
    {
        result[dimIndex] = SplineParamsDimVector::fromVector(it, it + dimSize);
    }
    return result;
}
...
static std::vector<real> const sampleSplineValuesFull
{
...
//! 1 atom, order 3
static auto const sampleSplineValues3_1 = getSplineInput(sampleSplineValuesFull, 3, 0, 1);
//! 2 atoms, order 3
static auto const sampleSplineValues3_2 = getSplineInput(sampleSplineValuesFull, 3, 1, 2);
//! 13 atoms, order 3
static auto const sampleSplineValues3_13 = getSplineInput(sampleSplineValuesFull, 3, 3, 13);
//! 1 atom, order 4
static auto const sampleSplineValues4_1 = getSplineInput(sampleSplineValuesFull, 4, 0, 1);
//! 2 atoms, order 4
static auto const sampleSplineValues4_2 = getSplineInput(sampleSplineValuesFull, 4, 1, 2);
//! 13 atoms, order 4
static auto const sampleSplineValues4_13 = getSplineInput(sampleSplineValuesFull, 4, 3, 13);
//! 1 atom, order 5
static auto const sampleSplineValues5_1 = getSplineInput(sampleSplineValuesFull, 5, 0, 1);
//! 2 atoms, order 5
static auto const sampleSplineValues5_2 = getSplineInput(sampleSplineValuesFull, 5, 1, 2);
//! 13 atoms, order 5
static auto const sampleSplineValues5_13 = getSplineInput(sampleSplineValuesFull, 5, 3, 13);
...
static std::vector<real> const sampleSplineDerivativesFull
{
...
//! 1 atom, order 3
static auto const sampleSplineDerivatives3_1 = getSplineInput(sampleSplineDerivativesFull, 3, 0, 1);
//! 2 atoms, order 3
static auto const sampleSplineDerivatives3_2 = getSplineInput(sampleSplineDerivativesFull, 3, 1, 2);
//! 13 atoms, order 3
static auto const sampleSplineDerivatives3_13 = getSplineInput(sampleSplineDerivativesFull, 3, 3, 13);
//! 1 atom, order 4
static auto const sampleSplineDerivatives4_1 = getSplineInput(sampleSplineDerivativesFull, 4, 0, 1);
//! 2 atoms, order 4
static auto const sampleSplineDerivatives4_2 = getSplineInput(sampleSplineDerivativesFull, 4, 1, 2);
//! 13 atoms, order 4
static auto const sampleSplineDerivatives4_13 = getSplineInput(sampleSplineDerivativesFull, 4, 3, 13);
//! 1 atom, order 5
static auto const sampleSplineDerivatives5_1 = getSplineInput(sampleSplineDerivativesFull, 5, 0, 1);
//! 2 atoms, order 5
static auto const sampleSplineDerivatives5_2 = getSplineInput(sampleSplineDerivativesFull, 5, 1, 2);
//! 13 atoms, order 5
static auto const sampleSplineDerivatives5_13 = getSplineInput(sampleSplineDerivativesFull, 5, 3, 13);

//! 2 sample grids - only non-zero values have to be listed
static std::vector<SparseRealGridValuesInput> const sampleGrids
{
...
//! Moved out from instantiations for readability
auto instPmeOrder3 = ::testing::Values(3);
//! Moved out from instantiations for readability
...
// TODO there should be a way to condense these instantiations into one

//! Instantiation of the PME gathering test with 1 atom, order of 3
INSTANTIATE_TEST_CASE_P(SaneInput1_3, PmeGatherTest, ::testing::Combine(inputBoxes, instPmeOrder3, inputGridSizes, inputGrids, inputForceTreatment,
                                                                            ::testing::Values(sampleGridLineIndices1),
                                                                            ::testing::Values(sampleSplineValues3_1),
                                                                            ::testing::Values(sampleSplineDerivatives3_1),
                                                                            ::testing::Values(sampleCharges1)
                                                                        ));
//! Instantiation of the PME gathering test with 2 atoms, order of 3
INSTANTIATE_TEST_CASE_P(SaneInput2_3, PmeGatherTest, ::testing::Combine(inputBoxes, instPmeOrder3, inputGridSizes, inputGrids, inputForceTreatment,
                                                                            ::testing::Values(sampleGridLineIndices2),
                                                                            ::testing::Values(sampleSplineValues3_2),
                                                                            ::testing::Values(sampleSplineDerivatives3_2),
                                                                            ::testing::Values(sampleCharges2)
                                                                        ));
//! Instantiation of the PME gathering test with 13 atoms, order of 3
INSTANTIATE_TEST_CASE_P(SaneInput13_3, PmeGatherTest, ::testing::Combine(inputBoxes, instPmeOrder3, inputGridSizes, inputGrids, inputForceTreatment,
                                                                             ::testing::Values(sampleGridLineIndices13),
                                                                             ::testing::Values(sampleSplineValues3_13),
                                                                             ::testing::Values(sampleSplineDerivatives3_13),
