...
static std::vector<RVec> const sampleForcesFull {
    RVec {
        0.02f, 0.87f, 0.95f
    }, RVec {
        0.66f, 0.67f, 0.38f
...
class PmeGatherTest : public ::testing::TestWithParam<GatherInputParameters>
{
    private:
        ...
        static InputDataByAtomCount inputAtomDataSets_;
    ...
    public:
        ...
        {
            auto                gridLineIndicesIt = sampleGridLineIndicesFull.begin();
            auto                chargesIt         = sampleChargesFull.begin();
            for (auto atomCount : atomCounts)
            {
                ...
                for (auto pmeOrder : pmeOrders)
                {
                    ...
                    {
                        splineData.splineValues[dimIndex] = SplineParamsDimVector::fromVector(sampleSplineValuesFull.begin() + dimIndex * dimSize,
                                                                                              sampleSplineValuesFull.begin() + (dimIndex + 1) * dimSize);
                        splineData.splineDerivatives[dimIndex] = SplineParamsDimVector::fromVector(sampleSplineDerivativesFull.begin() + dimIndex * dimSize,
                                                                                                   sampleSplineDerivativesFull.begin() + (dimIndex + 1) * dimSize);
                    }
                    atomData.splineDataByPmeOrder[pmeOrder] = splineData;
                }
                inputAtomDataSets_[atomCount] = atomData;
            }
        }
        //! The test
        void runTest()
        {
            ...
            auto inputAtomData       = inputAtomDataSets_[atomCount];
            ...
            for (const auto &mode : modesToTest)
            {
                ...
                if (!supportedInput)
                {
                    ...
                    EXPECT_THROW(pmeInitWithAtoms(inputRec, mode.first, inputAtomData.coordinates, inputAtomData.charges, box), NotImplementedError);
                }
                else
                {
                    ...
                    for (const auto &context : contextsToTest)
                    {
                        ...
                        PmeSafePointer pmeSafe = pmeInitWithAtoms(inputRec, mode.first, inputAtomData.coordinates, inputAtomData.charges, box);
                        ...
                        auto inputForcesFull(sampleForcesFull);
...
InputDataByAtomCount PmeGatherTest::inputAtomDataSets_;
...
INSTANTIATE_TEST_CASE_P(SaneInput, PmeGatherTest, ::testing::Combine(::testing::ValuesIn(sampleBoxes),
                                                                         ::testing::ValuesIn(pmeOrders),
                                                                         ::testing::ValuesIn(sampleGridSizes),
                                                                         ::testing::ValuesIn(sampleGrids),
