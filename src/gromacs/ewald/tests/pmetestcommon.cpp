...
void PmeTestEnvironment::hardwareInit()  //(gmx_hw_info_t *&hardwareInfo, gmx_gpu_opt_t *&gpuOptions)
{
    ...
    hardwareInfo_.reset(gmx_detect_hardware(log, commrec, true));
    pick_compatible_gpus(&hardwareInfo_.get()->gpu_info, gpuOptions_.get());
    gmx_select_gpu_ids(log, commrec, &hardwareInfo_.get()->gpu_info, false, gpuOptions_.get());
    // TODO: how do we make sure which GPUs are being tested?
}

void PmeTestEnvironment::SetUp()
{
#if GMX_GPU == GMX_GPU_CUDA //TODO remove conditional?
    hardwareInit();
#endif
}
                               ...
                               bool computeSplines, bool spreadCharges)
{
    ...
    switch (mode)
    {
        ...
        case PmeCodePath::CUDA:
            ...
            // TODO clean up unused stuff such as atc?
            break;
    ...
    for (size_t atomIndex = 0; atomIndex < atomCount; atomIndex++)
    {
        ...
        for (size_t orderIndex = 0; orderIndex < pmeOrder; orderIndex++)
        {
            for (size_t dimIndex = 0; dimIndex < DIM; dimIndex++)
            {
                ...
                GMX_RELEASE_ASSERT(cpuValueIndex < atomCount * pmeOrder, "wtf");
