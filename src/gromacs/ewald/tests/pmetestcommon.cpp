...
#include "gromacs/fft/parallel_3dfft.h" //FIXME remove
                                ...
                                )
{
    ...
    switch (mode)
    {
        ...
        case CodePath::CUDA:
            ...
            //FIXME pme_gpu_start_step(pmeSafe->gpu, boxTemp, as_rvec_array(coordinates.data()));
            break;
...
void pmePerformSolve(const gmx_pme_t *pme, CodePath mode,
                     ...
                     PmeSolveOptions options)
{
    t_complex      *h_grid                   = pmeGetComplexGridInternal(pme);
    const bool      computeEnergyAndVirial   = true;
    const bool      useLorentzBerthelot      = false;
    const size_t    threadIndex              = 0;
    const bool      YZXGridOrder             = (options == PmeSolveOptions::YZXGridOrder);
    switch (mode)
    {
        case CodePath::CPU:
            if (options != PmeSolveOptions::YZXGridOrder)
            {
                GMX_THROW(InternalError("Test not implemented for this mode"));
            }
            switch (method)
        ...
        case CodePath::CUDA:
            ...
            {
                case PmeSolveAlgorithm::Normal:
                    pme_gpu_solve(pme->gpu, h_grid, computeEnergyAndVirial, YZXGridOrder);
                    break;
...
//! Getting plain index into the complex 3d grid //TODO: inline/ tparam
inline size_t pmeGetGridPlainIndexInternal(const IVec &index, const IVec &paddedGridSize, PmeSolveOptions gridOrdering)
{
    size_t result;
    switch (gridOrdering)
    {
        case PmeSolveOptions::YZXGridOrder:
            result = (index[YY] * paddedGridSize[ZZ] + index[ZZ]) * paddedGridSize[XX] + index[XX];
            break;

        case PmeSolveOptions::XYZGridOrder:
...
{
    pmeSetGridInternal<real>(pme, mode, PmeSolveOptions::XYZGridOrder, gridValues);
}
...
template<typename ValueType>
static SparseGridValuesOutput<ValueType> pmeGetGridInternal(const gmx_pme_t *pme, CodePath mode, PmeSolveOptions gridOrdering)
{
    IVec       gridSize, paddedGridSize;
    ValueType *grid;
    pmeGetGridAndSizesInternal<ValueType>(pme, grid, gridSize, paddedGridSize);
...
{
    return pmeGetGridInternal<real>(pme, mode, PmeSolveOptions::XYZGridOrder);
}
