...
template <typename ValueType>
static GridParameters<ValueType> getGridParameters(const gmx_pme_t *pme);
...
GridParameters<t_complex> getGridParameters<t_complex>(const gmx_pme_t *pme)
{
    ...
    gmx_parallel_3dfft_complex_sizes(pme->pfft_setup[gridIndex], p.gridSize, p.paddedGridSize); //TODO: what about YZX ordering?
    return p;
}
