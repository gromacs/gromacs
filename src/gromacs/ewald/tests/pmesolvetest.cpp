        ...
        void RunTest()
        {
            ...
            for (const auto &mode : modesToTest)
            {
                ...
                for (const auto &point : nonZeroGridValuesOutput) //sOuput
                {
                    ...
                    // additional safeguard for denormal numbers as they cause an exception in string conversion
                    if (fabs(point.second.re) >= GMX_REAL_MIN)
                    {
                        gridValuesChecker.checkReal(point.second.re, (valueId + " re").c_str());
                    }
                    if (fabs(point.second.im) >= GMX_REAL_MIN)
                    {
