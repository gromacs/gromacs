...
typedef std::tuple<Matrix3x3, IVec, SparseComplexGridValues, real, real, real, PmeSolveAlgorithm> SolveInputParameters;
...
class PmeSolveTest : public ::testing::TestWithParam<SolveInputParameters>
{
    ...
    public:
        ...
        void RunTest()
        {
            ...
            Matrix3x3                 box;
            IVec                      gridSize;
            SparseComplexGridValues   nonZeroGridValues;
            real                      epsilon_r;
            real                      ewaldCoeff_q;
            real                      ewaldCoeff_lj;
            ...
            for (const auto &mode : modesToTest)
            {
                ...
                PmePerformSolve(pmeSafe, mode.first, method, cellVolume);
                ...
                SparseComplexGridValues nonZeroGridValuesOutput;
                real                    energy;
                Matrix3x3               virial;
                PmeFetchOutputsSolve(pmeSafe, mode.first, method, nonZeroGridValuesOutput, energy, virial);

                /* Transformed grid */
                TestReferenceChecker gridValuesChecker(checker.checkCompound("NonZeroGridValues", "ComplexSpaceGrid"));
                const auto           ulpToleranceGrid = 4;        // 48;
                gridValuesChecker.setDefaultTolerance(relativeToleranceAsUlp(1.0, ulpToleranceGrid));
                for (const auto &point : nonZeroGridValuesOutput) //sOuput
                {
                    std::string valueId = formatString("Cell %d %d %d", point.first[XX], point.first[YY], point.first[ZZ]);
                    // we want and additional safeguard for denormal numbers as they cause an exception in string conversion
                    // but then using GMX_REAL_MIN causes an "unused item warning" for single precision builds
                    if (fabs(point.second.re) >= GMX_FLOAT_MIN)
                    {
                        gridValuesChecker.checkReal(point.second.re, (valueId + " re").c_str());
                    }
                    if (fabs(point.second.im) >= GMX_FLOAT_MIN)
                    {
                        gridValuesChecker.checkReal(point.second.im, (valueId + " im").c_str());
                    }
                }
                ...
                const auto           ulpToleranceVirial = 100;
                virialChecker.setDefaultTolerance(relativeToleranceAsUlp(1.0, ulpToleranceVirial));
                for (int i = 0; i < DIM; i++)
                {
                    for (int j = 0; j <= i; j++)
                    {
...
static std::vector<SparseComplexGridValues> const sampleGrids
{
    SparseComplexGridValues {{
                                 IVec {
                                     0, 0, 0
                                 }, t_complex {
                                     3.5f, 6.7f
