                         ...
                         int nx, int ny, int nz, int pme_order)
{
    ...
    for (int k = 2; k < splineOrder; k++)
    {
        ...
        for (int l = 1; l < k; l++)
        {
            data[k - l] = div*(l*data[k - l - 1] + (k - l + 1)*data[k - l]);
        }
        data[0] = div*data[0];
    }
    double div      = 1.0/splineOrder;
    for (int l = 0; l <  splineOrder; l++)
    {
        data[splineOrder - l] = div*(l*data[splineOrder - l - 1] + (splineOrder - l + 1)*data[splineOrder - l]);
    }
    data[0] = div*data[0];
