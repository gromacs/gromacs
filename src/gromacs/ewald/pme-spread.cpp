...
static void transposeWrapper(T gmx_unused *a)
{
    if (packSize > 1)
    {
        ...
        {
            GMX_ASSERT(packSize == 4, "transpose() is only implemented for 4 elements");
        }
    }
}
...
static void makeBsplines(splinevec theta, splinevec dtheta,
                         ...
                         const real *coefficient, gmx_bool bDoSplines)
{
    if (nr & (packSize - 1))
    {
    ...
    for (int i = 0; i < nr; i += packSize)
    {
         ...
         */
        if (packSize == 1 && coefficient[ind[i]] == 0 && !bDoSplines)
        {
            continue;
        }
