 ...
 * \tparam[in] gridOrdering             Specifies the dimenion ordering of the complex grid.
                   ...
                   bool computeEnergyAndVirial, GridOrdering gridOrdering)
{
    ...
    if (gridOrdering == GridOrdering::YZX)
    {
        if (computeEnergyAndVirial)
        {
            pme_solve_kernel<true, GridOrdering::YZX> <<< blocks, threads, 0, stream>>> (*kernelParamsPtr);
        }
        else
        {
            pme_solve_kernel<false, GridOrdering::YZX> <<< blocks, threads, 0, stream>>> (*kernelParamsPtr);
        }
    }
    else if (gridOrdering == GridOrdering::XYZ)
    {
        if (computeEnergyAndVirial)
        {
            pme_solve_kernel<true, GridOrdering::XYZ> <<< blocks, threads, 0, stream>>> (*kernelParamsPtr);
        }
        else
        {
            pme_solve_kernel<false, GridOrdering::XYZ> <<< blocks, threads, 0, stream>>> (*kernelParamsPtr);
        }
    }
