...
__global__ void pme_solve_kernel(const struct pme_gpu_cuda_kernel_params_t kernelParams)
{
    ...
    if ((indexMajor < localCountMajor) && (indexMiddle < localCountMiddle) && (indexMinor < localCountMinor))
    {
        ...
        const bool      notZeroPoint = (gridIndex != 0);
        //TODO check if this is better than (kMinor > 0 || kMajor > 0 || kMiddle > 0);
        float           mMinor       = kMinor;
    ...
    if (computeEnergyAndVirial)
    {
        ...
        __shared__ float sm_virialAndEnergy[PME_GPU_VIRIAL_AND_ENERGY_COUNT * blockSize];
