    ...
    )
{
    ...
    if ((indexMajor < localCountMajor) && (indexMiddle < localCountMiddle) && (indexMinor < localCountMinor))
    {
        ...
        auto        p0_back = globalGridPtr; //TODO
        // YZX
        auto        ix         = indexMinor, iy = indexMajor, iz = indexMiddle;
        const bool  debugPrint = (ix == 4) && (iy == 2) && (iz == 7);
        //const bool debugPrint = (ix == 1) && (iy == 1) && (iz == 1);
        if (debugPrint)
        {
            printf("grid is %p\n", globalGrid);
            printf("%s grid value %s %g %g [%d %d %d]\n", "GPU", "before", p0_back->x, p0_back->y, ix, iy, iz);
        }
        ...
        if (debugPrint)
        {
            printf("%s grid value %s %g %g [%d %d %d]\n", "GPU", "after", p0_back->x, p0_back->y, ix, iy, iz);
        }
    }
                   ...
                   bool computeEnergyAndVirial, bool yzxGridOrder)
{
    ...
    //    const bool   yzxGridOrder = true; //copyInputAndOutputHostRenameMePlease;//!pme_gpu_performs_FFT(pmeGpu); // FIXME gmx_bool
    ...
    printf("hello GPU size ! %d %d %d\n", local_size[XX], local_size[YY], local_size[ZZ]);
    printf("hello GPU data ! %d %d %d\n", local_ndata[XX], local_ndata[YY], local_ndata[ZZ]);

    float2     *d_grid = (float2 *)kernelParamsPtr->grid.d_fourierGrid;
    if (copyInputAndOutputHostRenameMePlease)
    {
        cu_copy_H2D_async(d_grid, h_grid, gridSize, stream);
    }
    ...
    if (yzxGridOrder)
    {
        if (computeEnergyAndVirial)
        {
            ...
            (local_ndata[majorDim], local_ndata[middleDim], local_ndata[minorDim],
             ...
             local_size[minorDim],                           local_size[middleDim],
             *kernelParamsPtr);
        }
        else
        {
    ...
    if (copyInputAndOutputHostRenameMePlease)
    {
        cu_copy_D2H_async(h_grid, d_grid, gridSize, stream);
