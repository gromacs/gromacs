                   ...
                   GridOrdering gridOrdering, bool computeEnergyAndVirial)
{
    ...
    if (gridOrdering == GridOrdering::YZX)
    {
        kernelPtr = computeEnergyAndVirial ?
            pmeGpu->contextHandle_->impl_->solveYZXEnergyKernel :
            pmeGpu->contextHandle_->impl_->solveYZXKernel;
    }
    else if (gridOrdering == GridOrdering::XYZ)
    {
        kernelPtr = computeEnergyAndVirial ?
            pmeGpu->contextHandle_->impl_->solveXYZEnergyKernel :
            pmeGpu->contextHandle_->impl_->solveXYZKernel;
    }
