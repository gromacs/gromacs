...
__launch_bounds__(sc_gatherMaxThreadsPerBlock<parallelExecutionWidth>,
        ...
        void pmeGatherKernel(const PmeGpuKernelParams kernelParams)
{
    ...
    else
    {
        ...
        if constexpr (c_useAtomDataPrefetch)
        {
            ...
            pme_gpu_stage_atom_data<float, atomsPerBlock, 1, false>(sm_coefficients, gm_coefficientsA);

            /* Staging coordinates */
            pme_gpu_stage_atom_data<float3, atomsPerBlock, 1, false>(sm_coordinates, gm_coordinates);
            __syncthreads();
    ...
    if constexpr (numGrids == 2)
    {
        ...
        // Reduction of partial force contributions
        reduceAtomForces<order, atomDataSize, blockSize, parallelExecutionWidth>(
                sm_forces_numGrids2, atomIndexLocal, splineIndex, kernelParams.grid.realGridSizeFP, fx, fy, fz);
        __syncthreads();
