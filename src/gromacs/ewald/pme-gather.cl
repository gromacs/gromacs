...
template <
    ...
    >
#endif
inline void reduce_atom_forces(__local float3 * __restrict__ sm_forces,
...
#pragma unroll
        ...
        {
            *((__local float *)(&sm_forces[atomIndex]) + dimIndex) = (sm_forceTemp[dimIndex][sourceIndex] + sm_forceTemp[dimIndex][sourceIndex + 1]) * n;
        }
    }
}
...
#pragma unroll
    ...
    __local float3 sm_forces[atomsPerBlock]; //FIXME float3?
...
#pragma unroll
        for (int i = 0; i < numIter; i++)
        {
            ...
            {
                const float outputForceComponent = ((__local float *)sm_forces)[outputIndexLocal * 4 / 3]; //FIXME
                if (overwriteForces)
                {
