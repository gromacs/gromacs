 ...
 * For details, please see how pme-program.cl is compiled in pme-gpu-program-impl.cpp.
 *
 ...
 * \param[out]    gm_virialAndEnergy   Reduced virail and enrgy (only with computeEnergyAndVirial == true)
    ...
    #pragma unroll
                for (int reductionStride = warp_size >> 1; reductionStride >= 1; reductionStride >>= 1)
                {
                    if (lane < reductionStride)
                    {
                        ...
                        /* This added intra-"warp" barrier makes execution on Intel OpenCL correct.
                         * Should probably be conditional on minimal SIMD width.
                         * #2519
                         */
                        sharedMemoryBarrier();
                    }
