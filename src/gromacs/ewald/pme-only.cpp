            ...
            for (const auto &sender : pme_pp->ppRanks)
            {
                if (sender.numAtoms > 0)
                {
                    ...
                    if (useGpuForPme && !*bNS_ptr)
                    {
                        ...
                        //send buffer size to PP rank
                        int msize = sender.numAtoms*3;
                        MPI_Isend(&msize, 1, MPI_INT, sender.rankId, 0,
                                  pme_pp->mpi_comm_mysim,
                                  &pme_pp->req[messages++]);

                        //recieve data from PP rank
                        MPI_Irecv(&x_d[nat*3], msize, MPI_FLOAT,
                                        ...
                                        bool useGpuForPme, bool bNS)
{
    ...
    for (const auto &receiver : pme_pp->ppRanks)
    {
        ...
        if (useGpuForPme && !bNS)
        {
            ...
            //send buffer size to PP rank
            int msize = (ind_end-ind_start)*3;
            MPI_Isend(&msize, 1, MPI_INT, receiver.rankId, 0,
                      pme_pp->mpi_comm_mysim, &pme_pp->req[messages++]);

            //Send data to PP rank
            MPI_Isend(&f_d[ind_start*3], msize, MPI_FLOAT,
                      receiver.rankId, 0,
                      pme_pp->mpi_comm_mysim, &pme_pp->req[messages++]);
