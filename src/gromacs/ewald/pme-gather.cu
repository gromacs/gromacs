...
//! CC 2.x CUDA limitation
#define GMX_CUDA_MIN_BLOCKS_PER_MP_CC2X 8
//! Gathering min blocks per CUDA multiprocessor - for CC2.x, we just take the CUDA limit of 8 to avoid the warning
constexpr int c_gatherMinBlocksPerMP = (GMX_PTX_ARCH < 300) ? GMX_CUDA_MIN_BLOCKS_PER_MP_CC2X : (GMX_CUDA_MAX_THREADS_PER_MP / c_gatherMaxThreadsPerBlock);
