                                                       ...
                                                       float         &fz)
{
...
#endif
    {
        ...
        if (splineIndex < DIM)
        {
            const float n = read_grid_size(realGridSizeFP, splineIndex);
            float       f = 0.0f;
            for (int j = atomIndexLocal * particleDataSize; j < (atomIndexLocal + 1) * particleDataSize; j++)
            {
 ...
 * \tparam[in] atomsPerBlock        The number of particles processed by a single block; FIXME
                    ...
                    ) //TODO: separate in and out?
{
    ...
    if (pme_gpu_is_testing(pmeGpu))
    {
        ...
        // could clear only the padding and not the whole thing, but whatever
    ...
    const int atomsPerBlock          =  PME_SPREADGATHER_ATOMS_PER_BLOCK; //FIXME
