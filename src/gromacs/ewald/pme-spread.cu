...
/*! \brief
 * Loads atom data from global to shared memory.
 *
 * \tparam[in] T                 Data type (float/int/...)
 * \tparam[in] atomsPerBlock     Number of atoms processed by a block - should be account for in the size of the shared memory array.
 ...
 */
template<typename T,
         const int atomsPerBlock, const int dataCountPerAtom>
...
{
    // FIXME: with padding disabled, this would ignore spline data alignment
...
/*! \brief
 * PME GPU spline calculation. This corresponds to the CPU functions calc_interpolation_idx and make_bsplines.
 *
...
__device__ __forceinline__ void calculate_splines(const pme_gpu_cuda_kernel_params_t     kernelParams,
                                                  ...
                                                  const int                              globalAtomIndex, // TODO pass a boolean?
                                                  const int                              localAtomIndex,
                                                  const int                              globalIndexBase,
                                                  const int                              dimIndex,
                                                  const int                              orderIndex)
{
    constexpr int        atomsPerBlock     = PME_SPREADGATHER_ATOMS_PER_BLOCK; //FIXME
    ...
    const int        sharedMemoryIndex = localAtomIndex * DIM + dimIndex;
    ...
    const int globalCheck = pme_gpu_check_atom_data_index(globalAtomIndex, kernelParams.atoms.nAtoms);
    ...
    if (localCheck && globalCheck)
    {
        ...
        if (orderIndex == 0)
        {
            int           tableIndex, tInt;
            float         n, t;
            const float3  x = sm_coordinates[localAtomIndex];
        ...
        const int chargeCheck = pme_gpu_check_atom_charge(sm_coefficients[localAtomIndex]);
        if (chargeCheck)
        {
            float       div;
            int         k;
...
#pragma unroll
                for (int l = 1; l < (k - 1); l++)
                {
                    data[(k - l - 1) * dataSize + dataOffset] = div * ((dr + l) * data[(k - l - 2) * dataSize + dataOffset] +
                                                                       (k - l - dr) * data[(k - l - 1) * dataSize + dataOffset]);
                }
                data[0 * dataSize + dataOffset] = div * (1.0f - dr) * data[0 * dataSize + dataOffset];
            }

            const int particleWarpIndex = localAtomIndex % PME_SPREADGATHER_ATOMS_PER_WARP;
            const int warpIndex         = localAtomIndex / PME_SPREADGATHER_ATOMS_PER_WARP;
...
#pragma unroll
            ...
            {
                data[(order - l - 1) * dataSize + dataOffset] = div * ((dr + l) * data[(order - l - 2) * dataSize + dataOffset] +
                                                                       (order - l - dr) * data[(order - l - 1) * dataSize + dataOffset]);
            }
            data[0 * dataSize + dataOffset] = div * (1.0f - dr) * data[0 * dataSize + dataOffset];
                                               ...
                                               const float * __restrict__             sm_theta)
{
    ...
    if (chargeCheck & globalCheck)
    {
        ...
        int          iy        = sm_gridlineIndices[localIndex * DIM + YY] - offy + ithy;
        if (wrapY & (iy >= ny))
        {
            iy -= ny;
        }
        int    iz        = sm_gridlineIndices[localIndex * DIM + ZZ] - offz + ithz;
        if (iz >= nz)
        {
            iz -= nz;
        }
...
#pragma unroll
        for (int ithx = 0; (ithx < order); ithx++)
        {
            ...
            const int globalIndex = ix * pny * pnz + constOffset;
...
__global__ void pme_spline_and_spread_kernel(const pme_gpu_cuda_kernel_params_t kernelParams)
{
    ...
    if (computeSplines)
    {
        ...
        __syncthreads();
        calculate_splines<order>(kernelParams,
                                 (const float3 *)sm_coordinates,
                                 sm_coefficients,
                                 sm_theta,
                                 sm_gridlineIndices,
                                 globalCalcIndex,
                    ...
                    bool             spreadCharges)
{
    ...
    switch (order)
    {
        case 4:
        {
            pme_gpu_start_timing(pmeGpu, gtPME_SPLINEANDSPREAD); // TODO: proper ID
            if (computeSplines)
            {
                if (spreadCharges)
                {
                    pme_spline_and_spread_kernel<4, true, true, wrapX, wrapY> <<< nBlocksSpread, dimBlockSpread, 0, stream>>> (*kernelParamsPtr);
                }
                else
                {
                    pme_spline_and_spread_kernel<4, true, false, wrapX, wrapY> <<< nBlocksSpread, dimBlockSpread, 0, stream>>> (*kernelParamsPtr);
                }
            }
            else
            {
                pme_spline_and_spread_kernel<4, false, true, wrapX, wrapY> <<< nBlocksSpread, dimBlockSpread, 0, stream>>> (*kernelParamsPtr);
            }
