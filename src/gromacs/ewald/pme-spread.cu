...
struct texture<int, 1, cudaReadModeElementType> &pme_gpu_get_gridline_texref()
{
    return gridlineIndicesTableTextureRef;
}

/*! Returns the reference to the fractShifts texture. */
struct texture<float, 1, cudaReadModeElementType> &pme_gpu_get_fract_shifts_texref()
{
