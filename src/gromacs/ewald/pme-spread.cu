...
//! Spreading max block size
constexpr int c_spreadMaxThreadsPerBlock = 8 * warp_size;
...
#define SPLINE_DATA(i) (*SPLINE_DATA_PTR(i))
    ...
    if (localCheck && globalCheck)
    {
        ...
        if (orderIndex == 0)
        {
            ...
            sm_fractCoords[sharedMemoryIndex]    += LDG<float>(gm_fractShiftsTable + tableIndex);
            sm_gridlineIndices[sharedMemoryIndex] = LDG<int>(gm_gridlineIndicesTable + tableIndex);
#endif
                    ...
                    bool             spreadCharges)
{
    ...
    const int atomsPerBlock = c_spreadMaxThreadsPerBlock / PME_SPREADGATHER_THREADS_PER_ATOM;
    // TODO: pick smaller block size in runtime if needed (e.g. on 660 Ti where 50% occupancy is ~25% faster than 100% occupancy with RNAse (~50k atoms))
    // If doing so, change atomsPerBlock in the kernels as well (inline function?).
    // TODO: also consider using cudaFuncSetCacheConfig() for preferring shared memory on older architectures (for spline data mostly)
    ...
    // The code below mostly does copying back for unit tests. TODO: move it somewhere?
