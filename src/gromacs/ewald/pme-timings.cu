...
float pme_gpu_timing::getTotalTimeMilliseconds() const
{
    return totalMilliseconds_;
}

unsigned int pme_gpu_timing::getCallCount() const
{
    return callCount_;
}
...
void pme_gpu_init_timings(const pme_gpu_t *pmeGPU)
{
    if (pme_gpu_timings_enabled(pmeGPU))
    {
        ...
        pmeGPU->archSpecific->timingEvents.resize(gtPME_EVENT_COUNT);
    }
}
