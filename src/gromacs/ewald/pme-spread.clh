                                    ...
                                    const int                                   dataCountPerAtom)
{
    ...
    const size_t globalIndexBase  = getBlockIndex(XX) * atomsPerBlock * dataCountPerAtom;
                           ...
                           __global float * __restrict__       gm_grid)
{
    ...
    if (chargeCheck & globalCheck)
    {
        ...
        __local const float *sm_thetaX         = sm_theta + (thetaOffsetBase + XX * dimStride);
...
#pragma unroll
        for (int ithx = 0; (ithx < order); ithx++)
        {
            ...
            atomicAdd_g_f(gm_grid + gridIndexGlobal, sm_thetaX[ithx * orderStride] * constVal);
        }
    }
}
