...
#include "pme.cuh" //TODO this inclusion is only for PmeGpuCudaKernelParams - which is small but might warrant having pme-gpu-types-impl.h?
#endif
...
int pme_gpu_get_atoms_per_warp(const PmeGpu *pmeGpu)
{
    const int order = pmeGpu->common->pme_order;
    GMX_ASSERT(order > 0, "Invalid PME order");
    return pmeGpu->contextHandle_->impl_->warpSize / PME_SPREADGATHER_THREADS_PER_ATOM;
}
...
void pme_gpu_realloc_and_copy_fract_shifts(PmeGpu *pmeGpu)
{
    ...
    allocateDeviceBuffer(&kernelParamsPtr->grid.d_fractShiftsTable, newFractShiftsSize, pmeGpu->contextHandle_->impl_->context);
    allocateDeviceBuffer(&kernelParamsPtr->grid.d_gridlineIndicesTable, newFractShiftsSize, pmeGpu->contextHandle_->impl_->context);
...
void pme_gpu_init_internal(PmeGpu *pmeGpu)
{
    ...
    cl_device_id                device_id       =  pmeGpu->contextHandle_->getDeviceInfo()->ocl_gpu_id.ocl_device_id;
    cl_int                      clError;
    pmeGpu->archSpecific->pmeStream = clCreateCommandQueue(pmeGpu->contextHandle_->impl_->context,
...
void pme_gpu_destroy_specific(const PmeGpu *pmeGpu)
{
    ...
    if (clError != CL_SUCCESS)
    {
        gmx_warning("Failed to destory PME command queue");
    }
#endif
}
                    ...
                    bool             spreadCharges)
{
    ...
    const size_t threadsPerBlockKernelLimit = c_spreadMaxWarpsPerBlock * pmeGpu->contextHandle_->impl_->warpSize;
#if GMX_GPU == GMX_GPU_OPENCL
    const int    threadsPerBlockFinal = std::min(threadsPerBlockKernelLimit, pmeGpu->contextHandle_->getDeviceInfo()->maxWorkGroupSize);
#else
    const int    threadsPerBlockFinal = threadsPerBlockKernelLimit;
#endif
                   ...
                   GridOrdering gridOrdering, bool computeEnergyAndVirial)
{
    ...
    const size_t threadsPerBlockKernelLimit = c_solveMaxWarpsPerBlock * pmeGpu->contextHandle_->impl_->warpSize;
#if GMX_GPU == GMX_GPU_OPENCL
    const int    threadsPerBlockFinal = std::min(threadsPerBlockKernelLimit, pmeGpu->contextHandle_->getDeviceInfo()->maxWorkGroupSize);
#else
    const int    threadsPerBlockFinal = threadsPerBlockKernelLimit;
    ...
    const int    gridLinesPerBlock  = std::max(threadsPerBlockFinal / gridLineSize, 1);
    ...
    const size_t warpSize           = pmeGpu->contextHandle_->impl_->warpSize;
                    ...
                    )
{
    ...
    const size_t threadsPerBlockKernelLimit = c_gatherMaxWarpsPerBlock * pmeGpu->contextHandle_->impl_->warpSize;
#if GMX_GPU == GMX_GPU_OPENCL
    const int    threadsPerBlockFinal = std::min(threadsPerBlockKernelLimit, pmeGpu->contextHandle_->getDeviceInfo()->maxWorkGroupSize);
#else
    const int    threadsPerBlockFinal = threadsPerBlockKernelLimit;
