                      ...
                      gmx_bool                  *bPrinting)
{
    ...
    if (!wallcycle_have_counter())
    {
        ...
        md_print_warn(cr, fp_log, "NOTE: Cycle counters on ARMv7a not enabled in linux kernel, cannot use PME-PP load balancing\n");
#else
        md_print_warn(cr, fp_log, "NOTE: Cycle counting is not supported on this architecture, cannot use PME-PP load balancing\n");
#endif
    }
