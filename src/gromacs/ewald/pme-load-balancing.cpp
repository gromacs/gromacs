...
static void switch_to_stage1(pme_load_balancing_t *pme_lb)
{
    ...
    /* While increasing start we might have skipped setups that we never timed,
     * include the setups down to the setup that we actually timed, so we will
     * time those during stage 1.
     */
