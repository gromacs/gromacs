                               ...
                               real              *ewaldcoeff_lj)
{
    ...
    *step      = -1;
    ...
    do
    {
        ...
        if (cnb.flags & PP_PME_COORD)
        {
            ...
            *step           = cnb.step;
            ...
            if (pme->bFEP_q && !(flags & PP_PME_CHARGEB))
            {
                gmx_incons("PME-only rank should do free-energy calculations, "
                           "but did not receive B-state charges");
            }

            if (pme->bFEP_lj && !(flags & PP_PME_SQRTC6B))
            {
                gmx_incons("PME-only rank should do free-energy calculations, "
                           "but did not receive B-state C6-values");
            }
    ...
    while (!(flags & (PP_PME_COORD | PP_PME_FINISH)));

    status = ((flags & PP_PME_FINISH) ? pmerecvqxFINISH : pmerecvqxX);
#else
