...
void pme_gpu_init_specific(pme_gpu_t *pmeGPU)
{
    ...
    // This should correspond to the values of PME_USE_TEXTURES and PME_USE_TEXOBJ, as those duplicate the logic for the device code.
    pmeGPU->archSpecific->tableHandling = PME_USE_TEXTURES ?
        ((pmeGPU->deviceInfo->prop.major >= 3) ? GpuTableHandling::TextureObjects : GpuTableHandling::TextureReferences) :
        GpuTableHandling::NoTextures;
...
void pme_gpu_reinit_3dfft(const pme_gpu_t *pmeGPU)
{
    ...
    {
        pmeGPU->archSpecific->fftSetup.resize(0); // this erases and reallocates all the grids, is this necessary?
