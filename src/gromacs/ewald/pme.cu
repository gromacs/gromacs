...
{
    cudaError_t stat = cudaStreamWaitEvent(pmeGPU->archSpecific->pmeStream, pmeGPU->archSpecific->syncSpreadAtomDataD2H, 0);
...
void pme_gpu_init_sync_events(const pme_gpu_t *pmeGPU)
{
    ...
    stat = cudaEventCreateWithFlags(&pmeGPU->archSpecific->syncSpreadAtomDataD2H, cudaEventDisableTiming);
    CU_RET_ERR(stat, "cudaEventCreate on syncSpreadAtomDataD2H failed");
...
void pme_gpu_destroy_sync_events(const pme_gpu_t *pmeGPU)
{
    ...
    stat = cudaEventDestroy(pmeGPU->archSpecific->syncSpreadAtomDataD2H);
    CU_RET_ERR(stat, "cudaEventDestroy failed on syncSpreadAtomDataD2H");
