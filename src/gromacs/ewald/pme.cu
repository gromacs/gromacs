...
void pme_gpu_realloc_forces(const PmeGpu *pmeGPU)
{
    ...
    pmeGpu->staging->h_forces.resize(pmeGPU->nAtoms);
}
...
void pme_gpu_copy_input_forces(const PmeGpu *pmeGPU)
{
    ...
    cu_copy_H2D_async(pmeGPU->kernelParams->atoms.d_forces, const_cast<float *>(pmeGpu->h_forces.data()), forcesSize, pmeGPU->archSpecific->pmeStream);
}
