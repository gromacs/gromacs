...
void pme_gpu_copy_input_forces(PmeGpu *pmeGPU)
{
    ...
    cu_copy_H2D_async(pmeGPU->kernelParams->atoms.d_forces, reinterpret_cast<float *>(pmeGPU->staging.h_forces.data()), forcesSize, pmeGPU->archSpecific->pmeStream);
}
...
void pme_gpu_copy_output_forces(PmeGpu *pmeGPU)
{
    const size_t forcesSize   = DIM * pmeGPU->kernelParams->atoms.nAtoms * sizeof(float);
    GMX_ASSERT(forcesSize > 0, "Bad number of atoms in PME GPU");
    cu_copy_D2H_async(pmeGPU->staging.h_forces.data(), pmeGPU->kernelParams->atoms.d_forces, forcesSize, pmeGPU->archSpecific->pmeStream);
}
