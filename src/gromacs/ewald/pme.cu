...
void pme_gpu_realloc_and_copy_input_coefficients(const pme_gpu_t *pmeGPU, const float *h_coefficients)
{
    ...
    pme_gpu_make_sure_memory_is_pinned((void **)&h_coefficients, newCoefficientsSize);
...
void pme_gpu_init_specific(pme_gpu_t *pmeGPU)
{
    ...
    // Important: this should correspond to the values of PME_USE_TEXTURES and PME_USE_TEXOBJ, as these duplicate the logic for the device code.
