...
void pme_gpu_sync_grid(const gmx_pme_t *pme, const gmx_fft_direction dir)
{
    ...
    gmx_bool syncGPUGrid = ((dir == GMX_FFT_REAL_TO_COMPLEX) ? true : pme_gpu_performs_solve(pme->gpu));
    if (syncGPUGrid)
    {
...
void pme_gpu_reinit(gmx_pme_t *pme, const gmx_hw_info_t *hwinfo, const gmx_gpu_opt_t *gpu_opt)
{
    ...
    if (firstInit)
    {
        ...
        pme->gpu->settings.bGPUSingle = (pme->nnodes == 1);
        /* A convenience variable. */

        pme->gpu->settings.bGPUFFT = !pme_gpu_uses_dd(pme->gpu) && !getenv("GMX_PME_GPU_FFTW");
...
void pme_gpu_destroy(gmx_pme_t *pme)
{
    ...
    pme_gpu_destroy_timings(pme->gpu);
...
{
    if (!pme_gpu_enabled(pme)) // this is a check should be performed outside, on a much higher level
    {
        return;
    }
