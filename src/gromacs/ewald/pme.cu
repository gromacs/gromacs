...
void pme_gpu_realloc_and_copy_bspline_values(const pme_gpu_t *pmeGpu)
{
    ...
    const int splineValuesOffset[DIM] = {
        0,
        kernelParamsPtr->grid.realGridSize[XX],
        kernelParamsPtr->grid.realGridSize[XX] + kernelParamsPtr->grid.realGridSize[YY]
    };
    memcpy((void *)&kernelParamsPtr->grid.splineValuesOffset, &splineValuesOffset, sizeof(splineValuesOffset));

    const int newSplineValuesSize = kernelParamsPtr->grid.realGridSize[XX] +
        kernelParamsPtr->grid.realGridSize[YY] + kernelParamsPtr->grid.realGridSize[ZZ];
    cu_realloc_buffered((void **)&kernelParamsPtr->grid.d_splineModuli, nullptr, sizeof(float),
                        &pmeGpu->archSpecific->splineValuesSize, &pmeGpu->archSpecific->splineValuesSizeAlloc,
                        newSplineValuesSize, pmeGpu->archSpecific->pmeStream, true);

    for (int i = 0; i < DIM; i++)
    {
        /* TODO: use pinning for pmeGpu->common->bsp_mod[i] */
        cu_copy_H2D_async(kernelParamsPtr->grid.d_splineModuli + splineValuesOffset[i], pmeGpu->common->bsp_mod[i].data(),
                          kernelParamsPtr->grid.realGridSize[i] * sizeof(float), pmeGpu->archSpecific->pmeStream);
    }
}
...
void pme_gpu_init_specific(pme_gpu_t *pmeGPU, const gmx_hw_info_t *hwinfo, const gmx_gpu_opt_t *gpu_opt)
{
    ...
    pmeGPU->archSpecific->useTextureObjects = (pmeGPU->deviceInfo->prop.major >= 3);
