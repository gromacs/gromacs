 ...
 * true:  The atom data GPU buffers are padded with zeroes so that the number of atoms
 ...
 * \param[in] pmeGPU  The PME GPU structure.
 */
CUDA_FUNC_QUALIFIER void pme_gpu_init_specific(pme_gpu_t           *CUDA_FUNC_ARGUMENT(pmeGPU)) CUDA_FUNC_TERM
...
/*! \libinternal \brief
 * Updates the unit cell parameters if needed.
 *
 * \param[in] pmeGPU         The PME GPU structure.
