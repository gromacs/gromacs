                      ...
                      gmx_bool *bPrinting)
{
    ...
    if ((!bUseGPU && !bSepPMERanks) || !wallcycle_have_counter())
    {
                    ...
                    gmx_bool             *bPrinting)
{
    ...
    if (!pme_lb->bBalance &&
        (!pme_lb->bSepPMERanks || (step_rel <= PMETune_period*ir->nstlist)))
    {
        /* When we're not balancing and not measuring PP/PME imbalance
         * during the first 50*nstlist steps: deactivate the tuning.
         */
