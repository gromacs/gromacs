...
struct pme_load_balancing {
    ...
    int          cur;                /* the index (in setup) of current setup */
                      ...
                      gmx_bool *bPrinting)
{
    ...
    if ((!bUseGPU && (cr->duty & DUTY_PME)) || !wallcycle_have_counter())
    {
    ...
    if (pme_lb->bSepPMERanks)
    {
         ...
         * so we will start balancing after load imbalance is observed.
         */
    ...
    if (DOMAINDECOMP(cr) && cr->dd->nnodes > 1 && bUseGPU)
    {
                    ...
                    gmx_bool             *bPrinting)
{
    ...
    {
        /* When we're not balancing and not measuring PP/PME imbalance
         * during the first steps of the run: deactivate the tuning.
         */
        pme_lb->bActive = FALSE;
    }
