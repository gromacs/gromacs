...
struct pme_spline_work *make_pme_spline_work(int gmx_unused order)
{
    ...
    for (of = 0; of < 2*GMX_SIMD4_WIDTH-(order-1); of++)
    {
        ...
        real_mask_S0      = simd4Load(tmp);
        real_mask_S1      = simd4Load(tmp+GMX_SIMD4_WIDTH);
        work->mask_S0[of] = simd4CmpLt(real_mask_S0, zero_S);
        work->mask_S1[of] = simd4CmpLt(real_mask_S1, zero_S);
    }
#else
    work = NULL;
#endif
