                  ...
                  int              commplainsize)
{
    ...
    ol->send_id.resize(ol->noverlap_nodes);
    ol->recv_id.resize(ol->noverlap_nodes);
    for (auto b = 0; b < ol->noverlap_nodes; b++)
    {
    ...
    for (auto b = 0; b < ol->noverlap_nodes; b++)
    {
        ...
        /* Send */
        auto             fft_start = ol->s2g0[ol->send_id[b]];
        auto             fft_end   = ol->s2g0[ol->send_id[b] + 1];
        if (ol->send_id[b] < nodeid)
        {
            fft_start += ndata;
            fft_end   += ndata;
        }
        auto send_index1  = ol->s2g1[nodeid];
        ...
        auto recv_index1 = ol->s2g1[ol->recv_id[b]];
