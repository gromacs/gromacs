                           ...
                           PmeDeviceHostCopy                transferPmeForces)
{
    ...
    const unsigned int gridIndex  = 0;
    real              *fftgrid    = pme->fftgrid[gridIndex];
    pme_gpu_gather(pme->gpu, forceTreatment, reinterpret_cast<float *>(fftgrid), transferPmeForces);
    wallcycle_sub_stop(wcycle, ewcsLAUNCH_GPU_PME);
    wallcycle_stop(wcycle, ewcLAUNCH_GPU);
}
                             ...
                             bool                  sumForces)
{
    ...
    pme_gpu_reduce_outputs(flags, output, wcycle, forceWithVirial, enerd, sumForces);
}
