...
gmx_inline static void calc_exponentials_q(int, int, real f, SimdArrayRef<real> d_aligned, SimdArrayRef<real> r_aligned, SimdArrayRef<real> e_aligned)
{
    {
        SimdReal              f_simd(f);
...
#if defined PME_SIMD_SOLVE
/* Calculate exponentials through SIMD */
gmx_inline static void calc_exponentials_lj(int, int, SimdArrayRef<real> r_aligned, SimdArrayRef<real> factor_aligned, SimdArrayRef<real> d_aligned)
{
    ...
    for (iyz = iyz0; iyz < iyz1; iyz++)
    {
        ...
        if (bEnerVir)
        {
            ...
            calc_exponentials_q(kxstart, kxend, elfac,
                                SimdArrayRef<real>(denom, denom+kxend),
                                SimdArrayRef<real>(tmp1, tmp1+kxend),
                                SimdArrayRef<real>(eterm, eterm+kxend));
    ...
    for (iyz = iyz0; iyz < iyz1; iyz++)
    {
        ...
        if (bEnerVir)
        {
            ...
            calc_exponentials_lj(kxstart, kxend,
                                 SimdArrayRef<real>(tmp1, tmp1+kxend),
                                 SimdArrayRef<real>(tmp2, tmp2+kxend),
                                 SimdArrayRef<real>(denom, denom+kxend));
