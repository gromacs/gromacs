...
constexpr std::array<T, sizeof ... (Ns)>
generateArray(G g, absl::integer_sequence<T, Ns...>) noexcept
{
    return {{
                g(Ns) ...
            }};
...
constexpr std::array<T, N> generateArray(G g = {}) noexcept
{
...
#if defined __ICC || (defined __GNUC__ && __GNUC__ < 5 && !defined __clang__) || (defined __clang__ && __clang_major__ == 3 && __clang_minor__ < 7)
...
{
    *dst1 = shuffle(src, ifda<2, 0>());
    *dst2 = shuffle(src, ifda<2, c_simdWidth/2>());
}
...
{
    const T x = shuffle(a, b, ifma<2, 0>());
    const T y = shuffle(a, b, ifma<2, 1>());
    return x+y;
}
    ...
    for (iyz = iyz0; iyz < iyz1; iyz++)
    {
        ...
        if (bEnerVir)
        {
            ...
            for (kx = 0; kx < kxend; kx += c_simdWidth, p0 += c_simdWidth)
            {
                ...
                const auto      inRange = cvtIB2B(kx_S < PME_INT_T(kxend) && PME_INT_T(kxstart-1) < kx_S);
