...
static void realloc_work(struct pme_solve_work_t *work, int nkx)
{
    if (nkx > work->nalloc)
    {
        ...
        // operations.
        work->tmp1Storage.getUnpaddedArrayRef()[0] = 0;
        work->tmp1Storage.setPaddingRegionTo(0);
        // Note that tmp2 is only used for LJ-PME
        work->tmp2Storage.getUnpaddedArrayRef()[0] = 0;
        work->tmp2Storage.setPaddingRegionTo(0);
    }
}
    ...
    for (iyz = iyz0; iyz < iyz1; iyz++)
    {
        ...
        if (bEnerVir)
        {
            ...
            calc_exponentials_q(kxstart, kxend, elfac,
                                work->denomStorage.getSimdArrayRef(),
                                work->tmp1Storage.getSimdArrayRef(),
                                work->etermStorage.getSimdArrayRef());
                     ...
                     gmx_bool bEnerVir, int nthread, int thread)
{
    ...
    for (iyz = iyz0; iyz < iyz1; iyz++)
    {
        ...
        if (bEnerVir)
        {
            ...
            // Make sure tmp = exp(tmp) doesn't blow up over iterations
            work->tmp1Storage.setPaddingRegionTo(0);
            work->tmp2Storage.setPaddingRegionTo(0);

            calc_exponentials_lj(kxstart, kxend,
                                 work->tmp1Storage.getSimdArrayRef(),
                                 work->tmp2Storage.getSimdArrayRef(),
                                 work->denomStorage.getSimdArrayRef());
