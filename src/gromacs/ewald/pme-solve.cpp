...
template<typename T>
gmx_inline static void calc_exponentials_q(int start, real f, ArrayRef<T> d,
                                           ArrayRef<T> r, ArrayRef<T> e)
{
    ...
    {
        e[kx] = f_simd / d[kx] * exp(weak_cast<T>(r[kx]));
    }
}

/* Calculate exponentials through SIMD */
template <typename T>
gmx_inline static void calc_exponentials_lj(int start, ArrayRef<T> r, ArrayRef<T> factor, ArrayRef<T> d)
{
    const T        sqr_PI = sqrt(T(M_PI));
    ...
    for (size_t kx = start; kx != d.size(); ++kx)
    {
        d[kx]          = inv(weak_cast<T>(d[kx]));
        r[kx]          = exp(weak_cast<T>(r[kx]));
        const T tmp_mk = factor[kx];
        factor[kx]     = sqr_PI * tmp_mk * erfc(tmp_mk);
    }
}
                  ...
                  int nthread, int thread)
{
    ...
    for (iyz = iyz0; iyz < iyz1; iyz++)
    {
        ...
        if (bEnerVir)
        {
            ...
            calc_exponentials_q(kxstart, elfac,
                                ArrayRef<PME_T>(denom, denom+kxend),
                                ArrayRef<PME_T>(tmp1, tmp1+kxend),
                                ArrayRef<PME_T>(eterm, eterm+kxend));
                     ...
                     gmx_bool bEnerVir, int nthread, int thread)
{
    ...
    for (iyz = iyz0; iyz < iyz1; iyz++)
    {
        ...
        if (bEnerVir)
        {
            ...
            calc_exponentials_lj(kxstart,
                                 ArrayRef<PME_T>(tmp1, tmp1+kxend),
                                 ArrayRef<PME_T>(tmp2, tmp2+kxend),
                                 ArrayRef<PME_T>(denom, denom+kxend));
