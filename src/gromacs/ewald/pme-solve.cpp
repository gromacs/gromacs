...
template <int factor>
static int roundUpToMultipleOfFactor(int number)
{
    static_assert((factor & (factor - 1)) == 0, "factor should be a power of 2");
...
static void realloc_work(struct pme_solve_work_t *work, int nkx)
{
    if (nkx > work->nalloc)
    {
         ...
         */
        for (int i = 0; i < roundUpToMultipleOfFactor<c_simdWidth>(work->nalloc ); i++)
        {
            work->denom[i] = 1;
        }
    }
