 ...
 */
//This is a top-level file to generate all the PME OpenCL kernels

//FIXME
#define PmeGpuCudaKernelParams PmeGpuKernelParamsBase

//COMMENT
#define gmx_syncwarp()

// Assert placeholders, to not rip them out from OpenCL implementation
...
/*! \brief
 * Atomic addition of a float value into global memory. Copied from vectype_ops.clh (atomicAdd_g_f).
 * TODO: This only implements CAS, while we likely could use some native 32/64 bit atomics.
 */
DEVICE_INLINE void atomicAdd(volatile __global float *addr, float val)
{
    union {
        unsigned int u32;
        float        f32;
...
/*! \brief
 * Atomic addition of a float value into block-local memory. Copied from vectype_ops.clh (atomicAdd_l_f).
 * TODO: This only implements CAS, while we likely could use some native 32/64 bit atomics.
 */
DEVICE_INLINE void atomicAdd_l_f(volatile __local float *addr, float val)
{
    union {
        unsigned int u32;
        float        f32;
...
// TODO move those guys into a header /pass with the compiler defines
//! Spreading max block width in warps picked among powers of 2 (2, 4, 8, 16) for max. occupancy
#define c_spreadMaxWarpsPerBlock 8 //FIXME 4 for AMD
