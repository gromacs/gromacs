 ...
 * We still specify the alignment attribute - some compiler combinations break without it.
 * One could also try __attribute__ ((packed)), but it doesn't work with DeviceBuffer,
...
/*! \brief A macro which is only relevant to the OpenCL kernel compilation */
 ...
 */
struct GMX_GPU_ALIGNED PmeGpuAtomParams
{
 ...
 */
struct GMX_GPU_ALIGNED PmeGpuDynamicParams
{
 ...
 */
struct GMX_GPU_ALIGNED PmeGpuKernelParamsBase
{
