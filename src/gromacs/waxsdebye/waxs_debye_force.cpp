                              ...
                              FILE       *fplog)
{
    ...
    for (i = 0; (i <= 1); i++)
    {
            ...
            while (!ifs.eof())
            {
                ...
                if (sscanf(buf, "%15lf%15lf%15lf", &q, &S, &dS) == 3)
                {
                    ...
                    else
                    {
                        ...
                        GMX_RELEASE_ASSERT( (q_[n] != q), errbuf);
        ...
        {
            char buf[STRLEN];
            sprintf(buf, "Error opening input file %s!", waxs_data);

            GMX_RELEASE_ASSERT(true, buf);
        }
        ifs.close();
    }
    ...
    else
    {
        ...
        GMX_RELEASE_ASSERT(true, errbuf);
    }
                               ...
                               const t_inputrec *ir)
{
    ...
    {
        fp_out_ = new File(waxs_out, "w");
    }

    fp_alpha_ = NULL;
    if ((alpha_step_ != 0.0) && MASTER(cr) && (NULL != waxs_alpha))
    {
        fp_alpha_ = new File(waxs_alpha, "w");
    }
