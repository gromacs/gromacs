                              ...
                              FILE       *fplog)
{
    ...
    for (unsigned int i = 0; (i <= 1); i++)
    {
        ...
        gmx::TextInputFile ifs(waxs_data);
        std::string        line;
        int                n = 0;
        while (true == ifs.readLine(&line))
        {
            ...
            if (sscanf(buf, "%15lf%15lf%15lf", &q, &S, &dS) == 3)
            {
                ...
                {
                    /* If the q vectors are not exactly equal we can not
            ...
            else if (sscanf(buf, "%15lf%15lf", &q, &S) == 2)
            {
                ...
                {
                    /* check q vector consistency as above */
                    if (q != q_[n])
                    {
                        char errbuf[STRLEN];
                        sprintf(errbuf, "Inconsistency between reference data and difference data. Expected q[%d] to be %f but found %f", n, q_[n], q);
                               ...
                               const t_inputrec *ir)
{
    ...
    /* Initialize step count and alpha */
    step_       = 0;
    vtot_       = 0;
    alpha_      = 1.0;
    ...
    alpha_mode_ = ir->waxs.debye_alpha_mode;
    alpha_min_  = ir->waxs.debye_alpha_min;
    alpha_max_  = ir->waxs.debye_alpha_max;
    nstout_     = std::max(1, ir->waxs.nstout);
    nstcalc_    = std::max(1, ir->waxs.nstcalc);
    ...
    {
        fp_out_.reset(new TextOutputFile(waxs_out));
    }

    if (MASTER(cr) && (NULL != waxs_alpha))
    {
        fp_alpha_.reset(new TextOutputFile(waxs_alpha));
    }
    ...
    /* Fill the F_ array for all types
       allocate space first: atomic_number is one-based so add 1 to get F_'s size. */
...
#pragma omp parallel
    {
        ...
        int    b0 = t*(nbonds/nthreads);
        int    b1 = std::min(nbonds, (t+1)*(nbonds/nthreads));

        /* If the number of Debye terms is not divisible by
           nthreads, then the last thread takes up the slack */
        if (t == nthreads - 1)
        {
            b1 = nbonds;
        }
        ...
        for (int i = b0; (i < b1); )
        {
                ...
                ((0 < gj) && (gj < (int)F0_.size())))
            {
                ...
                {
                    /* We typically only store half the matrix of pairs */
                    SS0[t] += 2*dS;
                }
            }
        }
    }
    /* Global summing over threads */
    double S0 = 0;
                          ...
                          t_nrnb          *nrnb)
{
    ...
    for (int t = 0; (t < nthreads); t++)
    {
        SC[t].resize(Scalc_.size());
    }
    int nb = nbonds / 3;

    /* vector to count Debye terms */
    std::vector < int > nterms_threads;
    nterms_threads.resize(nthreads);

#pragma omp parallel
    {
        rvec         dx;
        real         dr2;
        int          ai, aj, gi, gj, type;
        int          t = gmx_omp_get_thread_num();

        int          b0 = 3*(t*(nb/nthreads));
        int          b1 = 3*(std::min(nb, (t+1)*(nb/nthreads)));
        ...
        for (int i = b0; (i < b1); )
        {
            ...
            if (ai == aj)
            {
                for (unsigned int n = 0; (n < Scalc_.size()); n++)
                {
            ...
            {
                real dr = dr2*gmx_invsqrt(dr2);
                for (unsigned int n = 0; (n < Scalc_.size()); n++)
                {
                    double qdr   = q_[n]*dr;
                    double fgigj = 2.0*F_[gi][n]*F_[gj][n];
                    SC[t][n] += fgigj*sin(qdr)/(qdr);
                }
            }
        }
    }
    ...
    /* sum up the number of Debye terms and react if things don't add up */
    int nterms = 0;
    for (int t = 0; (t < nthreads); t++)
    {
        nterms += nterms_threads[t];
    }
    if (nterms != nb)
    {
        char errbuf[STRLEN];
        sprintf(errbuf, "Only %d out of %d Debye terms taken into account!", nterms, nb);
        GMX_THROW(InternalError(errbuf));
    }
    ...
    for (unsigned int n = 0; n < Scalc_.size(); n++)
    {
        real dS  = dSexp_[n] - alpha_*(Scalc_[n] - Sref_[n]);
        A_[n]    = kWaxs_/(2*sqr(sigma_[n])) * dS;
        vtot_   += A_[n]*dS;
    }

    /* Find natom, the maximum atom index to consider */
    int natom = 0;
    for (int i = 0; (i < nbonds); i += 3)
    {
        natom = std::max(natom, std::max(iatoms[i+1], iatoms[i+2]));
    }
    natom++;

    /* loop over the scattering pairs again, this time with Scalc known */
    std::vector < std::vector < real > > FFF;
    FFF.resize(nthreads);
    for (int t = 0; (t < nthreads); t++)
    {
        FFF[t].resize(DIM*natom);
    }
#pragma omp parallel
    {
        int t  = gmx_omp_get_thread_num();
    ...
    for (int t = 0; (t < nthreads); t++)
    {
        for (int n = 0; (n < natom); n++)
        {
            for (int m = 0; (m < DIM); m++)
            {
                /* Multiply with nstcalc_ for multi time stepping!
                   See GROMACS manual section 3.4.7. */
                f[n][m] += nstcalc_ * FFF[t][DIM*n+m];
            }
        }
    }
...
void WaxsDebyeForce::dump()
{
    ...
    if ( ((step_ - 1) % nstout_) == 0)
    {
        if (fp_out_)
        {
            fp_out_->write("@type xy\n");
            for (n = 0; n < Scalc_.size(); n++)
            {
                snprintf(buf, sizeof(buf), "%8.3f  %8.3f  %8.3f\n",
        ...
        if (fp_alpha_)
        {
            snprintf(buf, sizeof(buf), "%8d  %8.5f \n", step_, alpha_);
            fp_out_->write(buf);
        }
    }
}
...
void WaxsDebyeForce::updateAlpha()
{
    switch (alpha_mode_)
    {
        ...
        default:
            GMX_THROW(InvalidInputError("alpha_mode_ should be 0 or 1"));
    }
}
