                              ...
                              FILE       *fplog)
{
            ...
            while (!ifs.eof())
            {
                ...
                if (sscanf(buf, "%15lf%15lf%15lf", &q, &S, &dS) == 3)
                {
                    ...
                    else
                    {
                        if (q_[n] != q)
                        {
                            gmx_fatal(FARGS, "Inconsistency between reference data and difference data. Expected q[%d] to be %f but found %f", n, q_[n], q);
                        }
                        dSexp_[n] = S;
                ...
                else if (sscanf(buf, "%15lf%15lf", &q, &S) == 2)
                {
                    ...
                    else
                    {
                        if (q_[n] != q)
                        {
                            gmx_fatal(FARGS, "Inconsistency between reference data and difference WAXS data. Expected q[%d] to be %f but found %f", n, q_[n], q);
                        }
                        dSexp_[n]  = S;
                    }
                    n++;
    ...
    else if ((ref_error.size() == 0) && (diff_error.size() == 0))
    {
        ...
        gmx_fatal(FARGS, "Inconsistent experimental WAXS error, expected 0 or %d values", (int) q_.size());
                               ...
                               const t_inputrec *ir)
{
    ...
    fp_out_ = NULL;
    if ((nstout_ > 0) && MASTER(cr))
    {
        fp_out_ = fopen(waxs_out, "w");
    }

    fp_alpha_ = NULL;
    if ((alpha_step_ != 0.0) && MASTER(cr) && (NULL != waxs_alpha))
    {
        fp_alpha_ = fopen(waxs_alpha, "w");
    }
    ...
    */
    F_.resize(cmt->max_atomic_number()+1);
    F0_.resize(cmt->max_atomic_number()+1);
...
void WaxsDebyeForce::Dump()
{
    ...
    if (step_ % nstout_ == 0)
    {
        if (NULL != fp_out_)
        {
            fprintf(fp_out_, "@type xy\n");
            for (n = 0; n < Scalc_.size(); n++)
            {
                fprintf(fp_out_, "%8.3f  %8.3f  %8.3f\n",
                        q_[n], alpha_*(Scalc_[n]-Sref_[n]), Scalc_[n]);
            }
            fprintf(fp_out_, "&\n");
            fflush(fp_out_);
        }
        if (NULL != fp_alpha_)
        {
            fprintf(fp_alpha_, "%8d  %8.5f  %8.3f\n", step_, alpha_, alpha_dot_);
            fflush(fp_alpha_);
        }
    }
}
        ...
        case 1:
            {
                ...
                fa=0; fb=0; fc=0; 
                for (n=0; n<q_.size(); n++){
                    fa += sqr(dSexp_[n] - a*(Scalc_[n] - Sref_[n]));
                    fb += sqr(dSexp_[n] - b*(Scalc_[n] - Sref_[n]));
                    fc += sqr(dSexp_[n] - c*(Scalc_[n] - Sref_[n]));
                }
