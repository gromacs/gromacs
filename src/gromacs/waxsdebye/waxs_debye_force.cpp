...
static double integrate_vector(const std::vector<double> q, const std::vector<double> S)
{
    double sq = 0;
                              ...
                              FILE       *fplog)
{
    ...
    for (unsigned int i = 0; (i <= 1); i++)
    {
        ...
        {
            char errbuf[STRLEN];
            sprintf(errbuf, "Error opening input file %s!", waxs_data);
                               ...
                               const t_inputrec *ir)
{
    ...
    {
        fp_alpha_.reset(NULL);
    }

    /* Read in the table for scattering factor data */
    ScatteringFactorTable cmt(sfactor);
