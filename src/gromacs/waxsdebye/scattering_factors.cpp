...
/*! \internal \file
 * \brief
 * Implements utilities for reading scattering factors
 *
...
//! Global libxml setting, indicate that the parser should work in validating mode.
...
static const char *bool2char(bool bbb)
{
...
bool ScatteringFactorTable::read(const char *datafile)
{
    ...
    if (NULL != debug)
    {
        fprintf(debug, "There are %d entries in the scatteringfactortable\n",
                (int) cm_.size());
    }
    return true;
}
