...
double FourierScatteringFactor::computeScatteringFactor(double q) const
{
    ...
    // add the polynomial
    // THIS IS AN UGLY FIX:
    //      p_[] NEEDS PROPER UNITS,
    //      p_[0] IS CURRENTLY ZERO AND NOT USED
    s += p_[1]*q*q*q*1e-3 + p_[2]*q*q*1e-2 + p_[3]*q*1e-1;

    // add the fourier series
    for (i = 1; i < a_.size(); i++)
    {
        s += a_[i]*cos(2*M_PI*(i)*(q-q0_)/(q1_-q0_));
        s += b_[i]*sin(2*M_PI*(i)*(q-q0_)/(q1_-q0_));
    }
    return s;
}

/*! \brief
 * Super-handy routine turns ptr into a double and throws in case of problems
 * \param[in] ptr  ptr to a string containing a double
 * \param[in] name description of the variable
 * \param[in] unit name of the unit for converting to double using gromacs routines
 * \return   the double
 */
...
bool ScatteringFactorTable::read(const char *datafile)
{
    ...
    int xmlDVCD = xmlDoValidityCheckingDefaultValue;
    ...
    xmlDoValidityCheckingDefaultValue = xmlDVCD;
