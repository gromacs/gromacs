...
double ScatteringFactorTable::computeScatteringFactor(int atomic_number, double q) const
{
    ...
    for (i = 0; (i < cm_.size()); i++)
    {
        ...
        if (cm_[i]->atomicNumber() == atomic_number)
        {
...
int ScatteringFactorTable::maxAtomicNumber() const
{
    unsigned int i         = 0;
    int          maxnumber = cm_[0]->atomicNumber();
    for (i = 0; (i < cm_.size()); i++)
    {
        maxnumber = std::max(maxnumber, cm_[i]->atomicNumber());
    }
    return maxnumber;
}
