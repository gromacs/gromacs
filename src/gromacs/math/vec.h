...
namespace gmx
{
// Since vec.h and vectypes.h independently declare `norm` and `norm2` in different
// namespaces, code that includes both headers but does not specify the namespace
// from which to use `norm` and `norm2` cannot properly resolve overloads without
// the following helper templates.
