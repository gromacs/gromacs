...
struct RMSD
{
    /*!\brief The numerator for the rmsd calculation.
     * \param[in] m mass of the atom
     * \param[in] x coordinate
     * \param[in] y coordinate
     * \returns contribution of this atom pair to numerator of rmsd calculation
     */
    static inline real numerator(real m, const gmx::RVec &x, const gmx::RVec &y)
    {
...
struct RhoMeasure
{
    /*!\brief The numerator for the rho-measure calculation.
     * \param[in] m mass of the atom
     * \param[in] x coordinate
     * \param[in] y coordinate
     * \returns contribution of this atom pair to numerator of rho-measure calculation
     */
    static inline real numerator(real m, const gmx::RVec &x, const gmx::RVec &y)
    {
        return RMSD::numerator(m, x, y);
    }
...
template <typename F>
real findStructureSimilarity(int nAtoms, const real *mass, const rvec *x, const rvec *xp, const int * index)
{
    real denominator = 0;
    real numerator   = 0;
    if (index != nullptr)
    {
        ...
        for (int i : indexRef)
        {
            numerator   += F::numerator(mass[i], x[i], xp[i]);
...
/*!\brief Mass weighted root mean square deviation.
   ...
   *\returns Mass-weighted RMSD between two structures
 * The total rmsd = sqrt(sum(m*(x-y)*(x-y)/sum(m)).
 */
template real findStructureSimilarity<RMSD>(int nAtoms, const real *mass, const rvec *x, const rvec *xp, const int *index);
/*!\brief Mass weighted rho similarity measure.
   *\param[in] nAtoms number of atoms of structures to compare
   *\param[in] mass Masses of the atoms for similarity comparison
   *\param[in] x the coordinates of the reference structure
   *\param[in] xp the coordinates of the structure to compare
 ...
 * rho = sqrt(sum(m*(x-y)*(x-y)/sum(m*(x+y)*(x+y)/4)).
 */
template real findStructureSimilarity<RhoMeasure>(int nAtoms, const real *mass, const rvec *x, const rvec *xp, const int *index);
