...
class MultiDimArray
{
    public:
        ...
        //! iterator type for linear iteration over the stored data, used,e.g., in free begin and end functions
        using iterator = typename TContainer::value_type *;
        //! const iterator type for linear iteration over the stored data, used,e.g., in free begin and end functions
        ...
        allocate(IndexType && ... index)
        {
            ...
            result.view_ = view_type(result.data_.data(), index ...);
            return result;
        }

        //! Copy constructor calls the copy constructor of the data, then sets a new matching view to the copied data
         ...
         * \returns ArrayRef to stored data.
         */
        constexpr ArrayRef<const value_type> data() const
        {
            return {data_.data(), data_.data()+data_.size()};
        }
    private:
...
/*! \brief Free begin function for MultidimArray.
 */
template <class TContainer, class Extents>
...
/*! \brief Free end function for MultidimArray.
 */
template <class TContainer, class Extents>
typename MultiDimArray<TContainer, Extents>::const_iterator
