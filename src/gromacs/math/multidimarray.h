 ...
 * \tparam TContainer Data type container for the data to be stored as MultiDimArray
 * \tparam Extents An extents class describing the arrays dimensions
 */
...
class MultiDimArray
{
    public:
         ...
         * \param[in] index multidimensional indices
         * \returns reference to array element
         */
        template< class... IndexType>
        reference operator()(IndexType && ... index)
        {
            static_assert( sizeof ... (index) == Extents::rank(), "Number of indices must match rank" );
    ...
    private:
        ...
        //! The data stored as multidimensional array.
        TContainer  data_;
};
 ...
 */
template <class TContainer, class Extents, class ... IndexType>
MultiDimArray<TContainer, Extents>
makeMultiDimArray(IndexType && ... index)
{
    static_assert( sizeof ... (index) == Extents::rank_dynamic(), "Number of indices must match the dynamic rank." );
    MultiDimArray<TContainer, Extents> multiDimArray;
    ...
    multiDimArray.data_.resize(map.required_span_size());
    // valid data_.data() requires resizing the container before assigning the view
    multiDimArray.view_ = basic_mdspan<typename TContainer::value_type, Extents>(multiDimArray.data_.data(), index ...);
