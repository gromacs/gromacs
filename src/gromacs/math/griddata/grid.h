...
class IGrid
{
    public:
         ...
         */
        virtual void setLatticeAndRescaleCell(const bounds<N> &lattice) = 0;

        /*! \brief
         * Lattice accessor.
         * \returns the lattice
         */
        virtual bounds<N> lattice() const      = 0;
...
{
    public:
        using NdVector   = typename CanonicalVectorBasis<N>::NdVector;
        using MultiIndex = offset<N>;
         ...
         */
        Grid(const CanonicalVectorBasis<N> &cell, const bounds<N> &lattice) : IGrid<N>(), cell_ (cell), lattice_ (lattice), unitCell_ (calculateUnitCellFromLatticeAndCell(lattice, cell)) {}
        /*! \brief
        ...
        void setLatticeAndRescaleCell(const bounds<N> &lattice) override
        {
            lattice_ = lattice;
        ...
        bounds<N> lattice() const override
        {
            return lattice_;
        }
    ...
    private:
         ...
         */
        NdVector multiIndexToNdVector(const offset<N> &index) const
        {
            NdVector ndVector;
         ...
         * \param[in] i N-dimensional index.
         */
        NdVector multiIndexToNdVector(const bounds<N> &index) const
        {
            NdVector ndVector;
         ...
         */
        CanonicalVectorBasis<N> calculateUnitCellFromLatticeAndCell(const bounds<N> &lattice, const CanonicalVectorBasis<N> cell)
        {
...
class GridWithTranslation : public Grid<N>
{
    public:
         ...
         */
        GridWithTranslation(const CanonicalVectorBasis<N> &cell, const bounds<N> &lattice, const NdVector &translation) : Grid<N>(cell, lattice), translation_ (translation) { }
        ...
        void readwrite(ISerializer * serializer) override
        {
            Grid<N>::readwrite(serializer);
            for (auto &t : translation_)
            {
                serializer->doFloat(&t);
            }
        }
