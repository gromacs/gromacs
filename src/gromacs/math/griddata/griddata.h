    ...
    public:
        ...
        GridData &operator=(const GridData &other)
        {
            ...
            else
            {
                ...
                if (grid_->lattice().getNumLatticePoints() != other.size())
                {
                    grid_ = nullptr;
                    this->resize(0);
                }
...
typedef class GridData<std::vector<float>, DIM> GridDataReal3D;
//! Three-dimensional complex numbers on a grid.
typedef class GridData<std::vector<t_complex>, DIM> GridDataComplex3D;
