...
class GridData : public TContainer
{
    public:
        ...
        typename TContainer::reference operator[](const offset<N> &index)
        {
         ...
         * \throws std::out_of_range if index is out of bounds for grid.
         * \returns Iterator to data element in grid at given index. */
        typename TContainer::const_reference operator[](const offset<N> &index) const
        {
            return *(std::begin(*this) + memoryOffset(index));
        }
         ...
         */
        ptrdiff_t memoryOffset(const offset<N> &offset) const
        {
            ptrdiff_t result = 0;
...
using GridDataReal3D = class GridData < std::vector < float, AlignedAllocator < float>>, DIM>;
