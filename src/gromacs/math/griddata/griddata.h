...
class GridData : public TContainer
{
    public:
        ...
        GridData(const GridData &other) : TContainer(other)
        {
        ...
        typename TContainer::iterator operator[](const typename IGrid<N>::MultiIndex &index)
        {
            int shift = {};
            for (size_t dim = 0; dim < N; ++dim)
            {
                shift += index[dim]*stride_[dim];
            }
            return std::begin(*this) + shift;
        }
        /*! \brief
         * Yield an iterator to a grid element via multi index.
         * \throws std::out_of_range if index is out of bounds for grid.
         * \returns Iterator to data element in grid at given index. */
        typename TContainer::const_iterator operator[](const typename IGrid<N>::MultiIndex &index) const
        {
            int shift = {};
            for (size_t dim = 0; dim < N; ++dim)
            {
        ...
        void readwrite(ISerializer * serializer)
        {
            ...
            {
                grid_ = GridWithTranslation<N>().duplicate();
            }
            // TODO: this will become safe, once static polymorphism is implemented
            grid_->readwrite(serializer);
    ...
    private:
        void setStride()
        {
            if (grid_ != nullptr)
            {
                ...
                {
                    stride_[dim] = stride_[dim-1] * grid_->lattice()[dim];
                }
            }
        };
