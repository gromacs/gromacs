...
class bounds
{
    public:
        ...
        bool contains(const offset<Rank> &offset) const noexcept
        {
            ...
            {
                if ( (offset[dim] < 0) || (bounds_[dim] <= offset[dim]))
                {
                    return false;
                }
            }
    ...
    private:
        /*!\brief The bounds of an N-dimensional array.
         */
 ...
 * The lowest rank index is incremented first. If any index hits the array bounds, it is
 * reset to zero and the next higher rank index is incremented.
 * E.g.: Within bounds (2,5,3), the stepping order is from an iterator is
 * (0,3,2) -> (1,3,2) -> (0,4,2) -> (1,4,2) == end
 * NOTE: Deviates from N3851, where the highest rank index is incremented first.
 * This deviation is to ensure consistency with GROMCAS practice where the first rank moves fastest.
 */
template <size_t Rank>
