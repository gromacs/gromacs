...
real datavectormeasure::sum(const std::vector<real> &vecData)
{
    return std::accumulate(vecData.cbegin(), vecData.cend(), 0.);
}

/*! \brief The minimum grid data value. */
real datavectormeasure::min(const std::vector<real> &vecData)
{
    return *std::min_element(vecData.cbegin(), vecData.cend());
}
/*! \brief The maximum grid data value. */
real datavectormeasure::max(const std::vector<real> &vecData)
{
    return *std::max_element(vecData.cbegin(), vecData.cend());
}
/*! \brief The mean grid data value. */
real datavectormeasure::mean(const std::vector<real> &vecData)
{
    return datavectormeasure::sum(vecData) / static_cast<real>(vecData.size());
}
/*! \brief
 * The size of the griddata in bytes.
 */
size_t datavectormeasure::data_size(const std::vector<real> &vecData)
{
    return vecData.size();
}

real datavectormeasure::sumOfSquareDeviation(const std::vector<real> &vecData)
{
    real data_mean = datavectormeasure::mean(vecData);
    return std::accumulate(vecData.cbegin(), vecData.cend(), 0.,
...
real datavectormeasure::normSquared(const std::vector<real> &vecData)
{
...
real datavectormeasure::norm(const std::vector<real> &vecData) { return sqrt(datavectormeasure::normSquared(vecData)); }
 ...
 */
real datavectormeasure::var(const std::vector<real> &vecData)
{
...
real datavectormeasure::rms(const std::vector<real> &vecData) { return sqrt(datavectormeasure::var(vecData)); }

real datavectormeasure::entropy(const std::vector<real> &realfield, real unitCellVolume)
{
