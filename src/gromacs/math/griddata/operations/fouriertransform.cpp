...
std::array<int, DIM> FourierTransform3D::columnMajorExtendToRowMajorExtend(const bounds<DIM> &extend) const
{
    return { static_cast<int>(extend[ZZ]), static_cast<int>(extend[YY]), static_cast<int>(extend[XX]) };
}

Grid < DIM, GridWithTranslation < DIM>> convertGridToReciprocalSpace(const Grid<DIM> &grid )
{
    MdFloatVector<DIM> scale {
        grid.lattice()
    };
    return {grid.cell().inverse().scaledCopy(scale), grid.lattice()};
}
...
{
    return { extend[XX] / 2 + 1, extend[YY], extend[ZZ] };
};

bounds<DIM> realGridExtendFromFourierTransfrom(const bounds<DIM> &extend)
{
    return { (extend[XX] - 1) * 2, extend[YY], extend[ZZ] };
};

FourierTransformRealToComplex3D::FourierTransformRealToComplex3D( )
{ };
std::unique_ptr < GridDataComplex3D> FourierTransformRealToComplex3D::result(const GridDataFloat3D &realInputGridData)
{
    /*
    ...
    complexTransformedGrid.setLatticeAndRescaleCell(
            fourierTransformGridExtendfromRealExtend(realInputGridData.getGrid().lattice()));
...
void FourierTransformRealToComplex3D::result(const GridDataFloat3D &realInputGridData,
                                             GridDataComplex3D     &complexTransformedGridData)
{
    ...
    if (plan_ == nullptr)
    {
        ...
        gmx_parallel_3dfft_init(&plan_,
                                columnMajorExtendToRowMajorExtend(realInputGridData.getGrid().lattice()).data(),
...
{
    auto realGrid = complexInputGridData.getGrid();
    realGrid.setLatticeAndRescaleCell(realGridExtendFromFourierTransfrom(complexInputGridData.getGrid().lattice()));
    auto realGridPointer         = convertGridToReciprocalSpace(realGrid);
    auto realTransformedGridData = std::unique_ptr <GridDataFloat3D>(new GridDataFloat3D(realGridPointer));
...
void
FourierTransformComplexToReal3D::result(const GridDataComplex3D   & /*complexInputGridData*/,
                                        GridDataFloat3D          &realTransformedGridData)
{
    if (plan_ == nullptr)
    {
        ...
        FunctionOnComplexGridData appliedFunction)
{
    ...
    auto        extend       = fieldGrid.lattice();

    auto        k             = RVec {
        0., 0., 0.
    };
    RVec deltakRow     = {fieldGrid.unitCell()[XX], 0, 0};
    RVec deltakColumn  = {0, fieldGrid.unitCell()[YY], 0};
    RVec deltakSection = {0, 0, fieldGrid.unitCell()[ZZ]};
    auto itMidSection  = field_.begin() +  field_.memoryOffset({0, 0, extend[ZZ] / 2 + 1});
    for (auto itValue = field_.begin(); itValue != itMidSection;
