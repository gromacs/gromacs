 ...
 */
void interpolateLinearly(const GridDataFloat3D &other,
                         GridDataFloat3D       *targetGrid);

GridDataFloat3D interpolateLinearly(const GridDataFloat3D &other,
                                    const Grid < DIM, GridWithTranslation < DIM>>     &targetGrid);
/*! \brief
    Interpolating after shifting and orienting the other grid.
 */
real getLinearInterpolationAt(const GridDataFloat3D                      &field,
                              const MdFloatVector<DIM>                   &r);
/*! \brief
    Interpolating after shifting and orienting the other grid.
 */
