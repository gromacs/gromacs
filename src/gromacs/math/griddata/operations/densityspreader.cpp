...
void DensitySpreader::zeroBuffers(int thread)
{
    ...
    if (!densitybuffer.getGrid().lattice().inLattice({{minIndex[XX], minIndex[YY], minIndex[ZZ]}}))
    {
        return;
    }
    const auto &maxIndex = gauss_transform_[thread].maximumUsedGridIndex();
    if (!densitybuffer.getGrid().lattice().inLattice({{maxIndex[XX], maxIndex[YY], maxIndex[ZZ]}}))
    {
        return;
    }
    ...
    for (int iz = minIndex[ZZ]; iz <= maxIndex[ZZ]; ++iz)
    {
        ...
        {
            const auto &beginXRow = densitybuffer.iteratorAtMultiIndex({{minIndex[XX], iy, iz}});
            const auto &endXRow   = densitybuffer.iteratorAtMultiIndex({{maxIndex[XX], iy, iz}})+1;
...
DensitySpreader::sumThreadLocalGrids_(const std::vector<IVec> &minimumUsedGridIndex, const std::vector<IVec> &maximumUsedGridIndex)
{
    ...
    std::array<int, DIM> gridStart;
    std::array<int, DIM> gridEnd;
    for (int i = XX; i <= ZZ; ++i)
    {
