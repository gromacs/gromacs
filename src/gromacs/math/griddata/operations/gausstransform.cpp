...
{
    auto firstLength = basis[XX];
    return equal_real(basis[YY], firstLength, ftol, abstol) && equal_real(basis[ZZ], firstLength, ftol, abstol);
}

void
GaussTransform::setGrid(GridDataFloat3D * grid, bool gaussianIsIntegrated)
{
    grid_                 = grid;
    minimumUsedGridIndex_ = {std::numeric_limits<int>::max(), std::numeric_limits<int>::max(), std::numeric_limits<int>::max()};
    ...
    nu_       = grid_->getGrid().unitCell()[XX]/sigma_;
    ...
    for (auto &spreader : OneDSpreader_)
    {
        ...
        {
            spreader = std::unique_ptr<ISpreader>(new GaussIntergralOneD(m_spread_, nu_));
        }
        else
        {
...
GaussTransform::tensor_product_()
{
    ...
    {
        minIndex[i] = std::max(ptrdiff_t(0), atomGridIndex_[i]-m_spread_);
        maxIndex[i] = std::min(atomGridIndex_[i]+m_spread_, grid_->getGrid().lattice()[i]-1);

        // return if spread density never reaches grid
        if (maxIndex[i] < 0 || minIndex[i] > grid_->getGrid().lattice()[i] - 1)
        {
            return;
        }
    ...
    {
        const int d_z              = zzIndex - atomGridIndex_[ZZ];
        const int localGridIndexZZ = d_z + m_spread_;

        // spread spheres instead of cubes
        const int yyIndexStart = std::max(minIndex[YY], atomGridIndex_[YY] - ceilSqrtLUT_[std::abs(d_z)][0]);
        const int yyIndexEnd   = std::min(maxIndex[YY], atomGridIndex_[YY] + ceilSqrtLUT_[std::abs(d_z)][0]);

        for (int yyIndex = yyIndexStart; yyIndex <= yyIndexEnd; ++yyIndex)
        {
            const int  d_y              = yyIndex - atomGridIndex_[YY];
            const int  localGridIndexYY = d_y + m_spread_;
            const real spread_zy        = spread_2d_[localGridIndexZZ][localGridIndexYY];

            const int  spreadWidth  = ceilSqrtLUT_[std::abs(d_z)][std::abs(d_y)];
            const int  xxIndexStart = std::min(maxIndex[XX], std::max(minIndex[XX], atomGridIndex_[XX] - spreadWidth));
            const int  xxIndexEnd   = std::max(minIndex[XX], std::min(maxIndex[XX], atomGridIndex_[XX] + spreadWidth));

            const int  xxIndexLocalStart      = xxIndexStart - atomGridIndex_[XX]+m_spread_;
            const int  numberSpreadVoxelsXX   = xxIndexEnd - xxIndexStart + 1;
            auto       voxel                  = grid_->begin() + grid_->memoryOffset({xxIndexStart, yyIndex, zzIndex});
...
GaussIntergralOneD::GaussIntergralOneD(int m_spread, real nu) :
    m_spread_(m_spread)
{
