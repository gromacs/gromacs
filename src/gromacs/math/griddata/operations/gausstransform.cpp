...
ArrayRef<real>  GaussIntergralOneD::spread( real weight, real x)
{
    ...
    std::transform(voxelBoundaryBuffer_.begin(), voxelBoundaryBuffer_.end(), erfBuffer_.begin(), [x](real voxelBoundary) {return (voxelBoundary-x)/sqrt(2); });
     #if GMX_SIMD_HAVE_REAL
    for (int i = 0; i < erfBuffer_.size(); i += GMX_SIMD_REAL_WIDTH)
    {
        SimdReal sx = load<SimdReal>(erfBuffer_ + i);
        sx = gmx::erf(sx);
        store(x + i, sx);
    }
