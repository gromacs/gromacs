...
FourierShellCorrelation::FourierShellCorrelation(const Grid < DIM, GridWithTranslation < DIM>> &realGrid)
{
    auto        reciprocalGrid = convertGridToReciprocalSpace(realGrid);
    const auto &unitcell       =  reciprocalGrid.unitCell();
    auto        spacing        = 2*std::max({unitcell[XX], unitcell[YY], unitcell[ZZ]});
    const auto &cell           = realGrid.cell();
    auto        highestK       = norm(RVec {cell[XX], cell[YY], cell[ZZ]});
...
std::vector<real>
FourierShellCorrelation::getFscCurve(const GridDataFloat3D &reference, const GridDataFloat3D &other)
{
    ...
    auto referenceFT = FourierTransformRealToComplex3D().normalize().result(reference);
    auto otherFT     = FourierTransformRealToComplex3D().normalize().result(other);
