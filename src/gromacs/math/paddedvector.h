...
//! Helper function to implement padding initialization, when required for BasicVector<T>
template <typename T, typename AllocatorType>
inline void resizeWithPaddingImpl(std::vector<T, AllocatorType> *v,
                                  const size_t newPaddedSize)
{
    v->resize(newPaddedSize);
}

//! Specialization of helper function to implement padding initialization for BasicVector<T>.
template <typename T, typename AllocatorType>
inline void resizeWithPaddingImpl(std::vector<BasicVector<T>, AllocatorType> *v,
                                  const size_t newPaddedSize)
{
    auto oldSize = v->size();
    v->reserve(newPaddedSize);
    // Ensure the padding region is initialized to zero.
    v->insert(v->end(), newPaddedSize - oldSize, BasicVector<T>(0,0,0));
}
...
class PaddedVector
{
    public:
        ...
        /*! \brief Returns the padded size for PaddedVector given the number of atoms
         *
         * \param[in] numElements  The number of T elements for which data will be stored
         */
        static index computePaddedSize(index numElements)
        {
            ...
            // We sometimes load a whole extra element when doing
            // 4-wide SIMD operations (which might e.g. be an RVec) so
            // we need to pad for that.
            index simdScatterAccessSize = numElements + 1;

            // For SIMD updates based on RVec, we might load
            // PaddingTraits<RVec>::widthInSimdBaseType (ie 3)
            // starting from the last RVec element, so that sets the
            // minimum extent of the padding. That extent must take
            // the initialized allocation up to the SIMD width of the
            // base type multiplied by the width of T in that base
            // type. But since storage_ contains RVec, we only have to
            // tell it the number of elements, which means to round up
            // to the next SIMD width.
            //
            // We don't want a dependence on the SIMD module for the
            // actual SIMD width of the base type, so we use maximum
            // for the base type via the traits. A little extra
            // padding won't really hurt.
    ...
    public:
        ...
        void reserveWithPadding(const size_type newExtent)
        {
            ...
            auto newPaddedExtent = computePaddedSize(newExtent);
        ...
        {
            auto newPaddedSize = computePaddedSize(newSize);
            // Ensure the padding region is zeroed if required
            detail::resizeWithPaddingImpl(&storage_, newPaddedSize);
            unpaddedEnd_ = begin() + newSize;
        }

        /*! \brief Resize the container to contain newSize elements,
         * plus the required padding, using \c v to initialize
         * elements. */
        void resizeWithPadding(const size_type newSize, value_type const &v)
        {
            auto newPaddedSize = computePaddedSize(newSize);
            storage_.resize(newPaddedSize, v);
            unpaddedEnd_ = begin() + newSize;
        }
