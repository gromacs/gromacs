...
//! Returns all the exception flags that are enabled
static int gmx_fegetexcept()
{
// The branch order is important here because MacOS might actually
// have legacy fegetexcept(), different from the GNU one.
#if (defined(__i386__) || defined(__x86_64__)) && defined(__APPLE__)
    static fenv_t fenv;
    return fegetenv (&fenv) ? -1 : ((~fenv.__control) & FE_ALL_EXCEPT); // negating because it's a mask
#elif HAVE_FEGETEXCEPT
    return fegetexcept();
#else
    return -1;
#endif
...
{
    return (gmx_fegetexcept() == c_FPexceptions);
}
