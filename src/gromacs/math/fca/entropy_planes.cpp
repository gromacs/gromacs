...
FcaEntropy::FcaEntropy(const int inNdata, const int inNbins)
{
    ...
    for (int i = 0; i < this->ngauss; i++)
    {
        ...
        this->gaussprime[i] = -1.0 / sqrt(2.0 * fca_utils::PI_constant()) / sigma * (1.0 / fca_utils::squareof(sigma) * (i - floor(this->ngauss / 2.0))) * exp(-fca_utils::squareof(1.0 / sigma * (i - floor(this->ngauss / 2.0))) / 2);
...
real FcaPlane::dbrent_plane(const real ax, const real bx, const real cx, FcaEntropy* entr, const real tol, real* xmin)
{
    ...
    exit(1);
    return fca_utils::limits_max<real>();
}
