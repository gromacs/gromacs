...
FcaEntropy::FcaEntropy(const int inNdata, const int inNbins)
{
    this->nbins = inNbins;
    this->ndata = inNdata;

    this->bins      = compat::make_unique<real[]>(nbins);
    this->p_quant   = compat::make_unique<real[]>(nbins);
    this->phi_quant = compat::make_unique<real[]>(nbins);
    this->wbins     = compat::make_unique<real[]>(nbins);
    this->eta       = compat::make_unique<real[]>(ndata);
    this->binndx    = compat::make_unique<int[]>(ndata);
    this->p         = compat::make_unique<real[]>(ndata);
    this->phi       = compat::make_unique<real[]>(ndata);

    const real sigma = 5 * (1 + log10(std::ceil(20000.0 / ndata))); /* fuer 2000 Punkte ist 5 zu niedrig, aber bei 20000 ist es super */
    this->ngauss     = std::ceil(sigma * 7);                        /* mit *14 ist es genauer, aber so reicht es eigentlich. cumsum(gradS) weicht zwar ein bisschen von S ab, aber hat selben hoch+tief punkte. */

    this->gauss      = compat::make_unique<real[]>(this->ngauss);
    this->gaussprime = compat::make_unique<real[]>(this->ngauss);
    this->Fquant     = compat::make_unique<real[]>(nbins);
    this->Hgradl     = compat::make_unique<real[]>(ndata);
    ...
    for (int i = 0; i < this->ngauss; i++)
    {
        ...
        this->gaussprime[i] = -1.0 / std::sqrt(2.0 * fca_utils::PI_constant()) / sigma * (1.0 / fca_utils::squareof(sigma) * (i - std::floor(this->ngauss / 2.0))) * exp(-fca_utils::squareof(1.0 / sigma * (i - std::floor(this->ngauss / 2.0))) / 2);
    }   /* muss dann jeweils mit dx malgenommen werden */
        /* Initialize to value, even if it is not used */
    this->dx = -1.;
}

void FcaEntropy::entropy_binning(const real s[])
{
    ...
    for (int i = 0; i < this->ndata; i++)
    {
        long int   mhash  = std::lrint(std::floor((s[i] - minv) / dx));
...
void FcaEntropy::entropy_convolution(const real kernel[], const real bins[], real result[]) const
{
...
real FcaEntropy::entropy_interpolate_p()
{
    real       S            = 0.0;
...
void FcaEntropy::entropy_interpolate_phi()
{
    const real invdx        = 1.0 / this->dx;
    const real inv2dx       = fca_utils::squareof(invdx);
    const int* mhash        = this->binndx.get();
    for (int i = 0; i < this->ndata; i++)
    {
        const real a = this->eta[i];
...
real FcaEntropy::entropy_grad(const real gfun[])
{
    const real invdx        = 1.0 / this->dx;
    const real inv2dx       = fca_utils::squareof(invdx);
    real       gS           = 0;
...
real FcaEntropy::entropy_1D(real* S, real* gS, const real s[], const real g[])
{
    entropy_binning(s);
...
FcaPlane::FcaPlane(const int inNdata)
{
    this->ndata = inNdata;
    this->proj1 = compat::make_unique<real[]>(ndata);
    this->proj2 = compat::make_unique<real[]>(ndata);
    this->s1    = compat::make_unique<real[]>(ndata);
    this->s2    = compat::make_unique<real[]>(ndata);
    this->g1    = compat::make_unique<real[]>(ndata);
    this->g2    = compat::make_unique<real[]>(ndata);
    this->theta = 0;
    this->S     = 0;
    this->gS    = 0;
}

void FcaPlane::init_plane(const std::unique_ptr< real[] > projx[], const real a1[], const real a2[], const int dim)
{
    for (int i = 0; i < this->ndata; i++)
    {
        this->proj1[i] = 0;
...
void FcaPlane::rotate_plane(const real theta)
{
...
void FcaPlane::entropy_plane(real* S, real* gS, FcaEntropy* entr)
{
    real S1  = 0, S2 = 0;
    real gS1 = 0, gS2 = 0;
...
real FcaPlane::minimize_plane(FcaEntropy* entr)
{
    ...
    {
        theta = fca_utils::PI_constant() / SCAN_STEPS / 2 * k;
        rotate_plane(theta);
    ...
    const real bx = fca_utils::PI_constant() / SCAN_STEPS / 2 * k_min;
    const real ax = fca_utils::PI_constant() / SCAN_STEPS / 2 * (k_min - 1);
    const real cx = fca_utils::PI_constant() / SCAN_STEPS / 2 * (k_min + 1);
...
real FcaPlane::dbrent_plane(const real ax, const real bx, const real cx, FcaEntropy* entr, const real tol, real* xmin)
{
    const int    ITMAX   = 100;
    ...
    for (int iter = 1; iter <= ITMAX; iter++)
    {
        ...
        const real tol1 = tol * std::fabs(x) + ZEPS;
    ...
    gmx_fatal(FARGS, "Too many iterations in routine DBRENT");
}
