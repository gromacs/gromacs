...
void FcaMaster::rand_array_int(ThreeFry2x64<64> &rng,
...
void FcaMaster::update_icmatrix(const int nr, const std::pair<int, int>* moves)
{
    for (int i = 0, ct = 0; ct < nr; i++)
    {
        ...
        if (com.isMaster())
        {
            ...
            const real atheta = std::fabs(theta);
            for (int j = 0; j < dim; j++)
            {
                                                               ...
                                                               fca_utils::Log& /*logger*/)
{
    ...
    {
        if (!indexesUsed[st.indexes.first]
            && !indexesUsed[st.indexes.second])
        {
      ...
      MIsum(0)
{
    ...
    if (com.isMaster())
    {
        this->to_compute = compat::make_unique<real[]>(dim*dim);
        this->move       = this->to_compute.get();
        this->theta      = compat::make_unique<double[]>(dim*dim);
        this->MI         = compat::make_unique<double[]>(dim*dim);
        for (int i = 0; i < dim; i++)
        {
            for (int k = 0; k < dim; k++)
            {
    ...
    /*  allocate ic_matrix */
    this->ic_matrix = compat::make_unique<std::unique_ptr< real[] >[]>(dim);
    this->icx       = compat::make_unique<std::unique_ptr< real[] >[]>(dim);
    this->S1        = compat::make_unique<real[]>(dim);
    for (int i = 0; i < dim; i++)
    {
        this->icx[i]       = compat::make_unique<real[]>(nframes);
        this->ic_matrix[i] = compat::make_unique<real[]>(dim);
        for (int k = 0; k < dim; k++)
        {
...
void FcaMaster::read_ic_matrix(const char inFilename[])
{
    ...
    for (int j = 0; j < dim; j++)
    {
        for (int i = 0; i < dim; i++)
        {
            ...
            if (nread < 1)
            {
                gmx_fatal(FARGS, "File does not contain enough elements to read (%dx%d)-matrix", dim, dim);
            }
        }
    }
    fclose(in);
...
void FcaMaster::compute_MI_matrix(real* to_compute, ThreeFry2x64<64>* rng)
{
    ...
    if (rng)
    {
        ...
        {
            perm_data[d] = compat::make_unique<real[]>(this->nframes);
        }
    }
                           ...
                           const real WEIGHT_AMP, const real WEIGHT_ANH)
{
    ...
    fca_utils::PSort::sort(&qsort_data[0], &qsort_data[dim], [&](const qsort_fca &a, const qsort_fca &b) {
                               const gmx_bool res = (WEIGHT_AMP * b.amp + WEIGHT_ANH * std::log(1e-9 + std::abs(b.anh)))
                                   > ((a.amp) * WEIGHT_AMP + std::log(1e-9 + std::abs(a.anh)) * WEIGHT_ANH);
                               return (res);
                           });
...
void FcaMaster::compute_mode_anharmonicity(const real* amplitude,
                                           real      * anharm) const
{
    const real logsp = std::log(2 * fca_utils::PI_constant());
    for (int i = 0; i < dim; i++)
    {
        anharm[i] = 0.5 * (1 + logsp + std::log(amplitude[i])) - S1[i];
    }
}
