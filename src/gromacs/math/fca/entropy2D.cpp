...
void FcaEntropy2D::entropy2D_binning(const real sx[], const real sy[])
{
    ...
    {
        const int  xhash    = std::lrint(std::floor((sx[i] - xmin) / dx));
        const int  yhash    = std::lrint(std::floor((sy[i] - ymin) / dy));
...
FcaEntropy2D::FcaEntropy2D(const int ndata, const int nbins, const bool bAccurate)
{
    ...
    this->bins    = compat::make_unique<real[]>(nbins * nbins);
    this->p_quant = compat::make_unique<real[]>(nbins * nbins);
    this->eta     = compat::make_unique<std::pair<real, real>[]>(ndata);
    this->binndx  = compat::make_unique<std::pair<int, int>[]>(ndata);
    ...
    this->gauss = compat::make_unique<real[]>(this->ngauss);
