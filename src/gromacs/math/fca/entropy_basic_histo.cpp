                                             ...
                                             const FcaBasicEntropy &entr)
{
    ...
    const real                binwidth       = exp(entr.log_kappa1) * std::sqrt(sig);
    const int                 nbins          = std::lrint(std::ceil((maxv - minv) / binwidth)) + 1;
    real                      norm           = binwidth;
    ...
    for (int i = 0; i < entr.ndata; i++)
    {
        const int ibin = std::lrint(std::floor((s[i] - minv) / binwidth));
        bins[ibin] += invn;
    }
                                             ...
                                             const FcaBasicEntropy &entr)
{
    ...
    const real                x_sig       = std::sqrt(xvar - xavg * xavg);
    const real                x_binwidth  = exp(entr.log_kappa2) * x_sig;
    const int                 x_nbins     = std::lrint(std::ceil((x_max - x_min) / x_binwidth)) + 1;

    const real                y_sig       = std::sqrt(yvar - yavg * yavg);
    const real                y_binwidth  = exp(entr.log_kappa2) * y_sig;
    const int                 y_nbins     = std::lrint(std::ceil((y_max - y_min) / y_binwidth)) + 1;
    ...
    for (int i = 0; i < entr.ndata; i++)
    {
        const int ix = std::lrint(std::floor((sx[i] - x_min) / x_binwidth));
        const int iy = std::lrint(std::floor((sy[i] - y_min) / y_binwidth));
