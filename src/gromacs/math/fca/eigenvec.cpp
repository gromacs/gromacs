                         ...
                         const char* AverageFile)
{
    ...
    else
    {
        ...
        this->xtop = compat::make_unique<rvec[]>(natoms);
        ...
        if (this->xref == nullptr)
        {
            if (bFit)
            {
                ...
                get_index((&atoms), IndexFile, 1, &this->nfit, fca_utils::make_auto_move_ptr(this->ifit),
                          &grpname);
                this->w_rls = compat::make_unique<real[]>((&atoms)->nr);
                if (bM && bDMR)
                {
        ...
        {       /* we have xref */
            ...
            this->w_rls = compat::make_unique<real[]>(this->nfit);
            for (int i = 0; (i < this->nfit); i++)
            {
                this->ifit[i] = i;
                if (bDMR && bM)
    ...
    this->sqrtm = compat::make_unique<real[]>(this->natoms);
    if (bM && bDMA)
    {
        for (int i = 0; (i < this->natoms); i++)
        {
            this->sqrtm[i] = std::sqrt((&atoms)->atom[this->index[i]].m);
        }
    }
    else
    {
                                                    ...
                                                    const bool resetBuffer) const
{
    ...
    if (bCorrection)
    {
        vel  = compat::make_unique<rvec[]>(ned);
        xdum = compat::make_unique<rvec[]>(ned);
        ...
        if (fread)
        {
            force = compat::make_unique<rvec[]>(ned);
            for (int i = 0; i < natoms; i++)
            {
