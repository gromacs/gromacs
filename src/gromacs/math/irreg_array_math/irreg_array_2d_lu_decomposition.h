    ...
    for (index_type j = 0; j < (n - 1); ++j)
    {
        ...
        for (index_type i = j + 1; i < n; ++i)
        {
            ...
            if (simdBlockBegin > j && simdBlockBegin < maxSimdBlockEnd)
            {
                ...
                {
                    aSimd = gmx::load<simd_float_type>(lu(j) + static_cast<ptrdiff_t>(k));
                    bSimd = gmx::load<simd_float_type>(lu(i) + static_cast<ptrdiff_t>(k));
                    aSimd = bSimd - c * aSimd;
...
matrixInverseLUPSimd(const IrregArray2D<T, Alloc> &a, IrregArray2D<T, Alloc> &aInv)
{
    ...
    array_type        lu(0, n - 1, 0, n - 1, static_cast<value_type>(0));

    // temporary work space
    vector_type  x(n, static_cast<value_type>(0));
    vector_type  y(n, static_cast<value_type>(0));
    array_type   b(0, n - 1, 0, n - 1, static_cast<value_type>(0));
    ...
    for (index_type i = 0; i < n; ++i)
    {
        ...
        for (index_type j = 0; j < n; ++j)
        {
            ...
            {
                aSimd = gmx::load<simd_float_type>(lu(j)    + static_cast<ptrdiff_t>(k));
                bSimd = gmx::load<simd_float_type>(y.data() + static_cast<ptrdiff_t>(k));
            ...
            tmpf += std::inner_product(lu(j)    + static_cast<ptrdiff_t>(simdBlockWidth), lu(j) + static_cast<ptrdiff_t>(j),
                                       y.data() + static_cast<ptrdiff_t>(simdBlockWidth), static_cast<value_type>(0));
            y[j] = b(i, p[j]) - tmpf;
        }
        ...
        for (index_type j = n - 1; j >= 0; --j)
        {
            ...
            if (simdBlockBegin > j && simdBlockBegin < maxSimdBlockEnd)
            {
                tmpf = std::inner_product(lu(j)    + static_cast<ptrdiff_t>(j_plus_1), lu(j) + static_cast<ptrdiff_t>(simdBlockBegin),
                                          x.data() + static_cast<ptrdiff_t>(j_plus_1), static_cast<value_type>(0));

                cSimd = gmx::setZero();
                for (index_type k = simdBlockBegin; k < maxSimdBlockEnd; k += simdWidth)
                {
                    aSimd = gmx::load<simd_float_type>(lu(j)    + static_cast<ptrdiff_t>(k));
                    bSimd = gmx::load<simd_float_type>(x.data() + static_cast<ptrdiff_t>(k));
                    cSimd = gmx::fma(aSimd, bSimd, cSimd);
                }
                tmpf += gmx::reduce(cSimd);

                tmpf += std::inner_product(lu(j)    + static_cast<ptrdiff_t>(maxSimdBlockEnd), lu(j) + static_cast<ptrdiff_t>(n),
                                           x.data() + static_cast<ptrdiff_t>(maxSimdBlockEnd), static_cast<value_type>(0));
            }
            else
            {
...
inline bool matrixInverseLUPRef(const IrregArray2D<T, Alloc> &a, IrregArray2D<T, Alloc> &aInv)
{
    ...
    for (index_type i = 0; i < n; ++i)
    {
        ...
        for (index_type j = 0; j < n; ++j)
        {
            const value_type tmpf = std::inner_product(lu(j), lu(j) + static_cast<ptrdiff_t>(j), y.data(), static_cast<value_type>(0));
            y[j] = b(i, p[j]) - tmpf;
        }
        ...
        for (index_type j = n - 1; j >= 0; --j)
        {
            const value_type tmpf = std::inner_product(lu(j)    + static_cast<ptrdiff_t>(j + 1), lu(j) + static_cast<ptrdiff_t>(n),
                                                       x.data() + static_cast<ptrdiff_t>(j + 1), static_cast<value_type>(0));
            x[j] = (y[j] - tmpf) / lu(j, j);
        }
