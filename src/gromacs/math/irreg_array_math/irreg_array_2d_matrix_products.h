...
/*! \libinternal \file
    ...
    + matrix vector product \f$ \mathbf{w} = \mathbf{A}\,\mathbf{v} \f$ 
        - input matrix \f$ \mathbf{A} \f$ stored as is                                       : missing
        - input matrix \f$ \mathbf{A} \f$ stored as transpose \f$ \mathbf{A}^{\mathrm{T}} \f$: missing
    + matrix product \f$ \mathbf{C} = \mathbf{A} \,\mathbf{B} \f$
        ...
        - input matrices stored as \f$ \mathbf{A}^{\mathrm{T}},\mathbf{B} \f$             : missing
        - input matrices stored as \f$ \mathbf{A}^{\mathrm{T}},\mathbf{B}^{\mathrm{T}} \f$: missing
...
matrixProductMatrixVectorSimd(const IrregArray2D<T, Alloc> &a, const std::vector<T, Alloc> &v, std::vector<T, Alloc> &w)
{
...
matrixProductMatrixVectorSimd(const IrregArray2D<T, Alloc> &a, const std::vector<T, Alloc> &v, std::vector<T, Alloc> &w)
{
    ...
    constexpr index_type simd4Width = SimdSetup<T>::simd4Width_;
#else
    const     index_type simdWidth  = SimdSetup<T>::simdWidth_;
    const     index_type simd4Width = SimdSetup<T>::simd4Width_;
#endif
    ...
    std::fill(w.begin(), w.end(), (value_type)0);
    // number of columns of matrix A
    const index_type n = a.getLength2(0);
    ...
    if (simdBlockWidth != 0)
    {
        for (index_type i = 0; i < m; ++i)
        {
            ...
            {
                vSimd = gmx::load(v.data() + j);
                aSimd = gmx::load(a(i) + j);
    ...
    const index_type simd4BlockWidth = (rest >= simd4Width ? rest - rest % simd4Width : 0);
 ...
 */
template<typename T, class Alloc = std::allocator<T> >
inline void matrixProductMatrixVectorRef(const IrregArray2D<T, Alloc> &a, const std::vector<T, Alloc> &v, std::vector<T, Alloc> &w)
{
    typedef typename IrregArray2D<T, Alloc>::value_type value_type;
    typedef typename IrregArray2D<T, Alloc>::index_type index_type;
 ...
 */
template<typename T, class Alloc = std::allocator<T> >
inline void matrixProductMatrixVector(const IrregArray2D<T, Alloc> &a, const std::vector<T, Alloc> &v, std::vector<T, Alloc> &w)
{
#if GMX_SIMD
    matrixProductMatrixVectorSimd(a, v, w);
#else
    matrixProductMatrixVectorRef(a, v, w);
#endif
}
...
inline
typename std::enable_if<!(gmx::SimdSetup<T>::doSimd_), void>::type
matrixProductVectorMatrixTSimd(const std::vector<T, Alloc> &v, const IrregArray2D<T, Alloc> &aT, std::vector<T, Alloc> &w)
{
...
matrixProductVectorMatrixTSimd(const std::vector<T, Alloc> &v, const IrregArray2D<T, Alloc> &aT, std::vector<T, Alloc> &w)
{
    ...
    const index_type m = aT.getLength2(0);
    // number of columns of matrix A, rows    of A^T
    const index_type n = aT.getLength1();
    std::fill(w.begin(), w.end(), (value_type)0);
    ...
    const index_type simdBlockWidth  = (m > simdWidth ? m - m % simdWidth : 0);
    if (simdBlockWidth != 0)
    {
 ...
 */
template<typename T, class Alloc = std::allocator<T> >
inline void matrixProductVectorMatrixTRef(const std::vector<T, Alloc> &v, const IrregArray2D<T, Alloc> &aT, std::vector<T, Alloc> &w)
{
    typedef typename IrregArray2D<T, Alloc>::value_type value_type;
    typedef typename IrregArray2D<T, Alloc>::index_type index_type;
 ...
 */
template<typename T, class Alloc = std::allocator<T> >
inline void matrixProductVectorMatrixT(const std::vector<T, Alloc> &v, const IrregArray2D<T, Alloc> &aT, std::vector<T, Alloc> &w)
{
#if GMX_SIMD
    matrixProductVectorMatrixTSimd(v, aT, w);
#else
...
inline
typename std::enable_if<!(gmx::SimdSetup<T>::doSimd_), void>::type
matrixProductVectorMatrixSimd(const std::vector<T, Alloc> &v, const IrregArray2D<T, Alloc> &a, std::vector<T, Alloc> &w)
{
...
matrixProductVectorMatrixSimd(const std::vector<T, Alloc> &v, const IrregArray2D<T, Alloc> &a, std::vector<T, Alloc> &w)
{
    ...
    if (simdBlockWidth != 0)
    {
        ...
        for (index_type j = 0; j < m; ++j)
        {
            ...
            {
                wSimd = gmx::load(w.data() + i);
                aSimd = gmx::load(a(j) + i);
                wSimd = gmx::fma(vSimd, aSimd, wSimd);
 ...
 */
template<typename T, class Alloc = std::allocator<T> >
inline void matrixProductVectorMatrixRef(const std::vector<T, Alloc> &v, const IrregArray2D<T, Alloc> &a, std::vector<T, Alloc> &w)
{
    typedef typename IrregArray2D<T, Alloc>::value_type value_type;
    typedef typename IrregArray2D<T, Alloc>::index_type index_type;
 ...
 */
template<typename T, class Alloc = std::allocator<T> >
inline void matrixProductVectorMatrix(const std::vector<T, Alloc> &v, const IrregArray2D<T, Alloc> &a, std::vector<T, Alloc> &w)
{
#if GMX_SIMD
    matrixProductVectorMatrixSimd(v, a, w);
#else
    matrixProductVectorMatrixRef(v, a, w);
#endif
}
...
inline
typename std::enable_if<!(gmx::SimdSetup<T>::doSimd_), void>::type
matrixProductMatrixTVectorSimd(const IrregArray2D<T, Alloc> &aT, const std::vector<T, Alloc> &v, std::vector<T, Alloc> &w)
{
 ...
 */
template<typename T, class Alloc = std::allocator<T> >
inline void matrixProductMatrixTVectorRef(const IrregArray2D<T, Alloc> &aT, const std::vector<T, Alloc> &v, std::vector<T, Alloc> &w)
{
    typedef typename IrregArray2D<T, Alloc>::value_type value_type;
    typedef typename IrregArray2D<T, Alloc>::index_type index_type;
 ...
 */
template<typename T, class Alloc = std::allocator<T> >
inline void matrixProductMatrixTVector(const IrregArray2D<T, Alloc> &aT, const std::vector<T, Alloc> &v, std::vector<T, Alloc> &w)
{
#if GMX_SIMD
    matrixProductMatrixTVectorSimd(aT, v, w);
#else
    matrixProductMatrixTVectorRef(aT, v, w);
#endif
}
...
inline
typename std::enable_if<!(gmx::SimdSetup<T>::doSimd_), void>::type
matrixProductSimd(const IrregArray2D<T, Alloc> &a, const IrregArray2D<T, Alloc> &b, IrregArray2D<T, Alloc> &c)
{
...
matrixProductSimd(const IrregArray2D<T, Alloc> &a, const IrregArray2D<T, Alloc> &b, IrregArray2D<T, Alloc> &c)
{
    ...
    for (index_type i = 0; i < m; ++i)
    {
        std::fill(c(i), c(i) + static_cast<ptrdiff_t>(n), (value_type)0);
    }
        ...
        for (index_type i = 0; i < m; ++i)
        {
            for (index_type k = 0; k < l; ++k)
            {
                ...
                for (index_type j = 0; j < simdBlockWidth; j += simdWidth)
                {
                    ...
                    bSimd = gmx::load(b(k) + j);
                    cSimd = gmx::load(c(i) + j);
 ...
 */
template<typename T, class Alloc = std::allocator<T> >
inline void matrixProductRef(const IrregArray2D<T, Alloc> &a, const IrregArray2D<T, Alloc> &b, IrregArray2D<T, Alloc> &c)
{
    typedef typename IrregArray2D<T, Alloc>::value_type value_type;
    typedef typename IrregArray2D<T, Alloc>::index_type index_type;
 ...
 */
template<typename T, class Alloc = std::allocator<T> >
inline void matrixProduct(const IrregArray2D<T, Alloc> &a, const IrregArray2D<T, Alloc> &b, IrregArray2D<T, Alloc> &c)
{
#if GMX_SIMD
    matrixProductSimd(a, b, c);
#else
    matrixProductRef(a, b, c);
#endif
}
 ...
 */
template<typename T, class Alloc = std::allocator<T> >
inline void matrixProductMatrixMatrix(const IrregArray2D<T, Alloc> &a, const IrregArray2D<T, Alloc> &b, IrregArray2D<T, Alloc> &c)
{
...
/*! \brief matrix product \f$ \mathbf{C} = \mathbf{A}\,\mathbf{B} \f$
    ...
    \param[in]   a        \f$ m \times l \f$ matrix \f$ \mathbf{A} \f$
    \param[in]   b        \f$ n \times l \f$ matrix \f$ \mathbf{B}^{\mathrm{T}} \f$
    \param[out]  c        the result: \f$ m \times  n \f$ matrix \f$ \mathbf{C} \f$
 */
template<typename T, class Alloc = std::allocator<T> >
inline
typename std::enable_if<!(gmx::SimdSetup<T>::doSimd_), void>::type
matrixProductMatrixMatrixTSimd(const IrregArray2D<T, Alloc> &a, const IrregArray2D<T, Alloc> &b, IrregArray2D<T, Alloc> &c)
{
    return matrixProductMatrixMatrixTRef(a, b, c);
}
...
matrixProductMatrixMatrixTSimd(const IrregArray2D<T, Alloc> &a, const IrregArray2D<T, Alloc> &b, IrregArray2D<T, Alloc> &c)
{
    ...
    const index_type simdBlockWidth  = (l > simdWidth ? l - l % simdWidth : 0);
    if (simdBlockWidth != 0)
    {
        // stores replicated elements of the row vectors of a
        simd_float_type aSimd(0);
        ...
        for (index_type i = 0; i < m; ++i)
        {
            for (index_type j = 0; j < n; ++j)
            {
                ...
                for (index_type k = 0; k < simdBlockWidth; k += simdWidth)
                {
                    ...
                    aSimd = gmx::load(a(i) + k);
                    bSimd = gmx::load(b(j) + k);
                    cSimd = gmx::fma(aSimd, bSimd, cSimd);
                }
...
/*! \brief matrix product \f$ \mathbf{C} = \mathbf{A}\,\mathbf{B} \f$,
           ...
           use SIMD vector operations for the marked parts that match the SIMD width
           Other variants of using SIMD operations for computing thr matrix product
           are conceivable, but this turned out to work exceptionally well, with
           speedups relative to the serial version much larger than one would expect
           from the SIMD width (up to ~24 x for T=double, AVX256, SIMD width = 4);
           Possible reasons: FMA operations are used, advantageous cache effects
           because contigous memory areas are traversed in the inner loops instead
           of a naive implementation that would traverse the columns of matrix B in
           the innermost loop?
 ...
 */
template<typename T, class Alloc = std::allocator<T> >
inline void matrixProductMatrixMatrixTRef(const IrregArray2D<T, Alloc> &a, const IrregArray2D<T, Alloc> &b, IrregArray2D<T, Alloc> &c)
{
    typedef typename IrregArray2D<T, Alloc>::value_type value_type;
    typedef typename IrregArray2D<T, Alloc>::index_type index_type;
 ...
 */
template<typename T, class Alloc = std::allocator<T> >
inline void matrixProductMatrixMatrixT(const IrregArray2D<T, Alloc> &a, const IrregArray2D<T, Alloc> &b, IrregArray2D<T, Alloc> &c)
{
#if GMX_SIMD
    matrixProductMatrixMatrixTSimd(a, b, c);
#else
    matrixProductMatrixMatrixTRef(a, b, c);
#endif
}
...
/*! \brief matrix product \f$ \mathbf{C} = \mathbf{A}\,\mathbf{B} \f$
    ...
    \param[in]   a        \f$ l \times m \f$ matrix \f$ \mathbf{A}^{\mathrm{T}} \f$
    \param[in]   b        \f$ n \times l \f$ matrix \f$ \mathbf{B}^{\mathrm{T}} \f$
    \param[out]  c        the result: \f$ m \times  n \f$ matrix \f$ \mathbf{C} \f$
 */
template<typename T, class Alloc = std::allocator<T> >
inline void
//typename std::enable_if<!(gmx::SimdSetup<T>::doSimd_), void>::type
matrixProductMatrixTMatrixTSimd(const IrregArray2D<T, Alloc> &a, const IrregArray2D<T, Alloc> &b, IrregArray2D<T, Alloc> &c)
{
    ...
    if (l * m < 1000)
    {
        return matrixProductMatrixTMatrixTRef(a, b, c);
    }
    else
    {
        array_type aT(0, m - 1, 0, l - 1);
        ...
        return matrixProductMatrixMatrixTSimd(aT, b, c);
    }
}
 ...
 */
template<typename T, class Alloc = std::allocator<T> >
inline void matrixProductMatrixTMatrixTRef(const IrregArray2D<T, Alloc> &a, const IrregArray2D<T, Alloc> &b, IrregArray2D<T, Alloc> &c)
{
    typedef typename IrregArray2D<T, Alloc>::value_type value_type;
    typedef typename IrregArray2D<T, Alloc>::index_type index_type;
 ...
 */
template<typename T, class Alloc = std::allocator<T> >
inline void matrixProductMatrixTMatrixT(const IrregArray2D<T, Alloc> &a, const IrregArray2D<T, Alloc> &b, IrregArray2D<T, Alloc> &c)
{
#if GMX_SIMD
    matrixProductMatrixTMatrixTSimd(a, b, c);
#else
    matrixProductMatrixTMatrixTRef(a, b, c);
#endif
}
...
inline
typename std::enable_if<!(gmx::SimdSetup<T>::doSimd_), void>::type
matrixProductMatrixTMatrixSimd(const IrregArray2D<T, Alloc> &a, const IrregArray2D<T, Alloc> &b, IrregArray2D<T, Alloc> &c)
{
    return matrixProductMatrixTMatrixRef(a, b, c);
}
 ...
 */
template<typename T, class Alloc = std::allocator<T> >
inline void matrixProductMatrixTMatrixRef(const IrregArray2D<T, Alloc> &a, const IrregArray2D<T, Alloc> &b, IrregArray2D<T, Alloc> &c)
{
    typedef typename IrregArray2D<T, Alloc>::value_type value_type;
    typedef typename IrregArray2D<T, Alloc>::index_type index_type;
 ...
 */
template<typename T, class Alloc = std::allocator<T> >
inline void matrixProductMatrixTMatrix(const IrregArray2D<T, Alloc> &a, const IrregArray2D<T, Alloc> &b, IrregArray2D<T, Alloc> &c)
{
#if GMX_SIMD
    matrixProductMatrixTMatrixSimd(a, b, c);
#else
    matrixProductMatrixTMatrixRef(a, b, c);
#endif
}
