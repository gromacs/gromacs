    ...
    public:
        ...
        class iterator
        {
            public:
                ...
                //! Value type
                using value_type = std::array<ptrdiff_t, Rank>;
                //! Difference type
                using difference_type = std::ptrdiff_t;
                //! Pointer type
                using pointer = std::array<ptrdiff_t, Rank> *;
                //! Reference type
                using reference = std::array<ptrdiff_t, Rank> &;
            ...
            beginIterator_
        {
            ...
            std::equal(std::begin(beginPosition), std::end(beginPosition),
                       std::begin(endPosition), std::less<ptrdiff_t>()) ? beginPosition : endPosition
        },
        endIterator_ {beginPosition, endPosition, endPosition}
        {}

        //! Construct a MultiDimIndexRange from a lattice extent, setting the begin to zero.
        explicit constexpr MultiDimIndexRange(const typename iterator::value_type &extent) :
            MultiDimIndexRange({0}, extent)
        {}
