...
class MdFloatVector
{
    public:
         ...
         */
        MdFloatVector(const real (&mdFloatVector)[Rank])
        {
...
class CanonicalVectorBasis
{
    public:
        ...
        /*! \brief Construct a basis from the length of the unit vectors.
         *
         * \param[in] lengths lengths of the basis vectors.
         */
        CanonicalVectorBasis(const real (&lengths)[Rank])
        {
            std::copy(std::begin(lengths), std::end(lengths), std::begin(basisVectorLengths_));
            for (size_t dim = 0; dim < Rank; ++dim)
            {
                basisVectorLengthsInverse_[dim] = 1./basisVectorLengths_[dim];
            }
