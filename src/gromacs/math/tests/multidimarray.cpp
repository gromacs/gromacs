...
class MultiDimArrayTest : public ::testing::Test
{
    ...
    protected:
        ...
        using array_type     = MultiDimArray<static_container_type, extents_type>;
        ...
        array_type         twoDArray_;
        dynamic_array_type twoDDyanmicArray_ {2, 2};
...
{
    MultiDimArray<std::array<float, 9>, extents_type> x;
...
{
    EXPECT_EQ(twoDDyanmicArray_.extent(0), 2);
    EXPECT_EQ(twoDDyanmicArray_.extent(1), 2);
}

TEST_F(MultiDimArrayTest, dynamicMultidDimArrayWorks)
{
    twoDDyanmicArray_(0, 1) = testNumber_;
    EXPECT_EQ(testNumber_, twoDDyanmicArray_(0, 1));
}
