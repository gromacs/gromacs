...
//! Test Matrix3x3 function for testing strictness of its descendants
void testMatrixMessUp(Matrix3x3 &dst)
{
    dst.setValue(XX, ZZ, 1.0); // Will mess up Matrix3x3Lower
    dst.setValue(ZZ, XX, 3.0); // Will mess up Matrix3x3Upper
}

TEST(Matrix3x3UpperTest, UpholdsTheConstraintAsMatrix3x3)
{
    // This tests that Matrix3x3Upper can be passed as Matrix3x3 &
    // and still uphold the constraint.
    std::vector<Matrix3x3Upper> v;
    matrix x = {{1, 2, 0}, {0, 5, 0}, {0, 0, 9}};
    v.emplace_back(x);
    EXPECT_DEATH(testMatrixMessUp(v[0]), "The matrix constraint was violated");
}
...
TEST(Matrix3x3LowerTest, UpholdsTheConstraintAsMatrix3x3)
{
    // This tests that Matrix3x3Lower can be passed as Matrix3x3 &
    // and still uphold the constraint.
    std::vector<Matrix3x3Lower> v;
