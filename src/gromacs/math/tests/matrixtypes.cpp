...
//! Matrix3x3 function for testing derived-to-base casting
void testMatrixCopy(const Matrix3x3 &src, matrix &dst)
{
    for (int i = 0; i < DIM; i++)
    {
        for (int j = 0; j < DIM; j++)
...
{
    // This tests that Matrix3x3Upper can be passed as const Matrix3x3 &.
    // The opposite (passing Matrix3x3 as const Matrix3x3Upper &) will not compile.
    std::vector<Matrix3x3Upper>      v2;
    matrix                           x = {{1, 2, 3}, {0, 5, 6}, {0, 0.0, 9}};
    v2.emplace_back(x);
    matrix v;
...
{
    // This tests that Matrix3x3Lower can be passed as const Matrix3x3 &.
    // The opposite (passing Matrix3x3 as const Matrix3x3Lower &) will not compile.
    std::vector<Matrix3x3Lower> v2;
    matrix                      x = {{1, 0, 0}, {4, 5, 0}, {7, 8, 9}};
    v2.emplace_back(x);
    matrix                      v;
    testMatrixCopy(v2[0], v);
