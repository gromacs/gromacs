...
typedef ::testing::Types<
        ...
        ArrayRefWithPadding<const float>,
        ArrayRefWithPadding<double>/*,
        ArrayRefWithPadding<const int32_t>,
        ArrayRefWithPadding<const double>*/
        > ArrayRefTypes;
...
class ArrayRefWithPaddingTest : public ::testing::Test
{
    public:
        ...
        typedef PaddedVector<NonConstValueType> PaddedVectorType;
        ...
        void runTests(ValueType     */*aData*/,
                      ArrayRefType  &arrayRefWithPadding)
        {
            ASSERT_EQ(aSize, arrayRefWithPadding.size());
...
{
    typename TestFixture::PaddedVectorType v(aSize);
    std::copy(this->a, this->a + aSize, v.begin());
...
TYPED_TEST(ArrayRefWithPaddingTest, ConstructFromPointersWorks)
{
    ...
    typename TestFixture::ArrayRefType arrayRef(v.data(), v.data() + v.size(), v.data() + v.paddedSize());
    this->runTests(this->a, arrayRef);
}
