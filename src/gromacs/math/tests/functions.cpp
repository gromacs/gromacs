...
TEST(FunctionTest, Log2I32Bit)
{
    ...
    {
        EXPECT_EQ(static_cast<unsigned int>(std::log2(i)), gmx::log2I(i))
        << "Error in log2 values when only using the lowest 32 bits of 32-bit value." << std::endl;
    }
    for (std::uint32_t i = 0x7FFFFFF0; i <= 0xFFFFFFFE; i++)
    {
        EXPECT_EQ(static_cast<unsigned int>(std::log2(i)), gmx::log2I(i))
        << "Error in log2 values when using the very highest bits of 32-bit value." << std::endl;
    }
}

TEST(FunctionTest, Log2I64Bit)
{
    for (std::uint64_t i = 0; i < 0xFF; i++)
    {
        EXPECT_EQ(static_cast<unsigned int>(std::log2(i)), gmx::log2I(i))
        << "Error in log2 values when only using the lowest 32 bits of 64-bit value." << std::endl;
    }
    for (std::uint64_t i = 0x000000007FFFFFF0ULL; i <= 0x0000000100000001ULL; i++)
    {
        EXPECT_EQ(static_cast<unsigned int>(std::log2(i)), gmx::log2I(i))
        << "Error in log2 values when swapping from low to high 32 bits of 64-bit value." << std::endl;
    }
    for (std::uint64_t i = 0x7FFFFFFFFFFFFFF0ULL; i <= 0xFFFFFFFFFFFFFFFEULL; i++)
    {
        EXPECT_EQ(static_cast<unsigned int>(std::log2(i)), gmx::log2I(i))
        << "Error in log2 values when using the very highest bits of 64-bit value." << std::endl;
    }
}
