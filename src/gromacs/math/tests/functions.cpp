...
TEST(FunctionTest, FastsqrtFloat)
{
    ...
    for (float f = 0; f < 10.0; f += 1.0)
    {
        result.push_back(gmx::fastsqrt(f));
    }
    checker.checkSequence(result.begin(), result.end(), "FastsqrtFloat");
}
...
TEST(FunctionTest, FastsqrtDouble)
{
    ...
    {
        result.push_back(gmx::fastsqrt(d));
    }
    checker.checkSequence(result.begin(), result.end(), "FastsqrtDouble");
}
...
TEST(FunctionTest, FastsqrtInteger)
{
    ...
    for (int i = 0; i < 10; i++)
    {
        result.push_back(gmx::fastsqrt(i));
    }
    checker.checkSequence(result.begin(), result.end(), "FastsqrtInteger");
}
