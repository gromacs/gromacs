...
using Implementations = ::testing::Types< std::allocator<int>,
                                          std::allocator<float>,
...
TYPED_TEST(PaddedVectorTest, ConstructsResizesAndReserves)
{
    ...
    EXPECT_EQ(v.unpaddedSize(), v.unpaddedArrayRef().size());
    EXPECT_EQ(v.paddedSize(),   v.paddedArrayRef().size());
    EXPECT_LE(v.unpaddedSize(), v.paddedArrayRef().size());

    VectorType vReserved;
    vReserved.reserveWithPadding(5);
    fillInput(&vReserved, 1);

    EXPECT_EQ(vReserved.unpaddedSize(), vReserved.unpaddedArrayRef().size());
    EXPECT_EQ(vReserved.paddedSize(),   vReserved.paddedArrayRef().size());
    EXPECT_LE(vReserved.unpaddedSize(), vReserved.paddedArrayRef().size());
...
TYPED_TEST(PaddedVectorTest, CanCopyAssign)
{
    ...
    compareViews(v.unpaddedArrayRef(), w.unpaddedArrayRef()); 
}
...
TYPED_TEST(PaddedVectorTest, CanMoveAssign)
{
    ...
    compareViews(v.unpaddedArrayRef(), x.unpaddedArrayRef()); 
