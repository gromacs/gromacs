...
/*! \libinternal \file
 * \brief
 * Declares gmx::SimdRVecMemory
 *
 * \author Mark Abraham <mark.j.abraham@gmail.com>
 * \ingroup module_math
 */
// TODO eventually this will replace paddedvector.h
#ifndef GMX_MATH_SIMDRVECMEMORY_H
#define GMX_MATH_SIMDRVECMEMORY_H
...
//! \libinternal Type trait to support SimdMemory
template <typename T>
struct SimdMemoryTypeTraits {};

//! \libinternal Type trait to support SimdMemory<real>.
template <>
...
//! \libinternal Type trait to support SimdMemory<RVec>.
template <>
 ...
 * getUnpaddedArrayRef()[size()-1] (used in bonded and constraint
 * code). This is simply ensured by making sure that the padded region
 * has at least as many bytes as \c s_simdWidth.
 *
 ...
 * either an ArrayRef of T or its SimdType version (typically
 * SimdReal), depending on the needs of that code.
 *
 * Internally, the memory is a std::vector of SimdType objects, so
 * that tooling that understands std::vector will work correctly.
 */
template <typename T>
class SimdMemory
{
    private:
        ...
        using StorageType = std::vector<SimdType, AlignedAllocator<SimdType> >;

        /*! \brief Helper function for computing the padded size.
         *
         * Internally, the storage is maintained as SimdTypes. */
        SizeType computePaddedSize(const SizeType numElements)
        {
            ...
            SizeType           maxBaseTypeNeeded = (numElements * s_packSize + 2 * s_simdWidth);
            SizeType           numSimdTypeNeeded = maxBaseTypeNeeded / s_simdWidth;
            return numSimdTypeNeeded;
        }
    ...
    public:
        ...
        SimdMemory() :
            storage_(),
            unpadded_end_(reinterpret_cast<PointerType>(storage_.data()))
        {}
        //! Constructor.
        explicit SimdMemory(const SizeType numElements) :
            storage_(computePaddedSize(numElements)),
            unpadded_end_(reinterpret_cast<PointerType>(storage_.data()) + numElements)
        {}
        //! Constructor.
        explicit SimdMemory(SimdMemory const &o) :
            storage_(o.storage_),
            unpadded_end_(reinterpret_cast<PointerType>(storage_.data()) + o.size())
        {}
        //! Constructor.
        ...
        {}
        //! Reserves enough storage for \c numElements ValueTypes, but does not initialize it.
        void reserveWithPadding(const SizeType numElements)
        {
            ...
            auto newNumSimdType = computePaddedSize(numElements);
            storage_.reserve(newNumSimdType);
            unpadded_end_ = reinterpret_cast<PointerType>(storage_.data()) + numElements;
        }
        //! Resizes the storage to fit \c numElements ValueTypes and default-initializes it.
        void resizeWithPadding(const SizeType numElements)
        {
            auto newNumSimdType = computePaddedSize(numElements);
            storage_.resize(newNumSimdType);
            unpadded_end_ = reinterpret_cast<PointerType>(storage_.data()) + numElements;
        }
        //! Returns the size of the storage in ValueType-sized objects.
        SizeType size() const { return unpadded_end_ - reinterpret_cast<const PointerType>(storage_.data()); }
        ...
        //! Returns a view of the storage in ValueTypes, without padding.
        ArrayRef<ValueType> getUnpaddedArrayRef()
        {
            return arrayRefFromPointers<ValueType>(reinterpret_cast<PointerType>(storage_.data()), unpadded_end_);
        }
        //! Returns a const view of the storage in ValueTypes, without padding.
        ConstArrayRef<ValueType> getUnpaddedConstArrayRef() const
        {
            return constArrayRefFromPointers<ValueType>(reinterpret_cast<const PointerType>(storage_.data()), unpadded_end_);
        }
        //! Returns a view of the storage in SimdType, with any necessary padding.
        ArrayRef<SimdType> getPaddedArrayRef()
        {
            // TODO Check codegen, maybe we care about performance
            return arrayRefFromArray<SimdType>(storage_.data(), storage_.size());
        }
        //! Returns a const view of the storage in SimdType, with any necessary padding.
        ConstArrayRef<SimdType> getPaddedConstArrayRef() const
        {
            return constArrayRefFromArray<SimdType>(storage_.data(), storage_.size());
        }
        ...
        //! Let calling code handle initialize the padding region to \c value.
    ...
    private:
        ...
        //! Points to the end of the range of ValueTypes for which storage was requested, ie. before any padding.
