...
{
    public:
        void testIsWithin(const NeighborhoodSearchTestData &data);
        void testMinimumDistance(const NeighborhoodSearchTestData &data);
        void testNearestPoint(const NeighborhoodSearchTestData &data);
        void testPairSearch(const NeighborhoodSearchTestData &data);
        ...
        const NeighborhoodSearchTestData &data)
{
    ...
    for (i = data.testPositions_.begin(); i != data.testPositions_.end(); ++i)
    {
        ...
        EXPECT_EQ(bWithin, nbsearch_.isWithin(i->x))
        ...
        const NeighborhoodSearchTestData &data)
{
    ...
    for (i = data.testPositions_.begin(); i != data.testPositions_.end(); ++i)
    {
        ...
        EXPECT_NEAR_REL(refDist, nbsearch_.minimumDistance(i->x), 20*GMX_REAL_EPS);
    }
}
        ...
        const NeighborhoodSearchTestData &data)
{
    ...
    for (i = data.testPositions_.begin(); i != data.testPositions_.end(); ++i)
    {
        const gmx::AnalysisNeighborhoodPair pair = nbsearch_.nearestPoint(i->x);
        if (pair.isValid())
        {
        ...
        const NeighborhoodSearchTestData &data)
{
    ...
    for (i = data.testPositions_.begin(); i != data.testPositions_.end(); ++i)
    {
        ...
        gmx::AnalysisNeighborhoodPairSearch search =
            nbsearch_.startPairSearch(i->x);
        gmx::AnalysisNeighborhoodPair       pair;
        while (search.findNextPair(&pair))
        {
            EXPECT_EQ(0, pair.secondIndex());
...
TEST_F(NeighborhoodSearchTest, SimpleSearch)
{
    ...
    nb_.setParameters(data.cutoff_, data.refPosCount_);
    nb_.setMode(gmx::AnalysisNeighborhood::eSearchMode_Simple);
    nbsearch_ = nb_.initSearch(&data.pbc_, data.refPosCount_, data.refPos_);
    ASSERT_EQ(gmx::AnalysisNeighborhood::eSearchMode_Simple, nbsearch_.mode());

    testIsWithin(data);
    testMinimumDistance(data);
    testNearestPoint(data);
    testPairSearch(data);
}
...
TEST_F(NeighborhoodSearchTest, GridSearchBox)
{
    ...
    nbsearch_ = nb_.initSearch(&data.pbc_, data.refPosCount_, data.refPos_);
    ASSERT_EQ(gmx::AnalysisNeighborhood::eSearchMode_Grid, nbsearch_.mode());
