 ...
 */
static void convert_value(SelectionParserValue *value, e_selvalue_t type,
 ...
 */
static int cmp_int_range(const void *a, const void *b)
{
    if (((int *)a)[0] < ((int *)b)[0])
    {
        return -1;
 ...
 * secondarily based on length (longer ranges come first).
 */
static int cmp_real_range(const void *a, const void *b)
{
    ...
    else
    {
        if (n != param->val.nr)
        {
            ...
            GMX_THROW(InvalidInputError(
                    "Only one value or 'A to B' range is "
                    "supported in this context"));
        }
        if (param->val.type == INT_VALUE)
        {
    ...
    for (value = values.begin(); value != values.end(); ++value)
    {
        ...
        switch (param->val.type)
        {
            ...
            case STR_VALUE:
                ...
                break;
            case POS_VALUE:
                copy_rvec(value->u.x, param->val.u.p->x[i++]);
                break;
 ...
 */
static SelectionTreeElementPointer add_child(const SelectionTreeElementPointer &root, gmx_ana_selparam_t *param,
                                 ...
                                 gmx_ana_selparam_t *param, int i, void *scanner)
{
    ...
    {
        case INT_VALUE:
            sel->v.u.i = &param->val.u.i[i];
            break;
        case REAL_VALUE:
            sel->v.u.r = &param->val.u.r[i];
            break;
        case STR_VALUE:
            sel->v.u.s = &param->val.u.s[i];
            break;
        case POS_VALUE:
            sel->v.u.p = &param->val.u.p[i];
            break;
        case GROUP_VALUE:
            sel->v.u.g = &param->val.u.g[i];
            break;
        default: /* Error */
