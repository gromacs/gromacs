...
static real get_mass(const gmx_mtop_atomlookup_t lookup, int atomnr)
{
    t_atom *atom;
    gmx_mtop_atomnr_to_atom(lookup, atomnr, &atom);
    return atom->m;
}
...
gmx_calc_com(const gmx_mtop_t *top, rvec x[], int nrefat, const int index[], rvec xout)
{
    ...
    for (int m = 0; m < nrefat; ++m)
    {
        ...
        const real mass = get_mass(lookup, ai);
        for (int j = 0; j < DIM; ++j)
        {
            xout[j] += mass * x[ai][j];
        }
 ...
 */
void
gmx_calc_cog_pbc(const gmx_mtop_t *top, rvec x[], t_pbc *pbc,
 ...
 */
void
gmx_calc_com_pbc(const gmx_mtop_t *top, rvec x[], t_pbc *pbc,
                 int nrefat, const int index[], rvec xout)
{
    GMX_RELEASE_ASSERT(top != nullptr,
                       "No masses available while mass weighting was requested");
    ...
    if (pbc)
    {
        ...
        do
        {
            ...
            for (int m = 0; m < nrefat; ++m)
            {
                rvec       dx, xtest;
                const int  ai   = index[m];
                const real mass = get_mass(lookup, ai) / mtot;
                pbc_dx(pbc, x[ai], xout, dx);
                rvec_add(xout, dx, xtest);
                for (int j = 0; j < DIM; ++j)
                {
 ...
 */
void
gmx_calc_comg_pbc(const gmx_mtop_t *top, rvec x[], t_pbc *pbc,
