...
cl_R Slater_5S_6S(cl_R r,cl_R xi,cl_R xj)
{
  ...
  rxj = r*xj;
  if (xi == xj) {
        if (r == 0LL) {  S = (235451LL*xi)/1.572864e6

    ; } else {  S = (1LL/r)*((-(1e9*cl_float(153272826,precision)+515128320000LL) + (1e9*cl_float(153272826,precision)+515128320000LL)*exp(2LL*rxi) - 
      ...
      if (r == 0LL) { S = (xi*xj*(5LL*Power(xi,20LL) + 105LL*Power(xi,19LL)*xj + 
    ...
    ; } else { S = (1LL/r)*((4677750LL*exp(2LL*(rxi + rxj))*Power(Power(rxi,2LL) - Power(rxj,2LL),21LL) + 
