...
class not_null
{
    public:
        ...
        template <typename = enable_if_t<!std::is_same<std::nullptr_t, T>::value> >
        ...
        constexpr remove_pointer_t<T> operator*() const { return *get(); }
        //! \}
...
{
    return not_null < remove_cv_t < remove_reference_t<T>>>{
               std::forward<T>(t)
    };
}
