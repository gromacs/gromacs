...
/*! \internal \file
 *  \brief
 *  Explicitly instantiate NBNXM HIP Foreign FEP kernels, all flavors
 *
                                  ...
                                  const int           nLambda)
{
    ...
    {
        const int nj0      = __shfl(gm_jIndex[wid_global], 0, c_parallelExecutionWidth);
        const int nj1      = __shfl(gm_jIndex[wid_global + 1], 0, c_parallelExecutionWidth);
        const int ai       = __shfl(gm_iinr[wid_global], 0, c_parallelExecutionWidth);
        const int shiftIdx = __shfl(gm_shift[wid_global], 0, c_parallelExecutionWidth);
        ...
        for (int i = nj0; i < nj1; i += c_parallelExecutionWidth)
        {
            ...
            r2                            = fmaxf(r2, c_nbnxmMinDistanceSquared);
                    ...
                    for (int k = 0; k < 2; k++)
                    {
                        ...
                        if (nonZeroState)
                        {
                            ...
                            if (c6c12AB[k].x != 0.0F || c6c12AB[k].y != 0.0F)
                            {
                                ...
                                {
                                    ljForceSwitch<true, true>(dispersionShift,
                                                              repulsionShift,
                                                              rVdwSwitch,
                                                              c6c12AB[k].x,
                                                              c6c12AB[k].y,
...
void launchNbnxmFepForeignKernel(NbnxmGpu* nb, const InteractionLocality iloc)
{
    ...
    gmx::dispatchTemplatedFunction(
            ...
            {
                constexpr int c_parallelExecutionWidth = sc_gpuParallelExecutionWidth(sc_layoutType);
                ...
                auto kernelPtr = nbfeForeignKernel<elecType_, vdwType_, useSoftCore_, sc_layoutType>;
                auto kernelName =
                        getFepForeignKernelName<elecType_, vdwType_, useSoftCore_, sc_layoutType>();
                auto kernelArgs =
                        prepareGpuKernelArguments(kernelPtr, config, atdat, nbparam, feplist, &nLambda);
