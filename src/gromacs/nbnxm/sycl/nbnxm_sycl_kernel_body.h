 ...
 *
 */
template<PairlistType pairlistType, bool doPruneNBL, bool doCalcEnergies, enum ElecType elecType, enum VdwType vdwType, typename CommandGroupHandler>
                        ...
                        const bool            calcShift)
{
    ...
    return [=](sycl::nd_item<3> itemIdx) [[sycl::reqd_sub_group_size(subGroupSize)]]
    {
        ...
        {
            auto*    plistCJPacked = indexedAddress(gm_plistCJPacked, jPacked);
...
static void launchNbnxmKernel(const DeviceStream& deviceStream, const int numSci, Args&&... args)
{
    ...
    auto kernelFunctionBuilder =
            nbnxmKernel<pairlistType, doPruneNBL, doCalcEnergies, elecType, vdwType, CommandGroupHandler>;
