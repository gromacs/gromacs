...
void gpu_init_atomdata(NbnxmGpu* nb, const nbnxm_atomdata_t* nbat)
{
    ...
    if constexpr (gmx::GpuConfigurationCapabilities::NonbondedFE)
    {
        if (bFepGpuNonBonded)
        {
            ...
            {
                nb->fephostdata->q4Host[4 * k]     = nbat->params().qA[k];
                nb->fephostdata->q4Host[4 * k + 1] = nbat->params().qB[k];
            }
            ...
            if (useLjCombRule(nb->nbparam->vdwType))
            {
                ...
                for (int k = 0; k < numAtoms; k++)
                {
                    nb->fephostdata->ljComb4Host[4 * k]     = nbat->params().ljCombA[2 * k];
                    nb->fephostdata->ljComb4Host[4 * k + 1] = nbat->params().ljCombA[2 * k + 1];
                    nb->fephostdata->ljComb4Host[4 * k + 2] = nbat->params().ljCombB[2 * k];
                    nb->fephostdata->ljComb4Host[4 * k + 3] = nbat->params().ljCombB[2 * k + 1];
                }
