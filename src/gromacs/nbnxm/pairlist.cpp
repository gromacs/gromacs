...
template<PairlistType layoutType>
static std::vector<NbnxnPairlistGpu<layoutType>> createGpuPairlists(int numLists, PinningPolicy pinPolicy)
{
    auto lists = std::vector<NbnxnPairlistGpu<layoutType>>();
    /* Only list 0 is used on the GPU, use normal allocation for i>0 */
    lists.emplace_back(NbnxnPairlistGpu<layoutType>{ pinPolicy });
    ...
    isCpuType_(!sc_isGpuSpecificPairlist(pairlistParams.pairlistType))
{
    ...
    else
    {
        gmx::dispatchTemplatedFunction(
                [&](auto layoutType_)
                {
                    if constexpr (sc_isGpuSpecificPairlist(layoutType_))
                    {
                        gpuLists_ = createGpuPairlists<layoutType_>(numLists, pinPolicy);
                    }
                },
