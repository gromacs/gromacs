               ...
               matrix                   box)
{
    ...
    NbnxnListParameters listParams(kernelSetup.kernelType, ir->rlist);
    ...
    return std::make_unique<nonbonded_verlet_t>(&pairlistSets, &nbs, &nbat, kernelSetup, gpu_nbv);
}

} // namespace Nbnxm

nonbonded_verlet_t::nonbonded_verlet_t(std::unique_ptr<PairlistSets>     *pairlistSets,
                                       std::unique_ptr<nbnxn_search>     *nbs_ptr,
                                       std::unique_ptr<nbnxn_atomdata_t> *nbat_ptr,
                                       const Nbnxm::KernelSetup          &kernelSetup,
                                       gmx_nbnxn_gpu_t                   *gpu_nbv_ptr) :
    kernelSetup_(kernelSetup)
{
    GMX_RELEASE_ASSERT(pairlistSets && *pairlistSets, "Need valid pairlistSets");
    GMX_RELEASE_ASSERT(nbs_ptr && *nbs_ptr, "Need valid search object");
    GMX_RELEASE_ASSERT(nbat_ptr && *nbat_ptr, "Need valid atomdata object");

    pairlistSets_ = std::move(*pairlistSets);
    nbs           = std::move(*nbs_ptr);
    nbat          = std::move(*nbat_ptr);
