...
static constexpr int c_xStride4xN =
        std::max(GMX_SIMD_REAL_WIDTH, sc_iClusterSize(KernelType::Cpu4xN_Simd_4xN));
                                        ...
                                        NbnxnPairlistCpuWork* work)
{
    ...
    const int ia = xIndexFromCi<KernelType::Cpu4xN_Simd_4xN>(ci);
                                          ...
                                          int* gmx_restrict        numDistanceChecks)
{
    ...
    int jclusterFirst = cjFromCi<KernelType::Cpu4xN_Simd_4xN, 0>(firstCell);
    int jclusterLast  = cjFromCi<KernelType::Cpu4xN_Simd_4xN, 1>(lastCell);
    ...
    while (!InRange && jclusterFirst <= jclusterLast)
    {
        ...
        *numDistanceChecks += 2;
        ...
        {
            const int xind_f = xIndexFromCj<KernelType::Cpu4xN_Simd_4xN>(
                    cjFromCi<KernelType::Cpu4xN_Simd_4xN, 0>(jGrid.cellOffset()) + jclusterFirst);
    ...
    while (!InRange && jclusterLast > jclusterFirst)
    {
        ...
        *numDistanceChecks += 2;
        ...
        {
            const int xind_l = xIndexFromCj<KernelType::Cpu4xN_Simd_4xN>(
                    cjFromCi<KernelType::Cpu4xN_Simd_4xN, 0>(jGrid.cellOffset()) + jclusterLast);
    ...
    if (jclusterFirst <= jclusterLast)
    {
        for (int jcluster = jclusterFirst; jcluster <= jclusterLast; jcluster++)
        {
            ...
            nbnxn_cj_t cjEntry;
            cjEntry.cj   = cjFromCi<KernelType::Cpu4xN_Simd_4xN, 0>(jGrid.cellOffset()) + jcluster;
