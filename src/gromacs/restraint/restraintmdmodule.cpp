...
RestraintMDModuleImpl::RestraintMDModuleImpl(std::shared_ptr<IRestraintPotential>  restraint,
    ...
    forceProvider_(compat::make_unique<RestraintForceProvider>(restraint,
                                                               sites)),
    outputProvider_(compat::make_unique<RestraintOutputProvider>()),
    optionProvider_(compat::make_unique<RestraintOptionProvider>())
{
    GMX_ASSERT(forceProvider_, "Class invariant implies non-null ForceProvider.");
    GMX_ASSERT(outputProvider_, "Class invariant implies non-null OutputProvider.");
    GMX_ASSERT(optionProvider_, "Class invariant implies non-null OptionProvider.");
}
                          ...
                          const std::vector<unsigned long int> &sites)
{
    auto implementation = compat::make_unique<RestraintMDModuleImpl>(std::move(restraint),
                                                                     sites);
    std::unique_ptr<RestraintMDModule> newModule {
