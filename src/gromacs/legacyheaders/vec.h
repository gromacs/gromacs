...
static gmx_inline void mtmul(const matrix a, const matrix b, matrix dest)
{
...
static gmx_inline void m_inv_ur0(const matrix src, matrix dest)
{
...
static gmx_inline void mvmul_ur0(const matrix a, const rvec src, rvec dest)
{
    dest[ZZ] = a[ZZ][XX]*src[XX]+a[ZZ][YY]*src[YY]+a[ZZ][ZZ]*src[ZZ];
...
static void m_rveccopy(int dim, const rvec *a, rvec *b)
{
    /* b = a */
    int i;
...
static void matrix_convert(matrix box, const rvec vec, const rvec angle_in_degrees)
{
    rvec angle;
    svmul(DEG2RAD, angle_in_degrees, angle);
    box[XX][XX] = vec[XX];
