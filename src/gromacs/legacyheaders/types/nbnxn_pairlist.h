...
/* We currently store the reduction flags as bits in an unsigned 64bit.
 * This limits the number of flags to 64.
 * As we anyhow don't expect reasonable performance with
 * more than 64 threads, we put in this hard limit.
 */
#define NBNXN_BUFFERFLAG_MAX_THREADS  64

/* Flags for telling if threads write to force output buffers */
typedef struct {
    int               nflag;       /* The number of flag blocks                         */
    gmx_large_uint_t *flag;        /* Bit i is set when thread i writes to a cell-block */
