...
#define PAR(cr)        ((NULL != (cr)) && (cr)->nnodes > 1)

//! True of this is the master node
#define MASTER(cr)     ((NULL == (cr)) || ((cr)->nodeid == 0) || !PAR(cr))
...
#define DOMAINDECOMP(cr)   (NULL != (cr) && (NULL != (cr)->dd) && PAR(cr))

//! Are we doing multiple independent simulations
#define MULTISIM(cr)       (NULL != (cr) && (NULL != (cr)->ms))

//! Are we the master node of a multisimulation
#define MASTERSIM(ms)      (NULL != (ms) && ((ms)->sim == 0))
