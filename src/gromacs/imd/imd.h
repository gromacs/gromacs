...
extern void write_IMDgroup_to_file(gmx_bool bIMD, t_inputrec *ir, t_state *state, gmx_mtop_t *sys, const char *fn);

extern void dd_make_local_IMD_atoms(gmx_bool bIMD, gmx_domdec_t *dd, t_IMD *imd);

extern void IMD_init(
        t_inputrec    *ir,
        t_commrec     *cr,
...
extern void IMD_prepare_energies(gmx_bool bIMD, t_gmx_IMD IMDSetup, gmx_enerdata_t *enerd, int step, gmx_bool bHaveNewEnergies);

extern void IMD_send_positions(t_IMD *imd);

extern void IMD_prep_energies_send_positions(t_IMD *imd, gmx_enerdata_t *enerd, gmx_int64_t step, gmx_bool bCalcEner, gmx_wallcycle_t wcycle);
