...
/*! \defgroup module_imd Interactive molecular dynamics
 * \ingroup group_mdrun
 *
 * \brief
 ...
 *
 * \brief
 * Enum for types of IMD messages. We use the same records as the NAMD/VMD IMD implementation.
 *
 */
 ...
 * \param md           Parameters of the MD atoms.
 * \param nfile        Number of files.
 * \param fnm          Standard filename struct.
 * \param oenv         Output options.
 * \param imdport      Port to use for IMD connections.
 * \param imdfreq      Initial IMD update (=communication) frequency.
 * \param Flags        Flags passed over from main, used to determine
 *                     whether or not we are appending.
 */
extern void IMD_init(t_inputrec *ir, t_commrec *cr, gmx_mtop_t *top_global,
                     FILE *fplog, int defnstimd, rvec x[], t_mdatoms *md,
                     int nfile, const t_filenm fnm[], output_env_t oenv,
                     int imdport, int imdfreq, unsigned long  Flags);
 ...
 *
 */
extern void IMD_prepare_energies(gmx_bool bIMD, t_IMD *imd, gmx_enerdata_t *enerd,
 ...
 *
 */
extern void IMD_prep_energies_send_positions(t_IMD *imd, gmx_enerdata_t *enerd, gmx_int64_t step,
                                             gmx_bool bHaveNewEnergies, gmx_wallcycle_t wcycle);
