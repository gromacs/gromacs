...
typedef struct gmx_IMD
{
    ...
    gmx_int32_t    *vmd_f_ind;       /* VMD indices                                */
    ...
    rvec           *sendxbuf;        /* buffer to make mols. whole before sending  */
...
/* Write out the group of atoms selected for interactive manipulation. Called by grompp */
...
/* Send positions from rvec, therefore we need a separate send buffer and conversion to Angstrom */
...
static t_gmx_IMD_setup* imd_create(int imdatoms, int nstimddef)
{
...
/* This wraps tryconnect in order to make it "blocking". Used when the simulation
 * should wait for an incoming connection (GMX_IMDWAIT). */
...
/* Copy IMD forces to MD forces, i.e. do conversion from Cal->Joule and from
...
static void imd_readcommand(t_gmx_IMD_setup *IMDsetup)
{
    ...
    while (IMDsetup->clientsocket && (imdsock_tryread(IMDsetup->clientsocket, 0, 0) > 0 || IMDpaused))
    {
        ...
        switch (itype)
        {
            /* IMD asks us to terminate the simulation, check if the user allowed
             * this via the GMX_IMDTERM variable. */
            case IMD_KILL:
        ...
        unsigned long  Flags)
{
...
#ifndef GMX_IMD
    /* All IMD entry routines are within "if (ir->bIMD)" conditionals.
     * If IMD is not supported, we set bIMD to FALSE so that the IMD routines
     * directly return to the calling function. */
    ir->bIMD = FALSE;
    ...
    if (MASTER(cr))
    {
        ...
        if ((imdport) || (Flags & MD_IMDWAIT) || (Flags & MD_IMDTERM) || (Flags & MD_IMDPULL))
        {
            bIMDenabled = TRUE;
        }
    ...
    ir->imd->setup = imd_create(ir->imd->nat > 0 ? ir->imd->nat : nat_total, defnstimd);
    IMDsetup       = ir->imd->setup;

    /* We might need to open an output file for IMD forces data */
    if (MASTER(cr))
    {
        IMDsetup->outf = open_imd_out(fn, ir->imd->setup, nat_total, oenv, Flags);
    }
 ...
 * Also checks for new IMD connection and synchronizes the nodes. */
        ...
        gmx_wallcycle_t wcycle)
{
    ...
    if (imdstep)
    {
         ...
         * and put molecules back into the box if necessary */
        if (IMDsetup->bConnected)
        {
            ...
            communicate_group_positions(cr, IMDsetup->xa, IMDsetup->xa_shifts, IMDsetup->xa_eshifts, bNS,
                                        x, IMDsetup->nat, IMDsetup->nat_loc, IMDsetup->ind_loc,
                                        IMDsetup->xa_ind, IMDsetup->xa_old, box);

            /* Avoid extra-communication before next NS step. */
    ...
    else
    {
        if (bNS)
        {
            /* We already communicated the positions since last NS step */
...
extern void IMD_prep_energies_send_positions(t_IMD *imd, gmx_enerdata_t *enerd,
                                             gmx_int64_t step, gmx_bool bHaveNewEnergies,
                                             gmx_wallcycle_t wcycle)
{
    wallcycle_start(wcycle, ewcIMD);
    ...
    IMD_prepare_energies(TRUE, imd, enerd, step, bHaveNewEnergies);
...
/* Apply the interactive pulling forces, if bIMD is TRUE. */
