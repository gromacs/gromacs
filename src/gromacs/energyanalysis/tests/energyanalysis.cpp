                  ...
                  gmx::test::TestReferenceChecker &checker)
{
    ...
    for (int i = 0; (i < nColumn); i++)
    {
        ...
        {
            char buf[32];
            snprintf(buf, sizeof(buf), "result%d", i);
            checker.checkSequenceArray(result[i].size(), &result[i][0], buf);
        }
    }
}
...
class SimpleTest : public gmx::test::CommandLineTestBase
{
    public:
        ...
        void runTest(const CommandLine &args, const char *option, int nColumn)
        {
            ...
            checkXvgFile(getOutputFile(option), nColumn, 1e-3, rootChecker);
        }
};
