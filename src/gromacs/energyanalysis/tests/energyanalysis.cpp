...
#include "../modules/freeenergydifference.h"
...
class EnergyanalysisTest : public ::testing::Test
{
    protected:
        ...
        void test(gmx::EnergyAnalysisModulePointer energyanalysis, char * option[], int nrOptions, const std::string testName)
        {
        ...
        void testResult(std::string testName)
        {
            ...
            for (int colum = 1; colum  < nrColums; colum++)
            {
                ...
                {
                    checker.setDefaultTolerance(gmx::test::relativeToleranceAsFloatingPoint(1, percentBound));
...
TEST_F (EnergyanalysisTest, fluctProps) {
    ...
    gmx::EnergyAnalysisModulePointer energyanalysis = info.create();
...
TEST_F (EnergyanalysisTest, viscosity) {
    ...
    std::string option2 = gmx::test::TestFileManager::getInputFilePath((testName + ".xvg").c_str());
    option[curentOption++] = (char *)"-evis"; //option does not work atm Anders Gärdenäs 13-08-2014
    option[curentOption++] = const_cast<char *>(option2.c_str());
