...
class EnergyTest : public gmx::test::CommandLineTestBase
{
    public:
        ...
        void runTest(const CommandLine                &args,
                     ...
                     gmx::EnergyAnalysisModulePointer  (*module)())
        {
            ...
            gmx::ICommandLineOptionsModulePointer runner(gmx::EnergyAnalysisRunner::createModule(module()));
            ASSERT_EQ(0, gmx::test::CommandLineTestHelper::runModuleDirect(
                              std::move(runner), &cmdline));
            checkOutputFiles();
        }
};
...
TEST_F(EnergyTest, ExtractEnergy)
{
    ...
    setOutputFile("-o", "energy.xvg", ExactTextMatch());
    runTest(CommandLine(cmdline), tolerance, &EnergyInfo::create);
}
...
TEST_F(EnergyTest, ExtractFluctuations)
{
    ...
    double            tolerance = 0.001;
    setOutputFile("-convergence", "convergence.xvg", ExactTextMatch());
...
TEST_F(EnergyTest, EinsteinViscosity)
{
    ...
    setOutputFile("-evis", "einstein.xvg", ExactTextMatch());
...
TEST_F(EnergyTest, EinsteinViscosityIntegral)
{
    const char *const cmdline[] = { "viscosity" };
    double            tolerance = 0.1;
    setOutputFile("-eivis", "einsteinintegral.xvg", ExactTextMatch());
    runTest(CommandLine(cmdline), tolerance, &ViscosityInfo::create);
}
...
TEST_F(EnergyTest, FanourgakisViscosity)
{
    const char *const cmdline[] = { "viscosity" };
    double            tolerance = 0.1;
    setOutputFile("-pcorr", "presscorr.xvg", ExactTextMatch());
    runTest(CommandLine(cmdline), tolerance, &ViscosityInfo::create);
}
