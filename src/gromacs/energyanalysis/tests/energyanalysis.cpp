...
#include "gromacs/energyanalysis/energyhandler.h"
...
class EnergyTest : public gmx::test::CommandLineTestBase
{
    public:
        ...
        void runTest(const CommandLine          &args,
                     ...
                     EnergyAnalysisRunnerPointer (*module)())
        {
            ...
            ASSERT_EQ(0, cth.runModuleDirect(module(), &cmdline));
...
TEST_F(EnergyTest, SimpleExtractEnergy)
{
    ...
    setOutputFile("-o", "energy.xvg", test::ExactTextMatch());
...
TEST_F(EnergyTest, EinsteinViscosity)
{
    ...
    setOutputFile("-evis", "einstein.xvg", test::ExactTextMatch());
...
TEST_F(EnergyTest, EinsteinViscosityIntegral)
{
    const char *const cmdline[] = { "viscosity" };
    double            tolerance = 0.1;
    setOutputFile("-eivis", "einsteinintegral.xvg", test::ExactTextMatch());
    runTest(CommandLine(cmdline), tolerance, &ViscosityInfo::create);
}
...
TEST_F(EnergyTest, FanourgakisViscosity)
{
    const char *const cmdline[] = { "viscosity" };
    double            tolerance = 0.1;
    setOutputFile("-pcorr", "presscorr.xvg", test::ExactTextMatch());
    runTest(CommandLine(cmdline), tolerance, &ViscosityInfo::create);
}
