...
class FluctpropsTest : public IntegrationTestFixture
{
    public:
        FluctpropsTest() : data_(), checker_(data_.rootChecker()) {}
        ...
        void runTest(const char *stdin)
        {
            ...
            std::string xvgFileName = fileManager_.getTemporaryFilePath("energy.xvg");
            caller.addOption("-o", xvgFileName);
            caller.addOption("-nmol", 1000);
            caller.append("-fluc");
            caller.append("-fluct_props");

            redirectStringToStdin(stdin);
            EXPECT_EQ(0, gmx_energy(caller.argc(), caller.argv()));

            EXPECT_TRUE(Path::exists(xvgFileName));
            // TODO David, does this need to be passed through gmx
            // analyse or something? So far, the results don't
            // resemble the ones in your patch.
            TextInputFile    xvgFile(xvgFileName);
            XvgMatchSettings settings;
            auto             outputFilesChecker = checker_.checkCompound("OutputFiles", "Files");
            auto             fileChecker        = outputFilesChecker.checkCompound("File", "-o");
...
class ViscosityTest : public IntegrationTestFixture
{
    public:
                     ...
                     int         tolerance = 4)
        {
            ...
            XvgMatchSettings settings;
            settings.tolerance = ulpTolerance(tolerance);
            auto             outputFilesChecker = checker_.checkCompound("OutputFiles", "Files");
            auto             fileChecker        = outputFilesChecker.checkCompound("File", "-o");
            checkXvgFile(&xvgFile, &fileChecker, settings);
        }
...
TEST_F(ViscosityTest, FanourgakisViscosity)
{
    // TODO This writes Bulk Viscosity after omitting to write Shear
    // Viscosity, so I need advice from David about how to run the old
    // code.
    runTest("-corr", "enecorr.xvg", 3000);
}
