...
class FluctPropsModule : public EnergyAnalysisModule
{
    public:
         ...
         */
        unsigned int addTerm(const std::vector<std::string> &eName,
...
unsigned int FluctPropsModule::addTerm(const std::vector<std::string> &eName,
                                       const std::vector<std::string> &eUnit,
                                       const char                     *term)
{
    for (unsigned int i = 0; (i < eName.size()); i++)
    {
        ...
        {
            EnergyTerm et(i, ehelper_->storeData(), term, eUnit[i]);
...
void FluctPropsModule::initAnalysis(const std::vector<std::string> &eName,
                                    const std::vector<std::string> &eUnit)
{
...
void FluctPropsModule::addAnalysisFrame(t_enxframe *fr)
{
