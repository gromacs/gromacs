...
bool FluctProps::finalizeAnalysis()
{
    ...
    /* Temperature */
    if (true == (bTemp = getEnergyTerm(F_TEMP, &Temp, &TempStddev)))
    {
        printf("Temperature                              = %10g +/- %10g K\n",
               Temp, TempStddev);
    }

    if (true == (bEnthalpy = getEnergyTerm("Enthalpy", &Enthalpy, &EnthalpyStddev)))
    {
        printf("Enthalpy                                 = %10g +/- %10g kJ/mol\n",
               Enthalpy/getNmol(), EnthalpyStddev/getNmol());
        bTotal = false;
    }
    else
    {
        if (true == (bTotal = getEnergyTerm(F_ETOT, &Total, &TotalStddev)))
        {
            printf("Total energy                         = %10g +/- %10g kJ/mol\n",
                   Total/getNmol(), TotalStddev/getNmol());
        }
        else
        {
    ...
    if (true == (bVolume = getEnergyTerm("Volume", &Volume, &VolumeStddev)))
    {
        printf("Volume                                   = %10g +/- %10g nm^3/molecule\n",
               Volume/getNmol(), VolumeStddev/getNmol());
    }

    bHV = getEnergyTerm(HVstring, &HV, &HVStddev);
    ...
    if (bTemp)
    {
        ...
        {
            /* Compute cP */
            double varh = dsqr(EnthalpyStddev)/getNmol();
        ...
        else if (bTotal)
        {
            ...
            cv    = KILO*((varet/getNmol())/(BOLTZ*Temp*Temp));
        ...
        if (bHV)
        {
            ...
            printf("Coefficient of Thermal Expansion Alpha_P = %10g (1/K)\n",
                   alpha);
            dcp     = ((pow(NANO, 3.0)*Volume*AVOGADRO/getNmol())*Temp*
                       dsqr(alpha)/(kappa));
