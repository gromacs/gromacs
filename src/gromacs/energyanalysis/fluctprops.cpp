...
class FluctPropsModule : public EnergyAnalysisModule
{
    ...
    private:
        ...
        //! Index to Enthalpy (not found if INT_MAX)
        unsigned int          iEnthalpy_;

        //! Index to Volume (not found if INT_MAX)
        unsigned int          iVolume_;

        //! Index to Temperature (not found if INT_MAX)
        unsigned int          iTemp_;

        //! Index to Total Energy (not found if INT_MAX)
        unsigned int          iTotal_;

        //! Index to Enthalpy*Volume (not found if INT_MAX)
        unsigned int          iHV_;
...
void FluctPropsModule::finalizeAnalysis()
{
    ...
    if (bTemp)
    {
        ...
        if (bHV)
        {
            alpha   = ((KILO/AVOGADRO)*(HV - Volume*Enthalpy)/
                       (Volume*BOLTZMANN*Temp*Temp));
        }
        else if (bEnthalpy && bVolume)
        {
            double              v_sum, h_sum, vh_sum, v_aver, h_aver, vh_aver;
            vh_sum = v_sum = h_sum = 0;
            gmx_int64_t         nframes = 0;
            EnergyTermIterator  etiV    = ehelper_->etSearch(iVolume_);
            EnergyTermIterator  etiH    = ehelper_->etSearch(iEnthalpy_);
            EnergyFrameIterator ev      = etiV->beginEF();
            EnergyFrameIterator eh      = etiH->beginEF();
            for (; (ev < etiV->endEF() && eh < etiH->endEF()); ++ev, ++eh)
            {
                double v = ev->energy()*NANO*NANO*NANO;
                double h = KILO*eh->energy()/AVOGADRO;
                v_sum  += v;
                h_sum  += h;
                vh_sum += (v*h);
                nframes++;
            }
            vh_aver = vh_sum / nframes;
            v_aver  = v_sum  / nframes;
            h_aver  = h_sum  / nframes;
            alpha   = (vh_aver-v_aver*h_aver)/(v_aver*BOLTZMANN*Temp*Temp);
        }
        if (0 != alpha)
        {
...
EnergyInfoPointer FluctPropsInfo::create()
{
    energyanalysis::EnergyAnalysisModulePointer eamp(new energyanalysis::FluctPropsModule);
