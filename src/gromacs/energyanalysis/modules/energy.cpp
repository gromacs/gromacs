...
class EnergyEnergyModule : public EnergyAnalysisModule
{
    public:
        ...
        void initAnalysis(ArrayRef<const EnergyNameUnit>  eNU,
...
/*! \libinternal\brief
 * Sum energies if requested and add new energy term.
 * \param[in] ehelper Help structure storing the energies.
 */
static void sumEnergies(EnergyTermContainer *ehelper)
{
    if (ehelper->storeData())
    {
        ...
        int64_t nEner = ehelper->begin()->numFrames();

        for (int64_t i = 0; (i < nEner); i++)
        {
            double sum = 0;
            ...
            etsum.addFrame(ehelper->begin()->findFrame(i)->time(),
                           i, 1, 0.0, 0.0, sum);
        }
