...
EnergyEnergyModule::EnergyEnergyModule() : fp_(nullptr), fc_(nullptr), bSum_(false)
{
...
void EnergyEnergyModule::initAnalysis(ConstArrayRef<EnergyNameUnit>  eNU,
                                      OutputEnvPointer              &oenv)
{
    std::vector<int> set;

    // Select which energies to use
    bool bVerbose = (output_env_get_verbosity(oenv.get()) > 0);
    if (termList_.size() == 0)
    {
        try
        {
...
void EnergyEnergyModule::sumEnergies()
{
    if (ehelper_.storeData())
    {
        ...
        gmx_int64_t nEner = ehelper_.begin()->numFrames();
...
void EnergyEnergyModule::finalizeAnalysis(OutputEnvPointer &oenv)
{
    if (bSum_)
    {
        sumEnergies();
    ...
    for (EnergyTermIterator eti = ehelper_.begin(); eti < ehelper_.end(); ++eti)
    {
        eti->calculateDrift();
        eti->computeErrorEstimate(ehelper_.nBlocks());
    }
    printStatistics(stdout, ehelper_.begin(), ehelper_.end(), ehelper_.nBlocks());
    viewOutput(oenv);
}

void EnergyEnergyModule::viewOutput(OutputEnvPointer &oenv)
{
    if (fnEnergy_.size() > 0)
    {
        do_view(oenv.get(), fnEnergy_.c_str(), "-nxy");
    }
    if (fnFluctConv_.size() > 0)
    {
        do_view(oenv.get(), fnFluctConv_.c_str(), "-nxy");
    }
}
