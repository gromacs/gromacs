...
void ViscosityModule::initAnalysis(const std::vector<EnergyNameUnit> &eNU)
{
    bool bFound[vNR];
    int  nfound = 0;
    for (int j = 0; (j < vNR); j++)
    ...
    for (std::vector<EnergyNameUnit>::const_iterator enu = eNU.begin(); (enu < eNU.end()); ++enu, ++i)
    {
        for (int j = 0; (j < vNR); j++)
        {
            ...
            {
                EnergyTerm et(i, ehelper_.storeData(), enu->energyName, enu->energyUnit);
                             ...
                             double fit[6], gmx_output_env_t *oenv)
{
    ...
    (void) do_lmfit(nout, &P[0], &(dy[0]), Dt, NULL,
                    0, t_end, oenv,
...
void ViscosityModule::finalizeAnalysis()
{
    ...
    if (1 || (fnEinstein_.size() > 0) || (fnEinsteinIntegral_.size() > 0))
    {
    ...
    bp = &(bulkP[0]);
    low_do_autocorr(NULL, ehelper_.outputEnvironment(), leg[0], nframes,
                    5, nout, sp, Dt,
                    eacNormal, 1, TRUE, FALSE, FALSE, 0.0, 0.0, 0);

    /* Now for bulk viscosity */
    low_do_autocorr(NULL, ehelper_.outputEnvironment(), leg[1], nframes,
                    1, nout, &bp, Dt,
                    eacNormal, 1, TRUE, FALSE, FALSE, 0.0, 0.0, 0);
    sfree(tempArgs);
    ...
    if ((nout > 1) )
    {
        ...
        {
            fp = xvgropen(fnPressureAcf_.c_str(),
            ...
            xvgr_legend(fp, npleg, pleg, ehelper_.outputEnvironment());
        }
        double      bInt, sInt;
        bInt = sInt = 0;
        for (int i = 0; (i < nout); i++)
                                 ...
                                 real V, real T)
{
    ...
    {
        fpi = xvgropen(fnEinsteinIntegral_.c_str(),
