...
class ViscosityModule : public EnergyAnalysisModule
{
    ...
    private:
        ...
        //! Use Fanourgakis method
        bool                               useFanourgakis_;
                                  ...
                                  ICommandLineOptionsModuleSettings *settings)
{
    ...
    options->addOption(BooleanOption("fanourgakis").store(&useFanourgakis_)
                           .description("Use fit to pressure autocorrelation to compute shear viscosity (experimental)"));
...
static double calcEintSum(gmx_int64_t i, const EnergyTermIterator a, const EnergyTermIterator b)
{
...
static double calcEintDiff(gmx_int64_t i, const EnergyTermIterator a, const EnergyTermIterator b)
{
    return 0.5*(a->findFrame(i)->energySum() - b->findFrame(i)->energySum())/a->findFrame(i)->nSum();
}

void ViscosityModule::finalizeAnalysis(OutputEnvPointer &oenv)
{
...
//! Macro to determine number of elements to average over
#define maxNumberComponents 5
    ...
    if (useFanourgakis_)
    {
        ...
        std::vector<real> shearP[maxNumberComponents], bulkP;
        for (gmx_int64_t i = 0; (i < nframes); i++)
        {
            /* See Fanourgakis J. Phys. Chem. A 2012, 116, 2564-2570 */
            int n = 0;
            shearP[n++].push_back(etxyz[vPresXY]->findFrame(i)->energy() - eAver[vPresXY]);
            shearP[n++].push_back(etxyz[vPresYZ]->findFrame(i)->energy() - eAver[vPresYZ]);
            shearP[n++].push_back(etxyz[vPresZX]->findFrame(i)->energy() - eAver[vPresZX]);
            shearP[n++].push_back(0.5*(etxyz[vPresXX]->findFrame(i)->energy() -
                                       etxyz[vPresYY]->findFrame(i)->energy()));
            shearP[n++].push_back(0.5*(etxyz[vPresYY]->findFrame(i)->energy() -
        ...
        real     *sp[maxNumberComponents], *bp;
        for (int i = 0; (i < numberComponents); i++)
        {
            sp[i] = &(shearP[i][0]);
        }
        bp = &(bulkP[0]);
        low_do_autocorr(nullptr, oenv.get(), leg[0], nframes,
                        numberComponents, nout, sp, Dt,
                        eacNormal, 1, TRUE, FALSE, FALSE, 0.0, 0.0, 0);

        /* Now for bulk viscosity */
        low_do_autocorr(nullptr, oenv.get(), leg[1], nframes,
                        1, nout, &bp, Dt,
                        eacNormal, 1, TRUE, FALSE, FALSE, 0.0, 0.0, 0);
        sfree(tempArgs);

        double factor = (eAver[vVolume]*1e-26/(BOLTZMANN*eAver[vTemperature]));
        if ((nout > 1) && fnPressureAcf_.size() > 0)
        {
            double bulk0, bulkfit[6];
            double shear0, shearfit[6];

            /* Run the analysis based on the bulk pressure fluctuations */
            bulk0 = bulkP[0];
            ...
            if (fnPressureAcf_.size() > 0)
            {
                ...
                xvgr_legend(fp.get(), npleg, pleg, oenv.get());
            }
            double      bInt, sInt;
            bInt = sInt = 0;
            for (int i = 0; (i < nout); i++)
                                 ...
                                 OutputEnvPointer &oenv)
{
    ...
    gmx_int64_t         nframes = ehelper_.begin()->numFrames()+1;
...
//! Number of legend elements in the graph
#define maxNumberGraphs 6
    const char *legend[maxNumberGraphs];
    int   numberGraphs = 0;
    ...
    if (fnEinsteinIntegral_.size() > 0)
    {
        fpi.reset(xvgropen(fnEinsteinIntegral_.c_str(),
                           ...
                           oenv.get()));
        xvgr_legend(fpi.get(), numberGraphs, legend, oenv.get());
    }
    gmx::unique_cptr<FILE, xvgrclose> fp(nullptr);
    if (fnEinstein_.size() > 0)
    {
        fp.reset(xvgropen(fnEinstein_.c_str(),
                          ...
                          oenv.get()));
        xvgr_legend(fp.get(), numberGraphs, legend, oenv.get());
    }
