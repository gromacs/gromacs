...
ViscosityModule::ViscosityModule() : volume_(0.0), setnm_(setnames)
{
}
                                 ...
                                 OutputEnvPointer &oenv)
{
    ...
    gmx_int64_t         nframes = ehelper_.begin()->numFrames();
    ...
    const char *legend[] = {
        "P\\sxy\\N-<P\\sxy\\N>", "P\\sxz\\N-<P\\sxz\\N>",
        "P\\syz\\N-<P\\syz\\N>", "P\\sxx\\N-P\\syy\\N",
        "P\\syy\\N-P\\szz\\N", "Average"
    };
    ...
    if (fnEinsteinIntegral_.size() > 0)
    {
        fpi.reset(xvgropen(fnEinsteinIntegral_.c_str(),
                           ...
                           oenv.get()));
        xvgr_legend(fpi.get(), 6, legend, oenv.get());
    }
    gmx::unique_cptr<FILE, xvgrclose> fp(nullptr);
    if (fnEinstein_.size() > 0)
    {
        fp.reset(xvgropen(fnEinstein_.c_str(),
                          ...
                          oenv.get()));
        xvgr_legend(fp.get(), 6, legend, oenv.get());
    }
