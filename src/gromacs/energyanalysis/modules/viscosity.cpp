...
void ViscosityModule::finalizeAnalysis()
{
    ...
    if ((nout > 1) )
    {
        ...
        fit_pressure_acf(nout, Dt, shearP[0], shearfit,
                         ehelper_.outputEnvironment());
        std::unique_ptr<FILE, void(*)(FILE*)> fp(nullptr, nullptr);
        if (fnPressureAcf_.size() > 0)
        {
            fp = std::unique_ptr<FILE, void(*)(FILE*)>
                    (xvgropen(fnPressureAcf_.c_str(),
                                 ...
                                 real V, real T)
{
    ...
    std::unique_ptr<FILE, void(*)(FILE*)> fpi(nullptr, nullptr);
    if (fnEinsteinIntegral_.size() > 0)
    {
        fpi = std::unique_ptr<FILE, void(*)(FILE*)>
                (xvgropen(fnEinsteinIntegral_.c_str(),
