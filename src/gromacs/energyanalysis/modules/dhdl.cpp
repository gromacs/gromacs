...
class DhdlEnergyModule : public EnergyAnalysisModule
{
    public:
        //! Constructor
        DhdlEnergyModule() : fp_dhdl_(nullptr),
                             nblocks(0), nhists(0), nsamples(0), nlambdas(0)
        {
        }
        ...
        void analyzeFrame(t_enxframe             *fr,
                          const gmx_output_env_t *oenv) override;
...
void DhdlEnergyModule::initAnalysis(gmx_unused ArrayRef<const EnergyNameUnit>,
                                    gmx_unused const gmx_output_env_t *)
{
    // Read the topology
    {
        ...
        read_tpx(fnTopology_.c_str(), &inputrec_, box, &natoms, nullptr, nullptr, &mtop);
    }
}
...

{
    ...
    if (nblock_hist > 0)
    {
        ...
        for (int i = 0; i < fr->nblock; i++)
        {
            ...
            if (blk->id == enxDHHIST)
            {
                ...
                for (int j = 0; j < nhist; j++)
                {
                    ...
                    lg[0] = legend.c_str();
                    xvgr_new_dataset(fp_dhdl_.get(), setnr, 1, lg, oenv);
                    setnr++;
        ...
        nsamples += static_cast<int>(sum/nblock_hist);
    }
    else
    {
        /* raw dh */
        ...
        for (int i = 0; i < len; i++)
        {
            ...
            for (int j = 0; j < fr->nblock; j++)
            {
                ...
                if (blk->id == enxDH)
                {
                    ...
                    {
                        fprintf(fp_dhdl_.get(), "%4d", static_cast<int>(value));   /* if expanded ensembles and zero, this is a state value, it's an integer. We need a cleaner conditional than if j==1! */
                    }
                    else
                    {
                        fprintf(fp_dhdl_.get(), " %#.12g", value);
                    }
                }
            }
...
void DhdlEnergyModule::finalizeAnalysis(gmx_unused const gmx_output_env_t *)
{
    printf("\nWrote %d lambda values with %d samples as ",
           nlambdas, nsamples);
    if (nhists > 0)
    {
        printf("%d dH histograms ", nhists);
    }
    if (nblocks > 0)
    {
        printf("%d dH data blocks ", nblocks);
    }
    printf("to %s\n", fnEnergy_.c_str());
}
