...
DhdlEnergyModule::DhdlEnergyModule() : fp_dhdl_(nullptr)
{
    ...
    if (!fp_dhdl_)
    {
        if (nblock_dh > 0)
        {
            ...
            /* TODO this is an ugly hack that needs to be fixed: this will only
               ...
               the ener.edr. */
            std::unique_ptr<FILE> fp_dhdl_(open_dhdl(fnEnergy_.c_str(),
                                                     inputrec_,
        ...
        else
        {
            ...
            std::unique_ptr<FILE> fp_dhdl_(xvgropen_type(fnEnergy_.c_str(), title.c_str(),
