...
class DhdlEnergyModule : public EnergyAnalysisModule
{
    public:
        //! Constructor
        DhdlEnergyModule() : inputrec_(nullptr), fp_dhdl_(nullptr),
...
void DhdlEnergyModule::initAnalysis(gmx_unused ConstArrayRef<EnergyNameUnit>,
...

{
    ...
    if (!fp_dhdl_)
    {
        ...
        else
        {
            ...
            xvgr_subtitle(fp_dhdl_.get(), buf.c_str(), oenv.get());
        }
    }
        ...
        for (int i = 0; i < fr->nblock; i++)
        {
            ...
            if (blk->id == enxDHHIST)
            {
                ...
                for (int j = 0; j < nhist; j++)
                {
                    ...
                    lg[0] = legend.c_str();
                    xvgr_new_dataset(fp_dhdl_.get(), setnr, 1, lg, oenv.get());
                    setnr++;
...
void DhdlEnergyModule::finalizeAnalysis(gmx_unused OutputEnvPointer &)
{
