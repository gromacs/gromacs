...
class FluctPropsModule : public EnergyAnalysisModule
{
    public:
         ...
         */
        unsigned int addTerm(ConstArrayRef<EnergyNameUnit> eNU,
                             const char                   *term);
...
unsigned int FluctPropsModule::addTerm(ConstArrayRef<EnergyNameUnit> eNU,
                                       const char                   *term)
{
    unsigned int i = 0;
    for (auto enu : eNU)
...
void FluctPropsModule::initAnalysis(ConstArrayRef<EnergyNameUnit>  eNU,
                                    OutputEnvPointer              &oenv)
{
    ...
    {
        iHV_          = eNU.size();
        energyTermHV_.reset(new EnergyTerm(iHV_, true,
...
void FluctPropsModule::finalizeAnalysis(OutputEnvPointer &oenv)
{
    ...
    /* Temperature */
    if ((bTemp = ehelper_.energyTerm(F_TEMP, &Temp, &TempStddev)))
    {
...
void FluctPropsModule::viewOutput(OutputEnvPointer &oenv)
{
    if (fnFluctConv_.size() > 0)
    {
