                                   ...
                                   ICommandLineOptionsModuleSettings *settings)
{
    static const char *const desc[] = {
        "[THISMODULE] will compute the following properties:[PAR]",
        "Property                        Energy terms needed[PAR]",
        "---------------------------------------------------[PAR]",
        "Heat capacity C[SUB]p[sub] (NPT sims):    Enthalpy, Temp     [PAR]",
        "Heat capacity C[SUB]v[sub] (NVT sims):    Etot, Temp         [PAR]",
        "Thermal expansion coeff. (NPT):  Enthalpy, Vol, Temp[PAR]",
        "Isothermal compressibility:      Vol, Temp          [PAR]",
        "Adiabatic bulk modulus:          Vol, Temp          [PAR]",
        "---------------------------------------------------[PAR]",
...
void FluctPropsModule::initAnalysis(const std::vector<EnergyNameUnit> &eNU)
{
    ...
    {
        iHV_          = eNU.size();
        energyTermHV_ = new EnergyTerm(iHV_, true,
...
void FluctPropsModule::analyzeFrame(t_enxframe *fr)
{
    ...
    if (fr->nre > 0)
    {
        ...
        if (iHV_ != INT_MAX)
        {
            ...
            if (NULL != fc_)
            {
                fprintf(fc_, "  %10g", energyTermHV_->standardDeviation());
            }
        }
        if (NULL != fc_)
        {
