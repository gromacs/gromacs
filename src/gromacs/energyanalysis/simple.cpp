...
void SimpleEnergyModule::initOptions(IOptionsContainer                 *options,
                                     ICommandLineOptionsModuleSettings *settings)
{
...
void SimpleEnergyModule::initAnalysis(const std::vector<EnergyNameUnit> &eNU)
{
    std::vector<int> set;
...
void SimpleEnergyModule::sumEnergies()
{
    if (ehelper_->storeData())
    {
        ...
        for (gmx_int64_t i = 0; (i < nEner); i++)
        {
            ...
            etsum.addData(ehelper_->etBegin()->findFrame(i)->time(),
                          i, 1, 0.0, 0.0, sum);
        }
...
EnergyAnalysisModulePointer SimpleInfo::create()
{
    return EnergyAnalysisModulePointer(new SimpleEnergyModule());
}
