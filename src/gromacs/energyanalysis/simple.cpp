...
class SimpleEnergyModule : public EnergyAnalysisModule
{
    ...
    private:
        ...
        SelectionOptionInfo *termInfo_;
        //! String to store terms to select
        SelectionList        termList_;
...
void SimpleEnergyModule::initAnalysis(std::vector<energyNameUnit> &eNU)
{
    ...
    if (termList_.size() == 0)
    {
        ...
        set = select_energies(eNU, bVerbose, &input);
    }
    else
    {
        GMX_THROW(APIError("Wait for implementation of Selection Stuff"));
    }
...
void SimpleEnergyModule::analyzeFrame(t_enxframe *fr)
{
