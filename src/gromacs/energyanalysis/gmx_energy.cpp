...
int gmx_energy(int argc, char *argv[])
{
    ...
    static gmx_bool    bSum    = FALSE, bFee = FALSE, bFeDiff = FALSE, bPrAll = FALSE, bFluct = FALSE, bDriftCorr = FALSE;
    ...
    t_pargs            pa[]    = {
        ...
        { "-fediff",   FALSE, etBOOL,  {&bFeDiff},
          "Do a free energy difference estimate. For this you need to provide two energy files, see complete help text." },
...
#define NFILE asize(fnm)
    ...
    else if (bFeDiff)
    {
        ...
        gmx::FreeEnergyDifference *fee = new(gmx::FreeEnergyDifference);

        fee->setRunAverFile(opt2fn("-ravg", NFILE, fnm));
