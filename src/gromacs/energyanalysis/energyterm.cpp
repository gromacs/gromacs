...
EnergyTerm::EnergyTerm(unsigned int findex,
                       ...
                       std::string  eUnit) : eTerm_(eTerm), eUnit_(eUnit),
                                             ...
                                             isEner_(false), drift_(false),
                                             a_(0), b_(0),  R_(0), chi2_(0)
{
...
void EnergyTerm::calculateDrift()
{
    drift_ = false;
    if (numFrames() > 2)
    {
...
bool EnergyTerm::removeDrift()
{
    ...
    for (gmx_int64_t i = 0; (i < numFrames()); i++)
    {
...
void EnergyTerm::computeErrorEstimate(unsigned int nb)
{
    if (!storeData())
    {
