...
class ViscosityModule : public EnergyAnalysisModule
{
    public:
         ...
         * \param[in/out] eNU   Names and units of the energy terms.
         */
...
void ViscosityModule::initAnalysis(std::vector<energyNameUnit> &eNU)
{
    bool bFound[vNR];
    int  nfound = 0;
    for (int j = 0; (j < vNR); j++)
    ...
    unsigned int i = 0;
    for (std::vector<energyNameUnit>::iterator enu = eNU.begin(); (enu < eNU.end()); ++enu, ++i)
    {
        for (int j = 0; (j < vNR); j++)
        {
...
void ViscosityModule::finalizeAnalysis()
{
    ...
    if ((fnEinstein_.size() > 0) || (fnEinsteinIntegral_.size() > 0))
    {
        ...
        int    nSum = etxyz[vPresXY]->searchEF(0)->nSum();
        GMX_ASSERT (0 < nSum, "Number of data points should be > 0");
        double fac = Dt/nSum;
        for (gmx_int64_t i = 0; (i < nframes-1); i++)
        {
            eneint[0].push_back(eneint[0].back() +
                                fac*(etxyz[vPresXY]->searchEF(i)->energySum() -
                                     eAver[vPresXY]));
            eneint[1].push_back(eneint[1].back() +
                                fac*(etxyz[vPresXZ]->searchEF(i)->energySum() -
                                     eAver[vPresXZ]));
            eneint[2].push_back(eneint[2].back() +
                                fac*(etxyz[vPresYZ]->searchEF(i)->energySum() -
                                     eAver[vPresYZ]));
            eneint[3].push_back(eneint[2].back() +
                                0.5*fac*(etxyz[vPresXX]->searchEF(i)->energySum() -
                                         etxyz[vPresYY]->searchEF(i)->energySum()));
            eneint[4].push_back(eneint[2].back() +
                                0.5*fac*(etxyz[vPresYY]->searchEF(i)->energySum() -
                                         etxyz[vPresZZ]->searchEF(i)->energySum()));
        }
    ...
    for (gmx_int64_t i = 0; (i < nframes); i++)
    {
        ...
        shearP[0].push_back(0.5*(etxyz[vPresXX]->searchEF(i)->energy() -
                                 etxyz[vPresYY]->searchEF(i)->energy()));
        shearP[1].push_back(0.5*(etxyz[vPresYY]->searchEF(i)->energy() -
                                 etxyz[vPresZZ]->searchEF(i)->energy()));
        shearP[2].push_back(etxyz[vPresXY]->searchEF(i)->energy() - eAver[vPresXY]);
        shearP[3].push_back(etxyz[vPresYZ]->searchEF(i)->energy() - eAver[vPresYZ]);
        shearP[4].push_back(etxyz[vPresZX]->searchEF(i)->energy() - eAver[vPresZX]);
        // Average pressure fluctuations for Bulk viscosity
        bulkP.push_back(etxyz[vPressure]->searchEF(i)->energy() - eAver[vPressure]);
    }
...
EnergyInfoPointer ViscosityInfo::create()
{
