...
void ViscosityModule::initAnalysis(const std::vector<std::string> &eName,
                                   const std::vector<std::string> &eUnit)
{
    bool bFound[vNR];
    int  nfound = 0;
    ...
    for (unsigned int i = 0; (i < eName.size()); i++)
    {
        for (int j = 0; (j < vNR); j++)
        {
            if (eName[i] == setnm_[j])
            {
                EnergyTerm et(i, ehelper_->storeData(), eName[i], eUnit[i]);
...
void ViscosityModule::addAnalysisFrame(t_enxframe *fr)
{
