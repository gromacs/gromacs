...
class EnergyTerm
{
    private:
        ...
        double                   evarTot_;
        /*! \brief Do we have exact data from the simulations?
         *
    ...
    public:
        ...
        const std::string energyTerm() const { return eTerm_; }

        //! Return the name corresponding to the energy unit
        const std::string energyUnit() const { return eUnit_; }
        ...
        bool storeData() const { return bStoreData_; }

        //! Is this a true energy or e.g. Temperature
        bool isEner() const { return bIsEner_; }

        //! Is the exact data available?
        bool bExactData() const { return bExactData_; }

        //! Return iterator to begin looping over energy frames
        EnergyFrameIterator beginEF() const { return ef_.begin(); }

        //! Return iterator to end looping over energy frames
        EnergyFrameIterator endEF() const { return ef_.end(); }
        ...
        gmx_int64_t nEnergy() const { return ef_.size(); }

        //! Return the length of the data set in time
        double timeSpan() const { return t1_ - t0_; }
        ...
        //! Return the length of the data set in time
        gmx_int64_t nSteps() const { return 1 + (step1_ - step0_); }
