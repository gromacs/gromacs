...
class FreeEnergyDifference : public SimpleEnergy
{
    private:
...
void FreeEnergyDifference::initOptions(Options *options)
{
...
bool FreeEnergyDifference::initAnalysis(std::vector<std::string> eName,
                                        std::vector<std::string> eUnit)
{
    if ((fileIndex_ < 2) && SimpleEnergy::initAnalysis(eName, eUnit))
    {
        return true;
    }
    else
...
EnergyAnalysisModulePointer FreeEnergyDifferenceInfo::create()
{
    return EnergyAnalysisModulePointer(new FreeEnergyDifference);
}
}
