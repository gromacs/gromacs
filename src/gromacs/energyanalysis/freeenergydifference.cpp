...
void FreeEnergyDifferenceModule::finalizeAnalysis()
{
    ...
    if (getRunAverFile().size() > 0)
    {
        ...
        xvgr_legend(fp, 2, ravgleg, helper()->outputEnvironment());
    }
    fprintf(stdout, "\n%-24s %10s\n",
...
EnergyAnalysisRunnerPointer FreeEnergyDifferenceInfo::create()
{
    EnergyAnalysisModulePointer eamp(new energyanalysis::FreeEnergyDifferenceModule);
