...
class FreeEnergyDifferenceModule : public SimpleEnergyModule
{
    ...
    public:
         ...
         */
        virtual void initAnalysis(std::vector<std::string> &eName,
         ...
         */
        virtual void addDataSet(std::string &name);
...
void FreeEnergyDifferenceModule::initAnalysis(std::vector<std::string> &eName,
                                              std::vector<std::string> &eUnit)
{
    if (fileIndex_ < 2)
    {
        SimpleEnergyModule::initAnalysis(eName, eUnit);
    }
    else
    {
        GMX_THROW(APIError("Too many files provided for free energy difference analysis"));
    }
}

void FreeEnergyDifferenceModule::addDataSet(std::string &name)
{
    if (fileIndex_ < 1)
    {
        fileIndex_++;
...
void FreeEnergyDifferenceModule::addAnalysisFrame(t_enxframe *fr)
{
    if ((fileIndex_ == 0) || (fileIndex_ == 1))
    {
        SimpleEnergyModule::addAnalysisFrame(fr);
    }
}
...
ICommandLineOptionsModule *FreeEnergyDifferenceInfo::create()
{
