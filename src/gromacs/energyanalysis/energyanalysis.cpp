...
bool EnergyAnalysis::readEneFile()
{
    gmx_bool     bCont;
    ener_file_t  fp = open_enx(getEneFile().c_str(), "r");
    t_enxframe   frame;
    int          timecheck = 0;
    bool         bOK;
    ...
    // Document successful reading of file.
    bRead_ = true;
...
void EnergyAnalysis::sumEnergies()
{
    if (getStoreData())
    {
        EnergyTerm etsum(0, "Total", "kJ/mol");
        etsum.setStoreData(getStoreData());
        ...
        for (gmx_int64_t i = 0; (i < nEner); i++)
        {
            ...
            etsum.addData(etBegin()->searchEF(i)->getT(),
                          etBegin()->searchEF(i)->getE(),
                          1, 0, 0, sum);
        }
        addEnergyTerm(etsum);
    }
...
void EnergyAnalysis::printStatistics(FILE *fp)
{
    if (nEnergyTerm() > 0)
    {
        ...
        fprintf(fp, "Error estimate based on averaging over %d block%s of %g ps.\n",
