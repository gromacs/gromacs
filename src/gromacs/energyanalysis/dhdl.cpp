...
bool DhdlEnergy::readInit(int nre, gmx_enxnm_t enm[])
{
    return SimpleEnergy::readInit(nre, enm);
}

bool DhdlEnergy::readFrame(t_enxframe *fr)
    ...
    if (!*fp_dhdl)
    {
        if (nblock_dh > 0)
        {
            ...
            /* TODO this is an ugly hack that needs to be fixed: this will only
               ...
               the ener.edr. */
            *fp_dhdl = open_dhdl(filename, &ir, getOutputEnvironment());
        }
        else
        {
            ...
            *fp_dhdl = xvgropen_type(filename, title, label_x, label_y, exvggtXNY, getOutputEnvironment());
            sprintf(buf, "T = %g (K), %s = %g", temp, lambda, start_lambda);
            xvgr_subtitle(*fp_dhdl, buf, getOutputEnvironment());
        }
    }
        ...
        for (i = 0; i < fr->nblock; i++)
        {
            ...
            if (blk->id == enxDHHIST)
            {
                ...
                for (j = 0; j < nhist; j++)
                {
                    ...
                    lg[0] = legend;
                    xvgr_new_dataset(*fp_dhdl, setnr, 1, lg, getOutputEnvironment());
                    setnr++;
