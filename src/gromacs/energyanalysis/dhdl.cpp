        ...
        if (!fp_dhdl_)
        {
            if (nblock_dh > 0)
            {
                ...
                /* TODO this is an ugly hack that needs to be fixed: this will only
                   ...
                   the ener.edr. */
                fp_dhdl_ = open_dhdl(fnEnergy_.c_str(),
                                     &inputrec_,
            ...
            else
            {
                ...
                fp_dhdl_ = xvgropen_type(fnEnergy_.c_str(), title.c_str(), label_x.c_str(), label_y.c_str(), exvggtXNY,
                                         ehelper_->outputEnvironment());
                std::string buf = gmx::formatString("T = %g (K), %s = %g",
                                                    temp, lambda.c_str(), start_lambda);
                xvgr_subtitle(fp_dhdl_, buf.c_str(),
                              ehelper_->outputEnvironment());
            }
        }
            ...
            for (i = 0; i < fr->nblock; i++)
            {
                ...
                if (blk->id == enxDHHIST)
                {
                    ...
                    for (j = 0; j < nhist; j++)
                    {
                        ...
                        lg[0] = legend.c_str();
                        xvgr_new_dataset(fp_dhdl_, setnr, 1, lg, ehelper_->outputEnvironment());
                        setnr++;
            ...
            for (i = 0; i < len; i++)
            {
                ...
                for (j = 0; j < fr->nblock; j++)
                {
                    ...
                    if (blk->id == enxDH)
                    {
                        ...
                        {
                            fprintf(fp_dhdl_, "%4d", (int)value);   /* if expanded ensembles and zero, this is a state value, it's an integer. We need a cleaner conditional than if j==1! */
                        }
                        else
                        {
                            if (ehelper_->doublePrecision())
                            {
                                fprintf(fp_dhdl_, " %#.12g", value);   /* print normal precision */
                            }
                            else
                            {
                                fprintf(fp_dhdl_, " %#.8g", value);   /* print normal precision */
                            }
                        }
                    }
                }
