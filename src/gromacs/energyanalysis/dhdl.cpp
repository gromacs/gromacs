...
class DhdlEnergyModule : public SimpleEnergyModule
{
    private:
        t_inputrec     ir;
        FILE         **fp_dhdl;
    ...
    public:
        ...
        virtual ~DhdlEnergyModule() {};

        /*! \brief
         * extract parameters from topology file
         * \param[in] topnm Topoogy file name
         */
        void setParameters(const char *topnm);
...
void DhdlEnergyModule::setParameters(const char *topnm)
{
    gmx_mtop_t  mtop;
    int         natoms;
    matrix      box;
...
DhdlEnergyModule::DhdlEnergyModule()
{
    init_inputrec(&ir);
    fp_dhdl    = NULL;
    filename   = NULL;
...
void DhdlEnergyModule::initOptions(Options                           *options,
                                   ICommandLineOptionsModuleSettings *settings)
{
    ...
    options->addOption(FileNameOption("outDhdl")
                           .filetype(eftPlot)
                           .outputFile()
                           .store(&fnEnergy_)
...
bool DhdlEnergyModule::initAnalysis(std::vector<std::string> eName,
                                    std::vector<std::string> eUnit)
{
    return SimpleEnergyModule::initAnalysis(eName, eUnit);
}

bool DhdlEnergyModule::addAnalysisFrame(t_enxframe *fr)
    ...
    if (!*fp_dhdl)
    {
        if (nblock_dh > 0)
        {
            ...
            /* TODO this is an ugly hack that needs to be fixed: this will only
               ...
               the ener.edr. */
            *fp_dhdl = open_dhdl(filename, &ir, ehelper_->getOutputEnvironment());
        }
        else
        {
            ...
            *fp_dhdl = xvgropen_type(filename, title.c_str(), label_x.c_str(), label_y.c_str(), exvggtXNY,
                                     ehelper_->getOutputEnvironment());
            snprintf(buf, sizeof(buf), "T = %g (K), %s = %g", temp, lambda.c_str(), start_lambda);
        ...
        for (i = 0; i < fr->nblock; i++)
        {
            ...
            if (blk->id == enxDHHIST)
            {
                ...
                for (j = 0; j < nhist; j++)
                {
                    ...
                    if (!derivative)
                    {
                        snprintf(legend, sizeof(legend), "N(%s(%s=%g) | %s=%g)",
                                 deltag.c_str(), lambda.c_str(), foreign_lambda,
                                 lambda.c_str(), start_lambda);
                    }
                    else
                    {
                        snprintf(legend, sizeof(legend), "N(%s | %s=%g)",
                                 dhdl.c_str(), lambda.c_str(), start_lambda);
                    }

                    lg[0] = legend;
                    xvgr_new_dataset(*fp_dhdl, setnr, 1, lg, ehelper_->getOutputEnvironment());
                    setnr++;
        ...
        for (i = 0; i < len; i++)
        {
            ...
            for (j = 0; j < fr->nblock; j++)
            {
                ...
                if (blk->id == enxDH)
                {
                    ...
                    {
                        if (ehelper_->getDoublePrecision())
                        {
...
bool DhdlEnergyModule::finalizeAnalysis()
{
    return true;
}
...
ICommandLineOptionsModule *DhdlInfo::create()
{
    energyanalysis::EnergyAnalysisModulePointer eamp(new energyanalysis::DhdlEnergyModule);
