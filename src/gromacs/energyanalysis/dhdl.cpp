...
class DhdlEnergyModule : public EnergyAnalysisModule
{
    private:
        ...
        t_inputrec     ir;
        //! Array of file pointers for output
        FILE         **fp_dhdl;
        //! Energy file
...
void DhdlEnergyModule::initAnalysis(const std::vector<std::string> &eName,
                                    const std::vector<std::string> &eUnit)
{
    std::vector<int> set;
    ...
    /*if (ehelper_->nDataSet() == 0)
       {
        GMX_THROW(InvalidInputError("No data sets in initAnalysis."));
        }*/
        ...
        if (!*fp_dhdl)
        {
            if (nblock_dh > 0)
            {
                ...
                /* TODO this is an ugly hack that needs to be fixed: this will only
                   ...
                   the ener.edr. */
                *fp_dhdl = open_dhdl(fnEnergy_.c_str(), &ir, ehelper_->outputEnvironment());
            }
            else
            {
                ...
                *fp_dhdl = xvgropen_type(fnEnergy_.c_str(), title.c_str(), label_x.c_str(), label_y.c_str(), exvggtXNY,
            ...
            for (i = 0; i < fr->nblock; i++)
            {
                ...
                if (blk->id == enxDHHIST)
                {
                    ...
                    for (j = 0; j < nhist; j++)
                    {
                        ...
                        lg[0] = legend.c_str();
                        xvgr_new_dataset(*fp_dhdl, setnr, 1, lg, ehelper_->outputEnvironment());
                        setnr++;
...
EnergyInfoPointer DhdlInfo::create()
{
