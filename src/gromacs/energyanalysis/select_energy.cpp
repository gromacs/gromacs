...
/*! \internal \file
 * \brief
 * Implements classes in select_energy.h.
 *
...
//                                 const std::string                                &terms)
{
    ...
    for (std::vector<gmx::energyanalysis::energyNameUnit>::iterator enu = eNU.begin();
         (enu < eNU.end()); ++enu)
    {
    ...
    if (bVerbose)
    {
             ...
             (enu < eNU.end()); ++enu, ++k)
        {
            ...
            if (!bLong)
            {
                printf("%3d  %-14s", k+1, newnm[k].c_str());
                j++;
                if (j == 4)
                {
                    j = 0;
            ...
            {
                printf("%3d  %-34s", k+1, newnm[k].c_str());
                j++;
                if (j == 2)
                {
                    j = 0;
    ...
    {
        std::vector<std::string> subs = gmx::splitString(line);

        for (std::vector<std::string>::iterator sub = subs.begin(); (sub < subs.end()); ++sub)
        {
            char *endptr;
            ...
            {
                GMX_THROW(gmx::InvalidInputError("Invalid energy selection " + *sub));
            }
        }
    }
