...
void EnergyHelper::printStatistics(FILE *fp)
{
    if (nEnergyTerm() > 0)
    {
        ...
        for (eti = etBegin(); (eti < etEnd()); ++eti)
        {
            std::string drift("N/A");
            if (eti->calculateDrift())
            {
                drift = std::to_string(eti->driftA() * eti->timeSpan());
            }
            std::string errorEstimate("N/A");
            if (eti->storeData())
            {
                errorEstimate = std::to_string(eti->errorEstimate(nb));
            }
