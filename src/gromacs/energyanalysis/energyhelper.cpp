...
/*! \internal \file
 * \brief
 * Implements classes in energyhelper.h.
 *
...
void EnergyHelper::initOptions(IOptionsContainer *options)
{
...
void EnergyHelper::setStoreData(bool bStoreData)
{
...
bool EnergyHelper::energyTerm(std::string term, double *e, double *stddev)
{
    for (EnergyTermIterator eti = etBegin(); (eti < etEnd()); ++eti)
    {
...
EnergyTermIterator EnergyHelper::etSearch(unsigned int findex)
{
...
EnergyTermIterator EnergyHelper::etSearch(std::string eTerm)
{
    EnergyTermIterator eti;
    for (eti = etBegin(); (eti < etEnd()); ++eti)
    {
...
bool EnergyHelper::energyTerm(unsigned int ftype, double *e, double *stddev)
{
    return energyTerm(interaction_function[ftype].longname, e, stddev);
}

void EnergyHelper::printStatistics(FILE *fp)
{
    if (nEnergyTerm() > 0)
    {
        char               buf[256];
        ...
        for (eti = etBegin(); (eti < etEnd()); ++eti)
        {
            ...
            {
                std::snprintf(drift, sizeof(drift), "%10g", eti->driftA() * eti->timeSpan());
            }
            else
            {
                std::snprintf(drift, sizeof(drift), "N/A");
            }
            if (eti->storeData() && (nb > 1))
            {
                std::snprintf(errorEstimate, sizeof(errorEstimate), "%10g", eti->errorEstimate(nb));
            }
            else
            {
                std::snprintf(errorEstimate, sizeof(errorEstimate), "N/A");
            }
...
void EnergyHelper::printXvgLegend(FILE *fp)
{
    const char **leg;
    char       **kkk;
...
void EnergyHelper::printEnergies(std::string outputFile)
{
    ...
    if (storeData() && (nEner > 0))
    {
        ...
        for (EnergyTermIterator eti = etBegin(); (eti < etEnd()); ++eti)
        {
            ...
            for (gmx_int64_t i = 0; (i < nEner); i++)
            {
                if (doublePrecision())
                {
                    fprintf(fp, "%15e  %15e\n", eti->searchEF(i)->time(), eti->searchEF(i)->energy());
                }
                else
                {
                    fprintf(fp, "%10g  %10g\n", eti->searchEF(i)->time(), eti->searchEF(i)->energy());
