...
void EnergyHelper::initOptions(Options *options)
{
...
bool EnergyHelper::getEnergyTerm(std::string term, double *e, double *stddev)
{
    for (EnergyTermIterator eti = etBegin(); (eti < etEnd()); ++eti)
    {
        if (eti->getEterm() == term)
        {
            *e      = eti->average();
...
EnergyTermIterator EnergyHelper::etSearch(std::string eTerm)
{
    ...
    for (eti = etBegin(); (eti < etEnd()); ++eti)
    {
        if (eti->getEterm().compare(eTerm) == 0)
        {
            break;
        }
    }
...
bool EnergyHelper::getEnergyTerm(unsigned int ftype, double *e, double *stddev)
{
...
void EnergyHelper::printXvgLegend(FILE *fp)
{
    ...
    for (EnergyTermIterator eti = etBegin(); (eti < etEnd()); ++eti)
    {
        kkk[i] = strdup(eti->getEterm().c_str());
        leg[i] = kkk[i];
        i++;
    }
