...
/*! \internal \file
 * \brief
 * Implements classes in select.h.
 *
...
//! Convert array of booleans to array of indexes
...
//! Chomp off newline (similar to Perl function)
...
void select_by_name(const std::vector<std::string> &nm,
                    std::vector<int>               &set,
                    bool                            bVerbose,
                    std::string                     terms)
{
    ...
    if (terms.size() > 0)
    {
        ...
        while (iss >> sub)
        {
            for (unsigned int kk = 0; (kk < nm.size()); kk++)
            {
                if (gmx_strcasecmp(nm[kk].c_str(), sub.c_str()) == 0)
                {
                    bE[kk] = true;
                }
            }
                ...
                do
                {
                    if (!bEOF)
                    {
                        ...
                        if (nss == 1)
                        {
                            ...
                            {
                                fprintf(stderr, "number %u is out of range\n",
                                        nind);
                            }
                        }
                        else
