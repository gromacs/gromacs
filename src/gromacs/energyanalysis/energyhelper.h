 ...
 *
 * This code can extract energy terms from energy files or from
...
class EnergyHelper
{
    private:
        ...
        //! Data set names (there may be more than one)
    ...
    public:
        ...
        virtual ~EnergyHelper() {};

        //! Additional generic command line options
        void initOptions(Options *options);

        //! Add a data set name
        void addDataSet(std::string name) { dataSet_.push_back(name); }

        //! Number of data sets used
        int nDataSet() { return dataSet_.size(); }

        //! Get the name of the latest data set
        std::string currentDataSet() { return dataSet_.back(); }

        //! Begin iterator for data set names
        std::vector<std::string>::iterator dataSetBegin() { return dataSet_.begin(); }

        //! End iterator for data set names
        std::vector<std::string>::iterator dataSetEnd() { return dataSet_.end(); }
         ...
         * \return true if succesfull, false otherwise
         */
        bool getEnergyTerm(std::string term, double *e, double *stddev);
