...
FreeEnergyEstimate::FreeEnergyEstimate()
{
    refTemp_ = 0;
}
...
{
    if ((fileIndex() == 0) && SimpleEnergy::initAnalysis(nre, enm))
    {
        return true;
    }
    else
...
bool FreeEnergyEstimate::addDataSet(std::string name)
{
    return (fileIndex() < 2) && SimpleEnergy::addDataSet(name);
}
