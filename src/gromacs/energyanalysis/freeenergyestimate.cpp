...
bool FreeEnergyEstimate::readInit(int nre, gmx_enxnm_t enm[])
{
    return SimpleEnergy::readInit(nre, enm);
}

bool FreeEnergyEstimate::readFrame(t_enxframe *fr)
{
    return SimpleEnergy::readFrame(fr);
}

bool FreeEnergyEstimate::FreeEnergyEstimate::readFinalize()
{
    const char * ravgleg[] = {
        "\\8D\\4E = E\\sB\\N-E\\sA\\N",
