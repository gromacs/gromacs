...
TEST(CpuInfoTest, SupportLevel)
{
    ...
    if (c.supportLevel() >= gmx::CpuInfo::SupportLevel::LogicalProcessorInfo)
    {
        ...
        {
            EXPECT_GT(l.socket,   0) << "Impossible socket index for logical processor. " << commonMsg << std::endl;
            EXPECT_GT(l.core,     0) << "Impossible core index for logical processor. " << commonMsg << std::endl;
            EXPECT_GT(l.hwThread, 0) << "Impossible hwthread index for logical processor. " << commonMsg << std::endl;
        }
    }
}
