...
hardwareTopologyPrepareDetection()
{
    ...
    if (!architectureIsX86 && !architectureIsPowerPC)
    {
                                     ...
                                     const gmx::HardwareTopology gmx_unused &hardwareTopology)
{
    ...
    if (countConfigured >= 0 && countConfigured != countFromDetection)
    {
        ...
        if (architectureIsX86 && countConfigured == 2*countFromDetection)
        {
            GMX_LOG(mdlog.info).
        ...
        if (architectureIsPowerPC && countConfigured == 8*countFromDetection)
        {
            GMX_LOG(mdlog.info).
