 ...
 *
 */
void hardwareTopologyDoubleCheckDetection(FILE gmx_unused                          *fplog,
                                          const gmx::HardwareTopology gmx_unused   &hardwareTopology)
{
    ...
    if (fplog)
    {
        int countFromDetection;
        if (hardwareTopology.supportLevel() >= gmx::HardwareTopology::SupportLevel::LogicalProcessorCount)
        {
            countFromDetection = hardwareTopology.machine().logicalProcessorCount;
        }
        else
        {
            countFromDetection = -1;
        }
