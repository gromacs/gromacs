                              ...
                              const gmx::HardwareTopology *hardwareTopology)
{
    ...
    if (countFromDetection != countOnline)
    {
        ...
        else
        {
            ...
            {
                md_print_warn(cr, fplog,
                              "Note: %d CPUs configured in the hardware, but only %d of them "
                              "are online. This likely means that HyperThreading is disabled. "
                              "Enabling it would be likely to have performance benefits. "
                              "GROMACS will use the online CPU count.",
