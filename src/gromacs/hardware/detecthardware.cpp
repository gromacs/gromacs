                   ...
                   const t_commrec      *cr)
{
    ...
    /*
       ...
       //FIXME this was causing an early return
     */                                                                                                                                                                                                                                                                                                                                                                                                    //FIXME this was causing an early return
    ...
    if (!gpu_opt->bUserSet)
    {
        ...
        output += gmx::formatString("%d compatible GPU%s %s present, with ID%s %s\n",
                                    gpu_opt->n_dev_compatible, //FIXME why is this 0 in the rotation test?
                                    bPluralGpus ? "s" : "",
                                    bPluralGpus ? "are" : "is",
                                    bPluralGpus ? "s" : "",
                                    gpuIdsString.c_str());
    ...
    output += gmx::formatString("%zu GPU%s %sselected for this run.\n", // FIXME this shows 2 instead of 1 somehow on a single rank
                                      ...
                                      const GpuTaskManager &gpuTasks)
{
    ...
    int        gpuRankCount     = gpuRankIndicator; // valid for serial case
    if (useMpi)
    {
#if GMX_MPI
        MPI_Comm   physicalnode_comm;    // TODO store and reuse the physical node communicator?
