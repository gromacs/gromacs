...
GpuIdManager g_GpuTasks;

static void print_gpu_stats(const gmx::MDLogger  &mdlog,
...
/*! \brief \internal
 * This function is responsible for mapping the GPUs to the processes in a single node (filling the gpu_opt->dev_use array).
 *
 * \param[in,out]    gpu_opt              Input/output GPU assignment data.
 * \param[in]        dev_use_count_node   Total count of GPU tasks to be used on this node.
 * \param[in]        print_error          Boolean used for printing the error message only on one rank.
 *
 * This selects the GPUs we will use. This is an operation local to each physical
 * node. If we have less MPI ranks than GPUs, we will waste some GPUs.
 */
static void assign_rank_gpu_ids(gmx_gpu_opt_t *gpu_opt, int dev_use_count_node, bool print_error)
{
...
{
    const gmx_bool useAnyGPU = (cr->dev_use_count > 0);
    if (!useAnyGPU)
    {
    ...
    if (gpu_opt->bUserSet)
    {
        ...
        if (!res)
        {
            print_gpu_stats(mdlog, gpu_info);
    ...
    else if (getenv("GMX_EMULATE_GPU") == NULL)
    {
        ...
        bool firstGpuRank = (cr->dev_use_index == 0) && (cr->dev_use_count > 0);
        assign_rank_gpu_ids(gpu_opt, cr->dev_use_count_node, firstGpuRank);
    }
    ...
    for (int gpuIndex = cr->dev_use_index; gpuIndex < cr->dev_use_index + cr->dev_use_count; gpuIndex++)
    {
                              ...
                              bool             useGpuPME)
{
    ...
    int gpuIndex = cr->dev_use_index;
    if ((cr->duty & DUTY_PP) && useGpuNB)
    {
        g_GpuTasks.setGpuIndex(GpuTask::NB, gpuIndex);
        gpuIndex++;
    }
    if ((cr->duty & DUTY_PME) && useGpuPME)
    {
        g_GpuTasks.setGpuIndex(GpuTask::PME, gpuIndex);
        gpuIndex++;
    }
}
