 ...
 */
void communicate_gpu_tasks_counts(const t_commrec *cr,
...
#if GMX_MPI
    if (PAR(cr)) // thread MPI fails with a single node as it is not properly initialized
    {
        const int nrank_intranode = cr->nrank_intranode;
        MPI_Comm  physicalnode_comm;
        int       rank_world;
        MPI_Comm_rank(MPI_COMM_WORLD, &rank_world);
        MPI_Comm_split(MPI_COMM_WORLD, gmx_physicalnode_id_hash(), rank_world, &physicalnode_comm);
...
/*! \internal \brief
 * This function is responsible for mapping the GPUs to the processes in a single node
