                                       ...
                                       int *devUseIndex, int *devUseCountNode)
{
    ...
    *devUseCountNode = gpuTasksCountRank;
    if ((PAR(cr) || MULTISIM(cr))) // Checking for MPI to be initialized
    {
        ...
        /* Getting total amount of GPU tasks on this node: the prefix sum from the last rank is the full sum */
...
void GpuTaskAssignmentManager::selectRankGpus(const gmx::MDLogger &mdlog, const t_commrec *cr, bool userSetGpuIds)
{
    ...
    if (userSetGpuIds)
    {
        ...
        /* Check whether the number of manually provided GPU IDs corresponds to the total number of GPU ranks */
        int        gpuRankIndicator = (tasksToAssign_.size() > 0) ? 1 : 0;
        /* Default value for serial case */
        ...
        if (gpuOpt_->n_dev_use != gpuRanksNodeCount)
        {
            /* This was moved from gmx_check_hw_runconf_consistency() and uses same strings */
            ...
            const std::string gpuRanksNodePlural = (gpuRanksNodeCount > 1) ? "s" : "";
            gmx_fatal(FARGS,
                      "Incorrect launch configuration: mismatching number of GPU ranks and GPU IDs%s.\n"
                      "%s was started with %d GPU rank%s%s in total, but you've provided %d GPU ID%s.",
                      pernode.c_str(), programName, gpuRanksNodeCount, gpuRanksNodePlural.c_str(), pernode.c_str(),
...
GpuContextsMap GpuTaskManager::rankGpuContexts() const
{
    return gpuContextsByTasks_;
}
                                   ...
                                   bool useGpuNB, bool userSetGpuIds)
{
    ...
    assigner.selectRankGpus(mdlog, cr, userSetGpuIds);
