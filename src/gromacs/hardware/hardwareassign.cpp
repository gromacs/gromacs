...
#if GMX_MPI
    if ((PAR(cr) || MULTISIM(cr))) // a copypasted check for MPI to be actually initialized
    {
                              ...
                              bool                 useGpuPME)
{
    ...
    {
        gpu_opt->gpuTasks.get()->setGpuIndex(GpuTask::NB, gpuIndex);
        gpuIndex++;
    }
    if ((cr->duty & DUTY_PME) && useGpuPME)
    {
        gpu_opt->gpuTasks.get()->setGpuIndex(GpuTask::PME, gpuIndex);
        gpuIndex++;
    }
}
