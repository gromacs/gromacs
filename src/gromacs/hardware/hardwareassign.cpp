                             ...
                             gmx_gpu_opt_t        *gpu_opt)
{
     ...
     */
    const bool useGpuNB  = (cr->duty & DUTY_PP) && gpu_opt->taskPreferences->maybeUseGpu(GpuTask::NB);
    const bool useGpuPME = (cr->duty & DUTY_PME) && gpu_opt->taskPreferences->maybeUseGpu(GpuTask::PME);
    gpu_opt->dev_use_count = (useGpuNB ? 1 : 0) + (useGpuPME ? 1 : 0);
    ...
    if (gpu_opt->taskPreferences->definitelyUseGpuForSomething() && (GMX_GPU == GMX_GPU_NONE))
    {
    ...
    if (gpu_opt->taskPreferences->definitelyUseGpuForSomething() && gpu_info->n_dev_compatible == 0)
    {
                              ...
                              gmx_gpu_opt_t       *gpu_opt)
{
    ...
    if ((cr->duty & DUTY_PP) && gpu_opt->taskPreferences->definitelyUseGpu(GpuTask::NB))
    {
        gpu_opt->gpuTasks->setGpuIndex(GpuTask::NB, gpuIndex);
        gpuIndex++;
    }
    if ((cr->duty & DUTY_PME) && gpu_opt->taskPreferences->definitelyUseGpu(GpuTask::PME))
    {
