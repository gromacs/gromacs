...
/*! \brief cpuinfo standalone macro for 32-bit x86 */
#define GMX_CPUINFO_X86_IA32   (defined __i386__ || defined __i386 || defined _X86_ || defined _M_IX86)
/*! \brief cpuinfo standalone macro for 64-bit x86 */
#define GMX_CPUINFO_X86_64     (defined __x86_64__ || defined __amd64__ || defined _M_X64 || defined _M_AMD64)
/*! \brief cpuinfo standalone macro for IBM Power */
#define GMX_CPUINFO_IBM_POWER  (defined __powerpc__ || defined __ppc__ || defined __PPC__ )
/*! \brief cpuinfo standalone macro for 32-bit Arm */
#define GMX_CPUINFO_ARM        ( (defined __arm__ || defined __arm || defined _M_ARM) && !CPUINFO_ARM_AARCH )
/*! \brief cpuinfo standalone macro for 64-bit Arm */
#define GMX_CPUINFO_ARM_AARCH  (defined __aarch64__ || ( defined _M_ARM && _M_ARM >= 8 ))
/*! \brief cpuinfo standalone macro for Microsoft visual studio */
#define GMX_CPUINFO_MSVC       (defined _MSC_VER)
...
executeX86CpuID(unsigned int     gmx_unused level,
                ...
                unsigned int *              edx)
{
#if GMX_CPUINFO_X86_IA32 || GMX_CPUINFO_X86_64
...
#        if GMX_CPUINFO_X86_IA32 && defined(__PIC__)
...
CpuInfo::CpuInfo()
    : vendor_(CpuInfo::Vendor::Unknown), family_(0), model_(0), stepping_(0)
{
    brandString_ = "Unknown CPU brand"; // might be overwritten later

#if GMX_CPUINFO_X86_IA32 || GMX_CPUINFO_X86_64

    vendor_            = detectX86Vendor();
    detectX86Features(&brandString_, &family_, &model_, &stepping_, &features_);
    logicalProcessors_ = detectX86LogicalProcessors();

#else   // not x86

#    if GMX_CPUINFO_ARM || GMX_CPUINFO_ARM_AARCH
    vendor_  = CpuInfo::Vendor::Arm;
#    elif GMX_CPUINFO_POWER
    vendor_  = CpuInfo::Vendor::Ibm;
#    endif

#    if GMX_CPUINFO_ARM_AARCH
    features_.insert(Feature::Arm_Neon);      // ARMv8 always has Neon
    features_.insert(Feature::Arm_NeonAsimd); // ARMv8 always has Neon-asimd
#    endif
    ...
    detectProcCpuInfo(&vendor_, &brandString_, &family_, &model_, &stepping_, &features_);

#endif  // x86 or not

    if (logicalProcessors_.size() > 0)
    {
        supportLevel_ = CpuInfo::SupportLevel::LogicalProcessorInfo;
    }
    else if (features_.size() > 0)
    {
        supportLevel_ = CpuInfo::SupportLevel::Features;
    }
    else if (vendor_ != CpuInfo::Vendor::Unknown || brandString_ != "Unknown CPU brand")
    {
        supportLevel_ = CpuInfo::SupportLevel::Name;
    }
    else
    {
        supportLevel_ = CpuInfo::SupportLevel::None;
    }
}
