               ...
               HardwareTopology::Machine *        machine)
{
    ...
    else
    {
        ...
        const hwloc_obj* hwlocMachine = hwloc_get_next_obj_by_type(topo, HWLOC_OBJ_MACHINE, nullptr);
                  ...
                  HardwareTopology::Machine *        machine)
{
    ...
    for (auto &p : pcidevs)
    {
        const hwloc_obj * ancestor = hwloc_get_ancestor_obj_by_type(topo, HWLOC_OBJ_NUMANODE, p);
        int               numaId;
        if (ancestor != nullptr)
        {
