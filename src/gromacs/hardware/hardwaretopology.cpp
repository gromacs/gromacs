    ...
    : supportLevel_(SupportLevel::None)
{
    ...
    machine_.logicalProcessorCount   = 0;
    machine_.numa.baseLatency        = 0.0;
    machine_.numa.maxRelativeLatency = 0.0;

#if GMX_HWLOC
    parseHwLoc(&machine_, &supportLevel_);
#endif

    // If something went wrong in hwloc (or if it was not present) we might
    // have more information in cpuInfo
    if (supportLevel_ < SupportLevel::Basic)
    {
        parseCpuInfo(&machine_, &supportLevel_);
    }

    // If we did not manage to get anything from either hwloc or cpuInfo, find the cpu count at least
    if (supportLevel_ == SupportLevel::None)
    {
        // No topology information; try to detect the number of logical processors at least
        machine_.logicalProcessorCount = detectLogicalProcessorCount();
        if (machine_.logicalProcessorCount > 0)
        {
