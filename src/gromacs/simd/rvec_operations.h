...
gmx_simd_gather_rvec_dist_pair_index(const rvec      *v,
                                     ...
                                     gmx_simd_real_t *dz)
{
#if defined GMX_SIMD4_HAVE_REAL && defined GMX_SIMD4_HAVE_LOAD3 && defined GMX_SIMD4_HAVE_SIMD_TRANSPOSE
    int              i;
    ...
    {
        d[i] = gmx_simd4_sub_r(gmx_simd4_load3_r(&(v[pair_index[i*2 + 0]][0])),
                               gmx_simd4_load3_r(&(v[pair_index[i*2 + 1]][0])));
    }
                                    ...
                                    gmx_simd_real_t *dz)
{
    ...
    for (i = 0; i < GMX_SIMD_REAL_WIDTH; i++)
    {
        d[i] = gmx_simd4_sub_r(gmx_simd4_load3_r(&(v[index0[i]][0])),
                               gmx_simd4_load3_r(&(v[index1[i]][0])));
    }
...
gmx_simd_store_vec_to_rvec(gmx_simd_real_t  x,
                           ...
                           rvec            *v)
{
#if defined GMX_SIMD4_HAVE_REAL && defined GMX_SIMD4_HAVE_STORE3 && defined GMX_SIMD4_HAVE_SIMD_TRANSPOSE
    int              i;
