...
class SimdArrayRef
{
    ...
    public:
        SimdArrayRef(std::vector<T, gmx::Allocator<float, gmx::AlignedAllocationPolicy> > &v)
 ...
 * in PME solve), and possibly unaligned addresses such as
 * getUnpaddedArrayRef()[size()-1] (used in bonded, constraint and
 * update code). This is simply ensured by making sure that the padded
 * region has at least as many bytes as \c s_simdWidth * 3.
 *
...
class SimdMemory
{
    private:
        ...
        SizeType computePaddedSize(const SizeType numElements)
        {
            ...
            SizeType           maxBaseTypeNeeded = (numElements * s_packSize + 4 * s_simdWidth);
