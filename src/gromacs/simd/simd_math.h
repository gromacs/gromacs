...
maskedInvsqrt(SimdFloat x, SimdFBool m)
{
    SimdFloat lu = maskedRsqrt(x, m);
#if (GMX_SIMD_RSQRT_BITS < GMX_SIMD_ACCURACY_BITS_SINGLE)
    lu = rsqrtIter(lu, x);
#endif
#if (GMX_SIMD_RSQRT_BITS*2 < GMX_SIMD_ACCURACY_BITS_SINGLE)
 ...
 */
static inline SimdFloat gmx_simdcall
maskedInv(SimdFloat x, SimdFBool m)
{
    SimdFloat lu = maskedRcp(x, m);
#if (GMX_SIMD_RCP_BITS < GMX_SIMD_ACCURACY_BITS_SINGLE)
    lu = rcpIter(lu, x);
#endif
#if (GMX_SIMD_RCP_BITS*2 < GMX_SIMD_ACCURACY_BITS_SINGLE)
...
sqrt(SimdFloat x)
{
    SimdFloat  res = maskedInvsqrt(x, isNonZero(x));
    return res*x;
}
...
maskedInvsqrt(SimdDouble x, SimdDBool m)
{
    SimdDouble lu = maskedRsqrt(x, m);
#if (GMX_SIMD_RSQRT_BITS < GMX_SIMD_ACCURACY_BITS_DOUBLE)
    lu = rsqrtIter(lu, x);
#endif
#if (GMX_SIMD_RSQRT_BITS*2 < GMX_SIMD_ACCURACY_BITS_DOUBLE)
 ...
 */
static inline SimdDouble gmx_simdcall
maskedInv(SimdDouble x, SimdDBool m)
{
    SimdDouble lu = maskedRcp(x, m);
#if (GMX_SIMD_RCP_BITS < GMX_SIMD_ACCURACY_BITS_DOUBLE)
    lu = rcpIter(lu, x);
#endif
#if (GMX_SIMD_RCP_BITS*2 < GMX_SIMD_ACCURACY_BITS_DOUBLE)
...
sqrt(SimdDouble x)
{
    return x * maskedInvsqrt(x, isNonZero(x));
}
...
erf(SimdDouble x)
{
    ...
    res_erf  = PolyAP0 * maskedInv(PolyAQ0, mask_erf);
    ...
    res_erfcB = PolyBP0 * maskedInv(PolyBQ0, notmask_erf);

    res_erfcB = res_erfcB * xabs;

    // Calculate erfc() in range [4.5,inf]
    w       = maskedInv(xabs, notmask_erf);
    w2      = w * w;
    ...
    // The denominator polynomial can be zero outside the range
    res_erfcC = PolyCP0 * maskedInv(PolyCQ0, notmask_erf);
...
erfc(SimdDouble x)
{
    ...
    // Calculate erfc() in range [4.5,inf]
    w       = maskedInv(xabs, isNonZero(xabs));
    w2      = w * w;
...
atan(SimdDouble x)
{
    ...
    t1     = (xabs + mone) * maskedInv(xabs - mone, mask1);
    t2     = mone * maskedInv(xabs, mask2);
 ...
 */
static inline SimdDouble
maskedInvsqrtSingleAccuracy(SimdDouble x, SimdDBool m)
{
    SimdDouble lu = maskedRsqrt(x, m);
#if (GMX_SIMD_RSQRT_BITS < GMX_SIMD_ACCURACY_BITS_SINGLE)
    lu = rsqrtIter(lu, x);
 ...
 */
static inline SimdDouble gmx_simdcall
maskedInvSingleAccuracy(SimdDouble x, SimdDBool m)
{
    SimdDouble lu = maskedRcp(x, m);
#if (GMX_SIMD_RCP_BITS < GMX_SIMD_ACCURACY_BITS_SINGLE)
    lu = rcpIter(lu, x);
...
{
    return x * maskedInvsqrtSingleAccuracy(x, isNonZero(x));
}
...
erfSingleAccuracy(SimdDouble x)
{
    ...
    t       = maskedInvSingleAccuracy(y, msk_erf);
    w       = t - one;
    t2      = t * t;
    w2      = w * w;
...
tanSingleAccuracy(SimdDouble x)
{
    ...
    p       = blend( p, maskedInvSingleAccuracy(p, mask), mask);
    return p;
}
...
asinSingleAccuracy(SimdDouble x)
{
    ...
    z1    = half * (one - xabs);
    mask2 = (xabs < one);
    q1    = z1 * maskedInvsqrtSingleAccuracy(z1, mask2);
    q2    = xabs;
    z2    = q2 * q2;
    z     = blend(z2, z1, mask);
    q     = blend(q2, q1, mask);
...
acosSingleAccuracy(SimdDouble x)
{
    ...
    z     = half * (one - xabs);
    mask3 = (xabs < one);
    z     = z * maskedInvsqrtSingleAccuracy(z, mask3);
    z     = blend(x, z, mask1);
...
atanSingleAccuracy(SimdDouble x)
{
    ...
    x     = blend(x, maskedInvSingleAccuracy(x, mask2), mask2);
...
atan2SingleAccuracy(SimdDouble y, SimdDouble x)
{
    ...
    xinv      = maskedInvSingleAccuracy(x, mask_xnz);
    p         = y * xinv;
 ...
 */
static inline SimdFloat
maskedInvsqrtSingleAccuracy(SimdFloat x, SimdFBool m)
{
    return maskedInvsqrt(x, m);
}
 ...
 */
static inline SimdFloat
maskedInvSingleAccuracy(SimdFloat x, SimdFBool m)
{
    return maskedInv(x, m);
}
