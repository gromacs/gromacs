...
sqrt(SimdFloat x)
{
    SimdFloat  res = maskedInvsqrt(x, setZero() < x);
    return res*x;
}
...
sqrt(SimdDouble x)
{
    return x * maskedInvsqrt(x, setZero() < x);
}
...
{
    return x * maskedInvsqrtSingleAccuracy(x, setZero() < x);
}
