 ...
 *
 * IMPORTANT: This file is generic for all SIMD architectures, so you cannot
...
/*! Accept lsb errors for 1/sqrt(x) and 1/x, so float target is 22 bits */
#define GMX_SIMD_MATH_TARGET_SINGLE_BITS 22

/*! Accept "double" that has 2x single precision, so 2 N-R iterations are OK */
...
/*! SIMD utility function to sum a+b+c+d for SIMD floats */
...
/*! Calculate 1/sqrt(x) for two SIMD floats. This is largely useless in single,
 *  but by defining it for all precisions we can optimize the double precision
 *  case by performing everything but the last step in single precision,
 *  and provide a real-value routine where we don't care about precision.
 */
...
gmx_simd_sqrt_f(gmx_simd_float_t x)
{
    ...
    res  = gmx_simd_blendv_f(gmx_simd_invsqrt_f(x), gmx_simd_setzero_f(), mask);
...
gmx_simd_tan_f(gmx_simd_float_t x)
{
    ...
    x       = gmx_simd_fnmadd_f(y, argred2, x);
    x       = gmx_simd_fnmadd_f(y, argred3, x);
    x       = gmx_simd_xor_f(gmx_simd_blendzero_f(gmx_simd_set1_f(-0.0f), mask), x); // set sign to neg where m true
#else
    const gmx_simd_float_t  quarter         = gmx_simd_set1_f(0.25f);
    const gmx_simd_float_t  half            = gmx_simd_set1_f(0.5f);
...
gmx_simd_asin_f(gmx_simd_float_t x)
{
    ...
    q1    = gmx_simd_blendv_f(q1, gmx_simd_setzero_f(), gmx_simd_cmpeq_f(xabs, one));
    q2    = xabs;
...
gmx_simd_acos_f(gmx_simd_float_t x)
{
    ...
    z     = gmx_simd_blendv_f(z, gmx_simd_setzero_f(), gmx_simd_cmpeq_f(xabs, one));
...
gmx_simd_atan2_f(gmx_simd_float_t y, gmx_simd_float_t x)
{
    ...
    gmx_simd_float_t       p, aoffset;
    gmx_simd_float_t       xsign, ysign;
    gmx_simd_fbool_t       mask_xneg;
    gmx_simd_fbool_t       mask_xlt0, mask_ylt0, mask_ygt0, mask_xgt0;
    gmx_simd_fbool_t       mask_ym0, mask_yp0, mask_xp0, mask_x0, mask_y0;

    mask_xlt0 = gmx_simd_cmplt_f(x, gmx_simd_setzero_f());
    mask_ylt0 = gmx_simd_cmplt_f(y, gmx_simd_setzero_f());
    aoffset   = gmx_simd_blendzero_f(pi, mask_xlt0);
    aoffset   = gmx_simd_blendv_f(aoffset, gmx_simd_fneg_f(aoffset), mask_ylt0);

    p         = gmx_simd_mul_f(y, gmx_simd_inv_f(x));
    p         = gmx_simd_atan_f(p);
    p         = gmx_simd_add_f(p, aoffset);

    /* Handle all the special cases (See atan2 manpage). This can likely be done
     * better, but a lot of SIMD implementations out there are really buggy, and
     * atan2() is pathological since the result makes sudden jumps at several of
     * these values. Since atan2() is not super-critical in Gromacs we make do
     * with this less-than perfect solution to check for the special values.
     *
     * Extract sign bits of x/y correctly, even when they are +0.0 or -0.0.
     * Note that x==-0.0 does not count as x<0 !
     */
    xsign     = gmx_simd_xor_sign_f(gmx_simd_set1_f(1.0), x);
    ysign     = gmx_simd_xor_sign_f(gmx_simd_set1_f(1.0), y);
    mask_xneg = gmx_simd_cmplt_f(xsign, gmx_simd_setzero_f());

    mask_xgt0 = gmx_simd_cmplt_f(gmx_simd_setzero_f(), x);
    mask_ygt0 = gmx_simd_cmplt_f(gmx_simd_setzero_f(), y);
    mask_x0   = gmx_simd_cmpeq_f(x, gmx_simd_setzero_f());
    mask_y0   = gmx_simd_cmpeq_f(y, gmx_simd_setzero_f());
    mask_xp0  = gmx_simd_and_fb(mask_x0, gmx_simd_cmplt_f(gmx_simd_setzero_f(), xsign));
    mask_yp0  = gmx_simd_and_fb(mask_y0, gmx_simd_cmplt_f(gmx_simd_setzero_f(), ysign));
    mask_ym0  = gmx_simd_and_fb(mask_y0, gmx_simd_cmplt_f(ysign, gmx_simd_setzero_f()));

    p         = gmx_simd_blendv_f(p, pi, gmx_simd_and_fb(mask_yp0, mask_xneg));
    p         = gmx_simd_blendv_f(p, gmx_simd_fneg_f(pi), gmx_simd_and_fb(mask_ym0, mask_xneg));

    p         = gmx_simd_blendv_f(p, halfpi, gmx_simd_and_fb(mask_ygt0, mask_x0));
    p         = gmx_simd_blendv_f(p, gmx_simd_fneg_f(halfpi), gmx_simd_and_fb(mask_ylt0, mask_x0));

    p         = gmx_simd_blendv_f(p, y, gmx_simd_and_fb(mask_y0, gmx_simd_or_fb(mask_xp0, mask_xgt0)));
    return p;
}
 ...
 * added to 1/r this will be insignificant.
 */
...
/*! Calculate 1/sqrt(x) for two SIMD doubles. When the SIMD single width is
 *  larger than double, we can optimize this by performing everything but the
 *  last step in single precision.
 */
static gmx_inline void
...
gmx_simd_sqrt_d(gmx_simd_double_t x)
{
    ...
    res  = gmx_simd_blendv_d(gmx_simd_invsqrt_d(x), gmx_simd_setzero_d(), mask);
...
gmx_simd_tan_d(gmx_simd_double_t x)
{
    ...
    x       = gmx_simd_fnmadd_d(y, argred2, x);
    x       = gmx_simd_fnmadd_d(y, argred3, x);
    x       = gmx_simd_xor_d(gmx_simd_blendzero_d(gmx_simd_set1_d(-0.0), mask), x); // set sign to neg where m true
#else
    const gmx_simd_double_t  quarter         = gmx_simd_set1_d(0.25);
    const gmx_simd_double_t  half            = gmx_simd_set1_d(0.5);
...
gmx_simd_atan2_d(gmx_simd_double_t y, gmx_simd_double_t x)
{
    ...
    const gmx_simd_double_t halfpi      = gmx_simd_set1_d(M_PI/2.0);
    gmx_simd_double_t       p, aoffset;
    gmx_simd_double_t       xsign, ysign;
    gmx_simd_dbool_t        mask_xneg;
    gmx_simd_dbool_t        mask_xlt0, mask_ylt0, mask_ygt0, mask_xgt0;
    gmx_simd_dbool_t        mask_ym0, mask_yp0, mask_xp0, mask_x0, mask_y0;

    mask_xlt0 = gmx_simd_cmplt_d(x, gmx_simd_setzero_d());
    mask_ylt0 = gmx_simd_cmplt_d(y, gmx_simd_setzero_d());
    aoffset   = gmx_simd_blendzero_d(pi, mask_xlt0);
    aoffset   = gmx_simd_blendv_d(aoffset, gmx_simd_fneg_d(aoffset), mask_ylt0);

    p         = gmx_simd_mul_d(y, gmx_simd_inv_d(x));
    p         = gmx_simd_atan_d(p);
     ...
     * Note that x==-0.0 does not count as x<0 !
     */
    xsign     = gmx_simd_xor_sign_d(gmx_simd_set1_d(1.0), x);
    ysign     = gmx_simd_xor_sign_d(gmx_simd_set1_d(1.0), y);
    mask_xneg = gmx_simd_cmplt_d(xsign, gmx_simd_setzero_d());

    mask_xgt0 = gmx_simd_cmplt_d(gmx_simd_setzero_d(), x);
    mask_ygt0 = gmx_simd_cmplt_d(gmx_simd_setzero_d(), y);
    mask_x0   = gmx_simd_cmpeq_d(x, gmx_simd_setzero_d());
    mask_y0   = gmx_simd_cmpeq_d(y, gmx_simd_setzero_d());
    mask_xp0  = gmx_simd_and_db(mask_x0, gmx_simd_cmplt_d(gmx_simd_setzero_d(), xsign));
    mask_yp0  = gmx_simd_and_db(mask_y0, gmx_simd_cmplt_d(gmx_simd_setzero_d(), ysign));
    mask_ym0  = gmx_simd_and_db(mask_y0, gmx_simd_cmplt_d(ysign, gmx_simd_setzero_d()));

    p         = gmx_simd_blendv_d(p, pi, gmx_simd_and_db(mask_yp0, mask_xneg));
    p         = gmx_simd_blendv_d(p, gmx_simd_fneg_d(pi), gmx_simd_and_db(mask_ym0, mask_xneg));

    p         = gmx_simd_blendv_d(p, halfpi, gmx_simd_and_db(mask_ygt0, mask_x0));
    p         = gmx_simd_blendv_d(p, gmx_simd_fneg_d(halfpi), gmx_simd_and_db(mask_ylt0, mask_x0));

    p         = gmx_simd_blendv_d(p, y, gmx_simd_and_db(mask_y0, gmx_simd_or_db(mask_xp0, mask_xgt0)));
    return p;
}
 ...
 * See gmx_simd_pmecorrF_f() for details about the approximation.
 *
 * This double version achieves an accuracy slightly lower than 1e-10; when
 * added to 1/r this will be insignificant.
 *
 */
...
gmx_simd_pmecorrF_d(gmx_simd_double_t z2)
{
    ...
    polyFD1        = gmx_simd_mul_d(FD5, z4);
    polyFD0        = gmx_simd_mul_d(FD4, z4);
    polyFD1        = gmx_simd_add_d(polyFD1, FD3);
    polyFD0        = gmx_simd_add_d(polyFD0, FD2);
    polyFD1        = gmx_simd_mul_d(polyFD1, z4);
    polyFD0        = gmx_simd_mul_d(polyFD0, z4);
    polyFD1        = gmx_simd_add_d(polyFD1, FD1);
    polyFD0        = gmx_simd_add_d(polyFD0, FD0);
    polyFD1        = gmx_simd_mul_d(polyFD1, z2);
    polyFD0        = gmx_simd_add_d(polyFD0, polyFD1);

    polyFD0        = gmx_simd_inv_d(polyFD0);

    polyFN0        = gmx_simd_mul_d(FN10, z4);
    polyFN1        = gmx_simd_mul_d(FN9, z4);
    polyFN0        = gmx_simd_add_d(polyFN0, FN8);
    polyFN1        = gmx_simd_add_d(polyFN1, FN7);
    polyFN0        = gmx_simd_mul_d(polyFN0, z4);
    polyFN1        = gmx_simd_mul_d(polyFN1, z4);
    polyFN0        = gmx_simd_add_d(polyFN0, FN6);
    polyFN1        = gmx_simd_add_d(polyFN1, FN5);
    polyFN0        = gmx_simd_mul_d(polyFN0, z4);
    polyFN1        = gmx_simd_mul_d(polyFN1, z4);
    polyFN0        = gmx_simd_add_d(polyFN0, FN4);
    polyFN1        = gmx_simd_add_d(polyFN1, FN3);
    polyFN0        = gmx_simd_mul_d(polyFN0, z4);
    polyFN1        = gmx_simd_mul_d(polyFN1, z4);
    polyFN0        = gmx_simd_add_d(polyFN0, FN2);
    polyFN1        = gmx_simd_add_d(polyFN1, FN1);
    polyFN0        = gmx_simd_mul_d(polyFN0, z4);
    polyFN1        = gmx_simd_mul_d(polyFN1, z2);
    polyFN0        = gmx_simd_add_d(polyFN0, FN0);
    polyFN0        = gmx_simd_add_d(polyFN0, polyFN1);
 ...
 *
 * See gmx_simd_pmecorrV_f() for details about the approximation.
 *
 * This double version achieves an accuracy slightly lower than 1e-10; when
 * added to 1/r this will be insignificant.
 */
...
gmx_simd_pmecorrV_d(gmx_simd_double_t z2)
{
    ...
    polyVD1        = gmx_simd_mul_d(VD5, z4);
    polyVD0        = gmx_simd_mul_d(VD4, z4);
    polyVD1        = gmx_simd_add_d(polyVD1, VD3);
    polyVD0        = gmx_simd_add_d(polyVD0, VD2);
    polyVD1        = gmx_simd_mul_d(polyVD1, z4);
    polyVD0        = gmx_simd_mul_d(polyVD0, z4);
    polyVD1        = gmx_simd_add_d(polyVD1, VD1);
    polyVD0        = gmx_simd_add_d(polyVD0, VD0);
    polyVD1        = gmx_simd_mul_d(polyVD1, z2);
    polyVD0        = gmx_simd_add_d(polyVD0, polyVD1);

    polyVD0        = gmx_simd_inv_d(polyVD0);

    polyVN1        = gmx_simd_mul_d(VN9, z4);
    polyVN0        = gmx_simd_mul_d(VN8, z4);
    polyVN1        = gmx_simd_add_d(polyVN1, VN7);
    polyVN0        = gmx_simd_add_d(polyVN0, VN6);
    polyVN1        = gmx_simd_mul_d(polyVN1, z4);
    polyVN0        = gmx_simd_mul_d(polyVN0, z4);
    polyVN1        = gmx_simd_add_d(polyVN1, VN5);
    polyVN0        = gmx_simd_add_d(polyVN0, VN4);
    polyVN1        = gmx_simd_mul_d(polyVN1, z4);
    polyVN0        = gmx_simd_mul_d(polyVN0, z4);
    polyVN1        = gmx_simd_add_d(polyVN1, VN3);
    polyVN0        = gmx_simd_add_d(polyVN0, VN2);
    polyVN1        = gmx_simd_mul_d(polyVN1, z4);
    polyVN0        = gmx_simd_mul_d(polyVN0, z4);
    polyVN1        = gmx_simd_add_d(polyVN1, VN1);
    polyVN0        = gmx_simd_add_d(polyVN0, VN0);
    polyVN1        = gmx_simd_mul_d(polyVN1, z2);
    polyVN0        = gmx_simd_add_d(polyVN0, polyVN1);
...
/*! SIMD real sincos(x,&sin,&val) */
#    define gmx_simd_sincos_r         gmx_simd_sincos_f
/*! SIMD real sin(x). Use sincos() if you need both sin & cos! */
#    define gmx_simd_sin_r            gmx_simd_sin_f
/*! SIMD real cos(x). Use sincos() if you need both sin & cos! */
...
/*! Analytic PME force correction. See gmx_simd_pmecorrF_f() for details. */
#    define gmx_simd_pmecorrF_r       gmx_simd_pmecorrF_f
/*! Analytic PME potential correction. See gmx_simd_pmecorrV_f() for details. */
