...
#ifdef GMX_DOUBLE
#error "Double precision isn't supported on Intel MIC yet"
#endif
...
gmx_cpsgn_nonneg_pr(gmx_mm_ps a, gmx_mm_ps b)
{
    ...
    //TODO (only bond): Bitwise operations on floating points can be done after casting to int. That allows us to do it the same way as AVX which might be faster.
...
#define gmx_anytrue_pb _mm512_mask2int //This returns the full mask (not 1 for True). But given that any non-zero is True this is OK.
...
{
    //TODO (only bond): optimize that both are calculated together. Or (if if that isn't fast on MIC) don't call sincos if only one is needed.
    *s = _mm512_sin_ps(a);
    *c = _mm512_cos_ps(a);
    return 0;
}
