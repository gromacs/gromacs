...
{
#ifndef NDEBUG
    int alignReq = (GMX_SIMD_DOUBLE_WIDTH > 4) ? 4 : GMX_SIMD_DOUBLE_WIDTH;
    /* Make sure:
     ...
     */
    assert((size_t)offset % (GMX_SIMD_DINT32_WIDTH*sizeof(std::int32_t)) == 0);
    assert((size_t)base % (alignReq*sizeof(double)) == 0);
    assert(align % alignReq == 0);
#endif
...
{
#ifndef NDEBUG
    int alignReq = (GMX_SIMD_DOUBLE_WIDTH > 2) ? 2 : GMX_SIMD_DOUBLE_WIDTH;
    /* Make sure:
     * 1) the offset pointer fulfills hardware alignment demand
     * 2) the base pointer fulfills hardware alignment demand
     * 3) the specified alignment is a multiple of hardware alignment demand
 ...
 *
 * \copydetails simdExpandScalarsToTripletsF
 */
static inline void
 ...
 *
 * \copydetails simdGatherLoadUBySimdIntTransposeF
 */
template <int align>
static inline void
                                  ...
                                  SimdDouble *         v1)
{
    ...
    simdGatherLoadUBySimdIntTransposeD<align>(base, offset, v0, v1);
}
                          ...
                          SimdDouble         v3)
{
    ...
    assert((size_t)m % (alignReq*sizeof(double)) == 0);
#endif

    /* Note that the 4 here corresponds to the 4 m-elements, not any SIMD width */
    double sum[4];

    sum[0] = simdReduceD(v0);
    sum[1] = simdReduceD(v1);
    sum[2] = simdReduceD(v2);
    sum[3] = simdReduceD(v3);
...
simdLoadDualHsimdD(const double *  m0,
                   const double *  m1)
{
    SimdDouble        a;
    ...
    for (int i = 0; i < GMX_SIMD_DOUBLE_WIDTH/2; i++)
    {
        a.r[i]                           = m0[i];
 ...
 */
static inline SimdDouble
simdLoadDupHsimdD(const double *  m)
{
    SimdDouble        a;
 ...
 */
static inline SimdDouble
simdLoad1DualHsimdD(const double *  m)
{
    SimdDouble        a;
...
template <int align>
static inline void
simdGatherLoadTransposeHsimdD(const double  *       base0,
                              const double  *       base1,
                              std::int32_t          offset[],
                              SimdDouble *          v0,
                              SimdDouble *          v1)
{
     ...
     * 3) the specified alignment is a multiple of hardware alignment demand
     */
    assert((size_t)offset % (GMX_SIMD_DINT32_WIDTH/2*sizeof(std::int32_t)) == 0);
    assert((size_t)base0 % (alignReq*sizeof(double)) == 0);
    assert((size_t)base1 % (alignReq*sizeof(double)) == 0);
    assert(align % alignReq == 0);
#endif
 ...
 */
static inline double
simdReduceIncr4ReturnSumHsimdD(double *           m,
                               SimdDouble         v0,
                               SimdDouble         v1)
{
#ifndef NDEBUG
