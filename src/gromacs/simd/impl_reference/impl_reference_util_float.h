 ...
 */
const int c_simdBestPairAlignmentF = 2;
                        ...
                        SimdFloat *  triplets2)
{
    for (std::size_t i = 0; i < scalar.simdInternal_.size(); i++)
    {
        ...
        triplets1->simdInternal_[i] = scalar.simdInternal_[(i + GMX_SIMD_FLOAT_WIDTH) / 3];
        triplets2->simdInternal_[i] = scalar.simdInternal_[(i + 2 * GMX_SIMD_FLOAT_WIDTH) / 3];
    }
}
                             ...
                             SimdFloat *         v1)
{
    ...
    gatherLoadUBySimdIntTranspose<align>(base, offset, v0, v1);
}
