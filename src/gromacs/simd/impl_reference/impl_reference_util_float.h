...
{
#ifndef NDEBUG
    int alignReq = (GMX_SIMD_FLOAT_WIDTH > 4) ? 4 : GMX_SIMD_FLOAT_WIDTH;
    /* Make sure:
     ...
     */
    assert((size_t)offset % (GMX_SIMD_FINT32_WIDTH*sizeof(std::int32_t)) == 0);
    assert((size_t)base % (alignReq*sizeof(float)) == 0);
...
{
#ifndef NDEBUG
    int alignReq = (GMX_SIMD_FLOAT_WIDTH > 2) ? 2 : GMX_SIMD_FLOAT_WIDTH;
    /* Make sure:
     * 1) the offset pointer fulfills hardware alignment demand
     * 2) the base pointer fulfills hardware alignment demand
     * 3) the specified alignment is a multiple of hardware alignment demand
                          ...
                          SimdFloat         v3)
{
    ...
    assert((size_t)m % (alignReq*sizeof(float)) == 0);
#endif
                   ...
                   const float *  m1)
{
    ...
    assert((size_t)m0 % (GMX_SIMD_FLOAT_WIDTH/2*sizeof(float)) == 0);
    assert((size_t)m1 % (GMX_SIMD_FLOAT_WIDTH/2*sizeof(float)) == 0);
...
simdLoadDupHsimdF(const float *  m)
{
    ...
    /* Make sure the memory pointer is aligned */
    assert((size_t)m % (GMX_SIMD_FLOAT_WIDTH/2*sizeof(float)) == 0);
                              ...
                              SimdFloat *          v1)
{
     ...
     * 3) the specified alignment is a multiple of hardware alignment demand
     */
    assert((size_t)offset % (GMX_SIMD_FINT32_WIDTH/2*sizeof(std::int32_t)) == 0);
    assert((size_t)base0 % (alignReq*sizeof(float)) == 0);
    assert((size_t)base1 % (alignReq*sizeof(float)) == 0);
    assert(align % alignReq == 0);
#endif
