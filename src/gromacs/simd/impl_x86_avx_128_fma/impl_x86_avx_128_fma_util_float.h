                     ...
                     SimdFloat *          v2)
{
    ...
    if ( (align & 0x3) == 0)
    {
        v0->simdInternal_ = _mm_loadu_ps( base + align * offset[0] );
        v1->simdInternal_ = _mm_loadu_ps( base + align * offset[1] );
        v2->simdInternal_ = _mm_loadu_ps( base + align * offset[2] );
        t3                = _mm_loadu_ps( base + align * offset[3] );
    }
    else
    {
        v0->simdInternal_ = gmx_mm_maskload_ps(base + align * offset[0], mask);
        v1->simdInternal_ = gmx_mm_maskload_ps(base + align * offset[1], mask);
        v2->simdInternal_ = gmx_mm_maskload_ps(base + align * offset[2], mask);
        t3                = gmx_mm_maskload_ps(base + align * offset[3], mask);
    }
    _MM_TRANSPOSE4_PS(v0->simdInternal_, v1->simdInternal_, v2->simdInternal_, t3);
}
                       ...
                       SimdFloat            v2)
{
    ...
    __m128i mask = _mm_set_epi32(0, -1, -1, -1);
    _MM_TRANSPOSE4_PS(v0.simdInternal_, v1.simdInternal_, v2.simdInternal_, t3);
    gmx_mm_maskstore_ps(base + align * offset[0], mask, v0.simdInternal_);
    gmx_mm_maskstore_ps(base + align * offset[1], mask, v1.simdInternal_);
    gmx_mm_maskstore_ps(base + align * offset[2], mask, v2.simdInternal_);
