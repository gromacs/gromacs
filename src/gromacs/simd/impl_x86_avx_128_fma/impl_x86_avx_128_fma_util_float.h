                          ...
                          SimdFloat *          v2)
{
    ...
    if ( (align & 0x3) == 0)
    {
        v0->r = _mm_loadu_ps( base + align * offset[0] );
        v1->r = _mm_loadu_ps( base + align * offset[1] );
        v2->r = _mm_loadu_ps( base + align * offset[2] );
        t3    = _mm_loadu_ps( base + align * offset[3] );
    }
    else
    {
        v0->r = gmx_mm_maskload_ps(base + align * offset[0], mask);
        v1->r = gmx_mm_maskload_ps(base + align * offset[1], mask);
        v2->r = gmx_mm_maskload_ps(base + align * offset[2], mask);
        t3    = gmx_mm_maskload_ps(base + align * offset[3], mask);
    }
    _MM_TRANSPOSE4_PS(v0->r, v1->r, v2->r, t3);
}
                            ...
                            SimdFloat            v2)
{
    ...
    __m128i mask = _mm_set_epi32(0, -1, -1, -1);
    _MM_TRANSPOSE4_PS(v0.r, v1.r, v2.r, t3);
    gmx_mm_maskstore_ps(base + align * offset[0], mask, v0.r);
    gmx_mm_maskstore_ps(base + align * offset[1], mask, v1.r);
    gmx_mm_maskstore_ps(base + align * offset[2], mask, v2.r);
...
simdExpandScalarsToTripletsF(SimdFloat    scalar,
                             ...
                             SimdFloat *  triplets2)
{
    triplets0->r = _mm_permute_ps(scalar.r, _MM_SHUFFLE(1, 0, 0, 0));
    triplets1->r = _mm_permute_ps(scalar.r, _MM_SHUFFLE(2, 2, 1, 1));
    triplets2->r = _mm_permute_ps(scalar.r, _MM_SHUFFLE(3, 3, 3, 2));
}
                          ...
                          SimdFloat  v3)
{
    ...
    __m128 b = _mm_add_ps(v0.r, _mm_permute_ps(v0.r, _MM_SHUFFLE(1, 0, 3, 2)));
