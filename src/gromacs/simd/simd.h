...
#define GMX_ALIGNED(type, alignment) alignas((GMX_SIMD_HAVE_DOUBLE && \
                                              (!GMX_SIMD_HAVE_FLOAT || GMX_SIMD_DOUBLE_WIDTH*2 >= GMX_SIMD_FLOAT_WIDTH)) ? \
                                             (((GMX_SIMD_DOUBLE_WIDTH < 4 && GMX_SIMD4_HAVE_DOUBLE) ? 4 : GMX_SIMD_DOUBLE_WIDTH)*sizeof(double)) : \
                                             (GMX_SIMD_FLOAT_WIDTH*sizeof(float))) \
    type
