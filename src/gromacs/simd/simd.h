...
struct ReverseSimdTraits<const T>
{
    ...
    using tag = typename ReverseSimdTraits<T>::tag;
};
...
{
    return simdLoad(m, typename ReverseSimdTraits<T>::tag());
}
...
{
    return simdLoad(m.data(), typename ReverseSimdTraits<T>::tag());
}
...
//TODO: Unaligned load+reference need to be simplified same as aligned version

template <typename T> //can be either float/double/int, each const or non-const
class SimdUReference;

template<typename T>
static inline SimdUReference<T> gmx_simdcall
loadU(T *m);
...
class SimdUReference
{
    ...
    private:
        ...
        template<typename U>
        friend SimdUReference<U> gmx_simdcall
        loadU(U *m);
