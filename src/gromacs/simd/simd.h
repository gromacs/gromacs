...
{
    public:
        //! \brief Construct from pointer (only the load function should call this)
        explicit SimdLoadIProxyInternal(const std::int32_t *m) : m_(m) {}
...
{
    return SimdLoadIProxyInternal(m);
}
...
{
    public:
        //!\brief Construct from pointer (only the loadU function should call this)
        explicit SimdLoadUIProxyInternal(const std::int32_t *m) : m_(m) {}
...
{
    return SimdLoadUIProxyInternal(m);
}
...
{
    public:
        //!\brief Construct empty object. Only the setZero function should call this.
...
setZero()
{
    return SimdSetZeroProxyInternal();
}
