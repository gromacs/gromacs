 ...
 * The function forwarded here don't need to be implemented by each implementation.
 * For width=4 all functions are forwarded and for width=8 all but load4NOffset are forwarded.
 */
#if GMX_SIMD_HAVE_FLOAT
...
static inline SimdFloat gmx_simdcall
load4NOffset(const float* f, int)
{
    return loadU(f);
}
#elif GMX_SIMD_FLOAT_WIDTH == 8 && GMX_SIMD_HAVE_HSIMD_UTIL_FLOAT
static inline SimdFloat gmx_simdcall
loadNDuplicate4(const float* f)
{
    return load1DualHsimd(f);
}
static inline SimdFloat gmx_simdcall
load4DuplicateN(const float* f)
{
...
loadNDuplicate4(const double* f)
{
    return SimdDouble(*f);
}
static inline SimdDouble gmx_simdcall
...
static inline SimdDouble gmx_simdcall
load4NOffset(const double* f, int)
{
    return load(f);
}
#elif GMX_SIMD_DOUBLE_WIDTH == 8 && GMX_SIMD_HAVE_HSIMD_UTIL_DOUBLE
static inline SimdDouble gmx_simdcall
loadNDuplicate4(const double* f)
{
    return load1DualHsimd(f);
