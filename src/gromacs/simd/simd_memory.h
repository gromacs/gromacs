...
//Careful the Simd version is by default assuming alignment (unaligned version is TODO)
#if GMX_SIMD_HAVE_FLOAT
template<>
class ArrayRef<SimdFloat>  : public SimdArrayRef<float>
{
    using Base = SimdArrayRef<float>;
    using Base::Base;
};
#endif
#if GMX_SIMD_HAVE_DOUBLE
template<>
class ArrayRef<SimdDouble> : public SimdArrayRef<double>
{
    using Base = SimdArrayRef<double>;
    using Base::Base;
};
#endif
