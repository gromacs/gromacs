...
gmx_simd_set_exponent_ibm_qpx(vector4double x)
{
    ...
    /* Store to memory for shifts. vec_round() is important here, since
     * IBM is stupid enough to use DIFFERENT rounding modes for vec_round()
     * and vec_ctid() by default, and we do require that they are consistent.
     */
    vec_st(vec_ctid(vec_round(x)), 0, idata);
    /* Perform integer arithmetics in general registers. */
