...
static inline void gmx_simdcall
gatherLoadBySimdIntTranspose(const double * base, SimdDInt32 simdoffset, SimdDouble *v, Targs... Fargs)
{
    if (align > 1)
    {
        simdoffset = fastMultiply<align>(simdoffset);
    }
    v->simdInternal_ = _mm512_i32gather_pd(simdoffset.simdInternal_, base, sizeof(double));
    gatherLoadBySimdIntTranspose<1>(base+1, simdoffset, Fargs ...);
}

template <int align, typename ... Targs>
static inline void gmx_simdcall
gatherLoadUBySimdIntTranspose(const double *base, SimdDInt32 simdoffset, Targs... Fargs)
{
