...
class SimdDouble
{
    public:
        ...
        print(const char *s)
        {
            printf("%s:", s);
            double d[4];
            _mm256_storeu_pd(d, simdInternal_);
...
static inline SimdDouble gmx_simdcall
maskedAdd(SimdDouble a, SimdDouble b, SimdDBool m)
{
    return {
               _mm256_and_pd(_mm256_add_pd(a.simdInternal_, b.simdInternal_), m.simdInternal_)
    };
}

static inline SimdDouble gmx_simdcall
maskedMul(SimdDouble a, SimdDouble b, SimdDBool m)
{
    return {
...
maskedFma(SimdDouble a, SimdDouble b, SimdDouble c, SimdDBool m)
{
    return {
...
static inline SimdDouble
maskedRsqrt(SimdDouble x, SimdDBool m)
{
#ifndef NDEBUG
...
static inline SimdDouble
maskedRcp(SimdDouble x, SimdDBool m)
{
#ifndef NDEBUG
    x.simdInternal_ = _mm256_blendv_pd(_mm256_set1_pd(1.0), x.simdInternal_, m.simdInternal_);
#endif
