                     ...
                     SimdFloat *          v2)
{
    ...
    else
    {
        ...
        v0->simdInternal_ = _mm_movelh_ps(t1, t3);
        v1->simdInternal_ = _mm_movehl_ps(t3, t1);
        t5                = _mm_unpacklo_ps(t5, t6);
        t7                = _mm_unpacklo_ps(t7, t8);
        v2->simdInternal_ = _mm_movelh_ps(t5, t7);
    }
}
#endif
                             ...
                             SimdFloat *    v3)
{
    ...
    if (align == 4 || align == 8 || align == 12 || align == 16)
    {
        ...
        v0->simdInternal_ = _mm_load_ps(base + extract<0>(offset) );
        v1->simdInternal_ = _mm_load_ps(base + extract<1>(offset) );
        v2->simdInternal_ = _mm_load_ps(base + extract<2>(offset) );
        v3->simdInternal_ = _mm_load_ps(base + extract<3>(offset) );
    }
    else
    {
        ...
        v0->simdInternal_ = _mm_load_ps(base + align * extract<0>(offset) );
        v1->simdInternal_ = _mm_load_ps(base + align * extract<1>(offset) );
        v2->simdInternal_ = _mm_load_ps(base + align * extract<2>(offset) );
        v3->simdInternal_ = _mm_load_ps(base + align * extract<3>(offset) );
    }
    _MM_TRANSPOSE4_PS(v0->simdInternal_, v1->simdInternal_, v2->simdInternal_, v3->simdInternal_);
}
                             ...
                             SimdFloat *     v1)
{
    ...
    {
        v0->simdInternal_ = _mm_castpd_ps(_mm_load_sd(reinterpret_cast<const double *>(base + extract<0>(offset))));
        v1->simdInternal_ = _mm_castpd_ps(_mm_load_sd(reinterpret_cast<const double *>(base + extract<1>(offset))));
        t1                = _mm_castpd_ps(_mm_load_sd(reinterpret_cast<const double *>(base + extract<2>(offset))));
        t2                = _mm_castpd_ps(_mm_load_sd(reinterpret_cast<const double *>(base + extract<3>(offset))));
    }
    else
    {
        v0->simdInternal_ = _mm_castpd_ps(_mm_load_sd(reinterpret_cast<const double *>(base + align * extract<0>(offset))));
        v1->simdInternal_ = _mm_castpd_ps(_mm_load_sd(reinterpret_cast<const double *>(base + align * extract<1>(offset))));
        t1                = _mm_castpd_ps(_mm_load_sd(reinterpret_cast<const double *>(base + align * extract<2>(offset))));
        t2                = _mm_castpd_ps(_mm_load_sd(reinterpret_cast<const double *>(base + align * extract<3>(offset))));
    }
    t1                = _mm_unpacklo_ps(v0->simdInternal_, t1);
    t2                = _mm_unpacklo_ps(v1->simdInternal_, t2);
    v0->simdInternal_ = _mm_unpacklo_ps(t1, t2);
