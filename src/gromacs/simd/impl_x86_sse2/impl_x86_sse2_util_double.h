                         ...
                         SimdDouble *          v3)
{
    ...
    v0->r = _mm_unpacklo_pd(t1, t2);
    v1->r = _mm_unpackhi_pd(t1, t2);
    v2->r = _mm_unpacklo_pd(t3, t4);
    v3->r = _mm_unpackhi_pd(t3, t4);
}
                            ...
                            SimdDouble          v2)
{
    ...
    _mm_store_sd(base + align * offset[0] + 2, v2.r);
    _mm_storeu_pd(base + align * offset[1], t2);
    _mm_storeh_pi(reinterpret_cast< __m64 *>(base + align * offset[1] + 2), _mm_castpd_ps(v2.r));
}
                           ...
                           SimdDouble          v2)
{
    ...
    t5          = _mm_unpacklo_pd(v0.r, v1.r);
    t6          = _mm_unpackhi_pd(v0.r, v1.r);
    t7          = _mm_unpackhi_pd(v2.r, v2.r);

    t1          = _mm_add_pd(t1, t5);
    t2          = _mm_add_sd(t2, v2.r);
    t3          = _mm_add_pd(t3, t6);
    t4          = _mm_add_sd(t4, t7);
...
{
    triplets0->r = _mm_shuffle_pd(scalar.r, scalar.r, _MM_SHUFFLE2(0, 0));
    triplets1->r = _mm_shuffle_pd(scalar.r, scalar.r, _MM_SHUFFLE2(1, 0));
    triplets2->r = _mm_shuffle_pd(scalar.r, scalar.r, _MM_SHUFFLE2(1, 1));
}
#endif
                                  ...
                                  SimdDouble *    v3)
{
    ...
    {
        assert(std::size_t(base + simdExtractDI<0>(offset)) % 16 == 0);
        assert(std::size_t(base + simdExtractDI<1>(offset)) % 16 == 0);

        t1  = _mm_load_pd(base + simdExtractDI<0>(offset));
        t2  = _mm_load_pd(base + simdExtractDI<1>(offset));
        t3  = _mm_load_pd(base + simdExtractDI<0>(offset) + 2);
        t4  = _mm_load_pd(base + simdExtractDI<1>(offset) + 2);
    }
    else
    {
        assert(std::size_t(base + align * simdExtractDI<0>(offset)) % 16 == 0);
        assert(std::size_t(base + align * simdExtractDI<1>(offset)) % 16 == 0);

        t1  = _mm_load_pd(base + align * simdExtractDI<0>(offset));
        t2  = _mm_load_pd(base + align * simdExtractDI<1>(offset));
        t3  = _mm_load_pd(base + align * simdExtractDI<0>(offset) + 2);
        t4  = _mm_load_pd(base + align * simdExtractDI<1>(offset) + 2);
    }
    v0->r = _mm_unpacklo_pd(t1, t2);
    v1->r = _mm_unpackhi_pd(t1, t2);
    v2->r = _mm_unpacklo_pd(t3, t4);
                                   ...
                                   SimdDouble *    v1)
{
    ...
    {
        t1  = _mm_loadu_pd(base + simdExtractDI<0>(offset));
        t2  = _mm_loadu_pd(base + simdExtractDI<1>(offset));
    }
    else
    {
        t1  = _mm_loadu_pd(base + align * simdExtractDI<0>(offset));
        t2  = _mm_loadu_pd(base + align * simdExtractDI<1>(offset));
    }
    v0->r = _mm_unpacklo_pd(t1, t2);
    v1->r = _mm_unpackhi_pd(t1, t2);
}
