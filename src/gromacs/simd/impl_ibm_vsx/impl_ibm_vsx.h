 ...
 */
#undef vector
/* g++ is unhappy with the clash of vector bool and the C++ reserved 'bool', which
 * is solved by undefining bool and reyling on __bool. However, that does not work
 * with xlc, which requires is to use bool. Solve the conflict by defining
 * a new vsx_bool.
 */
#ifdef __GNUC__
...
#else
#define gmx_vsx_extract_f(a, i)   vec_extract(a, i)
#endif
#else
/* IBM xlC */
