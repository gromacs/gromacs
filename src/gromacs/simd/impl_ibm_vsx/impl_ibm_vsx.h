 ...
 *
 * Unfortunately GCC and xlC appear to have diverged in their naming of
 * intrinsics, and this far we have only had access to GCC for testing.
 * There are alternative define sections for the cases we are aware of, and
 * since xlC is more complete if anything, it should be trivial to fix in
 * case we missed something.
 * If you have access to xlC, please provide feedback!
 */
...
#else
/* IBM xlC, maybe others */
#    define gmx_simd_load_f(m)     vec_xlw4(0, (const float *)(m))
...
#else
/* IBM xlC, maybe others */
#    define gmx_simd_load_fi(m)     vec_xlw4(0, (const int *)m)
...
#ifdef __GNUC__
/* gcc up to at least version 4.9 is buggy with vec_extract(), use inline asm instead */
#    define gmx_simd_extract_fi(a, i)   ({ int res;  __asm__ ("mfvsrwz %0,%1" : "=r" (res) : "ww" ((vec_xxsldwi((a), (a), (((2-(i))&0x3)))))); res; })
#else
/* IBM xlC, maybe others */
#    define gmx_simd_extract_fi(a, i)   vec_extract(a, i)
#endif
...
#define gmx_simd_anytrue_fb(a)     vec_any_ne(a, (__vector __bool int)vec_splats(0))
...
#define gmx_simd_anytrue_fib(a)          vec_any_ne(a, (__vector __bool int)vec_splats(0))
...
#ifdef __GNUC__
#    define gmx_simd_load_d(m)      vec_vsx_ld(0, (__vector double *)(m))
#    define gmx_simd_store_d(m, x)  vec_vsx_st(x, 0, (__vector double *)(m))
#else
/* IBM xlC, maybe others */
#    define gmx_simd_load_d(m)      vec_xlw4(0, (const double *)(m))
#    define gmx_simd_store_d(m, x)  vec_xstw4(x, 0, (const double *)(m))
#endif
...
#define gmx_simd_dint32_t          __vector int
 ...
 */
#    define gmx_simd_cvtt_d2i(a)   ({ __vector int res; __asm__ ("xvcvdpsxws %0,%1" : \
                                                                 "=ww" (res) : "ww" ((__vector double) (a))); res; })
#    define gmx_simd_cvt_i2d(a)    ({ __vector double res; __asm__ ("xvcvsxwdp %0,%1" : \
                                                                    "=ww" (res) : "ww" ((__vector int) (a))); res; })
#else
/* IBM xlC, maybe others */
...
#ifdef __GNUC__
/* gcc up to at least version 4.9 is buggy with vec_extract(), use inline asm instead */
#    define gmx_simd_extract_di(a, i)    ({ int res;  __asm__ ("mfvsrwz %0,%1" : "=r" (res) : "ww" ((vec_xxsldwi((a), (a), (((2-(i*2))&0x3)))))); res; })
#else
#    define gmx_simd_extract_di(a, i)    vec_extract(a, i)
#endif
...
#define gmx_simd_dbool_t           __vector __bool long long
...
#define gmx_simd_and_db(a, b)      (__vector int)vec_and((__vector int)a, (__vector int) b)
#define gmx_simd_or_db(a, b)       (__vector int)vec_or((__vector int)a, (__vector int) b)
#define gmx_simd_anytrue_db(a)     vec_any_ne((__vector int)a, (__vector __bool int)vec_splats(0))
...
/* Boolean & comparison operations on gmx_simd_fint32_t */
#define gmx_simd_dibool_t          __vector __bool int
...
#define gmx_simd_anytrue_dib(a)          vec_any_ne(a, (__vector __bool int)vec_splats(0))
#define gmx_simd_blendzero_di(a, sel)    vec_and(a, (__vector int)sel)
#define gmx_simd_blendnotzero_di(a, sel) vec_andc(a, (__vector int)sel)
#define gmx_simd_blendv_di(a, b, sel)    vec_sel(a, b, sel)
/* Conversions between different booleans */
#define gmx_simd_cvt_db2dib(x)     (__vector __bool int)(x)
#define gmx_simd_cvt_dib2db(x)     (__vector __bool long long)(x)
...
/* Convenience define to work around GCC/xlC compiler differences */
#ifdef __GNUC__
...
/* vec_extract() is buggy in gcc, at least up to version 4.9. Use inline asm. */
#    define gmx_vsx_extract_f(a, i)   ({ float res;  __asm__ ("mfvsrwz %0,%1" : \
                                                              "=r" (res) : "ww" ((__builtin_vsx_xxsldwi((a), (a), (((2-(i))&0x3)))))); res; })
#else
/* IBM xlC, maybe others */
...
#    define gmx_vsx_f2d(x)       vec_cvf(x, 0)
#    define gmx_vsx_d2f(x)       vec_cvf(x, 0)
#    define gmx_vsx_extract_f    vec_extract
#endif

static void
print_vf(const char *s, gmx_simd_float_t x)
{
    float data[4];
    int   i;
...
static void
print_vd(const char *s, gmx_simd_double_t x)
{
    double data[2];
    int    i;

    vec_vsx_st(x, 0, (__vector double *)data);
    printf("%s:", s);
    for (i = 0; i < 2; i++)
    {
        printf("%20.15g ", data[i]);
    }
    printf("\n");
}

static void
print_vi(const char *s, gmx_simd_dint32_t x)
{
    int data[4];
    int i;
...
{
    __vector int vi;
    __vector int d0 = vec_splats(m[0]);
    __vector int d1 = vec_splats(m[1]);
    vi = (__vector int)vec_xxpermdi((__vector double)d1, (__vector double)d0, 0x0);
    return vi;
}
...
gmx_simd_get_exponent_d_ibm_vsx(gmx_simd_double_t x)
{
    ...
    iexp = (__vector int)gmx_simd_and_d(x, expmask);
...
gmx_simd_set_exponent_d_ibm_vsx(gmx_simd_double_t x)
{
     ...
     */
    iexp = vec_sl(iexp, (__vector unsigned)vec_splats((int)20));
...
gmx_simd_cvt_dd2f_ibm_vsx(gmx_simd_double_t d0, gmx_simd_double_t d1)
{
    ...
    print_vd("DD2F start D0", d0);
    print_vd("DD2F start d1", d1);
...
gmx_simd4_dotproduct3_f_ibm_vsx(gmx_simd4_float_t a, gmx_simd4_float_t b)
{
    ...
    sum = vec_add(sum, vec_xxsldwi(c, c, 3));
    return gmx_vsx_extract_f(sum, 0);
}
