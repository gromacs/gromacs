...
public:
    ...
    // gcc-4.9 does not recognize that we use the parameter
    SimdFInt32(std::int32_t gmx_unused i)
...
public:
    ...
    // Internal utility constructor to simplify return statements
    SimdFBool(__vector vsxBool int simd)
        : simdInternal_(simd) {}
...
public:
    ...
    // Internal utility constructor to simplify return statements
    SimdFIBool(__vector vsxBool int simd)
        : simdInternal_(simd) {}
...
static inline SimdFloat gmx_simdcall copysign(SimdFloat x, SimdFloat y)
{
    ...
    __vector float res;
    __asm__("xvcpsgnsp %x0,%x1,%x2"
            : "=wf"(res)
            : "wf"(y.simdInternal_), "wf"(x.simdInternal_));
    return {
        res
    };
#else
