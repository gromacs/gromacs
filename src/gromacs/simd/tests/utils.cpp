    ...
    public:
        ...
        bool ScaleableAlmostEquals(const ::testing::internal::FloatingPoint<RawType> &rhs, real scaleMaxUlps) const
        {
            ...
            /* The IEEE standard says that any comparison operation
               ...
               0xffffffff as true, and that is a NaN when interpreted
               as a real. So we need the ability to test for that,
               first. */
            if (TestFunctions::HardwareSimdFunctionSet::isSimdTrue(this->ReinterpretBits(this->bits())) &&
                TestFunctions::HardwareSimdFunctionSet::isSimdTrue(rhs.ReinterpretBits(rhs.bits())))
            {
                return true;
            }
...
int g_numberOfRepeats = 1000;

GMX_TEST_OPTIONS(SIMDTestOptions, options)
{
    options->addOption(gmx::IntegerOption("numrepeats")
                           .store(&g_numberOfRepeats)
                           .description("Number of times to repeat tests with new random data"));
}
