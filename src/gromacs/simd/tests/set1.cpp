...
typedef SimdFunctionTest<real> SimdFunctionSet1;
             ...
             real                *result)
{
    ...
    result_pr = function(inputs[0][0]);
    simdFunctionSet.store_pr(result, result_pr);
}

/* Wrapper function because the macro wrapped is defined with a
   parameter. */
static gmx_inline gmx_mm_pr gmx_set1_pr_wrapper(real a)
{
    return gmx_set1_pr(a);
}

TEST_F(SimdFunctionSet1, gmx_set1_pr_Works)
{
    prepare(1, 1);
    RunTest(ReferenceFunctions::gmx_simd_ref_set1_pr,
            gmx_set1_pr_wrapper, real(0));
}
