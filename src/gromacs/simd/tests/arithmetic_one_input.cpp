...
callFunction(SimdFunctionSet     &simdFunctionSet,
             ...
             real                *result)
{
    typename SimdFunctionSet::realType a, result_pr;

    a         = simdFunctionSet.load_pr(inputs[0]);
    result_pr = function(a);
...
{
#ifdef GMX_DOUBLE
    maxUlps_ = 500.0; // empirically determined to be enough on x86
#endif
    ...
    RunTest(ReferenceFunctions::gmx_simd_ref_rsqrt_pr,
            gmx_invsqrt_pr, real(0));
}
...
{
#ifdef GMX_DOUBLE
    maxUlps_ = 100.0; // empirically determined to be enough on x86
#endif
    prepare(1, 1, 0, 5);
    RunTest(ReferenceFunctions::gmx_simd_ref_sqrt_pr,
            gmx_sqrt_pr, real(0));
}
...
#ifdef GMX_DOUBLE
    maxUlps_ = 8.0; // empirically determined to be enough on x86
#endif
    prepare(1, 1, 0, 5);
    RunTest(ReferenceFunctions::gmx_simd_ref_exp_pr,
            gmx_exp_pr, real(0));
}
...
#endif
    prepare(1, 1, -0.5, 2);
    RunTest(ReferenceFunctions::gmx_simd_ref_acos_pr,
            gmx_acos_pr, real(0));
}

/* Wrapper function because the macro wrapped is defined with
   a parameter. */
static gmx_inline gmx_mm_pr gmx_round_pr_wrapper(gmx_mm_pr a)
{
    return gmx_round_pr(a);
}
...
TEST_F(SimdFunctionWithOneRealInput, gmx_round_pr_Works)
{
    ...
    prepare(1, 1, -0.5, 100);
    RunTest(ReferenceFunctions::gmx_simd_ref_round_pr,
            gmx_round_pr_wrapper, real(0));
}
...
/* Wrapper function because the macro wrapped is defined with
   a parameter. */
static gmx_inline gmx_mm_pr gmx_floor_pr_wrapper(gmx_mm_pr a)
{
    return gmx_floor_pr(a);
}
...
TEST_F(SimdFunctionWithOneRealInput, gmx_floor_pr_Works)
{
     ...
     * testing. */
    prepare(1, 1, 0, 1024);
    RunTest(ReferenceFunctions::gmx_simd_ref_floor_pr,
            gmx_floor_pr_wrapper, real(0));
}
...
TEST_F(SimdFunctionWithOneRealInput, FunctionPairDoingTruncationOfReals_Works)
{
    ...
    RunTest(ReferenceFunctions::gmx_simd_ref_truncate_real_pr,
            gmx_truncate_real_pr, real(0));
}
#endif
