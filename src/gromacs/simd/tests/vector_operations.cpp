...
TEST(SimdTestVectorOps, gmxSimdCalcRsqR)
{
    ...
    GMX_EXPECT_SIMD_REAL_NEAR(simdR2, gmx_simd_calc_rsq_r(simdX, simdY, simdZ), 2, 0); // Allow 2 ulp difference
}
...
TEST(SimdTestVectorOps, gmxSimdIprodR)
{
    ...
    GMX_EXPECT_SIMD_REAL_NEAR(iprod, gmx_simd_iprod_r(aX, aY, aZ, bX, bY, bZ), 2, 0); // Allow 2 ulp difference
}
...
TEST(SimdTestVectorOps, gmxSimdNorm2R)
{
    ...
    GMX_EXPECT_SIMD_REAL_NEAR(simdNorm2, gmx_simd_norm2_r(simdX, simdY, simdZ), 2, 0); // Allow 2 ulp difference
}
...
TEST(SimdTestBase, gmxSimdCprodR)
{
    ...
    GMX_EXPECT_SIMD_REAL_NEAR(refcX, cX, 2, 0); // Allow 2 ulp difference
    GMX_EXPECT_SIMD_REAL_NEAR(refcY, cY, 2, 0); // Allow 2 ulp difference
    GMX_EXPECT_SIMD_REAL_NEAR(refcZ, cZ, 2, 0); // Allow 2 ulp difference
}
