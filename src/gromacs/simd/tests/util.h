...
class SimdTest : public ::testing::Test
{
    public:
        ...
        {
#ifdef GMX_DOUBLE
            ulpTol = 255LL; //!< Aim for ~twice the precision we have in single
#else
            ulpTol = 10LL;  //!< Be a bit liberal so compiler optimization doesn't bite us
#endif
            absTol = 0;
        }
        void setUlpTol(gmx_int64_t newTol)   { ulpTol = newTol; }
        void setAbsTol(real newTol)          { absTol = newTol; }
    ...
    protected:
        gmx_int64_t            ulpTol; //!< Default tolerance in units-in-last-position
        real                   absTol; //!< Default absolute tolerance
};
