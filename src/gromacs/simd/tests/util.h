...
#define GMX_EXPECT_SIMD_REAL_EQ(ref, tst) EXPECT_PRED_FORMAT4(::simdTest::compareSimdRealUlp, ref, tst, 0, 0)

/// \brief Test if a SIMD real is within tolerance of reference SIMD or real scalar.
///
/// If the reference value is a scalar, test is performed against all SIMD elements.
#define GMX_EXPECT_SIMD_REAL_NEAR(ref, tst, ulpTol, absTol) EXPECT_PRED_FORMAT4(::simdTest::compareSimdRealUlp, ref, tst, ulpTol, absTol)

/// \brief Compare test math function against reference.
///
/// The test is performed over the input value range and tolerances specified.
/// We need to use a pair for the range since Google test only provides predicate
...
#define GMX_EXPECT_SIMD_FUNC_NEAR(refFunc, tstFunc, range, ulpTol, absTol) \
    EXPECT_PRED_FORMAT5(::simdTest::compareSimdMathFunction, refFunc, tstFunc, range, ulpTol, absTol)
...
#define GMX_EXPECT_SIMD_INT_EQ(ref, tst)  EXPECT_PRED_FORMAT2(::simdTest::compareSimdInt32, ref, tst)
