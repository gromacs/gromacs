 ...
 * are ambigious when applied to value returned from load.
 *
 ...
 * Thus e.g. load(sqrt(m)) should be ambigious. This test makes sure that
 * this does not compile. This got previously broken by introducing templates
 * which influenced the overload resolution.
 *
 * The test execution code in CMakeLists.txt only tests that the code doesn't
 * compile. When making changes to this test, it is important to test that this
 * code does correctly compile besides causing the ambigious overload error.
 * This can be tested by enabling the test execution in CMakeLists.txt for
 * a non-simd build. Then the test should fail because for such a build the
 * code is non-ambigious because only the scalar version exists.
 *
