...
TEST(SimdTestMath, gmxSimdInvsqrtR)
{
    ...
    GMX_EXPECT_SIMD_FUNC_NEAR(ref_invsqrt, gmx_simd_invsqrt_r, range, defaultTol, 0);
}
...
TEST(SimdTestMath, gmxSimdInvsqrtPairR)
{
     ...
     */
    GMX_EXPECT_SIMD_FUNC_NEAR(ref_invsqrt, tst_invsqrt_pair0, range, 4*defaultTol, 0);
    GMX_EXPECT_SIMD_FUNC_NEAR(ref_invsqrt, tst_invsqrt_pair1, range, 4*defaultTol, 0);
}

TEST(SimdTestMath, gmxSimdSqrtR)
{
    // Just make sure sqrt(0)=0 works and isn't evaluated as 0*1/sqrt(0)=NaN
    GMX_EXPECT_SIMD_REAL_NEAR(setSimd3R(0, 2, 3), gmx_simd_sqrt_r(setSimd3R(0, 4, 9)), defaultTol, 0);
}
...
TEST(SimdTestMath, gmxSimdInvR)
{
    ...
    GMX_EXPECT_SIMD_FUNC_NEAR(ref_inv, gmx_simd_inv_r, range, defaultTol, 0);
    // test >0
    std::pair<real, real>  range2(1e-10, 1e10);
    GMX_EXPECT_SIMD_FUNC_NEAR(ref_inv, gmx_simd_inv_r, range2, defaultTol, 0);
}
...
TEST(SimdTestMath, gmxSimdLogR)
{
    ...
    GMX_EXPECT_SIMD_FUNC_NEAR(ref_log, gmx_simd_log_r, range, defaultTol, 0);
}
...
TEST(SimdTestMath, gmxSimdExp2R)
{
    ...
    GMX_EXPECT_SIMD_FUNC_NEAR(ref_exp2, gmx_simd_exp2_r, range, defaultTol, 0);
}
#endif
...
TEST(SimdTestMath, gmxSimdExpR)
{
    ...
    GMX_EXPECT_SIMD_FUNC_NEAR(ref_exp, gmx_simd_exp_r, range, defaultTol, 0);
}
...
TEST(SimdTestMath, gmxSimdErfR)
{
    ...
    GMX_EXPECT_SIMD_FUNC_NEAR(ref_erf, gmx_simd_erf_r, range, defaultTol, GMX_REAL_MIN);
}
...
TEST(SimdTestMath, gmxSimdErfcR)
{
    ...
    std::pair<real, real>  range(-9, 9);
    GMX_EXPECT_SIMD_FUNC_NEAR(ref_erfc, gmx_simd_erfc_r, range, 4+defaultTol, GMX_REAL_MIN);
}
...
TEST(SimdTestMath, gmxSimdSinR)
{
    ...
    GMX_EXPECT_SIMD_FUNC_NEAR(ref_sin, gmx_simd_sin_r, range, defaultTol, 0);
    // Range reduction leads to accuracy loss, so we might want higher tolerance here
    std::pair<real, real>  range2(-10000, 10000);
    GMX_EXPECT_SIMD_FUNC_NEAR(ref_sin, gmx_simd_sin_r, range2, 2*defaultTol, 0);
}
...
{
    std::pair<real, real>  range(-8*M_PI, 8*M_PI);
    GMX_EXPECT_SIMD_FUNC_NEAR(ref_cos, gmx_simd_cos_r, range, defaultTol, 0);
    // Range reduction leads to accuracy loss, so we might want higher tolerance here
    std::pair<real, real>  range2(-10000, 10000);
    GMX_EXPECT_SIMD_FUNC_NEAR(ref_cos, gmx_simd_cos_r, range2, 2*defaultTol, 0);
}
...
TEST(SimdTestMath, gmxSimdTanR)
{
    ...
    std::pair<real, real>  range(-8*M_PI, 8*M_PI);
    GMX_EXPECT_SIMD_FUNC_NEAR(ref_tan, gmx_simd_tan_r, range, defaultTol, 0);
    // Range reduction leads to accuracy loss, so we might want higher tolerance here
    std::pair<real, real>  range2(-10000, 10000);
    GMX_EXPECT_SIMD_FUNC_NEAR(ref_tan, gmx_simd_tan_r, range2, 2*defaultTol, 0);
}
...
TEST(SimdTestMath, gmxSimdAsinR)
{
    ...
    GMX_EXPECT_SIMD_FUNC_NEAR(ref_asin, gmx_simd_asin_r, range, defaultTol, 0);
}
...
TEST(SimdTestMath, gmxSimdAcosR)
{
    ...
    std::pair<real, real>  range(-1, 1);
    GMX_EXPECT_SIMD_FUNC_NEAR(ref_acos, gmx_simd_acos_r, range, defaultTol, 0);
}
...
TEST(SimdTestMath, gmxSimdAtanR)
{
    ...
    GMX_EXPECT_SIMD_FUNC_NEAR(ref_atan, gmx_simd_atan_r, range, defaultTol, 0);
}
...
TEST(SimdTestMath, gmxSimdAtan2R)
{
    ...
    GMX_EXPECT_SIMD_REAL_NEAR(atan2(1.0, 1.0), gmx_simd_atan2_r(rSimd_1_2_3, rSimd_1_2_3), defaultTol, 0);
    GMX_EXPECT_SIMD_REAL_NEAR(atan2(-1.0, 1.0), gmx_simd_atan2_r(rSimd_m1_m2_m3, rSimd_1_2_3), defaultTol, 0);
    GMX_EXPECT_SIMD_REAL_NEAR(atan2(-1.0, -1.0), gmx_simd_atan2_r(rSimd_m1_m2_m3, rSimd_m1_m2_m3), defaultTol, 0);
    GMX_EXPECT_SIMD_REAL_NEAR(atan2(1.0, -1.0), gmx_simd_atan2_r(rSimd_1_2_3, rSimd_m1_m2_m3), defaultTol, 0);
    // special values should be same as for iso standard (see atan2 man page)
    GMX_EXPECT_SIMD_REAL_NEAR(atan2(0.0, -0.0), gmx_simd_atan2_r(zero, mzero), defaultTol, 0);
    GMX_EXPECT_SIMD_REAL_NEAR(atan2(-0.0, -0.0), gmx_simd_atan2_r(mzero, mzero), defaultTol, 0);
    GMX_EXPECT_SIMD_REAL_NEAR(atan2(0.0, 0.0), gmx_simd_atan2_r(zero, zero), defaultTol, 0);
    GMX_EXPECT_SIMD_REAL_NEAR(atan2(-0.0, 0.0), gmx_simd_atan2_r(mzero, zero), defaultTol, 0);
    GMX_EXPECT_SIMD_REAL_NEAR(atan2(0.0, -1.0), gmx_simd_atan2_r(zero, rSimd_m1_m2_m3), defaultTol, 0);
    GMX_EXPECT_SIMD_REAL_NEAR(atan2(-0.0, -1.0), gmx_simd_atan2_r(mzero, rSimd_m1_m2_m3), defaultTol, 0);
    GMX_EXPECT_SIMD_REAL_NEAR(atan2(0.0, -1.0), gmx_simd_atan2_r(zero, rSimd_m1_m2_m3), defaultTol, 0);
    GMX_EXPECT_SIMD_REAL_NEAR(atan2(-0.0, 1.0), gmx_simd_atan2_r(mzero, rSimd_1_2_3), defaultTol, 0);
    GMX_EXPECT_SIMD_REAL_NEAR(atan2(1.0, 0.0), gmx_simd_atan2_r(rSimd_1_2_3, zero), defaultTol, 0);
    GMX_EXPECT_SIMD_REAL_NEAR(atan2(1.0, -0.0), gmx_simd_atan2_r(rSimd_1_2_3, mzero), defaultTol, 0);
    GMX_EXPECT_SIMD_REAL_NEAR(atan2(-1.0, 0.0), gmx_simd_atan2_r(rSimd_m1_m2_m3, zero), defaultTol, 0);
    GMX_EXPECT_SIMD_REAL_NEAR(atan2(-1.0, -0.0), gmx_simd_atan2_r(rSimd_m1_m2_m3, mzero), defaultTol, 0);
}
...
TEST(SimdTestMath, gmxSimdPmecorrForceR)
{
    ...
    GMX_EXPECT_SIMD_FUNC_NEAR(ref_pmecorrF, gmx_simd_pmecorrF_r, range, ulpTol, absTol);
}
...
TEST(SimdTestMath, gmxSimdPmecorrPotentialR)
{
    ...
    GMX_EXPECT_SIMD_FUNC_NEAR(ref_pmecorrV, gmx_simd_pmecorrV_r, range, ulpTol, absTol);
}
