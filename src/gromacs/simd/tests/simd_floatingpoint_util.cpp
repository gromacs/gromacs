...
#if (!defined(GMX_DOUBLE) && defined(GMX_SIMD_HAVE_LOADU_2_TRANSPOSE_FLOAT)) || \
    (defined(GMX_DOUBLE) && defined(GMX_SIMD_HAVE_LOADU_2_TRANSPOSE_DOUBLE))
TEST_F(SimdFloatingpointUtilTest, gmxSimdLoadU2Transpose)
{
    int                               i;
    int                               offset[GMX_SIMD_REAL_WIDTH];
    real                              data[GMX_SIMD_REAL_WIDTH+1];
    gmx_simd_real_t                   v0, v1;
    gmx::test::FloatingPointTolerance tolerance(gmx::test::defaultRealTolerance());

    EXPECT_TRUE(gmx_simd_pairs_storage_alignment_r <= 32 * GMX_SIMD_REAL_WIDTH);

    // To make sure we check unaligned memory access, we let the storage
    // locations overlap. If the width is 4, loading and transposing should give
    // v0 = [ 0 1 2 3 ] and v1 = [ 1 2 3 4 ]
    for (i = 0; i < GMX_SIMD_REAL_WIDTH; i++)
    {
        data[i]   = i;
        offset[i] = i;
    }
    data[GMX_SIMD_REAL_WIDTH] = GMX_SIMD_REAL_WIDTH;

    gmx_simd_loadu_2_transpose_r(data, offset, &v0, &v1);
    ...
    {
        EXPECT_REAL_EQ_TOL(i, m0_[i], tolerance);
        EXPECT_REAL_EQ_TOL(i + 1, m1_[i], tolerance);
    }
}
#endif
