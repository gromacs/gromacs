...
const gmx_simd_real_t rSimd_1_2_3    = setSimdFrom3R(1, 2, 3);
const gmx_simd_real_t rSimd_4_5_6    = setSimdFrom3R(4, 5, 6);
const gmx_simd_real_t rSimd_7_8_9    = setSimdFrom3R(7, 8, 9);
const gmx_simd_real_t rSimd_5_7_9    = setSimdFrom3R(5, 7, 9);
const gmx_simd_real_t rSimd_m1_m2_m3 = setSimdFrom3R(-1, -2, -3);
const gmx_simd_real_t rSimd_3_1_4    = setSimdFrom3R(3, 1, 4);
const gmx_simd_real_t rSimd_m3_m1_m4 = setSimdFrom3R(-3, -1, -4);
...
const gmx_simd_real_t rSimd_Exp      = setSimdFrom3R( 1.4055235171027452623914516e+18,
...
const gmx_simd_int32_t iSimd_1_2_3      = setSimdFrom3I(1, 2, 3);
const gmx_simd_int32_t iSimd_4_5_6      = setSimdFrom3I(4, 5, 6);
const gmx_simd_int32_t iSimd_7_8_9      = setSimdFrom3I(7, 8, 9);
const gmx_simd_int32_t iSimd_5_7_9      = setSimdFrom3I(5, 7, 9);
const gmx_simd_int32_t iSimd_1M_2M_3M   = setSimdFrom3I(1000000, 2000000, 3000000);
const gmx_simd_int32_t iSimd_4M_5M_6M   = setSimdFrom3I(4000000, 5000000, 6000000);
const gmx_simd_int32_t iSimd_5M_7M_9M   = setSimdFrom3I(5000000, 7000000, 9000000);
...
gmx_simd_real_t
setSimdFrom3R(real r0, real r1, real r2)
{
    std::vector<real> v(3);
    v[0] = r0;
    v[1] = r1;
...
printSimdReal(const gmx_simd_real_t simd)
{
    ...
    std::vector<real>   v = simdReal2Vector(simd);
    buf << "[ ";
...
{
    std::vector<real>             vref = simdReal2Vector(ref);
    std::vector<real>             vtst = simdReal2Vector(tst);
...
{
    return compareSimdRealUlp(refExpr, tstExpr, setSimdFrom1R(ref), tst);
}
                            ...
                            const gmx_simd_real_t ref, const gmx_simd_real_t tst)
{
    ...
    ulpTol_ = 0L;
    absTol_ = 0;
    return compareSimdRealUlp(refExpr, tstExpr, ref, tst);
...
SimdTest::compareSimdRealEq(const char * refExpr, const char * tstExpr,
                            real ref,             const gmx_simd_real_t tst)
{
    return compareSimdRealEq(refExpr, tstExpr, setSimdFrom1R(ref), tst);
}
...
gmx_simd_int32_t
setSimdFrom3I(int i0, int i1, int i2)
{
    std::vector<int> v(3);
    v[0] = i0;
    v[1] = i1;
...
{
    std::stringstream   buf;
    std::vector<int>    v = simdInt2Vector(simd);
    buf << "[ ";
...
{
    std::vector<int>    vref = simdInt2Vector(ref);
    std::vector<int>    vtst = simdInt2Vector(tst);
    bool                eq   = true;
    int                 i;
