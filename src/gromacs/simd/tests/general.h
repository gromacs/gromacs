    ...
    public SimdFunctionTestBase
{
    public:
        ...
        /*! \brief Prepare the input and output vectors, and the
          values that will control the generation of the output. */
        /*  Ideally, this would take place in the constructor or
            SetUp(), but neither of those can take any arguments. One
            could also template the whole test fixture class to avoid
            needing arguments to a constructor or function call at
            all, but you'd still need a separate typedef for most of
            the TEST_F fixtures to make things work. */
        void prepare(size_t numInputs,
                     size_t numOutputs,
                     InputType shift = 0,
                     real scale = 1.0)
        {
        ...
        /*! Generate random reals in scale * (shift + [0,1)). Used for
            testing most SIMD functions. */
        ...
        /*! Get a random SIMD-boolean in a real variable. Different
            ...
            different ways. Used for testing SIMD functions that
            handle logical control flow. */
        ...
        /*! \brief Do repetitive testing of the SIMD and reference
         *  versions of the function.
         *
         * \tparam ReferenceFunctionType The type signature of the
         *                               reference SIMD function
         * \tparam TestFunctionType The type signature of the
         *                          reference SIMD function
         * \tparam InputKind The kind of data present in the input
         *                   vectors (see booleanReal), which is
         *                   distinct from the compiler-type of data.
         *
         * Only the compiler needs to worry about the actual type that
         * is ReferenceFunctionType and TestFunctionType. Test cases
         * need to define a callFunction() that is overloaded to do
         * the right thing when called from RunTest with either
         * reference or test inputs. Then test case definitions can
         * just pass in the reference and test versions of the SIMD
         * function and forget about it.
         *
         * In practice, callFunction is implemented with templates,
         * because the reference and test function paths are normally
         * the same, so long as you can abstractly call the
         * appropriate implementation of the support functions (load,
         * store, etc.) and the function under test. This is supported
         * by the classes derived from SimdFunctionSetBase.
         *
