...
TEST_F(SimdFloatingpointTest, maskedFma)
{
    SimdBool m = isNonZero(setSimdRealFrom3R(1, 0, 1));
    GMX_EXPECT_SIMD_REAL_EQ(setSimdRealFrom3R(11, 0, 27),
                            maskedFma(rSimd_1_2_3, rSimd_4_5_6, rSimd_7_8_9, m)); // 1*4+7, etc.
}
...
TEST_F(SimdFloatingpointTest, maskedRsqrt)
{
     ...
     */
    setUlpTol(1LL << shiftbits);
    GMX_EXPECT_SIMD_REAL_NEAR(ref, maskedRsqrt(x, m));
}
...
TEST_F(SimdFloatingpointTest, maskedRcp)
{
     ...
     */
    setUlpTol(1LL << shiftbits);
    GMX_EXPECT_SIMD_REAL_NEAR(ref, maskedRcp(x, m));
}
...
TEST_F(SimdFloatingpointTest, isNonZeroAndSelectByMask)
{
