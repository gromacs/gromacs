...
#if defined(GMX_SIMD_HAVE_FLOAT) && defined(GMX_SIMD_HAVE_DOUBLE)
TEST_F(SimdFloatingpointTest, gmxSimdCvtFloat2Double)
{
    float                             f[2*GMX_SIMD_FLOAT_WIDTH];
    double                            d[3*GMX_SIMD_DOUBLE_WIDTH];
    float                            *pf;
    double                           *pd0;
    double                           *pd1;
    int                               i;
    gmx_simd_float_t                  vf;
    gmx_simd_double_t                 vd0;

    gmx::test::FloatingPointTolerance tolerance(gmx::test::defaultRealTolerance());

    // Create pointers to aligned float/double memory
    pf  = gmx_simd_align_f(f);
    pd0 = gmx_simd_align_d(d);
    pd1 = pd0 + GMX_SIMD_DOUBLE_WIDTH;
    ...
    vf = gmx_simd_load_f(pf);
#if (GMX_SIMD_FLOAT_WIDTH == 2*GMX_SIMD_DOUBLE_WIDTH)
    gmx_simd_double_t vd1;
    gmx_simd_cvt_f2dd(vf, &vd0, &vd1);
    gmx_simd_store_d(pd1, vd1);
#else
    vd0 = gmx_simd_cvt_f2d(vf);
#endif
    gmx_simd_store_d(pd0, vd0);

    for (i = 0; i < GMX_SIMD_FLOAT_WIDTH; i++)
    {
        EXPECT_REAL_EQ_TOL(pf[i], pd0[i], tolerance);
    }
}
...
{
    float                             f[2*GMX_SIMD_FLOAT_WIDTH];
    double                            d[2*GMX_SIMD_FLOAT_WIDTH]; // Yes, double length should be same as float
    float                            *pf;
    double                           *pd0;
    double                           *pd1;
    int                               i;
    ...
    vd0 = gmx_simd_load_d(pd0);
#if (GMX_SIMD_FLOAT_WIDTH == 2*GMX_SIMD_DOUBLE_WIDTH)
    gmx_simd_double_t vd1 = gmx_simd_load_d(pd1);
    vf = gmx_simd_cvt_dd2f(vd0, vd1);
#else
    vf = gmx_simd_cvt_d2f(vd0);
#endif
    gmx_simd_store_f(pf, vf);

    // This will check elements in pd1 too when double width is 2*single width
    for (i = 0; i < GMX_SIMD_FLOAT_WIDTH; i++)
    {
        EXPECT_REAL_EQ_TOL(pd0[i], pf[i], tolerance);
    }
}

#ifdef GMX_SIMD_EXTENDED
TEST_F(SimdFloatingpointTest, gmxSimdCvtfb2db)
{
    float            f[2*GMX_SIMD_FLOAT_WIDTH];
    double           d[2*GMX_SIMD_FLOAT_WIDTH];
    float           *pf;
    double          *pd0;
    double          *pd1;
    float            ref;
    int              i;
    gmx_simd_fbool_t vfb;
    gmx_simd_dbool_t vdb0;
    ...
    for (i = 0; i < GMX_SIMD_FLOAT_WIDTH; i++)
    {
        pf[i] = 1.0 + i % 3;                                            // 1,2,3,1,2,3,...
    }
    vfb = gmx_simd_cmplt_f(gmx_simd_load_f(pf), gmx_simd_set1_f(1.5f)); // true,false,false,true,false,false,...
...
#if (GMX_SIMD_FLOAT_WIDTH == 2*GMX_SIMD_DOUBLE_WIDTH)
    gmx_simd_dbool_t vdb1;
    gmx_simd_cvt_fb2dbdb(vfb, &vdb0, &vdb1);
    gmx_simd_store_d(pd1, gmx_simd_blendzero_d(gmx_simd_load_d(pd1), vdb1));
#else
    vdb0 = gmx_simd_cvt_fb2db(vfb);
#endif
    // Keep elements for i%3==0, set the others to zero (also for pd1 above if double width is 2*single width)
    gmx_simd_store_d(pd0, gmx_simd_blendzero_d(gmx_simd_load_d(pd0), vdb0));
...
TEST_F(SimdFloatingpointTest, gmxSimdCvtdb2fb)
{
    float            f[2*GMX_SIMD_FLOAT_WIDTH];
    double           d[2*GMX_SIMD_FLOAT_WIDTH];
    float           *pf;
    ...
    for (i = 0; i < GMX_SIMD_FLOAT_WIDTH; i++)
    {
        pd0[i] = 1.0 + i % 3;  // 1,2,3,1,2,3,...
    }
    vdb0 = gmx_simd_cmplt_d(gmx_simd_load_d(pd0), gmx_simd_set1_d(1.5f));
    vdb1 = gmx_simd_cmplt_d(gmx_simd_load_d(pd1), gmx_simd_set1_d(1.5f));
...
#if (GMX_SIMD_FLOAT_WIDTH == 2*GMX_SIMD_DOUBLE_WIDTH)
    vfb = gmx_simd_cvt_dbdb2fb(vdb0, vdb1);
#else
    vfb = gmx_simd_cvt_db2fb(vdb0);
#endif

    // Keep elements for i%3==0, set the others to zero
    gmx_simd_store_f(pf, gmx_simd_blendzero_f(gmx_simd_load_f(pf), vfb));
