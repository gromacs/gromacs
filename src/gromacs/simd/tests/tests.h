 ...
 * P = a pointer to a SIMD "vector" type
 ...
 * will work.
 *
 * gmx_set1_epi32, gmx_load_si, gmx_cvttpr_epi32, gmx_cvtepi32_pr,
 * gmx_castsi_pr are all involved in implementing exclusions. Those
 * implementations are highly architecture-specific, and I think they
 * will get bundled into a wrapper function that will probably live in
 * the nbnxn kernel module, and be tested at wrapper level there.
 */
...
// In practice, at most 4 reals are used right now. This produces
// 1024/8 = 128 independent sets of test variables. That seems like
// enough to be confident we'll hit edge cases decently often.
    ...
    public:
        ...
        SimdFunctionTest()
        {
            ...
            for (int i = 0; i < SIMD_TEST_NUM_REAL_VARIABLES; ++i)
            {
                ...
                {
                    unsignedInts[i][j] = gmx_rng_uniform_uint32(rng);
                    /* Generate random floating-point numbers. It is
                     * not clear what range makes any particular
                     * sense, so chose -0.5 <= x < 0.5 */
                    reals[i][j] = gmx_rng_uniform_real(rng) - 0.5;
                    /* Likewise, 0 <= x < 1 */
                    positiveReals[i][j] = gmx_rng_uniform_real(rng);
                    /* Likewise, 0 <= x < 10000 */
                    positiveReals[i][j] = 10000 * gmx_rng_uniform_real(rng);
                    /* Vector containing the SIMD version of truth of
                       whether one real was less than
                       another. Different SIMD architectures define
                       "true" differently, e.g. on x86 true is
                       0xffffffff, whereas on QPX true is greater than
                       or equal to either kind of zero. When QPX is
                       supported, or GMX_X86 exists, the following
                       lines will need to be versioned. */
                    real           comparison_temporary = (gmx_rng_uniform_real(rng) - 0.5);
                    BitManipulater manipulater;
                    manipulater.i      = (reals[i][j] < comparison_temporary) ? (UnsignedIntWithSizeOfReal) ~0 : 0;
        ...
        void call(ReferenceFunctionType referenceFunction, RealArray referenceReals);
        void call(TestFunctionType testFunction, RealArray testReals);
        void call(ReferenceFunctionType referenceFunction, const UIntArray referenceUInts);
        void call(TestFunctionType testFunction, const UIntArray testUInts);

        /*! \brief (regarding scaleMaxUlps) Some SIMD-based computation (like
        ...
        void Tester(ReferenceFunctionType referenceFunction, TestFunctionType testFunction, RealArray reals, real scaleMaxUlps = 1.0, int numOutputs = 1)
        {
        ...
        void Tester(ReferenceFunctionType referenceFunction, TestFunctionType testFunction, UIntArray unsignedInts, real scaleMaxUlps = 1.0, int numOutputs = 1)
        {
            for (int i = 0; i < SIMD_TEST_NUM_REAL_VARIABLES; i += SIMD_TEST_MAX_NUM_REAL_VARIABLES_PER_TEST)
            {
                call(referenceFunction, unsignedInts + i);
                call(testFunction, unsignedInts + i);
            }
        ...
        // TODO: recast this with vector<rvec>?
        UIntArray unsignedInts;
        RealArray reals;
