...
class ArrayRefTest : public test::SimdTest
{
    public:
                              ...
                              ArrayRefType &arrayRef)
        {
            ...
            GMX_EXPECT_SIMD_REAL_EQ(load<SimdReal>(a), arrayRef.front());
            GMX_EXPECT_SIMD_REAL_EQ(load<SimdReal>(a+(aSize-1)*width), arrayRef.back());

            auto it = arrayRef.begin();
            for (size_t i = 0; i != aSize; ++i, ++it)
            {
                GMX_EXPECT_SIMD_REAL_EQ(load<SimdReal>(a+i*width), arrayRef[i]);
                GMX_EXPECT_SIMD_REAL_EQ(load<SimdReal>(a+i*width), *it);
            }
        ...
        void runReadWriteTests(PointerType   a,
                               size_t        aSize,
                               ArrayRefType &arrayRef)
        {
            ASSERT_EQ(aSize, arrayRef.size());
...
{
    AlignedArray<typename TestFixture::ElementType, TestFixture::width*3> a;
...
{
    AlignedArray<typename TestFixture::ElementType, TestFixture::width*4> a;
    typename TestFixture::ArrayRefType arrayRef(a.data(), a.data()+a.size());
    this->runReadWriteTests(a.data(), 4, arrayRef);
}
