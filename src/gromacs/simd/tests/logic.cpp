...
{
/*! Helper function to do bit-wise AND on SIMD reals, to mimic
   x86. The official gmx_simd_ref_and_pb does a logical AND, so we
   can't use that for testing the actual SIMD implementations. */
...
/*! Helper function to do bit-wise OR on SIMD reals, to mimic x86. The
  official gmx_simd_ref_and_pb does a logical OR, so we can't use that
