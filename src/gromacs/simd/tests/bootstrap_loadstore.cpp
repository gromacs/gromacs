 ...
 * value must be less than or equal to the SIMD4 width.
 */
template <typename T, typename TSimd> void
 ...
 *
 * This means that Load3 and Store3 are tested to work on
 * unaligned memory */
...
#if defined GMX_SIMD4_HAVE_LOAD3 && defined GMX_SIMD4_HAVE_STORE3

//! Wrapper for SIMD macro to load three adjacent reals
gmx_simd4_real_t wrapperSimd4Load3R(real *m)
{
    return gmx_simd4_load3_r(m);
}
//! Wrapper for SIMD macro to store three adjacent reals
void wrapperSimd4Store3R(real *m, gmx_simd4_real_t s)
{
    gmx_simd4_store3_r(m, s);
}
...
TEST(SimdBootstrapTest, DISABLED_gmxSimdLoad3Store3R)
{
    simd4LoadStoreTester(wrapperSimd4Load3R, wrapperSimd4Store3R, gmx_simd_unalign_r, 0, 0, 3);
    /* This tests that gmx_simd4_store3_r writes only to the intended
     * memory, so there is no need for an explicit test that the store
     * is within bounds. */
}

TEST(SimdBootstrapTest, gmxSimdLoad3DoesNotTouchMemoryBeforeStart)
{
    real data[8] = { 1, 2, 3, 4, 5, 6, 7, 8};

    gmx_simd4_load3_r(data + 0);
    SUCCEED();
}

TEST(SimdBootstrapTest, gmxSimdLoad3DoesNotTouchMemoryPastEnd)
{
    real data[8] = { 1, 2, 3, 4, 5, 6, 7, 8};

    gmx_simd4_load3_r(data + 5);
    SUCCEED();
}

#endif /* defined GMX_SIMD4_HAVE_LOAD3 && defined GMX_SIMD4_HAVE_STORE3 */
