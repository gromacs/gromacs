...
namespace
{
// Tests for low-level implementations of SIMD alignment, load and store.
//
...
//
// gmx_simd_load_r(),gmx_simd_store_r()
// gmx_simd_loadu_r(),gmx_simd_storeu_r()
//
// gmx_simd_load_i(),gmx_simd_store_i()
// gmx_simd_loadu_i(),gmx_simd_storeu_i()
//
// gmx_simd4_load_r(),gmx_simd4_store_r()
// gmx_simd4_loadu_r(),gmx_simd4_storeu_r()


TEST(SimdTestBootstrap, gmxSimdAlign)
{
#ifdef GMX_SIMD_HAVE_REAL
    real rdata[GMX_SIMD_REAL_WIDTH*2];
    for (int i = 0; i < GMX_SIMD_REAL_WIDTH; i++)
    {
        ASSERT_EQ(((size_t)gmx_simd_align_r(&rdata[i]) & (GMX_SIMD_REAL_WIDTH*sizeof(real)-1)), (size_t)0);
    }
#endif
    ...
    {
        ASSERT_EQ(((size_t)gmx_simd_align_i(&idata[i]) & (GMX_SIMD_INT32_WIDTH*sizeof(int)-1)), (size_t)0);
    }
#endif
}
                    ...
                    const int loadOffset, const int storeOffset, const int simdWidth)
{
    ...
    {
        ASSERT_EQ(pCopySrc[i], pCopyDst[i]) << "SIMD load or store not moving data correctly for element " << i;
    }

    for (i = 0; i < simdWidth*5; i++)
    {
        ASSERT_EQ(src[i], (T)(1+i)) << "Side effect on source memory, i = " << i;
        if (&dst[0]+i < pCopyDst || &dst[0]+i >= pCopyDst+simdWidth)
        {
            ASSERT_EQ(dst[i], (T)(-1-i)) << "Side effect on destination memory, i = " << i;
        }
    }
}

#ifdef GMX_SIMD_HAVE_REAL
//! \brief Wrapper for SIMD macro to load aligned floating-point data.
gmx_simd_real_t wrapperSimdLoadR(real *m)
{
    return gmx_simd_load_r(m);
}
//! \brief Wrapper for SIMD macro to store to aligned floating-point data.
void            wrapperSimdStoreR(real *m, gmx_simd_real_t s)
{
    gmx_simd_store_r(m, s);
}

TEST(SimdTestBootstrap, gmxSimdLoadStoreR)
{
    simdLoadStoreTester(wrapperSimdLoadR, wrapperSimdStoreR, gmx_simd_align_r, 0, 0, GMX_SIMD_REAL_WIDTH);
}

#    ifdef GMX_SIMD_HAVE_LOADU
//! \brief Wrapper for SIMD macro to load unaligned floating-point data.
gmx_simd_real_t WrapperSimdLoadUR(real *m)
{
    return gmx_simd_loadu_r(m);
}

TEST(SimdTestBootstrap, gmxSimdLoadUR)
{
    for (int i = 0; i < GMX_SIMD_REAL_WIDTH; i++)
    {
...
//! \brief Wrapper for SIMD macro to store to unaligned floating-point data.
void WrapperSimdStoreUR(real *m, gmx_simd_real_t s)
{
    gmx_simd_storeu_r(m, s);
}

TEST(SimdTestBootstrap, gmxSimdStoreUR)
{
    for (int i = 0; i < GMX_SIMD_REAL_WIDTH; i++)
    {
...
#    ifdef GMX_SIMD_HAVE_STOREU
//! \brief Wrapper for SIMD macro to store to unaligned integer data.
void wrapperSimdStoreUI(int *m, gmx_simd_int32_t s)
{
    gmx_simd_storeu_i(m, s);
}

TEST(SimdTestBootstrap, gmxSimdStoreUI)
{
    for (int i = 0; i < GMX_SIMD_INT32_WIDTH; i++)
    {
                     ...
                     const int loadOffset, const int storeOffset)
{
    ...
    {
        ASSERT_EQ(pCopySrc[i], pCopyDst[i]) << "SIMD4 load or store not moving data correctly for element " << i;
    }
...
#    ifdef GMX_SIMD_HAVE_STOREU
//! \brief Wrapper for SIMD4 macro to store to unaligned floating-point data.
void WrapperSimd4StoreUR(real *m, gmx_simd4_real_t s)
{
    gmx_simd4_storeu_r(m, s);
}

TEST(SimdTestBootstrap, gmxSimd4StoreUR)
{
    for (int i = 0; i < GMX_SIMD4_WIDTH; i++)
    {
