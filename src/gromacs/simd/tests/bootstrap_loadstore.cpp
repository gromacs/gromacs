                    ...
                    const int loadOffset, const int storeOffset, const int simdWidth)
{
    ...
    T *       pCopySrc = simdAlignFn(src) + simdWidth + loadOffset;
    T *       pCopyDst = simdAlignFn(dst) + simdWidth + storeOffset;
    int       i;
...
#ifdef GMX_SIMD_HAVE_REAL
// Tests for gmx_simd_real_t load & store operations. Define wrapper functions
...
{
    simdLoadStoreTester(wrapperSimd4LoadR, wrapperSimd4StoreR, gmx_simd4_align_r, 0, 0, GMX_SIMD4_WIDTH);
}
...
TEST(SimdTestBootstrap, gmxSimd4LoadUR)
{
    ...
    {
        simdLoadStoreTester(WrapperSimd4LoadUR, wrapperSimd4StoreR, gmx_simd4_align_r, i, 0, GMX_SIMD4_WIDTH);
    }
}
#    endif
...
TEST(SimdTestBootstrap, gmxSimd4StoreUR)
{
    for (int i = 0; i < GMX_SIMD4_WIDTH; i++)
    {
        simdLoadStoreTester(wrapperSimd4LoadR, WrapperSimd4StoreUR, gmx_simd4_align_r, 0, i, GMX_SIMD4_WIDTH);
    }
}
#    endif
#endif
