...
AnalysisDataStorage::collectFrames()
{
    ...
    {
// NOTE: The line below shouldn't be wrong, but this could potentially be dangerous
    ...
    for (size_t i=0; i<impl_->frames_.size(); i++)
    {
        ...
        if (impl_->frames_[i].frame->header_.x() != 0)
        {
            recv_size++;
...
namespace mpi
{
    ...
    //! declaring Status is a static data type
