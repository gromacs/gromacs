...
{
template <>
inline data_layout mpi_type_traits<gmx::AnalysisDataStorageFrame>::get_layout(
        const gmx::AnalysisDataStorageFrame& adsf)
{
    return struct_layout_builder(adsf,2).add(adsf.header_).add(adsf.values_).
            build();
}
}//mpi
