...
AnalysisDataModuleManager::Impl::Impl()
    : bAllowMissing_(true), frameCount_(0),
      state_(eNotStarted), currIndex_(-1)
{
...
int
AnalysisDataModuleManager::finishedFrameCount() const
{
    return impl_->frameCount_;
}
...
AnalysisDataModuleManager::notifyFrameStart(const AnalysisDataFrameHeader &header) const
{
    ...
    GMX_ASSERT(header.index() == impl_->frameCount_, "Out of order frames");
...
void
AnalysisDataModuleManager::notifyFrameFinish(const AnalysisDataFrameHeader &header)
{
    GMX_ASSERT(impl_->state_ == Impl::eInFrame, "notifyFrameStart() not called");
