...
void
AbstractPlotModule::setLegend(ArrayRef<const char *> setname)
{
    const int nsets = setname.size();
...
AbstractPlotModule::frameStarted(const AnalysisDataFrameHeader &frame)
{
    ...
    if (!impl_->bOmitX_)
    {
        std::fprintf(impl_->fp_, impl_->xformat_.c_str(), frame.x() * impl_->xscale_);
    }
}
...
void
AnalysisDataVectorPlotModule::setWriteMask(ArrayRef<const bool> bWrite)
{
