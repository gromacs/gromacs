    ...
    public:
                        ...
                        const bool *present) const
        {
                 ...
                 ++past)
            {
                real  pastx, pastdx;
                const real *pasty;
                ASSERT_TRUE(source_->getDataWErr(frameIndex_ - past,
                                                 &pastx, &pastdx, &pasty, NULL))
                    << "Current / requested: "
                    << frameIndex_ << " / " << frameIndex_ - past;
                checkFrame(pastx, pastdx, 0, data_->columnCount(), pasty,
                           data_->frame(frameIndex_ - past));
                if (past > 0)
                {
                                           ...
                                           AbstractAnalysisData *source)
{
    ...
    if (firstcol < 0 || n <= 0 || firstcol + n > data.columnCount())
    {
        GMX_THROW(gmx::test::TestException("Out-of-range columns"));
    }
