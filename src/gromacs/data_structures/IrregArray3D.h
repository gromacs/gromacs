...
/*! \file
    ...
    \date Feb 2015
 */
#ifndef GMX_IrregArray3D_H
#define GMX_IrregArray3D_H

#include "IrregArray2D.h"

namespace gmx
{


/*! \class IrregArray3D IrregArray3D.h "gromacs/data_structures/IrregArray3D.h"
...
class IrregArray3D
{
    ...
    public:
         ...
         */
        IrregArray3D() : begin1(0), begin2(0), begin3(0), end1(0), end2(0), end3(0), is_irreg(false), arr(NULL)
        {
        }
        //! copy constructor
        IrregArray3D(const IrregArray3D<value_type> &ini_t) : begin1(0), begin2(0), begin3(0), arr(NULL), is_irreg(false)
        {
            initArray(ini_t);
        }
        ...
        IrregArray3D(const index_type x1, const index_type x2, const index_type y1, const index_type y2, const index_type z1, const index_type z2) :
            begin1(0), begin2(0), begin3(0), end1(0), end2(0), end3(0), is_irreg(false), arr(NULL)
        ...
        void initArray(const IrregArray3D<T_foreign, A_foreign> &ini)
        {
            if (ini.is_irreg)
            {
                initArray(ini.begin1, ini.end1, ini.irreg, ini.irreg_mat);
            }
            else
            {
                initArray(ini.begin1, ini.end1, ini.begin2, ini.end2, ini.begin3, ini.end3);
            }
            for (index_type i = begin1; i <= end1; i++)
            {
                for (index_type j = begin2; j <= getEnd2(i); j++)
                {
                    ...
                    {
                        arr[i][j][k] = static_cast<value_type>(ini(i, j, k));
                    }
                }
            }
        }
        ...
        void initArray(const index_type x1, const index_type x2, const IrregArray1D<size_type> &sizes1, const IrregArray2D<size_type> &sizes2)
        {
            ...
            irreg_mat.initArray(getBegin1(), getEnd1(), irreg, 0);
            ...
            allocateArray(getBegin1(), getBegin2(), getBegin3(), dim1, irreg, irreg_mat);
        }
        /*! \brief   constructor helper function, assign the book keeping data and allocate memory for
        ...
        void initArray(const index_type x1, const index_type x2, const index_type y1, const index_type y2, const IrregArray1D<size_type> &sizes)
        {
            ...
            end3   = 0;
            irreg.initArray(x1, x2);
            irreg_mat.initArray(x1, x2, y1, y2, 0);
            for (index_type i = x1; i <= x2; i++)
            {
                irreg[i] = dim2;
                for (index_type j = y1; j <= y2; j++)
                {
                    irreg_mat(i, j) = sizes(j);
                }
            }
            allocateArray(begin1, begin2, begin3, dim1, irreg, irreg_mat);
        }
        /*! \brief   assign the book keeping data and allocate memory for the
                     book keeping and data arrays
        ...
        void initArray(index_type x1, index_type x2, const IrregArray1D<size_type> &sizes, index_type z1, index_type z2)
        {
            ...
            end3   = z2;
            irreg.initArray(x1, x2);
            irreg_mat.initArray(x1, x2, sizes, dim3);
            for (index_type i = x1; i <= x2; i++)
            {
                irreg[i] = sizes[i];
                for (index_type j = begin2; j < begin2 + sizes[i]; j++)
                {
                    irreg_mat(i, j) = dim3;
                }
            }
        ...
        void allocateArray(const index_type x1, const index_type x2, const index_type y1, const index_type y2, const index_type z1, const index_type z2)
        {
            ...
            try
            {
                arr          = p2_allocator.allocate(dim1+1);
                arr         -= begin1;
                arr[x1]      = p_allocator.allocate(dim1*dim2);
                arr[x1]     -= begin2;
                arr[x1][y1]  = allocator.allocate(dim1*dim2*dim3);
                arr[x1][y1] -= begin3;
                // since we dont run over all elements in the first dimension ...
                for (index_type i = y1 + 1; i <= y2; i++)
                {
                    arr[x1][i] = arr[x1][i-1] + dim3;
                }
                // set the rest
                for (index_type i = x1 + 1; i <= x2; i++)
                {
                    arr[i]     = arr[i-1] + dim2;
                    arr[i][y1] = arr[i-1][y1] + dim2*dim3;
                    for (index_type j = y1 + 1; j <= y2; j++)
                    {
                        arr[i][j] = arr[i][j-1] + dim3;
                    }
                }
            }
            catch (std::bad_alloc)
            ...
            for (index_type i = getBegin1(); i <= getEnd1(); ++i)
            {
                for (index_type j = getBegin2(); j <= getEnd2(); ++j)
                {
                    ...
                    {
                        allocator.construct( &this->operator()(i, j, k), value_type());
                    }
                }
            }
        ...
        void allocateArray(const index_type s1, const index_type s2, const index_type s3, const index_type d1, const IrregArray1D<size_type> &irr, const IrregArray2D<size_type> &irr_mat)
        {
            ...
            try
            {
                arr = p2_allocator.allocate(d1);
                arr = arr - s1;
                for (index_type i = s1; i <= e1; ++i)
                {
                    arr[i]  = p_allocator.allocate(irr[i]);
                    arr[i] -= s2;
                }
                for (index_type i = s1; i <= e1; i++)
                {
                    ...
                    {
                        arr[i][j]  = allocator.allocate(irr_mat(i, j));
                        arr[i][j] -= s3;
                    }
                }
            }
            ...
            if (arr != NULL)
            {
                ...
                for (index_type i = getBegin1(); i <= getEnd1(); ++i)
                {
                    for (index_type j = getBegin2(); j <= getEnd2(i); ++j)
                    {
                        for (index_type k = getBegin3(); k <= getEnd3(i, j); ++k)
                        {
                            allocator.destroy( &this->operator()(i, j, k));
                        }
                    }
                }
                ...
                if (!is_irreg)
                {
                    ...
                    allocator.deallocate(arr[begin1][begin2], dim1*dim2*dim3+1);
                    arr[begin1] += begin2;
                    p_allocator.deallocate(arr[begin1], dim1*dim2+1);
                    arr += begin1;
                    p2_allocator.deallocate(arr, dim1+1);
                }
                else
                {
                    for (index_type i = getBegin1(); i <= getEnd1(); i++)
                    {
                        ...
                        {
                            arr[i][j] += begin3;
                            allocator.deallocate(arr[i][j], getLength3(i, j));
                        }
                        arr[i] += begin2;
                        p_allocator.deallocate(arr[i], getLength2(i));
                    }
                    arr += begin1;
                    p2_allocator.deallocate(arr, getLength1());
                }
                arr = NULL;
            }
