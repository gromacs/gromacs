                 ...
                 const OutputQuantities     &output)
{
    ...
    int numNonZeroFshiftEntries = 0;
    for (const auto &fshiftEntry : output.fshift)
    {
        for (const auto component : fshiftEntry)
        {
            if (gmx_numzero(component))
            {
                ++numNonZeroFshiftEntries;
            }
        }
    }
    checker->checkInteger(numNonZeroFshiftEntries, "Number Of Non-zero shift force components");
    checker->checkVector(output.fshift[CENTRAL], "ShiftForces");
...
TEST_P (BondedTest, IfuncMorse)
{
    ...
    iparams.morse.betaA = iparams.morse.betaB = 2.0;
    const real lambda   = 0.0;
