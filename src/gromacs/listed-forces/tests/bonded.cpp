...
/*! \brief Utility to fill iparams struct
 *
...
void fillIparams(int ftype, bool fep, t_iparams *ip)
{
    switch (ftype)
    {
        ...
        case F_MORSE:
            if (fep)
            {
                ip->morse = { 0.8, 50.0, 2.0, 0.7, 40.0, 1.6 };
            }
            else
            {
        ...
        case F_LINEAR_ANGLES:
            if (fep)
            {
                ip->linangle = { 50.0, 0.4, 40.0, 0.6 };
            }
            else
            {
                ip->linangle = { 50.0, 0.5, 50.0, 0.5 };
            }
            break;
        case F_CROSS_BOND_BONDS:
            // No FEP support.
            ip->cross_bb = { 0.8,  0.7, 45.0 };
            break;
        case F_CROSS_BOND_ANGLES:
            // No FEP support.
            ip->cross_ba = { 0.8, 0.7, 0.3, 45.0 };
            break;
        case F_UREY_BRADLEY:
            if (fep)
            {
                ip->u_b = { 100.0, 45.0, 0.3, 5.0, 90.0, 47.0, 0.32, 7.0 };
            }
            else
            {
                ip->u_b = { 100.0, 45.0, 0.3, 5.0, 100.0, 45.0, 0.3, 5.0 };
            }
            break;
        case F_QUARTIC_ANGLES:
            // No FEP support
            ip->qangle = { 87.0, 1.1, 2.2, 3.3, 4.5, 6.7 };
            break;
        case F_PDIHS:
            if (fep)
            {
        ...
        case F_CBTDIHS:
            if (fep)
            {
                ip->rbdihs = { -5.35, 13.6, 8.4, -16.7, 0.3, 12.4, -6.35, 12.6, 8.1, -10.7, 0.9, 15.4 };
            }
            else
            {
                ip->rbdihs = { -5.35, 13.6, 8.4, -16.7, 0.3, 12.4, -5.35, 13.6, 8.4, -16.7, 0.3, 12.4 };
            }
            break;
        default:
            std::string msg = "Death horror error. Unknown ftype " + toString(ftype);
            GMX_RELEASE_ASSERT(false, msg.c_str());
    }
}
...
/*! \brief Utility to determine abs tolerance
 *
 * \param[in]  ftype  Function type
 * \param[out] ftol   Float tolerance
 * \param[out] dtol   Double tolerance
 */
void absToleranceFtype(int ftype, float *ftol, double *dtol)
{
    struct mytol {
        float ftol; double dtol;
    };
    std::unordered_map<int, mytol> tol =
    { { F_G96ANGLES, { 8e-3, 1e-6 } },
      { F_RBDIHS,    { 2e-4, 1e-6 } }};
    if (tol.find(ftype) != tol.end())
    {
        *ftol = tol[ftype].ftol;
        *dtol = tol[ftype].dtol;
    }
...
class ListedForcesTest : public ::testing::TestWithParam<std::tuple<int, std::vector<gmx::RVec>, int, bool> >
{
    protected:
        matrix                     box_;
        t_pbc                      pbc_;
            ...
            checker_(refData_.rootChecker())
        {
            ...
            float  ftol;
            double dtol;
            absToleranceFtype(ftype_, &ftol, &dtol);
            test::FloatingPointTolerance tolerance(test::FloatingPointTolerance(ftol, 1.0e-6,
                                                                                dtol, 1.0e-12,
                          ...
                          const real                  lambda)
        {
            ...
            output.energy = bondedFunction(ftype_)(iatoms.size(),
                                                   ...
                                                   &ddgatindex);
            // Internal consistency test of both test input above
            // and bonded functions.
            std::string msg("When not doing FEP dVdlambda should be 0 for ftype ");
            msg.append(interaction_function[ftype_].name);
            GMX_RELEASE_ASSERT(fep_ || output.dvdlambda == 0.0, msg.c_str());
            // Check the outpur values
...
INSTANTIATE_TEST_CASE_P(Bond, ListedForcesTest, ::testing::Combine(::testing::ValuesIn(c_fTypeBonds), ::testing::ValuesIn(c_coordinatesForTests), ::testing::ValuesIn(c_pbcForTests), ::testing::ValuesIn(c_Fep)));

INSTANTIATE_TEST_CASE_P(Angle, ListedForcesTest, ::testing::Combine(::testing::ValuesIn(c_fTypeAngles), ::testing::ValuesIn(c_coordinatesForTests), ::testing::ValuesIn(c_pbcForTests), ::testing::ValuesIn(c_Fep)));

INSTANTIATE_TEST_CASE_P(Dihedral, ListedForcesTest, ::testing::Combine(::testing::ValuesIn(c_fTypeDihs), ::testing::ValuesIn(c_coordinatesForTests), ::testing::ValuesIn(c_pbcForTests), ::testing::ValuesIn(c_Fep)));
}  // namespace
