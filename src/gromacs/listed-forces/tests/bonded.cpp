...
struct iListInput
{
    public:
        ...
        iListInput setQuarticAngles(real theta, real c[5]) //real c0, real c1, real c2, real c3, real c4)
        {
        ...
        iListInput setProperDihedrals(real phiA, real cpA, int nmult, real phiB, real cpB)
        {
            ftype              = F_PDIHS;
            ...
            iparams.pdihs.mult = nmult;
            fep                = (phiA != phiB || cpA != cpB);
            return *this;
        }
        iListInput setProperDihedrals(real phiA, real cpA, int nmult)
        {
            return setProperDihedrals(phiA, cpA, nmult, phiA, cpA);
        }
        iListInput setRbDihedrals(real c0A, real c1A, real c2A, real c3A, real c4A, real c5A,
                                  real c0B, real c1B, real c2B, real c3B, real c4B, real c5B)
        {
            ftype                  = F_RBDIHS;
            iparams.rbdihs.rbcA[0] = c0A;
            iparams.rbdihs.rbcA[1] = c1A;
            iparams.rbdihs.rbcA[2] = c2A;
            iparams.rbdihs.rbcA[3] = c3A;
            iparams.rbdihs.rbcA[4] = c4A;
            iparams.rbdihs.rbcA[5] = c5A;
            iparams.rbdihs.rbcB[0] = c0B;
            iparams.rbdihs.rbcB[1] = c1B;
            iparams.rbdihs.rbcB[2] = c2B;
            iparams.rbdihs.rbcB[3] = c3B;
            iparams.rbdihs.rbcB[4] = c4B;
            iparams.rbdihs.rbcB[5] = c5B;
            fep                    = (c0A != c0B || c1A != c1B || c2A != c2B ||
                                      c3A != c3B || c4A != c4B || c5A != c5B);
            return *this;
        }
        iListInput setRbDihedrals(real c0A, real c1A, real c2A, real c3A, real c4A, real c5A)
        {
            return setRbDihedrals(c0A, c1A, c2A, c3A, c4A, c5A,
                                  c0A, c1A, c2A, c3A, c4A, c5A);
        }
};
...
std::vector<iListInput> c_InputBonds =
{
    ...
    { iListInput().setHarmonic(F_BONDS, 0.15, 500.0, 0.17, 400.0) },
    { iListInput(1e-3f, 1e-6).setHarmonic(F_G96BONDS, 0.15, 50.0) },
    { iListInput().setHarmonic(F_G96BONDS, 0.15, 50.0, 0.17, 40.0) },
    { iListInput().setCubic(0.16, 50.0, 2.0) },
    { iListInput().setMorse(0.15, 50.0, 2.0, 0.17, 40.0, 1.6) },
    { iListInput().setMorse(0.15, 30.0, 2.7) },
    { iListInput().setFene(0.4, 5.0) }
};

static real cQuarticAngles[5] = { 1.1, 2.2, 3.3, 4.5, 6.7 };

//! Function types for testing angles. Add new terms at the end.
std::vector<iListInput> c_InputAngles =
{
    { iListInput().setHarmonic(F_ANGLES, 100.0, 50.0) },
    { iListInput().setHarmonic(F_ANGLES, 100.15, 50.0, 95.0, 30.0) },
    { iListInput(1e-3f, 1e-6).setHarmonic(F_G96ANGLES, 100.0, 50.0) },
    { iListInput().setHarmonic(F_G96ANGLES, 100.0, 50.0, 95.0, 30.0) },
    { iListInput().setLinearAngle(50.0, 0.4) },
    { iListInput().setLinearAngle(50.0, 0.4, 40.0, 0.6) },
    { iListInput().setCrossBondBonds(0.8, 0.7, 45.0) },
    { iListInput().setCrossBondAngles(0.8, 0.7, 0.3, 45.0) },
    { iListInput().setUreyBradley(950.0, 46.0, 0.3, 5.0) },
    { iListInput().setUreyBradley(100.0, 45.0, 0.3, 5.0, 90.0, 47.0, 0.32, 7.0) },
    { iListInput().setQuarticAngles(87.0, cQuarticAngles ) }
};

//! Function types for testing dihedrals. Add new terms at the end.
std::vector<iListInput> c_InputDihs =
{
    { iListInput().setProperDihedrals(-100.0, 10.0, 2, -80.0, 20.0) },
    { iListInput().setProperDihedrals(-105.0, 15.0, 2) },
    { iListInput().setHarmonic(F_IDIHS, 100.0, 50.0) },
    { iListInput().setHarmonic(F_IDIHS, 100.15, 50.0, 95.0, 30.0) },
    { iListInput().setRbDihedrals(-5.35, 13.6, 8.4, -16.7, 0.3, 12.4,
                                  -6.35, 12.6, 8.1, -10.7, 0.9, 15.4) },
    { iListInput().setRbDihedrals(-7.35, 13.6, 8.4, -16.7, 1.3, 12.4 ) }
};
