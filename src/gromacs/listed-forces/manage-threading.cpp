                           ...
                           int thread, int nthread)
{
    ...
    if (nblock > f_thread->mask_nalloc)
    {
        f_thread->mask_nalloc = over_alloc_large(nblock);
        srenew(f_thread->mask, f_thread->mask_nalloc);
    }
...
#pragma omp parallel for num_threads(bt->nthreads) schedule(static)
    ...
    if (nblock_tot > bt->mask_nalloc)
    {
        bt->mask_nalloc = over_alloc_large(nblock_tot);
        srenew(bt->mask_index, bt->mask_nalloc);
        srenew(bt->mask,       bt->mask_nalloc);
    }
    bt->nblock = 0;
