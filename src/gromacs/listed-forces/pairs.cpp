                 ...
                 int *global_atom_index)
{
    ...
    for (i = 0; (i < nbonds); )
    {
        ...
        else
        {
            ...
            if (fr->nbv->nbat->params().zeta_matrix.size() > 0)
            {
...
#if !GMX_SIMD
        if (nbv->nbat->params().zeta_matrix.size() > 0)
        {
            const nbnxn_atomdata_t *nbat          = nbv->nbat;
            int                     ntype         = nbat->params().numTypes-1;
            real                    zeta          = nbat->params().zeta_matrix[md->typeA[ai[0]]*ntype+md->typeA[aj[0]]];
            real                    gauss_erf     = 1;
            real                    gauss_exp     = 0;
            if (zeta > 0)
            {
