...
real free_energy_evaluate_single(real r2, real sc_r_power, real alpha_coul,
                                 ...
                                 real *velectot, real *vvdwtot, real *dvdl)
{
    ...
    {
        rp             = std::pow(r2, 0.5*sc_r_power);  /* not currently supported as input, but can handle it */
        rpm2           = rp/r2;
    }
    ...
    for (i = 0; i < 2; i++)
    {
        if ((c6[i] > 0) && (c12[i] > 0))
        {
            ...
            sigma2[i]       = std::pow(0.5*c12[i]/c6[i], 1.0/3.0);
    ...
    for (i = 0; i < 2; i++)
    {
        ...
        if ( (qq[i] != 0) || (c6[i] != 0) || (c12[i] != 0) )
        {
            ...
            r_coul           = std::pow(rpinv, -1.0/sc_r_power);
            ...
            r_vdw            = std::pow(rpinv, -1.0/sc_r_power);
