                 ...
                 t_fcdata *fcd, t_state *state, gmx_bool bIsREMD)
{
    ...
    else
    {
        ...
        {
            gmx_fatal(FARGS, "Time-averaged distance restraints are not supported with domain decomposition. You can use OpenMP parallelization on a single node.");
        }
    ...
    if (cr && DOMAINDECOMP(cr) && ir->nstdisreout > 0)
    {
        /* With DD we currently only have local pair information available */
        gmx_fatal(FARGS, "With domain decomposition distance-restraint pair output is not supported. Use nstdisreout=0.");
    }
                     ...
                     t_fcdata *fcd, history_t *hist)
{
    ...
    if (fcd->disres.nsystems > 1)
    {
        ...
        GMX_ASSERT(cr->ms != NULL, "We need multisystem with nsystems>1");
