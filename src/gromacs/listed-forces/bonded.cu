...
static void
init_gpu_bonded(const int rank, gmx_grppairener_t *grppener)
{
    cudaError_t stat;
                  ...
                  const int xsize, const t_mdatoms *md, gmx_grppairener_t *grppener)
{
    ...
    for (int i = 0; i < F_NRE; i++)
    {
        ...
        if (iList.size() > 0)
        {
            ...
            // Allocate if we do not have enough memory
            // We should consider over-allocating.
            if (iList.size() > iListDevice.nalloc)
            {
                if (iListDevice.iatoms)
                {
                    stat = cudaFree(iListDevice.iatoms);
                    CU_RET_ERR(stat, "cudaFree failed");
                }
                stat = cudaMalloc(&iListDevice.iatoms, iList.size()*sizeof(t_iatom));
                CU_RET_ERR(stat, "cudaMalloc failed");
                iListDevice.nalloc = iList.size();
            }
            //copy  the data
