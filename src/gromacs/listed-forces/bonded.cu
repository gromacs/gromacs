...
update_gpu_bonded(GpuBondedLists *gpuBondedLists)
{
    ...
    for (int i = 0; i < F_NRE; i++)
    {
        ...
        if (iList.size() > 0)
        {
            ...
            //copy  the data
            stat = cudaMemcpy(iListDevice.iatoms, iList.iatoms.data(), iList.size()*sizeof(t_iatom),
...
void
do_bonded_gpu_finalize(t_forcerec *fr, const int flags,  gmx_enerdata_t *enerd)
{
    GpuBondedLists *gpuBondedLists = fr->gpuBondedLists;
