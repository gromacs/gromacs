                ...
                void                 *streamPtr)
{
    ...
    copyToDeviceBuffer(&gpuBondedLists->forceparamsDevice, ffparams.iparams.data(),:w
                       0, ffparams.numTypes(),
                      ...
                      fvec         *fshiftDevicePtr)
{
    ...
    for (int ftype : ftypesOnGpu)
    {
        ...
        if (iList.size() > 0)
        {
            ...
            {
                pdihs_gpu<calcEnerVir> <<< blocks, threads, 0, *stream>>>
                (&vtot_d[ftype], nbonds,
    ...
    for (int ftype : ftypesOnGpu)
    {
        ...
        if (iList.size() > 0)
        {
            ...
            if (ftype == F_BONDS)
            {
                bonds_gpu<calcEnerVir> <<< blocks, threads, 0, *stream>>>
                (&vtot_d[ftype], nbonds,
                 iatoms, forceparams_d,
                 xq, force, fshiftDevicePtr,
                 pbcAiuc);
            ...
            if (ftype == F_ANGLES)
            {
                angles_gpu<calcEnerVir> <<< blocks, threads, 0, *stream>>>
                (&vtot_d[ftype], nbonds,
                 iatoms, forceparams_d,
                 xq, force, fshiftDevicePtr,
                 pbcAiuc);
            ...
            {
                urey_bradley_gpu<calcEnerVir> <<< blocks, threads, 0, *stream>>>
                (&vtot_d[ftype], nbonds,
                 iatoms, forceparams_d,
                 xq, force, fshiftDevicePtr,
                 pbcAiuc);
            ...
            if (ftype == F_RBDIHS)
            {
                rbdihs_gpu<calcEnerVir> <<< blocks, threads, 0, *stream>>>
                (&vtot_d[ftype], nbonds,
                 iatoms, forceparams_d,
                 xq, force, fshiftDevicePtr,
                 pbcAiuc);
            ...
            if (ftype == F_IDIHS)
            {
                idihs_gpu<calcEnerVir> <<< blocks, threads, 0, *stream>>>
                (&vtot_d[ftype], nbonds,
                 iatoms, forceparams_d,
                 xq, force, fshiftDevicePtr,
                 pbcAiuc);
...
bonded_gpu_get_energies(t_forcerec *fr,  gmx_enerdata_t *enerd)
{
    ...
    // FIXME: Why on earthe does this throw:
    //          error: no instance of function template "copyFromDeviceBuffer" matches the argument list
    //                    argument types are: (float *, float *, size_t, size_t, cudaStream_t, GpuApiCallBehavior, std::nullptr_t)
    // 
    size_t        startOff = 0;
    size_t        numValues = F_NRE;
