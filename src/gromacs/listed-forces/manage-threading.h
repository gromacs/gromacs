...
struct GpuBondedLists
{
    ...
    t_iparams         *forceparamsDevice = nullptr; /**< Force field parameters on the device */
    t_ilist            iListsDevice[F_NRE];         /**< Interaction lists on the device */
    std::vector<real>  vtot;                        /**< Host side energy buffer */
    real              *vtotDevice   = nullptr;      /**< Device side energies */

    void              *stream;                      /**< Pointer to the bonded stream */
};
...
/*! \brief Returns where bonded interactions are assigned to the GPU */
static inline bool
bonded_gpu_have_interactions(const GpuBondedLists *gpuBondedLists)
{
