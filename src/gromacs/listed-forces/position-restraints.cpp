...
/* compute flat-bottomed positions restraints */
{
    ...
    {
        m_add(forceWithVirial->virial, virial, forceWithVirial->virial);
    }
            ...
            int refcoord_scaling, int ePBC, const rvec comA, const rvec comB)
{
    ...
    GMX_ASSERT(!computeForce || forceWithVirial != nullptr, "When forces are requested we need a force object");
    ...
    rvec       *f      = (computeForce ? as_rvec_array(forceWithVirial->force.data()) : nullptr);
    real        vtot   = 0.0;
    matrix      virial = { { 0 } };
    for (i = 0; (i < nbonds); )
    {
    ...
    if (computeForce && forceWithVirial->computeVirial)
    {
        m_add(forceWithVirial->virial, virial, forceWithVirial->virial);
    }
