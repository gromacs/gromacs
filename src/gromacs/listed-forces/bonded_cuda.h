...
void update_gpu_bonded(const t_idef *idef,
                       int size,  const t_mdatoms *md, gmx_grppairener_t *grppener) GPU_FUNC_TERM

GPU_FUNC_QUALIFIER
void do_bonded_gpu(t_forcerec *fr, const t_inputrec *ir, const t_idef *idef,
                   int numEnergyGroups,
                   int flags, const t_graph *graph, int natoms, rvec x[],
                   const matrix box) GPU_FUNC_TERM

GPU_FUNC_QUALIFIER
void do_bonded_gpu_finalize(t_forcerec *fr, int flags, int natoms,
                            rvec *input_force, gmx_enerdata_t *enerd) GPU_FUNC_TERM

GPU_FUNC_QUALIFIER
void reset_gpu_bonded(int size, int nener) GPU_FUNC_TERM
