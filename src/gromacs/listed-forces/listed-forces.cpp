...
static void
zero_thread_output(struct bonded_threading_t *bt, int thread, int natoms)
{
    f_thread_t *f_t = &bt->f_t[thread];

    if (natoms > f_t->f_nalloc)
    {
        f_t->f_nalloc = over_alloc_large(natoms);
        srenew(f_t->f, f_t->f_nalloc);
    }

    for (int b = 0; b < bt->nblock; b++)
    {
        int ind = bt->mask_index[b];

        if (bitmask_is_set(f_t->mask[ind], thread))
        {
            int a0 = ind*reduction_block_size;
            int a1 = std::min((ind + 1)*reduction_block_size, natoms);
            for (int a = a0; a < a1; a++)
            {
                clear_rvec(f_t->f[a]);
            }
...
#pragma omp parallel for num_threads(bt->nthreads) schedule(static)
    for (thread = 0; thread < bt->nthreads; thread++)
    {
        try
        {
            ...
            {
                zero_thread_output(bt, thread, fr->natoms_force);
