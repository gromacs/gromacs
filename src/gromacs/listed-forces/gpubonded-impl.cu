...
{
    GMX_ASSERT(haveInteractions_, "Cannot launch bonded energy transfer without kernel call");
...
void
GpuBonded::Impl::getEnergyTerms(gmx_enerdata_t *enerd)
{
    GMX_ASSERT(haveInteractions_, "Cannot transfer bonded energies without kernel call");
...
void
GpuBonded::getEnergyTerms(gmx_enerdata_t *enerd)
{
    impl_->getEnergyTerms(enerd);
}
