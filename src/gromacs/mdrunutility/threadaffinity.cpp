                           ...
                           int **localityOrder)
{
    ...
    // Only warn about the first problem per node: if layout is already valid,
    // set invalidValue to false as well.
    invalidValue = validLayout && (pin_offset + threads > hwThreads);
    if (invalidWithinSimulation(cr, invalidValue))
    {
        /* We are oversubscribing, don't pin */
        md_print_warn(cr, fplog,
    ...
    else
    {
        ...
        invalidValue = validLayout && (pin_offset + (threads-1)*(*pin_stride) >= hwThreads);
    }
    if (invalidWithinSimulation(cr, invalidValue))
    {
        /* We are oversubscribing, don't pin */
