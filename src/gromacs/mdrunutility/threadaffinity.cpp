                        ...
                        gmx::IThreadAffinityAccess            *affinityAccess)
{
    ...
    int  offset              = hardwareOptions.corePinningOffset.get();
    int  core_pinning_stride = hardwareOptions.corePinningStride.get();
    if (offset != 0)
    {
    ...
    bool affinityIsAutoAndNumThreadsIsNotAuto =
        (hardwareOptions.threadAffinity.get() == threadaffAUTO &&
         !hardwareOptions.totNumThreadsIsAuto());
    bool issuedWarning;
    bool validLayout
                              ...
                              gmx_bool                       bAfterOpenmpInit)
{
    ...
    if (!bAfterOpenmpInit)
    {
        ...
        if (hardwareOptions->threadAffinity.get() != threadaffOFF)
        {
            ...
            if (!gmx_omp_check_thread_affinity(&message))
            {
                ...
                if (hardwareOptions->threadAffinity.get() == threadaffON ||
                    hardwareOptions->totNumThreadsIsAuto())
                {
    ...
    if (!bAllSet)
    {
        if (hardwareOptions->threadAffinity.get() == threadaffAUTO)
        {
            if (!bAfterOpenmpInit)
            {
                GMX_LOG(mdlog.warning).asParagraph().appendText(
