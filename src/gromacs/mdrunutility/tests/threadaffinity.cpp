...
class ThreadAffinityTest : public gmx::test::ThreadAffinityTestBase
{
    public:
        void setAffinity(int nthread_local)
        {
            helper_.setAffinity(this, nthread_local);
        }
...
TEST_F(ThreadAffinityTest, HandlesPinningFailureWithOneThreadFailing)
{
    setAffinityOption(threadaffON);
    helper_.setLogicalProcessorCount(2);
    helper_.expectAffinintySet(0);
    helper_.expectAffinintySetThatFails(1);
    setAffinity(2);
}
#endif
