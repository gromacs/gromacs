...
class ThreadAffinityTestBase : public ::testing::Test
{
    public:
        ...
        const gmx_hw_opt_t *getHardwareOptions() const
        {
            return hwOpt_;
        }
...
class ThreadAffinityTestHelper
{
    public:
        ...
        void setAffinity(const ThreadAffinityTestBase *test, int nthread_local)
        {
            if (hwTop_ == nullptr)
            {
                setLogicalProcessorCount(1);
            }
            gmx_set_thread_affinity(nullptr, cr_, test->getHardwareOptions(),
                                    *hwTop_, nthread_local, &affinityAccess_);
        }
