...
void AccumulateGlobals::notifyReductionRequired(IAccumulateGlobalsClient *clientNotifying)
{
...
void AccumulateGlobals::notifyClientsAfterCommunication()
{
    if (c_debugBuild)
    {
        for (IAccumulateGlobalsClient *clientToNotify : clientsToNotifyThisStep_)
        {
            clientToNotify->notifyAfterCommunication();
        }
        // Reset the list for next usage.
        clientsToNotifyThisStep_.resize(0);
    }
