 ...
 */
/*! \libinternal \file
 * \brief Declares classes and interfaces for components that
 * collaborate to accumulate simulation variables across PP ranks
 * during an MD step.
 *
 * This collaboration is described in the following interaction diagram.
 *
 * \msc
   wordwraparcs=true;
 ...
 *
 * \todo Once all/enough modules have been converted to use this
 * framework, then this class can also have the responsibility of
 * holding a communicator and doing the MPI across collaborating
 * ranks, e.g. with PP duty. Then getReductionView() will no longer
 * be needed.
 */
class AccumulateGlobals
{
    ...
    public:
        //! Permits clients to notify that MPI reduction is required this step.
        void notifyReductionRequired();
    ...
    private:
        ...
        //! The set of clients to notify after reduction completes.
        ArrayRef<IAccumulateGlobalsClient * const> clients_;
...
class AccumulateGlobalsBuilder
{
    public:
        ...
        void registerClient(IAccumulateGlobalsClient *newClient);
    ...
    private:
        ...
        std::vector<IAccumulateGlobalsClient *> clients_;
};
