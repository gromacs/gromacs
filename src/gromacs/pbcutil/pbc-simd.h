...
/* The SIMD PBC routine seems to corrupt the memory with MSVC 2010.
 * MSVC 2012 produces correct results, so we require at least MSVC 2012.
 * Calling code should ensure not to call this function with MSVC 2010 or older
 * (if called anyhow, compilation with fail so we can't have incorrect runs).
 */
 ...
 * routine should be low. On e.g. Intel Haswell/Broadwell is takes 8 cycles.
 */
...
#endif /* GMX_SIMD_HAVE_REAL && !(defined _MSC_VER && _MSC_VER < 1700) */
