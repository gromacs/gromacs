 ...
 * \param pbc_simd   Pointer to aligned memory with 9*GMX_SIMD_REAL_WIDTH reals
 *                   describing the box vectors unrolled by GMX_SIMD_REAL_WIDTH.
 *                   These are sorted in a slightly non-standard
 *                   order to make sure we load them in order in the routine
 *                   pbc_correct_dx_simd().
