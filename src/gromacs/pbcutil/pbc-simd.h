                    ...
                    const pbc_simd_t *pbc)
{
    ...
    shz = simdRound(simdMul(*dz, pbc->inv_bzz));
    *dx = simdFnmadd(shz, pbc->bzx, *dx);
    *dy = simdFnmadd(shz, pbc->bzy, *dy);
    *dz = simdFnmadd(shz, pbc->bzz, *dz);

    shy = simdRound(simdMul(*dy, pbc->inv_byy));
    *dx = simdFnmadd(shy, pbc->byx, *dx);
    *dy = simdFnmadd(shy, pbc->byy, *dy);

    shx = simdRound(simdMul(*dx, pbc->inv_bxx));
    *dx = simdFnmadd(shx, pbc->bxx, *dx);
}
