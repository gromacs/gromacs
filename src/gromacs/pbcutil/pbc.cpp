...
real max_cutoff2(int ePBC, matrix box)
{
     ...
     */
    min_hv2 = static_cast<real>(std::min(0.25*norm2(box[XX]), 0.25*norm2(box[YY])));
    if (ePBC != epbcXY)
    {
        min_hv2 = std::min(min_hv2, static_cast<real>(0.25*norm2(box[ZZ])));
    }
    ...
    {
        min_ss = std::min(box[XX][XX], std::min(box[YY][YY] - static_cast<real>(fabs(box[ZZ][YY])), box[ZZ][ZZ]));
    }
