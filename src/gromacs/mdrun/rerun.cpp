...
void gmx::Integrator::do_rerun()
{
    ...
    if (ir->efep != efepNO)
    {
        /* Set free energy calculation frequency to nstdhdl. */
    ...
    if (constr)
    {
        GMX_LOG(mdlog.warning).asParagraph().
                    appendText("NOTE: Simulations has constraints. Rerun does not recalculate constraints.\n"
                               "      Rerun will not calculate the virial.\n");
    }
    ...
    if (MASTER(cr))
    {
        ...
        // Check if we read velocities. If yes, issue warning and ignore them
        // (now and for all subsequent steps).
        if (rerun_fr.bV)
        {
    ...
    while (!bLastStep)
    {
         ...
         */
        {
            ...
            do_md_trajectory_writing(fplog, cr, nfile, fnm, step, step_rel, t,
                                     ...
                                     top_global, fr,
                                     outf, mdebin, ekind, f,
                                     isCheckpointingStep, doRerun, bLastStep,
...

        {
            ...
            compute_globals(fplog, gstat, cr, ir, fr, ekind, state, mdatoms, nrnb, vcm,
                            ...
                            CGLO_GSTAT | CGLO_ENERGY | (constr ? 0 : CGLO_PRESSURE)
                            | (shouldCheckNumberOfBondedInteractions ? CGLO_CHECK_NUMBER_OF_BONDED_INTERACTIONS : 0)
                            );
            checkNumberOfBondedInteractions(mdlog, cr, totalNumberOfBondedInteractions,
                                            top_global, top, state,
        ...
        if (MASTER(cr))
        {
            ...
            // If no constraints, we want the virial, so we have calculated the pressure -
            // but the pressure is not what most users would expect, since the kinetic energy is zero.
            // Explicitly set to zero here.
            clear_mat(pres);
            enerd->term[F_PRES] = 0;
