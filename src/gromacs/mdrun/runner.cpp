...
int Mdrunner::mdrunner()
{
    ...
    try
    {
        gpuIdsAvailable = parseUserGpuIds(hardwareOptions.gpuIdsAvailable.get());
            ...
            for (size_t j = i+1; j != gpuIdsAvailable.size(); ++j)
            {
                if (gpuIdsAvailable[i] == gpuIdsAvailable[j])
                {
                    GMX_THROW(InvalidInputError(formatString("The string of available GPU device IDs '%s' may not contain duplicate device IDs",
                                                             hardwareOptions.gpuIdsAvailable.get().c_str())));
                }
            }
        }
    }
    ...
    try
    {
        userGpuTaskAssignment = parseUserGpuIds(hardwareOptions.gpuTaskAssignment.get());
    }
    GMX_CATCH_ALL_AND_EXIT_WITH_FATAL_ERROR;
    ...
    check_and_update_hw_opt_1(mdlog, &hardwareOptions, cr, domdecOptions.numPmeRanks);
    ...
    if (GMX_THREAD_MPI && SIMMASTER(cr))
    {
        ...
        try
        {
            ...
            useGpuForPme = decideWhetherToUseGpusForPmeWithThreadMpi
                    ...
                    canUseGpuForPme, hardwareOptions.nthreads_tmpi.get(), domdecOptions.numPmeRanks);
        ...
        if (hardwareOptions.nthreads_tmpi.isSetByUser() &&
            (hardwareOptions.nthreads_tmpi.get() != nThreadsSelected))
        {
        ...
        cr = spawnThreads(hardwareOptions.nthreads_tmpi.get());
    ...
    gmx_omp_nthreads_init(mdlog, cr,
                          ...
                          hardwareOptions.nthreads_omp_pme.get(),
    ...
    if (hardwareOptions.threadAffinity.get() != threadaffOFF)
    {
