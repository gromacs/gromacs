...
int Mdrunner::mdrunner()
{
    ...
    checkAndUpdateHardwareOptions(mdlog, &hw_opt, cr, domdecOptions.numPmeRanks);
    ...
    if (thisRankHasDuty(cr, DUTY_PP))
    {
        ...
        // mdatoms should not be null here, but we check to please the static analyzer
        std::unique_ptr<Constraints> constr;
        if (mdAtoms != nullptr && mdAtoms->mdatoms() != nullptr)
        {
            constr = makeConstraints(mtop, *inputrec, doEssentialDynamics,
