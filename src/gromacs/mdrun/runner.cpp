...
// Move assignment operator is noexcept per style requirements (supporting
// standard library containers, etc.) but this implementation does not offer
// much of an exception safety guarantee because of the natures of the data members.
Mdrunner &Mdrunner::operator=(Mdrunner &&handle) noexcept
{
    if (this != &handle)
    {
        // To do: get no-throw guarantee for hw_opt
        // static_assert(std::is_nothrow_move_assignable<decltype(hw_opt)>::value,
        //              "hw_opt member is not nothrow move assignable.");
        // The following may result in copy assignment instead of move assignment.
        // Assignment may throw, causing ungraceful exit due to the noexcept specification
        // on Mdrunner::operator=().
        hw_opt          = std::move(handle.hw_opt);
        static_assert(std::is_nothrow_move_assignable<decltype(filenames)>::value,
                      "filenames member is not nothrow move assignable.");
        filenames       = std::move_if_noexcept(handle.filenames);
        oenv            = handle.oenv;
        static_assert(std::is_nothrow_move_assignable<decltype(mdrunOptions)>::value,
                      "mdrunOptions member is not nothrow move assignable.");
        mdrunOptions    = std::move_if_noexcept(handle.mdrunOptions);
        static_assert(std::is_nothrow_move_assignable<decltype(domdecOptions)>::value,
                      "domdecOptions member is not nothrow move assignable.");
        domdecOptions   = std::move_if_noexcept(handle.domdecOptions);
        nbpu_opt        = handle.nbpu_opt;
        pme_opt         = handle.pme_opt;
        pme_fft_opt     = handle.pme_fft_opt;
        nstlist_cmdline = handle.nstlist_cmdline;
        static_assert(std::is_nothrow_move_assignable<decltype(replExParams)>::value,
                      "replExParams member is not nothrow move assignable.");
        replExParams    = std::move_if_noexcept(handle.replExParams);
