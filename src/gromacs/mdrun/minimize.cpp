    ...
    if (confout != nullptr)
    {
        if (DOMAINDECOMP(cr))
        {
            ...
            if (!bX)
            {
                ...
                dd_collect_vec(cr->dd, &state->s_local, state->s_local.x, globalXRef);
            }
        }
        else
        {
 ...
 * => use &ems->s_local for upd_mdebin to comply with function signature
 */
void
Integrator::do_lbfgs()
{
    ...
    *sc              = ems;
    ...
    if (MASTER(cr))
    {
        /* Copy stuff to the energy bin for easy printing etc. */
        upd_mdebin(mdebin, FALSE, FALSE, static_cast<double>(step),
                   mdatoms->tmass, enerd, &ems.s_local, inputrec->fepvals, inputrec->expandedvals, ems.s_local.box,
                   nullptr, nullptr, vir, pres, nullptr, mu_tot, constr);
