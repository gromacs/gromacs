    ...
    if (confout != nullptr)
    {
        if (DOMAINDECOMP(cr))
        {
            ...
            if (!bX)
            {
                ...
                dd_collect_vec(cr->dd, &state->s, state->s.x.unpaddedArrayRef(), globalXRef);
            }
        }
        else
        {
...
Integrator::do_lbfgs()
{
    ...
    *sc              = ems;
    ...
    for (step = 0; (number_steps < 0 || step <= number_steps) && !converged; step++)
    {
        ...
        mdoutf_write_to_trajectory_files(fplog, cr, outf, mdof_flags,
                                         top_global, step, static_cast<real>(step), &ems.s, state_global, observablesHistory, ems.f.unpaddedArrayRef());
