...
void gmx::Integrator::do_md()
{
    ...
    CheckpointSignalSetter     * checkpointSignalSetter  = nullptr;
    CheckpointSignalHandler    * checkpointSignalHandler = nullptr;
    std::vector<ISignalSetter *> stopSignalSetters;
    StopSignalHandler          * stopSignalHandler  = nullptr;
    ResetSignalSetter          * resetSignalSetter  = nullptr;
    ResetSignalHandler         * resetSignalHandler = nullptr;
    ...
    int  nstSignalComm         = nstglobalcomm;
    {
        ...
        SignalHelper::makeCheckpointSignalClasses(&signals[eglsCHKPT], ir, cr, mdrunOptions, simulationsShareState,
                                                  bNS, bLastStep, step, bGStat, bCPT, walltime_accounting,
                                                  &checkpointSignalSetter, &checkpointSignalHandler);
        SignalHelper::makeStopSignalClasses(&signals[eglsSTOPCOND], simulationsShareState, ir, cr,
                                            mdrunOptions, nstSignalComm, fplog, bNS, step, bLastStep,
                                            walltime_accounting, stopSignalSetters, &stopSignalHandler);
        SignalHelper::makeResetSignalClasses(&signals[eglsRESETCOUNTERS], !resetCountersIsLocal, ir, cr,
                                             mdrunOptions, walltime_accounting, wcycle, mdlog, step, step_rel,
                                             fplog, (use_GPU(fr->nbv) ? fr->nbv : nullptr), fr->pmedata, nrnb,
                                             pme_loadbal, &resetSignalSetter, &resetSignalHandler);
    }
    ...
    /* Delete signal setter / handler */
    SignalHelper::deleteCheckpointSignalClasses(&checkpointSignalSetter,
                                                &checkpointSignalHandler);
    SignalHelper::deleteStopSignalClasses(stopSignalSetters,
                                          &stopSignalHandler);
    SignalHelper::deleteResetSignalClasses(&resetSignalSetter,
                                           &resetSignalHandler);
