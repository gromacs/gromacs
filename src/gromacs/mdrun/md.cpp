...
void gmx::Integrator::do_md()
{
    ...
    if (ir->bDoHybridMCMD)
    {
        ...
        acceptOrRewindInstance       = gmx::compat::make_unique<AcceptOrRewind>(metropolisStepMehlig, state_global, cr);
    ...
    while (!bLastStep)
    {
...
#endif
                accepted = acceptOrRewind->updateTState(step, state, enerd, fplog, cr, top_global, ir,
                                                        enforcedRotation, &f, mdAtoms, top, fr, vsite,
                                                        constr, nrnb, wcycle, upd,
                                                        &shouldCheckNumberOfBondedInteractions);
            }
            else
            {
        ...
        if (ir->bDoHybridMCMD)
        {
            hybridMCMDVelocities = hybridMCMDVelocitiesInstance.get(); // needed to fix stupid clang warning
