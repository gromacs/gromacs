...
void gmx::Integrator::do_md()
{
    ...
    std::array<SignalHandler*, eglsNR> signalHandlers {};
    ...
    int  nstSignalComm         = nstglobalcomm;
    {
        ...
        signalHandlers[eglsCHKPT] = new CheckpointSignalHandler(&signals[eglsCHKPT],
                                                                ir, cr, mdrunOptions,
                                                                bGStat, bNS, bLastStep,
                                                                step, bCPT,
                                                                walltime_accounting);
        signalHandlers[eglsSTOPCOND] = new StopSignalHandler(&signals[eglsSTOPCOND],
                                                             ir, cr, mdrunOptions,
                                                             nstSignalComm, fplog,
                                                             bNS, step, bLastStep,
                                                             walltime_accounting);
        signalHandlers[eglsRESETCOUNTERS] = new ResetSignalHandler(&signals[eglsRESETCOUNTERS],
                                                                   ir, cr, mdrunOptions,
                                                                   step, step_rel,
                                                                   walltime_accounting, wcycle,
                                                                   mdlog, fplog,
        ...
        if (simulationsShareState)
        {
            signalHandlers[eglsCHKPT]->need_sync();
            signalHandlers[eglsSTOPCOND]->need_sync();
        }
        if (!resetCountersIsLocal)
        {
            signalHandlers[eglsRESETCOUNTERS]->need_sync();
        }
    }
    ...
    while (!bLastStep)
    {
        ...
        signalHandlers[eglsSTOPCOND]->handle_signal();
        ...
        signalHandlers[eglsCHKPT]->handle_signal();
        ...
        signalHandlers[eglsRESETCOUNTERS]->handle_signal();
