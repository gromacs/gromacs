...
void gmx::Integrator::do_md()
{
    ...
    if (DOMAINDECOMP(cr))
    {
        top.reset(dd_init_local_top(top_global));
        ...
        top = dd_partition_system(fplog, mdlog, ir->init_step, cr, TRUE, 1,
                                  state_global, top_global, ir,
                                  state, &f, mdAtoms, top.get(), fr,
                                  vsite, constr,
                                  nrnb, nullptr, FALSE);
    ...
    else
    {
        ...
        top = mdAlgorithmsSetupAtomData(cr, ir, top_global, top.get(), fr,
