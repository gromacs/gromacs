...
void gmx::Integrator::do_md()
{
    ...
    {
        state_global->state_change_natoms(state_global->natoms);
    ...
    while (!bLastStep)
    {
        ...
        if ( (membed != nullptr) && (!bLastStep) )
        {
            ...
            /* - Without domain decomposition, the global state is copied to the local state at line 470, approximately
             * - it is a deep copy, not a copy of a pointer
             * => all pieces of information added to the global state after line 470 will not be known by the local state!!!
             */
            else
            {
