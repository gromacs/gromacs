                      ...
                      warninp_t wi)
{
    ...
    for (int g = 0; g < pull->ngroup; g++)
    {
        ...
        {
            bool groupObeysPbc = true;
            if (pull->bSetPbcRefToPrevStepCOM)
            {
                groupObeysPbc = pullCheckPbcWithinGroup(*pull_work,
                ...
                {
                    char buf[STRLEN];
                    sprintf(buf, "When using the option pull-pbc-ref-prev-step-com a centrally "
            ...
            if (groupObeysPbc)
            {
                ...
                if (!groupObeysPbc)
                {
                    ...
                    sprintf(buf,
                            ...
                            g, c_pullGroupPbcMargin, pull->group[g].pbcatom);
                    if (!pull->bSetPbcRefToPrevStepCOM)
                    {
                        strcat(buf, " Try setting the pull-pbc-ref-prev-step-com option to true.");
                    }
                    set_warning_line(wi, nullptr, -1);
                    warning(wi, buf);
                }
