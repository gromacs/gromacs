                      ...
                      warninp_t wi)
{
    ...
    if (groupThatFailsPbc >= 0)
    {
        char buf[STRLEN];
        sprintf(buf,
                "Pull group %d has atoms at a distance larger than %f times half the box size from the PBC atom (%d). This means the COM is not well defined in the periodic system.",
                groupThatFailsPbc,
