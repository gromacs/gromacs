                   ...
                   const gmx_output_env_t *oenv)
{
    ...
    for (c = 0; c < pull->ncoord; c++)
    {
        ...
        double        value;
        real          init = 0;
        gmx_bool      bConvert_radians_to_degrees;
        char          units_str[STRLEN], errormsg[STRLEN];
        ...
        bConvert_radians_to_degrees = (pcrd->eGeom == epullgANGLE || pcrd->eGeom == epullgDIHEDRAL || pcrd->eGeom == epullgANGLEAXIS);

        if (bConvert_radians_to_degrees)
        {
            value *= RAD2DEG;
        }

        sprintf(units_str, bConvert_radians_to_degrees ?  "deg" : "nm");
        fprintf(stderr, " %10.3f %s", value, units_str);
        ...
        if (pcrd->eGeom == epullgDIST)
        {
            sprintf(errormsg, "The initial pull distance (%g) needs to be non-negative with geometry %s. If you want a signed distance, use geometry %s instead.",
                    pcrd->init, EPULLGEOM(pcrd->eGeom), EPULLGEOM(epullgDIR));
            GMX_RELEASE_ASSERT(pcrd->init >= 0, errormsg);
        ...
        {
            sprintf(errormsg,  "The initial pull reference angle (%g) is outside of the allowed range of 0 to 180 degrees.", pcrd->init);
            GMX_RELEASE_ASSERT((pcrd->init >= 0 && pcrd->init <= 180), errormsg);
        }
        else if (pcrd->eGeom == epullgDIHEDRAL)
        {
            if (pcrd->init < -180 || pcrd->init > 180)
            {
                gmx_fatal(FARGS,  "The initial pull reference angle (%g) is outside of the allowed range of -180 to 180 degrees.",
                          pcrd->init);
            }
        }
