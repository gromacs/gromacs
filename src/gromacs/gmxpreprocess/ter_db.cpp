                         ...
                         gpp_atomtype *atype)
{
    ...
    for (const auto &entry : tb)
    {
        ...
        {
            if (entry.hack[j].oname != nullptr && entry.hack[j].nname != nullptr)
            {
                nrepl++;
            }
            else if (entry.hack[j].oname == nullptr && entry.hack[j].nname != nullptr)
            {
                nadd++;
            }
            else if (entry.hack[j].oname != nullptr && entry.hack[j].nname == nullptr)
            ...
            {
                gmx_fatal(FARGS, "invalid hack (%s) in termini database", entry.name.c_str());
            }
        }
        if (nrepl)
        {
            ...
            for (int j = 0; j < entry.nhack; j++)
            {
                if (entry.hack[j].oname != nullptr && entry.hack[j].nname != nullptr)
                {
                    fprintf(out, "%s\t", entry.hack[j].oname);
                    print_atom(out, entry.hack[j].atom, atype);
                }
            }
        }
        if (nadd)
        {
            ...
            for (int j = 0; j < entry.nhack; j++)
            {
                if (entry.hack[j].oname == nullptr && entry.hack[j].nname != nullptr)
                {
                    print_ab(out, entry.hack[j], entry.hack[j].nname);
                    print_atom(out, entry.hack[j].atom, atype);
                }
            }
        }
        ...
        for (int bt = 0; bt < ebtsNR; bt++)
        {
            ...
            {
                fprintf(out, "[ %s ]\n", btsNames[bt]);
                for (int j = 0; j < entry.rb[bt].nb; j++)
                {
                    for (int k = 0; k < btsNiatoms[bt]; k++)
                    {
                        fprintf(out, "%s%s", k ? "\t" : "", entry.rb[bt].b[j].a[k]);
                    }
                    if (entry.rb[bt].b[j].s)
                    {
                        fprintf(out, "\t%s", entry.rb[bt].b[j].s);
                    }
                    fprintf(out, "\n");
                }
            }
...
AtomModificationBlock *choose_ter(gmx::ArrayRef<AtomModificationBlock *> tb, const char *title)
{
    ...
    for (const auto &entry : tb)
    {
        bool bIsZwitterion = (0 == gmx_wcmatch("*ZWITTERION*", entry->name.c_str()));
        printf("%2d: %s%s\n", i, entry->name.c_str(),
