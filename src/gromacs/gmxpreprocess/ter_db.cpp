        ...
        else
        {
            ...
            if (kwnr >= ebtsNR)
            {
                ...
                if (kwnr == ekwRepl || kwnr == ekwDel)
                {
                    ...
                    tb.back().hack.back().oname = gmx_strdup(buf);
                ...
                if (kwnr == ekwRepl || kwnr == ekwAdd)
                {
                    ...
                    if (tb.back().hack.back().nname == nullptr)
                    {
                        if (tb.back().hack.back().oname != nullptr)
                        {
                            tb.back().hack.back().nname = gmx_strdup(tb.back().hack.back().oname);
                        }
                        else
                        {
            ...
            else if (kwnr >= 0 && kwnr < ebtsNR)
            {
                ...
                for (j = 0; j < btsNiatoms[kwnr]; j++)
                {
                    ...
                    {
                        tb.back().rb[kwnr].b.back().a[j] = gmx_strdup(buf);
                    }
                    else
                    {
                ...
                for (; j < MAXATOMLIST; j++)
                {
                    tb.back().rb[kwnr].b.back().a[j] = nullptr;
                }
                strcpy(buf, "");
                sscanf(line+n, "%s", buf);
                tb.back().rb[kwnr].b.back().s = gmx_strdup(buf);
            }
            else
            {
    ...
    for (int i = 0; i < tb.size(); i++)
    {
        ...
        else
        {
            ...
            if (bFoundGenericTerminus)
            {
                 ...
                 */
                int j;
                for (j = 0; j < gmx::index(list.size()) && strstr((*list[j]).name, s) == nullptr; j++)
                {
                    ;
                }
