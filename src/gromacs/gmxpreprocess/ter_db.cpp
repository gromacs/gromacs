...
static void print_ter_db(const char *ff, char C, gmx::ArrayRef<const SystemModificationInstructions> tb,
                         gpp_atomtype *atype)
{
    ...
    for (const auto &modification : tb)
    {
        ...
        if (std::any_of(modification.hack.begin(), modification.hack.end(),
                        [](const auto &mod)
                        { return mod.type() == ModificationType::Replace; }))
        {
            fprintf(out, "[ %s ]\n", kw_names[ekwRepl-ebtsNR-1]);
            for (const auto &hack : modification.hack)
            {
                if (hack.type() == ModificationType::Replace)
                {
        ...
        if (std::any_of(modification.hack.begin(), modification.hack.end(),
                        [](const auto &mod)
                        { return mod.type() == ModificationType::Add; }))
        {
            fprintf(out, "[ %s ]\n", kw_names[ekwAdd-ebtsNR-1]);
            for (const auto &hack : modification.hack)
            {
                if (hack.type() == ModificationType::Add)
                {
        ...
        if (std::any_of(modification.hack.begin(), modification.hack.end(),
                        [](const auto &mod)
                        { return mod.type() == ModificationType::Delete; }))
        {
            fprintf(out, "[ %s ]\n", kw_names[ekwDel-ebtsNR-1]);
            for (const auto &hack : modification.hack)
            {
                if (hack.type() == ModificationType::Delete)
                {
                    fprintf(out, "%s\n", hack.oname.c_str());
                }
            }
...
static void read_ter_db_file(const char                                  *fn,
                             std::vector<SystemModificationInstructions> *tbptr,
                             gpp_atomtype                                *atype)
{
    ...
    SystemModificationInstructions *block = nullptr;
    while (!feof(in))
    {
        if (get_header(line, header))
        {
            ...
            if (kwnr == NOTSET)
            {
                tbptr->emplace_back(SystemModificationInstructions());
                block = &tbptr->back();
        ...
        else
        {
            ...
            if (kwnr >= ebtsNR)
            {
                ...
                block->hack.emplace_back(ModificationInstruction());
                ModificationInstruction *hack = &block->hack.back();
...
int read_ter_db(const char *ffdir, char ter,
                std::vector<SystemModificationInstructions> *tbptr, gpp_atomtype *atype)
{
...
std::vector<SystemModificationInstructions *>
filter_ter(gmx::ArrayRef<SystemModificationInstructions> tb,
           const char                                   *resname)
{
    ...
    std::vector<SystemModificationInstructions *> list;
        ...
        else
        {
            ...
            if (bFoundGenericTerminus)
            {
                ...
                auto found = std::find_if(list.begin(), list.end(),
                                          [&s](const SystemModificationInstructions *b)
...
SystemModificationInstructions *choose_ter(gmx::ArrayRef<SystemModificationInstructions *> tb, const char *title)
{
    int sel, ret;
