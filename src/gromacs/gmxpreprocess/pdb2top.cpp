...
static void at2bonds(t_params *psb, gmx::ArrayRef<MoleculePatches> globalPatches,
                     t_atoms *atoms,
                     rvec x[],
            ...
            if (patch->nr != 0)
            {
                ...
                else
                {
                    ...
                    switch (patch->type())
                    {
                        ...
                        {
                            /* we're adding: */
                            add_atom_to_restp(posres, l, &(*patch));
                            break;
                        }
                        ...
                        case PatchType::Replace:
                        {
                            ...
                            srenew(tmp, 1);
                            *tmp                 = gmx_strdup(patch->nname.c_str());
...
static bool atomname_cmp_nr(const char *anm, const Patch *patch, int *nr)
{
...
static bool match_atomnames_with_rtp_atom(t_atoms               *pdba,
                                          rvec                  *x,
                                          int                    atind,
                                          PreprocessResidue     *rptr,
                                          const MoleculePatches &singlePatch,
                                          bool                   bVerbose)
{
    int      resnr;
    char    *oldnm;
         ...
         patch++)
    {
            ...
            gmx::equalCaseInsensitive(oldnm, patch->oname))
        {
            ...
            {
                if (patch != selfPatch &&
                    selfPatch->type() == PatchType::Replace &&
            ...
            if (found == rptr->atomname.end())
            {
                     ...
                     rtpModification++)
                {
                    ...
                    std::string start_at;
                    if (rtpModification->type() == PatchType::Add &&
        ...
        else if (patch->type() == PatchType::Delete &&
                 gmx::equalCaseInsensitive(oldnm, patch->oname))
        {
