...
static void do_ssbonds(t_params *ps, t_atoms *atoms,
                       gmx::ArrayRef<const t_ssbond> ssbonds, bool bAllowMissing)
{
    for (const auto &bond : ssbonds)
    {
        int ri = bond.res1;
        int rj = bond.res2;
        int ai = search_res_atom(bond.a1.c_str(), ri, atoms,
                                 "special bond", bAllowMissing);
        int aj = search_res_atom(bond.a2.c_str(), rj, atoms,
                                 "special bond", bAllowMissing);
        if ((ai == -1) || (aj == -1))
        {
            gmx_fatal(FARGS, "Trying to make impossible special bond (%s-%s)!",
                      bond.a1.c_str(), bond.a2.c_str());
        }
...
void pdb2top(FILE *top_file, const char *posre_fn, const char *molname,
             ...
             const char *ffdir,
             real mHmult,
             gmx::ArrayRef<const t_ssbond> ssbonds,
             real long_bond_dist, real short_bond_dist,
