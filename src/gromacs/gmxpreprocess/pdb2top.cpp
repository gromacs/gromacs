...
static void at2bonds(t_params *psb, gmx::ArrayRef<SystemModificationInstructions> globalModifications,
                     t_atoms *atoms,
                     rvec x[],
                     real long_bond_dist, real short_bond_dist)
{
    ...
    for (int resind = 0; (resind < atoms->nres) && (i < atoms->nr); resind++)
    {
        ...
        for (int j = 0; j < globalModifications[resind].rb[ebtsBONDS].nb; j++)
        {
            /* Unfortunately we can not issue errors or warnings
             * for missing atoms in bonds, as the hydrogens and terminal atoms
             * have not been added yet.
             */
            int ai = search_atom(globalModifications[resind].rb[ebtsBONDS].b[j].a[0], i, atoms,
                                 ptr, TRUE);
            int aj = search_atom(globalModifications[resind].rb[ebtsBONDS].b[j].a[1], i, atoms,
                                 ptr, TRUE);
            if (ai != -1 && aj != -1)
            {
                ...
                add_param(psb, ai, aj, nullptr, globalModifications[resind].rb[ebtsBONDS].b[j].s);
            }
        }
        /* add bonds from list of hacks (each added atom gets a bond) */
        while ( (i < atoms->nr) && (atoms->atom[i].resind == resind) )
        {
            for (const auto &h : globalModifications[resind].hack)
            {
                if ( ( h.tp > 0 ||
                       h.type() == ModificationType::Add ) &&
                     strcmp(h.a[0].c_str(), *(atoms->atomname[i])) == 0)
                {
                    switch (h.tp)
                    {
...
static void add_atom_to_restp(t_restp *restp, int at_start, const ModificationInstruction *hack)
{
    char        buf[STRLEN];
    int         k;
    const char *Hnum = "123456";

    strcpy(buf, hack->nname.c_str());
    buf[strlen(buf)+1] = '\0';
    if (hack->nr > 1)
    {
        buf[strlen(buf)] = '-';
    ...
    for (k = 0; k < hack->nr; k++)
    {
        ...
        restp->atom     [at_start+1+k] = hack->atom.back();
...
void get_hackblocks_rtp(std::vector<SystemModificationInstructions> *globalModifications, t_restp **restp,
                        ...
                        bool bAllowMissing)
{
    ...
    globalModifications->resize(nres);
    snew(*restp, nres);
    /* first the termini */
    for (int i = 0; i < nterpairs; i++)
    {
        ...
        {
            copyModificationBlocks(*ntdb[i], &globalModifications->at(rn[i]));
        }
        if (rc[i] >= 0 && ctdb[i] != nullptr)
        {
            mergeAtomAndBondModifications(*ctdb[i], &globalModifications->at(rc[i]));
        }
    }
    ...
    for (int i = 0; i < nres; i++)
    {
        ...
        *resinfo[i].rtp = search_rtp(key, nrtp, rtp);
        ...
        bRM = merge_t_bondeds(res->rb, globalModifications->at(i).rb, tern >= 0, terc >= 0);
    ...
    for (auto modifiedResidue = globalModifications->begin();
         modifiedResidue != globalModifications->end();
         modifiedResidue++)
    {
        const int pos = std::distance(globalModifications->begin(), modifiedResidue);
        for (auto modification = modifiedResidue->hack.begin();
             modification != modifiedResidue->hack.end();
             modification++)
        {
            if (modification->nr != 0)
            {
                /* find atom in restp */
                int l;
                for (l = 0; l < (*restp)[pos].natom; l++)
                {
                    if ( ( modification->oname.empty() &&
                           strcmp(modification->a[0].c_str(), *(*restp)[pos].atomname[l]) == 0 ) ||
                         ( !modification->oname.empty() &&
                           strcmp(modification->oname.c_str(), *(*restp)[pos].atomname[l]) == 0 ) )
                    {
                        break;
                    }
                }
                /* Reaching this point means that the modification renames or deletes.
                ...
                if (l == (*restp)[pos].natom)
                {
                    ...
                    if (modification->type() == ModificationType::Add)
                    {
                        gmx_fatal(FARGS,
                                  ...
                                  modification->oname.empty() ?
                                  modification->a[0].c_str() : modification->oname.c_str(),
                ...
                else
                {
                    switch (modification->type())
                    {
                        ...
                        {
                            /* we're adding: */
                            add_atom_to_restp(&(*restp)[pos], l, &(*modification));
                            break;
                        }
                        ...
                        case ModificationType::Replace:
                        {
                            ...
                            (*restp)[pos].atom[l]      =       modification->atom.back();
                            *(*restp)[pos].atomname[l] = gmx_strdup(modification->nname.c_str());
                            if (modification->cgnr != NOTSET)
                            {
                                (*restp)[pos].cgnr   [l] = modification->cgnr;
                            }
                            break;
                        }
                    }
...
static bool atomname_cmp_nr(const char *anm, const ModificationInstruction *hack, int *nr)
{
...
static bool match_atomnames_with_rtp_atom(t_atoms                              *pdba,
                                          ...
                                          int                                   atind,
                                          t_restp                              *rptr,
                                          const SystemModificationInstructions &modInstruction,
                                          bool                                  bVerbose)
{
    int      resnr;
    char    *oldnm;
    ...
    bDeleted = FALSE;
    for (auto modification = modInstruction.hack.begin();
         modification != modInstruction.hack.end();
         modification++)
    {
        if (modification->type() == ModificationType::Replace &&
            gmx::equalCaseInsensitive(oldnm, modification->oname))
        {
            /* This is a replace entry. */
            /* Check if we are not replacing a replaced atom. */
            bool bReplaceReplace = false;
            for (auto selfModification = modInstruction.hack.begin();
                 selfModification != modInstruction.hack.end();
                 selfModification++)
            {
                if (modification != selfModification &&
                    selfModification->type() == ModificationType::Replace &&
                    gmx::equalCaseInsensitive(selfModification->nname, modification->oname))
                {
            ...
            std::string newnm = modification->nname;
            int         k;
            ...
            if (k == rptr->natom)
            {
                ...
                {
                    std::string start_at;
                    if (rtpModification->type() == ModificationType::Add &&
                        atomname_cmp_nr(newnm.c_str(), &(*rtpModification), &anmnr))
                    {
                        ...
                        add_atom_to_restp(rptr, start_nr, &(*modification));
                ...
                if (!bFoundInAdd)
                {
                    gmx_fatal(FARGS, "Could not find an 'add' entry for atom named '%s' corresponding to the 'replace' entry from atom name '%s' to '%s' for tdb or hdb database of residue type '%s'",
                              newnm.c_str(),
                              modification->oname.c_str(), modification->nname.c_str(),
                              rptr->resname);
                }
            }
...
void match_atomnames_with_rtp(t_restp restp[], gmx::ArrayRef<SystemModificationInstructions> globalModifications,
                              t_atoms *pdba, rvec *x,
                              bool bVerbose)
{
    int          i, j;
    ...
    for (i = 0; i < pdba->nr; i++)
    {
        ...
        if (j == rptr->natom)
        {
            ...
            if (match_atomnames_with_rtp_atom(pdba, x, i,
                                              rptr, globalModifications[pdba->atom[i].resind],
                                              bVerbose))
            {
...
void pdb2top(FILE *top_file, const char *posre_fn, const char *molname,
             ...
             int nrtp, t_restp rtp[],
             t_restp *restp, gmx::ArrayRef<SystemModificationInstructions> globalModifications,
             bool bAllowMissing,
             ...
             bool bRenumRes, bool bRTPresname)
{
    ...
    /* Make bonds */
    at2bonds(&(plist[F_BONDS]), globalModifications,
             atoms, *x,
    ...
    gen_nnb(&nnb, plist);
    print_nnb(&nnb, "NNB");
    gen_pad(&nnb, atoms, restp, plist, excls, globalModifications, bAllowMissing);
    done_nnb(&nnb);
    ...
    /* cleaning up */
    freeModificationBlock(globalModifications);
