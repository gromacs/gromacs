        ...
        while ( (i < atoms->nr) && (atoms->atom[i].resind == resind) )
        {
            for (const auto &patch : globalPatches[resind].hack)
            {
                if ( ( patch.tp > 0 ||
                     ...
                     strcmp(patch.a[0].c_str(), *(atoms->atomname[i])) == 0)
                {
                    switch (patch.tp)
                    {
...
static void add_atom_to_restp(t_restp *restp, int at_start, const Patch *patch)
{
    char        buf[STRLEN];
    int         k;
...
void get_hackblocks_rtp(std::vector<MoleculePatches> *globalPatches, t_restp **restp,
            ...
            if (patch->nr != 0)
            {
                ...
                for (l = 0; l < (*restp)[pos].natom; l++)
                {
                    if ( ( patch->oname.empty() &&
                           strcmp(patch->a[0].c_str(), *(*restp)[pos].atomname[l]) == 0 ) ||
                         ( !patch->oname.empty() &&
                           strcmp(patch->oname.c_str(), *(*restp)[pos].atomname[l]) == 0 ) )
                    {
                        break;
                    }
                }
...
static bool match_atomnames_with_rtp_atom(t_atoms                              *pdba,
                                          ...
                                          int                                   atind,
                                          t_restp                              *rptr,
                                          const MoleculePatches                &modInstruction,
                                          bool                                  bVerbose)
{
    int      resnr;
    char    *oldnm;
