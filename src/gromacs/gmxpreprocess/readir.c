...
static void calc_nrdf(gmx_mtop_t *mtop, t_inputrec *ir, char **gnames)
{
    ...
    if (ir->nstcomm != 0)
    {
        /* Subtract 3 (or less with frozen dimensions) from the number
         * of degrees of freedom in each vcm group when com translation
         * is removed and 6 when rotation is removed as well.
         */
