                  ...
                  warninp_t wi)
{
    ...
    if (ir->cutoff_scheme == ecutsVERLET &&
        ir->verletbuf_tol > 0 &&
        ir->nstlist > 1 &&
        (EI_SD(ir->eI) || ((ir->eI == eiMD || EI_VV(ir->eI)) && (ir->etc == etcVRESCALE || ir->etc == etcBERENDSEN))))
    {
        /* Check if a too small Verlet buffer might potentially
         * cause more error than the thermostay can couple off.
         */
         ...
         */
        max_T_error = 0.5*tau*ir->verletbuf_tol/(nrdf_min*BOLTZ*T);
