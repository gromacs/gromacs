...
void print_ab(FILE *out, const ModificationInstruction &hack, const char *nname)
{
...
void read_ab(char *line, const char *fn, ModificationInstruction *hack)
{
    int  nh, tp, ns;
    char a[4][12];
    char hn[32];
...
static void read_h_db_file(const char *hfn, std::vector<SystemModificationInstructions> *amb)
{
    ...
    while (fgets2(line, STRLEN-1, in))
    {
        ...
        amb->emplace_back(SystemModificationInstructions());
        SystemModificationInstructions *block = &amb->back();
    ...
    if (!amb->empty())
    {
        ...
        std::sort(amb->begin(), amb->end(),
                  [](const SystemModificationInstructions &a1, const SystemModificationInstructions &a2)
...
search_h_db(gmx::ArrayRef<SystemModificationInstructions> amb, char *key)
{
    return std::find_if(amb.begin(), amb.end(),
                        [&key](const SystemModificationInstructions &a)
