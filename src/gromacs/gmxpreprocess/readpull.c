                       ...
                       warninp_t wi)
{
    ...
    ITYPE("pull-nstfout",     pull->nstfout, 10);
    ...
    for (i = 1; i < pull->ncoord + 1; i++)
    {
        ...
        init_pull_coord(pcrd, dim_buf, origin_buf, vec_buf);
    }
...
void make_pull_coords(t_pull *pull)
{
    ...
    for (c = 0; c < pull->ncoord; c++)
    {
        ...
        {
            if (pcrd->group[0] != pull_group_cylinder)
            {
                gmx_fatal(FARGS, "With pull geometry %s, the first pull group should always be %d", EPULLGEOM(pcrd->eGeom), pull_group_cylinder);
            }
