...
#define MAXCOL 7
            ...
            for (b = 0; (b < nspec); b += MAXCOL)
            {
                ...
                for (i = b; (i < e); i++)
                {
                    std::string buf = gmx::formatString("%s%d", pdba->atomname[sgp[i]]->c_str(), sgp[i]+1);
                ...
                for (i = b+1; (i < nspec); i++)
                {
                    std::string buf = gmx::formatString("%s%d", pdba->resinfo[pdba->atom[sgp[i]].resind].name->c_str(),
                                                        pdba->resinfo[specp[i]].nr);
                    fprintf(stderr, "%8s", buf.c_str());
                    buf = gmx::formatString("%s%d", pdba->atomname[sgp[i]]->c_str(), sgp[i]+1);
                    fprintf(stderr, "%8s", buf.c_str());
                    e2 = std::min(i, e);
                    for (j = b; (j < e2); j++)
                    {
