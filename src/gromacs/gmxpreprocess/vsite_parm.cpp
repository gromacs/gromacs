...
static void get_bondeds(int nrat, gmx::ArrayRef<const t_iatom> atoms,
...
static at2vsitebond_t *make_at2vsitebond(int natoms, gmx::ArrayRef<t_params> plist)
{
    bool           *bVSI;
    int             nrcheck;
    at2vsitebond_t *at2vb;
    ...
    for (int ftype = 0; (ftype < F_NRE); ftype++)
    {
        ...
        if (nrcheck > 0)
        {
            for (int i = 0; (i < plist[ftype].nr()); i++)
            {
                gmx::ArrayRef<const int> aa = plist[ftype].param[i].a;
                for (int j = 0; j < nrcheck; j++)
                {
                    if (bVSI[aa[j]])
                    {
                        int nr = at2vb[aa[j]].nr;
                        if (nr % 10 == 0)
                        {
...
static at2vsitecon_t *make_at2vsitecon(int natoms, gmx::ArrayRef<t_params> plist)
{
    bool          *bVSI;
    at2vsitecon_t *at2vc;
    ...
    for (int ftype = 0; (ftype < F_NRE); ftype++)
    {
        if (interaction_function[ftype].flags & IF_CONSTRAINT)
        {
            for (int i = 0; (i < plist[ftype].nr()); i++)
            {
                int ai = plist[ftype].param[i].ai();
                int aj = plist[ftype].param[i].aj();
                if (bVSI[ai] && bVSI[aj])
                {
...
static void check_vsite_constraints(gmx::ArrayRef<t_params> plist,
                                    int cftype, const int vsite_type[])
{
    int       n  = 0;
    t_params *ps = &(plist[cftype]);
    for (int i = 0; (i < ps->nr()); i++)
    {
        for (int k = 0; k < 2; k++)
        {
            int atom = ps->param[i].a[k];
...
static void clean_vsite_bonds(gmx::ArrayRef<t_params> plist, t_pindex pindex[],
                              int cftype, const int vsite_type[])
{
    int                  nvsite, nOut;
    int                  nconverted, nremoved;
    int                  atom, oatom;
    bool                 bThisFD, bThisOUT, bFirstTwo;
    t_params            *ps;

    std::vector<t_param> cleaned;
    ...
    nremoved   = 0;
    nOut       = 0;
    for (int i = 0; (i < ps->nr()); i++) /* for all bonds in the plist */
    {
        int                      vsnral      = 0;
        gmx::ArrayRef<const int> first_atoms;
            ...
            if (vsite_type[atom] != NOTSET && vsite_type[atom] != F_VSITEN)
            {
                ...
                if (!bRemove)
                {
                    ...
                    if (nvsite == 1)
                    {
                        ...
                        first_atoms = plist[pindex[atom].ftype].param[pindex[atom].parnr].a;
                    }
                    else
                    {
                        GMX_ASSERT(vsnral != 0, "nvsite > 1 must have vsnral != 0");
                        GMX_ASSERT(!first_atoms.empty(), "nvsite > 1 must have valid first_atoms");
                        ...
                        if (vsnral == NRAL(pindex[atom].ftype)-1)
                        {
                            for (int m = 0; (m < vsnral) && !bKeep; m++)
                            {
                                ...
                                bool                     bPresent = false;
                                gmx::ArrayRef<const int> atoms    = plist[pindex[atom].ftype].param[pindex[atom].parnr].a;
        ...
        else
        {
            ...
            for (int k = 0; (k < 2) && !bKeep; k++) /* for all atoms in the bond */
            {
                ...
                if (vsite_type[atom] == NOTSET)
                {
                    ...
                    {
                        GMX_ASSERT(!first_atoms.empty(), "If we've seen a vsite before, we know what its first atom index was");
            ...
            if (!( bAllFD && bFirstTwo ) )
            {
                ...
                for (int m = 0; m < vsnral && !bKeep; m++) /* all constr. atoms */
                {
                    ...
                    for (int ftype = 0; ftype < F_NRE; ftype++)
                    {
                        if (interaction_function[ftype].flags & IF_CONSTRAINT)
                        {
                            for (int j = 0; (j < plist[ftype].nr()) && !bPresent; j++)
                            {
        ...
        if (bKeep)
        {
            ...
            cleaned.push_back(ps->param[i]);
        }
        else if (IS_CHEMBOND(cftype))
        {
            plist[F_CONNBONDS].param.push_back(ps->param[i]);
            nconverted++;
        }
        else
        {
    ...
    if (nremoved)
    {
        ...
        fprintf(stderr, "Removed   %4d %15ss with virtual sites, %5d left\n",
                nremoved, interaction_function[cftype].longname, size);
    }
    if (nconverted)
    {
        int size = cleaned.size();
        fprintf(stderr, "Converted %4d %15ss with virtual sites to connections, %5d left\n",
                nconverted, interaction_function[cftype].longname, size);
    }
    if (nOut)
    {
...
static void clean_vsite_angles(gmx::ArrayRef<t_params> plist, t_pindex pindex[],
                               int cftype, const int vsite_type[],
                               at2vsitecon_t *at2vc)
{
    std::vector<t_param> cleaned;
    t_params            *ps     = &(plist[cftype]);
    for (int i = 0; (i < ps->nr()); i++) /* for all angles in the plist */
    {
        int                      vsnral      = 0;
        gmx::ArrayRef<const int> first_atoms;
    ...
    {
        fprintf(stderr, "Removed   %4d %15ss with virtual sites, %5d left\n",
                ps->nr()-size, interaction_function[cftype].longname, size);
    }
    ps->param = cleaned;
}

static void clean_vsite_dihs(gmx::ArrayRef<t_params> plist, t_pindex pindex[],
                             int cftype, const int vsite_type[])
{
    t_params            *ps = &(plist[cftype]);
    std::vector<t_param> cleaned;

    for (int i = 0; (i < ps->nr()); i++) /* for all dihedrals in the plist */
    {
        int                      vsnral      = 0;
        gmx::ArrayRef<const int> first_atoms;
...
void clean_vsite_bondeds(gmx::ArrayRef<t_params> plist, int natoms, bool bRmVSiteBds)
{
    int           *vsite_type;
    t_pindex      *pindex;
    at2vsitecon_t *at2vc;
    ...
    for (int ftype = 0; ftype < F_NRE; ftype++)
    {
        if (interaction_function[ftype].flags & IF_VSITE)
        {
            ...
            {
                int vsite = plist[ftype].param[i].ai();
    ...
    if (nvsite)
    {
        ...
        for (int ftype = 0; ftype < F_NRE; ftype++)
        {
            ...
            if ((interaction_function[ftype].flags & IF_VSITE) &&
                ftype != F_VSITEN)
            {
                for (int parnr = 0; (parnr < plist[ftype].nr()); parnr++)
                {
                    int k               = plist[ftype].param[parnr].ai();
