    ...
    for (auto &mol : mols)
    {
        ...
        for (int bb = 0; (bb < F_NRE); bb++)
        {
            if ((interaction_function[bb].flags & IF_BTYPE) && (bb != F_MORSE))
            {
                ...
                for (auto harmonic = mol.plist[bb].param.begin();
                     harmonic != mol.plist[bb].param.end(); )
                {
                    int  ni   = harmonic->ai();
                    int  nj   = harmonic->aj();
                    real edis =
                        search_e_diss(n2m, t2m,
                                      atype->atomNameFromType(mol.system.atoms[ni].type_),
                                      atype->atomNameFromType(mol.system.atoms[nj].type_));
                    if (edis != 0)
                    {
                        real              b0             = harmonic->c0();
                        real              kb             = harmonic->c1();
                        ...
                        mol.plist[F_MORSE].param.emplace_back(
                                FFParameter(atoms, forceParam, ""));
                        harmonic = mol.plist[bb].param.erase(harmonic);
                    }
                    else
                    {
                        ++harmonic;
