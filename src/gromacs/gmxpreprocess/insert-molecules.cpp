			       ...
			       std::unordered_map<char *, int> &namesRemovedCounts)
{
    ...
    {
      if(namesRemovedCounts.count(it->second) == 0) { namesRemovedCounts[it->second] = 1; }
      else { namesRemovedCounts[it->second] += 1; }
    }
    return true;
}
			...
			std::unordered_map<char *, int> &namesRemovedCounts)
{
	...
	//! temporary to allow checking if we removed anything this iteration
