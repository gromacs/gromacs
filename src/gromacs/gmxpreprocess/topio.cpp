...
static void generate_mimic_excl(gmx_moltype_t *molt,
                                std::unordered_map<uint, bool> qm_atoms,
                                uint ind_offset)
{
    ...
    for (int ftype = 0; ftype < F_NRE; ftype++)
    {
        ...
        while (j < molt->ilist[ftype].nr)
        {
            ...
            if (nratoms == 2)
            {
                ...
                bexcl  = (qm_atoms[a1 + ind_offset] && qm_atoms[a2 + ind_offset]);
            ...
            else
            {
                ...
                {
                    if (qm_atoms[molt->ilist[ftype].iatoms[jj] + ind_offset])
                    {
                        numQmAtoms++;
                    }
                }
    ...
    {
        qmexcl.index[i] = j;
        if (qm_atoms[ind_offset + i])
        {
            uint k      = 0;
            int  search = 0;
            while (k < qm_nr)
            {
                if (qm_atoms[search + ind_offset])
                {
                    qmexcl.a[k+j] = search;
                    k++;
                }
...
void generate_qmexcl(gmx_mtop_t *sys, t_inputrec *ir, warninp_t wi, gmx_bool mimic)
{
    ...
    gmx_molblock_t                *molb;
    gmx_bool                       bQMMM;
    std::unordered_map<uint, bool> qm_map;
    std::unordered_map<uint, uint> types_map;
