               ...
               const int *rN, const int *rC)
{
    ...
    {
        ahptr = search_h_db(ah, *pdba->resinfo[rnr].rtp);
        if (ahptr)
        {
    ...
    for (int j = 0; j < hbr->nhack(); j++)
    {
                ...
                strcmp(atomname, hbr->hack[j].oname) == 0) ) )
        {
            ...
            if (hbr->hack[j].tp > 0 || hbr->hack[j].oname == nullptr)
            {
                ...
                {
                    int size = abi->size();
                    expand_hackblocks_one(hbr, abi->at(size-hbr->hack[j].nr()+k).nname,
                                          abi, bN, bC);
                }
            }
        }
...
static void expand_hackblocks(t_atoms *pdba, gmx::ArrayRef<t_hackblock>  hb,
                              gmx::ArrayRef < std::vector < t_hack>> ab,
...
static int check_atoms_present(t_atoms *pdba, gmx::ArrayRef < std::vector < t_hack>> ab)
{
    int nadd = 0;
    for (int i = 0; i < pdba->nr; i++)
    {
...
static void calc_all_pos(t_atoms *pdba, rvec x[], gmx::ArrayRef < std::vector < t_hack>> ab,
                         bool bCheckMissing)
{
    int      ii = 0;
#define MAXH 4
