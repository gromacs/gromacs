    ...
    for (auto it = hbr.hack.begin(); it != hbr.hack.end(); it++)
    {
        ...
        if (!bIgnore &&
            ( ( ( it->tp > 0 || it->oname.empty() ) &&
                strcmp(atomname, it->ai()) == 0 ) ||
              ( !it->oname.empty() &&
                strcmp(atomname, it->oname.c_str()) == 0) ) )
        {
...
#define MAXH 4
    ...
    for (int i = 0; i < pdba->nr; i++)
    {
        ...
        for (auto it = ab[i].begin(); it != ab[i].end(); it++)
        {
            ...
            if (it->type() == HackType::Add && it->tp > 0)
            {
                bool bFoundAll = true;
                for (int m = 0; (m < it->nctl && bFoundAll); m++)
                {
                    int ia = pdbasearch_atom(it->a[m].c_str(), rnr, pdba,
                                             bCheckMissing ? "atom" : "check",
                                             !bCheckMissing);
                    if (ia < 0)
                    {
                        /* not found in original atoms, might still be in t_hack (ab) */
                        hacksearch_atom(&ii, &jj, it->a[m].c_str(), ab, rnr, pdba);
                        if (ii >= 0)
                        {
                ...
                if (bFoundAll)
                {
                    ...
                    calc_h_pos(it->tp, xa, xh, &l);
                    for (int m = 0; m < it->nr; m++)
                    {
                        auto next = it+m;
    ...
    for (int i = 0; (i < natoms); i++)
    {
        ...
        if (ab[i].empty()  || (!ab[i][0].nname.empty()) )
        {
            ...
            for (auto it = ab[i].begin(); it != ab[i].end(); it++)
            {
                ...
                if (it->type() == HackType::Replace ||
                    strcmp(it->oname.c_str(), *newpdba->atomname[newi]) == 0)
                {
                    /* add or replace */
