...
static void hacksearch_atom(int *ii, int *jj, const char *name,
                            gmx::ArrayRef < std::vector < Patch>> patches,
...
static std::vector<MoleculePatches>
getMoleculePatchess(t_atoms *pdba,
...
{
    std::vector<MoleculePatches> modBlock(pdba->nres);
    /* make space */
    /* first the termini */
...
static void expand_hackblocks_one(const MoleculePatches &modInstruction,
                                  const char *atomname,
                                  std::vector<Patch> *globalPatches,
                                  bool bN, bool bC)
{
    ...
    for (auto &modifiedResidue : modInstruction.hack)
    {
                ...
                strcmp(atomname, modifiedResidue.oname.c_str()) == 0) ) )
        {
            ...
            {
                globalPatches->push_back(modifiedResidue);
                Patch *patch = &globalPatches->back();
                patch->bXSet = false;
                ...
                else
                {
                    if (gmx_debug_at)
                    {
                        fprintf(debug, "Hack '%s' %d, replacing nname '%s' with '%s' (old name '%s')\n",
                                atomname, pos,
                                patch->nname.c_str(), modifiedResidue.nname.c_str(),
                                patch->oname.empty() ? "" : patch->oname.c_str());
                    }
                    patch->nname = modifiedResidue.nname;
                }
...
static void expand_hackblocks(t_atoms *pdba, gmx::ArrayRef<const MoleculePatches> hb,
                              gmx::ArrayRef < std::vector < Patch>> patches,
...
static int check_atoms_present(t_atoms *pdba, gmx::ArrayRef < std::vector < Patch>> patches)
{
    int nadd = 0;
    for (int i = 0; i < pdba->nr; i++)
    {
...
static void calc_all_pos(t_atoms *pdba, rvec x[], gmx::ArrayRef < std::vector < Patch>> patches,
                         bool bCheckMissing)
{
    int      ii, l = 0;
#define MAXH 4
    ...
    for (int i = 0; i < pdba->nr; i++)
    {
             ...
             patch += patch->nr)
        {
            ...
            if (patch->type() == PatchType::Add && patch->tp > 0)
            {
                bool bFoundAll = true;
...
static int add_h_low(t_atoms **pdbaptr, rvec *xptr[],
                     gmx::ArrayRef<MoleculePatches> globalPatches,
                     int nterpairs,
                     std::vector<MoleculePatches *> ntdb,
                     std::vector<MoleculePatches *> ctdb,
                     ...
                     bool bUpdate_pdba, bool bKeep_old_pdba)
{
...

    {
        ...
        std::vector<MoleculePatches> hb =
            getMoleculePatchess(pdba, globalPatches, nterpairs, ntdb, ctdb, rN, rC);
    ...
    if (bUpdate_pdba)
    {
        /* we don't have to add atoms that are already present in pdba,
           so we will remove them from the patches (t_hack) */
    ...
    for (int i = 0; (i < natoms); i++)
    {
        ...
        if (patches[i].empty()  || (!patches[i][0].nname.empty()) )
        {
            ...
            for (auto patch = patches[i].begin();
                 ...
                 patch++)
            {
                if (patch->type() == PatchType::Add) /* add */
                {
                    newi++;
                    if (newi >= natoms+nadd)
                    {
                ...
                if (!patch->nname.empty() &&
                    (patch->oname.empty() ||
                     strcmp(patch->oname.c_str(), *newpdba->atomname[newi]) == 0))
                {
                    /* add or replace */
                    if (patch->type() == PatchType::Add && patch->bAlreadyPresent)
                    {
