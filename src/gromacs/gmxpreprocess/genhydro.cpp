               ...
               const int *rN, const int *rC)
{
    ...
    {
        const t_hackblock *ahptr = search_h_db(ah, *pdba->resinfo[rnr].rtp);
        if (ahptr)
        {
...
static void expand_hackblocks_one(const t_hackblock &hbr, char *atomname,
    ...
    for (int j = 0; j < hbr.nhack(); j++)
    {
                ...
                strcmp(atomname, hbr.hack[j].oname) == 0) ) )
        {
            ...
            for (int k = 0; k < hbr.hack[j].nr(); k++)
            {
                ...
                if (abi->back().nname == nullptr)
                {
                    if (abi->back().oname == nullptr)
                    {
                ...
                else
                {
                    if (gmx_debug_at)
                    {
                        fprintf(debug, "Hack '%s' %d, replacing nname '%s' with '%s' (old name '%s')\n",
                                ...
                                abi->back().oname ? abi->back().oname : "");
                    }
                    sfree(abi->back().nname);
                    abi->back().nname = gmx_strdup(hbr.hack[j].nname);
                }
                ...
                else if (hbr.hack[j].tp == 11 && k >= 2)
                {
                    ...
                    /* Ugly hardcoded name hack */
                    srenew(abi->back().nname, 4);
                    abi->back().nname[0] = 'L';
                    abi->back().nname[1] = 'P';
                    abi->back().nname[2] = '1' + k - 2;
                    abi->back().nname[3] = '\0';
                }
                else if (hbr.hack[j].nr() > 1)
                {
                    /* adding more than one atom, number them */
                    int l = strlen(abi->back().nname);
                    srenew(abi->back().nname, l+2);
                    abi->back().nname[l]   = '1' + k;
                    abi->back().nname[l+1] = '\0';
                }
            }
            ...
            if (hbr.hack[j].tp > 0 || hbr.hack[j].oname == nullptr)
            {
                for (int k = 0; k < hbr.hack[j].nr(); k++)
                {
                    expand_hackblocks_one(hbr, abi->at(oldSize + k).nname,
                                          abi, bN, bC);
                }
            }
        }
    ...
    for (int i = 0; i < pdba.nr; i++)
    {
        ...
        for (int j = 0; j < gmx::index(ab[i].size()); j++)
        {
            if (ab[i][j].oname == nullptr)
            {
                ...
                int k = pdbasearch_atom(ab[i][j].nname, rnr, pdba, "check", TRUE);
                if (k != -1)
                {
    ...
    for (int i = 0; i < pdba.nr; i++)
    {
        ...
        for (int j = 0; j < gmx::index(ab[i].size()); j += ab[i][j].nr())
        {
            ...
            if (ab[i][j].oname == nullptr && ab[i][j].tp > 0)
            {
                ...
                {
                    int ia = pdbasearch_atom(ab[i][j].a[m], rnr, pdba,
...
static void free_ab(int natoms, gmx::ArrayRef < std::vector < t_hack>> ab)
{
                     ...
                     bool bUpdate_pdba, bool bKeep_old_pdba)
{
    ...
    for (int i = 0; (i < natoms); i++)
    {
        ...
        if (ab[i].empty() || (ab[i][0].nname != nullptr) )
          ...
          bool bUpdate_pdba, bool bKeep_old_pdba)
{
    ...
    do
    {
        ...
        nnew = add_h_low(pdbaptr, xptr, ah, nterpairs, ntdb, ctdb, rN, rC, FALSE,
                         abptr, bUpdate_pdba, bKeep_old_pdba);
        niter++;
        if (niter > 100)
        {
