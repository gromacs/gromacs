...
static void expand_hackblocks_one(const MoleculePatchDatabase &newPatch,
                                  ...
                                  bool bN, bool bC)
{
    //! Copy needed because function is recursive.
    //NOLINTNEXTLINE(performance-unnecessary-copy-initialization)
    const std::string localName = atomName;
    ...
    for (auto &singlePatch : newPatch.hack)
    {
        ...
        if (!bIgnore &&
            ( ( ( singlePatch.tp > 0 || singlePatch.oname.empty() ) &&
                singlePatch.a[0] == localName ) ||
              ( singlePatch.oname == localName) ) )
        {
            ...
            for (int k = 0; k < singlePatch.nr; k++)
            {
                ...
                /* if we're adding (oname==NULL) and don't have a new name (nname)
                   yet, build it from localName */
                if (patch->nname.empty())
                {
                    if (patch->oname.empty())
                    {
                        patch->nname    = localName;
                        patch->nname[0] = 'H';
                    }
                }
                else
...
static int check_atoms_present(t_atoms *pdba, gmx::ArrayRef < std::vector < MoleculePatch>> patches)
{
    int nadd = 0;
    for (int i = 0; i < pdba->nr; i++)
    {
