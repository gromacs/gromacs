...
static void expand_hackblocks_one(const t_hackblock &hbr, const char *atomname,
    ...
    for (int j = 0; j < hbr.nhack(); j++)
    {
        ...
        if (!bIgnore &&
            ( ( ( hbr.hack[j].tp > 0 || hbr.hack[j].oname.empty() ) &&
              ...
              ( strcmp(atomname, hbr.hack[j].oname.c_str()) == 0) ) )
        {
            ...
            for (int k = 0; k < hbr.hack[j].nr(); k++)
            {
                ...
                if (abi->back().nname.empty())
                {
                    ...
                    {
                        abi->back().nname    = gmx_strdup(atomname);
                        abi->back().nname[0] = 'H';
                    }
                }
                else
            ...
            if (hbr.hack[j].tp > 0 || hbr.hack[j].oname.empty())
            {
                for (int k = 0; k < hbr.hack[j].nr(); k++)
                {
                    expand_hackblocks_one(hbr, abi->at(abi->size() - hbr.hack[j].nr() + k).nname.c_str(),
                                          abi, bN, bC);
                }
            }
        }
