              ...
              warninp *wi)
{
    ...
    forceParam.reserve(MAXFORCEPARAM);
                          ...
                          warninp                   *wi)
{
    ...
    for (int i = 0; (i < nr); i++)
    {
        ...
        if (equalEitherForwardOrBackward(bParams, testParams))
        {
            ...
            const bool identicalParameters = std::equal(bt->param[i].forceParam().begin(),
                                                        bt->param[i].forceParam().begin() + nrfp, b.forceParam().begin());
    ...
    if (addBondType)
    {
        ...
        bt->param.emplace_back(FFParameter(b.atoms(), b.forceParam(), b.name()));
        ...
        bt->param.emplace_back(FFParameter(atoms, forceParam, b.name()));
    }
}
                              ...
                              FFParameter *p, int c_start, bool bB, bool bGenPairs)
{
    ...
    if (!bFound)
    {
        auto foundParameter = std::find_if(bt[ftype].param.begin(), bt[ftype].param.end(),
                                           [&paramAtoms, &at, &bB](const auto &param)
                                           { return findIfAllNBAtomsMatch(param.atoms(), paramAtoms, at, bB); });
        if (foundParameter != bt[ftype].param.end())
        {
            bFound = true;
        }
    }
...
static int findNumberOfDihedralAtomMatches(const FFParameter            &currentParamFromParameterArray,
                                           const FFParameter            &parameterToAdd,
                                           const t_atoms                *at,
                                           const PreprocessingAtomTypes *atypes,
                                           bool                          bB)
{
                                         ...
                                         bool                          bB)
{
    ...
    else if (bB)
    {
        for (int i = 0; i < atomsFromCurrentParameter.ssize(); i++)
        {
            if (atypes->bondAtomParameterFromType(
                                 ...
                                 int *nparam_def)
{
    ...
    if (ftype == F_PDIHS || ftype == F_RBDIHS || ftype == F_IDIHS || ftype == F_PIDIHS)
    {
        ...
        while (pos != bt[ftype].param.end() && nmatch_max < 4)
        {
               ...
               warninp *wi)
{
    ...
    if (bBonded)
    {
        ...
        else if (interaction_function[ftype].nrfpA == 0)
        {
            bFoundA = true;
        }
        ...
        else if (interaction_function[ftype].nrfpB == 0)
        {
            bFoundB = true;
        }
    }
