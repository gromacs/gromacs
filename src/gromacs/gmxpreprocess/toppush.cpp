              ...
              warninp_t wi)
{
    ...
    else
    {
        ...
        warning_error_and_exit(wi, errbuf, FARGS);
        // This return statement isn't reachable in practice.  clang
        // static analyzer would like to know that
        // warning_error_and_exit has the gmx_noreturn attribute, so
        // that the use of c[i] in the next loop can't use undefined
        // values. However, compilers can't generate code for that
        // function that does not have a return statement, because
        // they can't deduce that gmx_fatal has always been called.
        // If we were using exceptions, none of this would be a thing
        // (but much cleaning up is needed first).
        return;
    }
    for (i = 0; (i < nrfp); i++)
    {
