...
void push_at (t_symtab *symtab, PreprocessingAtomTypes *at, PreprocessingBondAtomType *bat,
              ...
              warninp *wi)
{
    ...
    batype_nr = bat->addBondAtomType(symtab, btype);
             ...
             warninp                                  *wi)
{
    ...
    for (i = 0; (i < nral); i++)
    {
        ...
        else if ((bat != nullptr) && ((p.a[i] = bat->bondAtomTypeFromName(alc[i])) == NOTSET))
        {
                       ...
                       warninp *wi)
{
    ...
    for (i = 0; (i < 4); i++)
    {
        ...
        {
            if ((p.a[i] = bat->bondAtomTypeFromName(alc[i])) == NOTSET)
            {
              ...
              warninp                                  *wi)
{
    ...
    {
        if ((at != nullptr) && ((p.a[i] = bat->bondAtomTypeFromName(alc[i])) == NOTSET))
        {
            auto message = gmx::formatString("Unknown atomtype %s\n", alc[i]);
            warning_error(wi, message);
        }
        ...
        GMX_RELEASE_ASSERT(bat != nullptr, "Need valid PreprocessingBondAtomType object");
        bt[F_CMAP].cmapAtomTypes.emplace_back(bat->bondAtomTypeFromName(alc[i]));
    }
