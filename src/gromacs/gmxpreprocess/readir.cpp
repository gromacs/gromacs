                         ...
                         warninp_t wi)
{
    ...
    int i = 0;
    for (const auto &groupName : groupsFromMdpFile)
    {
        /* Lookup the group name in the block structure */
        gid = search_string(groupName.c_str(), block->nr, gnames);
              ...
              warninp_t wi)
{
    ...
    if (simulatedAnnealingGroupNames.size() == 1 &&
        !simulatedAnnealingGroupNames[0].empty() &&
        (simulatedAnnealingGroupNames[0][0] == 'n' ||
         simulatedAnnealingGroupNames[0][0] == 'N'))
    {
    ...
    else
    {
        ...
        if (!simulatedAnnealingGroupNames.empty())
        {
            ...
            for (i = 0; i < nr; i++)
            {
                if (!simulatedAnnealingGroupNames[i].empty() &&
                    (simulatedAnnealingGroupNames[i][0] == 'n' ||
                     simulatedAnnealingGroupNames[i][0] == 'N'))
                {
                    ir->opts.annealing[i] = eannNO;
                }
                else if (!simulatedAnnealingGroupNames[i].empty() &&
                         (simulatedAnnealingGroupNames[i][0] == 's' ||
                          simulatedAnnealingGroupNames[i][0] == 'S'))
                {
                    ...
                    bAnneal               = TRUE;
                         ...
                         (simulatedAnnealingGroupNames[i][0] == 'p' ||
                          simulatedAnnealingGroupNames[i][0] == 'P'))
                {
