            ...
            WriteMdpHeader writeMdpHeader, warninp_t wi)
{
    ...
    printStringNewline(&inp, "FITTING TO A DENSITY MAP");
    printStringNoNewline(&inp, "Add an extra potential resulting from a density map (yes/no)");
    ir->bDensityFitting = get_eeenum(&inp, "DensityFitting", yesno_names, wi);
    if (ir->bDensityFitting)
    {
        gmx::DensfitData parameters;
        parameters.read_densparams(&inp, wi);
