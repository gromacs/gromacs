...
class MdpErrorHandler : public gmx::IKeyValueTreeErrorHandler
{
    public:
        ...
        virtual bool onError(gmx::UserInputError *ex, const std::vector<std::string> &context)
        {
    ...
    private:
        std::string getOptionName(const std::vector<std::string> &context)
        {
            if (mapping_ != nullptr)
            {
                std::vector<std::string> path = mapping_->originalPath(context);
