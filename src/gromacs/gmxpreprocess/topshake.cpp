                ...
                if (interaction_function[ftype].flags & IF_BTYPE)
                {
                    ...
                    for (int ftype_a = 0; (gmx::ssize(*bonds) > 0 && ftype_a < F_NRE); ftype_a++)
                    {
                        if (interaction_function[ftype_a].flags & IF_ATYPE)
                        {
                            ...
                            for (auto parm = pr->param.begin(); parm != pr->param.end(); )
                            {
                                const FFParameter *ang = &(*parm);
#ifdef DEBUG
                                    ...
                                    (numhydrogens == 1 && toupper(**(info[ang->aj()])) == 'O'))
                                {
                                    ...
                                    if (bFound)
                                    {
                                        std::vector<real> forceParm;
                                        /* apply law of cosines */
                                        forceParm.emplace_back(std::sqrt( b_ij*b_ij + b_jk*b_jk -
                                                                          2.0*b_ij*b_jk*cos(DEG2RAD*ang->c0()) ));
                                        forceParm.emplace_back(forceParm.back());
#ifdef DEBUG
                                        printf("p: %d, q: %d, dist: %12.5e\n", atomNumbers[0],
                                               atomNumbers[1], forceParm[0]);
#endif
                                        add_param_to_list (&(plist[F_CONSTR]), FFParameter(atomNumbers, forceParm, ""));
            ...
            if (interaction_function[ftype].flags & IF_BTYPE)
            {
                ...
                for (auto parm = pr->param.begin(); parm != pr->param.end(); )
                {
                         ...
                         (count_hydrogens (info, 2, parm->atoms()) > 0) )
                    {
                        ...
                        std::vector<real> forceParm   = { parm->c0(), NOTSET, parm->c2()};
                        add_param_to_list (&(plist[F_CONSTR]), FFParameter(atomNumbers, forceParm, ""));
                        parm = pr->param.erase(parm);
                    }
                    else
                    {
                        ++parm;
