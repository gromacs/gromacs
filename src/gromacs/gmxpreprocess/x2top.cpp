...
static gpp_atomtype *set_atom_type(t_symtab *tab, t_atoms *atoms, SystemParameters *bonds,
...
static void lo_set_force_const(SystemParameters *plist, real c[], int nrfp, bool bRound,
...
static void set_force_const(gmx::ArrayRef<SystemParameters> plist, real kb, real kt, real kp, bool bRound,
                            bool bParam)
{
...
static void calc_angles_dihs(SystemParameters *ang, SystemParameters *dih, const rvec x[], bool bPBC,
                             matrix box)
{
    int    t1, t2, t3;
...
static void print_pl(FILE *fp, gmx::ArrayRef<const SystemParameters> plist, int ftp, const char *name,
                     char ***atomname)
{
    if (plist[ftp].nr > 0)
    {
...
static void print_rtp(const char *filenm, const char *title, t_atoms *atoms,
                      gmx::ArrayRef<const SystemParameters> plist, gpp_atomtype *atype, int cgnr[])
{
    FILE *fp;
    int   i, tp;
    char *tpnm;
