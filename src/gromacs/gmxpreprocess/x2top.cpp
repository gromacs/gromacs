    ...
    for (i = 0; (i < atoms->nr); i++)
    {
        ...
        for (j = i+1; (j < atoms->nr); j++)
        {
            ...
            if (is_bond(nnm, nmt, *atoms->atomname[i], *atoms->atomname[j],
                        std::sqrt(dx2)))
            {
                add_param_to_list (bond, FFParameter({{i, j}}, {{std::sqrt(dx2)}}, ""));
                nbond[i]++;
                nbond[j]++;
            }
        }
                               ...
                               bool bDih, bool bParam)
{
    ...
    for (auto &param : plist->param)
    {
        ...
        std::vector<real> forceParam(2*nrfp);
        for (int j = 0; (j < nrfp); j++)
        {
            forceParam[j]      = c[j];
            forceParam[nrfp+j] = c[j];
        }
        param = FFParameter(param.atoms(), forceParam, "");
    }
}
                             ...
                             matrix box)
{
    ...
    for (auto &angle : ang->param)
    {
        ...
        angle = FFParameter(angle.atoms(), {{th}}, angle.name());
    }
    for (auto dihedral : dih->param)
    {
        int  ai = dihedral.ai();
        int  aj = dihedral.aj();
        int  ak = dihedral.ak();
        ...
        dihedral = FFParameter(dihedral.atoms(), {{ph}}, dihedral.name());
    }
}
...
static void print_pl(FILE *fp, gmx::ArrayRef<const InteractionTypeParameters> plist, int ftp, const char *name,
                     char ***atomname)
{
    if (!plist[ftp].param.empty())
    {
        fprintf(fp, "\n");
        fprintf(fp, "[ %s ]\n", name);
        int nrfp = interaction_function[ftp].nrfpA;
        for (const auto &param : plist[ftp].param)
        {
            ...
            for (int j = 0; (j < nrfp) && (j < forceParam.ssize()); j++)
            {
                      ...
                      int                                             cgnr[])
{
    ...
    for (i = 0; (i < atoms->nr); i++)
    {
        ...
        if ((tpnm = atypes->atomNameFromType(tp)) == nullptr)
        {
