                       ...
                       gmx_atomprop_t aps)
{
    ...
    const char  *topinout;
    int          line;
    gmx_bool     bSystem, bMolecules;
    int          i, lastSolventResidueIndex, nsol = 0;
    double       mtot;
    real         vol, mm;

    lastSolventResidueIndex  = atoms->nres -1;
    nsol                     =  1 + lastSolventResidueIndex - firstSolventResidueIndex;
    ...
    if (ftp2bSet(efTOP, NFILE, fnm) )
    {
        ...
        if (nsol > 0)
        {
            char * currRes  = *atoms->resinfo[firstSolventResidueIndex].name;
            int    resCount = 0;

            // Iterate through solvent molecules and increment a count until new resname found
            for (int i = firstSolventResidueIndex; i <= lastSolventResidueIndex; i++)
            {
                if (strcmp(*(atoms->resinfo[i].name), currRes) == 0)
                {
                    resCount += 1;
                }
                else
                {
                    ...
                    fprintf(stdout, "Adding line for %d solvent molecules with resname (%s) to "
                            "topology file (%s)\n", resCount, currRes, topinout);
                    fprintf(fpout, "%-15s %5d\n", currRes, resCount);
                    strcpy(currRes, *atoms->resinfo[i].name);
                    resCount = 1;
                }
            }
            // One more print needed for last residue
            fprintf(stdout, "Adding line for %d solvent molecules with resname (%s) to "
                    "topology file (%s)\n", resCount, currRes, topinout);
            fprintf(fpout, "%-15s %5d\n", currRes, resCount);
        }
