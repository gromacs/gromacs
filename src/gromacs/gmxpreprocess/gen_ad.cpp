...
static int get_impropers(t_atoms *atoms, gmx::ArrayRef<SystemModificationInstructions> amb, t_param **improper,
                         bool bAllowMissing)
{
...
static void gen_excls(t_atoms *atoms, t_excls *excls, gmx::ArrayRef<SystemModificationInstructions> amb,
                      bool bAllowMissing)
{
    int astart = 0;
...
void gen_pad(t_nextnb *nnb, t_atoms *atoms, t_restp rtp[],
             t_params plist[], t_excls excls[], gmx::ArrayRef<SystemModificationInstructions> amb,
             bool bAllowMissing)
{
    ...
    if (!amb.empty())
    {
        gen_excls(atoms, excls, amb, bAllowMissing);
    ...
    nimproper = get_impropers(atoms, amb, &improper, bAllowMissing);
