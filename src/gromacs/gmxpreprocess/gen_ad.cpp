...
static bool was_dihedral_set_in_rtp(const FFParameter &dih)
{
    ...
    return dih.forceParam().back() == DIHEDRAL_WAS_SET_IN_RTP;
}
...
static int dcomp(const FFParameter &d1, const FFParameter &d2)
{
    ...
    else
    {
        ...
        return std::lexicographical_compare(d1.name().begin(), d1.name().end(),
                                            d2.name().begin(), d2.name().end());
    }
}
...
static int n_hydro(gmx::ArrayRef<const int> a, char ***atomname)
{
    ...
    {
        GMX_RELEASE_ASSERT(atom < a.end(), "Can not go beyond end of vector");
    ...
    if (!globalPatches.empty())
    {
        for (int i = 0; (i < atoms->nres); i++)
        {
            ...
            for (const auto &bondeds : impropers->b)
            {
                ...
                {
                    /* Not broken out */
                    improper.emplace_back(FFParameter(ai, {}, bondeds.s.c_str()));
                }
            }
            ...
            for (int k = 0; (k < nnb->nrexcl[j1][1]); k++)
            {
                ...
                if (k1 != i)
                {
                    ...
                    if (i < k1)
                    {
                        ...
                        ang.push_back(FFParameter(atomNumbers, {}, name.c_str()));
                    }
                            ...
                            if ((l1 != i) && (l1 != j1))
                            {
                                    ...
                                    do
                                    {
                                        ...
                                        for (auto &bondeds : hbdih->b)
                                        {
                                            ...
                                            if (bFound)
                                            {
                                                 ...
                                                 */
                                                nFound++;
                                                std::vector<real> forceParam = {DIHEDRAL_WAS_SET_IN_RTP};
                                                dih.push_back(FFParameter(atomNumbers, forceParam, name.c_str()));
                                                /* Set the next direct in case the rtp contains
                                                   multiple entries for this dihedral.
                                                 */
                                                dih.push_back(FFParameter({{ i, j1, k1, l1}}, {}, ""));
                                            }
                                        }
                                    }
                                    while (res < maxres);
                                ...
                                if (nbd == 3)
                                {
                                    ...
                                    if (!bExcl)
                                    {
                                        ...
                                        {
                                            pai.push_back(FFParameter({{i1, i2}}, {}, ""));
                                        }
                                    }
                                }
                            }
        ...
        for (int i = 0; i < atoms->nres; i++)
        {
            ...
            for (auto &bondeds : hbang->b)
            {
                ...
                if (bFound)
                {
                    ...
                    ang.push_back(FFParameter(atomNumbers, {}, bondeds.s.c_str()));
                }
            }
            ...
            for (auto &bondeds : hbdih->b)
            {
                ...
                if (bFound)
                {
                    ...
                    dih.push_back(FFParameter(atomNumbers, {}, bondeds.s.c_str()));
                }
            }
        }
    }
