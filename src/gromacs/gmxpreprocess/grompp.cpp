 ...
 */
static gmx_bool haveDecoupledModeInMol(const gmx_moltype_t *molt,
                                       ...
                                       real                 massFactorThreshold)
{
    ...
    gmx_bool           haveDecoupledMode = false;
    for (int ftype = 0; ftype < F_NRE; ftype++)
    {
        if (interaction_function[ftype].flags & IF_ATYPE)
        {
            ...
            for (int i = 0; i < il->nr; i += 1 + nral)
            {
                    ...
                    atomToConstraints.index[a1 + 1] - atomToConstraints.index[a1] >= 3)
                {
                    ...
                    gmx_bool foundAtom0  = false;
                    gmx_bool foundAtom2  = false;
                                       ...
                                       warninp_t         wi)
{
    ...
    gmx_bool       lincsWithSufficientTolerance = (ir->eConstrAlg == econtLINCS && ir->nLincsIter >= lincsIterationThreshold && ir->nProjOrder >= lincsOrderThreshold);
    gmx_bool       shakeWithSufficientTolerance = (ir->eConstrAlg == econtSHAKE && ir->shake_tol <= 1.1*shakeToleranceThreshold);
