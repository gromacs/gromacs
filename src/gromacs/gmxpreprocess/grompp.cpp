...
#define NFILE asize(fnm)
    ...
    {
        GMX_RELEASE_ASSERT(sys.atomtypes.zeta[i] >= 0, "The distribution constants cannot be negative.");
        if (sys.atomtypes.zeta[i] > 0)
        {
            usingGaussianCharges = true;
            GMX_RELEASE_ASSERT(sys.atomtypes.zeta[i] > ewaldcoeff_q_sq2, "The distributed charges\n"
                               "are not sufficiently screened at the cut-off distance.\n"
