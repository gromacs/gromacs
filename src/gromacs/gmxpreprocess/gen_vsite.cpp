                         ...
                         const char atom1[], const char atom2[])
{
    ...
    while (j < vsitetop[i].nbonds &&
           ( strcmp(atom1, vsitetop[i].bond[j].atom1) != 0 || strcmp(atom2, vsitetop[i].bond[j].atom2) != 0) &&
           ( strcmp(atom2, vsitetop[i].bond[j].atom1) != 0 || strcmp(atom1, vsitetop[i].bond[j].atom2) != 0))
    {
        j++;
    }
                          ...
                          const char atom2[], const char atom3[])
{
    ...
    while (j < vsitetop[i].nangles &&
           ( strcmp(atom1, vsitetop[i].angle[j].atom1) != 0 ||
             strcmp(atom2, vsitetop[i].angle[j].atom2) != 0 ||
             strcmp(atom3, vsitetop[i].angle[j].atom3) != 0) &&
           ( strcmp(atom3, vsitetop[i].angle[j].atom1) != 0 ||
             strcmp(atom2, vsitetop[i].angle[j].atom2) != 0 ||
             strcmp(atom1, vsitetop[i].angle[j].atom3) != 0))
    {
        j++;
    }
...
static void count_bonds(int atom, t_params *psb, gmx::ArrayRef<const AtomInfo> atoms,
                     ...
                     gmx::ArrayRef<const ResidueType> rt)
{
    ...
    {
        /* get type from rtp */
        rtpp   = get_restp(*(ri[at[atom].resind_].name_), nrtp, rtp);
        bNterm = isResidueTypeProtein(rt, *(ri[at[atom].resind_].name_)) &&
            (at[atom].resind_ == 0);
        j    = search_jtype(rtpp, *(at[atom].atomname), bNterm);
        type = rtpp->atom[j].type_;
    }
    return type;
}
...
static int gen_vsites_6ring(gmx::ArrayRef<AtomInfo> at, int *vsite_type[], t_params plist[],
...
static int gen_vsites_phe(gmx::ArrayRef<AtomInfo> at, int *vsite_type[], t_params plist[],
...
static int gen_vsites_trp(gpp_atomtype_t atype, rvec *newx[],
                          ...
                          gmx::ArrayRef<AtomInfo> at, int *vsite_type[], t_params plist[],
...
#define NMASS 2
    ...
    *nadd += NMASS;
    for (j = i0; j < at.size(); j++)
    {
        (*o2n)[j] = j+*nadd;
    }
    srenew(*newx, at.size()+*nadd);
...
static int gen_vsites_his(gmx::ArrayRef<AtomInfo> at, int *vsite_type[], t_params plist[],
                          int nrfound, int *ats, t_vsitetop *vsitetop, int nvsitetop)
{
    int  nvsite, i;
...
void do_vsites(int nrtp, t_restp rtp[], gpp_atomtype_t atype,
               AtomResiduePdb *system, t_symtab *symtab, rvec *x[],
...
#define MAXATOMSPERRESIDUE 16
    ...
    for (i = 0; (i < natoms); i++)
    {
        ...
        if (bVsiteAromatics &&
            ...
            isResidueTypeProtein(rt, *(system->resinfo[resind].name_)) )
        {
            /* mark this residue */
            ...
            switch (whatres)
            {
                case resPHE:
                    ...
                    nvsite += gen_vsites_phe(system->atoms, vsite_type, plist, nrfound, ats, vsitetop, nvsitetop);
                    break;
                case resTRP:
                    if (debug)
                    {
                    ...
                    nvsite += gen_vsites_trp(atype, &newx, &newatom, &o2n,
                                             ...
                                             system->atoms, vsite_type, plist, nrfound, ats, add_shift, vsitetop, nvsitetop);
                    break;
                case resTYR:
                    if (debug)
                    {
                ...
                case resHIS:
                    ...
                    nvsite += gen_vsites_his(system->atoms, vsite_type, plist, nrfound, ats, vsitetop, nvsitetop);
                    break;
                case NOTSET:
        ...
        if ( ((*vsite_type)[i] == NOTSET) && is_hydrogen(*(system->atoms[i].atomname)))
        {
            ...
            /* get Heavy atom type */
            tpHeavy = get_atype(Heavy, system->atoms, system->resinfo, nrtp, rtp, rt);
...
#define NMASS 2
                    ...
                    for (j = 0; j < nrHatoms; j++)
                    {
                        mHtot                += get_amass(Hatoms[j], system->atoms, system->resinfo, nrtp, rtp, rt);
                        system->atoms[Hatoms[j]].m_ = system->atoms[Hatoms[j]].mB_ = 0;
                    }
                    mtot              = mHtot + get_amass(Heavy, system->atoms, system->resinfo, nrtp, rtp, rt);
                    ...
                    for (j = 0; (j < NMASS); j++)
                    {
                        ...
                        name[k+2]              = '\0';
                        newatom[ni0+j].atomname     = put_symtab(symtab, name);
