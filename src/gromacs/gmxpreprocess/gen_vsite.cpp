...
{
    VirtualSiteConfiguration(std::string type, bool planar,
                             int nhyd, std::string nextheavy, std::string dummy)
        : atomtype(std::move(type)), isplanar(planar), nhydrogens(nhyd), nextheavytype(std::move(nextheavy)),
          dummymass(std::move(dummy))
    {}
...
{
    VirtualSiteTopology(std::string name) : resname(std::move(name))
    {}
    //! Residue name.
    std::string resname;
    ...
    {
        VirtualSiteBond(std::string a1, std::string a2, real v) :
            atom1(std::move(a1)), atom2(std::move(a2)), value(v)
        {}
        //! Atom 1 in bond.
        std::string atom1;
        //! Atom 2 in bond.
    ...
    {
        VirtualSiteAngle(std::string a1, std::string a2, std::string a3, real v) :
            atom1(std::move(a1)), atom2(std::move(a2)), atom3(std::move(a3)), value(v)
        {}
        //! Atom 1 in angle.
        std::string atom1;
        //! Atom 2 in angle.
                                ...
                                std::vector<VirtualSiteTopology>      *vsitetoplist)
{
    ...
    while (fgets2(pline, STRLEN-2, ddb.get()) != nullptr)
    {
        ...
        if (strlen(pline) > 0)
        {
            ...
            {
                auto found = std::find_if(vsitetoplist->begin(), vsitetoplist->end(),
                ...
                switch (curdir)
                {
                    ...
                    case DDB_NH2:
                        ...
                        s2String = s2;
                        s3String = s3;
                        if (n < 3 && gmx::equalCaseInsensitive(s2String, "planar"))
                        {
                              ...
                              const std::string                            &atomtype)
{
     ...
     */
    int      res;
    auto     found = std::find_if(vsiteconflist.begin(), vsiteconflist.end(),
...
{
    auto found = std::find_if(vsitetop.begin(), vsitetop.end(),
    ...
    auto foundBond = std::find_if(found->bond.begin(), found->bond.end(),
                          ...
                          const std::string                       &atom3)
{
    ...
    auto foundAngle = std::find_if(found->angle.begin(), found->angle.end(),
                                   ...
                                   { return ((atom1 == entry.atom1 && atom2 == entry.atom2 && atom3 == entry.atom3) ||
                                             ...
                                             (atom1 == entry.atom3 && atom2 == entry.atom3 && atom3 == entry.atom1)); });
