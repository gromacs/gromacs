...
int pdb2gmx::run()
{
    ...
    pdb_ch_[numChains_-1].natom = natom_-pdb_ch_[numChains_-1].start;
    ...
    for (int chain = 0; (chain < numChains_); chain++)
    {
        ...
        else
        {
            ...
            if (this_chainid_ != ' ')
            {
                suffix_.append(gmx::formatString("_chain_%c", this_chainid_));
            }
            ...
            if (nid_used_ > 0)
            {
                suffix_.append(gmx::formatString("%d", nid_used_+1));
            }

            if (suffix_.length() > 0)
            {
                molname_.append(p_restype_);
                molname_.append(suffix_);
            }
            else
            {
                molname_ = p_restype_;
        ...
        {
            bITP_   = true;
            itp_fn_ = topologyFile_;
            printf("Chain time...\n");
            //construct the itp file name
            gmx::stripSuffixIfPresent(itp_fn_, ".top");
            itp_fn_.append(molname_);
            itp_fn_.append(".itp");
            //now do the same for posre
            gmx::stripSuffixIfPresent(posre_fn, ".itp");
            posre_fn.append(molname_);
            posre_fn.append(".itp");
            if (posre_fn.compare(itp_fn_) == 0)
            {
            ...
            incls_[nincl_-1] = gmx_strdup(itp_fn_.c_str());
            itp_file_        = gmx_fio_fopen(itp_fn_.c_str(), "w");
        }
        else
        {
            bITP_ = false;
        ...
        {
            mols_[nmol_].name = gmx_strdup(molname_.c_str());
            mols_[nmol_].nr   = 1;
        }
        nmol_++;
        ...
        if (bITP_)
        {
            print_top_comment(itp_file_, itp_fn_.c_str(), ffdir_, TRUE);
        }
        ...
        pdb2top(top_file2_, posre_fn.c_str(), molname_.c_str(), pdba_, &x_, atype_, &symtab_,
                nrtp_, restp_,
    ...
    else
    {
        ...
        if (!fflib_fexist(waterFile.c_str()))
        {
