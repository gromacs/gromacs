...
int pdb2gmx::run()
{
    ...
    {
        rc_ = gmx_residuetype_get_type(rt_, rtprename[i].gmx, &p_restype_);

        /* Only add names if the 'standard' gromacs/iupac base name was found */
        if (rc_ == 0)
        {
            gmx_residuetype_add(rt_, rtprename[i].main, p_restype_);
            gmx_residuetype_add(rt_, rtprename[i].nter, p_restype_);
            gmx_residuetype_add(rt_, rtprename[i].cter, p_restype_);
            gmx_residuetype_add(rt_, rtprename[i].bter, p_restype_);
        }
    }
    ...
    if (bNewRTP_)
    {
        ...
        print_resall(fp_, nrtp, restp, atype);
        gmx_fio_fclose(fp_);
    }
    ...
    for (int chain = 0; (chain < numChains_); chain++)
    {
        ...
        pdb2top(top_file2_, posre_fn.c_str(), molname_.c_str(), pdba_, &x_, atype, &symtab_,
                nrtp, restp,
