...
#define SAC(s) (s) ? "+" : ""
    ...
    int pos = 0;
    for (const auto &entry : amb)
    {
        fprintf(out, "%3d %4s %2d\n",
                pos, SS(entry.name), entry.nhack);
        if (entry.nhack)
        {
            for (int j = 0; j < entry.nhack; j++)
            {
                fprintf(out, "%d: %d %4s %4s %1s %2d %d %4s %4s %4s %4s\n",
                        j, entry.hack[j].nr,
                        SSC(entry.hack[j].oname), SSC(entry.hack[j].nname),
                        SAC(entry.hack[j].atom), entry.hack[j].tp, entry.hack[j].cgnr,
                        SSC(entry.hack[j].ai()), SSC(entry.hack[j].aj()),
                        SSC(entry.hack[j].ak()), SSC(entry.hack[j].al()) );
            }
        }
        for (int j = 0; j < ebtsNR; j++)
        {
            if (entry.rb[j].nb)
            {
                fprintf(out, " %c %d:", btsNames[j][0], entry.rb[j].nb);
                int k;
                for (k = 0; k < entry.rb[j].nb; k++)
                {
                    fprintf(out, " [");
                    for (int l = 0; l < btsNiatoms[j]; l++)
                    {
                        fprintf(out, " %s", entry.rb[j].b[k].a[l]);
                    }
                    fprintf(out, " %s]", SSC(entry.rb[j].b[k].s));
                }
                fprintf(out, " Entry matched: %s\n", yesno_names[entry.rb[j].b[k].match]);
            }
        }
        fprintf(out, "\n");
    }
