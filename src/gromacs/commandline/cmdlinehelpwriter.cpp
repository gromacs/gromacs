      ...
      title_(title), header_(NULL), bDidOutput_(false)
{
    if (context.outputFormat() == eHelpOutputFormat_Console)
    {
        ...
        consoleFormatter_->setFoldLastColumnToNextLine(10);
        consoleFormatter_->addColumn(NULL, 7, false);
        consoleFormatter_->addColumn(NULL, 18, false);
        consoleFormatter_->addColumn(NULL, 16, false);
        consoleFormatter_->addColumn(NULL, 28, false);
        consoleFormatter_->addColumn(NULL, 1, true);
    }
}
...
void OptionsListFormatter::formatOption(const OptionInfo &option)
{
    ...
    if (context_.outputFormat() == eHelpOutputFormat_Console)
    {
        ...
        if (!info.empty())
        {
            consoleFormatter_->addColumnLine(3, "(" + info + ")");
        }
        consoleFormatter_->addColumnHelpTextBlock(4, context_, description);
...
void CommandLineHelpWriter::writeHelp(const CommandLineHelpContext &context)
{
    ...
    formatter.startSection("Options to specify input files:[PAR]");
    filter.formatSelected(OptionsFilter::eSelectInputFileOptions,
                          &formatter, impl_->options_);
    formatter.finishSection();
    formatter.startSection("Options to specify input/output files:[PAR]");
    filter.formatSelected(OptionsFilter::eSelectInputOutputFileOptions,
                          &formatter, impl_->options_);
    formatter.finishSection();
    formatter.startSection("Options to specify output files:[PAR]");
    filter.formatSelected(OptionsFilter::eSelectOutputFileOptions,
                          &formatter, impl_->options_);
    formatter.finishSection();
