...
void CommandLineParser::parse(int *argc, char *argv[])
{
    ...
    for (int i = 1; i != *argc; ++i)
    {
        ...
        // Remove recognized options if applicable or complain about the first
        // argument being an unknown option
        if (!bInOption)
        {
            if (impl_->bSkipUnknown_)
            {
            ...
            // An exception is the help functionality, which expects a command.
            else if (i == 1 && strncmp(argv[0], "help", 4) != 0)
            {
                std::string message =
                    "Expected a command-line option after '" + std::string(argv[0]) + "' (starting with '-'), not '" + std::string(arg) + "'";
                GMX_THROW(InvalidInputError(message));
            }
        }
    }
