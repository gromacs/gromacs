...
class ElectricField : public IInputRecExtension, public IForceProvider
{
    ...
    private:
        /*! \brief Extract relevant fields from an old-style mdp file
         *
         ...
         *
         * \param[in] dim    The direction XX, YY, or ZZ
         * \param[in] Field  Static components of the field
         * \param[in] wi     Warning control structure
         */
...
{
    // New style MDP options
    char        field_x[STRLEN], field_y[STRLEN], field_z[STRLEN];
    field_x[0]  = '\0';
    field_y[0]  = '\0';
    field_z[0]  = '\0';

    // Old style MDP. Will be removed soon.
    ...
    STYPE ("ElectricField-X", field_x,    NULL);
    STYPE ("ElectricField-Y", field_y,    NULL);
    STYPE ("ElectricField-Z", field_z,    NULL);
    ...
    *inp_p  = inp;
    ...
    // Extract old style
    decodeMdpOld(XX, efield_x, efield_xt, wi);
    decodeMdpOld(YY, efield_y, efield_yt, wi);
    decodeMdpOld(ZZ, efield_z, efield_zt, wi);
    // Extract new style
    decodeMdp(XX, field_x, wi);
    decodeMdp(YY, field_y, wi);
    decodeMdp(ZZ, field_z, wi);
}

void ElectricField::decodeMdpOld(int         dim,
                                 const char *staticField,
                                 const char *dynamicField,
                                 warninp_t   wi)
{
    ...
    if (!sxt.empty())
    {
        ...
        if (sxt.size() < 2)
        {
            sprintf(warn_buf, "Missing electric field time-dependent parameters");
            warning_error(wi, warn_buf);
        }
        if (sxt.size() > 4)
        {
            sprintf(warn_buf, "Too many electric field time-dependent parameters");
            warning_error(wi, warn_buf);
        }
        real omega = fromString<real>(sxt[1]);
        real t0    = 0;
        real sigma = 0;
        if (sxt.size() >= 3)
        {
            t0    = fromString<real>(sxt[2]);
        }
        if (sxt.size() >= 4)
        {
            sigma = fromString<real>(sxt[3]);
        }
                              ...
                              warninp_t   wi)
{
    // TODO: Better context for the exceptions from the converters (possibly
    // also convert them to warning_errors).
    std::vector<std::string> sx  = splitString(field);
    if (sx.size() == 4)
    {
        // New style input
        real E0    = fromString<real>(sx[0]);
        real omega = fromString<real>(sx[1]);
        real t0    = fromString<real>(sx[2]);
        real sigma = fromString<real>(sx[3]);
        setFieldTerm(dim, E0, omega, t0, sigma);
    }
    else if (sx.size() > 0)
    {
        char warn_buf[STRLEN];
        sprintf(warn_buf, "Need exactly four terms for electric field: E0, omega, t0, sigma");
        warning_error(wi, warn_buf);
