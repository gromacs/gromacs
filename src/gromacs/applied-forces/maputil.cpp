...
// #define WITHCCP4
#ifdef WITHCCP4
// #include "/home/ckutzne/ccp4/ccp4-6.3.0/include/ccp4/cmaplib.h"
using namespace CMap_io;

#else
typedef struct _CMMFile CMMFile;  //!< Dummy typedef for compilation without CCP4 library
#define FLOAT32 0;                //!< ditto
#endif
...
// Doxygen should ignore the following struct, which will be obsolete anyway soon
//! \cond
//! \brief This data structure holds all the data extracted from a ccp4 cmap datafile
...
static void updateParametersThatAreTimeDependent(real time, t_densfit *densfit)
{
    ...
    srenew(df->tmpgrid, df->vox_per_atom * ompthreads);  /* TODO: don't call srenew every time step! */
    srenew(df->tmpgrid2, df->vox_per_atom * ompthreads);
}
...
extern void map_print_header(gmx_unused FILE *fpout, gmx_unused const char *fn)
{
#ifdef WITHCCP4
    int          i;
    char        *title;
    ...
    CMMFile *mfile = static_cast<CMMFile *>(ccp4_cmap_open(fn, O_RDONLY));
    ...
    fprintf(fpout, "Axes order : %d %d %d\n", axes_order[0], axes_order[1], axes_order[2]);
    fprintf(fpout, "Map_dim    : %d %d %d\n", map_dim[0], map_dim[1], map_dim[2]);
    fprintf(fpout, "Spacegroup : %d\n", spacegroup);
    fprintf(fpout, "Min        : %g\n", min);
    fprintf(fpout, "Max        : %g\n", max);
    fprintf(fpout, "Mean       : %g\n", mean);
    fprintf(fpout, "RMS        : %g\n", rms);
    fprintf(fpout, "Skew_trans : %g %g %g\n", skew_trans[0], skew_trans[1], skew_trans[2]);
    fprintf(fpout, "Skew_mat   : "); for (i = 0; i < 9; i++)
    {
...
extern void do_map_ccp4(gmx_bool bRead, gmx_unused t_mapdata **ptr_map, const char *fn,
                        gmx_unused gmx_bool bOverwrite, gmx_bool bVerbose, FILE *DumpHeader)
{
#ifdef WITHCCP4
    CMMFile   *fp = NULL;
    int        n;
    ...
    if (NULL == fn)
    {
        gmx_fatal(FARGS, "%sdo_map_ccp4: no filename given, cannot %s map!\n", FitStr, bRead ? "read" : "write");
    }

    if (bRead && !gmx_fexist(fn) )
    {
        gmx_fatal(FARGS, "Could not find ccp4 input file '%s' with electron density map\n", fn);
    }

    if (bVerbose)
    {
        fprintf(stderr, "%sOpening ccp4 cmap file '%s' for %s.\n", FitStr, fn, bRead ? "reading" : "writing");
    }
    ...
    fp = static_cast<CMMFile *> (ccp4_cmap_open(fn, bRead ? O_RDONLY : O_WRONLY));

    if (FLOAT32 != ccp4_cmap_get_datamode(fp))
    {
        gmx_fatal(FARGS, "The only supported ccp4 file data mode is FLOAT32.\n");
    }
    ...
    if (bRead)
    {
        gmx_fatal(FARGS, "Cannot read CCP4 map: Not compiled with CCP4 support!\n");
    }
    else
    {
        fprintf(stderr, "WARNING: Cannot write CCP4 map: Not compiled with CCP4 support!\n");
    }
#endif

    if (DumpHeader)
    {
        map_print_header(DumpHeader, fn);
    }
}
...
//! \brief As exp, but for n elements
...
//            int sum = elements[XX] + elements[YY] + elements[ZZ];
            ...
            aarr[ZZ] = &df->tmpgrid[0 + elements[XX] + elements[YY] + th*df->vox_per_atom];
...
extern real calc_correlation_coeff(t_densfit *densfit, FILE *log)
{
    ...
    denominator = sqrt(calc_sum_rhoA_rhoB(map_exp, map_exp) * calc_sum_rhoA_rhoB(map_sim, map_sim));
...
#pragma omp parallel for num_threads(nth) schedule(static) \
    ...
    for (l = 0; l < densfit->nat_loc; l++)
    {
        ...
        for (image = 0; image < num_images; image++)
        {
            ...
            garr[ZZ] = &df->tmpgrid2[0 + elements[XX] + elements[YY] + th*df->vox_per_atom];
...
extern void finish_density_fitting(t_densfit *densfit)
{
    gmx_densfit *df = densfit->df;
...
extern real add_densfit_forces(t_inputrec *ir, rvec *f, t_commrec *cr, gmx_int64_t step, real time)
{
    int          ii, l;
        ...
        gmx_wallcycle                 *wcycle)
{
    const char     *fn       = NULL;
    t_densfit      *densfit  = NULL;
    densfit_t       df       = NULL; /* Pointer to the density fitting buffer variables */
...
//        put_atoms_in_box_omp(ir->ePBC, box, densfit->nat, df->x_assembled);
        fprintf(stderr, "Can not call put_atoms_in_box yet\n");
    }
...
//    get_map_spacing(densfit, TRUE); TODO
    ...
    if (MASTER(cr))
    {
        if (bOutputMap)
        {
            ...
            do_map_ccp4(FALSE, &densfit->map_sim, fn, TRUE, df->bVerbose, FALSE);
            do_map_ccp4(FALSE, &densfit->map_ref, "map_ref", TRUE, df->bVerbose, FALSE);
        }
