...
//! isotopes
enum class Isotope {
    H, D, He, Li, Be, B, C, N, O, F, Ne, Na, Mg, Al, Si, P,
    S, Cl, Ar, K, Ca, Mn, Fe, Cu, Zn, Se, Br, Kr, Rb, I, Xe,
    maxIsotope
};
...
//! convert istope string to enum
Isotope getIsotope(const std::string &isotope);
...
class ComputeScattering
{
    public:
        ...
        //! computes scattering intensity at zero scattering angle; for normalization
        double computeS0MC(Selection sel, float coverage, int seed);

        //! computes the scattering intensity for a given scattering angle q
        double computeIntensity(t_pbc *pbc, double q, Selection sel, float modulation);

        //! computes the scattering intensity for a given scattering angle q using Monte Carlo method
        double computeIntensityMC(t_pbc *pbc, double q, Selection sel, float coverage, int seed, float modulation);
...
class XrayInfo : public ComputeScattering
{
    public:
        //! constructor
        XrayInfo(std::vector<Isotope> isotopes, std::vector<double> qList, bool bSolvent);
...
class NeutronInfo : public ComputeScattering
{
    public:
        //! constructor
        NeutronInfo(const std::vector<Isotope> &isotopes);
