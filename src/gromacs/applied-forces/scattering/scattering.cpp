...
void readXrayScatteringFactors(std::vector<xrayStructureFactor> &xrayScatteringFactors_)
{
    ...
    while (get_a_line(scatteringFile, line, 1000))
    {
                   ...
                   &b1, &b2, &b3, &b4, &c) == 11)
        {
            ...
            xrayScatteringFactors_.push_back(xsf);
        }
    }
    fclose(scatteringFile);
};

double XrayInfo::computeS0(Selection sel)
{
...
double NeutronInfo::computeS0(Selection sel)
{
    //significant adding errors if Debeye is not a double
    double    DebeyeS0 = 0;
    const int posCount = sel.posCount();
...
double XrayInfo::getFormFactor(int i, int j, double q)
{
...
double NeutronInfo::getFormFactor(int i, int j, double gmx_unused)
{
    auto scatterLength_i = getScatterLength(i);
    auto scatterLength_j = getScatterLength(j);
    return 2 * scatterLength_i * scatterLength_j;
}

double XrayInfo::computeIntensity(t_pbc *pbc, double q, Selection sel)
{
    //significant adding errors if Debeye is not a double
    double    Debeye   = 0;
    const int posCount = sel.posCount();
...
double NeutronInfo::computeIntensity(t_pbc *pbc, double q, Selection sel)
{
    //significant adding errors if Debeye is not a double
    double    Debeye   = 0;
    const int posCount = sel.posCount();
...
{
    std::string element    = elements_[i];
    double      scattering = scatterFactors_[element];
    return scattering;
};
...
{
    std::string element    = elements_[i];
    CromerMan   CM         = scatterFactors_[element];
...
NeutronInfo makeNeutronInfo(const TopologyInformation &top)
{
    ...
    readNeutronScatteringFactors(neutronScatteringFactors);
    std::map<std::string, double>       scatterFactors;

    for (auto scatter : neutronScatteringFactors)
    {
        scatterFactors[scatter.element] = scatter.scatterLength;
    }
    ...
    {
        std::string element = atoms.atom[i].elem;
        elements.push_back(element);
    }
    return NeutronInfo(elements, scatterFactors);
};
...
XrayInfo makeXrayInfo(const TopologyInformation &top)
{
    ...
    readXrayScatteringFactors(xrayScatteringFactors);
    std::map<std::string, CromerMan> scatterFactors;

    for (auto scatter : xrayScatteringFactors)
    {
        scatterFactors[scatter.element] = scatter.CM;
    }
    ...
    return XrayInfo(elements, scatterFactors);
};
