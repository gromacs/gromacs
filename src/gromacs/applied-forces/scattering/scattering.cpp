...
void ComputeScattering::computeForces(t_pbc *pbc, double q, Selection sel, double scatterDiff)
{
    const int posCount = sel.posCount();
    scatterForces_.resize(DIM * posCount);
    double prefactor = scatterDiff * forceConstant_;
    for (int i = 0; i < posCount; ++i)
    {
        const SelectionPosition &pi = sel.position(i);
        auto index_i = pi.refId();
        ...
        for (int j = 0; j < i; ++j)
        {
            ...
            double dxNorm = norm(dx);
            double qDist =  dxNorm * q;
            double sinQDist = sin(qDist);
            double cosQDist = cos(qDist);
            double force = prefactor * formFactor * (qDist * cosQDist - sinQDist) / pow(dxNorm, 3);
            for (int m = 0; m < DIM; m++)
            {
                scatterForces_[DIM * index_i + m] += force * dx[m];
                scatterForces_[DIM * index_j + m] -= force * dx[m];
            }
        }
    }
    for (size_t i = 0; i < scatterForces_.size(); i+=DIM)
    {
       printf ("q: %0.2f index: %d force x: %9.4f force y: %9.4f force z: %9.4f\n",
               q, i, scatterForces_[i], scatterForces_[i+1], scatterForces_[i+2]);
    }
};
