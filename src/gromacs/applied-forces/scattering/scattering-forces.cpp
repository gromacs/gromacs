...
{
    if (isotope == "H")  {return Isotope::H;}
    if (isotope == "D")  {return Isotope::D;}
    if (isotope == "He") {return Isotope::He;}
    if (isotope == "Li") {return Isotope::Li;}    
    if (isotope == "Be") {return Isotope::Be;}
    if (isotope == "B")  {return Isotope::B;}    
    if (isotope == "C")  {return Isotope::C;}
    if (isotope == "N")  {return Isotope::N;}    
    if (isotope == "O")  {return Isotope::O;}
    if (isotope == "F")  {return Isotope::F;}    
    if (isotope == "Ne") {return Isotope::Ne;}
    if (isotope == "Na") {return Isotope::Na;}    
    if (isotope == "Mg") {return Isotope::Mg;}
    if (isotope == "Al") {return Isotope::Al;}    
    if (isotope == "Si") {return Isotope::Si;}
    if (isotope == "P")  {return Isotope::P;}    
    if (isotope == "S")  {return Isotope::S;}
    if (isotope == "Cl") {return Isotope::Cl;}
    if (isotope == "Ar") {return Isotope::Ar;}
    if (isotope == "K")  {return Isotope::K;}
    if (isotope == "Ca") {return Isotope::Ca;}
    if (isotope == "Mn") {return Isotope::Mn;}
    if (isotope == "Fe") {return Isotope::Fe;}
    if (isotope == "Cu") {return Isotope::Cu;}
    if (isotope == "Zn") {return Isotope::Zn;}
    if (isotope == "Se") {return Isotope::Se;}
    if (isotope == "Br") {return Isotope::Br;}
    if (isotope == "Kr") {return Isotope::Kr;}
    if (isotope == "Rb") {return Isotope::Rb;}
    if (isotope == "I")  {return Isotope::I;}
    if (isotope == "Xe") {return Isotope::Xe;}
    else
    {
        std::string message = formatString("Isotope '%s' is not in the scattering database\n",
                                           isotope.c_str());
...
std::vector<Isotope> getIsotopes(const t_atoms *atoms)
{
    ...
    for (int i = 0; i < atoms->nr; ++i)
    {
        ...
        isotopes.push_back(getIsotope(elem));
    }
    return isotopes;
};

NeutronInfo::NeutronInfo(const std::vector<Isotope> &isotopes)
    : isotopes_(isotopes)
{
    std::vector<NeutronStructureFactor>     neutronScatterFactors = readNeutronScatteringFactors();
    std::vector<double> scatterFactors (static_cast<int>(Isotope::maxIsotope));
    for (auto scatter : neutronScatterFactors)
    {
        Isotope isotope = getIsotope(scatter.isotope);
        scatterFactors.at(static_cast<int>(isotope)) = scatter.scatterLength;
    }
    scatterFactors_ = scatterFactors;
};

XrayInfo::XrayInfo(std::vector<Isotope> isotopes, std::vector<double> qList, bool bSolvent)
    : isotopes_(std::move(isotopes))
{
    ...
    for (auto q : qList)
    {
        for (auto scatter : xrayScatterFactors)
        {
            ...
            {
                double rho    = 0.334;
                double volume = pow(scatter.CM.vol, (2.0/3.0));
                scattering   -= rho * scatter.CM.vol * exp( -(q * q * volume) /(4.0 * M_PI));
            }
            Isotope isotope = getIsotope(scatter.isotope);
...
double ComputeScattering::computeS0MC(Selection sel, float coverage, int seed)
{
    ...
    {
        fprintf (stderr, "\rS0 done %3.1f%%     ", (100.0*(i+1))/scaledPosCount);
        ...
        for (int j = 0; j < scaledPosCount; ++j)
        {
            ...
            if (pi.mass() == 0 || pj.mass() == 0) { continue; } //mass == 0 means vsite
...
double ComputeScattering::computeIntensityMC(t_pbc *pbc, double q, Selection sel, float coverage, int seed, float modulation)
{
    //significant adding errors if Debeye is not a double
    double                      Debeye   = 0;
    const int                   posCount = sel.posCount();
    ...
    for (int i = 0; i < scaledPosCount; ++i)
    {
        fprintf (stderr, "\rIntensity done %3.1f%%     ", (100.0*(i+1))/scaledPosCount);
        int                      rand_i  = distribution(rng);
        const SelectionPosition &pi      = sel.position(rand_i);
        const int                index_i = sel.position(rand_i).atomIndices()[0];
        for (int j = 0; j < scaledPosCount; ++j)
        ...
        Debeye *= std::exp(-modulation * q * q);
    }
    return Debeye;
};


double ComputeScattering::computeIntensity(t_pbc *pbc, double q, Selection sel, float modulation)
{
    //significant adding errors if Debeye is not a double
    double    Debeye   = 0;
    const int posCount = sel.posCount();
