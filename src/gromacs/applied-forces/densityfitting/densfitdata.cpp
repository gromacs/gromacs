...
bool DensfitData::outputMapThisStep(int64_t step) const
{
    return do_per_step(step, nstmapout_);
}

bool DensfitData::fitThisStep(int64_t step) const
{
...
const GridDataFloat3D &DensfitData::referenceMap() const
{
    return map_ref_;
}
...
void DensfitData::read_densparams(std::vector<t_inpfile> *inp, warninp *wi)
{

    printStringNewline(inp, "Density Potential Type: 'cross-correlation' or 'cross-entropy' ");
    const auto densityPotentialEnumInt = get_eeenum(inp, "densfit-potential", densityPotentialNames, wi);
    densityPotential_ = DensityPotential(densityPotentialEnumInt);

    printStringNewline(inp, "Index group of atoms used for spreading");
    char         groupCharArray[STRLEN];

    setStringEntry(inp, "spread-group", groupCharArray, "");
    group_.name_ = std::string(groupCharArray);
    char         referenceDensityCharArray[STRLEN];
    setStringEntry(inp, "reference-density", referenceDensityCharArray, "");
    ...
    printStringNewline(inp, "List of times (ps) at which density fitting / refinement parameters "
                       "are given and exactly matched");
    printStringNewline(inp, "interpolate linearly in between");
    char timePointsString[STRLEN];
    setStringEntry(inp, "densfit-time", timePointsString, "");

    printStringNewline(inp, "Gaussian width(s) used to transform the discrete atomic positions "
                       "into a density (one sigma per refinement point)");
    char densfitSigmasString[STRLEN];
    setStringEntry(inp, "densfit-sigma", densfitSigmasString, "");

    printStringNewline(inp, "Strength constant(s) k (kJ/mol) of map potential V_fit = k*(1 - "
                       "corr.coeff.) (one k per refinement point)");
    char densfitForceConstantString[STRLEN];
    setStringEntry(inp, "densfit-k", densfitForceConstantString, "");

    printStringNewline(inp, "Cutoff distance (in sigmas) for density spreading, spread only within "
                       "+/- sigma*dist");
    dist_ = get_ereal(inp, "dist", 4.0, wi);
    printStringNewline(inp, "Calculate V_fit every nstfit time steps");
    nstfit_ = get_eint(inp, "nstfit", 1, wi);
    printStringNewline(inp, "Write diagnostic output every nstout time steps");
    nstout_ = get_eint(inp, "nstout", 1, wi);
    printStringNewline(inp, "Write simulated density map to file every nstmapout time steps");
    nstmapout_ = get_eint(inp, "nstmapout", 1000, wi);
    printStringNewline(inp, "Keep and number simulated density maps (results in a bunch of map files)");
    bKeepAndNumberMaps_ = get_eeenum(inp, "KeepMaps", yesno_names, wi);
    printStringNewline(inp, "Normalize maps to unity.");
    normalizeMapsToUnity_ = get_eeenum(inp, "densfit-normalizemapstounity", yesno_names, wi);
