...
const std::vector<real> &TimeDependentDensfitParameters::timePoints() const
{
    return timePoints_;
}

const std::vector<real> &
TimeDependentDensfitParameters::forceConstants() const
{
...
real TimeDependentDensfitParameters::currentK(real time) const
{
...
int DensfitData::nAtoms() const { return nat_; }
...
void DensfitData::printToOut(FILE *fp, const gmx_output_env_t *oenv) const
{
    ...
    fprintf(fp, "# Output is written in intervals of %d time step%s.\n", nstout_,
            nstout_ > 1 ? "s" : "");
    fprintf(fp, "# Number of refinement steps is %lu.\n",
            timeDependent_.timePoints().size());
...
const t_mapdata &DensfitData::referenceMap() { return map_ref_; }

bool DensfitData::keepAndNumberMaps() const { return bKeepAndNumberMaps_; };

int DensfitData::nStepsMapOutput() const { return nstmapout_; }

int DensfitData::nStepsFit() const { return nstfit_; }

int *DensfitData::indices() const { return ind_; }
...
bool DensfitData::areTimeDependent()
{
    // parameteres are time dependent if more than one time point is given.
    return timeDependent_.timePoints().size() > 1;
};
...
void DensfitData::makeGroups(char *groupname, t_blocka *grps, char **gnames)
{
                                  ...
                                  gmx_inputrec_strings *is)
{
    ...
    CTYPE("Temperature(s) (K) (one T per refinement point) - leave empty if you "
          "do not want to change the temperature!");
    ...
    timeDependent_ = TimeDependentDensfitParameters(
                is->densfit_time, is->densfit_k, is->densfit_sigma, wi);
