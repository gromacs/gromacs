...
DensitySpreader::DensitySpreader(const IGrid<DIM> &grid, int numberOfThreads, int n_sigma, real sigma, bool integrate) :
    simulated_density_(grid.duplicate()), number_of_threads_(numberOfThreads)
{
    ...
    #pragma omp critical
        {
            ...
            if (integrate)
            {
                gauss_transform_.emplace_back(new IntegratedGaussian());
            }
            else
            {
...
const GridDataReal3D &
DensitySpreader::spreadLocalAtoms(const std::vector<RVec> &x, const std::vector<real> &weights)
{
...
#pragma omp parallel num_threads(number_of_threads_)
    {
        ...
        containeroperation::setZero(&simulated_density_buffer_[thread]);
        gauss_transform_[thread]->setGrid(&simulated_density_buffer_[thread]);
...
DensitySpreader::sumThreadLocalGrids_(const std::vector<IVec> &minimumUsedGridIndex, const std::vector<IVec> &maximumUsedGridIndex)
{
    ...
    for (int gridIndexZZ = gridStart[ZZ]; gridIndexZZ <= gridEnd[ZZ]; ++gridIndexZZ)
    {
        for (int gridIndexYY = gridStart[YY]; gridIndexYY <= gridEnd[YY]; ++gridIndexYY)
        {
            ...
            std::vector<real>::iterator simulatedDensityVoxelIterator = simulated_density_.iteratorAtMultiIndex(xRowStartIndex);
