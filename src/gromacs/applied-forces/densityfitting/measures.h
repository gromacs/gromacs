...
class IDensityDensityMeasure
{
    public:
        virtual void
        evaluateDensityDensityDerivative(const GridDataReal3D &simulatedMap, real forceConstant) = 0;
        virtual const GridDataReal3D &densityDensityDerivative()       = 0;
        virtual real goodnessOfFit(const GridDataReal3D &simulatedMap) = 0;
};

class DensityCrossCorrelationMeasure : public IDensityDensityMeasure
{
    public:
        DensityCrossCorrelationMeasure(const GridDataReal3D &referenceMap);
        void
        evaluateDensityDensityDerivative(const GridDataReal3D &simulatedMap, real forceConstant) override;
        const GridDataReal3D &densityDensityDerivative() override;
        real goodnessOfFit(const GridDataReal3D &simulatedMap) override;
    private:
        bool           hasNormalizedSimulatedMap;
        real           normlizationFactorReferenceMap_;
        real           normlizationFactorSimulatedMap_;
        GridDataReal3D normalizedSimulatedMap_;
        GridDataReal3D normalizedReferenceMap_; //< Reference map with zero mean and unit variance for speed-up of cross correlation calculations

        GridDataReal3D derivativeMap_;
};

class DensityMeanSquareDeviationMeasure : public IDensityDensityMeasure
{
    public:
        DensityMeanSquareDeviationMeasure(const GridDataReal3D &referenceMap);
        ...
        real goodnessOfFit( const GridDataReal3D &simulatedMap) override;
    private:
        const GridDataReal3D &referenceMap_;
        GridDataReal3D        derivativeMap_;
};
...
{
    public:
        DensityCrossEntropyMeasure(const GridDataReal3D &referenceMap);
        void
        evaluateDensityDensityDerivative(const GridDataReal3D &simulatedMap, real forceConstant) override;
        const GridDataReal3D &densityDensityDerivative() override;
        real goodnessOfFit(const GridDataReal3D &simulatedMap) override;
