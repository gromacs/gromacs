...
class DensfitData
{
    public:
        ...
        DensfitData(const DensfitData &other);
        DensfitData &operator=(const DensfitData &other) = default;
        void broadcast(const t_commrec *cr);
        ...
        void setReferenceMap(const GridDataReal3D &reference);
        const GridDataReal3D &referenceMap() const;
        real dist() const;
        ...
        bool outputMapThisStep(gmx_int64_t step) const;
        bool fitThisStep(gmx_int64_t step) const;

        const TimeDependentDensfitParameters &timeDependent() const;

    private:
        GridDataReal3D                 map_ref_; /* The reference=experimental map to fit to        */
