                         ...
                         gmx_wallcycle *wcycle)
{
    ...
    if (MASTER(cr))
    {
        localDensfitData_->goodnessOfFit(parameters_->densityPotential(), parameters_->referenceMap());
    }

    localDensfitData_->densityDensityDerivative(parameters_->densityPotential(), parameters_->referenceMap());

    if (output_ && parameters_->outputMapThisStep(step) && parameters_->keepAndNumberMaps())
    {
