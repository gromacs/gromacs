...
real DensfitParameters::currentK(real time) const
{
    return interpolateLinearly(time, npoints_, time_values_, k_values_);
}
real DensfitParameters::currentT(real time) const
{
    return interpolateLinearly(time, npoints_, time_values_, temp_values_);
}
real DensfitParameters::currentSigma(real time) const
{
    return interpolateLinearly(time, npoints_, time_values_, sigma_values_);
}

//! \brief TODO
real DensfitParameters::interpolateLinearly(real time, int npoints, real *time_values,
                                            real *y_values) const
{
    real x, y;
    int  j;
...
int DensfitParameters::nAtoms() const
{
    return nat_;
}

void DensfitParameters::printToOut(FILE *fp) const
{
...
void DensfitParameters::setNstFitFromEnv(const t_commrec *cr, int nstlist, FILE *fp)
{
    char *env;
    char  buf[STRLEN];
    int   env_nstfit;
...
void DensfitParameters::broadcast(const t_commrec *cr)
{
...
bool DensfitParameters::keepAndNumberMaps() const
{
    return bKeepAndNumberMaps_;
};

int DensfitParameters::nStepsMapOutput() const
{return nstmapout_; }


int DensfitParameters::timePoints() const
{
    return npoints_;
}

int DensfitParameters::nStepsFit() const
{
    return nstfit_;
}

int * DensfitParameters::indices() const
{
    return ind_;
}

void DensfitParameters::print(FILE * fp, int indent) const
{
...
void DensfitParameters::do_fio(t_fileio *fio, bool bRead)
{
...
void DensfitParameters::read_densparams(int *ninp_p, t_inpfile **inp_p, warninp * wi, gmx_inputrec_strings *is)
{
...
real DensfitParameters::dist() const
{
    return dist_;
};
                             ...
                             bool bVerbose, bool bAppend, bool bParallel)
{
    ...
    this->df_ = std::unique_ptr<gmxDensfit>(
...
void Densfit::do_forces(matrix box)
{
    ...
    sim_dens_1d = map_sim_.vox;    /* Calculated density map */
    ref_dens_1d = map_ref_.vox;    /* Reference density map */
...
#pragma omp parallel for num_threads(nth) schedule(static)
    ...
    sum_rho_exp2        = df_->sum_rho_exp2;
    sum_rho_sim2        = calc_sum_rhoA_rhoB(map_sim_.vox, map_sim_.vox);
    sum_rho_exp_rho_sim =
        calc_sum_rhoA_rhoB(map_ref_.vox, map_sim_.vox);

    term1_prefactor = df_->k / sqrt(sum_rho_exp2 * sum_rho_sim2);
    term2_prefactor = -df_->k * sum_rho_exp_rho_sim /
        (sqrt(sum_rho_exp2) * pow(sum_rho_sim2, 1.5));

    V_vox            = df_->spacing * df_->spacing * df_->spacing;
    prefactor        = -M_PI * df_->sigma * df_->sigma / (6.0 * V_vox);
    OOsigma2_fac     = 1.5 / (df_->sigma * df_->sigma);
    sqrtOOsigma2_fac = sqrtf(OOsigma2_fac);

    nx = map_sim_.map_dim[XX];
    ny = map_sim_.map_dim[YY];
...
real Densfit::calc_correlation_coeff(FILE *log)
{
    ...
    for (i = 0; i < 3; i++)
    {
        if (map_ref_.map_dim[i] != map_sim_.map_dim[i])
        {
            gmx_fatal(FARGS, "Map dimensions must agree");
        }
    }

    numerator = calc_sum_rhoA_rhoB(map_ref_.vox, map_sim_.vox);

    denominator = sqrt(calc_sum_rhoA_rhoB(map_ref_.vox, map_ref_.vox) *
                   ...
                   gmx_bool bVerbose)
{
    ...
    grid_spacing = get_map_spacing(map_ref_, MASTER(cr) ? stdout : NULL);
    ...
    df_->sum_rho_exp2 = calc_sum_rhoA_rhoB(map_ref_.vox, map_ref_.vox);
                         ...
                         gmx_wallcycle *wcycle)
{
    ...
    if (MASTER(cr))
    {
        if (do_per_step(step, parameters_.nStepsMapOutput()))
        {
            ...
            MrcFile().write("map_ref", mapDataToGridData(map_ref_));
        }
...
t_mapdata Densfit::referenceMapCopy() const { return map_ref_; };
