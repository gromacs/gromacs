...
struct gmxDensfit {
    gmxDensfit(const DensfitParameters &parameters,
    ...
    *expVector;   /**< same for exp values             */
    private:
        ...
        void initParametersThatCanBeTimeDependent(
            const DensfitParameters &densfitParameters);
};

//! \brief Allocate and initialize the arrays for assembled positions, forces
//! and atomic weights
gmxDensfit::gmxDensfit(const DensfitParameters &parameters,
                       rvec *x_densfit_whole, /* Can be NULL! */
                       real grid_spacing, gmx_bool bVerbose, gmx_bool bAppend,
                       gmx_bool bParallel)
{
    ...
    snew(this->x_assembled, parameters.nat);
    snew(this->f_loc, parameters.nat);
    snew(this->w_assembled, parameters.nat);
    snew(this->x_old, parameters.nat);
    snew(this->x_shifts, parameters.nat);
    snew(this->extra_shifts, parameters.nat);
    snew(this->c_ind, parameters.nat);
    ...
    for (int i = 0; i < parameters.nat; i++)
    {
...
void gmxDensfit::initParametersThatCanBeTimeDependent(
        const DensfitParameters &densfitParameters)
{
    int ompthreads;

    this->sigma = densfitParameters.sigma_values[0];
    this->k     = densfitParameters.k_values[0];
    this->temp  = densfitParameters.temp_values[0];
...
DensfitParameters::DensfitParameters() {}

DensfitParameters::DensfitParameters(int npoint, real sigma, real k,
                                     real sigma_dist, int nAtoms, int *index,
                                     bool bVerbose)
{
    npoints = npoint;
    snew(time_values, npoints);
    snew(temp_values, npoints);
    snew(k_values, npoints);
    snew(sigma_values, npoints);
...
void DensfitParameters::makeGroups(char *groupname, t_blocka *grps,
                                   char **gnames)
{
    /* Group index from group name */
    int ig = search_string(groupname, grps->nr, gnames);
    nat = grps->index[ig + 1] - grps->index[ig];

    fprintf(stderr, "Density fitting group '%s' has %d atoms\n", groupname, nat);
    snew(ind, nat);
    for (int i = 0; i < nat; i++)
    {
        ind[i] = grps->a[grps->index[ig] + i];
    }
}
...
{
    parameters_ = DensfitParameters(1, sigma, k, sigma_dist, nat, ind, bVerbose);
    setPrivateData(nullptr, grid_spacing, bVerbose, 0, 0);
};

Densfit::Densfit(const DensfitParameters &parameters)
{
    parameters_ = DensfitParameters(parameters);
}
...
//! \brief TODO
real Densfit::interpolateLinearly(real time, int npoints, real *time_values,
                                  real *y_values)
{
    real x, y;
    int  j;
...
void Densfit::updateParametersThatAreTimeDependent(real time)
{
    ...
    df->temp =
        interpolateLinearly(time, parameters_.npoints, parameters_.time_values,
                            parameters_.temp_values);
    assert(df->temp >= -1);
    df->sigma =
        interpolateLinearly(time, parameters_.npoints, parameters_.time_values,
                            parameters_.sigma_values);
    assert(df->sigma > 0.0);
    df->k = interpolateLinearly(time, parameters_.npoints,
                                parameters_.time_values, parameters_.k_values);
     ...
     * we have to spread the density on is saved in voxrange (for each dimension
     * x, y, and z */
    df->voxrange     = ceil(0.5 * df->sigma * parameters_.dist / df->spacing);
...
void Densfit::dump_x(int nodeid, gmx_int64_t step)
{
    ...
    for (int i = 0; i < parameters_.nat; i++)
    {
...
void Densfit::dump_f(const char *fn, t_commrec *cr)
{
    ...
    if (cr && PAR(cr))
    {
        gmx_sum(3 * parameters_.nat, f[0], cr);
    }
                             ...
                             bool bVerbose, bool bAppend, bool bParallel)
{
    ...
    this->df = std::unique_ptr<gmxDensfit>(
                new gmxDensfit(parameters_, x_densfit_whole, grid_spacing, bVerbose,
                               bAppend, bParallel));
}
...
void Densfit::setSpacingFromMap(const t_mapdata &map)
{
    ...
    for (int i = 0; i < DIM; i++)
    {
        df->spacing += A2NM * map.cell[i] / map.map_dim[i];
    }
    df->spacing /= 3.;
}

const t_mapdata &Densfit::simulatedMap() { return map_sim; }
...
void Densfit::spread_atoms(matrix box)
{
    ...
    if (df->bVerbose)
    {
        fprintf(stderr,
                ...
                FitStr, parameters_.nat, map_dim[XX], map_dim[YY], map_dim[ZZ]);
        //        t0 = gettime();
        //        c1 = gmx_cycles_read();
    }
    ...
    spread_atoms_low(df->x_assembled, parameters_.nat, box);
...
void Densfit::do_forces(matrix box)
{
    ...
    ivec               elements;
    double             V_vox; // TODO: with pressure coupling, the grid spacing needs to be
                              // adjusted!!!
    double             prefactor;
    double             left;
...
#pragma omp parallel for num_threads(nth) schedule(static)
    ...
    sum_rho_exp_rho_sim =
        calc_sum_rhoA_rhoB(this->map_ref.vox, this->map_sim.vox);

    term1_prefactor = df->k / sqrt(sum_rho_exp2 * sum_rho_sim2);
    term2_prefactor = -df->k * sum_rho_exp_rho_sim /
...
#pragma omp parallel for num_threads(nth) schedule(static) private( \
    term1_sum, term2_sum, pos, istart, istop, elements, aarr, garr, i, ix, iy, \
    iz, left, ind, erfx, erfy, erfz, expx, expy, expz, erfy_erfz, expy_erfz, \
    erfy_expz, ptr_ref, ptr_sim, ref_dens, sim_dens, drho_dxl, force, tmp1, \
    tmp2, th, num_images, image, \
    img) shared(dfp, ref_dens_1d, sim_dens_1d, box, nx, ny, OOsigma2_fac, \
    ...
    for (l = 0; l < this->nat_loc; l++)
    {
        ...
        num_images = get_images(dfp->x_assembled[dfp->c_ind[l]], box, img, istart,
...
real Densfit::calc_correlation_coeff(FILE *log)
{
    ...
    denominator = sqrt(calc_sum_rhoA_rhoB(this->map_ref.vox, this->map_ref.vox) *
                       calc_sum_rhoA_rhoB(this->map_sim.vox, this->map_sim.vox));
...
FILE *Densfit::open_out(const char *fn, const gmx_output_env_t *oenv)
{
    ...
    else
    {
        ...
        fprintf(fp, "# Density fitting forces for %d atoms are updated every %d "
                "time step%s.\n",
                parameters_.nat, parameters_.nstfit,
                parameters_.nstfit != 1 ? "s" : "");
...
void Densfit::get_nstfit_from_env(t_commrec *cr, int nstlist)
{
    ...
    if (PAR(cr))
    {
        gmx_bcast(sizeof(parameters_.nstfit), &(parameters_.nstfit), cr);
    }
    ...
    if (MASTER(cr))
    {
        if (parameters_.nstfit > 1 && (parameters_.nstfit % nstlist != 0))
        {
            fprintf(stderr, "\n%s\n"
                   ...
                   gmx_bool bVerbose)
{
    ...
    if (!PAR(cr))
    {
        this->nat_loc = parameters_.nat;
        this->ind_loc = parameters_.ind;
    }

    new_map_sim_from_ref(map_ref);

    /* Make space for the density fitting private data, allocate x, w, and f
     * arrays, ... */
    snew(x_densfit_pbc, parameters_.nat); /* There ... */
    if (MASTER(cr))
    {
        ...
        for (i = 0; i < parameters_.nat; i++)
        {
            copy_rvec(x_pbc[parameters_.ind[i]], x_densfit_pbc[i]);
        }
        sfree(x_pbc); /* ... and back again */
    }
    if (PAR(cr))
    {
        gmx_bcast(parameters_.nat * sizeof(rvec), x_densfit_pbc, cr);
    }
...
void Densfit::assemble_atoms_for_spread(rvec x[])
{
    ...
    for (i = 0; i < parameters_.nat; i++)
    {
        copy_rvec(x[parameters_.ind[i]], df->x_assembled[i]);
    }
}
...
void Densfit::dd_make_local_df_indices(gmx_domdec_t *dd)
{
    ...
    dd_make_local_group_indices(dd->ga2la, parameters_.nat, parameters_.ind,
                                &this->nat_loc, &this->ind_loc, &this->nalloc_loc,
                                this->df->c_ind);
                         ...
                         real time)
{
    ...
    if (df->out_dens && do_per_step(step, parameters_.nstout))
    {
        fprintf(df->out_dens, "%12.5e%12g%12.5e%12.5e%12.7f%12.5e\n", time,
                df->temp, df->k, df->sigma, df->cc, df->k * (1.0 - df->cc));
    }
                         ...
                         gmx_wallcycle *wcycle)
{
     ...
     */
    if ((!do_per_step(step, parameters_.nstfit) &&
         (FALSE == df->bUpdateShifts)) ||
        (parameters_.nstfit < 1))
    {
        return;
    }

    /* Update the time-dependent parameters like k, sigma, etc. */
    if (parameters_.npoints > 1)
    {
    ...
    communicate_group_positions(
            cr, df->x_assembled, df->x_shifts, df->extra_shifts, df->bUpdateShifts,
            as_rvec_array(x.data()), parameters_.nat, this->nat_loc, this->ind_loc,
            df->c_ind, df->x_old, box);
    ...
    if (ir->ePBC != epbcNONE)
    {
        auto xAssembledArrayRef = arrayRefFromArray(
                    reinterpret_cast<gmx::RVec *>(df->x_assembled), parameters_.nat);
    ...
    if (PAR(cr))
    {
        ...
        nspread = ceil((real)parameters_.nat / nnodes);
        istart  = cr->nodeid * nspread;

        if ((nnodes - 1) == cr->nodeid)
        {
            nspread = parameters_.nat - nspread * (nnodes - 1);
        }
    }
    else
    {
        istart  = 0;
        nspread = parameters_.nat;
    }
    ...
    if (MASTER(cr))
    {
        if (do_per_step(step, parameters_.nstmapout))
        {
            char fn_with_step[STRLEN];

            if (parameters_.bKeepAndNumberMaps)
            {
...
t_mapdata Densfit::referenceMapCopy() const { return map_ref; };

const DensfitParameters &Densfit::parameters() const { return parameters_; }

void Densfit::setParameters(const DensfitParameters &parameters)
{
...
void Densfit::broadcast(const t_commrec *cr)
{
     ...
     * by pointers.
     */
    for (int i = 0; i < parameters_.npoints; i++)
    {
        snew_bc(cr, parameters_.time_values, parameters_.npoints);
        snew_bc(cr, parameters_.temp_values, parameters_.npoints);
        snew_bc(cr, parameters_.k_values, parameters_.npoints);
        snew_bc(cr, parameters_.sigma_values, parameters_.npoints);
    }
    nblock_bc(cr, parameters_.npoints, parameters_.time_values);
    nblock_bc(cr, parameters_.npoints, parameters_.temp_values);
    nblock_bc(cr, parameters_.npoints, parameters_.k_values);
    nblock_bc(cr, parameters_.npoints, parameters_.sigma_values);

    snew_bc(cr, parameters_.ind, parameters_.nat);
    nblock_bc(cr, parameters_.nat, parameters_.ind);
    if (nweight > 0)
    {
        snew_bc(cr, weight, parameters_.nat);
        nblock_bc(cr, parameters_.nat, weight);
    }

    block_bc(cr, map_ref);

    int grid1d = map_ref.grid[XX] * map_ref.grid[YY] * map_ref.grid[ZZ];
    if (!MASTER(cr))
    {
        /* For now it is sufficient to have the map title on the master node only */
        map_ref.title = nullptr;
        map_ref.vox.resize(grid1d);
    }
    nblock_bc(cr, grid1d, map_ref.vox.data());
}

void Densfit::new_map_sim_from_ref(const t_mapdata &map_ref)
{
    map_sim.datamode = static_cast<int>(MrcMetaData::MrcDataMode::float32);

    /* The simulated map copies the map_dim and grid settings from the reference
     * map. */
    for (int i = 0; i < 3; i++)
    {
        map_sim.map_dim[i]    = map_ref.map_dim[i];
        map_sim.grid[i]       = map_ref.grid[i];
        map_sim.axes_order[i] = map_ref.axes_order[i];
    }
    for (int i = 0; i < 6; i++)
    {
        map_sim.cell[i] = map_ref.cell[i];
    }
    map_sim.title = strdup("Calculated map");

    allocate_density_grid(map_ref.map_dim, &map_sim.vox);
}
} // namespace gmx
