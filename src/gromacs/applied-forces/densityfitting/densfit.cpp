...
void Densfit::init(int ePBC, gmx_mtop_t *mtop, rvec *x, matrix box, gmx::LocalAtomSetManager * atomsets, const t_commrec *cr)
{
    ...
    auto densfitAtoms = atomsets->add(parameters_->fittingGroup().ind_.size(), parameters_->fittingGroup().ind_.data());
...
void Densfit::initOutputFromCommandLineParameters(FILE *fplog, int nfile, const t_filenm *fnm, const gmx_output_env_t *oenv, gmx_bool bAppend, gmx_bool bVerbose)
{
    ...
    if (bVerbose && parameters_ != nullptr)
    {
                         ...
                         gmx_wallcycle *wcycle)
{
    ...
    if (MASTER(cr))
    {
        if (parameters_->outputMapThisStep(step))
        {
            ...
            gmx::MrcFile().write(outputMapFileName, df_->simulatedMap());
            gmx::MrcFile().write("map_ref.ccp4", parameters_->referenceMap());
        }
