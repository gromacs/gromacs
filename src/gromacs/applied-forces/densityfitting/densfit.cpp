...
void Densfit::updateParametersThatAreTimeDependent(real time)
{
    ...
    df_->sigma = parameters_->timeDependent().currentSigma(time);
    df_->k     = parameters_->timeDependent().currentK(time);
...
void Densfit::spread_atoms_low(rvec x[], int natoms, matrix box)
{
    ...
    prefactor /= V_vox;
    nx         = df_->map_sim_.map_dim[XX];
    ny         = df_->map_sim_.map_dim[YY];
    nz         = df_->map_sim_.map_dim[ZZ];
    ngrid      = nx * ny * nz;
 ...
 * density fitting group */
#pragma omp parallel for num_threads(nth) schedule(static) \
    shared(nx, ny, nz, ngrid, natoms, x, box, c_fac, sim_dens_1d) private( \
    ptr, aarr, istart, istop, image, pos, num_images, img, i, ix, iy, iz, \
    th, elements, ax, ay, az, ayaz, left) default(none)
    for (n = 0; n < natoms; n++)
    {
...
real Densfit::calc_correlation_coeff(FILE *log)
{
    ...
    denominator = sqrt(calc_sum_rhoA_rhoB(parameters_->referenceMap().vox,
...
{
    (df_->map_sim_).datamode =
        static_cast<int>(MrcMetaData::MrcDataMode::float32);

    /* The simulated map copies the map_dim and grid settings from the reference
     * map. */
    for (int i = 0; i < 3; i++)
    {
        (df_->map_sim_).map_dim[i]    = map_ref.map_dim[i];
        (df_->map_sim_).grid[i]       = map_ref.grid[i];
        (df_->map_sim_).axes_order[i] = map_ref.axes_order[i];
    }
    for (int i = 0; i < 6; i++)
    {
        (df_->map_sim_).cell[i] = map_ref.cell[i];
    }
