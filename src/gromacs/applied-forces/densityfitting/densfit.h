...
class Densfit
{
    public:
         ...
         */
        GridDataReal3D referenceMapCopy() const;
        ...
        void add_forces(rvec *f, gmx_unused t_commrec *cr, gmx_int64_t step,
                        real time);

        void setParameters(const DensfitData &parameters);

        //! \TODO: remove
        void setReferenceMap(const GridDataReal3D &referenceMap);
    ...
    private:
        std::unique_ptr<DensfitData>      parameters_; //< constant, global densityfitting data
        std::unique_ptr<LocalDensfitData> df_;         //< variable, node-local data
        std::unique_ptr<DensfitOutput>    output_;     //< parameters for outputting data
