...
{
    public:
        LocalDensfitData(LocalAtomSet localAtomSet, const DensfitData &parameters, rvec *x_densfit_whole /* Can be NULL! */);
         ...
         * output file
         */
        void dump_f(const char *fn, const t_commrec *cr);
        ...
        bool afterDomainDecomposition() const;

        void communicate(PaddedArrayRef<RVec> x, const t_commrec * cr, matrix box, int ePBC);

        void spreadAtoms(const t_commrec * cr, matrix box);

        void sumSimulatedMap(const t_commrec * cr);
        void calculateCorrelationCoefficient(const GridDataReal3D &referenceMap);
    private:
        ...
        std::vector<RVec> x_old;         /**< x_assembled from last step                     */
