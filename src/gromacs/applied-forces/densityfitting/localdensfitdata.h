...
class LocalDensfitData
{
    public:
        ...
        const GridDataFloat3D &simulatedMap() const;
    ...
    private:
        ...
        GridDataFloat3D    map_sim_;       /* The map simulated from atomic position data     */
        DensitySpreader    densitySpreader_;

        std::unique_ptr<IDensityDensityMeasure> measure_;

        std::unique_ptr<DensfitForces>          densfitForces_;
};
}
