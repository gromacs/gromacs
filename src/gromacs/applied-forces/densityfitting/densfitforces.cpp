...
void inline inPlaceTransform(PaddedArrayRef<real> valueArray)
{
    ...
    for (int i = 0; i < simdAlignedSize; i += GMX_SIMD_REAL_WIDTH)
    {
    ...
    for (int i = simdAlignedSize; i < valueArray.size(); ++i)
    {
...
DensfitForces::DensfitForces(const Grid < DIM, GridWithTranslation < DIM>> &grid, real sigma, real nSigma) :
    grid_(grid)
{
...
{
    sigma_    = sigma;
    voxrange_ = ceil(0.5 * sigma * nSigma / grid_.unitCell()[XX]);
    nu_       = (grid_.unitCell()[XX] + grid_.unitCell()[YY] + grid_.unitCell()[ZZ]) / (3.0 * sigma);
...
RVec DensfitForces::force(const RVec            &x,
                          const GridDataFloat3D &densityDensityDerivative)
{
    auto              atomGridIndex_ =
        grid_.coordinateToFloorMultiIndex({x[XX], x[YY], x[ZZ]});
    ...
    if (atomGridIndex_[XX] - voxrange_ >= grid_.lattice()[XX] ||
        atomGridIndex_[YY] - voxrange_ >= grid_.lattice()[YY] ||
        atomGridIndex_[ZZ] - voxrange_ >= grid_.lattice()[ZZ])
    {
        return {};
    }
    ...
    {
        dx[i] = (x[i] - grid_.multiIndexToCoordinate(atomGridIndex_)[i]) / sigma_;
    }
    ...
    const auto iStart = std::max(ptrdiff_t(0), atomGridIndex_[XX] - voxrange_);
    const auto iEnd   =
        std::min(grid_.lattice()[XX], atomGridIndex_[XX] + voxrange_);
    ...
    {
        offset<DIM> currentIndex {
            {
                atomGridIndex_[XX], atomGridIndex_[YY], atomGridIndex_[ZZ] + iz
            }
        };
        if (!grid_.lattice().contains(currentIndex))
        {
            continue;
        }
        ...
        for (int iy = -voxrange_; iy <= voxrange_; iy++)
        {
            ...
            auto       derivativeMapIterator =
                densityDensityDerivative.begin()+ densityDensityDerivative.memoryOffset({iStart, atomGridIndex_[YY] + iy, atomGridIndex_[ZZ] + iz});
