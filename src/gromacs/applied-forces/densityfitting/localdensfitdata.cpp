...
#pragma omp parallel for num_threads(nth) schedule(static)
    ...
    const double term2_prefactor = -k * sum_rho_exp_rho_sim / (sqrt(sum_rho_exp2) * pow(sum_rho_sim2, 1.5));
    ...
    const double prefactor       = -sqrtf(2) / (sqrtf(M_PI));
    ...
    for (size_t l = 0; l < localAtoms.numAtomsLocal(); l++)
    {
        ...
        for (int dim = XX; dim <= ZZ; ++dim)
        {
            ...
            for (int ix = -voxrange; ix <= voxrange + 1; ix++)
            {
                ...
                garr[dim][i]   = -0.5 * left * left;
                aarr[dim][i++] = left / sqrtf(2);
            }
        }
        ...
        for (int iz = -voxrange; iz <= voxrange; iz++)
        {
            ...
            for (int iy = -voxrange; iy <= voxrange; iy++)
            {
                ...
                for (int ix = -voxrange; ix <= voxrange; ix++)
                {
                    ...
                    const auto factor = prefactor * (term1_prefactor * ref_dens +  term2_prefactor * sim_dens);

                    force[XX] += factor * drho_dxl[XX];
                    force[YY] += factor * drho_dxl[YY];
                    force[ZZ] += factor * drho_dxl[ZZ];
