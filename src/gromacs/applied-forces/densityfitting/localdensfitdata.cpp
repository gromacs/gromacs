...
#pragma omp parallel for num_threads(nth) schedule(static)
    ...
    const auto &derivativeMap   = measure_->densityDensityDerivative();
    const auto &forceCalculator = densfitForces_.get();
#pragma omp parallel for num_threads(nth) schedule(static) shared(forceCalculator, derivativeMap) default(none)
    for (size_t l = 0; l < localAtoms.numAtomsLocal(); l++)
    {
        const RVec force = forceCalculator->force(x_assembled[localAtoms.collectiveIndex()[l]], derivativeMap);
        rvec_inc(f_loc[l], force);
    }
}

const GridDataFloat3D &LocalDensfitData::simulatedMap() const
{
    return map_sim_;
}
