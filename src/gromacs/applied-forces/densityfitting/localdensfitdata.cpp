...
real innerProduct(PaddedArrayRef<const real> reference, PaddedArrayRef<const real> comparand)
{
#if GMX_SIMD_HAVE_REAL
    SimdReal   simdSum = 0.;
...
#pragma omp parallel for num_threads(nth) schedule(static)
    ...
    const double dRhoDxPrefactor = sqrt(2./M_PI) * 1/(2.*2.*2.);

#pragma omp parallel for num_threads(nth) schedule(static) shared(grid, stderr) default(none)
    ...
    for (size_t l = 0; l < localAtoms.numAtomsLocal(); l++)
    {
        ...
        const int  iyOffset = 2*voxrange+2;
        const int  izOffset = 2*(2*voxrange+2);
        real       forceX   = 0.;
        real       forceY   = 0.;
        real       forceZ   = 0.;
...
#pragma omp simd
                for (int ix = -voxrange; ix <= voxrange; ix++)
                {
                    ...
                    const auto   factor =  *derivativeMapIterator * dRhoDxPrefactor;
        ...
        rvec_inc(f_loc[l], RVec {forceX, forceY, forceZ});
...
{
    std::transform(std::begin(referenceMap), std::end(referenceMap), std::begin(map_sim_), std::begin(derivativeMap_), T::derivative);
    containeroperation::multiply(&derivativeMap_, forceConstant_);
}

void LocalDensfitData::densityDensityDerivative(DensityPotential potential, const GridDataReal3D &referenceMap)
{
    switch (potential)
    {
        case gmx::DensityPotential::CrossCorrelation:
            densityDensityDerivative<gmx::gaussianLogLikelihoodFunctor>(referenceMap);
            break;
        case gmx::DensityPotential::CrossEntropy:
            densityDensityDerivative<gmx::poissonLogLikelihoodFunctor>(referenceMap);
            break;
        case gmx::DensityPotential::NumberOfPotentials:
            break;
    }
}
...
void LocalDensfitData::goodnessOfFit(const GridDataReal3D &referenceMap)
{
    ...
    goodnessOfFit_ = innerProduct<T::value>(referenceMap, map_sim_);
}

void LocalDensfitData::goodnessOfFit(DensityPotential potential, const GridDataReal3D &referenceMap)
{
    switch (potential)
    {
        case gmx::DensityPotential::CrossCorrelation:
            goodnessOfFit<gmx::gaussianLogLikelihoodFunctor>(referenceMap);
            break;
        case gmx::DensityPotential::CrossEntropy:
            goodnessOfFit<gmx::poissonLogLikelihoodFunctor>(referenceMap);
            break;
        case gmx::DensityPotential::NumberOfPotentials:
            break;
    }
...
template void LocalDensfitData::goodnessOfFit<gaussianLogLikelihoodFunctor>(const GridDataReal3D &referenceMap);
template void LocalDensfitData::densityDensityDerivative<gaussianLogLikelihoodFunctor>(const GridDataReal3D &referenceMap);

template void LocalDensfitData::goodnessOfFit<poissonLogLikelihoodFunctor>(const GridDataReal3D &referenceMap);
template void LocalDensfitData::densityDensityDerivative<poissonLogLikelihoodFunctor>(const GridDataReal3D &referenceMap);
