    ...
    localAtoms(localAtomSet)
{
    ...
    const auto nAtoms = parameters.fittingGroup().nat_;
    map_sim_.setGrid(parameters.referenceMap().getGrid().duplicate());
    snew(x_assembled, nAtoms);
    snew(f_loc, nAtoms);
    snew(x_old, nAtoms);
    snew(x_shifts, nAtoms);
    snew(extra_shifts, nAtoms);
     ...
     * x, y, and z */
    voxrange =
        ceil(0.5 * sigma * parameters.dist() / spacing);
...
#pragma omp parallel for ordered num_threads(ompthreads) schedule(static) \
    ...
    sum_rho_exp2 = calc_sum_rhoA_rhoB(referenceMap, referenceMap);
    map_sim_.setGrid(referenceMap.getGrid().duplicate());
}
...
void LocalDensfitData::dump_f(const char *fn, t_commrec *cr)
{
    rvec *f;
    FILE *fpout = NULL;
...
#pragma omp parallel for num_threads(nth) schedule(static) \
    ...
    const auto   x          = x_assembled + istart;
...
#pragma omp parallel for num_threads(nth) schedule(static) \
    shared(natoms, x, box) default(none)
...
#pragma omp parallel for num_threads(nth) schedule(static) \
    ...
    for (size_t l = 0; l < localAtoms.numAtomsLocal(); l++)
    {
        ...
        int num_images =
            get_images(dfp->x_assembled[dfp->localAtoms.collectiveIndex()[l]], box, img, istart, istop,
...
void LocalDensfitData::communicate( PaddedArrayRef<RVec> x, const t_commrec * cr, matrix box, int ePBC)
{
    communicate_group_positions(
            cr, x_assembled, x_shifts, extra_shifts,
            bUpdateShifts, as_rvec_array(x.data()), localAtoms.numAtomsGlobal(),
            localAtoms.numAtomsLocal(), localAtoms.localIndex().data(), localAtoms.collectiveIndex().data(), x_old, box);
 ...
 */
    if (ePBC != epbcNONE)
    {
        auto xAssembledArrayRef = arrayRefFromArray(
                    reinterpret_cast<gmx::RVec *>(x_assembled), localAtoms.numAtomsGlobal());
...
void LocalDensfitData::spreadAtoms(t_commrec * cr, matrix box)
{
