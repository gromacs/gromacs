...
{
    char buffer[STRLEN];
    sprintf(buffer, "%12g%12.5e%12.5e%12.7f%12.5e", 0., k, sigma, goodnessOfFit_, -k * goodnessOfFit_);
    return buffer;
}
...
{
    const double term2_prefactor = -innerProduct<multipliesFunctor>(referenceMap, map_sim_);
    const auto   forceConstant   = k;
    std::transform(std::begin(referenceMap), std::end(referenceMap), std::begin(map_sim_), std::begin(derivativeMap_),
                   [forceConstant, term2_prefactor](real reference, real simulated){
                       return forceConstant * (reference + term2_prefactor * simulated);
                   });
}

void LocalDensfitData::goodnessOfFit(const GridDataReal3D &referenceMap)
{
    goodnessOfFit_ = innerProduct<multipliesFunctor>(referenceMap, map_sim_);
}
