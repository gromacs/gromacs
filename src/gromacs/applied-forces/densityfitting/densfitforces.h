...
{
    public:
        DensfitForces(const Grid < DIM, GridWithTranslation < DIM>> &grid, real sigma, real nSigma);
        RVec force(const RVec &x, const GridDataFloat3D &densityDensityDerivative);
        void setSigma(real sigma, real nSigma);

    private:
        Grid < DIM, GridWithTranslation < DIM>> grid_;
