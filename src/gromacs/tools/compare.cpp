                      ...
                      real ftol, real abstol)
{
    for (int m = 0; (m < DIM); m++)
    {
        ...
        cmp_real(fp, buf, -1, f1.a(m), f2.a(m), ftol, abstol);
        cmp_real(fp, buf, -1, f1.omega(m), f2.omega(m), ftol, abstol);
        cmp_real(fp, buf, -1, f1.t0(m), f2.t0(m), ftol, abstol);
        cmp_real(fp, buf, -1, f1.sigma(m), f2.sigma(m), ftol, abstol);
    }
}
