...
#define NFILE asize(fnm)
    ...
    else
    {
        ...
        if (bExtend)
        {
            ir->nsteps = ir->nsteps - (run_step - ir->init_step) + static_cast<gmx_int64_t>(extend_t/ir->delta_t + 0.5);
        ...
        else if (bUntil)
        {
            printf("nsteps = %s, run_step = %s, current_t = %g, until = %g\n",
                   ...
                   run_t, until_t);
            ir->nsteps = static_cast<gmx_int64_t>((until_t - run_t)/ir->delta_t + 0.5);
