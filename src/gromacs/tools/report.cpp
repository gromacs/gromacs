...
class Report : public ICommandLineOptionsModule
{
    public:
        Report()
...
int Report::run()
{
    t_state    state;
    t_inputrec ir;
    read_tpx_state(inputTopology_.c_str(), &ir, &state, &top_);
    if (writeLatex_)
    {
        TextOutputFile *file = (new TextOutputFile(outputFileLatex_));
        writeInformation(file, true, ir, top_, true);
        delete(file);
    }
    if (writePlainText_)
    {
        TextOutputFile *file = (new TextOutputFile(outputFileUnformatted_));
        writeInformation(file, false, ir, top_, true);
        delete(file);
    }
    TextOutputFile         &stdoutFile = TextOutputFile::standardOutput();
    writeInformation(&stdoutFile, false, ir, top_, false);
