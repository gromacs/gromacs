...
class Report : public ICommandLineOptionsModule, public ITopologyProvider
{
    public:
        Report()
            : bLatex_(false), bWrite_(false), bFullTopology_(false)
        {
        }
    ...
    private:
         ...
         */
        void writeHeader(TextWriter *writer, const std::string &text, const std::string &section);
...
void Report::writeHeader(TextWriter *writer, const std::string &text, const std::string &section)
{
