...
class Report : public ICommandLineOptionsModule, public ITopologyProvider
{
    public:
        Report()
            : bLatex_(false), bFullTopology_(false), top_()
        {
        }
    ...
    private:
        ...
        //! Boolean reporting if writing to the output file is requested.
        bool                bLatex_;
                         ...
                         ICommandLineOptionsModuleSettings *settings)
{
    const char *const desc[] = {
        ...
        "terminal or to a LaTeX formatted output file if run with",
        "the [TT]-m[tt] option.",
    ...
    options->addOption(FileNameOption("m")
                           ...
                           .store(&outputFile_).storeIsSet(&bLatex_)
...
void Report::writeInformation()
{
    ...
    TextOutputFile         *latexFile = nullptr;
    TextWriter              writer(bLatex_ ? latexFile = (new TextOutputFile(outputFile_)) : &stdoutFile);
    writer.ensureEmptyLine();
