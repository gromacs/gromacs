                             ...
                             int*           npme_optimal)        /* OUT: Optimal number of PME nodes */
{
    ...
    bRefinedGrid = !(info->nkx[k_win] == info->nkx[0] && info->nky[k_win] == info->nky[0]
...
static void launch_simulation(gmx_bool    bLaunch,          /* Should the simulation be launched? */
...
static void make_npme_list(const char* npmevalues_opt, /* Make a complete list with all
                                                        * possibilities or a short list that keeps
                                                        * only reasonable numbers of PME nodes */
                           ...
                           int  maxPMEnodes)  /* Maximum number of PME nodes                      */
{
    ...
    for (i = 0; i < nlistmax - 2; i++)
    {
        ...
        switch (eNPME)
        {
            case eNpmeAll: min_factor = 1; break;
            case eNpmeReduced: min_factor = 2; break;
            case eNpmeSubset:
...
static void do_the_tests(FILE*  fp,                    /* General tune_pme output file           */
                                                        ...
                                                        * constructing mdrun command lines */
{
    ...
    cmdline_length = std::strlen(cmd_mpirun) + std::strlen(cmd_np) + std::strlen(cmd_mdrun)
                     + std::strlen(cmd_args_bench) + std::strlen(tpr_names[0]) + 100;
    ...
    if (npme_fixed < -1)
    {
        make_npme_list(npmevalues_opt, pmeentries, &nPMEnodes, nnodes, minPMEnodes, maxPMEnodes);
    }
    else
    {
        *pmeentries = 1;
    ...
    for (k = 0; k < nr_tprs; k++)
    {
        ...
        for (i = 0; i < *pmeentries; i++)
        {
            ...
            for (nr = 0; nr < repeats; nr++)
            {
                ...
                {
                    fprintf(stdout,
                            "Skipping remaining passes of unsuccessful setting, see log file for "
                            "details.\n");
                    count += repeats - (nr + 1);
                    break;
                }
...
static void check_input(int             nnodes,
                        int             repeats,
                        int*            ntprs,
                        real*           rmin,
                        real            rcoulomb,
                        ...
                        t_pargs*        pa)
{
    ...
    {
        gmx_fatal(FARGS,
                  "Checkpoint input (-cpi) and benchmark checkpoint output (-bcpo) files must not "
                  "be identical.\n"
    ...
    if (*ntprs > 1)
    {
        if (gmx_within_tol(*rmin, rcoulomb, GMX_REAL_EPS)
            && gmx_within_tol(rcoulomb, *rmax, GMX_REAL_EPS)) /* We have just a single rc */
        {
            fprintf(stderr,
    ...
    if (*ntprs > 1)
    {
        ...
        {
            fprintf(stderr,
                    "WARNING: Applying extreme scaling factor. I hope you know what you are "
                    "doing.\n");
        }
    }
...
static void
create_command_line_snippets(gmx_bool bAppendFiles,
...
int gmx_tune_pme(int argc, char* argv[])
{
    const char* desc[] = {
        ...
        "for a higher accuracy of the measurements, you should set [TT]-resetstep[tt] to a higher ",
        "value.",
        ...
        "command-line argument. This works exactly like [TT]mdrun -gpu_id[tt], does not imply a ",
        "mapping,",
        "and merely declares the eligible set of GPU devices. [TT]gmx-tune_pme[tt] will construct ",
        "calls to",
    ...
    const char* procstring[]     = { nullptr, "np", "n", "none", nullptr };
    const char* npmevalues_opt[] = { nullptr, "auto", "all", "subset", nullptr };
    ...
    t_pargs pa[] = {
        ...
        { "-npstring",
          ...
          etENUM,
          { procstring },
          "Name of the [TT]$MPIRUN[tt] option that specifies the number of ranks to use ('np', or "
          "'n'; use 'none' if there is no such option)" },
        { "-ntmpi",
          FALSE,
          etINT,
          { &nthreads },
          "Number of MPI-threads to run the tests on (turns MPI & mpirun off)" },
        { "-r", FALSE, etINT, { &repeats }, "Repeat each test this often" },
        { "-max", FALSE, etREAL, { &maxPMEfraction }, "Max fraction of PME ranks to test with" },
        { "-min", FALSE, etREAL, { &minPMEfraction }, "Min fraction of PME ranks to test with" },
        { "-npme",
          FALSE,
          etENUM,
          { npmevalues_opt },
          "Within -min and -max, benchmark all possible values for [TT]-npme[tt], or just a "
          "reasonable subset. "
          "Auto neglects -min and -max and chooses reasonable values around a guess for npme "
          "derived from the .tpr" },
        { "-fix",
          FALSE,
          etINT,
          { &npme_fixed },
          "If >= -1, do not vary the number of PME-only ranks, instead use this fixed value and "
          "only vary rcoulomb and the PME grid spacing." },
        { "-rmax",
          FALSE,
          etREAL,
          { &rmax },
          ...
          "downscaling)" },
        { "-rmin", FALSE, etREAL, { &rmin }, "If >0, minimal rcoulomb for -ntpr>1" },
        { "-scalevdw", FALSE, etBOOL, { &bScaleRvdw }, "Scale rvdw along with rcoulomb" },
        { "-ntpr",
          FALSE,
          etINT,
          { &ntprs },
          "Number of [REF].tpr[ref] files to benchmark. Create this many files with different "
          "rcoulomb scaling factors depending on -rmin and -rmax. "
        ...
        { "-nsteps",
          ...
          "[REF].tpr[ref], add [REF].cpt[ref] steps)" },
    ...
    if ((nnodes > 2) && (npme_fixed < -1))
    {
        ...
        else
        {
            ...
            fprintf(stdout,
                    ...
                    "separate PME ranks are always tested.\n");
        }
    }
    else
    {
