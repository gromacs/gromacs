...
typedef struct
{
    ...
    double  averageNumGigaCycles;
    float  *nanoSecondsPerDay;
    float   averageNanoSecondsPerDay;
    float  *pmeForceLoad;        /* PME mesh/force load average*/
    float   averagePmeForceLoad; /* Average average ;) ... */
    char   *mdrunCommandLine;    /* Mdrun command line used for this test */
} t_perf;


typedef struct
{
    int             numInputFiles;    /* The number of tpr and mdp input files */
    gmx_int64_t     originalNumSteps; /* Number of steps to be done in the real simulation */
    gmx_int64_t     originalInitStep; /* Init step for the real simulation */
    real           *rcoulomb;         /* The coulomb radii [0...numInputFiles] */
...
//! Utility function for printing a line of dashes as a separator
...
//! Wrapper function around system calls
...
//! Clean up output files during exit
...
//! Parses a logfile to extract mdrun performance data
                  ...
                  int nranks)
{
    ...
    fp = fopen(logfile, "r");
    perfdata->pmeForceLoad[test_nr]        = -1.0;
    perfdata->numPmeRanksUsed              = -1;
    ...
    while (fgets(line, STRLEN, fp) != NULL)
    {
        ...
        {
            case eFoundNothing:
                /* Look for domain decomp grid and separate PME ranks: */
                if (str_starts(line, matchstrdd))
                {
                    sscanf(line, "Domain decomposition grid %8d x %8d x %8d, separate PME nodes %8d",
                           &(perfdata->nx), &(perfdata->ny), &(perfdata->nz), &npmeSetting);
                    if (perfdata->npmeSetting == -1)
                    {
                        perfdata->numPmeRanksUsed = npmeSetting;
                    }
                    else if (perfdata->npmeSetting != npmeSetting)
                    {
            ...
            case eFoundDdStr:
                ...
                {
                    sscanf(&line[strlen(matchstrbal)], "%20f", &(perfdata->pmeForceLoad[test_nr]));
                }
            ...
            case eFoundAccountingStr:
                ...
                {
                    sscanf(line, "Total %*f %20lf", &(perfdata->numGigaCycles[test_nr]));
            ...
            case eFoundCycleStr:
                ...
                {
                    ndum = sscanf(line, "%60s %20f %20f %20f %20f", dumstring, &dum1, &dum2, &dum3, &dum4);
                    /* (ns/day) is the second last entry, depending on whether GMX_DETAILED_PERF_STATS was set in print_perf(), nrnb.c */
                    perfdata->nanoSecondsPerDay[test_nr] = (ndum == 5) ? dum3 : dum1;
                    fclose(fp);
...
//! Analyzes mdrun performance data for runs with different -npme
//! settings and .tpr files to find the optimum way to run mdrun
gmx_bool analyze_data(
        ...
        int           nranks,
        int           ntprs,
        int           numNpmeSettingsTested,
        int           nrepeats,
        t_inputinfo  *info,
        int          *bestTprFileIndex, /* OUT: Nr of mdp file with best settings */
        int          *bestNpmeSetting)  /* OUT: Optimal number of PME ranks */
{
    int      i, j, k;
    int      lineNumber    = 0, bestLineNumber = -1;
    int      bestNpmeIndex = -1;
    double   s             = 0.0; /* standard deviation */
    t_perf  *pd;
    char     strbuf[STRLEN];
    char     str_pmeForceLoad[13];
    ...
    *bestTprFileIndex = -1;
    ...
    for (k = 0; k < ntprs; k++)
    {
        for (i = 0; i < numNpmeSettingsTested; i++)
        {
            /* Select the right dataset: */
            pd = &(perfdata[k][i]);

            pd->averageNumGigaCycles     = 0.0;
            pd->averagePmeForceLoad      = 0.0;
            pd->averageNanoSecondsPerDay = 0.0;

            if (pd->npmeSetting == -1)
            {
                sprintf(strbuf, "(%3d)", pd->numPmeRanksUsed);
            }
            else
            {
                sprintf(strbuf, "     ");
            ...
            {
                pd->averageNumGigaCycles    += pd->numGigaCycles[j];
                pd->averagePmeForceLoad     += pd->pmeForceLoad[j];
            }
            pd->averageNumGigaCycles    /= nrepeats;
            pd->averagePmeForceLoad     /= nrepeats;

            for (j = 0; j < nrepeats; j++)
            {
                if (pd->nanoSecondsPerDay[j] > 0.0)
                {
                    pd->averageNanoSecondsPerDay += pd->nanoSecondsPerDay[j];
                }
                else
                {
                    /* Somehow the performance number was not aquired for this run,
                     * therefor set the average to some negative value: */
                    pd->averageNanoSecondsPerDay = -1.0f*nrepeats;
                    break;
                }
            }
            pd->averageNanoSecondsPerDay /= nrepeats;

            /* Nicer output: */
            if (pd->averagePmeForceLoad > 0.0)
            {
                sprintf(str_pmeForceLoad, "%12.3f", pd->averagePmeForceLoad);
            }
            else
            {
                sprintf(str_pmeForceLoad, "%s", "         -  ");
            }


            /* We assume we had a successful run if both averages are positive */
            if (pd->averageNumGigaCycles > 0.0 && pd->averageNanoSecondsPerDay > 0.0)
            {
                ...
                if (nrepeats > 1)
                {
                    ...
                    for (j = 0; j < nrepeats; j++)
                    {
                        s += pow( pd->numGigaCycles[j] - pd->averageNumGigaCycles, 2 );
                    }
                    s /= (nrepeats - 1);
                    s  = sqrt(s);

                    fprintf(fp, "%4d %3d %4d%s %12.3f %12.3f %12.3f %s",
                            lineNumber, k, pd->npmeSetting, strbuf, pd->averageNumGigaCycles, s,
                            pd->averageNanoSecondsPerDay, str_pmeForceLoad);
                    if (nranks > 1)
                    {
                ...
                if ( (*bestTprFileIndex == -1) || (pd->averageNumGigaCycles < perfdata[*bestTprFileIndex][bestNpmeIndex].averageNumGigaCycles) )
                {
                    *bestTprFileIndex    = k;
                    bestNpmeIndex        = i;
    ...
    *bestNpmeSetting = perfdata[*bestTprFileIndex][bestNpmeIndex].npmeSetting;

    if (1 == numNpmeSettingsTested)
    {
        /* We stuck to a fixed number of PME-only ranks */
        sprintf(strbuf, "settings No. %d", *bestTprFileIndex);
    }
    else
    {
        /* We have optimized the number of PME-only ranks */
        if (*bestNpmeSetting == -1)
        {
            sprintf(strbuf, "%s", "the automatic number of PME ranks");
        }
        else
        {
            sprintf(strbuf, "%d PME ranks", *bestNpmeSetting);
        }
    }
    fprintf(fp, "Best performance was achieved with %s", strbuf);
    if ((nrepeats > 1) && (numNpmeSettingsTested > 1))
    {
        fprintf(fp, " (see line %d)", bestLineNumber);
    }
    fprintf(fp, "\n");

    /* Only mention settings if they were modified: */
    bRefinedCoul = !gmx_within_tol(info->rcoulomb[*bestTprFileIndex], info->rcoulomb[0], GMX_REAL_EPS);
    bRefinedVdW  = !gmx_within_tol(info->rvdw[*bestTprFileIndex], info->rvdw[0], GMX_REAL_EPS);
    bRefinedGrid = !(info->nkx[*bestTprFileIndex] == info->nkx[0] &&
                     info->nky[*bestTprFileIndex] == info->nky[0] &&
                     info->nkz[*bestTprFileIndex] == info->nkz[0]);
    ...
    if (bRefinedCoul)
    {
        fprintf(fp, "   New Coulomb radius: %f nm (was %f nm)\n", info->rcoulomb[*bestTprFileIndex], info->rcoulomb[0]);
    }

    if (bRefinedVdW)
    {
        fprintf(fp, "   New Van der Waals radius: %f nm (was %f nm)\n", info->rvdw[*bestTprFileIndex], info->rvdw[0]);
    }

    if (bRefinedGrid)
    {
        fprintf(fp, "   New Fourier grid xyz: %d %d %d (was %d %d %d)\n", info->nkx[*bestTprFileIndex], info->nky[*bestTprFileIndex], info->nkz[*bestTprFileIndex],
...
void get_program_paths(gmx_bool bUseThreadMpi, char *mpirunNameToUse[], char *mdrunNameToUse[])
{
    ...
    if (!bUseThreadMpi)
    {
        ...
        {
            *mpirunNameToUse = strdup(cp);
        }
        else
        {
            *mpirunNameToUse = strdup(defaultMpirunName);
        }
    }
    else
    {
        *mpirunNameToUse = strdup(emptyMpirunName);
    }

    if ( (cp = getenv("MDRUN" )) != NULL)
    {
        *mdrunNameToUse  = strdup(cp);
    }
    else
    {
        *mdrunNameToUse  = strdup(defaultMdrunName);
    }
}

/*! \brief Check that the commands will run mdrun (perhaps via mpirun) by
 * running a very quick test simulation. Requires MPI environment or
 * GPU support to be available if applicable.
 *
 * \todo Implement feature to parse the log file to get the list of
                       ...
                       gmx_bool    bNeedGpuSupport)
{
    ...
    if (bUseThreadMpi)
    {
        snew(command, strlen(mdrunNameToUse) + strlen(argumentsForRanks) + strlen(filename) + 50);
        sprintf(command, "%s%s-version -maxh 0.001 1> %s 2>&1", mdrunNameToUse, argumentsForRanks, filename);
    }
    else
    {
        snew(command, strlen(mpirunNameToUse) + strlen(argumentsForRanks) + strlen(mdrunNameToUse) + strlen(filename) + 50);
        sprintf(command, "%s%s%s -version -maxh 0.001 1> %s 2>&1", mpirunNameToUse, argumentsForRanks, mdrunNameToUse, filename);
    }
    ...
    if (bUseThreadMpi)
    {
        if (bMPI)
        {
            gmx_fatal(FARGS, "Need a thread-MPI version of mdrun. This one\n"
                      "(%s)\n"
...
//! Build a string containing an mdrun command-line -gpu_id flag and
//! argument. Handles the no-GPU case by emitting an empty string.
char *make_gpu_id_command_line(int numRanks, int numPmeRanks, const t_eligible_gpu_ids *gpu_ids)
{
    ...
    {
        int   numPpRanks, maxNumRanksForEachGpu;
        int   gpu_id, rank;
        ...
        maxNumRanksForEachGpu      = numPpRanks / gpu_ids->n;
        if (maxNumRanksForEachGpu * gpu_ids->n != numPpRanks)
        {
            /* Some GPUs will receive more work than others, which
             * we choose to be those with the lowest indices */
            maxNumRanksForEachGpu++;
        }
        ...
        for (gpu_id = 0, rank = 0; gpu_id < gpu_ids->n; gpu_id++)
        {
            ...
            for (rank_for_this_GPU = 0;
                 rank_for_this_GPU < maxNumRanksForEachGpu && rank < numPpRanks;
...
//! Launch the optimized mdrun simulation
void launch_simulation(
        ...
        char                     *mpirunNameToUse,   /* Command for mpirun */
        char                     *argumentsForRanks, /* Switch for -np or -ntmpi or empty */
        char                     *mdrunNameToUse,    /* Command for mdrun */
        char                     *argumentsForMdrun, /* Arguments for mdrun */
        const char               *simulation_tpr,    /* This tpr will be simulated */
        int                       nranks,            /* Number of ranks to use */
        int                       npmeSetting,       /* Number of PME ranks to use */
        const t_eligible_gpu_ids *gpu_ids)           /* Struct containing GPU IDs for
                                                      * constructing mdrun command lines */
{
    char  *command, *argumentsForGpuIds;


    /* Make enough space for the system call command,
     * (200 extra chars for -npme ... etc. options should suffice): */
    snew(command, strlen(mpirunNameToUse)+strlen(mdrunNameToUse)+strlen(argumentsForRanks)+strlen(argumentsForMdrun)+strlen(simulation_tpr)+200);

    argumentsForGpuIds = make_gpu_id_command_line(nranks, npmeSetting, gpu_ids);

    /* Note that the -passall options requires argumentsForMdrun to be at the end
     * of the command line string */
    if (bUseThreadMpi)
    {
        sprintf(command, "%s%s-npme %d -s %s %s %s",
                mdrunNameToUse, argumentsForRanks, npmeSetting, simulation_tpr, argumentsForMdrun, argumentsForGpuIds);
    }
    else
    {
        sprintf(command, "%s%s%s -npme %d -s %s %s %s",
                mpirunNameToUse, argumentsForRanks, mdrunNameToUse, npmeSetting, simulation_tpr, argumentsForMdrun, argumentsForGpuIds);
    }
    ...
    if (bLaunch)
    {
        ...
        printSeparatorLine(stdout);
        fflush(stdout);
        gmx_system_call(command);
    }
}

//! Produce a .tpr file for the optimal run conditions
...
/*! \brief Make additional .tpr files with more computational load for the
 * direct-space ranks. */
...
/*! \brief Clean up after benchmark runs. Rename the files we want to
 * keep to some meaningful filename and delete the rest */
             ...
             int npmeSetting, int nr, gmx_bool bKeepStderr)
{
    ...
    for (i = 0; i < nfile; i++)
    {
        ...
        else if (strcmp(opt, "-bg") == 0)
        {
            ...
            sprintf(newfilename, "%s_no%d_np%d_npme%d%s", opt2fn("-bg", nfile, fnm), k, nranks, npmeSetting, numstring);
        ...
        else if (strcmp(opt, "-err") == 0)
        {
            ...
            sprintf(newfilename, "%s_no%d_np%d_npme%d%s", fn, k, nranks, npmeSetting, numstring);
            if (gmx_fexist(fn))
            {
                if (bKeepStderr)
                {
...
void make_npme_list(
        ...
        int        *nentries,       /* Number of entries we put in the npmeSettings list */
        int        *npmeSettings[], /* Each entry contains the value for -npme          */
        int         nranks,         /* Total number of ranks to do the tests on         */
        int         minPmeRanks,    /* Minimum number of PME ranks                      */
        int         maxPmeRanks)    /* Maximum number of PME ranks                      */
{
    int i, npme, npp;
    ...
    GMX_RELEASE_ASSERT(npmevalues_opt, "Option string for -npme was NULL");
    ...
    if (nranks > 2)
    {
        nlistmax = maxPmeRanks - minPmeRanks + 3;
        if (0 == minPmeRanks)
        {
            nlistmax--;
        }
    ...
    for (i = 0; i < nlistmax - 2; i++)
    {
        ...
        switch (eNPME)
        {
            ...
            case eNpmeSubset:
                 ...
                 * root of the number of PP ranks */
        ...
        {
            (*npmeSettings)[nlist] = npme;
            nlist++;
        }
    }
    /* We always test 0 PME ranks and the automatic number */
    *nentries                = nlist + 2;
    (*npmeSettings)[nlist  ] =  0;
    (*npmeSettings)[nlist+1] = -1;

    fprintf(stderr, "Will try the following %d different values for -npme:\n", *nentries);
    for (i = 0; i < *nentries-1; i++)
    {
        fprintf(stderr, "%d, ", (*npmeSettings)[i]);
    }
    fprintf(stderr, "and %d (auto).\n", (*npmeSettings)[*nentries-1]);
}
...
void init_perfdata(t_perf *perfdata[], int ntprs, int datasets, int repeats)
{
    ...
    for (k = 0; k < ntprs; k++)
    {
        ...
        for (i = 0; i < datasets; i++)
        {
            for (j = 0; j < repeats; j++)
            {
                snew(perfdata[k][i].numGigaCycles, repeats);
                snew(perfdata[k][i].nanoSecondsPerDay, repeats);
                snew(perfdata[k][i].pmeForceLoad, repeats);
            }
        }
    }
}
...
/*! \brief Check for errors when running a particular .tpr file
 *
 * \todo This check is really for a functional .tpr, and if we need it,
 * it should take place for every .tpr, and the logic for it should be
 * immediately inside the loop over k, not in this inner loop.
 *
 * \todo -npme 0 is more likely to succeed at low parallelism than the
 * default of -npme -1, but is more likely to fail at the scaling
 * limit when the PP domains may be too small. "mpirun -np 1 mdrun" is
                       ...
                       const t_filenm *fnm, int nfile)
{
    ...
    sprintf(command, "%s -npme 0 -nsteps 1 -quiet", mdrunCommandLine);
...
//! Make all the calls to mdrun to measure performance
void do_the_tests(
        ...
        int                       minPmeRanks,       /* Min fraction of ranks to use for PME   */
        int                       fixedNpmeSetting,  /* If >= -1, test fixed number of PME
                                                      * ranks only                             */
        const char               *npmevalues_opt,    /* Which -npme values should be tested    */
        t_perf                  **perfdata,          /* Here the performace data is stored     */
        int                      *pmeentries,        /* Entries in the npmeSettings list       */
        int                       repeats,           /* Repeat each test this often            */
        int                       nranks,            /* Total number of ranks = nPP + nPME     */
        int                       nr_tprs,           /* Total number of tpr files to test      */
        gmx_bool                  bUseThreadMpi,     /* Whether to use thread-MPI              */
        char                     *mpirunNameToUse,   /* mpirun command string                  */
        char                     *argumentsForRanks, /* "-np", "-n", whatever mpirun needs     */
        char                     *mdrunNameToUse,    /* mdrun command string                   */
        char                     *argumentsForBench, /* arguments for mdrun in a string        */
                                                      ...
                                                      * constructing mdrun command lines */
{
    ...
    cmdline_length =  strlen(mpirunNameToUse)
        + strlen(argumentsForRanks)
        + strlen(mdrunNameToUse)
        + strlen(argumentsForBench)
    ...
    if (bUseThreadMpi)
    {
        sprintf(cmd_stub, "%s%s", mdrunNameToUse, argumentsForRanks);
    }
    else
    {
        sprintf(cmd_stub, "%s%s%s ", mpirunNameToUse, argumentsForRanks, mdrunNameToUse);
    }

    /* Create a list of numbers of PME ranks to test */
    if (fixedNpmeSetting < -1)
    {
        make_npme_list(npmevalues_opt, pmeentries, &npmeSettings,
                       nranks, minPmeRanks, maxPmeRanks);
    }
    else
    {
        *pmeentries  = 1;
        snew(npmeSettings, 1);
        npmeSettings[0] = fixedNpmeSetting;
    ...
    for (k = 0; k < nr_tprs; k++)
    {
        ...
        /* Loop over various numbers of PME ranks: */
        for (i = 0; i < *pmeentries; i++)
        {
            char *argumentsForGpuIds = NULL;

            pd = &perfdata[k][i];

            argumentsForGpuIds = make_gpu_id_command_line(nranks, npmeSettings[i], gpu_ids);

            /* Loop over the repeats for each scenario: */
            for (nr = 0; nr < repeats; nr++)
            {
                pd->npmeSetting = npmeSettings[i];

                /* Add -npme and -s to the command line and save it. Note that
                 * the -passall (if set) options requires argumentsForBench to be
                 * at the end of the command line string */
                snew(pd->mdrunCommandLine, cmdline_length);
                sprintf(pd->mdrunCommandLine, "%s-npme %d -s %s %s %s",
                        cmd_stub, pd->npmeSetting, tpr_names[k], argumentsForBench, argumentsForGpuIds);

                /* To prevent that all benchmarks fail due to a show-stopper argument
                 * on the mdrun command line, we make a quick check first */
                if (bFirst)
                {
                    make_sure_it_runs(pd->mdrunCommandLine, cmdline_length, fp, fnm, nfile);
                }
                bFirst = FALSE;
                ...
                sprintf(command, "%s 1> /dev/null 2>%s", pd->mdrunCommandLine, opt2fn("-err", nfile, fnm));
                fprintf(stdout, "%s\n", pd->mdrunCommandLine);
                gmx_system_call(command);
                ...
                ret = parse_logfile(opt2fn("-bg", nfile, fnm), opt2fn("-err", nfile, fnm),
                                    pd, nr, presteps, cpt_steps, nranks);
                ...
                if (-1 == pd->npmeSetting)
                {
                    sprintf(buf, "(%3d)", pd->numPmeRanksUsed);
                }
                else
                {
                    sprintf(buf, "     ");
                ...
                /* Nicer output */
                if (pd->pmeForceLoad[nr] > 0.0)
                {
                    sprintf(str_pmeForceLoad, "%12.3f", pd->pmeForceLoad[nr]);
                }
                else
                {
                    sprintf(str_pmeForceLoad, "%s", "         -  ");
                ...
                fprintf(fp, "%4d%s %12.3f %12.3f %s    %s", pd->npmeSetting,
                        buf, pd->numGigaCycles[nr], pd->nanoSecondsPerDay[nr], str_pmeForceLoad, ParseLog[ret]);
                ...
                cleanup(fnm, nfile, k, nranks, pd->npmeSetting, nr, ret == eParselogFatal);

                /* If the first run with this number of ranks already failed, do not try again: */
                if (pd->numGigaCycles[0] <= 0.0 && repeats > 1)
                {
...
//! Validate the user input to gmx tune-pme
void check_input(
        int             nranks,
        int             repeats,
        int            *ntprs,
        ...
        t_pargs        *pa)
{
    ...
    if ( (0 == strcmp(opt2fn("-cpi", nfile, fnm), opt2fn("-bcpo", nfile, fnm)) ) && (simulationPart > 1) )
    {
    ...
    if (nranks < 1)
    {
        gmx_fatal(FARGS, "Number of (thread-)MPI ranks must be a positive integer.");
    }
    ...
    /* If a fixed number of PME ranks is set we do rcoulomb and PME gird tuning
     * only. We need to check whether the requested number of PME-only ranks
     * makes sense. */
    if (fixedNpmeSetting > -1)
    {
        /* No more than 50% of all ranks can be assigned as PME-only ranks. */
        if (2*fixedNpmeSetting > nranks)
        {
            gmx_fatal(FARGS, "Cannot have more than %d PME-only ranks for a total of %d ranks (you chose %d).\n",
                      nranks/2, nranks, fixedNpmeSetting);
        }
        if ((fixedNpmeSetting > 0) && (5*fixedNpmeSetting < nranks))
        {
            fprintf(stderr, "WARNING: Only %g percent of the ranks are assigned as PME-only ranks.\n",
                    100.0*((real)fixedNpmeSetting / (real)nranks));
        }
...
//! Returns TRUE when "opt" is needed for the launch of the final simulation
...
void create_command_line_snippets(
        ...
        int       nfile,
        t_filenm  fnm[],
        char     *argumentsForBench[],  /* command line arguments for benchmark runs */
        char     *argumentsForLaunch[], /* command line arguments for simulation run */
        char      extra_args[])         /* Add this to the end of the command line */
{
    ...
    snew(*argumentsForBench, 1);
    snew(*argumentsForLaunch, 1);
    *argumentsForLaunch[0] = '\0';
    *argumentsForBench[0]  = '\0';
    ...
    if (presteps > 0)
    {
        ...
        add_to_string(argumentsForBench, strbuf);
    }
    /* These switches take effect only at launch time */
    if (FALSE == bAppendFiles)
    {
        add_to_string(argumentsForLaunch, "-noappend ");
    }
    if (bKeepAndNumCPT)
    {
        add_to_string(argumentsForLaunch, "-cpnum ");
    }
    if (bResetHWay)
    {
        add_to_string(argumentsForLaunch, "-resethway ");
    }
    ...
    for (i = 0; i < nfile; i++)
    {
        ...
        {
            add_to_string(argumentsForLaunch, strbuf);
        }
    }

    add_to_string(argumentsForBench, extra_args);
    add_to_string(argumentsForLaunch, extra_args);
}


//! Set the "set" flag for option `opt`
...
/*! \brief This routine inspects the tpr file and ...
...
float inspect_tpr(int nfile, t_filenm fnm[], real *rcoulomb)
{
    ...
    *rcoulomb = ir.rcoulomb;
    ...
    /* Return the estimate for the number of PME ranks */
    return pme_load_estimate(&mtop, &ir, state.box);
}

//! Couples some file options for complex simulation output cases
...
//! C-style main function for gmx tune-pme
int gmx_tune_pme(int argc, char *argv[])
{
    const char     *desc[] = {
        "For a given number of ranks ([TT]-np[tt] or [TT]-ntmpi[tt]), [THISMODULE] systematically",
        ...
        "[gmx-mdrun] and add [TT]-np[tt] (or [TT]-ntmpi[tt] as appropriate)",
        "for the number of ranks for the test. You can also add [TT]-r[tt]",
    ...
    int             maxPmeRanks, minPmeRanks;
    float           guessPmeRatio;                          /* guessed PME:PP ratio based on the tpr file */
    float           guessPmeRanks;
    int             fixedNpmeSetting     = -2;              /* If >= -1, use only this number
                                                             * of PME-only ranks                */
    int             ntprs                = 0;
    ...
    gmx_int64_t     newNumSteps          = -1;  /* -1 indicates: not set by the user */
    ...
    int             bestNpmeSetting, bestTprFileIndex;
    int             simulationPart = 1; /* For benchmarks with checkpoint files */
    char            bbuf[STRLEN];


    /* Default program names if nothing else is found */
    char               *mpirunNameToUse = NULL, *mdrunNameToUse = NULL;
    char               *argumentsForBench, *argumentsForLaunch;
    char               *argumentsForRanks           = NULL;

    /* IDs of GPUs that are eligible for computation */
    char               *eligibleGpuIds              = NULL;
    ...
    t_pargs      pa[] = {
        ...
        /* g_tune_pme options: */
        /***********************/
        { "-np",       FALSE, etINT,  {&nranks},
          "Number of MPI ranks to use in the tests (must be > 2 for separate PME ranks)" },
        { "-npstring", FALSE, etENUM, {procstring},
          "Use this string to specify the number of ranks to use with [TT]$MPIRUN[tt]"},
        { "-ntmpi",    FALSE, etINT,  {&nthreads},
          "Number of thread-MPI ranks to use in the tests (turns off the use of MPI and mpirun)"},
        ...
        { "-fix",      FALSE, etINT,  {&fixedNpmeSetting},
        ...
        { "-simsteps", FALSE, etINT64, {&newNumSteps},
        ...
        { "-gpu_id",   FALSE, etSTR,  {&eligibleGpuIds},
...
#define NFILE asize(fnm)
    ...
    if (opt2parg_bSet("-ntmpi", asize(pa), pa))
    {
        ...
        {
            fprintf(stderr, "WARNING: -npstring has no effect when using thread-MPI.\n");
        }
    ...
    {
        GMX_RELEASE_ASSERT(procstring[0], "Option string for -npstring was NULL");

        /* This string will be used for MPI runs and will appear after the
         * mpirun command. */
        if (strcmp(procstring[0], "none") != 0)
        {
            sprintf(bbuf, " %s %d ", procstring[0], nranks);
        }
        else
        {
            sprintf(bbuf, " ");
    ...
    if (opt2bSet("-cpi", NFILE, fnm))
    {
        ...
        read_checkpoint_simulation_part(opt2fn("-cpi", NFILE, fnm),
                                        &simulationPart, &cpt_steps, cr,
                                        FALSE, NFILE, fnm, NULL, NULL);
        sfree(cr);
        simulationPart++;
        /* simulationPart will now be 1 if no checkpoint file was found */
        if (simulationPart <= 1)
        {
    ...
    check_input(nranks, repeats, &ntprs, &rmin, rcoulomb, &rmax,
                maxPmeFraction, minPmeFraction, fixedNpmeSetting,
                bench_nsteps, fnm, NFILE, simulationPart, presteps,
                asize(pa), pa);
    /* Check any GPU IDs passed make sense, and fill the data structure for them */
    snew(gpu_ids, 1);
    parse_digits_from_plain_string(eligibleGpuIds, &gpu_ids->n, &gpu_ids->ids);

    /* Determine the maximum and minimum number of PME ranks to test,
     * the actual list of settings is build in do_the_tests(). */
    if ((nranks > 2) && (fixedNpmeSetting < -1))
    {
        if (0 == strcmp(npmevalues_opt[0], "auto"))
        {
            ...
            {
                /* More PME than PP work, probably we do not need separate PME ranks at all! */
                maxPmeRanks = nranks/2;
                minPmeRanks = nranks/2;
            }
            else
            {
                /* PME : PP load is in the range 0..1, let's test around the guess */
                guessPmeRanks = nranks/(1.0 + 1.0/guessPmeRatio);
                minPmeRanks   = (int) floor(0.7*guessPmeRanks);
                maxPmeRanks   = (int) ceil(1.6*guessPmeRanks);
        ...
        else
        {
            ...
            maxPmeRanks = (int) floor(maxPmeFraction*nranks);
            minPmeRanks = (int) floor(minPmeFraction*nranks);
            minPmeRanks = std::max(minPmeRanks, 0);
            fprintf(stdout, "Will try runs with %d ", minPmeRanks);
            if (maxPmeRanks != minPmeRanks)
            {
                fprintf(stdout, "- %d ", maxPmeRanks);
            }
    ...
    /* Get the commands we need to set up the runs from environment variables */
    get_program_paths(bUseThreadMpi, &mpirunNameToUse, &mdrunNameToUse);
    if (bBenchmark && repeats > 0)
    {
        check_mdrun_works(bUseThreadMpi, mpirunNameToUse, argumentsForRanks, mdrunNameToUse, NULL != eligibleGpuIds);
    }
    ...
    /********************************************************************************/
    /* Main loop over all scenarios we need to test: tpr files, PME ranks, repeats  */
    /********************************************************************************/
    snew(perfdata, ntprs);
    if (bBenchmark)
    {
        do_the_tests(fp, tpr_names, maxPmeRanks, minPmeRanks, fixedNpmeSetting, npmevalues_opt[0], perfdata, &pmeentries,
                     repeats, nranks, ntprs, bUseThreadMpi, mpirunNameToUse, argumentsForRanks, mdrunNameToUse,
                     argumentsForBench, fnm, NFILE, presteps, cpt_steps, gpu_ids);

        fprintf(fp, "\nTuning took%8.1f minutes.\n", (gmx_gettime()-seconds)/60.0);

        /* Analyse the results and give a suggestion for optimal settings: */
        bKeepTPR = analyze_data(fp, opt2fn("-p", NFILE, fnm), perfdata, nranks, ntprs, pmeentries,
                                repeats, info, &bestTprFileIndex, &bestNpmeSetting);
        ...
        else
        {
            ...
            modifyFinalTprSettings(bOverwrite ? (newNumSteps+cpt_steps) : info->originalNumSteps,
                                   info->originalInitStep, tpr_names[bestTprFileIndex], simulation_tpr);
        }
        ...
        launch_simulation(bLaunch, fp, bUseThreadMpi, mpirunNameToUse, argumentsForRanks, mdrunNameToUse,
                          argumentsForLaunch, simulation_tpr, nranks, bestNpmeSetting, gpu_ids);
    }
    gmx_ffclose(fp);
