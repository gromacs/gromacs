...
//! Somehow Doxygen can't recognize the docs in the header file for this function.
void fillTopologyInformationFromInputFile(const char          *filename,
                                          TopologyInformation *topInfo)
{
    GMX_RELEASE_ASSERT(topInfo != nullptr, "Must have a topInfo to fill");

    topInfo->mtop_ = gmx::compat::make_unique<gmx_mtop_t>();
    readConfAndTopology(filename, &topInfo->bTop_, topInfo->mtop_.get(),
                        &topInfo->ePBC_, &topInfo->xtop_, nullptr,
                        topInfo->boxtop_);
    // TODO: Only load this here if the tool actually needs it; selections
    // take care of themselves.
    for (gmx_moltype_t &moltype : topInfo->mtop_->moltype)
    {
