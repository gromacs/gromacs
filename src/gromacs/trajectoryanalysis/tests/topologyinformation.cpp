...
TEST(TopologyInformation, CantWorkWithoutReadingAFile)
{
    ...
    auto releasedAtoms = topInfo.releaseAtoms();
    EXPECT_TRUE(releasedAtoms);
    ASSERT_TRUE(topInfo.atoms());
    EXPECT_NE(releasedAtoms.get(), topInfo.atoms());
    EXPECT_EQ(0, topInfo.atoms()->nr);
...
void runCommonTests(const TopologyInformation &topInfo, const int numAtoms)
{
    ...
    auto releasedAtoms = topInfo.releaseAtoms();
    EXPECT_TRUE(releasedAtoms);
    // Another released t_atoms must be different pointer to a deep copy.
    EXPECT_NE(releasedAtoms.get(), topInfo.releaseAtoms().get());
    ASSERT_TRUE(topInfo.atoms());
    // A normal t_atoms * must be different pointer to a deep copy.
    EXPECT_NE(releasedAtoms.get(), topInfo.atoms());
...
TEST(TopologyInformation, WorksWithGroFile)
{
    ...
    topInfo.fillFromInputFile(fileManager.getInputFilePath("lysozyme.gro"));
    EXPECT_FALSE(topInfo.hasFullTopology());
    runCommonTests(topInfo, numAtoms);
    EXPECT_EQ(-1, topInfo.ePBC());

    // Check the per-atom data
    ASSERT_EQ(numAtoms, topInfo.atoms()->nr);
    EXPECT_TRUE(topInfo.atoms()->haveMass);
    // TODO atommass.dat assumes united atom CA, which is probably not expected behaviour
    EXPECT_FLOAT_EQ(13.019, topInfo.atoms()->atom[26].m);
    EXPECT_FALSE(topInfo.atoms()->haveCharge);
    EXPECT_FALSE(topInfo.atoms()->haveType);
    EXPECT_EQ(0, topInfo.atoms()->atom[26].type);
    EXPECT_EQ(0, topInfo.atoms()->atom[26].atomnumber);
    EXPECT_EQ(1, topInfo.atoms()->atom[26].resind);
    // gro files don't have the information that pdb files might
    EXPECT_FALSE(topInfo.atoms()->havePdbInfo);
    EXPECT_FALSE(topInfo.atoms()->pdbinfo);
    EXPECT_EQ(10, topInfo.atoms()->nres);
    ASSERT_TRUE(topInfo.atoms()->resinfo);
    ASSERT_TRUE(topInfo.atoms()->resinfo[4].name);
    EXPECT_STREQ("ARG", *topInfo.atoms()->resinfo[4].name);
    EXPECT_EQ(5, topInfo.atoms()->resinfo[4].nr);
    EXPECT_EQ(0, topInfo.atoms()->resinfo[4].chainnum);
    EXPECT_EQ(' ', topInfo.atoms()->resinfo[4].chainid);
}
...
TEST(TopologyInformation, WorksWithPdbFile)
{
    ...
    TopologyInformation topInfo;
    TestFileManager     fileManager;
    topInfo.fillFromInputFile(fileManager.getInputFilePath("lysozyme.pdb"));
    EXPECT_FALSE(topInfo.hasFullTopology());
    runCommonTests(topInfo, numAtoms);
    ...
    EXPECT_TRUE(topInfo.atoms()->havePdbInfo);
    ASSERT_TRUE(topInfo.atoms()->pdbinfo);
    EXPECT_EQ(10, topInfo.atoms()->nres);
    ASSERT_TRUE(topInfo.atoms()->resinfo);
    ASSERT_TRUE(topInfo.atoms()->resinfo[4].name);
    EXPECT_STREQ("ARG", *topInfo.atoms()->resinfo[4].name);
    EXPECT_EQ(5, topInfo.atoms()->resinfo[4].nr);
    EXPECT_EQ(0, topInfo.atoms()->resinfo[4].chainnum);
    EXPECT_EQ('B', topInfo.atoms()->resinfo[4].chainid);
}
...
TEST(TopologyInformation, WorksWithTprFromPdbFile)
{
    ...
    std::string tprName = fileManager.getTemporaryFilePath(name + ".tpr");
    {
        ...
        caller.addOption("-p", fileManager.getInputFilePath(name));
        caller.addOption("-c", fileManager.getInputFilePath(name + ".pdb"));
    ...
    ASSERT_EQ(numAtoms, topInfo.atoms()->nr);
    EXPECT_TRUE(topInfo.atoms()->haveMass);
    EXPECT_FLOAT_EQ(12.011, topInfo.atoms()->atom[26].m);
    EXPECT_TRUE(topInfo.atoms()->haveCharge);
    EXPECT_TRUE(topInfo.atoms()->haveType);
    EXPECT_EQ(2, topInfo.atoms()->atom[26].type);
    EXPECT_EQ(6, topInfo.atoms()->atom[26].atomnumber);
    EXPECT_EQ(1, topInfo.atoms()->atom[26].resind);
    ...
    EXPECT_EQ(0, topInfo.atoms()->resinfo[4].chainid);
}
