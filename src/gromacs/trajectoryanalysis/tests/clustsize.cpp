...
class ClustsizeTest : public CommandLineTestBase
{
    public:
        ...
        void runTest(const char *option, const char *filename, bool mol,
                     bool cutoff)
        {
            ...
            if (mol)
            {
                ...
                const char *const flags[] = { "-mol" };
                CommandLine       args    = CommandLine(flags);
                cmdline.merge(args);
            }
            else
            {
            ...
            if (cutoff)
            {
                const char *const flags[] = { "-cut", "0.3" };
                CommandLine       args    = CommandLine(flags);
                cmdline.merge(args);
            }
...
{
    runTest("-mc", "maxclust.xvg", false, false);
}

TEST_F(ClustsizeTest, NClust)
{
    runTest("-nc", "nclust.xvg", false, false);
}

TEST_F(ClustsizeTest, AvClust)
{
    runTest("-ac", "avclust.xvg", false, false);
}

TEST_F(ClustsizeTest, HistoClust)
{
    runTest("-hc", "histoclust.xvg", false, false);
}

TEST_F(ClustsizeTest, MaxClustCutoff)
{
    runTest("-mc", "maxclust.xvg", false, true);
}

TEST_F(ClustsizeTest, NClustCutoff)
{
    runTest("-nc", "nclust.xvg", false, true);
}

TEST_F(ClustsizeTest, AvClustCutoff)
{
    runTest("-ac", "avclust.xvg", false, true);
}

TEST_F(ClustsizeTest, HistoClustCutoff)
{
    runTest("-hc", "histoclust.xvg", false, true);
}

TEST_F(ClustsizeTest, MaxClustMol)
{
    runTest("-mc", "maxclust.xvg", true, false);
}

TEST_F(ClustsizeTest, NClustMol)
{
    runTest("-nc", "nclust.xvg", true, false);
}

TEST_F(ClustsizeTest, AvClustMol)
{
    runTest("-ac", "avclust.xvg", true, false);
}

TEST_F(ClustsizeTest, HistoClustMol)
{
    runTest("-hc", "histoclust.xvg", true, false);
}
