...
TEST_F(DosModuleTest, ComputesFD)
{
    ...
    EXPECT_REAL_EQ_TOL(-1.98, FD(100, 0.01), tolerance);
...
TEST_F(DosModuleTest, ComputesYYY)
{
    gmx::test::FloatingPointTolerance tolerance(
            gmx::test::defaultRealTolerance());
    EXPECT_REAL_EQ_TOL(-1.9501997, YYY(0.01, 0.5), tolerance);
...
TEST_F(DosModuleTest, ComputesFluidicity)
{
    ...
    double toler = 1e-4;
    EXPECT_REAL_EQ_TOL(0.5411987, calc_fluidicity(1, toler), tolerance);
