...
class Fma : public TrajectoryAnalysisModule
{
    public:
        ...
        using MatrixClass      = Pls2dArray;
        /** \brief The matrix index datatype to be used in pls_denham */
        using MatrixClassIndex = Pls2dArray::index_type;
        /** \brief The allocator datatype */
    ...
    private:
        ...
        rvec                                   *refstruct_;
        t_atoms                                 refatoms_;
        std::vector<real, AllocatorClass>       fitmass_;
        std::vector<int>                        fitindices_;
        std::vector<int>                        anaindices_;

        real vmean(const real *x, int n, int step);
        real corrcoeff(const std::vector<real, AllocatorClass> &x, const std::vector<real, AllocatorClass> &y, int n, int xstep, int ystep);
                  ...
                  const TopologyInformation         &top)
{
    ...
    fitindices_.resize(fitsel_.atomCount());
    ...
    anaindices_.resize(anasel_.atomCount());
    ...
    reset_x(ifit, &fitindices_.front(),
            total_n_atoms, NULL,
            refstruct_, fitmass_.data());

    nSamples_ = read_xvg(fnY_.c_str(), &ydata, &dimY_);

    y_.initArray(Fma::MatrixClassIndex(0), Fma::MatrixClassIndex(dimY_-1),
    ...
    fframes_.initArray(Fma::MatrixClassIndex(0), Fma::MatrixClassIndex(nTrain_+nValidate_ - 1),
                  ...
                  TrajectoryAnalysisModuleData * /*pdata*/)
{
    ...
    if (keepFrame)
    {
        ...
        do_fit(total_n_atoms, fitmass_.data(), refstruct_, fr.x);
...
Fma::finishAnalysis(int /*nframes*/)
{
    ...
    w_.initArray(Fma::MatrixClassIndex(0), Fma::MatrixClassIndex(anasel_.atomCount() * DIM-1),
                 Fma::MatrixClassIndex(0), Fma::MatrixClassIndex(nPLSvec_-1));
    q_.resize(nPLSvec_);

    pls_denham(fframes_, yTrain_, nTrain_, fframes_.getLength2(), nPLSvec_, w_, q_);
 ...
 */
real Fma::vmean(const real *x, int n, int step)
{
    int  i;
    real m = 0.0;
 ...
 */
real Fma::corrcoeff(const std::vector<real, AllocatorClass> &x, const std::vector<real, AllocatorClass> &y, int n, int xstep, int ystep)
{
    int  i;
    real cxy = 0;
    real cxx = 0;
