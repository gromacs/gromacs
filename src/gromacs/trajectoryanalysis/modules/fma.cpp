...
class Fma : public TrajectoryAnalysisModule
{
    public:
        ...
        using MatrixClass      = typename gmx::IrregArray2D<real, typename gmx::SimdSetup<real>::allocator_type>;
        using MatrixClassIndex = typename gmx::IrregArray2D<real, typename gmx::SimdSetup<real>::allocator_type>::index_type;
        using AllocatorClass   = typename gmx::SimdSetup<real>::allocator_type;
    private:
        class ModuleData;

        /*! \brief File Name for saving FMA result vectors */
        std::string                             fnFMAvec_;
        /*! \brief File Name for saving enesemble-weighted FMA result vectors */
        ...
        bool                                    mwfit_;
        bool                                    optDim_;
        std::unique_ptr<MatrixClass>            y_;
        std::vector<real, AllocatorClass>       yTrain_;
        std::vector<real, AllocatorClass>       yValidate_;
        real                                    avgy_;
        int                                     dimY_;
        int                                     nSamples_;
        int                                     nFrames_;
        ...
        int                                     total_n_atoms;
        int                                     nModeFrames_;
        std::unique_ptr<MatrixClass>            fframes_;
        std::vector<real, AllocatorClass>       times_;
        std::vector<int>                        frameOrder_;
        std::vector<real, AllocatorClass>       avg_;
        std::unique_ptr<MatrixClass>            w_;
        std::vector<real, AllocatorClass>       q_;
        Selection                               fitsel_;
        Selection                               anasel_;
        rvec                                   *refstruct_;
        t_atoms                                 refatoms_;
        std::vector<real, AllocatorClass>       fitmass;
        std::vector<int>                        fitindices;
        std::vector<int>                        anaindices;

        real vnorm(real *x, int n);
        real vmean(real *x, int n, int step);
        real corrcoeff(std::vector<real, AllocatorClass> x, std::vector<real, AllocatorClass> y, int n, int xstep, int ystep);
                 ...
                 TrajectoryAnalysisSettings *settings)
{
    ...
    options->addOption(FileNameOption("v")
                           ...
                           .defaultType(efTRR).description("FMA result vectors"));
                  ...
                  const TopologyInformation         &top)
{
    ...
    reset_x(ifit, &fitindices.front(),
            total_n_atoms, NULL,
            refstruct_, fitmass.data());

    nSamples_ = read_xvg(fnY_.c_str(), &ydata, &dimY_);

    y_.reset(new Fma::MatrixClass(Fma::MatrixClassIndex(0), Fma::MatrixClassIndex(dimY_-1),
                                  Fma::MatrixClassIndex(0), Fma::MatrixClassIndex(nSamples_-1)));
    ...
    fframes_.reset(new Fma::MatrixClass(Fma::MatrixClassIndex(0), Fma::MatrixClassIndex(nTrain_+nValidate_ - 1),
                                        Fma::MatrixClassIndex(0), Fma::MatrixClassIndex(DIM * anasel_.atomCount() - 1)));
                  ...
                  TrajectoryAnalysisModuleData * /*pdata*/)
{
    ...
    if (keepFrame)
    {
        ...
        do_fit(total_n_atoms, fitmass.data(), refstruct_, fr.x);
...
Fma::finishAnalysis(int /*nframes*/)
{
    ...
    if (optDim_)
    {
        ...
        for (l = 1; l <= nPLSvec_; l++)
        {
            Fma::MatrixClass w_l(Fma::MatrixClassIndex(0), Fma::MatrixClassIndex(anasel_.atomCount() * DIM -1),
                                 Fma::MatrixClassIndex(0), Fma::MatrixClassIndex(l-1));
            q_.resize(l);

            std::vector<real, Fma::AllocatorClass> PLSvec(anasel_.atomCount()*DIM);
            std::vector<real, Fma::AllocatorClass> mYt(nTrain_);
            std::vector<real, Fma::AllocatorClass> mYv(nValidate_);

            pls_denham(*fframes_, yTrain_, fframes_->getLength1(), nTrain_, l, &w_l, &q_);
            for (i = 0; i < anasel_.atomCount(); i++)
            {
                for (j = 0; j < DIM; j++)
                {
    ...
    w_.reset(new Fma::MatrixClass(Fma::MatrixClassIndex(0), Fma::MatrixClassIndex(anasel_.atomCount() * DIM-1),
                                  Fma::MatrixClassIndex(0), Fma::MatrixClassIndex(nPLSvec_-1)));
    q_.resize(nPLSvec_);

    pls_denham(*fframes_, yTrain_, fframes_->getLength1(), nTrain_, nPLSvec_, w_.get(), &q_);
...
Fma::writeOutput()
{
    ...
    real              minProjectionEw            = GMX_REAL_MAX;
    real              maxProjectionEw            = GMX_REAL_MIN;
    real              projEw                     = 0.0;
    real              v2   = 0.0;
    real              vEw2 = 0.0;
    ...
    std::vector<std::vector<real, Fma::AllocatorClass> > modelY(2, std::vector<real, Fma::AllocatorClass> (nSamples_));
    ...
    outputfile   = gmx_trr_open(fnFMAvec_.c_str(), "w");
    outputfileEw = gmx_trr_open(fnFMAvecEw_.c_str(), "w");
    ...
    gmx_trr_write_frame(outputfileEw, 0, 0.0, lambda,
                        box, anasel_.atomCount(), structure, NULL, NULL);
    ...
    for (k = 0; k < nPLSvec_; k++)
    {
        ...
        gmx_trr_write_frame(outputfileEw, k+1, q_[k], lambda,
                            box, anasel_.atomCount(), structure, NULL, NULL);
    }


    gmx_trr_close(outputfile);
    gmx_trr_close(outputfileEw);
    ...
    for (i = 0; i < nSamples_; i++)
    {
        ...
        for (j = 0; j < anasel_.atomCount()*DIM; j++)
        {
            ...
            projEw       += q_[0] * (*w_)(j, 0) * ((*fframes_)(frameOrder_[i], j));
        }
        if (frameOrder_[i] < nTrain_)
        {
        ...
        if (projEw > maxProjectionEw)
        {
            maxProjectionEw = projEw;
        }

        if (projEw < minProjectionEw)
        {
            minProjectionEw = projEw;
        }
    ...
    for (k = 0; k < nModeFrames_; k++)
    {
        for (i = 0; i < anasel_.atomCount(); i++)
        {
            for (j = 0; j < DIM; j++)
            {
                refstruct_[anasel_.atomIndices()[i]][j] = avg_[i*DIM + j] + (minProjectionEw + (maxProjectionEw-minProjectionEw)/nModeFrames_*k - avgy_) / vEw2 * q_[0] * (*w_)((i*DIM + j), 0);
            }
        }
 ...
 */
real Fma::corrcoeff(std::vector<real, AllocatorClass> x, std::vector<real, AllocatorClass> y, int n, int xstep, int ystep)
{
    int  i;
    real cxy = 0;
    real cxx = 0;
    ...
    mx = vmean(x.data(), n, xstep);
    my = vmean(y.data(), n, ystep);
