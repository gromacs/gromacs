...
class Fma : public TrajectoryAnalysisModule
{
    public:
        ...
        /** \brief The datatype to use to represent matrix size/index */
        using MatrixClassIndex = MatrixClass::index_type;
    private:
        class ModuleData;
        ...
        real corrcoeff(const VectorClass &x, const VectorClass &y, int n, int xstep, int ystep);
...
Fma::finishAnalysis(int /*nframes*/)
{
    ...
    if (optDim_)
    {
        ...
        for (l = 1; l <= nPLSvec_; l++)
        {
            ...
            Fma::MatrixClass X(Fma::MatrixClassIndex(0), Fma::MatrixClassIndex(nTrain_ - 1),
                               Fma::MatrixClassIndex(0), Fma::MatrixClassIndex(DIM * anasel_.atomCount() - 1));
            for (Fma::MatrixClass::size_type idx_row = 0; idx_row < X.getLength1(); ++idx_row)
            {
                for (Fma::MatrixClass::size_type idx_col = 0; idx_col < X.getLength2(); ++idx_col)
                {
                    X(idx_row, idx_col) = fframes_(idx_row, idx_col);
                }
            }

            pls.pls_denham(X, yTrain_, nTrain_, fframes_.getLength2(), l, &w_l, &q_);
            for (i = 0; i < anasel_.atomCount(); i++)
            {
                for (j = 0; j < DIM; j++)
                {
    ...
    pls.pls_denham(X, yTrain_, nTrain_, fframes_.getLength2(), nPLSvec_, &w_, &q_);
 ...
 */
real Fma::corrcoeff(const VectorClass &x, const VectorClass &y, int n, int xstep, int ystep)
{
    real       cxy = 0;
    real       cxx = 0;
    real       cyy = 0;

    const real mx = vmean(x.data(), n, xstep);
    const real my = vmean(y.data(), n, ystep);
