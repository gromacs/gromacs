...
class Fma : public TrajectoryAnalysisModule
{
    ...
    private:
        ...
        real corrcoeff(FVector x, FVector y, int n, int xstep, int ystep);
                  ...
                  TrajectoryAnalysisModuleData * /*pdata*/)
{
    ...
    if (keepFrame)
    {
        ...
        do_fit(total_n_atoms, fitmass.toF(), refstruct_, fr.x);
...
Fma::finishAnalysis(int /*nframes*/)
{
    ...
    if (optDim_)
    {
        ...
        {
            w_.resize(anasel_.atomCount() * DIM, l);
            q_.resize(l);

            FVector PLSvec(anasel_.atomCount()*DIM);
            FVector mYt(nTrain_);
            FVector mYv(nValidate_);

            pls_denham(&fframes_, &yTrain_, nTrain_, l, &w_, &q_);
            for (i = 0; i < anasel_.atomCount(); i++)
            {
                for (j = 0; j < DIM; j++)
                {
    ...
    pls_denham(&fframes_, &yTrain_, nTrain_, nPLSvec_, &w_, &q_);
 ...
 */
real Fma::corrcoeff(FVector x, FVector y, int n, int xstep, int ystep)
{
    int  i;
    real cxy = 0;
    real cxx = 0;
    ...
    mx = vmean(x.toF(), n, xstep);
    my = vmean(y.toF(), n, ystep);
