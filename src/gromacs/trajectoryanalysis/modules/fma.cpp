 ...
 * the preset GROMACS precision (i.e. double or single precision)
 */
#ifdef GMX_DOUBLE
...
class Fma : public TrajectoryAnalysisModule
{
    ...
    private:
        ...
        real corrcoeff(real *x, real *y, int n, int xstep, int ystep);
        void pls_denham(real *x, real *y, int n, int k, int ln, int a, real *w, real *q);
    ...
    if (keepFrame)
    {
        ...
        for (i = 0; i < anasel_.atomCount(); i++)
        {
            for (j = 0; j < DIM; j++)
            {
                ...
                if (useForAverage)
                {
                    avg_[(DIM*i) + j] += frames_[frameNumber  + ((DIM*i) + j) * (nTrain_ + nValidate_)]/real(nTrain_);
                }
            }
        }
    }
 ...
 */
real Fma::corrcoeff(real *x, real *y, int n, int xstep, int ystep)
{
    int  i;
    real cxy = 0;
    real cxx = 0;
 ...
 */
void Fma::pls_denham(real *x, real *y, int n, int k, int ln, int a, real *w, real *q)
{
    ...
    /*   w_1 = x' * y*/
    GMX_LAPACK(gemv, GEMV) (    &T, &n, &k, &c_done, x, &ln, y, &c_ione, &c_dzero, w, &c_ione);

    /* t_1 = x * w_1 */
    GMX_LAPACK(gemv, GEMV) (&N, &n, &k, &c_done, x, &ln, w, &c_ione, &c_dzero, t, &c_ione);

    GMX_LAPACK(copy, COPY) (&n, t, &c_ione, qrt, &c_ione);
    GMX_LAPACK(copy, COPY) (&n, y, &c_ione, rsd, &c_ione);

    /* perform regression
       min(rsd) abs(y - qrt * rsd) */
    GMX_LAPACK(gels, GELS) (    &N,      &n, &c_ione, &c_ione, qrt,  &n,  rsd,  &n,  dum, &lwork, &info );
    ...
    GMX_LAPACK(gemv, GEMV) (    &N,      &n, &c_ione, &c_dnegone,   t,      &n,     q, &c_ione, &c_done, rsd, &c_ione);


    for (i = 1; i < a; i++)
    {
        /* w_i := X' * rsd */
        GMX_LAPACK(gemv, GEMV) (&T, &n, &k, &c_done, x, &ln, rsd, &c_ione, &c_dzero, &w[i*k], &c_ione);
        /* t_i := X * w_i */
        GMX_LAPACK(gemv, GEMV) (&N, &n, &k, &c_done, x, &ln, &w[i*k], &c_ione, &c_dzero, &t[n*i], &c_ione);

        idum = n*a;
        GMX_LAPACK(copy, COPY) (&idum, t, &c_ione, qrt, &c_ione);
        GMX_LAPACK(copy, COPY) (&n, y, &c_ione, rsd, &c_ione);

        idum = i + 1;
        /* run regression */
        GMX_LAPACK(gels, GELS) ( &N, &n, &idum, &c_ione, qrt,  &n, rsd,  &n,  dum, &lwork, &info );

        GMX_LAPACK(copy, COPY) (&a,     rsd, &c_ione,     q, &c_ione);
        ...
        /* calculate residuals rsd := y - t_1*q_1
           using DGEMV          (y := alpha*A*x + beta*y) */
        idum = i + 1;
        GMX_LAPACK(gemv, GEMV) (    &N,      &n, &idum, &c_dnegone,   t,      &n,     q, &c_ione, &c_done, rsd, &c_ione);

    }

    GMX_LAPACK(gemv, GEMV) (  &N, &k, &a, &c_done,      w, &k, q, &c_ione, &c_dzero,   b, &c_ione);
}
