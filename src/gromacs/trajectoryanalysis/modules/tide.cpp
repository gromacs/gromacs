...
//! \brief TODO
float calc_sum_rhoA_rhoB_mask(const std::vector<float> &vec1,
...
//! \brief TODO
float global_correlation_coeff(const std::vector<real> &map1,
                               const std::vector<real> &map2,
                               const Mask              &mask)
{
...
class TimeAveragedDensity final : public TrajectoryAnalysisModule
{
    ...
    private:
        ...
        void writeMaskedMap(const std::string &inputFileName, const GridDataReal3D &map,
                            const Mask &mask);
 ...
 */
void TimeAveragedDensity::writeMaskedMap(const std::string &inputFileName, const GridDataReal3D &map,
                                         const Mask &mask)
{
    if (inputFileName.empty())
    {
        fprintf( stderr, "No filename given for X-PLOR output file, not outputting densities\n");
        return;
    }

    XplorData xplorOutput = xplorFromGridData(map, referenceCell.lattice().extend());
                                      ...
                                      TrajectoryAnalysisSettings *settings)
{
    ...
    options->addOption(IntegerOption("size")
                           .store(mapDimensions.nGridPoints.data())
                           .vector()
                           .description("(NA,NB,NC) = total number of grid "
                                        "points along each cell edge"));

    options->addOption(IntegerOption("min")
                           .store(mapDimensions.Nmin.data())
                           .vector()
        ...
        const TopologyInformation                   &top)
{
    ...
    // create grid from reference map if provided, alternatively from  -size, -min, -max, -box options
    XplorData xplorData;
    if (!fnInputMap_.empty())
    {
        fprintf(stderr, "=== Reading reference map, ignoring -size, -min, -max, and -box\n");
        FILE     *fp = gmx_ffopen(fnInputMap_.c_str(), "r");
        xplorData = XplorData(fp);
        fclose(fp);
    }
    else
    {
        const float orthogonalDegAngle  = 90;
        xplorData.crs_start   = mapDimensions.Nmin;
        xplorData.crs_end     = mapDimensions.Nmax;
        xplorData.extend      = mapDimensions.nGridPoints;
        xplorData.cell_length = mapDimensions.box;
        xplorData.cell_angles = {orthogonalDegAngle, orthogonalDegAngle, orthogonalDegAngle};
    }
    map_ref       = gridDataFromXplor(xplorData);
    referenceCell = cellFromXplor(xplorData);

    map_av.setGrid(map_ref.getGrid().duplicate());
    map_sd.setGrid(map_av.getGrid().duplicate());
    map_block.setGrid(map_av.getGrid().duplicate());

    /* read mask map if provided */
    if (!fnMaskMap_.empty())
    {
        printf("=== Reading mask map\n");
        FILE     *fp = gmx_ffopen(fnMaskMap_.c_str(), "r");
        mask_ = Mask(gridDataFromXplor(XplorData(fp)), mask_cutoff);
        fclose(fp);
    }
...
void TimeAveragedDensity::analyzeFrame(int frnr, const t_trxframe &fr, t_pbc * /*pbc*/, TrajectoryAnalysisModuleData *pdata)
{
    ...
    for (int atomIndex = 0; atomIndex < sel.atomCount(); atomIndex++)
    {
        ...
        const auto &gaussianParameter = gaussianParametersPerAtomType.at(atomName);
    ...
    DensitySpreader spreader(map_av.getGrid(), 1, 6, 1.);
    ...
    for (const auto &para : atomIndexPerParameter)
    {
        if (para.first.amplitude > 0 && para.first.precision > 0)
        {
            ...
            spreader.spreadLocalAtoms(para.second, weights);
        }
    }
