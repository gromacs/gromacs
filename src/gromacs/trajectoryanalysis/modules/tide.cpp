...
//! \brief Set the Gaussian coefficients A and B
std::map < int, std::vector < GaussianParameters>> set_gaussian_coeff( std::string parameterFileName )
{
    // read gaussian parameters from file if provided, else use defaults
    FILE *parameterFile;
    if (!parameterFileName.empty())
    {
        fprintf(stdout, "Reading Gaussian coefficients from %s .\n", parameterFileName.c_str());
        parameterFile = fopen(parameterFileName.c_str(), "r");
    }
    else
    {
        fprintf(stdout, "Reading default Gaussian coefficients from %s\n", parameterFileName.c_str());
        parameterFile = libopen(parameterFileName.c_str());
    }
    ...
    while (get_a_line(parameterFile, line, 1000))
    {
        ...
        float amplitude;
        float precision;
        if (sscanf(line, " %d %e %e", &currentAtomNumber, &amplitude, &precision) == 3)
        {
            gaussianParametersLookup[currentAtomNumber].push_back({amplitude, precision});
        }
    }
...
class TimeAveragedDensity final : public TrajectoryAnalysisModule
{
    ...
    private:
        std::map < int, std::vector < GaussianParameters>> setGaussianCoeffFromSigma( );
...
std::map < int, std::vector < GaussianParameters>> TimeAveragedDensity::setGaussianCoeffFromSigma( )
{
    // read gaussian parameters from file if provided, else use defaults
    auto parameterFile = libopen("emcrosssections.dat");
    std::map < int, std::vector < GaussianParameters>> gaussianParametersLookup;
    char line[1000];
    // all non-header lines
    while (get_a_line(parameterFile, line, 1000))
        ...
        const TopologyInformation                   &top)
{
    ...
    if (sigmaIsSet_)
    {
        gaussianParametersPerAtomicNumber_ = setGaussianCoeffFromSigma();
    }
    else
    {
        gaussianParametersPerAtomicNumber_ = set_gaussian_coeff(fnGaussParameters_);
    }
    ...
    else
    {
        if (top.hasTopology())
        {
            fprintf(stderr, "\nWARNING: Guessing atomic number from atom names\n");
            ...
            for (int i = 0; i < atoms.nr; ++i)
            {
                ...
                if (atomicNumber < 0)
                {
                    fprintf(stderr, "\nWARNING: Cannot guess atomic number for '%s' - ignoring for spreading. Is this a vitual site?\n", *atoms.atomname[i]);
                }
        ...
        {
            fprintf(stderr, "\nWARNING: Cannot find topology information, setting all atom weights to unity\n");
        }
    }
...
void TimeAveragedDensity::analyzeFrame(int frnr, const t_trxframe &fr, t_pbc * /*pbc*/, TrajectoryAnalysisModuleData *pdata)
{
    ...
    for (int atomIndex = 0; atomIndex < sel.atomCount(); atomIndex++)
    {
        ...
        if (!atomicNumbersIndex_.empty())
        {
            ...
            {
                fprintf(stderr, "WARNING: Cannot determine atomic number of atom #%d. Falling back to atomic number 0.\n", sel.atomIndices()[atomIndex]);
            }
        }
        try
        {
        ...
        {
            fprintf(stderr, "WARNING: Cannot find atomic number %d in spread parameters.\n         Ignoring atom number %d from selection.\n", atomicNumber, sel.atomIndices()[atomIndex]);
        }
    }
...
void TimeAveragedDensity::writeOutput()
{
    ...
    if (!fnReferenceMap_.empty())
    {
        fprintf(stderr, "\nCorrelation coefficient between computed and reference maps: %f .\n",
