...
void subtract_maps(const t_mapdata &map1, const t_mapdata &map2,
                   t_mapdata *result, const t_mapdata &mask)
{
    // do subtraction if maps have the same dimensions, else abort with fatal
    // error
    if (!mapsHaveSameGrid(map1, map2) || !mapsHaveSameGrid(map1, *result) ||
    ...
    for (size_t l = 0; l < map1.vox.size(); l++)
    {
        if (mask.vox[l] > 0.5)
        {
            result->vox[l] = map2.vox[l] - map1.vox[l];
        }
        else
        {
            result->vox[l] = nanf(""); // masked point is not considered
        }
    }
}

//! \brief log of map: result = - ln ( rho(map) )  PSEUDO FREE ENERGY
void minus_log_map(const t_mapdata &map, t_mapdata *result,
                   const t_mapdata &mask)
{
    // do operation if maps have the same dimensions, else abort else abort with
    // fatal error
    if (!mapsHaveSameGrid(map, *result) || !mapsHaveSameGrid(map, mask))
    {
        gmx_fatal(FARGS, "Map dimensions must be identical");
    }
...
//! PSEUDO FREE ENERGY
void minus_log_division_maps(const t_mapdata &map1,
                             ...
                             const t_mapdata &mask)
{
    ...
    for (size_t l = 0; l < mask.vox.size(); l++)
    {
        if (mask.vox[l] > 0.5) // discard masked points
        {
            if (map1.vox[l] != 0)
            {
                result->vox[l] = -log(map2.vox[l] / map1.vox[l]);
            }
            else
            {
...
//! positions
void setMapMeanAndRms(t_mapdata *map, bool incl_zero, const t_mapdata &mask)
{
...
void local_correlation_coeff(const t_mapdata &map1, const t_mapdata &map2, const t_mapdata &mask,
...
void TimeAveragedDensity::finishAnalysis(int nframes)
{
    ...
    setMapMeanAndRms(&map_av, incl_zero, map_mask);
    setMapMeanAndRms(&map_sd, incl_zero, map_mask);

    // calculate the difference map_diff = map_av - map_ref to the reference map (if requested)
    if (!fnDifferenceMap_.empty())
    {
        subtract_maps(map_ref, map_av, &map_diff, map_mask);
        // include zero in the calculation of mean and rms
        setMapMeanAndRms( &map_diff, true, map_mask);
    }

    // calculate minus log (if requested)
    if (!mlog_.empty())
    {
        // if there is a ref map, map_mlog = - ln ( map_av /  map_ref )
        if (!fnInputMap_.empty())
        {
            minus_log_division_maps(map_ref, map_av, &map_mlog, map_mask);
        }
        // if no ref map, map_mlog = -ln (map_av)
        else
        {
            minus_log_map(map_av, &map_mlog, map_mask);
        }
        // do include zero in the calculation of mean and rms
        setMapMeanAndRms( &map_mlog, true, map_mask);
...
void TimeAveragedDensity::writeOutput()
{
    ...
    {
        print_to3dmap(fnMapAverage_, map_av, map_mask.vox);
    }

    // print the standard deviation to file 3dmap_sigma
    if (!fnMapStandardDeviation_.empty())
    {
        print_to3dmap(fnMapStandardDeviation_, map_sd, map_mask.vox);
    }

    // print the difference map to file diff
    if (!fnDifferenceMap_.empty())
    {
        print_to3dmap(fnDifferenceMap_, map_diff, map_mask.vox);
    }

    // print the difference map to file diff
    if (!mlog_.empty())
    {
        print_to3dmap(mlog_, map_mlog, map_mask.vox);
    }
    ...
    if (!loc_corr_.empty())
    {
        print_to3dmap(loc_corr_, map_loc_corr, map_mask.vox);
    }
}
