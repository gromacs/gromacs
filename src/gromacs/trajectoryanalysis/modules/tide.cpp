...
std::vector<std::string> attypeStringsFromTop(const t_topology &top)
{
...
void sd_map(const GridDataReal3D &averageMap_, GridDataReal3D *stddevMap_,
            float Nframes)
{
    if (Nframes > 1)
    {
...
class TimeAveragedDensity final : public TrajectoryAnalysisModule
{
    ...
    private:
        ...
        std::string                      fnLogRationMap_;
        std::string                      fnLocalCorrelationMap_;
        std::string                      fnCorrelation_;

        bool                             normalizeMaps_    = true;                                 // Normalization
        bool                             keepZero_         = false;                                // include zero-density values in the spatial average and stdev calculation (over the grid points)
        ...
        std::vector<std::string>         attypeString_;                                            // array will store the index of the
                                      ...
                                      TrajectoryAnalysisSettings *settings)
{
    static const char *const desc[] = {
        ...
        "atoms from a trajectory or structure."
        "The density is calculated as a linear combination of Gaussians:\n",
        "rho(R)=sum_{i=1}^{i=Ngaussians} Ai*exp(-Bi*R^2)\n",

        "=== Gaussian coefficients:\n",
        "By default, considers 2-Gaussian coefficients (A and B), derived "
        "from Electron Microscopy for H,C,N,O,P,S.\n",
        ...
        "-shiftlogmap will set the mean value of mlog to zero.\n"

        "-corr: Time-resolved global correlation between the computed and the "
        "reference map.\n",
        "The correlation is computed in time windows defined by -timeWindow_.\n",
        "-loc_corr: Local correlation between the computed and the reference map "
        "in XPLOR format. \n",
        "For a small cubic grid of dimensions (nSlide_,nSlide_,nSlide_), the "
        ...
        "gmx tide -f traj.xtc -s conf.gro -mi mi.xplor -nonorm -keepZero_ "
        ...
        "-keepZero_ -maskcutoff 1e-5 -select \'group \"Protein\"\' -map_av "
        "-diff\n",
        ...
        "gmx tide -f traj.xtc -s conf.gro -mi mi.xplor -nSlide_ 5 -select \'group "
        "\"Protein\"\' -loc_corr \n",
    ...
    options->addOption(FileNameOption("gaussparameters").filetype(eftGenericData)
                           .inputFile().store(&fnGaussParameters_)
    ...
    options->addOption(FileNameOption("averagemap").filetype(eftCCP4).outputFile()
    ...
    options->addOption(FileNameOption("logmap").filetype(eftCCP4).outputFile()
                           .store(&fnLogRationMap_).defaultBasename("logmap")
    ...
    options->addOption( BooleanOption("keepzero").store(&keepZero_)
                            .description("Keep zero density points in the calculation of the spatial mean and stdev, over the grid"));
        ...
        const TopologyInformation                   &top)
{
    ...
    {
        gaussianParametersPerAtomType_ = set_gaussian_coeff(fnGaussParameters_);
    }
    ...
    attypeString_                  = attypeStringsFromTop(*(top.topology()));
...
void TimeAveragedDensity::analyzeFrame(int frnr, const t_trxframe &fr, t_pbc * /*pbc*/, TrajectoryAnalysisModuleData *pdata)
{
    ...
    for (int atomIndex = 0; atomIndex < sel.atomCount(); atomIndex++)
    {
        ...
        std::string atomName =
            attypeString_[sel.atomIndices()[atomIndex]].substr(0, 1);
    ...
    if (!fnCorrelation_.empty())
    {
        if (fr.time / timeWindow_ == floor(fr.time / timeWindow_) && fr.time > 0)
        {
...
void TimeAveragedDensity::finishAnalysis(int nframes)
{
    ...
    sd_map(averageMap_, &stddevMap_, nframes);

    // calculate the difference map_diff = averageMap_ - referenceMap_ to the reference map (if requested)
    if (!fnDifferenceMap_.empty())
    {
        GridDataReal3D map_diff(averageMap_.getGrid());
        map_diff = containermeasure::subtract(referenceMap_, averageMap_);
    ...
    if (!fnLogRationMap_.empty())
    {
        GridDataReal3D map_mlog(averageMap_.getGrid());
        // with ref map, map_mlog = -ln( averageMap_ / referenceMap_ ), else -ln(averageMap_)
        if (!fnReferenceMap_.empty())
        {
            // TODO: mask division by zero first
            map_mlog = containermeasure::negLogRatio(referenceMap_, averageMap_);
        }
        else
        {
            // TODO: mask division by zero first
            map_mlog = containermeasure::negLog(averageMap_);
        }
        ...
        if (!fnLogRationMap_.empty())
        {
            writeMaskedMap(fnLogRationMap_, map_mlog, mask_);
        }
    }
    ...
    if (!fnReferenceMap_.empty())
    {
        printf("The correlation coefficient between computed and reference maps is "
               "%f\n",
               correlationCoefficient(referenceMap_, averageMap_, mask_));
        // calculate local correlation if requested
        if (!fnLocalCorrelationMap_.empty())
        {
            auto map_loc_corr = averageMap_;
            local_correlation_coeff(referenceMap_, averageMap_, mask_, &map_loc_corr,
                                    nSlide_);
            writeMaskedMap(fnLocalCorrelationMap_, map_loc_corr, mask_);
        }
    }
...
{
    // print the average to file 3daverageMaperage_
