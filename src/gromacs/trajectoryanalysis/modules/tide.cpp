...
void set_gaussian_coeff( std::string gauss_coeff_, std::vector<float> *A, std::vector<float> *B, std::vector<char> *attypes,  int Ngaussians, int Nattypes  )
{
    ...
    if (!gauss_coeff_.empty())
    {
        ...
        for (int n = 0; n < Nattypes; n++)
        {
            ...
            attypes->push_back(*tmptype);

            // A parameters
            for (int col = 0; col < Ngaussians; col++)
            {
                int id_1D_aux = index_array(col, n, 0, Ngaussians, 6, 0);
...
void accumulate_denstiy( gmx_unused t_mapdata *map_av, gmx_unused t_mapdata *map_inst, gmx_unused t_mapdata *map_sd, int natoms, Selection sel,  float res[], float L0[], int ncutoff[], float cutoff, int Ngaussians, std::vector<float> Ai, std::vector<float> Bi,  std::vector<int> attype_position)
{
                                  ...
                                  const TopologyInformation                    &top)
{
    ...
    attypes.resize(Nattypes);      // attypes array
