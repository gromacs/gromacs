...
void localCorrelationCoefficient(const GridDataReal3D &map1, const GridDataReal3D &map2, const Mask &mask,
                                 GridDataReal3D *localcorrelationmap, int maxDist)
{
    ...
    for (size_t i = 0; i < map1.size(); ++i)
    {
        ...
        float sum1  = 0.;
        float sum2  = 0.;
        // slide over points, centered around maxDist to accumulate the density
        for (int iZ = latticeIndex[ZZ] - maxDist; iZ <= latticeIndex[ZZ] + maxDist; iZ++)
        {
            for (int iY = latticeIndex[YY] - maxDist; iY <= latticeIndex[YY] + maxDist; iY++)
            {
                for (int iX = latticeIndex[XX] - maxDist; iX <= latticeIndex[XX] + maxDist; iX++)
                {
...
class TimeAveragedDensity final : public TrajectoryAnalysisModule
{
    ...
    private:
        ...
        int                      nVoxelLocalCorrelation_       = 5;   // Size of the sliding grid for local-correlation calculation
                                      ...
                                      TrajectoryAnalysisSettings *settings)
{
    static const char *const desc[] = {
        ...
        "At all grid points, the correlation between average and reference within a cubic grid of (nVoxelLocalCorrelation_,nVoxelLocalCorrelation_,nVoxelLocalCorrelation_) is stored[PAR]",
        "",
    ...
    options->addOption(IntegerOption("nslide").store(&nVoxelLocalCorrelation_)
                           .description("Maximum voxel distance for local correlation calculation"));
...
void TimeAveragedDensity::writeOutput()
{
    ...
    if (!fnReferenceMap_.empty())
    {
        ...
        if (!fnLocalCorrelationMap_.empty())
        {
            ...
            localCorrelationCoefficient(referenceMap_, averageMap_, mask_, &map_localcorrelationmap,
                                        nVoxelLocalCorrelation_);
