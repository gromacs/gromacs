...
{
    std::transform(std::begin(targetMap->vox), std::end(targetMap->vox),
                   std::begin(addedMap.vox), std::begin(targetMap->vox),
                   std::plus<float>());
}
...
void divideDensityByValue(gmx_unused t_mapdata *map_av, float Nframes)
{
    std::transform(std::begin(map_av->vox), std::end(map_av->vox),
                   std::begin(map_av->vox),
                   [Nframes](float voxelValue) { return voxelValue / Nframes; });
}
...
{
    std::transform(std::begin(mask->vox), std::end(mask->vox),
                   std::begin(mask->vox), [mask_cutoff](float maskMapValue) {
                       return maskMapValue <= mask_cutoff ? 0 : 1;
                   });
}

//! \brief Subtract maps: rho(map3) = rho(map2)-rho(map1)
void subtract_maps(gmx_unused t_mapdata *map1, gmx_unused t_mapdata *map2,
                   gmx_unused t_mapdata *map3, gmx_unused t_mapdata *mask)
{
    // do subtraction if maps have the same dimensions, else abort with fatal
    // error
    if (!mapsHaveSameGrid(map1, map2) || !mapsHaveSameGrid(map1, map3) ||
        !mapsHaveSameGrid(map1, mask))
    {
...
void minus_log_map(gmx_unused t_mapdata *map, gmx_unused t_mapdata *map2,
                   gmx_unused t_mapdata *mask)
{
    // do operation if maps have the same dimensions, else abort else abort with
    // fatal error
    ...
    for (size_t l = 0; l < map->vox.size(); l++)
    {
        if (mask->vox[l] > 0.5) // discard masked points
        {
            ...
            {
                gmx_fatal(FARGS, "Density at %d is zero or less in the map. Discard "
                          "this point by using -mask.\n",
                          l);
            }
        }
        else
...
//! \brief log of division of maps: rho(map3) = - ln ( rho(map2) / rho(map1) )
//! PSEUDO FREE ENERGY
void minus_log_division_maps(gmx_unused t_mapdata *map1,
                             gmx_unused t_mapdata *map2,
                             gmx_unused t_mapdata *map3,
                             gmx_unused t_mapdata *mask)
{
    // do operation if maps have the same dimensions, else abort else abort with
    // fatal error
    ...
    for (size_t l = 0; l < mask->vox.size(); l++)
    {
        if (mask->vox[l] > 0.5) // discard masked points
        {
            ...
            {
                gmx_fatal(FARGS, "Density at point %d is zero in the dividing map1: "
                          "-ln[rho(map2)/rho(map1)]. You could discard this "
                          "point from the calculation by using -mask.\n",
                          l);
            }
        }
    }
...
{
    std::transform(std::begin(map->vox), std::end(map->vox), std::begin(map->vox),
                   [S](float voxelValue) { return voxelValue + S; });
}

//! \brief Compute spatial average over the map: <rho(map)> over all cell
//! positions
void do_spatial_average_maps(gmx_unused t_mapdata *map, bool incl_zero,
                             gmx_unused t_mapdata *mask)
{
    ...
    {
        if (mask->vox[l] >
            0.5)                               // 0=masked point , 1=unmasked point to be considered
        {
...
//! \brief TODO
float calc_sum_rhoA_rhoB_mask(gmx_unused t_mapdata *map1,
                              gmx_unused t_mapdata *map2,
                              gmx_unused t_mapdata *mask)
{
    ...
    double sum_ijk = 0.0;
    for (int l = 0; l < map1->map_dim[XX] * map1->map_dim[YY] * map1->map_dim[ZZ];
         l++)
    {
        if (mask->vox[l] >
            0.5)    // 0=masked point , 1=unmasked point to be considered
...
//! \brief TODO
float global_correlation_coeff(gmx_unused t_mapdata *map1,
                               gmx_unused t_mapdata *map2,
                               gmx_unused t_mapdata *mask)
{
    if (!mapsHaveSameGrid(map1, map2) || !mapsHaveSameGrid(map1, mask))
    ...
    denominator = sqrt(calc_sum_rhoA_rhoB_mask(map2, map2, mask) *
...
void local_correlation_coeff(t_mapdata *map1, t_mapdata *map2, t_mapdata *mask,
                             t_mapdata *loc_corr, int Nodd)
{
    ...
    if (!mapsHaveSameGrid(map1, map2) || !mapsHaveSameGrid(map1, mask) ||
        !mapsHaveSameGrid(map1, loc_corr))
    {
        gmx_fatal(FARGS, "Map dimensions must be identical");
    }
...
//! O, P, and S
void set_gaussian_coeff(
        std::string gauss_coeff_,
        std::map < std::string, std::vector < GaussianParameters>> *gaussianParamters)
{
    gaussianParamters->clear();
    ...
    else
    {
        ...
        pfile = libopen("electronscattering.dat");
    }
    // loop over the atoms
    char line[1000];
    while (get_a_line(pfile, line, 1000))
    {
        char  currentAtomType[100];
        float amplitude;
        float precision;
        sscanf(line, " %s %e %e", currentAtomType, &amplitude, &precision);
        (*gaussianParamters)[std::string(currentAtomType)].push_back({amplitude, precision});
    }
    fclose(pfile);
}
 ...
 */
void print_to3dmap(const char *inputFileName, gmx_unused t_mapdata *map,
                   gmx_unused t_mapdata *mask)
{
    ...
    xplorOutput.maskData(mask->vox, 0.5);
...
void init_map(gmx_unused t_mapdata *map, const std::string &title, int datamode,
              const std::array<float, 2 * DIM> &cell,
              const std::array<int, DIM> &grid,
              const std::array<int, DIM> &origin,
              const std::array<int, DIM> &axes_order,
              const std::array<int, DIM> &map_dim, int spacegroup, double min,
              double max, float mean, float rms)
{
    ...
    map->spacegroup = spacegroup; // this will be assigned arbitrarily. It won't
                      ...
                      gmx_unused t_mapdata *ref_map)
{
    ...
    init_map(ref_map, "ref", 2, cell, xplorData.extend, xplorData.crs_start,
             {{0, 1, 2}}, bin, 1, *(minmax.first), *(minmax.second),
...
void accumulate_denstiy(gmx_unused t_mapdata *map_av,
                        ...
                        RVec L0, IVec ncutoff, float cutoff,
                        const std::map < std::string, std::vector < GaussianParameters>> &Ai,
                        const std::vector<std::string> &attypeString)
{
    ...
    // Distance between the atom and the grid_vector:  R_atom_grid= R_trans -
    // grid_vec
    rvec  R_atom_grid;
    ...
    // === Count the visit of each one of the surrounding atoms (i.e. lipids) in
    // the proper grid of the lattice
    ...
    for (int atomIndex = 0; atomIndex < sel.atomCount(); atomIndex++)
    {
        ...
        const auto &gaussianParameter = Ai.at(atomName);
                    ...
                    for (int nz = k - ncutoff[ZZ]; nz <= k + ncutoff[ZZ]; nz++)
                    {
                            ...
                            nz < map_av->map_dim[ZZ])
                        {
                            ...
                            if (dist_atom_grid < cutoff)
                            {
                                ...
                                {
                                    currentContribution += g.amplitude *
                                        exp(-g.precision *
                                            dist_atom_grid * dist_atom_grid);
                                }
                                int index_aux = index_array(nx, ny, nz, map_av->map_dim[XX],
                                                            map_av->map_dim[YY]);
                                map_inst->vox[index_aux] += currentContribution;

                            } // Finish if statement that only distances smaller than the cut
                              // off are considered for the density
...
void print_rho_sigma(const char *inputFileName, rvec res,
                     gmx_unused t_mapdata *map_av,
                     gmx_unused t_mapdata *map_sd)
{
...
class TimeAveragedDensity final : public TrajectoryAnalysisModule
{
    ...
    private:
        ...
        RVec                             L0  = {0., 0., 0.}; // origin of the lattice with respect to the origin of
                                                             // the map Nmin*res
        ...
        std::map < std::string, std::vector < GaussianParameters>> A; // Ai factors (Ngaussians x Natoms 1D array)
        ...
        const TopologyInformation                   &top)
{
    ...
    set_gaussian_coeff(gauss_coef_, &A);
    ...
    std::array<int, DIM> axes_order = {
        {0, 1, 2}
    };            // x, y, z (guess) but  not used in tide anyways
    ...
    init_map(&map_av, "map", 2, cell, nGridPoints, Nmin, axes_order, bin, 1, 0, 0,
             0, 0);
    printMapInfo(map_av);
                                       ...
                                       TrajectoryAnalysisModuleData *pdata)
{
    ...
    // === ACCUMMULATE DENSITY===
    accumulate_denstiy(&map_av, &map_inst, &map_sd, sel, res, L0, ncutoff, cutoff,
                       A, attypeString);
...
void TimeAveragedDensity::finishAnalysis(int /*nframes*/)
{
    ...
    if (!diff_.empty())
    {
        subtract_maps(&map_ref, &map_av, &map_diff,
                      &map_mask); // map_diff = map_av - map_ref
        do_spatial_average_maps(
                &map_diff, true,
                &map_mask); // do include zero in the calculation of mean and rms
    }
    ...
    if (!mlog_.empty())
    {
        ...
        if (!mi_.empty())
        {
            minus_log_division_maps(
                    &map_ref, &map_av, &map_mlog,
                    &map_mask); // map_mlog = - ln ( map_av /  map_ref )
        }
        // no ref map
        else
        {
    ...
    if (!mi_.empty())
    {
        ...
        if (!loc_corr_.empty())
        {
            local_correlation_coeff(&map_ref, &map_av, &map_mask, &map_loc_corr,
                                    Nslide);
