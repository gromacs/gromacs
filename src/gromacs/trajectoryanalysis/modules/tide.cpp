...
void localCorrelationCoefficient(const GridDataReal3D &map1, const GridDataReal3D &map2, const Mask &mask,
                                 GridDataReal3D *localcorrelationmap, float range)
{
...
class TimeAveragedDensity final : public TrajectoryAnalysisModule
{
    ...
    private:
        ...
        float                    rlocal_ = 0.3;                       // local-correlation range in nm
                                      ...
                                      TrajectoryAnalysisSettings *settings)
{
    static const char *const desc[] = {
        ...
        "At all grid points, the correlation between average and reference within a cubic grid of (rlocal,rlocal,rlocal) is stored[PAR]",
        "",
    ...
    options->addOption(FloatOption("rlocal").store(&rlocal_)
...
{
    if (!fnReferenceMap_.empty() && !fnLocalCorrelationMap_.empty())
    {
