                ...
                for (int ny = j - ncutoff[YY]; ny <= j + ncutoff[YY]; ny++)
                {
                    for (int nz = k - ncutoff[ZZ]; nz <= k + ncutoff[ZZ]; nz++)
                    {
                            ...
                            nz < map_av->map_dim[ZZ])
                        {
                            ...
                            dist_atom_grid = norm(R_atom_grid);
...
void TimeAveragedDensity::writeOutput()
{
    ...
    {
        print_to3dmap(fnMapAverage_.c_str(), &map_av, &map_mask);
    }

    // print the standard deviation to file 3dmap_sigma
    if ((!fnMapStandardDeviation_.empty()) && (Nframes > 1))
    {
        print_to3dmap(fnMapStandardDeviation_.c_str(), &map_sd, &map_mask);
    }

    // print the difference map to file diff
    if (!fnDifferenceMap_.empty())
    {
        print_to3dmap(fnDifferenceMap_.c_str(), &map_diff, &map_mask);
    }
