...
void set_mask( std::string map3d_mask_, gmx_unused t_mapdata *mask, float mask_cutoff)
{
    ...
    {
        std::fill(std::begin(mask->vox), std::end(mask->vox), 1.f);
        return;
    }
                    ...
                    for (int nz = k-ncutoff[ZZ]; nz <= k+ncutoff[ZZ]; nz++)
                    {
                        ...
                        if (nx >= 0 && nx < map_av->map_dim[XX] && ny >= 0 && ny < map_av->map_dim[YY] && nz >= 0 && nz < map_av->map_dim[ZZ])
                        {
                            ...
                            if (dist_atom_grid < cutoff)
                            {
                                ...
                                {
                                    int index_oneD_aux   = index_array(g, position, 0, Ngaussians, natoms); // turn (position,g) into a one-dimensional array.
...
class TimeAveragedDensity final : public TrajectoryAnalysisModule
{
    ...
    private:
        ...
        // entire cell
        IVec nGridPoints = {250, 250, 250}; // (NA,NB,NC)
        IVec Nmin        = {0, 0, 0};       // (AMIN,BMIN,CMIN)
        IVec Nmax        = {249, 249, 249}; // (AMAX,BMAX,CMAX)
        RVec box         = {10, 10, 10};    //(boxA,boxB,boxC)

        // subcell where the density is actually computed
        IVec    bin = {0, 0, 0};            // Grid points (0 0 0) (working lattice)
        ...
        Selection                        refsel_;
        SelectionList                    sel_;
        int     Ntotal   = 0; // total number of atoms
        int     Nattypes = 6; // Nummber of atomtypes (default = 6)
                                 ...
                                 TrajectoryAnalysisSettings *settings)
{
    ...
    options->addOption(IntegerOption("size").store(nGridPoints).vector()
...
//=== INPUT REFERENCE MAP ====
    ...
    if (!mi_.empty())
    {
        ...
        for (int i = 0; i < DIM; i++)
        {
            ...
            nGridPoints[i] = map_ref.grid[i]; // cell grid points
    ...
    init_map(&map_av, title, 2, cell, nGridPoints, Nmin,  axes_order, bin, 1, min, max, mean, rms, skew_mat, skew_trans, 1);
    init_map(&map_inst, title, 2, cell, nGridPoints, Nmin,  axes_order, bin, 1, min, max, mean, rms, skew_mat, skew_trans, 0);
    init_map(&map_sd, title, 2, cell, nGridPoints, Nmin,  axes_order, bin, 1, min, max, mean, rms, skew_mat, skew_trans, 0);
    init_map(&map_diff, title, 2, cell, nGridPoints, Nmin,  axes_order, bin, 1, min, max, mean, rms, skew_mat, skew_trans, 0);
    init_map(&map_mlog, title, 2, cell, nGridPoints, Nmin,  axes_order, bin, 1, min, max, mean, rms, skew_mat, skew_trans, 0);
    init_map(&map_block, title, 2, cell, nGridPoints, Nmin,  axes_order, bin, 1, min, max, mean, rms, skew_mat, skew_trans, 0);
    init_map(&map_loc_corr, title, 2, cell, nGridPoints, Nmin,  axes_order, bin, 1, min, max, mean, rms, skew_mat, skew_trans, 0);

    // if mask was not provided:
    if (mask_.empty() == 1)
    {
        init_map(&map_mask, title, 2, cell, nGridPoints, Nmin,  axes_order, bin, 1, min, max, mean, rms, skew_mat, skew_trans, 0 );
    }
                                  ...
                                  TrajectoryAnalysisModuleData *pdata)
{
    ...
    auto nr    = sel.posCount();          // NUMBER OF ATOMS IN THE SELECTION (nr
