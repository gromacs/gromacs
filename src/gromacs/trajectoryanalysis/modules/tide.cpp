...
bool mapsHaveSameGrid(gmx_unused t_mapdata *map1, gmx_unused t_mapdata *map2 )
{
    ...
    if (map1->grid != map2->grid)
    {
        return false;
    }
    if (map1->origin != map2->origin)
    {
        return false;
    }
    if (map1->map_dim != map2->map_dim)
    {
        return false;
    }
    return true;
...
//=== assign the information read in the xplor file to the map ===
    ...
    init_map(ref_map, title, 2, cell, N, MIN,  axes_order, bin, 1, min, max, av, stdev, skew_mat, skew_trans);
...
//=== INPUT REFERENCE MAP ====
    ...
    // ====  initialize the maps =====
    char                     title[] = {'m', 'a', 'p'};
    std::array<float, 2*DIM> cell    = {{box[XX], box[YY], box[ZZ], 90., 90., 90.}};
    //N  Number of grid points within box
    //  MIN[i]; // Origin of the working lattice
    std::array<int, DIM> axes_order = {{0, 1, 2}};                  // x, y, z (guess) but  not used in tide anyways
    // bin = map_dim is the working lattice
