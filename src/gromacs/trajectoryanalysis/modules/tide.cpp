 ...
 */
void print_to3dmap( char *N_file, rvec N, rvec Nmin, rvec Nmax, rvec BOX, ivec bin, rvec res, float density[] )
{
    ...
    fprintf(pfile, "       2\n");
    fprintf(pfile, "REMARKS Lattice Nbins: %d %d %d\n", bin[XX], bin[YY], bin[ZZ]);
    fprintf(pfile, "REMARKS Lattice resolution (nm) %f %f %f\n", res[XX], res[YY], res[ZZ]);
...
// Print the density values in 6 columns
                fprintf(pfile, "%12.5E", density[index_aux] );
                ncols++;
...
void print_rho_sigma( char *N_file, ivec bin, rvec res,  rvec min, float density[], float density_sigma[] )
{
...
class TimeAveragedDensity : public TrajectoryAnalysisModule
{
    ...
    private:
        ...
        std::string                      map_av_, map_sd_, rho_;


        char *map3d_average; // char for the output average map
        char *map3d_sigma;   // output 3d map file (standard dev)
        char *rho_lattice;   // output 3d lattice density (the same than the map but in a grid - x,y,z,rho,sigma - format)
                                 ...
                                 TrajectoryAnalysisSettings *settings)
{
    static const char *const desc[] = {
        "gtide computes the TIme-averaged electron DEnsity of a group of atoms within a defined box."
    ...
    options->addOption(FileNameOption("map_av")
                           .filetype(eftOptionFileType_NR).outputFile()
                           .store(&map_av_).defaultBasename("map_av")
                           .description("Average density map in xplor format"));

    options->addOption(FileNameOption("map_sd")
                           .filetype(eftOptionFileType_NR).outputFile()
                           .store(&map_sd_).defaultBasename("map_sd")
                           .description("St. dev. density map in xplor format"));
 ...
 */
    ...
    if (!map_av_.empty())
    {
        map3d_average = new char [map_av_.size()+1];
        strcpy (map3d_average, map_av_.c_str());
    }

    if (!map_sd_.empty())
    {
        // sd xplor
        map3d_sigma = new char [map_sd_.size()+1];
        strcpy (map3d_sigma, map_sd_.c_str());
    }

    if (!rho_.empty())
    {
        // sd xplor
        rho_lattice = new char [rho_.size()+1];
        strcpy (rho_lattice, rho_.c_str());
    }
...
TimeAveragedDensity::finishAnalysis(int /*nframes*/)
{
    ...
    // === Calculate the density, normalizing the lattice valuex ===
...
TimeAveragedDensity::writeOutput()
{
    ...
    // print the average 3dmap_average
    print_to3dmap(map3d_average, N, Nmin, Nmax, BOX, bin, res,   lattice);

    // print the standard deviation to 3dmap_sigma
    print_to3dmap(map3d_sigma, N, Nmin, Nmax, BOX, bin, res,   lattice_squared);

    //print to rho.dat file
    print_rho_sigma(rho_lattice, bin, res,  Nmin, lattice, lattice_squared );
}
