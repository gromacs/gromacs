...
void do_spatial_average_maps(gmx_unused t_mapdata *map, bool incl_zero, gmx_unused t_mapdata *mask  )
{
...

    {
        for (l = 0; l < map->map_dim[XX]*map->map_dim[YY]*map->map_dim[ZZ]; l++)
        {
            ...
            {
                if (map->vox[l] == 0)          // zero density point
                {
                    if (incl_zero)
                    {
                        consider_this = 1;
...
//! \brief TODO
void local_correlation_coeff(gmx_unused t_mapdata *map1, gmx_unused t_mapdata *map2, gmx_unused t_mapdata *mask, gmx_unused t_mapdata *loc_corr, int Nodd )
{
    ...
    if (same_dims)
    {
        ...
        int       l, i, j, k, i_sli, j_sli, k_sli, id_aux;
        float     numerator, denominator, sum12, sum11, sum22,  cc;
        ...
        for (i = Nhalf; i < map1->map_dim[XX]-Nhalf; i++)
        {
            for (j = Nhalf; j < map1->map_dim[YY]-Nhalf; j++)
            {
                for (k = Nhalf; k < map1->map_dim[ZZ]-Nhalf; k++)
                {
                    ...
                    for (i_sli = i - Nhalf; i_sli <=  i + Nhalf; i_sli++)
                    {
                        for (j_sli = j - Nhalf; j_sli <=  j + Nhalf; j_sli++)
                        {
                            for (k_sli = k - Nhalf; k_sli <=  k + Nhalf; k_sli++)
                            {
                                id_aux = index_array(i_sli, j_sli, k_sli, map1->map_dim[XX], map1->map_dim[YY], map1->map_dim[ZZ]);
                    ...
                    // compute correlation
                    numerator   = sum12;
                    denominator = sqrt(sum22 * sum11);
                    cc          = numerator / denominator;

                    // assign correlation to middle point of sliding grid (i,j,k)
                    id_aux                = index_array(i, j, k, map1->map_dim[XX], map1->map_dim[YY], map1->map_dim[ZZ]);
...
void set_gaussian_coeff( std::string gauss_coeff_, float A[], float B[], char attypes[],  int Ngaussians, int Nattypes  )
{

    int  n, col, id_1D_aux;
    char tmptype[1];
    /*
       char   gauss_coef_attype[1]; // atom
       char attypes[6]={'H', 'C' , 'N', 'O'  , 'P' , 'S' };
       int aux, position, col, id_1D_aux;
       char * pch;
     */
    ...
    if (!gauss_coeff_.empty())
    {
        ...
        // loop over the atoms
        for (n = 0; n < Nattypes; n++)
        {

            // read the following parameters
            fscanf (pfile, "%s", tmptype);                             // atomtype
            attypes[n] = tmptype[0];

            // A parameters
            for (col = 0; col < Ngaussians; col++)
            {
                id_1D_aux = index_array(col, n, 0, Ngaussians, 6, 0);
                fscanf (pfile, "%e", &A[id_1D_aux]);                             // A factors ;
                //printf("%f ",A[id_1D_aux]);
            }
            ...
            {
                id_1D_aux = index_array(col, n, 0, Ngaussians, 6, 0);
                fscanf (pfile, "%e", &B[id_1D_aux]);                             // Bi factors
                //printf("%f ",B[id_1D_aux]);
            }
...
int attype_index( char *attype, char attypes[], int Nattypes )
{
    int   aux, position;
    aux = attype[0]; // attype into auxiliary integer (only first character considered)
    char *pch;
    pch      = strchr(attypes, aux);
    position = pch-attypes;  // this will be the position number in the attypes array (e.g. H=0 , C=1, N=2 , O=3 ; P=4  ; S=5)
...
void set_attype_position_top( char attypes[], int attype_position[], int Natoms, int Nattypes, const TopologyInformation &top)
{
    int   aux;
    char *atomname;
    char  atomtype[1];
    ...
    {
        atomname    = *(top.topology()->atoms.atomname[ n]);
        atomtype[0] = atomname[0];
        aux         = attype_index(atomtype, attypes, Nattypes);

        if (aux >= 0 && aux < Nattypes)
        {
            attype_position[n] = aux;
        }
        else
        {
            gmx_fatal(FARGS, "atom type %.1s was not found in Gaussian-coefficient attype data base\nInclude this atom in the data base (-Gauss_coef)\nor exclude this atom from input structure and input trajectory", atomtype);
        }
        //printf("%d %.1s %d\n",n,atomtype,attype_position[n]);
    }
}
...
void print_to3dmap(const char *N_file, gmx_unused t_mapdata *map, gmx_unused t_mapdata *mask )
{
    ...
    if (compare_maps(map, mask))
    {
        ...
        fprintf(fp, "%12.5E%12.5E%12.5E%12.5E%12.5E%12.5E\n", map->cell[XX]*10.0, map->cell[YY]*10.0, map->cell[ZZ]*10.0, map->cell[3], map->cell[4], map->cell[5]   );
        ...
        for (k = 0; k < map->map_dim[ZZ]; k++) // loop over z
        {                                      // Print the k value
            fprintf(fp, "       %d\n", k);
            ...
            for (j = 0; j < map->map_dim[YY]; j++)       // loop over y

            {
                for (i = 0; i < map->map_dim[XX]; i++)       // loop over x
...
void read_xplor_3dmap(const char *N_file, gmx_unused t_mapdata *ref_map )
{
    ...
    int   file_parser = 0; // variable to indicate if the last line of the file was reached
...
// auxiliary variables
    int i, j, k, k_read, index_aux, row_ZYX;


    // ==== header ====

    // determine which line has the ZYX string
    int row  = 1; // variable to monitor the row of the scanned file
    do
    {
        fgets(tmp_char, 80, fp);

        int comparison = strncmp(tmp_char, "ZYX", 3 );
        if (comparison == 0)
        {
            file_parser = 1; row_ZYX = row;
        }
    ...
    {                                              // Print the k value
        ...
        else
        {
            ...
            for (j = 0; j < bin[YY]; j++)                           // loop over y
            {
                ...
                {
                    index_aux = index_array(i, j, k, bin[XX], bin[YY], bin[ZZ]);
...
void accumulate_denstiy( gmx_unused t_mapdata *map_av, gmx_unused t_mapdata *map_inst, gmx_unused t_mapdata *map_sd, int natoms, Selection sel,  float res[], float L0[], int ncutoff[], float cutoff, int Ngaussians, float Ai[], float Bi[],  int attype_position[] )
{
    ...
    //=== AUXILIARY VARIABLES TO THE INDEX POSITION IN THE ATTYPE ARRAY ===
    int position, atomindex;
    ...
    for (n = 0; n < natoms; n++)
    {
        ...
        atomindex = sel.atomIndices()[n];
        position  = attype_position[atomindex]; // e.g. for default Gaussian-coeff array: H=0 , C=1 , N=2 , O=3, P=4, S=5
                    ...
                    for (nz = k-ncutoff[ZZ]; nz <= k+ncutoff[ZZ]; nz++)
                    {
                        ...
                        if (nx >= 0 && nx < map_av->map_dim[XX] && ny >= 0 && ny < map_av->map_dim[YY] && nz >= 0 && nz < map_av->map_dim[ZZ])
                        {
                            ...
                            if (dist_atom_grid < cutoff)
                            {
                                ...
                                {
                                    index_oneD_aux   = index_array(g, position, 0, Ngaussians, natoms, 0); // turn (position,g) into a one-dimensional array.
                                ...
                                index_aux                 = index_array(nx, ny, nz, map_av->map_dim[XX], map_av->map_dim[YY], map_av->map_dim[ZZ]);
    ...
    for (i = 0; i < map_av->map_dim[XX]; i++)         // loop over x
    {
        for (j = 0; j < map_av->map_dim[YY]; j++)     // loop over y
        {
            ...
            {                                         //=== set the absolute coordinate system (the simulation box)by adding (minx,miny,minz) to each lattice value
                // x,y,z in Angstrom
                x = 10*(i+map_av->origin[XX])*res[XX];
                y = 10*(j+map_av->origin[YY])*res[YY];
                z = 10*(k+map_av->origin[ZZ])*res[ZZ];
...
class TimeAveragedDensity final : public TrajectoryAnalysisModule
{
    ...
    private:
        ...
        float  *A               = nullptr;      // Ai factors (Ngaussians x Natoms 1D array)
        float  *B               = nullptr;      // Bi factors (Ngaussians x Natoms 1D array)
        char   *attypes         = nullptr;      // atomtypes for A and B array
        int    *attype_position = nullptr;      // array will store the index of the atom type for the A and B Gaussian-coefficient arrays

        // === parse trajectory ===
        int parse_traj;

        // === define the lattice that will store the density for each point===
        t_mapdata         *map_ref       = nullptr; /* The reference=experimental map to compare to        */
        t_mapdata         *map_av        = nullptr; /* The computed timed-averaged map         */
        t_mapdata         *map_inst      = nullptr; /* The computed instantaneous         */
        t_mapdata         *map_sd        = nullptr; /* The computed timed-averaged standard deviation map */
        t_mapdata         *map_diff      = nullptr; /* diff map = computed_map - ref_map */
        t_mapdata         *map_mask      = nullptr; /* mask map*/
        t_mapdata         *map_mlog      = nullptr; /* minus log map*/
        t_mapdata         *map_loc_corr  = nullptr; /* local correlation map*/
        t_mapdata         *map_block     = nullptr; /* map for block averaging*/
...
// Defaults
    ...
    BOX[XX]  = 10.0; BOX[YY] = 10.0; BOX[ZZ] = 10.0;
...
// set parse_traj to one so the analysis is done
    ...
    snew(A, Nattypes*Ngaussians);  // a factors
    snew(B, Nattypes*Ngaussians);  // b factors
    snew(attypes, Nattypes);       // attypes array
    snew(attype_position, Ntotal); // index to attype array based on atom name for all atoms in top

    set_gaussian_coeff(gauss_coef_, A, B, attypes, Ngaussians, Nattypes);
    set_attype_position_top(attypes, attype_position, Ntotal, Nattypes, top );
...
//=== INPUT REFERENCE MAP ====
    ...
    if (mi_.empty() == 0)
    {
        ...
        snew (map_ref, 1 );                 //allocate memory for the reference map
        read_xplor_3dmap(mi_.c_str(), map_ref );
    }


    /* === Mask map === */
    snew (map_mask, 1 );     //allocate memory for the mask map
    ...
    snew (map_av, 1 );               //map average
    snew (map_inst, 1 );             //map instantaneous
    snew (map_sd, 1 );               //map standard deviation
    snew (map_diff, 1 );             //map difference
    snew (map_mlog, 1 );             //map minus log
    snew (map_block, 1 );            //map block averaging
    snew (map_loc_corr, 1 );         //map local correlation
