...
void init_map(gmx_unused t_mapdata *map, const std::string &title, int datamode, const std::array<float, 2*DIM> &cell, const std::array<int, DIM> &grid, const std::array<int, DIM> &origin, const std::array<int, DIM> &axes_order, const std::array<int, DIM> &map_dim, int spacegroup, double min, double max, float mean, float rms)
{
...
//=== assign the information read in the xplor file to the map ===
    ...
    std::array<int, DIM>     bin = {xplorData.crs_end[XX]-xplorData.crs_start[XX]+1, xplorData.crs_end[YY]-xplorData.crs_start[YY]+1, xplorData.crs_end[ZZ]-xplorData.crs_start[ZZ]+1};

    auto                     minmax = std::minmax_element(std::begin(xplorData.data), std::end(xplorData.data));
    init_map(ref_map, "ref", 2, cell, xplorData.extend, xplorData.crs_start,  {{0, 1, 2}}, bin, 1, *(minmax.first), *(minmax.second), xplorData.mean_value, xplorData.rms_value);
...
//=== INPUT REFERENCE MAP ====
    ...
    init_map(&map_av, "map", 2, cell, nGridPoints, Nmin,  axes_order, bin, 1, 0, 0, 0, 0);
    printMapInfo(map_av);
