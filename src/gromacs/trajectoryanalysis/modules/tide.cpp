...
std::map < int, std::vector < GaussianParameters>> TimeAveragedDensity::setGaussianCoeff(bool hasSigma)
{
    ...
    {
        fprintf(stdout, "Reading Gaussian coefficients from %s .\n", fnGaussParameters_.c_str());
        parameterFile = fopen(fnGaussParameters_.c_str(), "r");
    }
    else
    {
        if (hasSigma)
        {
            fprintf(stdout, "Reading electron microscopy Gauss weights from 'emcrosssections.dat'\n");
            parameterFile = libopen("emcrosssections.dat");
        }
        else
        {
            fprintf(stdout, "Reading default Gaussian coefficients from 'electronscattering.dat'\n");
