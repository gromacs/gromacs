...
std::map < int, std::vector < GaussianParameters>> TimeAveragedDensity::setGaussianCoeff(bool hasSigma)
{
    ...
    if (parameterFile == nullptr && !hasSigma)
    {
        ...
        const TopologyInformation                   &top)
{
    ...
    {
        InconsistentInputError("Cannot use ' -type charge ' without full topology information (i.e. a tpr file), because this is the only way to determine the partial charge per atom.");
    }
...
void TimeAveragedDensity::analyzeFrame(int frnr, const t_trxframe &fr, t_pbc * /*pbc*/, TrajectoryAnalysisModuleData *pdata)
{
    ...
    for (const auto &para : atomCoordinatesPerPrecision)
    {
        ...
        if (precision > 0)
        {
            ...
            spreader.spreadLocalAtoms({ para.second.data(), para.second.data()+para.second.size() }, { weights.data(), weights.data()+weights.size() });
        }
    }
    ...
    if (!fnCorrelation_.empty())
    {
        ...
        if (round(steps) == round(floor(steps)) && fr.time >= 0)
        {
            ...
            dh.startFrame(dataFrameNumber_, fr.time);
