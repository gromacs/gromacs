 ...
 */
//static char *SRCID_g_fca_c = "$Id: g_project.c,v 1.5 2004/11/12 17:17:36 olange Exp $";
...
#include "utils.hpp"
// TODO #include "error.h"
// TODO #include "statutil.h"
// TODO #include "copyrite.h"
...
void write_fca_valfile_fca(FILE* out, const int dim, const real* mat, const int natoms,
...
std::vector<int> select_eigen_vectors(const int nbAtoms, const int nvec, const int eignr[], const int first, int last = -1)
{
    if (last == -1)
    {
        last = nbAtoms * DIM;
    ...
    fprintf(stderr, "%lu eigenvectors selected for output", outvec.size());
...
/*int main(int argc, char* argv[]) {
   ...
   //        logger.icx         = fopen("ic_lastproj.log","w");
   ...
   #ifdef MPICC
    AssertMpi(MPI_Bcast(&dim, 1, MPI_INT, MASTER_RANK, MPI_COMM_WORLD));
    AssertMpi(MPI_Bcast(&nframes, 1, MPI_INT, MASTER_RANK, MPI_COMM_WORLD));
   #endif
    ...
    ///////////////////////////////////////////////////////////////////////////////////////////////////////
   #ifdef MPICC
    AssertMpi(MPI_Bcast(&number_of_moves, 1, MPI_INT, MASTER_RANK, MPI_COMM_WORLD));
    AssertMpi(MPI_Bcast(&bMinimize, 1, MPI_INT, MASTER_RANK, MPI_COMM_WORLD));
   #endif
    /// ---- Minimize
    if(bMinimize) {
    ...
    ///////////////////////////////////////////////////////////////////////////////////////////////////////
   #ifdef MPICC
    AssertMpi(MPI_Bcast(&bSort, 1, MPI_INT, MASTER_RANK, MPI_COMM_WORLD));
    AssertMpi(MPI_Bcast(&amplitude_weight, 1, MPI_INT, MASTER_RANK, MPI_COMM_WORLD));
   #endif
   ...
   #ifdef MPICC
    // distribute flag
    AssertMpi(MPI_Bcast(&bWriteMI, 1, MPI_INT, MASTER_RANK, MPI_COMM_WORLD));
    AssertMpi(MPI_Bcast(&log_kappa1, 1, MPI_DOUBLE, MASTER_RANK, MPI_COMM_WORLD));
    AssertMpi(MPI_Bcast(&log_kappa2, 1, MPI_DOUBLE, MASTER_RANK, MPI_COMM_WORLD));
    AssertMpi(MPI_Bcast(&bBasicHisto, 1, MPI_INT, MASTER_RANK, MPI_COMM_WORLD));
   #endif
    // compute MI matrix
    if(bWriteMI) {
        if(mpi.isMaster()) {
        ...
        if(bBasicHisto) {
            ...
            gmx::ThreeFry2x64<8> rng = rng(123456, gmx::RandomDomain::Other);
...
class Fca : public TrajectoryAnalysisModule
{
    public:
        Fca();
...
void
Fca::initOptions(IOptionsContainer          *options,
...
void
Fca::initAnalysis(const TrajectoryAnalysisSettings   & /*settings*/,
...
void
Fca::analyzeFrame(int /*frnr*/, const t_trxframe &fr, t_pbc * /*pbc*/,
...
void
Fca::finishAnalysis(int /*nframes*/)
{
}
...
const char FcaInfo::name[]             = "fca";
const char FcaInfo::shortDescription[] = "Functional Mode Analysis";

TrajectoryAnalysisModulePointer FcaInfo::create()
{
    return TrajectoryAnalysisModulePointer(new Fca);
}
