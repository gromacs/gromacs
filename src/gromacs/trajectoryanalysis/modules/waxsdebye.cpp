                        ...
                        const TopologyInformation        &top)
{
    ...
    nbonds_ = 0;
    iatoms_wd_ = NULL;
    for(int k = 0; (k < mtop_->nmoltype); k++)
    {
        int nn = mtop_->moltype[k].ilist[F_WAXS_DEBYE].nr;
        if (nn > 0)
        {
            nbonds_ += nn;
            srenew(iatoms_wd_, nbonds_);
            memcpy(&iatoms_wd_[nbonds_ - nn], 
                   mtop_->moltype[k].ilist[F_WAXS_DEBYE].iatoms,
                   nn*sizeof(iatoms_wd_[0]));
        }
    }

    vsite_ = init_vsite(mtop_, cr_, TRUE);

    ntypes_ = mtop_->ffparams.ntypes;
    snew(iparams_, ntypes_);
    memcpy(iparams_, mtop_->ffparams.iparams,
                        ...
                        TrajectoryAnalysisModuleData *pdata)
{
    ...
    if (NULL != vsite_)
    {
        construct_vsites_mtop(vsite_, mtop_, fr.x);
    }
