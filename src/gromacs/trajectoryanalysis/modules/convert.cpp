...
/*! \brief
 * Helper to provide conversion logic between different trajectory file types
 * Handles the file opening and closing for appropiate input and output files
 *
 * \ingroup module_trajectoryanalysis
 */

class TrajectoryFileHandling
{
    public:
        TrajectoryFileHandling(t_filenm &fnm)
            : internal_(fnm), filename_(nullptr), file_status_(efNR)
        {
        }

        void getFileTypeFromName(const *char fn, int *ftp)
        {
            ftp =fn2ftp(fn);
        }
        void closeFile(const t_filenm fnm);

        void getFileName(const t_filenm fnm, *const char name);
        void setFileName(t_filenm *fnm, *const char oldname, *const char newname);
