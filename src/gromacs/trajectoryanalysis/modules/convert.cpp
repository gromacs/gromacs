...
class Convert : public TrajectoryAnalysisModule
{
    ...
    private:
        OutputManager                        *output_ = nullptr;
        FrameManager                         *frame_ = nullptr;
        Selection                             sel_;
...
Convert::initOptions(IOptionsContainer *options, TrajectoryAnalysisSettings *settings)
{
    ...
    output_ = new OutputManager(&sel_);



    output_->initFileOptions(options);
...
Convert::initAnalysis(const TrajectoryAnalysisSettings   & /*settings*/,
                      const TopologyInformation         &top)
{
    output_->checkOptions(&top);
    output_->initOutput();
    frame_ = new FrameManager(&sel_, output_->metadata());
}
                      ...
                      TrajectoryAnalysisModuleData * /*pdata*/)
{
    ...
    output_->writeFrame(frame_->modifyFrame(&fr));
}
