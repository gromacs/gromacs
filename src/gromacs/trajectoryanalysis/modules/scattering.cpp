...
Scattering::Scattering()
    : startQ_(0.0), endQ_(0.0),
      qSpacing_(0.0), coverage_(0.0),
      numQ_(0), seed_(0),
      bNormalize_(true), bLogarithm_(false),
      bMCScattering_(false),
      bQSpacingSet_(false), bNumQSet_(false),
      bSeedSet_(false), bCoverageSet_(false),
      scatterEnum_(enXray),
                         ...
                         const TopologyInformation        &top)
{
    ...
    for (size_t g = 0; g < sel_.size(); ++g)
    {
        ...
        {
            computeScattering_.push_back(compat::make_unique<XrayInfo>(isotopes, qList_));
        }
    ...
    for (size_t g = 0; g < sel.size(); ++g)
    {
        ...
        for (size_t qi = 0; qi != qList_.size(); ++qi)
        {
            ...
            else
            {
                ...
                {
                    Debeye = computeScattering_[g]->computeIntensityMC(pbc, qList_[qi], sel[g], coverage_, seed_);
                }
            }
            if (!fnReferenceScattering_.empty())
            {
