...
class Scattering : public TrajectoryAnalysisModule
{
    ...
    private:
        ...
        void readScattering(std::string  parameterFileName);
...
Scattering::initOptions(IOptionsContainer *options, TrajectoryAnalysisSettings *settings)
{
    ...
    options->addOption(EnumOption<ScatterType>("scatter-type").enumValue(ScatterEnum)
                           .store(&scatterEnum_)
                           .description("whether to perform x-ray or neutron scattering"));
...
Scattering::optionsFinished(TrajectoryAnalysisSettings * /*settings*/)
{
    ...
    {
        readScattering(fnReferenceScattering_);
        startQ_   = qList_[0];
        qSpacing_ = qList_[1] - startQ_;
        for (size_t qi = 1; qi != qList_.size(); ++qi)
        {
            if (qSpacing_ != qList_[qi] - qList_[qi - 1])
            {
...
void Scattering::readScattering(std::string parameterFileName)
{
                         ...
                         const TopologyInformation        &top)
{
    ...
    for (size_t g = 0; g < sel_.size(); ++g)
    {
        ...
        {
            ComputeScattering *pCS = new auto(makeXrayInfo(top));
            computeScattering_.push_back(pCS);
        }
        if (scatterEnum_ == enNeutron)
        {
            ComputeScattering *pCS = new auto(makeNeutronInfo(top));
            computeScattering_.push_back(pCS);
        }
