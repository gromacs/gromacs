...
Scattering::Scattering()
    : startQ_(0.0), endQ_(0.0), qSpacing_(0.0), coverage_(0.0), modulation_(0),
      numQ_(0), seed_(0), bSolvent_(false), bNormalize_(true),
      bLogarithm_(false), bMCScattering_(false), bQSpacingSet_(false),
      bStartQSet_(false), bEndQSet_(false),bNumQSet_(false),
      bSeedSet_(false), bCoverageSet_(false), bModulationSet_(false), scatterEnum_(enXray),
      averageModule_(compat::make_unique<AnalysisDataAverageModule>())
{
    ...
    registerAnalysisDataset(&distances_, "scattering");
}
...
Scattering::initOptions(IOptionsContainer *options, TrajectoryAnalysisSettings *settings)
{
    static const char *const desc[] = {
        ...
        "[TT]-reference[TT]. The format of the reference should be q-value",
        "intensity (error) each on a separate line.[PAR]",
    ...
    options->addOption(DoubleOption("startq").store(&startQ_)
                           .storeIsSet(&bStartQSet_)
                           .defaultValue(0.0)
                           .description("smallest q value (nm)"));
    options->addOption(DoubleOption("endq").store(&endQ_)
                           .storeIsSet(&bEndQSet_)
                           .defaultValue(0.5)
                           .description("largest q value (nm)"));
    options->addOption(DoubleOption("qspacing").store(&qSpacing_)
                           .storeIsSet(&bQSpacingSet_)
                           .defaultValue(0.0)
                           .description("spacing of q values (nm)"));
    options->addOption(DoubleOption("coverage").store(&coverage_)
                           .storeIsSet(&bCoverageSet_)
                           .defaultValue(0.1)
                           .description("coverage of Monte Carlo (%)"));
    options->addOption(DoubleOption("modulation").store(&modulation_)
                           .storeIsSet(&bModulationSet_)
                           .defaultValue(2.3)
                           .description("random seed for Monte Carlo"));
    options->addOption(IntegerOption("numq").store(&numQ_)
                           .storeIsSet(&bNumQSet_)
                           .defaultValue(256)
                           .description("number of q values to evaluate"));
    options->addOption(IntegerOption("seed").store(&seed_)
                           .storeIsSet(&bSeedSet_)
                           .defaultValue(1)
                           .description("random seed for Monte Carlo"));
    options->addOption(BooleanOption("solvent").store(&bSolvent_)
                           .defaultValue(true)
                           .description("subtract solvent density"));
    ...
    options->addOption(BooleanOption("mc").store(&bMCScattering_)
                           .defaultValue(true)
...
{
    if (!bMCScattering_ && (bCoverageSet_ || bSeedSet_))
    {
        GMX_THROW(InvalidInputError("You cannot set -seed or -coverage unless you specify -mc"));
    }
    if ((bMCScattering_ && coverage_ <= 0) || coverage_ >= 1)
    {
    ...
    {
        if (bStartQSet_ || bEndQSet_ || bNumQSet_ || bQSpacingSet_)
        {
            std::string msg ="none of [-startq -endq -numq -qspacing] can be set if a reference is given";
            GMX_THROW(InconsistentInputError(msg));
        }
        readReferenceScattering();
        startQ_   = qList_[0];
        qSpacing_ = qList_[1] - startQ_;
        // q values should not be more than 2 so this tolerance is good for now
        auto spacingLambda = [this](double a, double b){ return std::abs(a-b) - qSpacing_ > 0.000001; };
    ...
    else if (qSpacing_ > 0)
    {
        ...
        if (qVal != endQ_)
        {
            GMX_THROW(InvalidInputError("The requested -qspacing did not result in an integer number of q values"));
        }
        numQ_ = qList_.size();
    }

    if (bModulationSet_ && scatterEnum_ == enNeutron)
    {
        GMX_THROW(InvalidInputError("Modulation has not been tested with neutron scattering yet and so cannot be used"));
    }
    if (!bModulationSet_)
    {
        modulation_ = 0; //no modulation factor
    }
}
    ...
    while (get_a_line(fp, line, STRLEN))
    {
        ...
        else
        {
            const std::string message = formatString(
                        "The reference scattering file, %s was not properly formatted.",
                        "The format of the reference should be q-value intensity (error) ",
                        "each on a separate line.\n", fnReferenceScattering_.c_str());
                         ...
                         const TopologyInformation        &top)
{
    ...
    for (size_t g = 0; g < sel_.size(); ++g)
    {
        ...
        {
            computeScattering_.push_back(compat::make_unique<XrayInfo>(isotopes, qList_, bSolvent_));
        }
        if (scatterEnum_ == enNeutron)
        {
            computeScattering_.push_back(compat::make_unique<NeutronInfo>(isotopes));
        }
        if (!bMCScattering_)
        {
        ...
        {
            scatteringAtQ0_.push_back(computeScattering_[g]->computeS0MC(sel_[g], coverage_, seed_));
        }
    }
    ...
    for (size_t g = 0; g < sel.size(); ++g)
    {
        ...
        for (size_t qi = 0; qi != qList_.size(); ++qi)
        {
            ...
            else
            {
                if (!bMCScattering_)
                {
                    Debeye = computeScattering_[g]->computeIntensity(pbc, qList_[qi], sel[g], modulation_);
                }
                else
                {
                    Debeye = computeScattering_[g]->computeIntensityMC(pbc, qList_[qi], sel[g], coverage_, seed_, modulation_);
                }
            }
            //TODO figure out best way to handle normalization and reference together
            if (bNormalize_)
            {
                Debeye /= scatteringAtQ0_[g];
            }            
