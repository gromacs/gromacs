...
Scattering::initOptions(IOptionsContainer *options, TrajectoryAnalysisSettings *settings)
{
    static const char *const desc[] = {
        ...
        " q can be written out with -o. It is possible to set the start of ",
        "the q-value range with -startq and the end of the q-value range ",
        "with -endq. The number of scattering points can be set with -numq",
        " or the spacing between q-values can be set with -qspacing. If ",
        "one of these values is set the other must explicitly be set to ",
        "zero. Comparison to a reference scattering curve can also be ",
        "performed, but the q-values must be equally spaced in the",
        "reference file provided by option -reference",
...
Scattering::optionsFinished(TrajectoryAnalysisSettings * /*settings*/)
{
    ...
    if (!fnReferenceScattering_.empty())
    {
        ...
        auto spacingLambda = [this](double a, double b){return a-b!=qSpacing_;};
