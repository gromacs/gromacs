...
class OutputManager
{
    public:
         ...
         */
        OutputManager() : trr_(nullptr), filetype_(efNR), sel_(nullptr), mtop_(nullptr), atoms_(nullptr)
        {}
        /*! \brief
         * Minimal used constructor with pointer to the correct selection used for file output.
         */
        explicit OutputManager(std::string name, Selection *sel, const gmx_mtop_t *mtop, t_atoms *atoms) : name_(name), trr_(nullptr), filetype_(efNR), sel_(sel), mtop_(mtop), atoms_(atoms)
        {
            strcpy(filemode_, "w");
        }
        /*! \brief
         ...
         */
        OutputManager(OutputManager &&old)
        {
            name_     = std::move(old.name_);
            trr_      = std::move(old.trr_);
            filetype_ = std::move(old.filetype_);
