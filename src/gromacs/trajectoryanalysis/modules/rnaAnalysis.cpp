    ...
    while (fp.readLine(&line))
    {
                  ...
                  (elements.size() >= 8) )
        {
            ...
            if (gmx_atomprop_query(atomprop_, epropMass, "???",
                                   aname.c_str(), &m))
            {
                bp->addBasePairAtom(x, anum, aname, rname, m);
            }
            else
            {
...
void RnaAnalysis::readTopology(const TopologyInformation &top)
{
    ...
    for (unsigned int i = 0; i < rnaRes.size(); i++)
    {
        ...
        if (NULL != ri)
        {
            ...
            if (NULL != debug)
            {
                fprintf(debug, "Just pushed a residue %c%d with %d atoms.\n",
...
{
    GMX_ASSERT((ri->residueType() != 'T'), "Thymine not supported!");
    if (NULL != debug)
    {
        fprintf(debug, "residueType %c, nAtoms %d\n", ri->residueType(), ri->nAtoms());
    }
                                  ...
                                  int                 vecSize)
{
    ...
    *RMSD = (*bpi)->computeRootMeanSquareDeviation(vec, vecSize);
    if (NULL != debug)
    {
        fprintf(debug, "template: %s %s; bases: %c%d %c%d; RMSD = %g\n",
                (*bpi)->bondName((*bpi)->bondType()).c_str(),
                isomerismName((*bpi)->isomerismType()),
                (*bpi)->nucleotideType(0), nucleotideInfo_[base1]->residueNumber(),
                (*bpi)->nucleotideType(1), nucleotideInfo_[base2]->residueNumber(),
                *RMSD);
    }
    return bond;
}
                               ...
                               TrajectoryAnalysisModuleData *pdata)
{
    ...
    for (unsigned int base1 = 0; base1 < nucleotideInfo_.size(); base1++)
    {
        ...
        while (pairSearch.findNextPair(&pair))
        {
            ...
            unsigned int dimerSize = 0;

            // Concatenate the two arrays and set values
            // Exclude atoms with atom types which should not be used during analysis
            // (e.g. hydrogen atoms)
            dimerSize = fillVector(nucleotideInfo_[base1], nucleotideInfo_[base2],
                                   sel.coordinates(), vec);
            dimerSize = fillVector(nucleotideInfo_[base2], nucleotideInfo_[base1],
        ...
        for (frameI = found.begin(); frameI != frameEnd; ++frameI)
        {
            ...
            for (posI = frameI->begin(); posI != posEnd; ++posI)
            {
                ...
                if (Xpm && (DB == "bps"))
                {
                    ...
                    matrix[posI->base2()][posI->base1()] = (*bpi)->bondType() +1;
                    // Only got 16 colours
                    ...
                    matrix[posI->base1()][posI->base2()] = (*bpi)->isomerismType() * 10;
                }
