...
class BasePair
{
    public:
        ...
        // Add an atom
        void addAtom(rvec x, std::string atomname, real m);
        ...
        {
            bondAtoms.push_back(bondAtom(atomName[ai], atomName[aj]));
        }
        ...
        // Return true if it is the same base pair
        bool swapNucleotides(resInfo * mol0, resInfo * mol1)
        {
            return (getNucleotides(1) ==  mol0->type) && (getNucleotides(0) ==  mol1->type);
        }
                          ...
                          const TopologyInformation        &top)
{
    ...
    std::string str1   = "amber99sb-ildn.ff/" + DB + ".pdb";
    ...
    for (int i = 0; i < nrResidues; i++)
    {
        for (; cAtom < iAtoms->nr; cAtom++)
        {
            ...
            // When the last atom in the resiude has been found
    ...
    while (NULL != fgets2(line, 100, file))
    {
        ...
        else if (NULL != bp)
        {
            ...
            else if ( (pdbType.compare("ATOM") == 0) || (pdbType.compare("HETATM") == 0))
            {
                ...
                if (gmx_atomprop_query(atomprop, epropMass, rname.c_str(),
                                       aname.c_str(), &m))
                {
                    bp->addAtom(x, aname, m);
                }
            }
            ...
            else if (pdbType.compare("ENDMDL") == 0)
            {
                ...
                reset_x_ndim(3, bp->getNrAtoms(),  NULL,
                             bp->getNrAtoms(), NULL,
                             bp->getPos(), bp->getMass());

                dataBase.push_back(*bp);
                delete bp;
                bp = NULL;
            }
        }
                          ...
                          TrajectoryAnalysisModuleData *pdata)
{
    ...
    if (fr.time == 0)
    {
        std::cout << "Using an RNA structure file or just one frame of an MD trajectory." << std::endl << "Please, check your RNA structure file or your  input trajectory." << std::endl << std::endl;
    }
    else
    {
        std::cout << "Using " << fr.time << " frames of input MD trajectory." << std::endl << "Please, check your input trajectory." << std::endl << std::endl;
    }
    ...
    for (int base1 = 0; base1 < nrResidues; base1++)
    {
        ...
        while (pairSearch.findNextPair(&pair))
        {
            ...
            int totalSize = residueInfo[base1].size + residueInfo[base2].size;

            // Concatenate the two arrays and set values
            for (int i = 0; i < totalSize; i++)
            {
                // Exclude invalid atom types ( hydrogen etc)
                if (residueInfo[base1].size > i)
                {
                    if (residueInfo[base1].invalid[i])
                    {
                        adjustment++;
                        continue;
                    }
                    ...
                    vec[(i - adjustment)][0] = sel.coordinates()[i +residueInfo[base1].start][0];
                    vec[(i - adjustment)][1] = sel.coordinates()[i +residueInfo[base1].start][1];
                    vec[(i - adjustment)][2] = sel.coordinates()[i +residueInfo[base1].start][2];

                }           // end if
                else
                {
                    if (residueInfo[base2].invalid[i - residueInfo[base1].size])
                    {
                        adjustment++;
                        continue;
                    }
                    // Sets the second part of the arrays
                    vec[(i - adjustment)][0] = sel.coordinates()[i +residueInfo[base2].start - residueInfo[base1].size][0];
                    vec[(i - adjustment)][1] = sel.coordinates()[i +residueInfo[base2].start - residueInfo[base1].size][1];
                    vec[(i - adjustment)][2] = sel.coordinates()[i +residueInfo[base2].start - residueInfo[base1].size][2];
            ...
            adjustment = 0;
            totalSize  = residueInfo[base1].size + residueInfo[base2].size;
            for (int i = 0; i < totalSize; i++)
            {
                if (residueInfo[base2].size > i)
                {
                    if (residueInfo[base2].invalid[i])
                    {
                        adjustment++;
                        continue;
                    }
                    vec_swap[(i - adjustment)][0] = sel.coordinates()[i +residueInfo[base2].start][0];
                    vec_swap[(i - adjustment)][1] = sel.coordinates()[i +residueInfo[base2].start][1];
                    vec_swap[(i - adjustment)][2] = sel.coordinates()[i +residueInfo[base2].start][2];

                }
                else
                {
                    if (residueInfo[base1].invalid[i - residueInfo[base2].size])
                    {
                        adjustment++;
                        continue;
                    }
                    vec_swap[(i - adjustment)][0] = sel.coordinates()[i +residueInfo[base1].start - residueInfo[base2].size][0];
                    vec_swap[(i - adjustment)][1] = sel.coordinates()[i +residueInfo[base1].start - residueInfo[base2].size][1];
                    vec_swap[(i - adjustment)][2] = sel.coordinates()[i +residueInfo[base1].start - residueInfo[base2].size][2];
                }
            }
            totalSize = totalSize - adjustment;
            ...
            for (int i = 0; i < nrTemplates; i++)
            {
                ...
                // check if there are missing atoms in base - if there are missing atoms, base won't be used
                if (((dataBase[i].sameNucleotides(&residueInfo[base1], &residueInfo[base2])) ||
                     ((dataBase[i].swapNucleotides(&residueInfo[base1], &residueInfo[base2])) &&
                      (DB == "bps"))) &&
                    (((residueInfo[base1].type == 'A') && (residueInfo[base1].size >= 30)) ||
                     ((residueInfo[base1].type == 'U') && (residueInfo[base1].size >= 28)) ||
                     ((residueInfo[base1].type == 'G') && (residueInfo[base1].size >= 31)) ||
                     ((residueInfo[base1].type == 'C') && (residueInfo[base1].size >= 29))) &&
                    (((residueInfo[base2].type == 'A') && (residueInfo[base2].size >= 30)) ||
                     ((residueInfo[base2].type == 'U') && (residueInfo[base2].size >= 28)) ||
                     ((residueInfo[base2].type == 'G') && (residueInfo[base2].size >= 32)) ||
                     ((residueInfo[base2].type == 'C') && (residueInfo[base2].size >= 29))))
                {
                    ...
                    if ((dataBase[i].swapNucleotides(&residueInfo[base1], &residueInfo[base2])) &&
                        ((DB == "bps") && (DB != "hbonds")))
                    {
                        base1_new = base2;
                        base2_new = base1;
                    ...
                    if (!dimReset && bRMSD)
                    {
                        ...
                        if ((dataBase[i].sameNucleotides(&residueInfo[base1], &residueInfo[base2])) && ((DB == "bps") || (DB == "hbonds")))
                        {
                            reset_x_ndim(3, baseSize,  NULL,
                                         baseSize, NULL, vec, dataBase[i].getMass());
                            dimReset = true;
                        }
                        if ((dataBase[i].swapNucleotides(&residueInfo[base1], &residueInfo[base2])) && ((DB == "bps") && (DB != "hbonds")))
                        {
                            reset_x_ndim(3, baseSize,  NULL,
                    ...
                    bond = dataBase[i].getHBondDistance(base1_new, base2_new, bondDist, sel);
                    ...
                    if (bRMSD && bond)
                    {
                        ...
                        if ((dataBase[i].swapNucleotides(&residueInfo[base1], &residueInfo[base2])) && ((DB == "bps") && (DB != "hbonds")))
                        {
                            RMSD = dataBase[i].getRMSD(vec_swap, baseSize);
                        }
                    }
                    ...
                    if ((!bRMSD || (max > RMSD)) && bond)
                    {
                        ...
                        if (everyBPList)
                        {
                            if (residueInfo[base1_new].resNr <= residueInfo[base2_new].resNr)
                            {
                                std::cout << "base pair " << residueInfo[base1_new].type << residueInfo[base1_new].resNr << " - " << residueInfo[base2_new].type << residueInfo[base2_new].resNr;

                                if ((DB == "bps") && (pdbTemplates[minIndex].substr(8, 3) == "cis"))
                                {
                                    std::cout << "\t\t" << pdbTemplates[minIndex].substr(3, 4) << " cis";
                                }
                                if ((DB == "bps") && (pdbTemplates[minIndex].substr(8, 5) == "trans"))
                                {
                                    std::cout << "\t\t" << pdbTemplates[minIndex].substr(3, 4) << " trans";
                                }
                                if (Info)
                                {
                                    std::cout << std::endl << "RMSD to template: " << RMSD << std::endl;
                                }
                            }
                            else
                            {
                                std::cout << "base pair " << residueInfo[base2_new].type << residueInfo[base2_new].resNr << " - " << residueInfo[base1_new].type << residueInfo[base1_new].resNr;
                                if ((DB == "bps") && (pdbTemplates[minIndex].substr(8, 3) == "cis"))
                                {
                                    std::cout << "\t\t" << pdbTemplates[minIndex].substr(5, 2) << pdbTemplates[minIndex].substr(3, 2) << " cis";
                                }
                                if ((DB == "bps") && (pdbTemplates[minIndex].substr(8, 5) == "trans"))
                                {
                                    std::cout << "\t\t" << pdbTemplates[minIndex].substr(5, 2) << pdbTemplates[minIndex].substr(3, 2) << " trans";
                                }
                                if (Info)
                                {
                                    std::cout << std::endl << "RMSD to template: " << RMSD << std::endl;
...
// Add one atom.
void
BasePair::addAtom(rvec x, std::string atomname, real m)
{
    atomName.push_back(atomname);
    srenew(atomPos, 1+atomName.size());
    srenew(atomMass, 1+atomName.size());
    copy_rvec(x, atomPos[atomName.size()]);
    atomMass[atomName.size()] = m;
}
