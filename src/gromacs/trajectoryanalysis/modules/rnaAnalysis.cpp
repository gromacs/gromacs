                         ...
                         TrajectoryAnalysisSettings *settings)
{
    ...
    // Add option for optional output plot file
...
bool RnaAnalysis::analyzeBasePair(BasePairPtrIterator bpi,
                                  ...
                                  size_t              vecSize)
{
    bool bond = (*bpi)->isHBond(base1, base2,
                                pbc, bondDist_, sel);
    *RMSD = (*bpi)->computeRootMeanSquareDeviation(base1, base2, vec, vecSize);
    ...
    return (*RMSD < 0.05) || (*RMSD < 0.22 && bond);
}
                               ...
                               TrajectoryAnalysisModuleData *pdata)
{
    ...
    for (size_t base1i = 0; base1i < nucleotideInfo_.size(); base1i++)
    {
        ...
        while (pairSearch.findNextPair(&pair))
        {
            ...
            auto         base1 = *nucleotideInfo_[base1i];
            auto         base2 = *nucleotideInfo_[base2i];

            PairInfo     tempPair(base1i, base2i, 1000.0, templateDB_.end());
        ...
        for (frameI = found.begin(); frameI != frameEnd; ++frameI)
        {
            ...
            if (Xpm && (DB == "bps"))
            {
                ...
                write_xpm_split(file, 0, titel, legend,
                                ...
                                axis.data(), axis.data(), matrix.data(), 0.0, 16.0, &nlevels, rlo, rhi, 0.0, 16.0, &nlevels, true, rlo, rhi);
...
bool RnaAnalysis::isValid(const char * name)
{
    ...
    if ((!phosphateRmsd_ && strchr(name, 'P') != NULL) ||
        ...
        (strchr(name, '\'') != NULL && strchr(name, 'H') != NULL))
    {
        return false;
    }
