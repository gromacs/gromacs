...
void RnaAnalysis::printBasePair(const PairInfo &tempPair)
{
    ...
    fprintf(rnaLog, "base pair %c%zu - %c%zu",
                                  ...
                                  size_t              vecSize)
{
    ...
    *RMSD = (*bpi)->computeRootMeanSquareDeviation(base1, base2, vec, vecSize);
    ...
    real mirrored_RMSD = (*bpi)->computeRootMeanSquareDeviation(base1, base2, mirrored_vec, vecSize);
        ...
        while (pairSearch.findNextPair(&pair))
        {
                 ...
                 (bpi < templateDB_.end()); ++bpi)
            {
                ...
                if (score < tempPair.score())
                {
                    ...
                    tempPair.setMirror(swap ? swap_mirror : match_mirror);
...
RnaAnalysis::writeOutput()
{
    if ((Xpm && (DB == "bps")) || statistic)
    {
        ...
        std::vector<std::string> pdbStatistic;
        ...
        if (statistic)
        {
                 ...
                 (bpi < templateDB_.end()); ++bpi)
            {
                ...
                if (DB == "bps")
                {
                    snprintf(buf, sizeof(buf), "%s%s_%c%c-%s-%s_%c%c-%s-%s.pdb",
                             temp,
                             isomerismName((*bpi)->isomerismType()),
                             ...
                             (*bpi)->nucleotideTypeChar(1),
                             (*bpi)->nucleotideTypeChar(0),
                             (*bpi)->bondName(true).c_str(),
                             (*bpi)->subType(true).c_str());
                }
                else
                {
                ...
                pdbStatistic.push_back(std::string(buf));
        ...
        for (frameI = found.begin(); frameI != frameEnd; ++frameI)
        {
            ...
            for (posI = frameI->begin(); posI != posEnd; ++posI)
            {
                ...
                if (statistic)
                {
                    ...
                    char title[100];
                    sprintf(title, "%g %g%s%s", posI->score(), posI->RMSD(),
                            posI->swap() ? " swap" : "", posI->mirror() ? " mirror" : "");
                    
                    // Write a pdb file
                    const char *mode = model_nr[pairIndex] == 1 ? "w+" : "a+";
                    FILE *outfile = gmx_fio_fopen(pdbStatistic[pairIndex].c_str(), mode);
                    write_pdbfile_indexed(outfile, title, iAtoms,
                                          frVec->vec, (*bpi)->getEpbc(),
                                          *(*bpi)->box(),
                                          'A',
                                          model_nr[pairIndex],
                                          dimerSize, atomId, NULL, false);
                    gmx_fio_fclose(outfile);
                    model_nr[pairIndex]++;
                }
                if (Xpm && (DB == "bps"))
                {
