...
class BasePair
{
    public:
        ...
        {
            if (getNucleotides(1) ==  mol0->type && getNucleotides(0) ==  mol1->type)
            {
                return true;
            }
            else
                          ...
                          const TopologyInformation        &top)
{
    ...
    struct _finddata_t   finddata       = (struct _finddata_t)0;
    int                  first          = 0;
    long                 hFile          = 0;

    // Input path option
    std::string  tempstr = "amber99sb-ildn.ff/RNAtemplates/" + DB + "/";
    const char * dirname = tempstr.c_str();
    int          len;
    char       * tmpname = 0;
    ...
    for (int base1 = 0; base1 < nrResidues; base1++)
    {
        ...
        while (pairSearch.findNextPair(&pair))
        {
            ...
            for (int i = 0; i < nrTemplates; i++)
            {
                ...
                if (((dataBase[i].sameNucleotides(&residueInfo[base1], &residueInfo[base2])) || ((dataBase[i].swapNucleotides(&residueInfo[base1], &residueInfo[base2])) && (DB == "bps"))) && (((residueInfo[base1].type == 'A') && (residueInfo[base1].size >= 30)) || ((residueInfo[base1].type == 'U') && (residueInfo[base1].size >= 28)) || ((residueInfo[base1].type == 'G') && (residueInfo[base1].size >= 31)) || ((residueInfo[base1].type == 'C') && (residueInfo[base1].size >= 29))) && (((residueInfo[base2].type == 'A') && (residueInfo[base2].size >= 30)) || ((residueInfo[base2].type == 'U') && (residueInfo[base2].size >= 28)) || ((residueInfo[base2].type == 'G') && (residueInfo[base2].size >= 32)) || ((residueInfo[base2].type == 'C') && (residueInfo[base2].size >= 29))))
                {
