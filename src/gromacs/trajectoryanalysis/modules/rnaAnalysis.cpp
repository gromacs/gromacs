                          ...
                          const TopologyInformation        &top)
{
        ...
        for (; cAtom <= iAtoms->nr; cAtom++)
        {
            ...
            else if (rnaRes[i] == iAtoms->atom[cAtom].resind)
            {
                if (!taken)
                {
                    taken                   = true;
                    ri.start                = cAtom;
                    ri.type                 = *iAtoms->resinfo[rnaRes[i]].name[0];
                    ri.resNr                = iAtoms->resinfo[rnaRes[i]].nr;
                }
                // If it is an invalid atom
                ri.invalid[currentNrAtoms] = !getAtomType(*iAtoms->atomname[cAtom], &type);
                // Set atom name
                ri.name[currentNrAtoms] = *iAtoms->atomname[cAtom];
                currentNrAtoms++;
            }
        }
    ...
    fprintf(rnaLog, "Number of RNA bases in input file: %lu \n", rnaRes.size());
                          ...
                          TrajectoryAnalysisModuleData *pdata)
{
    ...
    if (statistic)
    {
        ...
        {
            stop = nucleotideInfo_[r].start + nucleotideInfo_[r].size;
            for (int i = nucleotideInfo_[r].start; i < stop; i++)
            {
    ...
    {
        repVec[i][0] = sel.coordinates()[ri->start+offsetAtom][0];
        repVec[i][1] = sel.coordinates()[ri->start+offsetAtom][1];
        repVec[i][2] = sel.coordinates()[ri->start+offsetAtom][2];
    }
    ...
    for (unsigned int base1 = 0; base1 < nucleotideInfo_.size(); base1++)
    {
        ...
        while (pairSearch.findNextPair(&pair))
        {
            ...
            int totalSize = nucleotideInfo_[base1].size + nucleotideInfo_[base2].size;
            ...
            for (int i = 0; i < totalSize; i++)
            {
                ...
                if (nucleotideInfo_[base1].size > i)
                {
                    ...
                    vec[(i - adjustment)][0] = sel.coordinates()[i +nucleotideInfo_[base1].start][0];
                    vec[(i - adjustment)][1] = sel.coordinates()[i +nucleotideInfo_[base1].start][1];
                    vec[(i - adjustment)][2] = sel.coordinates()[i +nucleotideInfo_[base1].start][2];

                }
                else
                {
                    if (nucleotideInfo_[base2].invalid[i - nucleotideInfo_[base1].size])
                    {
                        adjustment++;
                        continue;
                    }
                    // Sets the second part of the arrays
                    vec[(i - adjustment)][0] = sel.coordinates()[i +nucleotideInfo_[base2].start - nucleotideInfo_[base1].size][0];
                    vec[(i - adjustment)][1] = sel.coordinates()[i +nucleotideInfo_[base2].start - nucleotideInfo_[base1].size][1];
                    vec[(i - adjustment)][2] = sel.coordinates()[i +nucleotideInfo_[base2].start - nucleotideInfo_[base1].size][2];
            ...
            for (int i = 0; i < totalSize; i++)
            {
                if (nucleotideInfo_[base2].size > i)
                {
                    ...
                    vec_swap[(i - adjustment)][0] = sel.coordinates()[i +nucleotideInfo_[base2].start][0];
                    vec_swap[(i - adjustment)][1] = sel.coordinates()[i +nucleotideInfo_[base2].start][1];
                    vec_swap[(i - adjustment)][2] = sel.coordinates()[i +nucleotideInfo_[base2].start][2];

                }
                else
                {
                    if (nucleotideInfo_[base1].invalid[i - nucleotideInfo_[base2].size])
                    {
                        adjustment++;
                        continue;
                    }
                    vec_swap[(i - adjustment)][0] = sel.coordinates()[i +nucleotideInfo_[base1].start - nucleotideInfo_[base2].size][0];
                    vec_swap[(i - adjustment)][1] = sel.coordinates()[i +nucleotideInfo_[base1].start - nucleotideInfo_[base2].size][1];
                    vec_swap[(i - adjustment)][2] = sel.coordinates()[i +nucleotideInfo_[base1].start - nucleotideInfo_[base2].size][2];
                }
            }
                 ...
                 (bpi < dataBase_.end()); ++bpi)
            {
                ...
                if ( ((*bpi)->checkTemplate(nucleotideInfo_[base1].type,
                                            nucleotideInfo_[base2].type)) &&
                     (((nucleotideInfo_[base1].type == 'A') && (nucleotideInfo_[base1].size >= 30)) ||
                      ((nucleotideInfo_[base1].type == 'U') && (nucleotideInfo_[base1].size >= 28)) ||
                      ((nucleotideInfo_[base1].type == 'G') && (nucleotideInfo_[base1].size >= 31)) ||
                      ((nucleotideInfo_[base1].type == 'C') && (nucleotideInfo_[base1].size >= 29))) &&
                     (((nucleotideInfo_[base2].type == 'A') && (nucleotideInfo_[base2].size >= 30)) ||
                      ((nucleotideInfo_[base2].type == 'U') && (nucleotideInfo_[base2].size >= 28)) ||
                      ((nucleotideInfo_[base2].type == 'G') && (nucleotideInfo_[base2].size >= 32)) ||
                      ((nucleotideInfo_[base2].type == 'C') && (nucleotideInfo_[base2].size >= 29))))
                {
                    ...
                    bool bondNonSwap = (*bpi)->getHBondDistance(nucleotideInfo_[base1],
                                                                nucleotideInfo_[base2],
                                                                bondDist, sel);
                    bool bondSwap    = (*bpi)->getHBondDistance(nucleotideInfo_[base1_swap],
                    ...
                    if (bRMSD && (bondNonSwap || bondSwap))
                    {
                        ...
                        //std::cout << "template: " << (*bpi)->getBondName((*bpi)->getBondType()) << "; bases: " << (*bpi)->nucleotideType(0) << nucleotideInfo_[base1].resNr << " " << (*bpi)->nucleotideType(1) << nucleotideInfo_[base2].resNr << "; bondNonSwap = " << bondNonSwap << ", bondSwap = " << bondSwap << "; RMSDNonSwap = " << RMSDNonSwap << "; RMSDSwap: " << RMSDSwap << std::endl;
                    }
                    ...
                    if ((!bRMSD || (maxRMSD+(*bpi)->getTemplateRMSD() > RMSD)) && bond)
                    {
                        if (max > RMSD)
                        {
                            ...
                            if (everyBPList)
                            {
                                if (nucleotideInfo_[base1_swap].resNr <= nucleotideInfo_[base2_swap].resNr)
                                {
                                    fprintf(rnaLog, "base pair %c%i - %c%i",
                                            nucleotideInfo_[base1_swap].type,
                                            nucleotideInfo_[base1_swap].resNr,
                                            nucleotideInfo_[base2_swap].type,
                                            nucleotideInfo_[base2_swap].resNr);
                                ...
                                {
                                    fprintf(rnaLog, "base pair %c%i - %c%i", nucleotideInfo_[base2_swap].type, nucleotideInfo_[base2_swap].resNr, nucleotideInfo_[base1_swap].type, nucleotideInfo_[base1_swap].resNr);
                                ...
                                if (Info)
                                {
                                    for  (int con = 0; con < dataBase_[minIndex]->getNrBondAtoms(); con++)
                                    {
                                        ...
                                        atomId1 = nucleotideInfo_[tempPair.base1].start + atomId1;
                                        atomId2 = nucleotideInfo_[tempPair.base2].start + atomId2;
...
RnaAnalysis::writeOutput()
{
    ...
    if ((Xpm && (DB == "bps")) || statistic)
    {
        ...
        {
            axis[i] = nucleotideInfo_[i].resNr;
        }
        ...
        for (frameI = found.begin(); frameI != frameEnd; ++frameI)
        {
            ...
            for (posI = frameI->begin(); posI != posEnd; ++posI)
            {
                ...
                {
                    adjustment = 0;
                    int totalSize = nucleotideInfo_[posI->base1].size + nucleotideInfo_[posI->base2].size;
                    // Concatenate the two arrays and set values to them
                    for (int i = 0; i < totalSize; i++)
                    {
                        // Exclude invalid atom types ( hydrogen etc)
                        if (nucleotideInfo_[posI->base1].size > i)
                        {
                            ...
                            atomId[(i - adjustment)] = i + nucleotideInfo_[posI->base1].start;
                        }
                        else
                        {
                            if (nucleotideInfo_[posI->base2].invalid[i - nucleotideInfo_[posI->base1].size])
                            {
                                adjustment++;
                                continue;
                            }
                            // Sets the second part of the array
                            atomId[(i - adjustment)] = i +nucleotideInfo_[posI->base2].start - nucleotideInfo_[posI->base1].size;
                        }
                    }
                    totalSize = totalSize - adjustment;
...
{
    for (int x = 0; x < nucleotideInfo_[i].size; x++)
    {
        if (strcmp(name, nucleotideInfo_[i].name[x]) == 0)
        {
            return x;
        }
    }
