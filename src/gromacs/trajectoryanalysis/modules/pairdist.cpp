...
//! Enum value to store the selected value for `-refgroup`/`-selgroup`.
enum GroupType
{
    eGroupType_All,
    eGroupType_Residue,
...
PairDistance::PairDistance()
    : TrajectoryAnalysisModule(PairDistanceInfo::name, PairDistanceInfo::shortDescription),
      ...
      refGroupCount_(0), maxGroupCount_(0), initialDist2(0.0), cutoff2_(0.0)
{
...
PairDistance::initOptions(Options *options, TrajectoryAnalysisSettings * /*settings*/)
{
    static const char *const desc[] = {
        ...
        "selection (givent with [TT]-ref[tt]) and one or more other selections",
        ...
        "in [TT]-ref[tt]), use [TT]-refgroup[tt] and/or [TT]-selgroup[tt]",
        "to specify how the selections should be grouped.[PAR]",
    ...
    options->addOption(StringOption("refgroup").storeEnumIndex(&refGroupType_)
                           .defaultEnumIndex(0).enumValue(c_groupTypes)
                           .description("Group of -ref positions to compute the min/max over"));
    options->addOption(StringOption("selgroup").storeEnumIndex(&selGroupType_)
                           .defaultEnumIndex(0).enumValue(c_groupTypes)
                           .description("Group of -sel positions to compute the min/max over"));
...
class PairDistanceModuleData : public TrajectoryAnalysisModuleData
{
    public:
         ...
         *
         * This is needed to identify whether there were additional pairs
         * outside the cutoff that were not covered by the neihborhood search.
         */
