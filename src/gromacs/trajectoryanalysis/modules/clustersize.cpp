...
class ClusterSize : public TrajectoryAnalysisModule
{
    ...
    private:
        ...
        Selection                         sel_;
        //! Data structure for the maxclust file (largest cluster)
        AnalysisData                      maxclust_;
        //! Data structure for the nclust file (number of clusters)
        AnalysisData                      nclust_;
        //! Data structure for the avclust file (average cluster size)
        AnalysisData                      avclust_;
        //! Internal data structure for histogramming
        AnalysisDataAverageModulePointer  adata_;
        //! Count molecules rather than atoms
        bool                              mol_;
        //! Number of molecules
        size_t                            nmol_;
        //! Cut-off for determining whether two atoms are part of a cluster
        double                            cutoff_;
        //! Color RGB for the lower bound in matrix - default red
        RVec                              rlo_;
        //! Color RGB for the higher bound in matrix - default blue
        RVec                              rhi_;
        //! Number of levels in xpm output
        int                               nlevels_;
        //! Number of frames to skip in analysis
        int                               nskip_;
        //! Internal structure
...
//! Select selection group
static int initSelectionGroups(Selection *sel, const gmx_mtop_t *top, bool mol)
{
    if (mol)
    {
