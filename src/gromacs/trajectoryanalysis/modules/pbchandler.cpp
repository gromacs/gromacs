...
void Pbchandler::setBox(const matrix box)
{
    copy_mat(box, box_);
}
...
Pbchandler::doPBC(const t_trxframe *input, t_trxframe *output, const gmx_mtop_t *mtop)
{
    switch (selPBCType_)
    {
        case (ePBCTypeNoJump):
            removeJump(output);
            centerSystem(input, output, selCenteringType_);
            break;
        case (ePBCTypeCluster):
            cluster(input, output, mtop);
            centerSystem(input, output, selCenteringType_);
            break;
        case (ePBCTypeAtom):
            centerSystem(input, output, selCenteringType_);
...
Pbchandler::shiftCoord(const int atomStart, const int atomEnd, rvec shift, rvec *coord, const bool checkValid)
{
    ...
    if (checkValid)
    {
        ...
        {
            size_t pos = std::find(indices.begin(), indices.end(), i) - indices.begin();
            if (pos < indices.size())
            {
...
void
Pbchandler::centerSystem(const t_trxframe *input, t_trxframe *output, int centeringType)
{
    ...
    for (int i = 0; i < natoms; i++)
    {
        for (int m = 0; m < DIM; m++)
        {
            if (input->x[i][m] < cmin[m])
            {
                cmin[m] = input->x[i][m];
            }
            else if (input->x[i][m] > cmax[m])
            {
                cmax[m] = input->x[i][m];
            }
        }
    ...
    calc_box_center(centeringType, box_, boxCenter);
    RVec dx(0, 0, 0);
    for (int m = 0; m < DIM; m++)
    {
...
void
Pbchandler::cluster(const t_trxframe *input, t_trxframe *output, const gmx_mtop_t *mtop)
{
    t_pbc     pbc;

    RVec      boxCenter(0, 0, 0);

    calc_box_center(selCenteringType_, box_, boxCenter);

    /* Initiate the pbc structure */
    std::memset(&pbc, 0, sizeof(pbc));
    set_pbc(&pbc, output->ePBC, box_);
     ...
     */
    real              min_dist2   = 10*gmx::square(trace(box_));
    int               imol_center = -1;
    int               ncluster    = 0;
    ...
    while (nadded < ncluster)
    {
        ...
        min_dist2   = 10*gmx::square(trace(box_));
        int imin        = -1;
        int jmin        = -1;
...
Pbchandler::comAtom(t_trxframe *output)
{
    ...
    switch (selUnitCellType_)
    {
        case eUnitCellTypeRectangular:
            put_atoms_in_box(output->ePBC, box_, positionsArrayRef);
            break;
        case eUnitCellTypeTriclinic:
            put_atoms_in_triclinic_unitcell(selCenteringType_, box_, positionsArrayRef);
            break;
        case eUnitCellTypeCompact:
            put_atoms_in_compact_unitcell(output->ePBC, selCenteringType_, box_, positionsArrayRef);
            break;
...
Pbchandler::comMolecule(const t_trxframe *input, t_trxframe *output, const gmx_mtop_t *mtop)
{
    ...
    for (int i = 0; (i < nMols); i++)
    {
        ...
        {
            case eUnitCellTypeRectangular:
                put_atoms_in_box(output->ePBC, box_, newComArrayRef);
                break;
            case eUnitCellTypeTriclinic:
                put_atoms_in_triclinic_unitcell(selCenteringType_, box_, newComArrayRef);
                break;
            case eUnitCellTypeCompact:
                put_atoms_in_compact_unitcell(output->ePBC, selCenteringType_, box_, newComArrayRef);
                break;
        }
        RVec shift(0, 0, 0);
