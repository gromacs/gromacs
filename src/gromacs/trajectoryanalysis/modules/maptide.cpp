...
const char *const cDensityTypeEnum[] = {"electron-scattering", "cryo-EM", "vdW", "charge"};
...
class TimeAveragedDensity final : public TrajectoryAnalysisModule
{
    ...
    private:
        ...
        //! The library file with the spreading parameters for electron scattering
        ...
        GridDataFloat3D              referenceMap_;
        //! The computed timed-averaged map
        GridDataFloat3D              averageMap_;
        //! The computed timed-averaged standard deviation map
        GridDataFloat3D              stddevMap_;
        //! The block-averaged map
        GridDataFloat3D              blockAverageMap_;
                                      ...
                                      TrajectoryAnalysisSettings *settings)
{
    static const char *const desc[] = {
        ...
        "",
        "By default ([TT]-type electron-scattering[tt]), 4 Gaussian coefficients from"
        ...
        " maps, masked points will be set to zero or any other, user specified"
        ...
        const TopologyInformation                   &top)
{
    ...
    {
        ArrayRef<const t_atom> atoms({top.atoms()->atom,
                                      top.atoms()->atom+top.atoms()->nr});
    ...
    Grid < DIM, GridWithTranslation < DIM>> referenceGrid;
    ...
    if (!fnReferenceMap_.empty())
    {
        ...
        referenceGrid       = referenceMap_.getGrid();
    }
    else
    {
        output_("Using reference topologly to construct density grid");
        referenceGrid = encompassingGridFromCoordinates(top.x(), spacing_, margin_);
    }

    averageMap_.setGrid(referenceGrid);
    stddevMap_.setGrid(referenceGrid);
    ...
    {
        blockAverageMap_.setGrid(referenceGrid);
        if (fnReferenceMap_.empty())
        {
...
std::map < int, std::vector < GaussianParameters>> TimeAveragedDensity::setGaussianCoeff(bool hasSigma)
{
    ...
    else
    {
        ...
        {
            case enXRay:
                output_("Reading default Gaussian coefficients from '" + eScatteringDatabase_ + "' .");
                parameterFile = gmx::openLibraryFile(eScatteringDatabase_);
                break;
            case enCryoEM:
                output_("Reading electron microscopy Gauss weights from '" + cryoEmScatteringDatabase_ + "'");
                parameterFile = gmx::openLibraryFile(cryoEmScatteringDatabase_);
                break;
            default:
                break;
        }
    ...
    if (parameterFile == nullptr && !hasSigma && densityType_ != enVdW)
    {
        GMX_THROW(FileIOError("Cannot open file " + fnGaussParameters_ +
                              " or library file for reading and no spreading option -sigma was given. "
                              "Exiting now because cannot reasonably guess how to spread atom density."));
    }
    ...
    if (parameterFile != nullptr)
    {
        ...
        while (get_a_line(parameterFile.get(), line, maxLineLength))
        {
            int   currentAtomType;
            float amplitude;
            if (hasSigma)
            ...
            else
            {
                ...
                {
                    gaussianParametersLookup[currentAtomType].push_back(
                            {amplitude, precision});
                }
            }
            char atomName[maxLineLength];
            if (sscanf(line, "%d %s ", &currentAtomType, atomName))
            {
                atomicNumberTable_[atomName] = currentAtomType;
            }
        }
