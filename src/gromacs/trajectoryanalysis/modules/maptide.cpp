...
class TimeAveragedDensity final : public TrajectoryAnalysisModule
{
    ...
    private:
        ...
        std::map<std::string, real> atomicNumberTable_;                                        //< Translate between atom name and atomic number
        ...
        if (top.hasTopology())
        {
            ...
            for (int i = 0; i < atoms.nr; ++i)
            {
                ...
                if (atomicNumber < 0 || !atomicNumberTable_.empty())
                {
                    ...
                    while (atomicNumberTable_.find(atomName) == atomicNumberTable_.end() && atomName.size() != 0)
                    {
                        atomName.pop_back();
                    }
                    if (atomName.size() == 0)
