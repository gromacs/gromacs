...
#include "gromacs/legacyheaders/pdbio.h"
                  ...
                  const TopologyInformation        &top)
{
    ...
    index_.reserve(surfaceSel_.posCount());
    if (bDGsol)
    {
...
void computeAreas(const Selection &surfaceSel, const Selection &sel, bool bMap,
                  ...
                  std::vector<real> *resAreaWork)
{
    ...
    {
        const int  ii = bMap ? sel.position(i).mappedId() : i;
                  ...
                  TrajectoryAnalysisModuleData *pdata)
{
    ...
    const bool           bResAt    = !res_a_.empty();
    ...
    int   retval = nsc_dclm_pbc(surfaceSel.coordinates().data(), &radii_[0],
                                ...
                                &index_[0], pbc != NULL ? pbc->ePBC : epbcNONE,
    ...
    if (bResAt)
    {
        computeAreas(surfaceSel, surfaceSel, false, area, dgsFactor_,
                     &totalArea, &dgsolv, aah, rah, &res_a_);
    }
    for (size_t g = 0; g < outputSel.size(); ++g)
    {
        ...
        computeAreas(surfaceSel, outputSel[g], true, area, dgsFactor_,
                     &totalArea, &dgsolv, aah, rah, &res_a_);
