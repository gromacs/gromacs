...
void
Filehandler::initFrame(const t_trxframe *input)
{
    newFrame_ = *input;
}
...
void
Filehandler::modifyFrame()
{
    const Selection *sel    = &sel_;
    int              natoms = sel->atomCount();

    t_trxframe       temp;
    clear_trxframe(&temp, true);
    temp = newFrame_;

    temp.time   = writeDoubles_.time;
    temp.bV     = (newFrame_.bV && writeBool_.bV);
    temp.bF     = (newFrame_.bF && writeBool_.bF);
    temp.natoms = natoms;
    temp.bPrec  = (newFrame_.bPrec && writeBool_.bP);
    temp.prec   = writeDoubles_.prec;
    temp.atoms  = &atoms_;
    temp.bAtoms = writeBool_.bA;
    ...
    for (int i = 0; i < natoms; i++)
    {
        ...
        copy_rvec(newFrame_.x[pos], temp.x[i]);
        if (temp.bV)
        {
            copy_rvec(newFrame_.v[pos], temp.v[i]);
        }
        if (temp.bF)
        {
            copy_rvec(newFrame_.f[pos], temp.f[i]);
        }
    }
    newFrame_ = temp;
    done_frame(&temp);
...
void
Filehandler::initWriteBool(t_writeFileBool *writeBool)
{
    writeBool->bGC     = false;
    writeBool->bP      = false;
    writeBool->bV      = false;
...
void
Filehandler::initWriteDoubles(t_writeFileDoubles *writeDouble)
{
    writeDouble->prec = 0.0;
    writeDouble->time = 0.0;
}
