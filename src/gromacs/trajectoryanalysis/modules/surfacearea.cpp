                 ...
                 atom_id index[], int ePBC, matrix box)
{
    ...
    if (box)
    {
        ...
        nxbox = std::max(1.0, floor(norm(box[XX])/ra2max));
        nybox = std::max(1.0, floor(norm(box[YY])/ra2max));
        nzbox = std::max(1.0, floor(norm(box[ZZ])/ra2max));
        if (debug)
        {
    ...
    else
    {
        ...
        for (iat_xx = 0; (iat_xx < nat); iat_xx++)
        {
            iat           = index[iat_xx];
            pco           = coords[iat];
            i             = (int) std::max(floor((pco[XX]-xmin)/ra2max), 0.0);
            i             = std::min(i, nxbox-1);
            j             = (int) std::max(floor((pco[YY]-ymin)/ra2max), 0.0);
            j             = std::min(j, nybox-1);
            l             = (int) std::max(floor((pco[ZZ]-zmin)/ra2max), 0.0);
            l             = std::min(l, nzbox-1);
            i             = i+j*nxbox+l*nxy;
            wkat1[iat_xx] = i;
            wkbox[i]++;
