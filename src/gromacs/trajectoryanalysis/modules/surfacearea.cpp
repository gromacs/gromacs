...
void SurfaceAreaCalculator::setRadii(const ConstArrayRef<real> &radii)
{
    impl_->radii_ = radii;
    if (!radii.empty())
    {
        const real maxRadius = *std::max_element(radii.begin(), radii.end());
        ...
        real **at_area, real **lidots, int *n_dots) const
{
    ...
    *area  = 0;
    ...
    nsc_dclm_pbc(x, impl_->radii_, nat,
