...
// For debugging.
// to print a temporary structure in the `vec` array to an xyz file, which is
// under a directory named "sth" (it should be created in advance).
void printxyz(const BasePair &temp, const ResInfo &base1, const ResInfo &base2,
              const char *fname, rvec *vec, size_t vecSize, const char *remark)
{
    std::string filename("./sth/");
    static int  ii = 0;
    ii++;
    filename += std::to_string(ii) + "_" + temp.templateName() + "_"
        + fname + ".xyz";
    ...
    for (size_t i = 0; i < base1.nAtoms(); ++i)
    {
        if (!base1.invalid(i))
        {
            names.push_back(base1.atomName(i));
        }
    }
    for (size_t i = 0; i < base2.nAtoms(); ++i)
    {
        if (!base2.invalid(i))
        {
            names.push_back(base2.atomName(i));
        }
    }
    GMX_ASSERT(names.size() == vecSize, "abc");
    FILE *fo = fopen(filename.c_str(), "a");
    fprintf(fo, "%zu\n", vecSize);
    fprintf(fo, "%s %s\n", fname, remark);
    for (size_t i = 0; i < vecSize; ++i)
    {
        fprintf(fo, "%4s%15.5f %15.5f %15.5f\n", names[i].c_str(), vec[i][0]*10, vec[i][1]*10, vec[i][2]*10);
    }
    fclose(fo);
}
                                              ...
                                              rvec *vec, size_t baseSize)
{
    ...
    if (base1.residueNumber() == 24 && base2.residueNumber() == 24 &&
        nucleotideType(0) == Guanine && nucleotideType(1) == Guanine &&
        isomerismType() == Trans && bondName().compare("WW") == 0 &&
        subType().compare("1") == 0)
    {
        printxyz(*this, base1, base2, "", x, baseSize, "");
        printxyz(*this, base1, base2, "", vec, baseSize, "");
        // For debugging. Output a specific temporary structure.
        auto qa = 0;
    }
...
std::string BasePair::templateName(bool swap) const
{
    char buf[256];
    snprintf(buf, sizeof(buf), "%c%c %s %s-%s",
             nucleotideTypeChar(swap ? 1 : 0), nucleotideTypeChar(swap ? 0 : 1),
             isomerismName(isomerismType()), bondName(swap).c_str(),
             subType(swap).c_str());
    return std::string(buf);
}

std::string BasePair::pairName(bool swap) const
{
    char buf[256];
    snprintf(buf, sizeof(buf), "%c%s-%s",
             isomerismType() == Cis ? 'c' : 't', bondName(swap).c_str(),
             subType(swap).c_str());
    return std::string(buf);
}

std::string BasePair::subType(bool swap) const
{
    return subtype_[swap ? 1 : 0];
}
...
void BasePair::setNucleotides(const std::string &bases)
{
    for (int i = 0; i < 2; i++)
    {
        type[i] = searchNucleotide(bases[i + 4]);
    }
}
...
{
    GMX_RELEASE_ASSERT((bondTypes.size() == 6),
                       "bondTypes string should be length 6 characters. Check your RNA database.");
    bondtype_[0] = searchBondType(bondTypes.substr(1, 1));
    bondtype_[1] = searchBondType(bondTypes.substr(2, 1));
    iso          = bondTypes[0] == 'c' ? Cis : Trans;
}


void BasePair::setBondSubtypes(const std::string &subtypes)
{
    size_t p = subtypes.find('-');
    GMX_RELEASE_ASSERT((p != std::string::npos),
                       "subtype string should be two strings split with '-'. Check your RNA database.");
    subtype_[0] = subtypes.substr(0, p);
    subtype_[1] = subtypes.substr(p + 1);
}
...
std::pair<real, real> BasePair::isHBond(const ResInfo   &base1,
                                        const ResInfo   &base2,
                                        const t_pbc     *pbc,
                                        double           maxDist,
                                        const Selection &s)
{
         ...
         (bai < bondAtoms.end()); ++bai)
    {
        ...
        {
            return std::make_pair<real, real>(0, bondAtoms.size());
        }
...
//            return false;
    ...
    if (distances.size() > 0)
    {
        auto di = std::min_element(distances.begin(), distances.end());
        if (*di > maxDist)
        {
            auto mean = std::accumulate(distances.begin(), distances.end(), 0.0) / distances.size();
            va -= (mean - maxDist) * 1000;
        }
    }
    return std::make_pair(va, ia);
}
