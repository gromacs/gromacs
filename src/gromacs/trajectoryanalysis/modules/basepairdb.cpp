...
static inline void copy_nrvec(const rvec *a, rvec *b,
                              const unsigned int startn,
                              const unsigned int endn)
{
    unsigned int i;
    for (i = 0; i < endn - startn; ++i)
    {
        b[i][XX] = a[i + startn][XX];
        b[i][YY] = a[i + startn][YY];
        b[i][ZZ] = a[i + startn][ZZ];
    }
}
// Calculate the RMSD
real BasePair::computeRootMeanSquareDeviation(const ResInfo *tempID1,
...
//    GMX_RELEASE_ASSERT((baseSize == nrAtoms()), buf);
    ...
    else
    {
        ...
        snew(mass, baseSize);
        snew(x, baseSize);
        if (phosphateRmsd_ && tempID1->endDirectionality() == FivePrimeEnd) {
            std::copy(&atomMass_[3], &atomMass_[splitPoint_], mass);
            copy_nrvec(x_, x, 3, splitPoint_);
            np += splitPoint_ - 3;
        } else {
            std::copy(&atomMass_[0], &atomMass_[splitPoint_], mass);
            copy_nrvec(x_, x, 0, splitPoint_);
            np += splitPoint_;
        }
        if (phosphateRmsd_ && tempID2->endDirectionality() == FivePrimeEnd) {
            std::copy(&atomMass_[splitPoint_ + 3], &atomMass_[nr], mass);
            copy_nrvec(x_, x, splitPoint_ + 3, nr);
            np += nr - splitPoint_ - 3;
        } else {
            std::copy(&atomMass_[splitPoint_], &atomMass_[nr], mass);
            copy_nrvec(x_, x, splitPoint_, nr);
            np += nr - splitPoint_;
        }
                               ...
                               real m)
{
    ...
    if (checkAtom(atomname))
    {
        ...
        if (resNumber_.size() > 0 && resnum != resNumber_.back())
            splitPoint_ = (unsigned int) resNumber_.size();
...
Nucleotide BasePair::nucleotideType(unsigned int i)
{
    GMX_RELEASE_ASSERT(i < 2, "nucleotideType out of range");
    return type[i];
}

char BasePair::nucleotideTypeChar(unsigned int index)
{
