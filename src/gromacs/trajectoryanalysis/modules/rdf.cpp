...
class Rdf : public TrajectoryAnalysisModule
{
    ...
    private:
        ...
        //! Topology with the exclusions used by neighborhood searching.
        ExpandedTopologyPtr                       expandedTop_;
                  ...
                  const TopologyInformation        &top)
{
    ...
    if (bExclusions_)
    {
        ...
        // TODO This could instead re-use the expandedTopology_ from
        // TrajectoryAnalysisRunnerCommon::Impl for PBC handling,
        // if/when enough tools need the same infrastructure for that.
        expandedTop_ = makeExpandedTopology(top);
        if (expandedTop_->excls.nr == 0)
        {
            GMX_THROW(InconsistentInputError("-excl is set, but the file provided to -s does not define exclusions"));
        }
        nb_.setTopologyExclusions(&expandedTop_->excls);
    }
}
