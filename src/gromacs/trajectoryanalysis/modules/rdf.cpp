...
class Rdf : public TrajectoryAnalysisModule
{
    ...
    private:
         ...
         *
         * The first column contains the number of atoms in `refSel_` for that
         * frame.  There are `sel_.size()` more columns, each containing the
         * number density of positions for one selection.
         */
                  ...
                  const TopologyInformation        &top)
{
    ...
    if (bSurface)
    {
        ...
        for (int i = 0; i < refSel_.posCount(); ++i)
        {
            ...
            refSel_.setOriginalId(i, surfaceGroupCount_);
        }
        ++surfaceGroupCount_;
    }
                         ...
                         const t_trxframe                 &fr)
{
    ...
    rdf_->init(histogramFromRange(0.0, rmax_).binWidth(binwidth_ / 2.0));
}
...
Rdf::finishAnalysis(int /*nframes*/)
{
    ...
    for (size_t g = 0; g < sel_.size(); ++g)
    {
        ...
        if (bNormalize_)
        {
            normfac = 1.0 / (normAve_->average(0, g + 1) * refPosCount);
        }
        else
        {
            normfac = 1.0 / (binwidth_ * refPosCount);
        }
    ...
    if (!fnCumulative_.empty())
    {
        ...
        cumulativeRdf->scaleAll(1.0 / refPosCount);
