...
enum BondType
{
    Ww, Ss, Hh, BondTypeSize = Hh
};

//! Names of bond types
const char *const btName[] = {"W", "S", "H"};
...
class ResInfo
{
    public:
         ...
         */
        ResInfo(char type, size_t resNr, size_t start, size_t chain) : type_(searchNucleotide(type)), resNr_(resNr),
                                                                       start_(start), chain_(chain), endD_(NotEnd)
        {}

        //! Return the residue type
        Nucleotide residueType() const
        { return type_; }

        //! Return the charactor of above.
        char residueTypeChar() const
        { return nucleotideToChar(type_); }

        //! Return the residue number
        size_t residueNumber() const
        { return resNr_; }

        //! Return the index of the first atom in the coordinates
        size_t atomStart() const
        { return start_; }
        ...
        EndDirectionality endDirectionality() const
        { return endD_; }
...
class BondAtom
{
    public:
        ...
        const std::string &atomI() const
        { return ai_; }

        //! The first atom in the bonding
        const std::string &atomJ() const
        { return aj_; }
...
class BasePair
{
    public:
        ...
        //! Set bond subtypes
        void setBondSubtypes(const std::string &subtypes);
        ...
        std::string templateName(bool swap = false) const;

        //! Return a string describing the pair type.
        std::string pairName(bool swap = false) const;

        //! Return the subtype name
        std::string subType(bool swap = false) const;
        ...
        std::pair<real, real> isHBond(const ResInfo   &base1,
                                      const ResInfo   &base2,
                                      const t_pbc     *pbc,
                                      double           bondDist,
                                      const Selection &sel);
        ...
        size_t nrBondAtoms() const
        { return bondAtoms.size(); }

        /*! \brief
         * Set the periodic boundary condition variables
         */
        void setPBC(int ePBC, matrix box)
        {
            ePBC_ = ePBC;
            copy_mat(box_, box);
        }
        ...
        real getTemplateScore()
        { return templateScore_; }

        //! set the RMSD of the template
        void setTemplateScore(real score)
        {
            templateScore_ = score;
        }

        //! set if the structure is artificially modeled
    ...
    private:
        ...
        // for H-boud
        const real min_angle = static_cast<real>(2.00712864); // 115 degrees
        // Return type of atom
