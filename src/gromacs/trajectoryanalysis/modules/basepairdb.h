...
class resInfo
{
    public:
        ...
        int searchAtom(const char *name) const;

        unsigned int nAtoms() const { return atomName_.size(); }

        void addAtom(std::string name, bool invalid)
        {
...
class BasePair
{
    public:
        ...
        unsigned int nrAtoms() { return atomName_.size(); }
        ...
        const char * bondIndex(int bond, int pair);
