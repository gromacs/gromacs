...
class ConvertTrj : public TrajectoryAnalysisModule
{
    ...
    private:
        ...
        Selection                                          sel_;
        std::string                                        name_;
        CoordinateOutputUserOptions                        flags_;
};
...
ConvertTrj::initOptions(IOptionsContainer *options, TrajectoryAnalysisSettings *settings)
{
    static const char *const desc[] = {
        ...
        "in the underlying trajectory data to obtain slimer output files, or to",
    ...
    options->addOption(SelectionOption("select").store(&sel_).dynamicMask()
                           .defaultSelectionText("all").description("Selection of atoms to write to the file"));
...
ConvertTrj::initAnalysis(const TrajectoryAnalysisSettings    & /*settings*/,
                         const TopologyInformation          &top)
{
    t_atoms                  globalAtoms = gmx_mtop_global_atoms(top.mtop());
    CoordinateOutputAdapters adapters    = flags_.registerModules(&globalAtoms);
    adapters.emplace_back(compat::make_unique<OutputSelector>(sel_));
    output_ = createOutputManager(top.mtop(),
                                  sel_,
                                  name_,
                                  std::move(adapters));
}
