...
/*! \brief
 * Helper class managing clusters
 */
...
class ClusterManager
{
    public:
        ...
        /*! \brief Return the cluster index for this atom
         *
         * \param[in] atom_id The atom for which cluster ID is wanted
         * \return the cluster index
         */
        int index(int atom_id) const;

        /*! \brief Add a new cluster and populate it with this atom
         *
         * \param atom_id The atom to add
         */
        void addCluster(int atom_id);

        /*! \brief Add an atom to a cluster
         *
         * \param cluster The cluster to add to
         * \param atom_id The atom to add
         */
        void addToCluster(int cluster, int atom_id);

        /*! \brief Merge cluster src int dst and delete src
         *
         * \param[in] dst The destination cluster ID
         * \param[in] src The source cluster ID
         */
...
class ClustSize : public TrajectoryAnalysisModule
{
    ...
    private:
        ...
        /*! \brief
         * Histogram module that computes the actual histogram from `clustSizeDist_`.
         *
        ...
        //! Axis for histograms in xpm files
        std::vector<real>                        time_axis_;
                        ...
                        const TopologyInformation        &top)
{
    ...
    GMX_RELEASE_ASSERT(mtop, "No topology!");
    sel_.initOriginalIdsToGroup(mtop, INDEX_ATOM);
    plotSettings_ = settings.plotSettings();

    // Do some molecule stuff if needed.
    nmol_ = 0;
    if (mol_)
    {
    ...
    if (mol_)
    {
        clustSizeDist_.setColumnCount(0, nmol_);
    }
    else
    {
        clustSizeDist_.setColumnCount(0, sel_.atomCount());
    }
    ...
    int n = (mol_ ? nmol_ : sel_.atomCount());
    pairCounts_->init(histogramFromRange(0.0, n).binWidth(1.0).integerBins(true));
}
                        ...
                        TrajectoryAnalysisModuleData *pdata)
{
    ...
    // Copy coordinates for selection
    std::vector<RVec> xx;
    for (const auto &mi : sel.atomIndices())
    {
        xx.push_back(fr.x[mi]);
    }

    // Use neighborsearching tools!
    AnalysisNeighborhoodSearch     nbsearch = nb_.initSearch(pbc, sel);
    AnalysisNeighborhoodPair       pair;
    AnalysisNeighborhoodPositions  positions(xx);
    ...
    cluster_index.resize(sel.atomIndices().size());
    for (size_t ci = 0; ci < cluster_index.size(); ci++)
    {
        cluster_index[ci] = ci;
    }
    // If we have selected the mol option merge all the clusters
    // that together make up a molecule. The cluster merging procedure
    // in the main loop below will make sure that the molecules stay
    // together.
    if (mol_)
    {
        for (const auto &ai : sel.atomIndices())
        {
            for (const auto &aj : sel.atomIndices())
            {
                if (aj > ai && molIndex_[ai] == molIndex_[aj])
                {
                    cluster_index[aj] = cluster_index[ai];
                }
            }
        }
    }
    while (pairSearch.findNextPair(&pair))
    {
        ...
        /* Merge clusters: check for all atoms/mols whether they are in
         * cluster cluster_index[pj] and if so,
         * put them in cluster_index[pi]
         */
        if (false && mol_)
        {
            // Change atom numbers to molecule numbers
            pi = molIndex_[pi];
            pj = molIndex_[pj];
        }
        // The neighborsearch code yields a full matrix so we have to
        // check for pj > pi.
        if (pj > pi && cluster_index[pi] != cluster_index[pj])
        {
            int cpj = cluster_index[pj];
    ...
    // Now count the size of each cluster. The result depends on
    // whether the mol option was used.
    std::vector<int> cluster_size;
    if (mol_)
    {
        cluster_size.resize(nmol_, 0);
        std::vector<bool> molCounted;
        molCounted.resize(nmol_, false);
        for (size_t ci = 0; ci < cluster_index.size(); ci++)
        {
            int mi = molIndex_[ci];
            if (!molCounted[mi])
            {
                cluster_size[molIndex_[cluster_index[ci]]] += 1;
                molCounted[mi] = true;
            }
        }
    }
    else
    {
        ...
        for (const auto &ci : cluster_index)
        {
            cluster_size[ci] += 1;
        }
    }
    if (dhhc.isValid())
    {
        ...
        time_axis_.push_back(fr.time);
    }
    int nclust        = 0;
    int maxclust_size = 0;
    for (const auto &cs : cluster_size)
    {
        if (cs > 0)
        {
            nclust++;
    ...
    if (!fnCSize_.empty() || !fnCSizeW_.empty())
    {
        ...
        if (!fnCSize_.empty())
        {
            ...
            write_xpm3(fp, 0, "Cluster size distribution", "# clusters",
                       timebuf, "Size",
                       time_axis_.size(), yaxis.size(),
                       time_axis_.data(), yaxis.data(),
