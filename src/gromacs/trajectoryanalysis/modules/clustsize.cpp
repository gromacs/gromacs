                        ...
                        TrajectoryAnalysisModuleData *pdata)
{
    ...
    AnalysisNeighborhoodPositions  positions(sel.coordinates().data(),
                                             sel.posCount());
    AnalysisNeighborhoodSearch     nbsearch   = nb.initSearch(pbc, positions);
    ...
    std::vector<int>               cluster_index (sel.posCount());
    std::copy(sel.mappedIds().begin(), sel.mappedIds().end(),
              cluster_index.begin());
    ...
    while (pairSearch.findNextPair(&pair))
    {
        ...
        {
            std::replace(cluster_index.begin(), cluster_index.end(),
    ...
    cluster_size.resize(nparticles_, 0);
    if (mol_)
    {
        std::vector<bool> molCounted;
        molCounted.resize(nparticles_, false);
        for (size_t ci = 0; ci < cluster_index.size(); ci++)
        {
            int mi = sel.mappedIds()[ci];
            if (!molCounted[mi])
            {
                cluster_size[cluster_index[ci]] += 1;
                molCounted[mi]                   = true;
            }
        }
    }
    ...
    if (dhnc.isValid())
    {
        dhnc.setPoint(0, std::count_if(cluster_size.begin(),
                                       cluster_size.end(),
                                       [](int i) { return i > 0; }));
        dhnc.finishFrame();
    }
