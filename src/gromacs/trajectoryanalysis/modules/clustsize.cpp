...
class ClustSize : public TrajectoryAnalysisModule
{
    ...
    private:
        ...
        //! Cut-off distance
        double                                   cutoff_;
        //! For xpm files the starting color
        RVec                                     rlo_;
        //! For xpm files the ending color
        RVec                                     rhi_;
        //! Number of trajectory frames to skip to speed up analysis
        int                                      nskip_;
                        ...
                        const TopologyInformation        &top)
{
    ...
    int nmol = 0;
    for (int k = 0; k < mtop->nmolblock; k++)
    {
        nmol += mtop->molblock[k].nmol;
    }
    if (mol_)
    {
        nparticles_ = nmol;
    ...
    {
        nparticles_ = mtop->natoms;
    }
    // Some code inspired by rdf.cpp
    // Set the total number of data points in the histogram to the number of molecules
    ...
    addPlotModule(&maxClust_, fnMaxClust_, "Max cluster size",
                  settings, "#molecules");
    addPlotModule(&nClust_, fnNClust_, "Number of clusters",
                  settings, "N");
                        ...
                        TrajectoryAnalysisModuleData *pdata)
{
    ...
    bool doClusterDistribution = (!fnHistoClust_.empty() ||
                                  !fnCSize_.empty() ||
                                  !fnCSizeW_.empty());
    if (doClusterDistribution)
    {
    ...
    // Put each atom or whole molecule in a cluster of it's own
    for (size_t mi = 0; mi < nparticles_; mi++)
    {
        cluster_index.push_back(mi);
    ...
    while (pairSearch.findNextPair(&pair))
    {
        ...
        if (pj > pi && cluster_index[pi] != cluster_index[pj])
        {
            ...
            for (size_t mi = 0; mi < nparticles_; mi++)
            {
                if (cluster_index[mi] == cpj)
                {
                    cluster_index[mi] = cluster_index[pi];
                    cluster_size[cpj]--;
    ...
    if (mol_)
    {
        ...
        inMaxClust.resize(nparticles_, false);
        int               k = 0;
...
ClustSize::finishAnalysis(int /* nframes */)
{
    if (!fnCSize_.empty())
    {
        ...
        rlo.r = rlo_[0]; rlo.g = rlo_[1]; rlo.b = rlo_[2];
        rhi.r = rhi_[0]; rhi.g = rhi_[1]; rhi.b = rhi_[2];
        // TODO: insert correct time unit
        std::string timebuf = formatString("Time (ps)");

        printf("Time size %lu, csreal %lu matrix size %lu x %lu\n",
               time_axis_.size(),
               csreal.size(),
               cs_xpm.size(),
               yaxis.size());
        ...
        max_num_clust += 1;
        write_xpm(fp, 0, "Cluster size distribution", "# clusters",
                  timebuf.c_str(), "Size",
                  time_axis_.size(), yaxis.size(),
                  time_axis_.data(), yaxis.data(),
                  cs_xpm.data(), 0, max_num_clust-1,
                  rlo, rhi, &max_num_clust);
        gmx_ffclose(fp);
    }
