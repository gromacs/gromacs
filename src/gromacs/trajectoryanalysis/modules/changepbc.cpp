...
ChangePBC::optionsFinished(TrajectoryAnalysisSettings *settings)
{
    ...
    pbc_.checkOptions(writeSettings_->getSel(), settings);
...
//    pbc_.initPBCTypes(selPBCType_,selUnitCellType_,selCenteringType_);
//    pbc_.initPBCSelections(sel_,selCOM_,selCent_,selClust_);
                      ...
                      TrajectoryAnalysisModuleData * /*pdata*/)
{
    ...
    if(writeSettings_->getUseNewBox())
    {
        matrix newMatrix;
        clear_mat(newMatrix);
        std::vector<real> newBox = writeSettings_->getNewBoxDimensions();
    ...
    pbc_.doPBC(&fr, file_.getFrameForModification(), writeSettings_->getMtop());
    file_.writeFrame();
}
