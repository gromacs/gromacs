                         ...
                         const TopologyInformation        &top)
{
    ...
    gmx_atomprop_t aps;
    t_atoms *atoms;
    ConstArrayRef<int> atomind;
    ConstArrayRef<int>::iterator ai;
    
    // Check whether we have any atoms at all, otherwise bail out
    ...
    atomind = sel_[0].atomIndices();
    for(ai=atomind.begin(); (ai<atomind.end()); ai++)
    {
    ...
    // Initiate theneighborsearching code
                         ...
                         TrajectoryAnalysisModuleData *pdata)
{
    ...
    dh.setPoint(0,frac);
    // Add the total volume to the data set in column 0
    dh.setPoint(1,V);
...
{
    // Final result comes from statistics module in analysis framework
    printf("Free volume %.2f +/- %.2f %%\n",
           adata_->average(0),adata_->stddev(0));
    printf("Total volume %.2f +/- %.2f nm^3\n",
           adata_->average(1),adata_->stddev(1));
}
