...
class Trajectory : public TrajectoryAnalysisModule
{
    ...
    private:
        void setVectorData(int frnr, real time, AnalysisDataHandle handle,
                           const SelectionList &sel,
                           const rvec & (SelectionPosition::* entry)() const);
...
void
Trajectory::setVectorData(int frnr, real time, AnalysisDataHandle handle,
                          const SelectionList &sel,
                          const rvec & (SelectionPosition::* entry)() const)
{
    if (!handle.isValid())
    ...
    handle.startFrame(frnr, time);
    for (size_t g = 0; g < sel.size(); ++g)
    {
        handle.selectDataSet(g);
        for (int i = 0; i < sel[g].posCount(); ++i)
        {
            const auto &pos = sel[g].position(i);
            handle.setPoints(i*3, 3, (pos.*entry)(), pos.selected());
        }
    }
    handle.finishFrame();
}
                         ...
                         TrajectoryAnalysisModuleData *pdata)
{
    ...
    setVectorData(frnr, fr.time, pdata->dataHandle(xdata_), sel,
                  &SelectionPosition::x);
    if (fr.bV)
    {
        setVectorData(frnr, fr.time, pdata->dataHandle(vdata_), sel,
                      &SelectionPosition::v);
    }
    if (fr.bF)
    {
        setVectorData(frnr, fr.time, pdata->dataHandle(fdata_), sel,
                      &SelectionPosition::f);
    }
}
