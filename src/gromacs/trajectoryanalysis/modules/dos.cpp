...
//! Tolerance used in different aspects of the code
static const real toler = 1e-5;

static const char *velocityName(Velocity v, bool bShort)
{
    switch (v)
    {
    case V_T:
        return (bShort ? "Trans" : "Translation");
    case V_A:
        return (bShort ? "Angul" : "Angular motion");
    case V_V:
        return (bShort ? "Vibr"  : "Vibration");
    case V_R:
        return (bShort ? "Rotat" : "Rotational motion");
    case V_ATOM:
        return (bShort ? "Atomic" : "Atomic motion");
    case V_SUM:
        return (bShort ? "Sum"    : "Sum of it all");
    default:
        return nullptr;
    }
};

static const char *velocityName(int v, bool bShort)
{
    return velocityName(static_cast<Velocity>(v), bShort);
}
...
class DosProps
{
    private:
        ...
        double              Emd_, E0_;
        double              f_, y_, fy_, ry_, z_, nrho_, Delta_, beta_, partMass_;
        double              hsdf_, ttdf_;
    ...
    public:
        ...
        //! Return the velocity type (translation, angular, vibration)
        ...
        void setTemperature(double T) { Temperature_ = T; beta_ = 1.0/(BOLTZ*T); }
        ...
        void setAllDos(int i, double val) { range_check(i, 0, alldos_.size()); alldos_[i] = val; }

        void incrementAllDos(int i, double val) { range_check(i, 0, alldos_.size()); alldos_[i] += val; }
        ...
        void setVec(int i, int j, double value) { range_check(i, 0, vec_.size()); vec_[i][j] = value; }

        double vec(int i, int j) const { range_check(i, 0, vec_.size()); return vec_[i][j]; }

        real **rawVector() { return vec_.data(); }

        //! return number of degrees of freedom for this part
        double numDoF() const { return dof_; }
        
        //! set number of degrees of freedom for this part
...
DosProps::DosProps()
{
    ...
    Emd_          = 0;
    E0_           = 0;
    f_            = y_ = fy_ = ry_ = z_ = nrho_ = Delta_ = beta_ = partMass_ = 0;
    hsdf_         = ttdf_ = 0;
    sigHS_        = Shs_ = Sig_ = 0;
    DiffACF_      = DiffDoS_ = 0;
                       ...
                       const gmx_output_env_t  *oenv)
{
    ...
    if (fnDos.size() > 0)
    {
        ...
        snprintf(buf, sizeof(buf), "%s-%s",
                 velocityName(velocity_, true), fnDos.c_str());
...
Dos::Dos() : top_(nullptr), oenv_(nullptr), r1_(nullptr), v1_(nullptr)
{
    fplog_ = nullptr;
    for (int t = 0; t < V_NR; t++)
    {
        dp_[t].setVelocity(static_cast<Velocity>(t));
    }
    bRecip_      = false;
    dumpW_       = false;
    bAtomic_     = false;
...
void Dos::printResume()
{
    ...
    int nitem = sizeof(items)/sizeof(items[0]);
    for (int k = 0; (k < nitem); k++)
    {
        ...
        for (int t = 0; (t <= V_SUM); t++)
        {
            ...
            {
                case 0:
                    fprintf(fplog_, "  %10s", velocityName(static_cast<Velocity>(t), true));
                    break;
                case 1:
...
#pragma omp parallel
    ...
    for (int t = 0; (t < V_NR); t++)
    {
        int nMol = dp_[t].numParticles();
        if ((dp_[t].numDoF() > 0) && (nMol > 0))
        {
            dp_[t].setTemperature(dp_[t].allDoS(0)/(dp_[t].numDoF()*BOLTZ*nMol));
        }
    }
    ...
    for (int t = 0; t < V_NR; t++)
    {
        if ((dp_[t].numDoF() > 0) && (dp_[t].numParticles() > 0))
        {
            ...
            complex *in, *out;
            int      j;
            double   bfac = 2.0*dt_*dp_[t].beta();

            snew(in, N_tot);
            snew(out, N_tot);
            for (j = 0; (j < N_tot); j++)
            {
                in[j][0] = bfac*dp_[t].allDoS(j);
                in[j][1] = 0;
            }
            for (; (j < N_tot); j++)
            {
