...
//! \cond
// Doxygen complains about these forward declarations as duplicates,
// which is rather the point!
void done_and_delete_atoms(t_atoms *);
void done_and_sfree_localtop(gmx_localtop_t *);
//! \endcond
...
using AtomsDataPtr = unique_cptr<t_atoms, done_and_delete_atoms>;

//! Deleter for gmx_localtop_t, needed until it has a proper destructor.
using ExpandedTopologyPtr = unique_cptr<gmx_localtop_t, done_and_sfree_localtop>;
 ...
 * the t_atoms, so releasing the ownership of the lazily constructed
 * data is possible. The getters and releasers are const for callers,
 * which correctly expresses that the topology information is not
 * being changed, merely copied and presented in a different form.
 *
 * \ingroup module_trajectoryanalysis
 */
class TopologyInformation
