...
class TrajectoryAnalysisRunnerCommon::Impl : public ITopologyProvider
{
    public:
        ...
        //! A fully expanded topology, needed for handling PBC
        ExpandedTopologyPtr         expandedTopology_;
...
TrajectoryAnalysisRunnerCommon::Impl::initFirstFrame()
{
    ...
    {
        expandedTopology_ = makeExpandedTopology(topInfo_);
        gpbc_             = gmx_rmpbc_init(&expandedTopology_->idef,
                                           topInfo_.ePBC(), topInfo_.mtop()->natoms);
    }
}
