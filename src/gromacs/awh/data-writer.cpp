                                                   ...
                                                   const Bias &bias, const std::vector<float> &pmf)
{
    ...
    switch (evar)
    {
        ...
        case evarCOORDVALUE:
            ...
            break;
        case evarPMF:
            block_[b].data[m] = bias.coordPoint[m].inTargetRegion() ? pmf[m] : 0;
            break;
        case evarBIAS:
            block_[b].data[m] = bias.coordPoint[m].inTargetRegion() ? calcConvolvedBias(bias, bias.grid->point(m).value) : 0;
            break;
        case evarVISITS:
            block_[b].data[m] = bias.coordPoint[m].visits_tot;
            break;
        case evarWEIGHTS:
            block_[b].data[m] = bias.coordPoint[m].weightsum_tot;
            break;
        case evarTARGET:
            block_[b].data[m] =  bias.coordPoint[m].target;
            break;
        default:
...
int BiasWriter::writeSubblocks(t_enxsubblock *sub)
{
...
void AwhBiasCollection::writeToEnergyframe(t_enxframe *frame) const
{
    ...
    for (auto &bias : bias_)
    {
        numSubblocks += bias.writer->numBlocks();
    }
    ...
    for (auto &bias : bias_)
    {
        energySubblockCount += bias.writer->writeSubblocks(&(awhenergyblock->sub[energySubblockCount]));
    }
}
