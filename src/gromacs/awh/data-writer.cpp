...
static double get_coord_normvalue(const awh_bias_params_t *awh_bias_params, int dimIndex)
{
    /* AWH may use different units internally but here we convert to user units */
    return scaleInternalToUserInput(awh_bias_params, dimIndex, 1);
}

/* TODO: not used, could remove. */
static double get_distribution_normvalue(const awh_bias_t *awh_bias, const awh_bias_params_t *awh_bias_params)
{
    double volume = 1;
    for (int d =  0; d < awh_bias->ndim; d++)
    {
        double   spacing = awh_bias->grid->axis[d].spacing;
        spacing *= scaleInternalToUserInput(awh_bias_params, d, 1);
...
static double get_normvalue(int evar, const awh_bias_t *awh_bias, const awh_bias_params_t *awh_bias_params, int count)
{
    double normvalue = 0;
    ...
    {
        case evarCOORDVALUE:
            normvalue = get_coord_normvalue(awh_bias_params, count);
            break;
        case evarVISITS:
        case evarWEIGHTS:
        case evarTARGET:
...
static void init_bias_writer(bias_writer_t *writer,
                             const awh_bias_t *awh_bias, const awh_bias_params_t *awh_bias_params)
{
    int blockcount;
    ...
    for (int evar = 0; evar < evarNR; evar++)
    {
        ...
        for (int b = bstart; b < bstart + var_nblock[evar]; b++)
        {
            init_block(&writer->block[b], npoints,
                       get_normtype(evar),
                       get_normvalue(evar, awh_bias, awh_bias_params, var_blockcount));
            blockcount++;
        }
    }
}
                                          ...
                                          const awh_t *awh, const awh_params_t * awh_params)
{
    ...
    {
        init_bias_writer(&energywriter->writer[k], &awh->awh_bias[k], &awh_params->awh_bias_params[k]);
    }
