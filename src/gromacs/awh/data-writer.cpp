...
{
    return writer->varToBlock[evar] >= 0;
}
...
static double get_normvalue(int evar, const Bias &bias, int count)
{
    ...
    switch (evar)
    {
        ...
        case evarWEIGHTS:
        case evarTARGET:
            normvalue = static_cast<double>(bias.coordPoint.size());
            break;
        default:
            break;
    }
...
BiasWriter::BiasWriter(const Bias &bias)
{
    ...
    for (int evar = 0; evar < evarNR; evar++)
    {
        ...
        {
            npoints        = bias.coordPoint.size();
        }
        int bstart         = varToBlock[evar];
        int var_blockcount = 0;
        for (int b = bstart; b < bstart + var_nblock[evar]; b++)
        {
            init_block(&block[b], npoints,
                       get_normtype(evar),
 ...
 * \param[in] bias        The AWH bias.
 */
static void normalizeData(BiasWriter *writer, const Bias &bias)
{
    /* Normalize the data */
    for (auto &block : writer->block)
    {
        ...
        switch (block.normtype)
        {
            ...
            case enormtypeFREE_ENERGY:
                ...
                {
                    if (bias.coordPoint[m].inTargetRegion() && block.data[m] < minval)
                    {
                        minval = block.data[m];
                    }
                }
 ...
 */
static void transferVariablePointDataToWriter(int evar, BiasWriter *writer, int m,
        ...
        case evarMETA:
            switch (m)
            {
                ...
                case emetadataTARGETERROR:
                    ...
                    block[b].data[m] = bias.params.errorInitial*std::sqrt(bias.params.histSizeInitial/bias.state.histSize);
                    break;
                case emetadataSCALEDSAMPLEWEIGHT:
                    ...
                    block[b].data[m] = bias.state.scaledSampleWeight;
                    break;
            }
            break;
        case evarCOORDVALUE:
            ...
            {
                block[b].data[m] = bias.grid->point(m).value[d];
                b++;
            }
            break;
        case evarPMF:
            block[b].data[m] = bias.coordPoint[m].inTargetRegion() ? pmf[m] : 0;
            break;
        case evarBIAS:
            block[b].data[m] = bias.coordPoint[m].inTargetRegion() ? calcConvolvedBias(bias, bias.grid->point(m).value) : 0;
            break;
        case evarVISITS:
            block[b].data[m] = bias.coordPoint[m].visits_tot;
            break;
        case evarWEIGHTS:
            block[b].data[m] = bias.coordPoint[m].weightsum_tot;
            break;
        case evarTARGET:
            block[b].data[m] =  bias.coordPoint[m].target;
            break;
        default:
...
void BiasWriter::prepareBiasOutput(const Bias &bias, const gmx_multisim_t *ms)
{
    ...
    std::vector<float> *pmf = &block[varToBlock[evarPMF]].data;
    getPmf(bias, ms, pmf);
    ...
    {
        transferVariablePointDataToWriter(evarMETA, this, i, bias, *pmf);
    }
    for (size_t m = 0; m < bias.coordPoint.size(); m++)
    {
        for (int evar = 0; evar < evarNR; evar++)
        {
            ...
            transferVariablePointDataToWriter(evar, this, m, bias, *pmf);
        }
    }

    /* For looks of the output, normalize it */
    normalizeData(this, bias);
...
void AwhBiasCollection::writeToEnergyframe(t_enxframe *frame) const
{
    if (!bias_[0].writer->haveDataToWrite)
    {
        return;
    }
    ...
    for (auto &bias : bias_)
    {
        numSubblocks += bias.writer->block.size();
    }
    ...
    for (auto &bias: bias_)
    {
        BiasWriter *writer = bias.writer.get();
        write_awh_to_subblocks(&(awhenergyblock->sub[energySubblockCount]), writer);
        energySubblockCount += writer->block.size();

        GMX_ASSERT(writer->haveDataToWrite, "All writers should have the same haveDataToWrite value");
        /* Don't print again until a new dataset has been prepared */
        writer->haveDataToWrite = false;
    }
}
