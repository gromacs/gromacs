    ...
    potentialOffset_(0)
{
    ...
    for (int k = 0; k < awhParams->nbias; k++)
    {
        ...
        {
            int pullCoordIndex = awhBiasParams.dim_params[d].pull_coord_index;
            dimParams.emplace_back(DimParams(pullCoordIndex, ir->pull->coord[pullCoordIndex], beta));
        }

        bias_.emplace_back(Bias(fplog, cr, k, *awhParams, awhParams->awh_bias_params[k], dimParams, beta, ir->delta_t));
    }
