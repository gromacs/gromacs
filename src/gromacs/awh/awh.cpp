...
#include "gromacs/awh/awh-energywriter.h"
    ...
    if (fplog != NULL)
    {
        ...
        if (awh->bForce_correlation)
        {
            fprintf(fplog,
                    ...
                    awhstr, get_blocklength(awh->forcecorr),
                    bBlocklength_in_weight ? "" : "ps",
                    awhstr, get_nblocks(awh->forcecorr));
        }
        if (ndomains > 1)
        {
...
static void read_user_pmf_and_target(t_awh *awh, int awh_id, int nawh)
{
    ...
    filename_path = gmxlibfn(filename);
    nrows         = read_xvg(filename, &data, &ncols);

    if (nrows <= 0)
    {
        gmx_fatal(FARGS, "%s is empty!", filename_path);
    }
        ...
        if (val < val_prev)
        {
            ...
            else
            {
                gmx_fatal(FARGS, "The coordinate value of point %d, dimension 0 (%g) in %s smaller than"
                          ...
                          m, val, filename_path, m-1, val_prev);
            }
        }
        else
        {
    ...
    if (nrows_valid < 2)
    {
        gmx_fatal(FARGS, "%s contains too few data points (%d)."
                  ...
                  filename_path, nrows_valid);
    }

    /* Get a data point for each AWH grid point */
    snew(grid2data_index, awh->grid->npoints);
    map_grid_to_datagrid(grid2data_index, data, nrows_valid, filename_path, awh->grid);
                     ...
                     awh_params_t     *awh_params)
{
    ...
    awh->bShared_update  = awh_params->bShare && (cr != NULL) && MULTISIM(cr) && (cr->ms->nsim > 1);
    awh->update_weight   = awh->nstupdate_free_energy/awh->nstsample_coord*(awh->bShared_update ? cr->ms->nsim : 1);
    for (int d = 0; d < awh->ndim; d++)
    {
    ...
    init_coord_points(awh, awh_params, awh_id, nawh, awh->bShared_update ? cr->ms->nsim : 1);
    ...
    if (awh->bForce_correlation)
    {
        ...
        awh->forcecorr = init_correlation_grid(awh->npoints, awh->ndim,
                                               nblocks, blocklength,
                        ...
                        const awhbias_params_t  *awhbias_params)
{
    ...
    const bool     bForce_correlation         = awhbias_params->bForce_correlation;
    ...
    awhbias->writer = init_awhbias_energywriter(awhbias_params->nstout,
                                                awhbias_params->bForce_correlation,
                                                awhbias, ir->pull);
...
static void update_force_correlation(correlation_grid_t *forcecorr, const t_awh *awh, double t)
{
    for (int n = 0; n < awh->grid->point[awh->coord_value_index].nneighbors; n++)
    {
        int      m_neighbor;
        ...
        /* Add the force data of this neighbor point. Note: the sum of these forces is the convolved force.
           ...
           we're interested in. */
        calc_umbrella_force(awh, awh->betak, m_neighbor, force_from_neighbor);
...
static void do_sampling(t_awh *awh, double t)
{
    /* Force correlation */
    if (awh->bForce_correlation)
    {
        update_force_correlation(awh->forcecorr, awh, t);
    }
