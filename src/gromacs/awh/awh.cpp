    ...
    nstout_(awhParams->nstout)
{
    ...
    for (int k = 0; k < awhParams->nbias; k++)
    {
        ...
        {
            dimParams.emplace_back(DimParams(awhBiasParams.dim_params[d], *ir->pull, beta));
        }

        bias_.emplace_back(Bias(fplog, ir, cr, k, *awhParams, awhParams->awh_bias_params[k], dimParams, beta));
    }
                 ...
                 FILE                   *fplog)
{
    ...
    for (auto &bias : bias_)
    {
         ...
         * pull coordinates.
         */
        bias.setCoordValue(pull_work, &pbc);
         ...
         */
        bias.applyForceToPullCoords(biasForce,
                                  ...
                                  const t_commrec  *cr)
{
    ...
    for (size_t k = 0; k < bias_.size(); k++)
    {
        if (MASTER(cr))
        {
            bias_[k].restoreStateFromHistory(&awhHistory.bias[k]);
        }

        if (PAR(cr))
        {
            bias_[k].broadcast(cr);
        }
    }
}
...
void Awh::updateHistory(AwhHistory *awhHistory) const
{
    ...
    for (size_t k = 0; k < awhHistory->bias.size(); k++)
    {
        bias_[k].updateHistory(&awhHistory->bias[k]);
    }
}
...
{
    if (bias_[0].numEnergySubblocksToWrite() == 0)
    {
        return;
    }
    ...
    for (auto &bias : bias_)
    {
        numSubblocks += bias.numEnergySubblocksToWrite();
    }
    ...
    for (auto &bias : bias_)
    {
        energySubblockCount += bias.writeToEnergySubblocks(&(awhenergyblock->sub[energySubblockCount]));
    }
}
