...
{
    return value_is_in_grid(awh_bias->coord_value, awh_bias->grid);
}
...
int get_coord_value_index(const awh_bias_t *awh_bias)
{
    ...
    return get_closest_index_in_grid(awh_bias->coord_value, awh_bias->grid);
}

static void print_log_init(const awh_bias_t *awh_bias, const awh_dim_params_t *awh_dim_params, int awh_id, FILE *fplog,
                           bool bBlocklength_in_weight, int ndomains)
{
    if (fplog != NULL)
    {
        char           awhstr[STRLEN];
...
static void read_user_pmf_and_target(awh_bias_t *awh_bias, int awh_id, int nbias)
{
...
static void init_coordpoints(awh_bias_t *awh_bias, const awh_bias_params_t *awh_bias_params,
                             int awh_id, int nbias, int nsharing_sims)
{
    coordpoint_t *coordpoint = awh_bias->coordpoint;
    ...
    {
        read_user_pmf_and_target(awh_bias, awh_id, nbias);
                     ...
                     awh_bias_params_t          *awh_bias_params)
{
    ...
    init_coordpoints(awh_bias, awh_bias_params, awh_id, nbias, awh_bias->numSharedUpdate);
    ...
    print_log_init(awh_bias, awh_bias_params->dim_params, awh_id, fplog,
...
static gmx_bool prev_step_had_replica_exchange(const awh_bias_t *awh_bias, gmx_int64_t step)
{
    /* Replica exchange occurs at the end of the MD loop, after the AWH update. The AWH reaction to the coordinate swap therefore comes one
       step later. */
    return (step > 0) && do_at_step(awh_bias->nstreplica_exchange, step - 1);
}
                ...
                int                      nstreplica_exchange)
{
    ...
    const bool     bConvolve_force        = awh_params->bConvolve_force;
    const bool     bForce_correlation     = awh_params->bForce_correlation;
    ...
    for (int k = 0; k < awh->nbias; k++)
    {
        ...
        awh->awh_bias[k].bConvolve_force          = bConvolve_force;
        awh->awh_bias[k].bForce_correlation       = bForce_correlation;
        awh->awh_bias[k].nstreplica_exchange      = nstreplica_exchange;

        init_awh(fplog, ir, cr, k, awh->nbias, &awh->awh_bias[k], &awh_params->awh_bias_params[k]);
    }


    /* Keep an array with the data to print to the energy file */
    awh->writer = init_awh_energywriter(awh_params->nstout,
                                        awh_params->bForce_correlation,
                                        awh, ir->pull);
...
static void calc_umbrella_force(const awh_bias_t *awh_bias, const awh_dvec k, int point, awh_dvec force)
{
    for (int d = 0; d < awh_bias->ndim; d++)
    {
...
static int sample_coord_refvalue_index(const awh_bias_t *awh_bias, gmx_int64_t step, int seed)
{
    ...
    int n_sampled  = get_sample_from_distribution(awh_bias->prob_weight_neighbor, awh_bias->grid->point[awh_bias->coord_value_index].nneighbors,
                                                  step, seed, awh_bias->pull_coord_index[0]);

    return awh_bias->grid->point[awh_bias->coord_value_index].neighbor[n_sampled];
}

static void set_bias_force(const awh_bias_t *awh_bias, awh_dvec force, int *refvalue_index_ptr, gmx_int64_t step, int seed)
{
    if (awh_bias->bConvolve_force)
    {
        calc_convolved_force(awh_bias, force);
    }
    else
    {
        calc_umbrella_force(awh_bias, awh_bias->k, (*refvalue_index_ptr), force);
        ...
        if (do_at_step(awh_bias->nstmove_refvalue, step))
        {
            ...
            (*refvalue_index_ptr) = sample_coord_refvalue_index(awh_bias, step, seed);

            calc_umbrella_force(awh_bias, awh_bias->k, (*refvalue_index_ptr), new_force);
...
static void make_local_update_list(awh_bias_t *awh_bias, const gmx_multisim_t *ms, int **update_list_ptr, int *nupdate_ptr)
{
    ...
    while (bExists_point)
    {
        bExists_point = get_next_point_in_local_grid(awh_bias->grid, origin_dim, npoints_dim, &point_index);

        if (bExists_point && in_target_region(&awh_bias->coordpoint[point_index]))
        {
    ...
    if (awh_bias->numSharedUpdate > 1)
    {
        merge_shared_update_lists(&update_list, &nupdate, awh_bias->npoints, ms);
    }
...
static void sum_histograms(awh_bias_t *awh_bias, const gmx_multisim_t *ms, const int *local_update_list, int nlocal_update)
{
    ...
    for (int ilocal = 0; ilocal < nlocal_update; ilocal++)
    {
        ...
        reaction_coord_visits[ilocal] = awh_bias->coordpoint[iglobal].visits_iteration;
    }
    ...
    for (int ilocal = 0; ilocal < nlocal_update; ilocal++)
    {
        ...
        awh_bias->coordpoint[iglobal].weightsum_covering += weight_distr[ilocal];
        awh_bias->coordpoint[iglobal].weightsum_tot      += weight_distr[ilocal];
        awh_bias->coordpoint[iglobal].visits_tot         += reaction_coord_visits[ilocal];
...
static void update(awh_bias_t *awh_bias, int awh_id, const gmx_multisim_t *ms, double t, gmx_int64_t step, FILE *fplog)
{
    ...
    make_local_update_list(awh_bias, ms, &local_update_list, &nlocal_update);

    /* Add samples to histograms and sync sims if needed */
    sum_histograms(awh_bias, ms, local_update_list, nlocal_update);

    /* Update target? */
    bUpdate_target = do_at_step(awh_bias->nstupdate_target, step);

    /* The weighthistogram size after this update. */
    histsize_new = get_new_histsize(awh_bias, awh_id, t, step, &bCovered, fplog);
     ...
     */
    bGlobal_update = bUpdate_target || bCovered || do_at_step(awh_bias->nstreplica_exchange, step);

    /*  Make the update list */
    if (bGlobal_update)
    {
        snew(update_list, awh_bias->npoints);
        nupdate = 0;
        for (int m = 0; m < awh_bias->npoints; m++)
        {
    ...
    get_histogram_scaling_new_update(awh_bias, histsize_new, awh_bias->histsize, bCovered,
    ...
    {
        coordpoint_t *coordpoint_to_update = &awh_bias->coordpoint[update_list[iupdate]];
    ...
    for (int iupdate = 0; iupdate < nupdate; iupdate++)
    {
        update_bias_point(&awh_bias->coordpoint[update_list[iupdate]]);
    }
...
static void sample_pmf(awh_bias_t *awh_bias)
{
    ...
    bIn_array   = coord_value_is_in_grid(awh_bias);
...
static void do_awh_step(awh_bias_t *awh_bias, int awh_id, const gmx_multisim_t *ms, double t, gmx_int64_t step, int seed, FILE *fplog)
{
    ...
    if (awh_bias->bConvolve_force || do_at_step(awh_bias->nstsample_coord, step))
    {
        awh_bias->coord_value_index =  get_coord_value_index(awh_bias);
    ...
    set_bias_force(awh_bias, awh_bias->bias_force, &awh_bias->coord_refvalue_index, step, seed);
                        ...
                        FILE                 *fplog)
{
    for (int k = 0; k < awh->nbias; k++)
    {
        do_awh_step(&awh->awh_bias[k], k, ms, t, step, awh->seed, fplog);
    }
}
...
{
    double invbeta        = awh->awh_bias[0].k[0]/awh->awh_bias[0].betak[0]; /* We just store beta*k and k, not beta. */
    ...
    double bias_potential = awh->convolved_bias_shift*invbeta;
    ...
    for (int k = 0; k < awh->nbias; k++)
    {
        ...
        /* Convert the bias potential from units of kT to energy units and add to
           the potential sum. */
        bias_potential += -awh_bias->convolved_bias*invbeta;
    }

    return bias_potential;
}

static void update_convolved_bias_shift(const awh_t *awh, gmx_int64_t step, double *convolved_bias_shift_ptr)
{
    for (int k = 0; k < awh->nbias; k++)
    {
        awh_bias_t *awh_bias = &awh->awh_bias[k];
        ...
        if (do_at_step(awh_bias->nstupdate_free_energy, step))
        {
            ...
            *(convolved_bias_shift_ptr) += (calc_convolved_bias(awh_bias, awh_bias->coord_value) - awh_bias->convolved_bias);
        }
    }
}
                ...
                FILE                   *fplog)
{
    ...
    do_awh_step(awh, ms, t, step, fplog);
    ...
    update_convolved_bias_shift(awh, step, &awh->convolved_bias_shift);
