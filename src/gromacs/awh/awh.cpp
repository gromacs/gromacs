                ...
                const awh_params_t      *awh_params)
{
    ...
    const bool     bConvolve_force        = (awh_params->ePotential == eawhpotentialCONVOLVED);
...
static void set_umbrella_force(const awh_bias_t *awh_bias, awh_dvec force, int *refvalue_index_ptr, gmx_int64_t step, int seed)
{
                        ...
                        const gmx_multisim_t *ms, double t, gmx_int64_t step, int seed, FILE *fplog)
{
    ...
    {
        set_umbrella_force(awh_bias, awh_bias->bias_force, &awh_bias->coord_refvalue_index, step, seed);
    }
