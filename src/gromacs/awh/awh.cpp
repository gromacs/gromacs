 ...
 * obtaining the reaction coordinate values and pasing the computed forces.
...
class BiasCoupledToSystem
{
    public:
         ...
         *
         * \param[in] biasHistory  The history of the bias to restore.
        ...
        /*! \brief Set the coordinate value(s) and evolves the bias at every step.
         *
         ...
         */
        void calcForceAndUpdateBias(awh_dvec                coordValue,
...
{
    GMX_RELEASE_ASSERT(*biasPtr != nullptr && biasPtr != nullptr, "We need a pointer to an initialized Bias");
    ...
    potentialOffset_(0)
{
    ...
    for (int k = 0; k < awhParams.numBias; k++)
    {
        ...
        biasCoupledToSystem_.emplace_back(BiasCoupledToSystem(&biasPtr, pullCoordIndex));
    }
...
/* Perform an AWH update every MD step. */
void BiasCoupledToSystem::calcForceAndUpdateBias(awh_dvec                coordValue,
                                                 const gmx_multisim_t   *ms,
                                                 double                  t,
                                                 gmx_int64_t             step,
                                                 gmx_int64_t             awhSeed,
                                  ...
                                  const t_commrec  *cr)
{
    ...
    {
        biasCoupledToSystem_[k].restoreStateFromHistory(awhHistory->bias[k], cr);
    }
}
