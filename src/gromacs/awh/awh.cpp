    ...
    potentialOffset_(0)
{
    ...
    for (int k = 0; k < awhParams->nbias; k++)
    {
        ...
        /* Initialize the bias */
        biasAndCoupling.bias = std::unique_ptr<Bias>(new Bias(fplog, cr, k, *awhParams, awhParams->awh_bias_params[k], dimParams, beta, ir->delta_t));
    }
...
real Awh::update(struct pull_t          *pull_work,
                 int                     ePBC,
                 ...
                 FILE                   *fplog)
{
    ...
    for (auto &biasAndCoupl : biasAndCoupling_)
    {
        Bias                   &bias           = *biasAndCoupl.bias.get();
        const std::vector<int> &pullCoordIndex = biasAndCoupl.pullCoordIndex;
        ...
        {
            double coordValue;
            get_pull_coord_value(pull_work, pullCoordIndex[d], &pbc,
                                 &coordValue);
         ...
         */
        awh_dvec biasForce;
        bias.doStep(biasForce, &biasPotential, &potentialJump,
        ...
        for (int d = 0; d < bias.ndim(); d++)
        {
            apply_external_pull_coord_force(pull_work, pullCoordIndex[d],
...
void Awh::initHistoryFromState(AwhHistory *awhHistory) const
{
    ...
    awhHistory->bias.resize(biasAndCoupling_.size());

    for (size_t k = 0; k < awhHistory->bias.size(); k++)
    {
        AwhBiasHistory *biasHistory = &awhHistory->bias[k];
        biasHistory->pointState.resize(biasAndCoupling_[k].bias->pointState().size());
    }
}

/* Restore the AWH state from history. */
                                  ...
                                  const t_commrec  *cr)
{
    ...
    if (MASTER(cr))
    {
        GMX_RELEASE_ASSERT(awhHistory.bias.size() == biasAndCoupling_.size(), "AWH state and history bias count should match");
    ...
    for (size_t k = 0; k < biasAndCoupling_.size(); k++)
    {
        Bias *bias = biasAndCoupling_[k].bias.get();
...
void Awh::updateHistory(AwhHistory *awhHistory) const
{
    ...
    for (size_t k = 0; k < awhHistory->bias.size(); k++)
    {
        biasAndCoupling_[k].bias->updateHistory(&awhHistory->bias[k]);
    }
}
