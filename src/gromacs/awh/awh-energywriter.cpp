...
#include "awh-energywriter.h"

#include <assert.h>
#include <cmath>

#include "gromacs/awh/awh-correlation.h"
...
typedef struct awh_block_t {
    int     npoints;                      /* Number of data points in block */
    int     normtype;                     /* How to normalize the output data */
    double  normvalue;                    /* The normalization value */
    double *data;                         /* The data */
} awh_block_t;

typedef struct awh_writer_t {
    int             nblock;                /* Number of blocks */
    int             var_to_block[evarNR];  /* Start block index for each variable */
    awh_block_t    *block;                 /* The data blocks */
} awh_writer_t;

typedef struct awhbias_energywriter_t {
    int             nstout;                /* Printing interval */
    int             nwriter;               /* Number of AWH biases to write */
    awh_writer_t   *writer;                /* The writers */
    bool            bPrint;                /* Ready to print? */
} awhbias_energywriter_t;


static bool has_var_block(awh_writer_t *writer, int evar)
{
    return writer->var_to_block[evar] >= 0;
}

static int get_var_startblock(awh_writer_t *writer, int evar)
{
    return writer->var_to_block[evar];
}

bool time_to_write(gmx_int64_t step, const awhbias_energywriter_t *writer)
{
...
static double get_coord_normvalue(const t_awh *awh, const pull_params_t *pull_params, int awh_dimindex)
{
    /* AWH may use different units internally but here we convert to user units */
    return coord_value_conversion_factor_internal2userinput(awh, pull_params, awh_dimindex);
}

static double get_distribution_normvalue(const t_awh *awh, const pull_params_t *pull_params)
{
    double volume = 1;
    for (int d =  0; d < awh->ndim; d++)
    {
        double   spacing = awh->grid->axis[d].spacing;

        spacing     *= coord_value_conversion_factor_internal2userinput(awh, pull_params, d);

        /* To get numbers on the order of 1 for angle intervals we normalize angle distributions to 180(^ndim) */
        if (pull_coordinate_is_angletype(&pull_params->coord[awh->pull_coord_index[d]]))
        {
            spacing *= 180;
        }
        if (spacing > 0)
...
static double get_normvalue(int evar, const t_awh *awh, const pull_params_t *pull_params, int count)
{
    double normvalue = 0;
    ...
    {
        case evarCOORDVALUE:
            normvalue = get_coord_normvalue(awh, pull_params, count);
            break;
        case evarVISITS:
        case evarWEIGHTS:
        case evarTARGET:
...
static void set_writer_data(awh_writer_t *writer, double *data)
{
...
static void init_awh_block(awh_block_t *block, int npoints, int normtype, double normvalue)
{
    block->data      = NULL;
                            ...
                            const t_awh *awh, const pull_params_t *pull_params)
{
    ...
    for (int evar = 0; evar < evarNR; evar++)
    {
        ...
        npoints        = (evar == evarMETA) ? emetadataNR : awh->npoints;
        bstart         = writer->var_to_block[evar];
        var_blockcount = 0;
        for (int b = bstart; b < bstart + var_nblock[evar]; b++)
        {
            init_awh_block(&writer->block[b], npoints,
                           get_normtype(evar),
                           get_normvalue(evar, awh, pull_params, var_blockcount));
            blockcount++;
        }
    }
}
...
{
    double                 *data;
    awhbias_energywriter_t *energywriter;
    ...
    energywriter->nwriter = awhbias->nawh;
    ...
    for (int k = 0; k < energywriter->nwriter; k++)
    {
        awh_writer_t *writer = &energywriter->writer[k];
...
static void normalize_data(awh_writer_t *writer, const t_awh *awh)
{
    /* Normalize the data */
    for (int b = 0; b < writer->nblock; b++)
    {
        awh_block_t *block    = &writer->block[b];
        double       minval   = GMX_DOUBLE_MAX;
        double       inv_norm = 0;
        ...
        switch (block->normtype)
        {
            ...
            case enormtypeFREE_ENERGY:
                ...
                for (int m = 0; m < block->npoints; m++)
                {
                    if (write_point_to_output(awh, m) && block->data[m] < minval)
                    {
                        minval = block->data[m];
                    }
                }
                for (int m = 0; m < block->npoints; m++)
                {
                    if (write_point_to_output(awh, m))
                    {
                        block->data[m] -= minval;
                    }
                }
            ...
            case enormtypeDISTRIBUTION:
                ...
                for (int m = 0; m < block->npoints; m++)
                {
                    block->data[m] *= inv_norm*block->normvalue;
                }
                break;
            default:
...
static void transfer_variable_point_data_to_writer(int evar, awh_writer_t *writer, int m, const awh_params_t *awh_params,
                                                   const t_awh *awh, const double *pmf)
{
    ...
    awh_block_t *block      = writer->block;
        ...
        case evarMETA:
            switch (m)
            {
                ...
                case emetadataTARGETERROR:
                    ...
                    block[b].data[m] = awh_params->error_initial*std::sqrt(awh->histsize_initial/awh->histsize);
                    break;
                case emetadataLOG_RELATIVE_SAMPLEWEIGHT:
                    ...
                    block[b].data[m] = awh->log_relative_sampleweight;
                    break;
            }
            break;
        case evarCOORDVALUE:
            for (int d = 0; d < awh->ndim; d++)
            {
                block[b].data[m] = awh->grid->point[m].value[d];
                b++;
            }
            break;
        case evarPMF:
            block[b].data[m] = write_point_to_output(awh, m) ? pmf[m] : 0;
            break;
        case evarBIAS:
            block[b].data[m] = write_point_to_output(awh, m) ? calc_convolved_bias(awh, awh->grid->point[m].value) : 0;
            break;
        case evarVISITS:
            block[b].data[m] = awh->coord_point[m].visits_tot;
            break;
        case evarWEIGHTS:
            block[b].data[m] = awh->coord_point[m].weightsum_tot;
            break;
        case evarTARGET:
...
static void prep_awh_output(awh_writer_t *writer, const awh_params_t *awh_params, const t_awh *awh, const gmx_multisim_t *ms)
{
    double     *pmf;
    ...
    if (write_point_to_output(awh, awh->coord_refvalue_index))
    {
        get_pmf(awh, ms, pmf);
    }
    ...
    {
        transfer_variable_point_data_to_writer(evarMETA, writer, i, awh_params, awh, pmf);
    }
    for (int m = 0; m < awh->npoints; m++)
    {
        for (int evar = 0; evar < evarNR; evar++)
        {
            ...
            transfer_variable_point_data_to_writer(evar, writer, m, awh_params, awh, pmf);
        }
    }

    /* For looks of the output, normalize it */
    normalize_data(writer, awh);
}
                         ...
                         const t_awhbias *awhbias, const gmx_multisim_t *ms)
{
    ...
    {
        prep_awh_output(&energywriter->writer[k], &awhbias_params->awh_params[k], &awhbias->awh[k], ms);
    }

    energywriter->bPrint = TRUE;
}

static void write_awh_to_subblocks(t_enxsubblock *sub, const awh_writer_t *writer)
{
    for (int b = 0; b < writer->nblock; b++)
    {
        sub[b].type  = xdr_datatype_double;
        sub[b].nr    = writer->block[b].npoints;
        sub[b].dval  = writer->block[b].data;
    }
}
