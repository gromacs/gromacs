...
static int get_index_along_axis_for_value(const grid_axis_t *axis, double value)
{
    ...
    if (axis->spacing > 0)
    {
        /* TODO: change args for get_period_function to be compatible to use here. */
        double length = get_interval_length_periodic(axis->origin, value, axis->npoints_period*axis->spacing);

        /* Shift by +0.5 so we can use floor or integer casting below to get the integer index */
        index = static_cast<int>(floor(length/axis->spacing + 0.5));
    }
