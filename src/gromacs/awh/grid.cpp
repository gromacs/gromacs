...
static int getClosestIndexInGridAlongAxis(const GridAxis &axis, double value)
{
    ...
    if (index < 0 || index >= axis.numPoints())
    {
        if (axis.isPeriodic())
        {
            GMX_RELEASE_ASSERT(index >= 0 && index < axis.numPointsInPeriod(),
                               "Index not in periodic interval 0 for AWH periodic axis");
            int endDistance    = (index - (axis.numPoints() - 1));
            int originDistance = (axis.numPointsInPeriod() - index);
            index = originDistance < endDistance ? 0 : axis.numPoints() - 1;
        }
        else
        {
            index = (index < 0) ? 0 : (axis.numPoints() - 1);
        }
    }
...
Grid::Grid(const std::vector<DimParams> &dimParams, const awh_dim_params_t *awhDimParams)
{
    ...
    for (size_t d = 0; d < dimParams.size(); d++)
    {
        ...
        double pointDensity = std::sqrt(dimParams[d].betak)*c_pointDensity;
