 ...
 */
static void gridToSubgridIndex(const grid_t *grid,
 ...
 */
static bool subgridToGridIndex(const grid_t   *grid,
 ...
 */
static int pointDistanceAlongAxis(const grid_axis_t *axis, double x, double x0)
{
    int distance = 0;
 ...
 * \param[in] grid                       The grid.
 * \param[in,out] neighbor_index_array   Array to fill with neighbor indices.
 * \returns the number of neighbors of the point.
 */
static int set_neighbors_of_grid_point(int point_index, const grid_t *grid, int *neighbor_index_array)
{
                          ...
                          const char *correctFormatMessage)
{
    ...
    for (int d = grid->ndim - 1; d >= 0; d--)
    {
        ...
        while ((pointIndex < ndata) && (!gmx_within_tol((firstValue - data[d][pointIndex]), 0, GMX_REAL_EPS)));
