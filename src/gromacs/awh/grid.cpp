 ...
 *
 * \param[in] axis   The axis to query.
 ...
 *
 * \param[in] axus  The axis to query.
 * \returns the period.
 */
static double get_gridaxis_period(const GridAxis &axis)
{
    return axis.npoints_period*axis.spacing;
}
...
double get_deviation_from_point_along_gridaxis(const Grid *grid, int dimIndex, int pointindex, double value)
{
    ...
    return get_deviation_periodic(value, pointvalue, get_gridaxis_period(grid->axis(dimIndex)));
}

/* Gets the length of the grid for the given dimension. */
double get_gridaxis_length(const GridAxis &axis)
{
    return get_interval_length_periodic(axis.origin, axis.end,
                                        get_gridaxis_period(axis));
}
 ...
 */
static bool value_is_in_grid(const awh_dvec               value,
                             const std::vector<GridAxis> &axis)
{
    ...
    for (size_t d = 0; d < axis.size(); d++)
    {
        ...
        if (!(index >= 0 && index < axis[d].npoints))
        {
            return false;
        }
    }
...
{
    return value_is_in_grid(value, grid->axis());
}
 ...
 */
static int get_closest_index_in_grid(const awh_dvec               value,
                                     const std::vector<GridAxis> &axis)
{
    ...
    for (size_t d = 0; d < axis.size(); d++)
    {
        index_multi[d] = get_closest_index_in_grid_along_axis_for_value(axis[d], value[d]);
    }
...
{
    return get_closest_index_in_grid(value, grid->axis());
}
...
size_t Grid::initPointNeighbors()
{
    ...
    for (size_t m = 0; m < point_.size(); m++)
    {
        ...
        maxNumNeighbors = std::max(maxNumNeighbors, neighbor->size());
    }
...
void Grid::initPoints(const awh_dvec period)
{
    ...
    for (size_t d = 0; d < axis_.size(); d++)
    {
        ...
        npoints_dim_work[d] = axis_[d].npoints;
    }
    ...
    for (auto &point : point_)
    {
        for (size_t d = 0; d < axis_.size(); d++)
        {
            point.value[d] = axis_[d].origin + index_work[d]*axis_[d].spacing;
        ...
        step_in_multidim_array(axis_.size(), npoints_dim_work, index_work);
    }
    ...
    for (size_t d = 0; d < axis_.size(); d++)
    {
        axis_[d].end = point_[point_.size() - 1].value[d];
    }
}
...
Grid::Grid(int numDims, const DimParams *dimParams,
           const awh_dvec origin, const awh_dvec end,
           const awh_dvec period, int *nneighbors_alloc_ptr)
{
    axis_.resize(numDims);

    /* Define the discretization along each dimension */
    int npoints = 1;
    for (int d = 0; d < numDims; d++)
    {
        init_gridaxis(&axis_[d], origin[d], end[d], dimParams[d].betak, period[d]);
        npoints *= axis_[d].npoints;
    }

    point_.resize(npoints);

    /* Set their values */
    initPoints(period);

    /* Keep a neighbor list for each point */
    *nneighbors_alloc_ptr = initPointNeighbors();
}
                          ...
                          const char *correctFormatMessage)
{
    ...
    for (size_t d = 0; d < axis_.size(); d++)
    {
        ...
        int npoints    = axis_[d].npoints;

        axis_[d].origin = data[d][0];
        axis_[d].end    = data[d][ndata - 1];

        /* The period should be the same for the AWH grid and the data grid. */
        double period     = get_gridaxis_period(grid->axis(d));
        double axisLength = get_interval_length_periodic(axis_[d].origin, axis_[d].end, period);

        axis_[d].spacing        = npoints > 1 ? axisLength/(npoints - 1) : period;
        axis_[d].npoints_period = static_cast<int>(std::round(period/axis_[d].spacing));
    }
    ...
    for (size_t m = 0; m < grid->numPoints(); m++)
    {
        ...
        if (!value_is_in_grid(grid->point(m).value, axis_))
        {
        ...
        (*gridpoint_to_datapoint)[m] = get_closest_index_in_grid(grid->point(m).value, axis_);
    }
}
