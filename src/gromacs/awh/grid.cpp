...
double getDeviationFromPointAlongGridaxis(const Grid *grid, int dimIndex, int pointindex, double value)
{
    ...
    return getDeviationPeriodic(value, pointValue, grid->axis(dimIndex).period());
}

/* Gets the length of the grid for the given dimension. */
double getGridaxisLength(const GridAxis &axis)
{
    return getIntervalLengthPeriodic(axis.origin, axis.end, axis.period());
}
                               ...
                               int point, awh_ivec subgridIndex)
{
    ...
    for (int d = 0; d < grid->ndim(); d++)
    {
        ...
        makeIndexPeriodic(&subgridIndex[d], grid->axis(d).npoints_period);
...
static int pointDistanceAlongAxis(const GridAxis &axis, double x, double x0)
{
    ...
    if (axis.spacing > 0)
    {
        ...
        makeIndexPeriodic(&distance, axis.npoints_period);
    }
 ...
 * \returns true if the value is in the grid.
 */
bool valueIsInGrid(const awh_dvec value, const Grid *grid)
{
    return valueIsInGrid(value, grid->axis());
}
 ...
 */
static int getClosestIndexInGridAlongAxis(const GridAxis &axis, double value)
{
 ...
 */
static int getClosestIndexInGrid(const awh_dvec               value,
                                 const std::vector<GridAxis> &axis)
{
    ...
    for (size_t d = 0; d < axis.size(); d++)
    {
        index_multi[d] = getClosestIndexInGridAlongAxis(axis[d], value[d]);
    }
 ...
 * \returns the point index closest to the value.
 */
int getClosestIndexInGrid(const awh_dvec value, const Grid *grid)
{
    return getClosestIndexInGrid(value, grid->axis());
}
...
void Grid::initPoints(const awh_dvec period)
{
    ...
    for (auto &point : point_)
    {
        for (size_t d = 0; d < axis_.size(); d++)
        {
            ...
            /* clang thinks period[d] can be garbage */
#ifndef __clang_analyzer__
            if (period[d] > 0)
            {
                makeValuePeriodic(&point.value[d], period[d]);
            }
#endif
                          ...
                          double inv_pointvariance, double period)
{
    ...
    double axisLength = getIntervalLengthPeriodic(axis->origin, axis->end, period);
...
Grid::Grid(const std::vector<DimParams> &dimParams, const awh_dim_params_t *awhDimParams)
{
    ...
    for (size_t d = 0; d < dimParams.size(); d++)
    {
        ...
        init_gridaxis(&axis_[d], origin, end, dimParams[d].betak, period[d]);
                       ...
                       const char *correctFormatMessage)
{
    ...
    for (size_t d = 0; d < axis_.size(); d++)
    {
        ...
        double period     = grid->axis(d).period();
        double axisLength = getIntervalLengthPeriodic(axis_[d].origin, axis_[d].end, period);
    ...
    for (size_t m = 0; m < grid->numPoints(); m++)
    {
        ...
        if (!valueIsInGrid(grid->point(m).value, axis_))
        {
        ...
        (*gridpointToDatapoint)[m] = getClosestIndexInGrid(grid->point(m).value, axis_);
    }
}
