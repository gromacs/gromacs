                          ...
                          double inv_pointvariance, double period)
{
    ...
    else
    {
        ...
        axis->npoints     = 1 + static_cast<int>(ceil(c_pointDensity*nstdevs));
    }

    /* Set point spacing based on the number of points */
    if (bAxisIsPeriodic)
    {
        if (axisLength > 0 && axis->npoints > 1)
        {
        ...
        {
            GMX_RELEASE_ASSERT(axis->npoints == 1, "With axisLength=0 we expect 1 point");
            axis->npoints_period = 1;
            axis->spacing        = period;
            axis->npoints        = 1;
        }
    }
