...
void Grid::initPoints(const awh_dvec period)
{
    ...
    for (auto &point : point_)
    {
        for (size_t d = 0; d < axis_.size(); d++)
        {
            ...
            if (period[d] > 0)
            {
                makeValuePeriodic(&point.coordValue[d], period[d]);
            }
#endif
    ...
    for (size_t d = 0; d < axis_.size(); d++)
    {
        axis_[d].end = point_[point_.size() - 1].coordValue[d];
    }
}
                       ...
                       const char *correctFormatMessage)
{
    ...
    for (size_t m = 0; m < grid->numPoints(); m++)
    {
        ...
        (*gridpointToDatapoint)[m] = getClosestIndexInGrid(grid->point(m).coordValue, axis_);
    }
}
