...
{
    static_assert(Grid::c_numPointsPerSigma == 1.0, "Currently the test code assumes exactly 1 point per sigma. This can be changed, but requires some consideration of rouding issues.");
    const int scopeInPoints      = static_cast<int>(Grid::c_scopeCutoff);
    ...
    awhDimParams[0].origin       =  0;
    awhDimParams[0].end          = 10;
