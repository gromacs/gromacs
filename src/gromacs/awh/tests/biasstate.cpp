...
static AwhTestParameters getAwhTestParameters()
{
    ...
    awhBiasParams.dimParams            = params.awhDimParams;
...
class BiasStateTest : public ::testing::TestWithParam<const char *>
{
    public:
        ...
        //! Constructor
        BiasStateTest() :
            params_(getAwhTestParameters()),
            dimParams_({ DimParams(1, 15, 1), DimParams(1, 15, 1) }
                       ),
            grid_(dimParams_, params_.awhBiasParams.dimParams),
            biasParams_(params_.awhParams, params_.awhBiasParams, dimParams_, 1.0, 1.0, BiasParams::DisableUpdateSkips::no, nullptr, grid_.axis(), 0),
            biasState_(params_.awhBiasParams, 1.0, dimParams_, grid_)
        {
...
// Test reading of the two different supported PMF+target input formats
