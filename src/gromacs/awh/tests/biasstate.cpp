...
static AwhTestParameters getAwhTestParameters()
{
    ...
    AwhDimParams &awhDimParams0   = params.awhDimParams[0];
    ...
    AwhDimParams &awhDimParams1   = params.awhDimParams[1];
    ...
    awhBiasParams.dimParams            = &params.awhDimParams[0];
...
{
    public:
        AwhTestParameters      params_;     //!< AWH inputrec parameters
        std::vector<DimParams> dimParams_;  //!< Bias dimension parameters
        Grid                   grid_;       //!< The grid
        BiasParams             biasParams_; //!< Bias parameters
        BiasState              biasState_;  //!< The bias state

        //! Constructor
        BiasStateTest() :
            params_(std::move(getAwhTestParameters())),
            dimParams_({ DimParams(1.0, 15.0, 1.0), DimParams(1.0, 15.0, 1.0) }
                       ),
            grid_(dimParams_, params_.awhBiasParams.dimParams), //!< Keep doxygen happy
            biasParams_(params_.awhParams, params_.awhBiasParams, dimParams_, 1.0, 1.0, BiasParams::DisableUpdateSkips::no, nullptr, grid_.axis(), 0), //!< Keep doxygen happy
            biasState_(params_.awhBiasParams, 1.0, dimParams_, grid_) //!< Keep doxygen happy
        {
            ...
            biasState_.initGridPointState(params_.awhBiasParams, dimParams_, grid_, biasParams_, filename, params_.awhParams.numBias, nullptr);
        }
};
...
TEST_P(BiasStateTest, InitializesFromFile)
{
    ...
    /* Compute the mean square displacement from the expected values in the file.
