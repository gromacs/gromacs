...
struct AwhTestParameters
{
    ...
    AwhDimParams  awhDimParams0;
    AwhDimParams  awhDimParams1;
...
static AwhTestParameters getAwhTestParameters()
{
    ...
    awhParams.numBias                    = 1;
    awhParams.awhBiasParams              = &params.awhBiasParams;
...
class BiasStateTest : public ::testing::TestWithParam<const char *>
{
    public:
        ...
        BiasStateTest()
        {
            const AwhTestParameters params = getAwhTestParameters();
            std::vector<DimParams>  dimParams;
            dimParams.push_back(DimParams(1.0, 15.0, params.beta));
            dimParams.push_back(DimParams(1.0, 15.0, params.beta));
            Grid                    grid(dimParams, params.awhBiasParams.dimParams);
            BiasParams              biasParams(params.awhParams, params.awhBiasParams, dimParams, 1.0, 1.0, BiasParams::DisableUpdateSkips::no, nullptr, grid.axis(), 0);
            biasState_ = std::unique_ptr<BiasState>(new BiasState(params.awhBiasParams, 1.0, dimParams, grid));
            ...
            biasState_->initGridPointState(params.awhBiasParams, dimParams, grid, biasParams, filename, params.awhParams.numBias, nullptr);
        }
};
