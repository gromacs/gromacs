...
const double g_coords[] = {
    ...
    0.68,
    0.71,
    // Without the next value I get indentical bias for all points
    //0,73
};
...
TEST_P(BiasTest, ProducesCorrectBias)
{
    ...
    printf("inInitial %d convolve %d skipUpd %d\n", bias.state().inInitialStage, bias.params().convolveForce, bias.params().skipUpdates());
    for (auto &point : bias.pointState())
    {
        printf(" %f %f ", point.bias(), point.logPmfsum());
    }
    printf("\n");
}
