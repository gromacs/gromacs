...
//! Convenience typedef
typedef std::tuple<bool, int, bool> BiasTestParameters;
...
class BiasTest : public ::testing::TestWithParam<BiasTestParameters>
{
    public:
        ...
        {
            bool convolveForce;
            int  numSamplesUpdateFreeEnergy;
            bool allowUpdateSkips;
            std::tie(convolveForce, numSamplesUpdateFreeEnergy, allowUpdateSkips) = GetParam();
            ...
            awhParams.nsamples_update_free_energy = numSamplesUpdateFreeEnergy;
...
TEST_P(BiasTest, ProducesCorrectBias)
{
    ...
    printf("convolve %d nstFup %d skipUpd %d\n", bias.params().convolveForce, bias.params().numSamplesUpdateFreeEnergy, bias.params().skipUpdates());
