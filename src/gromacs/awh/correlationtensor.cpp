 ...
 * \param[in] currentAccumulatedLength  Sampling length of all data collected up to the current
 * step, in time or weight. \returns the block index.
 */
    ...
    if (!firstBlock)
    {
        ...
        for (int d1 = 0; d1 < numDim; d1++)
        {
            ...
            for (int d2 = 0; d2 <= d1; d2++)
            {
                ...
                GMX_ASSERT(sumOverBlocksSquareBlockWeight_,
                           "Denominator should be > 0 (should be guaranteed by the conditional "
                           "above)");
                double oldAverageX =
                        c1.sumOverBlocksBlockWeightBlockWeightX / sumOverBlocksSquareBlockWeight_;

                double newSumWeightBlockWeightY =
                        c2.sumOverBlocksBlockWeightBlockWeightX + blockSumWeight_ * c2.blockSumWeightX;
                double newSumSquareBlockWeight =
...
void CorrelationTensor::addData(double weight, gmx::ArrayRef<const double> data, bool blockLengthInWeight, double t)
{
...
CorrelationTensor::CorrelationTensor(int numDim, int numBlockData, double blockLengthInit)
{
