 ...
 */
static void init_bias_history_from_state(awh_bias_history_t *bias_history, const Bias *bias)
{
                                 ...
                                 const AwhBiasCollection  *awh)
{
    ...
    awh_history->nbias = awh->bias.size();
    snew(awh_history->bias, awh_history->nbias);
    for (int k = 0; k < awh_history->nbias; k++)
    {
        init_bias_history_from_state(&awh_history->bias[k], &awh->bias[k]);
    }
}
 ...
 * \param[in] bias               Bias state.
 */
static void update_bias_history(awh_bias_history_t *bias_history, const Bias &bias)
{
 ...
 */
static void restore_bias_state_from_history(const awh_bias_history_t *bias_history, Bias *bias)
{
                                    ...
                                    AwhBiasCollection    *awh)
{
    ...
    for (int k = 0; k < awh_history->nbias; k++)
    {
        restore_bias_state_from_history(&awh_history->bias[k], &awh->bias[k]);
    }
}
                        ...
                        const AwhBiasCollection  *awh)
{
    ...
    for (int k = 0; k < awh_history->nbias; k++)
    {
        update_bias_history(&awh_history->bias[k], awh->bias[k]);
    }
}
