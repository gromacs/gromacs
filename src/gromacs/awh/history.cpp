 ...
 */
static void init_bias_history_from_state(awh_bias_history_t *bias_history, const AwhBias *bias)
{
    init_bias_history(bias_history);

    bias_history->npoints = bias->coordPoint.size();

    snew(bias_history->coordpoint, bias_history->npoints);
}

/* Allocate and initialize an AWH history with the given AWH state. */
void init_awh_history_from_state(awh_history_t            *awh_history,
                                 const AwhBiasCollection  *awh)
{
    init_awh_history(awh_history);

    awh_history->nbias = awh->awhBias.size();
    snew(awh_history->bias, awh_history->nbias);
    for (int k = 0; k < awh_history->nbias; k++)
    {
        init_bias_history_from_state(&awh_history->bias[k], &awh->awhBias[k]);
    }
}
 ...
 * \param[in] bias               Bias state.
 */
static void update_bias_history(awh_bias_history_t *bias_history, const AwhBias &bias)
{
    ...
    bias_history->refCoordpoint = state.refCoordpoint;

    for (int m = 0; m < bias_history->npoints; m++)
    {
        const CoordPoint &coordPoint = bias.coordPoint[m];
        bias_history->coordpoint[m].target              = coordPoint.target;
        bias_history->coordpoint[m].free_energy         = coordPoint.free_energy;
        bias_history->coordpoint[m].bias                = coordPoint.bias;
        bias_history->coordpoint[m].weightsum_iteration = coordPoint.weightsum_iteration;
        bias_history->coordpoint[m].weightsum_covering  = coordPoint.weightsum_covering;
        bias_history->coordpoint[m].weightsum_tot       = coordPoint.weightsum_tot;
        bias_history->coordpoint[m].weightsum_ref       = coordPoint.weightsum_ref;
        bias_history->coordpoint[m].last_update_index   = coordPoint.last_update_index;
        bias_history->coordpoint[m].log_pmfsum          = coordPoint.log_pmfsum;
        bias_history->coordpoint[m].visits_iteration    = coordPoint.visits_iteration;
        bias_history->coordpoint[m].visits_tot          = coordPoint.visits_tot;
    }

    bias_history->in_initial              = state.in_initial;
    bias_history->equilibrateHistogram    = state.equilibrateHistogram;
    bias_history->histsize                = state.histsize;

    bias_history->origin_index_updatelist = multidim_gridindex_to_linear(bias.grid.get(),
                                                                         state.origin_updatelist);
    bias_history->end_index_updatelist    = multidim_gridindex_to_linear(bias.grid.get(),
                                                                         state.end_updatelist);

    bias_history->scaledSampleWeight      = state.scaledSampleWeight;
    bias_history->maxScaledSampleWeight   = state.maxScaledSampleWeight;
}
 ...
 */
static void restore_bias_state_from_history(const awh_bias_history_t *bias_history, AwhBias *bias)
{
    BiasState *state = &bias->state;

    state->refCoordpoint = bias_history->refCoordpoint;

    for (size_t m = 0; m < bias->coordPoint.size(); m++)
    {
        const awh_coordpoint_history_t &coordPoint = bias_history->coordpoint[m];
    ...
    state->in_initial             = bias_history->in_initial;
    state->equilibrateHistogram   = bias_history->equilibrateHistogram;
    state->histsize               = bias_history->histsize;

    linear_gridindex_to_multidim(bias->grid.get(), bias_history->origin_index_updatelist, state->origin_updatelist);
    linear_gridindex_to_multidim(bias->grid.get(), bias_history->end_index_updatelist, state->end_updatelist);

    state->scaledSampleWeight     = bias_history->scaledSampleWeight;
    state->maxScaledSampleWeight  = bias_history->maxScaledSampleWeight;
}
                                    ...
                                    AwhBiasCollection    *awh)
{
    ...
    for (int k = 0; k < awh_history->nbias; k++)
    {
        restore_bias_state_from_history(&awh_history->bias[k], &awh->awhBias[k]);
    }
}
                        ...
                        const AwhBiasCollection  *awh)
{
    ...
    for (int k = 0; k < awh_history->nbias; k++)
    {
        update_bias_history(&awh_history->bias[k], awh->awhBias[k]);
    }
}
