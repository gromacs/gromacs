                                                   ...
                                                   const Bias &bias, const std::vector<float> &pmf)
{
    ...
    switch (evar)
    {
        case evarMETA:
            switch (m)
            {
                ...
                case emetadataTARGETERROR:
                    ...
                    block_[b].data[m] = bias.params().errorInitial*std::sqrt(bias.params().histSizeInitial()/bias.state().histogramSize());
                    break;
        ...
        case evarPMF:
            ...
            break;
        case evarBIAS:
            block_[b].data[m] = bias.state().points()[m].inTargetRegion() ? calculateConvolvedBias(bias.dimParams(), bias.grid(), bias.state().points(), bias.grid().point(m).coordValue) : 0;
            break;
        case evarVISITS:
            block_[b].data[m] = bias.state().points()[m].visits_tot;
            break;
        case evarWEIGHTS:
