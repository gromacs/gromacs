 ...
 */
static double get_coord_normvalue(const Bias &bias, int dimIndex)
{
 ...
 */
static double get_normvalue(int evar, const Bias &bias, int count)
{
    double normvalue = 0;
    ...
    {
        case evarCOORDVALUE:
            normvalue = get_coord_normvalue(bias, count);
            break;
        case evarVISITS:
        case evarWEIGHTS:
        case evarTARGET:
...
BiasWriter::BiasWriter(const Bias &bias)
{
    ...
    for (int evar = 0; evar < evarNR; evar++)
    {
        ...
        {
            npoints        = bias.pointState().size();
        }
        int bstart         = varToBlock_[evar];
        int var_blockcount = 0;
        for (int b = bstart; b < bstart + var_nblock[evar]; b++)
        {
            init_block(&block_[b], npoints,
                       get_normtype(evar),
                       get_normvalue(evar, bias, var_blockcount));
            blockCount++;
        }
    }
...
static void normalizeBlock(Block *block, const Bias &bias)
{
    ...
    switch (block->normtype)
    {
        ...
        case enormtypeFREE_ENERGY:
            ...
            {
                if (bias.pointState()[m].inTargetRegion() && block->data[m] < minval)
                {
                    minval = block->data[m];
                }
            }
                                                   ...
                                                   const Bias &bias, const std::vector<float> &pmf)
{
    ...
    int numCorrelation = bias.forceCorr().tensorSize();
        ...
        case evarMETA:
            switch (m)
            {
                ...
                case emetadataTARGETERROR:
                    ...
                    block_[b].data[m] = bias.params().errorInitial*std::sqrt(bias.params().histSizeInitial/bias.state().histSize);
                    break;
                case emetadataSCALEDSAMPLEWEIGHT:
                    ...
                    block_[b].data[m] = bias.state().scaledSampleWeight;
                    break;
            }
            break;
        case evarCOORDVALUE:
            ...
            break;
        case evarPMF:
            block_[b].data[m] = bias.pointState()[m].inTargetRegion() ? pmf[m] : 0;
            break;
        case evarBIAS:
            block_[b].data[m] = bias.pointState()[m].inTargetRegion() ? calcConvolvedBias(bias, bias.grid()->point(m).coordValue) : 0;
            break;
        case evarVISITS:
            block_[b].data[m] = bias.pointState()[m].visits_tot;
            break;
        case evarWEIGHTS:
            block_[b].data[m] = bias.pointState()[m].weightsumTot;
            break;
        case evarTARGET:
            block_[b].data[m] =  bias.pointState()[m].target();
            break;
        case evarFORCECORRVOL:
            block_[b].data[m] = getCorrelationVolumeElement(bias.forceCorr().corr()[m], bias.forceCorr().dtSample);
            break;
        case evarFRICTION:
            /* Store force correlation in units of friction, i.e. time/length^2 */
            for (int n = 0; n < numCorrelation; n++)
            {
                block_[b].data[m] = getCorrelationTimeIntegral(bias.forceCorr().corr()[m], n, bias.forceCorr().dtSample);
                b++;
            }
            break;
        default:
