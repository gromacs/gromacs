...
#include "gromacs/awh/awh-math.h"
...
static const double pointdensity        = 3;
static const double scope_cutoff        = 5;
static const int    max_nneighbors_axis = pointdensity*scope_cutoff*2 + 1;
...
static bool gridaxis_is_periodic(const t_awh_grid_axis *axis)
{
    return axis->npoints_period  > 0;
}

static double get_gridaxis_period(const t_awh_grid *grid, int dimindex)
{
...
{
    double origin = grid->axis[dimindex].origin, end = grid->axis[dimindex].end;

    return get_interval_length_periodic(origin, end, get_gridaxis_period(grid, dimindex));
}
 ...
 */
static void global_to_local_index(const t_awh_grid *grid, const awh_ivec origin_local_dim,
                                  int global_linear_index, awh_ivec local_index_dim)
{
    for (int d = 0; d < grid->ndim; d++)
    {
        ...
        {
            /* Note: this function does nothing if the period is 0 */
 ...
 */
static bool local_to_global_index(const t_awh_grid *grid, const awh_ivec origin_local_dim, const awh_ivec local_index_dim,
                                  int *global_linear_index_ptr)
{
    bool     bIs_global_point;
    awh_ivec global_index_dim;

    bIs_global_point = TRUE;

    /* Check and apply boundary conditions for each dimension */
    for (int d = 0; d < grid->ndim && bIs_global_point; d++)
    {
        ...
        if (global_index_dim[d] < 0 ||  global_index_dim[d] > grid->axis[d].npoints - 1)
        {
            ...
            if (gridaxis_is_periodic(&grid->axis[d]))
            {
                ...
                if (nwrapped <= 0)
                {
                    bIs_global_point = FALSE;
                }
            }
            else /* non-periodic boundaries */
            {
                bIs_global_point = FALSE;
            }
        }
 ...
 */
bool get_next_point_in_local_grid(const t_awh_grid *grid, const awh_ivec origin_local_dim, const awh_ivec npoints_local_dim,
                                  int *global_linear_index_ptr)
{
    int       new_global_linear_index;
    awh_ivec  local_index_dim = {0};
    bool      bStep, bFound_point;
    ...
    else
    {
        ...
        bFound_point =  local_to_global_index(grid, origin_local_dim, local_index_dim, &new_global_linear_index);
    }

    bStep = !bFound_point;

    /* Step in the local grid until we either find a valid grid point or we cannot step any more. */
    while (bStep && !bFound_point)
    {
        /* If we step, the local index is updated */
        bStep = step_in_multidim_array(grid->ndim, npoints_local_dim, local_index_dim);
    ...
    if (bFound_point)
    {
        *global_linear_index_ptr = new_global_linear_index;
    }

    return bFound_point;
}

static int get_index_along_axis_for_value(const t_awh_grid_axis *axis, double value)
{
    int index = 0;
...
bool value_is_in_grid(const awh_dvec value, const t_awh_grid *grid)
{
    ...
    for (int d = 0; d < grid->ndim && bIn_range; d++)
    {
        int index = get_index_along_axis_for_value(&grid->axis[d], value[d]);
        bIn_range = (index >= 0) && (index < grid->axis[d].npoints);
    }

    return bIn_range;
}

static int get_closest_index_in_grid_along_axis_for_value(const t_awh_grid_axis *axis, double value)
{
...
static int set_neighbors_of_grid_point(int point_index, t_awh_grid *grid, int *neighbor_index_array)
{
    int            nneighbors, neighbor_index;
    ...
    for (int d = 0; d < grid->ndim; d++)
    {
        ...
        ncandidates_dim[d] = std::min(max_nneighbors_axis, grid->axis[d].npoints);
    ...
    while (bExists_point)
    {
        bExists_point = get_next_point_in_local_grid(grid, origin_local_dim, ncandidates_dim, &neighbor_index);
...
static int init_grid_point_neighbors(t_awh_grid *grid)
{
    int       nneighbors_max, all_neighbors_counter;
    int      *all_neighbors;
    ...
    for (int d = 0; d < grid->ndim; d++)
    {
        ...
        nneighbors_max *= max_nneighbors_axis;
    }

    snew(all_neighbors, grid->npoints*nneighbors_max);
    all_neighbors_counter = 0;
    for (int m = 0; m < grid->npoints; m++)
    {
        grid->point[m].neighbor = &(all_neighbors[all_neighbors_counter]);
        all_neighbors_counter  += nneighbors_max;
    }
    all_neighbors = NULL;
...
static void init_grid_points(t_awh_grid *grid, const awh_dvec period)
{
...
static void init_gridaxis(t_awh_grid_axis *axis, double origin, double end,
                          double inv_pointvariance, double period)
{
    const bool bAxis_is_periodic = period > 0;

    axis->origin = origin;
    axis->end    = end;

    /* Explicit number of points not given by user.
       Automatically determine number of points based on the user given endpoints
       and the expected fluctuations in the umbrella. */
    if (axis->end - axis->origin == 0)
    {
        axis->npoints = 1;
    }
    ...
    else
    {
        ...
        if (bAxis_is_periodic)
        {
            axislength  = get_interval_length_periodic(axis->origin, axis->end, period);
        }
        else
        {
            axislength = axis->end - axis->origin;
        }
        nstdevs               = axislength*std::sqrt(inv_pointvariance);
        axis->npoints         = std::max(2, static_cast<int>(ceil(pointdensity*nstdevs)));
    }
    ...
    if (bAxis_is_periodic)
    {
        if (axis->npoints > 1)
        {
            ...
            axis->npoints_period = static_cast<int>(ceil(period/axislength*(axis->npoints - 1)));
                      ...
                      const awh_dvec period, int *nneighbors_alloc_ptr)
{
    ...
    /* The multidimensional index and value for each point in the grid. Allocate memory in one chunk. */
    snew(grid->point, grid->npoints);
    snew(pointindex_tmp, grid->npoints*grid->ndim);
    snew(pointvalue_tmp, grid->npoints*grid->ndim);

    index_tmp = 0;
    for (int m = 0; m < grid->npoints; m++)
    {
        grid->point[m].index = &(pointindex_tmp[index_tmp]);
        grid->point[m].value = &(pointvalue_tmp[index_tmp]);
