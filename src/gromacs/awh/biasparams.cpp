                                      ...
                                      const std::vector<GridAxis>  &gridAxis)
{
    ...
    {
        GMX_ASSERT(dimParams[d].betak > 0, "Inverse temperature (beta) and force constant (k) should be positive.");
        ...
        GMX_ASSERT(gridAxis[d].length()/sigma < std::numeric_limits<int>::max(), "The axis length in units of sigma should fit in an int");
    ...
    GMX_ASSERT(awhParams.numSamplesUpdateFreeEnergy > 0, "When checking for AWH coverings, the number of samples per AWH update need to be > 0.");
    ...
    GMX_ASSERT(numStepsCheck % numStepsUpdate == 0, "Only check covering at free energy update steps");
