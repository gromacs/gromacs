...
#ifndef GMX_MDLIB_AWH_CORRELATION_HISTORY_H
#define GMX_MDLIB_AWH_CORRELATION_HISTORY_H
...
/* \brief
 * Initialize a correlation grid history struct.
 *
 * This would be called if there is not an correlation grid initialized but
 * but basic parameters are available.
 *
...
/* \brief
 * Initialize a correlation grid history with the given correlation grid state.
 *
...
/* \brief
 * Initialize a correlation grid history when restarting from a checkpoint.
 *
 * This function assumes that the master rank has read a checkpoint
 * and initialized its correlation grid history.
 *
 * \param[in,out] corrgrid_hist  Correlation grid history to initialize.
