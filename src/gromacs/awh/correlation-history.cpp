...
static void unpack_correlation_history(correlation_history_t *corr_flat, const correlation_grid_history_t *corrgrid_hist)
{
    ...
    for (int i = 0; i < corrgrid_hist->ncorrmatrix; i++)
    {
        ...
        {
            corr_flat[k] = corrgrid_hist->corrmatrix_hist[i].corr_hist[j];
            k++;
        }
    }
}
static void unpack_blockdata_history(correlation_blockdata_history_t *blockdata_flat, const correlation_grid_history_t *corrgrid_hist)
{
    ...
    for (int i = 0; i < corrgrid_hist->ncorrmatrix; i++)
    {
        for (int j = 0; j < corrgrid_hist->ncorr; j++)
        {
            ...
            {
                blockdata_flat[l] = corrgrid_hist->corrmatrix_hist[i].corr_hist[j].blockdata[k];
                l++;
            }
        }
    }
...
static void pack_correlation_history(correlation_history_t *corr_flat, correlation_grid_history_t *corrgrid_hist)
{
    ...
    for (int i  = 0; i < corrgrid_hist->ncorrmatrix; i++)
    {
        corrgrid_hist->corrmatrix_hist[i].corr_hist = &(corr_flat[j]);
...
static void pack_blockdata_history(correlation_blockdata_history_t *blockdata_flat, correlation_grid_history_t *corrgrid_hist)
{
    ...
    for (int i = 0; i < corrgrid_hist->ncorrmatrix; i++)
    {
        for (int j = 0; j < corrgrid_hist->ncorr; j++)
        {
            corrgrid_hist->corrmatrix_hist[i].corr_hist[j].blockdata = &(blockdata_flat[k]);
...
static void broadcast_initialize_correlation_grid_history(correlation_grid_history_t *corrgrid_hist, const t_commrec *cr)
{
    ...
    if (!MASTER(cr))
    {
        snew(corrgrid_hist->corrmatrix_hist, corrgrid_hist->ncorrmatrix);
    }
    gmx_bcast(sizeof(correlation_matrix_history_t)*corrgrid_hist->ncorrmatrix, corrgrid_hist->corrmatrix_hist, cr);
...
void init_correlation_grid_history_from_checkpoint(correlation_grid_history_t *corrgrid_hist, const t_commrec *cr)
{
    ...
    if (PAR(cr))
    {
        GMX_RELEASE_ASSERT((MASTER(cr) && corrgrid_hist->corrmatrix_hist != NULL) ||
                           (!MASTER(cr) && corrgrid_hist->corrmatrix_hist == NULL),
...
void init_correlation_grid_history(correlation_grid_history_t *corrgrid_hist, int ncorrmatrix, int ncorr, int nblockdata)
{
    ...
    snew(corrgrid_hist->corrmatrix_hist, ncorrmatrix);
...
void init_correlation_grid_history_from_state(correlation_grid_history_t *corrgrid_hist, const correlation_grid_t *corrgrid)
{
...
{
    GMX_RELEASE_ASSERT(corrgrid_hist->corrmatrix_hist != NULL, "AWH force correlation matrix not initialized when updating history");

    /* Matrix for each grid point */
    for (int m = 0; m < corrgrid->ncorrmatrix; m++)
    {
        correlation_matrix_history_t *corrmatrix_hist = &corrgrid_hist->corrmatrix_hist[m];
        ...
        for (int k = 0; k < corrmatrix->ncorr; k++)
        {
            ...
            t_correlation         *corr      = &corrmatrix->corr[k];
            ...
            for (int l = 0; l < corr->nblockdata; l++)
            {
                ...
                t_correlation_blockdata         *blockdata      = &corr->blockdata[l];
