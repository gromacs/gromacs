...
{
    const int nstsample_coord       = awhParams->nstsample_coord;
    const int nstupdate_free_energy = awhParams->nsamples_update_free_energy*nstsample_coord;

    /* Copy over the parameters */
    bias_.resize(awhParams->nbias);
    ...
    for (int k = 0; k < awhParams->nbias; k++)
    {
        ...
        initBias(fplog, ir, cr, k, awhParams->nbias, &bias_[k], awhParams->awh_bias_params[k]);
    }

    /* Need to register the AWH coordinates to be allowed to apply forces to the pull coordinates. */
    registerBiasWithPull(awhParams, pull_work);
    ...
    else if (MASTER(cr))
    {
        *awhHistoryPtr = new AwhHistory();
        initHistoryFromState(*awhHistoryPtr);
    }
}
