                                        ...
                                        FILE                    *fplog)
{
    ...
    int         my_id = get_domain_id(awh_bias_params, grid, awh_bias.state.coord_value_index);

    if (fplog != NULL)
    {
        /* Print partitioning info for this simulation. */
        int domain_imin, domain_imax;
        get_domain_boundary(awh_bias_params, grid, my_id, &domain_imin, &domain_imax);
        ...
        {
            fprintf(fplog, "[%g, %g]",
                    awh_bias.dimParams[d].scaleInternalToUserInput(grid->point(domain_imin).value[d]),
                    awh_bias.dimParams[d].scaleInternalToUserInput(grid->point(domain_imax).value[d]));
    ...
    else
    {
        ...
        for (int id = 0; id < ndomains; id++)
        {
            ...
            int domain_imin, domain_imax;
            get_domain_boundary(awh_bias_params, grid, id, &domain_imin, &domain_imax);
            ...
            for (int d = 0; d < awh_bias.ndim; d++)
            {
                ...
                fprintf(stderr, "[%.4g, %.4g]",
                        awh_bias.dimParams[d].scaleInternalToUserInput(grid->point(coord_imin).value[d]),
                        awh_bias.dimParams[d].scaleInternalToUserInput(grid->point(coord_imax).value[d]));
...
static void partition_domain(AwhBias *awh_bias, const awh_bias_params_t *awh_bias_params, int point_index)
{
    ...
    for (size_t m = 0; m < awh_bias->coordPoint.size(); m++)
    {
        ...
        for (int d = 0; d < awh_bias->ndim; d++)
        {
            int index_d = awh_bias->grid->point(m).index[d];
                                            ...
                                            const Grid *grid, double dt_sample)
{
    ...
    for (int d = 0; d < grid->ndim(); d++)
    {
        double L = get_gridaxis_length(grid->axis(d));
        if (L > 0)
        {
...
static void read_user_pmf_and_target(AwhBias *awh_bias, int nbias)
{
    ...
    grid2data_index.resize(awh_bias->grid->numPoints());
                          ...
                          awh_bias_params_t          *awh_bias_params)
{
    ...
    awh_bias->grid = std::unique_ptr<Grid>(new Grid(awh_bias->ndim, awh_bias->dimParams, grid_origin, grid_end, grid_period));
    ...
    for (int d = 0; d < awh_bias->ndim; d++)
    {
        int index_d                 = awh_bias->grid->point(state->coord_value_index).index[d];
        state->origin_updatelist[d] = index_d;
        state->end_updatelist[d]    = index_d;
    }

    /* Initialize free energy functions and biases */
    awh_bias->coordPoint.resize(awh_bias->grid->numPoints());
    awh_bias->updateList.resize(awh_bias->grid->numPoints());
                             ...
                             struct pull_t      *pull_work)
{
    ...
    {
        const awh_bias_params_t &bias_params = awh_params->awh_bias_params[k];

        for (int d = 0; d < bias_params.ndim; d++)
        {
            register_external_pull_potential(pull_work, bias_params.dim_params[d].pull_coord_index, external_potential_string);
        }
    }
}
                               ...
                               bool                     startingFromCheckpoint)
{
    ...
    register_bias_with_pull(awh_params, pull_work);
