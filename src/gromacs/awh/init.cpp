                                        ...
                                        FILE                    *fplog)
{
    ...
    if (fplog != NULL)
    {
        ...
        {
            fprintf(fplog, "[%g, %g]",
                    scaleInternalToUserInput(awh_bias_params, d, awh_bias->grid->point[domain_imin].value[d]),
                    scaleInternalToUserInput(awh_bias_params, d, awh_bias->grid->point[domain_imax].value[d]));
        ...
        for (int id = 0; id < ndomains; id++)
        {
            ...
            for (int d = 0; d < awh_bias->ndim; d++)
            {
                ...
                fprintf(stderr, "[%.4g, %.4g]",
                        scaleInternalToUserInput(awh_bias_params, d, awh_bias->grid->point[coord_imin].value[d]),
                        scaleInternalToUserInput(awh_bias_params, d, awh_bias->grid->point[coord_imax].value[d]));
...
static void read_user_pmf_and_target(awh_bias_t *awh_bias, int nbias, const awh_bias_params_t *awh_bias_params)
{
    ...
    for (int d = 0; d < awh_bias->ndim; d++)
    {
        double scalingFactor = scaleUserInputToInternal(awh_bias_params, d, 1);
        if (scalingFactor == 1)
        {
            continue;
        }
                             ...
                             int nbias, int nsharing_sims, const gmx_multisim_t *ms)
{
    ...
    {
        read_user_pmf_and_target(awh_bias, nbias, awh_bias_params);
                          ...
                          awh_bias_params_t          *awh_bias_params)
{
    ...
    for (int d = 0; d < awh_bias->ndim; d++)
    {
        ...
        grid_origin[d]                   = scaleUserInputToInternal(awh_bias_params, d, awh_bias_params->dim_params[d].origin);
        grid_end[d]                      = scaleUserInputToInternal(awh_bias_params, d, awh_bias_params->dim_params[d].end);
        grid_period[d]                   = scaleUserInputToInternal(awh_bias_params, d, awh_bias_params->dim_params[d].period);
        coord_value_init[d]              = scaleUserInputToInternal(awh_bias_params, d, awh_bias_params->dim_params[d].coord_value_init);
    }
