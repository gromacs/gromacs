                                        ...
                                        FILE                    *fplog)
{
    ...
    int         my_id = getDomainId(awhBiasParams, grid, bias.state.coordValueIndex);
    ...
    if (fplog != NULL)
    {
        ...
        {
            fprintf(fplog, "[%g, %g]",
                    bias.dimParams[d].scaleInternalToUserInput(grid.point(domain_imin).value[d]),
                    bias.dimParams[d].scaleInternalToUserInput(grid.point(domain_imax).value[d]));
            if (d < bias.ndim - 1)
            {
                fprintf(fplog, ", ");
            }
    ...
    else
    {
        ...
        for (int id = 0; id < numDomains; id++)
        {
            ...
            for (int d = 0; d < bias.ndim; d++)
            {
                int coord_imin = static_cast<int>(std::floor(1.0*id_dim[d]/ninterval_dim[d]*(bias.coordPoint.size() - 1)));
                int coord_imax = static_cast<int>(std::floor(1.0*(id_dim[d] + 1)/ninterval_dim[d]*(bias.coordPoint.size() - 1)));

                fprintf(stderr, "[%.4g, %.4g]",
                        bias.dimParams[d].scaleInternalToUserInput(grid.point(coord_imin).value[d]),
                        bias.dimParams[d].scaleInternalToUserInput(grid.point(coord_imax).value[d]));
 ...
 */
static void partitionDomain(Bias *bias, const awh_bias_params_t &awhBiasParams, int point_index)
{
    int                domain_imin, domain_imax;
    ...
    getDomainBoundaryForPoint(awhBiasParams, *bias->grid.get(), point_index, &domain_imin, &domain_imax);

    /* Convert linear index to multidimensional index */
    for (int d = 0; d < bias->grid->ndim(); d++)
    {
        npoints_dim[d] = bias->grid->axis(d).npoints;
    }
    linear_array_index_to_multidim(domain_imin, bias->grid->ndim(), npoints_dim, domain_imin_dim);
    linear_array_index_to_multidim(domain_imax, bias->grid->ndim(), npoints_dim, domain_imax_dim);

    double targetSum = 0.;
    for (size_t m = 0; m < bias->coordPoint.size(); m++)
    {
        CoordPoint &coordPoint = bias->coordPoint[m];
        for (int d = 0; d < bias->ndim; d++)
        {
            int index_d = bias->grid->point(m).index[d];
 ...
 */
static double getInitialHistSizeEstimate(const DimParams *dimParams, const awh_bias_params_t &awhBiasParams,
 ...
 */
static void setFreeEnergyToConvolvedPmf(Bias *bias, const gmx_multisim_t *ms)
{
    ...
    for (size_t m = 0; m < bias->coordPoint.size(); m++)
    {
        bias->coordPoint[m].free_energy = convolvedPmf[m];
    }
}
 ...
 */
static void readUserPmfAndTargetDistribution(Bias *bias, int nbias)
{
    ...
    map_grid_to_datagrid(&grid2data_index, data, nrows, filename, bias->grid.get(), correctFormatMessage);
    ...
    for (size_t m = 0; m < bias->coordPoint.size(); m++)
    {
        ...
        bias->coordPoint[m].log_pmfsum = -data[colindex_pmf][grid2data_index[m]];
        ...
        bias->coordPoint[m].target_constant_weight = target;
    }
 ...
 */
static void normalizePmf(Bias *bias, int numSharingSims)
{
    ...
    double numSamples = bias->state.histSize/numSharingSims;
    ...
    for (auto &coordPoint : bias->coordPoint)
    {
        if (coordPoint.inTargetRegion())
        {
            coordPoint.log_pmfsum += logRenorm;
        }
    }
}
 ...
 * \param[in] ms              Struct for multi-simulation communication.
 */
static void initCoordPoints(Bias *bias, const awh_bias_params_t &awhBiasParams,
                            int nbias, int numSharingSims, const gmx_multisim_t *ms)
{
    ...
    {
        readUserPmfAndTargetDistribution(bias, nbias);
        setFreeEnergyToConvolvedPmf(bias, ms);
    }

    /* We need to borrow update functions to initialize the target and bias.
       Note that free_energy needs to be set here. */
    initTargetAndBias(bias);

    /* Set the initial reference weighthistogram. */
    for (auto &coordPoint : bias->coordPoint)
    {
        coordPoint.weightsum_ref = bias->state.histSize*coordPoint.target;
    }
    ...
    normalizePmf(bias, numSharingSims);
}
 ...
 * \param[in] awhBiasParams  Bias parameters.
 */
static void initBias(FILE                     *fplog,
                     const t_inputrec         *ir,
                     const t_commrec          *cr,
                     int                       biasIndex,
                     int                       nbias,
                     ...
                     const awh_bias_params_t  &awhBiasParams)
{
    ...
    bias->biasIndex                  = biasIndex;

    BiasParams          *params      = &bias->params;
    ...
    params->eTarget = awhBiasParams.eTarget;

    switch (params->eTarget)
    {
        case eawhtargetCUTOFF:
            params->target_param = awhBiasParams.targetCutoff;
            break;
        case eawhtargetBOLTZMANN:
        case eawhtargetLOCALBOLTZMANN:
            params->target_param = awhBiasParams.targetBetaScaling;
            break;
        default:
    ...
    bias->ndim              = awhBiasParams.ndim;
    params->numSharedUpdate = (awhBiasParams.bShare && (cr != NULL) && MULTISIM(cr) ? cr->ms->nsim : 1);
    ...
    for (int d = 0; d < bias->ndim; d++)
    {
        DimParams    *dimParams             = &bias->dimParams[d];

        dimParams->pullCoordIndex           = awhBiasParams.dim_params[d].pull_coord_index;
    ...
    for (int d = 0; d < bias->ndim; d++)
    {
        const DimParams &dimParams = bias->dimParams[d];
        grid_origin[d]             = dimParams.scaleUserInputToInternal(awhBiasParams.dim_params[d].origin);
        grid_end[d]                = dimParams.scaleUserInputToInternal(awhBiasParams.dim_params[d].end);
        grid_period[d]             = dimParams.scaleUserInputToInternal(awhBiasParams.dim_params[d].period);
        coordValueInit[d]          = dimParams.scaleUserInputToInternal(awhBiasParams.dim_params[d].coord_value_init);
    }
    ...
    state->in_initial            = awhBiasParams.eGrowth == eawhgrowthEXP_LINEAR;
    state->equilibrateHistogram  = awhBiasParams.equilibrateHistogram;
    ...
    state->coordValueIndex = get_closest_index_in_grid(coordValueInit, bias->grid.get());
    state->refCoordpoint   = state->coordValueIndex;

    /* The minimum and maximum multidimensional point indices that are affected by the next update */
    for (int d = 0; d < bias->ndim; d++)
    {
        int index_d                 = bias->grid->point(state->coordValueIndex).index[d];
        state->origin_updatelist[d] = index_d;
        state->end_updatelist[d]    = index_d;
    }

    /* Initialize free energy functions and biases */
    bias->coordPoint.resize(bias->grid->numPoints());
    bias->updateList.resize(bias->grid->numPoints());
    initCoordPoints(bias, awhBiasParams, nbias, params->numSharedUpdate, (cr != NULL ? cr->ms : NULL));
    ...
    if (getNumDomains(awhBiasParams) > 1)
    {
        ...
        partitionDomain(bias, awhBiasParams, state->coordValueIndex);
    }
    ...
    {
        printInitializationToLog(*bias, awhBiasParams, fplog);
    }
}
...
/* Constructor for an AWH working struct for mdrun. */
AwhBiasCollection::AwhBiasCollection(FILE                *fplog,
                                     const t_inputrec    *ir,
                                     const t_commrec     *cr,
                                     ...
                                     bool                 startingFromCheckpoint)
{
    ...
    bias_.resize(awh_params->nbias);
    convolveForce_   = (awh_params->ePotential == eawhpotentialCONVOLVED);
    seed_            = awh_params->seed;
    potentialOffset_ = 0;

    for (int k = 0; k < awh_params->nbias; k++)
    {
        /* Some things are common for all biases */
        bias_[k].params.nstsample_coord       = nstsample_coord;
        bias_[k].params.nstupdate_free_energy = nstupdate_free_energy;

        initBias(fplog, ir, cr, k, awh_params->nbias, &bias_[k], awh_params->awh_bias_params[k]);
    }
    ...
    {
        GMX_RELEASE_ASSERT(!MASTER(cr) || *awhHistoryPtr != nullptr, "The master rank should have the history when starting from checkpoint.");

        restoreStateFromHistory(**awhHistoryPtr, cr);
    }
    else if (MASTER(cr))
    {
        *awhHistoryPtr = new AwhHistory();
        initAwhHistoryFromState(*awhHistoryPtr, this);
    }
}
