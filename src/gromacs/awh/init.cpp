                                        ...
                                        FILE                    *fplog)
{
    ...
    int my_id = get_domain_id(awh_bias_params, awh_bias->grid.get(), awh_bias->coord_value_index);
                             ...
                             int nbias, int nsharing_sims, const gmx_multisim_t *ms)
{
    ...
    for (auto &coordPoint : awh_bias->coordPoint)
    {
        coordPoint.weightsum_ref = awh_bias->histsize*coordPoint.target;
    }
                          ...
                          awh_bias_params_t          *awh_bias_params)
{
    ...
    awh_bias->equilibrateHistogram      = awh_bias_params->equilibrateHistogram;
    ...
    awh_bias->grid        = std::unique_ptr<Grid>(init_grid(awh_bias->ndim, awh_bias->betak, grid_origin, grid_end, grid_period, &nneighbors_alloc));
    ...
    awh_bias->histsize_initial = get_initial_histsize_estimate(awh_bias->betak, awh_bias_params, awh_bias->grid.get(), awh_bias->nstsample_coord*ir->delta_t);
    ...
    awh_bias->coord_value_index    = get_closest_index_in_grid(coord_value_init, awh_bias->grid.get());
                             ...
                             struct pull_t            *pull_work)
{
    for (auto &awhBias : awh->awhBias)
    {
        ...
        {
            register_external_pull_potential(pull_work, awhBias.pull_coord_index[d], external_potential_string);
        }
    }
}
                            ...
                            const awh_params_t      *awh_params)
{
    ...
    for (int k = 0; k < awh_params->nbias; k++)
    {
        ...
        awh->awhBias[k].nstsample_coord          = nstsample_coord;
        awh->awhBias[k].nstupdate_free_energy    = nstupdate_free_energy;
