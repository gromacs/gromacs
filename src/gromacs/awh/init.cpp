...
double scaleInternalToUserInput(const pull_params_t *pull_params, int pullCoordIndex, double value)
{
    double scaleFactor = pull_coordinate_is_angletype(&pull_params->coord[pullCoordIndex]) ? RAD2DEG : 1;
    return value*scaleFactor;
}

double scaleUserInputToInternal(const pull_params_t *pull_params, int pullCoordIndex, double value)
{
    double scaleFactor = pull_coordinate_is_angletype(&pull_params->coord[pullCoordIndex]) ? DEG2RAD : 1;
    return value*scaleFactor;
}
...
static void read_user_pmf_and_target(awh_bias_t *awh_bias, int nbias, const pull_params_t *pull_params)
{
    ...
    for (int d = 0; d < awh_bias->ndim; d++)
    {
        double scalingFactor = scaleUserInputToInternal(pull_params, awh_bias->pull_coord_index[d], 1);
        if (scalingFactor == 1)
        {
            continue;
        }
                             ...
                             const pull_params_t *pull_params)
{
    ...
    {
        read_user_pmf_and_target(awh_bias, nbias, pull_params);
                          ...
                          awh_bias_params_t          *awh_bias_params)
{
    ...
    for (int d = 0; d < awh_bias->ndim; d++)
    {
        ...
        grid_origin[d]                   = scaleUserInputToInternal(pull_params, awh_bias->pull_coord_index[d], awh_bias_params->dim_params[d].origin);
        grid_end[d]                      = scaleUserInputToInternal(pull_params, awh_bias->pull_coord_index[d], awh_bias_params->dim_params[d].end);
        grid_period[d]                   = scaleUserInputToInternal(pull_params, awh_bias->pull_coord_index[d], awh_bias_params->dim_params[d].period);
        coord_value_init[d]              = scaleUserInputToInternal(pull_params, awh_bias->pull_coord_index[d], awh_bias_params->dim_params[d].coord_value_init);
    }
     ...
     */
    awh_bias->weight_scaling            = awh_bias->eTarget == eawhtargetLOCALBOLTZMANN ? awh_bias->target_param : 1;
