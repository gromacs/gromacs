           ...
           const awh_bias_params_t  &awhBiasParams)
{
    ...
    params.nstsample_coord       = awhParams.nstsample_coord;
    params.nstupdate_free_energy = awhParams.nsamples_update_free_energy*awhParams.nstsample_coord;
    ...
    {
        dimParams[d].pullCoordIndex           = awhBiasParams.dim_params[d].pull_coord_index;

        const t_pull_coord *pullCoord         = &pull_params->coord[dimParams[d].pullCoordIndex];

        dimParams[d].userCoordUnitsToInternal = pull_coordinate_is_angletype(pullCoord) ? DEG2RAD : 1;
        dimParams[d].k                        = pullCoord->k;
        dimParams[d].betak                    = beta*dimParams[d].k;
    }

    for (int d = 0; d < ndim; d++)
    {
        grid_origin[d]             = dimParams[d].scaleUserInputToInternal(awhBiasParams.dim_params[d].origin);
        grid_end[d]                = dimParams[d].scaleUserInputToInternal(awhBiasParams.dim_params[d].end);
        grid_period[d]             = dimParams[d].scaleUserInputToInternal(awhBiasParams.dim_params[d].period);
    ...
    grid = std::unique_ptr<Grid>(new Grid(ndim, dimParams, grid_origin, grid_end, grid_period));

    /* Estimate and initialize histSizeInitial. The estimation depends on the grid. */
    params.histSizeInitial = getInitialHistSizeEstimate(dimParams, awhBiasParams, grid.get(), params.nstsample_coord*ir->delta_t);
    ...
    state.gridpointIndex = get_closest_index_in_grid(coordValueInit, grid.get());
    ...
    for (int d = 0; d < ndim; d++)
    {
        int index_d                = grid->point(state.gridpointIndex).index[d];
        state.origin_updatelist[d] = index_d;
        state.end_updatelist[d]    = index_d;
    }

    /* Initialize free energy functions and biases */
    pointState.resize(grid->numPoints());
    updateList.resize(grid->numPoints());
    initGridPointState(this, awhBiasParams, awhParams.nbias, params.numSharedUpdate, (cr != NULL ? cr->ms : NULL));
    ...
    if (getNumDomains(awhBiasParams) > 1)
    {
        ...
        partitionDomain(this, awhBiasParams, state.gridpointIndex);
    }
    ...
    for (int d = 0; d < ndim; d++)
    {
        ...
        double spacing         = grid->axis(d).spacing;
