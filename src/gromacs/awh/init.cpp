 ...
 * \returns domain id.
 */
static int get_domain_id(const awh_bias_params_t *awh_bias_params, const Grid *grid, int point)
{
 ...
 */
static void get_domain_boundary(const awh_bias_params_t *awh_bias_params, const Grid *grid,
                                int domain_id,
 ...
 * \param[in,out] point_max    Maximum boundary point index.
 */
static void get_domain_boundary_for_point(const awh_bias_params_t *awh_bias_params, const Grid *grid, int point,
                                          int *point_min, int *point_max)
{
                                        ...
                                        FILE                    *fplog)
{
    ...
    int my_id = get_domain_id(awh_bias_params, awh_bias.grid.get(), awh_bias.state.coord_value_index);
    int domain_imin, domain_imax;

    if (fplog != NULL)
    {
        /* Print partitioning info for this simulation. */
        get_domain_boundary(awh_bias_params, awh_bias.grid.get(), my_id, &domain_imin, &domain_imax);
        ...
        {
            fprintf(fplog, "[%g, %g]",
                    awh_bias.dimParams[d].scaleInternalToUserInput(awh_bias.grid->point[domain_imin].value[d]),
                    awh_bias.dimParams[d].scaleInternalToUserInput(awh_bias.grid->point[domain_imax].value[d]));
    ...
    else
    {
        ...
        for (int id = 0; id < ndomains; id++)
        {
            ...
            get_domain_boundary(awh_bias_params, awh_bias.grid.get(), id, &domain_imin, &domain_imax);

            fprintf(stderr, "\n");
            for (int d = 0; d < awh_bias.ndim; d++)
            {
                int coord_imin = static_cast<int>(std::floor(1.0*id_dim[d]/ninterval_dim[d]*(awh_bias.coordPoint.size() - 1)));
                int coord_imax = static_cast<int>(std::floor(1.0*(id_dim[d] + 1)/ninterval_dim[d]*(awh_bias.coordPoint.size() - 1)));

                fprintf(stderr, "[%.4g, %.4g]",
                        awh_bias.dimParams[d].scaleInternalToUserInput(awh_bias.grid->point[coord_imin].value[d]),
                        awh_bias.dimParams[d].scaleInternalToUserInput(awh_bias.grid->point[coord_imax].value[d]));
 ...
 * \param[in,out] fplog        Log file.
 */
static void print_log_init(const AwhBias           &awh_bias,
                           const awh_bias_params_t *awh_bias_params,
                           FILE                    *fplog)
{
    char           awhstr[STRLEN];
 ...
 */
static void partition_domain(AwhBias *awh_bias, const awh_bias_params_t *awh_bias_params, int point_index)
{
    int                domain_imin, domain_imax;
    ...
    get_domain_boundary_for_point(awh_bias_params, awh_bias->grid.get(), point_index, &domain_imin, &domain_imax);
    ...
    {
        CoordPoint &coordPoint = awh_bias->coordPoint[m];
        for (int d = 0; d < awh_bias->ndim; d++)
        {
            if (awh_bias->grid->point[m].index[d] < domain_imin_dim[d] || awh_bias->grid->point[m].index[d] > domain_imax_dim[d])
            {
                coordPoint.target = 0;
                coordPoint.bias   = -GMX_DOUBLE_MAX; /* the bias = log(target) + const = -infty */
            }
        }
        target_sum += coordPoint.target;
    }

    /* Renormalize target distribution to 1 and scale down the histogram size */
    awh_bias->state.histsize          *= target_sum;
    awh_bias->params.histsize_initial  = awh_bias->state.histsize;
    inv_target_sum                     = 1./target_sum;
    for (auto &coordPoint : awh_bias->coordPoint)
    {
        coordPoint.target             *= inv_target_sum;
    }
}
 ...
 */
static double get_initial_histsize_estimate(const DimParams *dimParams, const awh_bias_params_t *awh_bias_params,
                                            const Grid *grid, double dt_sample)
{
    int      ndim_skip;
 ...
 */
static void set_free_energy_to_convolved_pmf(AwhBias *awh_bias, const gmx_multisim_t *ms)
{
 ...
 */
static void read_user_pmf_and_target(AwhBias *awh_bias, int nbias)
{
    ...
    for (int d = 0; d < awh_bias->ndim; d++)
    {
        double scalingFactor = awh_bias->dimParams[d].scaleUserInputToInternal(1);
        if (scalingFactor == 1)
        {
            continue;
        }
    ...
    grid2data_index.resize(awh_bias->grid->point.size());
    map_grid_to_datagrid(&grid2data_index, data, nrows, filename, awh_bias->grid.get(), correctFormatMessage);
 ...
 */
static void normalize_pmf(AwhBias *awh_bias,  int nsharing_sims)
{
    ...
    for (auto &coordPoint : awh_bias->coordPoint)
    {
        ...
        {
            expsum_pmf += std::exp(coordPoint.log_pmfsum);
            expsum_f   += std::exp(-coordPoint.free_energy);
        }
    }
    nsamples = awh_bias->state.histsize/nsharing_sims;
    ...
    for (auto &coordPoint : awh_bias->coordPoint)
    {
        if (coordPoint.inTargetRegion())
        {
            coordPoint.log_pmfsum += log_renorm;
        }
    }
}
 ...
 * \param[in] ms               Struct for multi-simulation communication.
 */
static void init_coordpoints(AwhBias *awh_bias, const awh_bias_params_t *awh_bias_params,
                             int nbias, int nsharing_sims, const gmx_multisim_t *ms)
{
    ...
    {
        coordPoint.bias                                    = 0;
        coordPoint.free_energy                             = 0;

        coordPoint.target                                  = 1;
        coordPoint.target_constant_weight                  = 1;

        coordPoint.weightsum_iteration                     = 0;
        coordPoint.weightsum_covering                      = 0;
        coordPoint.weightsum_tot                           = 0;
        coordPoint.weightsum_ref                           = 1;

        coordPoint.last_update_index                       = 0;

        coordPoint.log_pmfsum                              = 0;

        coordPoint.visits_iteration                        = 0;
        coordPoint.visits_tot                              = 0;
    }
    ...
    for (auto &coordPoint : awh_bias->coordPoint)
    {
        coordPoint.weightsum_ref = awh_bias->state.histsize*coordPoint.target;
    }
                          ...
                          awh_bias_params_t          *awh_bias_params)
{
    ...
    BiasParams          *params      = &awh_bias->params;
    ...
    params->eTarget         = awh_bias_params->eTarget;

    switch (params->eTarget)
    {
        case eawhtargetCUTOFF:
            params->target_param    = awh_bias_params->targetCutoff;
            break;
        case eawhtargetBOLTZMANN:
        case eawhtargetLOCALBOLTZMANN:
            params->target_param    = awh_bias_params->targetBetaScaling;
            break;
        default:
            params->target_param    = 0;
            break;
    }

    awh_bias->ndim          = awh_bias_params->ndim;
    params->numSharedUpdate = (awh_bias_params->bShare && (cr != NULL) && MULTISIM(cr) ? cr->ms->nsim : 1);
    params->update_weight   = params->nstupdate_free_energy/params->nstsample_coord*params->numSharedUpdate;
    ...
    params->invBeta = 1./beta;
    for (int d = 0; d < awh_bias->ndim; d++)
    {
        DimParams    *dimParams             = &awh_bias->dimParams[d];

        dimParams->pullCoordIndex           = awh_bias_params->dim_params[d].pull_coord_index;

        const t_pull_coord *pullCoord       = &pull_params->coord[dimParams->pullCoordIndex];

        dimParams->userCoordUnitsToInternal = pull_coordinate_is_angletype(pullCoord) ? DEG2RAD : 1;
        dimParams->k                        = pullCoord->k;
        dimParams->betak                    = beta*dimParams->k;
    }

    for (int d = 0; d < awh_bias->ndim; d++)
    {
        const DimParams &dimParams  = awh_bias->dimParams[d];
        grid_origin[d]              = dimParams.scaleUserInputToInternal(awh_bias_params->dim_params[d].origin);
        grid_end[d]                 = dimParams.scaleUserInputToInternal(awh_bias_params->dim_params[d].end);
        grid_period[d]              = dimParams.scaleUserInputToInternal(awh_bias_params->dim_params[d].period);
        coord_value_init[d]         = dimParams.scaleUserInputToInternal(awh_bias_params->dim_params[d].coord_value_init);
    }

    BiasState *state = &awh_bias->state;

    /* The initial sample weight is set to 1 and we keep the logarithm. */
    state->scaledSampleWeight    = 0;
    state->maxScaledSampleWeight = 0;

    state->in_initial            = awh_bias_params->eGrowth == eawhgrowthEXP_LINEAR;
    state->equilibrateHistogram  = awh_bias_params->equilibrateHistogram;
     ...
     */
    params->localWeightScaling      = (params->eTarget == eawhtargetLOCALBOLTZMANN ? params->target_param : 1);
    params->idealWeighthistUpdate   = (params->eTarget != eawhtargetLOCALBOLTZMANN);
    ...
    switch (params->eTarget)
    {
        ...
        case eawhtargetBOLTZMANN:
            ...
            params->nstupdate_target = std::max((100*params->nstsample_coord/params->nstupdate_free_energy)*params->nstupdate_free_energy,
        ...
        case eawhtargetLOCALBOLTZMANN:
            ...
            params->nstupdate_target = params->nstupdate_free_energy;
            break;
        default:
    ...
    awh_bias->grid        = std::unique_ptr<Grid>(init_grid(awh_bias->ndim, awh_bias->dimParams, grid_origin, grid_end, grid_period, &nneighbors_alloc));

    /* The transition probability weights in a neighborhood of the current coordinate */
    awh_bias->probWeightNeighbor.resize(nneighbors_alloc);

    /* Estimate and initialize histsize_initial. The estimation depends on the grid. */
    params->histsize_initial = get_initial_histsize_estimate(awh_bias->dimParams, awh_bias_params, awh_bias->grid.get(), params->nstsample_coord*ir->delta_t);
    state->histsize          = params->histsize_initial;
    ...
    state->coord_value_index = get_closest_index_in_grid(coord_value_init, awh_bias->grid.get());
    state->refCoordpoint     = state->coord_value_index;

    /* The minimum and maximum multidimensional point indices that are affected by the next update */
    for (int d = 0; d < awh_bias->ndim; d++)
    {
        state->origin_updatelist[d] = awh_bias->grid->point[state->coord_value_index].index[d];
        state->end_updatelist[d]    = awh_bias->grid->point[state->coord_value_index].index[d];
    }

    /* Initialize free energy functions and biases */
    awh_bias->coordPoint.resize(awh_bias->grid->point.size());
    awh_bias->updateList.resize(awh_bias->grid->point.size());
    init_coordpoints(awh_bias, awh_bias_params, nbias, params->numSharedUpdate, (cr != NULL ? cr->ms : NULL));
    ...
    if (get_ndomains(awh_bias_params) > 1)
    {
        ...
        partition_domain(awh_bias, awh_bias_params, state->coord_value_index);
    }

    /* Translate the point cover diameter fraction to a radius in units of number of points. */
    for (int d = 0; d < awh_bias->ndim; d++)
    {
        double coverRadius = 0.5*awh_bias_params->dim_params[d].coverDiameter;
...
void register_bias_with_pull(const AwhBiasCollection  *awh,
                             struct pull_t            *pull_work)
{
    for (auto &awhBias : awh->awhBias)
    {
        ...
        {
            register_external_pull_potential(pull_work, awhBias.dimParams[d].pullCoordIndex, external_potential_string);
        }
    }
}
...
{
    AwhBiasCollection *awh                   = new AwhBiasCollection;
    ...
    awh->awhBias.resize(awh_params->nbias);
    ...
    for (int k = 0; k < awh_params->nbias; k++)
    {
        ...
        awh->awhBias[k].params.nstsample_coord       = nstsample_coord;
        awh->awhBias[k].params.nstupdate_free_energy = nstupdate_free_energy;

        init_awh_bias(fplog, ir, cr, k, awh_params->nbias, &awh->awhBias[k], &awh_params->awh_bias_params[k]);
    }
...
{
    AwhBiasCollection *awh = init_awh(fplog, ir, cr, awh_params);
    ...
    if (*awh_history_ptr == nullptr)
    {
        snew(*awh_history_ptr, 1);
    }
    awh_history_t *awh_history = *awh_history_ptr;
