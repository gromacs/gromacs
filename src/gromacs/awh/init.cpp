...
static void partitionDomain(Bias *bias, const awh_bias_params_t &awhBiasParams, int point_index)
{
    ...
    {
        PointState &pointState = bias->pointState[m];
        for (int d = 0; d < bias->ndim; d++)
        {
            ...
            {
                pointState.target = 0;
                pointState.setBiasToMinusInfinity(); /* the bias = log(target) + const = -infty */
            }
        }
    ...
    {
        pointState.target        *= invTargetSum;
    }
}
...
static void setFreeEnergyToConvolvedPmf(Bias *bias, const gmx_multisim_t *ms)
{
    ...
    for (size_t m = 0; m < bias->pointState.size(); m++)
    {
        bias->pointState[m].setFreeEnergy(convolvedPmf[m]);
    }
}
...
static void readUserPmfAndTargetDistribution(Bias *bias, int nbias)
{
    ...
    for (size_t m = 0; m < bias->pointState.size(); m++)
    {
        ...
        bias->pointState[m].setLogPmfsum(-data[colindex_pmf][grid2data_index[m]]);
        ...
        bias->pointState[m].target_constant_weight = target;
    }
...
static void normalizePmf(Bias *bias, int numSharingSims)
{
    ...
    for (auto &pointState : bias->pointState)
    {
        ...
        {
            expsum_pmf += std::exp(pointState.logPmfsum());
            expsum_f   += std::exp(-pointState.freeEnergy());
        }
    }
 ...
 * These may later be modified because of user input data, normalization,
 * target type, etc. Note that later initializations may depend on
 * the values being set here.
 */
PointState::PointState() : bias_(0),
                           freeEnergy_(0),

                           target(1),
                           target_constant_weight(1),
...
void PointState::setInitialReferenceWeightHistogram(const BiasState &biasState)
{
    weightsumRef_ = biasState.histSize*target;
}
 ...
 * \param[in] ms              Struct for multi-simulation communication.
 */
static void initGridPointState(Bias *bias, const awh_bias_params_t &awhBiasParams,
                               int nbias, int numSharingSims, const gmx_multisim_t *ms)
{
    ...
    for (auto &pointState : bias->pointState)
    {
        pointState.setInitialReferenceWeightHistogram(bias->state);
    }
                     ...
                     const awh_bias_params_t  &awhBiasParams)
{
    ...
    state->gridpointIndex = get_closest_index_in_grid(coordValueInit, bias->grid.get());
    state->refGridpoint   = state->gridpointIndex;

    /* The minimum and maximum multidimensional point indices that are affected by the next update */
    for (int d = 0; d < bias->ndim; d++)
    {
        int index_d                 = bias->grid->point(state->gridpointIndex).index[d];
        state->origin_updatelist[d] = index_d;
        state->end_updatelist[d]    = index_d;
    }

    /* Initialize free energy functions and biases */
    bias->pointState.resize(bias->grid->numPoints());
    bias->updateList.resize(bias->grid->numPoints());
    initGridPointState(bias, awhBiasParams, nbias, params->numSharedUpdate, (cr != NULL ? cr->ms : NULL));
    ...
    if (getNumDomains(awhBiasParams) > 1)
    {
        ...
        partitionDomain(bias, awhBiasParams, state->gridpointIndex);
    }
