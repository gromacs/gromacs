                       ...
                       const std::vector<GridAxis>  &gridAxis)
{
    ...
    histSizeInitial = getInitialHistSizeEstimate(dimParams, awhBiasParams, gridAxis, nstsample_coord*mdTimeStep);
}

DimParams::DimParams(int                 pullCoordIndexInit,
                     const t_pull_coord &pullCoord,
                     double              beta) :
    k(pullCoord.k),
    betak(beta*pullCoord.k),
    pullCoordIndex(pullCoordIndexInit),
...
Bias::Bias(FILE                         *fplog,
    ...
    params_(awhParams, awhBiasParams, dimParams_, beta, mdTimeStep, cr, grid_->axis_),
    tempWorkSpace_()
{
