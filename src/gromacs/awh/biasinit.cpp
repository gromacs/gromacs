                                         ...
                                         const std::vector<GridAxis> &gridAxis, double dt_sample)
{
    ...
    {
        double L = get_gridaxis_length(gridAxis[d]);
        if (L > 0)
        {
                                             ...
                                             std::vector<PointState> *pointState)
{
    ...
    map_grid_to_datagrid(&grid2data_index, data, nrows, filename, bias.grid(), correctFormatMessage);
    ...
    for (size_t m = 0; m < pointState->size(); m++)
    {
        ...
        (*pointState)[m].target_constant_weight = target;
    }
                       ...
                       const std::vector<GridAxis>  &gridAxis)
{
    ...
    nstsample_coord       = awhParams.nstsample_coord;
    nstupdate_free_energy = awhParams.nsamples_update_free_energy*awhParams.nstsample_coord;
    ...
    update_weight    = nstupdate_free_energy/nstsample_coord*numSharedUpdate;
    ...
    switch (eTarget)
    {
        ...
        case eawhtargetBOLTZMANN:
            ...
            nstupdate_target = std::max((100*nstsample_coord/nstupdate_free_energy)*nstupdate_free_energy,
                                        nstupdate_free_energy);
            break;
        case eawhtargetLOCALBOLTZMANN:
            ...
            nstupdate_target = nstupdate_free_energy;
            break;
        default:
...
DimParams::DimParams(const awh_dim_params_t &awhDimParams,
                     const pull_params_t    &pullParams,
                     double                  beta)
{
    pullCoordIndex                = awhDimParams.pull_coord_index;

    const t_pull_coord &pullCoord = pullParams.coord[pullCoordIndex];

    userCoordUnitsToInternal      = pull_coordinate_is_angletype(&pullCoord) ? DEG2RAD : 1;
    k                             = pullCoord.k;
    betak                         = beta*pullCoord.k;
}
...
Bias::Bias(FILE                         *fplog,
    ...
    params_(awhParams, awhBiasParams, dimParams_, beta, ir->delta_t, cr, grid_->axis_),
    tempWorkSpace_()
{
    ...
    state_.gridpointIndex = get_closest_index_in_grid(coordValueInit, grid_.get());
