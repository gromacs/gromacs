...
/* The local Boltzmann distribution is special because the target distribution is updated as a function of the reference weighthistogram. */
    ...
    const double histogramSize = bias.state().histSize;
    for (auto &ps : *pointState)
    {
        ps.setInitialReferenceWeightHistogram(histogramSize);
    }
...
Bias::Bias(FILE                          *fplog,
    ...
    params_(awhParams, awhBiasParams, dimParams_, beta, mdTimeStep, disableUpdateSkips, cr, grid_->axis()),
    tempWorkSpace_(),
    numWarningsIssued_(0)
{
    ...
    state_.gridpointIndex = grid_->nearestIndex(coordValueInit);
    ...
    state_.points.resize(grid_->numPoints());
