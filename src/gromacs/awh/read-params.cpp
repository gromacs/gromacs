...
const char *eawhgrowth_names[eawhgrowthNR+1] = {
    "exp-linear", "linear", "none", nullptr
};
                            ...
                            warninp_t wi, bool bComment)
{
    ...
    /* The conversion factor from internal to external units is inherited from the pull coordinate. */
    dim_params->userCoordUnitsToInternal = pull_coordinate_is_angletype(&pull_params->coord[icoord]) ? DEG2RAD : 1;
                             ...
                             const t_inputrec *ir, warninp_t wi, bool bComment)
{
    ...
    if (bComment)
    {
        CTYPE("Estimated initial PMF error (kT)");
    }
    ...
    if (bComment)
    {
        CTYPE("Growth rate of the reference histogram determining the bias update size: exp-linear, none, or linear");
    }
    ...
    if (awh_bias_params->ndim <= 0 ||
        awh_bias_params->ndim > c_awhBiasMaxNumDim)
    {
        gmx_fatal(FARGS, "%s-ndim (%d) needs to be > 0 and at most %d\n", prefix,  awh_bias_params->ndim, c_awhBiasMaxNumDim);
    }
...
double scaleInternalToUserInput(const awh_bias_params_t *awh_bias_params, int dimIndex, double value)
{
    return value/awh_bias_params->dim_params[dimIndex].userCoordUnitsToInternal;
}

double scaleUserInputToInternal(const awh_bias_params_t *awh_bias_params, int dimIndex, double value)
{
    return value*awh_bias_params->dim_params[dimIndex].userCoordUnitsToInternal;
}
                                    ...
                                    const t_grpopts *inputrec_group_options, warninp_t wi)
{
    ...
    for (int k = 0; k < awh_params->nbias; k++)
    {
        ...
        for (int d = 0; d < awh_bias_params->ndim; d++)
        {
            ...
            dim_params->coord_value_init = scaleInternalToUserInput(awh_bias_params, d, dim_params->coord_value_init);
        }
    }
