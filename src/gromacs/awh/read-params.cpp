...
awh_params_t *read_awh_params(int *ninp_p, t_inpfile **inp_p, const t_inputrec *ir, warninp_t wi)
{
    ...
    if (ir->nstenergy == 0 || awh_params->nstout % ir->nstenergy != 0)
    {
        char buf[STRLEN];
...
/* Sets AWH parameters that need state parameters such as the box vectors and the initial coordinates. */
                                    ...
                                    const t_grpopts *inputrec_group_options, warninp_t wi)
{
    ...
    AwhBiasCollection *awh = init_awh(NULL, inputrec, NULL, awh_params);
