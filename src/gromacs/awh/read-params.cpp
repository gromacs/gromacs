                                     ...
                                     warninp_t        wi)
{
    ...
    {
        warning_error(wi, "Sharing of biases over multiple simulations is requested, but no bias is marked as shared (share-group > 0)");
    }
}
    ...
    for (int k = 0; k < awhParams->numBias; k++)
    {
        ...
        for (int d = 0; d < awhBiasParams->ndim; d++)
        {
            ...
            dimParams->coordValueInit =
                get_pull_coord_value(pull_work, dimParams->pullCoordIndex, &pbc);
