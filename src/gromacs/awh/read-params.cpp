...
static void read_awhdim(int *ninp_p, t_inpfile **inp_p, const char *prefix,
                        awhdim_params_t *dim_params, const pull_params_t *pull_params,
...
static void check_input_consistency_awh(const awh_params_t *awh_params, warninp_t wi)
{
    ...
    {
        if (awh_params->dim_params[d].ninterval > 1)
        {
            if (awh_params->bShare)
            {
                gmx_fatal(FARGS, "Partitioning the AWH dimension %d into %d intervals is not compatible "
                          ...
                          d + 1, awh_params->dim_params[d].ninterval);
            }
            else if (awh_params->dim_params[d].interval_overlap == 1)
            {
        ...
        else if ((awh_params->dim_params[d].ninterval == 1) && (awh_params->dim_params[d].interval_overlap < 1))
        {
...
static void read_awhparams(int *ninp_p, t_inpfile **inp_p, awh_params_t *awh_params, const char *prefix,
                           const t_inputrec *ir, warninp_t wi, bool bComment)
{
    ...
    EETYPE(opt, awh_params->eTarget, eawhtarget_names);
    ...
    RTYPE(opt, awh_params->target_param, 0);

    switch (awh_params->eTarget)
    {
        case eawhtargetCONSTANT:
            if (awh_params->target_param != 0)
            {
                sprintf(warningmsg, "%s was set explicitly but will not be used for target type %s.",
                        opt, EAWHTARGET(awh_params->eTarget));
                warning(wi, warningmsg);
            }
            break;
        case eawhtargetCUTOFF:
            if (awh_params->target_param <= 0)
            {
                gmx_fatal(FARGS, "%s = %g is not useful for target type %s.",
                          opt, awh_params->target_param, EAWHTARGET(awh_params->eTarget));
            }
            break;
        case eawhtargetBOLTZMANN:
        case eawhtargetWEIGHTHIST:
            if (awh_params->target_param < 0 || awh_params->target_param > 1)
            {
                gmx_fatal(FARGS, "%s = %g is not useful for target type %s.",
                          opt, awh_params->target_param, EAWHTARGET(awh_params->eTarget));
            }
    ...
    EETYPE(opt, awh_params->eGrowth, eawhgrowth_names);
    ...
    EETYPE(opt, awh_params->bUser_data, yesno_names);
    ...
    RTYPE(opt, awh_params->error_initial, 4);
    ...
    ITYPE(opt, awh_params->ndim, 0);

    if (awh_params->ndim <= 0)
    {
        gmx_fatal(FARGS, "%s-ndim (%d) should be > 0\n", prefix,  awh_params->ndim);
    }
    if (awh_params->ndim > 2)
    {
    ...
    EETYPE(opt, awh_params->bShare, yesno_names);

    snew(awh_params->dim_params, awh_params->ndim);
    for (int d = 0; d < awh_params->ndim; d++)
    {
        bComment = bComment && d == 0;
        sprintf(prefixdim, "%s-dim%d", prefix, d + 1);
        read_awhdim(&ninp, &inp, prefixdim, &awh_params->dim_params[d], ir->pull, wi, bComment);
    }

    /* Check consistencies here that cannot be checked at read time at a lower level. */
    check_input_consistency_awh(awh_params, wi);

    *ninp_p   = ninp;
    *inp_p    = inp;
}

static void check_input_consistency_awhbias(const awhbias_params_t *awhbias_params)
{
    /* Each pull coord can map to at most 1 AWH coord */
    for (int k1 = 0; k1 < awhbias_params->nawh; k1++)
    {
        awh_params_t *awh_params1 = &awhbias_params->awh_params[k1];

        /* k1 is the reference AWH, k2 is the AWH we compare with (can be equal to k1) */
        for (int k2 = k1; k2 < awhbias_params->nawh; k2++)
        {
            for (int d1 = 0; d1 < awh_params1->ndim; d1++)
            {
                awh_params_t *awh_params2 = &awhbias_params->awh_params[k2];

                /* d1 is the reference dimension of the reference AWH. d2 is the dim index of the AWH to compare with. */
                for (int d2 = 0; d2 < awh_params2->ndim; d2++)
                {
                    /* Give an error if (d1, k1) is different from (d2, k2) but the pull coordinate is the same */
                    if ( (d1 != d2 || k1 != k2) && (awh_params1->dim_params[d1].pull_coord_index == awh_params2->dim_params[d2].pull_coord_index) )
                    {
                        ...
                        sprintf(errormsg, "One pull coordinate (%d) cannot be mapped to two separate AWH dimensions (awh%d-dim%d and awh%d-dim%d). "
                                ...
                                awh_params1->dim_params[d1].pull_coord_index + 1, k1 + 1, d1 + 1, k2 + 1, d2 + 1);
...
awhbias_params_t *read_awhbiasparams(int *ninp_p, t_inpfile **inp_p, const t_inputrec *ir, warninp_t wi)
{
    int               ninp;
    t_inpfile        *inp;
    char              opt[STRLEN], prefix[STRLEN], prefixawh[STRLEN];

    awhbias_params_t *awhbias_params;
    snew(awhbias_params, 1);
    ...
    ITYPE(opt, awhbias_params->nawh, 1);
    ...
    ITYPE(opt, awhbias_params->seed, -1);
    if (awhbias_params->seed == -1)
    {
    ...
    ITYPE(opt, awhbias_params->nstsample_coord, 10);

    CTYPE("Number of samples per moving the coordinate reference value");
    sprintf(opt, "%s-nsamples-refvalue", prefix);
    ITYPE(opt, awhbias_params->nsamples_move_refvalue, 5);

    CTYPE("Number of coordinate samples per free energy update and bias update");
    sprintf(opt, "%s-nsamples-update", prefix);
    ITYPE(opt, awhbias_params->nsamples_update_free_energy, 5);

    CTYPE("Convolve the bias force: yes or no");
    sprintf(opt, "%s-convolve-force", prefix);
    EETYPE(opt, awhbias_params->bConvolve_force, yesno_names);

    CTYPE("Monitor and output AWH bias force correlation");
    sprintf(opt, "%s-forcecorr", prefix);
    EETYPE(opt, awhbias_params->bForce_correlation, yesno_names);

    /* Read the parameters specific to each AWH bias */
    for (int k = 0; k < awhbias_params->nawh; k++)
    {
        bool bComment = (k == 0);
        sprintf(prefixawh, "%s%d", prefix, k + 1);
        read_awhparams(&ninp, &inp, &awhbias_params->awh_params[k], prefixawh, ir, wi, bComment);
    }

    /* Do a final consistency check before returning */
    check_input_consistency_awhbias(awhbias_params);

    *ninp_p   = ninp;
    *inp_p    = inp;

    return awhbias_params;
}
...
static void check_input_consistency_awhinterval(const awhbias_params_t *awhbias_params, warninp_t wi)
{
    for (int k = 0; k < awhbias_params->nawh; k++)
    {
        awh_params_t *awhparams = &awhbias_params->awh_params[k];
        for (int d = 0; d < awhparams->ndim; d++)
        {
            awhdim_params_t *dim_params            = &awhparams->dim_params[d];
...
/* Some AWH parameters are dependent on state parameters, e.g. the initial configuration which we don't have at the time we read the
   ...
   before earlier. */
void set_state_dependent_awhbias_params(awhbias_params_t *awhbias_params,
...
{
    /* The temperature is not really state depenendent but is not know when read_awhbiasparams is called (in get ir).
