...
//! Struct for organizing coordinate exchange for biased simulations.
...
/*! \brief
 * Query if two replicas are equivalent.
 *
 * \param[in] replex          Exchange struct.
 * \param[in] replica_id1     Id of first replica.
 * \param[in] replica_id2     Id of second replica.
 * \returns true if the replicas are equivalent.
 */
...
/*  Query if two replicas are up for exchanging. */
 ...
 *
 * \param[in] a    Void pointer to first integer
 * \param[in] b    Void pointer to second integer.
 * \returns (a - b)
 */
...
/*! \brief
 * Allocate and initialize an arrray with the the domain ids of all the simulations.
 *
 ...
 * \returns array with domain ids.
 */
static int *init_domain_id(const awh_replex_t *replex, const awh_bias_t *awh_bias, const gmx_multisim_t *ms)
{
    int      start              = -1;
    gmx_bool bFound_first_point = FALSE;

    for (int m = 0; m < awh_bias->npoints && !bFound_first_point; m++)
    {
        bFound_first_point = (awh_bias->coordpoint[m].target > 0);
        start              = m;
    }
...
/*! \brief
 *  Allocate and initialize bias replica exchange.
 *
 * \param[in] awh_bias               AWH bias.
 * \param[in] ms                     Struct for multisimulation communication.
 * \param[in] all_replicas_exchange  True if all all replicas exchange (are non-equivalent).
 * \returns a bias exchange struct.
 */
static awh_replex_t *init_bias_replica_exchange(const awh_bias_t *awh_bias, const gmx_multisim_t *ms, bool all_replicas_exchange)
{
    awh_replex_t *replex;
    ...
    replex->npoints     = awh_bias->npoints;
    ...
    {
        replex->domain_id = init_domain_id(replex, awh_bias, ms);
    }
...
/* Allocate, initialize and return an AWH replica exchange struct. */
                                        ...
                                        const int *exchange_indices, int bMulti_exchange)
{
    ...
    if (awh->nbias > 1)
    {
        gmx_fatal(FARGS, "AWH bias replica exchange only supports one AWH bias per replica, but the number of AWH biases is %d.", awh->nbias);
    }
    ...
    bool all_replicas_exchange = !((awh_params->awh_bias_params[0].eTarget == eawhtargetCONSTANT) && (!awh_params->awh_bias_params[0].bUser_data));

    replex = init_bias_replica_exchange(&awh->awh_bias[0], ms, all_replicas_exchange);
    ...
    else
    {
         ...
         */
        if (awh_params->awh_bias_params[0].ndim > 1)
        {
...
/*! \brief
 *  Prepare for replica exchange by updating the bias state.
 *
 * \param[in] awh_bias               AWH bias.
 * \param[in,out] replex             Exchange struct.
 * \param[in] ms                     Struct for multisimulation communication.
 * \returns true if state was update, false if exchange should not be attempted.
 */
static gmx_bool update_awh_replica_exchange_state(const awh_bias_t *awh_bias, awh_replex_t *replex, const gmx_multisim_t *ms)
{
    int      nreplicas_ok;
    ...
    bCoord_value_in_range = value_is_in_grid(awh_bias->coord_value, awh_bias->grid);

    if (bCoord_value_in_range)
    {
        replex->point_index[replex->replica_id] = get_closest_index_in_grid(awh_bias->coord_value, awh_bias->grid);
        ...
        {
            replex->point_bias[replex->replica_id][i] = calc_convolved_bias(awh_bias, awh_bias->grid->point[i].value);
        }
    }
...
/* Update the replica exchange struct with the current AWH bias. */
gmx_bool update_awh_replica_exchange_state(const awh_t *awh, awh_replex_t *replex, const gmx_multisim_t *ms)
{
    ...
    return update_awh_replica_exchange_state(&awh->awh_bias[0], replex, ms);
}

/* Calculate the AWH bias exchange probability delta.*/
