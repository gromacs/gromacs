...
{
    /* All point in the grid have the same number of elements */
    return corrgrid->corrmatrix[0].ncorr;
}

int get_nblocks(const correlation_grid_t *corrgrid)
{
    int                      nblockdata     = corrgrid->corrmatrix[0].corr[0].nblockdata;
    t_correlation_blockdata *blockdata      = corrgrid->corrmatrix[0].corr[0].blockdata;
...
static double get_covariance(const t_correlation *corr)
{
...
{
    t_correlation *corr     = &corrgrid->corrmatrix[pointindex].corr[corrindex];
    double         weight   = corr->blockdata[corr->nblockdata - 1].sum_w;
...
static void double_blocklengths(t_correlation_blockdata *blockdata, int nblockdata)
{
...
static void update_blocklengths(t_correlation_blockdata *blockdata, int nblockdata, double sampling_length)
{
...
static void add_block_to_correlation_integral(t_correlation_blockdata *blockdata,
...
static void add_data_to_block(t_correlation_blockdata *blockdata, double x, double y, double w)
{
...
static void update_correlation(t_correlation *corr, double x, double y, double w, double sampling_length)
{
    t_correlation_blockdata *all_data;
...
static double get_weight(const correlation_matrix_t *corrmatrix)
{
    ...
    t_correlation *corr = &corrmatrix->corr[0];
...
static void init_correlation_blockdata(t_correlation_blockdata *b, double blocklength)
{
    b->sum_wx  = 0;
    b->sum_wy  = 0;
    b->sum_w   = 0;
...
static void init_correlation(t_correlation *corr, int nblockdata, t_correlation_blockdata *blockdata,
                             int i1, int i2,
                             double blocklength, bool bConst_nblocks)
{
    ...
    if (bConst_nblocks)
    {
        ...
        {
            init_correlation_blockdata(&corr->blockdata[n], blocklength*scaling);
    ...
    else
    {
        ...
        init_correlation_blockdata(&corr->blockdata[0], blocklength);
        init_correlation_blockdata(&corr->blockdata[1], GMX_DOUBLE_MAX);
    }
}
                                          ...
                                          int nstsample)
{
    ...
    t_correlation           *corr_temp      = NULL;
    t_correlation_blockdata *blockdata_temp = NULL;
    ...
    nelem = get_nelem_in_corrmatrix(ndim);
