...
{
    const std::vector<BlockData> &blockData = corr.blockData();
    double                        weight    = blockData[blockData.size() - 1].simSumSqrW;
    double                        corrsum   = 0;
    if (weight > 0)
    {
	corrsum = corr.blockData()[0].correlationIntegral[tensorIndex]/weight;
    }
...
void CorrelationTensor::updateBlockLengths(double samplingLength)
{
    ...
    int numDoublings = ceil_log2(samplingLength/blockData_[blockData_.size() - 1].blockLength);
    ...
    blockLengthInWeight(measureBlockLengthInWeight)
{
    ...
    int numBlocks    = c_numCorrelationBlocks;
    int numBlockData = getNumBlockData(numBlocks);

    corr.resize(numPoints, CorrelationTensor(numDims, numBlockData, blockLengthInit));
}
