...
/* The correlation integral can get unreliable when few block contribute
 * to the block variance. We decide that the integral is unreliable when
 * one block contributes more than half of the total.
 */
//static const double c_maxSingleBlockWeight = 0.5;
static const double c_maxSingleBlockWeight = 1.0/3.0;


/*
static bool haveReliableIntegral(const correlation_blockdata_t &blockdata)
{
    return blockdata.wb2_max < c_maxSingleBlockWeight*blockdata.sum_wb2;
}
*/
...
double get_correlation_timeintegral(const correlation_grid_t *corrgrid, int pointindex, int corrindex)
{
    ...
    double         weight   = corr->blockdata[corr->nblockdata - 1].sum_w2_tot;
    double         corrsum  = 0;
    if (weight > 0)
    {
	corrsum = corr->blockdata[0].correlation_integral/weight;
    }

    return 0.5*corrsum*corrgrid->dt_sample;
}
 ...
 *
 * \param[in,out] corr           The correlation data.
 * \param[in] sampling_length    Sampling length of all data, in time or weight.
 */
static void update_blocklengths(correlation_t *corr, double sampling_length)
{
    correlation_blockdata_t *blockdata  = corr->blockdata;
    int                      nblockdata = corr->nblockdata;
...
{
    bool firstBlock = (blockdata->sum_wb2 == 0);
    ...
    if (!firstBlock)
    {
        avg_x_old = blockdata->sum_wbwx/blockdata->sum_wb2;
    }
    else
    {
    ...
    double w_block       = blockdata->sum_w;
    blockdata->sum_wbwx += w_block*blockdata->sum_wx;
    blockdata->sum_wbwy += w_block*blockdata->sum_wy;
    blockdata->sum_wb2  += w_block*w_block;
    blockdata->wb2_max   = std::max(blockdata->wb2_max, w_block);

    if (!firstBlock)
    {
        avg_y_new = blockdata->sum_wbwy/blockdata->sum_wb2;
...
static void update_correlation(correlation_t *corr, double x, double y, double w, double sampling_length)
{
    ...
    update_blocklengths(corr, sampling_length);
    ...
    for (int i = 0; i < corr->nblockdata - 1; i++)
    {
        ...
        if (corr->blockdata[i].blockindex_prev >= 0 && corr->blockdata[i].blockindex_prev != block_index)
        {
            ...
            add_block_to_correlation_integral(&corr->blockdata[i]);
        }
                                          ...
                                          int nstsample)
{
    ...
    {
        /* Default value */
        nblocks = c_numCorrelationBlocks;
    }
    if (nblocks > 0)
    {
        /* User value */
