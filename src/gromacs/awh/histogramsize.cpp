...
{
    /* Note that here we only copy the global state variables.
     * The other data members of PointState are only part of the
     * temporary, local state, which is zero at restart.
     */
...
void HistogramSize::storeState(AwhBiasStateHistory *stateHistory) const
{
    /* Note that here we only copy the global state variables.
     * The other data members of PointState are only part of the
     * temporary, local state, which is flushed to the global state
