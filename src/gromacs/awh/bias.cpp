...
void Bias::updateForceCorrelation(const std::vector<double> &probWeightNeighbor, double t)
{
    ...
    for (size_t n = 0; n < neighbor.size(); n++)
    {
        ...
        add_data_to_correlation_matrix(forcecorr_, indexNeighbor, weightNeighbor, forceFromNeighbor, t);
    }
}
...
void Bias::updateHistory(AwhBiasHistory *biasHistory) const
{
    ...
    update_correlation_grid_history(biasHistory->forcecorr, forcecorr_);
}
...
void Bias::restoreStateFromHistory(const AwhBiasHistory *biasHistory)
{
    ...
    restore_correlation_grid_state_from_history(biasHistory->forcecorr, forcecorr_);
}
