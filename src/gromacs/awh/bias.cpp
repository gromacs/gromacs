...
/*! \libinternal \file
 * \brief
 * Implements most of the Awh class.
 *
            ...
            std::vector<float> *pmf)
{
    ...
    if (bias.params.numSharedUpdate > 1)
    {
        ...
        gmx_sumf_sim(bias.pointState.size(), pmf->data(), ms);
...
static double freeEnergyMinimumValue(const Bias &bias)
{
    ...
    for (auto const &pointState : bias.pointState)
    {
        if (pointState.inTargetRegion() && pointState.freeEnergy() < f_min)
        {
            f_min = pointState.freeEnergy();
        }
    }
...
static void updateTarget(Bias *bias)
{
    ...
    for (auto &pointState : bias->pointState)
    {
        ...
        {
            double df         = pointState.freeEnergy() - f_max;
            pointState.target = 1/(1 + std::exp(df));
        }
        else if (params.eTarget == eawhtargetBOLTZMANN)
        {
            pointState.target = std::exp(-params.target_param*pointState.freeEnergy());
        }
        else if (params.eTarget == eawhtargetLOCALBOLTZMANN)
        {
            pointState.target = pointState.weightsumRef();
        }

        /* All target types can be modulated by a constant factor. */
        pointState.target *= pointState.target_constant_weight;

        sum_target += pointState.target;
    }
...
void initTargetAndBias(Bias *bias)
{
    ...
    GMX_ASSERT((bias->params.eTarget != eawhtargetLOCALBOLTZMANN) ||
               (bias->params.eTarget == eawhtargetLOCALBOLTZMANN && bias->pointState[0].weightsumRef() != 0),
    ...
    for (auto &pointState : bias->pointState)
    {
        ...
        else
        {
            ...
            pointState.setBiasToMinusInfinity();
        }
    }
}
...
/* Set pull forces for the pull coordinates this bias is responsible for. */
void Bias::applyForceToPullCoords(const awh_dvec    biasForce,
                                  ...
                                  tensor            virial) const
{
    ...
    {
        apply_external_pull_coord_force(pull_work, dimParams[d].pullCoordIndex,
...
static void checkOnData(const Bias &bias, double t, gmx_int64_t step, FILE *fplog)
{
    ...
    const int          nstcheck_on_data       = std::max(params.nstupdate_free_energy,
                                                         (static_cast<int>(bias.pointState.size())*100*params.nstsample_coord/params.nstupdate_free_energy)*params.nstupdate_free_energy);
                                    ...
                                    gmx_int64_t step, int seed)
{
    ...
    int n_sampled  = get_sample_from_distribution(probWeightNeighbor, bias->grid->point(bias->state.gridpointIndex).neighbor.size(),
                                                  step, seed, bias->biasIndex);

    return bias->grid->point(bias->state.gridpointIndex).neighbor[n_sampled];
}
                    ...
                    gmx_int64_t step, int seed)
{
    ...
    bias->state.refGridpoint = sampleReferenceGridpoint(bias, probWeightNeighbor, step, seed);

    awh_dvec newForce;
    double   newPotential =
        calcUmbrellaForceAndPotential(*bias, bias->state.refGridpoint, newForce);
...
static void doSkippedUpdatesForAllPoints(Bias *bias, gmx_int64_t step)
{
    ...
    for (auto &pointState : bias->pointState)
    {
        bool bUpdated = pointState.updateSkipped(bias->params, step, weighthistScaling, logPmfsumScaling);
...
static void doSkippedUpdatesInNeighborhood(Bias *bias, gmx_int64_t step)
{
    ...
    const std::vector<int> &neighbors = bias->grid->point(bias->state.gridpointIndex).neighbor;
    for (auto &neighbor : neighbors)
    {
        bool bUpdated = bias->pointState[neighbor].updateSkipped(bias->params, step, weighthistScaling, logPmfsumScaling);

        if (bUpdated)
        {
            bias->pointState[neighbor].updateBias();
        }
    }
}
 ...
 * \param[in,out] bias  The AWH bias.
 */
static void resetLocalUpdateRange(Bias *bias)
{
    ...
    for (int d = 0; d < bias->grid->ndim(); d++)
    {
        ...
        state->origin_updatelist[d] = bias->grid->point(state->gridpointIndex).index[d];
        state->end_updatelist[d]    = bias->grid->point(state->gridpointIndex).index[d];
    }
}
 ...
 */
static void sumHistograms(Bias *bias, const gmx_multisim_t *ms, const std::vector<int> &local_update_list, int nlocal_update)
{
    ...
    for (int ilocal = 0; ilocal < nlocal_update; ilocal++)
    {
        ...
        bias->pointState[iglobal].weightsum_covering += bias->pointState[iglobal].weightsum_iteration;
    }
    ...
    if (bias->params.numSharedUpdate > 1)
    {
        ...
        for (int ilocal = 0; ilocal < nlocal_update; ilocal++)
        {
            ...
            weight_distr[ilocal] = bias->pointState[iglobal].weightsum_iteration;
            coord_visits[ilocal] = bias->pointState[iglobal].visits_iteration;
        }
        ...
        for (int ilocal = 0; ilocal < nlocal_update; ilocal++)
        {
            ...
            bias->pointState[iglobal].weightsum_iteration = weight_distr[ilocal];
            bias->pointState[iglobal].visits_iteration    = coord_visits[ilocal];
        }
    }
    ...
    for (int ilocal = 0; ilocal < nlocal_update; ilocal++)
    {
        ...
        double weights = bias->pointState[iglobal].weightsum_iteration;
        double visits  = bias->pointState[iglobal].visits_iteration;
        bias->pointState[iglobal].weightsumTot += weights;
        bias->pointState[iglobal].visits_tot   += visits;
    }
}
...
static void normalizeFreeEnergyAndPmfSum(Bias *bias)
{
    ...
    for (auto &pointState : bias->pointState)
    {
        pointState.normalizeFreeEnergyAndPmfSum(minF);
    }
}

/*! \brief
 * Performs an update of a bias.
 *
...
static void update(Bias *bias, const gmx_multisim_t *ms, double t, gmx_int64_t step, FILE *fplog)
{
    ...
    /* Make a list of all local points, i.e. those that could have been touched since
       ...
       final update list. */
    int      nlocal_update;
    makeLocalUpdateList(bias, ms, &updateList, &nlocal_update);

    /* Reset the range for the next update */
    resetLocalUpdateRange(bias);

    /* Add samples to histograms for all local points and sync simulations if needed */
    sumHistograms(bias, ms, updateList, nlocal_update);
    ...
    for (int ilocal = 0; ilocal < nlocal_update; ilocal++)
    {
        ...
        /* We want to keep the absolute value of the free energies to be less c_largePositiveExponent
           ...
           broken. */
        if (std::fabs(bias->pointState[iglobal].freeEnergy()) > 0.5*c_largePositiveExponent)
        {
    ...
    {
        /* In the initial stage, the histogram grows dynamically as a function of the number of coverings. */
        int nstcheck_covered = std::max((static_cast<int>(bias->pointState.size())*params.nstsample_coord/params.nstupdate_free_energy)*params.nstupdate_free_energy,
    ...
    if (doUpdateTarget || haveCovered || !params.canSkipUpdates() || needToNormalizeFreeEnergy)
    {
        ...
        {
            if (bias->pointState[m].inTargetRegion())
            {
    ...
    {
        PointState *pointStateToUpdate = &bias->pointState[updateList[iupdate]];

        /* Do updates from previous update steps that were skipped because this point was at that time non-local. */
        if (params.canSkipUpdates())
        {
            pointStateToUpdate->updateSkipped(params, step, weighthistScalingSkipped, logPmfsumScalingSkipped);
        }

        /* Now do an update with new sampling data. */
        pointStateToUpdate->updateNew(params, step, weighthistScalingNew, logPmfsumScalingNew);
    ...
    for (int iupdate = 0; iupdate < nupdate; iupdate++)
    {
        bias->pointState[updateList[iupdate]].updateBias();
    }
}
                                     ...
                                     const std::vector<double> &probWeightNeighbor)
{
    ...
    for (size_t n = 0; n < neighbor.size(); n++)
    {
        bias->pointState[neighbor[n]].weightsum_iteration += probWeightNeighbor[n];
    }
...
static void sample_pmf(Bias *bias, double convolvedBias)
{
    ...
    PointState *pointState = &bias->pointState[bias->state.gridpointIndex];

    /* Save PMF sum and keep a histogram of the sampled coordinate values */
    if (bIn_array)
    {
        pointState->samplePmf(convolvedBias);
    }
}
...
void Bias::doStep(bool convolveForce,
                  ...
                  FILE *fplog)
{
    ...
    state.gridpointIndex = get_closest_index_in_grid(state.coordValue, grid.get());
    ...
    double convolvedBias;
    if (convolveForce || doAtStep(params.nstsample_coord, step))
    {
        if (params.canSkipUpdates())
        {
            doSkippedUpdatesInNeighborhood(this, step);
        }

        updateProbabilityWeightsAndConvolvedBias(this, probWeightNeighbor, &convolvedBias);

        if (step > 0 && doAtStep(params.nstsample_coord, step))
        {
            doObservableSampling(this, *probWeightNeighbor, convolvedBias);
        }
    }
    ...
    {
        /* Umbrella force */
        GMX_RELEASE_ASSERT(pointState[state.refGridpoint].inTargetRegion(),
                           "AWH bias grid point reference value is outside of the target region.");
        potential =
            calcUmbrellaForceAndPotential(*this, state.refGridpoint, biasForce);
        ...
        if (doAtStep(params.nstsample_coord, step))
        {
            newPotential = moveUmbrella(this, *probWeightNeighbor, biasForce, step, seed);
        }
        else
        {
    ...
    *awhPotential += potential;
    ...
    if (step > 0 && doAtStep(params.nstupdate_free_energy, step))
    {
        update(this, ms, t, step, fplog);

        if (convolveForce)
        {
            /* The update results in a potential jump, so we need the new convolved potential. */
            newPotential = -calcConvolvedBias(*this, state.coordValue)*params.invBeta;
        }
    }
...
void Bias::setCoordValue(struct pull_t *pull_work, const t_pbc *pbc)
{
    /* Keep own copy of current coordinate value. */
    for (int d = 0; d < ndim; d++)
    {
        get_pull_coord_value(pull_work, dimParams[d].pullCoordIndex, pbc,
                             &state.coordValue[d]);
    }
}
...
{
    GMX_RELEASE_ASSERT(biasHistory->pointState.size() == pointState.size(), "The AWH history setup does not match the AWH state.");
    ...
    {
        const PointState     &ps  = pointState[m];
    ...
    stateHistory->origin_index_updatelist = multidim_gridindex_to_linear(grid.get(),
                                                                         state.origin_updatelist);
    stateHistory->end_index_updatelist    = multidim_gridindex_to_linear(grid.get(),
...
void Bias::restoreStateFromHistory(const AwhBiasHistory *biasHistory)
{
    ...
    {
        pointState[m].setFromHistory(biasHistory->pointState[m]);
    }
    ...
    linear_gridindex_to_multidim(grid.get(), stateHistory.origin_index_updatelist, state.origin_updatelist);
    linear_gridindex_to_multidim(grid.get(), stateHistory.end_index_updatelist, state.end_updatelist);
...
{
    gmx_bcast(pointState.size()*sizeof(PointState), pointState.data(), cr);

    gmx_bcast(sizeof(BiasState), &state, cr);
}
