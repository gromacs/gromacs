            ...
            std::vector<float> *pmf)
{
    ...
    const std::vector<PointState> &pointState = bias.state().points;
 ...
 * \param[in] biasState     The AWH bias state.
                                    ...
                                    const awh_dvec                value)
{
    ...
    if (biasState.points[pointIndex].inTargetRegion())
    {
                     ...
                     std::vector<float> *convolvedPmf)
{
    ...
    for (size_t m = 0; m < numPoints; m++)
    {
        ...
        for (auto &neighbor : point.neighbor)
        {
            ...
            freeEnergyWeights += biasedWeightFromPoint(bias.dimParams(), bias.state(), bias.grid(),
 ...
 */
static bool isCheckStep(const BiasParams &params, const std::vector<PointState> &pointState, gmx_int64_t step)
{
...
void Bias::checkHistograms(double t, gmx_int64_t step, FILE *fplog)
{
    ...
    if (fplog == NULL || numWarningsIssued_ >= maxNumWarningsInRun || state_.inInitialStage ||
    ...
    double sumVisits = 0;
    double sumW      = 0;
    for (auto &pointState : state_.points)
    {
    ...
    int         numWarnings  = 0;
    size_t      numPoints    = grid_->numPoints();
    for (size_t m = 0; m < numPoints; m++)
    {
        /* Skip points close to boundary or non-target region */
        const GridPoint &gridPoint = grid_->point(m);
        bool             skipPoint = false;
        for (size_t n = 0; (n < gridPoint.neighbor.size()) && !skipPoint; n++)
        {
            int ineighbor = gridPoint.neighbor[n];
            skipPoint     = !state_.points[ineighbor].inTargetRegion();
            for (int d = 0; (d < grid_->ndim()) && !skipPoint; d++)
            {
                const GridPoint &neighborPoint = grid_->point(ineighbor);
                skipPoint =
                    neighborPoint.index[d] == 0 ||
                    neighborPoint.index[d] == grid_->axis(d).numPoints() - 1;
            }
        }

        /* Warn if the coordinate distribution less than the target distribution with a certain fraction somewhere */
        if (!skipPoint &&
            (state_.points[m].weightsumTot*invNormW*maxHistogramRatio > state_.points[m].visits_tot*invNormVisits ))
        {
        ...
        if (numWarnings >= maxNumWarningsInCheck)
        {
            break;
        }
    }
 ...
 * \param[in] dimParams    The bias dimensions parameters.
 * \param[in] grid         The grid.
 * \param[in] coordValue   The current coordinate value.
 * \param[in] point        Point for umbrella center.
 * \param[in,out] force    Force vector to set.
 * Returns the umbrella potential.
 */
static double calcUmbrellaForceAndPotential(const std::vector<DimParams> &dimParams,
                                            const Grid                   &grid,
                                            ...
                                            awh_dvec                      force)
{
    ...
    for (size_t d = 0; d < dimParams.size(); d++)
    {
        double dev = getDeviationFromPointAlongGridaxis(grid, d, point, coordValue[d]);
 ...
 */
static void calcConvolvedForce(const std::vector<DimParams> &dimParams,
                               ...
                               awh_dvec                      force)
{
    ...
    const std::vector<int> &neighbor = grid.point(state.gridpointIndex).neighbor;
    for (size_t n = 0; n < neighbor.size(); n++)
    {
        ...
        calcUmbrellaForceAndPotential(dimParams, grid, state.coordValue, indexNeighbor,
                                      forceFromNeighbor);
                          ...
                          gmx_int64_t step, int seed)
{
    ...
    state_.refGridpoint = sampleReferenceGridpoint(grid(), state_.gridpointIndex, probWeightNeighbor, step, seed, biasIndex);

    awh_dvec newForce;
    double   newPotential =
        calcUmbrellaForceAndPotential(dimParams_, grid(), state_.coordValue, state_.refGridpoint, newForce);
...
void Bias::setSkippedUpdateHistogramScaleFactors(double *weighthistScaling,
                                                 double *logPmfsumScaling)
{
    GMX_ASSERT(params_.skipUpdates(), "Calling function for skipped updates when skipping updates is not allowed");
    ...
    if (state_.inInitialStage)
    {
        ...
        setHistogramUpdateScaleFactors(params_, state_.histSize, state_.histSize,
...
void Bias::doSkippedUpdatesForAllPoints()
{
    ...
    setSkippedUpdateHistogramScaleFactors(&weighthistScaling, &logPmfsumScaling);
...
void Bias::doSkippedUpdatesInNeighborhood()
{
    ...
    {
        bool didUpdate = state_.points[neighbor].updateSkipped(params_, state_.numUpdates, weighthistScaling, logPmfsumScaling);

        if (didUpdate)
        {
            state_.points[neighbor].updateBias();
        }
    }
}
 ...
 * \param[in] biasState        The bias state.
 * \param[in] numSharedUpdate  The number of biases that shared updates.
 * \param[in] ms               Struct for multi-simulation communication.
                                ...
                                std::vector<int>     *updateList)
{
    ...
    for (int d = 0; d < grid.ndim(); d++)
    {
        origin_dim[d]  = biasState.originUpdatelist[d];
        npoints_dim[d] = biasState.endUpdatelist[d] - biasState.originUpdatelist[d] + 1;
    ...
    while (pointExists)
    {
        ...
        if (pointExists && biasState.points[point_index].inTargetRegion())
        {
    ...
    {
        mergeSharedUpdateLists(updateList, biasState.points.size(), ms);
    }
}
...
double Bias::newHistSizeInitialStage(double t, bool detectedCovering, FILE *fplog)
{
...
bool Bias::isCovered(const gmx_multisim_t *ms) const
{
    ...
    checkDim.resize(grid_->ndim());

    for (int d = 0; d < grid_->ndim(); d++)
    {
        size_t npoints = grid_->axis(d).numPoints();
    ...
    if (params_.eTarget == eawhtargetCUTOFF)
    {
        maxFreeEnergy = freeEnergyMinimumValue(state_.points) + params_.targetParam;
    }
    ...
    for (int d = 0; d < ndim(); d++)
    {
        weight_peak *= grid_->axis(d).spacing()*std::sqrt(dimParams_[d].betak*0.5*M_1_PI);
    }

    /* Project the sampling weights onto each dimension */
    for (size_t m = 0; m < grid_->numPoints(); m++)
    {
        const PointState &pointState = state().points[m];

        for (int d = 0; d < grid_->ndim(); d++)
        {
            int n = grid_->point(m).index[d];
    ...
    for (int d = 0; d < grid_->ndim(); d++)
    {
        labelCoveredPoints(checkDim[d].visited, checkDim[d].checkCovering, grid_->axis(d).numPoints(), grid_->axis(d).numPointsInPeriod(),
                           params_.coverRadius[d], &checkDim[d].covered);
    }
    ...
    if (params_.numSharedUpdate > 1)
    {
        /* For multiple dimensions this may not be the best way to do it. */
        for (int d = 0; d < grid_->ndim(); d++)
        {
            gmx_sumi_sim(grid_->axis(d).numPoints(), checkDim[d].covered.data(), ms);
        }
    }

    /* Now check if for each dimension all points are covered. Break if not true. */
    bool allPointsCovered = true;
    for (int d = 0; d < grid_->ndim() && allPointsCovered; d++)
    {
        for (int n = 0; n < grid_->axis(d).numPoints() && allPointsCovered; n++)
        {
...
double Bias::newHistSize(double t, bool covered, FILE *fplog)
{
    ...
    if (state_.inInitialStage)
    {
        ...
        if (state_.equilibrateHistogram && covered)
        {
            ...
            state_.equilibrateHistogram = !histogramIsEquilibrated(state_.points);
...
void Bias::updateFreeEnergyAndAddSamplesToHistogram(const gmx_multisim_t *ms, double t, gmx_int64_t step, FILE *fplog)
{
    ...
    /* Make a list of all local points, i.e. those that could have been touched since
       ...
       final update list. */
    makeLocalUpdateList(grid(), state_, params_.numSharedUpdate, ms,
                        &updateList);
    ...
    sumHistograms(&state_.points, params.numSharedUpdate, ms, updateList);
    ...
    for (auto &iglobal : updateList)
    {
        /* We want to keep the absolute value of the free energies to be less c_largePositiveExponent
           ...
           broken. */
        if (std::fabs(state_.points[iglobal].freeEnergy()) > 0.5*c_largePositiveExponent)
        {
    ...
    if (state_.inInitialStage)
    {
        detectedCovering = isCheckStep(params, state_.points, step) && isCovered(ms);
    }

    /* The weighthistogram size after this update. */
    double histSizeNew = newHistSize(t, detectedCovering, fplog);
    ...
    if (doUpdateTarget || detectedCovering || !params.skipUpdates() || needToNormalizeFreeEnergy)
    {
        ...
        updateList.resize(0);
        for (size_t m = 0; m < state_.points.size(); m++)
        {
            if (state_.points[m].inTargetRegion())
            {
                updateList.push_back(m);
            }
        }
    ...
    {
        setSkippedUpdateHistogramScaleFactors(&weighthistScalingSkipped, &logPmfsumScalingSkipped);
    }
    setHistogramUpdateScaleFactors(params_, histSizeNew, state_.histSize,
                                   &weighthistScalingNew, &logPmfsumScalingNew);

    /* Update free energy and reference weight histogram for points in the update list. */
    for (auto &pointIndex : updateList)
    {
        PointState *pointStateToUpdate = &state_.points[pointIndex];
    ...
    {
        normalizeFreeEnergyAndPmfSum(&state_.points);
    }

    if (doUpdateTarget)
    {
        /* The target distribution is always updated for all points at once. */
        updateTarget(&state_.points, params);
    }
    ...
    for (auto &pointIndex : updateList)
    {
        state_.points[pointIndex].updateBias();
    }
}
 ...
 */
static double updateProbabilityWeightsAndConvolvedBias(const std::vector<DimParams> &dimParams,
                                                       const BiasState              &biasState,
                                                       const Grid                   &grid,
                                                       std::vector<double>          *weight)
{
    /* Only neighbors of the current coordinate value will have a non-negligible chance of getting sampled */
    const std::vector<int> &neighbors = grid.point(biasState.gridpointIndex).neighbor;
    ...
    for (size_t n = 0; n < neighbors.size(); n++)
    {
        ...
        (*weight)[n] = biasedWeightFromPoint(dimParams, biasState, grid,
                                             neighbor, biasState.points[neighbor].bias(),
                                             biasState.coordValue);
        weightSum   += (*weight)[n];
    }
                         ...
                         const awh_dvec                coordValue)
{
    ...
    {
        weight_sum += biasedWeightFromPoint(dimParams, biasState, grid,
                                            neighbor, biasState.points[neighbor].bias(),
                                            coordValue);
    }
...
void Bias::sampleProbabilityWeights(const std::vector<double> &probWeightNeighbor)
{
    ...
    for (size_t n = 0; n < neighbor.size(); n++)
    {
        state_.points[neighbor[n]].weightsum_iteration += probWeightNeighbor[n];
    }
...
void Bias::sampleCoordAndPmf(const std::vector<double> &probWeightNeighbor,
                             double                     convolvedBias)
{
     ...
     */
    if (grid().covers(state_.coordValue))
    {
        /* Save PMF sum and keep a histogram of the sampled coordinate values */
        state_.points[state_.gridpointIndex].samplePmf(convolvedBias);
    }
                                  ...
                                  FILE *fplog)
{
     ...
     */
    state_.gridpointIndex = grid().nearestIndex(state_.coordValue);
    ...
    if (params.convolveForce || sampleCoord)
    {
        ...
        convolvedBias = updateProbabilityWeightsAndConvolvedBias(dimParams_, state_, grid(), probWeightNeighbor);

        if (step > 0 && sampleCoord)
        {
            sampleCoordAndPmf(*probWeightNeighbor, convolvedBias);
        }
    }
    ...
    {
        calcConvolvedForce(dimParams_, state_, grid(), *probWeightNeighbor,
                           biasForce);
    ...
    else
    {
        ...
        potential =
            calcUmbrellaForceAndPotential(dimParams_, grid(), state_.coordValue, state_.refGridpoint, biasForce);
    ...
    *awhPotential += potential;
    ...
    {
        updateFreeEnergyAndAddSamplesToHistogram(ms, t, step, fplog);
        ...
        if (params.convolveForce)
        {
            ...
            newPotential = -calcConvolvedBias(dimParams_, state_, grid(), state_.coordValue)*params.invBeta;
        }
    }
...
{
    GMX_RELEASE_ASSERT(biasHistory->pointState.size() == points.size(), "The AWH history setup does not match the AWH state.");

    AwhBiasStateHistory *stateHistory = &biasHistory->state;
    stateHistory->refGridpoint        = refGridpoint;

    for (size_t m = 0; m < biasHistory->pointState.size(); m++)
    {
        const PointState     &ps  = points[m];
    ...
    stateHistory->in_initial              = inInitialStage;
    stateHistory->equilibrateHistogram    = equilibrateHistogram;
    stateHistory->histSize                = histSize;
    ...
    stateHistory->scaledSampleWeight      = scaledSampleWeight;
    stateHistory->maxScaledSampleWeight   = maxScaledSampleWeight;
    stateHistory->numUpdates              = numUpdates;
}
                                   ...
                                   const Grid           &grid)
{
    ...
    refGridpoint = stateHistory.refGridpoint;

    for (size_t m = 0; m < points.size(); m++)
    {
        points[m].setFromHistory(biasHistory.pointState[m]);
    }

    inInitialStage         = stateHistory.in_initial;
    equilibrateHistogram   = stateHistory.equilibrateHistogram;
    histSize               = stateHistory.histSize;

    linear_gridindex_to_multidim(grid, stateHistory.origin_index_updatelist, originUpdatelist);
    linear_gridindex_to_multidim(grid, stateHistory.end_index_updatelist, endUpdatelist);

    scaledSampleWeight     = stateHistory.scaledSampleWeight;
    maxScaledSampleWeight  = stateHistory.maxScaledSampleWeight;
    numUpdates             = stateHistory.numUpdates;
}

/* Broadcast the bias state over the MPI ranks in this simulation. */
void BiasState::broadcast(const t_commrec *cr)
{
    std::vector<PointState> pointsTmp = move(points);
    GMX_RELEASE_ASSERT(points.capacity() == 0, "Can only broadcast the state when points has no memory allocated");

    gmx_bcast(sizeof(BiasState), this, cr);

    points = move(pointsTmp);

    gmx_bcast(points.size()*sizeof(PointState), points.data(), cr);
}
