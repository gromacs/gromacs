...
void Bias::updateFreeEnergyAndAddSamplesToHistogram(const gmx_multisim_t *ms, double t, gmx_int64_t step, FILE *fplog)
{
    ...
    if (state_.inInitialStage)
    {
        /* We can only check for covering here at free energy update steps */
        int numStepsUpdateFreeEnergy = params.numSamplesUpdateFreeEnergy*numStepsSampleCoord;
        /* To reduce the cost of checking, check when we have as many samples as points */
        int numStepsCheckCovered     = (1 + pointState_.size()/params.numSamplesUpdateFreeEnergy)*numStepsUpdateFreeEnergy;
        haveCovered                  = (step % numStepsCheckCovered == 0) && isCovered(*this, ms);
    }
    ...
    int nupdate;
    if (doUpdateTarget || haveCovered || !params.skipUpdates() || needToNormalizeFreeEnergy)
    {
