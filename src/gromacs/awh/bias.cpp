...
static double biasedWeightFromPoint(const Bias &bias, int point_index, double pointBias, const awh_dvec value)
{
    ...
    if (bias.pointState()[point_index].inTargetRegion())
    {
        ...
        {
            double dev = get_deviation_from_point_along_gridaxis(bias.grid(), d, point_index, value[d]);
                                  ...
                                  tensor            virial) const
{
    ...
    {
        apply_external_pull_coord_force(pull_work, dimParams_[d].pullCoordIndex,
...
static double calcUmbrellaForceAndPotential(const Bias &bias, int point, awh_dvec force)
{
    ...
    for (int d = 0; d < bias.ndim(); d++)
    {
        double dev = get_deviation_from_point_along_gridaxis(bias.grid(), d, point, bias.state().coordValue[d]);
...
void Bias::doSkippedUpdatesForAllPoints(gmx_int64_t step)
{
    ...
    {
        bool bUpdated = pointState.updateSkipped(params_, step, weighthistScaling, logPmfsumScaling);
...
void Bias::doSkippedUpdatesInNeighborhood(gmx_int64_t step)
{
    double weighthistScaling, logPmfsumScaling;
    ...
    {
        bool didUpdate = pointState_[neighbor].updateSkipped(params_, step, weighthistScaling, logPmfsumScaling);
...
static void makeLocalUpdateList(const Bias &bias, const gmx_multisim_t *ms, std::vector<int> *updateList, int *nupdate_ptr)
{
    ...
    while (pointExists)
    {
        pointExists = get_next_point_in_local_grid(bias.grid(), origin_dim, npoints_dim, &point_index);
...
void Bias::update(const gmx_multisim_t *ms, double t, gmx_int64_t step, FILE *fplog)
{
    ...
    {
        /* In the initial stage, the histogram grows dynamically as a function of the number of coverings. */
        int nstcheck_covered = std::max((static_cast<int>(pointState_.size())*params.nstsample_coord/params.nstupdate_free_energy)*params.nstupdate_free_energy,
 ...
 */
static void updateProbabilityWeightsAndConvolvedBias(const Bias          &bias,
...
{
    int                            point      = get_closest_index_in_grid(coordValue, bias.grid());
                                ...
                                double                     convolvedBias)
{
     ...
     */
    if (value_is_in_grid(state_.coordValue, grid()))
    {
                  ...
                  FILE *fplog)
{
    ...
    state_.gridpointIndex = get_closest_index_in_grid(state_.coordValue, grid());
    ...
    if (convolveForce || doAtStep(params.nstsample_coord, step))
    {
        if (params.canSkipUpdates())
        {
            doSkippedUpdatesInNeighborhood(step);
        }
...
void Bias::setCoordValue(struct pull_t *pull_work, const t_pbc *pbc)
{
    ...
    for (int d = 0; d < ndim(); d++)
    {
        get_pull_coord_value(pull_work, dimParams_[d].pullCoordIndex, pbc,
                             &state_.coordValue[d]);
    }
}
