...
static void checkOnData(Bias &bias, double t, gmx_int64_t step, FILE *fplog)
{
    ...
    if (fplog == NULL || bias.numWarningsIssued() >= maxNumWarningsInRun || bias.state().inInitialStage ||
        (step == 0) || isCheckStep(params, bias.pointState(), step))
    {
        return;
    }
    ...
    for (size_t m = 0; m < numPoints; m++)
    {
        ...
        for (size_t n = 0; (n < gridPoint.neighbor.size()) && !skipPoint; n++)
        {
            ...
            skipPoint     = !pointState[ineighbor].inTargetRegion();
        ...
        if (!skipPoint &&
            (pointState[m].weightsumTot*invNormW*maxHistogramRatio > pointState[m].visits_tot*invNormVisits ))
        {
            ...
            sprintf(warningmsg, "\nawh%d warning: "
                    ...
                    bias.biasIndex + 1, t, pointvaluestr, maxHistogramRatio);
            fprintf(fplog, "%s", wrap_lines(warningmsg, linewidth, indent, FALSE));

            numWarnings++;
            bias.incrementWarningsIssued();
        }
    ...
    if (bias.numWarningsIssued() >= maxNumWarningsInRun)
    {
...
{
    GMX_ASSERT(bias.params().skipUpdates(), "Calling function for skipped updates when skipping updates is not allowed");
 ...
 */
static void makeLocalUpdateList(const Bias &bias, const gmx_multisim_t *ms, std::vector<int> *updateList)
{
    ...
    for (int d = 0; d < bias.grid().ndim(); d++)
    {
        ...
        npoints_dim[d] = std::min(bias.grid().axis(d).numPoints(), npoints_dim[d]);
    }
    ...
    if (bias.params().numSharedUpdate > 1)
    {
        mergeSharedUpdateLists(updateList, bias.pointState().size(), ms);
    }
}
...
void Bias::updateFreeEnergyAndAddSamplesToHistogram(const gmx_multisim_t *ms, double t, gmx_int64_t step, FILE *fplog)
{
    ...
    /* Make a list of all local points, i.e. those that could have been touched since
       ...
       final update list. */
    makeLocalUpdateList(*this, ms, &updateList);
    ...
    sumHistograms(&pointState_, params.numSharedUpdate, ms, updateList);
    ...
    if (state_.inInitialStage)
    {
        detectedCovering = isCheckStep(params, pointState_, step) && isCovered(*this, ms);
    }
    ...
    for (auto &pointIndex : updateList)
    {
        PointState *pointStateToUpdate = &pointState_[pointIndex];
    ...
    for (auto &pointIndex : updateList)
    {
        pointState_[pointIndex].updateBias();
    }
}
...
{
    int                            point      = bias.grid().nearestIndex(coordValue);
