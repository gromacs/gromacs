...
static void checkOnData(const awh_bias_t *awh_bias, double t, gmx_int64_t step, FILE *fplog)
{
    ...
    const int          nstcheck_on_data       = std::max(awh_bias->nstupdate_free_energy,
                                                         (awh_bias->npoints*100*awh_bias->nstsample_coord/awh_bias->nstupdate_free_energy)*awh_bias->nstupdate_free_energy);
    int                nwarnings;
 ...
 */
static void labelCoveredPoints(const bool *visited, const bool *checkCovering, int npoints, int period,
                               int coverRadius, int *covered)
{
 ...
 */
static bool isCovered(const awh_bias_t *awh_bias, const gmx_multisim_t *ms)
{
    grid_t       *grid = awh_bias->grid;
    ...
    bool        **visited, **checkCovering;
    int         **covered;
    snew(visited, grid->ndim);
    snew(checkCovering, grid->ndim);
    snew(covered, grid->ndim);

    for (int d = 0; d < grid->ndim; d++)
    {
        snew(visited[d], grid->axis[d].npoints);
        snew(checkCovering[d], grid->axis[d].npoints);
        snew(covered[d], grid->axis[d].npoints);
    }
    ...
    for (int d = 0; d < grid->ndim; d++)
    {
        ...
        {
            visited[d][n]         = false;
            checkCovering[d][n]   = false;

            /* We use type int for the covering array since we may want to use gmx_sumi_sim. */
            covered[d][n]   = 0;
        }
    }
    ...
    for (int m = 0; m < grid->npoints; m++)
    {
        ...
        for (int d = 0; d < grid->ndim; d++)
        {
            ...
            visited[d][n] = visited[d][n] || (coordpoint->weightsum_covering > weight_peak);

            /* Check for covering if there is at least point in this slice that is in the target region and within the cutoff */
            checkCovering[d][n] = checkCovering[d][n] || (in_target_region(coordpoint) && coordpoint->free_energy < maxFreeEnergy);
        }
    }
    ...
    for (int d = 0; d < grid->ndim; d++)
    {
        labelCoveredPoints(visited[d], checkCovering[d], grid->axis[d].npoints, grid->axis[d].npoints_period,
                           awh_bias->coverRadius[d], covered[d]);
    }
    ...
    if (awh_bias->numSharedUpdate > 1)
    {
        ...
        for (int d = 0; d < grid->ndim; d++)
        {
            gmx_sumi_sim(grid->axis[d].npoints, covered[d], ms);
        }
    }
    ...
    int  d = 0;
    while (allPointsCovered && (d < grid->ndim))
    {
        int n = 0;
        while (allPointsCovered && n < grid->axis[d].npoints)
        {
            allPointsCovered = covered[d][n];
            n++;
        }
        d++;
    }
 ...
 */
static bool histogramIsEquilibrated(const awh_bias_t *awh_bias)
{
    ...
    for (int m = 0; m < awh_bias->npoints; m++)
    {
        ...
        totalWeight += awh_bias->coordpoint[m].weightsum_tot;
        ntargetPoints++;
    }
    ...
    for (int m = 0; m < awh_bias->npoints; m++)
    {
        double targetWeight  = awh_bias->coordpoint[m].target;
        double sampledWeight = awh_bias->coordpoint[m].weightsum_tot*inverseTotalWeight;

        /* Ignore these points. */
        if (!in_target_region(&awh_bias->coordpoint[m]) || targetWeight < minTargetWeight)
        {
            continue;
        }
 ...
 * \returns the new histogram size.
 */
static double newHistsize(awh_bias_t *awh_bias, double t, bool covered, FILE *fplog)
{
    double    histsize_new;
    if (awh_bias->in_initial)
    {
 ...
 * \param[in,out] awh_bias       AWH bias.
 */
static void normalizeFreeEnergyAndPmfSum(awh_bias_t *awh_bias)
{
    ...
    for (int m = 0; m < awh_bias->npoints; m++)
    {
        if (in_target_region(&awh_bias->coordpoint[m]))
        {
            ...
            awh_bias->coordpoint[m].free_energy -= minF;
            awh_bias->coordpoint[m].log_pmfsum  += minF;
        }
    }
}
...
static void update(awh_bias_t *awh_bias, const gmx_multisim_t *ms, double t, gmx_int64_t step, FILE *fplog)
{
    ...
    {
        /* In the initial stage, the histogram grows dynamically as a function of the number of coverings. */
        int nstcheck_covered = std::max((awh_bias->npoints*awh_bias->nstsample_coord/awh_bias->nstupdate_free_energy)*awh_bias->nstupdate_free_energy,
    ...
    /* Make the update list. Usually we try to only update local points but if the update has
       ...
       a renormalization is needed; or 5) replica exchange will be attempted at the end of this
       step requiring the bias to be up-to-date for all replica's neighborhoods. */
    int nupdate;
    if (updateTarget || haveCovered || !canSkipUpdates(awh_bias) || needToNormalizeFreeEnergy ||
        (awh_bias->nstreplica_exchange > 0 && do_at_step(awh_bias->nstreplica_exchange, step)))
    {
 ...
 *
 * \param[in,out] forcecorr   The force correlation grid.
 * \param[in]     awh_bias    AWH bias.
 * \param[in]     t           Time.
 */
static void update_force_correlation(correlation_grid_t *forcecorr, const awh_bias_t *awh_bias, double t)
{
    for (int n = 0; n < awh_bias->grid->point[awh_bias->coord_value_index].nneighbors; n++)
    {
        ...
        /* Add the force data of this neighbor point. Note: the sum of these forces is the convolved force.
           ...
           we're interested in. */
        calcUmbrellaForceAndPotential(awh_bias, awh_bias->betak, m_neighbor, force_from_neighbor);
                ...
                FILE                   *fplog)
{
    ...
    if (time_to_write(step, awh->writer))
    {
        ...
        for (int k = 0; k < awh->nbias; k++)
        {
            if (canSkipUpdates(awh->awh_bias))
            {
