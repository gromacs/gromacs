...
{
    /* The general formula used here is: scaling = [histogram size we want]/[histogram size we have + weight of added samples]. */

    if (awh_bias->in_initial)
    {
        /* These scaling factors account for keeping the histogram size constant in the initial stage. */
        *scale_weighthist_ptr = awh_bias->histsize/(awh_bias->histsize + awh_bias->update_weight*awh_bias->localWeightScaling);
        *scale_pmfsum_ptr     = std::log(awh_bias->histsize/(awh_bias->histsize + awh_bias->update_weight));
    }
    else
    {
        /* Linear growth. If localWeightScaling < 1 the growth is linear but scaled down. */
        *scale_weighthist_ptr     = 1;

        /* localWeightScaling < 1 should be reflected also by a corresponding scaling of the PMF histogram. Inuitively
           one might think that each PMF sample (see sample_pmf) should be scaled by localWeightScaling which gives
           each sample smaller but equal weight. In practice however, this doesn't yield an accurate PMF, presumably
           due to the fact that normalization of the bias is still changing initially and the bias comes into the PMF
           extraction exponentially. Instead, here the whole PMF histogram is rescaled to the right size which
           effectively give later samples more weight. Note that for localWeightScaling = 1 we get no rescaling. */
        double newPmfSumSize = awh_bias->histsize + awh_bias->localWeightScaling*awh_bias->update_weight;
        double oldPmfSumSize = awh_bias->histsize + awh_bias->update_weight;
        *scale_pmfsum_ptr    = std::log(newPmfSumSize/oldPmfSumSize);
    }
}
                                             ...
                                             bool bCovered, double *scale_weighthist_ptr, double *scale_pmfsum_ptr)
{
    ...
    if (bCovered)
    {
        ...
        *scale_weighthist_ptr = histsize_new/(histsize_old + awh_bias->update_weight*awh_bias->localWeightScaling);
        *scale_pmfsum_ptr     = std::log(histsize_new/(histsize_old + awh_bias->update_weight));
    }
    else
    {
        /* For all other cases, the scaling factor depends only on constant method parameters and the current
           histogram size. The scaling factor is then the same as that used when applying skipped updates.
           TODO: could improve on the function names (and organization?) here. */
                             ...
                             const gmx_multisim_t *ms, double t, gmx_int64_t step, int seed, FILE *fplog)
{
    ...
    else
    {
        ...
        /* The umbrella center is sampled as often as the coordinate.
