...
/* Updates the bias of a grid point. */
void PointState::updateBias()
{
    GMX_ASSERT(target > 0, "AWH target distribution must be > 0 to calculate the point bias.");

    bias_ = freeEnergy() + std::log(target);
}
 ...
 * global updates.
 */
void PointState::updateFreeEnergy(const BiasParams &params, double weightAtPoint)
{
    double weighthistSampled  = weightsumRef() + weightAtPoint;
    double weighthistTarget   = weightsumRef() + params.update_weight*target;

    double df                 = -std::log(weighthistSampled/weighthistTarget);
    freeEnergy_              += df;

    GMX_RELEASE_ASSERT(std::fabs(freeEnergy_) < c_largePositiveExponent,
...
void PointState::updateWeightHistogram(const BiasParams &params, double weightAtPoint, double scaleFactor)
{
    if (params.idealWeighthistUpdate)
    {
        /* Grow histogram using the target distribution. */
        weightsumRef_ += target*params.update_weight*params.localWeightScaling;
    }
    else
    {
 ...
 */
void PointState::update(const BiasParams &params,
 ...
 * them for each point
 */
bool PointState::updateSkipped(const BiasParams &params,
                               gmx_int64_t step, double weighthistScaling, double logPmfsumScaling)
{
    ...
    int last_update_index = step > 0 ? (step - 1)/params.nstupdate_free_energy : 0;
    int nupdates_skipped  = last_update_index - lastUpdateIndex_;
    ...
    lastUpdateIndex_ = last_update_index;
...
void PointState::updateNew(const BiasParams &params,
                           gmx_int64_t step, double weighthistScaling, double logPmfsumScaling)
{
    ...
    lastUpdateIndex_  = step/params.nstupdate_free_energy;
}
...
void PointState::normalizeFreeEnergyAndPmfSum(double minimumFreeEnergy)
{
    if (inTargetRegion())
    {
                       ...
                       FILE *fplog)
{
    ...
    state->gridpointIndex = get_closest_index_in_grid(state->coordValue, bias->grid.get());
    ...
    double convolvedBias;
    if (convolveForce || doAtStep(bias->params.nstsample_coord, step))
    {
        if (params.canSkipUpdates())
        {
    ...
    {
        /* Umbrella force */
        GMX_RELEASE_ASSERT(bias->pointState[state->refGridpoint].inTargetRegion(),
                           "AWH bias grid point reference value is outside of the target region.");
        potential =
            calcUmbrellaForceAndPotential(*bias, state->refGridpoint, biasForce);
...
real AwhBiasCollection::update(struct pull_t          *pull_work,
                               int                     ePBC,
                               const t_mdatoms        *mdatoms,
                               const matrix            box,
                               rvec                   *force,
