...
static int sampleReferenceCoordpoint(const AwhBias *awh_bias, gmx_int64_t step, int seed)
{
    ...
    int n_sampled  = get_sample_from_distribution(awh_bias->probWeightNeighbor, awh_bias->grid->point(awh_bias->state.coord_value_index).neighbor.size(),
...
double moveUmbrella(AwhBias *awh_bias, gmx_int64_t step, int seed)
{
    ...
    awh_bias->state.refCoordpoint = sampleReferenceCoordpoint(awh_bias, step, seed);
...
static bool isCovered(const AwhBias &awh_bias, const gmx_multisim_t *ms)
{
    ...
    for (int d = 0; d < grid->ndim(); d++)
    {
        labelCoveredPoints(checkDim[d].visited, checkDim[d].checkCovering, grid->axis(d).npoints, grid->axis(d).npoints_period,
                           awh_bias.state.coverRadius[d], &checkDim[d].covered);
    }
...
static void sampleProbabilityWeights(AwhBias *awh_bias)
{
    ...
    for (size_t n = 0; n < neighbor.size(); n++)
    {
        awh_bias->coordPoint[neighbor[n]].weightsum_iteration += awh_bias->probWeightNeighbor[n];
    }
                             ...
                             const gmx_multisim_t *ms, double t, gmx_int64_t step, int seed, FILE *fplog)
{
    ...
    if (convolveForce)
    {
        ...
        potential    =  -awh_bias->convolved_bias*params.invBeta;
    ...
    else
    {
        ...
        potential =
            calcUmbrellaForceAndPotential(*awh_bias, state->refCoordpoint, awh_bias->bias_force);
