            ...
            std::vector<float> *pmf)
{
    ...
    if (awh_bias.params.numSharedUpdate > 1)
    {
        ...
        gmx_sumf_sim(awh_bias.coordPoint.size(), pmf->data(), ms);
...
static double biasedWeightFromPoint(const AwhBias &awh_bias, int point_index, double pointBias, const awh_dvec value)
{
    ...
    if (awh_bias.coordPoint[point_index].inTargetRegion())
    {
        ...
        for (int d = 0; d < awh_bias.ndim; d++)
        {
            ...
            log_weight -= 0.5*awh_bias.dimParams[d].betak*dev*dev;
        }
...
{
    size_t             numPoints = awh_bias.grid.get()->numPoints();
    ...
    for (size_t m = 0; m < numPoints; m++)
    {
        ...
        const GridPoint &point             = awh_bias.grid.get()->point(m);
...
{
    const BiasParams &params = awh_bias->params;
    double            f_max  = 0;

    if (awh_bias->params.eTarget == eawhtargetCUTOFF)
    {
        f_max = freeEnergyMinimumValue(*awh_bias) + params.target_param;
    }
...
void initTargetAndBias(AwhBias *awh_bias)
{
    ...
    GMX_ASSERT((awh_bias->params.eTarget != eawhtargetLOCALBOLTZMANN) ||
               (awh_bias->params.eTarget == eawhtargetLOCALBOLTZMANN && awh_bias->coordPoint[0].weightsum_ref != 0),
 ...
 */
static void applyBiasForceToPullCoords(const AwhBias    &awhBias,
                                       ...
                                       tensor            virial)
{
    ...
    {
        apply_external_pull_coord_force(pull_work, awhBias.dimParams[d].pullCoordIndex,
...
static void checkOnData(const AwhBias &awh_bias, double t, gmx_int64_t step, FILE *fplog)
{
    ...
    const int          nstcheck_on_data       = std::max(params.nstupdate_free_energy,
                                                         (static_cast<int>(awh_bias.coordPoint.size())*100*params.nstsample_coord/params.nstupdate_free_energy)*params.nstupdate_free_energy);
    double             sum_W = 0, sum_visits = 0;
    double             inv_norm_visits, inv_norm_W;

    if (fplog == NULL || alltime_nwarnings >= max_alltime_nwarnings || awh_bias.state.in_initial ||
 ...
 */
static double calcUmbrellaForceAndPotential(const AwhBias &awh_bias, int point, awh_dvec force)
{
    double potential = 0;
    for (int d = 0; d < awh_bias.ndim; d++)
    {
        double dev = get_deviation_from_point_along_gridaxis(awh_bias.grid.get(), d, point, awh_bias.state.coord_value[d]);

        double k   = awh_bias.dimParams[d].k;
 ...
 */
static void calc_convolved_force(const AwhBias             &awh_bias,
                                 ...
                                 awh_dvec                   force)
{
    ...
    const std::vector<int> &neighbor = awh_bias.grid->point(awh_bias.state.coord_value_index).neighbor;
    for (size_t n = 0; n < neighbor.size(); n++)
    {
        ...
        calcUmbrellaForceAndPotential(awh_bias, indexNeighbor,
                                      forceFromNeighbor);
 ...
 */
static int sampleReferenceCoordpoint(const AwhBias *awh_bias,
                                     const std::vector<double> &probWeightNeighbor,
                                     gmx_int64_t step, int seed)
{
    ...
    int n_sampled  = get_sample_from_distribution(probWeightNeighbor, awh_bias->grid->point(awh_bias->state.coord_value_index).neighbor.size(),
                                                  step, seed, awh_bias->biasIndex);

    return awh_bias->grid->point(awh_bias->state.coord_value_index).neighbor[n_sampled];
}
 ...
 */
double moveUmbrella(AwhBias *awh_bias, const std::vector<double> &probWeightNeighbor,
                    awh_dvec biasForce,
                    gmx_int64_t step, int seed)
{
    /* Generate and set a new coordinate reference value */
    awh_bias->state.refCoordpoint = sampleReferenceCoordpoint(awh_bias, probWeightNeighbor, step, seed);

    awh_dvec newForce;
    double   newPotential =
        calcUmbrellaForceAndPotential(*awh_bias, awh_bias->state.refCoordpoint, newForce);
                                           ...
                                           double *weighthistScaling, double *logPmfsumScaling)
{
    ...
    *weighthistScaling = histsizeNew/(histsizeOld + awh_bias->params.update_weight*awh_bias->params.localWeightScaling);
    *logPmfsumScaling  = std::log(histsizeNew/(histsizeOld + awh_bias->params.update_weight));
}
...
{
    GMX_ASSERT(awh_bias->params.canSkipUpdates(), "Calling function for skipped updates when skipping updates is not allowed");

    if (awh_bias->state.in_initial)
    {
        ...
        setHistogramUpdateScaleFactors(awh_bias, awh_bias->state.histsize, awh_bias->state.histsize,
 ...
 * \param[in] params            AWH bias parameters.
 ...
 */
static bool update_point_skipped(CoordPoint *coordPoint, const BiasParams &params,
...
static void do_skipped_updates_for_all_points(AwhBias *awh_bias, gmx_int64_t step)
{
    ...
    for (auto &coordPoint : awh_bias->coordPoint)
    {
        bool bUpdated = update_point_skipped(&coordPoint, awh_bias->params, step, weighthistScaling, logPmfsumScaling);
...
static void do_skipped_updates_in_neighborhood(AwhBias *awh_bias, gmx_int64_t step)
{
    ...
    const std::vector<int> &neighbors = awh_bias->grid->point(awh_bias->state.coord_value_index).neighbor;
    for (auto &neighbor : neighbors)
    {
        bool bUpdated = update_point_skipped(&awh_bias->coordPoint[neighbor], awh_bias->params, step, weighthistScaling, logPmfsumScaling);
...
static void make_local_update_list(const AwhBias *awh_bias, const gmx_multisim_t *ms, std::vector<int> *updateList, int *nupdate_ptr)
{
    ...
    {
        origin_dim[d]  = awh_bias->state.origin_updatelist[d];
        npoints_dim[d] = awh_bias->state.end_updatelist[d] - awh_bias->state.origin_updatelist[d] + 1;
 ...
 * \param[in] logPmfsumScaling     Log of the scaling factor for the PMF histogram.
 */
static void update_point_new(CoordPoint *coordPoint, const BiasParams &params,
                             gmx_int64_t step, double weighthistScaling, double logPmfsumScaling)
{
...
static double newHistsizeInitialStage(AwhBias *awh_bias, double t, bool bCovered, FILE *fplog)
{
    ...
    double              scaleFactor = growthFactor/(1. + awh_bias->params.update_weight*awh_bias->params.localWeightScaling/state->histsize);

    bool                bExit        = (state->scaledSampleWeight - std::log(scaleFactor) <= prevMaxScaledSampleWeight);
    double              histsize_new = bExit ? state->histsize : state->histsize*growthFactor;
...
static bool isCovered(const AwhBias &awh_bias, const gmx_multisim_t *ms)
{
    ...
    if (awh_bias.params.eTarget == eawhtargetCUTOFF)
    {
        maxFreeEnergy = freeEnergyMinimumValue(awh_bias) + awh_bias.params.target_param;
    }
    ...
    for (int d = 0; d < awh_bias.ndim; d++)
    {
        weight_peak *= grid->axis(d).spacing*std::sqrt(awh_bias.dimParams[d].betak*0.5*M_1_PI);
    }
    ...
    for (int d = 0; d < grid->ndim(); d++)
    {
        labelCoveredPoints(checkDim[d].visited, checkDim[d].checkCovering, grid->axis(d).npoints, grid->axis(d).npoints_period,
                           awh_bias.params.coverRadius[d], &checkDim[d].covered);
    }
...
{
    if (!awh_bias.state.equilibrateHistogram)
    {
        return true;
    }
...
static double newHistsize(AwhBias *awh_bias, double t, bool covered, FILE *fplog)
{
    ...
    if (state->in_initial)
    {
        ...
        if (state->equilibrateHistogram && covered)
        {
            ...
            state->equilibrateHistogram = !histogramIsEquilibrated(*awh_bias);
    ...
    else
    {
        ...
        histsize_new = state->histsize + awh_bias->params.update_weight*awh_bias->params.localWeightScaling;
    }
...
static void update(AwhBias *awh_bias, const gmx_multisim_t *ms, double t, gmx_int64_t step, FILE *fplog)
{
    ...
    bool updateTarget = (params.eTarget != eawhtargetCONSTANT &&
    ...
    {
        /* In the initial stage, the histogram grows dynamically as a function of the number of coverings. */
        int nstcheck_covered = std::max((static_cast<int>(awh_bias->coordPoint.size())*params.nstsample_coord/params.nstupdate_free_energy)*params.nstupdate_free_energy,
    ...
    int nupdate;
    if (updateTarget || haveCovered || !params.canSkipUpdates() || needToNormalizeFreeEnergy)
    {
    ...
    setHistogramUpdateScaleFactors(awh_bias, histsize_new, awh_bias->state.histsize,
    ...
    for (int iupdate = 0; iupdate < nupdate; iupdate++)
    {
        ...
        if (params.canSkipUpdates())
        {
            update_point_skipped(coordPointToUpdate, params, step, weighthistScalingSkipped, logPmfsumScalingSkipped);
        }

        /* Now do an update with new sampling data. */
        update_point_new(coordPointToUpdate, params, step, weighthistScalingNew, logPmfsumScalingNew);
    ...
    awh_bias->state.histsize = histsize_new;
    ...
    awh_bias->state.scaledSampleWeight -= std::log(weighthistScalingNew);
                                                     ...
                                                     double              *convolvedBias)
{
    ...
    const std::vector<int> &neighbors = grid->point(awh_bias->state.coord_value_index).neighbor;
...
double calc_convolved_bias(const AwhBias &awh_bias, const awh_dvec coord_value)
{
    ...
    const GridPoint               &gridPoint  = awh_bias.grid->point(point);
    const std::vector<CoordPoint> &coordPoint = awh_bias.coordPoint;
                                     ...
                                     const std::vector<double> &probWeightNeighbor)
{
    ...
    for (size_t n = 0; n < neighbor.size(); n++)
    {
        awh_bias->coordPoint[neighbor[n]].weightsum_iteration += probWeightNeighbor[n];
    }
    ...
    for (int d = 0; d < awh_bias->grid->ndim(); d++)
    {
        ...
        awh_bias->state.origin_updatelist[d] = std::min(awh_bias->state.origin_updatelist[d], origin_d);
        awh_bias->state.end_updatelist[d]    = std::max(awh_bias->state.end_updatelist[d], end_d);
    }
}
 ...
 */
static void sample_pmf(AwhBias *awh_bias, double convolvedBias)
{
    /* Only save coordinate data that is in range (the given index is always
       in range even if the coordinate value is not). */
    bool        bIn_array  = value_is_in_grid(awh_bias->state.coord_value, awh_bias->grid.get());
    CoordPoint *coordPoint = &awh_bias->coordPoint[awh_bias->state.coord_value_index];
                        ...
                        double                     convolvedBias)
{
    ...
    sample_pmf(awh_bias, convolvedBias);

    /* Save probability weights for the update */
    sampleProbabilityWeights(awh_bias, probWeightNeighbor);
}
 ...
 * \param[in,out] fplog          Log file.
 */
static void doAwhBiasStep(AwhBias *awh_bias,
                          bool convolveForce,
                          ...
                          FILE *fplog)
{
    ...
    state->coord_value_index = get_closest_index_in_grid(state->coord_value, awh_bias->grid.get());
    ...
    double convolvedBias;
    if (convolveForce || do_at_step(awh_bias->params.nstsample_coord, step))
    {
        if (params.canSkipUpdates())
        {
        ...
        updateProbabilityWeightsAndConvolvedBias(awh_bias, probWeightNeighbor, &convolvedBias);

        if (step > 0 && do_at_step(params.nstsample_coord, step))
        {
            do_sampling(awh_bias, *probWeightNeighbor, convolvedBias);
        }
    }
    ...
    if (convolveForce)
    {
        calc_convolved_force(*awh_bias, *probWeightNeighbor, biasForce);
    ...
    {
        /* Umbrella force */
        GMX_RELEASE_ASSERT(awh_bias->coordPoint[state->refCoordpoint].inTargetRegion(),
                           "AWH bias coordinate reference value is outside of the target region.");
        potential =
            calcUmbrellaForceAndPotential(*awh_bias, state->refCoordpoint, biasForce);
        ...
        {
            newPotential = moveUmbrella(awh_bias, *probWeightNeighbor, biasForce, step, seed);
        }
        else
        {
    ...
    *awhPotential += potential;
    ...
    if (step > 0 && do_at_step(params.nstupdate_free_energy, step))
    {
        ...
        if (convolveForce)
        {
            ...
            newPotential = -calc_convolved_bias(*awh_bias, state->coord_value)*params.invBeta;
        }
    }
...
static void setAwhBiasCoordValue(AwhBias *awh_bias, struct pull_t *pull_work, const t_pbc *pbc)
{
    ...
    for (int d = 0; d < awh_bias->ndim; d++)
    {
        get_pull_coord_value(pull_work, awh_bias->dimParams[d].pullCoordIndex, pbc, &awh_bias->state.coord_value[d]);
    }
}
                ...
                FILE                   *fplog)
{
    ...
    for (auto &awhBias : awh->awhBias)
    {
         ...
         */
        applyBiasForceToPullCoords(awhBias, biasForce,
