...
namespace detail {
    ...
    struct HasTypeMember { typedef T type; };

    template<class T>
    struct HasTypeMember<T, typename void_t<typename T::type>::type> { typedef typename T::type type; };

    template<int N, typename T, typename Tuple>
    struct CompareField: disjunction<std::is_same<T, typename std::tuple_element<N, Tuple>::type>,
