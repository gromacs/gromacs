...
void set_lincs_matrix(struct gmx_lincsdata *li, real *invmass, real lambda)
{
    ...
    /* Construct the coupling coefficient matrix blmf */
    int th, ntriangle = 0, ncc_triangle = 0, ntriangle_cross_tasks = 0;
#pragma omp parallel for reduction(+: ntriangle, ncc_triangle, ntriangle_cross_tasks) num_threads(li->ntask) schedule(static)
    for (th = 0; th < li->ntask; th++)
    {
        set_lincs_matrix_task(li, &li->task[th], invmass,
                              &ncc_triangle, &ntriangle_cross_tasks);
        ntriangle = li->task[th].ntriangle;
    }
    li->ntriangle    = ntriangle;
    li->ncc_triangle = ncc_triangle;
    li->bTaskDepTri  = (ntriangle_cross_tasks > 0);

    if (debug)
    {
        fprintf(debug, "Of the %d constraints %d participate in triangles (cross thread tasks %d)\n",
                li->nc, li->ntriangle, ntriangle_cross_tasks);
