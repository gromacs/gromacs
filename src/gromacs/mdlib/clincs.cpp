...
typedef struct gmx_lincsdata {
    ...
    gmx_bool        bCrossTaskTriangles; /* are there triangle constraints that cross task borders? */
...
void set_lincs_matrix(struct gmx_lincsdata *li, real *invmass, real lambda)
{
    ...
    /* Construct the coupling coefficient matrix blmf */
    int th, ntriangle = 0, ncc_triangle = 0, nc_triangle_cross_task = 0;
#pragma omp parallel for reduction(+: ntriangle, ncc_triangle, nc_triangle_cross_task) num_threads(li->ntask) schedule(static)
    for (th = 0; th < li->ntask; th++)
    {
        set_lincs_matrix_task(li, &li->task[th], invmass,
                              &ncc_triangle, &nc_triangle_cross_task);
        ntriangle = li->task[th].ntriangle;
    }
    li->ntriangle           = ntriangle;
    li->ncc_triangle        = ncc_triangle;
    li->bCrossTaskTriangles = (nc_triangle_cross_task > 0);

    if (debug)
    {
        fprintf(debug, "Of the %d constraints %d participate in triangles (cross thread task %d)\n",
                li->nc, li->ntriangle, nc_triangle_cross_task);
