                                ...
                                real *rhs1, real *rhs2, real *sol)
{
    ...
    //const int *triangle = li_task->triangle, *tri_bits = li_task->tri_bits;

    b0        = li_task->b0;
    b1        = li_task->b1;
    //ntriangle = li_task->ntriangle;
    nrec      = lincsd->nOrder;
...
#pragma omp barrier
    ...
    // TODO: Implement indepedent triangle tasks
    //if (ntriangle > 0)
...
#pragma omp barrier
        ...
        // TODO: Remove when we have indepedent triangle tasks
#pragma omp master
        {
            for (rec = 0; rec < nrec; rec++)
            {
                // TODO: Remove this loop when we have indepedent triangle tasks
                int th;
                for (th = 0; th < lincsd->ntask; th++)
                {
                const lincs_task_t *li_task = &lincsd->task[th];
                int tb;
                ...
                for (tb = 0; tb < li_task->ntriangle; tb++)
                {
                    ...
                    b    = li_task->triangle[tb];
                    bits = li_task->tri_bits[tb];
                    mvb  = 0;
                    nr0  = blnr[b];
                    nr1  = blnr[b+1];
                    for (n = nr0; n < nr1; n++)
               ...
               struct gmx_lincsdata *li)
{
     ...
     */
    int nc_target, con, th;

    nc_target = (ncon_tot + li->ntask - 1)/li->ntask;
    ...
    for (th = 0; th < li->ntask; th++)
    {
        ...
        /* Continue filling the arrays where we left of with the previous task,
         * including padding for SIMD.
         */
        li_task->b0 = li->nc;

        while (con < ncon_tot && li->nc - li_task->b0 < nc_target)
        {
            if (li->con_index[con] == -1)
            {
                ...
                if (bDynamics || lenA != 0 || lenB != 0)
                {
                    ...
                    // TODO: group constraints involved in a triangle into the same task

                    if (li->ntask > 1 && !li->bTaskDep)
                    {
                        /* We can generate independent tasks.
                         * Currently this only supports constraint groups
...
static void cconerr(gmx_domdec_t *dd,
                    const struct gmx_lincsdata *lincsd,
                    rvec *x, t_pbc *pbc,
                         ...
                         int maxwarn, int *warncount)
{
    ...
    if (econq == econqCoord)
    {
        ...
        if (bLog && fplog)
        {
            cconerr(cr->dd, lincsd, xprime, pbc,
