...
static gmx_inline void gmx_simdcall
rvec_dist_to_simd_gather(const real      *x,
                         int              is,
                         const int       *at_ind0,
                         const int       *at_ind1,
                         gmx_simd_real_t *dx,
                         ...
                         gmx_simd_real_t *dz)
{
    __m256i i0, i1, one = _mm256_set1_epi32(1);

    i0 = gmx_simd_load_fi((at_ind0 + is));
    i1 = gmx_simd_load_fi((at_ind1 + is));

    {
        gmx_simd_real_t x0, x1;

        x0  = _mm256_i32gather_ps(x, i0, 4);
        x1  = _mm256_i32gather_ps(x, i1, 4);
        *dx = gmx_simd_sub_r(x0, x1);
    }

    {
        gmx_simd_real_t x0, x1;

        i0  = _mm256_add_epi32(i0, one);
        i1  = _mm256_add_epi32(i1, one);

        x0  = _mm256_i32gather_ps(x, i0, 4);
        x1  = _mm256_i32gather_ps(x, i1, 4);
        *dy = gmx_simd_sub_r(x0, x1);
    }
                 ...
                 real * gmx_restrict       sol)
{
    ...
    for (bs = b0; bs < b1; bs += GMX_SIMD_REAL_WIDTH)
    {
        ...
        rvec_dist_to_simd_gather(x[0], bs, bla_s0, bla_s1, &rx_S, &ry_S, &rz_S);
        ...
        rvec_dist_to_simd_gather(f[0], bs, bla_s0, bla_s1, &fx_S, &fy_S, &fz_S);
                      ...
                      gmx_bool bCalcVir, tensor rmdf)
{
     ...
     */
    calc_dr_x_f_simd(b0, b1, bla, lincsd->bla_s0, lincsd->bla_s1, x, f, blc,
                     &pbc_simd,
                  ...
                  real * gmx_restrict       sol)
{
    ...
    for (bs = b0; bs < b1; bs += GMX_SIMD_REAL_WIDTH)
    {
        ...
        rvec_dist_to_simd_gather(xp[0], bs, bla_s0, bla_s1, &rxp_S, &ryp_S, &rzp_S);
                     ...
                     gmx_bool bCalcVir, tensor vir_r_m_dr)
{
     ...
     * Compute the inner product of r and xp i-j and store in rhs1.
     */
    calc_dr_x_xp_simd(b0, b1, bla, lincsd->bla_s0, lincsd->bla_s1,
                      x, xp, bllen, blc,
                      &pbc_simd,
                      lincsd->th[th].simd_buf,
                      lincsd->th[th].simd_buf + GMX_SIMD_REAL_WIDTH*DIM,
               ...
               struct gmx_lincsdata *li)
{
    ...
    if (ncon_tot + simd_width > li->nc_alloc || li->nc_alloc == 0)
    {
        ...
        sfree(li->bla_s0);
        sfree(li->bla_s1);
        snew_aligned(li->bla_s0, li->nc_alloc, align_bytes);
        snew_aligned(li->bla_s1, li->nc_alloc, align_bytes);
    ...
    for (i = 0; i < ncon_tot; i++)
    {
        ...
        if (bDynamics || lenA != 0 || lenB != 0)
        {
            ...
            li->bla[2*con]   = a1;
            li->bla[2*con+1] = a2;
            li->bla_s0[con]  = a1*DIM;
            li->bla_s1[con]  = a2*DIM;
    ...
    if (simd_width > 1)
    {
        ...
        for (i = con; i < con_roundup; i++)
        {
            ...
            li->bla_s0[i]    = li->bla_s0[con - 1];
            li->bla_s1[i]    = li->bla_s1[con - 1];
