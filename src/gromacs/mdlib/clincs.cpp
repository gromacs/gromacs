...
void set_lincs_matrix(struct gmx_lincsdata *li, real *invmass, real lambda)
{
    ...
    // TODO: Convert to STS with reductions for three "triangle" variables
    for (int th = 0; th < li->ntask; th++)
    {
        try
        {
            int tmp_ncc_triangle = 0, tmp_nCrossTaskTriangles = 0;
            set_lincs_matrix_task(li, &li->task[th], invmass,
                                  &tmp_ncc_triangle, &tmp_nCrossTaskTriangles);
            ncc_triangle += tmp_ncc_triangle;
            nCrossTaskTriangles += tmp_nCrossTaskTriangles;
                         ...
                         int maxwarn, int *warncount)
{
    ...
    if (econq == econqCoord)
    {
        ...
        STS::getInstance("constrain")->parallel_for("lincs_loop", 0, lincsd->ntask, [&](int th)
        {
            try
            {
