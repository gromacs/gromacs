               ...
               struct gmx_lincsdata *li)
{
    ...
    for (th = 0; th < li->ntask; th++)
    {
         ...
         * constraints per task will often exceed nc_target.
         * Triangle constraints can also increase the count, but there are
         * relatively few of those, so we usually expect to get nc_target.
         */
        if (li->bTaskDep)
        {
            /* Increase nc_target to a multiple of GMX_SIMD_WIDTH,
             * since otherwise a lot of operations can be wasted.
             * There are several ways to round here, we choose the one
             * that alternates block size, which helps with Intel HT.
             */
            nc_target = ((ncon_tot*(th + 1))/li->ntask - li->nc_real + GMX_SIMD_REAL_WIDTH - 1) & ~(GMX_SIMD_REAL_WIDTH - 1);
        }
#endif
    ...
    assert(li->nc_real + (bDynamics ? 0 : nflexcon) == ncon_tot);
