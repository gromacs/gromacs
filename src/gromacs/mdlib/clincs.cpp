...
static gmx_inline void
copy_simd_vec_to_rvec(const gmx_simd_real_t  x,
                      const gmx_simd_real_t  y,
                      const gmx_simd_real_t  z,
                      real                  *buf,
                      rvec                  *v,
                      int                    is)
{
 ...
 */
static void calc_dr_x_f_simd(int b0, int b1,
                             const int *bla,
...
static void calc_dist_iter_simd(int                       b0,
                                int                       b1,
                                const int *               bla,
                                const rvec * gmx_restrict x,
                                const real * gmx_restrict bllen,
                                ...
                                gmx_bool *                bWarn)
{
    ...
    *bWarn = gmx_simd_anytrue_b(warn_S);
}
#endif /* GMX_SIMD_HAVE_REAL */
               ...
               struct gmx_lincsdata *li)
{
    ...
    if (simd_width > 1)
    {
        /* Copy the last atom pair indices up to a multiple of simd_width,
         * such that we can do all simd operations without have to worry
         * about end effects.
         */
        int con_roundup, i;
