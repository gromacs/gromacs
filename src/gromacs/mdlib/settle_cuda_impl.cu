                              ...
                              float*                               virialScaled)
{
    ...
    if (blockIdx.x*blockDim.x + threadIdx.x < nSettle)
    {
        int3            idxes = settles[blockIdx.x*blockDim.x + threadIdx.x];
        ...
        dist21 = pbcDxAiuc(pbcAiuc, x_hw2, x_ow1);
        dist31 = pbcDxAiuc(pbcAiuc, x_hw3, x_ow1);
        doh2   = pbcDxAiuc(pbcAiuc, xprime_hw2, xprime_ow1);

        sh_hw2     = xprime_hw2 - (xprime_ow1 + doh2);
        xprime_hw2 = xprime_hw2 - sh_hw2;

        doh3 =   pbcDxAiuc(pbcAiuc, xprime_hw3, xprime_ow1);
        ...
        if (updateVelocities || computeVirial)
        {
            ...
            if (computeVirial)
            {
                ...
                threadVirial[0*blockDim.x + threadIdx.x] = -(x_ow1.x*mdo.x + dist21.x*mdb.x + dist31.x*mdc.x);
                threadVirial[1*blockDim.x + threadIdx.x] = -(x_ow1.x*mdo.y + dist21.x*mdb.y + dist31.x*mdc.y);
                threadVirial[2*blockDim.x + threadIdx.x] = -(x_ow1.x*mdo.z + dist21.x*mdb.z + dist31.x*mdc.z);
                threadVirial[3*blockDim.x + threadIdx.x] = -(x_ow1.y*mdo.y + dist21.y*mdb.y + dist31.y*mdc.y);
                threadVirial[4*blockDim.x + threadIdx.x] = -(x_ow1.y*mdo.z + dist21.y*mdb.z + dist31.y*mdc.z);
                threadVirial[5*blockDim.x + threadIdx.x] = -(x_ow1.z*mdo.z + dist21.z*mdb.z + dist31.z*mdc.z);
                ...
                for (int divideBy = 2; divideBy <= blockDim.x; divideBy *= 2)
                {
                    ...
                    if (threadIdx.x < dividedAt && blockIdx.x*blockDim.x + threadIdx.x + dividedAt < nSettle)
                    {
                        for (int d = 0; d < 6; d++)
                        {
                            threadVirial[d*blockDim.x + threadIdx.x] += threadVirial[d*blockDim.x + (threadIdx.x + dividedAt)];
                        }
                    }
                    __syncthreads();
                }
                             ...
                             tensor           virialScaled)
{
    ...
    config.gridSize[0]      = (nSettle_ + c_threadsPerBlock - 1)/c_threadsPerBlock;
    config.gridSize[1]      = 1;
    config.gridSize[2]      = 1;
    config.sharedMemorySize = c_threadsPerBlock*6*sizeof(float);
    config.stream           = stream_;

    const int3    *atomIdsDevicePtr                  = atomIdsDevice_;
    ...
    /*kernelPtr<<< (nSettle_ + c_threadsPerBlock - 1)/c_threadsPerBlock, c_threadsPerBlock, 0, stream_ >>>
       (nSettle_,
       atomIdsDevice_,
       settleParameters_,
       xDevice_,
    ...
    CU_RET_ERR(stat, "In CUDA version SETTLE");
    ...
    : nAtom_(nAtom)
{
    ...
    for (unsigned mt = 0; mt < mtop.moltype.size(); mt++)
    {
        ...
        for (unsigned i = 0; i < iatoms.size()/nral1; i++)
        {
            ...
            //static_assert(mO == ow1.m, "Topology has different values for oxygen mass. Should be identical in order to use SETTLE.");
            //static_assert(hw2.m == hw3.m && hw2.m == mH, "Topology has different values for hydrogen mass. Should be identical in order to use SETTLE.");
        }
    }

    //static_assert(mO > 0, "Could not find oxygen mass in the topology. Needed in SETTLE.");
    //static_assert(mH > 0, "Could not find hydrogen mass in the topology. Needed in SETTLE.");
    ...
    //static_assert(settle_type >= 0, "settle_init called without settles");
    ...
    cudaError_t stat = cudaGetLastError();
    CU_RET_ERR(stat, "While CUDA version of SETTLE was initialized.");
                           ...
                           const t_mdatoms gmx_unused &md)
{
    ...
    nSettle_   = il_settle.nr/nral1;
    if ((int)atomIdsHost_.size() < nSettle_)
    {
        if (atomIdsHost_.size() != 0)
        {
            freeDeviceBuffer(&atomIdsDevice_);
        }
        atomIdsHost_.resize(nSettle_);
        allocateDeviceBuffer(&atomIdsDevice_, nSettle_, nullptr);
    }
    for (int i = 0; i < nSettle_; i++)
    {
        int3 settler;
        settler.x          = iatoms[i*nral1 + 1]; // Oxygen index
        settler.y          = iatoms[i*nral1 + 2]; // First hydrogen index
        settler.z          = iatoms[i*nral1 + 3]; // Second hydrogen index
        atomIdsHost_.at(i) = settler;
    }
    copyToDeviceBuffer(&atomIdsDevice_, atomIdsHost_.data(),
                       0, nSettle_,
                       stream_, GpuApiCallBehavior::Sync, nullptr);

    cudaError_t stat = cudaGetLastError();
    CU_RET_ERR(stat, "While constraints were set in CUDA version of SETTLE.");
}
...
void SettleCuda::Impl::copyCoordinatesToGpu(const rvec * x, const rvec * xp)
{
    ...
    cudaError_t stat = cudaGetLastError();
    CU_RET_ERR(stat, "While coordinates were copied to GPU in CUDA version of SETTLE.");
}
...
void SettleCuda::Impl::copyVelocitiesToGpu(const rvec * v)
{
    if (v != nullptr)
    {
        ...
        cudaError_t stat = cudaGetLastError();
        CU_RET_ERR(stat, "While velocities were copied to GPU in CUDA version of SETTLE.");
    }
}
...
void SettleCuda::Impl::copyCoordinatesFromGpu(rvec * xp)
{
    ...
    cudaError_t stat = cudaGetLastError();
    CU_RET_ERR(stat, "While coordinates were copied from GPU in CUDA version of SETTLE.");
}
...
void SettleCuda::Impl::copyVelocitiesFromGpu(rvec * v)
{
    ...
    cudaError_t stat = cudaGetLastError();
    CU_RET_ERR(stat, "While velocities were copied from GPU in CUDA version of SETTLE.");
}
