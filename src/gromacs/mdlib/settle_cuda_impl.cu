 ...
 *
 * Each thread correspond to a single constraints triangle (i.e. single water molecule).
 *
 * See original CPU version in settle.cpp
 *
 * \param [in]      nSettle          Number of constraints triangles (water molecules).
                              ...
                              float*   __restrict__                gm_virialScaled)
{
    ...
    if (blockIdx.x*blockDim.x + threadIdx.x < nSettle)
    {
        ...
        // \todo Can be reduced to one integer if atoms are consecutive in memory.
        int3            idxes = gm_settles[blockIdx.x*blockDim.x + threadIdx.x];
        ...
        xprime_ow1 = gm_xprime[idxes.x];
        xprime_hw2 = gm_xprime[idxes.y];
        xprime_hw3 = gm_xprime[idxes.z];
        ...
        gm_xprime[idxes.x]   = xprime_ow1;
        gm_xprime[idxes.y]   = xprime_hw2;
        gm_xprime[idxes.z]   = xprime_hw3;
    ...
    //\todo what follows should be separated out as a standard virial reduction subroutine
    if (computeVirial)
    {
    ...
    : nAtom_(nAtom)
{
    ...
    allocateDeviceBuffer(&d_x_, nAtom*DIM, nullptr);
    allocateDeviceBuffer(&d_xp_, nAtom*DIM, nullptr);
    allocateDeviceBuffer(&d_v_, nAtom*DIM, nullptr);
                           ...
                           const t_mdatoms gmx_unused &md)
{
    ...
    if ((int)h_atomIds_.size() < nSettle_)
    {
        if (h_atomIds_.size() != 0)
        {
            freeDeviceBuffer(&d_atomIds_);
        }
        h_atomIds_.resize(nSettle_);
        allocateDeviceBuffer(&d_atomIds_, nSettle_, nullptr);
    }
...
void SettleCuda::Impl::copyCoordinatesToGpu(const rvec * h_x, const rvec * h_xp)
{
    ...
    copyToDeviceBuffer(&d_xp_, (float3*)h_xp, 0, nAtom_, stream_, GpuApiCallBehavior::Sync, nullptr);
    cudaError_t stat = cudaGetLastError();
