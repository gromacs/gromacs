                             ...
                             tensor           virialScaled)
{
    ...
    int blockCount = (nSettle_ + blockSize - 1)/blockSize;
    ...
    kernelPtr<<< blockCount, blockSize, 0, stream_ >>>
    (nSettle_,
     atomIdsDevice_,
     settleParameters_,
     xDevice_,
    ...
    : nAtom_(nAtom)
{
    ...
    cudaMalloc(&xDevice_, nAtom_*DIM*sizeof(float));
    cudaMalloc(&xpDevice_, nAtom_*DIM*sizeof(float));
    cudaMalloc(&vDevice_, nAtom_*DIM*sizeof(float));
                           ...
                           const t_mdatoms gmx_unused &md)
{
    ...
    if ((int)atomIdsHost_.size() < nSettle_)
    {
        ...
        {
            cudaFree(atomIdsDevice_);
            cudaCheckError();
        }
        atomIdsHost_.resize(nSettle_);
        cudaMalloc(&atomIdsDevice_, nSettle_*sizeof(int3));
        cudaCheckError();
    }
    ...
    cudaMemcpy(atomIdsDevice_, atomIdsHost_.data(), nSettle_*sizeof(int3), cudaMemcpyHostToDevice);
    cudaCheckError();
}
