                     ...
                     int                         *numFlexibleConstraints)
{
    ...
    for (int ftype = F_CONSTR; ftype <= F_CONSTRNC; ftype++)
    {
        ...
        {
            bool bFlexCon = (flexibleConstraintTreatment != FlexibleConstraintTreatment::NoFlexibleConstraintsPresent &&
                             isConstraintFlexible(iparams, ilist.iatoms[i]));
            if (bFlexCon)
            {
                numFlexCon++;
            }
            if (flexibleConstraintTreatment == FlexibleConstraintTreatment::Include || !bFlexCon)
            {
                for (int j = 1; j < 3; j++)
                {
