...
typedef struct gmx_constr {
    ...
    bool              *bSettleError;  /* Thread local working data          */
                ...
                for (th = 0; th < nth; th++)
                {
                    try
                    {
                        ...
                        csettle(constr->settled,
                                ...
                                th == 0 ? &bSettleError : &constr->bSettleError[th]);
                    }
...
#pragma omp parallel for num_threads(nth) schedule(static)
        ...
        if (econq == econqCoord)
        {
            for (int th = 1; th < nth; th++)
            {
                bSettleError = bSettleError || constr->bSettleError[th];
            }
            ...
            if (bSettleError)
            {
                ...
                gmx_fatal(FARGS,
                          "\nstep " "%" GMX_PRId64 ": A water molecule can not be settled.\n"
                              ...
                              t_commrec *cr)
{
    ...
    if (nsettles > 0)
    {
        ...
        if (nthreads > 1 && constr->vir_r_m_dr_th == NULL)
        {
            ...
            snew(constr->bSettleError, nthreads);
        }
    }
