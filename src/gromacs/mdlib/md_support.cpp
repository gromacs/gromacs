                                   ...
                                   gmx_int64_t           value)
{
    ...
    std::vector<gmx_int64_t> buf(ms->nsim);

    /* send our value to all other master ranks, receive all of theirs */
    buf[ms->sim] = value;
    gmx_sumli_sim(ms->nsim, &buf[0], ms);
                     ...
                     gmx_bool *bSumEkinhOld, int flags)
{
    ...
    if (bTemp || bStopCM || bPres || bEner || bConstrain)
    {
        ...
        {
            std::vector<real> signalBuffer = prepareSignalBuffer(gs, bDoSignalling);
            if (PAR(cr))
            {
                wallcycle_start(wcycle, ewcMoveE);
                global_stat(fplog, gstat, cr, enerd, force_vir, shake_vir, mu_tot,
                            ir, ekind, constr, bStopCM ? vcm : NULL,
                            signalBuffer.size(), &signalBuffer[0],
                            top_global, state,
                            *bSumEkinhOld, flags);
                wallcycle_stop(wcycle, ewcMoveE);
            }
            handleSignals(gs, cr, bIntraSimSignal, bInterSimSignal, &signalBuffer);
            *bSumEkinhOld = FALSE;
        }
    }
                        ...
                        int nstglobalcomm, t_inputrec *ir)
{
    ...
    if (nstglobalcomm == -1)
    {
        ...
        else
        {
             ...
             *
             * TODO Does nstlist matter for the Verlet scheme? Can we
             * remove it when we remove the group scheme?
             */
