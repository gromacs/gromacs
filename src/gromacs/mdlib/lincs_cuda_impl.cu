                             ...
                             float                *virialScaled)
{
    ...
    if (c < nCons)
    {
        ...
        if (i != -1)
        {
            ...
            {
                int    index = n*nCons + c;
                int    c1    = coupledConstraintsIdxes[index]; //TODO Can be moved to local/shared memory
                            ...
                            tensor      virialScaled)
{
    ...
    // TODO Figure out how to satisfy prepareGpuKernelArguments(...) without introducing
    //      these intermediates. For instance, arguments may be hidden into a struct.
