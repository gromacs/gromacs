...
__global__ void lincs_kernel(const int             nCons,
                             ...
                             const real           *constraintsTargetLengths,
                             ...
                             real                 *virialScaled)
{
    ...
    if (c < nCons)
    {
        ...
        {
            // Collecting data
            float targetLength    = constraintsTargetLengths[c];
            ...
            if (computeVirial)
            {
                ...
                for (int divideBy = 2; divideBy <= blockDim.x; divideBy *= 2)
                {
                    ...
                    if (c < dividedAt && constraints[c + dividedAt].x != -1)
                    {
                        for (int d1 = 0; d1 < DIM; d1++)
                        {
                            for (int d2 = 0; d2 <= d1; d2++)
                            ...
                            tensor      virialScaled)
{
    ...
    /*const auto kernelArgs = prepareGpuKernelArguments(kernelPtr, config,
            ...
            &xDevice_, &xpDevice_,
            &nIter_, &nOrder_,
            &constraintsDevice_, &constraintsTargetLengthsDevice_,
    ...
    : nAtom_(nAtom), nIter_(nIter), nOrder_(nOrder)
{
    ...
    allocateDeviceBuffer(&virialScaledDevice_, DIM*DIM, nullptr);
