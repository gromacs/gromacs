                             ...
                             const float                 invdt)
{
    ...
    {
        // Making sure that all sm_rhs are saved before they are accessed below
        __syncthreads();
        float mvb = 0.0f;
    ...
    if (computeVirial)
    {
        ...
        // Dummy threads will have zeroes in their virial: targetLength, lagrangeScaled and rc are
        // all set to zero for them in the beggining of the kernel.
        // The sm_threadVirial will overlap with the sm_r and sm_rhs, but the latter are no longer in use.
                          ...
                          const t_mdatoms &md)
{
    ...
    coupledConstraintsIdxesHost.resize(maxCoupledConstraints*kernelParams_.numConstraintsThreads, -1);
    ...
    if (numConstraints > maxConstraintsNumberSoFar_)
    {
        ...
        allocateDeviceBuffer(&kernelParams_.d_coupledConstraintsIdxes, maxCoupledConstraints*kernelParams_.numConstraintsThreads, nullptr);
