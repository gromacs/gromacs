...
#include "gromacs/mdlib/lincs_cuda_impl.h"
 ...
 * \param[in]     updateVelocities          If the velocities should be computed (\todo template this).
 * \param[in,out] v                         Velocities to update.
 * \param[in]     invdt                     Inverse timestep (neede to update velocities).
 */
                            ...
                            tensor           virialScaled)
{
    ...
    // This can be set in the set() method, keeping all values as a fields of the class.
                          ...
                          const t_mdatoms &md)
{
    ...
    // as a one-dimentional array to ensure good memory alignment. It is adressed as [c + i*nConstraintsThreads],
    ...
    coupledConstraintsIdxesHost.resize(maxSpaceNeeded*nConstraintsThreads, -1);
    massFactorsHost.resize(maxSpaceNeeded*nConstraintsThreads, -1);
    ...
    if (nConstraint > maxConstraintsNumberSoFar)
    {
        ...
        cudaMalloc(&coupledConstraintsIdxesDevice, maxSpaceNeeded*nConstraintsThreads*sizeof(int));
        cudaMalloc(&massFactorsDevice, maxSpaceNeeded*nConstraintsThreads*sizeof(float));
        cudaMalloc(&matrixADevice, maxSpaceNeeded*nConstraintsThreads*sizeof(float));
    ...
    cudaMemcpy(coupledConstraintsIdxesDevice, coupledConstraintsIdxesHost.data(),
               maxSpaceNeeded*nConstraintsThreads*sizeof(int), cudaMemcpyHostToDevice);
    cudaMemcpy(massFactorsDevice, massFactorsHost.data(), maxSpaceNeeded*nConstraintsThreads*sizeof(float), cudaMemcpyHostToDevice);
