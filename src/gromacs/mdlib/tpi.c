              ...
              gmx_walltime_accounting_t walltime_accounting)
{
    ...
    gmx_int64_t     nsteps, stepblocksize = 0, step, key_mod, key;
    gmx_int64_t     seed;
    double          rnd[4];
    int             i, start, end;
    FILE           *fp_tpi = NULL;
    ...
    /* We use the frame step number as one random key.
     * The second key use the insertion (step) count. But we need
     * multiple random numbers per insertion. This numbers is not
     * fixed, since we generate random locations in a sphere by
     * putting locations in a cube and some of these fail.
    ...
    while (bNotLastFrame)
    {
        ...
        if (frame_step <= frame_step_prev)
        {
             ...
             * the step numbers as a key for random numbers.
             */
        ...
        while (step < nsteps)
        {
            /* Initialize the second key for random numbers using
            ...
            if (!bCavity)
            {
                ...
                if (bNS)
                {
                    ...
                    gmx_rng_cycle_2uniform(seed, RND_SEED_TPI, frame_step, key++, rnd);
                    gmx_rng_cycle_2uniform(seed, RND_SEED_TPI, frame_step, key++, rnd+2);
                    for (d = 0; d < DIM; d++)
                    {
