                       ...
                       float      *cycles_pme)
{
    int         i, j, status;
    int         donb_flags;
    gmx_bool    bDoEpot, bSepDVDL, bSB, bHaveDoneElPME = FALSE;
    int         pme_flags;
    matrix      boxs;
    rvec        box_size;
    real        Vsr, Vlr_q = 0, Vlr_lj = 0, Vcorr_q = 0, Vcorr_lj = 0;
    t_pbc       pbc;
    real        dvdgb;
    char        buf[22];
...
#define TAKETIME FALSE
    ...
    if (ir->nwall)
    {
        ...
        dvdl_lj = do_walls(ir, fr, box, md, x, f, lambda[efptVDW],
                           enerd->grpp.ener[egLJSR], nrnb);
        PRINT_SEPDVDL("Walls", 0.0, dvdl_lj);
...
#pragma omp parallel for num_threads(nthreads) schedule(static)
                for (t = 0; t < nthreads; t++)
                {
                    ...
                    rvec   *fnv;
                    tensor *vir_q, *vir_lj;
                    real   *Vcorrt_q, *Vcorrt_lj, *dvdl_qt, *dvdl_ljt;
                    if (t == 0)
                    {
                        fnv       = fr->f_novirsum;
                    ...
                    else
                    {
                        ...
                        dvdl_qt   = &fr->f_t[t].dvdl[efptCOUL];
                        dvdl_ljt  = &fr->f_t[t].dvdl[efptVDW];
                ...
                if (nthreads > 1)
                {
                    reduce_thread_forces(fr->natoms_force, fr->f_novirsum,
                                         ...
                                         &Vcorr_q, &Vcorr_lj, &dvdl_q, &dvdl_lj,
                                         nthreads, fr->f_t);
                }
            ...
            if (fr->n_tpi == 0)
            {
                Vcorr_q += ewald_charge_correction(cr, fr, lambda[efptCOUL], box,
                                                   &dvdl_q, fr->vir_el_recip);
            }

            PRINT_SEPDVDL("Ewald excl./charge/dip. corr.", Vcorr_q, dvdl_q);
            PRINT_SEPDVDL("Ewald excl. corr. LJ", Vcorr_lj, dvdl_q);
            enerd->dvdl_lin[efptCOUL] += dvdl_q;
            enerd->dvdl_lin[efptVDW]  += dvdl_q;
        }
        ...
        if ((EEL_PME(fr->eeltype) || EVDW_PME(fr->vdwtype)))
        {
            if (cr->duty & DUTY_PME)
            {
                ...
                PRINT_SEPDVDL("PME mesh", Vlr_q + Vlr_lj, dvdl_q+dvdl_lj);
            }
        }

        if (!bHaveDoneElPME && EEL_EWALD(fr->eeltype))
        {
            Vlr_q = do_ewald(ir, x, fr->f_novirsum,
                             md->chargeA, md->chargeB,
                             box_size, cr, md->homenr,
                             fr->vir_el_recip, fr->ewaldcoeff_q,
                             lambda[efptCOUL], &dvdl_q, fr->ewald_table);
            PRINT_SEPDVDL("Ewald long-range", Vlr_q, dvdl_q);
        }
        else if (!bHaveDoneElPME)
        {
            ...
            if (ir->cutoff_scheme != ecutsVERLET && fr->eeltype != eelRF_NEC)
            {
                ...
                enerd->term[F_RF_EXCL] =
                    RF_excl_correction(fr, graph, md, excl, x, f,
                                       fr->fshift, &pbc, lambda[efptCOUL], &dvdl_q);
            }

            enerd->dvdl_lin[efptCOUL] += dvdl_q;
            PRINT_SEPDVDL("RF exclusion correction",
                          enerd->term[F_RF_EXCL], dvdl_q);
        }
    }
    where();
    debug_gmx();
