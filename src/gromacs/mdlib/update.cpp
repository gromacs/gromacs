 ...
 * \param[in]    nstpcouple          Temperature coupling interval in steps
                         ...
                         const matrix M)
{
    ...
    bool doPressureCouple   = isPressureCouplingStep(step, ir);
    bool doNoseHoover       = (doTempCouple && ir->etc == etcNOSEHOOVER);
    bool doParrinelloRahman = (doTempCouple && ir->epc == epcPARRINELLORAHMAN);

    real dtPressureCouple   = (doPressureCouple ? ir->nstpcouple*dt : 0);

    if (doNoseHoover || ((doTempCouple || doNoseHoover) &&
                         (ekind->bNEMD || ekind->cosacc.cos_accel != 0)))
    {
        if (!doTempCouple && !doNoseHoover)
        {
            // If we don't scale x and v, we can ignore acceleration
        ...
        {
            // Avoid scaling the acceleration group velocities
