...
/*! \brief Sets if to apply Parrinello-Rahman pressure scaling */
enum class DoParrinelloRahman
{
    no, yes
};

/*! \brief Integrate using leap-frog with T-scaling and optionally Parrinello-Rahman p-coupling
 *
...
static void
updateMdLeapfrogSimple(int start, int nrend,
                       real dt, real dtPressureCouple,
                       const rvec * gmx_restrict invMassPerDim,
                       const t_grp_tcstat *tcstat, const unsigned short *cTC,
                       const matrix M,
                       const rvec * gmx_restrict x, rvec * gmx_restrict xprime,
                       rvec * gmx_restrict v, const rvec * gmx_restrict f)
{
    real lambdaGroup;
    ...
    for (int a = start; a < nrend; a++)
    {
        ...
        rvec dvPR;
        if (doParrinelloRahman == DoParrinelloRahman::yes)
        {
            // We update v in the loop below, so we need to pre-calculate
            // the whole vector here.
            for (int d = 0; d < DIM; d++)
            {
                dvPR[d] = dtPressureCouple*iprod(M[d], v[a]);
            }
        }

        for (int d = 0; d < DIM; d++)
        {
            real vNew = lambdaGroup*v[a][d] + dt*f[a][d]*invMassPerDim[a][d];
...
static void
updateMdLeapfrogGeneral(int start, int nrend,
                        bool doNoseHoover,
                        real dt, real dtPressureCouple,
                        const t_inputrec *ir,
                        const t_mdatoms *md,
...
#pragma warning( disable : 280 )
        ...
        switch (accelerationType)
        {
            ...
            case AccelerationType::cosine:
                copy_rvec(v[n], vrel);
                vrel[XX] -= std::cos(x[n][ZZ]*M_PI/box[ZZ][ZZ])*ekind->cosacc.vcos;
                break;
        }
        ...
        {
            real vNew =
                (lg*vrel[d] + (dt*invMassPerDim[n][d]*f[n][d] - factorNH*vrel[d]
                               - dtPressureCouple*iprod(M[d], vrel)))/(1 + factorNH);
            if (accelerationType == AccelerationType::group)
            {
...
static void do_update_md(int start, int nrend,
                         ...
                         const matrix M)
{
    GMX_ASSERT(xprime != x, "For SIMD optimization certain compilers need to have xprime != x");
    ...
    /* With acceleration groups, we accelarate in updateMdLeapFrogGeneral().
     * Cosine acceleration is applied elsewhere, but we should not T- or P-scale
     * the cosine velocity profile.
     */
    bool handleAcceleration = (ekind->bNEMD || (ekind->cosacc.cos_accel != 0 &&
                                                (doTempCouple  || doParrinelloRahman)));

    if (doNoseHoover || handleAcceleration)
    {
        ...
        {
            // Apply acceleration, avoid scaling acceleration group velocities
        ...
        {
            // Scale velocities relative to the cosine profile
    ...
    else
    {
        ...
        if (doParrinelloRahman)
        {
            if (haveSingleTempScaleValue)
            {
                ...
                <NumTempScaleValues::single, DoParrinelloRahman::yes>
                    (start, nrend, dt, dtPressureCouple,
                    invMassPerDim, tcstat, cTC, M, x, xprime, v, f);
            }
            else
            {
                updateMdLeapfrogSimple
                <NumTempScaleValues::multiple, DoParrinelloRahman::yes>
                    (start, nrend, dt, dtPressureCouple,
                    invMassPerDim, tcstat, cTC, M, x, xprime, v, f);
            }
        }
