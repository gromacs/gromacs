                         ...
                         const matrix M)
{
    ...
    /* We need to use updateMdLeapfrogGeneral for NH T-coupling
     * and/or non T/P-scaling of NEMD accelerated degrees of freedom
     */
    if (doNoseHoover || ((doTempCouple || doParrinelloRahman) &&
                         (ekind->bNEMD || ekind->cosacc.cos_accel != 0)))
    {
        /* When we don't scale x and v, we can ignore acceleration */
        if ((!ekind->bNEMD && ekind->cosacc.cos_accel == 0) ||
