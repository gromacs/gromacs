                       ...
                       const rvec * gmx_restrict f)
{
    ...
    float*       fXprime        = reinterpret_cast<float*>(xprime);
    float*       fV             = reinterpret_cast<float*>(v);
    const float* fF             = reinterpret_cast<const float*>(f);
    const float* fInvMassPerDim = reinterpret_cast<const float*>(invMassPerDim);
    ...
    for (int n = start*DIM; n < nrend*DIM; n++)
    {
         ...
         */
        real vNew = lambdaGroup*fV[n] + fF[n]*fInvMassPerDim[n]*dt;

        if (applyPRVScaling == ApplyParrinelloRahmanVScaling::diagonal)
        {
            vNew -= dtPressureCouple*pRVScaleMatrixDiagonal[d]*fV[n];
        }
        fV[n]      = vNew;
        fXprime[n] = fX[n] + vNew*dt;
    }
}
