...
#pragma omp parallel for num_threads(nthread) schedule(static)
    ...
    for (thread = 0; thread < nthread; thread++)
    {
        for (g = 0; g < opts->ngtc; g++)
        {
            ...
            /* TODO It may be true that ekinf and ekinh are never both
               used at the same point in the MD step. If so, then one
               can be eliminated, simplifying a bunch of handling of
               data in t_grp_tcstat, ekinstate_t and/or
               gmx_ekindata_t. However, to support leap-frog and
               md-vv-avek, we still need the ability to preserve the
               KE from the previous half step for the averaging. Also,
               there may be complications with the pressure
               calculation for md-vv-avek, which currently uses
               on-step velocities, where its KE calculation averages
