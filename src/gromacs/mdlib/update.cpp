                         ...
                         gmx_bool bNH, gmx_bool bPR)
{
    ...
    if (bNH || bPR)
    {
        if (bNH || cFREEZE || cACC || nFreeze[0][XX] || nFreeze[0][YY] || nFreeze[0][ZZ] || bNEMD)
        {
             ...
             */
#pragma omp simd private(imass, lg, vrel, vn)
...
#pragma omp simd private(imass, lg, vn)
            for (n = start; n < nrend; n++)
            {
                int    gt = 0;
                imass = invmass[n];
        ...
        {
            //TODO: if we decide to use pragma should be defined somewhere else and might also be done better with cmake check
#define GMX_OMP_SIMD (defined __GNUC__ && (__GNUC__ >= 5 || __GNUC_MINOR__ == 9)) || defined __ICL || defined __ICC || ( defined __clang__ && (__clang_major__ >= 4 || __clang_minor__ >= 7))
#if !GMX_SIMD_HAVE_REAL
#if 0        //correct with shell, vsite, non-isotropic
            lg = tcstat[0].lambda;
#if GMX_OMP_SIMD
#pragma omp simd private(imass, vn)
#endif
            for (n = start; n < nrend; n++)
            {
                imass = invmass[n];
...
#elif 1     //assmining diagonal M (isotropic)
            lg = tcstat[0].lambda;
#if GMX_OMP_SIMD
...
#endif
#else
#if 0       //intrinsic version with gatherLoadUTranspose<3>
