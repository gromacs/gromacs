                         ...
                         const matrix M)
{
     ...
     * and/or avoiding T/P-scaling of NEMD accelerated degrees of freedom
     */
    bool doAcceleration     = (ekind->bNEMD || ekind->cosacc.cos_accel != 0);
    bool needToAvoidScaling = (doAcceleration && (doTempCouple || doParrinelloRahman));

    if (doNoseHoover || needToAvoidScaling)
    {
        /* When we don't scale v, we can ignore acceleration and vice versa */
        if (!needToAvoidScaling)
        {
