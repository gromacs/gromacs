                         ...
                         nbnxn_free_t  *free)
{
    ...
    else if (nth > 16) /*TODO decide on cut-off*/
    {
                                                      ...
                                                      int                     nth)
{
    ...
    for (; next_pow2 < (unsigned int)nth; next_pow2 *= 2)
    {
        ;
    }
...
#pragma omp parallel num_threads(nth)
    {
        ...
        for (group_size = 2; group_size < 2*next_pow2; group_size *= 2)
        {
            ...
            if (group_size > 2)
            {
                ...
                tMPI_Atomic_set(&(nbat->syncStep[th]), group_size/2); /* completed previous step */
