...
#pragma omp parallel num_threads(nth)
    {
        ...
        for (group_size = 2; group_size < 2*next_pow2; group_size *= 2)
        {
            ...
            if (group_size > 2)
            {
                ...
                tMPI_Atomic_memory_barrier();                         /* gurantee data is safed before marking work as done */
...
#endif
            /* Position is permuted so that one of the 2 vectors beeing added was computed on the same thread in the previous step.
