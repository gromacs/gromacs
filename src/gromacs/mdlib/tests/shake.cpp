 ...
 * constraint
 *
 * SHAKE code is hard-wired to abuse t_ilist.iatoms to be a flat
 * vector of triplets of the index of the constraint type, and then
 * the indices of the two atoms involved. So for each constraint, we
 * must stride this vector by three to get access to its
 * information. */
                     ...
                     const std::vector<real>    &positions)
        {
            ...
            for (size_t i = 0; i != numConstraints; ++i)
            {
                for (size_t j = 1; j < 3; j++)
                {
                    ...
                    assert(iatom[i*constraintStride + j] < static_cast<iatom>(numAtoms));
                }
            }
