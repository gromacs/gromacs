...
class ConstraintsTest : public ::testing::Test
{
    public:
         ...
         */
        void checkFinalCoordinates(const std::vector<RVec> finalCoordinates, FloatingPointTolerance finalCoordinatesTolerance)
        {
            for (int i = 0; i < n; i++)
            {
                for (int d = 0; d < DIM; d++)
...
TEST_F(ConstraintsTest, LincsOneBond)
{
    ...
    FloatingPointTolerance tolerance = relativeToleranceAsFloatingPoint(0.1, 0.0000001);
    initSystem(2, c_oneBondMasses,
               c_oneBondConstraints, c_oneBondConstraintsR0, c_oneBondConstraintsFirstType,
               epbcNONE, c_infinitesimalBox,
               real(0.0), real(0.001), c_oneBondCoordinates);
