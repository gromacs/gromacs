...
{
    public:
        //! Database of water atom input positions (DIM reals per atom)
        ...
        //! Constructor
        SettleTest() :
            // Set up a vector of water atom positions (taken from spc216.gro) for use as test inputs
            positions_
        {
            .130, -.041, -.291,
            .120, -.056, -.192,
        ...
        updatedPositions_ {positions_},
        velocities_(positions_.size(), 0),
        box_ {{1.86206, 0, 0},
              {0, 1.86206, 0},
              {0, 0, 1.86206}}
        {
            set_pbc(&pbcNone_, epbcNONE, box_);
            set_pbc(&pbcXYZ_, epbcXYZ, box_);

            // Perturb some atom positions, to appear like an
...
TEST_P(SettleTest, SatisfiesConstraints)
{
    ...
    ASSERT_LE(numSettles, positions_.size() / (atomsPerSettle * DIM)) << "cannot test that many SETTLEs " << testDescription;
    ...
    csettle(settled, numThreads, threadIndex,
            ...
            positions_.data(), updatedPositions_.data(), reciprocalTimeStep,
...
// calculate the virial contribution. The tuple produced by Combine
// matches SettleTestParameters.
INSTANTIATE_TEST_CASE_P(WithParameters, SettleTest,
                            ::testing::Combine(::testing::Values(1, 4, 17), Bool(), Bool(), Bool()));
