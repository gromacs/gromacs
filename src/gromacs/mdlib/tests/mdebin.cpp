...
// open_memstream won't work on Windows, but we don't need coverage
// for mdebin or ebin there.
...
class PrintEbin : public ::testing::Test
{
    public:
        ...
        char    *logContents_;
        size_t   sizeOfLogContents_;
        unique_cptr<char, free_wrapper> logContentsGuard_;
        FILE    *log_;
        ...
        PrintEbin() : ebin_ {0}, mdebin_ {0},
        logContents_(nullptr), sizeOfLogContents_(0),
        logContentsGuard_(nullptr),
        log_(open_memstream(&logContents_, &sizeOfLogContents_)),
        logFileGuard_(log_),
...
TEST_F(PrintEbin, print_ebin_HandlesEmptyAverages)
{
    ...
    // Use RAII to free the contents memory.
    logContentsGuard_.reset(logContents_);

    checker_.checkString(logContents_, "log");
}
