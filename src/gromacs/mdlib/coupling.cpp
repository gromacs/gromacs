...
void trotter_update(t_inputrec *ir, gmx_int64_t step, gmx_ekindata_t *ekind,
                    gmx_enerdata_t *enerd, t_state_local *state,
...
extern void init_npt_masses(t_inputrec *ir, t_state_local *state, t_extmass *MassQ, gmx_bool bInit)
{
    int           n, i, j, d, ngtc, nh;
    t_grpopts    *opts;
    real          reft, kT, ndj, nd;
...
int **init_npt_vars(t_inputrec *ir, t_state_local *state, t_extmass *MassQ, gmx_bool bTrotter)
{
    int           i, j, nnhpres, nh;
    t_grpopts    *opts;
...
static real energyNoseHoover(const t_inputrec *ir, const t_state_local *state, const t_extmass *MassQ)
{
    real energy = 0;
...
static real energyPressureMTTK(const t_inputrec *ir, const t_state_local *state, const t_extmass *MassQ)
{
    real energy = 0;
...
static real energyVrescale(const t_inputrec *ir, const t_state_local *state)
{
    real energy = 0;
    for (int i = 0; i < ir->opts.ngtc; i++)
    {
