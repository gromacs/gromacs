                   ...
                   real               print_force)
{
    ...
    if (fr->cutoff_scheme == ecutsVERLET)
    {
        ...
        if (bFEP_NonBonded && gmx_omp_nthreads_get(emntNonbonded) > 1)
        {
            md_print_info(cr, fp, "\nNOTE: With the Verlet cut-off scheme and OpenMP, the perturbed non-bonded interaction are handled by the first OpenMP thread only. This leads to load imbalance. Use 1 OpenMP thread, if you can.\n");
        }
