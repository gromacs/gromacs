                                   ...
                                   t_nblist            *nlist)
{
    ...
    for (i = 0; i < nbl->na_sc; i++)
    {
        ...
        if (ai >= 0)
        {
            ...
            bFEP_i = gridi->fep[ind_i >> gridi->na_c_2log] & (1 << (i & (NBNXN_GPU_CLUSTER_SIZE - 1)));
            ...
            for (cj4_ind = cj4_ind_start; cj4_ind < cj4_ind_end; cj4_ind++)
            {
                ...
                {
                    if ((cj4->imei[0].imask & (1U << (gcj*GPU_NSUBCELL + (i >> gridi->na_c_2log)))) == 0)
                    {
                    ...
                    if (bFEP_i || fep_cj != 0)
                    {
                        for (j = 0; j < nbl->na_cj; j++)
                        {
                                ...
                                (!bDiagRemoved || ind_j >= ind_i))
                            {
                                ...
                                excl_pair = AMODWJ(j)*nbl->na_ci + (i & (NBNXN_GPU_CLUSTER_SIZE - 1));
                                excl_bit  = (1U << (gcj*GPU_NSUBCELL + (i>>gridj->na_c_2log)));
