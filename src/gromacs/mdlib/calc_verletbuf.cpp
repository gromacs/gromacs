...
static real energyDrift(const verletbuf_atomtype_t *att, int natt,
                        ...
                        real kT_fac,
                        const pot_derivatives_t *lj_disp,
                        const pot_derivatives_t *lj_rep,
                        ...
                        real rlist, real boxvol)
{
    ...
    for (int i = 0; i < natt; i++)
    {
        ...
        for (int j = i; j < natt; j++)
        {
            ...
            lj.md1 = c6*lj_disp->md1 + c12*lj_rep->md1;
            lj.d2  = c6*lj_disp->d2  + c12*lj_rep->d2;
            lj.md3 = c6*lj_disp->md3 + c12*lj_rep->md3;
             ...
             */
            pot *= 4*M_PI*sqr(rlist + std::sqrt(s2))/boxvol;
...
static real surface_frac(int cluster_size, real particle_distance, real rlist)
{
    ...
    switch (cluster_size)
    {
        ...
        case 4:
             ...
             */
            area_rel = (1.0 + 1/M_PI*(6*acos(1/std::sqrt(3))*d +
                                      std::sqrt(3)*d*d*(1.0 +
                                                        5.0/18.0*d*d +
                                                        7.0/45.0*d*d*d*d +
                             ...
                             real *rlist)
{
    ...
    particle_distance = pow(boxvol*std::sqrt(2)/mtop->natoms, 1.0/3.0);
    ...
    pot_derivatives_t lj_disp = { 0, 0, 0 };
    pot_derivatives_t lj_rep  = { 0, 0, 0 };
    ...
    if (ir->vdwtype == evdwCUT)
    {
        ...
        switch (ir->vdw_modifier)
        {
            ...
            case eintmodPOTSHIFT:
                ...
                lj_disp.md1 =      -6*pow(ir->rvdw, -7.0);
                lj_rep.md1  = reppow*pow(ir->rvdw, -(reppow+1));
                /* The contribution of the higher derivatives is negligible */
                break;
            case eintmodFORCESWITCH:
                /* At the cut-off: V=V'=V''=0, so we use only V''' */
                lj_disp.md3 = -md3_force_switch(6.0,    ir->rvdw_switch, ir->rvdw);
                lj_rep.md3  =  md3_force_switch(reppow, ir->rvdw_switch, ir->rvdw);
                break;
            case eintmodPOTSWITCH:
                ...
                lj_disp.md3 = -pow(ir->rvdw, -6.0   )*md3_pswf;
                lj_rep.md3  =  pow(ir->rvdw, -reppow)*md3_pswf;
                break;
            default:
    ...
    else if (EVDW_PME(ir->vdwtype))
    {
        ...
        lj_disp.md1 = -std::exp(-br2)*(br6 + 3.0*br4 + 6.0*br2 + 6.0)*pow(r, -7.0);
        lj_rep.md1  = reppow*pow(r, -(reppow+1));
        // The contribution of the higher derivatives is negligible
    }
    else
    {
    ...
    if (ir->coulombtype == eelCUT || EEL_RF(ir->coulombtype))
    {
        ...
        if (eps_rf > 0)
        {
            elec.md1 = elfac*(pow(ir->rcoulomb, -2.0) - 2*k_rf*ir->rcoulomb);
        }
        elec.d2      = elfac*(2*pow(ir->rcoulomb, -3.0) + 2*k_rf);
    }
