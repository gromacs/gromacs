...
DispersionCorrection::DispersionCorrection(const gmx_mtop_t          &mtop,
    ...
    eFep_(inputrec.efep),
    dispersionCorrectionTable_(nullptr)
{
...
void DispersionCorrection::print(const gmx::MDLogger &mdlog) const
{
    ...
    std::string text = gmx::formatString("Long Range LJ corr.: <C6> %10.4e", avcsix_[0]);
    if (eDispCorr_ == edispcAllEner ||
        eDispCorr_ == edispcAllEnerPres)
    {
        text += gmx::formatString(" <C12> %10.4e", avctwelve_[0]);
    }
...
{
    if (tableFileName && dispersionCorrectionTable_ == nullptr)
    {
    ...
    if (eDispCorr_ != edispcNO)
    {
            ...
            (ic.vdwtype == evdwSWITCH))
        {
            ...
            GMX_RELEASE_ASSERT(dispersionCorrectionTable_, "We need an initialized table"):
            ...
            GMX_ASSERT(dispersionCorrectionTable_->interaction == GMX_TABLE_INTERACTION_VDWREP_VDWDISP,
                       "Dispersion-correction code needs a table with both repulsion and dispersion terms");
            const real  scale  = dispersionCorrectionTable_->scale;
            const real *vdwtab = dispersionCorrectionTable_->data;
                ...
                (ic.vdwtype == evdwSHIFT))
            {
                 ...
                 */
                enershiftsix_    = static_cast<real>(-1.0/(rc3*rc3)) - 6.0*vdwtab[8*ri0];
                enershifttwelve_ = static_cast<real>( 1.0/(rc9*rc3)) - 12.0*vdwtab[8*ri0 + 4];
            }
            else if (ic.vdw_modifier == eintmodPOTSHIFT)
            {
                enershiftsix_    = static_cast<real>(-1.0/(rc3*rc3));
                enershifttwelve_ = static_cast<real>( 1.0/(rc9*rc3));
            }
             ...
             */
            eners[0] += 4.0*M_PI*enershiftsix_*rc3/3.0;
            eners[1] += 4.0*M_PI*enershifttwelve_*rc3/3.0;
        ...
        if (ic.vdwtype == evdwPME && ic.cutoff_scheme == ecutsGROUP)
        {
             ...
             */
            enershiftsix_ = gmx::power6(ic.ewaldcoeff_lj) / 6.0;

            eners[0] += -gmx::power3(std::sqrt(M_PI)*ic.ewaldcoeff_lj)/3.0;
            virs[0]  +=  gmx::power3(std::sqrt(M_PI)*ic.ewaldcoeff_lj);
        }

        enerdiffsix_    = eners[0];
        enerdifftwelve_ = eners[1];
        /* The 0.5 is due to the Gromacs definition of the virial */
        virdiffsix_     = 0.5*virs[0];
        virdifftwelve_  = 0.5*virs[1];
    }
}
                                     ...
                                     real *prescorr, real *enercorr, real *dvdlcorr) const
{
    ...
    *dvdlcorr = 0;
    ...
    if (eDispCorr_ != edispcNO)
    {
        ...
        real       dens;
        real       ninter;
        if (numAtomsForTpi_ > 0)
        {
            /* Only correct for the interactions with the inserted molecule */
            dens   = (numAtomsForDispersionCorrection_ - numAtomsForTpi_)*invvol;
            ninter = numAtomsForTpi_;
        }
        else
        {
            dens   = numAtomsForDispersionCorrection_*invvol;
            ninter = 0.5*numAtomsForDispersionCorrection_;
        }
        ...
        {
            avcsix    = avcsix_[0];
            avctwelve = avctwelve_[0];
        }
        else
        {
            avcsix    = (1 - lambda)*avcsix_[0]    + lambda*avcsix_[1];
            avctwelve = (1 - lambda)*avctwelve_[0] + lambda*avctwelve_[1];
        }

        const real enerdiff   = ninter*(dens*enerdiffsix_ - enershiftsix_);
        *enercorr            += avcsix*enerdiff;
        real       dvdlambda  = 0;
        if (eFep_ != efepNO)
        {
            dvdlambda += (avcsix_[1] - avcsix_[0])*enerdiff;
        }
        if (bCorrAll)
        {
            const real enerdiff  = ninter*(dens*enerdifftwelve_ - enershifttwelve_);
            *enercorr           += avctwelve*enerdiff;
            if (eFep_ != efepNO)
            {
                dvdlambda += (avctwelve_[1] - avctwelve_[0])*enerdiff;
            }
        }
        ...
        if (bCorrPres)
        {
            svir = ninter*dens*avcsix*virdiffsix_/3.0;
            if (eDispCorr_ == edispcAllEnerPres)
            {
                svir += ninter*dens*avctwelve*virdifftwelve_/3.0;
            }
