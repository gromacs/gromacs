                                               ...
                                               const char                *tableFileName)
{
        ...
        (ic.vdwtype == evdwSWITCH))
    {
         ...
         */
        eners[0] += 4.0*M_PI*iParams->enershiftsix_*rc3/3.0;
        eners[1] += 4.0*M_PI*iParams->enershifttwelve_*rc3/3.0;
    ...
    if (ic.vdwtype == evdwPME && ic.cutoff_scheme == ecutsGROUP)
    {
         ...
         */
        iParams->enershiftsix_ = gmx::power6(ic.ewaldcoeff_lj) / 6.0;

        eners[0] += -gmx::power3(std::sqrt(M_PI)*ic.ewaldcoeff_lj)/3.0;
        virs[0]  +=  gmx::power3(std::sqrt(M_PI)*ic.ewaldcoeff_lj);
    }

    iParams->enerdiffsix_    = eners[0];
    iParams->enerdifftwelve_ = eners[1];
    /* The 0.5 is due to the Gromacs definition of the virial */
    iParams->virdiffsix_     = 0.5*virs[0];
    iParams->virdifftwelve_  = 0.5*virs[1];
}
...

{
    ...
    if (eDispCorr_ != edispcNO)
    {
        ...
        if (bCorrPres)
        {
            virial = numCorr*density*avcsix*iParams_.virdiffsix_/3.0;
            if (eDispCorr_ == edispcAllEnerPres)
            {
                virial += numCorr*density*avctwelve*iParams_.virdifftwelve_/3.0;
            }
            /* The factor 2 is because of the Gromacs virial definition */
            corr.pressure = -2.0*invvol*virial*PRESFAC;

            for (int m = 0; m < DIM; m++)
            {
                corr.virialTensor[m][m]   = virial;
                corr.pressureTensor[m][m] = corr.pressure;
            }
        }
        ...
        if (debug)
        {
            ...
            if (bCorrPres)
            {
                fprintf(debug,
                        ...
                        corr.energy, corr.pressure, virial);
            }
            else
            {
