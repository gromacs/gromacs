						     ...
						     gmx::ArrayRef<const real>  nonbondedForceParameters)
{
    ...
    for (int q = 0; q < (inputrec.efep == efepNO ? 1 : 2); q++)
    {
	    ...
	    numAtomsToCorrectFor_ = mtop.natoms;
	    ...
	    numAtomsForDensity_      = mtop.natoms - atoms_tpi.nr;
            numAtomsToCorrectFor_    = atoms_tpi.nr;
...
{
    if (!haveValidParameters_)
    {
	GMX_RELEASE_ASSERT(tableFileName,
			   "tableFileName needs to be set at first call");
    ...
    iParams_.enershifttwelve_ = 0;
    iParams_.enerdiffsix_     = 0;
    iParams_.enerdifftwelve_  = 0;
    iParams_.virdiffsix_      = 0;
    iParams_.virdifftwelve_   = 0;
    ...
    if (eDispCorr_ != edispcNO)
    {
            ...
            (ic.vdwtype == evdwSWITCH))
        {
            ...
            GMX_ASSERT(iParams_.dispersionCorrectionTable_, "We need an initialized table");
            ...
            GMX_ASSERT(iParams_.dispersionCorrectionTable_->interaction == GMX_TABLE_INTERACTION_VDWREP_VDWDISP,
                       "Dispersion-correction code needs a table with both repulsion and dispersion terms");
            const real  scale  = iParams_.dispersionCorrectionTable_->scale;
            const real *vdwtab = iParams_.dispersionCorrectionTable_->data;
                ...
                (ic.vdwtype == evdwSHIFT))
            {
                 ...
                 */
                iParams_.enershiftsix_    = static_cast<real>(-1.0/(rc3*rc3)) - 6.0*vdwtab[8*ri0];
                iParams_.enershifttwelve_ = static_cast<real>( 1.0/(rc9*rc3)) - 12.0*vdwtab[8*ri0 + 4];
            }
            else if (ic.vdw_modifier == eintmodPOTSHIFT)
            {
                iParams_.enershiftsix_    = static_cast<real>(-1.0/(rc3*rc3));
                iParams_.enershifttwelve_ = static_cast<real>( 1.0/(rc9*rc3));
            }
             ...
             */
            eners[0] += 4.0*M_PI*iParams_.enershiftsix_*rc3/3.0;
            eners[1] += 4.0*M_PI*iParams_.enershifttwelve_*rc3/3.0;
        ...
        if (ic.vdwtype == evdwPME && ic.cutoff_scheme == ecutsGROUP)
        {
             ...
             */
            iParams_.enershiftsix_ = gmx::power6(ic.ewaldcoeff_lj) / 6.0;

            eners[0] += -gmx::power3(std::sqrt(M_PI)*ic.ewaldcoeff_lj)/3.0;
            virs[0]  +=  gmx::power3(std::sqrt(M_PI)*ic.ewaldcoeff_lj);
        }

        iParams_.enerdiffsix_    = eners[0];
        iParams_.enerdifftwelve_ = eners[1];
        /* The 0.5 is due to the Gromacs definition of the virial */
        iParams_.virdiffsix_     = 0.5*virs[0];
        iParams_.virdifftwelve_  = 0.5*virs[1];
    }

    haveValidParameters_ = true;
}

void DispersionCorrection::calculate(const matrix box, real lambda, tensor pres, tensor virial,
                                     real *prescorr, real *enercorr, real *dvdlcorr) const
{
    ...
    *dvdlcorr = 0;
    ...
    if (eDispCorr_ != edispcNO)
    {
	GMX_RELEASE_ASSERT(haveValidParameters_,
			   "Need valid parameters; setParameters() should have been called before calculate()");

        const bool bCorrAll  = (eDispCorr_ == edispcAllEner ||
                                eDispCorr_ == edispcAllEnerPres);
        ...
        const int  numAtoms  = topParams_.numAtomsToCorrectFor_;
        ...
        const real enerdiff   = numAtoms*(density*iParams_.enerdiffsix_ - iParams_.enershiftsix_);
        *enercorr            += avcsix*enerdiff;
        ...
        if (bCorrAll)
        {
            const real enerdiff  = numAtoms*(density*iParams_.enerdifftwelve_ - iParams_.enershifttwelve_);
        ...
        if (bCorrPres)
        {
            svir = numAtoms*density*avcsix*iParams_.virdiffsix_/3.0;
            if (eDispCorr_ == edispcAllEnerPres)
            {
                svir += numAtoms*density*avctwelve*iParams_.virdifftwelve_/3.0;
            }
