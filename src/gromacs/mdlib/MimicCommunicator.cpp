...
{
#ifdef GMX_MIMIC
    return MCL_init_client((char *) getCurrentDir().c_str());
#else
    gmx_fatal(FARGS, "GROMACS is compiled without MiMiC support! Please, recompile with -DGMX_MIMIC=ON");
#endif
}

void gmx::MimicCommunicator::sendInitData(gmx_mtop_t *mtop, gmx::HostVector<gmx::RVec> coords)
{
#ifdef GMX_MIMIC
...
void gmx::MimicCommunicator::getStepNumber(gmx_int64_t &nsteps)
{
#ifdef GMX_MIMIC
    int temp_step;
...
void gmx::MimicCommunicator::getCoords(rvec *x, int natoms)
{
#ifdef GMX_MIMIC
...
std::string gmx::MimicCommunicator::getCurrentDir()
{
