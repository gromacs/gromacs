...
ResetHandler::ResetHandler(
    ...
    resetCounterSignalReceived_(false),
...
bool ResetHandler::setSignalImpl(gmx_walltime_accounting_t walltime_accounting)
{
    ...
    if (secondsSinceStart > maximumHoursToRun_ * 60.0 * 60.0 * 0.495)
    {
        ...
        signalView_[0] = static_cast<double>(ResetSignal::doResetCounters);
...
void ResetHandler::resetCountersImpl(
        int64_t                     step,
        const MDLogger             &mdlog,
        FILE                       *fplog,
        const t_commrec            *cr,
...
int ResetHandler::getNumSimulationGlobalsRequired() const
{
    return 1;
}

void ResetHandler::setViewForSimulationGlobals(
...
void ResetHandler::notifyAfterSimulationCommunication()
{
