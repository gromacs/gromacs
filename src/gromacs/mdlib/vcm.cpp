...
t_vcm::t_vcm(const GmxGroups &groups, const t_inputrec &ir)
{
    ...
    if (mode != ecmNO)
    {
        nr = groups.groups[static_cast<int>(SimulationGroups::g_VCM)].nr;
        ...
        for (int g = 0; (g < nr); g++)
        {
            ...
            group_name[g] = *groups.groupNames[groups.groups[static_cast<int>(SimulationGroups::g_VCM)].nm_ind[g]];
