...
t_vcm *init_vcm(FILE *fp, gmx_groups_t *groups, t_inputrec *ir)
{
    ...
    if (vcm->mode != ecmNO)
    {
        ...
        snew(vcm->thread_vcm, gmx_omp_nthreads_get(emntDefault) * (vcm->nr+1));
    }
                  ...
                  rvec x[], rvec v[], t_vcm *vcm)
{
    ...
    if (vcm->mode != ecmNO)
    {
#pragma omp parallel private(g) num_threads(nthreads)
        {
            int t = gmx_omp_get_thread_num();
            for (g = 0; (g < vcm->nr+1); g++)
            {
                /* Reset linear momentum */
                t_vcm_thread *vcm_t = &vcm->thread_vcm[t*(vcm->nr+1)+g];
                vcm_t->mass = 0;
                clear_rvec(vcm_t->p);
                if (vcm->mode == ecmANGULAR)
                {
