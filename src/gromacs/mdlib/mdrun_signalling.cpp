...
{
    std::vector<real> buffer(eglsNR, 0);
    if (gs)
    {
        std::copy(gs->sig, gs->sig + eglsNR, buffer.begin());
    }
              ...
              std::vector<real>        *buffer)
{
    ...
    if (MULTISIM(cr) && bInterSimGS)
    {
        if (MASTER(cr))
        {
            ...
            gmx_sum_sim(eglsNR, &(*buffer)[0], cr->ms);
        }
        /* Communicate the signals from the master to the others */
        gmx_bcast(eglsNR*sizeof((*buffer)[0]), &(*buffer)[0], cr);
    }
