...
#pragma omp parallel num_threads(nth)
    ...
    if (flags & GMX_FORCE_ENERGY)
    {
        ...
        if (std::isnan(enerd->term[F_COUL_SR]))
        {
            gmx_fatal(FARGS, "One or more pairs of non-excluded atoms, either interacting or non-interacting, are at a distance closer than %g nm, which is not allowed.",
                      std::pow(GMX_REAL_MIN, 1.0/3.0));
        }
        if (std::isnan(enerd->term[F_LJ]))
        {
            gmx_fatal(FARGS, "One or more pairs of non-excluded atoms, either interacting or non-interacting, are at a distance closer than %g nm, which is not allowed.",
                      std::pow(GMX_REAL_MIN, 1.0/12.0));
        }
    }
}
