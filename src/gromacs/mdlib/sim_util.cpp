 ...
 * \param[in] ic             The interaction constants struct
 * \param[in]  box           The box matrix
 * \param[in]  x             Coordinate array
 * \param[in]  flags         Force flags
 * \param[in]  useGpuPme     Boolean indicating whether PME on GPU is enabled for the run
 * \param[in]  wcycle        The wallcycle structure
 */
static inline void launchPmeGpuSpread(gmx_pme_t                            *pmedata,
                                      const gmx_unused interaction_const_t *ic,
                                      matrix                                box,
                                      rvec                                  x[],
                                      int                                   flags,
                                      bool                                  useGpuPme,
                                      gmx_wallcycle_t                       wcycle)
{
    ...
    assert(useGpuPme == (!EVDW_PME(ic->vdwtype)));
    // we have to be using PME if we are here
    assert(useGpuPme == (EEL_PME(ic->eeltype)));
                                ...
                                DdCloseBalanceRegionAfterForceComputation ddCloseBalanceRegion)
{
    ...
    launchPmeGpuSpread(fr->pmedata, fr->ic, box, x, flags, useGpuPme, wcycle);
