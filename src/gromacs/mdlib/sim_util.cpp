...
static real averageKineticEnergyEstimate(const t_grpopts &groupOptions)
{
    ...
    /* The conditional with > also catches nrdf=0 */
...
/*! \brief  This routine checks if the potential energy is finite or large.
 *
 * Always checks that the potential energy is finite. If step equals
 * inputrec.init_step also checks for very large potential energy.
 * Terminates with a fatal error when a check fails.
                                         ...
                                         const t_inputrec     &inputrec)
{
    ...
    {
        gmx_fatal(FARGS, "Step %" PRId64 ": The total potential energy is %g, which is %s. The LJ and electrostatic contributions to the energy are %g and %g, respectively. A %s potential energy can be caused by overlapping interactions in bonded interactions or very large%s coordinate values. Usually this is caused by a badly or non-equilibrated initial configuration or incorrect interactions or parameters in the topology.",
                  step,
                  enerd.term[F_EPOT],
