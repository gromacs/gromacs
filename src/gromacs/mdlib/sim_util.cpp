                                        ...
                                        gmx_wallcycle_t                 wcycle)
{
    ...
    while (!isPmeGpuDone || !isNbGpuDone)
    {
        if (!isPmeGpuDone)
        {
            ...
            auto   completionType = (disableAlternating || isNbGpuDone) ?
                GpuTaskCompletion::Wait : GpuTaskCompletion::Check;
            isPmeGpuDone = pme_gpu_wait_or_check_if_finished(pmedata, wcycle, &pmeGpuForces,
        ...
        if (!isNbGpuDone)
        {
            auto completionType = (disableAlternating || isPmeGpuDone) ?
                GpuTaskCompletion::Wait : GpuTaskCompletion::Check;
            wallcycle_start_nocount(wcycle, ewcWAIT_GPU_NB_NL);
            isNbGpuDone = nbnxn_gpu_wait_or_check_if_finished(nbv->gpu_nbv,
                                                              flags, eatLocal,
                                ...
                                DdCloseBalanceRegionAfterForceComputation ddCloseBalanceRegion)
{
    ...
    static const bool disableAlternating = (getenv("GMX_DISABLE_ALTER_WAIT") != nullptr); // TODO remove or make it module global
