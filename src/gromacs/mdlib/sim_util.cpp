...
static inline void launchPmeGpuTask(gmx_int64_t       step,
                                    gmx_wallcycle_t   wcycle,
                                    const t_forcerec *fr,
                                    matrix            box,
                                    rvec              x[],
                                ...
                                DdCloseBalanceRegionAfterForceComputation ddCloseBalanceRegion)
{
    ...
    sendCoordinatesToPmeRanks(cr, step, wcycle, box, x, lambda, flags);

    launchPmeGpuTask(step, wcycle, fr, box, x, flags, useGpuPme, pmeRunMode, pmeGpuAccumulateInputForces);
    ...
    if (useGpuPme)
    {
        matrix vir_Q;
        real   Vlr_q;
        pme_gpu_get_results(fr->pmedata, wcycle, vir_Q, &Vlr_q);
        forceWithVirial.addVirialContribution(vir_Q);
        enerd->term[F_COUL_RECIP] += Vlr_q;
        sum_forces(as_rvec_array(forceWithVirial.force_.data()),
                   constArrayRefFromVector<gmx::RVec>(fr->forceBufferIntermediate->begin(),
                                                      fr->forceBufferIntermediate->end()));
    }
