                         ...
                         int flags)
{
    ...
    if (DOMAINDECOMP(cr))
    {
        ...
        if (bNS)
        {
            ...
            STS::getInstance("force")->skipRun("xcomm");
            STS::getInstance("force")->skipLoop("nbat_x_to_x");
        }
        else
        {
            markStepStart();
            STS::getInstance("force")->nextStep();
            STS::getInstance("force")->run("xcomm", [&]{
    ...
    if (STS::getCurrentInstance() != STS::getInstance("force")) {
        markStepStart();
        STS::getInstance("force")->nextStep();
    }
    int cycles_nb_force = 0;
    STS::getInstance("force")->run("nonbonded", [&]{
    wallcycle_start(wcycle, ewcNBFORCE);
    if (!bUseOrEmulGPU)
    {
    ...
    if (!bUseOrEmulGPU || bDiffKernels)
    {
        ...
        if (DOMAINDECOMP(cr))
        {
            STS::getInstance("force")->waitForTask("xcomm");
            do_nb_verlet(fr, ic, enerd, flags, eintNonlocal,
                         bDiffKernels ? enbvClearFYes : enbvClearFNo,
                         nrnb, wcycle);
        }
        cycles_nb_force = wallcycle_stop(wcycle, ewcNBFORCE);
    ...
    STS::getInstance("force")->run("lowlevel", [&] {
        if (DOMAINDECOMP(cr)) {
            STS::getInstance("force")->waitForTask("xcomm");
        }
    ...
    if (DOMAINDECOMP(cr))
    {
        ...
        if (wcycle)
        {
            dd_cycles_add(cr->dd, cycles_nb_force, ddCyclF);
            if (bUseGPU)
            {
