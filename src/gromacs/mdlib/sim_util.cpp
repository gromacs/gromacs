...
/*! Hack structure with force ouput buffers for do_force */
struct ForceOutputs
{
                  ...
                  gmx_wallcycle_t                      wcycle)
{
     ...
     */
    const bool           separateForceWithVirialBuffer = (bDoForces && (doVirial && fr->haveDirectVirialContributions));
    /* forceWithVirial uses the local atom range only */
    gmx::ForceWithVirial forceWithVirial (separateForceWithVirialBuffer ?
                                          *fr->forceBufferForDirectVirialContributions : force.unpaddedArrayRef(),
                                          doVirial);

    if (separateForceWithVirialBuffer)
    {
        /* TODO: update comment
                                ...
                                const DDBalanceRegionHandler &ddBalanceRegionHandler)
{
    ...
    if (alternateGpuWait)
    {
        alternatePmeNbGpuWaitReduce(fr->nbv, fr->pmedata, &force, &forceOut.forceWithVirial, fr->fshift, enerd, flags, pmeFlags, ppForceWorkload->haveGpuBondedWork, wcycle);
    }
