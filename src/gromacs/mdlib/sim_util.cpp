...
/*! \brief  This routine checks if the non-bonded pair energies that are calculated in the nbnxn kernels are finite.
 *
 * Note that passing this check does not guarantee finite forces,
 * but there is just a narrow pair-distance range where forces are
 * not finite and energies are finite.
 *
