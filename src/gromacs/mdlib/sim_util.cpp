                         ...
                         int flags)
{
    ...
    if (bDoForces)
    {
        if (inputrec->efield->applyField())
        {
            /* Compute forces due to electric field */
            calc_f_el(start, homenr,
                      mdatoms->chargeA, fr->f_novirsum,
                      inputrec->efield, t);
            if (MASTER(cr) && (NULL != field))
            {
                inputrec->efield->printComponents(field, t);
            }
        }
                        ...
                        int flags)
{
    ...
    if (bDoForces)
    {
        ...
        {
            /* Compute forces due to electric field */
            calc_f_el(start, homenr, mdatoms->chargeA, fr->f_novirsum,
                      inputrec->efield, t);
            if (MASTER(cr) && (NULL != field))
            {
                inputrec->efield->printComponents(field, t);
