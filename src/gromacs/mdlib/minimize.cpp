...
double do_cg(FILE *fplog, t_commrec *cr,
             int nfile, const t_filenm fnm[],
             const output_env_t gmx_unused oenv,
             gmx_bool bVerbose, gmx_bool gmx_unused bCompact,
             ...
             gmx_walltime_accounting_t walltime_accounting)
{
    ...
    gmx::em_state_t  *s_min, *s_a, *s_b, *s_c;
    gmx_localtop_t   *top;
    ...
    s_min = gmx::init_em_state();
    s_a   = gmx::init_em_state();
    s_b   = gmx::init_em_state();
    s_c   = gmx::init_em_state();
    ...
    /* Print to log file */
    gmx::print_em_start(fplog, cr, walltime_accounting, wcycle, CG);
    ...
    if (MASTER(cr))
    {
        gmx::sp_header(stderr, CG, inputrec->em_tol, number_steps);
    }
    if (fplog)
    {
        gmx::sp_header(fplog, CG, inputrec->em_tol, number_steps);
    }
     ...
     * We do not unshift, so molecules are always whole in congrad.c
     */
    gmx::evaluate_em_energy(fplog, cr,
    ...
    for (step = 0; (number_steps < 0 || (number_steps >= 0 && step <= number_steps)) && !converged; step++)
    {
        ...
        gmx::get_f_norm_max(cr, &(inputrec->opts), mdatoms, p, &pnorm, NULL, NULL);
        ...
        gmx::write_em_traj(fplog, cr, outf, do_x, do_f, NULL,
        ...
        {
            gmx::em_dd_partition_system(fplog, step, cr, top_global, inputrec,
                                        s_min, top, mdatoms, fr, vsite, constr,
                                        nrnb, wcycle);
        }

        /* Take a trial step (new coords in s_c) */
        gmx::do_em_step(cr, inputrec, mdatoms, fr->bMolPBC,
                        s_min, c, s_min->s.cg_p, s_c,
    ...
    if (s_min->fmax > inputrec->em_tol)
    {
        if (MASTER(cr))
        {
            gmx::warn_step(stderr, inputrec->em_tol, step-1 == number_steps, FALSE);
            gmx::warn_step(fplog, inputrec->em_tol, step-1 == number_steps, FALSE);
        }
        converged = FALSE;
    }
    ...
    if (MASTER(cr))
    {
        ...
        gmx::print_converged(stderr, CG, inputrec->em_tol, step,
                             converged, number_steps,
                             s_min->epot, s_min->fmax, s_min->a_fmax, fnormn);
        gmx::print_converged(fplog, CG, inputrec->em_tol, step,
                             converged, number_steps,
                             s_min->epot, s_min->fmax, s_min->a_fmax, fnormn);
    ...
    gmx::finish_em(cr, outf, walltime_accounting, wcycle);
                ...
                gmx_walltime_accounting_t walltime_accounting)
{
    ...
    /* Init em */
    gmx::init_em(fplog, LBFGS, cr, inputrec,
    ...
    /* Print to log file */
    gmx::print_em_start(fplog, cr, walltime_accounting, wcycle, LBFGS);
    ...
    if (MASTER(cr))
    {
        gmx::sp_header(stderr, LBFGS, inputrec->em_tol, number_steps);
    }
    if (fplog)
    {
        gmx::sp_header(fplog, LBFGS, inputrec->em_tol, number_steps);
    }
    ...
    for (step = 0; (number_steps < 0 || (number_steps >= 0 && step <= number_steps)) && !converged; step++)
    {
        ...
        gmx::get_f_norm_max(cr, &(inputrec->opts), mdatoms, f, &fnorm, &fmax, &nfmax);
    ...
    do_x = !do_per_step(step, inputrec->nstxout);
    do_f = !do_per_step(step, inputrec->nstfout);
    gmx::write_em_traj(fplog, cr, outf, do_x, do_f, ftp2fn(efSTO, nfile, fnm),
                       top_global, inputrec, step,
                       &ems, state, f);
    ...
    {
        double sqrtNumAtoms = sqrt(static_cast<double>(state->natoms));
        gmx::print_converged(stderr, LBFGS, inputrec->em_tol, step, converged,
                             number_steps, Epot, fmax, nfmax, fnorm/sqrtNumAtoms);
        gmx::print_converged(fplog, LBFGS, inputrec->em_tol, step, converged,
                             number_steps, Epot, fmax, nfmax, fnorm/sqrtNumAtoms);
                ...
                gmx_walltime_accounting_t walltime_accounting)
{
    ...
    s_min = gmx::init_em_state();
    s_try = gmx::init_em_state();

    /* Init em and store the local state in s_try */
    gmx::init_em(fplog, SD, cr, inputrec,
                 state_global, top_global, s_try, &top, &f, &f_global,
                 nrnb, mu_tot, fr, &enerd, &graph, mdatoms, &gstat, vsite, constr,
                 nfile, fnm, &outf, &mdebin, imdport, Flags, wcycle);

    /* Print to log file  */
    gmx::print_em_start(fplog, cr, walltime_accounting, wcycle, SD);
    ...
    {
        /* Print to the screen  */
        gmx::sp_header(stderr, SD, inputrec->em_tol, nsteps);
    }
    if (fplog)
    {
        gmx::sp_header(fplog, SD, inputrec->em_tol, nsteps);
    }
    ...
    while (!bDone && !bAbort)
    {
...
#endif
        {
            if (MASTER(cr))
            {
                gmx::warn_step(stderr, inputrec->em_tol, count == nsteps, constr != NULL);
                gmx::warn_step(fplog, inputrec->em_tol, count == nsteps, constr != NULL);
            }
            bAbort = TRUE;
        }
    ...
    gmx::write_em_traj(fplog, cr, outf, TRUE, inputrec->nstfout,
    ...
    if (MASTER(cr))
    {
        ...
        gmx::print_converged(stderr, SD, inputrec->em_tol, count, bDone, nsteps,
                             s_min->epot, s_min->fmax, s_min->a_fmax, fnormn);
        gmx::print_converged(fplog, SD, inputrec->em_tol, count, bDone, nsteps,
                             s_min->epot, s_min->fmax, s_min->a_fmax, fnormn);
    }
             ...
             gmx_walltime_accounting_t walltime_accounting)
{
    ...
    state_work = gmx::init_em_state();

    /* Init em and store the local state in state_minimum */
    gmx::init_em(fplog, NM, cr, inputrec,
    ...
    gmx::print_em_start(fplog, cr, walltime_accounting, wcycle, NM);
    ...
    gmx::get_state_f_norm_max(cr, &(inputrec->opts), mdatoms, state_work);
