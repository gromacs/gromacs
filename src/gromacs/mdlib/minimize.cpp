             ...
             gmx_walltime_accounting_t walltime_accounting)
{
    ...
    unsigned int nvsite = 0;
    get_atom_index(top_global, atom_index, &nvsite, &nshell);
...
#endif
                MPI_Send(dfdx[0], atom_index.size()*DIM, mpi_type, MASTERNODE(cr), cr->nodeid,
                         cr->mpi_comm_mygroup);
#endif
            }
            else
