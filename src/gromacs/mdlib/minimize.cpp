                ...
                gmx_walltime_accounting_t walltime_accounting)
{
    ...
    for (step = 0; (number_steps < 0 || (number_steps >= 0 && step <= number_steps)) && !converged; step++)
    {
        ...
        if (EpotC < EpotA || (gpc < 0 && EpotC < (EpotA+tmp)))
        {
            ...
            fnorm      = ems.fnorm;
            // Great, we found a better energy. Increase step for next iteration
            // if we are still going down. The alternative is that we moved too far
            // and the gradient is now positive in position C, although the energy
            // is still lower than our initial point A. In this case we reduce
            // the step size, but we still accept this position since it is more
            // efficient to use the information we have to find a better search
            // direction rather than wasting potential evaluations to find the
            // exact minimum along the line.
            if (gpc < 0)
            {
                stepsize *= 1.618034; /* The golden section */
            }
