                                              ...
                                              int               nsupercell)
{
    ...
    /* We can have maximally nsupercell*nbnxn_gpu_ncluster_per_cell sj lists */
    /* We can store 4 j-subcell - i-supercell pairs in one struct.
     * since we round down, we need one extra entry.
     */
    ncj4_max = ((nbl->work->cj_ind + nsupercell*nbnxn_gpu_ncluster_per_cell + nbnxn_gpu_jgroup_size - 1) >> NBNXN_GPU_JGROUP_SIZE_2LOG);
                                ...
                                nbnxn_free_t     *free)
{
    ...
    if (!nbl->bSimple)
    {
        GMX_ASSERT(nbnxn_gpu_ncluster_per_supercluster == nbnxn_gpu_ncluster_per_cell, "The search code assumes that the a super-cluster matches a search grid cell");

        assert(1 << NBNXN_GPU_JGROUP_SIZE_2LOG == nbnxn_gpu_jgroup_size);

        GMX_ASSERT(sizeof(nbl->cj4[0].imei[0].imask)*8 >= nbnxn_gpu_jgroup_size*nbnxn_gpu_ncluster_per_cell, "The i super-cluster cluster interaction mask does not contain a sufficient number of bits");
        GMX_ASSERT(sizeof(nbl->excl[0])*8 >= nbnxn_gpu_jgroup_size*nbnxn_gpu_ncluster_per_cell, "The GPU exclusion mask does not contain a sufficient number of bits");

#ifdef NBNXN_PBB_SIMD4
        assert(nbnxn_gpu_ncluster_per_cell % 4 == 0);
#endif
    ...
    {
#ifdef NBNXN_BBXXXX
        snew_aligned(nbl->work->pbb_ci, nbnxn_gpu_ncluster_per_cell/STRIDE_PBB*NNBSBB_XXXX, NBNXN_SEARCH_BB_MEM_ALIGN);
#else
        snew_aligned(nbl->work->bb_ci, nbnxn_gpu_ncluster_per_cell, NBNXN_SEARCH_BB_MEM_ALIGN);
#endif
    }
    snew_aligned(nbl->work->x_ci, nbnxn_gpu_ncluster_per_cell*nbnxn_gpu_cluster_size*DIM, NBNXN_SEARCH_BB_MEM_ALIGN);
#if GMX_SIMD
    snew_aligned(nbl->work->x_ci_simd, NBNXN_CPU_CLUSTER_I_SIZE*DIM*GMX_SIMD_REAL_WIDTH, GMX_SIMD_REAL_WIDTH);
#endif
    snew_aligned(nbl->work->d2, nbnxn_gpu_ncluster_per_cell, NBNXN_SEARCH_BB_MEM_ALIGN);
                                             ...
                                             const nbnxn_search_t nbs, real rl)
{
    const nbnxn_grid_t *grid;
    int                 b;
    int                 c[nbnxn_gpu_ncluster_per_cell+1];
    ...
    sum_nsp2 = 0;
    nsp_max  = 0;
    for (int si = 0; si <= nbnxn_gpu_ncluster_per_cell; si++)
    {
        c[si] = 0;
    }
    for (int i = 0; i < nbl->nsci; i++)
    {
        ...
        for (int j4 = nbl->sci[i].cj4_ind_start; j4 < nbl->sci[i].cj4_ind_end; j4++)
        {
            for (int j = 0; j < nbnxn_gpu_jgroup_size; j++)
            {
                ...
                for (int si = 0; si < nbnxn_gpu_ncluster_per_cell; si++)
                {
                    if (nbl->cj4[j4].imei[0].imask & (1U << (j*nbnxn_gpu_ncluster_per_cell + si)))
                    {
                        b++;
                    }
                }
    ...
    if (nbl->ncj4 > 0)
    {
        for (b = 0; b <= nbnxn_gpu_ncluster_per_cell; b++)
        {
    ...
    for (int ej = 0; ej < nbl->na_ci; ej++)
    {
        ...
        for (int ei = ej; ei < nbl->na_ci; ei++)
        {
            excl[w]->pair[(ej & (nbnxn_gpu_jgroup_size-1))*nbl->na_ci + ei] &=
                ~(1U << (sj_offset*nbnxn_gpu_ncluster_per_cell + i_cluster_in_cell));
        }
    }
}
...
#define PRUNE_LIST_CPU_ONE
    ...
    for (int subc = 0; subc < gridj->nsubc[scj]; subc++)
    {
        ...
        cj = scj*nbnxn_gpu_ncluster_per_cell + subc;

        cj_gl = gridj->cell0*nbnxn_gpu_ncluster_per_cell + cj;
        ...
        *ndistc += na_c*2;
        ...
        for (int ci = 0; ci < nbnxn_gpu_ncluster_per_cell; ci++)
        {
            if (ci == ci1)
            {
                break;
            ...
            *ndistc += na_c*na_c;
...
#endif
            {
                ...
                imask |= (1U << (cj_offset*nbnxn_gpu_ncluster_per_cell + ci));
        ...
        if (npair == 1 && d2l[ci_last] >= rbb2)
        {
            ...
            {
                imask &= ~(1U << (cj_offset*nbnxn_gpu_ncluster_per_cell + ci_last));
                npair--;
            }
        }
#endif
                                   ...
                                   t_nblist               *nlist)
{
    ...
    for (int c = 0; c < nbnxn_gpu_ncluster_per_cell; c++)
    {
        c_abs = sci*nbnxn_gpu_ncluster_per_cell + c;
        ...
        for (int i = 0; i < nbl->na_ci; i++)
        {
            ...
            if (ai >= 0)
            {
                ...
                bFEP_i = (gridi->fep[c_abs - gridi->cell0*nbnxn_gpu_ncluster_per_cell] & (1 << i));
                ...
                for (int cj4_ind = cj4_ind_start; cj4_ind < cj4_ind_end; cj4_ind++)
                {
                    ...
                    for (int gcj = 0; gcj < nbnxn_gpu_jgroup_size; gcj++)
                    {
                        ...
                        if ((cj4->imei[0].imask & (1U << (gcj*nbnxn_gpu_ncluster_per_cell + c))) == 0)
                        {
                            /* Skip this ci for this cj */
                            continue;
                        }

                        cjr = cj4->cj[gcj] - gridj->cell0*nbnxn_gpu_ncluster_per_cell;
                        ...
                        if (bFEP_i || fep_cj != 0)
                        {
                            ...
                            {
                                /* Is this interaction perturbed and not excluded? */
                                ind_j = (gridj->cell0*nbnxn_gpu_ncluster_per_cell + cjr)*nbl->na_cj + j;
                                aj    = nbs->a[ind_j];
                                if (aj >= 0 &&
                                    (bFEP_i || (fep_cj & (1 << j))) &&
                                    (!bDiagRemoved || ind_j >= ind_i))
                                {
                                    ...
                                    excl_bit  = (1U << (gcj*nbnxn_gpu_ncluster_per_cell + c));
                              ...
                              const t_blocka      *excl)
{
    ...
    ndirect = 0;
    while (cj_ind_first + ndirect <= cj_ind_last &&
           nbl_cj(nbl, cj_ind_first+ndirect) == sci*nbnxn_gpu_ncluster_per_cell + ndirect)
    {
        ndirect++;
    }
            ...
            for (int eind = excl->index[ai]; eind < excl->index[ai+1]; eind++)
            {
                ...
                if (se >= cj_first && se <= cj_last)
                {
                    ...
                    if (found >= 0)
                    {
                        ...
                        if (nbl_imask0(nbl, found) & (1U << (cj_mod_cj4(found)*nbnxn_gpu_ncluster_per_cell + si)))
                        {
                            w       = (inner_e >> 2);
                            ...
                            nbl_excl->pair[a_mod_wj(inner_e)*nbl->na_ci+inner_i] &=
                                ~(1U << (cj_mod_cj4(found)*nbnxn_gpu_ncluster_per_cell + si));
                        }
                    }
                }
            }
                            ...
                            int thread, int nthread)
{
     ...
     */
    nsp_max += nbnxn_gpu_ncluster_per_cell*nbnxn_gpu_jgroup_size/2;

    cj4_start = nbl->sci[nbl->nsci-1].cj4_ind_start;
    cj4_end   = nbl->sci[nbl->nsci-1].cj4_ind_end;
    j4len     = cj4_end - cj4_start;

    if (j4len > 1 && j4len*nbnxn_gpu_ncluster_per_cell*nbnxn_gpu_jgroup_size > nsp_max)
    {
        ...
        for (int cj4 = cj4_start; cj4 < cj4_end; cj4++)
        {
            ...
            nsp_cj4   = 0;
            for (int p = 0; p < nbnxn_gpu_ncluster_per_cell*nbnxn_gpu_jgroup_size; p++)
            {
...
static void set_icell_bbxxxx_supersub(const float *bb, int ci,
                                      ...
                                      float *bb_ci)
{
    int ia = ci*(nbnxn_gpu_ncluster_per_cell >> STRIDE_PBB_2LOG)*NNBSBB_XXXX;
    for (int m = 0; m < (nbnxn_gpu_ncluster_per_cell >> STRIDE_PBB_2LOG)*NNBSBB_XXXX; m += NNBSBB_XXXX)
    {
...
static void set_icell_bb_supersub(const nbnxn_bb_t *bb, int ci,
                                  ...
                                  nbnxn_bb_t *bb_ci)
{
    for (int i = 0; i < nbnxn_gpu_ncluster_per_cell; i++)
    {
        set_icell_bb_simple(bb, ci*nbnxn_gpu_ncluster_per_cell+i,
                            shx, shy, shz,
                            &bb_ci[i]);
    }
}
                                 ...
                                 nbnxn_list_work_t *work)
{
    ...
    int    ia = ci*nbnxn_gpu_ncluster_per_cell*na_c;
    for (int i = 0; i < nbnxn_gpu_ncluster_per_cell*na_c; i++)
    {
                                       ...
                                       nbnxn_list_work_t *work)
{
    ...
    for (int si = 0; si < nbnxn_gpu_ncluster_per_cell; si++)
    {
        ...
        {
            int io = si*na_c + i;
            int ia = ci*nbnxn_gpu_ncluster_per_cell*na_c + io;
...
static real minimum_subgrid_size_xy(const nbnxn_grid_t *grid)
{
    ...
    {
        return std::min(grid->sx/nbnxn_gpu_ncluster_per_cell_x,
                        grid->sy/nbnxn_gpu_ncluster_per_cell_y);
    }
}
                                ...
                                int                  *nsubpair_tot_est)
{
    ...
    ls[XX] = (grid->c1[XX] - grid->c0[XX])/(grid->ncx*nbnxn_gpu_ncluster_per_cell_x);
    ls[YY] = (grid->c1[YY] - grid->c0[YY])/(grid->ncy*nbnxn_gpu_ncluster_per_cell_y);
                                        ...
                                        gmx_bool            simple)
{
    ...
    if (!simple)
    {
        bbx /= nbnxn_gpu_ncluster_per_cell_x;
        bby /= nbnxn_gpu_ncluster_per_cell_y;
    }
