 ...
 */
static void rebalanceSimpleLists(int                              ncjTotal,
                                 int                              numLists,
                                      ...
                                      nbnxn_pairlist_set_t *listSet)
{
    ...
    {
        rebalanceSimpleLists(ncjTotal, numLists,
                             listSet->nbl, listSet->nbl_work, nbs->work);

        /* Swap the pointer of the sets of pair lists */
        nbnxn_pairlist_t **tmp = listSet->nbl;
        listSet->nbl           = listSet->nbl_work;
        listSet->nbl_work      = tmp;
    }
}
...
#pragma omp parallel for num_threads(nnbl) schedule(static)
    ...
    if (nbl_list->bSimple)
    {
        if (nnbl > 1)
        {
            checkRebalanceSimpleLists(nbs, nbl_list);
        }
    }
    else
    {
...
#pragma omp parallel for num_threads(nnbl) schedule(static)
    ...
    if (debug && nbl_list->nnbl > 1)
    {
        if (nbl[0]->bSimple)
        {
            print_nblist_statistics_simple(debug, nbl_list->nbl[0], nbs, rlist);
        }
        else
        {
    ...
    if (debug)
    {
        if (gmx_debug_at)
        {
            if (nbl[0]->bSimple)
            {
                print_nblist_ci_cj(debug, nbl_list->nbl[0]);
            }
            else
            {
