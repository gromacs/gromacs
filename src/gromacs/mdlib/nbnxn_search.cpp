                                 ...
                                 int csj, const nbnxn_bb_t *bb_j_all)
{
    ...
    bb_i_S0 = simd4LoadF(&bb_i_ci[si].lower[0]);
    bb_i_S1 = simd4LoadF(&bb_i_ci[si].upper[0]);
    bb_j_S0 = simd4LoadF(&bb_j_all[csj].lower[0]);
    bb_j_S1 = simd4LoadF(&bb_j_all[csj].upper[0]);

    dl_S    = simd4SubF(bb_i_S0, bb_j_S1);
    dh_S    = simd4SubF(bb_j_S0, bb_i_S1);

    dm_S    = simd4MaxF(dl_S, dh_S);
    dm0_S   = simd4MaxF(dm_S, simd4SetZeroF());

    return simd4DotProductF(dm0_S, dm0_S);
}
...
#define SUBC_BB_DIST2_SIMD4_XXXX_INNER(si, bb_i, d2) \
    {                                                \
        ...
        shi = si*NNBSBB_D*DIM;                       \
                                                 \
        xi_l = simd4LoadF(bb_i+shi+0*STRIDE_PBB);   \
        yi_l = simd4LoadF(bb_i+shi+1*STRIDE_PBB);   \
        zi_l = simd4LoadF(bb_i+shi+2*STRIDE_PBB);   \
        xi_h = simd4LoadF(bb_i+shi+3*STRIDE_PBB);   \
        yi_h = simd4LoadF(bb_i+shi+4*STRIDE_PBB);   \
        zi_h = simd4LoadF(bb_i+shi+5*STRIDE_PBB);   \
                                                 \
        dx_0 = simd4SubF(xi_l, xj_h);                 \
        dy_0 = simd4SubF(yi_l, yj_h);                 \
        dz_0 = simd4SubF(zi_l, zj_h);                 \
                                                 \
        dx_1 = simd4SubF(xj_l, xi_h);                 \
        dy_1 = simd4SubF(yj_l, yi_h);                 \
        dz_1 = simd4SubF(zj_l, zi_h);                 \
                                                 \
        mx   = simd4MaxF(dx_0, dx_1);                 \
        my   = simd4MaxF(dy_0, dy_1);                 \
        mz   = simd4MaxF(dz_0, dz_1);                 \
                                                 \
        m0x  = simd4MaxF(mx, zero);                   \
        m0y  = simd4MaxF(my, zero);                   \
        m0z  = simd4MaxF(mz, zero);                   \
                                                 \
        d2x  = simd4MulF(m0x, m0x);                   \
        d2y  = simd4MulF(m0y, m0y);                   \
        d2z  = simd4MulF(m0z, m0z);                   \
                                                 \
        d2s  = simd4AddF(d2x, d2y);                   \
        d2t  = simd4AddF(d2s, d2z);                   \
                                                 \
        simd4StoreF(d2+si, d2t);                      \
    }
                                     ...
                                     float *d2)
{
    ...
    xj_l = simd4Set1F(bb_j[0*STRIDE_PBB]);
    yj_l = simd4Set1F(bb_j[1*STRIDE_PBB]);
    zj_l = simd4Set1F(bb_j[2*STRIDE_PBB]);
    xj_h = simd4Set1F(bb_j[3*STRIDE_PBB]);
    yj_h = simd4Set1F(bb_j[4*STRIDE_PBB]);
    zj_h = simd4Set1F(bb_j[5*STRIDE_PBB]);
...
static gmx_bool subc_in_range_x(int na_c,
                                ...
                                real rl2)
{
    for (int i = 0; i < na_c; i++)
    {
        int i0 = (si*na_c + i)*DIM;
        for (int j = 0; j < na_c; j++)
        {
...
static gmx_bool subc_in_range_simd4(int na_c,
                                    ...
                                    real rl2)
{
    gmx::Simd4Real   ix_S0, iy_S0, iz_S0;
    gmx::Simd4Real   ix_S1, iy_S1, iz_S1;
    ...
    rc2_S   = gmx::simd4Set1(rl2);

    dim_stride = NBNXN_GPU_CLUSTER_SIZE/STRIDE_PBB*DIM;
    ix_S0      = gmx::simd4Load(x_i+(si*dim_stride+0)*STRIDE_PBB);
    iy_S0      = gmx::simd4Load(x_i+(si*dim_stride+1)*STRIDE_PBB);
    iz_S0      = gmx::simd4Load(x_i+(si*dim_stride+2)*STRIDE_PBB);
    ix_S1      = gmx::simd4Load(x_i+(si*dim_stride+3)*STRIDE_PBB);
    iy_S1      = gmx::simd4Load(x_i+(si*dim_stride+4)*STRIDE_PBB);
    iz_S1      = gmx::simd4Load(x_i+(si*dim_stride+5)*STRIDE_PBB);
    ...
    while (j0 < j1)
    {
        gmx::Simd4Real jx0_S, jy0_S, jz0_S;
        gmx::Simd4Real jx1_S, jy1_S, jz1_S;

        gmx::Simd4Real dx_S0, dy_S0, dz_S0;
        gmx::Simd4Real dx_S1, dy_S1, dz_S1;
        gmx::Simd4Real dx_S2, dy_S2, dz_S2;
        gmx::Simd4Real dx_S3, dy_S3, dz_S3;

        gmx::Simd4Real rsq_S0;
        gmx::Simd4Real rsq_S1;
        gmx::Simd4Real rsq_S2;
        gmx::Simd4Real rsq_S3;

        gmx::Simd4Bool wco_S0;
        gmx::Simd4Bool wco_S1;
        gmx::Simd4Bool wco_S2;
        gmx::Simd4Bool wco_S3;
        gmx::Simd4Bool wco_any_S01, wco_any_S23, wco_any_S;

        jx0_S = gmx::simd4Set1(x_j[j0*stride+0]);
        jy0_S = gmx::simd4Set1(x_j[j0*stride+1]);
        jz0_S = gmx::simd4Set1(x_j[j0*stride+2]);

        jx1_S = gmx::simd4Set1(x_j[j1*stride+0]);
        jy1_S = gmx::simd4Set1(x_j[j1*stride+1]);
        jz1_S = gmx::simd4Set1(x_j[j1*stride+2]);

        /* Calculate distance */
        dx_S0            = gmx::simd4Sub(ix_S0, jx0_S);
        dy_S0            = gmx::simd4Sub(iy_S0, jy0_S);
        dz_S0            = gmx::simd4Sub(iz_S0, jz0_S);
        dx_S1            = gmx::simd4Sub(ix_S1, jx0_S);
        dy_S1            = gmx::simd4Sub(iy_S1, jy0_S);
        dz_S1            = gmx::simd4Sub(iz_S1, jz0_S);
        dx_S2            = gmx::simd4Sub(ix_S0, jx1_S);
        dy_S2            = gmx::simd4Sub(iy_S0, jy1_S);
        dz_S2            = gmx::simd4Sub(iz_S0, jz1_S);
        dx_S3            = gmx::simd4Sub(ix_S1, jx1_S);
        dy_S3            = gmx::simd4Sub(iy_S1, jy1_S);
        dz_S3            = gmx::simd4Sub(iz_S1, jz1_S);

        /* rsq = dx*dx+dy*dy+dz*dz */
        rsq_S0           = gmx::simd4CalcRsq(dx_S0, dy_S0, dz_S0);
        rsq_S1           = gmx::simd4CalcRsq(dx_S1, dy_S1, dz_S1);
        rsq_S2           = gmx::simd4CalcRsq(dx_S2, dy_S2, dz_S2);
        rsq_S3           = gmx::simd4CalcRsq(dx_S3, dy_S3, dz_S3);

        wco_S0           = gmx::simd4CmpLt(rsq_S0, rc2_S);
        wco_S1           = gmx::simd4CmpLt(rsq_S1, rc2_S);
        wco_S2           = gmx::simd4CmpLt(rsq_S2, rc2_S);
        wco_S3           = gmx::simd4CmpLt(rsq_S3, rc2_S);

        wco_any_S01      = gmx::simd4OrB(wco_S0, wco_S1);
        wco_any_S23      = gmx::simd4OrB(wco_S2, wco_S3);
        wco_any_S        = gmx::simd4OrB(wco_any_S01, wco_any_S23);

        if (gmx::simd4AnyTrueB(wco_any_S))
        {
            return TRUE;
        }
                                             ...
                                             const nbnxn_search_t nbs, real rl)
{
    ...
    sum_nsp2 = 0;
    nsp_max  = 0;
    for (int si = 0; si <= GPU_NSUBCELL; si++)
    {
        c[si] = 0;
    }
...
#define PRUNE_LIST_CPU_ONE
    ...
    for (int cjo = 0; cjo < gridj->nsubc[scj]; cjo++)
    {
        ...
        *ndistc += na_c*2;
        ...
        for (int ci = 0; ci < GPU_NSUBCELL; ci++)
        {
            if (ci == ci1)
            {
                break;
                            ...
                            int thread, int nthread)
{
    ...
    if (j4len > 1 && j4len*GPU_NSUBCELL*NBNXN_GPU_JGROUP_SIZE > nsp_max)
    {
        ...
        for (int cj4 = cj4_start; cj4 < cj4_end; cj4++)
        {
            ...
            nsp_cj4   = 0;
            for (int p = 0; p < GPU_NSUBCELL*NBNXN_GPU_JGROUP_SIZE; p++)
            {
...
static void set_icell_bbxxxx_supersub(const float *bb, int ci,
                                      ...
                                      float *bb_ci)
{
    int ia = ci*(GPU_NSUBCELL>>STRIDE_PBB_2LOG)*NNBSBB_XXXX;
    for (int m = 0; m < (GPU_NSUBCELL>>STRIDE_PBB_2LOG)*NNBSBB_XXXX; m += NNBSBB_XXXX)
    {
                                 ...
                                 nbnxn_list_work_t *work)
{
    ...
    int    ia = ci*GPU_NSUBCELL*na_c;
    for (int i = 0; i < GPU_NSUBCELL*na_c; i++)
    {
                                       ...
                                       nbnxn_list_work_t *work)
{
    ...
    for (int si = 0; si < GPU_NSUBCELL; si++)
    {
        for (int i = 0; i < na_c; i += STRIDE_PBB)
        {
            int io = si*na_c + i;
            int ia = ci*GPU_NSUBCELL*na_c + io;
            for (int j = 0; j < STRIDE_PBB; j++)
            {
...
static real minimum_subgrid_size_xy(const nbnxn_grid_t *grid)
{
    ...
    {
        return std::min(grid->sx/GPU_NSUBCELL_X, grid->sy/GPU_NSUBCELL_Y);
    }
}
