...
/* Plain C function which determines if any atom pair between two clusters
 * is within distance sqrt(rl2).
 */
static gmx_bool clusterpair_in_range(int na_c,
...
#ifdef NBNXN_SEARCH_BB_SIMD4
/* 4-wide SIMD function which determines if any atom pair between two clusters,
                                     ...
                                     real rl2)
{
    ...
    assert(na_c == nbnxn_gpu_cluster_size);
                                           ...
                                           int               ncell)
{
     ...
     */
    ncj4_max = ((nbl->work->cj_ind + ncell*gpu_ncluster_per_cell + nbnxn_gpu_jgroup_size - 1) >> NBNXN_GPU_JGROUP_SIZE_2LOG);
                                ...
                                nbnxn_free_t     *free)
{
    ...
    snew_aligned(nbl->work->x_ci, gpu_ncluster_per_cell*nbnxn_gpu_cluster_size*DIM, NBNXN_SEARCH_BB_MEM_ALIGN);
#if GMX_SIMD
...
#define PRUNE_LIST_CPU_ONE
    ...
    for (int subc = 0; subc < gridj->nsubc[scj]; subc++)
    {
        ...
        cj = scj*gpu_ncluster_per_cell + subc;

        cj_gl = gridj->cell0*gpu_ncluster_per_cell + cj;
        ...
        *ndistc += na_c*2;
        ...
        for (int ci = 0; ci < gpu_ncluster_per_cell; ci++)
        {
            ...
            *ndistc += na_c*na_c;
            if (d2 < rbb2 ||
                (d2 < rl2 &&
                 clusterpair_in_range(na_c, ci, x_ci, cj_gl, stride, x, rl2)))
#else
        ...
        if (npair == 1 && d2l[ci_last] >= rbb2 &&
            !clusterpair_in_range(na_c, ci_last, x_ci, cj_gl, stride, x, rl2))
        {
                                       ...
                                       nbnxn_list_work_t *work)
{
    ...
    for (int si = 0; si < gpu_ncluster_per_cell; si++)
    {
        ...
        {
            int io = si*na_c + i;
            int ia = ci*gpu_ncluster_per_cell*na_c + io;
