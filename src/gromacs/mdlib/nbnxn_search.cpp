                            ...
                            real atom_density)
{
    rvec size;
    int  na_c;
    real tlen, tlen_x, tlen_y, nc_max;
    ...
    if (n > grid->na_sc)
    {
        ...
        if (grid->bSimple)
        {
            ...
            tlen   = std::pow(na_c/atom_density, 1.0/3.0);
            tlen_x = tlen;
            tlen_y = tlen;
        }
        else
        {
            ...
            tlen   = std::pow(grid->na_c/atom_density, 1.0/3.0);
         ...
         */
        grid->ncx = std::max(1, (int)(size[XX]/tlen_x));
        grid->ncy = std::max(1, (int)(size[YY]/tlen_y));
    }
    else
    {
        grid->ncx = 1;
                                ...
                                int *cxy_na)
{
    ...
    int n0 = a0 + (int)((thread+0)*(a1 - a0))/nthread;
    int n1 = a0 + (int)((thread+1)*(a1 - a0))/nthread;
    if (dd_zone == 0)
    {
        /* Home zone */
        for (int i = n0; i < n1; i++)
        {
            if (move == NULL || move[i] >= 0)
            {
                 ...
                 */
                int cx = (int)((x[i][XX] - grid->c0[XX])*grid->inv_sx);
                int cy = (int)((x[i][YY] - grid->c0[YY])*grid->inv_sy);
                           ...
                           real d2, real r2, int *cf, int *cl)
{
    ...
    *cl = std::min((int)((b1 - c0)*invs), nc-1);
                          ...
                          t_nblist               *nlist)
{
    ...
    if ((size_t)ngid*gridj->na_cj > sizeof(gid_cj)*8)
    {
...
real nbnxn_get_rlist_effective_inc(int cluster_size_j, real atom_density)
{
    ...
    return nbnxn_rlist_inc_outside_fac*std::pow(vol_inc_i + vol_inc_j, 1.0/3.0);
}
                                ...
                                int                  *nsubpair_tot_est)
{
    ...
    if (LOCAL_I(iloc))
    {
        ...
        /* 4 octants of a sphere */
        vol_est += 0.5*4.0/3.0*M_PI*std::pow(r_eff_sup, 3);
