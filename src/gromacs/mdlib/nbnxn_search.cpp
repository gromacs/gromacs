...
/* Returns the index in the coordinate array corresponding to the j-cluster index */template<int cj_size> static gmx_inline int x_ind_cj(int cj)
{
    if (cj_size == 2)
    {
        /* Coordinates are stored packed in groups of 4 */
...
#define PRUNE_LIST_CPU_ONE
    ...
    {
        cj4_ind   = nbl->work->cj_ind/nbnxn_gpu_jgroup_size;
...
#ifdef GMX_NBNXN_SIMD_4XN
                                        case nbnxnk4xN_SIMD_4xN:
                                            check_cell_list_space_simple(nbl, ci_to_cj(cl - cf, na_cj_2log)+2);
