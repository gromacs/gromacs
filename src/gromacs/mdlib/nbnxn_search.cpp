...
{
    Cpu4x4,   // i-cluster size 4, j-cluster size 4
    Simd4xN,  // i-cluster size 4, j-cluster size SIMD width
    Simd2xNN, // i-cluster size 4, j-cluster size half SIMD width
    Gpu8x8    // i-cluster size 8, j-cluster size 8 + super-clustering
};

#if GMX_SIMD

/* Returns the j-cluster size for the Simd4xN layout */
static constexpr int jClusterSize(std::integral_constant<NbnxnLayout, NbnxnLayout::Simd4xN> gmx_unused layout)
{
    return GMX_SIMD_REAL_WIDTH;
}

/* Returns the j-cluster size for the Simd4xN layout */
static constexpr int jClusterSize(std::integral_constant<NbnxnLayout, NbnxnLayout::Simd2xNN> gmx_unused layout)
{
...
static inline int cjFromCi(int ci)
{
    ...
    else if (jClusterSize == NBNXN_CPU_CLUSTER_I_SIZE*2)
    {
        return ci >> 1;
    }
}
...
static inline int cjFromCi(int ci)
{
    constexpr int clusterSize = jClusterSize(std::integral_constant<NbnxnLayout, layout>());
...
static inline int xIndexFromCj(int cj)
{
    ...
    else if (clusterSize == NBNXN_CPU_CLUSTER_I_SIZE*2)
    {
    ...
    for (int i = 0; i < c_superClusterSize; i++)
    {
        ...
        if (iAtom >= 0)
        {
            ...
            for (int exclIndex = exclusions.index[iAtom]; exclIndex < exclusions.index[iAtom + 1]; exclIndex++)
            {
                 ...
                 */
                if (diagRemoved && iSuperCluster*nbl->na_sc + i)
                {
                    continue;
                }
