 ...
 */
static void makeClusterListSimple(const nbnxn_grid_t *gridj,
            ...
            for (int ty = -shp[YY]; ty <= shp[YY]; ty++)
            {
                ...
                for (int tx = -shp[XX]; tx <= shp[XX]; tx++)
                {
                    ...
                    for (int cx = cxf; cx <= cxl; cx++)
                    {
                        ...
                        {
                            int c0 = gridj->cxy_ind[cx*gridj->ncy+cy];
                            int c1 = gridj->cxy_ind[cx*gridj->ncy+cy+1];
                            ...
                            if (c1 > c0 && d2zxy < rlist2)
                            {
                                 ...
                                 */
                                int cs = c0 + static_cast<int>(bz1_frac*(c1 - c0));
                                if (cs >= c1)
                                {
                                    cs = c1 - 1;
                                }
...
#define NBNXN_REFCODE 0
                                ...
                                if (gridi == gridj)
                                {
                                    ...
                                    if (!pbc_shift_backward || shift == CENTRAL)
                                    {
                                        cFirst = std::max(cFirst, ci);
                                    }
                                }

                                if (cFirst <= cLast)
                                {
                                    GMX_ASSERT(cFirst >= c0 && cLast < c1, "The range should reside within the current grid column");
                                    ...
                                    if (nbl->bSimple)
                                    {
                                         ...
                                         */
                                        check_cell_list_space_simple(nbl, 2*(cLast - cFirst + 1));
                                    }
                                    else
                                    {
                                        check_cell_list_space_supersub(nbl, cLast - cFirst + 1);
                                    }
