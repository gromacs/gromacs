...
{
    char *buf;
    int   env_length; /* lenght of the processed environment variable string */

    /* ORCA directory on the system */
    /* let it be the first env var processed -
     * on a success no files are modified/deleted
     */
    buf = getenv("GMX_ORCA_PATH");
    if (buf)
    {
        env_length = strlen(buf);
        snew(qm->orca_dir, env_length); /* mem01 alloc - do not free */
        memcpy(qm->orca_dir, buf, env_length);
    }
    else
    {
    ...
    {
        env_length = strlen(buf);
        snew(qm->orca_basename, env_length); /* mem02 alloc - do not free */
        memcpy(qm->orca_basename, buf, env_length);
        /* since we append the output to the BASENAME.out file,
           we should delete an existent old out-file here. */
        snew(buf, env_length+4); /* mem03 alloc - 4 extra for the extension */
        sprintf(buf, "%s.out", qm->orca_basename);
        remove(buf);
        sfree(buf); /* mem03 free */
    }
    else
    {
...
void write_orca_input(t_forcerec *fr, t_QMrec *qm, t_MMrec *mm)
{
    ...
    FILE      *out, *iofile;                  /* besides the out, only one file handle is
                                               ...
                                               */
    char      buf[200],
    *filename,                                /* name of the currently used file */
    *filename_ab;                             /* pointer in the filename for changing
                                               * only the file extension without writing the
                                               * basename each time
                                               */
    char       exclInName[] = "QMMMexcl.dat"; /* no need for dynamic alloc */
    t_QMMMrec *QMMMrec      = fr->qr;

    orca_b_length = strlen(qm->orca_basename);
    snew(filename, orca_b_length+10); /* mem01 alloc */
                                      /* extra 10 for the file extensions */
    filename_ab = filename + orca_b_length;
    memcpy(filename, qm->orca_basename, orca_b_length);
                      ...
                      t_QMrec *qm, t_MMrec *mm)
{
    ...
    char       buf[300], *filename, *filename_ab,
               err_msg1[] = "Unexpected end of ORCA output";
    real       QMener;
    ...
    orca_b_length = strlen(qm->orca_basename);
    snew(filename, orca_b_length+10); /* mem01 alloc */
    /* pointer in the filename just after the basename */
// cppcheck-suppress redundantCopy
    filename_ab = filename + orca_b_length;
    memcpy(filename, qm->orca_basename, orca_b_length);
...
// cppcheck-suppress redundantCopy
        ...
        for (i = 0; i < qm->nrQMatoms; i++)
        {
            if (fscanf(ifile, /* one step file input, no need for a buffer */
#if GMX_DOUBLE
                       "%s%lf%lf%lf",
#else
                       "%s%f%f%f",
...
// cppcheck-suppress redundantCopy
    ...
    for (i = 0; i < qm->nrQMatoms; i++)
    {
        if (fscanf(ifile,
#if GMX_DOUBLE
                   "%lf%lf%lf", /* take it easy, it really goes over newlines */
#else
                   "%f%f%f",
#endif
                   &QMgrad[i][XX],
