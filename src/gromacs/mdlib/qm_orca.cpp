...
void init_orca(t_QMrec *qm)
{
    ...
    qm->orca_dir = gmx_strdup(envOrcaPath);
    ...
    qm->orca_basename = gmx_strdup(envOrcaBasename);
...
void write_orca_input(t_forcerec *fr, t_QMrec *qm, t_MMrec *mm)
{
    ...
    {
        gmx_fatal(FARGS, "Cannot open ORCA input (%s) for writing.", fileName.c_str());
    }
    ...
    {
        gmx_fatal(FARGS, "No information on the calculation given in %s.", fileName.c_str());
    }
    while (!feof(iofile))
    {
...
void do_orca(const char *orca_dir, const char *basename)
{
    std::string systemCommand(orca_dir);

    systemCommand += "/orca " + std::string(basename) + ".inp >> " + std::string(basename) + ".out";
