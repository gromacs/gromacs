...
void init_orca(t_QMrec *qm)
{
    ...
    /* let it be the first env var processed - on a success no files are modified/deleted */
    ...
    if (buf)
    {
        ...
        snew(buf, env_length+4); /* mem03 alloc - 4 extra bytes for the file extension */
...
void write_orca_input(t_forcerec *fr, t_QMrec *qm, t_MMrec *mm)
{
    ...
    FILE      *out, *iofile; /* besides the out, only one file handle is being used at a time */
    char      buf[200], 
              *filename, /* name of the currently used file */
              *filename_ab, /* pointer in the filename allowing for changing only the file extension without writing the basename each time */ 
              exclInName[] = "QMMMexcl.dat"; /* its constant, no need for dynamic allocation */
    t_QMMMrec *QMMMrec = fr->qr;

    orca_b_length = strlen(qm->orca_basename);
    snew(filename, orca_b_length+10); /* extra 10 bytes for filename extensions */ /* mem01 alloc */
    ...
    if (qm->bTS || qm->bOPT)
    {
        ...
        if (QMMMrec->QMMMscheme != eQMMMschemeoniom && mm->nrMMatoms)
        {
            ...
            for (i = 0; i < qm->nrQMatoms; i++)
            {
                fprintf(iofile,
#if GMX_DOUBLE
                        "%10.7lf  %10.7lf\n", /* avoid compiler warnings about discrepancy between format strings and data types */
#else
                        "%10.7f  %10.7f\n",
#endif
                        qm->c6[i], qm->c12[i]);
            ...
            fprintf(out, "%s%s%s\n", "%LJCOEFFICIENTS \"", filename, "\""); /* let ORCA know where LJ coeffs are */
        }
    }
    ...
    if (QMMMrec->QMMMscheme != eQMMMschemeoniom && mm->nrMMatoms)
    {
        ...
        fprintf(out, "%s%s%s\n", "%pointcharges \"", filename, "\""); /* let ORCA know where the point charges are */
    }
    fprintf(out, "\n");
                      ...
                      t_QMrec *qm, t_MMrec *mm)
{
    ...
    filename_ab = filename + orca_b_length; /* pointer in the filename just after the basename */
    memcpy(filename, qm->orca_basename, orca_b_length);
        ...
        for (i = 0; i < qm->nrQMatoms; i++)
        {
            if(fscanf(ifile, /* one step file input, no need for a buffer */
                      ...
                      "%s%f%f%f",
                       ...
                       &qm->xQM[i][XX],
                       &qm->xQM[i][YY],
                       &qm->xQM[i][ZZ])!=4) /* if it didn't process 4 entries, its bad */
            {
                gmx_fatal(FARGS, err_msg1);
            }
            for (j = 0; j < DIM; j++)
