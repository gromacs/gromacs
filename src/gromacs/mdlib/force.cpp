...
#define TAKETIME FALSE
    ...
    if (ir->nwall)
    {
        ...
        real dvdl_walls = do_walls(*ir, *fr, box, *md, x, forceWithVirial,
                                   lambda[efptVDW],
    ...
    do_force_listed(wcycle, box, ir->fepvals, cr,
                    idef, (const rvec *) x, hist, f, forceWithVirial, fr,
...
#pragma omp parallel for num_threads(nthreads) schedule(static)
                for (t = 0; t < nthreads; t++)
                {
                    try
                    {
                         ...
                         * the forces in the normal, single forceWithVirial->force array.
                         */
        ...
        {
            Vlr_q = do_ewald(ir, x, as_rvec_array(forceWithVirial->force.data()),
                             md->chargeA, md->chargeB,
        ...
        if (flags & GMX_FORCE_VIRIAL)
        {
            m_add(forceWithVirial->virial, ewaldOutput.vir_q, forceWithVirial->virial);
            m_add(forceWithVirial->virial, ewaldOutput.vir_lj, forceWithVirial->virial);
        }
