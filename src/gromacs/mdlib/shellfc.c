...
typedef struct gmx_shellfc {
  ...
  /* ASPC-related data */
  gmx_bool    bAutoOmega;  /* Autoset guaranteed stable SCF relaxation parameter? */
  real    scf_omega;       /* SCF relaxation factor                    */
  int     aspc_k;          /* ASPC predictor order (predictor length - 2) */
  int     aspc_pass;       /* Current predictor order, ramps up from 0 to aspc_k at simulation start */
  rvec    **aspc_hist;     /* History of shell displacements, aspc_hist[aspc_k+2][nshell] */
  real    *aspc_coeff;     /* aspc_k+2 predictor coefficients (B[i] in Kolafa2004) */
} t_gmx_shellfc;
		       ...
		       struct gmx_shellfc *shfc)
{
  ...
  if (PAR(cr)) {
    if (DOMAINDECOMP(cr)) {
      if (shfc->aspc_k > 0) {
        gmx_fatal(FARGS,"ASPC not yet implemented with domain decomposition");
      }
      dd = cr->dd;
      a0 = 0;
      a1 = dd->nat_home;
...
/*!
 * \brief Update shell positions by one damped SCF iteration
 * \param shfc shellfc context
 * \param xcur original atom positions
 * \param xnew updated atom positions
 * \param f forces
 * \param ns number of shells
 * \param s shell descriptions
 *
 * Only the positions of shells are set in xnew, other atoms are untouched
 * (i.e. not copied from xcur).
 */
...
/*!
 * \brief Predict new shell positions using ASPC
 * \param log log stream
 * \param shfc shellfc context
 * \param x original atom positions
 * \param ns number of shells
 * \param s shell descriptions
 *
 * Shell positions in x[] are updated in place, other atoms untouched.
 *
 * At simulation start, the predictor length is gradually increased as
 * longer history becomes available, up to the final aspc_k+2 steps.
 * Every time the predictor length is increased, its coefficients are
 * recomputed here and SCF relaxation parameter is autoset to ensure
 * stability (only if bAutoOmega is TRUE, i.e. omega was not set manually).
 */
static void aspc_predict(FILE *log, struct gmx_shellfc *shfc, rvec x[], int ns, t_shell s[])
{
  ...
  /* If we're not yet at the desired predictor length, make it a step longer */
  if (shfc->aspc_pass < shfc->aspc_k) {
    ...
    /* If scf_omega was not set manually, use the stable value */
    if (shfc->bAutoOmega)
      shfc->scf_omega = o;
...
/*!
 * \brief Record final dipole moments for ASPC history
 * \param shfc shellfc context
 * \param x atom positions
 * \param ns number of shells
 * \param s shell descriptions
 *
 * Shell displacements relative to the nucleus the shell is bound to are
 * stored in aspc_hist - the ASPC predictor works with dipole moments
 * (relative displacements), not absolute shell positions.
 */
...
#define  Try (1-Min)             /* At start Try = 1 */
  ...
  if (nat > shfc->x_nalloc) {
    ...
    if (bASPC) {
      /* Record current shell positions as the first ASPC history point */
      /* Hopefully those are corrected positions from a previous run (if not,
       * nothing bad happens, the predictor does nothing in the first step)
       */
  ...
  *bConverged = (df[Min] < ftol);
  ...
  if (*bConverged && bASPC) {
    /* At least one corrector step is needed after prediction to keep ASPC working */
    /* This is cheap as we don't need to recompute forces, they're already converged */
  ...
  for(count=1; (!(*bConverged) && (count < number_steps)); count++) {
    ...
    if (bASPC) {
      /* New shell positions using simple SCF corrector */
    ...
    *bConverged = (df[Try] < ftol);
    ...
    /* Always accept the step with ASPC (decrease_step_size does not make sense there) */
  ...
  if (bASPC) {
    /* Record final dipoles for ASPC history */
