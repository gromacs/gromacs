...
dd_isend_zone_rvec(const gmx_domdec_t gmx_unused *dd,
                   const zone_comm_t gmx_unused *send, rvec gmx_unused *buf_s,
                   int tag,
                   MPI_Request gmx_unused *req, int gmx_unused *nreq)
{
#ifdef GMX_MPI
    if (send->natoms > 0)
    {
...
static void dd_halo_send_x_zone(gmx_domdec_t *dd, int zone,
                                ...
                                MPI_Request *req, int *nreq,
                                gmx_bool gmx_unused bCheck)
{
                             ...
                             gmx_bool bCheck)
{
    ...
    for (zone = 1; zone < dd->comm->zones.n; zone++)
    {
        ...
        dd_halo_send_x_zone(dd, zone, box, x,
                            zmpid->req, &zmpid->nreq,
                            bCheck);
    }
}
...
void dd_halo_move_x(gmx_domdec_t *dd, matrix box, rvec x[])
{
    ...
    dd_halo_initiate_send_x(dd, box, x, TRUE);
...
void dd_halo_move_f(gmx_domdec_t *dd, rvec f[], rvec fshift[])
{
    ...
    dd_halo_initiate_send_f(dd, f, TRUE);
                             ...
                             gmx_bool *bCornersDiffer)
{
    ...
    for (dim = 0; dim < DIM; dim++)
    {
        ...
        *bCornersDiffer = (corner_mb_d != corner_2b_d);
        ...
        for (d = 0; d < DIM; d++)
        {
             ...
             */
            if (!(dd->ndim == 1 && ((dd->dim[0] == XX && d == YY) ||
                                    (dd->dim[0] == YY && d == XX))))
            {
                corner_2b[d]   += corner_2b_d*box[dim][d]/box[dim][dim];
                corner_mb[d]   += corner_mb_d*box[dim][d]/box[dim][dim];
                corner_zone[d] += corner_z_d *box[dim][d]/box[dim][dim];
            }
        }
    }
    ...
    if (debug)
    {
        fprintf(debug, "halo zone %d corner 2b %6.3f %6.3f %6.3f mb %6.3f %6.3f %6.3f\n",
                zone,
...
setup_zone_comm(gmx_domdec_t *dd,
                ...
                real rc2_nonbonded, real rc2_bonded,
                const matrix box,
                const ivec tric_dir,
                const rvec *normal,
                const rvec skew_fac,
                ...
                zone_comm_t *recv)
{
    ...
    /* send->ncx is potentially lowered below */
    send->ncolumn = 0;
    send->natoms  = 0;
    ...
    for (cxy = 0; cxy < ncx*ncy; cxy++)
    {
        ...
        float      *bb_z_only;
        int         nbb, atom_start, bb_natoms, column_natoms;
        real        r2, rb2;
        ...
        do
        {
            ...
            bInRange = (r2 < rc2_nonbonded ||
                        (((bDistMB && rb2 < rc2_bonded) ||
                          (bDist2B && r2  < rc2_bonded)) &&
                         (!bBondComm || cell_missing_link[cxy + bb_last])));
        ...
        if ((bTriclinic || bBondComm) && bb_last > bb_first)
        {
            ...
            do
            {
                ...
                bInRange = (r2 < rc2_nonbonded ||
                            (((bDistMB && rb2 < rc2_bonded) ||
                              (bDist2B && r2  < rc2_bonded)) &&
                             (!bBondComm || cell_missing_link[cxy + bb_first])));
        ...
        if (bb_last >= bb_first)
        {
            ...
            if (send->natoms + at_end - at_start > send->at_nalloc)
            {
                send->at_nalloc = over_alloc_dd(send->natoms + at_end - at_start);
            ...
            {
                send->index_gl[send->natoms++] = dd->index_gl[at];
            }
    ...
    /* The next three operations are the only ones that depend on the call
     * sequence of setup_zone_comm for the different zones.
     * These determine the order of the non-local zones.
     */
    ...
    for (dim = 0; dim < dd->npbcdim; dim++)
    {
        if (zone_shift_dim[dim] == 1 && dd->ci[dim] == dd->nc[dim] - 1)
        {
    ...
    if (dd->nat_tot + recv->natoms > dd->cg_nalloc)
    {
                                     ...
                                     zone_dim_bounds_t *b)
{
    ...
    b[c].x0     = comm->cell_x0[dim];
    b[c].max_f0 = comm->cell_f0[dim_ind];
    b[c].min_f1 = comm->cell_f1[dim_ind];
    ...
    for (dic = dim_ind - 1; dic >= 0; dic--)
    {
        dd_sendrecv_real(dd, dic, dddirForward,
                         (real *)&b[cs],    cn*zdb_size,
                         (real *)&b[cs-cn], cn*zdb_size);

        dd_sendrecv_real(dd, dic, dddirBackward,
                         (real *)&b[cs],    cn*zdb_size,
                         (real *)&b[cs+cn], cn*zdb_size);

        for (ind = c - cn; ind < 3*cn; ind += 2*cn)
        {
            b[c].max_f0 = std::max(b[c].max_f0, b[ind].max_f0);
            b[c].min_f1 = std::max(b[c].min_f1, b[ind].min_f1);
        }
    ...
    comm->cell_f_max0[dim_ind] = b[c].max_f0;
    comm->cell_f_min1[dim_ind] = b[c].min_f1;
}
                                      ...
                                      gmx_bool           **cell_missing_link_ptr)
{
...

    {
        ...
        float      *bb_z_only;
        int         nbb, atom_start, column_natoms;
                              ...
                              gmx_bool bCellsChanged)
{
    ...
    /* This could be moved into setup_zone_comm or up */
    if (dd->nat_tot > fr->cg_nalloc)
    {
        fr->cg_nalloc = over_alloc_dd(dd->nat_tot);
        srenew(fr->cginfo, fr->cg_nalloc);
    }
    dd_set_cginfo(dd->index_gl, dd->nat_home, dd->nat_tot,
                  fr, NULL);
