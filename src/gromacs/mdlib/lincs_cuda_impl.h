...
class LincsCuda::Impl
{
    ...
    private:
        ...
        PbcAiuc             pbcAiuc_;                        //!< Periodic boundary data

        int                 nAtom_;                          //!< Number of atoms

        int                 nOrder_;                         //!< Order of expansion when inversing the matrix
        int                 nIter_;                          //!< Number of iterations used to correct the projection

        float3             *xDevice_;                        //!< Coordinates before the timestep (in the GPU memory)
        float3             *xpDevice_;                       //!< Coordinates after the timestep, before constraining (on GPU).
                                                             //   These will be changed upon constraining.
        float3             *vDevice_;                        //!< Velocities of atoms (on GPU)

        float              *inverseMassesDevice_;            //!< 1/mass for all atoms (GPU)

        std::vector<float>  virialScaledHost_;               //!< Scaled virial tensor (6 floats, GPU)
        float              *virialScaledDevice_;             //!< Scaled virial tensor (6 floats, GPU)

        int                 maxConstraintsNumberSoFar_;      //!< Maximum total of constraints assigned to this class so far.
                                                             //   If the new assigned number is larger, the GPU data arrays are reallocated.
        int                 nConstraintsThreads_;            //!< Total number of threads, which covers all constraints and gaps in the
                                                             //   ends of the thread blocks that are necessary to avoid inter-block
                                                             //   synchronizations.

        std::vector<int2>   constraintsHost_;                //!< List of constrained atoms (CPU memory)
        int2               *constraintsDevice_;              //!< List of constrained atoms (GPU memory)

        std::vector<float>  constraintsTargetLengthsHost_;   //!< Equilibrium distances for the constraints (CPU)
        float              *constraintsTargetLengthsDevice_; //!< Equilibrium distances for the constraints (GPU)

        std::vector<int>    coupledConstraintsCountsHost_;   //!< Number of constraints, coupled with the current one (CPU)
        int                *coupledConstraintsCountsDevice_; //!< Number of constraints, coupled with the current one (GPU)

        std::vector<int>    coupledConstraintsIdxesHost_;    //!< List of coupled with the current one (CPU)
        int                *coupledConstraintsIdxesDevice_;  //!< List of coupled with the current one (GPU)

        float              *matrixADevice_;                  //!< Elements of the coupling matrix.

        std::vector<float>  massFactorsHost_;                //!< Mass factors (CPU)
        float              *massFactorsDevice_;              //!< Mass factors (GPU)
