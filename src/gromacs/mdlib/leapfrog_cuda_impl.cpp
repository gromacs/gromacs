 ...
 */
void LeapFrogCuda::copyInverseMassesToGpu(gmx_unused const real *inverseMasses)
{
    GMX_ASSERT(false, "A CPU stub for LeapFrog was called insted of the correct implementation.");
}
 ...
 *
 * \param[in] *h_v  CPU pointer where velocities should be copied to.
 */
 ...
 * \param[out]    *d_xp Pointer to the coordinates after integrator update (on GPU)
 * \param[in,out] *d_v  Pointer to the velocities (on GPU)
