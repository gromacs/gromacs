...
class StopHandler final : public IAccumulateGlobalsClient
{
    public:
        ...
        void handleSignal()
        {
            if (static_cast<StopSignal>(lround(signalView_[0])) != StopSignal::noSignal)
            {
                stopImmediately_ = static_cast<StopSignal>(lround(signalView_[0])) == StopSignal::stopImmediately;
                stopAtNextNS_    = static_cast<StopSignal>(lround(signalView_[0])) == StopSignal::stopAtNextNSStep;
