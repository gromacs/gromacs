...
class StopHandler final
{
    public:
        ...
        bool stoppingAfterCurrentStep(bool bNS) const
        {
            ...
            return signal_.set < 0 || (signal_.set > 0 && ( bNS || neverUpdateNeighborlist_));
        }

    private:
        SimulationSignal &signal_;
        const             std::vector < std::function < signed char()>> stopConditions_;
 ...
 * The registered functions need to be of type `std::function<signed char()>`, i.e. not
 * taking any input arguments and returning a `signed char` signal which will get propagated
...
class StopHandlerBuilder final
{
    public:
         ...
         * which return a signed char. The return value of this function is then propagated to all
         * ranks, and allows to stop the simulation at the next global communication step (returned
         * signal < 0), or at the next NS step (returned signal > 0, allows for exact continuation).
         *
         ...
         */
        void registerStopCondition(std::function<signed char()> stopCondition);

        /*! \brief Create StopHandler
         *
         * Gets called in the scope of the integrator (aka do_md()) to get a pointer to the
         * StopHandler for the current simulations. Creates a copy of the currently registered
         * stop conditions, and adds the standard MD stop conditions (e.g. gmx::StopConditionTime,
         * gmx::StopConditionSignal). Initializes a new StopHandler with this extended vector of
    ...
    private:
         ...
         */
        std::vector < std::function < signed char()>> stopConditions_;
};
