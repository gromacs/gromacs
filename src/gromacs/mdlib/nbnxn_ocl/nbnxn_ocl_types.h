...
#include "gromacs/gpu_utils/gputimer_ocl.h"
...
typedef struct cl_timers
{
    ...
    GpuTimer prune_k[2];              /**< timer for the 1st pass list pruning kernel (l/nl, every PS step)   */
    bool     didPrune[2];             /**< true when we timed pruning and the timings need to be accounted for */
    GpuTimer rollingPrune_k[2];       /**< timer for rolling pruning kernels (l/nl, frequency depends on chunk size)  */
