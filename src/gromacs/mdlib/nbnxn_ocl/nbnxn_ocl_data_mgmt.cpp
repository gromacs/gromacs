...
static void CL_CALLBACK NotifyFunction( const char *pErrInfo, const void *, size_t, void *)
{
    if (pErrInfo != NULL)
    {
        printf("%s\n", pErrInfo ); // Parse hint
                         ...
                         int                        rank)
{
    ...
    context = clCreateContext(context_properties, 1, &device_id, NotifyFunction, NULL, &cl_error);
                    ...
                    gmx_bool                   bLocalAndNonlocal)
{
    ...
    nb->bPrefetchLjParam =
        ...
        ((nb->dev_info->vendor_e == OCL_VENDOR_AMD) || (nb->dev_info->vendor_e == OCL_VENDOR_INTEL) || (getenv("GMX_OCL_ENABLE_I_PREFETCH") != NULL));
