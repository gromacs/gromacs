...
static void do_nb_verlet_fep(t_forcerec *fr,
                             rvec x[],
                             rvec f[],
                             t_mdatoms *mdatoms,
                             t_lambda *fepvals,
                             ...
                             gmx_wallcycle_t wcycle)
{
    ...
    wallcycle_sub_start(wcycle, ewcsNONBONDED);
    gmx_nb_free_energy_kernel(fr->nbv->grp[ilocality].nbl_lists.nbl_fep[0],
    ...
    {
        kernel_data.flags  = (donb_flags & ~GMX_NONBONDED_DO_FORCE) | GMX_NONBONDED_DO_FOREIGNLAMBDA;
                         ...
                         int flags)
{
    ...
    if (fr->efep != efepNO)
    {
        /* Calculate the, potential, local and non-local free energy
         * interactions here. Happens both with and without GPU.
         */
        if (fr->nbv->grp[eintLocal].nbl_lists.nbl_fep[0]->nrj > 0)
        {
            do_nb_verlet_fep(fr, x, f, mdatoms,
                             inputrec->fepvals, lambda,
                             enerd, flags, eintLocal, nrnb, wcycle);
        }
            ...
            fr->nbv->grp[eintNonlocal].nbl_lists.nbl_fep[0]->nrj > 0)
        {
            do_nb_verlet_fep(fr, x, f, mdatoms,
                             inputrec->fepvals, lambda,
                             enerd, flags, eintNonlocal, nrnb, wcycle);
        }
    }
