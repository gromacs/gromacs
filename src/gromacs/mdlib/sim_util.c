                ...
                t_inputrec *ir, t_commrec gmx_unused *cr)
{
    ...
    if ((step >= ir->nstlist))
    {
        ...
        double elapsed_seconds     = seconds_since_epoch - runtime_get_start_time_stamp(runtime);
        double time_per_step       = elapsed_seconds/(step - ir->init_step + 1);

        double dt = (ir->nsteps + ir->init_step - step) * time_per_step;
