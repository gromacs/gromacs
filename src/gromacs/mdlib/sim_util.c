             ...
             gmx_bool *bSimAnn, t_vcm **vcm, unsigned long Flags)
{
    ...
    *bSimAnn = FALSE;
    ...
    if ((*outf)->tng)
    {
        ...
        if(ir->nstxout)
        {
            tng_util_pos_write_frequency_set((*outf)->tng, ir->nstxout);
        }
        if(ir->nstvout)
        {
            tng_util_vel_write_frequency_set((*outf)->tng, ir->nstvout);
        }
        if(ir->nstfout)
        {
            tng_util_force_write_frequency_set((*outf)->tng, ir->nstfout);
        }
        if(ir->nstxout || ir->nstvout || ir->nstfout)
        {
            tng_util_box_shape_write_frequency_set((*outf)->tng, lowcd);
        }
    }
#endif
