                         ...
                         int flags)
{
    ...
    gmx_bool            bDoLongRange, bDoForces, bUseGPU, bUseOrEmulGPU;
     ...
     */
    assert(!(bDoLongRange && bDoForces && (flags & GMX_FORCE_SEPLRF)));
    ...
    else
    {
        ...
        if (DOMAINDECOMP(cr))
        {
            ...
            dd_halo_initiate_send_x(cr->dd, box, x, FALSE);
    ...
    if (bDoForces && DOMAINDECOMP(cr))
    {
        ...
        wallcycle_start(wcycle, ewcINITCOMMF);
        dd_halo_initiate_send_f(cr->dd, f, FALSE);
        wallcycle_stop(wcycle, ewcINITCOMMF);
    }
