...
gmx_settledata_t settle_init(const gmx_mtop_t *mtop)
{
    ...
    {
        for (int i = 0; i < ilist[F_SETTLE].nr; i += 4)
        {
            if (settle_type == -1)
            {
                            ...
                            const t_mdatoms  *mdatoms)
{
    ...
    if (nsettle > 0)
    {
        ...
        if (nsettle + pack_size > settled->nalloc)
        {
            ...
            snew_aligned(settled->ow1, settled->nalloc, 16);
            snew_aligned(settled->hw2, settled->nalloc, 16);
            snew_aligned(settled->hw3, settled->nalloc, 16);
            snew_aligned(settled->virfac, settled->nalloc, 16);
        }

        for (int i = 0; i < nsettle; i++)
        {
            settled->ow1[i]    = iatoms[i*4 + 1];
            settled->hw2[i]    = iatoms[i*4 + 2];
            settled->hw3[i]    = iatoms[i*4 + 3];
             ...
             */
            settled->virfac[i] = (iatoms[i*4 + 1] < mdatoms->homenr ? 1 : 0);
        }
...
#if !GMX_SIMD_HAVE_REAL
// TODO: Move the functions below to the SIMD module and add all functions
static real inline load(const real *ptr)
{
    return *ptr;
}

static real inline max(const real r1, const real r2)
{
    return std::max(r1, r2);
}

static real inline reduce(const real r)
{
    return r;
}

static bool inline anyTrue(const bool b)
{
    return b;
}

template<int align>
static void inline gatherLoadUTranspose(const real *r, const int *offset, real *rx, real *ry, real *rz)
{
    *rx = r[offset[0]*align + 0];
    *ry = r[offset[0]*align + 1];
    *rz = r[offset[0]*align + 2];
}

template<int align>
static void inline transposeScatterStoreU(real *r, const int *offset, real rx, real ry, real rz)
{
    r[offset[0]*align + 0] = rx;
    r[offset[0]*align + 1] = ry;
    r[offset[0]*align + 2] = rz;
}
#endif

#if GMX_SIMD_HAVE_REAL
// TODO: Consider moving this to pbcutil/pbc-simd.h
static void inline pbc_dx_aiuc(const real       *pbc,
                               const SimdReal   *x1,
                               const SimdReal   *x2,
                               SimdReal         *dx)
{
    ...
    pbc_correct_dx_simd(&dx[XX], &dx[YY], &dx[ZZ], pbc);
}
#endif

/* The actual settle code, templated for real/SimdReal and for optimization */
template<typename T, typename TypeBool, int pack_size,
         typename pbc_type,
         bool bCorrectVelocity,
         bool bCalcVirial>
static void settle_template(const gmx_settledata_t settled,
                            int settle_start, int settle_end,
                            const pbc_type pbc,
                            const real b4[], real after[],
                            real invdt, real *v,
                            tensor vir_r_m_dr,
                            bool *bErrorOccurred)
{
    ...
    assert(settle_start % pack_size == 0);
    assert(settle_end   % pack_size == 0);
    ...
    for (int i = settle_start; i < settle_end; i += pack_size)
    {
        /* Here we pad up to pack_size with copies from the last valid entry.
        ...
        T          b4_ow1[DIM], b4_hw2[DIM], b4_hw3[DIM];

        gatherLoadUTranspose<3>(reinterpret_cast<const real *>(b4), ow1, &b4_ow1[XX], &b4_ow1[YY], &b4_ow1[ZZ]);
        gatherLoadUTranspose<3>(reinterpret_cast<const real *>(b4), hw2, &b4_hw2[XX], &b4_hw2[YY], &b4_hw2[ZZ]);
        gatherLoadUTranspose<3>(reinterpret_cast<const real *>(b4), hw3, &b4_hw3[XX], &b4_hw3[YY], &b4_hw3[ZZ]);

        T after_ow1[DIM], after_hw2[DIM], after_hw3[DIM];

        gatherLoadUTranspose<3>(reinterpret_cast<const real *>(after), ow1, &after_ow1[XX], &after_ow1[YY], &after_ow1[ZZ]);
        gatherLoadUTranspose<3>(reinterpret_cast<const real *>(after), hw2, &after_hw2[XX], &after_hw2[YY], &after_hw2[ZZ]);
        gatherLoadUTranspose<3>(reinterpret_cast<const real *>(after), hw3, &after_hw3[XX], &after_hw3[YY], &after_hw3[ZZ]);

        T dist21[DIM], dist31[DIM];
        T doh2[DIM], doh3[DIM];
        T sh_hw2[DIM], sh_hw3[DIM];

        pbc_dx_aiuc(pbc, b4_hw2, b4_ow1, dist21);

        pbc_dx_aiuc(pbc, b4_hw3, b4_ow1, dist31);

        /* Tedious way of doing pbc */
        pbc_dx_aiuc(pbc, after_hw2, after_ow1, doh2);
        for (int d = 0; d < DIM; d++)
        {
            sh_hw2[d]    = after_hw2[d] - (after_ow1[d] + doh2[d]);
            after_hw2[d] = after_hw2[d] - sh_hw2[d];
        }
        pbc_dx_aiuc(pbc, after_hw3, after_ow1, doh3);
        for (int d = 0; d < DIM; d++)
        {
            sh_hw3[d]    = after_hw3[d] - (after_ow1[d] + doh3[d]);
            after_hw3[d] = after_hw3[d] - sh_hw3[d];
        }
        ...
        for (int d = 0; d < DIM; d++)
        {
            ...
            com[d] = after_ow1[d] - a1[d];
        }
        T b1[DIM];
        for (int d = 0; d < DIM; d++)
        {
            b1[d]  = after_hw2[d] - com[d];
        }
        T c1[DIM];
        for (int d = 0; d < DIM; d++)
        {
            c1[d]  = after_hw3[d] - com[d];
        }
        /* 15 flops */
        ...
        for (int d = 0; d < DIM; d++)
        {
            after_ow1[d] = com[d] + a3[d];
        }
        for (int d = 0; d < DIM; d++)
        {
            after_hw2[d] = com[d] + b3[d] + sh_hw2[d];;
        }
        for (int d = 0; d < DIM; d++)
        {
            after_hw3[d] = com[d] + c3[d] + sh_hw3[d];
        }
        /* 9 flops + 6 pbc flops */

        transposeScatterStoreU<3>(reinterpret_cast<real *>(after), ow1, after_ow1[XX], after_ow1[YY], after_ow1[ZZ]);
        transposeScatterStoreU<3>(reinterpret_cast<real *>(after), hw2, after_hw2[XX], after_hw2[YY], after_hw2[ZZ]);
        transposeScatterStoreU<3>(reinterpret_cast<real *>(after), hw3, after_hw3[XX], after_hw3[YY], after_hw3[ZZ]);
        ...
        if (bCorrectVelocity || bCalcVirial)
        {
            ...
            if (bCorrectVelocity)
            {
                ...
                gatherLoadUTranspose<3>(reinterpret_cast<const real *>(v), ow1, &v_ow1[XX], &v_ow1[YY], &v_ow1[ZZ]);
                gatherLoadUTranspose<3>(reinterpret_cast<const real *>(v), hw2, &v_hw2[XX], &v_hw2[YY], &v_hw2[ZZ]);
                gatherLoadUTranspose<3>(reinterpret_cast<const real *>(v), hw3, &v_hw3[XX], &v_hw3[YY], &v_hw3[ZZ]);

                /* Add the position correction divided by dt to the velocity */
                for (int d = 0; d < DIM; d++)
                {
                    v_ow1[d] = fma(da[d], invdt, v_ow1[d]);
                }
                for (int d = 0; d < DIM; d++)
                {
                    v_hw2[d] = fma(db[d], invdt, v_hw2[d]);
                }
                for (int d = 0; d < DIM; d++)
                {
                    v_hw3[d] = fma(dc[d], invdt, v_hw3[d]);
                }
                /* 3*6 flops */

                transposeScatterStoreU<3>(reinterpret_cast<real *>(v), ow1, v_ow1[XX], v_ow1[YY], v_ow1[ZZ]);
                transposeScatterStoreU<3>(reinterpret_cast<real *>(v), hw2, v_hw2[XX], v_hw2[YY], v_hw2[ZZ]);
                transposeScatterStoreU<3>(reinterpret_cast<real *>(v), hw3, v_hw3[XX], v_hw3[YY], v_hw3[ZZ]);
            }
 ...
 */
template<typename T, typename TypeBool, int pack_size, typename pbc_type>
static void settle_template_wrapper(gmx_settledata_t settled,
                                    ...
                                    bool *bError)
{
    ...
    int nsettle_packs = (settled->nsettle + pack_size - 1)/pack_size;
    /* Round the end value up to give thread 0 more work */
    int settle_start  = ((nsettle_packs* thread      + nthread - 1)/nthread)*pack_size;
    int settle_end    = ((nsettle_packs*(thread + 1) + nthread - 1)/nthread)*pack_size;

    if (v != NULL)
    {
        if (!bCalcVirial)
        {
            settle_template<T, TypeBool, pack_size,
                            pbc_type,
                            true,
                            false>
                (settled, settle_start, settle_end,
                pbc,
                b4, after,
                invdt, v,
                NULL,
             ...
             bool *bError)
{
    ...
    settle_template_wrapper<SimdReal, SimdBool, GMX_SIMD_REAL_WIDTH,
                            const real *>(settled,
                                          nthread, thread,
                                          pbc_simd,
                                          b4, after,
                                          ...
                                          bError);
#else
    settle_template_wrapper<real, bool, 1,
                            const t_pbc *>(settled,
                                           nthread, thread,
                                           pbc,
                                           b4, after,
