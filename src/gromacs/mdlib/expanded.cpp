                           ...
                           gmx_int64_t seed, gmx_int64_t step)
{
    ...
    double                               pks;
    real                                 pnorm;
    gmx::ThreeFry2x64<0>                 rng({static_cast<gmx_uint64_t>(seed), gmx::RandomDomain_ExpandedEnsemble}); // We only draw once, so zero bits internal counter is fine
    ...
    {
        rng.restart({static_cast<gmx_uint64_t>(step), static_cast<gmx_uint64_t>(i)});
        dist.reset();
