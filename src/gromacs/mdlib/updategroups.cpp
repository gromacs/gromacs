...
static bool hasAnyFlexibleConstraints(const gmx_moltype_t &moltype,
                                      const t_iparams     *iparams)
{
    bool hasAnyFlexibleConstraints = false;

    for (auto &ilist : getILists(moltype.ilist, IF_CONSTRAINT))
    {
        if (ilist.functionType != F_SETTLE)
        {
            for (size_t i = 0; i < ilist.iatoms.size(); i += ilistStride(ilist))
            {
                ...
                {
                    hasAnyFlexibleConstraints = true;
                }
            }
        }
    }

    return hasAnyFlexibleConstraints;
}
                                  ...
                                  const t_iparams     *iparams)
{
    ...
    for (auto &ilist : getILists(moltype.ilist, IF_VSITE))
    {
        if (ilist.functionType == F_VSITE2 || ilist.functionType == F_VSITE3)
        {
            for (size_t i = 0; i < ilist.iatoms.size(); i += ilistStride(ilist))
            {
                ...
                if (ilist.functionType == F_VSITE3)
                {
...
/*! \brief Returns the range of atoms constrained to atom a (including a itself) */
static AtomExtremes
constraintAtomRange(int                    a,
                    const t_blocka        &at2con,
                    const InteractionList &ilistConstraints)
{
    AtomExtremes extremes = { a, a };
                       ...
                       const InteractionList &ilistConstraints)
{
    ...
    while (a <= lastAtom)
    {
        ...
        else
        {
            ...
            AtomExtremes extremes = constraintAtomRange(a, at2con, ilistConstraints);
            if (extremes.minAtom < firstAtom)
            {
                                          ...
                                          const t_iparams     *iparams)
{
     ...
     */
    if (hasAnyFlexibleConstraints(moltype, iparams) ||
...
{
    GMX_RELEASE_ASSERT(!hasAnyFlexibleConstraints(moltype, iparams),
