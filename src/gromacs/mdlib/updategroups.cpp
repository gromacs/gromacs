...
/* Attempts to compute the maximum radius of constrained atom in an update
 * group with 2 or 3 atoms when all partner atoms are connected to each
 * other by angle potentials. The temperature is used to compute a radius
 * that is not exceeded with a chance of 0.5*10^-9. Note that this computation
                      ...
                      const real                               temperature)
{
    ...
    GMX_ASSERT(numConstraints == numPartnerAtoms, "We expect as many constraints as partner atoms here");

    std::array<int, numPartnerAtoms> partnerAtoms;
    for (int i = at2con.index[centralAtom]; i < at2con.index[centralAtom + 1]; i++)
    {
        const int ind = at2con.a[i]*3;
        ...
        partnerAtoms[i - at2con.index[centralAtom]] = (a1 == centralAtom ? a2 : a1);
    }
    ...
    {
        /* Set number of stddevs such that change of exceeding < 0.5*10^-9 */
        constexpr real c_numSigma = 6.11;
        ...
        if (numPartnerAtoms == 2)
        {
             ...
             * with two equal sides. Return the distance from the COG to the
             * farthest two corners, e.g. the partner atoms.
             */
