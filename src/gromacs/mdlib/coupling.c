    ...
    for (i = 0; i < md->homenr; i++)  /* now loop over the list of atoms */
    {
        ...
        if (randomize[gc])
        {
            ...
            else
            {
                ...
                gmx_rng_cycle_2uniform(ir->andersen_seed, RND_SEED_ANDERSEN, step*2, ng, uniform);
...
/* Gamma distribution, adapted from numerical recipes */
{
    ...
    if (ia < 6)
    {
        do
        {
            ...
            for (j = 1; j <= ia; j++)
            {
                gmx_rng_cycle_2uniform(seed1, seed2, step, (*count)++, rnd);
                x *= rnd[0];
            }
        }
        while (x == 0);
...
static real gaussian_count(gmx_int64_t seed1, gmx_int64_t seed2,
                           gmx_int64_t step, gmx_int64_t *count)
{
    double rnd[2], x, y, r;
 ...
 */
    ...
    r = 2.0*vrescale_gamdev(nn/2, seed1, seed2, step, count);

    if (nn % 2 == 1)
    {
        gauss = gaussian_count(seed1, seed2, step, count);
 ...
 */
    ...
    rr = gaussian_count(seed, RND_SEED_VRESCALE, step, &count);

    ekin_new =
        kk +
        (1.0 - factor)*(sigma*(vrescale_sumnoises(ndeg-1, seed, RND_SEED_VRESCALE, step, &count) + rr*rr)/ndeg - kk) +
