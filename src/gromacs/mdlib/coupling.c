    ...
    for (i = 0; i < md->homenr; i++)  /* now loop over the list of atoms */
    {
        ...
        if (randomize[gc])
        {
            ...
            else
            {
                ...
                gmx_rng_cycle_4uniform(ir->andersen_seed, RND_SEED_ANDERSEN, step*2, ng, uniform);
            ...
            if (bRandomize)
            {
                ...
                gmx_rng_cycle_3gaussian_table(ir->andersen_seed, RND_SEED_ANDERSEN, step*2+1, ng, gauss);
                for (d = 0; d < DIM; d++)
                {
...
/* Gamma distribution, adapted from numerical recipes */
{
    ...
    if (ia < 6)
    {
        do
        {
            ...
            for (j = 1; j <= ia; j++)
            {
                gmx_rng_cycle_4uniform(seed1, seed2, step, count++, rnd);
                x *= rnd[0];
            }
        }
        while (x == 0);
    ...
    else
    {
        do
        {
            do
            {
                do
                {
                    /* Note that we use rnd[2] at the end */
                    gmx_rng_cycle_4uniform(seed1, seed2, step, count++, rnd);
                    v1 = rnd[0];
                    v2 = 2.0*rnd[1] - 1.0;
                }
 ...
 */
    ...
    {
        gmx_rng_cycle_3gaussian_table(seed1, seed2, step, count++,
                                      gauss);
        return gauss[0]*gauss[0];
    }
    else if (nn % 2 == 0)
    {
        return 2.0*vrescale_gamdev(nn/2, seed1, seed2, step, count);
    }
    else
    {
        gmx_rng_cycle_3gaussian_table(seed1, seed2, step, count++,
                                      gauss);
        return 2.0*vrescale_gamdev((nn-1)/2, seed1, seed2, step, count) + gauss[0]*gauss[0];
    }
}
 ...
 */
    int  count = 0;
    real factor, gauss[3], rr, ekin_new;
    ...
    gmx_rng_cycle_3gaussian_table(seed, RND_SEED_VRESCALE,
                                  step, count++,
                                  gauss);
    rr = gauss[0];
    ...
    ekin_new =
        kk +
        (1.0 - factor)*(sigma*(vrescale_sumnoises(ndeg-1, step, RND_SEED_VRESCALE, step, count) + rr*rr)/ndeg - kk) +
