...
/*! \libinternal \brief Parameters for SETTLE algorithm.
 *
...
{
    float   mO;              //!< Mass of oxygen atom
    float   mH;              //!< Mass of hydrogen atom
    float   wh;              //!< Relative hydrogen mass (i.e. mH/(mO+2*mH))
    float   dOH;             //!< Target distance between oxygen and hydrogen
    float   dHH;             //!< Target distance between hydrogen atoms
    float   ra;              //!< Double relative H mass times height of the H-O-H triangle.
    float   rb;              //!< Height of the H-O-H triangle minus ra.
    float   rc;              //!< Half the H-H distance.
    float   irc2;            //!< Reciprocal H-H distance
    /* For projection */
    float   imO;             //!< Reciprocal oxygen mass
    float   imH;             //!< Reciprocal hydrogen mass
    float   invdOH;          //!< Reciprocal O-H distance
    float   invdHH;          //!< Reciprocal H-H distance (again)
    matrix  invmat;          //!< Reciprocal projection matrix
};

/*! \brief Initializes a projection matrix.
 *
 * This is identical to CPU code. Needs to be unified.
 */
static void initializeProjectionMatrix(const real invmO, const real invmH, const real dOH, const real dHH,
 ...
 *
 * This is idential to CPU code. Needs to be unified.
 */
...
class SettleCuda::Impl
{
    ...
    private:
        ...
        std::vector<float>  virialScaledHost_;             //!< Scaled virial tensor (9 reals, GPU)
        float              *virialScaledDevice_;           //!< Scaled virial tensor (9 reals, GPU)

        int                 nSettle_;                      //!< Number of settles

        std::vector<int3>   atomIdsHost_;                  //!< Indexes of atoms (.x for oxygen, .y and.z for hydrogens (CPU)
        int3               *atomIdsDevice_;                //!< Indexes of atoms (.x for oxygen, .y and.z for hydrogens (GPU)
