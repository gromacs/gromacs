 ...
 */
static double simd_speedup_factor(gmx_bool bUseSIMD)
{
     ...
     * This factor is used for normalization in simd_speedup_factor().
     */
    const double simd_speedup_no_simd = 5.0;
    double speedup;
    ...
    if (bUseSIMD)
    {
         ...
         */
        speedup = sqrt(2*GMX_SIMD_REAL_WIDTH);
...
#else
        GMX_RELEASE_ASSERT(bUseSIMD == FALSE);
    /* No SIMD, no speedup */
    speedup      = 1.0;
#endif
     ...
     */
    return simd_speedup_no_simd/speedup;
}
                          ...
                          gmx_bool *bChargePerturbed, gmx_bool *bTypePerturbed)
{
    ...
    *cost_pp *= simd_speedup_factor(bHaveSIMD);
}
...
float pme_load_estimate(gmx_mtop_t *mtop, t_inputrec *ir, matrix box)
{
     ...
     */
    cost_bond = C_BOND*(ndistance_c   *simd_speedup_factor(FALSE) +
                        ndistance_simd*simd_speedup_factor(bHaveSIMD));
    ...
    if (EEL_PME(ir->coulombtype))
    {
        ...
        cost_solve  += f*C_PME_SOLVE*grid*simd_speedup_factor(bHaveSIMD);
    }
