...
double n_bonded_dx(gmx_mtop_t *mtop, const t_inputrec *ir, gmx_bool bPBCDX)
{
    ...
    {
        /* The SIMD accelerated bonded interactions are cheaper */
        simd_fac =
            nonsimd_step_frac +
            (1 - nonsimd_step_frac)*simd_acceleration_factor()/simd_acceleration_no_simd;
    }
...
#pragma novector /* Work-around for incorrect vectorization */
    ...
    if (!bPBCDX)
    {
        /* We currently have the dx count relative to non-SIMD code,
         * but we should return the dx count relative to SIMD code.
         */
        ndx *= simd_acceleration_no_simd;
    }
