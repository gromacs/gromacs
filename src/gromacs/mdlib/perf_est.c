 ...
 * This factor is used for normalization in simd_acceleration_factor().
 */
    ...
    for (mb = 0; mb < mtop->nmolblock; mb++)
    {
        ...
        for (ftype = 0; ftype < F_NRE; ftype++)
        {
            ...
            if (interaction_function[ftype].flags & IF_BOND)
            {
                ...
                *ndistance_c    += nbonds*nd_c;
                *ndistance_simd += nbonds*nd_simd;
            }
        }
        if (bExcl)
        {
            *ndistance_c += nmol*(molt->excls.nra - molt->atoms.nr)/2;
        }
    }

    if (debug)
    {
        fprintf(debug, "nr. of distance calculations in bondeds: C %.1f SIMD %.1f\n", *ndistance_c, *ndistance_simd);
    }
}
...
float pme_load_estimate(gmx_mtop_t *mtop, t_inputrec *ir, matrix box)
{
    ...
    /* C_BOND is the cost for SIMD bondeds, we need to convert for C bondeds */
    cost_bond = C_BOND*(ndistance_c*simd_acceleration_no_simd/simd_acceleration_factor() +
                        ndistance_simd);
