...
{
    GMX_RELEASE_ASSERT(!DOMAINDECOMP(cr), "broadcastXAndBox should only be used for special cases without domain decomposition");
    ...
    constexpr int xFlag   = (1 << estX);
    constexpr int boxFlag = (1 << estBOX);

    if (MASTER(cr))
    {
        GMX_RELEASE_ASSERT(state->flags & xFlag, "broadcastXAndBox should be called with x in the state");
        GMX_RELEASE_ASSERT((state->flags & !(xFlag | boxFlag)) == 0, "broadcastXAndBox should be called with only x and optionally box in the state");
    }
