...
struct vsite_thread_t {
    ...
    t_ilist  ilist1[F_NRE];  /* vsite ilists for the first task            */
    t_ilist  ilist2[F_NRE];  /* vsite ilists for the second task           */
    rvec     fshift[SHIFTS]; /* fshift accumulation buffer                 */
    matrix   dxdf;           /* virial dx*df accumulation buffer           */
    bool     useTask2;       /* If to use a second task: use ilist2, f_buf */
    /* Data only used by task2 */
    ...
    int     *ind;            /* Index to set elements in use, size nuse    */
    vsite_task_ind_t *task_ind; /* Index per task to set elements in use, size of ind nuse    */
    int      nalloc;         /* Allocation size of f_buf, use, ind, task_ind[].ind */
};
...
#pragma omp parallel num_threads(vsite->nthreads)
        {
            try
            {
                ...
                construct_vsites_thread(vsite,
                                        x, dt, v,
                                        ip, vsite->tdata[th]->ilist1,
                                        pbc_null);
                if (vsite->tdata[th]->useTask2)
                {
        ...
        construct_vsites_thread(vsite,
                                x, dt, v,
                                ip, vsite->tdata[vsite->nthreads]->ilist1,
                                pbc_null);
    }
}
                    ...
                    t_commrec *cr)
{
    ...
    else
    {
        ...
        spread_vsite_f_thread(vsite,
                              ...
                              idef->iparams,
                              vsite->tdata[vsite->nthreads]->ilist1,
                              g, pbc_null);
...
#pragma omp parallel num_threads(vsite->nthreads)
        {
            try
            {
                ...
                struct vsite_thread_t *tdata  = vsite->tdata[thread];
                ...
                if (tdata->useTask2)
                {
                     ...
                     * This is done using a thread-local force buffer.
                     * First we need to copy the input vsite forces to f_buf.
                     */
                    for (int i = 0; i < tdata->nvsite; i++)
                    {
                        copy_rvec(f[tdata->vsite_ind[i]],
                                  tdata->f_buf[tdata->vsite_ind[i]]);
                    }
                    spread_vsite_f_thread(vsite,
                                          x, tdata->f_buf, fshift_t,
                                          VirCorr, tdata->dxdf,
                                          idef->iparams,
                                          tdata->ilist2,
                                          g, pbc_null);
...
#if 0
                    /* This thread-parallel reduction using atomics can
                     * easily be replaced by an omp master region doing
                     * the reduction on a single thread without atomics,
                     * when atomics are not supported.
                     */
                    for (int i = 0; i < tdata->nuse; i++)
                    {
                        /* Only use atomic additions when really needed,
                         * i.e. when the index is out of our thread range.
                         */
                        int ind = tdata->ind[i];
                        if (ind <  tdata->range_start ||
                            ind >= tdata->range_end)
                        {
#pragma omp atomic
                            f[ind][XX] += tdata->f_buf[ind][XX];
#pragma omp atomic
                            f[ind][YY] += tdata->f_buf[ind][YY];
#pragma omp atomic
                            f[ind][ZZ] += tdata->f_buf[ind][ZZ];

                            clear_rvec(tdata->f_buf[ind]);
                        }
                    }
                    /* We need a barrier to ensure all cross thread-block
                     * atomic operations on f have finished before doing
                     * non-atomic operations on f.
                     * All non-atomic operations below operate on separated
                     * ranges of f for different threads.
                     */
#pragma omp barrier
                    /* Reduce the spread forces for the local atoms,
                     * clear f_buf so it's zero at the next call.
                     */
                    for (int i = 0; i < tdata->nuse; i++)
                    {
                        int ind = tdata->ind[i];
                        if (ind >= tdata->range_start &&
                            ind <  tdata->range_end)
                        {
                            rvec_inc(f[ind], tdata->f_buf[ind]);
                            clear_rvec(tdata->f_buf[ind]);
                        }
                    }
                    ...
                    for (int i = 0; i < tdata->nvsite; i++)
                    {
                        clear_rvec(f[tdata->vsite_ind[i]]);
                        clear_rvec(tdata->f_buf[tdata->vsite_ind[i]]);
                    }
#endif
...
#pragma omp barrier
                    ...
                    {
                        const vsite_task_ind_t *ti = &vsite->tdata[t]->task_ind[thread];
...
#pragma omp parallel for num_threads(vsite->nthreads) schedule(static)
        for (int thread = 0; thread < vsite->nthreads; thread++)
        {
            try
            {
                ...
                vsite->tdata[thread]->f_buf        = NULL;
                vsite->tdata[thread]->vsite_ind    = NULL;
                vsite->tdata[thread]->vsite_nalloc = 0;
                vsite->tdata[thread]->use          = NULL;
                vsite->tdata[thread]->nuse         = 0;
                vsite->tdata[thread]->ind          = NULL;
                snew(vsite->tdata[thread]->task_ind, vsite->nthreads);
                vsite->tdata[thread]->nalloc       = 0;
            }
            GMX_CATCH_ALL_AND_EXIT_WITH_FATAL_ERROR;
        }
        if (vsite->nthreads > 1)
...
static gmx_inline void set_task2_vsite(struct vsite_thread_t *tdata, int atom)
{
    if (tdata->nvsite + 1 > tdata->vsite_nalloc)
    {
        tdata->vsite_nalloc = over_alloc_large(tdata->nvsite + 1);
        srenew(tdata->vsite_ind, tdata->vsite_nalloc);
    }
    tdata->vsite_ind[tdata->nvsite++] = atom;
}

static gmx_inline void flag_f_buf(struct vsite_thread_t *tdata, int atom,
                                  int thread, int nthread, int natperthread)
{
    if (!tdata->use[atom])
    {
        tdata->use[atom]          = true;
        tdata->ind[tdata->nuse++] = atom;
        if (atom < tdata->range_start || atom >= tdata->range_end)
        {
        ...
        tdata->task_ind[thread].ind[tdata->task_ind[thread].n++] = atom;
    }
}
 ...
 * are assigned to task tdata->ilist1. Vsites that depend on non-local atoms
 * but not on other vsites are assigned to task tdata->ilist2.
 * task_ind[] is set for all vsites in our range, either to our local tasks
 * or to the single last task as task_ind[]=2*nthreads.
 */
static void assign_vsites_to_thread(struct vsite_thread_t *tdata,
                                    int                    thread,
                                    int                    nthread,
                                    int                    natperthread,
                                    int                   *task_ind,
                                    ...
                                    const unsigned short  *ptype)
{
    for (int ftype = f_vsite_start; ftype < f_vsite_end; ftype++)
    {
        tdata->ilist1[ftype].nr = 0;
        tdata->ilist2[ftype].nr = 0;
        ...
        for (int i = 0; i < ilist[ftype].nr; )
        {
            ...
            if (ftype != F_VSITEN)
            {
                for (int j = i + 2; j < i + nral1; j++)
                {
                    ...
                    {
                        if (!tdata->useTask2 || ptype[iat[j]] == eptVSite)
                        {
                         ...
                         * atomic reduction and a barrier between the two
                         * tasks.
                         */
                        task = nthread + thread;
                    }
            ...
            if (task == thread || task == nthread + thread)
            {
                ...
                if (task == thread)
                {
                    il_task = &tdata->ilist1[ftype];
                }
                else
                {
                    il_task = &tdata->ilist2[ftype];
                }
                ...
                {
                    set_task2_vsite(tdata, iat[i + 1]);
                    if (ftype != F_VSITEN)
                    {
                        for (int j = i + 2; j < i + nral1; j++)
                        {
                            flag_f_buf(tdata, iat[j],
    ...
    {
        if (tdata->task_ind[t].n > 0)
            printf("thread %2d t %2d %3d\n", t, thread, tdata->task_ind[t].n);
    }
}

static void assign_vsites_to_single_task(struct vsite_thread_t *tdata,
                                         int                    task,
                                         const int             *task_ind,
                                         const t_ilist         *ilist,
                                         const t_iparams       *ip)
{
    for (int ftype = f_vsite_start; ftype < f_vsite_end; ftype++)
    {
        ...
        int      inc     = nral1;
        t_iatom *iat     = ilist[ftype].iatoms;
        t_ilist *il_task = &tdata->ilist1[ftype];
...
#pragma omp parallel for num_threads(vsite->nthreads) schedule(static)
    for (int thread = 0; thread < vsite->nthreads; thread++)
    {
        try
        {
             ...
             */
            tdata->useTask2 = (vsite_atom_range <= 200000);
            if (tdata->useTask2)
            {
                ...
                if (natoms_use_in_vsites > tdata->nalloc)
                {
                    tdata->nalloc = over_alloc_large(natoms_use_in_vsites);
                    srenew(tdata->f_buf, tdata->nalloc);
                    srenew(tdata->ind,   tdata->nalloc);
                    for (int i = 0; i < vsite->nthreads; i++)
                    {
                        /* This allocation size is overkill, but simple */
                        srenew(tdata->task_ind[i].ind, tdata->nalloc);
                    }
                    srenew(tdata->use,   tdata->nalloc);
                    for (int i = 0; i < tdata->nalloc; i++)
                    {
                        clear_rvec(tdata->f_buf[i]);
                        tdata->use[i] = false;
                    }
                }
                for (int i = 0; i < vsite->nthreads; i++)
                {
                    tdata->task_ind[i].n = 0;
                }
            }

            /* Assign all vsites that can execute independently on thread */
    ...
    {
        fprintf(debug, "virtual site useTask2 %d nuse:",
                vsite->tdata[0]->useTask2);
        for (int th = 0; th < vsite->nthreads + 1; th++)
        {
            fprintf(debug, " %4d", vsite->tdata[th]->nuse);
        }
        fprintf(debug, "\n");
        ...
        for (int ftype = f_vsite_start; ftype < f_vsite_end; ftype++)
        {
            if (ilist[ftype].nr > 0)
            {
                ...
                {
                    fprintf(debug, " %4d %4d ",
                            vsite->tdata[th]->ilist1[ftype].nr,
                            vsite->tdata[th]->ilist2[ftype].nr);
                }
                fprintf(debug, "\n");
            }
        }
