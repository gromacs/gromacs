...
struct vsite_thread_t {
    t_ilist ilist1[F_NRE];    /* first level vsite ilists for this thread  */
    t_ilist ilist2[F_NRE];    /* second lebel vsite ilists for this thread */
...
#pragma omp parallel num_threads(vsite->nthreads)
        {
            try
            {
                ...
                construct_vsites_thread(vsite,
                                        ...
                                        pbc_null);
                /* Here we don't need a barrier (unlike the vsite spreading),
...
static gmx_inline void rvec_inc_atomic(rvec dest, const rvec source)
{
#pragma omp atomic
    dest[XX] += source[XX];
#pragma omp atomic
...
static void spread_vsite2(const t_iatom ia[], real a,
                          const rvec x[],
                          rvec f[], bool atomic, rvec fshift[],
                          const t_pbc *pbc, const t_graph *g)
{
    ...
    {
        rvec_inc_atomic(f[ai], fi);
        rvec_inc_atomic(f[aj], fj);
    }
    /* 6 Flops */
                                  ...
                                  const t_graph *g, const t_pbc *pbc_null)
{
    ...
    for (int ftype = f_vsite_end - 1; ftype >= f_vsite_start; ftype--)
    {
        ...
        for (int i = 0; i < nr; )
        {
            ...
            {
                case F_VSITE2:
                    spread_vsite2(ia, a1, x, f, atomic, fshift, pbc_null2, g);
                    break;
                case F_VSITE3:
                    b1 = ip[tp].vsite.b;
                    spread_vsite3(ia, a1, b1, x, f, atomic, fshift, pbc_null2, g);
                    break;
                case F_VSITE3FD:
                    b1 = ip[tp].vsite.b;
                    spread_vsite3FD(ia, a1, b1, x, f, atomic, fshift, VirCorr, dxdf, pbc_null2, g);
                    break;
                case F_VSITE3FAD:
                    b1 = ip[tp].vsite.b;
                    spread_vsite3FAD(ia, a1, b1, x, f, atomic, fshift, VirCorr, dxdf, pbc_null2, g);
                    break;
                case F_VSITE3OUT:
                    b1 = ip[tp].vsite.b;
                    c1 = ip[tp].vsite.c;
                    spread_vsite3OUT(ia, a1, b1, c1, x, f, atomic, fshift, VirCorr, dxdf, pbc_null2, g);
                    break;
                case F_VSITE4FD:
                    b1 = ip[tp].vsite.b;
                    c1 = ip[tp].vsite.c;
                    spread_vsite4FD(ia, a1, b1, c1, x, f, atomic, fshift, VirCorr, dxdf, pbc_null2, g);
                    break;
                case F_VSITE4FDN:
                    b1 = ip[tp].vsite.b;
                    c1 = ip[tp].vsite.c;
                    spread_vsite4FDN(ia, a1, b1, c1, x, f, atomic, fshift, VirCorr, dxdf, pbc_null2, g);
                    break;
                case F_VSITEN:
                    inc = spread_vsiten(ia, ip, x, f, atomic, fshift, pbc_null2, g);
                    break;
                default:
                    gmx_fatal(FARGS, "No such vsite type %d in %s, line %d",
                              ftype, __FILE__, __LINE__);
...
#pragma omp parallel num_threads(vsite->nthreads)
        {
            try
            {
                ...
                /* Spread the vsites that spread to multiple atom blocks.
                 * This is done using atomic reduction to f.
                 */
                spread_vsite_f_thread(vsite,
                                      x, f, true, fshift_t,
                                      VirCorr, vsite->tdata[thread]->dxdf,
                                      idef->iparams,
                                      vsite->tdata[thread]->ilist2,
                                      g, pbc_null);
                /* We need a barrier to ensure all cross thread-block
                 * force atomic additions have finished before starting
                 * the "normal" tasks below that run without atomics.
                 */
#pragma omp barrier
                           ...
                           t_block *cgs, int *a2cg)
{
    ...
    snew(vsite_pbc, f_vsite_start - f_vsite_start);
 ...
 * as given by thread and natperthread.
                                    ...
                                    const unsigned short  *ptype)
{
    for (int ftype = f_vsite_start; ftype < f_vsite_end; ftype++)
    {
        ...
        for (int i = 0; i < ilist[ftype].nr; )
        {
            ...
            if (iat[1 + i]/natperthread != thread)
            {
            ...
            if (ftype != F_VSITEN)
            {
                ...
                {
                    if (task_ind[iat[j]] != thread)
                    {
                        if (ptype[iat[j]] == eptVSite)
                        {
            ...
            if (task == thread || task == nthreads + thread)
            {
                ...
                if (il_task->nr + inc > il_task[ftype].nalloc)
                {
...
#pragma omp parallel for num_threads(vsite->nthreads) schedule(static)
    for (int thread = 0; thread < vsite->nthreads; thread++)
    {
        try
        {
            ...
            assign_vsites_to_thread(vsite->tdata[thread],
                                    thread, vsite->nthreads,
                                    task_ind, natperthread,
