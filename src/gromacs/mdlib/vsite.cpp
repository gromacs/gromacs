...
enum class PbcMode
{
    all,         // Apply PBC for all vsites
    chargeGroup, // Keep vsite in the same periodic image as the rest of it's charge group
    none         // No PBC treatment needed
};

/*! Returns the PBC mode based on the PBC of the system and vsite properties
 *
 * \param[in] pbcOrNullPtr  A pointer to a PBC struct or nullptr when no PBC treatment is required
 * \param[in] vsite         A pointer to the vsite struct, can be nullptr
 */
static PbcMode getPbcMode(const t_pbc       *pbcOrNullPtr,
                          const gmx_vsite_t *vsite)
{
    if (pbcOrNullPtr == nullptr)
    {
        return PbcMode::none;
    }
