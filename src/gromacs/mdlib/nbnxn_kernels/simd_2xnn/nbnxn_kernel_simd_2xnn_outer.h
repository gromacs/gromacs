...

{
    ...
    diagonal_jmi_S    = simdLoad(nbat->simd_2xnn_diagonal_j_minus_i);
    ...
    filter_S0 = simdLoadI(reinterpret_cast<const int *>(exclusion_filter + 0*UNROLLJ));
    filter_S2 = simdLoadI(reinterpret_cast<const int *>(exclusion_filter + 2*UNROLLJ));
#else
    filter_S0 = simdLoad(reinterpret_cast<const real *>(exclusion_filter + 0*UNROLLJ));
    filter_S2 = simdLoad(reinterpret_cast<const real *>(exclusion_filter + 2*UNROLLJ));
#endif
    ...
    for (n = 0; n < nbl->nci; n++)
    {
        ...
        ix_S0          = simdLoad1DualHsimd(x+scix);
        ix_S2          = simdLoad1DualHsimd(x+scix+2);
        iy_S0          = simdLoad1DualHsimd(x+sciy);
        iy_S2          = simdLoad1DualHsimd(x+sciy+2);
        iz_S0          = simdLoad1DualHsimd(x+sciz);
        iz_S2          = simdLoad1DualHsimd(x+sciz+2);
        ix_S0          = simdAdd(ix_S0, shX_S);
        ix_S2          = simdAdd(ix_S2, shX_S);
        iy_S0          = simdAdd(iy_S0, shY_S);
        iy_S2          = simdAdd(iy_S2, shY_S);
        iz_S0          = simdAdd(iz_S0, shZ_S);
        iz_S2          = simdAdd(iz_S2, shZ_S);

        if (do_coul)
        {
            SimdReal facel_S;

            facel_S    = simdSet1(facel);

            iq_S0      = simdLoad1DualHsimd(q+sci);
            iq_S2      = simdLoad1DualHsimd(q+sci+2);
            iq_S0      = simdMul(facel_S, iq_S0);
            iq_S2      = simdMul(facel_S, iq_S2);
        }

#ifdef LJ_COMB_LB
        hsig_i_S0 = simdLoad1DualHsimd(ljc+sci2);
        hsig_i_S2 = simdLoad1DualHsimd(ljc+sci2+2);
        seps_i_S0 = simdLoad1DualHsimd(ljc+sci2+STRIDE);
        seps_i_S2 = simdLoad1DualHsimd(ljc+sci2+STRIDE+2);
#else
#ifdef LJ_COMB_GEOM
        SimdReal   c6s_S0, c12s_S0;
        SimdReal   c6s_S2, c12s_S2;

        c6s_S0 = simdLoad1DualHsimd(ljc+sci2);

        if (!half_LJ)
        {
            c6s_S2 = simdLoad1DualHsimd(ljc+sci2+2);
        }
        c12s_S0 = simdLoad1DualHsimd(ljc+sci2+STRIDE);
        if (!half_LJ)
        {
            c12s_S2 = simdLoad1DualHsimd(ljc+sci2+STRIDE+2);
        }
...
#undef CALC_LJ
        ...
        real fShiftX = simdReduceIncr4ReturnSumHsimd(f+scix, fix_S0, fix_S2);
        real fShiftY = simdReduceIncr4ReturnSumHsimd(f+sciy, fiy_S0, fiy_S2);
        real fShiftZ = simdReduceIncr4ReturnSumHsimd(f+sciz, fiz_S0, fiz_S2);
