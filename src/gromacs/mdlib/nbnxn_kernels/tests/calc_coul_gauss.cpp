...
class CoulombTest : public gmx::test::CommandLineTestBase
{
    ...
    protected:
                         ...
                         real lambda)
        {
            ...
            snew(nbat->q, nbat->natoms);
            nbat->q[0]  = -1.000;
            nbat->q[1]  = 1.000;
            nbat->q[2]  = 0.000;
            nbat->q[3]  = 0.000;
            nbat->ntype = 4;
            snew(nbat->type, nbat->ntype);
            nbat->type[0] = 0;
            nbat->type[1] = 1;
            nbat->type[2] = 2;
            nbat->type[3] = 2;
            nbat->xstride = 3;
            snew(nbat->x, nbat->natoms*nbat->xstride);
            nbat->x[0]  = 0.000;
            nbat->x[1]  = 5.000;
            nbat->x[2]  = 10.000;
            nbat->x[3]  = 0.000;
            ...
            real  zeta[]      = {zeta1, zeta2, 0, 0};
            real *zeta_matrix = make_zeta_matrix(nbat->ntype, zeta);
            /* Now copy the zeta_matrix */
            snew(nbat->zeta_matrix, nbat->ntype*nbat->ntype);
            ...
            for (int i = 0; i < 9; i++)
            {
                ...
                nbat->x[3] = lambda*(i+1);
                /* In order to be able to initialize the number of elements should
                 * be fixed.
                 */
                real f[12] = {};
                real fshift[12] = {};
