...
#include "../simd/tests/base.h"

namespace SIMDTests
{

#ifdef TEST_4XN_TABLE_LOAD

#define GMX_SIMD_J_UNROLL_SIZE 1
#include "../simd_4xn/nbnxn_kernel_simd_4xn_common.h"

#elif defined TEST_2XNN_TABLE_LOAD

#define GMX_SIMD_J_UNROLL_SIZE 2
#include "../simd_2xnn/nbnxn_kernel_simd_2xnn_common.h"

#else
#error "Need to define a flavour of table load to test"
#endif
    ...
    public SimdFunctionTestBase
{
    public:
        ...
        SimdFunctionDoingTableLoad() : Parent(), indices(0),
                                       bDonePrepare_(false)
        {
            snew_aligned(indices, GMX_SIMD_WIDTH_HERE, GMX_SIMD_WIDTH_HERE * sizeof(real));
        }
        ...
        // TODO The duplication of callFunction_f* and RunTest_f* is
        // embarassing, but I do not know how to fix it in C++98. They
        // differ only in the value of numOutputs and the call
        // signatures of the wrapped function under test.

        template<class SimdFunctionSet,
                 typename FunctionType>
        void callFunction_f(SimdFunctionSet &simdFunctionSet,
                            ...
                            real            *_result)
        {
            ...
            indices   = simdFunctionSet.load_pr(_indices);
            ...
            snew_aligned(result, numOutputs, GMX_SIMD_WIDTH_HERE * sizeof(real));
        ...
        template<class SimdFunctionSet,
                 typename FunctionType>
        void callFunction_f_v(SimdFunctionSet &simdFunctionSet,
                              FunctionType     function,
                              real            *table,
                              real            *_indices,
                              size_t           numOutputs,
        ...
        void RunTest_f(ReferenceFunctionType referenceFunction,
                       TestFunctionType      testFunction)
        {
            GMX_ASSERT(bDonePrepare_, "Must call prepare() before RunTest()");
            GMX_ASSERT(numOutputs_ == 2, "Wrong number of outputs");
            for (int k = 0; k < g_numberOfRepeats; ++k)
            {
                ...
                for (unsigned int j = 0; j != GMX_SIMD_WIDTH_HERE; ++j)
                {
                    indices[j] = gmx_rng_uniform_real(rng_) * numTableEntries_;
                }

                callFunction_f(referenceSimd_,
                               referenceFunction,
                               table,
                               indices,
                               numOutputs_,
                               referenceResult_);
                callFunction_f(hardwareSimd_,
                               testFunction,
                               table,
                               indices,
                               numOutputs_,
        ...
        template<typename ReferenceFunctionType,
                 typename TestFunctionType>
        void RunTest_f_v(ReferenceFunctionType referenceFunction,
                         TestFunctionType      testFunction)
        {
            GMX_ASSERT(bDonePrepare_, "Must call prepare() before RunTest()");
            GMX_ASSERT(numOutputs_ == 3, "Wrong number of outputs");
            for (int k = 0; k < g_numberOfRepeats; ++k)
            {
                // Make random non-overflowing indices into the table
                for (unsigned int j = 0; j != GMX_SIMD_WIDTH_HERE; ++j)
                ...
                callFunction_f_v(referenceSimd_,
                                 referenceFunction,
                                 table,
                                 indices,
                                 numOutputs_,
                                 referenceResult_);
                callFunction_f_v(hardwareSimd_,
                                 testFunction,
                                 table,
                                 indices,
                                 numOutputs_,
