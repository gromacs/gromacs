...
void SimdFunctionDoingTableLoad_f_v::callFunction(SimdFunctionSet &simdFunctionSet,
                                                  ...
                                                  real            *_result)
{
    /* Temp array and pointer required by some implementations of
       load_table*. Allocate twice the necessary size to be sure SIMD
       alignment can be achieved within it. */
    int  ti_array[2*GMX_SIMD_WIDTH_HERE];
    int *ti = gmx_simd_align_int(ti_array);
