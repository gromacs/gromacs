...
/*! \internal \file
 * \brief
 * Tests for 2xnn kernel functionality for loading interaction
 * masks. This is a lot simpler than the 4xn case, because there is
 * only one implementation (AVX256 in single).
 *
                                    ...
                                    gmx_simd_ref_pb        *interact_S2)
{
    ...
    gmx_simd_ref_exclfilter mask_S = gmx_simd_ref_load1_exclfilter(excl);
    *interact_S0  = gmx_simd_ref_checkbitmask_pb(mask_S, filter_S0);
    *interact_S2  = gmx_simd_ref_checkbitmask_pb(mask_S, filter_S2);
}

//! Typedef for the test fixture
typedef SimdFunctionTest_kernel<unsigned> SimdFunction_load_interactions_2xnn;
...
{
    prepare(1, UNROLLI);
    RunTest(gmx_simd_ref_load_interactions_2xnn,
