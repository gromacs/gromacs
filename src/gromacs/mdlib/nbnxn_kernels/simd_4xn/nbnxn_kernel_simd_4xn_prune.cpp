...
/* Prune a single nbnxn_pairtlist_t entry with distance rlistPrune */
void
nbnxn_kernel_prune_4xn(nbnxn_pairlist_t          gmx_unused *nbl,
                       const nbnxn_atomdata_t    gmx_unused *nbat,
                       const rvec                gmx_unused *shift_vec,
                       real                      gmx_unused  rlistPrune)
{
#ifdef GMX_NBNXN_SIMD_4XN
    ...
    const SimdReal                  rlist2_S(rlistPrune*rlistPrune);

    /* Initialize the new list as empty and add pairs that are in range */
    int nciPruned = 0;
    int ncjPruned = 0;
    for (int i = 0; i < nbl->nci0; i++)
    {
        const nbnxn_ci_t * gmx_restrict ciEntry = &ciOrig[i];
