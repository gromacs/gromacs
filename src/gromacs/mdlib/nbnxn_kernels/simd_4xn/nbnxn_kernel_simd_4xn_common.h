                               ...
                               SimdBool                         *interact_S3)
{
    ...
    *interact_S0  = cvtIB2B(isNonZero( mask_pr_S & filter_S0 ));
    *interact_S1  = cvtIB2B(isNonZero( mask_pr_S & filter_S1 ));
    *interact_S2  = cvtIB2B(isNonZero( mask_pr_S & filter_S2 ));
    *interact_S3  = cvtIB2B(isNonZero( mask_pr_S & filter_S3 ));
#elif GMX_SIMD_HAVE_LOGICAL
#ifdef GMX_DOUBLE
    int      excl64    = excl;
    SimdReal mask_pr_S = load1(reinterpret_cast<real *>(&excl64));
#else
    SimdReal mask_pr_S = load1(reinterpret_cast<real *>(&excl));
#endif
    *interact_S0  = isNonZero( mask_pr_S & filter_S0 );
    *interact_S1  = isNonZero( mask_pr_S & filter_S1 );
    *interact_S2  = isNonZero( mask_pr_S & filter_S2 );
    *interact_S3  = isNonZero( mask_pr_S & filter_S3 );
#else
    // Neither real or integer bitwise logical operations supported.
    // Load masks from memory instead.
    *interact_S0  = isNonZero( load( simd_interaction_array + GMX_SIMD_REAL_WIDTH*((excl >> (0 * UNROLLJ)) & 0xF) ) );
    *interact_S1  = isNonZero( load( simd_interaction_array + GMX_SIMD_REAL_WIDTH*((excl >> (1 * UNROLLJ)) & 0xF) ) );
    *interact_S2  = isNonZero( load( simd_interaction_array + GMX_SIMD_REAL_WIDTH*((excl >> (2 * UNROLLJ)) & 0xF) ) );
    *interact_S3  = isNonZero( load( simd_interaction_array + GMX_SIMD_REAL_WIDTH*((excl >> (3 * UNROLLJ)) & 0xF) ) );
#endif
}
