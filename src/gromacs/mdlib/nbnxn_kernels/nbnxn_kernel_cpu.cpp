...
/* Reduce the group-pair energy buffers produced by a SIMD kernel
...
reduceGroupEnergyBuffers(int numGroups, int numGroups_2log,
    ...
    for (int i = 0; i < numGroups; i++)
    {
        ...
        for (int j1 = 0; j1 < numGroups; j1++)
        {
            for (int j0 = 0; j0 < numGroups; j0++)
            {
                ...
                {
                    Vvdw[i*numGroups + j0] += VSvdw[c + 0];
                    Vvdw[i*numGroups + j1] += VSvdw[c + 1];
                    Vc  [i*numGroups + j0] += VSc  [c + 0];
                    Vc  [i*numGroups + j1] += VSc  [c + 1];
                    c                      += unrollj + 2;
                }
            }
        }
...
#pragma omp parallel for schedule(static) num_threads(nthreads)
    for (int nb = 0; nb < nnbl; nb++)
    {
        ...
        else
        {
            ...
            {
                case 2:
                    reduceGroupEnergyBuffers<2>(nbat->nenergrp, nbat->neg_2log,
                                                out->VSvdw, out->VSc,
                                                out->Vvdw, out->Vc);
                    break;
                case 4:
                    reduceGroupEnergyBuffers<4>(nbat->nenergrp, nbat->neg_2log,
                                                out->VSvdw, out->VSc,
                                                out->Vvdw, out->Vc);
                    break;
                case 8:
                    reduceGroupEnergyBuffers<8>(nbat->nenergrp, nbat->neg_2log,
                                                out->VSvdw, out->VSc,
                                                out->Vvdw, out->Vc);
                    break;
                default:
