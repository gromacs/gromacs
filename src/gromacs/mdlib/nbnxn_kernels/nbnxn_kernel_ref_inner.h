...
#else       /*assumes UNROLLJ>UNROLLI*/
            skipmask = !((cj*2 == ci_sh && j <= i) || (cj*2 + 1 == ci_sh && j <= i+UNROLLJ/2));
#endif
#endif
#else
#define interact 1.0
...
#else
            dx  = xi[XX*UNROLLI+i] - x[cj*UNROLLJ*X_STRIDE+XX*UNROLLJ+j]; //assumes UNROLLJ>=UNROLLI
