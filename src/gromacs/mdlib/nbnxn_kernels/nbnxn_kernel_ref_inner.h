...

{
    ...
    for (i = 0; i < UNROLLI; i++)
    {
        ...
        ai = ci*UNROLLI + i;
#if defined CALC_COULOMB && defined CALC_COUL_GAUSS
        type_i_off = type[ai]*(ntype-1);
#endif
        type_i_off2 = type[ai]*ntype2;
...
#endif
            {
                c6      = nbfp[type_i_off2+type[aj]*2  ];
                c12     = nbfp[type_i_off2+type[aj]*2+1];
...
#error "No LJ Ewald combination rule defined"
            ...
            zeta             = nbat->zeta_matrix[type_i_off+type[aj]];
            ...
            {
                screening_factor = interact-std::erf(kappa*rsq*rinv);
            }
