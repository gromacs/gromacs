...
static inline void add_j_to_nblist(t_nblist *nlist, int j_atom)
{
    ...
    if (nlist->nrj >= nlist->maxnrj)
    {
        ...
        if ((nlist->maxnrj*sizeof(nlist->jjnr)) > INT_MAX)
        {
            gmx_fatal(FARGS, "Failed to allocate memory for %d interactions, which is larger than the maximum of %zu. If your simulation is not using interaction cutoffs, you should use them.", nlist->maxnrj, INT_MAX/sizeof(nlist->jjnr));
        }
                                      ...
                                      const t_excl *bexcl, gmx_bool i_is_j)
{
    ...
    if (nlist->nrj >= nlist->maxnrj)
    {
        ...
        if ((nlist->maxnrj*sizeof(nlist->excl)*MAX_CGCGSIZE) > INT_MAX)
        {
            gmx_fatal(FARGS, "Failed to allocate memory for %d interactions, which is larger than the maximum of %zu. If your simulation is not using interaction cutoffs, you should use them.", nlist->maxnrj, INT_MAX/(sizeof(nlist->excl)*MAX_CGCGSIZE));
        }
        if (gmx_debug_at)
        {
            fprintf(debug, "Increasing SR nblist (ielec=%d,ivdw=%d,type=%d,igeometry=%d) j size to %d\n",
