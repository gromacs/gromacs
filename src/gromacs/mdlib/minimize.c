             ...
             int imdport, int imdfreq, unsigned long gmx_unused Flags)
{
    ...
    init_imd(ir, cr, top_global, fplog, 1, state_global->natoms, state_global->x, mdatoms, nfile, fnm, NULL, imdport, imdfreq, Flags);
             ...
             gmx_walltime_accounting_t walltime_accounting)
{
    ...
    for (step = 0; (number_steps < 0 || (number_steps >= 0 && step <= number_steps)) && !converged; step++)
    {
        ...
        if (do_IMD(step, cr, TRUE, state_global->box, state_global->x, inputrec, 0) && MASTER(cr))
        {
