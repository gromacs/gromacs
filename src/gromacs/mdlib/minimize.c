...
void init_em(FILE *fplog, const char *title,
             ...
             int imdport, int imdfreq, unsigned long gmx_unused Flags)
{
    int  start, homenr, i;
    real dvdl_constr;
    ...
    IMD_init(ir, cr, top_global, fplog, 1, state_global->natoms, state_global->x, mdatoms, nfile, fnm, NULL, imdport, imdfreq, Flags);
             ...
             gmx_walltime_accounting_t walltime_accounting)
{
    ...
    init_em(fplog, CG, cr, inputrec,
            ...
            nfile, fnm, &outf, &mdebin, imdport, imdfreq, Flags);
    ...
    for (step = 0; (number_steps < 0 || (number_steps >= 0 && step <= number_steps)) && !converged; step++)
    {
        ...
        if (MASTER(cr))
        {
            ...
            IMD_prepare_energies(inputrec->bIMD, inputrec->imd->setup, enerd, step, TRUE);
    ...
    while (!bDone && !bAbort)
    {
        ...
        if (MASTER(cr))
        {
            ...
            if (s_try->epot < s_min->epot)
            {
                ...
                /* Prepare IMD energy record, if bIMD is TRUE. */
                IMD_prepare_energies(inputrec->bIMD, inputrec->imd->setup, enerd, count, TRUE);
             ...
             gmx_walltime_accounting_t walltime_accounting)
{
    ...
    init_em(fplog, NM, cr, inputrec,
            ...
            &f, &f_global,
            nrnb, mu_tot, fr, &enerd, &graph, mdatoms, &gstat, vsite, constr,
            nfile, fnm, &outf, NULL, imdport, imdfreq, Flags);
