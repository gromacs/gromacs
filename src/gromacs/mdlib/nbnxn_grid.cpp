                            ...
                            real atom_density)
{
    ...
    if (n > grid->na_sc)
    {
        ...
        else
        {
            ...
            tlen_x = tlen*nbnxn_gpu_ncluster_per_cell_x;
            tlen_y = tlen*nbnxn_gpu_ncluster_per_cell_y;
        }
    ...
    if (nc_max > grid->nc_nalloc)
    {
        ...
        else
        {
            ...
            pbb_nalloc = grid->nc_nalloc*nbnxn_gpu_ncluster_per_cell/STRIDE_PBB*NNBSBB_XXXX;
            snew_aligned(grid->pbb, pbb_nalloc, 16);
#else
            snew_aligned(grid->bb, grid->nc_nalloc*nbnxn_gpu_ncluster_per_cell, 16);
#endif
        }
                                   ...
                                   const nbnxn_grid_t  *grid)
{
    ...
    for (int c = 0; c < grid->nc; c++)
    {
        ...
        {
            int cs_w = (c*nbnxn_gpu_ncluster_per_cell + s)/STRIDE_PBB;
        ...
        {
            int cs = c*nbnxn_gpu_ncluster_per_cell + s;
            for (int d = 0; d < DIM; d++)
            {
    ...
    fprintf(fp, "ns bb: grid %4.2f %4.2f %4.2f abs %4.2f %4.2f %4.2f rel %4.2f %4.2f %4.2f\n",
            grid->sx/nbnxn_gpu_ncluster_per_cell_x,
            grid->sy/nbnxn_gpu_ncluster_per_cell_y,
            grid->atom_density > 0 ?
            grid->na_sc/(grid->atom_density*grid->sx*grid->sy*nbnxn_gpu_ncluster_per_cell_z) : 0.0,
            ba[XX], ba[YY], ba[ZZ],
            ba[XX]*nbnxn_gpu_ncluster_per_cell_x/grid->sx,
            ba[YY]*nbnxn_gpu_ncluster_per_cell_y/grid->sy,
            grid->atom_density > 0 ?
            ba[ZZ]/(grid->na_sc/(grid->atom_density*grid->sx*grid->sy*nbnxn_gpu_ncluster_per_cell_z)) : 0.0);
}
...
static void sort_cluster_on_flag(int na_c,
                                 ...
                                 int *flags)
{
    const int natoms_max = 8;
    int       sort1[natoms_max];
    int       sort2[natoms_max];

    assert(na_c <= natoms_max);
                      ...
                      nbnxn_bb_t gmx_unused *bb_work_aligned)
{
    ...
    if (nbs->bFEP)
    {
        ...
        c            = (a0 >> grid->na_c_2log) - grid->cell0*(grid->bSimple ? 1 : nbnxn_gpu_ncluster_per_cell);
        grid->fep[c] = 0;
                                  ...
                                  int *sort_work)
{
    ...
    int subdiv_y = nbnxn_gpu_ncluster_per_cell_x*subdiv_x;
    int subdiv_z = nbnxn_gpu_ncluster_per_cell_y*subdiv_y;
    ...
    for (int cxy = cxy_start; cxy < cxy_end; cxy++)
    {
        ...
        for (int sub_z = 0; sub_z < ncz*nbnxn_gpu_ncluster_per_cell_z; sub_z++)
        {
            ...
            if (sub_z % nbnxn_gpu_ncluster_per_cell_z == 0)
            {
                cz = sub_z/nbnxn_gpu_ncluster_per_cell_z;
                int c  = grid->cxy_ind[cxy] + cz;

                /* The number of atoms in this supercell */
                int na_c = std::min(grid->na_sc, na - (ash_z - ash));

                grid->nsubc[c] = std::min(nbnxn_gpu_ncluster_per_cell, (na_c + grid->na_c - 1)/grid->na_c);
            ...
            if (nbnxn_gpu_ncluster_per_cell_y > 1)
            {
            ...
            for (int sub_y = 0; sub_y < nbnxn_gpu_ncluster_per_cell_y; sub_y++)
            {
                int ash_y = ash_z + sub_y*subdiv_y;
                int na_y  = std::min(subdiv_y, na - (ash_y - ash));

                if (nbnxn_gpu_ncluster_per_cell_x > 1)
                {
                    /* Sort the atoms along x */
                    sort_atoms(XX, ((cz*nbnxn_gpu_ncluster_per_cell_y + sub_y) & 1), dd_zone,
                               nbs->a + ash_y, na_y, x,
                ...
                for (int sub_x = 0; sub_x < nbnxn_gpu_ncluster_per_cell_x; sub_x++)
                {
                    ...
                    fill_cell(nbs, grid, nbat,
                              ...
                              grid->na_c*(cx*nbnxn_gpu_ncluster_per_cell_x + sub_x) + (dd_zone >> 2),
                              grid->na_c*(cy*nbnxn_gpu_ncluster_per_cell_y + sub_y) + (dd_zone & 3),
                              grid->na_c*sub_z,
                              bb_work_aligned);
                }
            }
                       ...
                       nbnxn_atomdata_t *nbat)
{
    ...
    grid->na_sc     = (grid->bSimple ? 1 : nbnxn_gpu_ncluster_per_cell)*grid->na_c;
