...
void nbnxn_grids_init(nbnxn_search_t nbs, int nzone)
{
    ...
    /* TODO: remove */
    /*
    for (int z = 0; z < nbs->nzone; z++)
    {
        int ndomain = 1;
        if (z == 1 || z == 2) ndomain = 2;
        nbs->zone2grid[z + 1] = nbs->zone2grid[z] + ndomain;
        for (int g = nbs->zone2grid[z]; g < nbs->zone2grid[z + 1]; g++)
        {
            nbnxn_grid_init(&nbs->grid[g]);
        }
    }
    nbs->ngrid = nbs->zone2grid[nbs->nzone];
    */
}
                       ...
                       nbnxn_atomdata_t *nbat)
{
    ...
    if (grid_ind == 0)
    {
        ...
        /* Avoid zero density */
        if (dimensions->atom_density > 0)
        {
            grid->atom_density = dimensions->atom_density;
        }
        else
        {
                                 ...
                                 int *atom_start, int *bb_natoms, int *natoms)
{
    ...
    if (grid->bSimple)
    {
        *bb_natoms = std::max(grid->na_c, grid->na_cj);
                               ...
                               const int *cxy_natoms)
{
    ...
    grid->atom_end   = atom_start + natoms;
    /* TODO: Remove the last two arguments */
    calc_grid_atom_order(nbs, grid_ind, atom_start, atom_start + natoms);

    nbs->natoms_nonlocal = std::max(nbs->natoms_nonlocal, atom_start + natoms);
}
