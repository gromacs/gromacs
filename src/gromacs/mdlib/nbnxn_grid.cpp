 ...
 */
static int get_grid_nc_dim(real grid_size, real target_cell_size,
                           gmx_bool bDDrectangular, real cutoff)
{
    ...
    if (!bDDrectangular)
    {
         ...
         */
        nc = std::max(1, static_cast<int>(grid_size/target_cell_size + 0.25));
    }
    else
    {
         ...
         */
        int nc_min = std::max(1, static_cast<int>(grid_size/target_cell_size));
        int nc_max = std::max(1, static_cast<int>(grid_size/target_cell_size + 0.75));
        real comm_size_min;
        for (int i = nc_min; i <= nc_max; i++)
        {
            ...
            if (i == nc_min || comm_size < comm_size_min)
            {
                nc            = i;
                            ...
                            real pairlist_cutoff)
{
    ...
    if (n > grid->na_sc)
    {
        ...
        grid->ncx = get_grid_nc_dim(size[XX], tlen_x, DDrectX, pairlist_cutoff);
        grid->ncy = get_grid_nc_dim(size[YY], tlen_y, DDrectY, pairlist_cutoff);
    }
    else
    {
        grid->ncx = 1;
