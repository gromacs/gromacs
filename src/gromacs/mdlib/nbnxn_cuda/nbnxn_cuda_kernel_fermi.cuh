...
#else
{
    ...
    if (c_prefetchCj)
    {
        ...
        sm_nextSlotPtr += (c_nbnxnGpuClusterpairSplit * c_nbnxnGpuJgroupSize * sizeof(*cjs)/sizeof(*sm_nextSlotPtr));
    }

    /* shmem j force buffer */
    float *f_buf    = (float *)(sm_nextSlotPtr);
    sm_nextSlotPtr += (c_clSize * c_clSize * 3*sizeof(*f_buf)/sizeof(*sm_nextSlotPtr));
    /*********************************************************************/
