...
#else
{
    ...
    sm_nextSlotPtr = ((int *)(xqib + c_numClPerSupercl * c_clSize));
    ...
    if (c_prefetchCj)
    {
        ...
        sm_nextSlotPtr = (int *)(cjs + NTHREAD_Z * c_nbnxnGpuClusterpairSplit * c_nbnxnGpuJgroupSize);
    }

#ifndef LJ_COMB
    /* shmem buffer for i atom-type pre-loading */
    int *atib      = (int *)sm_nextSlotPtr;
    sm_nextSlotPtr = (int *)(atib + c_numClPerSupercl * c_clSize);
#else
    /* shmem buffer for i-atom LJ combination rule parameters */
    float2 *ljcpib = (float2 *)sm_nextSlotPtr;
    sm_nextSlotPtr = (int *)(ljcpib + c_numClPerSupercl * c_clSize);
#endif
    /*********************************************************************/
