...
#else
{
    ...
    unsigned superClInteractionMask = ((1U << c_numClPerSupercl) - 1U);

    /* shmem buffer for i x+q pre-loading */
    extern __shared__  float4 xqib[];
    /* shmem buffer for cj, for each warp separately */
    int *cjs     = ((int *)(xqib + c_numClPerSupercl * c_clSize)) + tidxz * 2 * nbnxn_gpu_jgroup_size;
#ifdef IATYPE_SHMEM
    /* shmem buffer for i atom-type pre-loading */
    int *atib    = ((int *)(xqib + c_numClPerSupercl * c_clSize)) + NTHREAD_Z * 2 * nbnxn_gpu_jgroup_size;
#endif
    ...
    for (j4 = cij4_start + tidxz; j4 < cij4_end; j4 += NTHREAD_Z)
    {
...
#endif
        {
            ...
            {
                cjs[tidxi + tidxj * nbnxn_gpu_jgroup_size/c_splitClSize] = pl_cj4[j4].cj[tidxi];
            }
...
#pragma unroll 4
            ...
            for (jm = 0; jm < nbnxn_gpu_jgroup_size; jm++)
            {
                if (imask & (superClInteractionMask << (jm * c_numClPerSupercl)))
                {
                    ...
                    cj      = cjs[jm + (tidxj & 4) * nbnxn_gpu_jgroup_size/c_splitClSize];
