...
#else
{
    ...
    sm_nextSlotPtr += (c_numClPerSupercl * c_clSize * sizeof(*xqib)/sizeof(*sm_nextSlotPtr));
    ...
    if (c_prefetchCj)
    {
        ...
        sm_nextSlotPtr += (NTHREAD_Z * c_nbnxnGpuClusterpairSplit * c_nbnxnGpuJgroupSize * sizeof(*cjs)/sizeof(*sm_nextSlotPtr));
    }

#ifndef LJ_COMB
    /* shmem buffer for i atom-type pre-loading */
    int *atib       = (int *)sm_nextSlotPtr;
    sm_nextSlotPtr += (c_numClPerSupercl * c_clSize * sizeof(*atib)/sizeof(*sm_nextSlotPtr));
#else
    /* shmem buffer for i-atom LJ combination rule parameters */
    float2 *ljcpib  = (float2 *)sm_nextSlotPtr;
    sm_nextSlotPtr += (c_numClPerSupercl * c_clSize * sizeof(*ljcpib)/sizeof(*sm_nextSlotPtr));
#endif
    /*********************************************************************/
