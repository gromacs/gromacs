                    ...
                    for (i = 0; i < NCL_PER_SUPERCL; i++)
                    {
                        if (imask & mask_ji)
                        {
...
#endif
                            {
...
#ifdef EXCLUSION_FORCES
#ifdef VDW_USER_GENERIC
                                F_invr  += qi * qj_f * int_bit * inv_r* inv_r * inv_r;
#else
                                F_invr  += qi * qj_f * int_bit * inv_r2 * inv_r;
#endif /* VDW_USER_GENERIC */
#else
#ifdef VDW_USER_GENERIC
                                F_invr  += qi * qj_f * inv_r * inv_r * inv_r;
#else
...
#ifdef EL_RF
#ifdef VDW_USER_GENERIC
                                F_invr  += qi * qj_f * (int_bit*inv_r * inv_r * inv_r - two_k_rf);
#else
...
#ifdef VDW_USER_GENERIC
                                F_invr  += qi * qj_f * (int_bit*inv_r*inv_r*inv_r + pmecorrF(beta2*r2)*beta3);
#else
...
#ifdef VDW_USER_GENERIC
                                F_invr  += qi * qj_f * (int_bit*inv_r*inv_r -
#else
...
#ifdef USE_TEXOBJ
                                          interpolate_nb_coul_Vtab(nbparam.nb_coul_Vtab_texobj, r2*inv_r, nb_coul_tab_scale);
#else
                                          interpolate_nb_coul_Vtab(r2*inv_r, nb_coul_tab_scale);
#endif
#endif /* CALC_ENERGIES */
                                ...
                                E_lj_p = int_bit *
                #ifdef USE_TEXOBJ
                                         interpolate_nb_vdw_Vtab(nbparam.nb_vdw_Vtab_texobj, r2*inv_r, nb_vdw_tab_scale);
                #else
                                         interpolate_nb_vdw_Vtab(r2*inv_r, nb_vdw_tab_scale);
                #endif // USE_TEXOBJ
