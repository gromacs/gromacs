 ...
 *  true a new list is pruned from immediately after search, when false the pruned list
 *  is re-pruned using the rlistRollingPrune distance.
 *
 *  Kernel launch parameters:
 *   - #blocks   = #pair lists, blockId = pair list Id
 *   - #threads  = NTHREAD_Z * c_clSize^2
 *   - shmem     = see nbnxn_cuda.cu:calc_shmem_required_pruneonly()
 *
...
__global__ void nbnxn_kernel_pruneonly_cuda(const cu_atomdata_t atdat,
...
#pragma unroll 8
                    for (int i = 0; i < c_numClPerSupercl; i++)
                    {
                        if (imaskCheck & mask_ji)
                        {
                            ...
                            /* If _none_ of the atoms pairs are in rlist range,
                               the bit corresponding to the current
                               cluster-pair in imask gets set to 0. */
                            if (haveFreshList && !__any(r2 < rlist_sq))
                            {
                                imaskFull &= ~mask_ji;
                            }
                            /* If any atom pair is within range, set the bit
                               corresponding to the current cluster-pair. */
                            if (__any(r2 < rlistRollingPrune_sq))
                            {
                                imaskNew |= mask_ji;
                            }
                        }
