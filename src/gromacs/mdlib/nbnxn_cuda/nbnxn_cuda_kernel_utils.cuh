...
/* These are duplicate extern texture reference declarations for the multiple
 * units compilation mode; this mode doesn't actually dereference them
 * (as it's pretty much impossible), so this only makes the shared codepath compile.
 * The "proper" texture references are created in nbnxn_cuda.cu for the single unit mode.
 */
