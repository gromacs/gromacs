 ...
 */
static inline __device__
float interpolate_coulomb_force_r(const cu_nbparam_t nbparam, float r, float scale)
{
...
#ifdef USE_TEXOBJ
    return fract1 * tex1Dfetch<float>(nbparam.coulomb_tab_texobj, index) +
           fract2 * tex1Dfetch<float>(nbparam.coulomb_tab_texobj, index + 1);
#else
...
/* GENERIC F */
static inline __device__
void interpolate_nb_generic_Ftab(const cu_nbparam_t nbparam, float r, float scale, float *F)
{
    float         normalized = scale * r;
    int           index      = (int) normalized;
    float         fract2     = normalized - index;
    float         fract1     = 1.0f - fract2;
#ifdef USE_TEXOBJ
    *F = fract1 * tex1Dfetch<float>(nbparam.nb_generic_Ftab_texobj, index) +
        fract2 * tex1Dfetch<float>(nbparam.nb_generic_Ftab_texobj, index+1);
#else
    *F = fract1 * tex1Dfetch(nb_generic_Ftab_texref, index) +
        fract2 * tex1Dfetch(nb_generic_Ftab_texref, index+1);
#endif
}

/* GENERIC VF Texture Reference */
static inline __device__
void interpolate_nb_generic_VFtab(const cu_nbparam_t nbparam, float r, float scale, float *F, float *V)
{
    float         normalized = scale * r;
    int           index      = (int) normalized;
    ...
    float         fract1     = 1.0f - fract2;
#ifdef USE_TEXOBJ
    float         F_x0 = tex1Dfetch<float>(nbparam.nb_generic_Ftab_texobj, index);
    *F = fract1 * F_x0 + fract2 * tex1Dfetch<float>(nbparam.nb_generic_Ftab_texobj, index+1);
    *V = tex1Dfetch<float>(nbparam.nb_generic_Vtab_texobj, index) - halfsc*fract2*(F_x0 + *F);
#else
    float         F_x0 = tex1Dfetch(nb_generic_Ftab_texref, index);
    *F = fract1 * F_x0 + fract2 * tex1Dfetch(nb_generic_Ftab_texref, index+1);
    *V = tex1Dfetch(nb_generic_Vtab_texref, index) - halfsc*fract2*(F_x0 + *F);
#endif
}
/* ***************************************************************************** */
/* VDW_LJ6 F Texture Reference */
static inline __device__
void interpolate_nb_vdw_LJ6_Ftab(const cu_nbparam_t nbparam, float r, float scale, float *F)
{
    float         normalized = scale * r;
    int           index      = (int) normalized;
    float         fract2     = normalized - index;
    float         fract1     = 1.0f - fract2;
#ifdef USE_TEXOBJ
    *F = fract1 * tex1Dfetch<float>(nbparam.nb_vdw_LJ6_Ftab_texobj, index) +
        fract2 * tex1Dfetch<float>(nbparam.nb_vdw_LJ6_Ftab_texobj, index+1);
#else
...
/* VDW_LJ6 VF */
static inline __device__
void interpolate_nb_vdw_LJ6_VFtab(const cu_nbparam_t nbparam, float r, float scale, float *F, float *V)
{
    float         normalized = scale * r;
    int           index      = (int) normalized;
    float         halfsc     = 0.5/scale;
    ...
    float         fract1     = 1.0f - fract2;
#ifdef TEX_OBJ
    float         F_x0 = tex1Dfetch<float>(nbparam.nb_vdw_LJ6_Ftab_texobj, index);
    *F = fract1 * F_x0 + fract2 * tex1Dfetch<float>(nbparam.nb_vdw_LJ6_Ftab_texobj, index+1);
    *V = tex1Dfetch<float>(nbparam.nb_vdw_LJ6_Vtab_texobj, index) - halfsc*fract2*(F_x0 + *F);
#else
...
static inline __device__
void interpolate_nb_vdw_LJ12_Ftab(const cu_nbparam_t nbparam, float r, float scale, float *F)
{
    float         normalized = scale * r;
    int           index      = (int) normalized;
    float         fract2     = normalized - index;
    float         fract1     = 1.0f - fract2;
#ifdef USE_TEXOBJ
    *F = fract1 * tex1Dfetch<float>(nbparam.nb_vdw_LJ12_Ftab_texobj, index) +
        fract2 * tex1Dfetch<float>(nbparam.nb_vdw_LJ12_Ftab_texobj, index+1);
#else
...
/* VDW_LJ12 VF */
static inline __device__
void interpolate_nb_vdw_LJ12_VFtab(const cu_nbparam_t nbparam, float r, float scale, float *F, float *V)
{
    float         normalized = scale * r;
    int           index      = (int) normalized;
    float         halfsc     = 0.5/scale;
    ...
    float         fract1     = 1.0f - fract2;
#ifdef USE_TEXOBJ
    float         F_x0 = tex1Dfetch<float>(nbparam.nb_vdw_LJ12_Ftab_texobj, index);
    *F = fract1 * F_x0 + fract2 * tex1Dfetch<float>(nbparam.nb_vdw_LJ12_Ftab_texobj, index+1);
    *V = tex1Dfetch<float>(nbparam.nb_vdw_LJ12_Vtab_texobj, index) - halfsc*fract2*(F_x0 + *F);
#else
...
/* EL_USER F */
static inline __device__
void interpolate_nb_coul_Ftab(const cu_nbparam_t nbparam, float r, float scale, float *F)
{
    float         normalized = scale * r;
    int           index      = (int) normalized;
    float         fract2     = normalized - index;
    float         fract1     = 1.0f - fract2;
#ifdef USE_TEXOBJ
    *F = fract1 * tex1Dfetch<float>(nbparam.nb_coul_Ftab_texobj, index) +
        fract2 * tex1Dfetch<float>(nbparam.nb_coul_Ftab_texobj, index+1);
#else
    *F = fract1 * tex1Dfetch(nb_coul_Ftab_texref, index) +
        fract2 * tex1Dfetch(nb_coul_Ftab_texref, index+1);
#endif
}
/* EL_USER VF */
static inline __device__
void interpolate_nb_coul_VFtab(const cu_nbparam_t nbparam, float r, float scale, float *F, float *V)
{
    float         normalized = scale * r;
    int           index      = (int) normalized;
    float         halfsc     = 0.5/scale;
    ...
    float         fract1     = 1.0f - fract2;
#ifdef USE_TEXOBJ
    float         F_x0 = tex1Dfetch<float>(nbparam.nb_coul_Ftab_texobj, index);
    *F = fract1 * F_x0 + fract2 * tex1Dfetch<float>(nbparam.nb_coul_Ftab_texobj, index+1);
    *V = tex1Dfetch<float>(nbparam.nb_coul_Vtab_texobj, index) - halfsc*fract2*(F_x0 + *F);
#else
    float         F_x0 = tex1Dfetch(nb_coul_Ftab_texref, index);
    *F = fract1 * F_x0 + fract2 * tex1Dfetch(nb_coul_Ftab_texref, index+1);
    *V = tex1Dfetch(nb_coul_Vtab_texref, index) - halfsc*fract2*(F_x0 + *F);
#endif
}
/* ********************************************************************************** */
