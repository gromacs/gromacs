...
//               int gmx_unused *global_atom_index) 
{
  ...
  int  m, ki, ai, aj, type;
  int li,lj;
  float dr, dr2, fbond=0, vbond=0, fij,dvdlambda_loc=0;
  rvec dx;
...
// inner product
     ...
     dvdlambda_loc = harmonic_gpu(forceparams[type].harmonic.krA,
                              forceparams[type].harmonic.krB,
                              forceparams[type].harmonic.rA,
                              forceparams[type].harmonic.rB,
                              dr, lambda, &vbond, &fbond); 

//     atomicAdd(dvdlambda, dvdlambda_loc);
...
//            int gmx_unused *global_atom_index)
{
   ...
   __shared__ rvec fshift_loc[SHIFTS];
    ...
    real dvdlambda_loc;
//    ivec jt, dt_ij, dt_kj; //used for graphs
...
//           int gmx_unused *global_atom_index)
{
    ...
    if(i<nbonds)
    {
        ...
        phi = dih_angle_gpu(x[ai], x[aj], x[ak], x[al], pbc, 
                     ...
                     r_ij, r_kj, r_kl, m, n,
                     &t1, &t2, &t3);  /*  84      */
        dvdlambda_loc = dopdihs_gpu(forceparams[type].pdihs.cpA,
...
static int nener_alloc[BO_MAX_RANKS];

static real *fr_pairsTable_data_d[BO_MAX_RANKS];
static int fr_pairsTable_alloc[BO_MAX_RANKS];
...
/*  for (int i=0;i<ntypes;i++ ) //possibly is allocted contigously, but can be safe
  {
    stat = cudaMemcpyAsync(&forceparams_d[rank][i],&iparams[i],sizeof(t_iparams),cudaMemcpyHostToDevice,streamBonded[rank]);
    CU_RET_ERR(stat, "cudaMemcpy failed");
   }*/
       ...
       if(ftype == F_PDIHS || ftype == F_PIDIHS ) {
        if(bCalcEnerVir) {
         pdihs_gpu <<<blocks,threads,0,streamBonded[rank]>>>
           ...
           pbc_tric_shift_d[rank],           // ishift_d[rank],
...
//copy forces back
  if(do_copy_this_step){
    ...
    for( i=0 ; i< natoms ; i++)
    {
      rvec_sub(input_force[i],f_bonded[rank][i],diff);
      tot_diff+=norm2(diff);
      tot_cpu +=norm2(input_force[i]);
      tot_gpu +=norm2(f_bonded[rank][i]);
    }
    printf("%d (force tot) :: %f %f %f\n",rank,tot_diff,tot_cpu,tot_gpu);
#else
    for( i=0 ; i< natoms ; i++)
    {
 ...
 for (i=0 ; i<  SHIFTS ; i++)
    {
      rvec_sub(fr->fshift[i],f_shift[rank][i],diff);
      tot_diff+=norm2(diff);
      tot_cpu +=norm2(fr->fshift[i]);
      tot_gpu +=norm2(f_shift[rank][i]);
//      printf("%d (shift force components) :: %d %f %f\n",rank,i,norm2(fr->fshift[i]),norm2(f_shift[rank][i]));
    }
    printf("%d (shift force tot) :: %f %f %f\n",rank,tot_diff,tot_cpu,tot_gpu);
#else  
  for (i = 0; i < SHIFTS; i++)
  {
  ...
  CU_RET_ERR(stat, "cudaMemcpy failed");
   ...
   for (i = 0; i < F_NRE; i++)
   {
    ...
    if(enerd->term[i] > 0 )
    {
      printf("%d (vtot) : %d %f %f %f\n",rank,i,fabs(enerd->term[i]-vtot[rank][i]),enerd->term[i],vtot[rank][i]);
    }  
#else
  ...
  {
    printf("%d (COUL 14 Tot) : %d %f %f %f\n",rank,i,fabs(grppener->ener[egCOUL14][i] - energygrp_elec[rank][i]), grppener->ener[egCOUL14][i] , energygrp_elec[rank][i]);
    printf("%d (LJ14 Tot) : %d %f %f %f\n",rank,i,fabs(grppener->ener[egLJ14][i] - energygrp_vdw[rank][i]), grppener->ener[egLJ14][i] , energygrp_vdw[rank][i]);
  }
#else
  for (i = 0; i < grppener->nener; i++)
  {
