...
/*---------------- BONDED CUDA kernels--------------*/
__global__ void
reset_gpu_bonded_kernel(float *vtot, float *dvd_lambda, fvec *force,
               ...
               int force_mapping[])
{
    ...
    if (i < nbonds)
    {
        ...
        ki = pbc_fvec_sub_gpu(pbc, x[ai], x[aj], dx,
...
__global__
void bonds_gpu_noener(const int nbonds, const int natoms,
                      const t_iatom forceatoms[], const t_iparams forceparams[],
                ...
                int force_mapping[])
{
    ...
    fvec             r_ij, r_kj;
    float            cos_theta, cos_theta2, theta, dVdt, va;
    ...
    if (i < nbonds)
    {
        ...
        dVdt = -forceparams[type].harmonic.krA * (theta - forceparams[type].harmonic.rA * DEG2RAD);
...
__global__
void angles_gpu_noener(int nbonds, int natoms,
                       const t_iatom forceatoms[], const t_iparams forceparams[],
                       const fvec x[], fvec force[],
                       const t_pbc *pbc, const fvec *pbc_hbox_diag, const matrix *pbc_box,
                       const fvec *pbc_mhbox_diag, const fvec  *pbc_fbox_diag,
...
static void dopdihs_gpu(float cpA, float phiA, int mult, float phi, float *V, float *F)
{
    ...
    sdphi = sinf(mdphi);
    *V    = cpA * (1.0 + cosf(mdphi));
    *F    = -cpA*mult*sdphi;
}

__device__
static void do_dih_fup_gpu(int i, int j, int k, int l,
...
{
    fvec  f_i, f_j, f_k, f_l;
    fvec  uvec, vvec, svec, dx_jl;
    float iprm, iprn, nrkj, nrkj2, nrkj_1, nrkj_2;
    float a, b, p, q, toler;
    ...
    {
        nrkj_1 = 1.0f/sqrt(nrkj2); // replacing std::invsqrt call
        ...
        b      = ddphi*nrkj/iprn;
        svmul_gpu(b, n, f_l);
        p      = iprod_gpu(r_ij, r_kj);
        p     *= nrkj_2;
        q      = iprod_gpu(r_kl, r_kj);
        q     *= nrkj_2;
...
#pragma unroll
        ...
        if (pbc)
        {
            t3 = pbc_fvec_sub_gpu(pbc, x[l], x[j], dx_jl,
                                  pbc_hbox_diag, pbc_box, pbc_mhbox_diag,
                                  pbc_fbox_diag,  pbc_tric_vec, pbc_tric_shift);
        }
        else
...
__device__
static void do_dih_fup_gpu_noener(int i, int j, int k, int l,
                                  int li, int lj, int lk, int ll, int natoms,
                                  float ddphi, fvec r_ij, fvec r_kj, fvec r_kl,
                                  fvec m, fvec n, fvec force[],
                                  const t_pbc *pbc,  const fvec *pbc_hbox_diag, const matrix *pbc_box,
                                  const fvec *pbc_mhbox_diag, const fvec  *pbc_fbox_diag,
                                  const fvec*  pbc_tric_vec,  const ivec*  pbc_tric_shift,
                                  const fvec x[], int t1, int t2, int t3)
...
__global__
void  pdihs_gpu_noener(int nbonds, int natoms,
                       const t_iatom forceatoms[], const t_iparams forceparams[],
                       const fvec x[], fvec f[],
                       const t_pbc *pbc, const fvec *pbc_hbox_diag, const matrix *pbc_box,
                       const fvec *pbc_mhbox_diag, const fvec  *pbc_fbox_diag,
...
void rbdihs_gpu(float *vtot, int nbonds, int natoms,
                ...
                int force_mapping[])
{
    const float      c0 = 0.0, c1 = 1.0, c2 = 2.0, c3 = 3.0, c4 = 4.0, c5 = 5.0;
    int              type, ai, aj, ak, al, i, j;
    int              li, lj, lk, ll;
    int              t1, t2, t3;
    fvec             r_ij, r_kj, r_kl, m, n;
    float            parm[NR_RBDIHS];
    float            cos_phi, phi, rbp;
    float            v, ddphi, sin_phi;
    float            cosfac;
...
__global__
void rbdihs_gpu_noener(int nbonds, int natoms,
                       const t_iatom forceatoms[], const t_iparams forceparams[],
                       const fvec x[], fvec f[],
                       const t_pbc *pbc,  const fvec *pbc_hbox_diag, const matrix *pbc_box,
                       const fvec *pbc_mhbox_diag, const fvec  *pbc_fbox_diag,
...
__global__
void  idihs_gpu_noener(int nbonds, int natoms,
                       const t_iatom forceatoms[], const t_iparams forceparams[],
                       const fvec x[], fvec f[], fvec fshift[],
                       const t_pbc *pbc,  const fvec *pbc_hbox_diag, const matrix *pbc_box,
                       const fvec *pbc_mhbox_diag, const fvec  *pbc_fbox_diag,
...
void pairs_gpu(int ftype, int nbonds, int natoms,
               ...
               float packed_R[], int packed_I[], int force_mapping[])
{
    float             qq, c6, c12;
    fvec              dx;
...
static float *dvdlambda_d[BO_MAX_RANKS];
...
// null status is used as a marker for no pbc
    ...
    stat = cudaMalloc(&dvdlambda_d[rank], efptNR*sizeof(float));
    CU_RET_ERR(stat, "cudaMalloc failed");
