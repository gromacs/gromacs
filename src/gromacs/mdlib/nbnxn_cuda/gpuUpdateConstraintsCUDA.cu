                  ...
                  matrix           box)
{
    ...
    //An update to GPU PBC data is required after every pressure step
    // TODO: fix this to only necessary steps
    if (bpbc)
    {
        cudaMemcpy(gpuUCD->pbc_hbox_diag_d, pbc->hbox_diag, sizeof(rvec), cudaMemcpyHostToDevice);
        cudaMemcpy(gpuUCD->pbc_box_d, pbc->box, sizeof(matrix), cudaMemcpyHostToDevice);
...

{
    ...
    cudaMemcpy(bErrorHasOccurred, gpuUCD->bErrorHasOccurred_d, sizeof(bool), cudaMemcpyDeviceToHost);
