...
#else
/* Prevent compilation in multiple compilation unit mode. Although we have
 ...
 */
#if (defined __CUDA_ARCH__) && (__CUDA_ARCH__ <= 210)
#error Due to an CUDA compiler bug, the CUDA non-bonded module can not be compiled with multiple compilation units for CC 2.x devices. If you have changed the nvcc flags manually, use GMX_CUDA_TARGET_* variables instead or  either set GMX_CUDA_NB_SINGLE_COMPILATION_UNIT=ON.
#endif
