                                       ...
                                       int                     numParts)
{
    ...
    if (bDoTime)
    {
        startEvent = (plist->needToPrune ? t->start_prune_k[iloc] : t->start_rollingPrune_k[iloc]);
        stopEvent  = (plist->needToPrune ? t->stop_prune_k[iloc]  : t->stop_rollingPrune_k[iloc]);
    }
                                 ...
                                 const int            iloc)
{
    ...
    if (timers->didRollingPrune[iloc])
    {
        ...
        timings->rollPruneTime.t += cu_event_elapsed(timers->start_rollingPrune_k[iloc],
