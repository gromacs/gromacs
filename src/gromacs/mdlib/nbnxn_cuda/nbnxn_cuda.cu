                             ...
                             int                     iloc)
{
    ...
    config.blockSize.x      = config.blockSize.y = c_clSize;
    config.blockSize.z      = num_threads_z;
    config.gridSize.x       = nblock;
    ...
    if (debug)
    {
        fprintf(debug, "Non-bonded GPU launch configuration:\n\tThread block: %ux%ux%u\n\t"
                ...
                "\tShMem: %zu\n",
                config.blockSize.x, config.blockSize.y, config.blockSize.z,
                config.gridSize.x, config.gridSize.y, plist->nsci*c_numClPerSupercl,
                                       ...
                                       int                     numParts)
{
    ...
    if (debug)
    {
        fprintf(debug, "Pruning GPU kernel launch configuration:\n\tThread block: %ux%ux%u\n\t"
                ...
                "\tShMem: %zu\n",
                config.blockSize.x, config.blockSize.y, config.blockSize.z,
                config.gridSize.x, config.gridSize.y, numSciInPart*c_numClPerSupercl,
                c_numClPerSupercl, plist->na_c,
                config.sharedMemorySize);
    }
    ...
    if (plist->haveFreshList)
    {
        launchGpuKernel(nbnxn_kernel_prune_cuda<true>, config, timingEvent,
                        adat, nbp, plist, &numParts, &part);
    }
    else
    {
        launchGpuKernel(nbnxn_kernel_prune_cuda<false>, config, timingEvent,
                        adat, nbp, plist, &numParts, &part);
    }
