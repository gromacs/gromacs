                               ...
                               bool                copyCoord)
{
    ...
    if (nbs == nullptr || gpu_nbv->xrvec == nullptr)
    {
        gpu_nbv->bGpuBufferOps = false;
        return false;
    }
    ...
    else if (pmedata->gpu == nullptr)
    {
        copyCoord = 1;
    }
    else if (pmedata->gpu->kernelParams == nullptr)
    {
        copyCoord = 1;
    }
    else if (pmedata->gpu->kernelParams->atoms.d_coordinates == nullptr)
    {
        copyCoord = 1;
    }
    ...
    if (copyCoord)
    {
        ...
        stat = cudaMemcpyAsync(&gpu_nbv->xrvec[copyAtomStart][0],
                               &x[copyAtomStart][0], nCopyAtoms*sizeof(rvec),
