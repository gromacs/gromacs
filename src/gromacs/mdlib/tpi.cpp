              ...
              gmx_walltime_accounting_t walltime_accounting)
{
    ...
    gmx::ThreeFry2x64<16>                rng({static_cast<gmx_uint64_t>(seed), gmx::RandomDomain_TestParticleInsertion}); // 16 bits internal counter => 2^16 * 2 = 131072 values per stream
    ...
    while (bNotLastFrame)
    {
        ...
        while (step < nsteps)
        {
             ...
             */
            rng.restart({static_cast<gmx_uint64_t>(frame_step), static_cast<gmx_uint64_t>(step)});
            ...
            if (!bCavity)
            {
                ...
                if (bNS)
                {
                    ...
                    for (d = 0; d < DIM; d++)
                    {
                        x_init[d] = dist(rng)*state->box[d][d];
                    }
                }
