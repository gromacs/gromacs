               ...
               int flags)
{
    ...
    const gmx_bool bDoingCombinedGrid = (flags & GMX_PME_DO_COULOMB) && bDoingLB;
...
#pragma omp parallel num_threads(pme->nthread) private(thread)
            {
                ...
                gmx_parallel_3dfft_execute(pme->pfft_setup[i], pme->pfft_setup[i+2],
                                           GMX_FFT_REAL_TO_COMPLEX, flags & GMX_PME_DO_COULOMB,
                                           !bDoingLB, thread, wcycle);
...
#pragma omp parallel num_threads(pme->nthread) private(thread)
        {
            ...
            for (i = 0; i < 2; i++)
            {
                ...
                gmx_parallel_3dfft_execute(pme->pfft_setup[i], pme->pfft_setup[i+2],
                                           GMX_FFT_COMPLEX_TO_REAL, flags & GMX_PME_DO_COULOMB,
                                           !bDoingLB, thread, wcycle);
            }
