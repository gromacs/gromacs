...
typedef struct gmx_pme {
    ...
    pmegrids_t pmegrid[9];  /* Grids on which we do spreading/interpolation,
...
inline static void calc_exponentials_q(int start, int end, real f, real *d_aligned, real *r_aligned, real *e_aligned)
{
    {
...
#ifdef PME_SIMD
/* Calculate exponentials through SIMD */
inline static void calc_exponentials_lj(int start, int end, real *r_aligned, real *d_aligned)
{
    gmx_mm_pr tmp_r;
    gmx_mm_pr tmp_d;
    gmx_mm_pr d_inv;
...
#else
inline static void calc_exponentials_lj(int start, int end, real *r, real *d)
{
    int kx;
    for (kx = start; kx < end; kx++)
    {
                            ...
                            gmx_bool bEnerVir, int nthread, int thread)
{
    ...
    for (iyz = iyz0; iyz < iyz1; iyz++)
    {
        ...
        if (bEnerVir)
        {
            ...
            calc_exponentials_lj(kxstart, kxend, tmp1, denom);
        ...
        else
        {
            ...
            for (kx = kxstart; kx < kxend; kx++)
            {
                ...
                eterm = -((1.0 - 2.0*factor*m2[kx])*tmp1[kx]
                 ...
                 int                 nthread)
{
    ...
    ndata[1]    = pme->nky;
    ndata[2]    = pme->nkz;
    pme->ngrids = ((ir->ljpme_comb == eljpmeLB) ? 9 : 4);
    ...
    {
        if (((ir->ljpme_comb == eljpmeLB) && i >= 2) || i % 2 == 0 || bFreeEnergy)
        {
    ...
    do /****** this is a quasi-loop over time steps! */
    {
        ...
        do
        {
            ...
            if (ret == pmerecvqxRESETCOUNTERS)
            {
                ...
                reset_pmeonly_counters(wcycle, runtime, mynrnb, ir, step);
            }
        }
               ...
               int flags)
{
     ...
     */
    for (q = 0; q < ((flags & GMX_PME_LJ_LB) ? 2 : 4); ++q)
    {
        if ((!pme->bFEP && q % 2 == 1)
...
#pragma omp parallel num_threads(pme->nthread) private(thread)
        {
            ...
            if (flags & GMX_PME_SOLVE)
            {
                ...
                if (thread == 0)
                {
                    wallcycle_start(wcycle, (q < 2 ? ewcPME_SOLVE : ewcLJPME));
                }
                if (q < 2)
                {
                    loop_count =
                ...
                if (thread == 0)
                {
                    wallcycle_stop(wcycle, (q < 2 ? ewcPME_SOLVE : ewcLJPME));
                    where();
...
#pragma omp parallel for num_threads(pme->nthread) schedule(static)
    ...
    /* For Lorentz-Bertholot combination rules in LJ-PME, we need to calculate
     * seven terms. */
...
#pragma omp parallel num_threads(pme->nthread) private(thread)
                ...
                where();
                bClearF = (bFirst && PAR(cr));
                scale   = pme->bFEP ? (q < 9 ? 1.0-lambda : lambda) : 1.0;
            ...
            else
            {
                ...
                *dvdlambda   += energy_AB[1] - energy_AB[0];
                for (i = 0; i < DIM; i++)
                {
                    for (j = 0; j < DIM; j++)
                    {
                        vir_q[i][j] += (1.0-lambda)*vir_AB[0][i][j] +
                            lambda*vir_AB[1][i][j];
                    }
                }
            }
