                                  ...
                                  real rtab, int flags)
{
    ...
    {
        printf ("etiLJ6 table: setting scalefactor to 1/6 for the forces\n");
        scalefactor = 1.0/6.0;
        i_type      = etiLJ6_Verlet;
    }
    if (flags & GMX_MAKETABLES_USER_VDW_LJ12)
    {
        printf ("etiLJ12 table: setting scalefactor to 1/12 for the forces\n");
    ...
    if (i_type >= etiCOUL_Verlet || i_type < etiNR_Verlet)
    {
        ...
        FILE *fp;
        fp = fopen ("tableLJ12.csv", "w");
        for (i = 0; i < table.size; i++)
        {
            if (i_type == etiLJ12_Verlet)
            {
                fprintf (fp, "%d \t %g \t %g\n", i/table.size, td[i_type].v[i], td[i_type].f[i]);
            }
