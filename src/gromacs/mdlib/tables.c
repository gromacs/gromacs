                                  ...
                                  real rtab, int flags)
{
    ...
    nx        = table.size = (int)(rtab*td[0].tabscale + 0.5);
    ...
    if (table.size != 1000)
    {
        gmx_fatal(FARGS, "Only tables of 1000 points are currently implemented with CUDA Verlet cut-off scheme.\n\t Please use a table of exactly 1000 points.");
    }
        
    snew_aligned(table.F, (nx+1)*sizeof(double), 32);
    snew_aligned(table.V, (nx+1)*sizeof(double), 32);
    ...
    {
        printf ("etiCOUL table: setting scalefactor to 1.0 for Coulomb\n");
        scalefactor = 1.0;
        i_type      = etiCOUL_Verlet;
    }
    if (flags & GMX_MAKETABLES_USER_VDW_LJ6)
    {
        printf ("etiLJ6 table: setting scalefactor to 1/6\n");
        scalefactor = 1.0/6.0;
        i_type      = etiLJ6_Verlet;
    }
    if (flags & GMX_MAKETABLES_USER_VDW_LJ12)
    {
        printf ("etiLJ12 table: setting scalefactor to 1/12\n");
        scalefactor = 1.0/12.0;
        i_type      = etiLJ12_Verlet;
    }
    if (flags & GMX_MAKETABLES_USER)
    {
        printf ("etiUSER table: setting scalefactor to 1.0 for LJ\n");
        scalefactor = 1.0;
