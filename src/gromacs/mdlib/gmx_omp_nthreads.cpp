 ...
 *
 * \param[in] yieldPoint  name of yield point
 * \return whether to yield, based on user-configured schedule
 */
bool shouldYield(std::string yieldPoint) {
    if (yieldPoint == "fft_comm0") {
        return gapTaskType[3] != 'E';
    }
    if (yieldPoint == "fft_comm1") {
        return gapTaskType[4] != 'E';
    }
    return false;
}
...
long getGapData(std::string taskName, std::vector<STSGapBoundary>& gapBounds) {
    ...
    int nthreadsOnlyNB = numNbls - numNblsLL;
    // -1 because thread 0 does not do NB or reduction
    int nthreadsLL = STS::getNumThreads() - nthreadsOnlyNB - 1;
    ...
    int stbegin = 0;
    int stend = nthreadsOnlyNB;
    long startTime = 0;
    for (int sti=stbegin; sti<stend; sti++) {
        startTime += st[sti]->getRunStartTime();
        totalWork += st[sti]->getRunEndTime() - st[sti]->getRunStartTime();
    }
    gapBounds.emplace_back(start,stbegin,stend,startTime/nthreadsOnlyNB);
    gapBounds.emplace_back(end,  stbegin,stend,maxTime);
    ...
    for (int gapNum : relevantGaps) {
        ...
        switch (gapNum) {
            case 3:
                endTime = fftCommEndTime[0];
                break;
            case 4:
                endTime = fftCommEndTime[1];
                break;
            case 6:
                endTime = maxTime;
            default:
        ...
        for (int sti=stbegin; sti<stend; sti++) {
            startTime += st[sti]->getRunStartTime();
            totalWork += st[sti]->getRunEndTime() - st[sti]->getRunStartTime();
            maxOverRun = std::max(maxOverRun, st[sti]->getRunEndTime() - endTime);
        }

        gapBounds.emplace_back(start,stbegin,stend,startTime/nthreadsLL);
        // Watch out for negative or zero gap sizes and compensate for overruns
        endTime = std::max(startTime/nthreadsLL+1, endTime - maxOverRun);
...
void setNblPortions(const std::vector<long> &subTaskTimes, long totalWork,
    ...
    if (taskName == "stdreduce") {
        for (size_t sti=0; sti<subTaskDist.size(); sti++) {
        ...
        int firstLLNB = numNbls-numNblsLL;
        // Nbr. lists for NB-only threads
        for (int sti=0; sti<firstLLNB; sti++) {
            au[sti] = std::max(1.0, round(subTaskDist[sti] * nblDenom));
            totalAUs += au[sti];
        }
        ...
        int nbli = firstLLNB;
        for (size_t sti=firstLLNB; sti < subTaskTimes.size(); sti++) {
            // -1 in denominator because thread 0 does not do NB
            int gapNum = (sti - firstLLNB) / (STS::getNumThreads() - firstLLNB - 1);
...
void sts_create_schedule_for_force_compute(const t_commrec *cr, bool doInit,
                  SchedType stype, double ratioThreadsForNB, char* gapTask)
{
    ...
    numNbls = nthreadsNB + (numNblBoth-1) * nthreadsBoth;
    numNblsLL = numNbls - nthreadsOnlyNB;
    ...
    int nblStartL  = nthreadsOnlyNB;
    int nblStartNL = nthreadsOnlyNB;
    for (int gapNum = 0; gapNum < NUM_WORK_GAPS; gapNum++) {
        ...
        switch(gapNum) {
            case 1:
                ...
                sts->assign_loop("calc_pidx_wrapper", threadsLL);
                break;
            case 2:
