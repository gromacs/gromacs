 ...
 * \param[in]     cFirst              The first cluster in the j-range, using i-cluster size indexing
 * \param[in]     cLast               The last cluster in the j-range, using i-cluster size indexing
                       ...
                       int                *numDistanceChecks)
{
    ...
    int jclusterFirst = ci_to_cj_simd_4xn(cFirst);
#if GMX_SIMD_REAL_WIDTH >= NBNXN_CPU_CLUSTER_I_SIZE
    int jclusterLast  = ci_to_cj_simd_4xn(cLast);
#else
    /* Set the correct last j-cluster with a j-cluster size of 2 */
    int jclusterLast  = ci_to_cj_simd_4xn(cLast + 1) - 1;
#endif
