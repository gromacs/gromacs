...
std::pair<std::unique_ptr<CheckpointSignalSetter>,
          std::unique_ptr<CheckpointSignalHandler>>
                                          ...
                                          gmx_walltime_accounting_t  walltime_accounting)
{
    ...
    {
        if (MASTER(cr)) {
            setter = std::unique_ptr<CheckpointSignalSetter>(
                    new CheckpointSignalSetter(sig, needSync, cr, mdrunOptions, bGStat, walltime_accounting));
        }
        handler = std::unique_ptr<CheckpointSignalHandler>(
                new CheckpointSignalHandler(sig, ir, mdrunOptions, bNS, bLastStep, step));
    }
...
std::pair<std::vector<std::unique_ptr<ISignalSetter>>,
                      std::unique_ptr<StopSignalHandler>>
SignalHelper::makeStopSignalClasses(gmx::SimulationSignal       * sig,
...
#if GMX_THREAD_MPI
    if (MASTER(cr)) {
        setters.emplace_back(std::unique_ptr<StopSignalSetterCondition>(
                new StopSignalSetterCondition(
                        sig, needSync, ir, mdrunOptions,
                        nstSignalComm, fplog)));
    }
#else
    setters.emplace_back(std::make_unique<StopSignalSetterCondition>(
    ...
    if (MASTER(cr) && mdrunOptions.maximumHoursToRun > 0) {
        setters.emplace_back(std::unique_ptr<StopSignalSetterTime>(
                new StopSignalSetterTime(
                        sig, needSync, ir, mdrunOptions,
                        fplog, bNS, step, walltime_accounting)));
    }

    auto handler = std::unique_ptr<StopSignalHandler>(
            new StopSignalHandler(sig, ir, bNS));
...
std::pair<std::unique_ptr<ResetSignalSetter>,
          std::unique_ptr<ResetSignalHandler>>
                                          ...
                                          const pme_load_balancing_t* pme_loadbal)
{
    ...
    if (mdrunOptions.timingOptions.resetHalfway)
    {
        ...
        handler = std::unique_ptr<ResetSignalHandler>(
                new ResetSignalHandler(
                        sig, cr, step, step_rel, walltime_accounting, wcycle,
                        mdlog, fplog, nbv, pme, nrnb, pme_loadbal));

        if (MASTER(cr) && (mdrunOptions.maximumHoursToRun > 0))
        {
            setter = std::unique_ptr<ResetSignalSetter>(
                    new ResetSignalSetter(sig, needSync, mdrunOptions, walltime_accounting));
        }
    }
