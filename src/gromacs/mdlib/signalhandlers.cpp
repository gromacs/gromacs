...
void SignalHelper::makeCheckpointSignalClasses(gmx::SimulationSignal    * sig,
                                               const t_inputrec         * ir,
                                               const t_commrec          * cr,
                                               const MdrunOptions        &mdrunOptions,
                                               bool                       needSync,
                                               ...
                                               gmx_bool                  &bCPT,
                                               gmx_walltime_accounting_t  walltime_accounting,
                                               CheckpointSignalSetter  ** setter,
                                               CheckpointSignalHandler ** handler)
{
    ...
    {
        *setter = new CheckpointSignalSetter(sig, needSync, cr, mdrunOptions,
                                             bGStat, walltime_accounting);
    }
    *handler = new CheckpointSignalHandler(sig, ir, mdrunOptions,
                                           bNS, bLastStep, step, bCPT);
}

void SignalHelper::deleteCheckpointSignalClasses(gmx::CheckpointSignalSetter  **setter,
                                                 gmx::CheckpointSignalHandler **handler)
{
    if (*handler != nullptr)
    {
        delete *handler;
...
void SignalHelper::makeStopSignalClasses(gmx::SimulationSignal       * sig,
                                         bool                          needSync,
                                         const t_inputrec            * ir,
                                         const t_commrec             * cr,
                                         const MdrunOptions           &mdrunOptions,
                                         ...
                                         gmx_bool                     &bLastStep,
                                         gmx_walltime_accounting_t     walltime_accounting,
                                         std::vector<ISignalSetter *> &setters,
                                         StopSignalHandler          ** handler)
{
#if GMX_THREAD_MPI
    if (MASTER(cr))
    {
        setters.emplace_back(new StopSignalSetterCondition(
                                     sig, needSync, ir, mdrunOptions,
                                     nstSignalComm, fplog));
    }
#else
    setters.emplace_back(new StopSignalSetterCondition(
                                 sig, needSync, ir, cr, mdrunOptions,
                                 nstSignalComm, fplog, bNS, step, walltime_accounting));
#endif

    if (MASTER(cr) && mdrunOptions.maximumHoursToRun > 0)
    {
        setters.emplace_back(new StopSignalSetterTime(
                                     sig, needSync, ir, mdrunOptions,
                                     fplog, bNS, step, walltime_accounting));
    }

    *handler = new StopSignalHandler(sig, ir, bNS, bLastStep);
}

void SignalHelper::deleteStopSignalClasses(std::vector<gmx::ISignalSetter *> &setters,
                                           gmx::StopSignalHandler           **handler)
{
    delete *handler;
...
void SignalHelper::makeResetSignalClasses(gmx::SimulationSignal     * sig,
                                          bool                        needSync,
                                          const t_inputrec          * ir,
                                          const t_commrec           * cr,
                                          const MdrunOptions         &mdrunOptions,
                                          ...
                                          ResetSignalSetter        ** setter,
                                          ResetSignalHandler       ** handler)
{
    if (mdrunOptions.timingOptions.resetHalfway)
    {
        ...
        *handler = new ResetSignalHandler(sig, cr, step, step_rel,
                                          walltime_accounting, wcycle,
                                          mdlog, fplog, nbv, pme, nrnb,
                                          pme_loadbal);

        if (MASTER(cr) && (mdrunOptions.maximumHoursToRun > 0))
        {
            *setter = new ResetSignalSetter(sig, needSync, mdrunOptions,
                                            walltime_accounting);
        }
        else
        {
...
void SignalHelper::deleteResetSignalClasses(gmx::ResetSignalSetter  **setter,
                                            gmx::ResetSignalHandler **handler)
{
    if (*handler != nullptr)
    {
        delete *handler;
