...
XvgTable readXvgTable(TextReader *reader)
{
    ...
    Regex xAxisLabel("[[:space:]]*xaxis[[:space:]]+label.*\".*\"");
    Regex yAxisLabel("[[:space:]]*yaxis[[:space:]]+label.*\".*\"");
    Regex dataSetLegendLabelOne("legend string[[:space:]]+[0-9]+[[:space:]]+.*\".*\"");
    Regex dataSetLegendLabelTwo("s[0-9]+[[:space:]]+legend.*\".*\"");
    ...
    while (reader->readLine())
    {
        ...
        else if (line[0] == '@')
        {
            ...
            if (xvgCommand.compare(0, 5, "title") == 0)
            {
                table.title_ = findQuotedString(xvgCommand);
            }
            else if (xvgCommand.compare(0, 8, "subtitle") == 0)
            {
    ...
    {
        /* In a perfect world, if we have a non-empty but incomplete set of legend strings, then we might flag that as an
           error, but there are probably several tools that write such files, and we should continue to support reading
           even old files if that is at all reasonable. Instead, rely on std::map to invent empty strings for us. */
