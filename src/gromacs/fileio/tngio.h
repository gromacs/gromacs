...
#if !defined(GMX_FILEIO_TNGIO_FOR_TOOLS_H) && !defined(GMX_FILEIO_TRXIO_H)
...
// gmx_bool tng_prepare_restart_from_checkpoint(tng_trajectory_t tng,
//                                              const t_inputrec *ir,
//                                              const gmx_bool    bUseLossyCompression);
/* Prepare for continuing from a checkpoint file */
...
/* Write the current frame set to disk. Perform compression etc. */

float tng_get_last_frame_time(tng_trajectory_t tng);
/* Get the time (in picoseconds) of the last frame */
