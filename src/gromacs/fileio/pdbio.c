                           ...
                           gmx_conect conect, gmx_bool bTerSepChains)
{
    ...
    for (ii = 0; ii < nindex; ii++)
    {
        ...
        gmx_fprintf_pdb_atomline(out, bWideFormat,
                                 pdbtp[type],
                                 i+1,
                                 nm,
                                 altloc,
                         ...
                         const char *  element)
{
    ...
    if (atom_name != NULL)
    {
        ...
        strncpy(tmp_atomname, start_name_in_col13 ? "" : " ", 2);
        strncat(tmp_atomname, atom_name, 4);
    }
    else
    {
        snprintf(tmp_atomname, 4, "");
    }
    ...
    n = fprintf(fp,
                bWideFormat ?
                "%-6s%5u %-4.4s%c%4.4s%c%4d%c   %10.5f%10.5f%10.5f%8.4f%8.4f    %2s\n" :
                "%-6s%5d %-4.4s%c%4.4s%c%4d%c   %8.3f%8.3f%8.3f%6.2f%6.2f          %2s\n",
                (record != NULL) ? record : "ATOM",
                atom_seq_number,
                tmp_atomname,
                alternate_location,
                tmp_resname,
