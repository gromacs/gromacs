...
struct MrcMetaData{
    ...
    MrcMetaData &fromGrid(const Grid < DIM, GridWithTranslation < DIM>> &grid);
    Grid < DIM, GridWithTranslation < DIM>> toGrid();

    std::array<int, 3> to_crs_order(const std::array<int, 3> &xyz_order);
    offset<DIM>        to_xyz_order(const std::array<int, 3> &i_crs);

    MrcMetaData       &set_grid_stats(const GridDataFloat3D::container_type &grid_data);
