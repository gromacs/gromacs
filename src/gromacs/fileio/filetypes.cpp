...
int fn2ftp(const std::string &fn)
{
    ...
    auto foundPosition =
        std::find_if(std::begin(deffile), std::end(deffile),
                     [&extension](const t_deffile &deffile)
                     {
                         return (deffile.ext != nullptr &&
                                 deffile.ext[0] != '\0' && // One entry is an empty string
                                 gmx::equalCaseInsensitive(extension, deffile.ext + 1));
                     });
