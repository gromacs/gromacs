            ...
            )
        {
            ...
            switch (fn2ftp(filename))
            {
                ...
                case efTRN:
                    ...
                    break;
                case efTNG:
                    /* Initialize topology and then set writing intervals. */
                    gmx_tng_open(filename, filemode[0], &of->tng);
                    if (filemode[0] == 'w')
                    {
                        gmx_tng_prepare_md_writing(of->tng, mtop, ir);
                    }
                    break;
                default:
            ...
            ir->nstxout_compressed > 0)
        {
            ...
            switch (fn2ftp(filename))
            {
                ...
                case efTNG:
                    ...
                    if (filemode[0] == 'w')
                    {
                        gmx_tng_prepare_low_prec_writing(of->tng_low_prec, mtop, ir);
                    }
                    break;
                default:
        ...
        if (mdof_flags & MDOF_X_COMPRESSED)
        {
            ...
            if (*n_x_compressed == -1)
            {
                *n_x_compressed = 0;
                ...
                {
                    if (ggrpnr(groups, egcCompressedX, i) == 0)
                    {
                        (*n_x_compressed)++;
                    }
                }
                if (*n_x_compressed != top_global->natoms)
                {
                    snew(*x_compressed, *n_x_compressed);
                }
            }
            if (*n_x_compressed == top_global->natoms)
            {
            ...
            if (write_xtc(of->fp_xtc, *n_x_compressed, step, t,
...
void done_mdoutf(gmx_mdoutf_t of)
{
     ...
     * gmx_tng_close returns a NULL pointer. */
    if (of->tng)
    {
