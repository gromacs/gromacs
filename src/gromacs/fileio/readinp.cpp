...
t_inpfile *read_inpfile(gmx::TextReader *reader, const char *fn, int *ninp,
                        warninp_t wi)
{
    t_inpfile *inp = NULL;

    using gmx::stripString;
    using gmx::stripComment;
    ...
    while (reader->readLine(&line))
    {
        ...
        auto equalsPosition = line.find_first_of('=');
        ...
        auto lbuf = line.substr(0, equalsPosition);
        lbuf = stripString(lbuf);
        if (lbuf.empty())
        {
        ...
        auto rbuf = line.substr(equalsPosition+1, std::string::npos);
        rbuf = stripString(rbuf);
        if (rbuf.empty())
        {
            // TODO ignoring such lines does not seem like good behaviour
            if (debug)
        ...
        int found_index = search_einp(countOfUniqueKeysFound, inp, lbuf.c_str());
        ...
        if (found_index == -1)
        {
            ...
            inp[countOfUniqueKeysFound-1].name       = gmx_strdup(lbuf.c_str());
            inp[countOfUniqueKeysFound-1].value      = gmx_strdup(rbuf.c_str());
        }
        else
        {
...
void write_inpfile(gmx::TextWriter *writer, const char *fn, int ninp, t_inpfile inp[], gmx_bool bHaltOnUnknown,
                   warninp_t wi)
{
    ...
    std::string output;
    output += gmx::niceHeader(fn);

    gmx::BinaryInformationSettings settings;
    settings.generatedByHeader(true);
    settings.linePrefix(";\t");
    output += gmx::printBinaryInformation(gmx::getProgramContext(), settings);
    ...
    for (int i = 0; (i < ninp); i++)
    {
        if (inp[i].bSet)
        {
            ...
            {
                output += formatString("%-24s\n", inp[i].name);
            }
            else
            {
                output += formatString("%-24s = %s\n", inp[i].name, inp[i].value ? inp[i].value : "");
            }
        }
