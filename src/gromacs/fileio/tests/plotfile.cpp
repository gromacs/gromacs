...
{
    public:
        PlotFileTest() : outputFile_("test.xvg")
        {
            setOutputFile("-o", outputFile_.c_str(), gmx::test::ExactTextMatch());
        ...
        gmx::PlotFile *myPlotFile()
        {
            std::string    s = fileManager().getTemporaryFilePath(outputFile_.c_str());
            printf("s = %s\n", s.c_str());
            gmx::PlotFile *xvg = new gmx::PlotFile(s.c_str(), "title", "xaxis",
                                                   "yaxis", oenv_);
            return xvg;
        }
        void addLegend()
        {
            gmx::PlotFile *xvg = myPlotFile();
