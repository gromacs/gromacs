...
TEST(MrcSerializer, DefaultHeaderHasRightSerialSize)
{
    ...
    const gmx::MrcDensityMapHeader inputHeader;
...
TEST(MrcSerializer, RoundTripReturnsInput)
{
    gmx::InMemorySerializer        serializer;
    const gmx::MrcDensityMapHeader inputHeader;
    serializeMrcDensityMapHeader(&serializer, inputHeader);
    ...
    gmx::InMemoryDeserializer      deserializer(serializedHeader);
    ...
    EXPECT_THAT(serializedHeader, Pointwise(Eq(), roundTripResult));
}
