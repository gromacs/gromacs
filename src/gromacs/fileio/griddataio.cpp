...
void MrcFile::Impl::do_mrc_data_(GridDataReal3D &grid_data, bool bRead)
{
    ...
    if (bRead)
    {
        if (file_size_ < headerBytes_c + meta_.extended_header.size() + long(lattice.getNumLatticePoints()*sizeof(float)) )
        {
        ...
        if (file_size_ > headerBytes_c + meta_.extended_header.size() +  long(lattice.getNumLatticePoints()*sizeof(float)) )
        {
            fprintf(stderr, "WARNING : Density format file size is %zd, "
                    ...
                    lattice.getNumLatticePoints()*sizeof(float));
        }
    }

    for (int section = 0; section  < meta_.num_crs[ZZ]; section++)
    {
        for (int row  = 0; row  < meta_.num_crs[YY]; row++)
        {
            for (int column  = 0; column  < meta_.num_crs[XX]; column++)
            {
