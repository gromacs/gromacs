...
void MrcFile::Impl::do_mrc_header_(bool bRead)
{
     ...
     * emdb convention 0 */
    // set the number of bytes in the extended header
    if (!bRead)
    {
        meta_.num_bytes_extened_header = meta_.extended_header.size();
        const int bytes_to_multiple_of_80 = meta_.num_bytes_extened_header % 80;
        if (bytes_to_multiple_of_80 != 0)
        {
            meta_.num_bytes_extened_header += 80 - bytes_to_multiple_of_80;
        }
        meta_.extended_header.resize(meta_.num_bytes_extened_header);
    }
    do_(&meta_.num_bytes_extened_header, bRead);
    ...
    if (bRead)
    {
        ...
        meta_.extended_header.resize(meta_.num_bytes_extened_header);
        if (meta_.num_bytes_extened_header > 0)
        {
