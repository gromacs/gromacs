...
class MrcFile::Impl
{
    public:
        ...
        void do_mrc_data_(GridDataFloat3D &grid_data, bool bRead);
        ...
        template <typename T> void do_(T * result, bool isReading)
        {
            if (isReading)
            {
...
void MrcFile::Impl::do_mrc_header_(bool bRead)
{
    ...
    do_(&(meta_.min_value ), bRead);
    do_(&(meta_.max_value ), bRead);
    do_(&(meta_.mean_value), bRead);
    ...
    do_(&meta_.space_group, bRead);

    /* 24 | NSYMBT | signed int | 80n
     * # of bytes in symmetry table (multiple of 80)
     * emdb convention 0 */
    int numBytesExtendedHeader = 0;
    do_(&numBytesExtendedHeader, bRead);
    if (numBytesExtendedHeader%80 != 0)
    {
        GMX_THROW(gmx::FileIOError("Read invalid number of bytes in symbol table from mrc/cpp4/imod file. Should be 80, but is " + std::to_string(numBytesExtendedHeader) + "instead."));
    }
    ...
    if (meta_.is_crystallographic)
    {
         ...
         * emdb convention 0 */
        int32_t hasSkewMatrix = meta_.has_skew_matrix ? 1 : 0;
        do_(&hasSkewMatrix, bRead);
        if (bRead)
        {
    ...
    {
        do_(&meta_.format_identifier[i], bRead);
    }
     ...
     */
    do_(&(meta_.machine_stamp), bRead);

    /* 55 | RMS | floating pt
     * Density root-mean-square deviation */
    do_(&(meta_.rms_value), bRead);
    ...
    if (bRead)
    {
        for (auto &label : meta_.labels)
        {
            ...
            if (fread(labelToRead.data(), 1, labelSize_c, file_) != labelSize_c)
            {
    ...
    {
        for (const auto &label : meta_.labels)
        {
            fprintf(file_, "%.*s", static_cast<int>(labelSize_c), label.c_str());
        }
    }
    ...
    if (bRead)
    {
        if (file_size_ < numBytesExtendedHeader + headerBytes_c)
        {
            GMX_THROW(gmx::FileIOError("Density format file is smaller than expected extended header."));
        }
        meta_.extended_header.resize(numBytesExtendedHeader);
        if (!meta_.extended_header.empty())
        {
            if (fgets(&((meta_.extended_header)[0]), numBytesExtendedHeader+1, file_) == nullptr)
            {
...
void MrcFile::Impl::do_mrc_data_(GridDataFloat3D &grid_data, bool bRead)
{
    const auto &lattice = grid_data.getGrid().lattice();
    if (bRead)
    {
        if (file_size_ < headerBytes_c + meta_.extended_header.size() + long(lattice.size()*sizeof(float)) )
        {
            GMX_THROW(gmx::FileIOError("Density format file is smaller than indicated in its header."));
        }
        if (file_size_ > headerBytes_c + meta_.extended_header.size() +  long(lattice.size()*sizeof(float)) )
        {
            fprintf(stderr, "WARNING : Density format file size is %zd, "
                    "however header (%zd) + symbol table (%zd) + data  (%zd) "
                    "is larger than indicated in its header. Reading anyway.. \n",
                    file_size_, headerBytes_c, meta_.extended_header.size(),
                    lattice.size()*sizeof(float));
        }
    }

    for (auto index : bounds<DIM>({meta_.num_crs[XX], meta_.num_crs[YY], meta_.num_crs[ZZ]}))
    {
        const auto currentOffset = meta_.to_xyz_order({static_cast<int>(index[XX]), static_cast<int>(index[YY]), static_cast<int>(index[ZZ])});
        do_(&(grid_data[currentOffset]), bRead);
    }
}
...
MrcFile::Impl::Impl() : file_(nullptr), file_size_(0), filetypes({"mrc", "ccp4", "imod", "map"}
                                                                 )
{
...
void MrcFile::write_with_own_meta(std::string filename, GridDataFloat3D &grid_data, const MrcMetaData &meta, bool bOwnGridStats)
{
    bool bRead = false;
    ...
    impl_->do_mrc_data_(*(const_cast<GridDataFloat3D*>(&grid_data)), bRead);
    impl_->close_file();
}

void MrcFile::write(std::string filename, const GridDataFloat3D &grid_data)
{
    bool bRead = false;
    impl_->meta_.set_grid_stats(grid_data);
    impl_->open_file(filename, bRead);
...
GridDataFloat3D MrcFile::read_with_meta(std::string filename, MrcMetaData *meta)
{
...
GridDataFloat3D MrcFile::read(std::string filename)
{
    bool bRead = true;
    impl_->open_file(filename, bRead);
    impl_->do_mrc_header_(bRead);
    auto result = GridDataFloat3D(impl_->meta_.toGrid());
