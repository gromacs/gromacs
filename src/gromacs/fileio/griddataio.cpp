...
void MrcFile::Impl::do_mrc_data_(GridDataReal3D &grid_data, bool bRead)
{
    ...
    {
        const auto &currentPosition =  grid_data[meta_.to_xyz_order({static_cast<int>(index[XX]), static_cast<int>(index[YY]), static_cast<int>(index[ZZ])})];
        do_(&(*currentPosition), bRead);
    }
}
