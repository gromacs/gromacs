...
{
#ifdef GMX_USE_TNG
    gmx_bool            bOK = TRUE, bRequestedIds = TRUE;
    ...
    {
        bRequestedIds = FALSE;
        snew(requestedIds, sizeof(gmx_int64_t) * DEFAULT_NUM_IDS);
        requestedIds[0] = TNG_TRAJ_BOX_SHAPE;
        requestedIds[1] = TNG_TRAJ_POSITIONS;
        requestedIds[2] = TNG_TRAJ_VELOCITIES;
        requestedIds[3] = TNG_TRAJ_FORCES;
        requestedIds[4] = TNG_GMX_LAMBDA;
    }
    ...
    /* If there were no requestedIds provide when calling the function they
     * were reserved in the function, which means they must be freed. */
    if (!bRequestedIds && requestedIds)
    {
        sfree(requestedIds);
    }
