...
struct tng_trajectory_output
{
    ...
    gmx_large_int_t  firstWrittenStep;
};
                         ...
                         gmx_large_int_t          step)
{
    ...
    {
        // TODO set time per frame, writing interval(=1?)
                             ...
                             t_trxframe             *frame)
{
    ...
    double time = frame->time * PICO / frame->step;
    // TODO GROMACS used to check before setting this, but why should we need to?
    tng_time_per_frame_set(tng->out, time);
    // kinda like write_trxframe(trxout, &frame, gc); from gmx_trjconv.c
    // TODO what is gc?
    fwrite_tng(tng->out,
               // TODO make compression configurable
               TRUE,
               frame->step - tng->firstWrittenStep,
               frame->time,
               0,
    ...
    switch (datatype)
    {
            ...
            if (sizeof(real) == sizeof(float))
            {
                ...
                else
                {
                    for (i = 0; i < natoms; i++)
                    {
                        for (j = 0; j < nvalues; j++)
                        {
                            to[i*3+j] = (real)((float *)from)[i*3+j] * fact;
                        }
                    }
                }
            }
        ...
        case TNG_INT_DATA:
            for (i = 0; i < natoms; i++)
            {
                for (j = 0; j < nvalues; j++)
                {
                    to[i*3+j] = (real)((int64_t *)from)[i*3+j] * fact;
                }
            }
            break;
        case TNG_DOUBLE_DATA:
            if (sizeof(real) == sizeof(double))
            {
                ...
                else
                {
                    for (i = 0; i < natoms; i++)
                    {
                        for (j = 0; j < nvalues; j++)
                        {
                            to[i*3+j] = (real)((double *)from)[i*3+j] * fact;
                        }
                    }
                }
            }
            ...
            break;
        default:
            /* TODO: Should print some error here */
            return;
    }
    return;
}
