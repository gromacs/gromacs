...
{
#ifdef GMX_USE_TNG
    /* FIXME: Currently only standard block types are read */
    ...
    if (*input)
    {
        ...
        if (index && nAtoms > 0)
        {
            setup_atom_subgroup(*output, nAtoms, index, indexGroupName);
        }
    ...
    else
    {
        /* TODO after trjconv is modularized: fix this so the user can
           ...
           tng_set_compression_precision(*output, ndec2prec(nDecimalsOfPrecision));
         */
        if (nAtoms >= 0)
        {
                         ...
                         const char      *name)
{
    ...
    if (stat == TNG_FAILURE)
    {
        ...
        for (int i = 0; i < nind; i++)
        {
            ...
            if (stat == TNG_SUCCESS)
            {
                tng_residue_id_of_particle_nr_get(tng, ind[i], &id);
                if (tng_chain_residue_find(tng, chain, temp_name, id, &res)
                    != TNG_SUCCESS)
                {
                    tng_chain_residue_w_id_add(tng, chain, temp_name, id, &res);
                }
                tng_residue_atom_w_id_add(tng, res, "", "", ind[i], &atom);
            }
        }
