...
/*! \libinternal \file
 * \brief
 * Helper class for making error messages when reading line-based input
 *
...
ErrorMessageMaker::ErrorMessageMaker() : lineIndex_(0), currentLine_(nullptr) {}
...
void ErrorMessageMaker::setCurrentLine(const std::string *line)
{
    lineIndex_++;
    currentLine_ = line;
}

InvalidInputError ErrorMessageMaker::make(const std::string &message) const
{
    if (currentLine_)
    {
