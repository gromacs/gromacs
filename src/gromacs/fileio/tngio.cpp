                               ...
                               const t_inputrec *ir)
{
    ...
    if (ir->nstxout)
    {
        tng_util_pos_write_interval_set(tng, ir->nstxout);
         ...
         * position writing. */
        tng_util_box_shape_write_interval_set(tng, ir->nstxout);
        ...
        tng_util_generic_write_interval_set(tng, ir->nstxout, 1, TNG_GMX_LAMBDA,
    ...
    /* TODO: we are not passing a frequency to the
       tng_util_*_write_interval_set() functions, we are passing an
       interval between events. Should this be fixed in the TNG
       API? */
    if (ir->nstvout)
    {
        tng_util_vel_write_interval_set(tng, ir->nstvout);
    }
    if (ir->nstfout)
    {
        tng_util_force_write_interval_set(tng, ir->nstfout);
    }
#endif
}
                ...
                const rvec *x, const rvec *v, const rvec *f)
{
    ...
    stat = tng_util_generic_with_time_write(tng, step, (double)(t)*0.000000000001, reinterpret_cast<float *>(&lambda),
