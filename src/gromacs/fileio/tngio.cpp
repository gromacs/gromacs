...
typedef tng_function_status (*write_data_func_pointer)(tng_trajectory_t,
                                                       ...
                                                       const char);
#ifdef GMX_DOUBLE
static set_writing_interval_func_pointer set_writing_interval = tng_util_generic_write_interval_double_set;
static write_data_func_pointer           write_data           = tng_util_generic_with_time_double_write;
#else
static set_writing_interval_func_pointer set_writing_interval = tng_util_generic_write_interval_set;
...
static void add_mtop(tng_trajectory_t  tng,
...
//         gcd = greatest_common_divisor_if_positive(gcd, ir->nstfout);
        ...
        if (ir->nstvout && (gcd == -1 || ir->nstvout < gcd))
        {
            gcd = ir->nstvout;
        }
        if (ir->nstfout && (gcd == -1 || ir->nstfout < gcd))
        {
            gcd = ir->nstfout;
        }
    }
                                  ...
                                  const t_inputrec *ir)
{
    ...
    tng_time_per_frame_set(tng, ir->delta_t * PICO);
    tng_compression_precision_set(tng, ir->x_compression_precision);
#else
    GMX_UNUSED_VALUE(tng);
    GMX_UNUSED_VALUE(mtop);
    GMX_UNUSED_VALUE(ir);
