                               ...
                               const t_inputrec *ir)
{
    ...
    lcd = lcd_if_positive(nstxout, nstvout);
    lcd = lcd_if_positive(lcd, nstfout);
    if (0 >= lcd)
    {
        return;
    }

    numFramesPerFrameSet = std::min(ir->nsteps,
    ...
    /* TODO: what does TNG do if you pass it a zero frequency? Do we
       even need to check this here? */
