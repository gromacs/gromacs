...
void tng_add_mtop(tng_trajectory_t gmx_unused  tng,
                  const gmx_mtop_t gmx_unused *mtop)
{
#ifdef GMX_USE_TNG
    int                  mol_it, at_it, i, j;
    char                 chain_name[2];
    ...
    for (mol_it = 0; mol_it < mtop->nmolblock; mol_it++)
    {
...
void tng_add_topology(tng_trajectory_t gmx_unused  tng,
                      const t_topology gmx_unused *top)
{
    ...
    for (mol_it = 0; mol_it < top->mols.nr; mol_it++)
    {
        tng_molecule_add(tng, "unknown molecule", &tng_mol);
        atoms = &top->atoms;
        ...
        // TODO is mols.nr useful?
        tng_molecule_cnt_set(tng, tng_mol, top->mols.nr);
    }
#endif
}
                              ...
                              const gmx_mtop_t gmx_unused *mtop)
{
    ...
    const t_atom        *at;
    const t_resinfo     *resin;
    int                  n_atoms = 0, i = 0, j, mol_it, at_it;
    ...
    tng_molecule_add(tng, "XTC GROUPS", &mol);
    ...
    for (mol_it = 0; mol_it < mtop->nmoltype; mol_it++)
    {
        ...
        for (j = 0; j < mtop->molblock[mol_it].nmol; j++)
        {
            for (at_it = 0; at_it < atoms->nr; at_it++, i++)
            {
                ...
                tng_residue_atom_add(tng, res, *(atoms->atomname[at_it]), *(atoms->atomtype[at_it]), &atom);
                n_atoms++;
            }
        }
    }
                               ...
                               const t_inputrec gmx_unused *ir)
{
    ...
    if (xout)
    {
        ...
        tng_set_writing_interval(tng, xout, 1, TNG_GMX_LAMBDA,
                                 ...
                                 TNG_GZIP_COMPRESSION);
        /* TODO: At what interval should we write lambdas? Currently using
         * the same as box shape. */
    }
    if (ir->nstvout)
    {
                ...
                const rvec      *f)
{
    ...
    stat = tng_write_data(tng, step, elapsedSeconds,
    ...
    stat = tng_write_data(tng, step, elapsedSeconds,
                          ...
                          TNG_NON_PARTICLE_BLOCK_DATA,
                          TNG_GZIP_COMPRESSION);
    /* TODO: Verify that lambda gets written properly.
     */
    bOK  = bOK && stat == TNG_SUCCESS;
