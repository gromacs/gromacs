              ...
              tng_trajectory_t *tng)
{
    ...
    if (mode == 'w' || mode == 'a')
    {
        /* FIXME: When adding data to the header, subsequent blocks might get
                                         ...
                                         const t_inputrec *ir)
{
    ...
    {
        /* FIXME: Currently we are not using the lowest common
         * denominator since there is no way to make sure that
         * there is output that often (e.g. write lambdas every
         * step even if there is no other output). */
...
//         lcd = lcd_if_positive(lcd, ir->nstfout);
        ...
        if(ir->nstvout && (lcd == -1 || ir->nstvout < lcd))
        {
            lcd = ir->nstvout;
        }
        if(ir->nstfout && (lcd == -1 || ir->nstfout < lcd))
        {
            lcd = ir->nstfout;
        }
    }
    ...
    /* If there are fewer steps than what would fit in one frame set make
     * the frame sets smaller. ir->nsteps + 1 since data is written at
     * step 0. */
    numFramesPerFrameSet = std::min((int) ir->nsteps + 1,
...
//     int  low_denom = -1, lowest = -1;
    ...
    if (xout)
    {
        ...
        /* FIXME: Currently we write lambdas at the same interval as
         * positions. Later we must save the writing intervals somehow
         * so that outputs can be made even if there is no x, v or f
         * data to write. */
        tng_set_writing_interval(tng, xout, 1, TNG_GMX_LAMBDA,
                                 "LAMBDAS", TNG_NON_PARTICLE_BLOCK_DATA,
                                 TNG_GZIP_COMPRESSION);

        /* FIXME: low_denom and lowest currently not used. */
...
//         }
    ...
    /* FIXME: See above. low_denom interval for lambdas is disabled. */
//     if (low_denom > 0)
//     {
                                  ...
                                  const t_inputrec *ir)
{
    ...
    tng_time_per_frame_set(tng, ir->delta_t * PICO);
    tng_compression_precision_set(tng, ir->xtcprec);
#else
    GMX_UNUSED_VALUE(tng);
    GMX_UNUSED_VALUE(mtop);
    GMX_UNUSED_VALUE(ir);
...
/* FIXME: Currently when restarting from a check point there might be
 * data that is written after the checkpoint. This is currently kept
 * intact, but inaccessible, by writing at the end of the file, but
 * with file pointers pointing from the frame set written at the
 * checkpoint to the new frame set and vice versa. */
// gmx_bool tng_prepare_restart_from_checkpoint(tng_trajectory_t tng,
//                                              const t_inputrec *ir,
//                                              const gmx_bool    bUseLossyCompression)
// {
//     int prevStep, minOut = -1;
//
//     if(bUseLossyCompression)
//     {
//         prevStep = ir->init_step - ir->nstxtcout;
//     }
//     else
//     {
//         if(ir->nstxout)
...
//             minOut = ir->nstxout;
//         }
//         if(ir->nstvout && (minOut < 0 || ir->nstvout < minOut))
//         {
//             minOut = ir->nstvout;
//         }
//         if(ir->nstfout && (minOut < 0 || ir->nstfout < minOut))
//         {
//             minOut = ir->nstfout;
//         }
//         if(minOut < 0)
...
//             minOut = 1;
//         }
//         prevStep = ir->init_step - minOut;
//     }
//
//     tng_util_prepare_append_after_frame(tng, prevStep);
//
//    return TRUE;
//}
