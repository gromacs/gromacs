                                 ...
                                 const gmx_mtop_t     *mtop)
{
    ...
    if (mtop->groups.numberOfGroupNumbers(static_cast<int>(SimulationGroups::g_CompressedX)) == 0)
    {
        return;
    }
    ...
    if (all_atoms_selected(mtop, SimulationGroups::g_CompressedX))
    {
        return;
    }
    ...
    nameIndex = *mtop->groups.groups[static_cast<int>(SimulationGroups::g_CompressedX)].nm_ind;
    ...
    for (const gmx_molblock_t &molBlock :  mtop->molblock)
    {
        ...
        for (int j = 0; j < molBlock.nmol; j++)
        {
            ...
            for (int atomIndex = 0; atomIndex < atoms->nr; atomIndex++, i++)
            {
                ...
                if (getGroupType(mtop->groups, SimulationGroups::g_CompressedX, i) != 0)
                {
                    continue;
                }
            ...
            if (bAtomsAdded)
            {
                for (int k = 0; k < F_NRE; k++)
                {
                    if (IS_CHEMBOND(k))
                    {
                        ...
                        for (int l = 1; l < ilist.size(); l += 3)
                        {
                            ...
                            if (getGroupType(mtop->groups, SimulationGroups::g_CompressedX, atom1) == 0 &&
                                getGroupType(mtop->groups, SimulationGroups::g_CompressedX, atom2) == 0)
                            {
                ...
                for (int l = 1; l < ilist.size(); l += 4)
                {
                    ...
                    if (getGroupType(mtop->groups, SimulationGroups::g_CompressedX, atom1) == 0)
                    {
                        if (getGroupType(mtop->groups, SimulationGroups::g_CompressedX, atom2) == 0)
                        {
                            tng_molecule_bond_add(tng, mol, atom1,
                                                  atom2, &tngBond);
                        }
                        if (getGroupType(mtop->groups, SimulationGroups::g_CompressedX, atom3) == 0)
                        {
                            tng_molecule_bond_add(tng, mol, atom1,
                                                  atom3, &tngBond);
                        }
