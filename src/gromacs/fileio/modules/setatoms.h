...
class SetAtoms : public ICoordinateOutput
{
    public:
         ...
         * framework.
         */
        explicit SetAtoms(t_atoms *atoms) : localAtoms_(atoms)
        {
        }
        /*! \brief
         * Copy constructor.
    ...
    private:
        ...
        bool haveLocalAtoms()
        {
            GMX_ASSERT(localAtoms_, "Cannot use method without setting t_atoms information");
            return localAtoms_->nr > 0 ? true : false;
        }
        //! Checking if t_trxframe has the atom information saved within.
        bool haveFrameAtoms()
        {
            return coordinates_.bAtoms;
        }
        /*! \brief
