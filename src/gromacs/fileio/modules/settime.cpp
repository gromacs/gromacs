...
void
SetTime::processFrame(const int framenumber, const t_trxframe &input)
{
    coordinates_ = input;
    ...
    if (bSetTimeStep_)
    {
        currentTime = startTime_ + framenumber*timeShift_;
    }
    else if (bSetStartTime_)
    {
        startTime_ += timeShift_;
        currentTime = startTime_;
    }

    coordinates_.time  = currentTime;
    coordinates_.bTime = true;
}
