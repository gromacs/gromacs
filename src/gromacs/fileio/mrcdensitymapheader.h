 ...
 */
struct CrystallographicLables
{
    CrystallographicLables();
    ...
    std::array<std::string, 10>                          labels_;
};
...
struct MrcDensityMapHeader{
    ...
    MrcDataMode            dataMode_;              //!< Data mode, currently only mode 2 is supported
    MachineStamp           machineStamp_;          //!< Reports endianess of map writing architecture

    std::array<char, 4>    formatIdentifier_;      //!< Expected to be "MAP "
    std::array<float, 15>  userDefinedFloat_;      //!< 15 freely defined float numbers

    CrystallographicLables labels_;                //!< Labels for crystallographic data
    ...
    std::array<int32_t, 3> numCrs_;         //!< Column, row and section count
