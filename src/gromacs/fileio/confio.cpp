                         ...
                         rvec **x, rvec **v, matrix box)
{
    ...
    *haveTopology = fn2bTPX(infile);
    if (*haveTopology)
    {
        ...
        if (periodicMolecules != nullptr)
        {
            *periodicMolecules = ir->bPeriodicMols;
        }
...
gmx_bool read_tps_conf(const char *infile, t_topology *top, int *ePBC,
                       gmx_bool *periodicMolecules, rvec **x, rvec **v, matrix box, gmx_bool requireMasses)
{
    bool        haveTopology;
    gmx_mtop_t *mtop;
    ...
    snew(mtop, 1);
    readConfAndTopology(infile, &haveTopology, mtop, ePBC, periodicMolecules, x, v, box);
