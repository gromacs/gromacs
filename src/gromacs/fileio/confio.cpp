                             ...
                             gmx_bool requireMasses)
{
    ...
    if (requireMasses && !atoms->haveMass)
    {
        atomsSetMassesBasedOnNames(atoms, TRUE);
                       ...
                       rvec **x, rvec **v, matrix box, gmx_bool requireMasses)
{
    ...
    snew(mtop, 1);
    readConfAndTopology(infile, &haveTopology, mtop, ePBC, x, v, box, requireMasses);
