...
GridDataReal3D gridDataFromXplor(const XplorData &xplor)
{
    ...
    ColumnMajorLattice<DIM>::MultiIndex extend;
...
Grid<DIM> cellFromXplor(const XplorData &xplor)
{
    ...
    ColumnMajorLattice<DIM>   lattice(ColumnMajorLattice<DIM>::MultiIndex(
                                              {{xplor.extend[XX], xplor.extend[YY], xplor.extend[ZZ]}}));
    return Grid<DIM>(cell, lattice);
}
                            ...
                            std::array<int, DIM> cellExtend)
{
    ...
    else
    {
        ...
        xplor.extend = map.getGrid().lattice().extend();
    }
    ...
    for (int dimension = XX; dimension <= ZZ; ++dimension)
    {
        xplor.crs_end[dimension] = xplor.crs_start[dimension] +
            map.getGrid().lattice().extend()[dimension] - 1;
    }
