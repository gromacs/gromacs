...
void MrcMetaData::fromGrid(const IGrid<DIM> &grid )
{
...
void MrcMetaData::set_grid_stats(const std::vector<float> &grid_data)
{
    min_value  = datavectormeasure::min(grid_data);
    max_value  = datavectormeasure::max(grid_data);
    mean_value = datavectormeasure::mean(grid_data);
    rms_value  = datavectormeasure::rms(grid_data);
}
