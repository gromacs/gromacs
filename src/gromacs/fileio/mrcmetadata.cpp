...
MrcMetaData &MrcMetaData::fromGrid(const Grid < DIM, GridWithTranslation < DIM>> &grid )
{
    setEMDBDefaults();
...
Grid < DIM, GridWithTranslation < DIM>> MrcMetaData::toGrid()
{
    RVec                gridCellLength;
    ...
    {
        translation = {(float) mrcUnitsToNm * *(extra.end()-3), (float) mrcUnitsToNm * *(extra.end()-2), (float) mrcUnitsToNm * *(extra.end()-1)};
    }

    Grid < DIM, GridWithTranslation < DIM>>  result(gridWithoutTranslation.cell(), gridWithoutTranslation.lattice(), translation);

    return result;
}

MrcMetaData &MrcMetaData::set_grid_stats(const GridDataFloat3D::container_type &grid_data)
{
