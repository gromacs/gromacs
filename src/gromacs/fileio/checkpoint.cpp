                            ...
                            gmx_bool reproducibilityRequested)
{
    ...
    {
        gmx_fatal(FARGS, "Cannot change integrator during a checkpoint restart, make a new .tpr with grompp");
    }

    if (headerContents->flags_state != state->flags)
    {
        gmx_fatal(FARGS, "Cannot change state flags during a checkpoint restart, make a new .tpr with grompp");
    }
