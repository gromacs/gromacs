...
{
    return do_cpte_reals_low(xd, cptp, ecpt, sflags, n, nullptr, v, list, ecprREAL);
}
...
static int do_cpte_real(XDR *xd, int cptp, int ecpt, int sflags,
                        real *r, FILE *list)
{
    return do_cpte_reals_low(xd, cptp, ecpt, sflags, 1, nullptr, &r, list, ecprREAL);
}
                        ...
                        int n, int **v, FILE *list)
{
    ...
    if (list == nullptr && v != nullptr && nf != n)
    {
    ...
    if (list || !(sflags & (1<<ecpt)) || v == nullptr)
    {
        snew(va, nf);
        vp = va;
    }
                         ...
                         int n, rvec **v, FILE *list)
{
    return do_cpte_reals_low(xd, cptp, ecpt, sflags,
                             n*DIM, nullptr, (real **)v, list, ecprRVEC);
}
                          ...
                          matrix v, FILE *list)
{
    ...
    ret = do_cpte_reals_low(xd, cptp, ecpt, sflags,
                            DIM*DIM, nullptr, &vr, nullptr, ecprMATRIX);
                           ...
                           int n, real **v, FILE *list)
{
    ...
    for (i = 0; i < n; i++)
    {
        reti = do_cpte_reals_low(xd, cptp, ecpt, sflags, n, nullptr, &(v[i]), nullptr, ecprREAL);
        if (list && reti == 0)
        {
                            ...
                            int n, matrix **v, FILE *list)
{
    ...
    ret = do_cpte_reals_low(xd, cptp, ecpt, sflags,
                            nf*DIM*DIM, nullptr, &vr, nullptr, ecprMATRIX);
    for (i = 0; i < nf; i++)
    {
        for (j = 0; j < DIM; j++)
        {
    ...
    for (i = 0; (i < estNR && ret == 0); i++)
    {
        if (fflags & (1<<i))
        {
            switch (i)
            {
                 ...
                 */
                case estLD_RNG:  ret      = do_cpte_ints(xd, cptpEST, i, sflags, 0, nullptr, list); break;
                case estLD_RNGI: ret      = do_cpte_ints(xd, cptpEST, i, sflags, 0, nullptr, list); break;
                case estMC_RNG:  ret      = do_cpte_ints(xd, cptpEST, i, sflags, 0, nullptr, list); break;
                case estMC_RNGI: ret      = do_cpte_ints(xd, cptpEST, i, sflags, 0, nullptr, list); break;
                           ...
                           FILE *list)
{
    ...
    if (bRead)
    {
        ...
        {
            snew(enerhist->dht, 1);
            enerhist->dht->ndh              = nullptr;
            enerhist->dht->dh               = nullptr;
                      ...
                      gmx_int64_t step, double t, t_state *state)
{
    ...
    if ((do_cpt_state(gmx_fio_getxdr(fp), FALSE, state->flags, state, nullptr) < 0)        ||
        (do_cpt_ekinstate(gmx_fio_getxdr(fp), flags_eks, &state->ekinstate, nullptr) < 0) ||
        (do_cpt_enerhist(gmx_fio_getxdr(fp), FALSE, flags_enh, state->enerhist, nullptr) < 0)  ||
        (do_cpt_df_hist(gmx_fio_getxdr(fp), flags_dfh, &state->dfhist, nullptr) < 0)  ||
        (do_cpt_EDstate(gmx_fio_getxdr(fp), FALSE, &state->edsamstate, nullptr) < 0)      ||
        (do_cpt_swapstate(gmx_fio_getxdr(fp), FALSE, &state->swapstate, nullptr) < 0) ||
                            ...
                            gmx_bool reproducibilityRequested)
{
    ...
    do_cpt_header(gmx_fio_getxdr(fp), TRUE, &file_version,
                  ...
                  &state->edsamstate.nED, &state->swapstate.eSwapCoords, nullptr);
    ...
    ret             = do_cpt_state(gmx_fio_getxdr(fp), TRUE, fflags, state, nullptr);
    ...
    *bReadEkin = ((flags_eks & (1<<eeksEKINH)) || (flags_eks & (1<<eeksEKINF)) || (flags_eks & (1<<eeksEKINO)) ||
    ...
    ret = do_cpt_enerhist(gmx_fio_getxdr(fp), TRUE,
                          flags_enh, state->enerhist, nullptr);
    if (ret)
    {
        cp_error();
    }
    ...
    ret = do_cpt_df_hist(gmx_fio_getxdr(fp), flags_dfh, &state->dfhist, nullptr);
    if (ret)
    {
        cp_error();
    }

    ret = do_cpt_EDstate(gmx_fio_getxdr(fp), TRUE, &state->edsamstate, nullptr);
    if (ret)
    {
        cp_error();
    }

    ret = do_cpt_swapstate(gmx_fio_getxdr(fp), TRUE, &state->swapstate, nullptr);
    if (ret)
    {
        cp_error();
    }
                                 ...
                                 int *nfiles, gmx_file_position_t **outputfiles)
{
    ...
    do_cpt_header(gmx_fio_getxdr(fp), TRUE, &file_version,
                  ...
                  &state->edsamstate.nED, &state->swapstate.eSwapCoords, nullptr);
    ret =
        do_cpt_state(gmx_fio_getxdr(fp), TRUE, state->flags, state, nullptr);
    if (ret)
    {
        cp_error();
    }
