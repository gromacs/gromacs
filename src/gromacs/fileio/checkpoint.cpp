...
enum {
    epullhPULL_N, epullhPULL_AVER, epullhPULL_SUM, epullhPULL_NSUM,
    epullhPULL_SUM_SIM, epullhPULL_NSUM_SIM,
    epullhPULL_NSTEPS, epullhPULL_NSTEPS_SIM,
    epullhNR
};
...
{
    "pull_n", "pull_aver", "pull_sum", "pull_nsum",
    "pull_sum_sim", "pull_nsum_sim",
    "pull_nsteps", "pull_nsteps_sim",
};
...
enum class StatePart
{
    ...
    pullHistory,      //!< Pull force and position statistics
...
static const char *entryName(StatePart part, int ecpt)
{
    switch (part)
    {
        ...
        case StatePart::pullHistory:       return epullh_names[ecpt];
...
//! \brief Returns and XDR processes for i/o of an XDR type
 ...
 * The formatting of the printing is set by \p higherLevelElementType.
 * When list==NULL only reads the elements.
 */
static bool_t listXdrVector(XDR *xd, StatePart part, int ecpt, int nf, int xdrType,
                            FILE *list, ElementType higherLevelElementType)
{
    bool_t        res = 0;

    unsigned int  elemSize = sizeOfXdrType(xdrType);
    char         *data;
    snew(data, nf*elemSize);
    res = xdr_vector(xd, data, nf, elemSize, xdrProc(xdrType));
    ...
    if (list != NULL)
    {
        ...
        {
            case xdr_datatype_int:
                pr_ivec(list, 0, entryName(part, ecpt), reinterpret_cast<int *>(data), nf, TRUE);
                break;
            case xdr_datatype_float:
#if !GMX_DOUBLE
                if (higherLevelElementType == ElementType::real3)
                {
                    pr_rvecs(list, 0, entryName(part, ecpt), reinterpret_cast<rvec *>(data), nf/3);
                }
                else
#endif
                {
                    ...
                    pr_fvec(list, 0, entryName(part, ecpt), reinterpret_cast<float *>(data), nf, TRUE);
                }
                break;
            case xdr_datatype_double:
#if GMX_DOUBLE
...
#endif
                {
                    ...
                    pr_dvec(list, 0, entryName(part, ecpt), reinterpret_cast<double *>(data), nf, TRUE);
                }
                break;
                       ...
                       FILE *list, ElementType higherLevelElementType)
{
    ...
    const int xdrTypeInTheCode = xdr_type<T>::value;
    ...
    if (list == NULL && (sflags & (1 << ecpt)))
    {
        ...
        if (!typesMatch)
        {
             ...
             * a regression test makes use of this.
             * But maybe we actually want to avoid automatic conversion.
             */
...
{
    return doVectorLow<T>(xd, part, ecpt, sflags, -1, NULL, NULL, vector, list, ElementType::real);
}
...
{
    return doVectorLow<T>(xd, part, ecpt, sflags, numElements, NULL, NULL, vector, list, ElementType::real);
}
...
{
    return doVectorLow<real>(xd, part, ecpt, sflags, n, NULL, v, NULL, list, ElementType::real);
}
...
static int do_cpte_n_reals(XDR *xd, StatePart part, int ecpt, int sflags,
                           int *n, real **v, FILE *list)
{
    return doVectorLow<real>(xd, part, ecpt, sflags, -1, n, v, NULL, list, ElementType::real);
}

static int do_cpte_real(XDR *xd, StatePart part, int ecpt, int sflags,
                        real *r, FILE *list)
{
    return doVectorLow<real>(xd, part, ecpt, sflags, 1, NULL, &r, NULL, list, ElementType::real);
}

static int do_cpte_ints(XDR *xd, StatePart part, int ecpt, int sflags,
                        int n, int **v, FILE *list)
{
    return doVectorLow<int>(xd, part, ecpt, sflags, n, NULL, v, NULL, list, ElementType::integer);
}
...
{
    return doVectorLow<double>(xd, part, ecpt, sflags, n, NULL, v, NULL, list, ElementType::real);
}
                          ...
                          matrix v, FILE *list)
{
    ...
    ret = doVectorLow<real>(xd, part, ecpt, sflags,
                            DIM*DIM, NULL, &vr, NULL, NULL, ElementType::matrix3x3);
                           ...
                           int n, real **v, FILE *list)
{
    ...
    for (i = 0; i < n; i++)
    {
        reti = doVectorLow<real>(xd, part, ecpt, sflags, n, NULL, &(v[i]), NULL, NULL, ElementType::matrix3x3);
        if (list && reti == 0)
        {
...
static void do_cpt_header(XDR *xd, gmx_bool bRead, int *file_version,
                          ...
                          int *flags_eks, int *flags_enh, int *flags_pullh, int *flags_dfh,
                          ...
                          FILE *list)
{
    ...
    *file_version = cpt_version;
    ...
    if (*file_version >= 17)
    {
        do_cpt_int_err(xd, "pull history flags", flags_pullh, list);
    }
}
                           ...
                           FILE *list)
{
    ...
    if (bRead)
    {
        ...
        {
            enerhist->dht                   = std::unique_ptr<delta_h_history_t>(new delta_h_history_t {});
    ...
    for (int i = 0; (i < eenhNR && ret == 0); i++)
    {
        if (fflags & (1<<i))
        {
            switch (i)
            {
                ...
                {
                    int numDeltaH = 0;
                    if (!bRead && enerhist->dht)
                    {
                        numDeltaH = enerhist->dht->dh.size();
                    }
                    do_cpt_int_err(xd, eenh_names[i], &numDeltaH, list);
                    if (bRead)
                    {
                        enerhist->dht->dh.resize(numDeltaH);
                    }
                    break;
                }
                case eenhENERGY_DELTA_H_LIST:
                    for (unsigned int j = 0; j < enerhist->dht->dh.size(); j++)
                    {
                        ret = doVector<real>(xd, part, i, fflags, &(enerhist->dht->dh[j]), list);
                    }
                    break;
...
static int do_cpt_pullhist(XDR *xd, gmx_bool bRead,
                           int fflags, energyhistory_t *pullhist,
                           FILE *list)
{
    int ret = 0;

    /* This is stored/read for backward compatibility */
    int  pullHistoryNumEntries = 0;
    if (bRead)
    {
        pullhist->nsteps     = 0;
        pullhist->nsum       = 0;
        pullhist->nsteps_sim = 0;
        pullhist->nsum_sim   = 0;
    }
    else if (pullhist != nullptr)
    {
        pullHistoryNumEntries = pullhist->ener_sum_sim.size();
    }
    else
    {
    ...
    const StatePart part = StatePart::pullHistory;
    for (int i = 0; (i < epullhNR && ret == 0); i++)
    {
        if (fflags & (1<<i))
        {
            switch (i)
            {
                case epullhPULL_N:     ret = do_cpte_int(xd, part, i, fflags, &pullHistoryNumEntries, list); break;
                case epullhPULL_AVER:  ret = doVector<double>(xd, part, i, fflags, pullHistoryNumEntries, &pullhist->ener_ave, list); break;
                case epullhPULL_SUM:   ret = doVector<double>(xd, part, i, fflags, pullHistoryNumEntries, &pullhist->ener_sum, list); break;
                case epullhPULL_NSUM:  do_cpt_step_err(xd, eenh_names[i], &pullhist->nsum, list); break;
                case epullhPULL_SUM_SIM: ret = doVector<double>(xd, part, i, fflags, pullHistoryNumEntries, &pullhist->ener_sum_sim, list); break;
                case epullhPULL_NSUM_SIM:   do_cpt_step_err(xd, eenh_names[i], &pullhist->nsum_sim, list); break;
                case epullhPULL_NSTEPS:     do_cpt_step_err(xd, eenh_names[i], &pullhist->nsteps, list); break;
                case epullhPULL_NSTEPS_SIM: do_cpt_step_err(xd, eenh_names[i], &pullhist->nsteps_sim, list); break;
                default:
                      ...
                      t_state *state, ObservablesHistory *observablesHistory)
{
    ...
    energyhistory_t *pullhist  = observablesHistory->pullHistory.get();
    int              flags_pullh = 0;
    if (pullhist != nullptr && (pullhist->nsum > 0 || pullhist->nsum_sim > 0))
    {
        flags_pullh |= (1<<eenhENERGY_N) | (1<<eenhENERGY_NSTEPS) | (1<<eenhENERGY_NSTEPS_SIM);
        if (pullhist->nsum > 0)
        {
            flags_pullh |= ((1<<eenhENERGY_AVER) | (1<<eenhENERGY_SUM) |
                            (1<<eenhENERGY_NSUM));
        }
        if (pullhist->nsum_sim > 0)
        {
            flags_pullh |= ((1<<eenhENERGY_SUM_SIM) | (1<<eenhENERGY_NSUM_SIM));
        }
    }
    ...
    do_cpt_header(gmx_fio_getxdr(fp), FALSE, &file_version,
                  ...
                  &flags_pullh, &flags_dfh, &nED, &eSwapCoords,
                  NULL);
    ...
    if ((do_cpt_state(gmx_fio_getxdr(fp), state->flags, state, NULL) < 0)        ||
        ...
        (do_cpt_pullhist(gmx_fio_getxdr(fp), FALSE, flags_pullh, pullhist, NULL) < 0)  ||
                            ...
                            gmx_bool reproducibilityRequested)
{
    ...
    ivec                 dd_nc_f;
    int                  natoms, ngtc, nnhpres, nhchainlength, nlambda, fflags, flags_eks, flags_enh, flags_pullh, flags_dfh;
    int                  nED, eSwapCoords;
    int                  d;
    int                  ret;
    gmx_file_position_t *outputfiles;
    ...
    do_cpt_header(gmx_fio_getxdr(fp), TRUE, &file_version,
                  ...
                  &fflags, &flags_eks, &flags_enh, &flags_pullh, &flags_dfh,
    ...
    *bReadEkin = ((flags_eks & (1<<eeksEKINH)) || (flags_eks & (1<<eeksEKINF)) || (flags_eks & (1<<eeksEKINO)) ||
    ...
    if (flags_pullh && observablesHistory->pullHistory == nullptr)
    {
        observablesHistory->pullHistory = std::unique_ptr<energyhistory_t>(new energyhistory_t {});
    }
    ret = do_cpt_pullhist(gmx_fio_getxdr(fp), TRUE,
                          flags_pullh, observablesHistory->pullHistory.get(), NULL);
    if (ret)
    {
        cp_error();
    }
    ...
    ret = do_cpt_swapstate(gmx_fio_getxdr(fp), TRUE, eSwapCoords, observablesHistory->swapHistory.get(), NULL);
    if (ret)
    {
        cp_error();
    }
                                   ...
                                   gmx_int64_t *step)
{
    ...
    ivec      dd_nc;
    int       nlambda;
    int       flags_eks, flags_enh, flags_pullh, flags_dfh;
    double    t;
    t_state   state;
    int       nED, eSwapCoords;
    t_fileio *fp;
    ...
    do_cpt_header(gmx_fio_getxdr(fp), TRUE, &file_version,
                  ...
                  &nlambda, &state.flags, &flags_eks, &flags_enh, &flags_pullh, &flags_dfh,
                  &nED, &eSwapCoords, NULL);
                                 ...
                                 int *nfiles, gmx_file_position_t **outputfiles)
{
    ...
    energyhistory_t pullhist = {};
    ret = do_cpt_pullhist(gmx_fio_getxdr(fp), TRUE,
                          flags_pullh, &pullhist, NULL);
    if (ret)
    {
        cp_error();
    }
    ...
    ret = do_cpt_swapstate(gmx_fio_getxdr(fp), TRUE, eSwapCoords, &swaphist, NULL);
    if (ret)
    {
        cp_error();
    }
...
void list_checkpoint(const char *fn, FILE *out)
{
    ...
    do_cpt_header(gmx_fio_getxdr(fp), TRUE, &file_version,
                  ...
                  &state.natoms, &state.ngtc, &state.nnhpres, &state.nhchainlength,
                  &nlambda, &state.flags,
                  &flags_eks, &flags_enh, &flags_pullh, &flags_dfh, &nED, &eSwapCoords,
                  out);
