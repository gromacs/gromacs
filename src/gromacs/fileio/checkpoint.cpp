                                   ...
                                   FILE *list, int eawhh)
{
    ...
    if (bRead)
    {
        initCorrelationGridHistory(corrgrid, corrgrid->ncorrmatrix, corrgrid->ncorr, corrgrid->nblockdata);
    }
    ...
    for (int j = 0; j < corrgrid->ncorrmatrix; j++)
    {
        ...
        for (int k = 0; k < corrgrid->ncorr; k++)
        {
            ...
            for (int l = 0; l < corrgrid->nblockdata; l++)
            {
                ...
                do_cpt_double_err(xd, eawhh_names[eawhh], &(blockdata->sum_w2_tot), list);
                do_cpt_double_err(xd, eawhh_names[eawhh], &(blockdata->sum_wbwx), list);
                do_cpt_double_err(xd, eawhh_names[eawhh], &(blockdata->sum_wbwy), list);
                do_cpt_double_err(xd, eawhh_names[eawhh], &(blockdata->sum_wb2), list);
