...
static inline xdrproc_t xdrProc(int xdr_type)
{
    switch (xdr_type)
    {
 ...
 *
 * When list!=NULL reads and lists the \o nf vector elements of type \p xdrType.
                       ...
                       FILE *list, int higherLevelElementType)
{
    ...
    if (list == NULL && (sflags & (1 << ecpt)))
    {
        ...
        if (xdrTypeInTheFile != xdrTypeInTheCode)
        {
        ...
        res = xdr_vector(xd, reinterpret_cast<char *>(vp), numElemInTheFile,
                         static_cast<unsigned int>(sizeof(T)),
                         xdrProc(xdrTypeInTheCode));
        if (res == 0)
        {
            return -1;
        }
