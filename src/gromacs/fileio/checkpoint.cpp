    ...
    if (!bNumberAndKeep && !ret)
    {
        if (gmx_fexist(fn))
        {
            ...
            if (!GMX_FAHCORE)
            {
                 ...
                 */
                gmx_file_copy(fn, previousFilename, FALSE);
            ...
            {
                gmx_file_rename(fn, previousFilename);
            }
        }
...
static void read_checkpoint(const std::string &fn, t_fileio *logfio,
                            const t_commrec *cr,
                            const ivec dd_nc,
                            int eIntegrator,
                            int *init_fep_state,
                            ...
                            gmx_bool reproducibilityRequested)
{
    ...
    {
        gmx_fatal(FARGS, "Cannot change integrator during a checkpoint restart. Perhaps you should make a new .tpr with grompp -f new.mdp -t %s", fn.c_str());
    }

    if (headerContents->flags_state != state->flags)
    {
        gmx_fatal(FARGS, "Cannot change a simulation algorithm during a checkpoint restart. Perhaps you should make a new .tpr with grompp -f new.mdp -t %s", fn.c_str());
    }
