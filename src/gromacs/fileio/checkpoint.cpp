                           ...
                           FILE *list)
{
    ...
    delta_h_history_t *deltaH = enerhist->deltaH.get();
            ...
            switch (i)
            {
                ...
                case eenhENERGY_DELTA_H_NN:
                {
                    ...
                    if (bRead)
                    {
                        if (deltaH == nullptr)
                        {
                            enerhist->deltaH.reset(new delta_h_history_t);
                            deltaH = enerhist->deltaH.get();
                        }
                ...
                case eenhENERGY_DELTA_H_LIST:
                    for (unsigned int j = 0; j < deltaH->dh.size(); j++)
                    {
                        ret = doVector<real>(xd, part, i, fflags, &(deltaH->dh[j]), list);
                    }
                    break;
                      ...
                      t_state *state, energyhistory_t *enerhist)
{
    ...
    if (enerhist->nsum > 0 || enerhist->nsum_sim > 0)
    {
        ...
        if (enerhist->deltaH.get() != nullptr)
        {
