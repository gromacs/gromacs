        ...
        if (fflags & (1<<i))
        {
            switch (i)
            {
                case eawhhIN_INITIAL:
                    do_cpt_int_err(xd, eawhh_names[i], &(awh_bias_history->in_initial), list); break;
                case eawhhHISTSIZE:
                    do_cpt_double_err(xd, eawhh_names[i], &(awh_bias_history->histsize), list); break;
                case eawhhNDIM:
                    do_cpt_int_err(xd, eawhh_names[i], &(awh_bias_history->ndim), list); break;
                case eawhhNPOINTS:
                    do_cpt_int_err(xd, eawhh_names[i], &(awh_bias_history->npoints), list); break;
                case eawhhCOORDPOINT:
                    ...
                    for (int j = 0; j < awh_bias_history->npoints; j++)
                    {
                        do_cpt_double_err(xd, eawhh_names[i], &(awh_bias_history->coordpoint[j].target), list);
                        do_cpt_double_err(xd, eawhh_names[i], &(awh_bias_history->coordpoint[j].free_energy), list);
                        do_cpt_double_err(xd, eawhh_names[i], &(awh_bias_history->coordpoint[j].bias), list);
                        do_cpt_double_err(xd, eawhh_names[i], &(awh_bias_history->coordpoint[j].weightsum_iteration), list);
                        do_cpt_double_err(xd, eawhh_names[i], &(awh_bias_history->coordpoint[j].weightsum_covering), list);
                        do_cpt_double_err(xd, eawhh_names[i], &(awh_bias_history->coordpoint[j].weightsum_tot), list);
                        do_cpt_double_err(xd, eawhh_names[i], &(awh_bias_history->coordpoint[j].weightsum_ref), list);
                        do_cpt_int_err(xd, eawhh_names[i], &(awh_bias_history->coordpoint[j].last_update_index), list);
                        do_cpt_double_err(xd, eawhh_names[i], &(awh_bias_history->coordpoint[j].log_pmfsum), list);
                        do_cpt_double_err(xd, eawhh_names[i], &(awh_bias_history->coordpoint[j].visits_iteration), list);
                        do_cpt_double_err(xd, eawhh_names[i], &(awh_bias_history->coordpoint[j].visits_tot), list);
                    }
                    break;
                case eawhhCOORD_REFVALUE_INDEX:
                    do_cpt_int_err(xd, eawhh_names[i], &(awh_bias_history->coord_refvalue_index), list); break;
...
static int do_cpt_awh_history(XDR *xd, gmx_bool bRead,
                              ...
                              FILE *list)
{
    ...
    if (fflags != 0)
    {
        ...
        {
            ret = do_cpt_awh_bias_history(xd, bRead, fflags, &awh_history->awh_bias_history[k], list);
            if (ret)
            {
                return ret;
            }
        }
        do_cpt_double_err(xd, "awh_convolved_bias_shift", &(awh_history->convolved_bias_shift), list);
    }
    return ret;
}
                      ...
                      gmx_int64_t step, double t, t_state *state)
{
    ...
    if ((do_cpt_state(gmx_fio_getxdr(fp), FALSE, state->flags, state, NULL) < 0)        ||
        ...
        (do_cpt_awh_history(gmx_fio_getxdr(fp), FALSE, flags_awhh, &state->awh_history, NULL) < 0)     ||
                            ...
                            gmx_bool bAppendOutputFiles, gmx_bool bForceAppend)
{
    ...
    *bReadEkin = ((flags_eks & (1<<eeksEKINH)) || (flags_eks & (1<<eeksEKINF)) || (flags_eks & (1<<eeksEKINO)) ||
    ...
    ret = do_cpt_awh_history(gmx_fio_getxdr(fp), TRUE,
