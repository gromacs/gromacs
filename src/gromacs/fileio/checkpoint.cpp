...
enum {
    epullhPULL_NCOORDS, epullhPULL_NSUM, epullhPULL_AVER, epullhPULL_SUM,
    epullhNR
};
...
{
    "pull_ncoords", "pull_nsum", "pull_aver", "pull_sum"
};
    ...
    for (int i = 0; (i < epullhNR && ret == 0); i++)
    {
        if (fflags & (1<<i))
        {
            switch (i)
            {
                ...
                case epullhPULL_AVER:     ret = doVector<double>(xd, part, i, fflags, pullHistoryNumCoords*dataGroupLen, &pullhist->ave, list); break;
                case epullhPULL_SUM:      ret = doVector<double>(xd, part, i, fflags, pullHistoryNumCoords*dataGroupLen, &pullhist->sum, list); break;
                default:
                      ...
                      t_state *state, ObservablesHistory *observablesHistory)
{
    ...
    if (pullxhist != nullptr && pullxhist->nsum > 0)
    {
        ...
        flags_pullxh |= ((1<<epullhPULL_NSUM) | (1<<epullhPULL_AVER) |
                         (1<<epullhPULL_SUM));
    }
    ...
    if (pullfhist != nullptr && pullfhist->nsum > 0)
    {
        ...
        flags_pullfh |= ((1<<epullhPULL_NSUM) | (1<<epullhPULL_AVER) |
                         (1<<epullhPULL_SUM));
    }
    ...
    if ((do_cpt_state(gmx_fio_getxdr(fp), state->flags, state, NULL) < 0)        ||
        ...
        (do_cpt_pullhist(gmx_fio_getxdr(fp), FALSE, flags_pullxh, pullxhist, 2, StatePart::pullXHistory, NULL) < 0)  ||
        (do_cpt_pullhist(gmx_fio_getxdr(fp), FALSE, flags_pullfh, pullfhist, 1, StatePart::pullFHistory, NULL) < 0)  ||
    ...
    *bReadEkin = ((flags_eks & (1<<eeksEKINH)) || (flags_eks & (1<<eeksEKINF)) || (flags_eks & (1<<eeksEKINO)) ||
    ...
    if (flags_pullxh)
    {
        ...
        ret = do_cpt_pullhist(gmx_fio_getxdr(fp), TRUE,
                              flags_pullxh, observablesHistory->pullXHistory.get(), 2, StatePart::pullXHistory, NULL);
        if (ret)
        {
            cp_error();
        }
    ...
    if (flags_pullfh)
    {
        ...
        ret = do_cpt_pullhist(gmx_fio_getxdr(fp), TRUE,
                              flags_pullfh, observablesHistory->pullFHistory.get(), 1, StatePart::pullFHistory, NULL);
        if (ret)
        {
            cp_error();
        }
                                 ...
                                 int *nfiles, gmx_file_position_t **outputfiles)
{
    ...
    ret = do_cpt_pullhist(gmx_fio_getxdr(fp), TRUE,
                          flags_pullxh, &pullxhist, 2, StatePart::pullXHistory, NULL);
    if (ret)
    {
        cp_error();
    }
    ...
    ret = do_cpt_pullhist(gmx_fio_getxdr(fp), TRUE,
                          flags_pullfh, &pullfhist, 1, StatePart::pullFHistory, NULL);
    if (ret)
    {
        cp_error();
    }
...
void list_checkpoint(const char *fn, FILE *out)
{
    ...
    if (ret == 0)
    {
        pullhistory_t pullxhist = {};
        ret = do_cpt_pullhist(gmx_fio_getxdr(fp), TRUE,
                              flags_pullxh, &pullxhist, 2, StatePart::pullXHistory, out);
    }
    ...
    if (ret == 0)
    {
        pullhistory_t pullfhist = {};
        ret = do_cpt_pullhist(gmx_fio_getxdr(fp), TRUE,
                              flags_pullfh, &pullfhist, 1, StatePart::pullFHistory, out);
    }
