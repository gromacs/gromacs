...
static void status_init(t_trxstatus *status)
{
    ...
    // TODO Fix whatever purpose this served
    //    init_tng_trajectory_output(status->tng);
    status->tng             = NULL;
}
...
{
    // TODO implement this properly once tng_trajectory_output_t is gone
    ...
    do
    {
        ...
        switch (ftp)
        {
            ...
            case efXTC:
                ...
                break;
            case efTNG:
                bRet      = read_next_tng_frame(status->tng, fr);
                fr->bPrec = FALSE;
                fr->bStep = bRet;
                fr->bTime = bRet;
                fr->bX    = bRet;
                     ...
                     const char *fn, t_trxframe *fr, int flags)
{
    ...
    switch (ftp)
    {
        ...
        case efXTC:
            ...
            break;
        case efTNG:
            if (!read_next_tng_frame((*status)->tng, fr))
            {
                fr->not_ok = DATA_NOT_OK;
                fr->natoms = 0;
                printincomp(*status, fr);
            ...
            {
                // TODO this assumptions are not true - bX and bBox in particular
                fr->bPrec = FALSE;
                fr->bStep = TRUE;
                fr->bTime = TRUE;
                fr->bX    = TRUE;
                 ...
                 real *t, rvec **x, matrix box)
{
    ...
    if (efTNG == fn2ftp(fn))
    {
        gmx_file("This tool does not support reading trajectory files in TNG format");
    }
                     ...
                     rvec x[], matrix box)
{
    ...
    if (status->tng)
    {
        gmx_file("This tool does not support trajectory files in TNG format");
    }
