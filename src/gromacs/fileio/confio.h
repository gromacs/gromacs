 ...
 * \param[out]    havePeriodicMolecules Bool that contains information concerning periodicity of system
 ...
 * \param[out]    ePBC          Enum reporting the type of PBC
 * \param[out]    havePeriodicMolecules Bool reporting if periodic molecules are present in the system.
 * \param[in,out] x             Coordinates will be stored when *x!=NULL
 * \param[in,out] v             Velocities will be stored when *v!=NULL
 * \param[out]    box           Box dimensions
...
gmx_bool read_tps_conf(const char *infile, struct t_topology *top,
                       int *ePBC, bool *havePeriodicMolecules, rvec **x, rvec **v, matrix box,
