...
/*! \file
 * \brief
 * Convert file data to GridDataFloat3D and back.
 *
...
XplorData xplorFromGridData(const GridDataFloat3D &map, std::array<int, DIM> unitCellExtend = {});
...
class MapConverter
{
    public:
        ...
        MapConverter(const GridDataFloat3D &grid);
        void to(std::string filename);
        const GridDataFloat3D &map();
    private:
