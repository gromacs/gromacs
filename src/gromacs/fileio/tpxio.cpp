...
enum tpxv {
    ...
    tpxv_PullLastStepCOMAsReference,                         /**< Enabled using the COM of the pull group of the last frame as reference for PBC */
...
static void do_pull_group(t_fileio *fio, t_pull_group *pgrp, gmx_bool bRead, int file_version)
{
    ...
    gmx_fio_ndo_real(fio, pgrp->weight, pgrp->nweight);
    gmx_fio_do_int(fio, pgrp->pbcatom);
    if (file_version >= tpxv_PullLastStepCOMAsReference)
    {
        gmx_fio_do_gmx_bool(fio, pgrp->setPbcReferenceToLastStepCOM);
    }
}
