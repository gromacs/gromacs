                        ...
                        int file_version, real *fudgeQQ)
{
    ...
    if (file_version >= tpxv_GenericParamsForElectricField)
    {
        ...
        {
            GMX_RELEASE_ASSERT(ir->moduleOptionParameters != nullptr,
                               "Module parameters should be present when writing inputrec");
            gmx::serializeKeyValueTree(*ir->moduleOptionParameters, &serializer);
        }
    }
    if (bRead)
    {
        ir->moduleOptionParameters = new gmx::KeyValueTreeObject(paramsBuilder.build());
    }
}
