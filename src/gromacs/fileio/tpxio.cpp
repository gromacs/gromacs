                          ...
                          int ePullOld, int eGeomOld, ivec dimOld)
{
    if (file_version >= tpxv_PullCoordNGroup)
    {
        ...
        {
            /* More groups in file than supported, this must a a new geometry
             * that is now supported by our current code. Since we will not
             * use the contents or the group (checks in the pull and WHAM
             * code ensure this), we can ignore the groups and set ngroup=0.
             */
            int *dum;
