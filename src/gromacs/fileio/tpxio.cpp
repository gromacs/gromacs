...
static int do_tpx(t_fileio *fio, gmx_bool bRead,
                  t_inputrec *ir, t_state_global *state, rvec *x, rvec *v,
                  gmx_mtop_t *mtop)
{
    t_tpxheader     tpx;
    gmx_bool        TopOnlyOK;
    ...
    {
        state->flags = 0;
        init_gtc_state_global(state, tpx.ngtc, 0, 0);
        if (x == nullptr)
        {
            ...
            state_change_natoms_global(state, tpx.natoms);
        }
    }
    ...
    if (bRead)
    {
        if (tpx.bIr && ir)
        {
            if (state->ngtc == 0)
            {
                ...
                init_gtc_state_global(state, ir->opts.ngtc, 0, ir->opts.nhchainlength);
            }
            if (tpx.bTop && mtop)
            {
                if (fileVersion < 57)
                     ...
                     const t_inputrec *ir, const t_state_global *state, const gmx_mtop_t *mtop)
{
    ...
    do_tpx(fio, FALSE,
           ...
           const_cast<t_state_global *>(state), nullptr, nullptr,
           const_cast<gmx_mtop_t *>(mtop));
    close_tpx(fio);
}

void read_tpx_state(const char *fn,
                    t_inputrec *ir, t_state_global *state, gmx_mtop_t *mtop)
{
    t_fileio *fio;
