...
TYPED_TEST(H5mdWriteNumericPrimitiveTest, WriteValueTo1dSetWritesToCorrectIndex)
{
    ...
    const H5mdDataSetBase<TypeParam> dataSet =
            H5mdFrameDataSetBuilder<TypeParam>(this->fileid(), "testDataSet")
                    .withNumFrames(valuesToWrite.size()) // enable writing the above values into frames
                    .build();
    ...
    H5Dread(dataSet.id(), dataSet.nativeDataType(), H5S_ALL, H5S_ALL, H5P_DEFAULT, readBuffer.data());
...
TYPED_TEST(H5mdWriteNumericPrimitiveTest, WriteValueForReadOnlyFileThrows)
{
...

    {
        ...
        const H5mdDataSetBase<TypeParam> dataSet =
                H5mdFrameDataSetBuilder<TypeParam>(file.fileid(), dataSetName)
...
TYPED_TEST(H5mdWriteBasicVectorListTest, CorrectRowOrderIsUsedForWritingArrayData)
{
    ...
    H5Dread(dataSet.id(), dataSet.nativeDataType(), H5S_ALL, H5S_ALL, H5P_DEFAULT, readBuffer1d.data());
...
TYPED_TEST(H5mdWriteBasicVectorListTest, WriteAfterResizingDataSetWorks)
{
    ...
    setNumFrames(dataSet, 2); // enable writing into index = 1
