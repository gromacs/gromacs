...
class OutputManager : public ICoordinateOutput
{
    public:
         ...
         */
        explicit OutputManager(std::string name, const Selection *sel, const gmx_mtop_t *mtop) : name_(name), trr_(nullptr), filetype_(efNR), sel_(sel)
        {
            setMtop(mtop);
            initOutput();
            flagModules_.clear();
        ...
        OutputManager(OutputManager &&old) : name_(std::move(old.name_)), trr_(std::move(old.trr_)),
                                             filetype_(std::move(old.filetype_)), sel_(std::move(old.sel_)), flagModules_(std::move(old.flagModules_))
        {
        }
