...
class OutputManager : public ICoordinateOutput
{
    public:
        ...
        explicit OutputManager(std::string name, const Selection *sel, const gmx_mtop_t *mtop) :
            ...
            name_(name), trr_(nullptr), filetype_(efNR), sel_(sel)
        {
            setMtop(mtop);
            setFiletype();
            flagModules_.clear();
         ...
         */
        OutputManager(OutputManager &&old) : ICoordinateOutput(old.moduleFlags_),
        ...
        /*! \brief
         * Local implementation of the processFrame method from the interface.
         *
         * \param[in] framenumber Number of current frame being processed.
         * \param[in] input Pointer to currently processed frame.
         */
         ...
         */
        t_atoms *getAtoms(t_trxframe *input) { return input->bAtoms ? input->atoms : nullptr; };
