                                       ...
                                       t_idef             *idef)
{
     ...
     *
     * NOTE: Inside this loop over all threads, we loop over all bonded
     * interactions. This code is running single threaded (difficult
     * to parallelize over threads). So the relative cost of this function
     * scales as nthread^2, which is very bad.
    ...
    for (t = 1; t <= nthread; t++)
    {
        ...
        while (nat_sum < nat_thread)
        {
             ...
             * If they are not assigned in increasing order, the result
             * and the balancing are good, but the memory access and
             * reduction cost will be higher.
             */
            int f_min;

            /* Find out which of the ntype has the lowest atom index */
            f_min = 0;
            for (f = 1; f < ntype; f++)
            {
            ...
            else
            {
                 ...
                 * chosen in the next iterations of the for loop above.
                 */
