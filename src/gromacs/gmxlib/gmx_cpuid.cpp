...
#if (_MSC_VER >= 1600)
    /* MSVC 10.0 or later. While MSVC 9.0-sp1 supports __cpuidex() in C, things go bad
     * with C++. Rather than using managed code in .NET, we accept to live without
     * topology information in earlier versions - we'll anyway move to hwloc instead soon.
     */
    __cpuidex(CPUInfo, static_cast<int>(level), static_cast<int>(ecxval));
    rc = 0;
#else
...
{
    unsigned int              fn, max_stdfn, max_extfn;
    ...
    if ((ret = execute_x86cpuid(0x0, 0, &eax, &ebx, &ecx, &edx)) != 0)
    {
        return ret;
    }
    max_stdfn = eax;
    if ((ret = execute_x86cpuid(0x80000000, 0, &eax, &ebx, &ecx, &edx)) != 0)
    {
        return ret;
    }
    max_extfn = eax;
    ...
    if (max_extfn >= 0x80000005)
    {
        ...
        {
            if ((ret = execute_x86cpuid(fn, 0, &eax, &ebx, &ecx, &edx)) != 0)
            {
                return ret;
            }
            std::memcpy(p, &eax, 4);
            std::memcpy(p+4, &ebx, 4);
            std::memcpy(p+8, &ecx, 4);
            std::memcpy(p+12, &edx, 4);
            p += 16;
        }
        *p = '\0';
        ...
        std::strncpy(cpuid->brand, p, GMX_CPUID_STRLEN);
    }
    else
    {
        std::strncpy(cpuid->brand, "Unknown CPU brand", GMX_CPUID_STRLEN);
    }

    /* Find basic CPU properties */
    if (max_stdfn >= 1)
    {
        if ((ret = execute_x86cpuid(0x1, 0, &eax, &ebx, &ecx, &edx)) != 0)
        {
            return ret;
        }
    ...
    {
        if ((ret = execute_x86cpuid(0x80000001, 0, &eax, &ebx, &ecx, &edx)) != 0)
        {
            return ret;
        }
    ...
    {
        if ((ret = execute_x86cpuid(0x80000007, 0, &eax, &ebx, &ecx, &edx)) != 0)
        {
            return ret;
        }
...
cpuid_renumber_elements(int *data, int n)
{
    ...
    unique = reinterpret_cast<int *>(std::malloc(sizeof(int)*n));
 ...
 */
static int
cpuid_x86_decode_apic_id(gmx_cpuid_t cpuid, int *apic_id, unsigned int core_bits, unsigned int hwthread_bits)
{
    int i, idx;
    int hwthread_mask, core_mask_after_shift;

    cpuid->hwthread_id     = reinterpret_cast<int *>(std::malloc(sizeof(int)*cpuid->nproc));
    cpuid->core_id         = reinterpret_cast<int *>(std::malloc(sizeof(int)*cpuid->nproc));
    cpuid->package_id      = reinterpret_cast<int *>(std::malloc(sizeof(int)*cpuid->nproc));
    cpuid->locality_order  = reinterpret_cast<int *>(std::malloc(sizeof(int)*cpuid->nproc));
...
cpuid_check_amd_x86(gmx_cpuid_t                cpuid)
{
    ...
    if (max_extfn >= 0x80000008)
    {
        ...
        int          * apic_id  = reinterpret_cast<int *>(std::malloc(sizeof(int)*cpuid->nproc));
        sched_getaffinity(0, sizeof(cpu_set_t), &save_cpuset);
        /* Get APIC id from each core */
        CPU_ZERO(&cpuset);
        for (int i = 0; i < cpuid->nproc && ret == 0; i++)
        {
            CPU_SET(i, &cpuset);
            sched_setaffinity(0, sizeof(cpu_set_t), &cpuset);
            ret        = execute_x86cpuid(0x1, 0, &eax, &ebx, &ecx, &edx);
            apic_id[i] = ebx >> 24;
            CPU_CLR(static_cast<unsigned int>(i), &cpuset);
        }
...
#define CPUID_HAVE_APIC
        ...
        unsigned int hwthread_bits = 0;
        /* Get number of core bits in apic ID - try modern extended method first */
        if (ret == 0)
        {
            if ((ret = execute_x86cpuid(0x80000008, 0, &eax, &ebx, &ecx, &edx)) == 0)
            {
                unsigned int core_bits = (ecx >> 12) & 0xf;
                if (core_bits == 0)
                {
                    /* Legacy method for old single/dual core AMD CPUs */
                    unsigned int i = ecx & 0xF;
                    for (core_bits = 0; (i>>core_bits) > 0; core_bits++)
                    {
                        ;
                    }
                }
                ret = cpuid_x86_decode_apic_id(cpuid, apic_id, core_bits, hwthread_bits);
            }
        }
        std::free(apic_id);
...
cpuid_check_intel_x86(gmx_cpuid_t                cpuid)
{
    ...
    if (max_stdfn >= 7)
    {
        if (( ret = execute_x86cpuid(0x7, 0, &eax, &ebx, &ecx, &edx)) != 0)
        {
            return ret;
        }
    ...
    if (cpuid->feature[GMX_CPUID_FEATURE_X86_HTT] && max_stdfn >= 4)
    {
        ...
        if ((ret = execute_x86cpuid(0x4, 0, &eax, &ebx, &ecx, &edx)) != 0)
        {
            return ret;
        }
    ...
    if (max_stdfn >= 0xB)
    {
        ...
        for (int i = 0; i < cpuid->nproc && ret == 0; i++)
        {
            CPU_SET(i, &cpuset);
            sched_setaffinity(0, sizeof(cpu_set_t), &cpuset);
            ret        = execute_x86cpuid(0xB, 0, &eax, &ebx, &ecx, &edx);
            apic_id[i] = edx;
            CPU_CLR(static_cast<unsigned int>(i), &cpuset);
        }
        /* Reset affinity to the value it had when calling this routine */
...
#define CPUID_HAVE_APIC
        ...
        {
            ret = execute_x86cpuid(0xB, 0, &eax, &ebx, &ecx, &edx);
            unsigned int hwthread_bits    = eax & 0x1F;
            ret = ret || execute_x86cpuid(0xB, 1, &eax, &ebx, &ecx, &edx);
            unsigned int core_bits        = (eax & 0x1F) - hwthread_bits;
            ret = ret || cpuid_x86_decode_apic_id(cpuid, apic_id, core_bits, hwthread_bits);
        }
        std::free(apic_id);
        cpuid->have_cpu_topology = (ret == 0);
#endif
...
{
    char *p;
    std::strncpy(s, in, maxlength);
    p = std::strchr(s, ':');
    if (p != NULL)
    {
        *p = '\0';
        while (std::isspace(*(--p)) && (p >= s))
        {
            *p = '\0';
        }
    }
...
cpuid_check_arm(gmx_cpuid_t                cpuid)
{
    ...
    if ( (fp = fopen("/proc/cpuinfo", "r")) != NULL)
    {
        while ( (fgets(buffer, sizeof(buffer), fp) != NULL))
        {
            ...
            if (!std::strcmp(buffer2, "Processor"))
            {
                std::strncpy(cpuid->brand, buffer3, GMX_CPUID_STRLEN);
            }
            else if (!std::strcmp(buffer2, "CPU architecture"))
            {
                cpuid->family = std::strtol(buffer3, NULL, 10);
                if (!std::strcmp(buffer3, "AArch64"))
                {
                    cpuid->family = 8;
                }
            }
            else if (!std::strcmp(buffer2, "CPU part"))
            {
                cpuid->model = std::strtol(buffer3, NULL, 16);
            }
            else if (!std::strcmp(buffer2, "CPU revision"))
            {
                cpuid->stepping = std::strtol(buffer3, NULL, 10);
            }
            else if (!std::strcmp(buffer2, "Features") && std::strstr(buffer3, "neon"))
            {
                cpuid->feature[GMX_CPUID_FEATURE_ARM_NEON] = 1;
            }
            else if (!std::strcmp(buffer2, "Features") && std::strstr(buffer3, "asimd"))
            {
...
cpuid_check_ibm(gmx_cpuid_t                cpuid)
{
    ...
    if ( (fp = fopen("/proc/cpuinfo", "r")) != NULL)
    {
        while ( (fgets(buffer, sizeof(buffer), fp) != NULL))
        {
            ...
            if (!std::strcmp(before_colon, "cpu") || !std::strcmp(before_colon, "Processor"))
            {
                std::strncpy(cpuid->brand, after_colon, GMX_CPUID_STRLEN);
            }
            if (!std::strcmp(before_colon, "model name") ||
                !std::strcmp(before_colon, "model") ||
                !std::strcmp(before_colon, "Processor") ||
                !std::strcmp(before_colon, "cpu"))
            {
                if (std::strstr(after_colon, "altivec"))
                {
                    cpuid->feature[GMX_CPUID_FEATURE_IBM_VMX] = 1;

                    if (!std::strstr(after_colon, "POWER6") && !std::strstr(after_colon, "Power6") &&
                        !std::strstr(after_colon, "power6"))
                    {
...
cpuid_check_vendor(void)
{
    ...
    std::memcpy(vendorstring, &ebx, 4);
    std::memcpy(vendorstring+4, &edx, 4);
    std::memcpy(vendorstring+8, &ecx, 4);

    vendorstring[12] = '\0';

    for (int i = GMX_CPUID_VENDOR_UNKNOWN; i < GMX_CPUID_NVENDORS; i++)
    {
        if (!std::strncmp(vendorstring, gmx_cpuid_vendor_string[i], 12))
        {
            vendor = static_cast<enum gmx_cpuid_vendor>(i);
        }
    }
    ...
    if ( (fp = fopen("/proc/cpuinfo", "r")) != NULL)
    {
        while ( (vendor == GMX_CPUID_VENDOR_UNKNOWN) && (fgets(buffer, sizeof(buffer), fp) != NULL))
        {
             ...
             */
            if (!std::strcmp(before_colon, "vendor_id")
                || !std::strcmp(before_colon, "vendor")
                || !std::strcmp(before_colon, "manufacture")
                || !std::strcmp(before_colon, "model")
                || !std::strcmp(before_colon, "Processor")
                || !std::strcmp(before_colon, "cpu"))
            {
                ...
                for (int i = GMX_CPUID_VENDOR_UNKNOWN; i < GMX_CPUID_NVENDORS; i++)
                {
                    ...
                    if (std::strstr(after_colon, gmx_cpuid_vendor_string[i])
                        || std::strstr(after_colon, gmx_cpuid_vendor_string_alternative[i]))
                    {
                        vendor = i;
                    }
                }
                ...
                if (vendor == GMX_CPUID_VENDOR_UNKNOWN &&
                    ((std::strstr(after_colon, "POWER") || std::strstr(after_colon, "Power") ||
                      std::strstr(after_colon, "power"))))
                {
...
gmx_cpuid_init               (gmx_cpuid_t *              pcpuid)
{
    ...
    cpuid = reinterpret_cast<gmx_cpuid_t>(std::malloc(sizeof(*cpuid)));
        ...
        default:
            ...
            if ( (fp = fopen("/proc/cpuinfo", "r")) != NULL)
            {
                ...
                while ( (found_brand == 0) && (fgets(buffer, sizeof(buffer), fp) != NULL))
                {
                    ...
                    if (!std::strcmp(buffer2, "model name") || !std::strcmp(buffer2, "cpu"))
                    {
                              ...
                              int                      n)
{
    ...
    {
        if (gmx_cpuid_feature(cpuid, static_cast<enum gmx_cpuid_feature>(feature)) == 1)
        {
#ifdef _MSC_VER
...
gmx_cpuid_simd_suggest  (gmx_cpuid_t                 cpuid)
{
    ...
    {
        if (std::strstr(gmx_cpuid_brand(cpuid), "SPARC64"))
        {
...
main(int argc, char **argv)
{
    ...
    if (!std::strncmp(argv[1], "-vendor", 3))
    {
        printf("%s\n", gmx_cpuid_vendor_string[cpuid->vendor]);
    }
    else if (!std::strncmp(argv[1], "-brand", 3))
    {
        printf("%s\n", cpuid->brand);
    }
    else if (!std::strncmp(argv[1], "-family", 3))
    {
        printf("%d\n", cpuid->family);
    }
    else if (!std::strncmp(argv[1], "-model", 3))
    {
        printf("%d\n", cpuid->model);
    }
    else if (!std::strncmp(argv[1], "-stepping", 3))
    {
        printf("%d\n", cpuid->stepping);
    }
    else if (!std::strncmp(argv[1], "-features", 3))
    {
        cnt = 0;
        for (i = 0; i < GMX_CPUID_NFEATURES; i++)
        {
    ...
    else if (!std::strncmp(argv[1], "-simd", 3))
    {
