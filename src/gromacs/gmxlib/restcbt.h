...
* where all the derivatives of the bending angle with respect to Cartesian coordinates are calculated as in Allen & Tildesley (pg 330-332)
* 
*  \param[in]  type           type of force parameters
*  \param[in]  forceparams    array of parameters for force computations
*  \param[in]  delta_ante     position difference between the first two particles
...
*  \param[out] ratio_ante     ratio between scalar products of delta_ante with delta_post 
                              and delta_ante with delta_ante
*  \param[out] ratio_post    ratio between scalar products of delta_ante with delta_post 
...
/*! \brief Compute factors for restricted dihedral potential.
*
...
* are calculated as in Allen & Tildesley (pg 330-332). Factors factor_phi_*  are coming from the 
* derivatives of the torsion angle (phi) with respect to the beads ai, aj, ak, al, (four) coordinates 
* and they are multiplied in the force computations with the differences of the particles positions 
* stored in parameters delta_ante, delta_crnt, delta_post.
*
*  \param[in]  type                             type of force parameters
*  \param[in]  forceparams                      array of parameters for force computations
*  \param[in]  delta_ante                       position difference between the first two particles
*  \param[in]  delta_crnt                       position difference between the middle pair of particles
*  \param[in]  delta_post                       position difference between the last two particles
...
* but also \f$\theta_{i-1}\f$ (denoted as theta_ante bellow) and \f$\theta_{i}\f$ (denoted as theta_post bellow) 
...
* in Allen & Tildesley (pg 330-332). Factors f_phi_* come from  the derivatives of the torsion angle 
* with respect to the beads ai, aj, ak, al (four) coordinates; f_theta_ante_* come from the derivatives of * the bending angle theta_ante (theta_{i-1} in formula above) with respect to the beads ai, aj, ak (three
