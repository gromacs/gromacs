                        ...
                        int *info)
{
	...
	*info = -((*n << 1) + 199);
    ...
    if (fabs(e)<GMX_FLOAT_MIN) {
	i__1 = *n;
	for (k = 2; k <= i__1; ++k) {
	    z__[k] = z__[(k << 1) - 1];
	}
	F77_FUNC(slasrt,SLASRT)("D", n, &z__[1], &iinfo);
	z__[(*n << 1) - 1] = d__;
	return;
    }
    ...
    if(fabs(negzro)>GMX_FLOAT_MIN)
      ieee = 0;
    neginf = one/negzro;
    if(neginf>=0)
      ieee = 0;
    newzro = negzro + zero;
    if(fabs(newzro-zero)>GMX_FLOAT_MIN)
      ieee = 0;
    posinf = one /newzro;
    if(posinf<=one)
      ieee = 0;
...
L100:
	...
	for (iwhilb = 1; iwhilb <= i__2; ++iwhilb) {
	    ...
	    F77_FUNC(slasq3,SLASQ3)(&i0, &n0, &z__[1], &pp, &dmin__, &sigma, &desig, &qmax, &
