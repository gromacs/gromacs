 ...
 *
 * This function is a duplicate of md_print_info() from md_logging.c
 * because we don't have commrec in multiple places where we would like to output
 * messages, so here we make the assumption that if fplog!=NULL we're printing
 ...
 *
 * This function is a duplicate of md_print_warn() from md_logging.c
 * because we don't have commrec in multiple places where we would like to output
 * messages, so here we make the assumption that if fplog!=NULL we're printing
 * on the master node's stderr.
 *
