 ...
 * the OpenCL source files. defaultJitDataPath knows how to handle both a launch
 * from the source tree and a launch from the install folder.
 * The user can override this behavior by defining GMX_OCL_FILE_PATH
 * environment variable.
 *
 * \return String with the full path to the main folder storing OpenCL kernels, or NULL if unknown
 */
static const char*
get_ocl_root_path()
{
    ...
    // Handle that defaultJitDataPath can return empty string
    if (ocl_root_path && ocl_root_path[0] == 0)
    {
        ocl_root_path = NULL;
    }
