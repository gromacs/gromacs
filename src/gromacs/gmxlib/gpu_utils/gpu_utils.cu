 ...
 */
gmx_bool init_gpu_application_clocks(FILE *fplog,int gpuid, const gmx_gpu_info_t *gpu_info)
{
 ...
 */
gmx_bool init_gpu(FILE *fplog,int mygpu, char *result_str,
...
#ifdef HAVE_NVML
    gpuid = gpu_opt->cuda_dev_use[mygpu];
    if ( gpu_info->cuda_dev[gpuid].nvml_is_restricted == NVML_FEATURE_DISABLED && 
