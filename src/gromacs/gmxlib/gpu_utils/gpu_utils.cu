 ...
 *
 * The variable \gpuid is the index of the GPU in the
 * gpu_info.cuda_dev array to handle the application clocks for.
 *
...
{
#ifndef HAVE_NVML
    md_print_warn( fplog, "Application clocks handling not possible for %s due to missing NVML support.\n", gpu_info->cuda_dev[gpuid].prop.name );
    return true;
#else /* HAVE_NVML defined */
    nvmlReturn_t nvml_stat = NVML_SUCCESS;
    char        *env;
    gmx_bool     use_applicaiton_clocks = true;
    ...
    {
        use_applicaiton_clocks = false;
    }
    if (use_applicaiton_clocks &&
        gpu_info->cuda_dev[gpuid].prop.major >= 3 &&
        gpu_info->cuda_dev[gpuid].prop.minor >= 5)
    {
        ...
        if (nvml_stat == NVML_SUCCESS)
        {
            ...
            gmx_bool     nvml_device_found = false;
            ...
            for (unsigned int nvml_device_idx = 0; nvml_stat == NVML_SUCCESS && nvml_device_idx < nvml_device_count; ++nvml_device_idx)
            {
                ...
                {
                    nvml_device_found = true;
                    break;
                }
            }
            if (nvml_device_found && nvml_stat == NVML_SUCCESS)
            {
                gpu_info->cuda_dev[gpuid].nvml_initialized       = nvml_device_found;
                ...
                {
                    //TODO: How to get access to t_commrec *cr and FILE *fplog here? Which md_print_info to use (see static function in nbnxn_cuda_data_mgmt.cu)?
                    //I think it would be good to have these message in the log file, because they
                    md_print_info( fplog, "Setting application clocks for %s to (%d,%d)\n", gpu_info->cuda_dev[gpuid].prop.name, max_mem_clock, max_sm_clock);
                ...
                {
                    //TODO: How to get access to t_commrec *cr and FILE *fplog here? Which md_print_warn to use (see static function in nbnxn_cuda_data_mgmt.cu)?
                    //I think it would be good to have these message in the log file, because they
                    md_print_warn( fplog, "Insufficient permissions to set application clocks for %s. Current values are (%d,%d). Max values are (%d,%d)\nUse nvidia-smi -acp to change application clock permissions.\n", gpu_info->cuda_dev[gpuid].prop.name, app_mem_clock, app_sm_clock, max_mem_clock, max_sm_clock);
                }
                else if (nvml_stat == NVML_SUCCESS && app_sm_clock == max_sm_clock)
                {
                    md_print_info( fplog, "Application clocks for %s are (%d,%d)\n", gpu_info->cuda_dev[gpuid].prop.name, app_mem_clock, app_sm_clock);
                }
                else
                {
                    //TODO: How to get access to t_commrec *cr and FILE *fplog here?
                    //I think it would be good to have these message in the log file, because they
                    md_print_warn( fplog,  "Application clocks handling failed because of NVML errors.\n");
                }
            }
        }
    }
        ...
        )
{
    ...
    if (gpuid != -1 &&
        gpu_info->cuda_dev[gpuid].nvml_is_restricted == NVML_FEATURE_DISABLED &&
        gpu_info->cuda_dev[gpuid].nvml_ap_clocks_changed)
    {
        nvml_stat = nvmlDeviceResetApplicationsClocks ( gpu_info->cuda_dev[gpuid].nvml_device_id );
...
#else
    // Avoid unused variable warnings. gmx_unused seems to not work for some versions of gcc.
    GMX_UNUSED_VALUE(mygpu);
    GMX_UNUSED_VALUE(gpu_info);
