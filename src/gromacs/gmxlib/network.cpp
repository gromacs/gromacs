...
void gmx_init_intranode_counters(t_commrec *cr)
{
    ...
    hash_pp_s[rank_world] = isRunningTask(cr, DUTY_PP) ? myhash : -1;
    ...
    for (i = 0; i < nrank_world; i++)
    {
        ...
        {
            nrank_pp_intranode++;
            if (isRunningTask(cr, DUTY_PP) && i < rank_world)
            {
                rank_pp_intranode++;
            }
        }
    ...
    {
        char sbuf[STRLEN];
        if (isRunningTask(cr, DUTY_PP) && isRunningTask(cr, DUTY_PME))
        {
            sprintf(sbuf, "PP+PME");
        }
        else
        {
            sprintf(sbuf, "%s", isRunningTask(cr, DUTY_PP) ? "PP" : "PME");
        }
