                              ...
                              gmx_int64_t key1, gmx_int64_t key2, real* rnd)
{
    ...
    rnd[1] = gaussian_table[rand.v[0] & ((1<<GAUSS_TABLE) - 1)];
                              ...
                              gmx_int64_t key1, gmx_int64_t key2, real* rnd)
{
    ...
    rnd[0] = gaussian_table[rand.v[0] >> (64 - GAUSS_TABLE)];
    rnd[1] = gaussian_table[rand.v[0] & ((1<<GAUSS_TABLE) - 1)];
    rnd[2] = gaussian_table[(rand.v[0] >> 32) & ((1<<GAUSS_TABLE) - 1)];
    rnd[3] = gaussian_table[rand.v[1] >> (64 - GAUSS_TABLE)];
    rnd[4] = gaussian_table[rand.v[1] & ((1<<GAUSS_TABLE) - 1)];
    rnd[5] = gaussian_table[(rand.v[1] >> 32) & ((1<<GAUSS_TABLE) - 1)];
}
