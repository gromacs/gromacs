...
/*
 * Gromacs nonbonded kernel:   nb_kernel_ElecEwG_VdwBham_GeomW4W4_VF_c
 * Electrostatics interaction: EwaldG
 * VdW interaction:            Buckingham
 * Geometry:                   Water4-Water4
 * Calculate force/pot:        PotentialAndForce
 */
void
nb_kernel_ElecEwG_VdwBham_GeomW4W4_VF_c
                     ...
                     t_nrnb                      * gmx_restrict        nrnb)
{
    ...
    int              vdwioffset1;
    real             ix1,iy1,iz1,fix1,fiy1,fiz1,iq1,isai1,alphai1;
    int              vdwioffset2;
    real             ix2,iy2,iz2,fix2,fiy2,fiz2,iq2,isai2,alphai2;
    int              vdwioffset3;
    real             ix3,iy3,iz3,fix3,fiy3,fiz3,iq3,isai3,alphai3;
    int              vdwjidx0;
    real             jx0,jy0,jz0,fjx0,fjy0,fjz0,jq0,isaj0,alphaj0;
    int              vdwjidx1;
    real             jx1,jy1,jz1,fjx1,fjy1,fjz1,jq1,isaj1,alphaj1;
    int              vdwjidx2;
    real             jx2,jy2,jz2,fjx2,fjy2,fjz2,jq2,isaj2,alphaj2;
    int              vdwjidx3;
    real             jx3,jy3,jz3,fjx3,fjy3,fjz3,jq3,isaj3,alphaj3;
    ...
    real             *charge;
    real             *gaussian;
    real             ewc,alphai,alphaij,felect,scale;
    int              nvdwtype;
    ...
    alphai1           = gaussian[inr+1];
    iq2              = facel*charge[inr+2];
    alphai2           = gaussian[inr+2];
    iq3              = facel*charge[inr+3];
    alphai3           = gaussian[inr+3];
    vdwioffset0      = 3*nvdwtype*vdwtype[inr+0];

    jq1              = charge[inr+1];
    alphaj1          = gaussian[inr+1];
    jq2              = charge[inr+2];
    alphaj2          = gaussian[inr+2];
    jq3              = charge[inr+3];
    alphaj3          = gaussian[inr+3];
    ...
    for(iidx=0; iidx<nri; iidx++)
    {
        ...
        for(jidx=j_index_start; jidx<j_index_end; jidx++)
        {
            ...
            /* EWALD ELECTROSTATICS WITH GAUSSIAN CHARGE DISTRIBUTIONS */

            alphaij          = alphai1 * alphaj1 * gmx_invsqrt(alphai1*alphai1 + alphaj1*alphaj1);
            ewrt             = r11*ewtabscale;
            ewitab           = ewrt;
            eweps            = ewrt-ewitab;
            ...
            scale            = alphaij/ewc;
            if (  r11 * scale < rcutoff ) {
                ewrt             = scale * r11*ewtabscale;
                ewitab           = ewrt;
                eweps            = ewrt-ewitab;
                ewitab           = 4*ewitab;
                felect           = (ewtab[ewitab]+eweps*ewtab[ewitab+1])*scale ;
                velec            -=  qq11*(rinv11 - (scale*ewtab[ewitab+2]-ewtabhalfspace*eweps*(scale*ewtab[ewitab]+felect)));
                felec            -=  qq11*rinv11*( rinvsq11 - scale*felect );
            }
            ...
            alphaij          = alphai1 * alphaj2 * gmx_invsqrt(alphai1*alphai1 + alphaj2*alphaj2);
            ewrt             = r12*ewtabscale;
            ewitab           = ewrt;
            eweps            = ewrt-ewitab;
            ...
            scale            = alphaij/ewc;
            if (  r12 * scale < rcutoff ) {
                ewrt             = scale * r12*ewtabscale;
                ewitab           = ewrt;
                eweps            = ewrt-ewitab;
                ewitab           = 4*ewitab;
                felect           = (ewtab[ewitab]+eweps*ewtab[ewitab+1])*scale ;
                velec            -=  qq12*(rinv12 - (scale*ewtab[ewitab+2]-ewtabhalfspace*eweps*(scale*ewtab[ewitab]+felect)));
                felec            -=  qq12*rinv12*( rinvsq12 - scale*felect );
            }
            ...
            alphaij          = alphai1 * alphaj3 * gmx_invsqrt(alphai1*alphai1 + alphaj3*alphaj3);
            ewrt             = r13*ewtabscale;
            ewitab           = ewrt;
            eweps            = ewrt-ewitab;
            ...
            scale            = alphaij/ewc;
            if (  r13 * scale < rcutoff ) {
                ewrt             = scale * r13*ewtabscale;
                ewitab           = ewrt;
                eweps            = ewrt-ewitab;
                ewitab           = 4*ewitab;
                felect           = (ewtab[ewitab]+eweps*ewtab[ewitab+1])*scale ;
                velec            -=  qq13*(rinv13 - (scale*ewtab[ewitab+2]-ewtabhalfspace*eweps*(scale*ewtab[ewitab]+felect)));
                felec            -=  qq13*rinv13*( rinvsq13 - scale*felect );
            }
            ...
            alphaij          = alphai2 * alphaj1 * gmx_invsqrt(alphai2*alphai2 + alphaj1*alphaj1);
            ewrt             = r21*ewtabscale;
            ewitab           = ewrt;
            eweps            = ewrt-ewitab;
            ...
            scale            = alphaij/ewc;
            if (  r21 * scale < rcutoff ) {
                ewrt             = scale * r21*ewtabscale;
                ewitab           = ewrt;
                eweps            = ewrt-ewitab;
                ewitab           = 4*ewitab;
                felect           = (ewtab[ewitab]+eweps*ewtab[ewitab+1])*scale ;
                velec            -=  qq21*(rinv21 - (scale*ewtab[ewitab+2]-ewtabhalfspace*eweps*(scale*ewtab[ewitab]+felect)));
                felec            -=  qq21*rinv21*( rinvsq21 - scale*felect );
            }
            ...
            alphaij          = alphai2 * alphaj2 * gmx_invsqrt(alphai2*alphai2 + alphaj2*alphaj2);
            ewrt             = r22*ewtabscale;
            ewitab           = ewrt;
            eweps            = ewrt-ewitab;
            ...
            scale            = alphaij/ewc;
            if (  r22 * scale < rcutoff ) {
                ewrt             = scale * r22*ewtabscale;
                ewitab           = ewrt;
                eweps            = ewrt-ewitab;
                ewitab           = 4*ewitab;
                felect           = (ewtab[ewitab]+eweps*ewtab[ewitab+1])*scale ;
                velec            -=  qq22*(rinv22 - (scale*ewtab[ewitab+2]-ewtabhalfspace*eweps*(scale*ewtab[ewitab]+felect)));
                felec            -=  qq22*rinv22*( rinvsq22 - scale*felect );
            }
            ...
            alphaij          = alphai2 * alphaj3 * gmx_invsqrt(alphai2*alphai2 + alphaj3*alphaj3);
            ewrt             = r23*ewtabscale;
            ewitab           = ewrt;
            eweps            = ewrt-ewitab;
            ...
            scale            = alphaij/ewc;
            if (  r23 * scale < rcutoff ) {
                ewrt             = scale * r23*ewtabscale;
                ewitab           = ewrt;
                eweps            = ewrt-ewitab;
                ewitab           = 4*ewitab;
                felect           = (ewtab[ewitab]+eweps*ewtab[ewitab+1])*scale ;
                velec            -=  qq23*(rinv23 - (scale*ewtab[ewitab+2]-ewtabhalfspace*eweps*(scale*ewtab[ewitab]+felect)));
                felec            -=  qq23*rinv23*( rinvsq23 - scale*felect );
            }
            ...
            alphaij          = alphai3 * alphaj1 * gmx_invsqrt(alphai3*alphai3 + alphaj1*alphaj1);
            ewrt             = r31*ewtabscale;
            ewitab           = ewrt;
            eweps            = ewrt-ewitab;
            ...
            scale            = alphaij/ewc;
            if (  r31 * scale < rcutoff ) {
                ewrt             = scale * r31*ewtabscale;
                ewitab           = ewrt;
                eweps            = ewrt-ewitab;
                ewitab           = 4*ewitab;
                felect           = (ewtab[ewitab]+eweps*ewtab[ewitab+1])*scale ;
                velec            -=  qq31*(rinv31 - (scale*ewtab[ewitab+2]-ewtabhalfspace*eweps*(scale*ewtab[ewitab]+felect)));
                felec            -=  qq31*rinv31*( rinvsq31 - scale*felect );
            }
            ...
            alphaij          = alphai3 * alphaj2 * gmx_invsqrt(alphai3*alphai3 + alphaj2*alphaj2);
            ewrt             = r32*ewtabscale;
            ewitab           = ewrt;
            eweps            = ewrt-ewitab;
            ...
            scale            = alphaij/ewc;
            if (  r32 * scale < rcutoff ) {
                ewrt             = scale * r32*ewtabscale;
                ewitab           = ewrt;
                eweps            = ewrt-ewitab;
                ewitab           = 4*ewitab;
                felect           = (ewtab[ewitab]+eweps*ewtab[ewitab+1])*scale ;
                velec            -=  qq32*(rinv32 - (scale*ewtab[ewitab+2]-ewtabhalfspace*eweps*(scale*ewtab[ewitab]+felect)));
                felec            -=  qq32*rinv32*( rinvsq32 - scale*felect );
            }
            ...
            alphaij          = alphai3 * alphaj3 * gmx_invsqrt(alphai3*alphai3 + alphaj3*alphaj3);
            ewrt             = r33*ewtabscale;
            ewitab           = ewrt;
            eweps            = ewrt-ewitab;
            ...
            scale            = alphaij/ewc;
            if (  r33 * scale < rcutoff ) {
                ewrt             = scale * r33*ewtabscale;
                ewitab           = ewrt;
                eweps            = ewrt-ewitab;
                ewitab           = 4*ewitab;
                felect           = (ewtab[ewitab]+eweps*ewtab[ewitab+1])*scale ;
                velec            -=  qq33*(rinv33 - (scale*ewtab[ewitab+2]-ewtabhalfspace*eweps*(scale*ewtab[ewitab]+felect)));
                felec            -=  qq33*rinv33*( rinvsq33 - scale*felect );
            }
    ...
    inc_nrnb(nrnb,eNR_NBKERNEL_ELEC_VDW_W4W4_VF,outeriter*41 + inneriter*286);
}
/*
 * Gromacs nonbonded kernel:   nb_kernel_ElecEwG_VdwBham_GeomW4W4_F_c
 * Electrostatics interaction: EwaldG
 * VdW interaction:            Buckingham
 * Geometry:                   Water4-Water4
 ...
 */
void
nb_kernel_ElecEwG_VdwBham_GeomW4W4_F_c
                    (t_nblist                    * gmx_restrict       nlist,
                     rvec                        * gmx_restrict          xx,
                     rvec                        * gmx_restrict          ff,
                     t_forcerec                  * gmx_restrict          fr,
                     ...
                     t_nrnb                      * gmx_restrict        nrnb)
{
    ...
    for(iidx=0; iidx<nri; iidx++)
    {
        ...
        for(jidx=j_index_start; jidx<j_index_end; jidx++)
        {
            ...
            if  (  r11 * scale < rcutoff ) {
                ...
                ewitab           = ewrt;
                eweps            = ewrt-ewitab;
                felect           = scale * ((1.0-eweps)*ewtab[ewitab]+eweps*ewtab[ewitab+1]);
                felec            -=  qq11*rinv11*( rinvsq11 - scale*felect );
	    }
    ...
    inc_nrnb(nrnb,eNR_NBKERNEL_ELEC_VDW_W4W4_F,outeriter*39 + inneriter*274);
}
