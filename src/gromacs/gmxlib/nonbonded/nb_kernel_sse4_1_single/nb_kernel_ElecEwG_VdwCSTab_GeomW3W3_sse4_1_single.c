...
/*
 * Gromacs nonbonded kernel:   nb_kernel_ElecEwG_VdwCSTab_GeomW3W3_VF_sse4_1_single
 ...
 */
void
nb_kernel_ElecEwG_VdwCSTab_GeomW3W3_VF_sse4_1_single
                     ...
                     t_nrnb                      * gmx_restrict        nrnb)
{
    ...
    alphaj0          = _mm_set1_ps(gaussian[inr+0]);
    ...
    for(iidx=0; iidx<nri; iidx++)
    {
        ...
        for(jidx=j_index_start; jidx<j_index_end && jjnr[jidx+3]>=0; jidx+=4)
        {
            ...
            /* EWALD ELECTROSTATICS WITH GAUSSIAN CHARGE DISTRIBUTIONS */
            /* Calculate Ewald table index by multiplying r with scale and truncate to integer */
            ewrt             = _mm_add_ps( _mm_mul_ps(alphai0,alphai0), _mm_mul_ps(alphaj1,alphaj1) );
            alphaij          = gmx_mm_invsqrt_ps(ewrt);
            ewrt             = _mm_mul_ps (alphai0,alphaij);
            ...
            rscale           = _mm_mul_ps(scale,r01);
            rscale2          = _mm_mul_ps(rscale,rscale);
            ewrt             = _mm_mul_ps(rscale,ewtabscale);
            if (gmx_mm_any_lt(rscale2,rcutoff2)) { 
            ewg_mask         = _mm_cmplt_ps(rscale2,rcutoff2 );
            //  ewrt = (r01*scale < rcutoff) ? ewrt : 0
            ewrt             = _mm_or_ps(_mm_and_ps(ewg_mask , ewrt) , _mm_andnot_ps(ewg_mask, zero));
            ewitab           = _mm_cvttps_epi32(ewrt);
            eweps            = _mm_sub_ps(ewrt,_mm_round_ps(ewrt, _MM_FROUND_FLOOR));
            ewitab           = _mm_slli_epi32(ewitab,2);
            ...
            felect           = _mm_mul_ps(_mm_add_ps(ewtabF,_mm_mul_ps(eweps,ewtabD)),scale);
            velect           = _mm_sub_ps(_mm_mul_ps(ewtabV,scale),_mm_mul_ps(_mm_mul_ps(ewtabhalfspace,eweps),_mm_add_ps(_mm_mul_ps(ewtabF,scale),felect)));
            velect           = _mm_mul_ps(qq01,_mm_sub_ps(rinv01,velect));
            velect           = _mm_or_ps(_mm_and_ps(ewg_mask , velect) , _mm_andnot_ps(ewg_mask, zero));
            velec            = _mm_sub_ps(velec,velect);            

            felect           = _mm_mul_ps(_mm_mul_ps(qq01,rinv01),_mm_sub_ps(rinvsq01,_mm_mul_ps(felect,scale)));
            felect           = _mm_or_ps(_mm_and_ps(ewg_mask , felect) , _mm_andnot_ps(ewg_mask, zero));
            felec            = _mm_sub_ps(felec,felect);            
            }
            ...
            /* EWALD ELECTROSTATICS WITH GAUSSIAN CHARGE DISTRIBUTIONS */
            /* Calculate Ewald table index by multiplying r with scale and truncate to integer */
            ewrt             = _mm_add_ps( _mm_mul_ps(alphai0,alphai0), _mm_mul_ps(alphaj2,alphaj2) );
            alphaij          = gmx_mm_invsqrt_ps(ewrt);
            ewrt             = _mm_mul_ps (alphai0,alphaij);
            ...
            rscale           = _mm_mul_ps(scale,r02);
            rscale2          = _mm_mul_ps(rscale,rscale);
            ewrt             = _mm_mul_ps(rscale,ewtabscale);
            if (gmx_mm_any_lt(rscale2,rcutoff2)) { 
            ewg_mask         = _mm_cmplt_ps(rscale2,rcutoff2 );
            //  ewrt = (r02*scale < rcutoff) ? ewrt : 0
            ewrt             = _mm_or_ps(_mm_and_ps(ewg_mask , ewrt) , _mm_andnot_ps(ewg_mask, zero));
            ewitab           = _mm_cvttps_epi32(ewrt);
            eweps            = _mm_sub_ps(ewrt,_mm_round_ps(ewrt, _MM_FROUND_FLOOR));
            ewitab           = _mm_slli_epi32(ewitab,2);
            ...
            felect           = _mm_mul_ps(_mm_add_ps(ewtabF,_mm_mul_ps(eweps,ewtabD)),scale);
            velect           = _mm_sub_ps(_mm_mul_ps(ewtabV,scale),_mm_mul_ps(_mm_mul_ps(ewtabhalfspace,eweps),_mm_add_ps(_mm_mul_ps(ewtabF,scale),felect)));
            velect           = _mm_mul_ps(qq02,_mm_sub_ps(rinv02,velect));
            velect           = _mm_or_ps(_mm_and_ps(ewg_mask , velect) , _mm_andnot_ps(ewg_mask, zero));
            velec            = _mm_sub_ps(velec,velect);            

            felect           = _mm_mul_ps(_mm_mul_ps(qq02,rinv02),_mm_sub_ps(rinvsq02,_mm_mul_ps(felect,scale)));
            felect           = _mm_or_ps(_mm_and_ps(ewg_mask , felect) , _mm_andnot_ps(ewg_mask, zero));
            felec            = _mm_sub_ps(felec,felect);            
            }
    ...
    inc_nrnb(nrnb,eNR_NBKERNEL_ELEC_VDW_W3W3_VF,outeriter*20 + inneriter*520);
}
/*
 * Gromacs nonbonded kernel:   nb_kernel_ElecEwG_VdwCSTab_GeomW3W3_F_sse4_1_single
 * Electrostatics interaction: EwaldG
 * VdW interaction:            CubicSplineTable
 * Geometry:                   Water3-Water3
 ...
 */
void
nb_kernel_ElecEwG_VdwCSTab_GeomW3W3_F_sse4_1_single
                    (t_nblist                    * gmx_restrict       nlist,
                     rvec                        * gmx_restrict          xx,
                     rvec                        * gmx_restrict          ff,
                     t_forcerec                  * gmx_restrict          fr,
