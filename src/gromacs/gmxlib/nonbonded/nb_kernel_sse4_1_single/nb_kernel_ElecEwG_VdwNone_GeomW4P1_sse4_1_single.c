...
/*
 * Gromacs nonbonded kernel:   nb_kernel_ElecEwG_VdwNone_GeomW4P1_VF_sse4_1_single
 ...
 */
void
nb_kernel_ElecEwG_VdwNone_GeomW4P1_VF_sse4_1_single
                     ...
                     t_nrnb                      * gmx_restrict        nrnb)
{
    ...
    int              vdwioffset1;
    __m128           ix1,iy1,iz1,fix1,fiy1,fiz1,iq1,isai1,alphai1;
    int              vdwioffset2;
    __m128           ix2,iy2,iz2,fix2,fiy2,fiz2,iq2,isai2,alphai2;
    int              vdwioffset3;
    __m128           ix3,iy3,iz3,fix3,fiy3,fiz3,iq3,isai3,alphai3;
    ...
    real             *charge;
    real             *gaussian;
    __m128           ewc, alphai, alphaij,velect,felect, scale,scalesq,ewg_mask,rscale,rscale2,zero;
    __m128i          ewitab;
    ...
    ewc              = _mm_set1_ps(fr->ewaldcoeff_q);
    ...
    alphai1          = _mm_set1_ps(gaussian[inr+1]);
    iq2              = _mm_mul_ps(facel,_mm_set1_ps(charge[inr+2]));
    alphai2          = _mm_set1_ps(gaussian[inr+2]);
    iq3              = _mm_mul_ps(facel,_mm_set1_ps(charge[inr+3]));
    alphai3          = _mm_set1_ps(gaussian[inr+3]);
    ...
    for(iidx=0; iidx<nri; iidx++)
    {
        ...
        for(jidx=j_index_start; jidx<j_index_end && jjnr[jidx+3]>=0; jidx+=4)
        {
            ...
            alphaj0          = gmx_mm_load_4real_swizzle_ps(gaussian+jnrA+0,gaussian+jnrB+0,
                                                              gaussian+jnrC+0,gaussian+jnrD+0);
            ...
            ewrt             = _mm_add_ps( _mm_mul_ps(alphai1,alphai1), _mm_mul_ps(alphaj0,alphaj0) );
            alphaij          = gmx_mm_invsqrt_ps(ewrt);
            ewrt             = _mm_mul_ps (alphai1,alphaij);
            alphaij          = _mm_mul_ps (ewrt,alphaj0);

            ewrt             = _mm_mul_ps(r10,ewtabscale);
            ewitab           = _mm_cvttps_epi32(ewrt);
            eweps            = _mm_sub_ps(ewrt,_mm_round_ps(ewrt, _MM_FROUND_FLOOR));
            scale            = _mm_div_ps(alphaij,ewc);
            scalesq          = _mm_mul_ps(scale,scale);
            zero             = _mm_setzero_ps();
            ...
            rscale           = _mm_mul_ps(scale,r10);
            rscale2          = _mm_mul_ps(rscale,rscale);
            ewrt             = _mm_mul_ps(rscale,ewtabscale);
            if (gmx_mm_any_lt(rscale2,rcutoff2)) { 
            ewg_mask         = _mm_cmplt_ps(rscale2,rcutoff2 );
            //  ewrt = (r10*scale < rcutoff) ? ewrt : 0
            ewrt             = _mm_or_ps(_mm_and_ps(ewg_mask , ewrt) , _mm_andnot_ps(ewg_mask, zero));
            ewitab           = _mm_cvttps_epi32(ewrt);
            eweps            = _mm_sub_ps(ewrt,_mm_round_ps(ewrt, _MM_FROUND_FLOOR));
            ewitab           = _mm_slli_epi32(ewitab,2);
            ewtabF           = _mm_load_ps( ewtab + gmx_mm_extract_epi32(ewitab,0) );
            ...
            ewtabFn          = _mm_load_ps( ewtab + gmx_mm_extract_epi32(ewitab,3) );
            _MM_TRANSPOSE4_PS(ewtabF,ewtabD,ewtabV,ewtabFn);
            felect           = _mm_mul_ps(_mm_add_ps(ewtabF,_mm_mul_ps(eweps,ewtabD)),scale);
            velect           = _mm_sub_ps(_mm_mul_ps(ewtabV,scale),_mm_mul_ps(_mm_mul_ps(ewtabhalfspace,eweps),_mm_add_ps(_mm_mul_ps(ewtabF,scale),felect)));
            velect           = _mm_mul_ps(qq10,_mm_sub_ps(rinv10,velect));
            velect           = _mm_or_ps(_mm_and_ps(ewg_mask , velect) , _mm_andnot_ps(ewg_mask, zero));
            velec            = _mm_sub_ps(velec,velect);            

            felect           = _mm_mul_ps(_mm_mul_ps(qq10,rinv10),_mm_sub_ps(rinvsq10,_mm_mul_ps(felect,scale)));
            felect           = _mm_or_ps(_mm_and_ps(ewg_mask , felect) , _mm_andnot_ps(ewg_mask, zero));
            felec            = _mm_sub_ps(felec,felect);            
            }
            ...
            /* EWALD ELECTROSTATICS WITH GAUSSIAN CHARGE DISTRIBUTIONS */
            /* Calculate Ewald table index by multiplying r with scale and truncate to integer */
            ewrt             = _mm_add_ps( _mm_mul_ps(alphai2,alphai2), _mm_mul_ps(alphaj0,alphaj0) );
            alphaij          = gmx_mm_invsqrt_ps(ewrt);
            ewrt             = _mm_mul_ps (alphai2,alphaij);
            alphaij          = _mm_mul_ps (ewrt,alphaj0);
            ...
            rscale           = _mm_mul_ps(scale,r20);
            rscale2          = _mm_mul_ps(rscale,rscale);
            ewrt             = _mm_mul_ps(rscale,ewtabscale);
            if (gmx_mm_any_lt(rscale2,rcutoff2)) { 
            ewg_mask         = _mm_cmplt_ps(rscale2,rcutoff2 );
            //  ewrt = (r20*scale < rcutoff) ? ewrt : 0
            ewrt             = _mm_or_ps(_mm_and_ps(ewg_mask , ewrt) , _mm_andnot_ps(ewg_mask, zero));
            ewitab           = _mm_cvttps_epi32(ewrt);
            eweps            = _mm_sub_ps(ewrt,_mm_round_ps(ewrt, _MM_FROUND_FLOOR));
            ewitab           = _mm_slli_epi32(ewitab,2);
            ...
            felect           = _mm_mul_ps(_mm_add_ps(ewtabF,_mm_mul_ps(eweps,ewtabD)),scale);
            velect           = _mm_sub_ps(_mm_mul_ps(ewtabV,scale),_mm_mul_ps(_mm_mul_ps(ewtabhalfspace,eweps),_mm_add_ps(_mm_mul_ps(ewtabF,scale),felect)));
            velect           = _mm_mul_ps(qq20,_mm_sub_ps(rinv20,velect));
            velect           = _mm_or_ps(_mm_and_ps(ewg_mask , velect) , _mm_andnot_ps(ewg_mask, zero));
            velec            = _mm_sub_ps(velec,velect);            

            felect           = _mm_mul_ps(_mm_mul_ps(qq20,rinv20),_mm_sub_ps(rinvsq20,_mm_mul_ps(felect,scale)));
            felect           = _mm_or_ps(_mm_and_ps(ewg_mask , felect) , _mm_andnot_ps(ewg_mask, zero));
            felec            = _mm_sub_ps(felec,felect);            
            }
            ...
            /* EWALD ELECTROSTATICS WITH GAUSSIAN CHARGE DISTRIBUTIONS */
            /* Calculate Ewald table index by multiplying r with scale and truncate to integer */
            ewrt             = _mm_add_ps( _mm_mul_ps(alphai3,alphai3), _mm_mul_ps(alphaj0,alphaj0) );
            alphaij          = gmx_mm_invsqrt_ps(ewrt);
            ewrt             = _mm_mul_ps (alphai3,alphaij);
            alphaij          = _mm_mul_ps (ewrt,alphaj0);
            ...
            rscale           = _mm_mul_ps(scale,r30);
            rscale2          = _mm_mul_ps(rscale,rscale);
            ewrt             = _mm_mul_ps(rscale,ewtabscale);
            if (gmx_mm_any_lt(rscale2,rcutoff2)) { 
            ewg_mask         = _mm_cmplt_ps(rscale2,rcutoff2 );
            //  ewrt = (r30*scale < rcutoff) ? ewrt : 0
            ewrt             = _mm_or_ps(_mm_and_ps(ewg_mask , ewrt) , _mm_andnot_ps(ewg_mask, zero));
            ewitab           = _mm_cvttps_epi32(ewrt);
            eweps            = _mm_sub_ps(ewrt,_mm_round_ps(ewrt, _MM_FROUND_FLOOR));
            ewitab           = _mm_slli_epi32(ewitab,2);
            ...
            felect           = _mm_mul_ps(_mm_add_ps(ewtabF,_mm_mul_ps(eweps,ewtabD)),scale);
            velect           = _mm_sub_ps(_mm_mul_ps(ewtabV,scale),_mm_mul_ps(_mm_mul_ps(ewtabhalfspace,eweps),_mm_add_ps(_mm_mul_ps(ewtabF,scale),felect)));
            velect           = _mm_mul_ps(qq30,_mm_sub_ps(rinv30,velect));
            velect           = _mm_or_ps(_mm_and_ps(ewg_mask , velect) , _mm_andnot_ps(ewg_mask, zero));
            velec            = _mm_sub_ps(velec,velect);            

            felect           = _mm_mul_ps(_mm_mul_ps(qq30,rinv30),_mm_sub_ps(rinvsq30,_mm_mul_ps(felect,scale)));
            felect           = _mm_or_ps(_mm_and_ps(ewg_mask , felect) , _mm_andnot_ps(ewg_mask, zero));
            felec            = _mm_sub_ps(felec,felect);            
            }
    ...
    inc_nrnb(nrnb,eNR_NBKERNEL_ELEC_W4_VF,outeriter*19 + inneriter*162);
}
/*
 * Gromacs nonbonded kernel:   nb_kernel_ElecEwG_VdwNone_GeomW4P1_F_sse4_1_single
 * Electrostatics interaction: EwaldG
 * VdW interaction:            None
 * Geometry:                   Water4-Particle
 ...
 */
void
nb_kernel_ElecEwG_VdwNone_GeomW4P1_F_sse4_1_single
                    (t_nblist                    * gmx_restrict       nlist,
                     rvec                        * gmx_restrict          xx,
                     rvec                        * gmx_restrict          ff,
                     t_forcerec                  * gmx_restrict          fr,
    ...
    for(iidx=0; iidx<nri; iidx++)
    {
        ...
        for(jidx=j_index_start; jidx<j_index_end && jjnr[jidx+3]>=0; jidx+=4)
        {
            ...
            gmx_mm_load_4pair_swizzle_ps(ewtab + gmx_mm_extract_epi32(ewitab,0),ewtab + gmx_mm_extract_epi32(ewitab,1),
                                         ewtab + gmx_mm_extract_epi32(ewitab,2),ewtab + gmx_mm_extract_epi32(ewitab,3),
                                         &ewtabF,&ewtabFn);
            felect           = _mm_add_ps(_mm_mul_ps( _mm_sub_ps(one,eweps),ewtabF),_mm_mul_ps(eweps,ewtabFn));
            felect           = _mm_mul_ps(felect,scale);
            felect           = _mm_mul_ps(_mm_mul_ps(qq10,rinv10),_mm_sub_ps(rinvsq10,_mm_mul_ps(felect,scale)));
            felect           = _mm_or_ps(_mm_and_ps(ewg_mask , felect) , _mm_andnot_ps(ewg_mask, zero));
            felec            = _mm_sub_ps(felec,felect);
            }
