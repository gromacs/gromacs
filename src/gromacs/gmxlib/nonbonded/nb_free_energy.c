...
#define  NSTATES  2
    ...
    gmx_bool      bConvertEwaldToCoulomb;
    ...
    for (n = 0; (n < nri); n++)
    {
        ...
        for (k = nj0; (k < nj1); k++)
        {
            ...
            if (bConvertEwaldToCoulomb && ( !bExactElecCutoff || r < rcoulomb ) )
            {
                ...
                if (ii == jnr)
                {
                    /* TODO Is the next comment correct? */
                    /* This is only possible with the Verlet scheme */
                    v_lr *= 0.5;
                }
            ...
            /* TODO document what this part is doing - it's something
               to do with lambda-weighting the component of this
               interaction that is handled in reciprocal space */
            if (bEwaldLJ && (!bExactVdwCutoff || r < rvdw))
            {
                real rs, frac, f_lr;
                int  ri;
                ...
                /* TODO Should ii == jnr lead to VV being multiplied
                   by 0.5 here, too? If so, this was an existing bug
                   in release-5-0 branch. */
