...
#define  NSTATES  2
    ...
    gmx_bool      bExactElecCutoff, bExactVdwCutoff, bEwaldOrRF;
    ...
    if (fr->cutoff_scheme == ecutsVERLET)
    {
        ...
        {
            icoul        = GMX_NBKERNEL_ELEC_COULOMB;
        }
    ...
    /* Electrostatics interactions that are (close to) 0 at the cut-off */
    bEwaldOrRF = (EEL_PME_EWALD(fr->eeltype) || EEL_RF(fr->eeltype));
    ...
    for (n = 0; (n < nri); n++)
    {
        ...
        for (k = nj0; (k < nj1); k++)
        {
            ...
            if (nlist->excl_fep == NULL || nlist->excl_fep[k])
            {
                ...
                for (i = 0; i < NSTATES; i++)
                {
                    ...
                    if ( (qq[i] != 0) || (c6[i] != 0) || (c12[i] != 0) )
                    {
                        ...
                        /* With Ewald and RF and soft-core we should put the cut-off
                         * on r, not on the soft-cored rC, as the real-space and
                         * reciprocal space contributions should (almost) cancel.
                         */
                        if (qq[i] != 0 &&
                            !(bExactElecCutoff &&
                              ((!bEwaldOrRF && rC >= rcoulomb) ||
                               (bEwaldOrRF && r >= rcoulomb))))
                        {
                            switch (icoul)
                            {
            ...
            else if (icoul == GMX_NBKERNEL_ELEC_REACTIONFIELD)
            {
                 ...
                 * since this simplifies the model and code.
                 */
                VV = krf*rsq - crf;
                FF = -2.0*krf;
