...
/*
 * Gromacs nonbonded kernel:   nb_kernel_ElecEwSw_VdwLJSw_GeomW3P1_VF_avx_128_fma_double
                     ...
                     t_nrnb                        * gmx_restrict        nrnb)
{
    ...
    sh_ewald         = _mm_set1_pd(kernel_data->ic->sh_ewald);
    ewtab            = kernel_data->ic->tabq_coul_FDV0;
    ewtabscale       = _mm_set1_pd(kernel_data->ic->tabq_scale);
    ewtabhalfspace   = _mm_set1_pd(0.5/kernel_data->ic->tabq_scale);
    ...
    for(iidx=0; iidx<nri; iidx++)
    {
        ...
        for(jidx=j_index_start; jidx<j_index_end-1; jidx+=2)
        {
            ...
            felec            = _mm_msub_pd( felec,sw , _mm_mul_pd(rinv00,_mm_mul_pd(velec,dsw)) );
            ...
            /* Evaluate switch function */
            /* fscal'=f'/r=-(v*sw)'/r=-(v'*sw+v*dsw)/r=-v'*sw/r-v*dsw/r=fscal*sw-v*dsw/r */
            felec            = _mm_msub_pd( felec,sw , _mm_mul_pd(rinv10,_mm_mul_pd(velec,dsw)) );
            velec            = _mm_mul_pd(velec,sw);
            ...
            /* Evaluate switch function */
            /* fscal'=f'/r=-(v*sw)'/r=-(v'*sw+v*dsw)/r=-v'*sw/r-v*dsw/r=fscal*sw-v*dsw/r */
            felec            = _mm_msub_pd( felec,sw , _mm_mul_pd(rinv20,_mm_mul_pd(velec,dsw)) );
            velec            = _mm_mul_pd(velec,sw);
...
/*
 * Gromacs nonbonded kernel:   nb_kernel_ElecEwSw_VdwLJSw_GeomW3P1_F_avx_128_fma_double
 * Electrostatics interaction: Ewald
 * VdW interaction:            LennardJones
 * Geometry:                   Water3-Particle
