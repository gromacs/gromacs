...
static std::string detected_hardware_string(const gmx_hw_info_t *hwinfo)
{
    ...
    s  = gmx::formatString("\n");
    s += gmx::formatString("Running on %d node%s with %d total hardware threads",
    ...
    s += gmx::formatString("Hardware detected on host %s (node of MPI rank %d):\n",
            host, rank);
#else
                                 ...
                                 const gmx_hw_info_t *hwinfo)
{
    ...
    detected = detected_hardware_string(hwinfo);
