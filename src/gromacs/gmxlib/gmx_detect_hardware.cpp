...
void gmx_parse_gpu_ids(gmx_gpu_opt_t *gpu_opt)
{
    ...
    if (env != NULL)
    {
        ...
        parse_digits_from_plain_string(env,
                                       ...
                                       &gpu_opt->dev_use);
#ifdef GMX_USE_OPENCL
        if (anyGpuIdIsRepeated(gpu_opt))
        {
            gmx_fatal(FARGS, "The OpenCL implementation only supports using exactly one PP rank per GPU");
        }
#endif
...
static void set_gpu_ids(gmx_gpu_opt_t *gpu_opt, int nrank, int rank)
{
    ...
    if (nrank > gpu_opt->n_dev_compatible)
    {
        ...
        {
#ifdef GMX_USE_OPENCL
            /* Our current OpenCL implementation only supports using
             * exactly one PP rank per GPU, so sharing is
             * impossible */
            nshare = 1;
#else
