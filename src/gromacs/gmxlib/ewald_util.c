                        ...
                        real lambda, real *dvdlambda)
{
    ...
    atom_id *AA;
    double   Vexcl_q, dvdl_excl; /* Necessary for precision */
    double   Vexcl_lj;
    real     one_4pi_eps;
    ...
    real     c6Ai, c6Bi, c6A = 0, c6B = 0, ewcdr2, ewcdr4, ewcdr5, c6L, rinv6;
    rvec     df, dx, mutot[2], dipcorrA, dipcorrB;
    tensor   dxdf_q, dxdf_lj;
    real     vol = box[XX][XX]*box[YY][YY]*box[ZZ][ZZ];
    real     L1, dipole_coeff, qqA, qqB, qqL, vr0_q, vr0_lj;
    /*#define TABLES*/
    /*TODO: TABLES variable never set, coulvdwtab only exists here, this should be investigated*/
#ifdef TABLES
    ...
    int         bDoingLBRule = (fr->ljpme_comb == eljpmeLB);
...
#else
                                    /* This is the code you would want
                                     * instead if not using tables:
                                     */
                                    ewcdr    = ewc_q*dr;
...
#define       R_ERF_R_INACC 0.1
                ...
                for (j = i1; (j < i2); j++)
                {
                    ...
                    if (k > i)
                    {
                        ...
                        if (EVDW_PME(fr->vdwtype))
                        {
                            ...
                            if (bDoingLBRule)
                            {
                                qqA *= pow(0.5*(sigmaA[i]+sigmaA[k]), 6)*sigma3A[k];
                                qqB *= pow(0.5*(sigmaB[i]+sigmaB[k]), 6)*sigma3B[k];
                            }
                        }
                        if (qqA != 0.0 || qqB != 0.0 || c6A != 0.0 || c6B != 0.0)
                        {
                            ...
                            if (dr2 != 0)
                            {
                                ...
                                if ((c6A != 0.0 || c6B != 0.0) && EVDW_PME(fr->vdwtype))
                                {
                                    ...
                                    vc        = c6L*v;
                                    Vexcl_lj += vc;
                                    fscal     = 6.0*vc*rinv + qqL*rinv6*exp(-ewcdr2)*ewc_lj*ewcdr5;
                                    svmul(fscal, dx, df);
                                    rvec_inc(f[k], df);
                                    rvec_dec(f[i], df);
                                    for (iv = 0; (iv < DIM); iv++)
    ...
    if (MASTER(cr) && thread == 0)
    {
        for (q = 0; q < (bFreeEnergy ? 2 : 1); q++)
        {
            if (calc_excl_corr)
            {
                ...
                if (EVDW_PME(fr->vdwtype))
                {
                    Vself_lj[q] = -pow(ewc_lj, 6)*fr->c6sum[q]/12;
                }
            }
    ...
    {
        *Vcorr_q = L1*(Vdipole[0] - Vself_q[0])
            + lambda*(Vdipole[1] - Vself_q[1])
            - Vexcl_q;
        *dvdlambda += Vdipole[1] - Vself_q[1]
            - (Vdipole[0] - Vself_q[0]) - dvdl_excl;
        if (EVDW_PME(fr->vdwtype))
        {
            *Vcorr_lj = -(L1*Vself_lj[0] + lambda*Vself_lj[1]) - Vexcl_lj;
        }
    }
    ...
    if (debug)
    {
        ...
        fprintf(debug, "q2sum = %g, Vself_q=%g c6sum = %g, Vself_lj=%g\n",
                L1*fr->q2sum[0]+lambda*fr->q2sum[1], L1*Vself_q[0]+lambda*Vself_q[1], L1*fr->c6sum[0]+lambda*fr->c6sum[1], L1*Vself_lj[0]+lambda*Vself_lj[1]);
