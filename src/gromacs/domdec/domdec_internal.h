...
/*! \internal \file
 *
 * \brief Declares implementation types for the domain decomposition module.
 *
...
/*! \brief The zone to zone atoms and pair-search grid communication setup
 *
...
typedef struct {
    ...
    int      ncx;               /**< Grid size along x */
    int      ncy;               /**< Grid size along y */
    ...
    int     *cxy_natoms;        /**< The number of atoms per grid column */
...
#endif
    int         nreq; /**< The number of outstanding requests */
} zones_mpi_dir_t;
...
typedef struct gmx_domdec_comm
{
    ...
    /* Load balacing stuff */
    int        eDLB;              /**< DLB selection: auto/on/off */
    gmx_bool   bDLB_locked;       /**< Is eDLB=edlbAUTO locked such that we currently can't turn it on? */
    gmx_bool   bDynLoadBal;       /**< Are we currently actually using DLB? */
    real     **slb_frac;          /**< Cell sizes for static load balancing, first index cartesian */
    ...
    zone_comm_t  zone_bw[DD_MAXZONE]; /**< Setup for communicating x backward */
    zone_comm_t  zone_fw[DD_MAXZONE]; /**< Setup for communicating f forward */
...
/*! /brief The non-bonded zone-pair setup for 3D domain decomposition
 *
 ...
 */
static const ivec dd_zp3[4] = {{0, 0, 8}, {1, 3, 6}, {2, 5, 6}, {3, 5, 7}};

/*! /brief The non-bonded zone-pair setup for 2D domain decomposition
 *
 * The first number is the i-zone, the second number the first j-zone seen by
 * this i-zone, the third number the last+1 j-zone seen by this i-zone.
 */
static const ivec dd_zp2[2] = {{0, 0, 4}, {1, 3, 4}};

/*! /brief The non-bonded zone-pair setup for 1D domain decomposition
 *
 * The first number is the i-zone, the second number the first j-zone seen by
 * this i-zone, the third number the last+1 j-zone seen by this i-zone.
 */
static const ivec dd_zp1[1] = {{0, 0, 2}};
