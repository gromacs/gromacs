                       ...
                       int                    *cginfo)
{
    ...
    {
        int a0 = atomGroups->blockStart(g);
        int a1 = atomGroups->blockEnd(g);
        if (move[g] == -1)
        {
                          ...
                          rvec cm_old, rvec cm_new, real pos_d)
{
    ...
    if (limitd > 0)
    {
        fprintf(fplog, "%s %d moved more than the distance allowed by the domain decomposition (%f) in direction %c\n",
                ...
                ddglatnr(dd, dd->atomGroups().blockStart(cg)), limitd, dim2char(dim));
    }
    else
    {
        ...
        fprintf(fplog, "%s %d moved more than the distance allowed by the domain decomposition in direction %c\n",
                dd->comm->bCGs ? "The charge group starting at atom" : "Atom",
                ddglatnr(dd, dd->atomGroups().blockStart(cg)), dim2char(dim));
    }
                         ...
                         int *move)
{
    ...
    {
        k0   = atomGroups.blockStart(cg);
        k1   = atomGroups.blockEnd(cg);
        nrcg = k1 - k0;
        if (nrcg == 1)
        {
