    ...
    mpi_comm_mysim_world_(mpi_comm_mysim_world)
{
    ...
    if (mpiCommPpGroup_ != MPI_COMM_NULL) // we're on a PP rank
    {
#if GMX_MPI && GMX_NVSHMEM
        int numPpRanks;
        MPI_Comm_size(mpiCommPpGroup_, &numPpRanks);
        groupRankToPE_.resize(numPpRanks, -1);
        int myPE = nvshmem_my_pe();
        MPI_Allgather(&myPE, 1, MPI_INT, groupRankToPE_.data(), 1, MPI_INT, mpiCommPpGroup_);
        int numRanksInSim;
