...
LocalAtomSetData::LocalAtomSetData(const int numberOfAtoms, const int *index)
{
    globalIndex_.assign(index, index+numberOfAtoms);

    /* if local and collective indices are not yet set in a domain decomposition step
     *  - local atom indices are global atom indices and
     *  - the collective index runs * from 0..numberOfAtoms-1
     * local and collective indices will be updated in setLocalAndCollectiveIndices
     * to match domain decompostion.
     */
    localIndex_.assign(index, index+numberOfAtoms);
    collectiveIndex_.resize(numberOfAtoms);
