...
LocalAtomSetData::LocalAtomSetData(const int numberOfAtoms, const int *index, bool bParallel)
{
    globalIndex_.assign(index, index+numberOfAtoms);

    /* if not running in parallel, local atom indices are global atom indices and
     * the collective index runs from 0..numberOfAtoms-1 and we are done with initialisation
