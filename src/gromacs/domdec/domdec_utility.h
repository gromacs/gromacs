 ...
 *
 * \param[in] cginfo_mb Array of charge group info per molblock
 * \param[in] cg_gl     The global charge group index
 *
 ...
 */
static gmx_inline int ddcginfo(const cginfo_mb_t *cginfo_mb, int cg_gl)
{
 ...
 *
 * \param[in]     index_gl Local to global atom index
 * \param[in]     cg0      Start charge group
 * \param[in]     cg1      End charge group
 * \param[in,out] fr       Forcerec, fr->cginfo gets updated
 * \param[in,out] bLocalCG Local CG boolean array, disguised as chars
 *
 ...
 * This function is costly, only call it when it can not be done in another way.
 */
void dd_set_cginfo_threaded(const int *index_gl, int cg0, int cg1,
                            t_forcerec *fr, char *bLocalCG);

/*! \brief Convert DD zone bounds to a corner coordinate.
 *
 * \param[in]     dd     DD data structure
 * \param[in]     box    The simulation unit cell
 * \param[in,out] corner Input zone bounds, output Cartesian coordinates
 *
 * Given zone bounds for three dimensions in \p corner, convert these
 * to Cartesian coordinates for the corresponding corner of the zone,
 * taking into account, potential, triclinic conversion and couplings.
 */
static gmx_inline void dd_zone_bounds_to_corner(const gmx_domdec_t *dd,
                                                const matrix        box,
                                                rvec                corner)
{
    ...
    assert(dd->npbcdim <= DIM);
    for (i = YY; i < dd->npbcdim; i++)
    {
        for (j = XX; j < i; j++)
        {
