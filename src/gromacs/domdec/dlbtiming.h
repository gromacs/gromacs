...
/* NOTE: The next five functions should not be called directly, but only
 *       through the DDBalanceRegionHandling interface. They are declared
 *       here so the interface can inline the boolean check so we avoid
 *       function call overheads when not using DLB
 */
...
/*! \libinternal
 * \brief Interface for starting and stopping the dynamic load balancing region
 */
class DDBalanceRegionHandling
{
    public:
        ...
        void openBeforeForceComputationCpu(DdAllowBalanceRegionReopen allowReopen) const
        {
            ...
            {
                ddOpenBalanceRegionCpu(dd_, allowReopen);
            }
        }
                                           ...
                                           DdBalanceRegionWaitedForGpu waitedForGpu) const
        {
            if (useBalancingRegion_)
            {
                ddCloseBalanceRegionGpu(dd_, waitCyclesGpuInCpuRegion, waitedForGpu);
            }
        }
