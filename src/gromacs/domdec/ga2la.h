...
class gmx_ga2la_t
{
    private:
        ...
        /*! Returns whether a only direct list is used */
        bool useDirectListOnly() const
        {
    ...
    public:
        ...
        void set(int a_gl,
                 ...
                 int cell)
        {
            if (useDirectListOnly())
            {
                direct_[a_gl].la   = a_loc;
                direct_[a_gl].cell = cell;
            ...
            if (hashed_[ind].ga >= 0)
            {
                ...
                int ind_prev = ind;
                while (hashed_[ind_prev].next >= 0)
                {
                    ind_prev = hashed_[ind_prev].next;
                }
                /* Search for space in the array */
                ind = startSpaceSearch_;
                while (ind < hashed_.size() && hashed_[ind].ga >= 0)
                {
                    ind++;
                }
                ...
                if (ind == hashed_.size())
                {
                    hashed_.resize(hashed_.size() + 1);
                }
                hashed_[ind_prev].next = ind;

                startSpaceSearch_ = ind + 1;
            }
            hashed_[ind].ga   = a_gl;
            hashed_[ind].la   = a_loc;
            hashed_[ind].cell = cell;
        }
        ...
        void unset(int a_gl)
        {
            ...
            do
            {
                if (hashed_[ind].ga == a_gl)
                {
                    if (ind_prev >= 0)
                    {
                        hashed_[ind_prev].next = hashed_[ind].next;
                 ...
                 int *cell) const
        {
            if (useDirectListOnly())
            {
                *a_loc = direct_[a_gl].la;
                *cell  = direct_[a_gl].cell;

                return (direct_[a_gl].cell >= 0);
            }
            ...
            do
            {
                if (hashed_[ind].ga == a_gl)
                {
                    *a_loc = hashed_[ind].la;
                    *cell  = hashed_[ind].cell;
        ...
        /*! \brief Returns whethe the global atom a_gl is a home atom
         *
         * \param[in]  a_gl  The global atom index
         * \param[out] a_loc If the return value is true, the local atom index
    ...
    private:
        std::vector<directEntry> direct_;           /**< Direct list */
        std::vector<hashEntry>   hashed_;           /**< The hash table list */
