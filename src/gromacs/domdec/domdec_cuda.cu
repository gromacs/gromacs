                                     ...
                                     const float         *xshift)
{
    ...
    float3 *data_dest = (float3*) &data_packed[3*idx];
    float3 *data_src  = (float3*) &data[3*map[idx]];
    float3 *shift     = (float3*) xshift;

    if (idx < mapSize)
    {
        if (usePBC)
        {
            *data_dest = *data_src + *shift;
        }
        else
        {
            *data_dest = *data_src;
                        ...
                        matrix                     box)
{
    ...
    if (size > ddGpu->indexMapSize) //(re-)allocate required structures
    {
        ...
        if (ddGpu->h_indexMap)
        {
            free(ddGpu->h_indexMap);
            cudaFree(ddGpu->d_indexMap); //TODO use gmx API
        }
        ddGpu->h_indexMap = (int*) malloc(size*sizeof(int));
        cudaMalloc(&ddGpu->d_indexMap, size*sizeof(int)); //TODO use gmx API

        if (ddGpu->d_sendBuf)
        {
            cudaFree(ddGpu->d_sendBuf); //TODO use gmx API
        }

        cudaMalloc(&ddGpu->d_sendBuf, size*sizeof(rvec));         //TODO use gmx API

        cudaMalloc((void**) &(ddGpu->d_xshift), 3*sizeof(float)); //TODO use gmx API
    ...
    ddGpu->usePBC         = usePBC;              // Period Boundary Conditions for this rank
    ddGpu->recvOffset     = nat_tot;             //offset for data recieved by this rank
    copy_rvec(box[dd->dim[0]], ddGpu->h_xshift); //three components of shift for PBC
    ...
    for (int g : ind.index)
    {
        ...
        {
            ddGpu->h_indexMap[n] = j; //add index to index map
            n++;
        }
    }
    ...
    //TODO use gmx API
    cudaMemcpy(ddGpu->d_indexMap, ddGpu->h_indexMap,
               ddGpu->indexMapSize*sizeof(int), cudaMemcpyHostToDevice);
    cudaMemcpy(ddGpu->d_xshift, ddGpu->h_xshift, 3*sizeof(float), cudaMemcpyHostToDevice);

    // Setup send and recv sizes
    ddGpu->copySizeSend = ddGpu->indexMapSize; //size sent by this rank
    // exchange sizes with remote ranks to get recv sizes
    MPI_Sendrecv(&ddGpu->indexMapSize, sizeof(ddGpu->indexMapSize), MPI_BYTE,
                 ddGpu->sendRank, 0, &ddGpu->copySizeRecv, sizeof(int), MPI_BYTE, ddGpu->recvRank, 0,
...
void dd_move_x_gpu(gmx_domdec_t *dd, rvec* x_d_ptr, void *stream_ptr)
{
    ...
    float           *sendBuf  = ddGpu->d_sendBuf;
    const float     *x_d      = (float*) x_d_ptr;
    const int       *indexMap = ddGpu->d_indexMap;
    const int        size     = ddGpu->indexMapSize;
    const float     *xshift   = ddGpu->d_xshift;

    auto             kernelFn = ddGpu->usePBC ? pack_send_buf_kernel<true> : pack_send_buf_kernel<false>;
    ...
    MPI_Irecv(&x_d_ptr[ddGpu->recvOffset][0], ddGpu->copySizeRecv*3, MPI_FLOAT, ddGpu->recvRank, 0,
              MPI_COMM_WORLD, &request[1]);

    MPI_Isend(ddGpu->d_sendBuf, ddGpu->copySizeSend*3, MPI_FLOAT, ddGpu->sendRank, 0,
