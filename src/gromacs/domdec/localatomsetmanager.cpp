...
LocalAtomSet
LocalAtomSetManager::add(const int numberOfAtoms, const int *index)
{
    impl_->atomSetData_.emplace_back(new internal::LocalAtomSetData(numberOfAtoms, index, impl_->bParallel_));
...
LocalAtomSetManager::setIndicesInDomainDecomposition(const gmx_ga2la_t  *ga2la)
{
    ...
    for (const auto &atom_set : impl_->atomSetData_)
    {
