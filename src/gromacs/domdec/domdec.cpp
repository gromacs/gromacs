...
{
    GMX_ASSERT(ddnatCON == ddnatNR-1, "If you add a ddnat or reorder entries, consider which atoms need to be present in mdatoms");
    /* We set mdatoms for all atoms, also the one communicated for constraints.
     * We could save some work by only setting invmass for the latter.
     */

    return dd->comm->nat[ddnatCON];
}
