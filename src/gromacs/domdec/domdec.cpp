                  ...
                  int natoms, const rvec x[], const matrix box)
{
    ...
    for (int i = 0; i < natoms; i++)
    {
        ...
        {
            c = 0;
            while (i >= dd->atomGroups().boundary(dd->comm->zones.cg_range[c + 1]))
            {
                c++;
            }
            b = c;
                            ...
                            const int *gcgs_index, int cg_start)
{
    ...
    int a = dd->atomGroups().boundary(cg_start);
                               ...
                               const RangePartitioning &atomGroups)
{
    ...
    for (int zone = 0; zone < nzone; zone++)
    {
        ...
        {
            ind.cell2at0[zone]  = atomGroups.boundary(g);
            g                  += ind.nrecv[zone];
            ind.cell2at1[zone]  = atomGroups.boundary(g);
        }
    }
}
