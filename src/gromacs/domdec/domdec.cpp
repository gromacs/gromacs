    ...
    if (Flags & MD_REPRODUCIBLE)
    {
        ...
        switch (dlbState)
        {
            ...
            case edlbsOffCanTurnOn:
                GMX_RELEASE_ASSERT(false, "edlbsOffCanTurnOn is not a valid initial state");
                break;
            case edlbsOnCanTurnOff:
                ...
                break;
            case edlbsOnUser:
                gmx_fatal(FARGS, "Reproducibility requested with dynamic load balancing, but load balanced runs can not be binary reproducible\n");
                break;
            default:
...
static void turn_on_dlb(FILE *fplog, t_commrec *cr, gmx_int64_t step)
{
    ...
    {
        char buf[STRLEN];
        sprintf(buf, "step %" GMX_PRId64 " Measured %.1f %% performance loss due to load imbalance, "
