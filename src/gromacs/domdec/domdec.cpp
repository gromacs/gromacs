                  ...
                  int natoms, const rvec x[], const matrix box)
{
    ...
    for (int i = 0; i < natoms; i++)
    {
        ...
        {
            c = 0;
            while (i >= dd->atomGroups().block(dd->comm->zones.cg_range[c + 1]).begin())
            {
                c++;
            }
            b = c;
                            ...
                            const int *gcgs_index, int cg_start)
{
    ...
    int a = dd->atomGroups().block(cg_start).begin();
                               ...
                               const RangePartitioning &atomGroups)
{
    ...
    for (int zone = 0; zone < nzone; zone++)
    {
        ...
        {
            ind.cell2at0[zone]  = atomGroups.block(g).begin();
            g                  += ind.nrecv[zone];
            ind.cell2at1[zone]  = atomGroups.block(g).begin();
        }
    }
}
                           ...
                           gmx::ArrayRef<gmx::RVec>      buf)
{
    ...
    for (cg = 0; cg < ncg; cg++)
    {
        cg0 = atomGroups->block(sort[cg].ind).begin();
        cg1 = atomGroups->block(sort[cg].ind).begin();
        for (i = cg0; i < cg1; i++)
        {
            copy_rvec(v[i], buf[a]);
            a++;
                          ...
                          int ncg_home_old)
{
    ...
    DDBufferAccess<gmx::RVec>     rvecBuffer(dd->comm->rvecBuffer, atomGroups.block(dd->ncg_home).begin());
    ...
    dd->comm->atomRanges.setEnd(DDAtomRanges::Type::Home, dd->atomGroups().block(dd->ncg_home).begin());
