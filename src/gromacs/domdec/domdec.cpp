    ...
    if (dd->nnodes > 1 && comm->load_sum > 0)
    {
        ...
        switch (dd->comm->dlbState)
        {
            case edlbsOffForever:
                dlbStateStr = "At the end of the run DLB is disabled by user input or due to observed performance loss.";
                break;
            case edlbsOffCanTurnOn:
                dlbStateStr = "During the run DLB was off due to low measured imbalance.";
                break;
            ...
            case edlbsOnCanTurnOff:
                dlbStateStr = "During the run DLB switched on due to measured imbalance.";
                break;
            case edlbsOnForever:
