 ...
 * if to trigger dynamic load balancing.
 */
static void dd_dlb_set_should_check_whether_to_turn_on_dlb(gmx_domdec_t *dd, gmx_bool bValue)
{
...
void dd_dlb_unlock(gmx_domdec_t *dd)
{
    ...
    if (dd->comm->eDLB == edlbAUTO)
    {
        ...
        dd_dlb_set_should_check_whether_to_turn_on_dlb(dd, !dd->comm->bDynLoadBal);
    }
}
                         ...
                         gmx_bool             bVerbose)
{
    ...
    if (comm->bRecordLoad && dd_load_count(comm) > 0)
    {
        ...
        if (comm->n_load_have % nddp_chk_dlb == nddp_chk_dlb - 1)
        {
            bCheckWhetherToTurnDlbOn = dd->comm->bCheckWhetherToTurnDlbOn;
        }
            ...
            (bVerbose && (ir->nstlist == 0 || nstglobalcomm <= ir->nstlist)))
        {
            ...
            if (bCheckWhetherToTurnDlbOn)
            {
                ...
                dd_dlb_set_should_check_whether_to_turn_on_dlb(dd, FALSE);
            }
        }
        comm->n_load_have++;
    }
