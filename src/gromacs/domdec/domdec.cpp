    ...
    if (dd->nnodes > 1 && comm->load_sum > 0)
    {
        ...
        switch (dd->comm->dlbState)
        {
            case edlbsOffForever:
                dlbStateStr = "permanently off due to user-input or performance loss observed"; break;
            case edlbsOffCanTurnOn:
                dlbStateStr = "off as no significant imbalance measured"; break;
            case edlbsOffTemporarilyLocked:
                dlbStateStr = "off temporarily (due to internal lock)"; break;
            case edlbsOnCanTurnOff:
                dlbStateStr = "on as significant imbalance measured"; break;
            case edlbsOnForever:
                dlbStateStr = "permanently on per user-request"; break;
            default:
