...
static void turn_off_dlb(FILE *fplog, t_commrec *cr, gmx_int64_t step)
{
    ...
    char          buf[STRLEN];
    sprintf(buf, "step %" GMX_PRId64 " Turning off dynamic load balancing, because the performance decreased.\n", step);
    dd_warning(cr, fplog, buf);
...
static void turn_off_dlb_forever(FILE *fplog, t_commrec *cr, gmx_int64_t step)
{
    ...
    char buf[STRLEN];
    sprintf(buf, "step %" GMX_PRId64 " Will no longer try dynamic load balancing, as it decreases performance.\n", step);
    dd_warning(cr, fplog, buf);
