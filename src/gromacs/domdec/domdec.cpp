                                    ...
                                    unsigned long Flags, const t_inputrec *ir)
{
    ...
    {
        std::string integratorName(EI(ir->eI));
        std::string reasonStr = "it is only supported with dynamics, not with integrator '" + integratorName + "'.";
        return forceDlbOffOrBail(dlbState, reasonStr, cr, fplog);
    }
    ...
    if (Flags & MD_REPRODUCIBLE)
    {
        ...
        {
            case edlbsOffUser:
            case edlbsOffForever:   // FIXME: not a valid initial state, shouldn't be listed here?!
                break;
            case edlbsOffCanTurnOn: /// FIXME: invalid initial state, shouldn't be listed here?!
            case edlbsOnCanTurnOff:
                reasonStr = "reproducibility was requested.";
                return forceDlbOffOrBail(dlbState, reasonStr, cr, fplog);
                break;
            /* FIXME: why would we allow this, users should pass consistent command line options, shouldn't they?
             * Or is there a valid use-case for -preod -dlb yes? */
            case edlbsOnUser:
