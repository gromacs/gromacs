...
static void dd_dlb_set_should_check_whether_to_turn_dlb_on(gmx_domdec_t *dd, gmx_bool bValue)
{
    if (dd->comm->dlbState == edlbsOffCanTurnOn)
    {
        ...
        if (bValue == TRUE)
        {
            /* Store the DD count, so we can skip the first nstlist steps */
            dd->comm->ddPartioningCountFirstDlbOff = dd->ddp_count;
        }
    }
}
...
static gmx_bool dd_dlb_get_should_check_whether_to_turn_dlb_on(gmx_domdec_t *dd)
{
    ...
    {
        /* We ignore the first nstlist steps at the start off the run
    ...
    if (comm->bRecordLoad && dd_load_count(comm) > 0)
    {
            ...
            (bVerbose && (ir->nstlist == 0 || nstglobalcomm <= ir->nstlist)))
        {
            ...
            if (dlbIsOn(comm))
            {
                ...
                /* We check more often for turning DLB off then on, since
                 * the DLB can rescale rapidly and we are already synchronizing
                 * often with DLB, so the overhead off the Bcast is not much.
                 */
                const int nddp_chk_dlb = 20;
                if (comm->dlbState == edlbsOnCanTurnOff &&
                    dd->comm->n_load_have % nddp_chk_dlb == nddp_chk_dlb - 1)
                {
                    gmx_bool turnOffDlb;
                    if (DDMASTER(dd))
                    {
                        /* If we the running averaged cycles with DLB are more
                         * the before we turned on DLB, turn off DLB.
                         * We will then check if running without DLB is faster.
                         */
            ...
            else if (bCheckWhetherToTurnDlbOn)
            {
                ...
                if (DDMASTER(dd))
                {
                    bools.turnOffDlbForever = FALSE;
                    bools.turnOnDlb         = FALSE;
                    ...
                    else
                    {
                        ...
                        {
                            bools.turnOnDlb = (dd_force_imb_perf_loss(dd) >= DD_PERF_LOSS_DLB_ON);
                        }
                    }
                }
