...
void change_dd_dlb_max_cutoff(t_commrec *cr, real cutoff)
{
    gmx_domdec_comm_t *comm;
...
void dd_dlb_set_lock(gmx_domdec_t *dd, gmx_bool bValue)
{
    ...
    if (dd->comm->eDLB == edlbAUTO)
    {
        ...
        dd->comm->bCheckWhetherToTurnDlbOn = TRUE;
    }
}
 ...
 * multiple times per step, since it might change comm->bCheckWhetherToTurnDlbOn
 * which is used to avoid collecting the timing data at every repartitioning.
 */
static gmx_bool get_bCheckWhetherToTurnDlbOn(gmx_domdec_t *dd)
{
    ...
    if (comm->eDLB == edlbAUTO && !comm->bDynLoadBal && !dd_dlb_is_locked(dd))
    {
        ...
        bCheckWhetherToTurnDlbOn
            = (comm->bCheckWhetherToTurnDlbOn ||
               comm->n_load_have % nddp_chk_dlb == nddp_chk_dlb - 1);

        comm->bCheckWhetherToTurnDlbOn = FALSE;
    }
    else
    {
        bCheckWhetherToTurnDlbOn = FALSE;
    }

    return bCheckWhetherToTurnDlbOn;
}
                         ...
                         gmx_bool             bVerbose)
{
    ...
    {
        bCheckWhetherToTurnDlbOn = get_bCheckWhetherToTurnDlbOn(dd);
