               ...
               gmx_wallcycle            *wcycle)
{
    ...
    for (int d = 0; d < dd->ndim; d++)
    {
        ...
        for (const gmx_domdec_ind_t &ind : cd->ind)
        {
            ...
            {
                rbuf = as_rvec_array(x.data()) + nat_tot;
            }
            else
            {
                rbuf = as_rvec_array(receiveBufferAccess.buffer.data());
            }
            /* Send and receive the coordinates */
            dd_sendrecv_rvec(dd, d, dddirBackward,
                             as_rvec_array(sendBuffer.data()), ind.nsend[nzone+1],
               ...
               gmx_wallcycle            *wcycle)
{
    ...
    for (int d = dd->ndim-1; d >= 0; d--)
    {
        ...
        for (int p = cd->numPulses() - 1; p >= 0; p--)
        {
            ...
            if (cd->receiveInPlace)
            {
                sbuf = as_rvec_array(f.data()) + nat_tot;
            }
            else
            {
                sbuf  = as_rvec_array(sendBufferAccess.buffer.data());
                int j = 0;
                for (int zone = 0; zone < nzone; zone++)
                {
                    for (int i = ind.cell2at0[zone]; i < ind.cell2at1[zone]; i++)
            ...
            dd_sendrecv_rvec(dd, d, dddirForward,
                             ...
                             as_rvec_array(receiveBuffer.data()), ind.nsend[nzone+1]);
...
void dd_atom_spread_real(gmx_domdec_t *dd, real v[])
{
    ...
    for (int d = 0; d < dd->ndim; d++)
    {
        ...
        for (const gmx_domdec_ind_t &ind : cd->ind)
        {
            ...
            {
                rbuf = as_rvec_array(receiveBufferAccess.buffer.data())[0];
            }
...
void dd_atom_sum_real(gmx_domdec_t *dd, real v[])
{
    ...
    for (int d = dd->ndim-1; d >= 0; d--)
    {
        ...
        for (int p = cd->numPulses() - 1; p >= 0; p--)
        {
             ...
             */
            DDBufferAccess<gmx::RVec>  receiveBufferAccess(comm->rvecBuffer, ind.nsend[nzone + 1]);
            gmx::ArrayRef<real>        receiveBuffer = gmx::arrayRefFromArray(reinterpret_cast<real *>(receiveBufferAccess.buffer.data()), ind.nsend[nzone + 1]);
            nat_tot -= ind.nrecv[nzone + 1];
            ...
            {
                sbuf  = as_rvec_array(sendBufferAccess.buffer.data())[0];
                int j = 0;
                for (int zone = 0; zone < nzone; zone++)
                {
                    for (int i = ind.cell2at0[zone]; i < ind.cell2at1[zone]; i++)
            ...
            dd_sendrecv_real(dd, d, dddirForward,
                             sbuf, ind.nrecv[nzone + 1],
                             receiveBuffer.data(),  ind.nsend[nzone + 1]);
            /* Add the received forces */
            int n = 0;
            for (int g : ind.index)
            {
                           ...
                           gmx::ArrayRef<gmx::RVec>           buf)
{
    if (atomGroups == nullptr)
    {
        ...
        order_vec_cg(sort, as_rvec_array(v.data()), buf);
                        ...
                        std::vector<gmx_cgsort_t>         *sort1)
{
    ...
    sort1->clear();
    sort1->reserve(stationary.size() + moved.size());
    size_t indexStationary = 0;
    size_t indexMoved      = 0;
    while (indexStationary < stationary.size() || indexMoved < moved.size())
    {
        if (indexStationary == stationary.size())
        {
            sort1->push_back(moved[indexMoved++]);
        }
        else if (indexMoved == moved.size())
        {
            sort1->push_back(stationary[indexStationary++]);
        }
        else if (stationary[indexStationary].nsc < moved[indexMoved].nsc ||
                 (stationary[indexStationary].nsc == moved[indexMoved].nsc &&
                  stationary[indexStationary].ind_gl < moved[indexMoved].ind_gl))
        {
            sort1->push_back(stationary[indexStationary++]);
        }
        else
    ...
    if (ncg_home_old >= 0)
    {
        ...
        for (int i = 0; i < dd->ncg_home; i++)
        {
            ...
            if (a[i] < movedValue)
            {
                ...
                if (i >= ncg_home_old || a[i] != sort->sort[i].nsc)
                {
                    ...
                    sort->moved.push_back(entry);
                }
                else
                {
                    /* This cg did not move */
                    sort->stationary.push_back(entry);
                }
            }
        }
        ...
        if (debug)
        {
            fprintf(debug, "ordered sort cgs: stationary %zu moved %zu\n",
                    sort->stationary.size(), sort->moved.size());
        }
        /* Sort efficiently */
        orderedSort(sort->stationary, sort->moved, &sort->sort);
    }
    else
    {
        std::vector<gmx_cgsort_t> &cgsort   = sort->sort;
        cgsort.clear();
                          ...
                          int ncg_home_old)
{
    ...
    switch (fr->cutoff_scheme)
    {
        case ecutsGROUP:
            ...
            break;
        case ecutsVERLET:
            dd_sort_order_nbnxn(fr, &sort->sort);
            break;
        default:
            gmx_incons("unimplemented");
    }
    ...
    /* Reorder the state */
    gmx::ArrayRef<const gmx_cgsort_t> cgsort = sort->sort;
    ...
    {
        /* Reorder cgcm */
        order_vec_cg(cgsort, cgcm, rvecBuffer.buffer);
    }
