                          ...
                          const char *where)
{
    ...
    {
        fprintf(stderr, "DD rank %d, %s: In bLocalCG %zu cgs are marked as local, whereas there are %zu\n", dd->rank, where, ngl, dd->globalAtomGroupIndices.size());
        nerr++;
    }
...
static gmx_domdec_comm_t *init_dd_comm()
{
    ...
    // \todo Replace this temporary clearing solution by proper initialization when organizing gmx_domdec_comm_t into sub-structs.
                   ...
                   dd_comm_setup_work_t *work)
{
    ...
    GMX_ASSERT(ibuf.size() == static_cast<size_t>(nsend), "Atom group buffer size should match nsend");
    GMX_ASSERT(vbuf.size() == static_cast<size_t>(nsend), "Position buffer size should match nsend");
...
#pragma omp parallel for num_threads(numThreads) schedule(static)
            ...
            ind->nsend[nzone + 1] = numAtomsToSend;
                         ...
                         gmx_bool             bVerbose)
{
    ...
    else if (state_local->ddp_count != dd->ddp_count)
    {
        ...
        /* Restore the atoms group indices from state_local */
