    ...
    if (dd->nnodes > 1 && comm->load_sum > 0)
    {
        ...
        switch (dd->comm->dlbState)
        {
            ...
            case edlbsOffForever:
                /* Currectly this can happen due to performance loss observed or cell size
                 * limitations or incompatibility with other settings observed during
                 * determineInitialDlbState(). */
                dlbStateStr = "DLB got disabled during the run due to observed performance loss or other limitations.";
                break;
