...
static void make_dd_communicators(FILE *fplog, t_commrec *cr, gmx_domdec_t *dd,
                                  int dd_node_order)
{
    gmx_domdec_comm_t *comm;
    int                CartReorder;
    ...
    if (cr->duty & DUTY_PP)
    {
        ...
        make_pp_communicator(fplog, cr, dd, CartReorder);
    }
    else
    {
...
static gmx_domdec_t *
init_domain_decomposition_setup(FILE *fplog, t_commrec *cr,
                                unsigned long Flags,
                                ...
                                int *npme_x, int *npme_y)
{
    ...
    else
    {
        ...
        if (dd->nc[XX] == 0 && comm->eDLB == edlbAUTO)
        {
            if (fplog)
            {
                fprintf(fplog, "The cell size with DLB is too large to fit %d ranks, trying again with dlb off\n",
                        cr->nnodes-cr->npmenodes);
            }
            /* Try to choose a grid again, now with DLB off */
            comm->eDLB = edlbNO;
            limit      = dd_choose_grid(fplog, cr, dd, ir, mtop, box, ddbox,
                                        nPmeRanks,
                                        comm->eDLB != edlbNO, dlb_scale,
...
gmx_domdec_t *
init_domain_decomposition(FILE *fplog, t_commrec *cr,
                          unsigned long Flags,
                          ivec nc, int nPmeRanks,
                          int dd_node_order,
                          real comm_distance_min, real rconstr,
                          ...
                          int *npme_x, int *npme_y)
{
    ...
    dd = init_domain_decomposition_setup(fplog, cr, Flags,
                                         nc, nPmeRanks,
    ...
    make_dd_communicators(fplog, cr, dd, dd_node_order);

    if (cr->duty & DUTY_PP)
    {
        set_ddgrid_parameters(fplog, dd, dlb_scale, ir, ddbox);
