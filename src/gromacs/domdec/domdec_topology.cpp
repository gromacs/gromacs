...
static void check_bonded_distance(real r2, int ftype, int a1, int a2,
                    ...
                    for (ai = 0; ai < nral; ai++)
                    {
                        ...
                        for (aj = 0; aj < nral; aj++)
                        {
                            ...
                            if (cgi != cgj)
                            {
                                ...
                                check_bonded_distance(rij2, ftype,
                                                      il->iatoms[i+1+ai],
                                                      il->iatoms[i+1+aj],
                                                      (nral == 2) ? bd_2b : bd_mb);
                            }
        ...
        for (ai = 0; ai < excls->nr; ai++)
        {
            ...
            for (j = excls->index[ai]; j < excls->index[ai+1]; j++)
            {
                ...
                if (cgi != cgj)
                {
                    ...
                    check_bonded_distance(rij2, -1, ai, excls->a[j], bd_2b);
                }
            }
        }
    }
                ...
                for (int i = 0; i < il->nr; i += 1+nral)
                {
                    for (int ai = 0; ai < nral; ai++)
                    {
                        ...
                        for (int aj = 0; aj < nral; aj++)
                        {
                            ...
                            check_bonded_distance(rij2, ftype, atom_i, atom_j,
                                                  (nral == 2) ? bd_2b : bd_mb);
                        }
                    }
                }
    ...
    for (mb = 0; mb < mtop->nmolblock; mb++)
    {
        ...
        else
        {
            ...
            for (mol = 0; mol < molb->nmol; mol++)
            {
                ...
                check_bonded_distance(bd_mol_2b.r2, bd_mol_2b.ftype,
                                      at_offset + bd_mol_2b.a1,
                                      at_offset + bd_mol_2b.a2,
                                      &bd_2b);
                check_bonded_distance(bd_mol_mb.r2, bd_mol_mb.ftype,
