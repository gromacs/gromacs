...
static void makeLocalAtomGroupsFromAtoms(gmx_domdec_t *dd)
{
    ...
    {
        for (int a = atomGroups.start(g); a < atomGroups.end(g); a++)
        {
...
{
    int a0 = dd->atomGroups().start(zones->cg_range[iz]);
    int a1 = dd->atomGroups().start(zones->cg_range[iz + 1]);
                                   ...
                                   int cg_start, int cg_end)
{
    ...
    int jla0 = dd->atomGroups().start(zones->izone[iz].jcg0);
    int jla1 = dd->atomGroups().start(zones->izone[iz].jcg1);

    n_excl_at_max = dd->reverse_top->n_excl_at_max;

    /* We set the end index, but note that we might not start at zero here */
    lexcls->nr = dd->atomGroups().start(cg_end);
    ...
    for (int cg = cg_start; cg < cg_end; cg++)
    {
        ...
        int la0 = dd->atomGroups().start(cg);
        int la1 = dd->atomGroups().end(cg);
...
static void check_exclusions_alloc(gmx_domdec_t *dd, gmx_domdec_zones_t *zones,
                                   t_blocka *lexcls)
{
    int nr = dd->atomGroups().start(zones->izone[zones->nizone - 1].cg1);
...
static void finish_local_exclusions(gmx_domdec_t *dd, gmx_domdec_zones_t *zones,
                                    t_blocka *lexcls)
{
    lexcls->nr = dd->atomGroups().start(zones->izone[zones->nizone - 1].cg1);
    ...
    if (dd->n_intercg_excl == 0)
    {
         ...
         */
        int la0 = dd->atomGroups().start(zones->izone[0].cg1);
         ...
         */
        lexcls->nr = dd->atomGroups().start(zones->izone[0].cg1);
    }
}
            ...
            try
            {
                ...
                rt->th_work[thread].nbonded =
                    make_bondeds_zone(dd, zones,
                                      ...
                                      izone,
                                      dd->atomGroups().start(cg0t),
                                      dd->atomGroups().start(cg1t));
