...
{
    int a0 = dd->atomGroups().block(zones->cg_range[iz]).begin();
    int a1 = dd->atomGroups().block(zones->cg_range[iz + 1]).begin();
                                   ...
                                   int cg_start, int cg_end)
{
    ...
    int jla0 = dd->atomGroups().block(zones->izone[iz].jcg0).begin();
    int jla1 = dd->atomGroups().block(zones->izone[iz].jcg1).begin();

    n_excl_at_max = dd->reverse_top->n_excl_at_max;

    /* We set the end index, but note that we might not start at zero here */
    lexcls->nr = dd->atomGroups().block(cg_end).begin();
    ...
    for (int cg = cg_start; cg < cg_end; cg++)
    {
        ...
        int la0 = dd->atomGroups().block(cg).begin();
        int la1 = dd->atomGroups().block(cg).end();
...
static void check_exclusions_alloc(gmx_domdec_t *dd, gmx_domdec_zones_t *zones,
                                   t_blocka *lexcls)
{
    int nr = dd->atomGroups().block(zones->izone[zones->nizone - 1].cg1).begin();
...
static void finish_local_exclusions(gmx_domdec_t *dd, gmx_domdec_zones_t *zones,
                                    t_blocka *lexcls)
{
    lexcls->nr = dd->atomGroups().block(zones->izone[zones->nizone - 1].cg1).begin();
    ...
    if (dd->n_intercg_excl == 0)
    {
         ...
         */
        int la0 = dd->atomGroups().block(zones->izone[0].cg1).begin();
         ...
         */
        lexcls->nr = dd->atomGroups().block(zones->izone[0].cg1).begin();
    }
}
            ...
            try
            {
                ...
                rt->th_work[thread].nbonded =
                    make_bondeds_zone(dd, zones,
                                      ...
                                      izone,
                                      dd->atomGroups().block(cg0t).begin(),
                                      dd->atomGroups().block(cg1t).begin());
