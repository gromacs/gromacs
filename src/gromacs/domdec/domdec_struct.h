...
typedef struct
{
    int   *indexMap     = nullptr; //map of indices to be sent from this rank
    int   *indexMap_d   = nullptr; //device copy of above
    int    indexMapSize = 0;       //size of index map
    float *sendBuf_d    = nullptr; //device buffer for sending packed data
    int    sendRank     = 0;       //rank to send data to
    int    recvRank     = 0;       //rank to recv data from
    int    copysize_s   = 0;       //send copy size from this rank
    int    copysize_r   = 0;       //recv copy size to this rank
    int    recvOffset   = 0;       //offset for data recieved by this rank
    bool   bPBC         = false;   //Periodic Boundary Conditions for this rank
    float  xshift[3];              //X shift values
    float* xshift_d;               //device copy of above
} ddGpu_t;
