...
/* domdec_gpu host-side structure */
typedef struct
{
    int   *h_indexMap     = nullptr; //map of indices to be sent from this rank
    int   *d_indexMap     = nullptr; //device copy of above
    int    indexMapSize   = 0;       //size of index map
    float *d_sendBuf      = nullptr; //device buffer for sending packed data
    int    sendRank       = 0;       //rank to send data to
    int    recvRank       = 0;       //rank to recv data from
    int    copySizeSend   = 0;       //send copy size from this rank
    int    copySizeRecv   = 0;       //recv copy size to this rank
    int    recvOffset     = 0;       //offset for data recieved by this rank
    bool   usePBC         = false;   //Periodic Boundary Conditions for this rank
    float  h_xshift[3];              //X shift values
    float* d_xshift;                 //device copy of above
} ddGpu_t;
...
struct gmx_domdec_t { //NOLINT(clang-analyzer-optin.performance.Padding)
    ...
    /* info required for GPU halo exchange comms */
    ddGpu_t ddGpu;
};
