 ...
 */
static void set_domain_comm_nc(domain_comm_t *domain_comm, int ncx, int ncy)
{
    domain_comm->ncx = ncx;
    domain_comm->ncy = ncy;
    if (domain_comm->ncx*domain_comm->ncy > domain_comm->column_nalloc)
    {
        domain_comm->column_nalloc = over_alloc_dd(domain_comm->ncx*domain_comm->ncy);
                  ...
                  domain_comm_t           *send)
{
    ...
    set_domain_comm_nc(send, ncx, ncy);
                       ...
                       domain_comm_t  *recv)
{
    ...
    if (debug)
    {
        fprintf(debug, "For domain %d, receiving %d atoms\n",
                domain_ind, recv->natoms);
    }

    /* Communicate corner0, corner1, sx, sy */
    dd_sendrecv_domain<real>(dd, domain_ind, dddirBackward,
                             &send->corner0[0], 8,
                             &recv->corner0[0], 8);
    ...
    set_domain_comm_nc(recv, recv->ncx, recv->ncy);
    ...
    for (int zone = 1; zone < zones->n; zone++)
    {
        ...
        do
        {
            ...
            dd_sendrecv_domain<int>(dd, dom_ind, dddirBackward,
                                    ...
                                    dd->index_gl + recv->offset, recv->natoms);

            /* Communicate the atom info flags, cheaper than extracting from mtop */
            dd_sendrecv_domain<int>(dd, dom_ind, dddirBackward,
                                    send->atominfo, send->natoms,
                                    fr->cginfo + recv->offset, recv->natoms);
             ...
             */
            if (!bCalledSetGrid || recv->natoms > 0)
            {
                nbnxn_set_grid_parameters(fr->nbv->nbs, zone,
                                          recv->offset, recv->natoms,
                                          recv->ncx, recv->ncy,
                                          recv->corner0, recv->corner1,
    ...
    if (debug)
    {
        ...
        for (int ind = 1; ind < comm->ndhalo[0]*comm->ndhalo[1]*comm->ndhalo[2]; ind++)
        {
            fprintf(debug, "domain %2d #atoms: %5d\n",
                    ind, comm->domain_forward[ind].natoms);
        }
    }
