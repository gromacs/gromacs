                             ...
                             gmx_bool *bCornersDiffer)
{
    ...
    *bCornersDiffer = FALSE;
    ...
    /* TODO: The same code is in domdec.cpp, we should make this a function */
        ...
        box[ZZ][1 - dd->dim[0]] != 0)
    {
        ...
        corner_2b[dim2]   -= corner_2b[ZZ]*box[ZZ][dim2]/box[ZZ][ZZ];
        corner_mb[dim2]   -= corner_mb[ZZ]*box[ZZ][dim2]/box[ZZ][ZZ];
        corner_zone[dim2] -= corner_zone[ZZ]*box[ZZ][dim2]/box[ZZ][ZZ];
    }
    ...
    for (dim = 0; dim < dd->npbcdim; dim++)
    {
        ...
        {
            corner_2b[dim2]   += corner_2b[dim]*box[dim][dim2]/box[dim][dim];
            corner_mb[dim2]   += corner_mb[dim]*box[dim][dim2]/box[dim][dim];
            corner_zone[dim2] += corner_zone[dim]*box[dim][dim2]/box[dim][dim];
        }
    }
                                     ...
                                     zone_dim_bounds_t *b)
{
    ...
    {
        /* NOTE: With only 2 domains in a dimensions we only need fw or bw */
        ...
        /* Loop over the central elements of the two received entries */
        for (ind = c - cn; ind < c + 2*cn; ind += 2*cn)
        {
            b[c].max_f0 = std::max(b[c].max_f0, b[ind].max_f0);
            b[c].min_f1 = std::min(b[c].min_f1, b[ind].min_f1);
        }
