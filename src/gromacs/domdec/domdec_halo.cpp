    ...
    if (comm->bInterCGBondeds)
    {
        ...
        for (dim_ind = 0; dim_ind < dd->ndim; dim_ind++)
        {
            ...
            if (dim < dd->npbcdim && zone_shift_dim[dim] && dd->nc[dim] == 2)
            {
                ...
                {
                    real cellThickness, rCheckBondedMargin, commDist = 0;
                              ...
                              gmx_bool bCellsChanged)
{
     ...
     */
    rc2_nonbonded = gmx::square(comm->cutoff);
    rc2_bonded    = gmx::square(comm->cutoff_mbody);
...
#pragma omp parallel for num_threads(nthread) schedule(static, 1)
    for (zone_ind = 1; zone_ind < zones->n; zone_ind++)
    {
        int          zone;
        zone_comm_t *send;
