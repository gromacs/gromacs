                      ...
                      T gmx_unused *buf_r, int gmx_unused n_r)
{
    ...
    {
        rank_s = dd->comm->zone_backward[zone].rank;
        rank_r = dd->comm->zone_forward[zone].rank;
    }
    else
    {
        rank_s = dd->comm->zone_forward[zone].rank;
        rank_r = dd->comm->zone_backward[zone].rank;
    }
...
{
    const zone_comm_t *send      = &dd->comm->zone_backward[zone];
                                  ...
                                  rvec f[], rvec fshift[])
{
    ...
    recv = &dd->comm->zone_backward[zone];
...
void dd_halo_initiate_recv_f(gmx_domdec_t *dd)
{
    ...
    for (zone = 1; zone < dd->comm->zones.n; zone++)
    {
         ...
         */
        dd_irecv_zone_recv(dd,
                           &dd->comm->zone_backward[zone],
                           dd->comm->zone_backward[zone].at_buf,
                           halo_tag_F,
                           &zmpi->recv);
    }
}
...
void dd_halo_initiate_send_f(gmx_domdec_t *dd, rvec f[])
{
    ...
    for (zone = 1; zone < dd->comm->zones.n; zone++)
    {
        ...
        send = &dd->comm->zone_forward[zone];
...
void dd_halo_complete_recv_f(gmx_domdec_t *dd, rvec f[], rvec fshift[])
{
    ...
    for (int zone = 1; zone < dd->comm->zones.n; zone++)
    {
        const rvec *f_buf = dd->comm->zone_backward[zone].at_buf;
...
corner_bb_distance2_rect(const ivec        zone_shift_dim,
...
static gmx_inline real
corner_bb_distance2_tric(const ivec        zone_shift_dim,
                         const rvec       *normal,
 ...
 */
static gmx_inline real
corner_bb_distance2(const ivec        zone_shift_dim,
                    gmx_bool          bTriclinic,
                    const rvec       *normal,
                    const ivec        sumSquares,
                    const real        corner[DIM],
                    const nbnxn_bb_t *bb)
{
    if (bTriclinic)
    {
        return corner_bb_distance2_tric(zone_shift_dim, normal, sumSquares,
                                        corner, bb);
    }
    else
    {
        return corner_bb_distance2_rect(zone_shift_dim, corner, bb);
    }
}
...
static void set_zone_comm_nc(zone_comm_t *zone_comm, int ncx, int ncy)
{
    zone_comm->ncx = ncx;
    zone_comm->ncy = ncy;
    if (zone_comm->ncx*zone_comm->ncy > zone_comm->column_nalloc)
    {
        zone_comm->column_nalloc = over_alloc_dd(zone_comm->ncx*zone_comm->ncy);
        srenew(zone_comm->column_natoms, zone_comm->column_nalloc);
        /* We usually need less space, but this is negligible on the total */
        srenew(zone_comm->column_atom_range, zone_comm->column_nalloc*2);
    }
}

/* Determine the corner for 2-body and multi-body interaction atom
                             ...
                             gmx_bool *bCornersDiffer)
{
    ...
    *bCornersDiffer = FALSE;
    ...
    {
        real corner_2body_dim, corner_multibody_dim, corner_zone_dim;

        /* This is the zone corner (simple, no staggering) */
        corner_zone_dim = dd->comm->cell_x0[dim];
        ...
        corner_zone[dim]      = corner_zone_dim;
    }
...
static void get_sumSquares(const ivec  zone_shift_dim,
                           const rvec *normal,
                           ivec        sumSquares)
{
                ...
                zone_comm_t             *send)
{
    ...
    for (int cxy = 0; cxy < ncx*ncy; cxy++)
    {
        ...
        real r2  = corner_bb_distance2(zone_shift_dim, bTriclinic,
                                       normal, sumSquares,
                                       corner_2body, &column_bb);
        real rb2 = 0;
        if (bDistMultiBody)
        {
            rb2  = corner_bb_distance2(zone_shift_dim, bTriclinic,
                                       normal, sumSquares,
                                       corner_multibody, &column_bb);
        }
        ...
        do
        {
            ...
            r2      = corner_bb_distance2(zone_shift_dim, bTriclinic,
                                          normal, sumSquares,
                                          corner_2body, bb_ptr);
            if (bDistMultiBody)
            {
        ...
        if ((bTriclinic || bBondComm) && bb_last > bb_first)
        {
            ...
            do
            {
                ...
                real r2, rb2;
                r2 = corner_bb_distance2(zone_shift_dim, bTriclinic, normal, sumSquares,
                                         corner_2body, bb_ptr);
                if (bDistMultiBody)
                {
                    rb2 = corner_bb_distance2(zone_shift_dim, bTriclinic, normal, sumSquares,
                                              corner_multibody, bb_ptr);
                }
        ...
        if (bb_last >= bb_first)
        {
             ...
             */
            int at_start = atom_start + bb_first*bb_natoms;
            int at_end   = atom_start + std::min((bb_last + 1)*bb_natoms,
                                                 column_natoms);

            /* Set the grid column atom count in the send buffer */
            send->column_natoms[cxy] = at_end - at_start;
            ...
            if (bb != NULL)
            {
                bb_z_max = std::max(bb_z_max, bb[bb_last].upper[ZZ]);
            }
            else
            {
                bb_z_max = std::max(bb_z_max, bb_z_only[bb_last*2 + 1]);
            }
        }
    }
    ...
    if (comm->bInterCGBondeds)
    {
        ...
        {
            int dim = dd->dim[dim_ind];
            if (dim < dd->npbcdim && zone_shift_dim[dim] && dd->nc[dim] == 2)
            {
                ...
                else
                {
                    ...
                    {
                        case XX:
                            commDist = send->ncx*send->size_x;
                            break;
                        case YY:
                            commDist = (cy_max + 1)*send->size_y;
                            break;
                        case ZZ:
                            commDist = bb_z_max - corner_zone[ZZ];
                            break;
                    }
                     ...
                     zone_comm_t  *recv)
{
    ...
    dd_sendrecv_zone<real>(dd, zone, dddirBackward,
                           &send->corner0[0], 8,
                           &recv->corner0[0], 8);
    ...
    dd_sendrecv_zone<int>(dd, zone, dddirBackward,
                          send->column_natoms, send->ncx*send->ncy,
                          recv->column_natoms, recv->ncx*recv->ncy);
}
...
#pragma omp parallel for num_threads(nthread) schedule(static, 1)
    for (int zone_ind = 1; zone_ind < zones->n; zone_ind++)
    {
        ...
        zone_comm_t *send = &comm->zone_backward[zone];
    ...
    {
        zone_comm_t *send = &comm->zone_backward[zone];
        zone_comm_t *recv = &comm->zone_forward[zone];
        ...
        zones->at_range[zone+1] = zones->at_range[zone] + recv->natoms;
    ...
    if (debug)
    {
        ...
        {
            fprintf(debug, " %d", comm->zone_forward[c].natoms);
        }
        fprintf(debug, "\n");
    }
