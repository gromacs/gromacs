...
TEST(LocalAtomSetManager, CanAddEmptyLocalAtomSet)
{
    ...
    LocalAtomSet           emptyGroup(manager.add(0, nullptr));
    const std::vector<int> emptyIntVector;
    ASSERT_THAT(emptyGroup.globalIndex(), testing::ContainerEq(emptyIntVector));
}
...
TEST(LocalAtomSetManager, CanAddandReadLocalAtomSetIndices)
{
    ...
    LocalAtomSet        newGroup(manager.add(index.size(), index.data()));
