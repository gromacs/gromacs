...
/*! \libinternal \file
   ...
   The class is derived from Erik Lindahl's gmx::alignedAllocator
 ...
 */
#ifndef GMX_UTILITY_DATASTRUCTURES_DETAILED_ALIGNED_ALLOCATOR_H
#define GMX_UTILITY_DATASTRUCTURES_DETAILED_ALIGNED_ALLOCATOR_H
...
/*! \brief Allocate aligned memory,
           version with adjustable padding and alignment

    \param[in]   bytes       Amount of memory (bytes) to allocate. It is valid to ask for
    ...
    \param[in]   padding     additional padding in bytes

   \return Valid pointer if the allocation worked, otherwise nullptr.

   The hardcoded standard settings from the version in utilities/alignedallocator.cpp
...
/*! \libinternal
    ...
    \tparam    alignment    alignment in byte
    \tparam    padding      padding   in byte
   ...
   \ingroup module_utility
 */
template <class T, unsigned int Alignment, unsigned int Padding>
class DetailedAlignedAllocator
{
    public:
        ...
        struct rebind
        {
            typedef DetailedAlignedAllocator<U, Alignment, Padding> other; //!< Align class U with our alignment and padding
        };
         ...
         */
        template <class U>
        explicit DetailedAlignedAllocator(const DetailedAlignedAllocator<U, Alignment, Padding> &) {}
        ...
        {
            void *p = internal::alignedMalloc(n*sizeof(T), Alignment, Padding);
         ...
         */
        template<class T2, unsigned int A2, unsigned int P2>
        bool
        operator==(const DetailedAlignedAllocator<T2, A2, P2> &gmx_unused rhs) const
        {
            return (std::is_same<T, T2>::value && Alignment == A2 && Padding == P2); GMX_UNUSED_VALUE(rhs);
        }
        ...
        template<class T2, unsigned int A2, unsigned int P2>
        bool
        operator!=(const DetailedAlignedAllocator<T2, A2, P2> &rhs) const { return !operator==(rhs); }

};

//! alias of DetailedAlignedAllocator with standard alignment and padding,
//! should do the same as the current AlignedAllocator
//! \tparam   T   data type to allocate storage for
template<class T> using DefaultAlignedAllocator = DetailedAlignedAllocator<T, 128, 128>;

}      // end namespace gmx

#endif // GMX_UTILITY_DATASTRUCTURES_DETAILED_ALIGNED_ALLOCATOR_H
