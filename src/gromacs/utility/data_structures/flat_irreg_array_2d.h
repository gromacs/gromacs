...
class FlatIrregArray2D
{
    ...
    public:
         ...
         */
        FlatIrregArray2D()
            : first1_(0), first2_(0), last1_(-1), last2_(-1), nelements_(0), isIrreg_(false), arr_(nullptr) {}
        ...
        FlatIrregArray2D(const std::vector<size_type, AForeign> &sizes)
            : first1_(0), first2_(0), last1_(-1), last2_(-1), nelements_(0), isIrreg_(false), arr_(nullptr)
        {
            allocateArray(0, sizes.size() - 1, sizes);
        }
        /*! \brief  constructor with memory allocation and content initialization
        ...
        FlatIrregArray2D(const std::initializer_list<std::initializer_list<value_type> > &ini)
            : first1_(0), first2_(0), last1_(-1), last2_(-1), nelements_(0), isIrreg_(true), arr_(nullptr)
        {
        ...
        {
            initArray(0, sizes.size() - 1, IrregArray1D<size_type>(sizes));
        }
        /*! \brief  allocate memory for an FlatIrregArray2D
        ...
        {
            initArray(sizes.getFirst1(), sizes.getLast1(), IrregArray1D<size_type>(sizes));
        }
        /*! \brief  allocate memory for an FlatIrregArray2D
    ...
    private:
        ...
        void allocateArray(const index_type x1, const index_type x2, const IrregArray1D<size_type, AForeign> &sizes)
        {
            ...
            {
                // zero length is OK, but size_type may be signed
