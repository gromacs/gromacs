...
/*! \file
   ...
   \brief
    ...
    are truncated to a maximum length defined via max_entry_size_.
...
class Symbuf
{
    ...
    private:
        ...
        //! \param[out]   mem   memory occupied by *this buffer after allocation
        void allocSymbuf(size_t * const mem)
        {
            try // allocate memory for the weak_ptrs
            {
                ...
                *mem = sizeof(Symbuf) + this_array_size;
            }
            catch (std::bad_alloc &)
            {
        ...
        bool containsEntry(const char * const s) const {return (getEntry(s) != nullptr); }
        ...
        //! \param[out]   topology   string, map of shared pointers from distinct shared_ptr sets referencing the string,
        //!               number of active shared_ptr instances of the respective set
        void getTopology(std::map<std::string, std::map<std::shared_ptr<Symbol>*, size_t> > &topology) const;
};
...
class Symtab
{
    public:
        //! the default constructor
        Symtab() : bufsize_(81), max_entry_size_(1024), nbuf_(0),
                   nr_(0), symbuf_(), mem_(sizeof(*this)), n_orphan_entries_(0)
        {
        }
        ...
        Symtab(const size_t &bs, const size_t &mes) : bufsize_(bs), max_entry_size_(mes), nbuf_(0),
                                                      nr_(0), symbuf_(), mem_(sizeof(*this)), n_orphan_entries_(0)
        {
        }
        //! explicitly state that no copying is allowed
        ...
        inline unsigned int getMaxEntrySize() const {return max_entry_size_; }
        ...
        bool containsEntry(const char * const s) const
        {
            ...
            return (nonconstGetEntry(s) != nullptr);
        }
        ...
        {return nonconstRenameEntry(before, after) != nullptr; }
    ...
    private:
        ...
        std::shared_ptr<std::shared_ptr<Symbol> > constructEntry(const char * const in, const size_t maxlen, size_t *tmp_mem);
        ...
        //! Symtab is notified whenever a shared_string called its destructor
        //! (not anymore at the moment, seemed unnecessary for normal usage)
        ...
        void trackOrphanEntries()
        {
            if (n_orphan_entries_.fetch_add((size_t)1, std::memory_order_seq_cst) > bufsize_)
            {
                n_orphan_entries_.store((size_t)0, std::memory_order_seq_cst);
                clean();
            }
        }
        ...
        const unsigned int        max_entry_size_;
        ...
        std::atomic<size_t>       n_orphan_entries_;
