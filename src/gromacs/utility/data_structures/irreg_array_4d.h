...
/*! \file
    ...
    \date Feb 2015
 */
#ifndef GMX_UTILITY_IRREG_ARRAY_4D_H
#define GMX_UTILITY_IRREG_ARRAY_4D_H
...
class IrregArray4D
{
    ...
    private:
        ...
        //! dimension1,2,3-specific end_ing index of dimension 4
    ...
    public:
         ...
         */
        IrregArray4D()
            : begin1_(0), begin2_(0), begin3_(0), begin4_(0), end1_(0), end2_(0), end3_(0), end4_(0), is_irreg_(false), arr_(nullptr)
        {
        }
        ...
        IrregArray4D(const index_type x1, const index_type x2, const size_type *sizes1, const IrregArray2D<size_type> &sizes2, const IrregArray3D<size_type> &sizes3)
            : begin1_(0), begin2_(0), begin3_(0), begin4_(0), end1_(0), end2_(0), end3_(0), end4_(0), is_irreg_(true), arr_(nullptr)
        {
        ...
        IrregArray4D(const index_type x1, const index_type x2, const size_type *sizes1, const index_type z1, const index_type z2, const size_type *sizes2)
            : begin1_(x1), begin2_(1), begin3_(z1), begin4_(1), end1_(0), end2_(0), end3_(0), end4_(0), is_irreg_(true), arr_(nullptr)
        {
        ...
        IrregArray4D(const index_type s1, const index_type s2, const index_type s3, const index_type s4, const index_type d1, const size_type *sizes1, const IrregArray2D<size_type> &sizes2, const value_type &ini)
            : begin1_(s1), begin2_(s2), begin3_(s3), begin4_(s4), end1_(s1+d1-1), end2_(0), end3_(0), end4_(0), is_irreg_(true), arr_(nullptr)
        {
        ...
        IrregArray4D(const index_type x1, const index_type x2, const IrregArray1D<size_type> &sizes1, const index_type z1, const index_type z2, const IrregArray1D<size_type> &sizes2, const value_type &ini)
            : begin1_(x1), begin2_(x1), begin3_(z1), begin4_(z1), end1_(x2), end2_(0), end3_(z2), end4_(0), is_irreg_(true), arr_(nullptr)
        {
        ...
        void initArray(const IrregArray4D<T_foreign, A_foreign> &ini)
        {
            if (ini.is_irreg_)
            {
                initArray(ini.begin1_, ini.end1_, ini.irreg_, ini.irreg_mat_, ini.irreg_ten_);
            }
            else
            {
                initArray(ini.begin1_, ini.end1_, ini.begin2_, ini.end2_, ini.begin3_, ini.end3_, ini.begin4_, ini.end4_);
            }
            for (index_type h1 = begin1_; h1 <= end1_; h1++)
            {
                for (index_type h2 = begin2_; h2 <= getEnd2(h1); h2++)
                {
                    for (index_type h3 = begin3_; h3 <= getEnd3(h1, h2); h3++)
                    {
                        for (index_type h4 = begin4_; h4 <= getEnd4(h1, h2, h3); h4++)
                        {
        ...
        void initArray(const index_type x1, const index_type x2, const IrregArray1D<size_type> &sizes1, const IrregArray2D<size_type> &sizes2, const IrregArray3D<size_type> &sizes3)
        {
            ...
            irreg_mat_.initArray(x1, x2, irreg_);
            for (index_type i = getBegin1(); i <= x2; i++)
            {
            ...
            irreg_ten_.initArray(x1, x2, irreg_, irreg_mat_);
            for (index_type i = getBegin1(); i <= x2; i++)
            {
                for (index_type j = getBegin2(); j <= getEnd2(i); j++)
                {
            ...
            allocateArray(begin1_, begin2_, begin3_, begin4_, irreg_, irreg_mat_, irreg_ten_);
        }
        /*! \brief general constructor helper function for irregularly shaped matrices and all special cases
        ...
        void initArray(const index_type x1, const index_type x2, const IrregArray1D<size_type> &sizes1, const index_type z1, const index_type z2, const IrregArray1D<size_type> &sizes2)
        {
            ...
            irreg_mat_.initArray(begin1_, end1_, sizes1, (size_type)0);
            ...
            irreg_ten_.initArray(begin1_, end1_, irreg_, irreg_mat_, (size_type)0);
            for (index_type i = begin1_; i <= end1_; i++)
            {
                for (index_type j = begin2_; j <= getEnd2(i); j++)
                {
        ...
        {
            return (is_irreg_ ? irreg_ten_(x, y, z) : (end4_ - begin4_ + 1));
        }
        ...
        {
            return (is_irreg_ ? (irreg_ten_(x, y, z) + begin4_ - 1) : end4_);
        }
        ...
        void allocateArray(const index_type s1, const index_type s2, const index_type s3, const index_type s4, const IrregArray1D<size_type> &sizes1, const IrregArray2D<size_type> &sizes2, const IrregArray3D<size_type> &sizes3)
        {
            try
            {
                arr_  = p3_allocator_.allocate(sizes1.getLength1());
                arr_ -= s1;
                for (index_type i = s1; i <= sizes1.getEnd1(); ++i)
                {
                    arr_[i]  = p2_allocator_.allocate(sizes1[i]);
                    arr_[i] -= s2;
                }
                for (index_type i = s1; i <= sizes1.getEnd1(); i++)
                {
                    ...
                    {
                        arr_[i][j]  = p_allocator_.allocate(sizes2(i, j));
                        arr_[i][j] -= s3;
                    }
                }
                for (index_type i = s1; i <= sizes1.getEnd1(); i++)
                {
                    for (index_type j = s2; j <= (s2 + static_cast<index_type>(sizes1[i]) - 1); ++j)
                    {
                        ...
                        {
                            arr_[i][j][t]  = allocator_.allocate(sizes3(i, j, t));
                            arr_[i][j][t] -= s4;
                        }
                    }
                }
        ...
        void deallocateArray()
        {
            if (arr_ != nullptr)
            {
                ...
                else
                {
                    ...
                    arr_[begin1_][begin2_][begin3_] += begin4_;
                    allocator_.deallocate(arr_[begin1_][begin2_][begin3_], d1*d2*d3*d4);
                    arr_[begin1_][begin2_] += begin3_;
                    p_allocator_.deallocate(arr_[begin1_][begin2_], d1*d2*d3);
                    arr_[begin1_] += begin2_;
                    p2_allocator_.deallocate(arr_[begin1_], d1*d2);
                    arr_ += begin1_;
