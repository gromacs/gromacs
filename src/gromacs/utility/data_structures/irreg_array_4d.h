...
class IrregArray4D
{
    ...
    public:
         ...
         */
        IrregArray4D()
            : first1_(0), first2_(0), first3_(0), first4_(0),
              last1_(-1), last2_(-1), last3_(-1), last4_(-1),
...
// Trick the clang_static_analyzer by a dummy call to an init function that it understands
         ...
         */
        IrregArray4D(const index_type x1, const index_type x2,
                     const index_type y1, const index_type y2,
                     const index_type z1, const index_type z2,
                     const index_type k1, const index_type k2)
            : IrregArray4D()
        {
        ...
        IrregArray4D(const index_type x1, const index_type x2,
                     const IrregArray1D<size_type, AForeign> &sizes1,
                     const IrregArray2D<size_type, AForeign> &sizes2,
                     const IrregArray3D<size_type, AForeign> &sizes3)
            : IrregArray4D()
        {
        ...
        IrregArray4D(const index_type x1, const index_type x2,
                     const IrregArray1D<size_type, AForeign> &sizes1,
                     const index_type z1, const index_type z2,
                     const IrregArray1D<size_type, AForeign> &sizes2,
                     const value_type &ini)
            : IrregArray4D()
        {
            ...
            : IrregArray4D()
        {
            ...
            if (isIrreg_)
            {
                ...
                IrregArray2D<size_type, allocator_size_type> irregArr2(0, static_cast<index_type>(ini.size()) - 1,
                                                                       irregArr1, 0);
                i = 0;
                for (l4_ilist_ptr_type iPtrL4 = ini.begin(); iPtrL4 != ini.end(); ++iPtrL4)
                {
                ...
                IrregArray3D<size_type, allocator_size_type> irregArr3(0, static_cast<index_type>(ini.size()) - 1,
                                                                       irregArr1, irregArr2, 0);
                i = 0;
                for (l4_ilist_ptr_type iPtrL4 = ini.begin(); iPtrL4 != ini.end(); ++iPtrL4)
                {
                    size_t j = 0;
                ...
                initArray(0, static_cast<index_type>(ini.size()) - 1, irregArr1, irregArr2, irregArr3);
            }
            else
            {
        ...
        void initArray(const IrregArray4D<TForeign, AForeign> &ini)
        {
            if (ini.arr_ != nullptr)
            {
                ...
                {
                    initArray(ini.first1_, ini.last1_,
                              ini.first2_, ini.last2_,
                              ini.first3_, ini.last3_,
                              ini.first4_, ini.last4_);
                }
            }
                       ...
                       const index_type k1, const index_type k2)
        {
            ...
            else
            {
                ...
                if (z1 > z2)
                {
                    std::string errorMessage(formatString("Error in %s::initArray(): called with illegal bounds first3 > last3.", typeid(*this).name()));
                    GMX_THROW(InvalidInputError(errorMessage));
                }
                if (k1 > k2)
                {
                    std::string errorMessage(formatString("Error in %s::initArray(): called with illegal bounds first4 > last4.", typeid(*this).name()));
                    GMX_THROW(InvalidInputError(errorMessage));
                }
            }
        }
                       ...
                       const value_type &ini)
        {
            ...
            {
                for (index_type h2 = getFirst2(); h2 <= getLast2(); h2++)
                {
                    for (index_type h3 = getFirst3(); h3 <= getLast3(); h3++)
                    {
                        for (index_type h4 = getFirst4(); h4 <= getLast4(); h4++)
                        {
        ...
        void initArray(const index_type x1, const index_type y1,
                       const index_type z1, const index_type k1,
                       const IrregArray1D<size_type, AForeign> &sizes1,
                       const IrregArray2D<size_type, AForeign> &sizes2,
                       const IrregArray3D<size_type, AForeign> &sizes3)
        {
            ...
            bool             length4AlwaysNonNegative = true;
            const index_type x2 = first1_ + sizes1.getLength1() - 1;
            for (index_type i = x1; i <= x2; ++i)
            {
                ...
                const index_type y2 = y1 + sizes1(i) - 1;
                for (index_type j = y1; j <= y2; ++j)
                {
                    if (static_cast<std::make_signed<size_type>::type>(sizes2(i, j)) < 1)
                    {
                        length3AlwaysGreaterZero = false;
                        break;
                    }

                    const index_type z2 = z1 + sizes2(i, j) - 1;
                    for (index_type k = z1; k <= z2; ++k)
                    {
                        ...
                        if (static_cast<std::make_signed<size_type>::type>(sizes3(i, j, k)) <= 0)
                        {
                            length4AlwaysNonNegative = false;
                            break;
                        }
                    }
                }
                if (!length3AlwaysGreaterZero || !length4AlwaysNonNegative)
                {
                    break;
                }
            }
            if (x2 >= x1 && length2AlwaysGreaterZero && length3AlwaysGreaterZero && length4AlwaysNonNegative)
            {
                deallocateArray();
                isIrreg_   = true;
                first1_    = x1;
            ...
            else
            {
                if (x2 < x1)
                {
                    std::string errorMessage("Error in %s::allocateArray(): called with illegal bounds first1 > last1.",
                                             typeid(*this).name());
                    GMX_THROW(InvalidInputError(errorMessage));
                }
                if (!length2AlwaysGreaterZero)
                {
                    std::string errorMessage("Error in %s::allocateArray(): called with illegal bounds first2 > last2.",
                                             typeid(*this).name());
                    GMX_THROW(InvalidInputError(errorMessage));
                }
                if (!length3AlwaysGreaterZero)
                {
                    std::string errorMessage("Error in %s::allocateArray(): called with illegal bounds first3 > last3.",
                                             typeid(*this).name());
                    GMX_THROW(InvalidInputError(errorMessage));
                }
                if (!length4AlwaysNonNegative)
                {
                    std::string errorMessage("Error in %s::allocateArray(): called with illegal bounds first4 > last4.",
                                             typeid(*this).name());
                    GMX_THROW(InvalidInputError(errorMessage));
                }
            }
                       ...
                       const IrregArray3D<size_type, AForeign> &sizes3)
        {
            ...
            IrregArray1D<size_type, allocator_size_type> tmpIrreg(first1_, last1_, 0);
                       ...
                       const IrregArray1D<size_type, AForeign> &sizes2)
        {
            ...
            last3_     = z2;
            last4_     = 0;
            // the allocator of the input arrays sizes1, sizes2 so create irregMat with the same allocator
            IrregArray2D<size_type, AForeign> irregMat(first1_, last1_, sizes1, 0);
            const index_type                  dim3 = z2 - z1 + 1;
            for (index_type i = first1_; i <= last1_; i++)
            {
                for (index_type j = first2_; j <= irregMat.getLast2(i); j++)
                {
                    irregMat(i, j) = dim3;
                }
            }
            irreg_.initArray(first1_, last1_, sizes1, irregMat, 0);
            for (index_type i = first1_; i <= last1_; i++)
            {
                for (index_type j = first2_; j <= irreg_.getLast2(i); j++)
                {
                    for (index_type k = first3_; k <= irreg_.getLast3(i, j); k++)
                    {
        ...
        inline size_type getLength3() const
        {
            if (!isIrreg_)
            {
        ...
        inline size_type getLength4(const index_type x, const index_type y,
                                    const index_type z)
        const
        {
        ...
        inline index_type getBegin3() const { return first3_; }
        //! get the first index of dimension 4
        inline index_type getBegin4() const { return first4_; }
        ...
        //! get the first index of dimension 3
        inline index_type getFirst3() const { return first3_; }
        //! get the first index of dimension 4
        inline index_type getFirst4() const { return first4_; }
        ...
        inline index_type getLast3() const
        {
            if (!isIrreg_)
            {
                return last3_;
        ...
        /*! \brief get the last index of dimension 3 for lower dimension indices x, y
            ...
            \param[in]    y       index in dimension 2
         */
        inline index_type getLast3(const index_type x, const index_type y) const
        {
            return (isIrreg_ ? irreg_.getLast3(x, y) : last3_);
        }
        //! get the last index of dimension 4
        inline index_type getLast4() const
        {
            if (!isIrreg_)
            {
                return last4_;
        ...
        /*! \brief get the last index of dimension 3 for lower dimension indices x, y, z
            ...
            \param[in]    z       index in dimension 3
         */
        inline index_type getLast4(const index_type x, const index_type y,
                                   const index_type z)
        const
        {
        ...
        inline index_type getEnd1() const { return last1_; }
        ...
        inline index_type getEnd3() const
        {
            if (!isIrreg_)
            {
                return last3_ + 1;
        ...
        /*! \brief get the index one past the last valid index of dimension 3 for lower dimension indices x, y, z
            ...
            \param[in]    z       index in dimension 3
         */
        inline index_type getEnd4(const index_type x, const index_type y, const index_type z) const
        {
...
// of the 2D/3D arrays right below the corresponding pointer manipulation line
 ...
 */
        ...
        inline value_type &operator()(const index_type x, const index_type y,
                                      const index_type z, const index_type k)
        {
            return arr_[x][y][z][k];
        }
        /*! \brief   const index operator(x,y,z) for functor-like usage
            ...
            \param[in]   k   index in dimension 4
         */
        inline const value_type &operator()(const index_type x, const index_type y,
                                            const index_type z, const index_type k)
        const
        {
            return arr_[x][y][z][k];
        ...
        /*! \brief   const index operator(x,y,z) for functor-like usage
            \param[in]   x   index in dimension 1
         */
        inline const value_type ***operator()(const index_type x)
        const
        {
            return arr_[x];
        }
...
// scan-build reports a false positive possible memory leak in allocateArray of
// the 2D/3D arrays right below the pointer manipulation line
#ifndef __clang_analyzer__
                arr_ -= getFirst1();
#endif
                for (index_type i = getFirst1(); i <= getLast1(); ++i)
