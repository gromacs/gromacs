...
class IrregArray4D
{
    ...
    private:
        ...
        typename std::conditional<IsFlat, value_type*, value_type****>::type arr_;
...
// clang-tidy diagnoses that an exception may be thrown in cases there isIrreg_ is guaranteed to be false
    ...
    private:
        ...
        void allocateArray()
        {
            ...
            if (length1() <= 0)
            {
                std::string errorMessage("Error in %s::allocateArray(): called with illegal bounds length1 <= 0.", typeid(*this).name());
            ...
            if (!isIrreg_)
            {
                if (length2() <= 0)
                {
                    std::string errorMessage("Error in %s::allocateArray(): called with illegal bounds length2 <= 0.", typeid(*this).name());
                    GMX_THROW(InvalidInputError(errorMessage));
                }
                if (length3() <= 0)
                {
                    std::string errorMessage("Error in %s::allocateArray(): called with illegal bounds length3 <= 0.", typeid(*this).name());
                    GMX_THROW(InvalidInputError(errorMessage));
                }
                if (static_cast<std::make_signed<size_type>::type>(length4()) < 0)
                {
                    std::string errorMessage("Error in %s::allocateArray(): called with illegal bounds length4 < 0.", typeid(*this).name());
                    GMX_THROW(InvalidInputError(errorMessage));
                }
            }
            allocateMemory();
