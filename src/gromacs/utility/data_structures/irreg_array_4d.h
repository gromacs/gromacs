...
class IrregArray4D
{
    ...
    public:
        ...
        {
            std::swap(first1_,  ini.first1_);
            std::swap(first2_,  ini.first2_);
            std::swap(first3_,  ini.first3_);
            std::swap(first4_,  ini.first4_);
            std::swap(last1_,   ini.last1_);
            std::swap(last2_,   ini.last2_);
            std::swap(last3_,   ini.last3_);
            std::swap(last4_,   ini.last4_);
            std::swap(isIrreg_, ini.isIrreg_);
            std::swap(arr_,     ini.arr_);
            irregTen_     = std::move(ini.irregTen_);
            allocator_    = std::move(ini.allocator_);
            p_allocator_  = std::move(ini.p_allocator_);
            p2_allocator_ = std::move(ini.p2_allocator_);
            p3_allocator_ = std::move(ini.p3_allocator_);
        }
 ...
 */
        ...
        IrregArray4D &operator=(IrregArray4D &&rhs) noexcept
        {
            ...
            {
                deallocateArray();
                std::swap(first1_,  rhs.first1_);
                std::swap(first2_,  rhs.first2_);
                std::swap(first3_,  rhs.first3_);
                std::swap(first4_,  rhs.first4_);
                std::swap(last1_,   rhs.last1_);
                std::swap(last2_,   rhs.last2_);
                std::swap(last3_,   rhs.last3_);
                std::swap(last4_,   rhs.last4_);
                std::swap(isIrreg_, rhs.isIrreg_);
                std::swap(arr_,     rhs.arr_);
                irregTen_     = std::move(rhs.irregTen_);
