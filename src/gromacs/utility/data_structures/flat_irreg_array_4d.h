...
class FlatIrregArray4D
{
    ...
    private:
        ...
        //! let other FlatIrregArray4D instantiations access private members (for the explicit conversion operator)
    ...
    public:
         ...
         */
        FlatIrregArray4D(const index_type x1, const index_type x2,
        ...
        FlatIrregArray4D(const index_type x1, const index_type x2,
                         ...
                         const FlatIrregArray3D<size_type, AForeign> &sizes3)
            : FlatIrregArray4D()
        {
            ...
            : FlatIrregArray4D()
        {
            ...
            if (isIrreg_)
            {
                ...
                FlatIrregArray3D<size_type, allocator_size_type> irregArr3(0, static_cast<index_type>(ini.size()) - 1,
        ...
        {
            // This condition should not be necessary because nelements_ and arr_ should always
            // be consistent but without it the clang analyzer reports a possible nullptr de-
            // reference of ini.arr_
            if (ini.arr_ != nullptr)
            {
                deallocateArray();
                irreg_     = ini.irreg_;
                ...
                last4_     = ini.last4_;
                nelements_ = 0;
                // allocate array allocates the array and sets nelements_
                allocateArray(ini.nelements_);
        ...
        {
            IrregArray1D<size_type, AForeign> sizes1(sizes3.getFirst1(), sizes3.getLast1(), 0);
            for (index_type i = sizes3.getFirst1(); i <= sizes3.getLast1(); ++i)
            {
                sizes1(i) = sizes3.getLength2(i);
            }
            FlatIrregArray2D<size_type, AForeign> sizes2(sizes3.getFirst1(), sizes3.getLast1(), sizes1, 0);
            for (index_type i = sizes3.getFirst1(); i <= sizes3.getLast1(); ++i)
            {
                       ...
                       const IrregArray1D<size_type, AForeign> &sizes2)
        {
            ...
            FlatIrregArray2D<size_type, allocator_size_type> irregMat(first1_, last1_, sizes1, dim3);
            ...
            FlatIrregArray3D<size_type, allocator_size_type> irregTen(first1_, last1_, irregVec, irregMat, 0);
            for (index_type i = first1_; i <= last1_; i++)
            {
                for (index_type j = first2_; j <= first2_ + static_cast<index_type>(irregVec(i)) - 1; j++)
                {
                    ...
                    {
                        irregTen(i, j, k) = sizes2(k);
                    }
                }
            }
            allocateArray(first1_, first2_, first3_, first4_, irregVec, irregMat, irregTen);
        }
        /*! \brief specialized constructor helper function for irregular, symmetrically shaped matrices
                   application: interaction energy matrix site1 form1(site1) site2 form2(site2)
        ...
        inline size_type getLength4(const index_type x, const index_type y, const index_type z) const
        {
            if (isIrreg_)
            {
        ...
        /*! \brief get the last index of dimension 3 for lower dimension indices x, y, z
            ...
            \param[in]    y       index in dimension 2
            \param[in]    z       index in dimension 3                       */
        inline index_type getLast4(const index_type x, const index_type y, const index_type z) const
        {
            if (isIrreg_)
            {
                if (x == last1_ && y == getLast2(last1_) && z == getLast3(last1_, getLast2(last1_)))
        ...
        /*! \brief get the index one past the last valid index of dimension 4 for lower dimension indices x, y, z
            ...
            \param[in]    y       index in dimension 2
            \param[in]    z       index in dimension 3                       */
        inline index_type getEnd4(const index_type x, const index_type y, const index_type z)
        const
        {
            if (isIrreg_)
            {
        ...
        /*! \brief  conversion operator (explicit to avoid unintended implicit conversion)
            ...
            \code
                 FlatIrregArray4D<TForeign, AForeign> newArr =
                    static_cast<FlatIrregArray4D<TForeign, AForeign> >
                    (
                        FlatIrregArray4D<TCurrent, ACurrent> oldArr
                    );
            \endcode
    ...
    private:
                           ...
                           const index_type k1, const index_type k2)
        {
            ...
            else
            {
                if (x1 > x2)
                {
                    std::string errorMessage(formatString("Error in %s::allocateArray(): called with illegal bounds first1 > last1.",
                                                          typeid(*this).name()));
                    GMX_THROW(InvalidInputError(errorMessage));
                }
                if (y1 > y2)
                {
                    std::string errorMessage(formatString("Error in %s::allocateArray(): called with illegal bounds first2 > last2.",
                                                          typeid(*this).name()));
                    GMX_THROW(InvalidInputError(errorMessage));
                }
                if (z1 > z2)
                {
                    std::string errorMessage(formatString("Error in %s::allocateArray(): called with illegal bounds first3 > last3.",
                                                          typeid(*this).name()));
                    GMX_THROW(InvalidInputError(errorMessage));
                }
                if (k1 > k2)
                {
                    std::string errorMessage(formatString("Error in %s::allocateArray(): called with illegal bounds first4 > last4.",
                                                          typeid(*this).name()));
                    GMX_THROW(InvalidInputError(errorMessage));
                }
            }
        ...
        void allocateArray(const index_type s1, const index_type s2, const index_type s3, const index_type s4, const IrregArray1D<size_type, AForeign> &sizes1, const FlatIrregArray2D<size_type, AForeign> &sizes2, const FlatIrregArray3D<size_type, AForeign> &sizes3)
        {
            ...
            size_1d_array_type irregVec(first1_, last1_);
            for (index_type i = first1_; i <= last1_; i++)
            {
                if (static_cast<std::make_signed<size_type>::type>(sizes1(i)) <= 0)
                {
            ...
            size_2d_array_type irregMat(first1_, last1_, irregVec);
            for (index_type i = first1_; i <= last1_; i++)
            {
                for (index_type j = first2_; j <= first2_ + static_cast<index_type>(irregVec(i)) - 1; j++)
                {
                    ...
                    {
                        std::string errorMessage(formatString("Error in %s::allocateArray(): called with illegal length %li < 1 for element %li, %li\n",
            ...
            for (index_type i = first1_; i <= last1_; i++)
            {
                for (index_type j = first2_; j <= first2_ + static_cast<index_type>(irregVec(i)) - 1; j++)
                {
                    for (index_type k = first3_; k <= first3_ + static_cast<index_type>(irregMat(i, j)) - 1; k++)
                    {
                        ...
                        {
                            std::string errorMessage(formatString("Error in %s::allocateArray(): called with illegal length %li < 0 for element %li, %li, %li\n",
