...
// Trick the clang_static_analyzer by a dummy call to an init function that it understands
            ...
            : IrregArray1D()
        {
            if (ini_vec.size() != 0)
            {
                const index_type x2 = static_cast<index_type>(ini_vec.size()) - 1;
                allocateArray(0, x2);
        ...
        //! get the end as in std containers, one past the last valid index of dimension 1
        inline index_type getEnd() const { return last1_ + 1; }
        ...
        inline size_type getNelements() const { return getLength(); }
        ...
        friend std::ostream &operator<<(std::ostream &output, const IrregArray1D &vec)
        {
            ...
            if (vec.arr_ != nullptr)
            {
                ...
                for (index_type i = vec.first1_; i <= vec.last1_; i++)
                {
                    ...
                    {
                        output << std::setw(floatWidth) << std::fixed << std::setprecision(floatPrec) << vec(i);
                    }
                    else
                    {
                        output << vec(i);
        ...
        /*! \brief  conversion operator (explicit to avoid unintended implicit conversion)
            ...
            \code
                IrregArray1D<TForeign, AForeign> newArr =
                    static_cast<IrregArray1D<TForeign, AForeign> >
                    (
                        IrregArray1D<TCurrent, ACurrent> oldArr
                    );
            \endcode
