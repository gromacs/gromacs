...
class shared_string
{
    public:
        ...
        typedef shared_ptr_type<shared_ptr_type<gmx::Symbol> > strPtr_type;
        ...
        void replaceAll(const shared_string * const sptr) { replaceAll(sptr->std_str()); }
        ...
        {
            replaceAll(s.std_str());
        }
        //! replace content of this shared_string and all other instances referring to the same string with the content of *s
        void replaceAll(const std::string * const s) {replaceAll(*s); }
        ...
        void replaceAll(const char * const csptr)
        {
            if (csptr != nullptr)
            {
                return replaceAll(std::string(csptr));
            }
            else
            {
                return;
