...
TEST_F(IrregArrayTest, IrregArray1DMove)
{
    ...
    debug << "test_arr1 after move construction: " << test_arr1 << endl;
    ...
    debug << "test_arr2 after move assignment: " << test_arr2 << endl;
}
...
TYPED_TEST(ParamIrregArray2DTest, 2DArrayUsage)
{
    ...
    sizes[5] = 3;
    sizes[6] = 9;
    array_type test_arr3(sizes, (value_type)99);
    EXPECT_NO_THROW_GMX(
            {
    ...
    sizes2[6] = 3;
    sizes2[7] = 9;
    array_type test_arr4(sizes2, (value_type)2);
    EXPECT_NO_THROW_GMX(
            {
...
TYPED_TEST(ParamIrregArray2DTest, 2DArrayMove)
{
    ...
    this->debug << "test_arr1 after move construction:\n" << test_arr1 << endl;
    ...
    this->debug << "test_arr2 after move assignment:\n" << test_arr2 << endl;
...
TYPED_TEST(ParamIrregArray3DTest, 3DArrayUsage)
{
    ...
    size_2d_array_type sizes2(sizes1, 7u);

    array_type         test_arr3(sizes2, 99u);
    EXPECT_NO_THROW_GMX(
            {
                size_type l = 1;
    ...
    sizes2.initArray(sizes1, 2u);

    array_type test_arr4(sizes2, 99u);
    EXPECT_NO_THROW_GMX(
            {
                size_type l = 1;
                for (index_type i = test_arr4.getFirst1(); i <= test_arr4.getLast1(); ++i)
...
TYPED_TEST(ParamIrregArray4DTest, 4DArrayUsage)
{
    ...
    array_type         test_arr4(sizes3, 99u);
    EXPECT_NO_THROW_GMX(
            {
                size_type m = 1;
                for (index_type i = test_arr3.getFirst1(); i <= test_arr3.getLast1(); ++i)
    ...
    sizes1[s1 + 5] = 3;
    sizes1[s1 + 6] = 9;
    array_type test_arr5(s1, s1 + (index_type)6, sizes1, s1, s1 + (index_type)6, sizes1, 5);
