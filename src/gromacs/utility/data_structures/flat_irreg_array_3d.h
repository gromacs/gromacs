...
class FlatIrregArray3D
{
    ...
    private:
        ...
        /*! irregMat_[i][j] = x  is the index of the first actual array element arr_[x] of the
            stripe FlatIrregArray3D[i][j][:] in dimension 3 of the of the represented 3D array */
        FlatIrregArray2D<size_type, allocator_size_type> irregMat_;
    ...
    public:
         ...
         */
        FlatIrregArray3D()
            : first1_(0), first2_(0), first3_(0), last1_(0), last2_(0), last3_(0), nelements_(0), isIrreg_(false), arr_(nullptr)
        {
        }
        //! copy constructor
        ...
        FlatIrregArray3D(const index_type x1, const index_type x2, const size_type *sizes1, const FlatIrregArray2D<size_type, AForeign> &sizes2)
            : first1_(0), first2_(0), first3_(0), last1_(0), last2_(0), last3_(0), nelements_(0), isIrreg_(false), arr_(nullptr)
         ...
         */
        template<class AForeign = allocator_size_type>
        FlatIrregArray3D(const index_type x1, const index_type x2, const size_type *sizes1, const FlatIrregArray2D<size_type, AForeign> &sizes2, const value_type &ini)
            : first1_(0), first2_(0), first3_(0), last1_(0), last2_(0), last3_(0), nelements_(0), isIrreg_(false), arr_(nullptr)
         ...
         */
        template<class AForeign = allocator_size_type>
        void initArray(const index_type x1, const index_type x2, const size_type *sizes1, const FlatIrregArray2D<size_type, AForeign> &sizes2)
        {
            isIrreg_ = true;
        ...
        size_type getLength3(const index_type x, const index_type y) const
        {
            if (isIrreg_)
            {
                ...
                {
                    return nelements_ - irregMat_(x, y);
                }
                else
                {
                    // get first element of next stripe in the underlying data array of irregMat_ to avoid further conditionals
                    const size_type        i       = irregMat_.getArrayIndex(x, y);
                    const size_type* const a       = irregMat_.getArray();
                    return a[i + 1] - a[i];
                }
            }
            else
        ...
        index_type getLast3(const index_type x, const index_type y) const
        {
            if (isIrreg_)
            {
                ...
                {
                    return nelements_ - irregMat_(x, y) + first3_ - 1;
                }
                else
                {
                    // get first element of next stripe in the underlying data array of irregMat_ to avoid further conditionals
        ...
        index_type getEnd3(const index_type x, const index_type y) const
        {
            if (isIrreg_)
            {
                if (x == last1_ && y == getLast2(x))
                {
                    return nelements_ - irregMat_(x, y) + first3_;
                }
                else
                {
                    // get first element of next stripe in the underlying data array of irregMat_ to avoid further conditionals
        ...
        size_type getArrayIndex(const index_type x, const index_type y, const index_type z) const
        {
            if (isIrreg_)
            {
                return irregMat_(x, y) + z - first3_;
            }
            else
            {
        ...
        size_type getSize() const
        {
            ...
            size += irregMat_.getSize() - sizeof(FlatIrregArray2D<size_type>);
            // the storage data
        ...
        void allocateArray(const index_type s1, const index_type s2, const index_type s3, const index_type d1, const IrregArray1D<size_type, AForeign> &irr, const FlatIrregArray2D<size_type, AForeign> &irr_mat)
        {
            ...
            irregMat_.initArray(first1_, last1_, irregVec);
            ...
            for (index_type i = first1_; i <= last1_; i++)
            {
                for (index_type j = first2_; j <= first2_ + static_cast<index_type>(irregVec(i)) - 1; j++)
                {
                    ...
                    {
                        irregMat_(i, j)  = 0;
                        last_index       = irregMat_(i, j);
                        last_length      = irr_mat(i, j);
                    }
                    else
                    {
                        irregMat_(i, j)  = last_index + last_length;
                        last_index       = irregMat_(i, j);
                        last_length      = irr_mat(i, j);
                    }
                }
