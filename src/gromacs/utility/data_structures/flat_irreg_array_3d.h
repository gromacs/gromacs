...
class FlatIrregArray3D
{
    ...
    private:
        ...
        //! let other FlatIrregArray3D instantiations access private members (for the explicit conversion operator)
...
// Trick the clang_static_analyzer by a dummy call to an init function that it understands
         ...
         */
        FlatIrregArray3D(const index_type x1, const index_type x2,
        ...
        FlatIrregArray3D(const index_type x1, const index_type x2,
                         ...
                         const FlatIrregArray2D<size_type, AForeign> &sizes2)
            : FlatIrregArray3D()
        {
            ...
            : FlatIrregArray3D()
        {
            ...
            if (isIrreg_)
            {
                ...
                FlatIrregArray2D<size_type, allocator_size_type> irregArr2(0, static_cast<index_type>(ini.size()) - 1,
                                                                           irregArr1, 0);
                i = 0;
                for (l3_ilist_ptr_type iPtrL3 = ini.begin(); iPtrL3 != ini.end(); ++iPtrL3)
                {
        ...
        void initArray(const index_type x1, const index_type x2,
                       ...
                       const value_type &ini)
        {
            IrregArray1D<size_type, AForeign> sizes1(x1, x2, 0);
        ...
        void initArray(const FlatIrregArray2D<size_type, AForeign> &sizes2,
                       const value_type &ini)
        {
            IrregArray1D<size_type, AForeign> sizes1(sizes2.getFirst1(), sizes2.getLast1(), 0);
                       ...
                       const IrregArray1D<size_type, AForeign> &sizes)
        {
            ...
            FlatIrregArray2D<size_type, allocator_size_type> tmp_irreg_mat(first1_, last1_, first2_, last2_, 0);
        ...
        /*! \brief get the index one past the last valid index of dimension 3 for lower dimension indices x, y
            \param[in]    x       index in dimension 1
            \param[in]    y       index in dimension 2                       */
        inline index_type getEnd3(const index_type x, const index_type y)
        const
        {
            if (isIrreg_)
            {
         ...
         */
        inline size_type getArrayIndex(const index_type x, const index_type y,
                                       const index_type z)
        const
        {
            if (isIrreg_)
        ...
        /*! \brief  conversion operator (explicit to avoid unintended implicit conversion)
            ...
            \code
                FlatIrregArray3D<TForeign, AForeign> newArr =
                    static_cast<FlatIrregArray3D<TForeign, AForeign> >
                    (
                        FlatIrregArray3D<TCurrent, ACurrent> oldArr
                    );
            \endcode
    ...
    private:
                           ...
                           const index_type z1, const index_type z2)
        {
            ...
            else
            {
                ...
                if (z1 > z2)
                {
                    std::string errorMessage(formatString("Error in %s::allocateArray(): called with illegal bounds first3 > last3.", typeid(*this).name()));
                    GMX_THROW(InvalidInputError(errorMessage));
                }
            }
        }
        ...
        void allocateArray(const index_type s1, const index_type s2, const index_type s3, const index_type d1, const IrregArray1D<size_type, AForeign> &irr, const FlatIrregArray2D<size_type, AForeign> &irr_mat)
        {
            ...
            if (static_cast<std::make_signed<size_type>::type>(dim1) < 0)
            {
                std::string errorMessage(formatString("Error in %s::allocateArray(): called with illegal bounds first1 > last1.", typeid(*this).name()));
                GMX_THROW(InvalidInputError(errorMessage));
            }
            ...
            for (index_type i = first1_; i <= last1_; i++)
            {
                if (static_cast<std::make_signed<size_type>::type>(irr(i)) < 0)
                {
            ...
            for (index_type i = first1_; i <= last1_; i++)
            {
                ...
                {
                    // zero length seems to be OK with the standard allocator, but size_type may be signed
                    if (static_cast<std::make_signed<size_type>::type>(irr_mat(i, j)) < 0)
                    {
                        std::string errorMessage(formatString("Error in %s::allocateArray(): called with illegal length %li < 0 for element %li, %li\n",
