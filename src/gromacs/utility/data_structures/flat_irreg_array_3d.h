...
class FlatIrregArray3D
{
    ...
    public:
         ...
         */
        FlatIrregArray3D()
            : first1_(0), first2_(0), first3_(0), last1_(0), last2_(0), last3_(0), nelements_(0), is_irreg_(false), arr_(nullptr)
        {
        }
        //! copy constructor
        ...
        index_type getLast3(const index_type x, const index_type y) const
        {
            if (is_irreg_)
            {
                ...
                {
                    return nelements_ - irreg_mat_(x, y) + first3_ - 1;
                }
                else
                {
                    // get first element of next stripe in the underlying data array of irreg_mat_ to avoid further conditionals
        ...
        index_type getEnd3(const index_type x, const index_type y) const
        {
            if (is_irreg_)
            {
                if (x == last1_ && y == getLast2(x))
                {
                    return nelements_ - irreg_mat_(x, y) + first3_;
                }
                else
                {
                    // get first element of next stripe in the underlying data array of irreg_mat_ to avoid further conditionals
        ...
        size_type getArrayIndex(const index_type x, const index_type y, const index_type z) const
        {
            if (is_irreg_)
            {
                return irreg_mat_(x, y) + z - first3_;
            }
            else
            {
        ...
        void allocateArray(const index_type s1, const index_type s2, const index_type s3, const index_type d1, const IrregArray1D<size_type, AForeign> &irr, const FlatIrregArray2D<size_type, AForeign> &irr_mat)
        {
            ...
            irreg_mat_.initArray(first1_, last1_, irreg_);
            size_type  last_length = 0;
            index_type last_index  = 0;
            for (index_type i = first1_; i <= last1_; i++)
            {
                for (index_type j = first2_; j <= first2_ + static_cast<index_type>(irreg_(i)) - 1; j++)
                {
                    // zero length seems to be OK with the standard allocator, but size_type may be signed
                    ...
                    {
                        irreg_mat_(i, j) = last_index + last_length;
                        last_index       = irreg_mat_(i, j);
                        last_length      = irr_mat(i, j);
                    }
                }
            ...
            const size_type n = last_index + irr_mat(last1_, first2_ + irreg_[last1_] - 1);
