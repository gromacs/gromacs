...
class IrregArray3D
{
    ...
    private:
        ...
        IrregArray2D<size_type, allocator_size_type> irregMat_;
    ...
    public:
         ...
         */
        IrregArray3D() : first1_(0), first2_(0), first3_(0), last1_(0), last2_(0), last3_(0), isIrreg_(false), arr_(nullptr)
        {
        }
        //! copy constructor
        //! \param[in]   iniT
        IrregArray3D(const IrregArray3D &iniT) : first1_(0), first2_(0), first3_(0), isIrreg_(false), arr_(nullptr)
        {
            initArray(iniT);
        }
        ...
        IrregArray3D(const IrregArray3D<value_type, AForeign> &iniT) : first1_(0), first2_(0), first3_(0), isIrreg_(false), arr_(nullptr)
        {
            initArray(iniT);
        }
        ...
        IrregArray3D(const index_type x1, const index_type x2, const index_type y1, const index_type y2, const index_type z1, const index_type z2)
            : first1_(0), first2_(0), first3_(0), last1_(0), last2_(0), last3_(0), isIrreg_(false), arr_(nullptr)
        {
        ...
        IrregArray3D(const index_type x1, const index_type x2, const size_type *sizes1, const IrregArray2D<size_type, AForeign> &sizes2)
            : first1_(0), first2_(0), first3_(0), last1_(0), last2_(0), last3_(0), isIrreg_(false), arr_(nullptr)
        {
         ...
         */
        template<class AForeign = allocator_size_type>
        IrregArray3D(const index_type x1, const index_type x2, const size_type *sizes1, const IrregArray2D<size_type, AForeign> &sizes2, const value_type &ini)
            : first1_(0), first2_(0), first3_(0), last1_(0), last2_(0), last3_(0), isIrreg_(false), arr_(nullptr)
        {
        ...
        void initArray(const IrregArray3D<TForeign, AForeign> &ini)
        {
            if (ini.arr_ != nullptr)
            {
                if (ini.isIrreg_)
                {
                    initArray(ini.first1_, ini.last1_, ini.irregMat_.irreg_, ini.irregMat_);
                }
                else
                {
         ...
         */
        template<class AForeign = allocator_size_type>
        void initArray(const index_type x1, const index_type x2, const size_type *sizes1, const IrregArray2D<size_type, AForeign> &sizes2)
        {
            isIrreg_ = true;
            IrregArray1D<size_type, allocator_size_type> tmp_vec(x1, x2);
            for (index_type i = x1; i <= x2; i++)
            {
                tmp_vec(i) = sizes1[i];
            }
        ...
        {
            IrregArray1D<size_type, allocator_size_type> tmpvec(y1, y2, (size_type)0);
        ...
        {
            IrregArray1D<size_type, allocator_size_type> tmpvec(x1, x2, (size_type)0);
            for (index_type i = x1; i <= x2; i++)
            {
                tmpvec(i) = sizes[i];
            }
        ...
        void initArray(const index_type x1, const index_type x2, const index_type y1, const index_type y2, const IrregArray1D<size_type, AForeign> &sizes)
        {
            ...
            last2_     = y2;
            last3_     = 0;
            irregMat_.initArray(x1, x2, y1, y2, 0);
            for (index_type i = x1; i <= x2; i++)
            {
                for (index_type j = y1; j <= y2; j++)
                {
                    irregMat_(i, j) = sizes(j);
                }
            }
            allocateArray();
        }
        ...
        void initArray(index_type x1, index_type x2, const IrregArray1D<size_type, AForeign> &sizes, index_type z1, index_type z2)
        {
            ...
            last2_   = 0;
            last3_   = z2;
            irregMat_.initArray(x1, x2, sizes, dim3);
            for (index_type i = x1; i <= x2; i++)
            {
        ...
        {
            return (isIrreg_ ? irregMat_.getLength2(x) : (last2_ - first2_ + 1));
        }
        ...
        {
            return (isIrreg_ ? irregMat_(x, y) : (last3_ - first3_ + 1));
        }
        ...
        {
            return (isIrreg_ ? irregMat_.getLast2(x) : last2_);
        }
        ...
        {
            return (isIrreg_ ? (irregMat_(x, y) + first3_ - 1) : last3_);
        }
        ...
        {
            return (isIrreg_ ? irregMat_.getEnd2(x) : (last2_ + 1));
        }
        ...
        {
            return (isIrreg_ ? (irregMat_(x, y) + first3_) : last3_ + 1);
        }
        ...
        size_type getSize() const
        {
            ...
            size += irregMat_.getSize() - sizeof(IrregArray2D<size_type>);
            // the storage data
    ...
    private:
        ...
        void allocateArray()
        {
            ...
            try
            {
                arr_ = p2_allocator_.allocate(getLength1());
                arr_ = arr_ - getBegin1();
                for (index_type i = getFirst1(); i <= getLast1(); ++i)
                {
                    arr_[i]  = p_allocator_.allocate(getLength2(i));
                    arr_[i] -= getFirst2();
                }
                for (index_type i = getFirst1(); i <= getLast1(); i++)
                {
                    for (index_type j = getFirst2(); j <= getLast2(i); ++j)
                    {
                        arr_[i][j]  = allocator_.allocate(getLength3(i, j));
                        arr_[i][j] -= getFirst3();
                    }
                }
            }
