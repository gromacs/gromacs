...
class IrregArray3D
{
    ...
    private:
        ...
        //! let other IrregArray3D instantiations access private members (for the explicit conversion operator)
...
// Trick the clang_static_analyzer by a dummy call to an init function that it understands
        ...
        IrregArray3D(const IrregArray3D<value_type, AForeign> &ini)
            : IrregArray3D()
        {
            initArray(ini);
        }
        ...
        IrregArray3D(IrregArray3D &&ini) noexcept
            : first1_(ini.first1_), first2_(ini.first2_), first3_(ini.first3_),
              last1_(ini.last1_), last2_(ini.last2_), last3_(ini.last3_),
         ...
         */
        IrregArray3D(const index_type x1, const index_type x2,
            ...
            : IrregArray3D()
        {
            ...
            if (isIrreg_)
            {
                ...
                initArray(0, static_cast<index_type>(ini.size()) - 1, irregArr1, irregArr2);
            }
            else
            {
                       ...
                       const value_type &ini)
        {
            ...
            for (index_type i = getFirst1(); i <= getLast1(); i++)
            {
                ...
                {
                    for (index_type k = getFirst1(); k <= getLast3(); k++)
                    {
                        arr_[i][j][k] = ini;
                    }
                }
                       ...
                       const IrregArray2D<size_type, AForeign> &sizes2)
        {
            ...
            bool length3AlwaysNonNegative  = true;
            for (index_type i = x1; i <= x2; ++i)
            {
                ...
                const index_type y1 = x1;
                const index_type y2 = x1 + sizes1(i) - 1;
                for (index_type j = y1; j <= y2; ++j)
                {
                    ...
                    if (static_cast<std::make_signed<size_type>::type>(sizes2(i, j)) <= 0)
                    {
                        length3AlwaysNonNegative = false;
                        break;
                    }
                }
                if (!length3AlwaysNonNegative)
                {
                    break;
                }
            }

            if (x2 >= x1 && length2AlwaysGreaterZero && length3AlwaysNonNegative)
            {
                isIrreg_   = true;
                first1_    = x1;
                first2_    = x1;
                ...
                irreg_.initArray(getFirst1(), getLast1(), irregVec, 0);
                for (index_type i = getFirst1(); i <= getLast1(); i++)
                {
         ...
         */
        template<class AForeign = allocator_size_type>
        void initArray(index_type x1, index_type x2,
                       const IrregArray1D<size_type, AForeign> &sizes,
                       index_type z1, index_type z2)
        {
            deallocateArray();
            isIrreg_ = true;
        ...
        inline size_type getLength3(const index_type x, const index_type y) const
        {
        ...
        size_type getSize() const
        {
            ...
            size += irreg_.getSize() - sizeof(size_2d_array_type);
            // the storage data
...
// right below the corresponding pointer manipulation line
        ...
        /*! \brief   const index operator(x,y) for functor-like usage
            ...
            \param[in]   y   index in dimension 2
         */
        const value_type *operator()(const index_type x, const index_type y)
        const
        {
            return arr_[x][y];
        }
...
// scan-build reports a false positive possible memory leak at the for loop line
#ifndef __clang_analyzer__
                arr_ -= getFirst1();
