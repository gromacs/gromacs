...
/*! \file
    ...
    \date Feb 2015
 */
#ifndef GMX_UTILITY_IRREG_ARRAY_2D_H
#define GMX_UTILITY_IRREG_ARRAY_2D_H
...
class IrregArray2D
{
    ...
    public:
        ...
        void initArray(const IrregArray2D<T_foreign, A_foreign> &ini)
        {
            if (ini.is_irreg_)
            {
                allocateArray(ini.getBegin1(), ini.getEnd1(), ini.irreg_);
            }
            else
            {
        ...
        void allocateArray(const index_type x1, const index_type x2, const index_type y1, const index_type y2)
        {
            ...
            if (static_cast<long int>(dim2) < 0)
            {
            ...
            try
            {
                arr_           = p_allocator_.allocate(dim1);
                arr_          -= begin1_;
                arr_[begin1_]  = allocator_.allocate(dim1*dim2);
                arr_[begin1_] -= y1;
                for (index_type i = begin1_ + 1; i <= x2; ++i)
                {
        ...
        void allocateArray(const index_type x1, const index_type x2, const IrregArray1D<size_type> &sizes)
        {
            ...
            try
            {
                ...
                for (index_type i = begin1_; i <= end1_; ++i)
                {
                    arr_[i]  = allocator_.allocate(sizes[i]);
                    arr_[i] -= begin2_;
                }
            }
            catch (std::bad_alloc)
        ...
        void deallocateArray()
        {
            if (arr_ != nullptr)
            {
                ...
                else
                {
                    ...
                    allocator_.deallocate(arr_[begin1_], getLength1() * getLength2());
                    arr_ += begin1_;
                    p_allocator_.deallocate(arr_, getLength1());
                    arr_ = nullptr;
                }
