...
class IrregArray2D
{
    ...
    public:
        ...
        IrregArray2D(IrregArray2D &&ini) noexcept
            : first1_(ini.first1_), first2_(ini.first2_), last1_(ini.last1_), last2_(ini.last2_),
            ...
            : first1_(0), first2_(0), last1_(-1), last2_(-1), isIrreg_(false), arr_(nullptr)
        {
            ...
            allocateArray((size_type)0, sizes.size() - 1, irregVec);
        }
         ...
         */
        template<class AForeign = allocator_size_type>
        IrregArray2D(const std::vector<size_type, AForeign> &sizes, const value_type &ini)
            : first1_(0), first2_(0), last1_(-1), last2_(-1), isIrreg_(false), arr_(nullptr)
        {
            IrregArray1D<size_type, allocator_size_type> irregVec(sizes);
            allocateArray((size_type)0, sizes.size() - 1, irregVec);
         ...
         */
        template<class AForeign = allocator_size_type>
        IrregArray2D(const IrregArray1D<size_type, AForeign> &sizes, const value_type &ini)
            : first1_(0), first2_(0), last1_(-1), last2_(-1), isIrreg_(false), arr_(nullptr)
        {
            allocateArray(sizes.getFirst1(), sizes.getLast1(), sizes);
            if (arr_ != nullptr)
        ...
        {
            initArray(0, sizes.size() - 1, IrregArray1D<size_type, allocator_size_type>(0, sizes.size() - 1, sizes));
        }
        /*! \brief  constructor with memory allocation but without content initialization
        ...
        {
            initArray(0, sizes.size() - 1, IrregArray1D<size_type, allocator_size_type>(0, sizes.size() - 1, sizes), ini);
        }
