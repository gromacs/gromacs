...
//! Replace all directory separators with the one used for joining
std::string normalize(const std::string &path)
{
    std::string result(path);
    if (DIR_SEPARATOR != cDirSeparator)
    {
        std::replace(result.begin(), result.end(), cDirSeparator,
                DIR_SEPARATOR);
    }
    return result;
}
...
{
    //Expects that the path doesn't contain "." or "..".
    //Should only be used on path returned from resolveSymlinks.
