...
bool Path::isSubdirectoryOf(const std::string &path, const std::string &parent)
{
    // TODO: Check that the match is for full directories.
    return startsWith(normalize(path), normalize(parent));
}
