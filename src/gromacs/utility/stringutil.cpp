...
bool compareStringsCaseInsensitve(const std::string &source, const std::string &target)
{
...
bool compareStringsCaseSensitve(const std::string &source, const std::string &target)
{
    return source.length() == target.length() &&
           std::equal(source.begin(), source.end(), target.begin());
}
