...
class ArrayRef
{
    public:
         ...
         *  Copy constructor.
         *
         * We need to make this explicit since MSVC2012 confuses the
         * EmptyArrayRef initializer for a copy constructur.
         */
        ArrayRef(const ArrayRef &src) : begin_(src.begin_), end_(src.end_) {}
        /*! \brief
         * Constructs an empty reference.
         *
...
{
    return ArrayRefFromPointers(begin, begin+size);
}
 ...
 */
template <typename T>
ArrayRef<T> ArrayRefFromIterators(typename std::vector<T>::iterator begin,
                                  typename std::vector<T>::iterator end)
{
    T * p_begin = (begin != end) ? &*begin : NULL;
    T * p_end   = p_begin + (end-begin);
    return ArrayRefFromPointers(p_begin, p_end);
}
...
class ConstArrayRef
{
    public:
         ...
         * EmptyArrayRef initializer for a copy constructur.
         */
        ConstArrayRef(const ConstArrayRef &src) : begin_(src.begin_), end_(src.end_) {}
        /*! \brief
         * Constructs an empty reference.
         *
         * This is provided for convenience, such that EmptyArrayRef can be
 ...
 */
template <typename T>
ConstArrayRef<T> ConstArrayRefFromIterators(typename std::vector<T>::const_iterator begin,
