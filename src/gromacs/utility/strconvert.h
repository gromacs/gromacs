...
static gmx_inline std::string intToString(const int &t)
{
    return formatString("%d", t);
}
//! \copydoc intToString(const int &)
static gmx_inline std::string int64ToString(const gmx_int64_t &t)
{
    return formatString("%" GMX_PRId64, t);
}
//! \copydoc intToString(const int &)
static gmx_inline std::string doubleToString(const double &t)
{
...
//! Implementation for string values. Does not throw.
template <> inline
std::string toString<std::string>(const std::string &t) { return t; }

/*! \brief
 * Prints a C-style string to a std::string in a manner similar to other types
 *
 * \tparam T Type of value to print (expected to be char).
 *
 * `T` can only be one of the types that is explicity supported (ie
 * char). The main use for this function is to write `toString<char>(value)`.
 *
 * \throws std::bad_alloc if out of memory. */
template <typename T> static inline std::string toString(const T *t);

//! Implementation for C string values.
template <> inline
std::string toString<char>(const char *t) { return std::string(t); }
