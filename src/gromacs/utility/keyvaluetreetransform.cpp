...
class KeyValueTreeBackMapping : public IKeyValueTreeBackMapping
{
    public:
        class Entry
        {
            public:
                Entry() = default;
                explicit Entry(const std::string &path) : sourcePath_(path) {}
                ...
                {
                    std::string sourcePath = "/" + joinStrings(path, "/");
                    if (value.isObject())
                    {
                        const KeyValueTreeObject &object = value.asObject();
                        for (const auto &prop : object.properties())
                        {
                            childEntries_[prop.key()] = Entry(sourcePath);
                        }
                    }
                    else
                    {
        ...
        virtual std::string originalPath(const std::string &path) const
        {
            std::vector<std::string> elements = splitDelimitedString(path.substr(1), '/');
