...
TEST(RegexBasicTest, ConstructsUsedInPdb2gmxTestsWork)
{
    EXPECT_TRUE(regexMatch(";	By user: jenkins (-1)", Regex("^;.*")));
    EXPECT_TRUE(regexMatch(";	By user: jenkins (-1)", Regex("^;[[:blank:]]*.*")));
    EXPECT_TRUE(regexMatch(";	By user: jenkins (-1)", Regex("^;[[:blank:]]*By user:.*")));
    EXPECT_TRUE(regexMatch(";	By user: jenkins (-1)", Regex("^;[[:blank:]]*By user:.*$")));
    EXPECT_TRUE(regexMatch(";	Executable:   D:\\jenkins-build\\workspace\\Matrix_PreSubmit_master\\7b6ef549\\gromacs\\bin\\gmxpreprocess-test.exe", Regex("^;.*")));
    EXPECT_TRUE(regexMatch(";	Executable:   D:\\jenkins-build\\workspace\\Matrix_PreSubmit_master\\7b6ef549\\gromacs\\bin\\gmxpreprocess-test.exe", Regex("^;[[:blank:]]*.*")));
    EXPECT_TRUE(regexMatch(";	Executable:   D:\\jenkins-build\\workspace\\Matrix_PreSubmit_master\\7b6ef549\\gromacs\\bin\\gmxpreprocess-test.exe", Regex("^;[[:blank:]]*Executable:.*")));
    EXPECT_TRUE(regexMatch(";	Executable:   D:\\jenkins-build\\workspace\\Matrix_PreSubmit_master\\7b6ef549\\gromacs\\bin\\gmxpreprocess-test.exe", Regex("^;[[:blank:]]*Executable:.*$")));
}
