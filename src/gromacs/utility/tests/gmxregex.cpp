...
//! Helper
gmx::ConstArrayRef<RegexTestHelper> regexToTest(regexArray);

TEST(RegexTest, Compiles)
{
    for (size_t i = 0; i != regexToTest.size(); ++i)
    {
        EXPECT_NO_THROW_GMX(gmx::Regex r(regexToTest[i].regex_));
    }
}
...
TEST(RegexTest, Matches)
{
    for (size_t i = 0; i != regexToTest.size(); ++i)
    {
        try
        {
            gmx::Regex r(regexToTest[i].regex_);
            EXPECT_TRUE(gmx::regexMatch(regexToTest[i].match_, r)) << "with regex '" << regexToTest[i].regex_ << "' and match '" << regexToTest[i].match_ << "'";
        }
        catch (...)
        {
            // If we failed to compile the regex, we don't want the unit
            // test output to spam about the unexpected exception
        }
    }
}
