 ...
 *  \param alignment  Alignment specification in bytes.
 *
 ...
 */
void *
alignedMalloc(std::size_t bytes, std::size_t alignment);
 ...
 *  \tparam alignment  Alignment of memory, specified in bytes
 *
 ...
 * aligned load and store operations in those containers. The memory will also
 * be padded by adding another alignment bytes at the end of the area, to help
...
class AlignedAllocator
{
    public:
        ...
        typedef size_t         size_type;       //!< Integer type to use for size of objects
        ...
        struct rebind
        {
            typedef AlignedAllocator<U, alignment> other; //!< Align class U with our alignment
        };

        /*! \brief Default constructor, required by standard library */
        inline
        AlignedAllocator() {}

        /*! \brief Copy constructor, required by standard library
         *
         *  \param a Allocator to be copied
         */
        AlignedAllocator(const AlignedAllocator gmx_unused &a) {}

        /*! \brief Templated copy constructor
         *
         *  \tparam U  Type of objects allocated by a
         *  \param  a  Allocator to be copied
         */
        template <class U>
        AlignedAllocator(const AlignedAllocator<U, alignment> gmx_unused &a) {}
         ...
         */
        pointer
        allocate(size_type n, typename std::allocator<void>::const_pointer gmx_unused hint = 0)
        {
            void *p = internal::alignedMalloc(n*sizeof(T), alignment);
         ...
         */
        void
        deallocate(pointer p, size_type gmx_unused n)
        {
         ...
         */
        size_type
        max_size() const { return size_type(-1) / sizeof(T); }
         ...
         */
        bool
        operator==(const AlignedAllocator &gmx_unused rhs) const { return true; }
