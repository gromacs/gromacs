...
typedef boost::shared_ptr<ErrorMessage> ErrorMessagePointer;

class ErrorMessage
{
    public:
        ErrorMessage(const char *reason) : reason_(reason) {}
        ErrorMessage(const std::string &reason) : reason_(reason) {}
        ...
        const std::vector<ErrorMessagePointer> &details() const
        {
            return details_;
        }
        ...
        void addExceptionAsDetails(const GromacsException &ex);

    private:
        std::string                             reason_;
        std::vector<std::string>                context_;
        std::vector<ErrorMessagePointer>        details_;
};
