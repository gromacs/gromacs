...
std::string bromacs()
{
    ...
    gmx::ConstArrayRef<const char *> bromacsArrayRef(bromacsArray);

    if (beCool())
    {
        std::default_random_engine            generator;
        std::uniform_int_distribution<size_t> distribution(0, bromacsArrayRef.size()-1);
        size_t index = distribution(generator);
        return bromacsArrayRef[index];
    }
    else
    {
        return "GROMACS";
...
std::string getCoolQuote()
{
    ...
    gmx::ConstArrayRef<Quote> quoteArrayRef(quoteArray);

    if (beCool())
    {
        std::default_random_engine            generator;
        std::uniform_int_distribution<size_t> distribution(0, quoteArrayRef.size()-1);
        size_t index = distribution(generator);
        return formatString("gcq#:%d \"%s\" (%s)", index, quoteArrayRef[index].text, quoteArrayRef[index].author);
    }
    else
    {
