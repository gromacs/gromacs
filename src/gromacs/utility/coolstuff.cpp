...
template <typename T>
size_t getRandomIndex(gmx::ConstArrayRef<T> arrayRef)
{
...
std::string bromacs()
{
    ...
    if (beCool())
    {
        auto index = getRandomIndex(ConstArrayRef<const char *>(bromacsArray));
        return bromacsArray[index];
    }
    else
    {
        return "GROMACS";
...
std::string getCoolQuote()
{
    ...
    if (beCool())
    {
        auto index = getRandomIndex(ConstArrayRef<Quote>(quoteArray));
        return formatString("gcq#:%d \"%s\" (%s)", index, quoteArray[index].text, quoteArray[index].author);
    }
    else
    {
