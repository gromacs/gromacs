...
#    ifdef GMX_OPENMP
         ...
         * that probably isn't a problem, we don't make a fuzz about it (no warning)
        ...
        {
            gmx_warning("Gromacs compiled with OpenMP support, but no thread support in MPI library. Keep your fingers crossed.");
            MPI_Init(argc, argv);
        }
#    else
        MPI_Init(argc, argv);
