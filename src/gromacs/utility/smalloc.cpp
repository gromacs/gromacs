    ...
    else
    {
        if ((p = malloc(size)) == nullptr)
        {
            gmx_fatal(errno, __FILE__, __LINE__,
                      ...
                      static_cast<gmx_int64_t>(size), name, file, line);
        }
    ...
    else
    {
        ...
        if ((p = calloc(nelem, elsize)) == nullptr)
        {
            gmx_fatal(errno, __FILE__, __LINE__,
                      ...
                      " elements of size %" GMX_PRId64
                      " for %s\n(called from file %s, line %d)",
                      static_cast<gmx_int64_t>(nelem), static_cast<gmx_int64_t>(elsize), name, file, line);
        }
#endif
    }
    return p;
    ...
    else
    {
        ...
        if (p == nullptr)
        {
            gmx_fatal(errno, __FILE__, __LINE__,
                      ...
                      "(called from file %s, line %d)",
                      static_cast<gmx_int64_t>(size), name, name, ptr, file, line);
        }
    }
    return p;
}
