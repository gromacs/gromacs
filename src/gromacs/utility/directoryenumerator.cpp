        ...
        std::vector<std::string> *result)
{
    ...
    while (dir.nextFile(&nextName))
    {
        if (debug)
        {
            std::fprintf(debug, "dir '%s' %d file '%s'\n",
                         dirname, static_cast<int>(count), nextName.c_str());
        }
    ...
    std::sort(result->begin() + oldCount, result->begin() + oldCount + count);
}
