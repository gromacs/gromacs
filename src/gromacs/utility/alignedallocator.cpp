 ...
 *
 *  \param bytes      Amount of memory (bytes) to allocate
 *  \param alignment  Alignment specification in bytes, must be a power of 2.
 *
 * \return Valid pointer if the allocation worked, otherwise nullptr. Asking
 *         for zero bytes of memory corresponds to a failed allocation.
 *
...
void *
alignedMalloc(std::size_t bytes, std::size_t requestedAlignment)
{
    if (bytes == 0)
    {
        return nullptr;
    ...
    // We round up to the next power of two, since some of the special allocators
    // will have problems for non-power-of-two alignments.
    std::size_t alignment = 1;
    while (alignment < requestedAlignment)
    {
        alignment <<= 1;
    }
