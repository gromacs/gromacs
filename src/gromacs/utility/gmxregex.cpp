...
bool regexMatch(const char *value, const Regex &regex)
{
    if (regex.impl_.get() == NULL)
    {
        return false;
    }
    return regex.impl_->match(value);
}

bool regexMatch(const std::string &value, const Regex &regex)
{
    return regexMatch(value.c_str(), regex);
}
