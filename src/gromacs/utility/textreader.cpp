...
const std::string &TextReader::currentLine() const
{
    return impl_->currentLine_;
}

std::string TextReader::currentLineTrimmed() const
{
    std::string  trimmedLine = impl_->currentLine_;
    const size_t endPos      = trimmedLine.find_last_not_of(" \t\r\n");
    if (endPos != std::string::npos)
    {
        trimmedLine.resize(endPos + 1);
    }
    return trimmedLine;
}
