...
/*! \brief Implements initialization for the GROMACS library.
 *
...
public:
     ...
     */
    static ProgramInfo &init(const char *realBinaryName, int *argc, char ***argv);
    /*! \brief
     * Initializes the Gromacs library.
     *
     * \param[in] argc  argc value passed to main().
     ...
     * Does not throw. Terminates the program on out-of-memory error.
     */
    static ProgramInfo &init(int *argc, char ***argv);
    //@}
    /*! \brief
...
private:
    //! Maintains counter of attempts to initialize MPI
    static int _initializationCounter;
};
