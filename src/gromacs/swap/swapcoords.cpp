        ...
        gmx_bool        bIsSolvent)
{
    ...
    /* Consistency checks */
    if (nMolNotInComp[eCompA] + nMolNotInComp[eCompB] != static_cast<int>(g->atomset.numAtomsGlobal() / g->apm))
    {
        fprintf(stderr, "%s Warning: Inconsistency while assigning '%s' molecules to compartments. !inA: %d, !inB: %d, total molecules %lu\n",
                SwS, g->molname, nMolNotInComp[eCompA], nMolNotInComp[eCompB], g->atomset.numAtomsGlobal()/g->apm);
    }

    int sum = g->comp[eCompA].nMol + g->comp[eCompB].nMol;
    if (sum != static_cast<int>(g->atomset.numAtomsGlobal()/g->apm))
    {
        fprintf(stderr, "%s Warning: %lu molecules are in group '%s', but altogether %d have been assigned to the compartments.\n",
                SwS, g->atomset.numAtomsGlobal()/g->apm, g->molname, sum);
    }
}
        ...
        const MdrunOptions       &mdrunOptions)
{
    ...
    for (int i = 0; i < s->ngrp; i++)
    {
        s->group.emplace_back(atomSets->add(sc->grp[i].nat, sc->grp[i].ind));
        ...
        if (!bAppend)
        {
            ...
            for (int ig = 0; ig < s->ngrp; ig++)
            {
                ...
                fprintf(s->fpout, "# %s group '%s' contains %lu atom%s",
                        ...
                        g->molname, g->atomset.numAtomsGlobal(), (g->atomset.numAtomsGlobal() > 1) ? "s" : "");
