...
typedef struct swap_compartment
{
    ...
    int                nat_before;            /**< Number of atoms before swapping.              */
...
typedef struct swap_group
{
    ...
    real              vacancy[eCompNR];       /**< How many particles need to be swapped in?       */
    int               fluxfromAtoB[eChanNR];  /**< Net flux of particles of this group per channel */
    int               nCyl[eChanNR];          /**< Number of particles residing in a channel       */
        ...
        const char    comment[])
{
    ...
    for (int iComp = 0; iComp < eCompNR; iComp++)
    {
        for (int ig = eSwapFixedGrpNR; ig < s->ngrp; ig++)
        {
            ...
            fprintf(s->fpout, "%10d%10.1f%10d", comp->nat, comp->nmolAv - comp->nmolReq, comp->inflow_net);
        }
    }
...
static void update_time_window(t_compartment *comp, int values, int replace, int apm)
{
    real average;
    int  i;
    ...
    if (replace >= 0)
    {
        comp->nmolPast[replace] = comp->nat / apm;
    }
        ...
        FILE           *fpout)
{
    ...
    in_cyl0 = is_in_channel(particle_position, s->group[eGrpSplit0].center, sc->cyl0u, sc->cyl0l, cyl0_r2, s->pbc, sd);
    in_cyl1 = is_in_channel(particle_position, s->group[eGrpSplit1].center, sc->cyl1u, sc->cyl1l, cyl1_r2, s->pbc, sd);
        ...
        switch (*channel_label)
        {
            case eChHistPassedNone:
                ...
                fprintf(stderr, " %s Warning! Step %s, ion %d moved from %s to %s\n",
                        SwS, gmx_step_str(step, buf), iparticle, DomainString[*comp_from], DomainString[*comp_now]);
                if (bRerun)
                {
                ...
                else
                {
                    ...
                    fprintf(s->fpout, " # Warning: step %s, ion %d moved from %s to %s (probably through the membrane)\n",
                            gmx_step_str(step, buf), iparticle,
...
/*! \brief Determines which atoms of the group are in which compartment */
static void compartmentalize_group_atoms(
        t_swapgrp      *g,
        t_commrec      *cr,
        t_swapcoords   *sc,
        matrix          box,
        ...
        gmx_bool        bRerun)
{
    ...
    for (comp = eCompA; comp <= eCompB; comp++)
    {
        ...
        /* Loop over the MOLECULES of this group */
        for (i = 0; i < g->nat; i += g->apm)
        {
            /* Is this first atom of the molecule in the compartment that we look at? */
            if (compartment_contains_atom(left, right, g->xc[i][sd], box[sd][sd], sc->bulkOffset[comp], &dist) )
            {
                /* Add the whole molecule to the list containing molecules of this type */
                for (int j = 0; j < g->apm; j++)
                {
                    add_to_list(i+j, &g->comp[comp], dist);
                }

                /* Master also checks through which channel each ion has passed */
                if (MASTER(cr) && (g->comp_now != NULL))
                {
                    globalAtomNr = g->ind[i] + 1; /* PDB index starts at 1 ... */
                    detect_flux_per_channel(g, globalAtomNr, comp, g->xc[i],
                                            &g->comp_now[i], &g->comp_from[i], &g->channel_label[i],
                                            sc, cyl0_r2, cyl1_r2, step, bRerun, fpout);
                }
            }
            else
            {
                not_in_comp[comp] += g->apm;
            }
        }
        /* Correct the time-averaged number of particles in the compartment */
        update_time_window(&g->comp[comp], sc->nAverage, replace, g->apm);
    }
    ...
    /* Consistency checks */
    if (not_in_comp[eCompA] + not_in_comp[eCompB] != g->nat)
    {
        if (NULL != fpout)
        {
            fprintf(fpout, "# Warning: Inconsistency while assigning atoms to compartments. !inA: %d, !inB: %d, total atoms %d\n",
                    not_in_comp[eCompA], not_in_comp[eCompB], g->nat);
            fflush(fpout);
        }
        else
        {
            fprintf(stderr, "%s rank %d: Inconsistency while assigning atoms to compartments. !inA: %d, !inB: %d, total atoms %d\n",
                    SwS, cr->nodeid, not_in_comp[eCompA], not_in_comp[eCompB], g->nat);

        }
    }
    sum =   g->comp[eCompA].nat + g->comp[eCompB].nat;
    if (sum != g->nat)
    {
        if (NULL != fpout)
        {
            fprintf(fpout, "# Warning: %d atoms are in the swap group, but altogether %d have been assigned to the compartments.\n",
                    g->nat, sum);
            fflush(fpout);
        }
        else
        {
            fprintf(stderr, "%s rank %d: %d atoms are in the swap group, but altogether %d have been assigned to the compartments.\n",
                    SwS, cr->nodeid, g->nat, sum);
        }
    }
}
        ...
        FILE         *fpout)
{
    ...
    for (comp = eCompA; comp <= eCompB; comp++)
    {
        ...
        for (i = 0; i < gsol->nat; i += gsol->apm)
        {
            if (compartment_contains_atom(left, right, gsol->xc[i][sd], box[sd][sd], sc->bulkOffset[comp], &dist))
            {
                /* Add the whole molecule to the list */
                for (j = 0; j < gsol->apm; j++)
                {
                    add_to_list(i+j, &gsol->comp[comp], dist);
                }
            }
            else
            {
                not_in_comp[comp] += gsol->apm;
            }
        }
    }
    ...
    if (NULL != fpout)
    {
        fprintf(fpout, "# Solv. molecules in comp.%s: %d   comp.%s: %d\n",
                CompStr[eCompA], gsol->comp[eCompA].nat/gsol->apm,
                CompStr[eCompB], gsol->comp[eCompB].nat/gsol->apm);
    }

    /* Consistency checks */
    if (not_in_comp[eCompA] + not_in_comp[eCompB] != gsol->nat)
    {
        if (NULL != fpout)
        {
            fprintf(fpout, "# Warning: Inconsistency during solvent compartmentalization. !inA: %d, !inB: %d, solvent atoms %d\n",
                    not_in_comp[eCompA], not_in_comp[eCompB], gsol->nat);
            fflush(fpout);
        }
        else
        {
            fprintf(stderr, "%s rank %d: Inconsistency during solvent compartmentalization. !inA: %d, !inB: %d, solvent atoms %d\n",
                    SwS, cr->nodeid, not_in_comp[eCompA], not_in_comp[eCompB], gsol->nat);
        }
    }
    sum = gsol->comp[eCompA].nat + gsol->comp[eCompB].nat;
    if (sum != gsol->nat)
    {
        if (NULL != fpout)
        {
        ...
        gmx_bool          bRerun)
{
    ...
    for (ig = eSwapFixedGrpNR; ig < s->ngrp; ig++)
    {
        ...
        compartmentalize_group_atoms(g, cr, sc, box, 0, s->fpout, bRerun);
        ...
        for (ic = 0; ic < eCompNR; ic++)
        {
            ...
            if (requested < 0)
            {
                g->comp[ic].nmolReq = g->comp[ic].nat / g->apm;
            }
            else
            {
        ...
        tot = (g->comp[eCompA].nat + g->comp[eCompB].nat) / g->apm;
        ...
        for (ic = 0; ic < eCompNR; ic++)
        {
            g->comp[ic].nmolAv = g->comp[ic].nat / g->apm;
            for (i = 0; i < sc->nAverage; i++)
            {
                g->comp[ic].nmolPast[i] = g->comp[ic].nat / g->apm;
            }
        }
    }
}
        ...
        t_swapcoords *swap)
{
    ...
    for (ig = eSwapFixedGrpNR; ig < s->ngrp; ig++)
    {
        ...
        for (ic = 0; ic < eCompNR; ic++)
        {
            ...
            gmx_bcast(sizeof(g->comp[ic].nat    ), &(g->comp[ic].nat    ), cr);
        ...
        int               ePBC)
{
    ...
    else
    {
        ...
        for (int ig = eSwapFixedGrpNR; ig < s->ngrp; ig++)
        {
            swapstateIons_t *gs = &swapstate->ionType[ig - eSwapFixedGrpNR];

            gs->nMol = sc->grp[ig].nmolReq[eCompA] + sc->grp[ig].nmolReq[eCompB];
        }
...
/* Returns TRUE if we started from an older .tpr and thus need to re-sort
 * anions and cations into separate groups */
        ...
        unsigned long           Flags)
{
    ...
    /* Update the time-averaged number of particles for all swap groups and compartments */
    for (int ig = eSwapFixedGrpNR; ig < sc->ngrp; ig++)
    {
        g = &s->group[ig];
        for (int ic = 0; ic < eCompNR; ic++)
        {
            update_time_window(&g->comp[ic], sc->nAverage, -1, g->apm);
        }
    }
        ...
        const char     type[])
{
     ...
     */
    for (i = 0; i < comp->nat_before; i += apm)
    {
        if (comp->dist[i] < d)
        {
            ibest = i;
    ...
    if (ibest < 0)
    {
        gmx_fatal(FARGS, "Could not get index of %s atom. Compartment contains %d %s group atoms before swaps (%d atoms/molecule).",
                  type, comp->nat_before, type, apm);
    }
        ...
        gmx_bool          bRerun)
{
    ...
    for (ig = eSwapFixedGrpNR; ig < s->ngrp; ig++)
    {
        ...
        compartmentalize_group_atoms(g, cr, sc, box, step, s->fpout, bRerun);
    }
    ...
    if (bSwap)
    {
        ...
        /* Save number of solvent atoms per compartment prior to any swaps */
        g->comp[eCompA].nat_before = g->comp[eCompA].nat;
        g->comp[eCompB].nat_before = g->comp[eCompB].nat;
        ...
        for (ig = eSwapFixedGrpNR; ig < s->ngrp; ig++)
        {
            ...
            for (ic = 0; ic < eCompNR; ic++)
            {
                /* Determine in which compartment particles are missing and where they are too many */
                g->vacancy[ic] = g->comp[ic].nmolReq - g->comp[ic].nmolAv;

                /* Save number of swap group atoms per compartment prior to swaps */
                g->comp[ic].nat_before = g->comp[ic].nat;
            }
        }
        ...
        for (ig = eSwapFixedGrpNR; ig < s->ngrp; ig++)
        {
            ...
            for (thisC = 0; thisC < eCompNR; thisC++)
            {
                ...
                while (g->vacancy[thisC] >= sc->threshold)
                {
                    ...
                    isol = get_index_of_distant_atom(&gsol->comp[thisC], gsol->apm, "solvent");

                    /* Get the xc-index of a particle from the other compartment */
                    iion = get_index_of_distant_atom(&g->comp[otherC], g->apm, "swap");
                    ...
                    if (MASTER(cr))
                    {
                        g->channel_label[iion] = eChHistPassedNone;
                        g->comp_from[iion]     = eDomainNotset;
                    }
                    /* That was the swap */
                    nswaps++;
                }
