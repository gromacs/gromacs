        ...
        gmx_mtop_t                 *mtop)
{
     ...
     * first solvent atom: */
    int molb = 0;
    gmx_mtop_atomnr_to_molblock_ind(mtop, ind[0], &molb, NULL, NULL);
    ...
    for (int i = 1; i < nat; i++)
    {
        gmx_mtop_atomnr_to_molblock_ind(mtop, ind[i], &molb, NULL, NULL);
        ...
        t_commrec    *cr)
{
    ...
    for (int i = 0; i < g->nat; i++)
    {
        gmx_mtop_atomnr_to_atom(mtop, g->ind[i], &molb, &atom);
        if (atom->q < 0)
        {
        ...
        unsigned long           Flags)
{
    ...
    for (int ig = eGrpSolvent; ig < s->ngrp; ig++)
    {
        ...
        {
            gmx_mtop_atomnr_to_atom(mtop, g->ind[j], &molb, &atom);
            g->m[j] = atom->m;
            charge += atom->q;
        }
