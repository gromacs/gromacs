 ...
 *
 * Function pointer used to tell the table filling routine whether it
...
class LongRangeInteractionTables
{
    public:
        ...
        const std::vector<double> &coulombFDV0() const { return tableCoulomb_->tableFDV0(); }
        //! Return the Van der Waals force + energy table
        const std::vector<double> &vdwFDV0() const { return tableVdw_->tableFDV0(); }
    private:
