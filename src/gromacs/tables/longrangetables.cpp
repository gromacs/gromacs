...
static void v_lj_ewald_lr_analytical(const double *beta, double r, double *e, double *f)
{
    if (r == 0)
    {
        *e = pow((*beta), 6.0)/6.0;
    ...
    else
    {
        ...
        double br6     = br4*br2;
        double r6      = pow(r, 6.0);
        double expmbr2 = exp(-br2);
        double gmxem   = gmx_expm1(-br2);
        /* In order to preserve (obtain) accuracy it is necessary to use
         * the gmx_expm1 function to replace (exp(x)-1) which becomes close to 0
         * as x -> 0.
...
{
    v_lj_ewald_lr_analytical(beta, r, e, f);
     ...
     */
    double x  = pow((*beta)*r, 2);
    double ea = pow(*beta, 6)*(pow(x, 7)*(x/887040.0 - 1/100800.0) + pow(x, 5)*(x/12960.0 - 1/1920.0) +
                               pow(x, 3)*(x/336.0 - 1/72.0) + x*(x/20.0 - 1/8.0) + 1/6.0);
    double fa = pow(*beta, 8)*r*(pow(x, 6)*(-x/55440.0 + 1/7200.0) + pow(x, 4)*(-x/1080.0 + 1/192.0) +
...
static gmx::QuadraticSplineTable *table_spline3_fill(unsigned int                    ntab,
                                                     double                          dx,
                                                     ...
                                                     const double                   *params)
{
    ...
    return new gmx::QuadraticSplineTable(ntab, dx, &(table_v[0]), &(table_f[0]));
}
                                                     ...
                                                     double                          r_tab)
{
    ...
    if (type_coul == eelEWALD || EEL_PME(type_coul))
    {
        ...
        {
            tableCoulomb_ = new QuadraticSplineTable(table_length, table_dx,
    ...
    if (EVDW_PME(type_vdw))
    {
        if (NULL != table_v_vdw)
        {
            tableVdw_   = new QuadraticSplineTable(table_length, table_dx,
