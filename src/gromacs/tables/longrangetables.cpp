...
/*! \file
 * \brief
 * Implements utilities for initializing long range correction tables
 *
...
void v_q_ewald_lr(const double *params, double r, double *e, double *f)
{
    ...
    else
    {
        *e = gmx_erfd(br)/r;
         ...
         */
        *f = gmx_erfd(br)/(r*r) - 2*(*params)*exp(-br*br)/(r*sqrt(M_PI));
    }
}
 ...
 * \param[out] f    Force
 */
static void v_lj_ewald_lr_analytical(const double *params, double r, double *e, double *f)
{
    if (r == 0)
    {
        *e = pow((*params), 6.0)/6.0;
        *f = 0;
    }
    else
    {
...
void v_lj_ewald_lr(const double *params, double r, double *e, double *f)
{
     ...
     */
    double x  = pow((*params)*r, 2);
    double ea = pow(*params, 6)*(pow(x, 7)*(x/887040.0 - 1/100800.0) + pow(x, 5)*(x/12960.0 - 1/1920.0) +
                                 pow(x, 3)*(x/336.0 - 1/72.0) + x*(x/20.0 - 1/8.0) + 1/6.0);
    double fa = pow(*params, 8)*r*(pow(x, 6)*(-x/55440.0 + 1/7200.0) + pow(x, 4)*(-x/1080.0 + 1/192.0) +
...
double longRangeCorrectionTableSpacing(int    type_coul,
...
unsigned int longRangeCorrectionTableSize(double rtab,
                                          double dx)
{
    return static_cast<unsigned int>(rtab/dx)+1;
}
