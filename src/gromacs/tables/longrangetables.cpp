                                            ...
                                            const double                   *params)
{
    ...
    /* Fill the input table at double the spacing */
    ntab *= 2;
    dx   /= 2.0;
    for (unsigned int i = 0; (i < ntab); i++)
    {
        double x;
        x = i*dx;
        table_v.push_back(v_ana(params, x));
    }
    return new gmx::SplineTable(ntab, dx, &(table_v[0]), NULL);
}
                                                    ...
                                                    double r_vdw)
{
    ...
    tabSpacing_  = 1.0/sc;
    double tabLength = std::max(r_coul, r_vdw);
