...
void v_q_ewald_lr(const double *beta, double r, double *e, double *f)
{
    ...
    {
        *e = gmx_erfd(br)/r;
        *f = -(2*(*beta)*exp(-br*br)/(r*sqrt(M_PI)) - gmx_erfd(br)/(r*r));
    }
}
...
void v_lj_ewald_lr(const double *beta, double r, double *e, double *f)
{
    ...
    else
    {
        ...
        *f  = -(6*gmx_expm1(-br2) + expbr2*(6*br2 + 3*br4 + br2*br4))/(r6*r);
    }
}
                                  ...
                                  double func_tol)
{
    ...
    /* This hack is needed to get the same table spacing as in older
     * version. This since the definition of the GMX_FLOAT_EPS changed.
     */
    sc_deriv *= sqrt(2.0);
...
void LongRangeInteractionTables::initTabScaleLength(int    type_coul,
                                                    ...
                                                    double r_tab)
{
    ...
    tabSize_   = 1+static_cast<int>(tabLength/tabSpacing_);
}

LongRangeInteractionTables::LongRangeInteractionTables(int                             type_coul,
                                                       double                          ewaldcoeff_coul,
                                                       double                          r_coul,
                                                       double                         *params_coul,
                                                       ...
                                                       double                          r_tab)
{
    ...
    if (type_coul == eelEWALD || EEL_PME(type_coul))
    {
        if (NULL != table_v_coul)
        {
            ...
            tabSpacing_   = tableCoulomb_->tabSpacing();
        }
        else
        {
            tableCoulomb_ = table_spline3_fill(tabSize_, tabSpacing_,
                                               v_coul, params_coul);
        }
    }
    if (EVDW_PME(type_vdw))
    {
        if (NULL != table_v_vdw)
        {
            ...
            tabSpacing_ = tableVdw_->tabSpacing();
        }
        else
        {
            tableVdw_ = table_spline3_fill(tabSize_, tabSpacing_,
                                           v_vdw, params_vdw);
        }
    }
    if ((NULL != tableCoulomb_) && (NULL != tableVdw_))
    {
        ...
        snprintf(buf, sizeof(buf), "Spacing for Coulomb %g and Vdw %g\n",
                 tableCoulomb_->tabSpacing(),
                 tableVdw_->tabSpacing());
        GMX_RELEASE_ASSERT((tableVdw_->tabSpacing() == tableCoulomb_->tabSpacing()),
                           buf);
    }
}

LongRangeInteractionTables::LongRangeInteractionTables(const LongRangeInteractionTables &L)
{
    tabSpacing_   = L.tabSpacing();
    tabSize_      = L.tabSize();
    tableCoulomb_ = NULL;
    tableVdw_     = NULL;
}
