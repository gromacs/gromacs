...
class InteractionTables
{
    public:
        ...
        ~InteractionTables();
        /*! \libinternal\brief Return table spacing (nm)
         */
        double tabScale() { return tabScale_; }
        /*! \libinternal\brief Return table length (nm)
         */
        double tabLength() { return tabLength_; }
         ...
         */
        void fillEwaldSpline3(double                         *params,
                              EwaldType                       et,
                              interaction_potential_function  v_lr);
        //! Return the Coulomb force table
        const std::vector<real> coulombF() const { return tabCoulF_; }
        //! Return the Coulomb potential table
        const std::vector<real> coulombV() const { return tabCoulV_; }
        //! Return the Coulomb force + energy table
        const std::vector<real> coulombFDV0() const { return tabCoulFDV0_; }
        //! Return the Van der Waals force table
        const std::vector<real> vdwF() const { return tabVdwF_; }
        //! Return the Van der Waals potential table
        const std::vector<real> vdwV() const { return tabVdwV_; }
        //! Return the Van der Waals force + energy table
        const std::vector<real> vdwFDV0() const { return tabVdwFDV0_; }
    private:
        //! Table scaling (nm)
        double            tabScale_;
        //! Table length (nm)
        double            tabLength_;
        //! Coulomb force table
        std::vector<real> tabCoulF_;
        //! Coulomb energy table
        std::vector<real> tabCoulV_;
         ...
         */
        std::vector<real> tabCoulFDV0_;

        //! Vdw force table for LJ-PME
        std::vector<real> tabVdwF_;
        //! Vdw energy table for LJ-PME
        std::vector<real> tabVdwV_;
