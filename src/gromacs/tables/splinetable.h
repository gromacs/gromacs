 ...
 *
 * Functions using the tables should use methods tabScale() and tabSize()
...
class SplineTable
{
    public:
        ...
        unsigned int tabSize() const { return tableFDV0_.size(); }

        //! Return the force + energy table
        const std::vector<double> &tableFDV0() const { return tableFDV0_; }
    private:
        //! Table scaling (nm)
        double              tabScale_;
