...
class QuadraticSplineTableTest : public ::testing::Test
{
    protected:
        ...
        {
            checker_.setDefaultTolerance(test::relativeToleranceAsFloatingPoint(1, 1000*GMX_REAL_EPS));
        }
        ...
        void testAnalytically()
        {
            ...
            test::FloatingPointTolerance toler =
                test::relativeToleranceAsFloatingPoint(1, 1e-4);
...
TEST_F(QuadraticSplineTableTest, EwaldLJ)
{
    ...
    for (int i = 1; (i < 8); i++)
    {
        ...
        v.push_back((-gmx_expm1(-br2) - expbr2*(br2 + 0.5*br4))/r6);
    }
    testSplineTableV(dx, v);
}
