                                                    ...
                                                    const std::pair<real, real>          &testRange)
{
    ...
    FloatingPointTolerance tolerance(relativeToleranceAsFloatingPoint(0.0, tolerance_));
    ...
    for (real x = testRange.first; x < testRange.second; x += dx)
    {
        ...
        //
        // We only fail the test if both of these tolerances are violated.

        FloatingPointDifference funcDiff(refFuncValue, testFuncValue);
        FloatingPointDifference derDiff(refDerValue, testDerValue);

        real                    allowedAbsTruncationErr = 5.0*std::abs(secondDerivative) * x * GMX_REAL_EPS;

        if ((!tolerance.isWithin(funcDiff) && funcDiff.asAbsolute() > allowedAbsTruncationErr) ||
            (!tolerance.isWithin(derDiff)  &&  derDiff.asAbsolute() > allowedAbsTruncationErr))
        {
            ADD_FAILURE()
