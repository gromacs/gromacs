...
TYPED_TEST(SplineTableTest, HandlesIncorrectNumericalInput)
{
    ...
    EXPECT_THROW_GMX(TypeParam( {{"EmptyVectors", functionValues, derivativeValues}},
                                0.001, {1.0, 2.0}), gmx::InconsistentInputError);
    ...
    EXPECT_THROW_GMX(TypeParam( {{"EmptyVectors", functionValues, derivativeValues}},
                                0.1, {1.0, 2.0}), gmx::InconsistentInputError);
    ...
    EXPECT_THROW_GMX(TypeParam( {{"NumericalBadLJ12", functionValues, badDerivativeValues}},
                                spacing, {0.2, 1.0}), gmx::InconsistentInputError);

    // Spacing 1e-3 is not sufficient for r^-12 in range [0.1,1.0]
    // Make sure we get a tolerance error
    EXPECT_THROW_GMX(TypeParam( {{"NumericalLJ12", functionValues, derivativeValues}},
                                spacing, {0.2, 1.0}), gmx::ToleranceError);
}
...
TYPED_TEST(SplineTableTest, NumericalInputPmeCorr)
{
    ...
    TypeParam  pmeCorrTable( {{"NumericalPMECorr", functionValues, derivativeValues}},
                             inputSpacing, range, tolerance);
...
TYPED_TEST(SplineTableTest, TwoFunctions)
{
    ...
    // These should really be asserts, since the following ones compare to these values,
    // but that appears to trigger an internal compiler error in MSVC
...
TYPED_TEST(SplineTableTest, ThreeFunctions)
{
    ...
    EXPECT_LT(funcErr1, TypeParam::defaultTolerance);
    EXPECT_LT(derErr1, TypeParam::defaultTolerance);
    EXPECT_LT(funcErr2, TypeParam::defaultTolerance);
    EXPECT_LT(derErr2, TypeParam::defaultTolerance);
