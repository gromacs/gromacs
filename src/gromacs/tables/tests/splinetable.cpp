                                                    ...
                                                    const std::pair<real, real>          &testRange)
{
    ...
    for (real x = testRange.first; x < testRange.second; x += dx)
    {
        ...
        // 1) First, we have the normal relative error of the test vs. reference value. Since
        //    the value can be zero, the absolute difference (test-ref) should be smaller than (tolerance*ref).
        // 2) Second, due to the loss-of-accuracy when calculating the index through rtable
        //    there is an internal absolute error that we can calculate (see below). This too applies
        //    to the absolute difference.
        //
        // In practice, we calculate both of these limits on the absolute value, take the maximum, and use that.

        real allowedFuncErr     = tolerance_ * std::abs(refFuncValue);
        real allowedDerErr      = tolerance_ * std::abs(refDerValue);
        real indexTruncationErr = 5.0*std::abs(secondDerivative) * x * GMX_REAL_EPS;

        allowedFuncErr          = std::max(allowedFuncErr, indexTruncationErr);
        allowedDerErr           = std::max(allowedDerErr, indexTruncationErr);

        real funcErr            = std::abs( testFuncValue - refFuncValue );
        real derErr             = std::abs( testDerValue  - refDerValue  );

        if (funcErr > allowedFuncErr || derErr > allowedDerErr)
        {
            ADD_FAILURE()
...
{
    std::pair<real, real>  range(0.15, 4.0);
    real                   tolerance = 1e-5;
