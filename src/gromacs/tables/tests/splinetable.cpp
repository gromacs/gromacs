...
class SplineTableTest : public ::testing::Test
{
    protected:
        ...
        {
            SplineTable st(ntable, dx, eSplineQuadratic, table_in_v, NULL);
        ...
        void testAnalytically(int           ntable,
                              double        dx,
                              const double *table_in_v)
        {
            SplineTable          st(ntable, dx, eSplineQuadratic, table_in_v, NULL);
            ...
            for (unsigned int i = 0; (i < 6); i++)
            {
                ...
                double eref  = r*r;
                double ediff = std::fabs((etab-eref)/eref);
                snprintf(buf, sizeof(buf), "energy is %g, should be %g, reldiff %g, r is %g",
                         etab, eref, ediff, r);
                GMX_RELEASE_ASSERT((ediff < toler), buf);
                double ftab  = st.force(r);
                double fref  = -2*r;
                double fdiff = std::fabs((ftab-fref)/fref);
                snprintf(buf, sizeof(buf), "force is %g, should be %g, reldiff %g, r is %g",
                         ftab, fref, fdiff, r);
                GMX_RELEASE_ASSERT((fdiff < toler), buf);
            }
        }
        ...
        {
            SplineTable st(ntable, dx, eSplineQuadratic, table_in_v, table_in_f);
            checker_.checkInteger(st.size(), "tableSize");
            checker_.checkDouble(st.spacing(), "tableSpacing");
            double r = 0.735*st.size()*st.spacing();
            checker_.checkDouble(r, "distance");
...
TEST_F(SplineTableTest, VHarmonic)
{
    /* Harmonic potential */
    double v[8];
    double dx = 0.1;
    ...
    testAnalytically(8, dx, v);
}

TEST_F(SplineTableTest, VFHarmonic)
{
    /* Harmonic potential */
    double v[8], f[8];
    double dx = 0.03;
...
TEST_F(SplineTableTest, EwaldLJ)
{
