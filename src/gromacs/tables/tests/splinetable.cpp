...
class SplineTableTest : public ::testing::Test
{
    protected:
        ...
        {
            SplineTable st(ntable, dx, table_in_v, NULL);
            checker_.checkInteger(st.tabSize(), "tableSize");
            checker_.checkDouble(st.tabSpacing(), "tableSpacing");
            double r = 0.735*st.tabSize()*st.tabSpacing();
            checker_.checkDouble(r, "distance");
            checker_.checkDouble(st.energy(r), "energy");
            checker_.checkDouble(st.force(r), "force");
            const std::vector<double> v = st.tableFDV0();
            checker_.checkSequence(v.begin(), v.end(), "FDV0");
        }
        ...
        {
            SplineTable st(ntable, dx, table_in_v, table_in_f);
            checker_.checkInteger(st.tabSize(), "tableSize");
            checker_.checkDouble(st.tabSpacing(), "tableSpacing");
            double r = 0.735*st.tabSize()*st.tabSpacing();
            checker_.checkDouble(r, "distance");
