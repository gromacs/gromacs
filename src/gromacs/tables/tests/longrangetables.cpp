                        ...
                        EVDW_PME(vdwtype))
                    {
                        for (int rc = 8; (rc <= 16); rc += 2)
                        {
                            ...
                            for (int rv = 8; (rv <= 16); rv += 2)
                            {
                                double rvdw = 0.1*rv;
                                char   buf[256];
                                double dx = longRangeCorrectionTableSpacing(eeltype,
                                                                            ewaldcoeff_q,
                                                                            rcoulomb,
                                                                            vdwtype,
                                                                            ewaldcoeff_lj,
    ...
    protected:
                       ...
                       double rvdw)
        {
            ...
            double dx            = longRangeCorrectionTableSpacing(eeltype,
                                                                   ...
                                                                   ewaldcoeff_lj,
                                                                   rvdw);
            unsigned int         ntable  = longRangeCorrectionTableSize(std::max(rcoulomb, rvdw),
                                                                        dx);
            SplineTable          coulomb(ntable, dx, eSplineQuadratic,
                                         v_q_ewald_lr, &ewaldcoeff_q);
            my_check(checker_, coulomb.F(), "coulombF");
            my_check(checker_, coulomb.V(), "coulombV");
            my_check(checker_, coulomb.FDV0(), "coulombFDV0");
            const SplineTable vdw(ntable, dx, eSplineQuadratic,
                                  v_lj_ewald_lr, &ewaldcoeff_lj);
