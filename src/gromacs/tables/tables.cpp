                               ...
                               unsigned int                    stride)
{
    ...
    if (table_in_v != NULL)
    {
        ...
        snprintf(buf, sizeof(buf),
                 "For filling a quadratic spline table with tabulated potential input only (i.e. no force input), the stride has to be a mulitple of 2, not odd (%d)", stride);
        GMX_RELEASE_ASSERT(!(table_in_f == NULL && stride % 2 != 0), buf);

        snprintf(buf, sizeof(buf),
                 "The table input size (%d) is smaller than the requested output table size (%d) times the stride (%d)",
                                     ...
                                     unsigned int                    table_in_stride)
{
    ...
    unsigned int ntab = 1+static_cast<int>(tabLength_/tabScale_);

    if (type_coul == eelEWALD || EEL_PME(type_coul))
    {
        table_spline3_fill(tabCoulF_, tabCoulV_, tabCoulFDV0_,
                           ntab, tabScale_, v_coul, params_coul,
    ...
    if (EVDW_PME(type_vdw))
    {
        table_spline3_fill(tabVdwF_, tabVdwV_, tabVdwFDV0_,
                           ntab, tabScale_, v_vdw, params_vdw,
