 ...
 */
void
fillSingleCubicSplineTableData(const std::vector<double>             &function,
...
CubicSplineTable::CubicSplineTable(const std::initializer_list<const AnalyticalFuncSpecType>   &funcSpecList,
                                   const std::pair<real, real>                                 &range,
                                   real                                                         tolerance)
    : numFuncInTable_(funcSpecList.size()), range_(range)
{
    ...
    for (auto thisFuncSpec : funcSpecList)
    {
        try
        {
            internal::throwUnlessDerivativeIsConsistentWithFunction(thisFuncSpec.function, thisFuncSpec.derivative, range_);
        }
        catch (gmx::GromacsException &ex)
        {
            ex.prependContext("Error generating cubic spline table for function '" + thisFuncSpec.desc + "'");
            throw;
        }
        ...
        double thisMinQuotient = internal::findSmallestQuotientOfFunctionAndThirdDerivative(thisFuncSpec.derivative, range_);
    ...
    for (auto thisFuncSpec : funcSpecList)
    {
        try
        {
            ...
            fillSingleCubicSplineTableData(thisFuncSpec.function,
                                           thisFuncSpec.derivative,
                                           range_,
                                           spacing,
                                           &tmpYfghTableData);
...
CubicSplineTable::CubicSplineTable(const std::initializer_list<const VectorFuncSpecType>   &funcSpecList,
                                   double                                                   inputSpacing,
                                   const std::pair<real, real>                             &range,
                                   real                                                     tolerance)
    : numFuncInTable_(funcSpecList.size()), range_(range)
{
    ...
    for (auto thisFuncSpec : funcSpecList)
    {
        try
        {
            ...
            if (thisFuncSpec.function.size() < range_.second / inputSpacing + 1)
            {
                GMX_THROW(InconsistentInputError("Table input vectors must cover requested range, and a margin beyond the upper endpoint"));
            }

            if (thisFuncSpec.function.size() != thisFuncSpec.derivative.size())
            {
                GMX_THROW(InconsistentInputError("Function and derivative vectors have different lengths"));
            }

            internal::throwUnlessDerivativeIsConsistentWithFunction(thisFuncSpec.function, thisFuncSpec.derivative, inputSpacing, range_);
        }
        catch (gmx::GromacsException &ex)
        {
            ex.prependContext("Error generating cubic spline table for function '" + thisFuncSpec.desc + "'");
        ...
        double thisMinQuotient = internal::findSmallestQuotientOfFunctionAndThirdDerivative(thisFuncSpec.derivative, inputSpacing, range_);
