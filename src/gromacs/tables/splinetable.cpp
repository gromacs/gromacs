...
void SplineTable::dump(FILE *fp) const
{
    for (unsigned int i = 0; (i < tableF_.size()); ++i)
    {
        fprintf(fp, "%10g  %10g  %10g\n", spacing()*i, tableV_[i], tableF_[i]);
    }
}

double SplineTable::force(double r) const
{
    GMX_RELEASE_ASSERT((r >= 0), "Distance r should be >= 0");
    unsigned int itab = static_cast<int>(std::floor(r/spacing()));
    GMX_RELEASE_ASSERT((itab < size()), "Distance r too large for table");
    double       dr = r - itab*spacing();
    double       f  = tableFDV0_[4*itab]+dr*tableFDV0_[4*itab+1];
    return f;
}

double SplineTable::energy(double r) const
{
    GMX_RELEASE_ASSERT((r >= 0), "Distance r should be >= 0");
    unsigned int itab = static_cast<int>(std::floor(r/spacing()));
    GMX_RELEASE_ASSERT((itab < size()), "Distance r too large for table");
