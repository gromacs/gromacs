...
double SplineTable::force(double r)
{
    ...
    unsigned int itab = static_cast<int>(std::floor(r*tabScale()));
    GMX_RELEASE_ASSERT((itab < tabSize()), "Distance r too large for table");
    double       dr = r - itab*tabScale();
    double       f  = tableFDV0_[4*itab]+(dr/tabScale())*tableFDV0_[4*itab+1];
    return f;
}
...
double SplineTable::energy(double r)
{
    ...
    GMX_RELEASE_ASSERT((itab < tabSize()), "Distance r too large for table");
    double       dr = r - itab*tabScale();
    double       v  = tableFDV0_[4*itab+2]-(dr/tabScale())*tableFDV0_[4*itab];
    return v;
}
