 ...
 * \f]
 *
 * Thus, if we replace the derivative in the nternal quadratic table data with
 *
 * \f[
 *  V' - \frac{1}{12}h^2 V'''
 * \f]
...
class QuadraticSplineTable
{
    ...
    public:
         ...
         */
        QuadraticSplineTable(const std::initializer_list<const AnalyticalFuncSpecType>   &funcSpecList,
         ...
         *       since you would waste a huge amount of memory.
         */
        QuadraticSplineTable(const std::initializer_list<const VectorFuncSpecType>   &funcSpecList,
                             double                                                   inputSpacing,
                             const std::pair<real, real>                             &range,
                             real                                                     tolerance = defaultTolerance);
                           ...
                           T *   derivativeValue) const
        {
            ...
            else
            {
                ...
                gatherLoadUBySimdIntTranspose<numFuncInTable>(derivativeMultiTableData_.data() + funcIndex, tabIndex + 1, &t1, &t2); // works for scalar T too
            }
                                      ...
                                      T *   derivativeValue2) const
        {
            ...
            // Load Derivative, Delta, Function, and Zero values for each table point.
            // The 4 refers to these four values - not any SIMD width.
            gatherLoadBySimdIntTranspose<4>(ddfzMultiTableData_.data() + 4*funcIndex0, numFuncInTable * tabIndex, &t0, &t1, &t2, &t3);
            t1                = t0 + eps * t1;
            *functionValue1   = fma( eps * T(halfSpacing_),  t0 + t1, t2);
            *derivativeValue1 = t1;

            gatherLoadBySimdIntTranspose<4>(ddfzMultiTableData_.data() + 4*funcIndex1, numFuncInTable * tabIndex, &t0, &t1, &t2, &t3);
            t1                = t0 + eps * t1;
                           ...
                           T *   derivativeValue2) const
        {
            ...
            else
            {
                ...
                gatherLoadUBySimdIntTranspose<numFuncInTable>(derivativeMultiTableData_.data() + funcIndex0, tabIndex + 1, &t1, &t2); // works for scalar T too
                *derivativeValue1 = fma(t1-t0, eps, t0);
                ...
                gatherLoadUBySimdIntTranspose<numFuncInTable>(derivativeMultiTableData_.data() + funcIndex1, tabIndex + 1, &t1, &t2); // works for scalar T too
                           ...
                           T *   derivativeValue3) const
        {
            ...
            else
            {
                ...
                *derivativeValue2 = fma(t1-t0, eps, t0);
                ...
                gatherLoadUBySimdIntTranspose<numFuncInTable>(derivativeMultiTableData_.data() + funcIndex2, tabIndex + 1, &t1, &t2); // works for scalar T too
