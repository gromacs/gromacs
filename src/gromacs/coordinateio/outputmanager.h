...
struct OutputAdapterContainer
{
    ...
    explicit OutputAdapterContainer(CoordinateOutputPointer module)
    {
        module_ = std::move(module);
    }
    //! Pointer to the module
    CoordinateOutputPointer module_;
};
//! List of output adapters
using CoordinateOutputAdapters = std::vector<OutputAdapterContainer>;
//! Convenience name for using a reference to a list of adapters.
using CoordinateOutputAdapterRef = gmx::ArrayRef<OutputAdapterContainer>;
 ...
 * ICoordinateOutput to perform changes to the meta information contained
...
class OutputManager
{
    ...
    private:
        ...
        OutputManager(std::string       name,
            ...
            mtop_(mtop),
            outputFlag_(flag),
            registeredModules_(efBaseOutputManager)
        {
        }
        ...
        /*! \brief  Adds new module to the process chain.
         *
         * \param[in] module unique_ptr to the module being added.
         */
        void addOutputAdapter(CoordinateOutputPointer module);

        /*! \brief
         * Check if module can be added.
         *
         * Checks if a module can be added according to the requirements of the
         * OutputManager, if it is already registered, or if an registered module
         * would violate the requirements for a new module.
         *
         * \param[in] newModule Flag specifying the new module to be added.
         */
        void canAddNewModule(unsigned long newModule);

        //! Return pointer to topology.
        const gmx_mtop_t *getMtop() const { return mtop_; }
        ...
        CoordinateOutputAdapters outputAdapters_;

        //! Pointer to topology information if available.
        const gmx_mtop_t *mtop_;

        //! Value for flag specifying the requirements.
        unsigned long outputFlag_;
        //! Value for flag to check which modules have been registered.
        unsigned long registeredModules_;
};
