...
class OutputManager : public IModuleCallback
{
    public:
        ...
        explicit OutputManager(std::string name, const Selection *sel, const gmx_mtop_t *mtop) :
            ...
            name_(std::move(name)), outputFile_(nullptr), filetype_(efNR), sel_(sel), mtop_(mtop)
        {
            setFiletype();
            flagModules_.clear();
        }
         ...
         */
        OutputManager &operator=(OutputManager &&old) noexcept
        {
            name_               = std::move(old.name_);
            outputFile_         = old.outputFile_;
            filetype_           = old.filetype_;
            sel_                = old.sel_;
            mtop_               = old.mtop_;
         ...
         */
        OutputManager(OutputManager &&old) noexcept : IModuleCallback(old.moduleFlags_),
                                                      name_(std::move(old.name_)),
                                                      outputFile_(old.outputFile_),
                                                      filetype_(old.filetype_),
                                                      sel_(old.sel_),
         ...
         */
        void writeFrame(t_trxframe write) const;
        ...
        bool haveMtop() const { return mtop_ != nullptr; }
