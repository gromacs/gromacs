 ...
 */
OutputManagerPointer createOutputManager(const gmx_mtop_t  *top,
                                         const Selection   &sel,
...
/*! \brief
 * Helper struct for the factory function that builds a new OutputManager.
 *
...
struct OutputManager::OutputManagerBuildHelper : public OutputManager
{
    public:
        ...
        OutputManagerBuildHelper(std::string              name,
            ...
            OutputManager(std::move(name), filetype, sel, mtop, std::move(adapters))
        {
        }
};
