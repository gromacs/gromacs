...
class SetPrecision : public IOutputAdapter
{
    public:
        ...
        explicit SetPrecision(int precision) : precision_(precision)
        {
            ...
            if (precision == 3)
            {
                moduleRequirements_ = efBaseOutputManager;
            }
            else
            {
                moduleRequirements_ = efChangeOutputPrecisionModule;
            }
        }
        /*! \brief
        ...
        //! Return local id.
        unsigned long getModuleIDFlag() override { return efChangeOutputPrecisionModule; }
