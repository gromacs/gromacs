...
{
    if (!atomsNeededByOutput_)
    {
        switch (atomFlag_)
        {
            case (ChangeSettingType::efUserNo):
                input->bAtoms = false;
                done_and_delete_atoms(input->atoms);
                break;
    ...
    else
    {
        ...
        else if (!haveAtoms(*input))
        {
            GMX_THROW(InconsistentInputError("Output method requests that atom information is "
                                             "available to write output while it is not."));
        }
    }
