 ...
 * \internal
 * \brief
 * Implements gmx::CoordinateOutputUserOptions.
 *
...
void
CoordinateOutputUserOptions::initFileOptions(IOptionsContainer *options)
{
...
void
CoordinateOutputUserOptions::checkOptions()
{
    ...
    if (setNewBox_ && box_ == ChangeFrameUnchangedYesType::efUnchanged)
    {
        box_ = ChangeFrameUnchangedYesType::efUserYes;
    }
    if (setNewPrecision_ && precision_ == ChangeFrameUnchangedYesType::efUnchanged)
    {
        precision_ = ChangeFrameUnchangedYesType::efUserYes;
    }
    if ((setNewTimeStep_ || setNewStartTime_) && frameTime_ == ChangeFrameTimeType::efUnchanged)
    {
        if (setNewTimeStep_ && setNewStartTime_)
        {
            frameTime_ = ChangeFrameTimeType::efBothTime;
        }
        else if (setNewTimeStep_)
        {
            frameTime_ = ChangeFrameTimeType::efTimeStep;
        }
        else
        {
            frameTime_ = ChangeFrameTimeType::efStartTime;
        }
    }
    ...
    haveCheckedOptions_ = true;
}

CoordinateOutputAdapters
CoordinateOutputUserOptions::registerModules(const t_atoms *globalAtoms)
{
    GMX_RELEASE_ASSERT(haveCheckedOptions_, "Need to check options before trying to add modules");
    CoordinateOutputAdapters output;
    if (velocity_ != ChangeSettingType::efUnchanged)
    {
        output.emplace_back(compat::make_unique<SetVelocities>(velocity_));
    }
    if (force_ != ChangeSettingType::efUnchanged)
    {
        output.emplace_back(compat::make_unique<SetForces>(force_));
    }
    if (precision_ != ChangeFrameUnchangedYesType::efUnchanged)
    {
        output.emplace_back(compat::make_unique<SetPrecision>(prec_));
    }
    if (atoms_ != ChangeSettingType::efUnchanged)
    {
        output.emplace_back(compat::make_unique<SetAtoms>(atoms_, globalAtoms));
    }
    if (frameTime_ != ChangeFrameTimeType::efUnchanged)
    {
        output.emplace_back(compat::make_unique<SetTime>(startTimeValue_, timeStepValue_, frameTime_));
    }
    if (box_ != ChangeFrameUnchangedYesType::efUnchanged)
    {
        output.emplace_back(compat::make_unique<SetBox>(newBox_));
    }
