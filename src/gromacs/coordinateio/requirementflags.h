...
/*! \file
 * \brief
 * Enum class defining the different requirements that outputadapter modules
 * have for the output file type. Depending on matching requirements, they can then be
 * added to an OutputManager object or not.
 *
 * \author Paul Bauer <paul.bauer.q@gmail.com>
 * \libinternal
 * \ingroup module_coordinateio
 */
#ifndef GMX_COORDINATEIO_REQUIREMENTFLAGS_H
#define GMX_COORDINATEIO_REQUIREMENTFLAGS_H

namespace gmx
{

/*!\brief
 * Enum class ModuleRequirements defines the flags specifying the requirements
 * of different outputadapter modules.
 *
 * When building the object for deciding on the output to a new coordinate file,
 * the outputmanager needs to be able to validate that the dependencies of attached modules
 * are fulfilled. Classes and interfaces that include the enum can check their
 * dependencies against the information encoded in the flags and can then perform
 * an appropriate reaction if there is a mismatch.
 *
