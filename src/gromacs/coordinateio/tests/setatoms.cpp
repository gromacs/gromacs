...
class SetAtomsTest : public gmx::test::CommandLineTestBase
{
    public:
        ...
        SetAtomsTest()
        {
            topology_.fillFromInputFile(TestFileManager::getInputFilePath("lysozyme.gro"));
        }
        ...
        SetAtoms *setAtoms(ChangeAtomsType type, bool haveAtomsAvailable)
        {
            if (!setAtoms_)
            {
                ...
                {
                    setAtoms_ = gmx::compat::make_unique<SetAtoms>(type, topology_.copyAtoms());
                }
                else
                {
                    setAtoms_ = gmx::compat::make_unique<SetAtoms>(type,  nullptr);
                }
            }
...
TEST_F(SetAtomsTest, RemovesExistingAtoms)
{
    ...
    SetAtoms *method = setAtoms(ChangeAtomsType::efUserNo, true);
...
TEST_F(SetAtomsTest, AddsNewAtoms)
{
    ...
    SetAtoms *method = setAtoms(ChangeAtomsType::efUserYes, true);
    EXPECT_NO_THROW(method->processFrame(0, &frame));
...
TEST_F(SetAtomsTest, ThrowsOnRequiredAtomsNotAvailable)
{
    ...
    SetAtoms *method = setAtoms(ChangeAtomsType::efRequired, false);
...
TEST_F(SetAtomsTest, ThrowsWhenUserAtomReplacementNotPossible)
{
    ...
    SetAtoms *method = setAtoms(ChangeAtomsType::efUserYes, false);
    EXPECT_THROW(method->processFrame(0, &frame), InconsistentInputError);
}
