 ...
 */
static void addEmptyModuleToOutputManager(const std::string &filename,
...
TEST(OutputManagerTest, BuilderFailsWithPdbAndNoAtoms)
{
    OutputManagerPointer     output;
    gmx_mtop_t               dummyTopology;
    OutputAdapters           adapters;
    EXPECT_THROW(output = createMinimalOutputManager("test.pdb",
                                                     &dummyTopology,
                                                     std::move(adapters)),
                 InconsistentInputError);
    EXPECT_EQ(output.get(), nullptr);
...
TEST(OutputManagerTest, BuilderFailsWithGroAndNoAtoms)
{
    OutputManagerPointer     output;
    gmx_mtop_t               dummyTopology;
    OutputAdapters           adapters;
    EXPECT_THROW(output = createMinimalOutputManager("test.gro",
                                                     &dummyTopology,
                                                     std::move(adapters)),
                 InconsistentInputError);
    EXPECT_EQ(output.get(), nullptr);
...
TEST(OutputManagerTest, BuilderAcceptsPdbGroWithAtoms)
{
    OutputManagerPointer     output;
    gmx_mtop_t               dummyTopology;
    {
        ...
        adapters.emplace_back(compat::make_unique<DummyOutputModule>(
                                      efChangeAtomInformationModule, efChangeAtomInformationModule));
        EXPECT_NO_THROW(output = createMinimalOutputManager("test.pdb",
                                                            &dummyTopology,
                                                            std::move(adapters)));
        EXPECT_NE(output.get(), nullptr);
    }
    {
        ...
        adapters.emplace_back(compat::make_unique<DummyOutputModule>(
                                      efChangeAtomInformationModule, efChangeAtomInformationModule));
        EXPECT_NO_THROW(output = createMinimalOutputManager("test.gro",
                                                            &dummyTopology,
                                                            std::move(adapters)));
        EXPECT_NE(output.get(), nullptr);
    }
 ...
 *
 * PDB and GRO need special test case because they require atoms information
 * to be present to create a valid outputmanager.
 */
