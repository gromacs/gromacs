...
//! Helper to test unsupported file names.
...
//! Helper to test supported file names.
class OutputSelectorSupportedFiles : public ModuleTest
{
    public:
        void prepareTest(const char *filename)
        {
...
TEST_P(OutputSelectorSupportedFiles, Works)
{
    prepareTest(GetParam());
}

/*!\brief
 * Character array of different filie names to test.
 *
 ...
 * Character array of different filie names to test.
 *
 * Names here work for OutputSelector module
 */
const char *const outputSelectorSupported[] = {
    "spc2-traj.trr",
#if GMX_USE_TNG
    "spc2-traj.tng",
#endif
...
INSTANTIATE_TEST_CASE_P(ModuleSupported,
                        OutputSelectorSupportedFiles, ::testing::ValuesIn(outputSelectorSupported));
