            ...
            gmx_bool calc_fshift)
{
    ...
    {
        forcebuf[                 (1 + ci_offset) * STRIDE_SI + tidx] = 0.0f;
        forcebuf[    STRIDE_DIM + (1 + ci_offset) * STRIDE_SI + tidx] = 0.0f;
        forcebuf[2 * STRIDE_DIM + (1 + ci_offset) * STRIDE_SI + tidx] = 0.0f;
    }
                ...
                if (imask_j)
                {
                    ...
                    for (cii = 0; cii < nsubi; cii++)
                    {
...
#endif
                        {
                            ...
                            forcebuf[                 (1 + ci_offset) * STRIDE_SI + tidx] += f_ij.x;
                            forcebuf[    STRIDE_DIM + (1 + ci_offset) * STRIDE_SI + tidx] += f_ij.y;
                            forcebuf[2 * STRIDE_DIM + (1 + ci_offset) * STRIDE_SI + tidx] += f_ij.z;
                        }
                    }
    ...
    for(ci_offset = 0; ci_offset < NSUBCELL; ci_offset++)
    {
        ...
        reduce_force_i(forcebuf + (1 + ci_offset) * STRIDE_SI, f,
