...
static void calc_interpolation_idx(gmx_pme_lj_t pme,pme_atomcomm_t *atc)
{
    int  i;
...
static void pmeredist_pd(gmx_pme_lj_t pme, bool forw,
...
static void dd_pmeredist_x_q(gmx_pme_lj_t pme, 
...
static void dd_pmeredist_f(gmx_pme_lj_t pme, pme_atomcomm_t *atc,
                           int n, rvec *f,
                           bool bAddF)
{
  int *commnode,*buf_index;
...
#ifdef GMX_MPI
static void 
gmx_sum_qgrid_dd(gmx_pme_lj_t pme, real *grid, int direction)
{
...
static int
copy_pmegrid_to_fftgrid(gmx_pme_lj_t pme, real *pmegrid, real *fftgrid)
{
...
static int
copy_fftgrid_to_pmegrid(gmx_pme_lj_t pme, real *fftgrid, real *pmegrid)
{
    ivec    local_fft_ndata,local_fft_offset,local_fft_size;
    ivec    local_pme_size;
    int     i,ix,iy,iz;
...
static void
wrap_periodic_pmegrid(gmx_pme_lj_t pme, real *pmegrid)
{
...
static void
unwrap_periodic_pmegrid(gmx_pme_lj_t pme, real *pmegrid)
{
    int     nx,ny,nz,pnx,pny,pnz,ny_x,overlap,ix,iy,iz;
...
static void spread_q_bsplines(gmx_pme_lj_t pme, pme_atomcomm_t *atc, 
                              real *grid)
{
...
static int solve_pme_yzx(gmx_pme_lj_t pme,t_complex *grid,
...
static void gather_f_bsplines(gmx_pme_lj_t pme,real *grid,
...
static real gather_energy_bsplines(gmx_pme_lj_t pme,real *grid,
                                   pme_atomcomm_t *atc)
{
...
static void make_bspline_moduli(splinevec bsp_mod,int nx,int ny,int nz,int order)
{
...
int gmx_pme_lj_destroy(FILE *log,gmx_pme_lj_t *pmedata)
{
    if(NULL != log)
    {
...
static double pme_load_imbalance(gmx_pme_lj_t pme)
{
    int    nma,nmi;
    double n1,n2,n3;
...
static void init_atomcomm(gmx_pme_lj_t pme,pme_atomcomm_t *atc, t_commrec *cr,
...
int gmx_pme_lj_init(gmx_pme_lj_t *         pmedata,
                    t_commrec *            cr,
                    int                    nnodes_major,
                    int                    nnodes_minor,
                    t_inputrec *           ir,
...
static void spread_on_grid(gmx_pme_lj_t pme,
...
void gmx_pme_lj_calc_energy(gmx_pme_lj_t pme,int n,rvec *x,real *q,real *V)
{
    pme_atomcomm_t *atc;
    real *grid;
...
int gmx_pme_lj_do(gmx_pme_lj_t pme,
