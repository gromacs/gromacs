              ...
              int flags)
{
    ...
    if (bDoForces)
    {
        ...
        if(bSepLRF && (do_per_step(step,inputrec->nstcalclr) || (bNS && inputrec->nstcalclr==-1)))
        {
    ...
    if(bSepLRF)
    {
        if (do_per_step(step,inputrec->nstcalclr) || (fr->bTwinRange && bNS && (inputrec->nstcalclr==-1)))
        {
              ...
              int flags)
{
    ...
    bSepLRF        = ((inputrec->nstcalclr!=1) && bDoForces &&
                      (flags & GMX_FORCE_SEPLRF) && (flags & GMX_FORCE_DOLR));
