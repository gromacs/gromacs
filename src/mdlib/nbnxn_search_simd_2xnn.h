...
static gmx_inline __m256 gmx_mm256_load4_hilo_ps(const real *a)
{
    __m128 a_SSE;

    a_SSE = _mm_load_ps(a);

    return _mm256_insertf128_ps(_mm256_castps128_ps256(a_SSE),a_SSE,1);
}

static gmx_inline __m256 gmx_mm256_load2_shift_ps(const real *a,real shift)
{
    __m128 a0,a1;
                      ...
                      nbnxn_list_work_t *work)
{
    ...
    x_ci->ix_SSE0 = gmx_mm256_load2_shift_ps(x + ia + 0*STRIDE_S + 0, shx);
    x_ci->iy_SSE0 = gmx_mm256_load2_shift_ps(x + ia + 1*STRIDE_S + 0, shy);
    x_ci->iz_SSE0 = gmx_mm256_load2_shift_ps(x + ia + 2*STRIDE_S + 0, shz);
    x_ci->ix_SSE2 = gmx_mm256_load2_shift_ps(x + ia + 0*STRIDE_S + 2, shx);
    x_ci->iy_SSE2 = gmx_mm256_load2_shift_ps(x + ia + 1*STRIDE_S + 2, shy);
    x_ci->iz_SSE2 = gmx_mm256_load2_shift_ps(x + ia + 2*STRIDE_S + 2, shz);
}
    ...
    while (!InRange && cjf <= cjl)
    {
        ...
        *ndistc += 2;
        ...
        else if (d2 < rl2)
        {
            ...
            jx_SSE  = gmx_mm256_load4_hilo_ps(x_j+xind_f+0*STRIDE_S);
            jy_SSE  = gmx_mm256_load4_hilo_ps(x_j+xind_f+1*STRIDE_S);
            jz_SSE  = gmx_mm256_load4_hilo_ps(x_j+xind_f+2*STRIDE_S);
    ...
    while (!InRange && cjl > cjf)
    {
        ...
        *ndistc += 2;
        ...
        else if (d2 < rl2)
        {
            ...
            jx_SSE  = gmx_mm256_load4_hilo_ps(x_j+xind_l+0*STRIDE_S);
            jy_SSE  = gmx_mm256_load4_hilo_ps(x_j+xind_l+1*STRIDE_S);
            jz_SSE  = gmx_mm256_load4_hilo_ps(x_j+xind_l+2*STRIDE_S);
