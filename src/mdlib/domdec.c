...
static void dd_collect_vec_sendrecv(gmx_domdec_t *dd,t_block *cgs_gl,
                                    rvec *lv,rvec *v)
{
...
static void dd_collect_vec_gatherv(gmx_domdec_t *dd,t_block *cgs_gl,
                                   rvec *lv,rvec *v)
{
    gmx_domdec_master_t *ma;
                    ...
                    rvec *lv,rvec *v)
{
    ...
    {
        dd_collect_vec_sendrecv(dd,cgs_gl,lv,v);
    }
    else
    {
        dd_collect_vec_gatherv(dd,cgs_gl,lv,v);
    }
}
                                  ...
                                  ivec np)
{
    ...
    for(d=0; d<DIM; d++)
    {
        ...
        else
        {
             ...
             * to the master distribution case and to determine np.
             */
            if (bMaster)
            {
...
static void set_dd_cell_sizes(gmx_domdec_t *dd,matrix box,bool bDynamicBox,
                              bool bUniform,bool bMasterState,int step)
{
    int  d;
    ivec np;
    ...
    if (dd->bDynLoadBal)
    {
        ...
        if (bMasterState)
        {
            set_dd_cell_sizes_slb(dd,box,bMasterState,np);
        }
        else
        {
					...
					t_inputrec *ir)
{
  ...
  dd->bInterCGcons = inter_charge_group_constraints(top);
  ...
  if (DDMASTER(dd)) {
    ...
    dd->ma = init_gmx_domdec_master_t(dd,top->cgs.nr,top->atoms.nr);
  }
