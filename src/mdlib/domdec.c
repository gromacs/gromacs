    ...
    else
    {
        ...
        if (comm->cycl_n[ddCyclWaitGPU] && comm->nrank_gpu_shared > 1)
        {
            ...
            if (comm->cycl_n[ddCyclF] > 1)
            {
                 ...
                 * as the one with the maximum F time. Furthermore, the step
                 * for the max F time should be chosen the same on all ranks.
...
void dd_setup_dd_dlb_gpu_sharing(gmx_domdec_t *dd, int gpu_id)
{
     ...
     */
    MPI_Comm_split(dd->mpi_comm_all, gmx_physicalnode_id_hash(), dd->rank,
