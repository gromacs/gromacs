...
sincos_sse2double(__m128d x, __m128d *sinval, __m128d *cosval)
{
    ...
    __m128d signbit           = _mm_set1_epi64x(_mm_set1_epi64x(0x8000000000000000ULL));
    __m128d tiny              = _mm_set1_epi64x(_mm_set1_epi64x(0x3e40000000000000ULL));
    ...
    sinMask = _mm_cmpeq_epi32(_mm_cmpeq_epi32( _mm_and_si128(offsetSin,ione), izero));
    cosMask = _mm_cmpeq_epi32(_mm_cmpeq_epi32( _mm_and_si128(offsetCos,ione), izero));
    ...
    sinMask = _mm_cmpeq_epi32(_mm_cmpeq_epi32( _mm_and_si128(offsetSin,itwo), izero));
    tsn    = _mm_xor_pd(signbit,ts);
    ts     = _mm_or_pd( _mm_and_pd(sinMask,ts) , _mm_andnot_pd(sinMask,tsn) );
	
    cosMask = _mm_cmpeq_epi32(_mm_cmpeq_epi32( _mm_and_si128(offsetCos,itwo), izero));
