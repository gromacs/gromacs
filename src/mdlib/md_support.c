                               ...
                               t_inputrec *ir, gmx_mtop_t *mtop)
{
    ...
    if (IR_TWINRANGE(*ir) && ir->nstlist > 1)
    {
        gmx_fatal(FARGS, "The implementation of multiple time stepping was fixed after 4.0.7. The new implementation is broken for twin-range cut-offs with nstlist > 1. Either use that version, or single-range cut-offs, or update the neighbour list every step.");
    }
}
