    ...
    for (j=0;j<efptNR;j++)
    {
        if (fep->separate_dvdl[j])
        {
            ...
            if (!get_names)
            {
                ...
                {
                    str += sprintf(str,"%g", fep->init_lambda);
                }
                else
                {
                    str += sprintf(str,"%g", fep->all_lambda[j][i]);
                }
            }
            else
            {
                str += sprintf(str,"%s", efpt_names[j]);
            }
                       ...
                       const output_env_t oenv)
{
    ...
    if (ir->efep != efepSLOWGROWTH)
    {
        if ( (fep->init_lambda >= 0)  && (n_lambda_terms == 1 ))
        {
            ...
            sprintf(&(buf[bufplace]),"%s = %g", lambda,fep->init_lambda);
        }
        else
        {
    ...
    for (i=0;i<efptNR;i++)
    {
        if (fep->separate_dvdl[i]) {
            ...
            {
                /* compatibility output */
                sprintf(buf,"%s %s %g",dhdl,lambda, fep->init_lambda);
            }
            else
            {
                ...
                sprintf(buf,"%s %s = %g",dhdl, efpt_names[i], lam);
            }
            setname[s] = strdup(buf);
            s+=1;
        }
    ...
    if (fep->n_lambda > 0)
    {
        ...
        {
            print_lambda_vector(fep, s-(nsetsbegin), FALSE, FALSE,
                                lambda_vec_str);
            if ( (fep->init_lambda >= 0)  && (n_lambda_terms == 1 ))
            {
                ...
                gmx_constr_t constr)
{
    ...
    if ((md->fp_dhdl || md->dhc) && bDoDHDL && (enerd->n_lambda > 0))
    {
        ...
        if (md->fp_dhdl)
        {
            ...
            {
                fprintf(md->fp_dhdl," %#.8g",md->dE[i-1]);
