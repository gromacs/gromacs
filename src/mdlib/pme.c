...
void gmx_pme_calc_energy(gmx_pme_t pme,int n,rvec *x,real *q,real *V)
{
    ...
    spread_on_grid(pme,atc,NULL,TRUE,FALSE,FALSE);
               ...
               int flags)
{
    ...
    for (q = 0; q < ((flags & GMX_PME_DO_LJ_LB) ? 2 : 4); ++q)
    {
        ...
        if (flags & GMX_PME_SOLVE)
        {
            ...
            {
                loop_count =
                    solve_pme_lj_yzx(pme,&cfftgrid,FALSE,ewaldcoeff_lj,vol,
                                     flags & GMX_PME_CALC_ENER_VIR,
                                     &energy_AB[q],vir_AB[q]);
            }
    ...
    if (flags & GMX_PME_DO_LJ_LB)
    {
        ...
        if (flags & GMX_PME_SOLVE)
        {
            ...
            wallcycle_start(wcycle,ewcPME_SOLVE);
            loop_count =
                solve_pme_lj_yzx(pme, &pme->cfftgrid[2], TRUE, ewaldcoeff_lj, vol,
                                 flags & GMX_PME_CALC_ENER_VIR,
