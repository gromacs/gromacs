...
/*
   Kernel launch parameters:
    ...
    - shmem     = (1 + NSUBCELL) * CLUSTER_SIZE^2 * 3 * sizeof(float)
             ...
             gmx_bool calc_fshift)
{
    ...
    extern __shared__ float f_buf[]; /* force buffer */
    ...
    {
        f_buf[                 (1 + ci_offset) * STRIDE_SI + tidx] = 0.0f;
        f_buf[    STRIDE_DIM + (1 + ci_offset) * STRIDE_SI + tidx] = 0.0f;
        f_buf[2 * STRIDE_DIM + (1 + ci_offset) * STRIDE_SI + tidx] = 0.0f;
    }
                ...
                if (imask_j)
                {
                    ...
                    for (cii = 0; cii < nsubi; cii++)
                    {
...
#endif
                        {
                            ...
                            f_buf[                 (1 + ci_offset) * STRIDE_SI + tidx] += f_ij.x;
                            f_buf[    STRIDE_DIM + (1 + ci_offset) * STRIDE_SI + tidx] += f_ij.y;
                            f_buf[2 * STRIDE_DIM + (1 + ci_offset) * STRIDE_SI + tidx] += f_ij.z;
                        }
                    }
    ...
    for(ci_offset = 0; ci_offset < NSUBCELL; ci_offset++)
    {
        ai  = (sci * NSUBCELL + ci_offset) * CLUSTER_SIZE + tidxi;  /* i atom index */
        reduce_force_i(f_buf + (1 + ci_offset) * STRIDE_SI, f,
