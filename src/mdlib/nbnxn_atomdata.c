                         ...
                         nbnxn_free_t  *free)
{
    ...
    if (simple)
    {
        ...
        snew_aligned(nbat->simd_4xn_diagonal_indices,
                     simd_4xn_diag_ind_size, NBNXN_MEM_ALIGN);
        for (j = 0; j < simd_4xn_diag_ind_size; j++)
        {
            nbat->simd_4xn_diagonal_indices[j] = j - 0.5;
        }

        snew_aligned(nbat->simd_2xnn_diagonal_indices,
                     simd_width, NBNXN_MEM_ALIGN);
        for (j = 0; j < simd_width/2; j++)
        {
            /* The j-cluster size is half the SIMD width */
            nbat->simd_2xnn_diagonal_indices[j]              = j - 0.5;
            /* The next half of the SIMD width is for i + 1 */
            nbat->simd_2xnn_diagonal_indices[simd_width/2+j] = j - 1 - 0.5;
        }
