...

{
...
#else
    {
        ...
        gmx_mm_pr masksh_pr_S = gmx_castsi_pr(gmx_set1_epi32(l_cj[cjind].excl>>(4*UNROLLJ)));
    ...
    jx_S        = gmx_loaddh_pr(x+ajx);
    jy_S        = gmx_loaddh_pr(x+ajy);
    jz_S        = gmx_loaddh_pr(x+ajz);
...
#ifdef LJ_COMB_GEOM
    c6s_j_S     = gmx_loaddh_pr(ljc+aj2+0);
    c12s_j_S    = gmx_loaddh_pr(ljc+aj2+STRIDE);
...
#ifdef LJ_COMB_LB
    hsig_j_S    = gmx_loaddh_pr(ljc+aj2+0);
    seps_j_S    = gmx_loaddh_pr(ljc+aj2+STRIDE);
...
#define     wco_vdw_S2    wco_S2
    ...
    gmx_store_hpr(f+ajx,
                  gmx_sub_hpr( gmx_load_hpr(f+ajx), gmx_sum4_hpr(tx_S0, tx_S2) ));
    gmx_store_hpr(f+ajy,
                  gmx_sub_hpr( gmx_load_hpr(f+ajy), gmx_sum4_hpr(ty_S0, ty_S2) ));
    gmx_store_hpr(f+ajz,
                  gmx_sub_hpr( gmx_load_hpr(f+ajz), gmx_sum4_hpr(tz_S0, tz_S2) ));
}
