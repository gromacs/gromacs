...
static inline void
decrement_j_atom_force(real     *fj,
                       gmx_mm_pr t_S0,
                       gmx_mm_pr t_S1,
                       gmx_mm_pr t_S2,
                       gmx_mm_pr t_S3)
{
    gmx_mm_pr fj_S    = gmx_load_pr(fj);
    gmx_mm_pr t_sum_S = gmx_sum4_pr(t_S0, t_S1, t_S2, t_S3);
    fj_S = gmx_sub_pr(fj_S, t_sum_S);
    gmx_store_pr(fj, fj_S);
}
