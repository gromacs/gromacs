...
#error "Only NBFP_STRIDE 2 and 4 are currently supported"
     ...
     */
    filter_S0    = gmx_load_exclusion_filter(exclusion_filter + 0*UNROLLJ*FILTER_POINTER_STRIDE);
    filter_S1    = gmx_load_exclusion_filter(exclusion_filter + 1*UNROLLJ*FILTER_POINTER_STRIDE);
    filter_S2    = gmx_load_exclusion_filter(exclusion_filter + 2*UNROLLJ*FILTER_POINTER_STRIDE);
    filter_S3    = gmx_load_exclusion_filter(exclusion_filter + 3*UNROLLJ*FILTER_POINTER_STRIDE);
