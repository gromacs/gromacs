...
#ifdef GMX_SIMD_HAVE_CHECKBITMASK_EPI32
    excl_mask_stride = (UNROLLJ*sizeof(real))/sizeof(*nbat->simd_excl_mask);

    mask_S0    = gmx_load_si(nbat->simd_excl_mask + 0*excl_mask_stride);
    mask_S1    = gmx_load_si(nbat->simd_excl_mask + 1*excl_mask_stride);
    mask_S2    = gmx_load_si(nbat->simd_excl_mask + 2*excl_mask_stride);
    mask_S3    = gmx_load_si(nbat->simd_excl_mask + 3*excl_mask_stride);
#else
