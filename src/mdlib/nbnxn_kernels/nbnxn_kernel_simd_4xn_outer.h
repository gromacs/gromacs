...
)
{
    ...
    gmx_mm_pr mask_S0, mask_S1;
#if UNROLLI*UNROLLJ <= 16
    gmx_mm_pr mask_S2, mask_S3;
#endif
...
#define NBFP_STRIDE  2
    ...
    mask_S0    = gmx_castsi_pr(gmx_load_si(nbat->simd_excl_mask + 0*GMX_NBNXN_SIMD_BITWIDTH/32));
    mask_S1    = gmx_castsi_pr(gmx_load_si(nbat->simd_excl_mask + 1*GMX_NBNXN_SIMD_BITWIDTH/32));
#if UNROLLI*UNROLLJ <= 16
    mask_S2    = gmx_castsi_pr(gmx_load_si(nbat->simd_excl_mask + 2*GMX_NBNXN_SIMD_BITWIDTH/32));
    mask_S3    = gmx_castsi_pr(gmx_load_si(nbat->simd_excl_mask + 3*GMX_NBNXN_SIMD_BITWIDTH/32));
#endif
