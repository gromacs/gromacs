...
# are two other files in those locations that are not generated. These
# contain the logic for the outer and inner loops of the kernels, as
...
# is not portable, only the functions for the optimal nbnxn kernel
# structure for the hardware selected at CMake time contain real
# kernel logic. A run-time error occurs if the other empty kernel
# functions are called (but it is impossible for these to ever be
# triggered).
