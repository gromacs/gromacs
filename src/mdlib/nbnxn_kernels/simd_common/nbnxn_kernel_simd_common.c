                                  ...
                                  real *Vvdw, real *Vc)
{
    ...
    const int unrollj_half = GMX_SIMD_WIDTH_HERE/(2*GMX_SIMD_J_UNROLL_SIZE);
    ...
    for (i = 0; i < ng; i++)
    {
        ...
        for (j1 = 0; j1 < ng; j1++)
        {
            for (j0 = 0; j0 < ng; j0++)
            {
                c = ((i*ng + j1)*ng_p2 + j0)*unrollj_half*simd_width/GMX_SIMD_J_UNROLL_SIZE;
                for (s = 0; s < unrollj_half; s++)
                {
                    ...
                    Vc  [i*ng+j0] += VSc  [c+0];
                    Vc  [i*ng+j1] += VSc  [c+1];
                    c             += simd_width/GMX_SIMD_J_UNROLL_SIZE + 2;
                }
            }
        }
    }
