...

{
...
#else
    __m128i    ti_S0, ti_S1, ti_S2, ti_S3;
#endif
...
#ifdef CHECK_EXCLS
#if UNROLLI*UNROLLJ <= 16
    {
        gmx_mm_pr mask_pr_S = gmx_castsi_pr(gmx_set1_epi32(l_cj[cjind].excl));

        int_S0  = gmx_cmpneq_pr(gmx_and_pr(mask_pr_S, mask_S0), zero_S);
        int_S1  = gmx_cmpneq_pr(gmx_and_pr(mask_pr_S, mask_S1), zero_S);
        int_S2  = gmx_cmpneq_pr(gmx_and_pr(mask_pr_S, mask_S2), zero_S);
        int_S3  = gmx_cmpneq_pr(gmx_and_pr(mask_pr_S, mask_S3), zero_S);
    }
#else
    {
        /* Up to 32 mask bits needed, avoid using the most significant bits,
         * as they can lead to special float values which can cause trouble.
         */
        gmx_mm_pr mask_pr_S   = gmx_castsi_pr(gmx_set1_epi32(l_cj[cjind].excl));
        /* We can't compare all 4*8=32 float bits: shift the mask */
        gmx_mm_pr masksh_pr_S = gmx_castsi_pr(gmx_set1_epi32(l_cj[cjind].excl>>(2*UNROLLJ)));

        int_S0  = gmx_cmpneq_pr(gmx_and_pr(mask_pr_S,   mask_S0), zero_S);
        int_S1  = gmx_cmpneq_pr(gmx_and_pr(mask_pr_S,   mask_S1), zero_S);
        int_S2  = gmx_cmpneq_pr(gmx_and_pr(masksh_pr_S, mask_S0), zero_S);
        int_S3  = gmx_cmpneq_pr(gmx_and_pr(masksh_pr_S, mask_S1), zero_S);
    }
#endif
#endif /* CHECK_EXCLS */
...
#ifdef COUNT_PAIRS
    {
        ...
        for (i = 0; i < UNROLLI; i++)
        {
            gmx_storeu_pr(tmp, i == 0 ? wco_S0 : (i == 1 ? wco_S1 : (i == 2 ? wco_S2 : wco_S3)));
            for (j = 0; j < UNROLLJ; j++)
            {
                if (!(tmp[j] == 0))
                {
...
#ifndef NBNXN_CUTOFF_USE_BLENDV
    rinv_S0     = gmx_and_pr(rinv_S0, wco_S0);
    rinv_S1     = gmx_and_pr(rinv_S1, wco_S1);
    rinv_S2     = gmx_and_pr(rinv_S2, wco_S2);
    rinv_S3     = gmx_and_pr(rinv_S3, wco_S3);
#else
    ...
    rinv_ex_S0  = gmx_and_pr(rinv_S0, int_S0);
    rinv_ex_S1  = gmx_and_pr(rinv_S1, int_S1);
    rinv_ex_S2  = gmx_and_pr(rinv_S2, int_S2);
    rinv_ex_S3  = gmx_and_pr(rinv_S3, int_S3);
#else
...
#define     rinv_ex_S3    rinv_S3
     ...
     */
#ifndef NBNXN_CUTOFF_USE_BLENDV
    brsq_S0     = gmx_mul_pr(beta2_S, gmx_and_pr(rsq_S0, wco_S0));
    brsq_S1     = gmx_mul_pr(beta2_S, gmx_and_pr(rsq_S1, wco_S1));
    brsq_S2     = gmx_mul_pr(beta2_S, gmx_and_pr(rsq_S2, wco_S2));
    brsq_S3     = gmx_mul_pr(beta2_S, gmx_and_pr(rsq_S3, wco_S3));
#else
...
#ifdef CHECK_EXCLS
    vc_sub_S0   = gmx_add_pr(vc_sub_S0, gmx_and_pr(sh_ewald_S, int_S0));
    vc_sub_S1   = gmx_add_pr(vc_sub_S1, gmx_and_pr(sh_ewald_S, int_S1));
    vc_sub_S2   = gmx_add_pr(vc_sub_S2, gmx_and_pr(sh_ewald_S, int_S2));
    vc_sub_S3   = gmx_add_pr(vc_sub_S3, gmx_and_pr(sh_ewald_S, int_S3));
#else
    ...
    vcoul_S0    = gmx_and_pr(vcoul_S0, wco_S0);
    vcoul_S1    = gmx_and_pr(vcoul_S1, wco_S1);
    vcoul_S2    = gmx_and_pr(vcoul_S2, wco_S2);
    vcoul_S3    = gmx_and_pr(vcoul_S3, wco_S3);
#endif
...
#define     wco_vdw_S3    wco_S3
...
#ifdef EXCL_FORCES
    rinvsix_S0  = gmx_and_pr(rinvsix_S0, int_S0);
    rinvsix_S1  = gmx_and_pr(rinvsix_S1, int_S1);
#endif
#ifndef HALF_LJ
...
#ifdef EXCL_FORCES
    rinvsix_S2  = gmx_and_pr(rinvsix_S2, int_S2);
    rinvsix_S3  = gmx_and_pr(rinvsix_S3, int_S3);
#endif
#endif
#ifdef VDW_CUTOFF_CHECK
    rinvsix_S0  = gmx_and_pr(rinvsix_S0, wco_vdw_S0);
    rinvsix_S1  = gmx_and_pr(rinvsix_S1, wco_vdw_S1);
#ifndef HALF_LJ
    rinvsix_S2  = gmx_and_pr(rinvsix_S2, wco_vdw_S2);
    rinvsix_S3  = gmx_and_pr(rinvsix_S3, wco_vdw_S3);
#endif
#endif
...
#ifdef EXCL_FORCES
    sir6_S0     = gmx_and_pr(sir6_S0, int_S0);
    sir6_S1     = gmx_and_pr(sir6_S1, int_S1);
#endif
#ifndef HALF_LJ
    sir6_S2     = gmx_mul_pr(sir2_S2, gmx_mul_pr(sir2_S2, sir2_S2));
    sir6_S3     = gmx_mul_pr(sir2_S3, gmx_mul_pr(sir2_S3, sir2_S3));
#ifdef EXCL_FORCES
    sir6_S2     = gmx_and_pr(sir6_S2, int_S2);
    sir6_S3     = gmx_and_pr(sir6_S3, int_S3);
#endif
#endif
#ifdef VDW_CUTOFF_CHECK
    sir6_S0     = gmx_and_pr(sir6_S0, wco_vdw_S0);
    sir6_S1     = gmx_and_pr(sir6_S1, wco_vdw_S1);
#ifndef HALF_LJ
    sir6_S2     = gmx_and_pr(sir6_S2, wco_vdw_S2);
    sir6_S3     = gmx_and_pr(sir6_S3, wco_vdw_S3);
#endif
#endif
    ...
    VLJ_S0      = gmx_and_pr(VLJ_S0, wco_vdw_S0);
    VLJ_S1      = gmx_and_pr(VLJ_S1, wco_vdw_S1);
#ifndef HALF_LJ
    VLJ_S2      = gmx_and_pr(VLJ_S2, wco_vdw_S2);
    VLJ_S3      = gmx_and_pr(VLJ_S3, wco_vdw_S3);
#endif
#ifdef CHECK_EXCLS
    /* The potential shift should be removed for excluded pairs */
    VLJ_S0      = gmx_and_pr(VLJ_S0, int_S0);
    VLJ_S1      = gmx_and_pr(VLJ_S1, int_S1);
#ifndef HALF_LJ
    VLJ_S2      = gmx_and_pr(VLJ_S2, int_S2);
    VLJ_S3      = gmx_and_pr(VLJ_S3, int_S3);
#endif
#endif
#ifndef ENERGY_GROUPS
