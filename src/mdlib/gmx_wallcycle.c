                         ...
                         int n, double c_sum, double tot)
{
    ...
    if (c_sum > 0)
    {
        ...
        fprintf(fplog, " %-19s %10s %10.3f %14.3f   %5.1f\n",
                name, num, wallt,
                                     ...
                                     int nrank, int nthread)
{
    ...
    fprintf(fplog, "                          Count      (s)        total sum      %%\n");
}
                     ...
                     gmx_wallcycle_t wc, wallclock_gpu_t *gpu_t)
{
    ...
    if (tot > 0)
    {
        c2t_pp  = realtime*nth_tot/(tot*npp*nth_pp);
        c2t_pme = realtime*nth_tot/(tot*npme*nth_pme);
    }
    else
    {
        c2t_pp  = 0;
    ...
    print_cycles(fplog, c2t_pp, "Rest", -1, tot - tot_for_pp, tot);
    fprintf(fplog, "%s\n", hline);
    print_cycles(fplog, c2t_pp, "Total", -1, tot*npp*nth_pp/nth_tot, tot);
    fprintf(fplog, "%s\n", hline);
    ...
    if (npme > 0)
    {
        ...
        for (i = ewcPPDURINGPME+1; i < ewcNR; i++)
        {
            ...
            {
                print_cycles(fplog, c2t_pme, wcn[i],
                             wc->wcc[i].n, cyc_sum[i], tot);
            }
        }

        fprintf(fplog, "%s\n", hline);
        print_cycles(fplog, c2t_pme, "Total", -1,
                     tot*npme*nth_pme/nth_tot, tot);
        fprintf(fplog, "%s\n", hline);
    }
