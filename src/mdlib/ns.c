...
static void init_nblist(FILE *log, t_nblist *nl_sr,t_nblist *nl_lr,
                        ...
                        gmx_bool bfree, int igeometry, gmx_bool badress)
{
    t_nblist *nl;
    int      homenr;
    int      i,nn;
    ...
    for(i=0; (i<2); i++)
    {
        ...
        nl->type        = GMX_NBLIST_INTERACTION_STANDARD;
	...
	if (badress)
	{
            nl->type       = GMX_NBLIST_INTERACTION_ADRESS;
	}
...
void init_neighbor_list(FILE *log,t_forcerec *fr,int homenr)
{
   ...
   for(i=0; i<fr->nnblists; i++) 
   {
       ...
       init_nblist(log,&nbl->nlist_sr[eNL_VDWQQ],&nbl->nlist_lr[eNL_VDWQQ],
                   maxsr,maxlr,ivdw,ivdwmod,ielec,ielecmod,FALSE,igeometry_def, badress);
       init_nblist(log,&nbl->nlist_sr[eNL_VDW],&nbl->nlist_lr[eNL_VDW],
                   maxsr,maxlr,ivdw,ivdwmod,GMX_NBKERNEL_ELEC_NONE,eintmodNONE,FALSE,igeometry_def, badress);
       init_nblist(log,&nbl->nlist_sr[eNL_QQ],&nbl->nlist_lr[eNL_QQ],
                   maxsr,maxlr,GMX_NBKERNEL_VDW_NONE,eintmodNONE,ielec,ielecmod,FALSE,igeometry_def, badress);
       init_nblist(log,&nbl->nlist_sr[eNL_VDWQQ_WATER],&nbl->nlist_lr[eNL_VDWQQ_WATER],
                   maxsr_wat,maxlr_wat,ivdw,ivdwmod,ielec,ielecmod, FALSE,igeometry_w, badress);
       init_nblist(log,&nbl->nlist_sr[eNL_QQ_WATER],&nbl->nlist_lr[eNL_QQ_WATER],
                   maxsr_wat,maxlr_wat,GMX_NBKERNEL_VDW_NONE,eintmodNONE,ielec,ielecmod, FALSE,igeometry_w, badress);
       init_nblist(log,&nbl->nlist_sr[eNL_VDWQQ_WATERWATER],&nbl->nlist_lr[eNL_VDWQQ_WATERWATER],
                   maxsr_wat,maxlr_wat,ivdw,ivdwmod,ielec,ielecmod, FALSE,igeometry_ww, badress);
       init_nblist(log,&nbl->nlist_sr[eNL_QQ_WATERWATER],&nbl->nlist_lr[eNL_QQ_WATERWATER],
                   maxsr_wat,maxlr_wat,GMX_NBKERNEL_VDW_NONE,eintmodNONE,ielec,ielecmod, FALSE,igeometry_ww, badress);
       ...
       if (fr->efep != efepNO) 
       {
           ...
           init_nblist(log,&nbl->nlist_sr[eNL_VDWQQ_FREE],&nbl->nlist_lr[eNL_VDWQQ_FREE],
                       maxsr,maxlr,ivdw,ivdwmod,ielecf,ielecmod,TRUE,GMX_NBLIST_GEOMETRY_PARTICLE_PARTICLE, badress);
           init_nblist(log,&nbl->nlist_sr[eNL_VDW_FREE],&nbl->nlist_lr[eNL_VDW_FREE],
                       maxsr,maxlr,ivdw,ivdwmod,GMX_NBKERNEL_ELEC_NONE,eintmodNONE,TRUE,GMX_NBLIST_GEOMETRY_PARTICLE_PARTICLE, badress);
           init_nblist(log,&nbl->nlist_sr[eNL_QQ_FREE],&nbl->nlist_lr[eNL_QQ_FREE],
                       maxsr,maxlr,GMX_NBKERNEL_VDW_NONE,eintmodNONE,ielecf,ielecmod,TRUE,GMX_NBLIST_GEOMETRY_PARTICLE_PARTICLE, badress);
       }  
   }
   /* QMMM MM list */
   if (fr->bQMMM && fr->qr->QMMMscheme != eQMMMschemeoniom)
   {
       init_nblist(log,&fr->QMMMlist,NULL,
                   maxsr,maxlr,0,0,ielec,ielecmod,FALSE,GMX_NBLIST_GEOMETRY_PARTICLE_PARTICLE, badress);
   }
    ...
    else
    {
	  ...
	  /* Loop over the j charge groups */
            for(j=0; (j<nj); j++)
            {
                ...
                if (iwater == esolSPC && jwater == esolSPC && wf[jj0] == 1)
                {
                    /* Interaction between two SPC molecules */
                    if (!bDoCoul)
                    {
                ...
                else if (iwater == esolTIP4P && jwater == esolTIP4P && wf[jj0] == 1)
                {
                    /* Interaction between two TIP4p molecules */
                    if (!bDoCoul)
                    {
