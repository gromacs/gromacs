 ...
 * and `gmxapi::GMXAPI_PATCH` so that compatibility checks can be performed at runtime.
...
/*
 * This header will appear in multiple translation units. We shouldn't define
 * these symbols publicly. We use constants instead of preprocessor defines to
 * better catch CMake errors.
 *
 * Note: it may be preferable for CMake to get the version from the header instead of the other way around.
 * For client code projects that prefer to be self-contained, it would be nice
 * to be able to pull the headers straight from the repository...
 */
