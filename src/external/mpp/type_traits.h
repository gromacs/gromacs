...
class datatype {
    public:
        datatype(MPI_Datatype dt = MPI_DATATYPE_NULL) : dt_(dt) { } //no explicit
        ~datatype() {
            if (!mpi_type_traits<T>::is_named() && dt_!=MPI_DATATYPE_NULL) {
                MPI_Type_free(&dt_);
            }
        }
        ...
        void reset(MPI_Datatype dt = MPI_DATATYPE_NULL) {
            datatype(dt).swap(*this);
        }
        void swap(datatype & b) {
            std::swap(b.dt_,dt_);
        }
    private:
...
struct mpi_type_traits<std::vector<T> > {
    ...
    static inline MPI_Datatype get_type_selection (const std::vector<T>& vec, const std::vector<int>& sel, const int begin, const int end) {
        if (vec.size()>0 && mpi_type_traits<T>::is_static()) {
            ...
            MPI_Datatype dt;
            datatype<T> elem_dt(mpi_type_traits<T>::get_type(vec.front()));
...
struct mpi_type_traits<std::list<T> > {
    ...
    static MPI_Datatype get_type(const std::list<T>& l) {
        ...
        *(type_it++) = mpi_type_traits<T>::get_type( l.front() ); //TODO use datatype for RAII/free
