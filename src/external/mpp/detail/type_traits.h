...
struct mpi_type_traits {
    //return datatype corresponding to C++ type. Template specialization and
    //not function overloading is used to make sure no implicit type conversion
    //happens. Should not be directly called but instead the datatype class
    //should be used (for RAII and caching).
    ...
    // and the extent of the mpi type return by get_layout is equal to sizeof(T)
...
class data_layout {
    public:
        ...
        //change datatype into array of same type. Only possible for static type.
        //should not be used directly but instead get_layout(Iter begin, Iter end)
        void makeArray(int size) { storage_->size_*=size; }
    private:
...
extern std::vector<data_layout> cached_types;
template<class T>
inline data_layout get_layout(const T& raw) {
    ...
    if (!mpi_type_traits<T>::is_static::value) {
        ...
        if (cached_type == MPI_DATATYPE_NULL) {
            ...
            cached_types.push_back(move(tmp));
        }
...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
// mpi_type_builder: For easy creation of custom datatypes
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
class mpi_type_builder {
public:
    // Creates a builder for the type passed in to the constructor.
    template <class C>
    explicit mpi_type_builder(const C& c, int nelem) :
    ...
    inline void add (const T& t) {
        type_.push_back(get_layout(t));
        max_ub = std::max(max_ub, (const void*)(&t+1));
    }
    // Returns an datatype for the type that the builder is tied to.
    data_layout build() {
        MPI_Datatype dt;
        assert(!type_.empty());
        std::vector<int> size;
        ...
        size.reserve(type_.size()+2);
        addr.reserve(type_.size()+2);
        e_dt.reserve(type_.size()+2);
        ...
        if (isStatic && type_[0].get_addr()!=baseAddr) {
            size.push_back(1);
            addr.push_back(0);
            e_dt.push_back(MPI_LB);
        }
        for (size_t i=0;i<type_.size();i++) {
            size.push_back(type_[i].get_size());
            assert(type_[i].get_addr() >= baseAddr || !isStatic);
            addr.push_back(type_[i].get_addr() - baseAddr);
            //one could do block compression as in get_layout if types are the same
            //or even if types are not the same if running on non-heterogenous hardware (so that MPI_BYTE copy is OK)
            e_dt.push_back(type_[i].get());
        }
...
private:
    //Private because it only works correctly for non-static. But needed from get_layout,
    //where we don't have access to the container to pass it as c.
    mpi_type_builder() : base_(0), max_ub(0), real_extent(0) {};
    template <class Iter>
    friend data_layout get_layout (Iter begin, Iter end, bool bAbsolute=false);
    const void           *base_ , *max_ub;
    std::vector<data_layout> type_;
    size_t                real_extent;
};
...
template <class Iter>
inline data_layout get_layout(Iter begin, Iter end, bool bAbsolute) {
    ...
    if (begin==end) {
        ...
        data_layout dt = get_layout(*begin);
        ...
        data_layout elem_dt = get_layout(*begin);
        int esize = elem_dt.get_size();
...
template<class E>
static inline data_layout get_layout_enum(const E& e) {
...
struct mpi_type_traits<std::vector<T> > : default_mpi_type_trait {
    static inline data_layout get_layout(const std::vector<T>& vec) {
        return mpi::get_layout(vec.begin(), vec.end());
    }
};
#ifdef MPP_CXX11_ARRAY
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
struct mpi_type_traits<std::list<T> > : default_mpi_type_trait {
    static data_layout get_layout(const std::list<T>& l) {
        return mpi::get_layout(l.begin(), l.end());
    }
};
}
