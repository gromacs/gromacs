...
//                assert(std::distance(sendBegin,sendEnd) == *recvCountBegin);
                ...
                if (recvBegin!=recvEnd) {
                    recv_dl = get_type_layout(*recvBegin);
                    for(int i=0; i<comm_size; i++) {
                        real_recv_count[i] = recvCountBegin[i]*recv_dl.get_size();
            ...
            MPI_Gatherv (
                send_dl.get_ptr(), send_dl.get_size(), send_dl.get(),
                recv_dl.get_ptr(), &real_recv_count.front(),
                &recv_disps.front(), recv_dl.get(), root, *this);
        } else {
            ...
            data_layout send_dl = get_range_layout(sendBegin, sendEnd);
            send_types[root] = send_dl.get();
            send_count[root] = send_dl.get_size();

            std::vector<MPI_Datatype> recv_types(comm_size);
            boost::scoped_array<data_layout> recv_dl(new data_layout[comm_size]);
            ...
            if (rank()==root) {
                ...
                for (int i=0; i<comm_size; i++) {
                    ...
                    recv_dl[i] = get_range_layout(typeDisp, typeEnd, true);
                    recv_types[i] = recv_dl[i].get();
                    recv_count[i] = recv_dl[i].get_size();
                    typeDisp = typeEnd;
                }
            }
            ...
            MPI_Alltoallw(
                send_dl.get_ptr(), &send_count.front(), &send_disps.front(), &send_types.front(),
