            ...
            case TYPEDO:
                ...
                switch (BITS(2))
                {
                    ...
                    case 2: /* dynamic block */
                        ...
                        state->mode = TABLE;
                        break;
                    case 3: strm->msg = (char*)"invalid block type"; state->mode = BAD;
                }
                DROPBITS(2);
                break;
            case STORED:
...
#endif
                state->mode = DONE;
            case DONE: ret = Z_STREAM_END; goto inf_leave;
...
long ZEXPORT inflateMark(strm) z_streamp strm;
{
    ...
    if (strm == Z_NULL || strm->state == Z_NULL)
        return ~0xFFFFL; /* Original code returned -1L << 16, but that is undefined behaviour */
    state = (struct inflate_state FAR*)strm->state;
