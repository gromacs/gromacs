                                                        ...
                                                        const char hash_mode)
{
    ...
    int64_t codec_id, medium_stride_len = 10, long_stride_len = 100;
    int64_t dist_exp = -9, temp_int, temp_int2;
//     int64_t frame_nr;
    double box_shape[9], prec = 1000, temp_double, time_per_frame = 2e-15;
    char atom_type[16], annotation[128];
    char user_name[] = "User1";
    char program_name[] = "tng_testing";
    char computer_name[] = "Unknown computer";
    ...
    tng_compression_precision_set(*traj, prec);
...
*/
    ...
    stat = tng_frame_set_nr_find(*traj, 55);
    if(stat != TNG_SUCCESS)
    {
        return(stat);
    }

    stat = tng_frame_set_nr_find(*traj, 155);
    if(stat != TNG_SUCCESS)
    {
        return(stat);
    }
    ...
    if(temp_int != 155 * n_frames_per_frame_set)
    {
...
tng_function_status tng_test_append(tng_trajectory_t traj, const char hash_mode)
{
    ...
    stat = tng_last_user_name_set(traj, "User2");
    if(stat != TNG_SUCCESS)
    {
    ...
    stat = tng_last_program_name_set(traj, "tng_testing");
    if(stat != TNG_SUCCESS)
    {
    ...
    stat = tng_last_computer_name_set(traj, "Unknown computer");
    if(stat != TNG_SUCCESS)
    {
