...
code around. 
The loop is used when a \myindex{watertype} is selected in the 
{\tt .mdp} file.

\subsection{Shake for Water - SETTLE}
\label{sec:settle}
We have implemented the \myindex{SETTLE} algorithm~\cite{Miyamoto92} which is
an analytical solution of \myindex{shake} specifically for water. 
SETTLE is used when a watertype is selected in the {\tt .mdp} file.

\subsection{Fortran Code}
Unfortunately, \myindex{Fortran} compilers are still better than C-compilers;
...
\begin{table}[h]
\caption{List of C functions and their Fortran equivalent, plus the source files. The 8 refers to double precision version of the same routine. In C it is not necessary to use a special version of the code due to the ue of {\tt typedef}.}
\centerline{
