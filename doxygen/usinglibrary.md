...
Known issues:
 ...
 - If \Gromacs is built with `GMX_GPU=ON`, CMake is refuses to generate import
   definitions for `libgromacs` due to incompatibility between this
   functionality and the way the CUDA libraries are built as intermediate
   static libraries.  So the CMake package configuration files can only be used
   with `GMX_GPU=OFF`.
