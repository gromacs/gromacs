...
def check_file(fileobj, reporter):
    ...
    if fileobj.is_source_file():
        ...
        reporter.file_error(fileobj, "non-installed header specified as part of public API")
    elif fileobj.get_documentation_type() < fileobj.get_api_type():
        reporter.file_error(fileobj, "API type ({0}) conflicts with documentation visibility ({1})".format(fileobj.get_api_type(), fileobj.get_documentation_type()))
...
def check_include(fileobj, includedfile, reporter):
    ...
    if includedfile.is_system():
        if includedfile.get_file():
            reporter.code_issue(includedfile, "includes a local file as {0}".format(includedfile))
    else:
        otherfile = includedfile.get_file()
        if not otherfile:
            reporter.code_issue(includedfile, "includes a non-local file as {0}".format(includedfile))
        elif fileobj.is_installed() and not includedfile.is_relative():
            reporter.code_issue(includedfile, "installed header includes {0} using a non-relative path".format(includedfile))
        if not otherfile:
            return
        if fileobj.is_installed() and not otherfile.is_installed():
            reporter.code_issue(includedfile, "installed header includes non-installed {0}".format(includedfile))
        filemodule = fileobj.get_module()
        othermodule = otherfile.get_module()
        if fileobj.is_documented() and otherfile.is_documented():
            if fileobj.get_documentation_type() > otherfile.get_documentation_type():
                reporter.code_issue(includedfile, "{0} file includes {1} file {2}".format(fileobj.get_documentation_type(), otherfile.get_documentation_type(), includedfile))
        check_api = (othermodule and othermodule.is_documented())
        if check_api and otherfile.get_api_type() < DocType.library and filemodule != othermodule:
            reporter.code_issue(includedfile, "included file {0} is not documented as exposed outside its module".format(includedfile))
...
def check_class(classobj, reporter):
    ...
    if classobj.is_documented():
        ...
        if classtype == DocType.public and not classobj.is_in_installed_file():
            reporter.doc_error(classobj, "has public documentation, but is not in an installed header")
        elif filetype is not DocType.none and classtype > filetype:
            reporter.doc_error(classobj, "is in {0} file(s), but appears in {1} documentation".format(filetype, classtype))
...
def check_member(member, reporter):
    ...
    if member.is_documented():
        if not member.is_visible():
            ...
            reporter.doc_note(member, "is documented, but is ignored by Doxygen, because its scope is not documented")
