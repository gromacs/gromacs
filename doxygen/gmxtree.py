...
class Module(object):
    ...
    Currently, this is only created when Doxygen documentation is found for the
    module.
    """
...
class Class(object):
    ...
    def get_location(self):
        return self._rawdoc.get_location()
...
class GromacsTree(object):
    ...
    walking the source tree.  At this point, only information that is
    accessible from file names and paths only is available.
    ...
    def _walk_dir(self, rootpath):
        ...
        rootdir = Directory(rootpath, None)
        self._dirs[rootpath] = rootdir
        for dirpath, dirnames, filenames in os.walk(rootpath):
            if 'contrib' in dirnames:
                dirnames.remove('contrib')
            if 'refdata' in dirnames:
                dirnames.remove('refdata')
            currentdir = self._dirs[dirpath]
            for dirname in dirnames:
                fullpath = os.path.join(dirpath, dirname)
                self._dirs[fullpath] = Directory(fullpath, currentdir)
            for filename in filenames:
                basename, extension = os.path.splitext(filename)
                if extension in ('.h', '.cuh', '.hpp', '.c', '.cc', '.cpp', '.cu'):
                    fullpath = os.path.join(dirpath, filename)
                    self._files[fullpath] = File(fullpath, currentdir)
                # TODO: Treat .cmakein
    ...
    def _load_dir(self, dirdoc, parent):
        ...
        if not os.path.isabs(path):
            self._reporter.xml_assert(dirdoc.get_xml_path(), "expected absolute path in Doxygen-produced XML file")
            return
        dirobj = self._dirs.get(path)
        if not dirobj:
    ...
    def _load_files(self):
        ...
        for filedoc in self._docset.get_files():
            ...
            if not os.path.isabs(path):
                self._reporter.xml_assert(filedoc.get_xml_path(), "expected absolute path in Doxygen-produced XML file")
                continue
            ...
            if not dirdoc:
                self._reporter.xml_assert(filedoc.get_xml_path(), "file is not in any directory in Doxygen")
                continue
    ...
    def set_installed_file_list(self, installedfiles):
        ...
        for path in installedfiles:
            if not os.path.isabs(path):
                self._reporter.input_error("installed files should be specified with absolute path: {0}".format(path))
                continue
            if path not in self._files:
                reporter.input_error("installed header not in source tree: {0}".format(path))
                continue
