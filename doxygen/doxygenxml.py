...
class Member(Entity):
    ...
    def has_same_body_location(self):
        return self.get_raw_location().has_same_body_location()

    def get_reporter_location(self):
        return self.get_raw_location().get_reporter_location()

    def get_body_location(self):
        return self.get_raw_location().get_body_location()
        ...
        for elem in rootelem:
            if elem.tag == 'name':
                ...
                if not self._load_element(elem):
                    ...
                    #reporter.xml_assert(xmlpath, "unknown member child element '{0}'".format(elem.tag))
                    pass
    ...
    def show(self):
        ...
        print 'Location:   {0}'.format(self.get_raw_location().get_location().get_full_string())
...
class EnumValue(Member):
    ...
    def get_raw_location(self):
        return self._enum.get_raw_location()
...
class Compound(Entity):
    ...
    def load_details(self):
        ...
        for elem in root[0]:
            if elem.tag == 'compoundname':
                if elem.text != self.get_name():
                    reporter.xml_assert(xmlpath, "compound name mismatch: '{0}' (in index.xml) vs. '{1}'".format(self.get_name(), elem.text))
            elif elem.tag == 'briefdescription':
                briefelem = elem
            elif elem.tag == 'detaileddescription':
                detailselem = elem
                ...
                if reftype == 'file':
                    ...
                    reporter.xml_assert(xmlpath, "unknown inner compound type '{0}'".format(reftype))
                ...
                if not self._load_element(elem):
                    reporter.xml_assert(xmlpath, "unknown compound child element '{0}'".format(elem.tag))
        if missing_members:
    ...
    def _unexpected_inner_compound(self, typename, compound):
        ...
        reporter = self._get_reporter()
        xmlpath = self._get_xml_path()
        reporter.xml_assert(xmlpath, "unexpected inner {0}: {1}".format(typename, compound))
...
class Class(Compound):
    ...
    def show(self):
        ...
        print 'Location:   {0}'.format(self._location.get_location().get_full_string())
        print 'Body loc:   {0}'.format(self._location.get_body_location().get_full_string())
            ...
            for memberelem in compoundelem.iter('member'):
                ...
                if refid in self._members:
                    ...
                    if not isinstance(member, membertype):
                        reporter.xml_assert(xmlpath, "id '{0}' used for multiple kinds of members".format(refid))
                        continue
                else:
                    membertype = _get_member_type_from_kind(kind)
                    if membertype is None:
                        reporter.xml_assert(xmlpath, "unknown member kind '{0}'".format(kind))
                        continue
            ...
            if len(memberlist) > 1:
                ...
                for member in memberlist:
                    if member.has_same_body_location():
                        if definition is not None:
                            self._reporter.xml_assert(None, "duplicate definition for a member '{0}'".format(definition))
                            continue
                        definition = member
                ...
                if otherdeclarations:
                    ...
                    for otherdeclaration in otherdeclarations:
                        details.append('{0}: another declaration is here'.format(otherdeclaration.get_reporter_location()))
                    details.append('{0}: definition is here'.format(declaration.get_body_location()))
                    self._reporter.code_issue(declaration, "duplicate declarations for a member '{0}'".format(declaration), details)
                    continue
