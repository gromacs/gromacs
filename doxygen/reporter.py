...
class Filter(object):
    ...
    def __init__(self, filterline):
        ...
        filepattern, line, text = filterline.split(':', 2)
        if filepattern == '*':
        ...
        if line == '*':
            self._linematcher = lambda x: x is not None
        elif line:
            line = int(line)
            self._linematcher = lambda x: x == line
        else:
            self._linematcher = lambda x: x is None
    ...
    def matches(self, message):
        ...
        if not self._filematcher(message.location.filename):
            return False
        if not self._linematcher(message.location.line):
            return False
...
class Reporter(object):
    ...
    def report_unused_filters(self):
        ...
        for filterobj in self._filters:
            if filterobj.get_match_count() == 0:
                ...
                self._write(Message('warning: unused filter: ' + filterobj.get_text()))
    ...
    def file_error(self, fileobj, message):
        ...
        self._report(Message('error: ' + message, location=fileobj.get_reporter_location()))

    def code_issue(self, entity, message, details=None):
        """Report an issue in a code construct (not documentation related)."""
        self._report(Message('warning: ' + message, details, location=entity.get_reporter_location()))

    def doc_error(self, entity, message):
        """Report an issue in documentation."""
        self._report(Message('error: ' + entity.get_name() + ': ' + message, location=entity.get_reporter_location()))

    def doc_note(self, entity, message):
        """Report a potential issue in documentation."""
        self._report(Message('note: ' + entity.get_name() + ': ' + message, location=entity.get_reporter_location()))
