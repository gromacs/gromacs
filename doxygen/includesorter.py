...
IncludeGroup.gmx_local = IncludeGroup(7)
IncludeGroup.gmx_external = IncludeGroup(8)
IncludeGroup.config = IncludeGroup(9)
...
class GroupedSorter(object):
    ...
    def _get_path(self, included_file, group, including_file):
        ...
        if not use_abspath:
            if group == IncludeGroup.gmx_general:
                use_abspath = True
    ...
    def format_include(self, fp, obj, prev):
        if prev:
            if prev[0] != obj[0]:
...
class IncludeSorter(object):
    ...
    def _sort_include_block(self, fp, block):
        includes = map(self._sortmethod.get_sortable_object, block.get_includes())
        includes.sort()
        prev = None
        for include in includes:
            self._sortmethod.format_include(fp, include, prev)
            prev = include
