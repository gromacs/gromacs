...
particular problems, rather than single-time reading.
...
the visibility and API definitions (currently, only the file-level comments are
checked by these targets).
...
listed using \c \\author commands.  This provides an point of contact if one
has questions.
...
is meant to be subclassed.  For example, if a class is meant to be subclasses
...
same methods from Doxygen, otherwise you will get warnings).
