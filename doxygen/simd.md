...
use a vendor-agnostic \Gromacs SIMD module, located under the
`src/simd` subdirectory.

Design of the \Gromacs SIMD module
==================================

The macros in `src/simd` are intended to be used for writing
...
The SIMD module handles the challenges above by introducing a number of datatypes;
...
common implementations only use half as many element for double as
...
<dd>
Integers used when converting to/from single precision. The routines
...
<dd>
Integers used when converting to/from double. On many architectures,
...
operations for SIMD4, but that is trivial to extend if we need to. The
...
<dd>
Support for and/andnot/or/xor on fp regs.
</dd>
...
`gmx_simd_sincos_{d|f}` around the `gmx_simd_sincos_{d|f}()` functions.
