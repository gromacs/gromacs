...
\item   \myindex{GMXNPRI}, for SGI systems only. When set, gives the
	default non-degrading priority (npri) for mdrun, e.g.\@
	setting \verb'setenv GMXNPRI 250' causes all runs to be
	performed at near-lowest priority by default.
\end{enumerate}

Some other environment variables are specific to one program, such as
\myindex{TOTAL} for the \myindex{do\_shift} program, and
\myindex{DSPP} for the \myindex{do\_dssp} program.
