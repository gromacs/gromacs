...
# use default flags for gcc/g77 on all systems, but catch the SGI/ia64 sgicc
...
# the libtool-1.4b beta distribution.
# See admin/README.patches for a list of changes!
##################################################################
...
# Determine the tags from the languages used - this is another
# hack by Erik L - if the flag for -g is set, then AC_PROG_*** has
# probably been called. NB: I removed the GCJ support...
tagnames=
...
# We never build shared libraries with the fortran linker;
# this support is only meant for including fortran objects
# in C libraries. You should normally always be able to
# perform the linking stage with the C compiler, anyway...
  ...
  *)
  ...
  aix4* | aix5*)
    ...
    if test "$aix_use_runtimelinking" = yes; then
      ...
      _LT_AC_TAGVAR(allow_undefined_flag, $1)=' $wl}-berok'
      # Determine the default libpath from the value encoded in an empty executable.
      # If we don't find anything, use the default library path according to the cc manual.
      AC_CHECK_AIX_LIBPATH
      _LT_AC_TAGVAR(hardcode_libdir_flag_spec, $1)='${wl}-blibpath:$libdir:'"$aix_libpath"
