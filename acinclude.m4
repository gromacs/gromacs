...
# the libtool-1.4e beta distribution.
...
[
  if test "$GCC" = yes; then
    ...
    *)
    ...
    linux*)
      # Check flags for non-gnu compilers on Linux. We check
      # the architectures too and not only the compiler names,
      # since they are often linked to cc.
      case "$host_cpu" in
      alpha*)
