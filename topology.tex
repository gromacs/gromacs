...
coordinate file \verb'*.gro' (positions and velocities), or trajectory
file \verb'*.trr' (positions, velocities, forces).
...
From {\gromacs} 4.5 implicit solvent is supported. A section in the
...
elaborate surface area algorithms, the one from Still et al in
particular.  Column 4 is the Vdw-radii for atom types that are used
for computing the Born radii. The dielectric offset is specified in
the mdp-file, and gets added to the input radii for the different
radii methods.  Column 5 is the scale factor for the HCT/OBC
models. The values are taken from the original HCT reference, as well
