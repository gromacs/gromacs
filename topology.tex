...
databases are stored in {\tt ffgmx-n.tdb} and {\tt ffgmx-c.tdb} for
the N- and C-terminus respectively. They contain information for the
{\tt pdb2gmx} program on how to connect new atoms to existing
ones. The termini database files also contain information on which
atoms should be removed or changed and which (improper) dihedral
...
\begin{itemize}
\item the parameter level (see Table~\ref{ta:topfile})
\item the molecule level, which should contain one or more molecule
      definitions (see Table~\ref{ta:topfile})
...
\hline
interaction 	& directive   	      & \#  & f. & parameters 				& p. \\
type	&		      	      & at. & tp &					& 	\\
\hline
\multicolumn{6}{c}{\bf Parameters} \\
\hline
		& {\tt defaults}	& & &	non-bonded function type; combination 	& \\
		&			& & &	rule; generate pairs (no/yes); & \\
		&			& & &	fudge LJ (); fudge QQ () & \\
\hline
		& {\tt atomtypes}	&   & 	& atom type; m (u); q (e); particle type; & \\
		&			&   &	& c$_6$ (\kJmolnm{6}); c$_{12}$ (\kJmolnm{12}) & \\
\hline
		& {\tt bondtypes}	& \multicolumn{3}{l}{(see directive {\tt bonds})}		& 	\\
		& {\tt constrainttypes}	& \multicolumn{3}{l}{(see directive {\tt constraints})}		& 	\\
		& {\tt pairtypes}	& \multicolumn{3}{l}{(see directive {\tt pairs})}		& 	\\
		& {\tt angletypes}	& \multicolumn{3}{l}{(see directive {\tt angles})}		& 	\\
proper dih.	& {\tt dihedraltypes}	& 2 & 1	& $\theta_{max}$ (deg); f$_c$ (\kJmol); multiplicity & X	\\
improper dih.	& {\tt dihedraltypes}	& 2 & 2	& $\theta_0$ (deg); f$_c$ (\kJmolrad{-2}) & X	\\
RB dihedral	& {\tt dihedraltypes}	& 2 & 3	& C$_{0,ldots,5}$ (\kJmol) 		&	\\
LJ 		& {\tt nonbond\_params}	& 2 & 1	& c$_6$ (\kJmolnm{6}); c$_{12}$ (\kJmolnm{12}) & \\
Buckingham    	& {\tt nonbond\_params}	& 2 & 2	& a (\kJmol); b (nm$^{-1})$; c$_6$ (\kJmolnm{6}) & \\
\hline
\multicolumn{6}{c}{\bf Molecule(s)} \\
\hline
		& {\tt moleculetype}	& & & 	molecule name; exclude neighbors \# bonds &	\\
		&			& & & 	away for non-bonded interactions 	&	\\
\hline
		& {\tt atoms}		& 1 & 	& atom type; residue number; 	& 	\\
		&			&   &	& residue name; atom name; 	& 	\\
		&			&   &	& charge group number; q (e); m (u) 	& X	\\
\hline
...
LJ 1-4		& {\tt pairs}		& 2 & 1	& c$_6$ (\kJmolnm{6}); c$_{12}$ (\kJmolnm{12}) & X \\
angle		& {\tt angles}		& 3 & 1	& $\theta_0$ (deg); f$_c$ (\kJmolrad{-2}) & X	\\
G96 angle	& {\tt angles}		& 3 & 2	& $\theta_0$ (deg); f$_c$ (\kJmol) & X	\\
proper dih.	& {\tt dihedrals}	& 4 & 1	& $\theta_{max}$ (deg); f$_c$ (\kJmol); multiplicity & X	\\
improper dih.	& {\tt dihedrals}	& 4 & 2	& $\theta_0$ (deg); f$_c$ (\kJmolrad{-2}) & X	\\
RB dihedral	& {\tt dihedrals}	& 4 & 3	& C$_{0,\ldots,5}$ (\kJmol) 		&	\\
...
distance res.	& {\tt distance\_restraints}	& 2 & 1	& type; index; low, up$_1$, up$_2$ (nm); factor () & \\
angle res.	& {\tt angle\_restraints}	& 4 & 1	& $\theta_0$ (deg); f$_c$ (\kJmol); multiplicity & X	\\
angle res. z & {\tt angle\_restraints\_z}	& 2 & 1	& $\theta_0$ (deg); f$_c$ (\kJmol); multiplicity & X	\\
exclusions	& {\tt exclusions}	& 1 & 	& atoms (index)				& 	\\
\hline
\multicolumn{6}{c}{\bf System} \\
\hline
		& {\tt system}		& & &	system name				&	\\
		& {\tt molecules}	& & &	molecule name; number of molecules	&	\\
\hline
\end{tabular}
}
\caption{The molecular topology. '\# at' is the number of atom
indices, 'f. tp' is function type, 'p.' indicates if this interaction can be
modified during free energy perturbation.}
\label{ta:topfile}
\end{table}
