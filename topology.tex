...
\item \raggedright
      ...
      \verb'[ position_restraints ]', \verb'[ distance_restraints ]',
...
\begin{table}
\centerline{\small\begin{tabular}{|llllll|}
\dline
interaction 	& directive   	      & \#  & f. & parameters 				& pert. \\
	&		      	      & at. & tp &					& 	\\
\hline
		& {\tt defaults}	& & &	non-bonded function type; combination rule; 	& \\
		&			& & &	generate pairs (no/yes); fudge LJ (); fudge QQ () & \\
\hline
		& {\tt atomtypes}	& 1 & 	& m (u); q (e); particle type (); 	&	\\
		&			&   &	& c$_6$ (\kJmolnm{6}); c$_{12}$ (\kJmolnm{12}) & \\
\hline
		& {\tt bondtypes}	&   & 	& (see interaction table)		& 	\\
		& {\tt constrainttypes}	&   & 	& (see interaction table)		& 	\\
		& {\tt pairtypes}	&   & 	& (see interaction table)		& 	\\
		& {\tt angletypes}	&   & 	& (see interaction table)		& 	\\
proper dih.	& {\tt dihedraltypes}	& 2 & 1	& $\theta_{max}$ (deg); f$_c$ (\kJmol) 	& X	\\
improper dih.	& {\tt dihedraltypes}	& 2 & 2	& $\theta_0$ (deg); f$_c$ (\kJmolnm{-2}) & X	\\
...
\hline
		& {\tt atoms}		& 1 & 	& atom type (name); residue (\#); 	& 	\\
		&			&   &	& residue (name); atom name (name); 	& 	\\
		&			&   &	& charge group (index); q (e); m (u) 	& X	\\
\hline
...
angle		& {\tt angles}		& 3 & 1	& $\theta_0$ (deg); f$_c$ (\kJmolrad{-1}) & X	\\
G96 angle	& {\tt angles}		& 3 & 2	& $\theta_0$ (deg); f$_c$ (\kJmoldeg{-2}) & X	\\
proper dih.	& {\tt dihedrals}	& 4 & 1	& $\theta_{max}$ (deg); f$_c$ (\kJmol) 	& X	\\
improper dih.	& {\tt dihedrals}	& 4 & 2	& $\theta_0$ (deg); f$_c$ (\kJmolnm{-2}) & X	\\
...
settle		& {\tt settles}		& 3 & 1	& d$_{OH}$, d$_{HH}$ (nm) 		& 	\\
...
posres	& {\tt position\_restraints}	& 1 & 1	& k$_{x}$, k$_{y}$, k$_{z}$ (\kJmolnm{-2}) & 	\\
wpol	& {\tt position\_restraints}	& 1 & 2	& ???	& 	\\
disres	& {\tt distance\_restraints}	& 2 & 1	& type, index (); low, up$_1$, up$_2$ (nm); factor () & \\
angres	& {\tt angle\_restraints}	& 4 & 1	& $\theta_0$ (deg); f$_c$ (\kJmol) 	& X	\\
angres z& {\tt angle\_restraints\_z}	& 2 & 1	& $\theta_0$ (deg); f$_c$ (\kJmol) 	& X	\\
excl		& {\tt exclusions}	& 1 & 	& atoms (index)				& 	\\
\dline
