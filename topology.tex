...
{\tt [~virtual\us{}sites?~]} (for backward compatibility the old name
...
associated with that interaction. All the kinds of interactions are
listed in Tables \ref{tab:topfile2} and \ref{tab:topfile3}. Example
excepts from such files follow:
...
of the topology file (see~\ssecref{topfile}):
\begin{eqnarray}
...
the {\tt [~nonbond\us{}params~]} section. Any combination that is
...
that uses zero for all solute non-bonded parameters, i.e. charges and LJ parameters.
...
in separate ``14-LJ'' and ``14-Coulomb'' entries per energy group pair.
Energies for type 3 are added to the ``LJ-SR'' and ``Coulomb-SR'' terms.
}{}
...
so that {\tt grompp}, {\tt make\us{}ndx} and analysis tools can recognize
...
hydrogens. For more complex virtual site constructs (e.g. when entire aromatic side chains
are made rigid) we also need information about the equilibrium bond lengths and angles
for all atoms in the side chain. This information is specified in the \verb+.vsd+ file for each force 
...
slightly different constraint distances between these mass centers. Note that we do {\em not} have to
...
in the virtual site generation code, so if you would like to extent it e.g.\ to nucleic acids you would also
...
contain 2 or 3 columns with atom names, followed by a number specifying the bond (in nm) or angle
(in degrees). Note that these are approximations of the equilibrated geometry for the entire molecule, 
...
LJ 		& {\tts nonbond\us{}params}	& 2 & 1	& $V^{(cr)}$; $W^{(cr)}$ & \\
Buckingham    	& {\tts nonbond\us{}params}	& 2 & 2	& $a$ (\kJmol); $b$ (nm$^{-1})$;  & \\
 & & & & $c_6$ (\kJmolnm{6}) & \\
\dline
\multicolumn{6}{c}{~} \\
...
LJ/C. pair NB	& {\tts pairs\us{}nb}	      & 2 & 1	& $q_i$, $q_j$ (e); $V^{(cr)}$; $W^{(cr)}$ &    \\
...
vsite2		& {\tts virtual\us{}sites2}	& 3 & 1	& $a$ ()					& 	\\
vsite3		& {\tts virtual\us{}sites3}	& 4 & 1	& $a$, $b$ ()				& 	\\
vsite3fd	& {\tts virtual\us{}sites3}	& 4 & 2	& $a$ (); $d$ (nm)				& 	\\
vsite3fad	& {\tts virtual\us{}sites3}	& 4 & 3	& $\theta$ (deg); $d$ (nm) 		& 	\\
vsite3out	& {\tts virtual\us{}sites3}	& 4 & 4	& $a$, $b$ (); $c$ (nm$^{-1}$) 		& 	\\
vsite4fdn	& {\tts virtual\us{}sites4}	& 5 & 2	& $a$, $b$ (); $c$ (nm);	   		& 	\\
vsite COG 	& {\tts virtual\us{}sitesn}	& 1 & 1	& one or more construc. atom ind.   		& 	\\
vsite COM	& {\tts virtual\us{}sitesn}	& 1 & 2	& one or more construc. atom ind.   		& 	\\
vsite COW  	& {\tts virtual\us{}sitesn}	& 1 & 3	& one or more pairs consisting of	& 	\\
                &                               &   &   & a construc. atom ind. and weight           &       \\
position res.	& {\tts position\us{}restraints}   & 1 & 1	& $k_{x}$, $k_{y}$, $k_{z}$ (\kJmolnm{-2}) & all	\\
restr. pot.	& {\tts bonds}		        & 2 & 10	& low, up$_1$, up$_2$ (nm);             & all	\\
               	&                   	        &   &  	& $k_{dr}$ (\kJmolnm{-2})               & 	\\
distance res.	& {\tts distance\us{}restraints}   & 2 & 1	& type; label; low, up$_1$, up$_2$ (nm); & \\
                &                               &   &   & weight () & \\
orient. res.    & \multicolumn{3}{l}{\tts orientation\us{}restraints} & &\\
                &	                        & 2 & 1	& exp.; label; $\alpha$; $c$ (U nm$^\alpha$); & \\
                &                               &   &   &  obs. (U); weight (U$^{-1}$) &\\
angle res.	& {\tts angle\us{}restraints}      & 4 & 1	& $\theta_0$ (deg); $k_c$ (\kJmol); & $\theta,k$	\\
                &                               &   &   & multiplicity & \\
angle res. z & {\tts angle\us{}restraints\us{}z}      & 2 & 1	& $\theta_0$ (deg); $k_c$ (\kJmol); & $\theta,k$	\\
                &                               &   &   & multiplicity & \\
\dline
\multicolumn{6}{c}{~} \\
\multicolumn{6}{l}{`\# at' is the number of atom indices}\\
...
{\bf {\tt [~position\us{}restraints~]} :} harmonically restrain the selected particles
...
combination rules) are defined in the {\tt [~nonbond\us{}params~]}
