...
\label{sec:dummytop}
Some force fields use \swapindex{dummy}{atom}s 
...
LJ/Coul. 1-4	& {\tt pairs}		& 2 & 1	& V$^{(a)}$; W$^{(a)}$ & X \\
...
constraint	& {\tt constraints}$^{(d)}$& 2 & 1	& b$_0$ (nm) 				& X	\\
...
\item generate pairs = no (the default, get 1-4 interactions from pair list)
or yes (generate 1-4 interactions from normal Lennard-Jones parameters using
FudgeLJ)
