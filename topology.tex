...
for builing blocks that come from an {\tt .rtp} file with the same base name.
...
The {\tt .r2b} can contain 2 or 4 columns. The 2-column format
has as the residue name in the first column and the building block name
in the second. The 4-column format has 2 additional columns with
the building block for the residue occuring in the N- and C-terminus
(this is useful for for instance the AMBER force field).
If one or both terminal versions are not present a dash should be entered
...
HISA & neutral histidine with C$_\delta$ protonated \\
HISB & neutral histidine with C$_\epsilon$ protonated \\
HISH & positive histidine with both C$_\delta$ and C$_\epsilon$ protonated \\
LYS  & neutral lysine \\
LYSH & protonated lysine \\
\dline
\end{tabular}
}
...
supports questionmark wildcards, which match a single character.
