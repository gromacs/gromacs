...
\end{verbatim}}
Atomic detail is used except for hydrogen atoms bound to (some) carbon
atoms, which we treated as {\em \normindex{united atoms}} and no special
...
\begin{enumerate}
\item[1]
{\em one planar hydrogen, e.g. rings or peptide bond}
...
degrees
\item[2]
{\em one single hydrogen, e.g. hydroxyl}
...
\item[3]
{\em two planar hydrogens, e.g. -NH{$_2$}}
...
\item[4]
{\em three tetrahedral hydrogens, e.g. -CH{$_3$}}
...
\item[5]
{\em one tetrahedral hydrogen, e.g. C{$_3$}CH}
one hydrogen atom (n1) is added in tetrahedral conformation such that
angle (n1-i-j)=(n1-i-k)=(n1-i-l)=109.5 degrees
\item[6]
{\em two tetrahedral hydrogens, e.g. C-CH{$_2$}-C}
two hydrogen atoms (n1,n2) are added in tetrahedral conformation on
the plane bissecting angle i-j-k with angle
(n1-i-n2)=(n1-i-j)=(n1-i-k)=109.5
\item[7]
{\em two water hydrogens}
two hydrogens are added according to GROMOS water geometry. The
symmetry axis will alternate between three coordinate axes in both
directions
\item[8]
{\em two carboxyl oxygens, -COO{$^-$}}
two oxygens (n1,n2) are added according to rule 3, with a distance of
0.136 nm and an angle of 117 degrees
\item[9]
{\em carboxyl oxygens and hydrogen, -COOH}
two oxygens (n1,n2) are added according to rule 3, with distances of
0.123 nm and 0.125 nm and angles of 121 and 115 degrees for n1 and n2
resp. One hydrogen (n') is added to n2 according to rule 2, where
n-i-j and n-i-j-k should be read as n'-n2-i and n'-n2-i-j resp.
\end{enumerate}
\item[C]
...
The program code to \normindex{generate hydrogen} positions, as well
as the text describing the connection were taken from the Gromos
Package~\cite{biomos} and extended for options 5 through 8. Note that
option 8 is used exclusively to add a COOH terminus. The others are in
principle generic, i.e. they can be used to add any type of atom. Note
however that only termini database support type specification for the 
atoms, the hydrogen database will always add hydrogens.
...
Here is an example of a topology file, \underline{urea.top}:
{\small
\begin{verbatim}
;
