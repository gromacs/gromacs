...
present in biological systems are parametrized in the force field,
...
and second neighbours), as these are already treated in bonded
...
The files are grouped per forcefield type (named e.g. {\tt gmx} for
the GROMACS forcefield or {\tt G43a1} for the GROMOS96
forcefield). All files for one forcefield have names beginning with
{\tt ff???} where {\tt ???} stands for the forcefield name.

\section{Particle type}
\label{sec:parttype}

In {\gromacs} there are 5 types of \normindex{particles}, see
Table~\ref{ta:ptype}. Only regular atoms and dummy particles are used
in {\gromacs}, nuclei, shells and bond shells are necessary for
polarizable forcefields, which we don't yet have.
...
The last 10 atomtypes are extra atomtypes with respect to the \gromosv{87}
...
The mass is listed in {\tt ff???.atp} (see~\ref{subsec:atomtype}), 
whereas the charge is listed
in {\tt ff???.rtp} (.rtp = {\bf r}esidue {\bf t}opology {\bf
...
dihedrals) are listed in {\tt ff???bon.itp}. The term {\tt func} can
...
\normindex{OPLS}~\cite{Jorgensen88} force field.
...
{\tt pdb2gmx} so no worries on that point.

The \normindex{1-4 interactions} are also listed for the 47 atom types in
{\tt ff???nb.itp} under [~pairtypes~]. It is possible to change them there
...
{\tt *.top} file (see section Topology file, ~\ref{subsec:topfile}). 
...
\gromos{96} {\tt .rtp} files. These strings are copied to the topolgy file
...
{\tt [~angles~]} field is only useful for overriding {\tt .itp}
parameters.

{\tt pdb2gmx} automatically generates one proper dihedral for every rotatable
bond, preferably on heavy atoms. When the {\tt [~ihedrals~]} field is used,
...
The \normindex{hydrogen database} is stored in {\tt ff???.hdb}.  The
\normindex{termini database}s are stored in {\tt ff???-n.tdb} and {\tt
ff???-c.tdb} for the N- and C-terminus respectively.  They contain
information for the {\tt pdb2gmx} program on how to connect new atoms
to existing ones. The termini database files also contain information
on which atoms should be removed or changed and which bonded
interactions should be added.

The format of the hydrogen database is as follows:
{\small\begin{verbatim}
; res   # additions
        # H add type    i       j       k
ALA     1
...
of additions. Then follow one line for each addition, on which we see:
\begin{itemize}
...
The format of the termini database is as follows (this is an example
from the {\tt ffgmx-c.tdb}):
{\small\begin{verbatim}
[ None ]
...
but the possible types of addition are extended by two more,
specifically for C-terminal additions:
\begin{enumerate}
...
in the {\tt ff???bon.itp}, see~\ref{subsec:bondparam}.
\end{itemize}
...
replaced by the name of the forcefield you are actually using.

{\bf {\tt [~moleculetype~]} :} defines the name of your molecule in this
{\tt *.top} and nrexcl = 3 stands for excluding 3 bonded atoms

{\bf {\tt [~atoms~]} :} defines the molecule, where {\tt nr} and {\tt type}
...
{\bf {\tt [~distance\_restraints~]} :} put restraints on the distance 
...
molecules. Make sure that in the {\tt spc.itp} file the molecule is
