...
; The first four columns are mandatory. Later columns are optional.
...
; Column 5 : 1 = generate all possible dihedrals from bonding connectivity
;            0 = generate only heavy-atom dihedrals from bonding connectivity
; Column 6 : number of excluded neighbors for non-bonded interactions
; Column 7 : 1 = generate 1,4 interactions between pairs of hydrogen atoms
;            0 = not
; Column 8 : 1 = remove improper dihedrals on same bond as a proper dihedral
;            0 = not

; The default values are as shown here:
...
{\tt [~bondedtypes~]} field, which is followed by at least four numbers,
indicating the default interaction type for bonds, angles, dihedrals, and
improper dihedrals. Further numbers control other behaviour as described
in the comments above. The file contains residue entries, which consist
...
bond, by default only on heavy atoms. When the {\tt [~dihedrals~]} field is used,
...
The default number of exclusions is set to 3, which means that non-bonded
interactions between atoms connected by at most 3 bonds are excluded. Pair
interactions are generated for all pairs of atoms that are separated by 3
bonds (but pairs of hydrogens as controlled by the above option).
