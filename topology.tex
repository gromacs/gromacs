...
\swapindex{atom}{type}s, a sample from the deprecated {\tt gmx.ff}
force field is listed below, with their corresponding masses (in
a.m.u.). This is the type of listing found in the {\tt atomtypes.atp} file (.atp
= {\bf a}tom {\bf t}ype {\bf p}arameter file).  Therefore, it is in this file
that you can change and/or add an atom type.
...
$\sigma$ (see \tabref{statprop} The mass is listed in {\tt atomtypes.atp}
...
In this example the atomtype is listed first, followed by five
...
termini that can be added to a molecule. In this example {\tt [~None~]}
is the first block, corresponding to a terminus that leaves
...
carboxyl group. Block names cannot be any of the following:
...
LJ 		& {\tts nonbond\us{}params}	& 2 & 1	& $V^{(a)}$; $W^{(a)}$ & \\
...
vsite4fd	& {\tts virtual\us{}sites4}	& 5 & 1	& $a$, $b$ (); $d$ (nm);	   		& 	\\
