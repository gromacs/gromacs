...
The \normindex{exclusions} (first and second neighbors) are generated by
...
to the naming convention used in the hydrogen database, see
\ref{subsec:hdb}. The atom names in the bonded interaction can be
...
files. These strings are copied to the topolgy file and can be
...
\label{subsec:hdb}
The \normindex{hydrogen database} is stored in {\tt ff???.hdb}. It
...
HD22}. This is important since atom naming in the {\tt .rtp} file (see
\ref{subsec:rtp}) must be the same. The format of the hydrogen
...
The \normindex{termini database}s are stored in {\tt ff???-n.tdb} and
...
carboxyl group. Blocknames cannot be any of the following: {\tt
...
see \ref{subsec:rtp}), but the possible types of addition are extended
...
\item charge
\end{itemize}
Like in the hydrogen database (see \ref{subsec:rtp}), when more then
