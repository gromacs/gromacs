...
(the different types are explained in~\secref{dummy}).
...
The hydrogen database (see \tabref{ffgmxhdb}) is stored in {\tt ffgmx.hdb}. The termini
...
Dummy constructions are grouped by atom type, which are specified by
...
dummy types 1, 2 and 3 (see \secref{dummy}) resp.\@ and 4 and 5
...
Ideally all dummy parameters should be calculated from the topology
information of the groups involved, this might one day be implemented.

\begin{figure}
\centerline{\psfig{figure=plots/dumtypes.eps,width=12cm}}
...
The dummy database in \verb'ffgmx.ddb' is written to convert all
...
appropriate length, see \figref{dumtypes}A. This removes the high
...
combination of two C-C bond vectors, see \figref{dumtypes}B.
...
atoms, see \figref{dumtypes}C. The arginine single amine hydrogen is
