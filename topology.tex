...
The files are grouped per forcefield type (named {\eg} {\tt gmx} for
...
[~constraints~]} type 1). There is a second constraints type ({\tt
[~constraints~]} type 2) which is not considered a bonded
interaction\index{constraint no connect}.

The \normindex{1-4 interactions} are also listed for the 47 atom types
...
parameters given below (see \secref{disre}).
...
in constraints instead of normal bonds.
