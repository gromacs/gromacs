...
The mechanism for generating bonds used by {\tt pdb2gmx} relies principally on linear linking
of residues to build a macromolecule. In some cases, {\ie} disulfides, a heme group, etc., it is 
necessary to create ``non-linear'' bonds.  The file {\tt specbond.dat} takes care of this function.
...
\item {\tt atomA} The atom in residue A that forms the bond.
\item {\tt nbondsA} The number of bonds {\tt atomA} can form.
\item {\tt resB} The name of residue B that participates in the bond.
\item {\tt atomB} The atom in residue B that forms the bond.
\item {\tt nbondsB} The number of bonds {\tt atomB} can form.
\item {\tt length} The reference length for the bond.  If {\tt atomA} and {\tt atomB} are not within
{\tt length} $\pm$ 10\%, no bond will be formed.
...
\item {\tt rewresB} The new name of residue B.
\end{enumerate}
