...
\begin{enumerate}
    ...
    \item Add the \texttt{\footnotesize openmm/lib} directory to your library path,  e.g. in bash:\\
    \texttt{\footnotesize export LD\_LIBRARARY\_PATH=path\_to\_gromacs/openmm/lib:\$LS\_LIBRARY\_PATH}.\\
    ...
    \texttt{\footnotesize path\_to\_gromacs/bin/mdrun-gpu -h} should display the standard mdrun help  
...
\begin{itemize}
    \item \texttt{\footnotesize cmake  -DGMX\_OPENMM=ON [-DCMAKE\_INSTALL\_PREFIX=desired\_install\_path]}
    \item \texttt{\footnotesize make mdrun}
    \item \texttt{\footnotesize make install-mdrun} ???
\end{itemize}
...
\texttt{\footnotesize mdrun-gpu -device "ACCELERATION:[DEV\_OPTION=VALUE,]... [OPTION].."}.\\

The option-list prefix \texttt{\footnotesize ACCELERATION} specifies which 
acceleration library should be used. At the moment, the only supported value 
is \texttt{\footnotesize OpenMM}. This is followed by the list of comma-separated
\texttt{\footnotesize DEV\_OPTION=VALUE} option-value pairs which define 
parameters for the selected acceleration platform. 
...
on testing, the default value is \texttt{\footnotesize memtest=15s}. 
It is possible to completely turn of memory testing by setting 
\texttt{\footnotesize memtest=15s}, however this is not advisable.
