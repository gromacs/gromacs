...
or multiple when {\tt energygrp_table} is set (see below).
...
different energy groups pairs through the {\tt .mdp} option {\tt energygrp_table}
(see \secref{mdpopt}).
Atoms that should interact with a different potential should
be put into different energy groups.
Between group pairs which are not listed in {\tt energygrp_table},
...
are hard coded in OpenMM, {\ie} {\tt obc_alpha=1}, {\tt obc_beta=0.8} and {\tt obc_gamma=4.85}.
...
\begin{enumerate}
    ...
    {\tt path_to_gromacs/bin/mdrun-gpu -h} should display the standard {\tt mdrun} help  
