...
{\tt pull\us{}geometry = cylinder} does not
use all the atoms of the reference group, but instead dynamically only those
within a cylinder with radius {\tt r\us{}1} around the pull vector going
...
can be defined with {\tt r\us{}0}, with a linear switch function that weighs
the contribution of atoms between {\tt r\us{}0} and {\tt r\us{}1} with
...
\right\rangle_{t_0}
\eeq
This can be done with {\tt g\us{}energy}.
...
Note that table look-up is significantly {\em
...
or multiple when {\tt energygrp\us{}table} is set (see below).
...
The $x$ should run from 0 to $r_c+1$ (the value of {\tt table\us{}extension} can be
...
different energy groups pairs through the {\tt .mdp} option {\tt energygrp\us{}table}
(see \secref{mdpopt}).
Atoms that should interact with a different potential should
be put into different energy groups.
Between group pairs which are not listed in {\tt energygrp\us{}table},
...
{\gromacs} 4.5 contains an experimental release of  GPU-accelerated
MD simulations using the \href{https://simtk.org/home/openmm}{OpenMM library}. 
The current release is targeted at developers and advanced users, and care should 
be taken before production use. The following should be noted before using 
the GPU accelerated {\tt mdrun-gpu}:
\begin{itemize}
...
they often do not translate very well onto the streaming architectures.
...
acceleration is usually only about 5 times relative to a 3GHz CPU.
\end{itemize}
...
The option {\tt md} is accepted, but keep in mind that the actual algorithm is not leap-frog.
...
No-cutoff, {\ie} {\tt rcoulomb=0} and {\tt rvdw=0}, is also supported.\\
...
are not supported, only {\tt tc-grps=System} will work. Remember that for heterogeneous
...
\item \textbf{Force fields:} Supported FF are Amber, CHARMM, OPLS-AA. {\gromos} is not supported.
\item \textbf{Implicit solvent:} Supported only with {\tt reaction-field} electrostatics.
The only supported algorithm for GB is OBC, and the default {\gromacs} values for the scale factors
are hard coded in OpenMM, {\ie} {\tt obc\us{}alpha=1}, {\tt obc\us{}beta=0.8} and {\tt obc\us{}gamma=4.85}.
\item \textbf{Constraints:} Constraints in OpenMM are done by a combination of SHAKE,
SETTLE and CCMA. Accuracy is based on the SHAKE tolerance as set by the {\tt shake\us{}tol} option.
...
\item \textbf{Non-equilibrium MD:} Option {\tt acc\us{}grps} is not supported.
...
For details about compatibility of NVIDIA drivers with the CUDA library and devices consult  
...
\begin{enumerate}
    ...
    Note that the distributed {\gromacs}-GPU packages do not contain the entire set of 
    ...
    {\small{\tt export LD\us{}LIBRARARY\us{}PATH=path\us{}to\us{}gromacs/openmm/lib:\$LD\us{}LIBRARY\us{}PATH}}\\
    ...
    \item Set the {\tt OPENMM\us{}PLUGIN\us{}DIR} environment variable to contain the path to the \\ 
    {\tt openmm/lib/plugins} directory, {\eg} in bash:\\ 
    {\small{\tt export OPENMM\us{}PLUGIN\us{}DIR=path\us{}to\us{}gromacs/openmm/lib/plugins}}

    \item At this point, running the command
    {\tt path\us{}to\us{}gromacs/bin/mdrun-gpu -h} should display the standard {\tt mdrun} help  
...
Also note that it is essential that the same version of CUDA be used to compile both
...
{\small
\begin{verbatim}
    export OPENMM\us{}ROOT\us{}DIR=path\us{}to\us{}custom\us{}openmm\us{}installpath
    cmake  -DGMX\us{}OPENMM=ON [-DCMAKE\us{}INSTALL\us{}PREFIX=desired\us{}install\us{}path]
    make mdrun
    make install-mdrun
\end{verbatim}}
...
{\small {\tt mdrun-gpu -device "ACCELERATION:[DEV\us{}OPTION=VALUE,]... [OPTION].."}} \\
...
is {\tt OpenMM}. This is followed by the list of comma-separated
{\tt DEV\us{}OPTION=VALUE} option-value pairs which define 
...
suffix representing the approximate amount of time in seconds that should be spent 
on testing; the default value is {\tt memtest=15s}. 
It is possible to completely turn off memory testing by setting 
{\tt memtest=off}, however this is not advisable.

\paragraph{Force-device} Option that enables running {\tt mdrun-gpu}
...
{\bf Note} that both the option names and the values are case-insensitive.
