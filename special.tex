...
{\gromacs} 4.5 provides an initial support for GPU acceleration through 
the \href{https://simtk.org/home/openmm}{OpenMM library}. 
Although limited in functionality and can only surpass by a large margin 
the CPU algorithms of standard GROMACS, {\gromacs}-GPU gives a preview 
of GPU-accelerated MD which we expect to be an important direction in the 
future. 
...
(for details see \ref{subsec-compatibility}. Make sure that the necessary CUDA drivers and 
...
\item The maximum size of the simulated systems depends on the GPU used. With a 
...
\item MD algorithms are complex and often does not translate very well onto the streaming architectures.
