...
might not be well defined. An example is a water slab which is connected
...
by experiment. It is useful for parameterizing the force field,
...
and as a dummy atom in the force field calculation. The link atoms, if
...
This is an experimental release provides support for GPU--accelerated
Molecular Dynamics simulations using he \href{https://simtk.org/home/openmm}{OpenMM library}. 
...
\item \textbf{Force fields:} Supported FF are Amber, CHARMM, OPLSAA. GROMOS is not supported.
...
\item \textbf{Pressure control:} OpenMM implements the Monte Carlo barostat. All values for {\tt pcoupl} are
thus accepted.
...
\begin{enumerate}
    ...
    \footnotesize{\tt export LD\_LIBRARARY\_PATH=path\_to\_gromacs/openmm/lib:\$LD\_LIBRARY\_PATH}.\\
    ...
    \item Set the \texttt{\footnotesize OPENMM\_PLUGIN\_DIR} environment variable to contain the path to 
    ...
    \footnotesize{\tt path\_to\_gromacs/bin/mdrun-gpu -h} should display the standard {\tt mdrun} help  
...
\begin{itemize}
    \item \footnotesize{\tt export OPENMM\_ROOT\_DIR=path\_to\_custom\_openmm\_installpath}
...
simulation will be carried out. The default value is 0, i.e. the first device. 
...
incorrect results. Therefore, gromacs-gpu has a built-in mechanism for 
...
\footnotesize{\tt memtest=off}, however this is not advisable.
...
crashes and therefor it is not encouraged.
