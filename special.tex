...
The $x$ should run from 0 to $r_c+1$ (the value of {\tt table\_extension} can be
...
different energy groups pairs through the {\tt .mdp} option {\tt energygrp\_table}
(see \secref{mdpopt}).
...
are hard coded in OpenMM, {\ie} {\tt obc\_alpha=1}, {\tt obc\_beta=0.8} and {\tt obc\_gamma=4.85}.
...
\begin{enumerate}
    ...
    {\small{\tt export LD\_LIBRARARY\_PATH=path\_to\_gromacs/openmm/lib:\$LD\_LIBRARY\_PATH}}\\
    ...
    \item Set the {\tt OPENMM\_PLUGIN\_DIR} environment variable to contain the path to the \\ 
    {\tt openmm/lib/plugins} directory, {\eg} in bash:\\ 
    {\small{\tt export OPENMM\_PLUGIN\_DIR=path\_to\_gromacs/openmm/lib/plugins}}

    \item At this point, running the command
    {\tt path\_to\_gromacs/bin/mdrun-gpu -h} should display the standard {\tt mdrun} help  
...
{\small {\tt mdrun-gpu -device "ACCELERATION:[DEV\_OPTION=VALUE,]... [OPTION].."}} \\
...
{\tt DEV\_OPTION=VALUE} option-value pairs which define 
