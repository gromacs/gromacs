...
if (CPPCHECK_EXECUTABLE AND UNIX)
    ...
    file(GLOB_RECURSE CPPCHECK_CPP_FILES_TO_IGNORE RELATIVE ${CMAKE_SOURCE_DIR}
        ${CMAKE_SOURCE_DIR}/src/contrib/*.cpp)
    list(REMOVE_ITEM CPPCHECK_CPP_FILES ${CPPCHECK_CPP_FILES_TO_IGNORE})

    set(_deplist)
    foreach (_filename ${CPPCHECK_C_FILES})
        set(_target_name cppcheck-${_filename}.stamp)
        string(REPLACE "/" "_" _target_name ${_target_name})
        list(APPEND _deplist ${_target_name})
        add_custom_command(
            OUTPUT ${_target_name}
            ...
            COMMAND ${CMAKE_COMMAND} -E touch ${CMAKE_CURRENT_BINARY_DIR}/${_target_name}
            WORKING_DIRECTORY ${CMAKE_SOURCE_DIR}
            IMPLICIT_DEPENDS C ${CMAKE_SOURCE_DIR}/${_filename}
            COMMENT "Running cppcheck for ${_filename}"
            VERBATIM)
    endforeach()
    foreach (_filename ${CPPCHECK_CPP_FILES})
        set(_target_name cppcheck-${_filename}.stamp)
        string(REPLACE "/" "_" _target_name ${_target_name})
        list(APPEND _deplist ${_target_name})
        add_custom_command(
            ...
            COMMAND ${CMAKE_COMMAND} -E touch ${CMAKE_CURRENT_BINARY_DIR}/${_target_name}
            WORKING_DIRECTORY ${CMAKE_SOURCE_DIR}
            IMPLICIT_DEPENDS CXX ${CMAKE_SOURCE_DIR}/${_filename}
            COMMENT "Running cppcheck for ${_filename}"
            VERBATIM)
    endforeach()
    add_custom_target(cppcheck DEPENDS ${_deplist})
else()
