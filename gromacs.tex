\documentclass[11pt,dvips]{book}
\usepackage{here,picins,fancy,epsfig,array,tabularx,makeidx,a4,citesort}
%\setpapersize{USletter}
\makeindex
\input{macros}
\sloppy
%\topmargin -1cm
\hoffset -1cm
\setlength{\textwidth}{15cm}
\setlength{\textheight}{22cm}
\renewcommand{\textfraction}{0.0}
\newcommand{\qtw}{3.75cm}  % 1/4 of the textwidth
\newcommand{\ttw}{4.33cm}  % 1/3 of the textwidth
\newcommand{\htw}{7.5cm}  % 1/2 of the textwidth
\newcommand{\ntw}{13cm} % 0.9 of the textwidth
\newcommand{\gmxver}{2.0}
\newcommand{\figref}[1]{Fig.~\ref{fig:#1}}
\newcommand{\figsref}[2]{Figs.~\ref{fig:#1} and ~\ref{fig:#2}}
\newcommand{\tabref}[1]{Table~\ref{tab:#1}}
\newcommand{\eqnref}[1]{eqn.~\ref{eqn:#1}}
\newcommand{\eqnsref}[2]{eqns.~\ref{eqn:#1} and \ref{eqn:#2}}
\newcommand{\secref}[1]{sec.~\ref{sec:#1}}
\newcommand{\chref}[1]{chapter~\ref{ch:#1}}
\newcommand{\wwwpage}{\bf http://rugmd0.chem.rug.nl/$\sim$gmx}
% NOTE: \wwwpage is explicitly included in the 'verbatim' citing instructions!
\newcommand{\undercons}{\centerline{\em Note: this section is still under construction}}
\setlength{\headwidth}{\textwidth}
\setlength{\headheight}{1.0cm}
%\addtolength{\headwidth}{\marginparsep}
%\addtolength{\headwidth}{\marginparwidth}
%\addtolength{\headheight}{7pt}

\begin{document}
\sloppy
%
%	F R O N T P A G E	
%
\thispagestyle{empty}
\begin{center}
{\Large\bf\underline {Groningen Machine for Chemical Simulations}}	\\[1cm]

{\LARGE\bf GROMACS USER MANUAL}				\\[1cm]
{\LARGE\bf Version \gmxver}				\\[1cm]
\end{center}

\begin{figure}[h]
\centerline{\psfig {figure=plots/warning.eps,width=1.8cm,angle=-80}}
\end {figure}

\begin{figure}[h]
\centerline{\psfig {figure=plots/pla.eps,width=\ntw}}
\end {figure}
\centerline{\em Phospholipase A2 ready to attack a lecithin monolayer.}

\newpage
\centerline{(this page was left blank on purpose)}

%
%	P R E F A C E
%
\newpage
\footrulewidth 0.4pt
\pagestyle{fancyplain}
\renewcommand{\chaptermark}[1]{\markboth{#1}{#1}} % remember chapter title
\renewcommand{\sectionmark}[1]{\markright{\thesection\ #1}}
                                                % section number and title
\lhead[\fancyplain{}{\bf\sf\thepage}]{\fancyplain{}{\bf\sf\rightmark}}
\rhead[\fancyplain{}{\bf\sf\leftmark}]{\fancyplain{}{\bf\sf\thepage}}
\cfoot{}

\pagenumbering{roman}
\setcounter{page}{1}

\begin{center}
{\LARGE\bf GROMACS USER MANUAL}				\\[1cm]
{\LARGE\bf Version \gmxver}				\\[1cm]
{\large\bf 
David van der Spoel 	\\
Aldert R. van Buuren 	\\
Emile Apol 		\\
Pieter J. Meulenhoff 	\\
D. Peter Tieleman	\\
Alfons L.T.M. Sijbers 	\\
Berk A. Hess		\\
K. Anton Feenstra	\\
Rudi van Drunen 	\\
Herman J.C. Berendsen}
\end{center}

\begin{figure}[H]
\centerline{\psfig{figure=texutil/bioson.eps,width=8cm}}
\end{figure}

\begin{table}[H]
\centerline{
\begin{tabular}{l}
\hline
(c) Copyright. \\
{\sf BIOSON} Research Institute and Laboratory of Biophysical Chemistry \\
University of Groningen \\ 
Nijenborgh 4\\
9747 AG Groningen \\
The Netherlands	\\
Fax: 31-50-634800	\\
\hline
\end{tabular}
}
\end{table}

\newpage
\section*{Preface \& Disclaimer.}
This manual is not complete and has no pretention to be complete, due
to lack of time of the contributors. It is meant as a source of
information and references for the {\gromacs} user.  It contains the
background physics of MD simulations and is still being worked on
which in some cases means that the information is not correct. Some of
the programs that are described in the online manual pages are not in
the standard distribution, and therefore some of the file types that
are listed in the appendix are obsolete.

When \boldindex{citing} this document in any scientific publication
please refer to it as:

\begin{quote}
van der Spoel, D., van Buuren, A.~R., Apol, E., Meulenhoff, P.~J.,
Tieleman, D.~P., Sij\-bers, A.~L.~T.~M., Hess, B., Feenstra, K.~A.,
van Drunen, R. and Berendsen, H.~J.~C.  
\hspace{1em} Gromacs {U}ser {M}anual version \gmxver
\hspace{1em} Nij\-enborgh 4, 9747 AG Groningen, The
Netherlands. Internet: http://rugmd0.chem.rug.nl/~gmx 1997
\end{quote}

or, if you use bibtex, you can directly copy the following:
\begin{verbatim}
@Manual{gmx20,
  title =        "Gromacs {U}ser {M}anual version 2.0",
  author =       "David van der Spoel and Aldert R. van Buuren and Emile
                 Apol and Pieter J. Meulenhoff and D. Peter Tieleman and
                 Alfons L. T. M. Sij\-bers and Berk Hess and K. Anton
                 Feenstra and Rudi van Drunen and Herman J. C. Berendsen",
  address =      "Nij\-enborgh 4, 9747 AG Groningen, The Netherlands.
                 Internet: http://rugmd0.chem.rug.nl/~gmx",
  year =         "1997"
}
\end{verbatim}

Any comment is welcome, please send it by e-mail to
{\tt gromacs@chem.rug.nl}. 
\medskip

Groningen, \today
\medskip

{\sf BIOSON} Research Institute and Department of Biophysical Chemistry	\\
University of Groningen \\ 
Nijenborgh 4				\\
9747 AG Groningen			\\
The Netherlands				\\
Fax: 31-50-634800

\newpage
\subsection*{Multimedia Manual}
If you  have access to a WWW browser such as NCSA \normindex{mosaic}
or \normindex{Netscape}
please look up
our HTML page: \\
{\wwwpage}.

\subsection*{Violated Copyrights}
The following commercial thingies may be mentioned here and there in the
text (plus some that we forgot here).
\begin{table}[h]
\begin{tabularx}{\linewidth}{lX}
\hline
GROMOS    & is a trademark of Biomos B.V.\\
SPARC     & is a trademark of Sun Microsystems inc. and Texas Instruments inc.\\
CM5       & is a trademark of Thinking Machines inc.\\
Quanta    & is a trademark of Molecular Simulations inc.\\
Cerius    & is a trademark of Molecular Simulations inc.\\
HyperChem & is a trademark of AutoDesk inc.\\
\hline
\end{tabularx}
\end{table}

The figure on frontpage was made with Molscript~\cite{Kraulis91}.

%
%	C O N T E N T S
%
\tableofcontents
\listoffigures
\listoftables

%
%	R E A L   M A N U A L
%
\newpage
\setcounter{page}{1}
\pagenumbering{arabic}

\include{intro}
\include{defunits}
\include{algorithms}
\include{forcefield}
\input{pppm}
\include{topology}
%\include{starting}
\include{special}
\include{analyse}

%
%	A P P E N D I C E S
%
\appendix
\include{install}
\include{implement}
\include{tables}
\include{lr-corr}
\include{averages}

%
% 	B I B L I O G R A P H Y
%
\bibliographystyle{texutil/proteins}
\bibliography{texutil/monster,texutil/unpubl}

%
%	I N D E X
%
\printindex

\end{document}
