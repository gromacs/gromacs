...
In this chapter we describe some details of the \myindex{parallel MD}  
algorithm used 
...
Modern day parallel computers, such as an \myindex{IBM SP/2} or a
\myindex{Cray T3E} consist of relatively
...
The {\gromacs} utility program \myindex{editconf} has an option to rotate a whole
computational box.
...
\item	Doing the Fourier transform of the charge distribution $\rho(\ve{r})$ 
	...
	programs (note that there are also \myindex{shared memory} versions of the
	FFTW code).\\
...
most \myindex{shared memory}
computers, such as the \myindex{SGI Origin} 
