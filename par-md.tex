...
algorithm used 
in {\gromacs}. This also includes some other information on neighborsearching and
...
{\gromacs} employs two different grids: the neighborsearching grid ({\nsgrid})
...
Modern day parallel computers, such as an \normindex{IBM SP/2} or a
\normindex{Cray T3E} consist of relatively
...
not directly visible for the programmer, a software layer (like \normindex{MPI} or 
...
The algorithm for neighborsearching then becomes:
\begin{enumerate}
...
\item	On each processor compute the neighborlist for all charge groups in
	its slab using the normal grid neighborsearching.
\end{enumerate}
...
The {\gromacs} utility program \normindex{editconf} has an option to rotate a whole
computational box.
...
\item	Fixed interaction lists (bonds, angles etc.) are kept each
	...
	(\normindex{grompp}) and care is taken that, as far as possible,
...
\item	Doing the Fourier transform of the charge distribution $\rho(\ve{r})$ 
	...
	programs (note that there are also \normindex{shared memory} versions of the
	FFTW code).\\
...
coordinates (or rather the index to coordinates) every time a neighborlist is made.
