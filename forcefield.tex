...
\begin {figure}
\centerline{\psfig{figure=plots/dummies.eps,width=13cm}}
\caption{The five different types of dummy construction in \protect\gromacs.}
\label{fig:dummies}
\end {figure}

There are five ways to construct dummies from surrounding atoms in
{\gromacs}. Note that all dummies types mentioned can be constructed
from types 2$''$ (normalized, in-plane) and 3 (non-normalized, out of
...
\begin{array}{c}
	\begin{array}{lclllll}
	\Fi &=& \Fdum &-& 
		\dfrac{b \cos \alpha}{|\rvij|} \ve{F}_1 &+&
		\dfrac{b \sin \alpha}{|\ve{r}_\perp|} \left( 
	...
	\Fj &=& &&
		...
		\ve{F}_3 \right)				\\[3ex]
	\Fk &=& && &&
		\dfrac{b \sin \alpha}{|\ve{r}_\perp|} \ve{F}_2	\\[3ex]
	\end{array}						\\[5ex]
	\mbox{where ~}
	\ve{F}_1 = \Fdum -
