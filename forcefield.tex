...
0 up to and including 180 degrees
\\{\bf dihedrals}:
...
\item[{\bf\sf COG.}]The center of geometry of $N$ atoms, this is a linear
combinations with equal weights:
\beq
  w_i = \frac{1}{N}
\eeq
\item[{\bf\sf COM.}]The center of mass of $N$ atoms, this is a linear
combination with weights:
\beq
  w_i = m_i \left(\sum_{i=1}^N m_i \right)^{-1}
\eeq
where $m_i$ is the mass of constructing atom $i$.
When in free-energy simulations the mass of particles is changed,
only the mass of the A-state is used for the weights.
