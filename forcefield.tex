...
dihedral potential~\cite{Jorgensen86a}. 
The OPLS potential function is given as the first 
four terms of a Fourier series:
\beq
V_{rb} (\phi_{ijkl}) ~=~ V_0 + \frac{1}{2} \left[V_1(1+\cos(\psi)) + V_2(
1-\cos(2\psi)) + V_3(1+\cos(3\psi))\right],
\eeq
...
\noindent{\bf Note:} With the latest incarnations of
OPLS (reviewed in ref.~\cite{Jorgensen2005a}) there are also torsion potentials that
contain a term with cos$^4(\phi)$.
} % Brace matches ifthenelse test for gmxlite
