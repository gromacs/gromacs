...
\begin{figure}
\centerline{\includegraphics[width=8cm]{plots/f_lj}}
...
\eeq
\begin{figure}
\centerline{\includegraphics[width=8cm]{plots/f_bham}}
...
\begin{figure}
\centerline{\raisebox{4cm}{\includegraphics[angle=270,width=5cm]{plots/bstretch}}\includegraphics[width=7cm]{plots/f_bond}}
...
\begin{figure}
\centerline{\raisebox{4cm}{\includegraphics[angle=270,width=5cm]{plots/angle}}\includegraphics[width=7cm]{plots/f_angle}}
...
\begin{figure}
\centerline{\includegraphics[width=8cm]{plots/f_imps}}
...
\begin{figure}
\centerline{\raisebox{4.5cm}{\includegraphics[angle=270,width=5cm]{plots/dih}}\includegraphics[width=7cm]{plots/f_dih}}
...
\begin{figure}
\centerline{\includegraphics[width=8cm]{plots/f_rbs}}
...
the table is read from the file {\tt table\us{}b0.xvg}.
...
\begin{figure}
\centerline{\includegraphics[width=8cm]{plots/f_pr}}
...
the {\tt [~distance\us{}restraints~]} section and is described below.
...
\item[{\tt disre\us{}weighting}: force-weighting in restraints with
         multiple pairs.]
        ...
        {\tt disre\us{}weighting = equal}. 
        If you instead set this option to {\tt disre\us{}weighting = conservative}
        you get conservative forces when {\tt disre\us{}tau = 0}.
\item[{\tt disre\us{}mixed}: how to calculate the violations.]
        {\tt disre\us{}mixed = no} gives normal time-averaged violations.
        When {\tt disre\us{}mixed = yes} the square root of the
        product of the time-averaged and the instantaneous
        violations is used.
\item[{\tt disre\us{}fc}: force constant $k_{dr}$ for distance restraints.] 
        $k_{dr}$  (\eqnref{disre}) can be set
        as variable {\tt disre\us{}fc = 1000} for a force constant of
...
\item[{\tt disre\us{}tau}: time constant for restraints.] 
        $\tau$ (\eqnref{ravdisre}) can be set
        as variable {\tt disre\us{}tau = 10} for a time constant of
        10 ps. Time averaging can be turned off by setting {\tt disre\us{}tau}
        to 0.
...
the topology in the section {\tt [~orientation\us{}restraints~]}.
...
\item[{\tt orire}: use orientation restraining.]
	...
	supply {\tt topol0.tpr}, {\tt topol1.tpr}, ... with different
	coordinates and/or velocities.
\item[{\tt orire\us{}fc}: force constant $k$ for orientation restraints.] 
...
\item[{\tt orire\us{}tau}: time constant $\tau$ for restraints.] 
        Set {\tt orire\us{}tau = 10} for a time constant of
        10 ps. Time averaging can be turned off by setting {\tt orire\us{}tau}
        to 0.
\item[{\tt orire\us{}fitgrp}: the fit group for the restraints.]
...
\item[{\tt nstorireout}: orientation output frequency.]
	...
	These can be analyzed using {\tt g\us{}energy}.
\end{description}
%} % Brace matches ifthenelse test for gmxlite
...
\eeq
Note that there is a sign error in Equation~1 of Noskov {\em et al.}~\cite{Noskov2005a}:
\beq
...
respectively, $\alpha$ is the soft-core parameter (set with {\tt sc\us{}alpha} 
in the {\tt .mdp} file), $p$ is the soft-core $\LAM$ power (set with 
{\tt sc\us{}power}), $\sigma$ is the radius of the interaction, which is 
$(C_{12}/C_6)^{1/6}$ or an input parameter ({\tt sc\us{}sigma}) when $C_6$ 
or $C_{12}$ is zero.
...
set with {\tt sc\us{}sigma} in the {\tt .mdp} file. These hydrogens
produce peaks in $\partial H/\partial\LAM$ at $\LAM$ is 0 and/or 1 for $p=1$
and close to 0 and/or 1 with $p=2$. Lowering {\tt sc\us{}sigma} will decrease
...
\item[{\bf\sf 4fdn.}]From four atoms, with a fixed distance, see separate Fig.\ \ref{fig:vsite_4fdn}.
...
\end{eqnarray}
        ...
        parameters. Note that the vectors $\mathbf{r}_{ik}$ and $\mathbf{r}_{ij}$ are not normalized
...
(from $-5$ to $5$) in each direction.  The {\tt ewald\us{}rtol} parameter
...
spacing for the FFT grid and {\tt pme\us{}order} controls the
...
PPM is currently disabled in GROMACS, but there are plans to
re-introduce it.
...
%testlr}. Try it if you think the {\tt mk\us{}ghat} procedure is a waste
%of time.
%} % Brace matches ifthenelse test for gmxlite
...
dihedrals), 45a3, 53a5, and 53a6 force fields are included.  All standard
...
development of the \gromosv{87} force field.
The \gromosv{96} force field has improvements
over the \gromosv{87} force field for proteins and small molecules.
...
When selecting the CHARMM force field in {\tt \normindex{pdb2gmx}} the default option is to use CMAP (dihedral cross terms for protein backbone), use {\tt -nocmap} flag otherwise.
