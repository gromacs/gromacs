...
\end{array}
\eeq
or, alternatively the Lorentz-Bertelot rules can be used. An arithmetic average is used for the sigma's, while a geometric average is used for the epsilon's (type 2):
\beq
\begin{array}{rcl}
...
\end{array}
\eeq
finally an geometric average for both the sigma's and epsilon's can be used (type 3):
\beq
\begin{array}{rcl}
...
\figref{imps}. Since the potential is harmonic it is discontinuous,
...
\eeq
(note that there is a sign error in Eqn. 1 of Noskov {\em et al.}~\cite{Noskov2005a}), where
\beq
...
respectively, $\alpha$ is the soft-core parameter, $p$ is the soft-core
$\LAM$ power, $\sigma$ is the radius of the interaction,
which is $(C_{12}/C_6)^{1/6}$ or a predefined value when
...
-V^A(r_A) + V^B(r_B) + \nonumber \\
 & &
\alpha \frac{p}{6} \left( - \LL \LAM^{p-1} F^A(r_A) \sigma_A^6 r_A^{-5} 
                          + \LAM \LL^{p-1} F^B(r_B) \sigma_B^6 r_B^{-5} \right)
\eea
...
$\alpha$$\approx$0.7 for $p$=1 and $\alpha$$\approx$1.5 for $p$=2.
...
and close to 0 and/or 1 with $p$=2. Lowering {\tt sc-sigma} will decrease
...
parameters, for a total of six nonbonded interaction parameters. See
...
comparison to experimental data and published in a peer-reviewd
...
all-atom parameters. That said, we describe the availabe options in
some detail.
...
normal {\gromacs} forcefield, since the extra hydrogens have no
Lennard-Jones interaction and zero charge. The only differences are in
the bond angle and improper dihedral angle terms. This forcefield is
