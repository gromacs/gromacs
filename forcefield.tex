...
When selecting the CHARMM force field in {\tt \normindex{pdb2gmx}} the default option is to use CMAP (for torsional correction map), use {\tt -nocmap} flag otherwise. The CMAP term is, in it's vanilla form implemented here, a function of the phi and psi backbone torsion angles and is defined in the rtp file by a [ cmap ] statement at the end of each residue supporting CMAP. The following five atoms define the two torsional angles, phi = atoms 1-4, and psi = atoms 2-5. The corresponding atom types are then matched to the correct CMAP type in the cmap.itp file that contains the correction maps.
