...
\beq
\ve{F}_i = f F(r_{ij}) q_i q_j \frac{\rvij}{r_{ij}}
\eeq
...
\end{array}
~ \mbox{ ~ where ~ } ~
 \theta_{ijk} = \arccos \frac{(\rvij \cdot \ve{r}_{kj})}{r_{ij}r_{kj}}
\eeq
...
list is usually generated by the \myindex{pdb2gmx} program.
...
\ve{F}_i~=~&~0 \hspace{4cm}  & r_{ij} < r_0         \\
 \: = ~&-~k_{dr}(r_{ij}-r_0)\frac{\rvij}{r_{ij}} & r_0 \le r_{ij} < r_1\\
 \: = ~&-~k_{dr}(r_1-r_0)\frac{\rvij}{r_{ij}}    & r_{ij} \ge r_1	
\end{array}
\eeq
...
\ve{F}_i~=~&~0 \hspace{4cm}  & r_{N} < r_0         \\
  = ~&-~ k_{dr}(r_{N}-r_0)\frac{\rvij}{r_{ij}} & r_0 \le r_{N} < r_1 \\
  = ~&-~ k_{dr}(r_1-r_0)\frac{\rvij}{r_{ij}}    & r_{N} \ge r_1	
\end{array}
\eeq
...
\beq
	\ve{r} ~=~ \ve{r}_i + a \rvij + b \ve{r}_{ik}
\eeq
...
\beq
	\ve{r} ~=~ \ve{r}_i + b \frac{  \rvij + a \ve{r}_{jk}  }
				     {| \rvij + a \ve{r}_{jk} |}	
\eeq
...
\beq
	...
	\ve{r}_\perp ~=~ \ve{r}_{jk} - 
			\frac{ \rvij \cdot \ve{r}_{jk} }
			     { \rvij \cdot \rvij }
			 \rvij
\eeq
	in this case the dummy is in the plane of the other three
	particles at a distance of $d$ from $i$ at an angle of
	$\theta$ with $\rvij$. Note that in this case $d$ and
...
\displaystyle	\ve{F}_j ~=~ \left(
				\frac{d \cos \theta}{|\rvij|} -
			\left( 1 + \frac{ \rvij \cdot \ve{r}_{jk} }
				        { \rvij \cdot \rvij }
			\right)
			\frac{d \sin \theta}{|\ve{r}_\perp|} 
			     \right) \ve{F}	\\
