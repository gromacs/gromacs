...
the table is read from the file {\tt table_b0.xvg}.
...
the topology in the section {\tt [~orientation_restraints~]}.
...
and close to 0 and/or 1 with $p=2$. Lowering {\tt sc_sigma} will decrease
...
(from $-5$ to $5$) in each direction.  The {\tt ewald_rtol} parameter
...
spacing for the FFT grid and {\tt pme_order} controls the
