...
\chapter{Force fields \index{force-field}}
\label{ch:ff}
...
\begin{figure}
\centerline{\raisebox{-1.8cm}{\includegraphics[angle=270,width=5cm]{plots/bstretch}}\includegraphics[angle=270,width=7cm]{plots/f_bond}}
...
\begin{figure}
\centerline{\raisebox{-1.5cm}{\includegraphics[angle=270,width=5cm]{plots/angle}}\includegraphics[angle=270,width=7cm]{plots/f_angle}}
...
\begin{figure}
\centerline{\raisebox{-1cm}{\includegraphics[angle=270,width=5cm]{plots/dih}}\includegraphics[angle=270,width=7cm]{plots/f_dih}}
...
\begin{figure}
\centerline{\includegraphics[angle=270,width=8cm]{plots/f_rbs}}
...
(\eqnref{Xi}) is non-trivial when dummy atoms are used. Since the
virial involves a summation over all the atoms (rather than virtual
particles) the forces most be redistributed from the dummies to the
atoms (using ~\eqnref{fdum}) {\em before} computation of the
virial. In some special cases where the forces on the atoms can be
written as a linear combination of the forces on the dummies (types 2
...
\figref{dummies} gives an overview of the available dummy constructions.
...
\eeq
For a shifted function we need to start the integration at 0.
This introduces a (relatively small) error,
because at short range $g(r)$ is not 1.
...
\bea
\Xi_{lr}&=&	\half N \rho \int_{r_c}^{\infty} 4\pi r^2 3\,C_6 \rij^{-6}\,  \dr	\nonumber\\
	&=&	6 \pi N \rho\,C_6  \int_{r_c}^{\infty} \rij^{-4}\dr \nonumber\\
	&=&	2 \pi N C_6 \rho r_c^{-3}
\eea
