...
\vspace{-2ex}\begin{description}
\item[{\bf md} ]\mbox{}\\
A \normindex{leap-frog} algorithm for integrating Newtons
equations.
\item[{\bf steep}]\mbox{}\\
...
\subsection{\normindex{Electrostatics} and \normindex{VdW}}
\begin{description}
\item[{\bf \normindex{coulomb}type:}]\mbox{}\\
\vspace{-2ex}\begin{description}
\item[\normindex{Cut-off}]\mbox{}\\
...
\normindex{charge group}s and diffusion between neighborlist
updates.
\item[{\bf User}]\mbox{}\\
{\tt mdrun} will now expect to find two files with user-defined
functions: {\tt rtab.xvg} for Repulsion, {\tt dtab.xvg} for 
\normindex{Dispersion}. These files should contain 5 columns:
the {\tt x} value, and the function value with its 1$^{st}$
to 3$^{rd}$ derivative. The {\tt x} should run from 0 [nm] to
...
\subsection{\normindex{Free Energy Perturbatrion}}
\begin{description}
